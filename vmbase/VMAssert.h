...
#undef NDEBUG
#elif !defined(NDEBUG)
// Define this AND LEAVE IT DEFINED to suppress asserts. It needs to stay
// defined because other headers, notably system headers, may reinclude
// assert.h. According to the standard and actual practice, assert.h can be
// included multiple times, and what happens each time depends on whether NDEBUG
...
#define static_assert(condition, message)                                      \
  typedef ::vmbase::static_assert_helper<sizeof(                               \
      ::vmbase::STATIC_ASSERTION_FAILED<(bool)(condition)>)>                   \
