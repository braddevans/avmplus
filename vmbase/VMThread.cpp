...
void WaitNotifyMonitor::wait() { m_condVar.wait(*this); }
...
VMThread::VMThread()
    : m_threadID(VMPI_nullThread()) // CID:12562 Uninitialized scalar field
      ,
      m_state(NOT_STARTED), m_joinerQty(0) {
  m_runnable = this;
...
VMThread::VMThread(Runnable *runnable)
    : m_runnable(runnable), m_state(NOT_STARTED), m_joinerQty(0) {
  int suffix = m_nextNameSuffix.incAndGet();
  char buf[64];
  VMPI_snprintf(buf, sizeof(buf), "Thread-%d", suffix);
  setNameFrom(buf);
...
bool VMThread::start(ThreadPriority priority, size_t stackSize,
                     size_t guardSize) {
  vmpi_thread_attr_t attr;
  VMPI_threadAttrInit(&attr);
  if (priority == HIGH_PRIORITY) {
...
bool VMThread::start(vmpi_thread_attr_t *attr) {
  SCOPE_LOCK_SP(m_joinMonitor) {
    ...
    if (m_state == NOT_STARTED) {
      if (VMPI_threadCreate(&m_threadID, attr, VMThread::startInternal,
                            (vmpi_thread_arg_t)(this))) {
        m_state = RUNNABLE;
        return true;
      }
    }
...
VMThread::startInternal(vmpi_thread_arg_t args) {
...
const char *VMThread::getName() const { return m_name; }
