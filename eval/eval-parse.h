...
//
// The code generator resolves those constants.  Values that either must be
// constants (identifiers used in 'namespace', 'const', and 'use namespace'
// definitions) or can be constants (identifiers used as the namespace value in
// qualified identifiers) are looked up during code generation by searching the
// context. The 'Program' and 'Function' (and probably 'Package', when we have
// that) ribs have separate lists of constant bindings with their values.  Those
// value lists are populated in a prepass when code generation enters a Program
// or Function, since a statement can reference a later constant binding at the
// same or outer scope.
...
public:
  Qualifier()
      : tag(QUAL_none), is_dynamic(0), is_final(0), is_native(0),
        is_override(0), is_static(0), name(NULL), metadata(NULL) {}
  QualifierTag tag;
  uint32_t is_dynamic;
  uint32_t is_final;
  uint32_t is_native;
...
public:
  Binding(NameComponent *ns, Str *name, Type *type_name, BindingKind kind)
      : ns(ns), name(name), type_name(type_name), kind(kind) {}
...
enum CodeType { CODE_Program, CODE_Class, CODE_Function };
...
public:
  BodyInfo(Allocator *allocator)
      : bindings(allocator), functionDefinitions(allocator),
        namespaces(allocator), openNamespaces(allocator), stmts(allocator),
        uses_finally(false), uses_catch(false), uses_goto(false),
        uses_arguments(false), uses_dxns(false), uses_super(false),
        empty_body(false) {}
...
public:
  SignatureInfo(Allocator *allocator)
      : ns(NULL), name(NULL), params(allocator), rest_param(NULL), numparams(0),
        optional_arguments(false), isVoid(false), isGetter(false),
        isSetter(false), return_type_name(NULL) {}
...
public:
  CodeBlock(CodeType tag, BodyInfo &body)
      : tag(tag), bindings(body.bindings.get()),
        functions(body.functionDefinitions.get()),
        namespaces(body.namespaces.get()),
        openNamespaces(body.openNamespaces.get()), stmts(body.stmts.get()),
        uses_finally(body.uses_finally), uses_catch(body.uses_catch),
        uses_goto(body.uses_goto), uses_arguments(body.uses_arguments),
        uses_dxns(body.uses_dxns), uses_super(body.uses_super),
...
public:
  Program(BodyInfo &body, Seq<TypeDefn *> *classes)
      : CodeBlock(CODE_Program, body), classes(classes) {}

  virtual void cogenBody(Cogen *cogen, Ctx *ctx, uint32_t activation);

  void cogenTypes(Cogen *cogen, Ctx *ctx, ABCTraitsTable *global_traits,
                  Seq<TypeDefn *> *classes);
  void cogenTypeHierarchy(Cogen *cogen, Ctx *ctx, ABCTraitsTable *global_traits,
                          TypeDefn *cls);
  void cogenType(Cogen *cogen, Ctx *ctx, ABCTraitsTable *global_traits,
                 ClassDefn *cls);
  void cogenType(Cogen *cogen, Ctx *ctx, ABCTraitsTable *global_traits,
                 InterfaceDefn *cls);
  ABCTraitsTable *cogenClassTraits(Cogen *cogen, Ctx *ctx,
                                   Seq<FunctionDefn *> *inst_methods,
                                   Seq<Binding *> *inst_vars);
  ...
  uint32_t *interfaceRefs(Cogen *cogen, Seq<Str *> *ifaces,
...
public:
  TypeDefn(uint32_t pos, NameComponent *ns, Str *name, bool is_interface)
      : pos(pos), ns(ns), name(name), is_interface(is_interface),
        is_generated(false), is_searching(false) {}
  ...
  bool is_generated;
  bool is_searching; // A mark used during class hierarchy traversal indicating
                     // that the class has already been visited
};

class ClassDefn : public TypeDefn {
public:
  ClassDefn(uint32_t pos, NameComponent *ns, Str *name, bool is_final,
            bool is_dynamic, Str *extends, Seq<Str *> *implements,
            Seq<FunctionDefn *> *cls_methods, Seq<Binding *> *cls_vars,
            Seq<NamespaceDefn *> *cls_namespaces, FunctionDefn *cls_init,
...
public:
  InterfaceDefn(uint32_t pos, NameComponent *ns, Str *name, Seq<Str *> *extends,
                Seq<FunctionDefn *> *inst_methods)
      : TypeDefn(pos, ns, name, true), extends(extends),
...
public:
  FunctionParam(Str *name, Type *type_name, Expr *default_value)
      : name(name), type_name(type_name), default_value(default_value) {}
...
// Somewhat mis-named, also used for function expressions (see LiteralFunction
// below), where name may be NULL

class FunctionDefn : public CodeBlock {
public:
  FunctionDefn(SignatureInfo &signature, BodyInfo &body,
               Seq<Stmt *> *inits = NULL)
      : CodeBlock(CODE_Function, body), ns(signature.ns), name(signature.name),
        params(signature.params.get()), numparams(signature.numparams),
        ...
        isGetter(signature.isGetter), isSetter(signature.isSetter),
        inits(inits) {}

  virtual void cogenBody(Cogen *cogen, Ctx *ctx, uint32_t activation);

  // Common code for function code generation.  Create MethodInfo and
  // MethodBodyInfo; generate code for the function; set the function's flags
  // properly.
  void cogenGuts(Compiler *compiler, Ctx *ctx, ABCMethodInfo **info,
                 ABCMethodBodyInfo **body);

  void cogenGuts(Compiler *compiler, Ctx *ctx, bool isMethod,
                 ABCMethodInfo **info, ABCMethodBodyInfo **body);

  NameComponent *ns; // Namespace of function or NULL.
  Str *name; // Name of function or NULL.  Not const because we sometimes need
             // to set it to NULL.
  Seq<FunctionParam *> *const params; // List of fixed parameters
  const uint32_t numparams;           // Number of fixed parameters
  FunctionParam *rest_param; // non-NULL iff we have a ... rest argument;
                             // name+type of that argument
  Type *return_type_name;    // may be NULL
  const bool
      optional_arguments; // true iff any of the parameters have default values
  const bool isGetter;    // true iff this is a getter function
  const bool isSetter;    // true iff this is a setter function
  Seq<Stmt *>
      *inits; // initializing statements executed in the outer scope on entry
};

class NamespaceDefn {
public:
  NamespaceDefn(NameComponent *ns, Str *name, Expr *value)
      : ns(ns), name(name), value(value) {}
  NameComponent *const ns;
  Str *const name;
  Expr *const value; // NULL for anonymous ones; TAG_literalString for string
                     // values; otherwise a NameExpr
};
...
public:
  ...
  virtual void cogen(Cogen *cogen,
                     Ctx *ctx) = 0; // Generates a reference to the type
...
public:
  SimpleType(QualifiedName *name, uint32_t pos = 0) : Type(pos), name(name) {}
...
public:
  InstantiatedType(QualifiedName *basename, Type *tparam, uint32_t pos = 0)
      : Type(pos), basename(basename), tparam(tparam) {}
...
public:
  ...
  virtual void cogen(Cogen *cogen,
                     Ctx *ctx) = 0; // override for value computation
...
public:
  LiteralObject(Seq<LiteralField *> *fields, uint32_t pos)
      : Expr(pos), fields(fields) {}
  virtual void cogen(Cogen *cogen, Ctx *ctx);
...
public:
  LiteralArray(Seq<Expr *> *elements, uint32_t pos)
      : Expr(pos), elements(elements) {}
  virtual void cogen(Cogen *cogen, Ctx *ctx);
...
public:
  LiteralVector(Type *type, Seq<Expr *> *elements, uint32_t pos)
      : Expr(pos), type(type), elements(elements) {}
  virtual void cogen(Cogen *cogen, Ctx *ctx);
  Type *const type;
  Seq<Expr *> *const elements;
...
public:
  XmlInitializer(Seq<Expr *> *exprs, bool is_list, uint32_t pos)
      : Expr(pos), exprs(exprs), is_list(is_list) {}
  virtual void cogen(Cogen *cogen, Ctx *ctx);
...
public:
  CallExpr(Expr *fn, Seq<Expr *> *arguments, uint32_t pos)
      : Expr(pos), fn(fn), arguments(arguments) {}
  virtual void cogen(Cogen *cogen, Ctx *ctx);
  Expr *const fn;
  Seq<Expr *> *const arguments;
};
...
public:
  FilterExpr(Expr *obj, Expr *filter, uint32_t pos)
      : Expr(pos), obj(obj), filter(filter) {}
  virtual void cogen(Cogen *cogen, Ctx *ctx);
  Expr *const obj;
  Expr *const filter;
};
...
public:
  DescendantsExpr(Expr *obj, QualifiedName *name, uint32_t pos)
      : Expr(pos), obj(obj), name(name) {}
  virtual void cogen(Cogen *cogen, Ctx *ctx);
  Expr *const obj;
  QualifiedName *const name;
};

enum Escapement { ESC_none, ESC_elementValue, ESC_attributeValue };
...
public:
  QualifiedName(NameComponent *qualifier, NameComponent *name, bool is_attr,
                uint32_t pos)
      : NameExpr(pos), qualifier(qualifier), name(name), is_attr(is_attr) {
    AvmAssert(name != NULL);
  }
  virtual void cogen(Cogen *cogen, Ctx *ctx);
...
public:
  ObjectRef(Expr *obj, QualifiedName *name, uint32_t pos)
      : NameExpr(pos), obj(obj), name(name) {
...
public:
  LabeledStmt(Str *label, Stmt *stmt)
      : label(label), stmt(stmt), address(NULL) {}
  virtual void cogen(Cogen *cogen, Ctx *ctx);
...
public:
  IfStmt(uint32_t pos, Expr *expr, Stmt *consequent, Stmt *alternate)
      : Stmt(pos), expr(expr), consequent(consequent), alternate(alternate) {}
  virtual void cogen(Cogen *cogen, Ctx *ctx);
  Expr *const expr;
...
public:
  WhileStmt(uint32_t pos, Expr *expr, Stmt *body)
      : LabelSetStmt(pos), expr(expr), body(body) {}
  virtual void cogen(Cogen *cogen, Ctx *ctx);
  Expr *const expr;
  Stmt *const body;
};
...
public:
  // pos should be the position of the "while" keyword, roughly, because it's
  // the expression that will go wrong if anything
  DoWhileStmt(uint32_t pos, Expr *expr, Stmt *body)
      : LabelSetStmt(pos), expr(expr), body(body) {}
  virtual void cogen(Cogen *cogen, Ctx *ctx);
  Expr *const expr;
  Stmt *const body;
...
public:
  ForStmt(uint32_t pos, Expr *init, Expr *test, Expr *update, Stmt *body)
      : LabelSetStmt(pos), init(init), test(test), update(update), body(body) {}
  virtual void cogen(Cogen *cogen, Ctx *ctx);
  Expr *const init;
  Expr *const test;
  Expr *const update;
...
public:
  ForInStmt(uint32_t pos, Expr *lhs, Expr *init, Expr *obj, Stmt *body,
            bool is_each)
      : LabelSetStmt(pos), is_each(is_each), lhs(lhs), init(init), obj(obj),
        body(body) {}
  virtual void cogen(Cogen *cogen, Ctx *ctx);
  const bool is_each;
  Expr *const lhs;
...
public:
  SwitchStmt(uint32_t pos, Expr *expr, Seq<CaseClause *> *cases)
      : Stmt(pos), expr(expr), cases(cases) {}
  virtual void cogen(Cogen *cogen, Ctx *ctx);
...
public:
  TryStmt(Seq<Stmt *> *tryblock, Seq<CatchClause *> *catches,
          Seq<Stmt *> *finallyblock)
      : tryblock(tryblock), catches(catches), finallyblock(finallyblock) {}
  virtual void cogen(Cogen *cogen, Ctx *ctx);
  void cogenWithFinally(Cogen *cogen, Ctx *ctx);
  void cogenNoFinally(Cogen *cogen, Ctx *ctx);
  void cgCatch(Cogen *cogen, Ctx *ctx, uint32_t code_start, uint32_t code_end,
               Label *Lend, CatchClause *catchClause);
...
class CatchClause {
public:
  CatchClause(Str *name, Type *type_name, Seq<Stmt *> *stmts)
      : name(name), type_name(type_name), stmts(stmts) {}
  Str *const name;
  Type *const type_name;
  Seq<Stmt *> *const stmts;
};
...
public:
  WithStmt(uint32_t pos, Expr *expr, Stmt *body)
      : Stmt(pos), expr(expr), body(body) {}
  virtual void cogen(Cogen *cogen, Ctx *ctx);
  Expr *const expr;
  Stmt *const body;
};

// This is for base class constructor calls; base class method calls are
// represented as ExprStmts containing ObjectRef nodes with a SuperExpr
// left-hand-side and a CallExpr right-hand-side.
class SuperStmt : public Stmt {
public:
  SuperStmt(uint32_t pos, Seq<Expr *> *arguments)
      : Stmt(pos), arguments(arguments) {}
  virtual void cogen(Cogen *cogen, Ctx *ctx);
  Seq<Expr *> *const arguments;
};
...
class Parser {
public:
  Parser(Compiler *compiler, Lexer *lexer, bool public_by_default,
...
private:
  ...
  enum {
    EFLAG_NoIn =
        1, // Expression parsing: do not allow the use of the "in" operator
  };
  ...
  enum {
    ...
    SFLAG_Class = 8,
    SFLAG_Interface = 16,
    SFLAG_Block = 32, // Statement and directive parsing: at block level inside
                      // one of the above
  };
  ...
  void functionDefinition(bool config, Qualifier *qualifier,
  ...
  FunctionDefn *functionGuts(Qualifier *qual, bool require_name,
                             bool getters_and_setters, bool require_body);
  Expr *varBindings(uint32_t *pos, NameComponent *ns, bool is_const,
                    bool is_static, int flags = 0, uint32_t *numbindings = NULL,
                    Expr **firstName = NULL);
  ...
  // Expression parser
  //
  // OPTIMIZEME: Make the parser work by operator precedence so that we have
  // only a small number of recursive levels from "commaExpression" to
  // "nameExpression" or "primaryExpression" (the common paths).
  ...
  void xmlEscape(XmlContext *ctx, const wchar *cs, const wchar *limit,
                 bool is_attr);
  ...
  Seq<FunctionDefn *> *filterConstructor(Str *name, Seq<FunctionDefn *> *fns,
                                         FunctionDefn **constructor);
  Seq<Stmt *> *filterStatements(Seq<Stmt *> *stmts,
                                Seq<Stmt *> **nonDefinitionStmts);
  FunctionDefn *constructClassConstructor(uint32_t pos, Str *name,
                                          BodyInfo *old_body,
                                          Seq<Stmt *> *init_stmts,
                                          Seq<Stmt *> *static_stmts);
  FunctionDefn *constructInstanceConstructor(uint32_t pos, Str *name,
                                             Seq<Stmt *> *init_stmts,
                                             FunctionDefn *constructor);
  ...
  enum RibType { RIB_Program, RIB_Class, RIB_Instance, RIB_Function };
  ...
  Binding *findBinding(NameComponent *ns, Str *name, BindingKind kind,
                       BindingRib *rib = NULL);
  void pushBindingRib(RibType tag);
  void popBindingRib();
  void addBinding(Str *name, BindingKind kind);
  void addVarBinding(NameComponent *ns, Str *name, Type *type_name,
                     BindingRib *rib = NULL);
  void addConstBinding(NameComponent *ns, Str *name, Type *type_name,
                       BindingRib *rib = NULL);
  ...
  void addUnqualifiedImport(
      Seq<Str *> *name); // import a.b.*; name would have length 2
  void addQualifiedImport(
      Seq<Str *> *name); // import a.b.c; name would have length 3
  void setUsesFinally();
  void setUsesCatch();
  ...
  public:
    ...
    Expr *value; // LiteralUndefined, LiteralNull, LiteralBoolean,
                 // LiteralString, LiteralInt, LiteralUInt, LiteralDouble
  };
  ...
  bool
  isConfigReference(Expr *e); // e is a QualifiedName with non-NULL SimpleName
                              // parts, and the ns names a known config ns
  ...
  // Utilities used by constant evaluator, pretty general but not used
  // elsewhere.
