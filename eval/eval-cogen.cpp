...
VarScopeCtx *Ctx::findVarScope() {
  Ctx *ctx = this;
  while (ctx->tag != CTX_Function && ctx->tag != CTX_ClassMethod &&
         ctx->tag != CTX_Program)
    ctx = ctx->next;
...
Cogen::Cogen(Compiler *compiler, ABCFile *abc, ABCTraitsTable *traits,
             ABCMethodBodyInfo *body, uint32_t first_temp)
    : compiler(compiler), abc(abc), allocator(compiler->allocator),
      code(compiler->allocator), labels(compiler->allocator), traits(traits),
      body(body), last_linenum(0), label_counter(0), temp_counter(first_temp),
      sets_dxns(false), need_activation(false), stack_depth(0),
      max_stack_depth(0), scope_depth(0) {}
...
void Cogen::stackMovement(AbcOpcode opcode, bool hasRTNS, bool hasRTName,
                          uint32_t pops) {
  stack_depth =
      stack_depth + opcodeInfo[opcode].stack - hasRTNS - hasRTName - pops;
  AvmAssert((int32_t)stack_depth >= 0);
  if (stack_depth > max_stack_depth)
    max_stack_depth = stack_depth;
}
...
void Cogen::emitJump(AbcOpcode opcode, Label *label) {
  // OPTIMIZEME: don't need to register backpatches for branches to known
  // labels.
  code.emitU8((uint8_t)opcode);
  code.emitS24(3);
  stackMovement(opcode);
  label->backpatches =
      ALLOC(Seq<uint32_t>, (code.size() - 3, label->backpatches));
}
...
void Cogen::fixupBackpatches(uint8_t *b) const {
  for (Seq<Label *> *labels = this->labels.get(); labels != NULL;
       labels = labels->tl) {
    uint32_t addr = labels->hd->address;
    bool backward = false;
    AvmAssert(addr != ~0U);
    for (Seq<uint32_t> *backpatches = labels->hd->backpatches;
         backpatches != NULL; backpatches = backpatches->tl) {
...
uint32_t Cogen::emitException(uint32_t from, uint32_t to, uint32_t target,
                              uint32_t type, uint32_t name_index) {
  return body->exceptions.addAtEnd(
      ALLOC(ABCExceptionInfo, (from, to, target, type, name_index)));
}
...
uint32_t Cogen::emitTypeName(Compiler *compiler, Type *t) {
  ...
  if (t->tag() == TAG_simpleType) {
    ...
    if (n->qualifier != NULL) {
      ...
      uint32_t ns =
          abc->addNamespace(CONSTANT_Namespace,
                            abc->addString(((SimpleName *)n->qualifier)->name));
      return abc->addQName(ns, abc->addString(((SimpleName *)n->name)->name));
    } else {
      AvmAssert(n->name->tag() == TAG_simpleName);
      return abc->addMultiname(compiler->NSS_public,
                               abc->addString(((SimpleName *)n->name)->name));
    }
  } else if (t->tag() == TAG_instantiatedType) {
    checkVectorType(compiler, ((InstantiatedType *)t)->basename);
    return abc->addTypeName(
        compiler->ID_Vector,
        emitTypeName(compiler, ((InstantiatedType *)t)->tparam));
  } else {
...
void Cogen::checkVectorType(Compiler *compiler, QualifiedName *t) {
  ...
  if (t->qualifier != NULL || t->name->tag() != TAG_simpleName ||
...
void Cogen::I_lookupswitch(Label *default_label, Label **case_labels,
                           uint32_t ncases) {
  AvmAssert(ncases > 0);
  ...
  default_label->backpatches =
      ALLOC(Seq<uint32_t>, (code.size() - 3, default_label->backpatches));
...
void FunctionDefn::cogenGuts(Compiler *compiler, Ctx *ctx, bool isMethod,
  ...
  for (Seq<FunctionParam *> *params = this->params; params != NULL;
       params = params->tl) {
    ...
    if (params->hd->default_value != NULL) {
      ...
      case TAG_literalNull:
        ...
        break;
      default:
        // EXTENDME: we can sort-of support arbitrary default values here if we
        // want to.
        //
        // AS3 does not support default value other than the six cases above.
        // Doing better would be nice.
        //
        // We can use one of the obscure namespace default values as a
        // placeholder, then generate code to test for that value and compute
        // the correct default value. But the signature of the function won't be
        // right; the type of the argument must be '*'.  May be close enough, as
        // long as we assign a provided argument value to a typed slot and get a
        // type check on entry.
        compiler->syntaxError(params->hd->default_value->pos,
  ...
  *info = ALLOC(ABCMethodInfo,
                (compiler, abc->addString(name), numparams, param_types.get(),
                 numdefaults, default_values.get(),
                 Cogen::emitTypeName(compiler, return_type_name)));
  traits = ALLOC(ABCTraitsTable, (compiler));
  *body = ALLOC(ABCMethodBodyInfo,
                (compiler, *info, traits,
                 1 + numparams + (uses_arguments || (rest_param != NULL)),
                 empty_body));
...
void CodeBlock::cogen(Cogen *cogen, Ctx *ctx) {
  ...
  if (fn && fn->inits != NULL) {
    // Erect a non-program context for the inits to execute within, to avoid
    // actions being taken as for CODE_Program.
  ...
  for (Seq<Binding *> *bindings = this->bindings; bindings != NULL;
       bindings = bindings->tl) {
    Binding *b = bindings->hd;
    uint32_t id =
        abc->addQName(cogen->emitNamespace(b->ns), cogen->emitString(b->name));
    uint32_t type_id = cogen->emitTypeName(compiler, b->type_name);
    switch (bindings->hd->kind) {
    case TAG_namespaceBinding: // FIXME: namespace bindings should be const, but
                               // the VM does not allow TAG_constBinding
    case TAG_varBinding:
  ...
  for (Seq<NamespaceDefn *> *namespaces = this->namespaces; namespaces != NULL;
       namespaces = namespaces->tl) {
    uint32_t id = abc->addQName(cogen->emitNamespace(namespaces->hd->ns),
                                cogen->emitString(namespaces->hd->name));
    uint32_t ns = 0;
    Expr *value = namespaces->hd->value;
    if (value == NULL)
      ns = abc->addNamespace(
          CONSTANT_Namespace,
          cogen->emitString(compiler->intern(compiler->namespace_counter++)));
    else if (value->tag() == TAG_literalString)
      ns =
          abc->addNamespace(CONSTANT_ExplicitNamespace,
                            cogen->emitString(((LiteralString *)value)->value));
    if (tag == CODE_Program)
      cogen->I_getlocal(0);
    else {
    ...
    if (ns != 0)
      ...
      compiler->internalError(
          0, "Namespace should have been resolved before code generation");
    }
  ...
  if (fn) {
    ...
    if (fn->uses_arguments || fn->rest_param) {
      ...
      if (fn->uses_arguments)
        cogen->I_setproperty(abc->addQName(
            cogen->emitNamespace(compiler->parser.defaultNamespace()),
  ...
  for (Seq<FunctionDefn *> *functions = this->functions; functions != NULL;
    ...
    uint32_t fname = abc->addQName(cogen->emitNamespace(func->ns),
                                   cogen->emitString(func->name));
    cogen->I_getlocal(activation);
...
uint32_t Cogen::buildNssetWithPublic(Seq<Namespace *> *ns) {
  ...
  while (ns != NULL) {
    if (ns->hd->tag() != TAG_commonNamespace)
      compiler->internalError(
          0, "Namespace should have been resolved before now.");
    CommonNamespace *cns = (CommonNamespace *)ns->hd;
    s.addAtEnd(
        abc->addNamespace(CONSTANT_Namespace, abc->addString(cns->name)));
    ns = ns->tl;
  }
...
uint32_t Cogen::emitNamespace(NameComponent *ns) {
  ...
  case T_Private:
    ...
    break;
  case T_Protected:
    compiler->internalError(0,
                            "No code generation for protected namespaces yet");
    break;
  case T_Public:
...
void Program::cogenTypes(Cogen *cogen, Ctx *ctx, ABCTraitsTable *global_traits,
...
void Program::cogenTypeHierarchy(Cogen *cogen, Ctx *ctx,
                                 ABCTraitsTable *global_traits, TypeDefn *ty) {
  if (!ty->is_generated) {
    if (ty->is_searching)
    ...
    if (ty->is_interface) {
      ...
      while (bases != NULL) {
        cogenTypeHierarchy(cogen, ctx, global_traits,
                           findInterface(cogen, iface->pos, bases->hd));
        bases = bases->tl;
      }
      ...
      if (base != NULL && base != cogen->compiler->SYM_Object)
        cogenTypeHierarchy(cogen, ctx, global_traits,
                           findClass(cogen, cls->pos, base));
      Seq<Str *> *bases = cls->implements;
      while (bases != NULL) {
        cogenTypeHierarchy(cogen, ctx, global_traits,
                           findInterface(cogen, cls->pos, bases->hd));
        bases = bases->tl;
      }
...
void Program::cogenType(Cogen *cogen, Ctx *ctx, ABCTraitsTable *global_traits,
                        ClassDefn *cls) {
  Allocator *allocator = cogen->allocator;
  Compiler *compiler = cogen->compiler;
  ABCFile *abc = cogen->abc;

  uint32_t class_name = abc->addQName(cogen->emitNamespace(cls->ns),
                                      cogen->emitString(cls->name));
  uint32_t base_name = abc->addMultiname(
      compiler->NSS_public, // FIXME: should be set of open namespaces
  ...
  uint32_t protectedNS =
      0; // FIXME, if not 0 then set CONSTANT_ClassProtectedNS
  uint32_t interface_count;
  uint32_t *interfaces =
      interfaceRefs(cogen, cls->implements, &interface_count);
  uint32_t cinit = cogenConstructor(cogen, ctx, cls->cls_init);
  uint32_t iinit = cogenConstructor(cogen, ctx, cls->inst_init);

  ABCTraitsTable *traits =
      cogenClassTraits(cogen, ctx, cls->inst_methods, cls->inst_vars);
  ABCTraitsTable *ctraits = cogenClassTraits(
      cogen, ctx, cls->cls_methods, cls->cls_vars); // FIXME: namespaces?

  ABCClassInfo *clsinfo = ALLOC(ABCClassInfo, (cinit, ctraits));
  ABCInstanceInfo *instinfo =
      ALLOC(ABCInstanceInfo, (class_name, base_name, flags, protectedNS,
                              interface_count, interfaces, iinit, traits));
...
void Program::cogenType(Cogen *cogen, Ctx *ctx, ABCTraitsTable *global_traits,
  ...
  uint32_t iface_name = abc->addQName(cogen->emitNamespace(iface->ns),
                                      cogen->emitString(iface->name));
  ...
  FunctionDefn *iface_cinit =
      ALLOC(FunctionDefn, (iface_signature, iface_body, NULL));
  uint32_t cinit = cogenConstructor(
      cogen, ctx, iface_cinit); // must have a function that just returns
  iface_body.empty_body = true;
  FunctionDefn *iface_iinit =
      ALLOC(FunctionDefn, (iface_signature, iface_body, NULL));
  uint32_t iinit =
      cogenConstructor(cogen, ctx, iface_iinit); // must have an empty body

  ABCTraitsTable *traits =
      cogenClassTraits(cogen, ctx, iface->inst_methods, NULL);
  ABCTraitsTable *ctraits = cogenClassTraits(cogen, ctx, NULL, NULL);

  ABCClassInfo *clsinfo = ALLOC(ABCClassInfo, (cinit, ctraits));
  ABCInstanceInfo *instinfo =
      ALLOC(ABCInstanceInfo, (iface_name, base_name, flags, protectedNS,
                              interface_count, interfaces, iinit, traits));
  uint32_t class_traits = abc->addClassAndInstance(clsinfo, instinfo);
...
uint32_t *Program::interfaceRefs(Cogen *cogen, Seq<Str *> *ifaces,
                                 uint32_t *interface_count) {
  Allocator *allocator = cogen->allocator;
  Compiler *compiler = cogen->compiler;
  ABCFile *abc = cogen->abc;
  ...
  while (ifaces != NULL) {
    refs[i] =
        abc->addMultiname(compiler->NSS_public, cogen->emitString(ifaces->hd));
    i++;
    ifaces = ifaces->tl;
  }
  *interface_count = n;
...
ABCTraitsTable *Program::cogenClassTraits(Cogen *cogen, Ctx *ctx,
  ...
  for (Seq<FunctionDefn *> *ms = inst_methods; ms != NULL; ms = ms->tl) {
    ...
    traits->addTrait(
        ALLOC(ABCMethodTrait, (abc->addQName(cogen->emitNamespace(ms->hd->ns),
                                             cogen->emitString(ms->hd->name)),
                               fn_info->index)));
  }
...
uint32_t Program::cogenConstructor(Cogen *cogen, Ctx *ctx,
...
int Program::pushBaseClasses(Cogen *cogen, ClassDefn *currcls) {
  ...
  uint32_t basenm =
      abc->addMultiname(compiler->NSS_public, cogen->emitString(base));
...
// FIXME: These need to know about imported classes and interfaces!  They need
// not always return a full ClassDefn / InterfaceDefn, just a rump; mostly we
// want the names of the base classes and interfaces.  The "imported" types are
// the types already present in the environment (in the run-time compiler) or
// the actually imported types from an ABC, in the batch compiler.  Looks like
// there will be some hook in the HostContext.

ClassDefn *Program::findClass(Cogen *cogen, uint32_t pos, Str *name) {
  for (Seq<TypeDefn *> *classes = this->classes; classes != NULL;
       classes = classes->tl) {
