...
inline void FunctionDefn::cogenGuts(Compiler *compiler, Ctx *ctx,
                                    ABCMethodInfo **info,
                                    ABCMethodBodyInfo **body) {
  cogenGuts(compiler, ctx, false, info, body);
}

inline bool Parser::newline() { return LP < L0; }

inline uint32_t Parser::position() { return L0 + line_offset; }

inline Token Parser::hd() { return T0; }
...
inline bool Parser::isRelational(Token t, bool in_allowed) {
  return t < T_OPERATOR_SENTINEL && tokenMapping[t].isRelational &&
         (in_allowed || t != T_In);
}
...
inline void Parser::setUsesArguments() { topRib->body.uses_arguments = true; }

inline void Parser::setUsesDefaultXmlNamespace() {
  topRib->body.uses_dxns = true;
}

inline void Parser::xmlAtom() { T0 = lexer->xmlAtom(&L0, &V0); }

inline void Parser::xmlPushback(wchar c) { lexer->xmlPushback(c); }
} // namespace RTC
} // namespace avmplus
