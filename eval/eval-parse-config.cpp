...
void Parser::addConfigBinding(Str *ns, Str *name, Expr *value) {
  configBindings =
      ALLOC(Seq<ConfigBinding *>,
            (ALLOC(ConfigBinding, (ns, name, value)), configBindings));
}
...
Expr *Parser::evaluateConfigDefinition(Str *ns, Expr *e) {
  ...
  // ns is the default namespace: it will be used to qualify any unqualified
  // name.
  ...
  case TAG_qualifiedName: {
    ...
    if (qname->qualifier->tag() != TAG_simpleName ||
        qname->name->tag() != TAG_simpleName)
      compiler->syntaxError(
          e->pos,
          SYNTAXERR_UNBOUND_CONST_NAME); // Specifically an illegal const name
    Str *ns = ((SimpleName *)qname->qualifier)->name;
    Str *name = ((SimpleName *)qname->name)->name;
    Expr *value = findConfigBinding(ns, name);
    if (value == NULL)
  ...
  case TAG_binaryExpr: {
    ...
    case OPR_logicalAnd:
      return boxBoolean(
          int(evaluateToBoolean(lhs)) + int(evaluateToBoolean(rhs)) == 2);
    case OPR_logicalOr:
      return boxBoolean(
          int(evaluateToBoolean(lhs)) + int(evaluateToBoolean(rhs)) != 0);
    case OPR_less: {
    ...
    case OPR_strictNotEqual: {
      ...
      if (lhs->tag() != rhs->tag()) {
        if (binary->op == OPR_equal || binary->op == OPR_notEqual) {
          if ((lhs->tag() == TAG_literalUndefined &&
               rhs->tag() == TAG_literalNull) ||
              (lhs->tag() == TAG_literalNull &&
               rhs->tag() == TAG_literalUndefined))
            return boxBoolean(true == equality);
          if ((lhs->tag() == TAG_literalString &&
               rhs->tag() == TAG_literalDouble) ||
              (lhs->tag() == TAG_literalDouble &&
               rhs->tag() == TAG_literalString))
            return boxBoolean(
                (evaluateToNumber(lhs) == evaluateToNumber(rhs)) == equality);
          if (lhs->tag() == TAG_literalBoolean ||
              rhs->tag() == TAG_literalBoolean)
            return boxBoolean(
                (evaluateToBoolean(lhs) == evaluateToBoolean(rhs)) == equality);
        }
      ...
      if (lhs->tag() == TAG_literalDouble)
        return boxBoolean((evaluateToNumber(lhs) == evaluateToNumber(rhs)) ==
                          equality);
      if (lhs->tag() == TAG_literalBoolean)
        return boxBoolean((evaluateToBoolean(lhs) == evaluateToBoolean(rhs)) ==
                          equality);
      if (lhs->tag() == TAG_literalString)
        return boxBoolean((evaluateToString(lhs) == evaluateToString(rhs)) ==
                          equality);
      failNonConstant(lhs);
      /*NOTREACHED*/
      break;
...
Expr *Parser::boxString(const char *s) {
  return ALLOC(LiteralString, (compiler->intern(s), 0));
}
...
double Parser::evaluateToNumber(Expr *e) {
  ...
  case TAG_literalNull:
    return 0.0;
  case TAG_literalBoolean:
    return ((LiteralBoolean *)e)->value ? 1.0 : 0.0;
  case TAG_literalDouble:
    return ((LiteralDouble *)e)->value;
#ifdef VMCFG_FLOAT
  case TAG_literalFloat:
    return ((LiteralFloat *)e)->value; // FIXME?
#endif
  case TAG_literalInt:
    return (double)(((LiteralInt *)e)->value);
  case TAG_literalUInt:
    return (double)(((LiteralUInt *)e)->value);
  case TAG_literalString:
    return strToDouble(((LiteralString *)e)->value);
  default:
    failNonConstant(e);
    return 0;
  }
...
bool Parser::evaluateToBoolean(Expr *e) {
  ...
  case TAG_literalNull:
    return false;
  case TAG_literalBoolean:
    return ((LiteralBoolean *)e)->value;
  case TAG_literalDouble: {
    double v = ((LiteralDouble *)e)->value;
    return !MathUtils::isNaN(v) && v != 0.0;
  }
#ifdef VMCFG_FLOAT
  case TAG_literalFloat: {
    float v = ((LiteralFloat *)e)->value;
    return !MathUtils::isNaNf(v) && v != 0.0f;
  }
#endif
  case TAG_literalInt:
    return ((LiteralInt *)e)->value != 0;
  case TAG_literalUInt:
    return ((LiteralUInt *)e)->value != 0;
  case TAG_literalString:
    return ((LiteralString *)e)->value->length > 0;
  default:
    failNonConstant(e);
    return 0;
  }
...
Str *Parser::evaluateToString(Expr *e) {
  switch (e->tag()) {
  case TAG_literalUndefined:
    return compiler->intern("undefined");
  case TAG_literalNull:
    return compiler->intern("null");
  case TAG_literalBoolean:
    return ((LiteralBoolean *)e)->value ? compiler->intern("true")
                                        : compiler->intern("false");
  case TAG_literalDouble:
    return doubleToStr(((LiteralDouble *)e)->value);
#ifdef VMCFG_FLOAT
  case TAG_literalFloat:
    return doubleToStr(((LiteralFloat *)e)->value);
#endif
  case TAG_literalInt:
    return doubleToStr(((LiteralInt *)e)->value);
  case TAG_literalUInt:
    return doubleToStr(((LiteralUInt *)e)->value);
  case TAG_literalString:
...
// Returns -1 for undefined, 0 for false, 1 for true.  Generally true means "x <
// y", false means "!(x < y)" and undefined means x and y are not comparable.

int Parser::evaluateRelational(Expr *lhs, Expr *rhs) {
  if (lhs->tag() == TAG_literalString && rhs->tag() == TAG_literalString)
    return (((LiteralString *)lhs)->value)
                       ->compareTo(((LiteralString *)rhs)->value) < 0
               ? 1
               : 0;
...
void Parser::failNonConstant(Expr *e) {
  compiler->internalError(position(),
                          "Non-constant value in expression evaluation, tag=%d",
                          int(e->tag()));
}
} // namespace RTC
} // namespace avmplus
