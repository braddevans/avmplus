...
public:
  ...
  uint32_t reported_size; // number of bytes reported by size(), checked by
                          // serialize()
};
...
private:
  ...
  SeqBuilder<ABCNamespaceInfo *>
      namespaces; // few enough of these that a sequentially searchable list is
                  // sufficient
  SeqBuilder<ABCNamespaceSetInfo *>
      namespaceSets; // few enough of these that a sequentially searchable list
                     // is sufficient
  SeqBuilder<ABCMultinameInfo *>
      multinames; // FIXME - *NOT* few enough of these that a sequential list is
                  // sufficient, but OK for now
...
public:
  ...
  ABCScriptInfo(Compiler *compiler, ABCMethodInfo *init_method,
...
public:
  ABCMethodInfo(Compiler *compiler, uint32_t name, uint32_t param_count,
                Seq<uint32_t> *params, uint32_t option_count,
                Seq<DefaultValue *> *default_values, uint32_t return_type);
...
public:
  ABCExceptionInfo(uint32_t from, uint32_t to, uint32_t target,
...
public:
  ABCMethodBodyInfo(Compiler *compiler, ABCMethodInfo *method_info,
                    ABCTraitsTable *traits, uint32_t first_temp, bool is_empty);
...
public:
  ABCSlotTrait(uint32_t name, uint32_t type_name, TraitKind kind)
      : ABCTrait(name, kind), type_name(type_name) {
...
public:
  ABCMethodTrait(uint32_t name, uint32_t method_info)
      : ABCTrait(name, TRAIT_Method), method_info(method_info) {}
...
public:
  ABCClassTrait(uint32_t name, uint32_t slot_id, uint32_t index)
      : ABCTrait(name, TRAIT_Class), slot_id(slot_id), index(index) {}
...
public:
  ABCInstanceInfo(uint32_t name, uint32_t super_name, uint8_t flags,
                  uint32_t protectedNS, uint32_t interface_count,
                  uint32_t *interfaces, uint32_t iinit, ABCTraitsTable *traits)
      : name(name), super_name(super_name), flags(flags),
        protectedNS(protectedNS), interface_count(interface_count),
        interfaces(interfaces), iinit(iinit), traits(traits) {}
...
public:
  ABCMultinameInfo(uint8_t kind, uint32_t ns_or_nsset, uint32_t name)
      : kind(kind), ns_or_nsset(ns_or_nsset), name(name) {}
  const uint8_t kind;
  const uint32_t
      ns_or_nsset;     // For CONSTANT_TypeName this is the parameterizedType
