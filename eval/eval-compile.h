...
/**
 * The compiler acts as a coordinating class, managing memory, interning
 * strings, knowing about settings, etc.  Pretty much everyone points to a
 * compiler instance, which in turn provides the host context and allocator
 * instances.
 */
class Compiler {
public:
  Compiler(HostContext *context, const wchar *filename, const wchar *src,
           uint32_t srclen, bool public_by_default);
  ~Compiler() { destroy(); }
  ...
  const wchar *const filename; // Some human-readable name of the originator of
                               // the script, NUL-terminated
  const uint32_t tableSize;    // Number of elements in strTable; the abc layer
                               // uses this for the multiname table size too

  // Flags, currently somewhat ad-hoc, that are used by the compiler to control
  // the language accepted.

  const bool
      es3_keywords; // True iff we should only recognize ECMAScript 3.0 keywords
  const bool standard_regex; // True iff we should only recognize standard regex
                             // syntax, not \<newline> nor the "x" flag
  const bool liberal_idents; // True iff we allow the use of \u to create
                             // identifers that look like keywords, or
                             // non-E262-sanctioned characters in identifiers
  const bool local_functions; // True iff we allow block-local function
                              // definitions (hoist name, init when reached)
  const bool octal_literals;  // True iff we should recognize 0[0-7]+ as octal
                              // (not in AS3)
  const bool origin_is_file;  // True iff input came from file, so "include"
                              // should be allowed
...
private:
  Str **const strTable; // fixed-length hash table using chaining on collision,
                        // chained through 'next' (in private heap)

  void internalWarningOrError(bool error, uint32_t lineno, const char *fmt,
                              va_list args);
