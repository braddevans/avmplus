...
template <class T> inline Seq<T> *SeqBuilder<T>::get() const { return items; }

template <class T> inline void SeqBuilder<T>::clear() { items = last = NULL; }
...
inline void ByteBuffer::emitS8(int8_t v) { emitU8((uint8_t)v); }
...
inline uint32_t StringBuilder::length() { return len; }
