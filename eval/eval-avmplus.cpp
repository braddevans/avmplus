...
public:
  AvmplusHostContext(AvmCore *core, Toplevel *toplevel)
      : HostContext(core), toplevel(toplevel), nextstring(0) {}
...
const wchar *AvmplusHostContext::readFileForEval(const wchar *basename,
                                                 const wchar *filename,
                                                 uint32_t *inputlen) {
  // FIXME: the mismatch between what eval needs and what the core API delivers
  // is just stunning.
  if (nextstring == sizeof(strings) / sizeof(strings[0]))
    throwInternalError("includes too deeply nested");
  StUTF16String str(core->readFileForEval(
      core->newStringUTF16(basename),
      core->newStringUTF16(
          filename))); // return value is already NUL-terminated
...
void AvmplusHostContext::throwSyntaxError(const char *msgz) {
  ...
  GCRef<SyntaxErrorObject> e = toplevel->syntaxErrorClass()->constructObject(
      core->newStringUTF8(msgz)->atom(), core->intToAtom(0));
  core->throwAtom(e->atom());
}

void AvmplusHostContext::throwInternalError(const char *msgz) {
  // FIXME - proper error ID?
  GCRef<ErrorObject> e = toplevel->errorClass()->constructObject(
      core->newStringUTF8(msgz)->atom(), core->intToAtom(0));
  core->throwAtom(e->atom());
}

ScriptBuffer compileProgram(AvmCore *core, Toplevel *toplevel, String *code,
                            String *filename) {
  ...
  RTC::Compiler compiler(&context, filename == NULL ? NULL : fn.c_str(),
                         src.c_str(), src.length(), true);
