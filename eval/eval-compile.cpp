...
namespace avmplus {
namespace RTC {
// Syntax error strings.  When these include formats they are to be interpreted
// as printf style formats for now, notably %s means utf8 or latin1 string. Note
// that a string may be used more than one place, so if you add or remove or
// change a format be sure to look for all occurrences of the string.  Note also
// that order matters crucially, see the enum in eval.h.
...
static const char *const syntax_errors[] = {
    ...
    "Unsupported default value (ABC restriction)",
    /* SYNTAXERR_ILLEGAL_TYPENAME */ "Illegal type name",
    /* SYNTAXERR_ILLEGAL_FIELDNAME */
    "Illegal field name: String, number, or identifier is required",
    ...
    /* SYNTAXERR_ILLEGAL_IN_INTERFACE */
    "Variable or constant definition not allowed in interface",
    /* SYNTAXERR_ONE_ARGUMENT_REQUIRED */ "Exactly one argument required",
    /* SYNTAXERR_NO_FUNCTIONS_IN_BLOCKS*/
    "Function definitions cannot be block-local",
    ...
    "Illegal embedded characters in XML literal",
    ...
    "The 'include' directive is only allowed in programs whose origin is a "
    "file",
    /* SYNTAXERR_INCLUDE_INACCESSIBLE */
    "An include file could not be opened or read",
    /* SYNTAXERR_REDEFINITION */ "Conflicting binding of name",
    /* SYNTAXERR_REDEFINITION_TYPE */
    "Conflicting binding of names: mismatching types",
    ...
    /* SYNTAXERR_ILLEGALCHAR_POSTNUMBER*/
    "Illegal character following numeric literal: %c",
    /* SYNTAXERR_EOI_IN_COMMENT */ "End of input in block comment",
    /* SYNTAXERR_ILLEGALCHAR */ "Illegal character in input: %c",
    /* SYNTAXERR_UNTERMINATED_STRING */
    ...
    /* SYNTAXERR_IDENT_IS_KWD */
    "Illegal identifier: escape sequence makes it look like a keyword",
    ...
    /* SYNTAXERR_ILLEGAL_OP_IN_CONSTEXPR */
    "Illegal operator in constant expression",
    /* SYNTAXERR_CONFIG_NAMESPACE_SHADOWING */
    "Definition shadows a configuration namespace",
    /* SYNTAXERR_ILLEGAL_METADATA */ "Bad or redundant metadata",
    /* SYNTAXERR_KWD_NAMESPACE_REQUIRED */ "'namespace' is required here",
    /* SYNTAXERR_CONFIG_NAMESPACE_NOT_ALLOWED */
    "A 'config' namespace may not be defined here",
    /* SYNTAXERR_CONFIG_NAMESPACE_MUST_BE_UNQUALIFIED */
    "'config namespace' must not have a qualifier",
    /* SYNTAXERR_DUPLICATE_CONFIG */
    "Redundant program configuration expression",
    ...
    /* SYNTAXERR_CIRCULAR_TYPE_HIERARCHY */
    "Cycle in the inheritance hierarchy",

};

// Assume that the number of unique identifiers in a program is roughly the
// square root of the raw length of the program.  Needs experimental
// verification.  May only be true for programs above a certain size; may want a
// lower limit on the table size.
//
// Using this "arbitrary" value for k we have to use mod when computing the hash
// bucket when interning.  Faster would be to round k up to a power of 2, then
// use k-1 as a mask in intern().
...
static const wchar default_filename[] = {'(', 'e', 'v', 'a', 'l', ' ', 's',
                                         't', 'r', 'i', 'n', 'g', ')', 0};

Compiler::Compiler(HostContext *context, const wchar *filename,
                   const wchar *src, uint32_t srclen, bool public_by_default)
    : context(context), allocator(new Allocator(this)),
      ...
      ,
      origin_is_file(filename != NULL), debugging(true), namespace_counter(1),
      strTable(makeStrTable(allocator, tableSize)), lexer(this, src, srclen),
      parser(this, &lexer, public_by_default), abc(this), SYM_(intern("")),
      SYM_AS3(intern("AS3")), SYM_AS3_vec(intern("__AS3__.vec")),
      SYM_Array(intern("Array")), SYM_CONFIG(intern("CONFIG")),
      SYM_Namespace(intern("Namespace")), SYM_Number(intern("Number")),
      SYM_Object(intern("Object")), SYM_RegExp(intern("RegExp")),
      SYM_Vector(intern("Vector")), SYM_XML(intern("XML")),
      SYM_XMLList(intern("XMLList")), SYM_anonymous(intern("anonymous")),
      SYM_arguments(intern("arguments")), SYM_children(intern("children")),
      SYM_config(intern("config")), SYM_each(intern("each")),
      SYM_extends(intern("extends")), SYM_get(intern("get")),
      SYM_implements(intern("implements")), SYM_int(intern("int")),
      SYM_length(intern("length")), SYM_namespace(intern("namespace"))
#ifdef DEBUG
      ,
      SYM_print(intern("print"))
#endif
      ,
      SYM_set(intern("set")), SYM_use(intern("use")), SYM_xml(intern("xml")),
      ...
      NS_AS3_vec(
          abc.addNamespace(CONSTANT_Namespace, abc.addString(SYM_AS3_vec))),
...
#endif
      ,
      NSS_public(abc.addNsset(ALLOC(
          Seq<uint32_t>, (NS_public, ALLOC(Seq<uint32_t>, (NS_internal)))))),
      MNL_public(abc.addMultinameL(NSS_public, false)),
      MNL_public_attr(abc.addMultinameL(NSS_public, true)) {
}

void Compiler::destroy() { delete allocator; }
...
void Compiler::compile() {
  ...
  ABCMethodInfo *global_info = ALLOC(
      ABCMethodInfo, (this, abc.addString("script$init"), 0, NULL, 0, NULL, 0));
  ABCMethodBodyInfo *global_body =
      ALLOC(ABCMethodBodyInfo, (this, global_info, global_traits, 1, false));
  uint32_t nsset = 0;
  nsset = global_body->cogen.buildNssetWithPublic(program->openNamespaces);
  ProgramCtx ctx(global_body->cogen.allocator, nsset, program->openNamespaces,
                 global_body->cogen.getTemp());
  program->cogenTypes(&global_body->cogen, &ctx, global_traits,
                      program->classes);
  ...
  global_body->clearTraits(); // the traits belong to the script, so don't emit
                              // them with the body too
...
void Compiler::internalWarningOrError(bool error, uint32_t lineno,
    ...
    VMPI_strcpy(lbuf, "Unknown");
  {
    ...
    VMPI_snprintf(buf, sizeof(buf), "%s:%s: Internal %s: ", fbuf, lbuf,
                  (error ? "error" : "warning"));
    buf[sizeof(buf) - 1] = 0;
  }
...
Str *Compiler::intern(const wchar *chars, uint32_t nchars) {
  ...
  Str *str = (Str *)allocator->alloc(
      sizeof(Str) +
      sizeof(wchar) *
          (nchars - 1 + 1)); // -1 for the elt in Str, +1 for the NUL
