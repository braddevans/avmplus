...
Stmt *Parser::statement(bool config) {
  ...
  case T_Function:
    if (compiler->local_functions) {
      // FIXME: should block-internal function definition be initialized on
      // block entry?
      ...
      return ALLOC(
          ExprStmt,
          (pos, ALLOC(AssignExpr,
                      (OPR_assign,
                       ALLOC(QualifiedName,
                             (NULL, ALLOC(SimpleName, (name)), false, pos)),
...
Expr *Parser::varBindings(uint32_t *pos, NameComponent *ns, bool is_const,
                          bool is_static, int flags, uint32_t *numbindings,
                          Expr **firstName) {
  ...
  for (;;) {
    ...
    */
    if (match(T_Assign)) {
      ...
      Expr *lhs =
          ALLOC(QualifiedName, (ns, ALLOC(SimpleName, (name)), false, *pos));
      if (is_const)
        addConstBinding(ns, name, type_name, is_static ? topRib->next : topRib);
      Expr *assign =
          ALLOC(AssignExpr, (is_const ? OPR_init : OPR_assign, lhs, init));
...
Stmt *Parser::importStatement() {
  ...
  Seq<Str *> *n = name.get();
  if (qualified) {
    compiler->internalWarning(position(),
                              "Unimplemented: Qualified import not supported "
                              "yet, using an unqualified import instead");
...
Stmt *Parser::forStatement() {
  ...
  if (hd() == T_Var) {
    uint32_t dummy = 0;
    init = varBindings(&dummy, defaultNamespace(), false, false, EFLAG_NoIn,
                       &numbindings, &lhs);
...
// This also parses super expressions when they appear in the statement
// position.
Stmt *Parser::superStatement() {
  ...
  if (argsPresent && hd() != T_Dot && hd() != T_DoubleDot &&
      hd() != T_LeftDotAngle && hd() != T_LeftBracket) {
    setUsesSuper();
