...
/**
 * Simple string.  Always allocated inside an Allocator heap.  Always
 * NUL-terminated for debugging convenience.
 */
...
#ifdef DEBUG
// Get up to limit-1 characters from s into buf, chop high bits, NUL-terminate,
// return buf
...
public:
  ...
  uint32_t length();
  Str *str();
  char *chardata(); // NUL-terminated array, freshly allocated on every call.
                    // Data are chopped to 7 bits.
 ...
 *
 * Used for all allocation by the run-time compiler except the allocation for
 * the final ABC bytevector.
 *
 ...
 * may be lost if a large request comes in and causes a new segment to be
 * allocated. We can fix that with a simple free list of large blocks, if it
 * turns out to be a problem in practice, or we can make the growth increment
 * relatively larger.
 */
class Allocator {
public:
  ...
  SBChunk *free_sbchunks; // shared among all StringBuilders working off this
                          // allocator
...
#define ALLOC(type, args) ::new (allocator->alloc(sizeof(type))) type args
...
template <class T> uint32_t length(Seq<T *> *elts) {
  uint32_t len = 0;
  while (elts != NULL) {
    elts = elts->tl;
    len++;
