...
const Parser::TokenMapping Parser::tokenMapping[] = {
    ...
    {0, 0, 0, 0, 0, 0, OPR_preDecr,
     OPR_postDecr}, // T_MinusMinus, note binop slot encodes postfix form
    ...
    {0, 0, 0, 0, 0, 0, OPR_preIncr,
     OPR_postIncr}, // T_PlusPlus, note binop slot encodes postfix form
    ...
    {1, 0, 0, 0, 0, 0, 0,
     OPR_rightShiftUnsigned},       // T_UnsignedRightShiftAssign,
    {0, 0, 0, 0, 0, 0, OPR_void, 0} // T_Void,
};

Parser::BindingRib::BindingRib(Allocator *allocator, BindingRib *next,
                               RibType tag)
    : tag(tag), signature(NULL), body(allocator), next(next) {}

Parser::Parser(Compiler *compiler, Lexer *lexer, bool public_by_default,
               uint32_t first_line)
    : compiler(compiler), allocator(compiler->allocator),
      public_by_default(public_by_default), line_offset(first_line - 1),
      ...
      publicNS(ALLOC(BuiltinNamespace, (T_Public))), topRib(NULL),
      classes(allocator), configNamespaces(NULL), configBindings(NULL),
      lexerStack(NULL), lexer(lexer), T0(T_LAST), T1(T_LAST), LP(0), L0(0),
      L1(0), included_input(NULL) {}
...
Program *Parser::program() {
  ...
  addNamespaceBinding(
      defaultNamespace(), compiler->SYM_AS3,
      ALLOC(LiteralString,
            (compiler->intern("http://adobe.com/AS3/2006/builtin"), 0)));
  while (hd() == T_Package)
    package();
...
// FIXME: must record the package name as a compound name prefix inside the
// package
// FIXME: must record that we're inside a package so that namespace qualifiers
// can be checked
void Parser::package() {
...
void Parser::directives(int flags) {
  ...
  SeqBuilder<Stmt *> *classStmts =
      (flags & SFLAG_Class) ? &(topRib->next->body.stmts) : NULL;

  // Directive parser machine state.

  SeqBuilder<Expr *> metadatas(allocator); // Metadata expressions, in order
  QualifiedName *configname; // Non-NULL if we have a config name, will also
                             // appear in metadatas
  Qualifier qual; // ns / native / static / prototype / dynamic / override /
                  // committed metadata
  bool metadata;  // We're still looking at metadata and have not moved to
                  // qualifiers
...
next_directive:
  ...
  case T_Identifier: {
    if (identValue() == compiler->SYM_config && hd2() == T_Namespace &&
        L0 == L1)
...
continue_directive:
  if (hd() == T_RightBrace || hd() == T_EOS) {
    ...
    addExprStatements((flags & SFLAG_Class) ? classStmts : instanceStmts,
                      metadatas.get());
    goto finish;
  }
...
configname_or_namespacename_or_statement : {
  // Here we're looking at some identifier.  It can be the start of: a config
  // name reference (always of the form NS::v), a namespace reference (anything
  // from a simple unqualified name through a string of namespace qualifiers and
  // finally a name), a labeled statement, or an expression statement.  The
  // special names private, public, protected, and internal do not appear.
  //
  // If it's followed by a colon then it's a label.  Otherwise we must parse it
  // as a full comma expression and then start disambiguating.
  //
  // In the AST, a simple unqualified name is a QualifiedName with a NULL
  // qualifier and a SimpleName for a name.
...
configname:
  ...
  if (configname != NULL) {
    ...
    while (!metadatas.isEmpty()) {
      ...
      ((flags & SFLAG_Class) ? classStmts : instanceStmts)
          ->addAtEnd(ALLOC(ExprStmt, (e->pos, e)));
      if (e == configname)
        break;
    }
  }
...
special_namespace_qualifier : {
  // The namespace qualifiers public, private, protected, and internal can be
  // used either by themselves as qualifiers or as part of a qualified name, eg,
  // 'internal::foo'.  In the latter case, we also risk looking at an expression
  // statement, eg, 'internal::foo() + fnord'.  However, the namespace
  // qualifiers are not themselves valid expressions, so we can't blithely parse
  // an expression here.  We must instead look ahead: if we're seeing '::' next
  // then we can parse an expression and then disambiguate between a qualified
  // namespace name or an expression statement, otherwise we take the keyword to
  // be a qualifier.
  //
  // Configuration names do not appear here because private, public, protected,
  // and internal never denote a configuration namespace.
...
namespace_directive : {
  ...
  if (!(flags &
        (SFLAG_Function | SFLAG_Toplevel | SFLAG_Package | SFLAG_Class)))
...
const_or_var_directive : {
  ...
  if (qual.is_dynamic || qual.is_final || qual.is_native || qual.is_override ||
      ...
      (qual.tag != QUAL_none &&
       !(flags & (SFLAG_Toplevel | SFLAG_Package | SFLAG_Class))))
    compiler->syntaxError(position(), SYNTAXERR_QUALIFIER_NOT_ALLOWED);
  // FIXME: if const, and at the top level, and the namespace is a config
  // namespace, evaluate and define a config const
  ((flags & SFLAG_Class) && qual.is_static ? classStmts : instanceStmts)
      ->addAtEnd(
          variableDefinition(evaluateConfigReference(configname), &qual));
  goto next_directive;
}
...
function_directive : {
  checkSimpleAttributes(&qual);
  if (qual.is_dynamic ||
      ((qual.is_final || qual.is_override || qual.is_static) &&
       !(flags & SFLAG_Class)) ||
      (qual.tag != QUAL_none &&
       !(flags & (SFLAG_Toplevel | SFLAG_Package | SFLAG_Class))))
    compiler->syntaxError(position(), SYNTAXERR_QUALIFIER_NOT_ALLOWED);
  functionDefinition(evaluateConfigReference(configname), &qual,
                     (flags & SFLAG_Class) != 0,
                     (flags & SFLAG_Interface) == 0);
  goto next_directive;
}
...
interface_directive : {
  ...
  if (qual.is_dynamic || qual.is_final || qual.is_native || qual.is_override ||
      qual.is_static || (qual.tag != QUAL_none && qual.tag != QUAL_public))
    compiler->syntaxError(position(), SYNTAXERR_QUALIFIER_NOT_ALLOWED);
...
void Parser::checkSimpleAttributes(Qualifier *qual) {
  ...
  if ((qual->is_dynamic > 1 || qual->is_final > 1 || qual->is_native > 1 ||
       qual->is_override > 1 || qual->is_static > 1) ||
...
// FIXME: This is incorrect because QualifiedName currently does not admit
// a::b::c
...
void Parser::classDefinition(bool config, int /*flags*/, Qualifier *qual) {
  ...
  // must distinguish between initializing stores and other statements.  We
  // hack: just filter the statements after the fact, variable definition stmts
  // are distinguishable from other statements.
  //
  ...
  //   }
  //
  // where "c = v" is part of the instance initializer.  Now put it in a
  // (static) loop. It should not be executed by that loop, but it is.
  ...
  Seq<Stmt *> *cls_init_stmts =
      filterStatements(cls->stmts.get(), &static_stmts);
  FunctionDefn *cls_init =
      constructClassConstructor(pos, name, cls, cls_init_stmts, static_stmts);

  // For the instance constructor only the initializing stores appear in the
  // statements list; this list becomes init1, whereas the constructor
  // (user-provided or hand-written) becomes init2.

  BodyInfo *instance = &topRib->body;
  FunctionDefn *constructor = NULL;
  Seq<FunctionDefn *> *inst_methods = filterConstructor(
      name, instance->functionDefinitions.get(), &constructor);
  ...
  FunctionDefn *inst_init =
      constructInstanceConstructor(pos, name, inst_init_stmts, constructor);

  ClassDefn *clsobj =
      ALLOC(ClassDefn, (pos, qualToNamespace(qual), name, qual->is_final > 0,
                        qual->is_dynamic > 0, extends, implements.get(),
...
ClassDefn::ClassDefn(uint32_t pos, NameComponent *ns, Str *name, bool is_final,
                     bool is_dynamic, Str *extends, Seq<Str *> *implements,
                     Seq<FunctionDefn *> *cls_methods, Seq<Binding *> *cls_vars,
                     Seq<NamespaceDefn *> *cls_namespaces,
                     FunctionDefn *cls_init, Seq<FunctionDefn *> *inst_methods,
                     Seq<Binding *> *inst_vars, FunctionDefn *inst_init)
    : TypeDefn(pos, ns, name, false), is_final(is_final),
      is_dynamic(is_dynamic), extends(extends), implements(implements),
      cls_methods(cls_methods), cls_vars(cls_vars),
      cls_namespaces(cls_namespaces), cls_init(cls_init),
      inst_methods(inst_methods), inst_vars(inst_vars), inst_init(inst_init) {}

FunctionDefn *Parser::constructClassConstructor(uint32_t pos, Str *name,
                                                BodyInfo *old_body,
                                                Seq<Stmt *> *init_stmts,
                                                Seq<Stmt *> *stmts) {
...
// For the instance init we want a function derived from the real constructor
// method, it is called CLASSNAME$iinit.  If there's no constructor method one
// is generated. Initializer statements are inserted at the beginning of the
// constructor body; these statements are the statements in the "inst" body.
// Finally, if there's no super call in the constructor method then one gets
// inserted at the beginning.
//
// The spec says:
//  - initializers are evaluated *in the instance scope*, not in the scope of
//  the
//    constructor.  Thus they cannot simply be added to the start of the
//    constructor body, somehow the code generator needs to be in on the joke.
//  - super is called at the beginning of the constructor body, after var init,
//    if not present.

FunctionDefn *Parser::constructInstanceConstructor(uint32_t pos, Str *name,
                                                   Seq<Stmt *> *init_stmts,
                                                   FunctionDefn *constructor) {
  if (constructor) {
    if (!constructor->uses_super)
      constructor->stmts = ALLOC(
          Seq<Stmt *>, (ALLOC(SuperStmt, (pos, NULL)), constructor->stmts));
...
Seq<Stmt *> *Parser::filterStatements(Seq<Stmt *> *stmts,
                                      Seq<Stmt *> **nonDefinitionStmts) {
...
Seq<FunctionDefn *> *Parser::filterConstructor(Str *name,
                                               Seq<FunctionDefn *> *fns,
                                               FunctionDefn **constructor) {
...
Seq<Binding *> *Parser::bindingsToVars(Seq<Binding *> *bindings) {
  ...
  while (bindings != NULL) {
    if (bindings->hd->kind == TAG_varBinding ||
        bindings->hd->kind == TAG_constBinding)
...
void Parser::interfaceDefinition(bool config, int /*flags*/, Qualifier *qual) {
  ...
  InterfaceDefn *iface = ALLOC(InterfaceDefn, (pos, qualToNamespace(qual), name,
                                               extends.get(), inst_methods));
...
//  - initialized on entry to the function (so defining them in blocks is pretty
//  silly)
...
void Parser::includeDirective() {
  ...
  // The current lexer state - including the state variables that are a part of
  // the parser object - gets pushed onto a stack, a new lexer is created for
  // the new input, and is installed.  Then we return, the caller must continue
  // parsing as if nothing had happened.  When the current lexer sees EOS it
  // calls onEOS() on the parser, which pops the lexer stack and generates a
  // token from the popped lexer.  The normal token processing path does not
  // slow down at all and the machinery is almost entirely transparent to the
  // parser.
  ...
  uint32_t inputlen;
  const wchar *input = compiler->context->readFileForEval(
      compiler->filename, newFile->s, &inputlen);
  if (input == NULL)
...
void Parser::popBindingRib() { topRib = topRib->next; }
...
static bool sameName(NameComponent *n1, NameComponent *n2) {
  ...
  switch (n1->tag()) {
  case TAG_simpleName:
    return ((SimpleName *)n1)->name ==
           ((SimpleName *)n2)
               ->name; // Str is interned, so pointer comparison works
  case TAG_wildcardName:
    return true;
  case TAG_computedName:
    return false;
  case TAG_builtinNamespace:
    return ((BuiltinNamespace *)n1)->t ==
           ((BuiltinNamespace *)n2)->t; // tokens are integer values
  default:
    return false;
  }
}
...
static bool sameType(Type *t1, Type *t2) {
  ...
  case TAG_instantiatedType: {
    ...
    return sameName(it1->basename, it2->basename) &&
           sameType(it1->tparam, it2->tparam);
  }
  default:
...
Binding *Parser::findBinding(NameComponent *ns, Str *name, BindingKind kind,
                             BindingRib *rib) {
  if (rib == NULL)
    rib = topRib;
  for (Seq<Binding *> *bindings = rib->body.bindings.get(); bindings != NULL;
       bindings = bindings->tl) {
...
void Parser::addClass(ClassDefn *cls) {
  ...
  if (!b)
    topRib->body.bindings.addAtEnd(
        ALLOC(Binding, (cls->ns, cls->name, NULL, TAG_classBinding)));
  else
...
void Parser::addInterface(InterfaceDefn *iface) {
  ...
  if (!b)
    topRib->body.bindings.addAtEnd(
        ALLOC(Binding, (iface->ns, iface->name, NULL, TAG_interfaceBinding)));
  else
    compiler->syntaxError(0, SYNTAXERR_REDEFINITION_TYPE);
...
void Parser::addVarBinding(NameComponent *ns, Str *name, Type *type_name,
                           BindingRib *rib) {
  if (rib == NULL)
    rib = topRib;
  Binding *b = findBinding(ns, name, TAG_varBinding, rib);
  if (!b)
    rib->body.bindings.addAtEnd(
        ALLOC(Binding, (ns, name, type_name, TAG_varBinding)));
  else if (!sameType(b->type_name, type_name))
    compiler->syntaxError(0, SYNTAXERR_REDEFINITION_TYPE);
}

void Parser::addConstBinding(NameComponent *ns, Str *name, Type *type_name,
                             BindingRib *rib) {
  if (rib == NULL)
    rib = topRib;
  ...
  rib->body.bindings.addAtEnd(
      ALLOC(Binding, (ns, name, type_name, TAG_constBinding)));
}

void Parser::addMethodBinding(NameComponent *ns, FunctionDefn *fn,
                              BindingRib *rib) {
  if (findBinding(ns, fn->name, TAG_methodBinding, rib))
    compiler->syntaxError(0, SYNTAXERR_REDUNDANT_METHOD);
  rib->body.bindings.addAtEnd(
      ALLOC(Binding, (ns, fn->name, NULL, TAG_methodBinding)));
...
void Parser::addNamespaceBinding(NameComponent *ns, Str *name, Expr *expr) {
  ...
  topRib->body.bindings.addAtEnd(
      ALLOC(Binding, (ns, name, NULL,
                      TAG_namespaceBinding))); // FIXME: type for 'Namespace'
...
void Parser::setUsesFinally() {
  ...
  if (topRib->tag == RIB_Instance && topRib->next != NULL &&
      topRib->next->tag == RIB_Class)
...
void Parser::setUsesSuper() { topRib->body.uses_super = true; }

void Parser::functionDefinition(bool config, Qualifier *qual,
                                bool getters_and_setters, bool require_body) {
  (void)config;
  uint32_t pos = position();
  FunctionDefn *fn =
      functionGuts(qual, true, getters_and_setters, require_body);
  // FIXME: if an interface method it's always public, plus functionGuts() will
  // have computed a namespace in signature.ns and we should use that.
  NameComponent *ns = qualToNamespace(qual);
  checkNoShadowingOfConfigNamespaces(pos, fn->name);
  if (topRib->tag == RIB_Instance) {
    // Class or interface.  If interface then is_static will never be true
    // because it's filtered during attribute parsing.
    if (qual->is_static) {
...
Stmt *Parser::variableDefinition(bool config, Qualifier *qual) {
  ...
  (void)config;
  Stmt *stmt =
      varStatement(qualToNamespace(qual), hd() == T_Const, qual->is_static > 0);
  semicolon();
  return stmt;
}
    ...
    for (;;) {
      if (hd() == T_TripleDot) {
        ...
        signature.rest_param =
            ALLOC(FunctionParam, (rest_name, rest_type_name, NULL));
        break;
      }
      ++signature.numparams;
      ...
      signature.params.addAtEnd(ALLOC(
          FunctionParam, (param_name, param_type_name, param_default_value)));
      if (hd() == T_RightParen)
        break;
      eat(T_Comma);
    }
...
BuiltinNamespace *Parser::internalNamespace() { return internalNS; }
...
NameComponent *Parser::qualToNamespace(Qualifier *qual) {
  ...
  case QUAL_name:
    compiler->internalError(
        position(), "Defintions in user-defined namespaces not implemented");
    /*NOTREACHED*/
  default:
    return NULL;
  }
...
void Parser::start() { T0 = lexer->lex(&L0, &V0); }
...
#else
    compiler->syntaxError(position(),
                          SYNTAXERR_WRONG_TOKEN); // FIXME: not adequate
#endif
  }
  next();
}
