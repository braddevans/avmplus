...
public:
  XmlContext(Compiler *compiler)
      : allocator(compiler->allocator), exprs(allocator), text(compiler),
        text_start(0) {}
...
void XmlContext::addText(Str *s) { text.append(s); }
...
void XmlContext::addText(wchar c) { text.append(c); }
...
void Parser::xmlEscape(XmlContext *ctx, const wchar *cs, const wchar *limit,
                       bool is_attr) {
  while (cs < limit) {
    wchar c;
    switch (c = *cs++) {
    ...
    case '\\':
      ...
      if (!is_attr)
        break;
      if (cs + 5 <= limit && cs[0] == 'u' && cs[1] == '0' && cs[2] == '0' &&
          cs[3] == '0') {
        switch (cs[4]) {
        case 'A':
        case 'a':
...
void Parser::xmlAssert(XmlContext *ctx, Token t, Escapement esc) {
  ...
  case T_XmlString:
    ...
    case ESC_elementValue:
      ...
      xmlEscape(ctx, V0.s->s + 1, V0.s->s + V0.s->length - 1, esc ==
      ESC_attributeValue); ctx->addText(V0.s->s[0]); break;
       */
    default:
      ctx->addText(V0.s);
      break;
