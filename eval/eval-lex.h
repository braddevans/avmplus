...
enum Token {
  ...
  T_RegexpLiteral,
  T_StringLiteral,
  T_XmlCDATA,   //  "<![CDATA[...]]>"  (including the punctuation, ditto for the
...
public:
  /**
   * @param compiler  The compiler structure, from which we take flags and
   * allocator
   ...
   *             of the input
   * @param keyword_or_ident  True iff this scanner is simply being used to
   * check whether an identifier that contains a backslash sequence looks like a
   * keyword.
   */
  Lexer(Compiler *compiler, const wchar *src, uint32_t srclen,
        bool keyword_or_ident = false);

  Token lex(uint32_t *linep, TokenValue *valuep); // Lex a token
  Token
  regexp(uint32_t *linep,
         TokenValue *valuep); // Following T_BreakSlash, to lex a regex literal
  Token divideOperator(
      uint32_t *linep); // Following T_BreakSlash, to lex a division operator
  Token rightAngle(uint32_t *linep); // Following T_BreakRightAngle, to lex '>'
                                     // at the end of a type instantiator
  Token leftAngle(uint32_t *linep); // Following T_BreakLeftAngle, to lex '<' in
                                    // a vector initializer
  Token rightShiftOrRelationalOperator(
      uint32_t *linep); // Following T_BreakRightAngle, to lex a shift or
                        // relational operator
  Token leftShiftOrRelationalOperator(
      uint32_t *linep); // Following T_BreakLeftAngle, to lex a shift or
                        // relational operator
   ...
   *   XmlSlashRightAngle
   *
   * For XmlComment, XmlCDATA, XmlProcessingInstruction, XmlName, XmlWhitespace,
   * XmlText, and XmlString, valuep->s is set to the actual text.
   */
...
private:
  ...
  enum {
    ...
    CHAR_ATTR_INITIAL =
        CHAR_ATTR_LETTER | CHAR_ATTR_UNDERBAR | CHAR_ATTR_DOLLAR,
  ...
  const wchar *mark;  // a remembered position, typically the start of a lexeme
                      // (not always valid)
  ...
  // Character attributes for the ASCII range, bit vectors of the CHAR_ATTR_
  // values above.
