...
class HalfMoonIR {
  ...
  /// an instance of a value is part of the one and only instruction that
  /// computes it as a result. Values are NOT held directly by the instructions
  /// consuming the value, rather, there are always use/def chain links
  /// inbetween (reword this). values are referred to instances of Use, use/def
  /// chain links, which are in turn referred to by the instructions using the
  /// value
  ///
  struct Def {
  ...
  /// These link together the uses of each value, and are held by the actual
  /// consuming instruction. When an instruction holds a Use it indicates that
  /// the instruction is data dependent on the instruction that computes it as a
  /// result.
  ///
  struct Use {
  ...
  /// base class of instructions. Most generic instruction does not have any
  /// results or uses..
  ///
  class Instr {
  public:
  ...
  class BinaryStmt : public Instr {
    ...
    Use next_stmt; // control input - continuation, points to a Def me of
                   // another Stmt
  ...
  Instr *term(BlockStmt *blockStmt) { return blockStmt->prev; }
  ...
  BlockStmt *block(Instr *instr) { return (BlockStmt *)instr->next; }
