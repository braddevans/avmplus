...
void debugWelcome() {
  ...
  printf("  DEBUG_STUB_LIST %d  compile calls to listed stubs instead of "
         "generate code\n",
         debug_stubs);
}
 ...
 */
static void initStubList(const char *env_var, const char *separator_chars,
                         bool bitmap[]) {
 ...
 * candidates. (This helps debugging as one can narrow down which inlining
 * decision is causing problems)
