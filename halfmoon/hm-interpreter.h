...
public:
  Interpreter(const InterpreterData &interpreter, MethodEnv *env, int argc,
              uint32_t *args, Value *values, uint32_t *args_out, Value *state)
      : values_(values), args_out_(args_out), env_(env),
        signature_(env->method->getMethodSignature()), argc_(argc),
        args_in_(args), interpreter_(interpreter), next_block_(0),
        saved_state_(state), frame_() {
    frame_.enter(core(), env);
  }

  ~Interpreter() { frame_.exit(core()); }

  /** Get the Value of a Def. */
  const Value &get(const Def *d) { return values_[defId(d)]; }

  /** Get the Value of def(u). */
  const Value &get(const Use &u) { return get(def(u)); }

  /** Get the Atom value of def(u). */
  Atom getAtom(const Use &u) { return get(u).atom_; }
  ...
  String *getString(const Use &u) { return get(u).string_; }

  Namespace *getNs(const Use &u) { return get(u).namespace_; }

  /** Get the ScriptObject* value of def(u). */
  ScriptObject *getObject(const Use &u) { return get(u).object_; }

  /** Get the ordinal (int) value of def(u). */
  int getOrdinal(const Use &u) { return get(u).ordinal_; }

  /** Get the MethodInfo* value of def(u). */
  MethodInfo *getMethod(const Use &u) { return get(u).method_; }

  /** Get the const Multiname* value of def(u). */
  const Multiname *getName(const Use &u) { return get(u).name_; }

  /** Get the int value of def(u). */
  int getInt(const Use &u) { return get(u).int_; }

  /** Get the uint value of def(u). */
  uint32_t getUint(const Use &u) { return (uint32_t)getInt(u); }

  /** Get the double value of def(u). */
  double getDouble(const Use &u) { return get(u).double_; }

  /** Get the Traits* value of def(u) */
  Traits *getTraits(const Use &u) { return get(u).traits_; }

  MethodEnv *getEnv(const Use &u) { return get(u).env_; }

  /** Return a reference to use to assign the value of def */
  Value &resultVal(const Def *def) { return values_[defId(def)]; }
  ...
  PrintWriter &console() { return core()->console; }
  bool isThisBoxed() { return env_->method->unboxThis() != 0; }
  AvmCore *core() { return env_->core(); }
  MethodSignaturep signature() { return signature_; }
  Lattice &lattice() { return interpreter_.ir_->lattice; }
