...
inline static void dbgInlineCallsite(CallStmt2 *call, Context *cxt,
                                     InstrGraph *callee_ir);
inline static void dbgPrintGraph(Context *cxt, InstrGraph *ir,
                                 const char *title);
struct InlineWorklistItem;
inline static void dbgPrintInlineWorklist(Context *cxt,
                                          SeqBuilder<InlineWorklistItem> &work);
inline static void dbgDumpIr(Instr *call_site, InstrGraph *outer_ir,
                             InstrGraph *callee_ir);
...
inline static void dbgPrintInlineWorklist(Context *,
                                          SeqBuilder<InlineWorklistItem> &) {}
 ...
 * can handle it, NOT whether it is correct to inline the callee at a specific
 * callsite!
 */
bool ableToInline(Context *cxt, CallStmt2 *call) {
  (void)cxt; // release build
  MethodInfo *method_info =
      targetOfCallSite(call); // getMethod(type(call->param_in()));

  if (!halfmoon::canCompile(method_info)) {
    INLINE_VERBOSE(
        cxt->out,
        " not able to inline because not halfmoon.canCompile: " << method_info);
    return false;
  }

  // TOOD: teach downstream inlining logic to fill in default parameters from
  // methodSig
  if (method_info->hasOptional()) {
    if (method_info->getMethodSignature()->param_count() !=
        call->arg_count() - 2) {
      INLINE_VERBOSE(
          cxt->out,
          " not able to inline because non-specified optional arguments: "
              << method_info);
      return false;
    }
  }
  if (!method_info->declaringScope()) {
    INLINE_VERBOSE(cxt->out, " not able to inline because not declaring scope: "
                                 << method_info);
    return false;
  }
  if (method_info->needRestOrArguments()) {
    INLINE_VERBOSE(cxt->out, " not able to inline because needRestOrArguments: "
                                 << method_info);
    return false;
  }
  if (dbgInlineSkip(method_info)) {
    INLINE_VERBOSE(cxt->out, " skip inline because listed in DEBUG env var: "
                                 << method_info);
    return false;
  }
  return true;
 ...
 *
 * @param cxt compilation context -- info like what method we're currently
 * compiling.
...
static bool isCorrectToStaticallyInline(Context *cxt, CallStmt2 *call_site) {
  ...
  Traits *declaring_traits =
      method_info->declaringTraits(); // class method is in.
  if (declaring_traits->final) {
    INLINE_VERBOSE(cxt->out, "correctToInline returns true for: "
                                 << method_info << " because class is final");
    return true;
  }
  ...
  if (method_info->isFinal()) {
    INLINE_VERBOSE(cxt->out, "correctToInline returns true for: "
                                 << method_info << " because method is final");
    return true;
  }
  INLINE_VERBOSE(cxt->out,
                 "correctToInline returns true false: " << method_info);
  return false;
}
 ...
 * Contest structs are sort of like a stack traceback, except for nested
 * inlines. (Probably a better way to think of them is as a path through the
 * call graph of an inline nest. If the callee appears on the path it's
 * recursive)
...
bool isRecursiveCallsite(Context *cxt, CallStmt2 *call_site) {
  ...
  if (method_info == cxt->method) {
    INLINE_VERBOSE(cxt->out, "isRecursiveCallsite returns true for: "
                                 << method_info
                                 << " because callsite is recursive");
    return true;
  }
  // if method is about to call a method somewhere on its call chain, it's
  // recursive
  for (const Context *aContext = cxt->caller; aContext != NULL;
       aContext = aContext->caller) {
...
InstrGraph *compileCallee(Context *cxt, CallStmt2 *call_site) {
  ...
  if (ir) {
    INLINE_VERBOSE(cxt->out, "found  IR (compiled previously) for: "
  ...
  TRY(core, kCatchAction_Rethrow) {
    INLINE_VERBOSE(cxt->out, "inliner requests compilation of: "
                                 << method_info->getMethodName());
  ...
  CATCH(Exception * exception) {
    ...
    // todo: fixme: this doesn't come close to cleaning up all the in-flight HM
    // state.
 ...
 * connectInputs hooks the defs of the actuals to the uses of the formal
 * parameters of the callee.
 *
 * this is almost identical to what we do to expand a statment template EXCEPT
 * that the callmethod statement takes extra arguments (like the one for the
 * methodname)
 *
...
void connectInputs(StartInstr *callee_start_instr, CallStmt2 *call_site) {
  ...
  int extra_formal_parameters =
      callee_start_instr->data_param_count() - num_data_actual_args;
  AvmAssert(extra_formal_parameters ==
            0); // TODO: make sure true for all callsites.
  ...
  for (int i = 0; i < num_data_actual_args; ++i) {
    ...
    Def *corresponding_formal_parm =
        callee_start_instr->data_param(i + extra_formal_parameters);
    // TODO: how do i distinguish between "FinalDefaultClass[O]~" and
    // FinalDefaultClass[A]~"
    AvmAssert(
        type(def_of_actual_arg)->isSubtypeOf(*type(corresponding_formal_parm)));
...
//   number of potential candidates that might be inlined in the containing
//   method.
//
...
//
//
bool shouldAttemptInline(Context *cxt, MethodInfo * /*candidate*/,
 ...
 * 0) clone the callee IR and locate the beginning and end of the callee
 * function. 0a if the callee has no flow (ie. its body is only one bb)
 * replaceStmt does the plumbing. o/w locate the bb at the head of the callee
 * (entry_list) and the bb at the tail (ret_list). 1) split the calling basic
 * block at the callsite 2) graft entry_list into the "outer" ir before the
 * callsite 3) graft ret_list into the "outer" ir after the callsite 4)
 * reconnect the uses and defs such that 4a) defs formerly feeding the callsite
 * now feed uses in the callee. 4b) uses in the caller formerly fed by the
 * return result of the callsite are now fed by defs in the callee that formerly
 * fed the return statement of the callee.
 *
 *             CALLER                                  CALLEE
 *                                       + prev_     +-----------+      <-----+
 *                                       |           |entry_list |            |
 ...
 *                                       |           |           |            |
 *                                       +----->     +-----------+     next_ -+
 * outputs: instruction lists have been sucked into the block and the
 * instruction at pos has been deleted:
 *
 *  +-prev_ +------------+ <---+
 *  |       |            |     |
 *  |       |   P        |     |
 ...
 *  |       | entry_list |     |
 *  |       |   ...      |     |
 *  |       |            |.....|......branch at end of entry_list has succs in
 * cloned callee
 *  +--->   +------------+next_+
 *
 ...
 *
 *  +prev_  +------------+  <---+
 *  |       |  ret_list  | .... |...... block at head of ret_list has preds in
 * cloned callee |       |  ...       |      | |       +------------+      | |
 * |   N        |      | |       |            |      |
 *  +---->  +------------+ next_+
 *
 ...
 *   ...
 *   U2   instr      (dCval)   //instruction consuming data value out of
 * callsite
 *
 *   1) we must connect the defs of D1 and D2 to uses in the head op(in place of
 * the callsite) 2) we must connect defs feeding the return statement in the
 * cloned callee graph with uses at U1, U2
 *
 *       D2:----+     //D2, D1 reconnected to parms of start.. (See
 * reconnectInputs) D1: +  | |  |               +----------------+ |  | | start
 * (parm1)  | v  v callsite          <<<arbitrary flow within callee >>> |  |
 * +----------------+ |  |               |   ret_list     | |  |               |
 * ...           | |  |                 A: ------+   // def reconnected to uses
 * at U1 (See reconnectOutputs) U1 <+  |                 C: --+   |   // def
 * reconnected to use at U2 U2 <---+                      v   v ret (d)
 *
 ...
 * </pre>
 */
void inlineCallsite(InstrGraph *outer_ir, InstrFactory *factory,
                    InstrGraph *callee_ir, const InlineWorklistItem &item) {
  ...
  // copy the callees start,return,throw instructions.. just floating between
  // graphs (?)
 ...
 * @param ir  HMIR of a method to scan for callsites, and inline those that
 * match policy
...
bool inlineGraph(Context *cxt, InstrGraph *ir, const char *title) {
  ...
  inline_enable_verbose |=
      debugIsInSkiplist("DEBUG_INLINE_VERBOSITY_CALLER", caller_name);
  ...
  for (SeqRange<InlineWorklistItem> iter(work); !iter.empty();
       iter.popFront()) {
 ...
 * env method name listed in DEBUG_INLINE_SKIP indicates that we should never
 * inline it. (Useful for working a specific problem with a specific callee)
 */
inline static bool dbgInlineSkip(MethodInfo *method_info) {
  // when DEBUG_INLINE_CANDIDATE env is set, allow ONLY inlines of methods
  // listed.
  const char *env = VMPI_getenv("DEBUG_INLINE_CANDIDATE");
  if (env != NULL && *env != 0) {
    return debugIsInSkiplist("DEBUG_INLINE_CANDIDATE",
                             method_info->getMethodName());
  }
  // otherwise, if DEBUG_INLINE_SKIP contains the name, skip it.
  // use like this:
  // env DEBUG_INLINE_SKIP="`cat inline-skip-list.txt`" INLINE=1 avmshell
  // -Dnodebugger xx.abc
  //
 ...
 *  returns true when env vars suggest we should skip examining callsites of the
 * method for inlining.
 */
inline static bool dbgInlineOnly(MethodInfo *method_info) {
 ...
 */
inline static void
dbgInlineCallsite(CallStmt2 *call, Context *cxt,
                  InstrGraph *callee_ir /*InlineWorklistItem& item*/) {
 ...
 */
inline static void
dbgPrintInlineWorklist(Context *cxt, SeqBuilder<InlineWorklistItem> &work) {
...
inline static void dbgPrintGraph(Context *cxt, InstrGraph *ir,
  ...
  char file_name[512];
  static int fnum = 0;
  VMPI_snprintf(file_name, 512, "%s-%d", title,
                fnum++); // groan, no snprintf on windows
