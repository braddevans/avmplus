...
void printLinkedUsers(Def &d) { return printLinkedUsers(&d); }
  ...
  for (AllInstrRange i(ir); !i.empty(); i.popFront()) {
    ...
    for (ArrayRange<Def> d = defRange(instr); !d.empty(); d.popFront(), ++pos) {
      ...
      if (use_counts[id][pos] != use_count) {
        ...
        printf("scanned: ");
        printScannedUsers(pos, uses[id]);
        printf("linked:  ");
        printLinkedUsers(def);
...
void TypeChecker::report(int arg_number, const Use &arg, const Type *sig_type,
  ...
  printf("typecheck failed: %s %s%d use %d: %s not %s %s\n", name(user(arg)),
         kInstrPrefix, user(arg)->id, arg_number, typeName(arg), relation,
         typeName(sig_type));
  fail();
}
    ...
    if (isBlockEnd(instr)) {
      ...
      if (end->catch_blocks != NULL) {
        for (CatchBlockRange r(end); !r.empty(); r.popFront()) {
          ...
          if (!found) {
            printf("missing exception back edge: i%d -> i%d", end->id,
                   r.front()->id);
          }
          AvmAssert(found);
        }
 ...
 * 4. unmark this block
 */
void checkSSABlock(BSI *block, Seq<BSI *> **children, BitSet *visited,
                   BSI **blockmap) {
...
struct LirBlock {
  ...
  LirBlock(LIns *last, LirBlock *next, int linear_id)
      : first_ins(0), last_ins(last), linear_id(linear_id), post_id(0),
        num_succs(0), succs(0), preds(0), loop_pred(0), prev(0), next(next),
        next_rpo(0), idom(0) {
    if (next)
      next->prev = this;
  }
...
public:
  DepthFirstSearch(LirBlock *b) : next_post_id(0), list(0) { dfs(b); }
...
enum LInsStatus { kNeverUsed, kUsed, kRetired };
...
public:
  LirCfg(Fragment *fragment)
      : printer(fragment->lirbuf->printer), last_ins(fragment->lastIns),
        num_blocks(0), first_block(0), last_block(0), error(false) {
    Allocator scratch;
    LirBlock::map_t blockmap(scratch);

    // Find blocks.
    int num_ins = 0;
    LirBlock *blocks = last_block =
        new (alloc) LirBlock(last_ins, 0, num_blocks++);
    LirReader r(last_ins);
    for (LIns *ins = r.read(); !ins->isop(LIR_start); ins = r.read()) {
      ...
      if (pred && !isAfter(use_block, pred)) {
        ...
        if (printer) {
          ...
          printf("LIR Error: Definition %s in B%d needs a LIR_live after B%d "
                 ...
                 pred->linear_id, printer->formatIns(&ib, use_ins),
                 use_block->linear_id, pred->linear_id, b->linear_id);
        }
        setError();
      }
    }
    LIns *alloc;
    if (use_ins->isCall() && (alloc = getAllocIns(def_ins)) != NULL &&
  ...
  void checkRetired(LIns *def_ins, LIns *use_ins) {
    ...
    if (printer) {
      ...
      InsBuf ib;
      printf(
          "LIR Error: Definition %s in B%d must come before Use: %s in B%d\n",
  ...
  bool isReachable(LirBlock *b) { return b == first_block || b->idom; }

  LirBlock *block(LIns *ins) { return insmap->get(ins); }

  int blockid(LIns *ins) { return block(ins)->linear_id; }
  ...
  void checkDominates(LIns *def_ins, LIns *use_ins) {
    ...
    if (printer) {
      ...
      InsBuf ib;
      printf(
          "LIR Error: Definition %s in B%d does not dominate Use: %s in B%d\n",
          printer->formatRef(&rb, def_ins, false), blockid(def_ins),
          printer->formatIns(&ib, use_ins), blockid(use_ins));
    }
    setError();
  ...
  void def(LIns *def_ins) { setStatus(def_ins, kRetired); }
