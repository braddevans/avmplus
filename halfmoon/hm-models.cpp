...
InstrKind toModelAtom(const Type *t) {
  switch (model(t)) {
  default:
    AvmAssert(false && "bad model for toModelAtom");
  case kModelString:
    return HR_string2atom;
  case kModelDouble:
    return HR_double2atom;
  case kModelScriptObject:
    return HR_scriptobject2atom;
...
InstrKind toModelDouble(const Type *t) {
  switch (model(t)) {
  default:
    AvmAssert(false && "bad model for toModelDouble");
  case kModelAtom:
    return HR_atom2double;
  case kModelInt:
    return isInt(t) ? HR_i2d : isUInt(t) ? HR_u2d : HR_MAX;
  }
}
...
InstrKind toModelInt(const Type *t, const Type *use_type) {
  ...
  switch (model(t)) {
  default:
    AvmAssert(false && "bad input model for toModelInt");
  case kModelAtom: {
    if (isBoolean(t)) {
      return HR_atom2bool;
    } else {
...
void ModelFixer::fixDefs() {
  ...
  InstrKind
      converts_kind[kConvertsMax]; // verify that the conversion is the same
      ...
      for (ArrayRange<Def> dr = defRange(instr); !dr.empty(); sr.popFront()) {
        ...
        for (UseRange u(*d); !u.empty();) {
          ...
          if (!submodelof(def_type, constraint)) {
            // need a conversion.  The same model may require different
            // conversions
...
const Type *ModelFixer::getConstraint(const Use &u) {
  ...
  case HR_callstatic:
    if (use_pos >= 2) {
      ...
      if (isEnv(env_type)) {
        MethodSignaturep sig =
            getMethod(type(call->param_in()))->getMethodSignature();
