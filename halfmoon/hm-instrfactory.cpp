...
InstrInfo *InfoManager::get(InstrKind kind, InstrFactory *factory) {
  ...
  if (!info) {
    InstrFactory newfactory(alloc_, &factory->lattice(),
                            &factory->infoManager());
    InstrGraph *ir =
        newfactory.hasTemplate(kind) ? newfactory.buildTemplate(kind) : NULL;
    const Type **insig = newfactory.buildInputSignature(kind);
    const Type **outsig = newfactory.buildOutputSignature(kind);
    info = new (alloc_) InstrInfo(INSTR::createInfo(kind, insig, outsig, ir));
...
NaryStmt0 *InstrFactory::toNaryStmt0(InstrKind k, Instr *instr) {
  AvmAssert(isNaryStmt0(kind(instr)) &&
            "unsupported source opcode for statement conversion");
...
///
CallStmt2 *InstrFactory::createCallStmt2(InstrKind kind, Def *effect,
                                         Def *param, Def *obj, int varargc) {
...
/// initializing the varargs
///
CallStmt3 *InstrFactory::createCallStmt3(InstrKind kind, Def *effect,
                                         Def *param, Def *index, Def *obj,
                                         int varargc) {
...
ConstantExpr *InstrFactory::newConstantExpr(InstrKind kind, const Type *type) {
  ...
  ConstantExpr *expr =
      new (alloc_) ConstantExpr(infos_.get<ConstantExpr>(kind, this));
...
UnaryExpr *InstrFactory::toUnaryExpr(InstrKind k, Instr *instr) {
  AvmAssert(isUnaryExpr(kind(instr)) &&
            "invalid source opcode for UnaryExpr conversion");
...
BinaryExpr *InstrFactory::toBinaryExpr(InstrKind k, Instr *instr) {
  AvmAssert(isBinaryExpr(kind(instr)) &&
            "invalid source opcode for BinaryExpr conversion");
  AvmAssert(isBinaryExpr(k) &&
            "invalid target opcode for BinaryExpr conversion");
...
UnaryStmt *InstrFactory::toUnaryStmt(InstrKind k, Instr *instr) {
  AvmAssert(isUnaryStmt(kind(instr)) &&
            "invalid source opcode for UnaryStmt conversion");
...
///
BinaryStmt *InstrFactory::newBinaryStmt(InstrKind k, Def *effect, Def *lhs,
                                        Def *rhs) {
...
BinaryStmt *InstrFactory::toBinaryStmt(InstrKind k, Instr *instr) {
  AvmAssert(isBinaryStmt(kind(instr)) &&
            "invalid source opcode for BinaryStmt conversion");
  AvmAssert(isBinaryStmt(k) &&
            "invalid target opcode for BinaryStmt conversion");
...
StartInstr *InstrFactory::newStartInstr(InstrKind kind, int paramc,
  ...
  StartInstr *start = new (alloc_)
      StartInstr(infos_.get<StartInstr>(kind, this), paramc, false, params);
...
StartInstr *InstrFactory::newStartInstr(MethodInfo *method) {
  ...
  int fixedc = 1 /* effect */ + 1 /* env */ + 1 /* this */ + sig->param_count();
...
CatchBlockInstr *InstrFactory::newCatchBlockInstr(int paramc) {
  Def *params = new (alloc_) Def[paramc];
  CatchBlockInstr *start = new (alloc_) CatchBlockInstr(
      infos_.get<CatchBlockInstr>(HR_catchblock, this), paramc, params);
  return start;
}
...
void InstrFactory::initCondInstr(Allocator &alloc, CondInstr *instr, Def *sel,
                                 int argc, Def *default_arg,
                                 const InstrInfo *arm_info) {
  instr->argc = argc;
...
LabelInstr *InstrFactory::newLabelInstr(int paramc) {
  ...
  LabelInstr *label =
      new (alloc_) LabelInstr(info, paramc, new (alloc_) Def[paramc]);
  for (int i = 0; i < paramc; ++i)
...
StopInstr *InstrFactory::createStopInstr(InstrKind k, int argc) {
  ...
  return new (alloc_)
      StopInstr(infos_.get<StopInstr>(k, this), argc, new (alloc_) Use[argc]);
}
...
///
StopInstr *InstrFactory::newStopInstr(InstrKind k, Def *effect, int data_argc,
                                      Def *data_args[]) {
  AvmAssert(isStopInstr(k) && "invalid opcode for StopInstr");
...
///
SafepointInstr *InstrFactory::newSafepointInstr(Def *effect, int argc,
                                                Def *args[]) {
...
/// state tuple, and value to set
///
GetlocalStmt *InstrFactory::newGetlocalStmt(int index, Def *effect, Def *state,
                                            Def *val) {
...
///
DeoptSafepointInstr *InstrFactory::newDeoptSafepointInstr(Def *effect, int argc,
                                                          Def *args[]) {
  InstrInfo *info =
      infos_.get<DeoptSafepointInstr>(HR_deopt_safepoint, argc, this);
...
/// Capture the inferred type of an expression, which should be a preceding
/// call. Used with new-style safepoints.
///
DeoptFinishCallInstr *InstrFactory::newDeoptFinishCallInstr(Def *effect,
                                                            Def *val) {
...
/// attempts to look up metadata for addresses for which no safepoint was
/// recorded. In the future, it may facilitate recovery of frame slots from
/// registers.
///
...
DebugInstr *InstrFactory::newDebugInstr(InstrKind kind, Def *effect, Def *val) {
  DebugInstr *debuginstr =
      new (alloc_) DebugInstr(infos_.get<DebugInstr>(kind, this));
  // placement new to initialize uses
...
DebugInstr2 *InstrFactory::newDebugInstr2(InstrKind kind, Def *effect,
                                          Def *val1, Def *val2) {
  DebugInstr2 *debuginstr =
      new (alloc_) DebugInstr2(infos_.get<DebugInstr2>(kind, this));
  // placement new to initialize uses
  new (&debuginstr->effect_in()) Use(debuginstr, effect);
...
Copier::Copier(InstrGraph *from_ir, InstrGraph *to_ir)
    : scratch0_(scratch_), map_(new (scratch0_) Instr *[from_ir->size()]),
      to_ir_(to_ir), to_alloc_(to_ir->alloc()) {}
...
Instr *Copier::deepCopy(Instr *instr) {
  ...
  case HR_if:
  case HR_switch:
    // uses on these instructions were placement-new'd in their clone()
    // function.
    break;
  default:
...
Instr *Copier::do_ArmInstr(ArmInstr *i) {
  ...
  a->arm_pos = i->arm_pos;
  a->owner = i->owner;
  AvmAssert(false && "If owner can be a shallow copy, then remove this "
                     "AvmAssert, otherwise fix this method!!!");
  return a;
}

Instr *Copier::do_IfInstr(IfInstr *oldif) {
  IfInstr *newif = new (to_alloc_) IfInstr(oldif->info);
  InstrFactory::initCondInstr(to_alloc_, newif, NULL, oldif->argc, NULL,
                              oldif->arms[0]->info);
  return newif;
}
...
Instr *Copier::do_SwitchInstr(SwitchInstr *oldsw) {
  SwitchInstr *newsw =
      new (to_alloc_) SwitchInstr(oldsw->info, oldsw->num_cases());
  InstrFactory::initCondInstr(to_alloc_, newsw, NULL, oldsw->argc, NULL,
                              oldsw->arms[0]->info);
  return newsw;
}
...
Instr *Copier::do_StopInstr(StopInstr *stop) {
  int argc = stop->argc;
  StopInstr *stop2 =
      new (to_alloc_) StopInstr(stop->info, argc, new (to_alloc_) Use[argc]);
  for (int i = 0; i < argc; ++i)
...
Instr *Copier::do_DeoptSafepointInstr(DeoptSafepointInstr *other) {
  DeoptSafepointInstr *vmstate =
      new (to_alloc_, other->info->num_uses, sizeof(Use))
          DeoptSafepointInstr(other->info);
...
Instr *Copier::do_DeoptFinishCallInstr(DeoptFinishCallInstr *other) {
  DeoptFinishCallInstr *instr =
      new (to_alloc_) DeoptFinishCallInstr(other->info);
  instr->safepoint = other->safepoint;
  return instr;
}
...
Instr *Copier::do_GotoInstr(GotoInstr *go) {
  ...
  GotoInstr *go2 =
      new (to_alloc_) GotoInstr(go->info, new (to_alloc_) Use[argc]);
  for (int i = 0; i < argc; ++i)
...
Instr *Copier::do_LabelInstr(LabelInstr *label) {
  ...
  return new (to_alloc_)
      LabelInstr(label->info, paramc, new (to_alloc_) Def[paramc]);
}
