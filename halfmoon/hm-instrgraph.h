...
class Def {
public:
  Def() : owner_(0), type_(0), uses_(0) {}
  explicit Def(Instr *owner) : owner_(owner), type_(0), uses_(0) {}
  Def(Instr *owner, const Type *t) : owner_(owner), type_(t), uses_(0) {}
  ...
  ///
  bool isUsed() const { return uses_ != 0; }
...
private:
  bool checkValid() const {
    ...
    return owner_ &&
           uint32_t((uintptr_t)owner_) != uint32_t(MMgc::GCHeap::FXFreshPoison);
  }
...
public:
  ...
  explicit Use(Instr *instr) : def_(0), owner_(instr) {}

  Use(Instr *instr, Def *d) : owner_(instr) { set(d); }
  ...
  /// or false if def_ == 0.
  operator bool() const { return def_ != 0; }
...
class UseRange {
public:
  explicit UseRange(const Def &d) : def(&d) {
    Use *N = use = d.uses_;
...
public:
  ...
  InstrInfo(int _num_uses, int _num_defs, int _uses_off, int _defs_off,
            InstrKind kind, const Type **insig, const Type **outsig,
            InstrGraph *ir = NULL)
      : kind_(kind), insig_(insig), outsig_(outsig),
        shape_(instr_attrs[kind].shape), num_uses(_num_uses),
        num_defs(_num_defs), uses_off(_uses_off), defs_off(_defs_off), ir(ir) {
    checkValid();
  }
  ...
  const char *name() const { return instr_attrs[kind_].name; }
...
public:
  const InstrInfo
      *info; // todo: encapsulate access to this, especially assignment.
...
///
inline InstrShape shape(const Instr *instr) { return instr->info->shape_; }

/// Return instr's opcode.
///
inline InstrKind kind(const Instr &instr) { return instr.info->kind_; }

/// Return instr's opcode.
///
inline InstrKind kind(const Instr *instr) { return kind(*instr); }
...
///
inline const Type *type(const Def *def) { return type(*def); }

/// Return the type of the def pointed to by use.
///
inline const Type *type(const Use &use) { return type(def(use)); }

/// Return the instruction that defines d.
///
inline Instr *definer(const Def &d) { return d.owner_; }

/// Return the instruction that defines d (overloaded for Def*).
///
inline Instr *definer(const Def *d) { return definer(*d); }

/// Return the instruction that defines the value being used by u.
///
inline Instr *definer(const Use &u) { return definer(def(u)); }

inline int definerId(const Def &d) { return definer(d)->id; }

inline int definerId(const Use &u) { return definer(u)->id; }

inline int definerId(const Def *d) { return definer(d)->id; }

/// Return the instruction that owns use.
///
inline Instr *user(const Use &use) { return use.owner_; }

/// Return definer(d)'s opcode.
///
inline InstrKind kind(const Def &d) { return kind(definer(d)); }
...
///
inline InstrKind kind(const Use &u) { return kind(def(u)); }
...
inline int pos(const Def *d) { return pos(*d); }
...
///
inline int defId(const Def *d) { return definer(d)->id + pos(*d); }
...
///
inline bool hasSubgraph(const Instr *i) { return i->info->ir != NULL; }

/// Return i's implementation InstrGraph.
///
inline InstrGraph *subgraph(const Instr *i) { return i->info->ir; }
...
///
///
template <class INSTR> inline INSTR *cast(Instr *instr) {
...
/// invariant shape.
///
template <class INSTR> inline const INSTR *cast(const Instr *instr) {
  AvmAssert(!instr || shape(instr) == INSTR::shape);
...
public:
  ...
  InstrFactory &factory() { return factory_; }
...
class InstrRange {
public:
  /// Initialize the range with a specific start and back_.  They must be
  /// connected.
  ///
  InstrRange(Instr *start, Instr *end) : front_(start), back_(end) {}
...
public:
  ...
  ///
  int size() const { return def_count_; }
  ...
  /// Remove instr from whatever list it is in, without regard for whether
  /// it is the last one or not.
  static void unlinkInstr(Instr *instr) { InstrRange::unlinkInstr(instr); }
  ...
  ///
  static bool isLinked(Instr *instr) { return instr->isLinked(); }
  ...
  /// \param list Infirst head of circular list of instructions (fragment) to be
  /// inserted.
  ///
  ...
  int def_count() const { return def_count_; }
  int instr_count() const { return instr_count_; }
...
public:
  SigRange(const Type **sig, int len, int vararg = -1)
      : sig(sig), len(len), vararg(vararg >= 0 ? vararg : len), pos(0) {}
...
class AllUsesRange {
public:
  explicit AllUsesRange(Instr *instr) : d(defRange(instr)), u() { next(); }

  bool empty() const { return d.empty() || u.empty(); }
...
class AllInstrRange {
public:
  AllInstrRange(InstrGraph *ir) : b(ir), i(0) {
    if (!b.empty())
      i = InstrRange((Instr *)b.popFront());
  }
  bool empty() const { return i.empty(); }
  Instr *front() const { return i.front(); }
  Instr *popFront() {
...
public:
  explicit PostorderBlockRange(InstrGraph *ir) : EachBlock(ir, false) {}
};
