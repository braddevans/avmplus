...
def genLabelDefSection(tem, end_label_name, indent):
  ...
  for label in list(tem.labels.values()):
...
def allReps():
  repset = set(replist +
                 list(instr_rep_overrides.values()) +
                 list(shape_rep_overrides.values()))
...
def getRepCount(rep, defs):
  ...
  if rep_counts is None:
    ...
    rep_counts = dict(list(zip(repnames, [defrepnames.count(repname) for repname in repnames])))
