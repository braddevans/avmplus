...
Value Interpreter::stub(MethodEnv *env, int32_t argc, uint32_t *args) {
  ...
  Value *values =
      (Value *)avmStackAlloc(core, vals_autoptr, num_values * sizeof(Value));
  void *argsout =
      avmStackAlloc(core, args_autoptr, interpreter.max_argc_ * sizeof(Value));
  int frame_size = env->method->getMethodSignature()->frame_size();
  Value *saved_state =
      (Value *)avmStackAlloc(core, state_autoptr, frame_size * sizeof(Value *));
  core->stackCheck(env);
...
Value atomToValue(Atom value, const ParamIter &p) {
  ...
  case kModelInt:
    ...
    default:
      AvmAssert(false && "bad builtinType");
    case BUILTIN_int:
      return Value(AvmCore::integer_i(value));
    case BUILTIN_uint:
      return Value(AvmCore::integer_u(value));
    case BUILTIN_boolean:
      return Value(atomGetBoolean(value));
    }
  case kModelString:
...
void Interpreter::jump(BlockEndInstr *end, BlockStartInstr *start) {
  ...
  for (ArrayRange<Value> t(temp, argc); !t.empty();
       t.popFront(), args.popFront())
    t.front() = get(args.front());
  ArrayRange<Def> defs(defRange(start));
  for (ArrayRange<Value> t(temp, argc); !t.empty();
       t.popFront(), defs.popFront())
    resultVal(&defs.front()) = t.front();
  next_block_ = start;
}

void Interpreter::do_goto(GotoInstr *go) { jump(go, go->target); }
  ...
  default:
    ...
    case kModelInt:
      result = isBoolean(t) ? Value(boolVal(t))
                            : isInt(t) ? Value(intVal(t)) : Value(uintVal(t));
      break;
    case kModelDouble:
...
void Interpreter::do_callinterface(CallStmt2 *instr) {
  ...
  MethodEnv *callee_env = getEnv(instr->param_in());
  doCallInterface(interface_info, callee_env, instr->arg_count(), instr->args(),
                  instr->value_out());
}
...
void Interpreter::coerceArgs(MethodSignaturep method_sig, int actual_argc_out,
  ...
  for (int i = 0; i < actual_argc_out; ++i, p.popFront()) {
    ...
    AvmAssert(p.model() == model(type(arg)) &&
              "Signature model doesn't match arg model");
...
void Interpreter::doCallInterface(MethodInfo *interface_info,
                                  MethodEnv *callee_env, int actual_argc_out,
                                  const Use *call_args, Def *value_out) {
...
void Interpreter::doCall(MethodEnv *callee_env, int actual_argc_out,
                         const Use *call_args, Def *value_out) {
  coerceArgs(callee_env->method->getMethodSignature(), actual_argc_out,
             call_args);
  Value &result = resultVal(value_out);
  // AVM calling conventions don't count the 'this' argument.
  int argc = actual_argc_out - 1;
...
void Interpreter::do_setslot(CallStmt2 *instr) {
  ...
  case kModelAtom:
    core()->atomWriteBarrier(core()->gc, object, (Atom *)ptr,
                             getAtom(value_in));
    break;
  case kModelInt:
...
void Interpreter::do_setlocal(SetlocalInstr *instr) {
  ...
  // occured on it.
  //
  // NOTE (stan, 22 Nov 13): This is not how the compiler works, and I beieve
  // tha above is wrong. The setlocal instr does indeed modify the state (which
  // is why it has a state in and out!) The safepoint instruction consumes that
  // state to keep the setlocal from becoming dead and to impose order
  // constraints due to the location of the safepoint instr in the effects
  // chain. However also note: if the safepoint instruction _is_ DCEed, so much
  // the better!
  //
  // TODO: Maybe implement getlocal? Not clear if it's really necessary; it's
  // only to support the debugger.
  //
