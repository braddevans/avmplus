...
public:
  IdentityAnalyzer(Def *def, Lattice *lattice) : def_(def), lattice_(lattice) {}
...
public: // dispatch() adapter methods.
  ...
  Def *do_castobject(UnaryExpr *i) {
    return coerceIdentity(i, lattice_->object_type[kTypeNullable]);
  }
  Def *do_caststring(UnaryStmt *i) {
    return coerceIdentity(i, lattice_->string_type[kTypeNullable]);
  }
  Def *do_tonumber(UnaryStmt *i) {
    return coerceIdentity(i, lattice_->double_type);
  }
  Def *do_toint(UnaryStmt *i) { return coerceIdentity(i, lattice_->int_type); }
  Def *do_touint(UnaryStmt *i) {
    return coerceIdentity(i, lattice_->uint_type);
  }
  Def *do_toboolean(UnaryExpr *i) {
    return coerceIdentity(i, lattice_->boolean_type);
  }
  Def *do_doubletoint32(UnaryExpr *i) {
    return coerceIdentity(i, lattice_->int_type);
  }
  Def *do_abc_findproperty(NaryStmt3 *i) { return doFindStmt(i); }
  Def *do_abc_findpropstrict(NaryStmt3 *i) { return doFindStmt(i); }
  Def *do_atom2scriptobject(UnaryExpr *i) {
    return doModelChange(i, HR_scriptobject2atom);
  }
