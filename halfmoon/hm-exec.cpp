...
void BaseExecMgr::verifyProfilingJit(MethodInfo *methodInfo,
  ...
  if (code) {
    ...
    jit.~ProfileLirEmitter(); // Explicit cleanup since destructor won't run
                              // otherwise.
  }
}

/// We finished profiling the method, at the next invocation recompile with
/// halfmoon Still pretty sure we don't have to set all 3 fields to redirect
/// control flow. What's the diff between env->_implGPR and env->method->implGPR
/// and invoker? I can't find a clear distinction
...
/// We found out a method is hot. At the next invocation, recompile with
/// instrumentation I'm pretty sure we don't have to set all 3 fields... but I
...
///
/// env->method->implGPR is written with the address of the JIT compiled code
/// during actual compilation. I'm not sure when env->method->implGPR is ever
/// directly loaded but its the same address as env->implGPR. The "this" object
/// must already be unboxed into the appropriate script object
///
...
///
/// I think this is the correct order, still slightly unsure / bug testing -
/// Mason
...
/// We detected a hot method. Recompile the method with profiling
/// instrumentation
/* static */
