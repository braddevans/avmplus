...
public:
  SpecialBuilder(InstrGraph *ir, InstrFactory *factory, Instr *pos,
                 const Use &effect_in)
      : InstrGraphBuilder(ir, factory, pos), ir_(ir), lattice_(&ir->lattice),
  ...
  template <class STMT> Def *addStmt(STMT *stmt) {
    addInstr(stmt);
    effect_ = stmt->effect_out();
    return stmt->value_out();
  }

  template <class EXPR> Def *addExpr(EXPR *expr) {
    addInstr(expr);
...
Specializer::Specializer(InstrGraph *ir)
    : ir_(ir), factory_(ir), lattice_(&ir->lattice) {}

void Specializer::specialize(Instr *instr) { do_instr(this, instr); }

InstrKind Specializer::getLoadEnvKind(const Type *object, bool is_interface) {
  switch (kind(object)) {
  default:
    AvmAssert(false && "Unknown load env type");
  case kTypeScriptObject:
    return is_interface ? HR_loadenv_interface : HR_loadenv;
  case kTypeString:
    return HR_loadenv_string;
  case kTypeNumber:
    return HR_loadenv_number;
  case kTypeBoolean:
    return HR_loadenv_boolean;
  case kTypeNamespace:
    return HR_loadenv_namespace;
  case kTypeAny:
  case kTypeVoid:
  case kTypeObject:
...
struct IndexOpcodes2 {
  IndexOpcodes vectorint_ops, vectoruint_ops, vectordouble_ops, array_ops,
      default_ops;
};
...
InstrKind choose(Lattice *l_, const Type *index_type, const Type *object_type,
  ...
  const IndexOpcodes &ops =
      subtypeof(object_type, l_->vectorint_type[kTypeNotNull])
          ? ops2.vectorint_ops
          : subtypeof(object_type, l_->vectoruint_type[kTypeNotNull])
                ? ops2.vectoruint_ops
                : subtypeof(object_type, l_->vectordouble_type[kTypeNotNull])
                      ? ops2.vectordouble_ops
                      : subtypeof(object_type, l_->array_type[kTypeNotNull])
                            ? ops2.array_ops
                            :
...
void Specializer::do_abc_getpropx(CallStmt3 *instr) {
  ...
  if (maybeIndex(type(instr->param_in()), index_type)) {
    factory_.toCallStmt3(
        choose(lattice_, index_type, type(instr->object_in()), get_ops), instr);
  }
}
...
void Specializer::do_abc_setpropx(CallStmt3 *instr) {
  ...
  if (maybeIndex(type(instr->param_in()), index_type)) {
    factory_.toCallStmt3(
        choose(lattice_, index_type, type(instr->object_in()), set_ops), instr);
  }
}
...
void Specializer::do_abc_initprop(CallStmt2 *instr) {
  ...
  //    // this is a) a slot access, and b) a legal assignment if the slot is
  //    const. factory_.toCallStmt2(HR_setprop_slot, instr);
  //  }
}
...
bool Specializer::specializeSlotCallProp(CallStmt2 *instr,
                                         CallAnalyzer *call_analyzer) {
  const TraitsBindingsp tb =
      getTraits(type(instr->object_in()))->getTraitsBindings();
...
bool Specializer::specializeMethodCallProp(CallStmt2 *instr,
                                           CallAnalyzer *call_analyzer) {
  ...
  SpecialBuilder builder(ir_, &factory_, instr, instr->effect_in());
  // so name is not a name anymore.. it's an ordinal.. presumably index of
  // virtual function slot (matz)
  Def *object_def =
      builder.coerceExpr(getParamTraits(call_analyzer->signature, 0), def(obj));
  obj = object_def;
  InstrKind env_kind =
      getLoadEnvKind(type(object_def), call_analyzer->is_interface);

  Def *env_param =
      call_analyzer->is_interface
  ...
  for (int i = 0; i < extra_argc; ++i) {
    Use &arg = instr->vararg(i);
    arg = builder.coerceExpr(getParamTraits(call_analyzer->signature, i + 1),
                             def(arg));
  }

  InstrKind methodKind =
      call_analyzer->is_interface ? HR_callinterface : HR_callmethod;
  instr->effect_in() = builder.effect();
  builder.toCallStmt2(methodKind, instr);
  return true;
}
...
void Specializer::do_abc_callprop(CallStmt2 *instr) {
  ...
  if (isSlot(b.binding) && (extra_argc == 1)) {
    if (specializeSlotCallProp(instr, &b))
      return;
  }
}
...
bool canBindConstructor(Traits *ctraits, int actual_argc) {
  // fixme: this is copied from CodegenLIR::emitConstruct(); factor it out
  // instead.
  Traits *itraits;
  MethodInfo *init;
  return ctraits && !ctraits->hasCustomConstruct &&
         (itraits = ctraits->itraits) != NULL &&
         (init = itraits->init) != NULL && init->isResolved() &&
...
void Specializer::do_construct(CallStmt2 *instr) {
  ...
  for (int i = 1, n = instr->arg_count(); i < n; ++i)
    args[i] =
        builder.coerceExpr(getParamTraits(signature, i), def(instr->arg(i)));
  CallStmt2 *call = factory_.newCallStmt2(HR_callmethod, builder.effect(), env,
                                          object, arg_count - 1, args + 1);
...
/**
 * Return true if values with this traits cannot be compared with pointer
 * identity.
 */
bool hasComplexEq(Traits *t) { return !t || t->hasComplexEqualityRules(); }
...
bool canComparePointers(const Type *x, const Type *y) {
  ...
  return (!hasComplexEq(xt) && !hasComplexEq(yt)) ||
         (isNull(x) && !hasComplexEq(yt)) || (isNull(y) && !hasComplexEq(xt));
}
