...
public:
  AbcBuilder(MethodInfo *, AbcGraph *abc, InstrFactory *, Toplevel *,
             ProfiledInformation *profiled_info, bool has_reachable_exceptions);
  ~AbcBuilder();
...
private:
  ...
  void visitInstr(const uint8_t *pc, AbcOpcode, uint32_t imm30, uint32_t imm30b,
                  int32_t imm8);
  Traits *getNamedTraits(uint32_t index);
  NameArity nameArity(const Type *name);
  NameArity nameArity(Def *name) { return nameArity(type(name)); }
  ...
  Def *naryStmt4(InstrKind, Def *name, Def *obj, Def *ns, Def *index,
                 Def *index2, Def **args, int extra_argc);
  ...
  Def *getTypedDef(const uint8_t *pc, Def *current_def, int input_index,
                   int input_count, int output_count);
  RecordedType getRecordedType(const uint8_t *pc, int input_index,
                               int input_count, int output_count);
  Def *typeSpecializedBinary(AbcOpcode abcop, const uint8_t *pc, Def *lhs,
                             Def *rhs);

private:
  Def *createConst(const Type *type) { return builder_.addConst(type); }
  Def *ordinalConst(int x) { return createConst(lattice_.makeOrdinalConst(x)); }
  ...
  Def *getLocal(int i) { return frame_[i]; }
  template <class E> FrameRange<E> frameRange(E *frame) {
    return FrameRange<E>(frame, stackp_, scopep_, stack_base_);
  }
  void returnStmt(Def *value) { stopStmt(HR_return, value, &return_label_); }
  void throwStmt(Def *value) { stopStmt(HR_throw, value, &throw_label_); }
  int scope_count() const { return scopep_ + 1 - scope_base_; }
  Def **scopes() const { return &frame_[scope_base_]; }
  Def *&state() { return frame_[state_pos_]; }
  Def *effect() { return frame_[effect_pos_]; }

  /// Get this function's env parameter.
  ///
  Def *env_param() { return cast<StartInstr>(ir_->begin)->data_param(0); }
