 ...
 * A correctly formed list is circular and the end of the list (and printout) is
 * when the next instruction points back at the list head. To prevent debug code
 * from looping when confronted with incorrect lists there is a limit to the
 * number of instructions that will be printed.
 */
...
/// A correctly formed list is circular and the end of the list (and printout)
/// is when the next instruction points back at the list head. To prevent debug
/// code from looping when confronted with incorrect lists there is a limit to
/// the number of instructions that will be printed.
///
...
///
void printAbcCfg(MethodInfo *, Seq<AbcBlock *> *, InstrGraph *,
                 const char *phase);
...
/// Print just the name of the def.  fixme: field is a historical term, change
/// it.
///
...
inline const char *typeName(const Def *d) { return typeName(*d); }
