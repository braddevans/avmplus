...
enum TypeKind {
  ...
  kTypeScriptObject, // any ScriptObject: user classes, final Object, activation
                     // objects
};
 ...
 */
inline bool isConcreteType(TypeKind k) { return !isAbstractType(k); }
 ...
 */
inline char modelDesc(ModelKind model) { return "ASNOID-"[model]; }
...
public:
  ...
  Type(const char *name, TypeKind kind, bool is_nullable, bool is_const)
      : name(name), kind(kind), is_nullable(is_nullable), is_const(is_const) {}
  ...
  virtual bool isConst() const { return is_const != 0; }

  bool isNull() const { return is_const && is_nullable; }
...
protected:
  ...
  friend class DataType; // needed for DataType::makeUnion to call
                         // Type::makeUnion...?
...
inline TypeKind kind(const Type *t) { return t->kind; }

inline bool isNullable(const Type *t) { return t->is_nullable != 0; }

inline bool isConst(const Type *t) { return t->isConst(); }

inline bool isDataType(const Type &t) { return isDataType(kind(&t)); }

/**
 * true if this type is a linear type which cannot be stored or copied.
 */
inline bool isLinear(const Type *t) {
  // todo what about tuple? Is it bad if multiple IR nodes use the same
  // safepoint? fixme: STATE should be linear too, but this prevents identity
  // opts which we count on, for now.
 ...
 */
template <TypeKind TYPE_KIND> class SimpleType : public Type {
protected:
  ...
  const Type *copy(Lattice &) const { return this; }
...
public:
  ...
  bool operator==(const Type &t) const { return t.kind == TYPE_KIND; }

  bool isSubtypeOf(const Type &t) const {
    return t.kind == TYPE_KIND || t.kind == kTypeTop;
  }

  size_t hashCode() const { return TYPE_KIND; }
};
...
inline bool isTop(const Type *t) { return t->kind == kTypeTop; }
...
public:
  ...
  // Note override
  bool isSubtypeOf(const Type &) const { return true; }
};

inline bool isBottom(const Type *t) { return t->kind == kTypeBottom; }
...
inline bool isTopData(const Type *t) { return t->kind == kTypeTopData; }
...
inline bool isEffect(const Type *t) { return t->kind == kTypeEffect; }
...
inline bool isState(const Type *t) { return t->kind == kTypeState; }
...
protected:
  ...
  const Type *makeUnion(const Type &t, Lattice &) const {
    return isSubtypeOf(t) ? &t
                          : t.isSubtypeOf(*this)
                                ? this
                                : t.kind == TYPE_KIND ? &SELF_CLASS::BASE : TOP;
  }
...
public:
  ...
  size_t hashCode() const {
    size_t const_hash =
        is_const ? nanojit::DefaultHash<VALUE_REP>::hash(const_value) : 0;
...
class TraitsType : public VMType<TraitsType, kTypeTraits, Traits *> {
  TraitsType() : VMType<TraitsType, kTypeTraits, Traits *>("<>", true) {}
...
inline bool isTraits(const Type *t) { return t->kind == kTypeTraits; }
...
class OrdinalType : public VMType<OrdinalType, kTypeOrdinal, int> {
  OrdinalType() : VMType<OrdinalType, kTypeOrdinal, int>("#", false) {}
...
inline bool isOrdinal(const Type *t) { return t->kind == kTypeOrdinal; }
...
class NameType : public VMType<NameType, kTypeName, const Multiname *> {
  NameType() : VMType<NameType, kTypeName, const Multiname *>("`*", false) {}
...
inline bool isName(const Type *t) { return t->kind == kTypeName; }
...
class MethodType : public VMType<MethodType, kTypeMethod, MethodInfo *> {
  MethodType() : VMType<MethodType, kTypeMethod, MethodInfo *>("?()", false) {}
...
inline bool isMethod(const Type *t) { return t->kind == kTypeMethod; }
...
class EnvType : public Type {
  ...
  EnvType() : Type("E", kTypeEnv, false, false) { method = NULL; }
  ...
  const Type *makeUnion(const Type &t, Lattice &) const {
    return isSubtypeOf(t)
               ? &t
               : t.isSubtypeOf(*this) ? this : t.kind == KIND ? &BASE : TOP;
  }
...
public: // TODO private to Lattice
  ...
  EnvType(MethodInfo *method)
      : Type(NULL, kTypeEnv, false, false), method(method) {
    AvmAssert(method != NULL);
  }
...
public:
  ...
  bool operator==(const EnvType &et) const { return method == et.method; }
  ...
  MethodInfo *getMethod() const { return method; }
};

inline bool isEnv(const Type *t) { return t->kind == kTypeEnv; }
...
protected:
  DataType(TypeKind kind, ModelKind model, Traits *traits, bool is_nullable,
           bool is_const)
      : Type(NULL, kind, is_nullable, is_const), traits(traits), model(model) {}

  virtual PrintWriter &printConstQual(PrintWriter &console,
                                      AvmCore *core) const;
 ...
 */
inline BuiltinType builtinType(Traits *t) { return Traits::getBuiltinType(t); }
 ...
 * SimpleDataType
 */
template <class SELF_CLASS, TypeKind TYPE_KIND, ModelKind MODEL_KIND,
          typename VALUE_REP>
...
protected:
  ...
  bool isTraitsSubtype(const DataType &t) const {
    ...
    return !ttraits || (traits && traits->subtypeof(ttraits)) ||
  ...
  bool isKindSubtype(const DataType &dt) const {
    return dt.kind == TYPE_KIND || dt.kind == kTypeAny ||
           dt.kind == kTypeObject ||
           (isNull() && dt.is_nullable && isPointer(model) &&
            isPointer(dt.model));
  }

  bool isConstSubtype(const DataType &dt) const {
    return !dt.isConst() || (isConst() && kind == dt.kind &&
...
public:
  ...
  bool operator==(const SELF_CLASS &st) const {
    return traits == st.traits && is_nullable == st.is_nullable &&
           is_const == st.is_const && const_value == st.const_value &&
           model == st.model;
  }
  ...
  bool isSubtypeOf(const DataType &dt) const {
    return isKindSubtype(dt) && isNullableSubtype(dt) && isTraitsSubtype(dt) &&
           isConstSubtype(dt);
  }

  bool isSubtypeOf(const Type &t) const {
    return t.kind == kTypeTop || t.kind == kTypeTopData ||
...
public:
  ...
  ModelKind defaultModelKind() const { return kModelAtom; }
 ...
 */
class ObjectType
    : public SimpleDataType<ObjectType, kTypeObject, kModelAtom, Atom> {
public:
...
public:
  ...
  Atom getAtomConst(AvmCore *) const { return const_value; }
};

/**
 * ScriptObjectType
 */
class ScriptObjectType
    : public SimpleDataType<ScriptObjectType, kTypeScriptObject,
                            kModelScriptObject, ScriptObject *> {
public:
  ...
  ModelKind defaultModelKind() const { return kModelScriptObject; }

  PrintWriter &printName(PrintWriter &console, AvmCore *core) const;

  Atom getAtomConst(AvmCore *) const { return const_value->atom(); }
};
 ...
 * NamespaceType
 */
class NamespaceType : public SimpleDataType<NamespaceType, kTypeNamespace,
                                            kModelNamespace, Namespace *> {
public:
 ...
 */
class StringType
    : public SimpleDataType<StringType, kTypeString, kModelString, String *> {

  bool constEquals(String *val) const { return const_value->equals(val); }
...
public:
  ...
  StringType(ModelKind model, Traits *traits, bool is_nullable, bool is_const,
             String *const_value);
 ...
 */
class BooleanType
    : public SimpleDataType<BooleanType, kTypeBoolean, kModelInt, bool> {
public:
  BooleanType(Lattice &lat, ModelKind model);

  BooleanType(Lattice &lat, bool const_value, ModelKind model);

  BooleanType(ModelKind model, Traits *traits, bool is_nullable, bool is_const,
              bool const_value);
...
inline bool isInt(double d) { return isWhole(d) && int32_t(d) == d; }

inline bool isUInt(double d) { return isWhole(d) && uint32_t(d) == d; }
...
struct Int32Range {
  ...
  size_t hashCode() const { return size_t(is_int) + size_t(is_uint); }
};

struct NumberConstraint {
  enum Kind { kValue, kInt32Range, kAll };
  ...
  NumberConstraint(Int32Range r) : kind(kInt32Range), range(r), value(0) {}
  ...
  bool equals(const NumberConstraint &c) const {
    return kind == c.kind &&
           ((kind == kAll) || (kind == kValue && valuesEqual(value, c.value)) ||
  ...
  static NumberConstraint makeValueUnion(double v1, double v2) {
    return valuesEqual(v1, v2)
               ...
               : (halfmoon::isInt(v1) && halfmoon::isInt(v2))
                     ? NumberConstraint(true, halfmoon::isUInt(v1) &&
                                                  halfmoon::isUInt(v2))
  ...
  static NumberConstraint makeRangeUnion(double v, const Int32Range &r) {
    ...
    return (is_int || is_uint) ? NumberConstraint(is_int, is_uint)
                               : NumberConstraint();
  }

  static NumberConstraint makeRangeUnion(const Int32Range &r1,
                                         const Int32Range &r2) {
  ...
  NumberConstraint makeUnion(const NumberConstraint &c) const {
    ...
    case kValue: {
      switch (c.kind) {
      case kValue:
        return makeValueUnion(value, c.value);
      case kInt32Range:
        return makeRangeUnion(value, c.range);
      default:
        return c;
      }
      break;
    ...
    case kInt32Range: {
      switch (c.kind) {
      case kValue:
        return makeRangeUnion(c.value, range);
      case kInt32Range:
        return makeRangeUnion(range, c.range);
      default:
        return c;
      }
      break;
  ...
  bool isConst() const { return kind == kValue; }

  bool isInt() const {
    return (kind == kValue && halfmoon::isInt(value)) ||
           (kind == kInt32Range && range.is_int);
  }

  bool isUInt() const {
    return (kind == kValue && halfmoon::isUInt(value)) ||
...
public:
  ...
  bool isSubtypeOf(const DataType &dt) const {
    return isKindSubtype(dt) && isTraitsSubtype(dt) && isConstSubtype(dt);
  }

  bool isSubtypeOf(const Type &t) const {
    return t.kind == kTypeTop || t.kind == kTypeTopData ||
           (t.kind == kTypeNumber ? isSubtypeOf((NumberType &)t)
  ...
  bool isConst() const { return constraint.isConst(); }

  bool isInt() const { return constraint.isInt(); }

  bool isUInt() const { return constraint.isUInt(); }
 ...
 */
inline bool isAny(const Type *t) { return kind(t) == kTypeAny; }

inline bool isNumber(const Type *t) { return kind(t) == kTypeNumber; }
...
inline bool isDouble(const Type *t) {
  return (kind(t) == kTypeNumber) && !isInt(t) && !isUInt(t);
}

inline bool isBoolean(const Type *t) { return kind(t) == kTypeBoolean; }

inline bool isString(const Type *t) { return kind(t) == kTypeString; }

inline bool isNamespace(const Type *t) { return kind(t) == kTypeNamespace; }
...
inline bool isInterface(const Type *t) { return getTraits(t)->isInterface(); }
 ...
 */
inline bool isSlot(Binding b) { return AvmCore::isSlotBinding(b) != 0; }

inline bool isVarSlot(Binding b) { return AvmCore::isVarBinding(b) != 0; }

inline bool isConstSlot(Binding b) { return AvmCore::isConstBinding(b) != 0; }

inline bool hasGetter(Binding b) { return AvmCore::hasGetterBinding(b); }

inline bool hasSetter(Binding b) { return AvmCore::hasSetterBinding(b); }

/**
 * Return true if b is a method binding.
 */
inline bool isMethod(Binding b) { return AvmCore::isMethodBinding(b); }
 ...
 */
inline uint32_t toSlot(Binding b) { return AvmCore::bindingToSlotId(b); }

/**
 * Return the disp_id (index in VTable) of the method binding b.
 */
inline uint32_t toMethod(Binding b) { return AvmCore::bindingToMethodId(b); }
...
public:
  ...
  bool operator==(const TypeKey &other) const { return *type_ == *other.type_; }

  bool operator!=(const TypeKey &other) const { return *type_ != *other.type_; }

  static size_t hash(const TypeKey &k) { return k.type_->hashCode(); }
 ...
 */
enum NullabilityKind { kTypeNullable, kTypeNotNull };
...
class Lattice {
  friend class Type;
  typedef HashMap<TypeKey, const Type *, TypeKey, UseOperEqual<TypeKey>>
      TypeTable;
...
public:
  ...
  const NameType *makeNameConst(PoolObject *pool, int index) {
    return (const NameType *)makeType(
        NameType(pool->precomputedMultiname(index)));
  }
  ...
  const NamespaceType *makeNamespaceConst(Namespace *val,
                                          ModelKind m = kModelNamespace);

  /** Create a scriptobject constant. */
  const ScriptObjectType *makeObjectConst(Traits *traits, ScriptObject *val,
                                          ModelKind m = kModelScriptObject);
...
public:
  ...
  bool empty() const { return i > ms->param_count(); }
  ...
  int disp() { return disp_; }

  ModelKind model() const {
    return defaultModelKind(i > ms->param_count() ? (Traits *)0
                                                  : ms->paramTraits(i));
  }
  ...
  String *&asString() const { return *((String **)argPtr()); }

  Namespace *&asNamespace() const { return *((Namespace **)argPtr()); }

  Atom &asAtom() const { return *((Atom *)argPtr()); }

  ScriptObject *&asObject() const { return *((ScriptObject **)argPtr()); }

  uint32_t &asUint() const { return *((uint32_t *)argPtr()); }

  int32_t &asInt() const { return *((int32_t *)argPtr()); }

  intptr_t &asIntptr() const { return *((intptr_t *)argPtr()); }

  double &asDouble() const { return *((double *)argPtr()); }

private:
  char *argPtr() const { return (char *)ap + disp_; }
