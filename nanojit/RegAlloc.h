...
inline Register msReg(uint32_t mask) {
  Register r = {msbSet32(mask)};
  return r;
}
inline Register msReg(uint64_t mask) {
  Register r = {msbSet64(mask)};
  return r;
}

// Return the lowest numbered Register in mask.
inline Register lsReg(uint32_t mask) {
  Register r = {lsbSet32(mask)};
  return r;
}
inline Register lsReg(uint64_t mask) {
  Register r = {lsbSet64(mask)};
  return r;
}
#endif
...
//  active registers & defining instructions is extended to all the
//  subregisters. More specifically:
//
//   * An LIns can appear more than once in 'active'. If it appears in active[r]
//   it must appear in all active[r1] where r1 is a subregister of r; and must
//   not appear in active[r2] where there is no relationship between r and r2.
//
...
public:
  ...
  // Ins: instruction that gets the register; allow - the allowed set of
  // (sub)registers regClass: a hint as to what register is needed - useful when
  // allocating temporary registers, where the instruction itself provides no
  // clue as to what kind of register is desired
  Register allocReg(LIns *ins, RegisterMask allow,
                    Register regClass = UnspecifiedReg);
  ...
  void allocTempReg2(RegisterMask allow1, Register &r1, RegisterMask allow2,
                     Register &r2, Register regClass1 = UnspecifiedReg,
                     Register regClass2 = UnspecifiedReg);
  // Allocates temporary register; see also the implementation notes
  Register allocTempRegIfAvailable(RegisterMask allow,
                                   Register regClass = UnspecifiedReg);
  ...
  debug_only(bool isConsistent(Register r, LIns *v) const;) private
      : LIns
            *_active[LastRegNum + 1]; // active[REGNUM(r)] = LIns that defines r
  int32_t
      _usepri[LastRegNum + 1]; // used priority. lower = more likely to spill.
  ...
  LIns *findVictim(RegisterMask allow, LIns *forIns = NULL,
                   Register regClass = UnspecifiedReg);
...
public:
  ...
  // A special value for the _nHints[] array;  if an opcode has this value, we
  // call nHint() in the back-end.  Used for cases where you need to look at
  // more than just the opcode to decide.
...
/* Here we select how to pick a register from a register set; the platform may
   declare its preference (lsreg, msreg), or may completely implement a cistp,
   method */
...
// returned register will be marked as being free immediately, it is the
// caller's responsibility to make sure that the regstate is not inspected or
// updated while the temp reg is still in use.
...
inline void RegAlloc::allocTempReg2(RegisterMask allow1, Register &r1,
                                    RegisterMask allow2, Register &r2,
                                    Register regClass1, Register regClass2) {
...
// Finds a register in 'allow' to store a temporary value (one not associated
// with a particular LIns). Never evicts instructions. Returns
// UnspecifiedRegister on failure. PARTIAL IMPLEMENTATION. ONLY WORKS ON GPRs.
// ASSUMES GPRs ARE NON-OVERLAPPING.
inline Register RegAlloc::allocTempRegIfAvailable(RegisterMask allow,
                                                  Register regClass) {
  NanoAssert((allow & GpRegs) == allow);
  (void)regClass;
