...
namespace nanojit {
enum LOpcode {
#define OP___(op, repKind, retType, isCse) LIR_##op,
...
#include "LIRopcode.tbl"
#undef OP___
NanoStaticAssert(LIR_start == 0 &&
                 LIR_sentinel <= 256); // It's ok if LIR_sentinel is 256 since
                                       // it's not actually used as opcode.

// 32-bit integer comparisons must be contiguous, as must 64-bit integer
// comparisons and 64-bit float comparisons.
NanoStaticAssert(LIR_eqi + 1 == LIR_lti && LIR_eqi + 2 == LIR_gti &&
                 LIR_eqi + 3 == LIR_lei && LIR_eqi + 4 == LIR_gei &&
                 LIR_eqi + 5 == LIR_ltui && LIR_eqi + 6 == LIR_gtui &&
                 LIR_eqi + 7 == LIR_leui && LIR_eqi + 8 == LIR_geui);
#ifdef NANOJIT_64BIT
NanoStaticAssert(LIR_eqq + 1 == LIR_ltq && LIR_eqq + 2 == LIR_gtq &&
                 LIR_eqq + 3 == LIR_leq && LIR_eqq + 4 == LIR_geq &&
                 LIR_eqq + 5 == LIR_ltuq && LIR_eqq + 6 == LIR_gtuq &&
                 LIR_eqq + 7 == LIR_leuq && LIR_eqq + 8 == LIR_geuq);
#endif
NanoStaticAssert(LIR_eqd + 1 == LIR_ltd && LIR_eqd + 2 == LIR_gtd &&
                 LIR_eqd + 3 == LIR_led && LIR_eqd + 4 == LIR_ged);
NanoStaticAssert(LIR_eqf + 1 == LIR_ltf && LIR_eqf + 2 == LIR_gtf &&
                 LIR_eqf + 3 == LIR_lef && LIR_eqf + 4 == LIR_gef);
...
static AccSet decompressMiniAccSet(MiniAccSet miniAccSet) {
  return (miniAccSet.val == MINI_ACCSET_MULTIPLE.val) ? ACCSET_ALL
                                                      : (1 << miniAccSet.val);
}
...
public:
  uintptr_t _address;
  uint32_t _typesig : 27; // 9 3-bit fields indicating arg type, by ARGTYPE
                          // above (including ret type): a1 a2 a3 a4 a5 ret
  AbiKind _abi : 3;
  uint32_t _isPure : 1; // _isPure=1 means no side-effects, result only depends
                        // on args
  AccSet _storeAccSet;  // access regions stored by the function
  verbose_only(const char *_name;)

      // The following encode 'r func()' through to 'r func(a1, a2, a3, a4, a5,
      // a6, a7, a8)'.
  ...
  static inline uint32_t typeSig3(ArgType r, ArgType a1, ArgType a2,
                                  ArgType a3) {
  ...
  static inline uint32_t typeSig8(ArgType r, ArgType a1, ArgType a2, ArgType a3,
                                  ArgType a4, ArgType a5, ArgType a6,
                                  ArgType a7, ArgType a8) {
  ...
  // Nb: uses right-to-left order, eg. sizes[0] is the size of the right-most
  // arg.
  ...
  inline bool isIndirect() const { return _address < 256; }
};
...
inline bool isLiveOpcode(LOpcode op) {
  return
      ...
      op == LIR_liveq ||
#endif
      op == LIR_livef || op == LIR_livef4 || op == LIR_livei || op == LIR_lived;
}
inline bool isRetOpcode(LOpcode op) {
  return
      ...
      op == LIR_retq ||
#endif
      op == LIR_retf || op == LIR_retf4 || op == LIR_reti || op == LIR_retd;
}
inline bool isCmovOpcode(LOpcode op) {
  return
      ...
      op == LIR_cmovq ||
#endif
      op == LIR_cmovf || op == LIR_cmovf4 || op == LIR_cmovi || op == LIR_cmovd;
}
inline bool isCmpIOpcode(LOpcode op) { return LIR_eqi <= op && op <= LIR_geui; }
inline bool isCmpSIOpcode(LOpcode op) { return LIR_eqi <= op && op <= LIR_gei; }
inline bool isCmpUIOpcode(LOpcode op) {
  return LIR_eqi == op || (LIR_ltui <= op && op <= LIR_geui);
}
#ifdef NANOJIT_64BIT
inline bool isCmpQOpcode(LOpcode op) { return LIR_eqq <= op && op <= LIR_geuq; }
inline bool isCmpSQOpcode(LOpcode op) { return LIR_eqq <= op && op <= LIR_geq; }
inline bool isCmpUQOpcode(LOpcode op) {
  return LIR_eqq == op || (LIR_ltuq <= op && op <= LIR_geuq);
}
#endif
inline bool isCmpDOpcode(LOpcode op) { return LIR_eqd <= op && op <= LIR_ged; }
inline bool isCmpFOpcode(LOpcode op) { return LIR_eqf <= op && op <= LIR_gef; }
inline bool isCmpF4Opcode(LOpcode op) { return LIR_eqf4 == op; }
inline bool isCmpOpcode(LOpcode op) {
  return isCmpIOpcode(op) ||
#if defined NANOJIT_64BIT
         isCmpQOpcode(op) ||
#endif
         isCmpFOpcode(op) || isCmpF4Opcode(op) || isCmpDOpcode(op);
}
...
private:
  ...
  // obfuscate ("blind") tainted values that would be inserted in the
  // instruction stream.  Nanojit doesn't really know anything about which
  // values are tainted, and relies on the generator of the LIR code to set the
  // the taint status of each literal correctly when generating a LIR_immX
  // instruction.
  //
...
public:
  ...
  inline void initLInsOp3(LOpcode opcode, LIns *oprnd1, LIns *oprnd2,
                          LIns *oprnd3);
  inline void initLInsOp4(LOpcode opcode, LIns *oprnd1, LIns *oprnd2,
                          LIns *oprnd3, LIns *oprnd4);
  inline void initLInsLd(LOpcode opcode, LIns *val, int32_t d, AccSet accSet,
                         LoadQual loadQual);
  inline void initLInsSt(LOpcode opcode, LIns *val, LIns *base, int32_t d,
                         AccSet accSet);
  ...
  bool isLive() const {
    return isV() || sharedFields.isResultLive ||
  ...
  // slot) for use?
  bool isExtant() { return isInReg() || isInAr(); }
  bool isInReg() { return sharedFields.inReg; }
  ...
  void clearReg() { sharedFields.inReg = 0; }
  bool isInAr() { return sharedFields.inAr; }
  uint32_t getArIndex() {
    NanoAssert(isInAr());
  ...
  void clearArIndex() { sharedFields.inAr = 0; }
  ...
  bool isTainted() const { return sharedFields.isTainted; }
  void setTainted(bool tainted) { sharedFields.isTainted = tainted; }
  ...
  // For LInsC.
  inline LIns *
  arg(uint32_t i) const; // right-to-left-order: arg(0) is rightmost
  ...
  bool isRet() const { return isRetOpcode(opcode()); }
  bool isCmp() const { return isCmpOpcode(opcode()); }
  bool isCall() const {
    return isop(LIR_callv) || isop(LIR_calli) ||
#if defined NANOJIT_64BIT
           isop(LIR_callq) ||
#endif
           isop(LIR_callf) || isop(LIR_callf4) || isop(LIR_calld);
  }
  bool isCmov() const { return isCmovOpcode(opcode()); }
  bool isStore() const { return isLInsSt(); }
  bool isLoad() const { return isLInsLd(); }
  bool isGuard() const {
  ...
  bool isImmI() const { return isop(LIR_immi); }
  bool isSafe() const { return isop(LIR_safe); }
  bool isEndsafe() const { return isop(LIR_endsafe); }
  // True if the instruction is a 32-bit integer immediate and
  // has the value 'val' when treated as a 32-bit signed integer.
  bool isImmI(int32_t val) const { return isImmI() && immI() == val; }
#ifdef NANOJIT_64BIT
  // True if the instruction is a 64-bit integer immediate.
  bool isImmQ() const { return isop(LIR_immq); }
#endif
  ...
  bool isImmAny() const { return isImmIorF() || isImmQorD() || isImmF4(); }

  bool isConditionalBranch() const {
    return isop(LIR_jt) || isop(LIR_jf) || isop(LIR_brsavpc) || isJov();
  }

  bool isUnConditionalBranch() const { return isop(LIR_j) || isop(LIR_jtbl); }

  bool isBranch() const {
    return isConditionalBranch() || isUnConditionalBranch();
  }

  LTy retType() const { return retTypes[opcode()]; }
  bool isV() const { return retType() == LTy_V; }
  bool isI() const { return retType() == LTy_I; }
#ifdef NANOJIT_64BIT
  bool isQ() const { return retType() == LTy_Q; }
#endif
  bool isD() const { return retType() == LTy_D; }
  bool isF() const { return retType() == LTy_F; }
  bool isF4() const { return retType() == LTy_F4; }
  bool isQorD() const {
    return
#ifdef NANOJIT_64BIT
        isQ() ||
...
LInsOp0 *LIns::toLInsOp0() const {
  return (LInsOp0 *)(uintptr_t(this + 1) - sizeof(LInsOp0));
}
LInsOp1 *LIns::toLInsOp1() const {
  return (LInsOp1 *)(uintptr_t(this + 1) - sizeof(LInsOp1));
}
LInsOp1b *LIns::toLInsOp1b() const {
  return (LInsOp1b *)(uintptr_t(this + 1) - sizeof(LInsOp1b));
}
LInsOp2 *LIns::toLInsOp2() const {
  return (LInsOp2 *)(uintptr_t(this + 1) - sizeof(LInsOp2));
}
LInsOp3 *LIns::toLInsOp3() const {
  return (LInsOp3 *)(uintptr_t(this + 1) - sizeof(LInsOp3));
}
LInsOp4 *LIns::toLInsOp4() const {
  return (LInsOp4 *)(uintptr_t(this + 1) - sizeof(LInsOp4));
}
LInsLd *LIns::toLInsLd() const {
  return (LInsLd *)(uintptr_t(this + 1) - sizeof(LInsLd));
}
LInsSt *LIns::toLInsSt() const {
  return (LInsSt *)(uintptr_t(this + 1) - sizeof(LInsSt));
}
LInsSk *LIns::toLInsSk() const {
  return (LInsSk *)(uintptr_t(this + 1) - sizeof(LInsSk));
}
LInsC *LIns::toLInsC() const {
  return (LInsC *)(uintptr_t(this + 1) - sizeof(LInsC));
}
LInsP *LIns::toLInsP() const {
  return (LInsP *)(uintptr_t(this + 1) - sizeof(LInsP));
}
LInsIorF *LIns::toLInsIorF() const {
  return (LInsIorF *)(uintptr_t(this + 1) - sizeof(LInsIorF));
}
LInsQorD *LIns::toLInsQorD() const {
  return (LInsQorD *)(uintptr_t(this + 1) - sizeof(LInsQorD));
}
LInsJtbl *LIns::toLInsJtbl() const {
  return (LInsJtbl *)(uintptr_t(this + 1) - sizeof(LInsJtbl));
}
LInsF4 *LIns::toLInsF4() const {
  return (LInsF4 *)(uintptr_t(this + 1) - sizeof(LInsF4));
}
LInsSafe *LIns::toLInsSafe() const {
  return (LInsSafe *)(uintptr_t(this + 1) - sizeof(LInsSafe));
}
...
void LIns::initLInsOp3(LOpcode opcode, LIns *oprnd1, LIns *oprnd2,
                       LIns *oprnd3) {
  initSharedFields(opcode);
...
void LIns::initLInsOp4(LOpcode opcode, LIns *oprnd1, LIns *oprnd2, LIns *oprnd3,
                       LIns *oprnd4) {
  initSharedFields(opcode);
  toLInsOp3()->oprnd_1 = oprnd1;
  toLInsOp3()->oprnd_2 = oprnd2;
...
void LIns::initLInsLd(LOpcode opcode, LIns *val, int32_t d, AccSet accSet,
  ...
  NanoAssert((loadQual >= 0) && (int16_t(loadQual) < 4));
  NanoAssert(isLInsLd());
}
void LIns::initLInsSt(LOpcode opcode, LIns *val, LIns *base, int32_t d,
                      AccSet accSet) {
  initSharedFields(opcode);
...
AccSet LIns::accSet() const { return decompressMiniAccSet(miniAccSet()); }
...
inline uint8_t LIns::paramArg() const {
  NanoAssert(isop(LIR_paramp));
  return toLInsP()->arg;
}
inline uint8_t LIns::paramKind() const {
  NanoAssert(isop(LIR_paramp));
  return toLInsP()->kind;
}

inline int32_t LIns::immI() const {
  NanoAssert(isImmI());
  return toLInsIorF()->immIorF;
}
inline int32_t LIns::immFasI() const {
  NanoAssert(isImmF());
  return toLInsIorF()->immIorF;
}
...
#ifdef NANOJIT_64BIT
inline int32_t LIns::immQlo() const {
  NanoAssert(isImmQ());
  return toLInsQorD()->immQorDlo;
}
uint64_t LIns::immQ() const {
  NanoAssert(isImmQ());
  return (uint64_t(toLInsQorD()->immQorDhi) << 32) |
         uint32_t(toLInsQorD()->immQorDlo);
}
#endif
inline int32_t LIns::immDlo() const {
  NanoAssert(isImmD());
  return toLInsQorD()->immQorDlo;
}
inline int32_t LIns::immDhi() const {
  NanoAssert(isImmD());
  return toLInsQorD()->immQorDhi;
}
...
uint32_t LIns::argc() const { return callInfo()->count_args(); }

LIns *LIns::callArgN(uint32_t n) const { return arg(argc() - n - 1); }
...
public:
  ...
  LirWriter(LirWriter *out) : out(out) {}
  virtual ~LirWriter() {}

  virtual LIns *ins0(LOpcode v) { return out->ins0(v); }
  virtual LIns *ins1(LOpcode v, LIns *a) { return out->ins1(v, a); }
  virtual LIns *ins2(LOpcode v, LIns *a, LIns *b) { return out->ins2(v, a, b); }
  ...
  virtual LIns *insImmF4(const float4_t &f4, bool tainted) {
    return out->insImmF4(f4, tainted);
  }
  ...
  virtual LIns *insComment(const char *str) { return out->insComment(str); }
  virtual LIns *insSkip(LIns *skipTo) { return out->insSkip(skipTo); }
  virtual LIns *insSwz(LIns *a, uint8_t mask) { return out->insSwz(a, mask); }

  // convenience functions

  LIns *insImmI(int32_t imm) { return insImmI(imm, false); }
#ifdef NANOJIT_64BIT
  LIns *insImmQ(uint64_t imm) { return insImmQ(imm, false); }
#endif
  LIns *insImmF(float f) { return insImmF(f, false); }
  LIns *insImmF4(const float4_t &f4) { return insImmF4(f4, false); }
  LIns *insImmD(double d) { return insImmD(d, false); }
  ...
  LIns *insEqI_0(LIns *oprnd1) { return ins2ImmI(LIR_eqi, oprnd1, 0); }

  // Inserts a pointer comparison to 0
  LIns *insEqP_0(LIns *oprnd1) { return ins2(LIR_eqp, oprnd1, insImmWord(0)); }
  ...
  LIns *insImmP(const void *ptr, bool tainted) {
#ifdef NANOJIT_64BIT
    return insImmQ((uint64_t)ptr, tainted);
#else
    return insImmI((int32_t)ptr, tainted);
#endif
  }
...
#ifdef _MSC_VER
#pragma warning(disable : 4996) // disable "strcpy is unsafe" warnings in Visual
                                // Studio, they break the build
#endif
...
private:
  ...
  struct Str {
    ...
    bool operator!=(const Str &str) const { return !(*this == str); }
  ...
  void addNameWithSuffix(LIns *i, const char *s, int suffix,
...
public:
  LirNameMap(Allocator &alloc)
      : alloc(alloc), lircounts(alloc), funccounts(alloc), namecounts(alloc),
        names(alloc) {}
...
private:
  ...
  const char *formatTaint(LIns *ref);
...
public:
  VerboseWriter(Allocator &alloc, LirWriter *out, LInsPrinter *printer,
                LogControl *logc, const char *prefix = "",
                bool always_flush = false)
      : LirWriter(out), code(alloc), printer(printer), logc(logc),
        prefix(prefix), always_flush(always_flush), never_flush(false) {}
  ...
  LIns *ins2(LOpcode v, LIns *a, LIns *b) { return add(out->ins2(v, a, b)); }
  ...
  LIns *insAlloc(int32_t size) { return add(out->insAlloc(size)); }
  LIns *insImmI(int32_t imm, bool tainted) {
    return add(out->insImmI(imm, tainted));
  }
  LIns *insSafe(LOpcode op, void *payload) {
    return add(out->insSafe(op, payload));
  }
#ifdef NANOJIT_64BIT
  LIns *insImmQ(uint64_t imm, bool tainted) {
    return add(out->insImmQ(imm, tainted));
  }
#endif
  LIns *insImmF(float f, bool tainted) { return add(out->insImmF(f, tainted)); }
  LIns *insImmF4(const float4_t &f4, bool tainted) {
    return add(out->insImmF4(f4, tainted));
  }
  LIns *insImmD(double d, bool tainted) {
    return add(out->insImmD(d, tainted));
  }
  LIns *insComment(const char *str) { return add_flush(out->insComment(str)); }
  LIns *insSwz(LIns *a, uint8_t mask) { return add(out->insSwz(a, mask)); }
};
#endif /* NJ_VERBOSE */
...
class CseFilter : public LirWriter {
  enum NLKind {
    ...
    NLImmF = 9,
    NLImmF4 = 10,
    // Need a value after "last" to outsmart compilers that insist last+1 is
    // impossible.
    NLInvalid,
    NLFirst = 0,
    NLLast = NLInvalid - 1,
...
#define nextNLKind(kind) NLKind(kind + 1)
  ...
  //     Don't start m_capNL too small, or we'll waste time growing and
  //     rehashing. Don't start m_capNL too large, will waste memory.
  //
  ...
  const uint8_t
      EMB_NUM_USED_ACCS; // number of access regions used by the embedding
  ...
  CseAcc miniAccSetToCseAcc(MiniAccSet miniAccSet, LoadQual loadQual) {
    NanoAssert(miniAccSet.val < NUM_ACCS ||
               miniAccSet.val == MINI_ACCSET_MULTIPLE.val);
    return (loadQual == LOAD_CONST)
               ? CSE_ACC_CONST
  ...
  static uint32_t
      hashImmQorD(uint64_t); // not NANOJIT_64BIT-only -- used by findImmD()
  ...
  LIns *findImmISmall(int32_t a, uint32_t &k, bool tainted);
  LIns *findImmILarge(int32_t a, uint32_t &k, bool tainted);
#ifdef NANOJIT_64BIT
  LIns *findImmQ(uint64_t a, uint32_t &k, bool tainted);
#endif
  LIns *findImmF(int32_t f, uint32_t &k, bool tainted);
  LIns *findImmF4(const float4_t &f, uint32_t &k, bool tainted);
  LIns *findImmD(uint64_t d, uint32_t &k, bool tainted);
  ...
  LIns *findLoad(LOpcode v, LIns *a, int32_t b, MiniAccSet miniAccSet,
                 LoadQual loadQual, uint32_t &k);
  LIns *findCall(const CallInfo *call, uint32_t argc, LIns *args[],
                 uint32_t &k);
...
public:
  ...
  LIns *insSafe(LOpcode op, void *payload);
#ifdef NANOJIT_64BIT
  LIns *insImmQ(uint64_t q, bool tainted);
#endif
  LIns *insImmF(float f, bool tainted);
  LIns *insImmF4(const float4_t &f, bool tainted);
  LIns *insImmD(double d, bool tainted);
  LIns *ins0(LOpcode v);
...
public:
  ...
  LIns *savedRegs[NumSavedRegs +
                  1]; // Allocate an extra element in case NumSavedRegs == 0
...
public:
  ...
  virtual LIns *read() { return in->read(); }
  virtual LIns *finalIns() { return in->finalIns(); }
};

// concrete
class LirReader : public LirFilter {
  LIns *_ins;      // next instruction to be read;  invariant: is never a skip
  LIns *_finalIns; // final instruction in the stream;  ie. the first one to be
                   // read
...
#undef OP___
  ...
  LIns *finalIns() { return _finalIns; }
};

verbose_only(void live(LirFilter *in, Allocator &alloc, Fragment *frag,
                       LogControl *);)
...
private:
  ...
  void errorStructureShouldBe(LOpcode op, const char *argDesc, int argN,
  ...
  void checkAccSet(LOpcode op, LIns *base, int32_t disp,
                   AccSet accSet); // defined by the embedder
...
public:
  ReverseLister(LirFilter *in, Allocator &alloc, LInsPrinter *printer,
                LogControl *logc, const char *title)
      : LirFilter(in), _alloc(alloc), _printer(printer), _title(title),
        _strs(alloc), _logc(logc), _prevIns(NULL) {}
 ...
 * Algortihm:
 *
 *  During the first pass (i.e. CfgLister::read()), we capture edges and
 * vertices at an instruction level.  Recall that this is during a backwards
 * pass, so for forward branches we won't know whether an instruction we've seen
 * already is the target of a branch or not, until we reach the branch.  I.e. we
 * see the target instructions first and don't yet know they are targets.
 *
 *  On the 2nd pass (i.e. printGmlCfg) we use the list of branch targets from
 * the first pass (i.e. _vertices) and group instructions into them.  This is
 * where _alt comes into play, it provides a map from any instruction to the
 * first instruction of the containing block.
 */
...
private:
  ...
  void printNode(FILE *f, InsList *nodeIns, LInsPrinter *printer,
                 InsSet *pseudo);

  void gmlNode(FILE *f, uint32_t id, const char *shape, const char *title);
  void gmlEdge(FILE *f, uint32_t srcId, uint32_t dstId, const char *style,
               const char *fill, const char *width, const char *text);
  ...
  Allocator &_alloc;
  HashMap<LIns *, LIns *>
      _alt; // allow edge src/dst to be re-mapped to another instruction
