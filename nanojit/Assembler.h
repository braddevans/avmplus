 ...
 *
 *   The Assembler class parses the LIR instructions starting at any point and
 * converts them to machine code.  It does the translation using expression
 * trees which are simply LIR instructions in the stream that have side-effects.
 * Any other instruction in the stream is simply ignored. This approach is
 * interesting in that dead code elimination occurs for 'free', strength
 *   reduction occurs fairly naturally, along with some other optimizations.
 *
 *   A negative is that we require state as we 'push' and 'pop' nodes along the
 * tree. Also, this is most easily performed using recursion which may not be
 * desirable in the mobile environment.
 *
 */
...
private:
  ...
  LIns *_entries[NJ_MAX_STACK_ENTRY]; /* maps to 4B contiguous locations
                                         relative to the frame pointer. NB:
                                         _entries[0] is always unused */
...
public:
  ...
  uint32_t
  reserveEntry(LIns *ins); /* return 0 if unable to reserve the entry */
  ...
  void validate();
  bool
  isValidEntry(uint32_t idx,
               LIns *ins) const; /* return true iff idx and ins are matched */
  ...
  private:
    const AR &_ar;
    // '_i' points to the start of the entries for an LIns, or to the first NULL
    // entry.
    uint32_t _i;

  public:
    inline Iter(const AR &ar) : _ar(ar), _i(1) {}
    bool next(LIns *&ins, uint32_t &nStackSlots,
              int32_t &offset); // get the next one (moves iterator forward)
  };
};
...
public:
  ...
  operator float4_t() const {
    float4_t f4value = {_val[0], _val[1], _val[2], _val[3]};
    return f4value;
  }
  bool keyEquals(const float4_key &other) const {
    return VMPI_memcmp(_val, other._val, sizeof(_val)) == 0;
  }
};
 ...
 */
class Assembler {
  friend class RegAlloc; // allow allocReg to evict instructions; allow init
                         // functions to access config settings
...
public:
  ...
  Assembler(CodeAlloc &codeAlloc, Allocator &dataAlloc, Allocator &alloc,
            LogControl *logc, const Config &config,
            MetaDataWriter *mdWriter = NULL);

  void compile(Fragment *frag, Allocator &alloc,
  ...
  void setNoiseGenerator(Noise *noise) {
    _noise = noise;
  } // used for attack mitigation; setting to 0 disables all mitigations
...
private:
  ...
  Register getBaseRegWithBlinding(LIns *base, int &d, RegisterMask allow,
                                  bool tainted, bool force, Register *rbp);
  Register getBaseRegWithBlindingUsingSpecifiedTemp(LIns *base, int &d,
                                                    RegisterMask allow,
                                                    bool tainted, bool force,
                                                    Register *rbp,
                                                    Register temp);
  void getBaseReg2WithBlinding(RegisterMask allowValue, LIns *value,
                               Register &rv, RegisterMask allowBase, LIns *base,
                               Register &rb, int &d, bool tainted, bool force,
                               Register *rbp);
  ...
  void codeAlloc(NIns *&start, NIns *&end,
                 NIns *&eip verbose_only(, size_t &nBytes),
                 size_t byteLimit = 0);

  bool deprecated_isKnownReg(Register r) { return r != deprecated_UnknownReg; }
...
#ifdef NJ_VERBOSE
  NIns *_nInsAfter; // next instruction (ascending) in current normal/exit code
                    // chunk (for verbose output)
...
#endif
#endif
  verbose_only(void asm_inc_m32(uint32_t *);) void asm_mmq(Register rd, int dd,
                                                           Register rs, int ds);
...
public:
  ...
  virtual void safepointStart(Assembler *assm, void *payload,
                              uint8_t *address) = 0;

  // Report assembly of LIR_endsafe, providing the current code address.
  virtual void safepointEnd(Assembler *assm, void *payload,
                            uint8_t *address) = 0;
