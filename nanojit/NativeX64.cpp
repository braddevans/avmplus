...
const char *gpRegNames32[] = {"eax",  "ecx",  "edx",  "ebx", "esp",  "ebp",
                              "esi",  "edi",  "r8d",  "r9d", "r10d", "r11d",
                              "r12d", "r13d", "r14d", "r15d"};

const char *gpRegNames8[] = {"al",   "cl",   "dl",   "bl",  "spl",  "bpl",
                             "sil",  "dil",  "r8l",  "r9l", "r10l", "r11l",
                             "r12l", "r13l", "r14l", "r15l"};

const char *gpRegNames8hi[] = {"ah", "ch", "dh", "bh"};

const char *gpRegNames16[] = {"ax",   "cx",   "dx",   "bx",  "spx",  "bpx",
                              "six",  "dix",  "r8x",  "r9x", "r10x", "r11x",
                              "r12x", "r13x", "r14x", "r15x"};

#ifdef _DEBUG
#define TODO(x) todo(#x)
static void todo(const char *s) {
  verbose_only(avmplus::AvmLog("%s", s);) NanoAssertMsgf(false, "%s", s);
}
#else
#define TODO(x)
#endif
...
// mode 00 with base == x101 means RIP+disp32 (RBP or R13), use mode 01 disp8=0
// instead mode 01 or 11 with base = x101 means disp32 + EBP or R13, not RIP
// relative base == x100 means SIB byte is present, so using ESP|R12 as base
// requires SIB rex prefix required to use RSP-R15 as 8bit registers in mod/rm8
// modes.

// take R12 out of play as a base register because using ESP or R12 as base
// requires the SIB byte
const RegisterMask BaseRegs = GpRegs & ~rmask(R12);

static inline int oplen(uint64_t op) { return op & 255; }
...
static inline uint64_t rexrb(uint64_t op, Register r, Register b) {
  ...
  uint64_t rex =
      ((op >> shift) & 255) | ((REGNUM(r) & 8) >> 1) | ((REGNUM(b) & 8) >> 3);
...
static inline uint64_t rexrxb(uint64_t op, Register r, Register x, Register b) {
  int shift = 64 - 8 * oplen(op);
  uint64_t rex = ((op >> shift) & 255) | ((REGNUM(r) & 8) >> 1) |
                 ((REGNUM(x) & 8) >> 2) | ((REGNUM(b) & 8) >> 3);
  return rex != 0x40 ? op | rex << shift : op - 1;
}
...
static inline uint64_t rexprb(uint64_t op, Register r, Register b) {
  ...
  return rex != 0x40
             ? op | rex << shift
             : ((op & ~(255LL << shift)) | (op >> (shift - 8) & 255) << shift) -
                   1;
}
...
static inline uint64_t mod_rxb(uint64_t op, Register r, Register x,
                               Register b) {
  return op | /*modrm*/ uint64_t((REGNUM(r) & 7) << 3) << 48 |
         /*sib*/ uint64_t((REGNUM(x) & 7) << 3 | (REGNUM(b) & 7)) << 56;
}
...
static inline uint64_t mod_disp32(uint64_t op, Register r, Register b,
  ...
  uint64_t mod =
      (((op >> 24) & 255) >> 6); // mod bits in addressing mode: 0,1,2, or 3
  if (mod == 2 && isS8(d)) {
    // op is:  0x[disp32=0][mod=2:r:b][op][rex][len]
    int len = oplen(op);
    op = (op & ~0xff000000LL) | (0x40 | (REGNUM(r) & 7) << 3 | (REGNUM(b) & 7))
                                    << 24;          // replace mod
    return op << 24 | int64_t(d) << 56 | (len - 3); // shrink disp, add disp8
  } else {
    // op is: 0x[disp32][mod][op][rex][len]
    return op | int64_t(d) << 32 |
           uint64_t((REGNUM(r) & 7) << 3 | (REGNUM(b) & 7)) << 24;
  }
}
...
// disp32 modrm8 form, when the disp fits in the instruction (opcode is 1-3
// bytes)
void Assembler::emitrm8(uint64_t op, Register r, int32_t d, Register b) {
  emit(rexrb8(mod_disp32(op, r, b, d), r, b));
}

// disp32 modrm form, when the disp fits in the instruction (opcode is 1-3
// bytes)
void Assembler::emitrm(uint64_t op, Register r, int32_t d, Register b) {
  emit(rexrb(mod_disp32(op, r, b, d), r, b));
}

// disp32 modrm form when the disp must be written separately (opcode is 4+
// bytes)
#define emit_disp32_sib(o, d)                                                  \
  (emit_disp32(((o)&0x00FFFFFFFFFFFFFFLL) << 8, d) >> 8) |                     \
      ((o)&0xFF00000000000000LL)
uint64_t Assembler::emit_disp32(uint64_t op, int32_t d) {
  if (isS8(d)) {
    NanoAssert(((op >> 56) & 0xC0) ==
               0x80); // make sure mod bits == 2 == disp32 mode
    underrunProtect(1 + 8);
    *(--_nIns) = (NIns)d;
    _nvprof("x64-bytes", 1);
...
// disp32 modrm form when the disp must be written separately (opcode is 4+
// bytes) p = prefix -- opcode must have a 66, F2, or F3 prefix
...
void Assembler::emitrxb_imm(uint64_t op, Register r, Register x, Register b,
                            int32_t imm) {
...
void Assembler::emitr_imm8(uint64_t op, Register b, int32_t imm8) {
  ...
  op |= uint64_t(imm8) << 56 | uint64_t(REGNUM(b) & 7)
                                   << 48; // modrm is 2nd to last byte
...
void Assembler::emitxm_abs(uint64_t op, Register r, int32_t addr32) {
  ...
  *((int32_t *)(_nIns -= 4)) = addr32;
  _nvprof("x64-bytes", 4);
  op = op | uint64_t((REGNUM(r) & 7) << 3)
                << 48;      // put rr[0:2] into mod/rm byte
...
bool Assembler::isTargetWithinS32(NIns *target, int32_t maxInstSize) {
  NanoAssert(target);
  // some instructions with S32 offsets take more than 8 bytes (e.g. packed
  // float loads like movaps/movups)
...
void Assembler::PUSHR(R r) {
  emitr(X64_pushr, r);
  asm_output("push %s", RQ(r));
}
void Assembler::POPR(R r) {
  emitr(X64_popr, r);
  asm_output("pop %s", RQ(r));
}
void Assembler::NOT(R r) {
  emitr(X64_not, r);
  asm_output("notl %s", RL(r));
}
void Assembler::NEG(R r) {
  emitr(X64_neg, r);
  asm_output("negl %s", RL(r));
}
void Assembler::IDIV(R r) {
  emitr(X64_idiv, r);
  asm_output("idivl edx:eax, %s", RL(r));
}

void Assembler::SHR(R r) {
  emitr(X64_shr, r);
  asm_output("shrl %s, ecx", RL(r));
}
void Assembler::SAR(R r) {
  emitr(X64_sar, r);
  asm_output("sarl %s, ecx", RL(r));
}
void Assembler::SHL(R r) {
  emitr(X64_shl, r);
  asm_output("shll %s, ecx", RL(r));
}
void Assembler::SHRQ(R r) {
  emitr(X64_shrq, r);
  asm_output("shrq %s, ecx", RQ(r));
}
void Assembler::SARQ(R r) {
  emitr(X64_sarq, r);
  asm_output("sarq %s, ecx", RQ(r));
}
void Assembler::SHLQ(R r) {
  emitr(X64_shlq, r);
  asm_output("shlq %s, ecx", RQ(r));
}

void Assembler::SHRI(R r, I i) {
  emit8(rexrb(X64_shri | U64(REGNUM(r) & 7) << 48, RZero, r), i);
  asm_output("shrl %s, %d", RL(r), i);
}
void Assembler::SARI(R r, I i) {
  emit8(rexrb(X64_sari | U64(REGNUM(r) & 7) << 48, RZero, r), i);
  asm_output("sarl %s, %d", RL(r), i);
}
void Assembler::SHLI(R r, I i) {
  emit8(rexrb(X64_shli | U64(REGNUM(r) & 7) << 48, RZero, r), i);
  asm_output("shll %s, %d", RL(r), i);
}
void Assembler::SHRQI(R r, I i) {
  emit8(rexrb(X64_shrqi | U64(REGNUM(r) & 7) << 48, RZero, r), i);
  asm_output("shrq %s, %d", RQ(r), i);
}
void Assembler::SARQI(R r, I i) {
  emit8(rexrb(X64_sarqi | U64(REGNUM(r) & 7) << 48, RZero, r), i);
  asm_output("sarq %s, %d", RQ(r), i);
}
void Assembler::SHLQI(R r, I i) {
  emit8(rexrb(X64_shlqi | U64(REGNUM(r) & 7) << 48, RZero, r), i);
  asm_output("shlq %s, %d", RQ(r), i);
}

void Assembler::SETE(R r) {
  emitr8(X64_sete, r);
  asm_output("sete %s", RB(r));
}
void Assembler::SETL(R r) {
  emitr8(X64_setl, r);
  asm_output("setl %s", RB(r));
}
void Assembler::SETLE(R r) {
  emitr8(X64_setle, r);
  asm_output("setle %s", RB(r));
}
void Assembler::SETG(R r) {
  emitr8(X64_setg, r);
  asm_output("setg %s", RB(r));
}
void Assembler::SETGE(R r) {
  emitr8(X64_setge, r);
  asm_output("setge %s", RB(r));
}
void Assembler::SETB(R r) {
  emitr8(X64_setb, r);
  asm_output("setb %s", RB(r));
}
void Assembler::SETBE(R r) {
  emitr8(X64_setbe, r);
  asm_output("setbe %s", RB(r));
}
void Assembler::SETA(R r) {
  emitr8(X64_seta, r);
  asm_output("seta %s", RB(r));
}
void Assembler::SETAE(R r) {
  emitr8(X64_setae, r);
  asm_output("setae %s", RB(r));
}
void Assembler::SETO(R r) {
  emitr8(X64_seto, r);
  asm_output("seto %s", RB(r));
}

void Assembler::ADDRR(R l, R r) {
  emitrr(X64_addrr, l, r);
  asm_output("addl %s, %s", RL(l), RL(r));
}
void Assembler::SUBRR(R l, R r) {
  emitrr(X64_subrr, l, r);
  asm_output("subl %s, %s", RL(l), RL(r));
}
void Assembler::ANDRR(R l, R r) {
  emitrr(X64_andrr, l, r);
  asm_output("andl %s, %s", RL(l), RL(r));
}
void Assembler::ORLRR(R l, R r) {
  emitrr(X64_orlrr, l, r);
  asm_output("orl %s, %s", RL(l), RL(r));
}
void Assembler::XORRR(R l, R r) {
  emitrr(X64_xorrr, l, r);
  asm_output("xorl %s, %s", RL(l), RL(r));
}
void Assembler::IMUL(R l, R r) {
  emitrr(X64_imul, l, r);
  asm_output("imull %s, %s", RL(l), RL(r));
}
void Assembler::CMPLR(R l, R r) {
  emitrr(X64_cmplr, l, r);
  asm_output("cmpl %s, %s", RL(l), RL(r));
}
void Assembler::MOVLR(R l, R r) {
  emitrr(X64_movlr, l, r);
  asm_output("movl %s, %s", RL(l), RL(r));
}

void Assembler::ADDQRR(R l, R r) {
  emitrr(X64_addqrr, l, r);
  asm_output("addq %s, %s", RQ(l), RQ(r));
}
void Assembler::SUBQRR(R l, R r) {
  emitrr(X64_subqrr, l, r);
  asm_output("subq %s, %s", RQ(l), RQ(r));
}
void Assembler::ANDQRR(R l, R r) {
  emitrr(X64_andqrr, l, r);
  asm_output("andq %s, %s", RQ(l), RQ(r));
}
void Assembler::ORQRR(R l, R r) {
  emitrr(X64_orqrr, l, r);
  asm_output("orq %s, %s", RQ(l), RQ(r));
}
void Assembler::XORQRR(R l, R r) {
  emitrr(X64_xorqrr, l, r);
  asm_output("xorq %s, %s", RQ(l), RQ(r));
}
void Assembler::CMPQR(R l, R r) {
  emitrr(X64_cmpqr, l, r);
  asm_output("cmpq %s, %s", RQ(l), RQ(r));
}
void Assembler::MOVQR(R l, R r) {
  emitrr(X64_movqr, l, r);
  asm_output("movq %s, %s", RQ(l), RQ(r));
}
void Assembler::MOVAPSR(R l, R r) {
  emitrr(X64_movapsr, l, r);
  asm_output("movaps %s, %s", RQ(l), RQ(r));
}
void Assembler::UNPCKLPS(R l, R r) {
  emitrr(X64_unpcklps, l, r);
  asm_output("unpcklps %s, %s", RQ(l), RQ(r));
}

void Assembler::CMOVNO(R l, R r) {
  emitrr(X64_cmovno, l, r);
  asm_output("cmovlno %s, %s", RL(l), RL(r));
}
void Assembler::CMOVNE(R l, R r) {
  emitrr(X64_cmovne, l, r);
  asm_output("cmovlne %s, %s", RL(l), RL(r));
}
void Assembler::CMOVNL(R l, R r) {
  emitrr(X64_cmovnl, l, r);
  asm_output("cmovlnl %s, %s", RL(l), RL(r));
}
void Assembler::CMOVNLE(R l, R r) {
  emitrr(X64_cmovnle, l, r);
  asm_output("cmovlnle %s, %s", RL(l), RL(r));
}
void Assembler::CMOVNG(R l, R r) {
  emitrr(X64_cmovng, l, r);
  asm_output("cmovlng %s, %s", RL(l), RL(r));
}
void Assembler::CMOVNGE(R l, R r) {
  emitrr(X64_cmovnge, l, r);
  asm_output("cmovlnge %s, %s", RL(l), RL(r));
}
void Assembler::CMOVNB(R l, R r) {
  emitrr(X64_cmovnb, l, r);
  asm_output("cmovlnb %s, %s", RL(l), RL(r));
}
void Assembler::CMOVNBE(R l, R r) {
  emitrr(X64_cmovnbe, l, r);
  asm_output("cmovlnbe %s, %s", RL(l), RL(r));
}
void Assembler::CMOVNA(R l, R r) {
  emitrr(X64_cmovna, l, r);
  asm_output("cmovlna %s, %s", RL(l), RL(r));
}
void Assembler::CMOVNAE(R l, R r) {
  emitrr(X64_cmovnae, l, r);
  asm_output("cmovlnae %s, %s", RL(l), RL(r));
}

void Assembler::CMOVQNO(R l, R r) {
  emitrr(X64_cmovqno, l, r);
  asm_output("cmovqno %s, %s", RQ(l), RQ(r));
}
void Assembler::CMOVQNE(R l, R r) {
  emitrr(X64_cmovqne, l, r);
  asm_output("cmovqne %s, %s", RQ(l), RQ(r));
}
void Assembler::CMOVQNL(R l, R r) {
  emitrr(X64_cmovqnl, l, r);
  asm_output("cmovqnl %s, %s", RQ(l), RQ(r));
}
void Assembler::CMOVQNLE(R l, R r) {
  emitrr(X64_cmovqnle, l, r);
  asm_output("cmovqnle %s, %s", RQ(l), RQ(r));
}
void Assembler::CMOVQNG(R l, R r) {
  emitrr(X64_cmovqng, l, r);
  asm_output("cmovqng %s, %s", RQ(l), RQ(r));
}
void Assembler::CMOVQNGE(R l, R r) {
  emitrr(X64_cmovqnge, l, r);
  asm_output("cmovqnge %s, %s", RQ(l), RQ(r));
}
void Assembler::CMOVQNB(R l, R r) {
  emitrr(X64_cmovqnb, l, r);
  asm_output("cmovqnb %s, %s", RQ(l), RQ(r));
}
void Assembler::CMOVQNBE(R l, R r) {
  emitrr(X64_cmovqnbe, l, r);
  asm_output("cmovqnbe %s, %s", RQ(l), RQ(r));
}
void Assembler::CMOVQNA(R l, R r) {
  emitrr(X64_cmovqna, l, r);
  asm_output("cmovqna %s, %s", RQ(l), RQ(r));
}
void Assembler::CMOVQNAE(R l, R r) {
  emitrr(X64_cmovqnae, l, r);
  asm_output("cmovqnae %s, %s", RQ(l), RQ(r));
}

void Assembler::MOVSXDR(R l, R r) {
  emitrr(X64_movsxdr, l, r);
  asm_output("movsxd %s, %s", RQ(l), RL(r));
}

void Assembler::MOVZX8(R l, R r) {
  emitrr8(X64_movzx8, l, r);
  asm_output("movzx %s, %s", RQ(l), RB(r));
}
...
// Also note that (unlike most SSE2 instructions) XORPS does not have a prefix,
// thus emitrr() should be used.
void Assembler::XORPS(R r) {
  emitrr(X64_xorps, r, r);
  asm_output("xorps %s, %s", RQ(r), RQ(r));
}
void Assembler::XORPS(R l, R r) {
  emitrr(X64_xorps, l, r);
  asm_output("xorps %s, %s", RQ(l), RQ(r));
}
void Assembler::DIVSD(R l, R r) {
  emitprr(X64_divsd, l, r);
  asm_output("divsd %s, %s", RQ(l), RQ(r));
}
void Assembler::MULSD(R l, R r) {
  emitprr(X64_mulsd, l, r);
  asm_output("mulsd %s, %s", RQ(l), RQ(r));
}
void Assembler::ADDSD(R l, R r) {
  emitprr(X64_addsd, l, r);
  asm_output("addsd %s, %s", RQ(l), RQ(r));
}
void Assembler::SUBSD(R l, R r) {
  emitprr(X64_subsd, l, r);
  asm_output("subsd %s, %s", RQ(l), RQ(r));
}
void Assembler::DIVSS(R l, R r) {
  emitprr(X64_divss, l, r);
  asm_output("divss %s, %s", RQ(l), RQ(r));
}
void Assembler::MULSS(R l, R r) {
  emitprr(X64_mulss, l, r);
  asm_output("mulss %s, %s", RQ(l), RQ(r));
}
void Assembler::ADDSS(R l, R r) {
  emitprr(X64_addss, l, r);
  asm_output("addss %s, %s", RQ(l), RQ(r));
}
void Assembler::SUBSS(R l, R r) {
  emitprr(X64_subss, l, r);
  asm_output("subss %s, %s", RQ(l), RQ(r));
}
void Assembler::DIVPS(R l, R r) {
  emitrr(X64_divps, l, r);
  asm_output("divps %s, %s", RQ(l), RQ(r));
}
void Assembler::MULPS(R l, R r) {
  emitrr(X64_mulps, l, r);
  asm_output("mulps %s, %s", RQ(l), RQ(r));
}
void Assembler::ADDPS(R l, R r) {
  emitrr(X64_addps, l, r);
  asm_output("addps %s, %s", RQ(l), RQ(r));
}
void Assembler::SUBPS(R l, R r) {
  emitrr(X64_subps, l, r);
  asm_output("subps %s, %s", RQ(l), RQ(r));
}
void Assembler::CVTSQ2SD(R l, R r) {
  emitprr(X64_cvtsq2sd, l, r);
  asm_output("cvtsq2sd %s, %s", RQ(l), RQ(r));
}
void Assembler::CVTSQ2SS(R l, R r) {
  emitprr(X64_cvtsq2ss, l, r);
  asm_output("cvtsq2ss %s, %s", RQ(l), RQ(r));
}
void Assembler::CVTSI2SD(R l, R r) {
  emitprr(X64_cvtsi2sd, l, r);
  asm_output("cvtsi2sd %s, %s", RQ(l), RL(r));
}
void Assembler::CVTSI2SS(R l, R r) {
  emitprr(X64_cvtsi2ss, l, r);
  asm_output("cvtsi2ss %s, %s", RQ(l), RL(r));
}
void Assembler::CVTSS2SD(R l, R r) {
  emitprr(X64_cvtss2sd, l, r);
  asm_output("cvtss2sd %s, %s", RQ(l), RL(r));
}
void Assembler::CVTSD2SS(R l, R r) {
  emitprr(X64_cvtsd2ss, l, r);
  asm_output("cvtsd2ss %s, %s", RQ(l), RQ(r));
}
void Assembler::CVTSD2SI(R l, R r) {
  emitprr(X64_cvtsd2si, l, r);
  asm_output("cvtsd2si %s, %s", RL(l), RQ(r));
}
void Assembler::CVTTSS2SI(R l, R r) {
  emitprr(X64_cvttss2si, l, r);
  asm_output("cvttss2si %s, %s", RL(l), RQ(r));
}
void Assembler::CVTTSD2SI(R l, R r) {
  emitprr(X64_cvttsd2si, l, r);
  asm_output("cvttsd2si %s, %s", RL(l), RQ(r));
}
void Assembler::UCOMISS(R l, R r) {
  emitrr(X64_ucomiss, l, r);
  asm_output("ucomiss %s, %s", RQ(l), RQ(r));
}
void Assembler::UCOMISD(R l, R r) {
  emitprr(X64_ucomisd, l, r);
  asm_output("ucomisd %s, %s", RQ(l), RQ(r));
}
void Assembler::MOVQRX(R l, R r) {
  emitprr(X64_movqrx, r, l);
  asm_output("movq %s, %s", RQ(l), RQ(r));
} // Nb: r and l are deliberately reversed within the emitprr() call.
void Assembler::MOVQXR(R l, R r) {
  emitprr(X64_movqxr, l, r);
  asm_output("movq %s, %s", RQ(l), RQ(r));
}
void Assembler::MOVDXR(R l, R r) {
  emitprr(X64_movdxr, l, r);
  asm_output("movd %s, %s", RQ(l), RQ(r));
}
void Assembler::MOVLHPS(R l, R r) {
  emitrr(X64_movlhps, l, r);
  asm_output("movlhps %s, %s", RQ(l), RQ(r));
}
void Assembler::PMOVMSKB(R l, R r) {
  emitprr(X64_pmovmskb, l, r);
  asm_output("pmovmskb %s, %s", RQ(l), RQ(r));
}
void Assembler::CMPNEQPS(R l, R r) {
  emitrr_imm8(X64_cmppsr, l, r, 4);
  asm_output("cmpneqps %s, %s", RL(l), RL(r));
}
...
void Assembler::MOVI(R r, I32 i32) {
  emitr_imm(X64_movi, r, i32);
  asm_output("movl %s, %d", RL(r), i32);
}
void Assembler::ADDLRI(R r, I32 i32) {
  emitr_imm(X64_addlri, r, i32);
  asm_output("addl %s, %d", RL(r), i32);
}
void Assembler::SUBLRI(R r, I32 i32) {
  emitr_imm(X64_sublri, r, i32);
  asm_output("subl %s, %d", RL(r), i32);
}
void Assembler::ANDLRI(R r, I32 i32) {
  emitr_imm(X64_andlri, r, i32);
  asm_output("andl %s, %d", RL(r), i32);
}
void Assembler::ORLRI(R r, I32 i32) {
  emitr_imm(X64_orlri, r, i32);
  asm_output("orl %s, %d", RL(r), i32);
}
void Assembler::XORLRI(R r, I32 i32) {
  emitr_imm(X64_xorlri, r, i32);
  asm_output("xorl %s, %d", RL(r), i32);
}
void Assembler::CMPLRI(R r, I32 i32) {
  emitr_imm(X64_cmplri, r, i32);
  asm_output("cmpl %s, %d", RL(r), i32);
}

void Assembler::ADDQRI(R r, I32 i32) {
  emitr_imm(X64_addqri, r, i32);
  asm_output("addq %s, %d", RQ(r), i32);
}
void Assembler::SUBQRI(R r, I32 i32) {
  emitr_imm(X64_subqri, r, i32);
  asm_output("subq %s, %d", RQ(r), i32);
}
void Assembler::ANDQRI(R r, I32 i32) {
  emitr_imm(X64_andqri, r, i32);
  asm_output("andq %s, %d", RQ(r), i32);
}
void Assembler::ORQRI(R r, I32 i32) {
  emitr_imm(X64_orqri, r, i32);
  asm_output("orq %s, %d", RQ(r), i32);
}
void Assembler::XORQRI(R r, I32 i32) {
  emitr_imm(X64_xorqri, r, i32);
  asm_output("xorq %s, %d", RQ(r), i32);
}
void Assembler::CMPQRI(R r, I32 i32) {
  emitr_imm(X64_cmpqri, r, i32);
  asm_output("cmpq %s, %d", RQ(r), i32);
}
void Assembler::MOVQI32(R r, I32 i32) {
  emitr_imm(X64_movqi32, r, i32);
  asm_output("movqi32 %s, %d", RQ(r), i32);
}

void Assembler::ADDLR8(R r, I32 i8) {
  emitr_imm8(X64_addlr8, r, i8);
  asm_output("addl %s, %d", RL(r), i8);
}
void Assembler::SUBLR8(R r, I32 i8) {
  emitr_imm8(X64_sublr8, r, i8);
  asm_output("subl %s, %d", RL(r), i8);
}
void Assembler::ANDLR8(R r, I32 i8) {
  emitr_imm8(X64_andlr8, r, i8);
  asm_output("andl %s, %d", RL(r), i8);
}
void Assembler::ORLR8(R r, I32 i8) {
  emitr_imm8(X64_orlr8, r, i8);
  asm_output("orl %s, %d", RL(r), i8);
}
void Assembler::XORLR8(R r, I32 i8) {
  emitr_imm8(X64_xorlr8, r, i8);
  asm_output("xorl %s, %d", RL(r), i8);
}
void Assembler::CMPLR8(R r, I32 i8) {
  emitr_imm8(X64_cmplr8, r, i8);
  asm_output("cmpl %s, %d", RL(r), i8);
}

void Assembler::ADDQR8(R r, I32 i8) {
  emitr_imm8(X64_addqr8, r, i8);
  asm_output("addq %s, %d", RQ(r), i8);
}
void Assembler::SUBQR8(R r, I32 i8) {
  emitr_imm8(X64_subqr8, r, i8);
  asm_output("subq %s, %d", RQ(r), i8);
}
void Assembler::ANDQR8(R r, I32 i8) {
  emitr_imm8(X64_andqr8, r, i8);
  asm_output("andq %s, %d", RQ(r), i8);
}
void Assembler::ORQR8(R r, I32 i8) {
  emitr_imm8(X64_orqr8, r, i8);
  asm_output("orq %s, %d", RQ(r), i8);
}
void Assembler::XORQR8(R r, I32 i8) {
  emitr_imm8(X64_xorqr8, r, i8);
  asm_output("xorq %s, %d", RQ(r), i8);
}
void Assembler::CMPQR8(R r, I32 i8) {
  emitr_imm8(X64_cmpqr8, r, i8);
  asm_output("cmpq %s, %d", RQ(r), i8);
}

void Assembler::IMULI(R l, R r, I32 i32) {
  emitrr_imm(X64_imuli, l, r, i32);
  asm_output("imuli %s, %s, %d", RL(l), RL(r), i32);
}

void Assembler::MOVQI(R r, U64 u64) {
  emitr_imm64(X64_movqi, r, u64);
  asm_output("movq %s, %p", RQ(r), (void *)u64);
}

void Assembler::LEARIP(R r, I32 d) {
  emitrm(X64_learip, r, d, RZero);
  asm_output("lea %s, %d(rip)", RQ(r), d);
}

void Assembler::LEALRM(R r, I d, R b) {
  emitrm(X64_lealrm, r, d, b);
  asm_output("leal %s, %d(%s)", RL(r), d, RL(b));
}
void Assembler::LEAQRM(R r, I d, R b) {
  emitrm(X64_leaqrm, r, d, b);
  asm_output("leaq %s, %d(%s)", RQ(r), d, RQ(b));
}
void Assembler::MOVLRM(R r, I d, R b) {
  emitrm(X64_movlrm, r, d, b);
  asm_output("movl %s, %d(%s)", RL(r), d, RQ(b));
}
void Assembler::MOVQRM(R r, I d, R b) {
  emitrm(X64_movqrm, r, d, b);
  asm_output("movq %s, %d(%s)", RQ(r), d, RQ(b));
}
void Assembler::MOVBMR(R r, I d, R b) {
  emitrm8(X64_movbmr, r, d, b);
  asm_output("movb %d(%s), %s", d, RQ(b), RB(r));
}
void Assembler::MOVSMR(R r, I d, R b) {
  emitprm(X64_movsmr, r, d, b);
  asm_output("movs %d(%s), %s", d, RQ(b), RS(r));
}
void Assembler::MOVLMR(R r, I d, R b) {
  emitrm(X64_movlmr, r, d, b);
  asm_output("movl %d(%s), %s", d, RQ(b), RL(r));
}
void Assembler::MOVQMR(R r, I d, R b) {
  emitrm(X64_movqmr, r, d, b);
  asm_output("movq %d(%s), %s", d, RQ(b), RQ(r));
}

void Assembler::MOVZX8M(R r, I d, R b) {
  emitrm_wide(X64_movzx8m, r, d, b);
  asm_output("movzxb %s, %d(%s)", RQ(r), d, RQ(b));
}
void Assembler::MOVZX16M(R r, I d, R b) {
  emitrm_wide(X64_movzx16m, r, d, b);
  asm_output("movzxs %s, %d(%s)", RQ(r), d, RQ(b));
}

void Assembler::MOVSX8M(R r, I d, R b) {
  emitrm_wide(X64_movsx8m, r, d, b);
  asm_output("movsxb %s, %d(%s)", RQ(r), d, RQ(b));
}
void Assembler::MOVSX16M(R r, I d, R b) {
  emitrm_wide(X64_movsx16m, r, d, b);
  asm_output("movsxs %s, %d(%s)", RQ(r), d, RQ(b));
}

void Assembler::MOVSDRM(R r, I d, R b) {
  emitprm(X64_movsdrm, r, d, b);
  asm_output("movsd %s, %d(%s)", RQ(r), d, RQ(b));
}
void Assembler::MOVSDMR(R r, I d, R b) {
  emitprm(X64_movsdmr, r, d, b);
  asm_output("movsd %d(%s), %s", d, RQ(b), RQ(r));
}
void Assembler::MOVSSRM(R r, I d, R b) {
  emitprm(X64_movssrm, r, d, b);
  asm_output("movss %s, %d(%s)", RQ(r), d, RQ(b));
}
void Assembler::MOVSSMR(R r, I d, R b) {
  emitprm(X64_movssmr, r, d, b);
  asm_output("movss %d(%s), %s", d, RQ(b), RQ(r));
}
void Assembler::MOVUPSRM(R r, I d, R b) {
  emitrm_wide(X64_movupsrm, r, d, b);
  asm_output("movups %s, %d(%s)", RQ(r), d, RQ(b));
}
void Assembler::MOVUPSMR(R r, I d, R b) {
  emitrm_wide(X64_movupsmr, r, d, b);
  asm_output("movups %d(%s), %s", d, RQ(b), RQ(r));
}
void Assembler::MOVUPSRMRIP(R r, I d) {
  emitrm_wide(X64_movupsrip, r, d, RZero);
  asm_output("movups %s, %d(rip)", RQ(r), d);
}
void Assembler::MOVAPSRM(R r, I d, R b) {
  emitrm_wide(X64_movapsrm, r, d, b);
  asm_output("movaps %s, %d(%s)", RQ(r), d, RQ(b));
}
void Assembler::MOVAPSRMRIP(R r, I d) {
  emitrm_wide(X64_movapsrip, r, d, RZero);
  asm_output("movaps %s, %d(rip)", RQ(r), d);
}
...
void Assembler::JMP8(S n, NIns *t) {
  emit_target8(n, X64_jmp8, t);
  asm_output("jmp %p", t);
}

void Assembler::JMP32(S n, NIns *t) {
  emit_target32(n, X64_jmp, t);
  asm_output("jmp %p", t);
}
void Assembler::JMP64(S n, NIns *t) {
  emit_target64(n, X64_jmpi, t);
  asm_output("jmp %p", t);
}
...
void Assembler::JMPXB(R indexreg, R tablereg) {
  emitxb(X64_jmpxb, indexreg, tablereg);
  asm_output("jmp [%s*8 + %s]", RQ(indexreg), RQ(tablereg));
}

void Assembler::JO(S n, NIns *t) {
  emit_target32(n, X64_jo, t);
  asm_output("jo %p", t);
}
void Assembler::JE(S n, NIns *t) {
  emit_target32(n, X64_je, t);
  asm_output("je %p", t);
}
void Assembler::JL(S n, NIns *t) {
  emit_target32(n, X64_jl, t);
  asm_output("jl %p", t);
}
void Assembler::JLE(S n, NIns *t) {
  emit_target32(n, X64_jle, t);
  asm_output("jle %p", t);
}
void Assembler::JG(S n, NIns *t) {
  emit_target32(n, X64_jg, t);
  asm_output("jg %p", t);
}
void Assembler::JGE(S n, NIns *t) {
  emit_target32(n, X64_jge, t);
  asm_output("jge %p", t);
}
void Assembler::JB(S n, NIns *t) {
  emit_target32(n, X64_jb, t);
  asm_output("jb %p", t);
}
void Assembler::JBE(S n, NIns *t) {
  emit_target32(n, X64_jbe, t);
  asm_output("jbe %p", t);
}
void Assembler::JA(S n, NIns *t) {
  emit_target32(n, X64_ja, t);
  asm_output("ja %p", t);
}
void Assembler::JAE(S n, NIns *t) {
  emit_target32(n, X64_jae, t);
  asm_output("jae %p", t);
}
void Assembler::JP(S n, NIns *t) {
  emit_target32(n, X64_jp, t);
  asm_output("jp  %p", t);
}

void Assembler::JNO(S n, NIns *t) {
  emit_target32(n, X64_jo ^ X64_jneg, t);
  asm_output("jno %p", t);
}
void Assembler::JNE(S n, NIns *t) {
  emit_target32(n, X64_je ^ X64_jneg, t);
  asm_output("jne %p", t);
}
void Assembler::JNL(S n, NIns *t) {
  emit_target32(n, X64_jl ^ X64_jneg, t);
  asm_output("jnl %p", t);
}
void Assembler::JNLE(S n, NIns *t) {
  emit_target32(n, X64_jle ^ X64_jneg, t);
  asm_output("jnle %p", t);
}
void Assembler::JNG(S n, NIns *t) {
  emit_target32(n, X64_jg ^ X64_jneg, t);
  asm_output("jng %p", t);
}
void Assembler::JNGE(S n, NIns *t) {
  emit_target32(n, X64_jge ^ X64_jneg, t);
  asm_output("jnge %p", t);
}
void Assembler::JNB(S n, NIns *t) {
  emit_target32(n, X64_jb ^ X64_jneg, t);
  asm_output("jnb %p", t);
}
void Assembler::JNBE(S n, NIns *t) {
  emit_target32(n, X64_jbe ^ X64_jneg, t);
  asm_output("jnbe %p", t);
}
void Assembler::JNA(S n, NIns *t) {
  emit_target32(n, X64_ja ^ X64_jneg, t);
  asm_output("jna %p", t);
}
void Assembler::JNAE(S n, NIns *t) {
  emit_target32(n, X64_jae ^ X64_jneg, t);
  asm_output("jnae %p", t);
}

void Assembler::JO8(S n, NIns *t) {
  emit_target8(n, X64_jo8, t);
  asm_output("jo %p", t);
}
void Assembler::JE8(S n, NIns *t) {
  emit_target8(n, X64_je8, t);
  asm_output("je %p", t);
}
void Assembler::JL8(S n, NIns *t) {
  emit_target8(n, X64_jl8, t);
  asm_output("jl %p", t);
}
void Assembler::JLE8(S n, NIns *t) {
  emit_target8(n, X64_jle8, t);
  asm_output("jle %p", t);
}
void Assembler::JG8(S n, NIns *t) {
  emit_target8(n, X64_jg8, t);
  asm_output("jg %p", t);
}
void Assembler::JGE8(S n, NIns *t) {
  emit_target8(n, X64_jge8, t);
  asm_output("jge %p", t);
}
void Assembler::JB8(S n, NIns *t) {
  emit_target8(n, X64_jb8, t);
  asm_output("jb %p", t);
}
void Assembler::JBE8(S n, NIns *t) {
  emit_target8(n, X64_jbe8, t);
  asm_output("jbe %p", t);
}
void Assembler::JA8(S n, NIns *t) {
  emit_target8(n, X64_ja8, t);
  asm_output("ja %p", t);
}
void Assembler::JAE8(S n, NIns *t) {
  emit_target8(n, X64_jae8, t);
  asm_output("jae %p", t);
}
void Assembler::JP8(S n, NIns *t) {
  emit_target8(n, X64_jp8, t);
  asm_output("jp  %p", t);
}

void Assembler::JNO8(S n, NIns *t) {
  emit_target8(n, X64_jo8 ^ X64_jneg8, t);
  asm_output("jno %p", t);
}
void Assembler::JNE8(S n, NIns *t) {
  emit_target8(n, X64_je8 ^ X64_jneg8, t);
  asm_output("jne %p", t);
}
void Assembler::JNL8(S n, NIns *t) {
  emit_target8(n, X64_jl8 ^ X64_jneg8, t);
  asm_output("jnl %p", t);
}
void Assembler::JNLE8(S n, NIns *t) {
  emit_target8(n, X64_jle8 ^ X64_jneg8, t);
  asm_output("jnle %p", t);
}
void Assembler::JNG8(S n, NIns *t) {
  emit_target8(n, X64_jg8 ^ X64_jneg8, t);
  asm_output("jng %p", t);
}
void Assembler::JNGE8(S n, NIns *t) {
  emit_target8(n, X64_jge8 ^ X64_jneg8, t);
  asm_output("jnge %p", t);
}
void Assembler::JNB8(S n, NIns *t) {
  emit_target8(n, X64_jb8 ^ X64_jneg8, t);
  asm_output("jnb %p", t);
}
void Assembler::JNBE8(S n, NIns *t) {
  emit_target8(n, X64_jbe8 ^ X64_jneg8, t);
  asm_output("jnbe %p", t);
}
void Assembler::JNA8(S n, NIns *t) {
  emit_target8(n, X64_ja8 ^ X64_jneg8, t);
  asm_output("jna %p", t);
}
void Assembler::JNAE8(S n, NIns *t) {
  emit_target8(n, X64_jae8 ^ X64_jneg8, t);
  asm_output("jnae %p", t);
}
void Assembler::JNP8(S n, NIns *t) {
  emit_target8(n, X64_jp8 ^ X64_jneg8, t);
  asm_output("jnp  %p", t);
}

void Assembler::CALL(S n, NIns *t) {
  emit_target32(n, X64_call, t);
  asm_output("call %p", t);
}
...
void Assembler::MOVQMI(R r, I d, I32 imm) {
  emitrm_imm32(X64_movqmi, r, d, imm);
  asm_output("movq %d(%s), %d", d, RQ(r), imm);
}
void Assembler::MOVLMI(R r, I d, I32 imm) {
  emitrm_imm32(X64_movlmi, r, d, imm);
  asm_output("movl %d(%s), %d", d, RQ(r), imm);
}
void Assembler::MOVSMI(R r, I d, I32 imm) {
  emitprm_imm16(X64_movsmi, r, d, imm);
  asm_output("movs %d(%s), %d", d, RQ(r), imm);
}
void Assembler::MOVBMI(R r, I d, I32 imm) {
  emitrm_imm8(X64_movbmi, r, d, imm);
  asm_output("movb %d(%s), %d", d, RQ(r), imm);
}

void Assembler::MOVQSPR(I d, R r) {
  emit(X64_movqspr | U64(d) << 56 | U64((REGNUM(r) & 7) << 3) << 40 |
       U64((REGNUM(r) & 8) >> 1) << 24);
  asm_output("movq %d(rsp), %s", d, RQ(r));
} // insert r into mod/rm and rex bytes
void Assembler::MOVQSPX(I d, R r) {
  emit(rexprb(X64_movqspx, RSP, r) | U64(d) << 56 |
       U64((REGNUM(r) & 7) << 3) << 40);
  asm_output("movq %d(rsp), %s", d, RQ(r));
}

void Assembler::XORPSA(R r, I32 i32) {
  emitxm_abs(X64_xorpsa, r, i32);
  asm_output("xorps %s, (0x%x)", RQ(r), i32);
}
void Assembler::XORPSM(R r, NIns *a64) {
  emitxm_rel(X64_xorpsm, r, a64);
  asm_output("xorps %s, (%p)", RQ(r), a64);
}

void Assembler::X86_AND8R(R r) {
  emit(X86_and8r | U64(REGNUM(r) << 3 | (REGNUM(r) | 4)) << 56);
  asm_output("andb %s, %s", RB(r), RBhi(r));
}
void Assembler::X86_SETNP(R r) {
  emit(X86_setnp | U64(REGNUM(r) | 4) << 56);
  asm_output("setnp %s", RBhi(r));
}
void Assembler::X86_SETE(R r) {
  emit(X86_sete | U64(REGNUM(r)) << 56);
  asm_output("sete %s", RB(r));
}
...
// If target address is unknown, i.e., a backward branch, allow for 64-bit
// address. Formerly, a 32-bit displacement would do, but layout randomization
// may result in larger displacements if a function is split between two
// randomly-placed regions.
...
void Assembler::asm_qbinop(LIns *ins) { asm_arith(ins); }
...
void Assembler::asm_shift(LIns *ins) {
  ...
  // In principle, there is a test for 'shouldBlind()', but it will always
  // succeed.
  if (b->isImmI()) {
    asm_shift_imm(ins);
    return;
...
void Assembler::asm_arith_imm(LIns *ins) {
  ...
  if (isS8(imm)) {
    ...
    case LIR_addxovi:
      ADDLR8(rr, imm);
      break; // XXX: bug 547125: could use LEA for LIR_addi
    case LIR_andi:
      ANDLR8(rr, imm);
      break;
    case LIR_ori:
...
bool Assembler::asm_arith_imm_blind(LIns *ins) {
  ...
  case LIR_addi:
    ADDLRI(rr, _blindMask32);
    ADDLRI(rr, imm - _blindMask32);
    break;
  case LIR_addq:
    ADDQRI(rr, _blindMask32);
    ADDQRI(rr, imm - _blindMask32);
    break;

  case LIR_subi:
    SUBLRI(rr, _blindMask32);
    SUBLRI(rr, imm - _blindMask32);
    break;
  case LIR_subq:
    SUBQRI(rr, _blindMask32);
    SUBQRI(rr, imm - _blindMask32);
    break;

  case LIR_andi:
    ANDLRI(rr, (imm & _blindMask32) | ~_blindMask32);
    ANDLRI(rr, (imm & ~_blindMask32) | _blindMask32);
    break;
  case LIR_andq:
    ANDQRI(rr, (imm & _blindMask32) | ~_blindMask32);
    ANDQRI(rr, (imm & ~_blindMask32) | _blindMask32);
    break;

  case LIR_ori:
    ORLRI(rr, imm & _blindMask32);
    ORLRI(rr, imm & ~_blindMask32);
    break;
  case LIR_orq:
    ORQRI(rr, imm & _blindMask32);
    ORQRI(rr, imm & ~_blindMask32);
    break;

  case LIR_xori:
    XORLRI(rr, _blindMask32);
    XORLRI(rr, imm ^ _blindMask32);
    break;
  case LIR_xorq:
    XORQRI(rr, _blindMask32);
    XORQRI(rr, imm ^ _blindMask32);
    break;
...
void Assembler::asm_call(LIns *ins) {
  if (!ins->isop(LIR_callv)) {
    Register rr =
        (ins->isop(LIR_calld) || ins->isop(LIR_callf) || ins->isop(LIR_callf4))
            ? XMM0
            : RAX;
  ...
  if (!call->isIndirect()) {
    verbose_only(if (_logc->lcbits & LC_Native) outputf("        %p:", _nIns);)
    ...
    // since RAX is usually the return value and will be allocated until that
    // point.
  ...
  for (int i = 0; i < argc; i++) {
    ...
    if ((ty == ARGTYPE_I || ty == ARGTYPE_UI || ty == ARGTYPE_Q) &&
...
#else
    else if ((ty == ARGTYPE_D || ty == ARGTYPE_F || ty == ARGTYPE_F4) &&
             fr < XMM8) {
      ...
      /* float4 is passed as a pointer to the value, so it still takes up as
       * much space as "void*" */
...
void Assembler::asm_stkarg(ArgType ty, LIns *p, int stk_off) {
  ...
  if (ty == ARGTYPE_I || ty == ARGTYPE_UI || ty == ARGTYPE_Q) {
    ...
    verbose_only({
      char str[1000];
      sprintf(str, "ArgType %x Opcode %x\n", ty, p->opcode());
      avmplus::AvmLog(str);
...
void Assembler::asm_ui2d(LIns *ins) {
  ...
  Register rr = prepareResultReg(ins, FpRegs);
  Register ra = findRegFor(a, GpRegs);
  // Because oprnd1 is 32bit, it's ok to zero-extend it without worrying about
  // clobbering.
...
void Assembler::asm_ffff2f4(LIns *ins) {
  ...
  freeResourcesOf(ins);
  Register rx = x->isInReg() ? findRegFor(x, FpRegs)
                             : findSpecificRegForUnallocated(x, rr);
  if (rx != rr)
    MOVAPSR(rr, rx);
}
...
void Assembler::asm_f4comp(LIns *ins) {
  ...
  switch (ins->opcode()) {
  default:
    NanoAssert(!"bad opcode for asm_f4comp()");
    break;
  case LIR_f4x:
    PSHUFD(rr, rb, PSHUFD_MASK(0, 0, 0, 0));
    break;
  case LIR_f4y:
    PSHUFD(rr, rb, PSHUFD_MASK(1, 1, 1, 1));
    break;
  case LIR_f4z:
    PSHUFD(rr, rb, PSHUFD_MASK(2, 2, 2, 2));
    break;
  case LIR_f4w:
    PSHUFD(rr, rb, PSHUFD_MASK(3, 3, 3, 3));
    break;
  case LIR_swzf4: {
...
Branches Assembler::asm_branch_helper(bool onFalse, LIns *cond, NIns *target) {
  ...
  if (target && isTargetWithinS8(target)) {
    ...
    //     B1:
    underrunProtect(
        22); // 14 bytes for JMP64 + 8 bytes (incl. overhang) for branchi helper
    NIns *skip = _nIns;
    JMP64(16, target); // 6 + 8 bytes (16)
    patch = _nIns;
    // Generate an 8-bit branch to a target that does not need to be patched.
    // Needs 8 bytes max.
...
void Assembler::asm_brsavpc_impl(LIns *flag, NIns *target) {
  ...
  // The length of this instruction sequence (16) is hardcoded into
  // asm_restorepc below!
  NIns *skip = _nIns;
  JMP64(16, target); // 6 + 8 bytes (16)
...
Branches Assembler::asm_branchd_helper(bool onFalse, LIns *cond, NIns *target) {
  ...
  if (condop == LIR_eqd) {
    if (onFalse) {
      ...
      underrunProtect(14); // ensure we have space for entire 32-bit branch
      ...
      if (target && isTargetWithinS32(target)) {
        ...
        underrunProtect(38); // ensure we have space for entire 64-bit branch
                             // sequence with overhang
        NIns *skip1 = _nIns;
        JMP64(16, target); // 6 + 8 bytes (16)
        patch1 = _nIns;
      ...
      if (target && isTargetWithinS32(target)) {
        ...
        underrunProtect(28); // ensure we have space for entire 64-bit branch
                             // sequence with overhang
        NIns *skip = _nIns;
        JMP64(16, target); // 6 + 8 bytes (16)
        patch1 = _nIns;
...
void Assembler::asm_condd(LIns *ins) {
  ...
  if (isCmpFOpcode(op))
    op = getCmpDOpcode(
        op); // the only difference between float/double is in asm_cmpd
  if (op == LIR_eqd) {
    ...
    // LIR_led/LIR_ged.
    Register r =
        prepareResultReg(ins, GpRegs); // x64 can use any GPR as setcc target
    MOVZX8(r, r);
    switch (op) {
    case LIR_ltd:
    case LIR_gtd:
...
void Assembler::asm_condf4(LIns *cond) {
  ...
  // unlike x86-32, with a rex prefix we can use any GP register as an 8bit
  // target
...
void Assembler::asm_cmpf4(LIns *cond) {
  ...
  /*
      ...
      // we could use PTEST but it's SSE4_1, we don't want to assume SSE4
     support yet PMOVMSKB gp, ra CMP      gp, 0
  */
  CMPLR8(gt, 0);
...
bool canRematLEA(LIns *ins) {
  ...
  case LIR_addq: {
    ...
    return ins->oprnd1()->isInRegMask(BaseRegs) &&
           (rhs = ins->oprnd2())->isImmQ() && isS32(rhs->immQ()) &&
...
bool RegAlloc::canRemat(LIns *ins) {
  // We cannot rematerialize tainted (blinded) integer literals, as the XOR
  // instruction used to synthesize the constant value may alter the CCs.  See
  // asm_restore() below.
  return (ins->isImmAny() &&
...
void Assembler::asm_ret(LIns *ins) {
  ...
  Register r =
      ins->isop(LIR_retd) || ins->isop(LIR_retf) || ins->isop(LIR_retf4) ? XMM0
                                                                         : RAX;
...
void Assembler::beginLoadRegs(LIns *ins, RegisterMask allow, Register &rr,
                              int32_t &dr, Register &rb, Register &orb) {
  dr = ins->disp();
  LIns *base = ins->oprnd1();
  bool force = forceDisplacementBlinding(ins->isTainted());
  // Allocation of r must precede getBaseRegWithBlinding(), as the latter may
  // allocate a temporary register. Once a temporary has been allocated, no
  // other allocations (within an overlapping regclass) may occur until the
  // temporary is dead.
  rr = prepareResultReg(ins, allow);
  rb = getBaseRegWithBlinding(base, dr, BaseRegs & ~rmask(rr), ins->isTainted(),
                              force, &orb);
}
...
void Assembler::asm_immf4(Register r, float4_t v, bool canClobberCCs,
  ...
  if (v0 == 0 && v1 == 0 && canClobberCCs) {
    ...
    if (v1 == 0 && !blind) {
      ...
      /*
          We must be sure that MOVAPSRMRIP/MOVAPSRMRIP does NOT cross into a new
         page. Hence - isTargetWithinS32 has to make room for 12 bytes (not 8),
         because emit_disp32() makes room for displacement (4 bytes) + full-size
         op (8 bytes)
      */
...
void Assembler::asm_store128(LOpcode op, LIns *value, int d, LIns *base,
                             bool tainted) {
  NanoAssert((value->isF4() && (op == LIR_stf4)));
  (void)op;
...
void Assembler::asm_store64(LOpcode op, LIns *value, int d, LIns *base,
  ...
  case LIR_stq: {
    ...
    if (value->isImmQ() && (c = value->immQ(), isS32(c)) &&
        !(value->isTainted() && shouldBlind(c))) {
      force =
          force || tainted; // If the store is tainted, and we are not going to
                            // blind the immediate, then blind the displacement.
      uint64_t c = value->immQ();
      Register orb;
      Register rb =
          getBaseRegWithBlinding(base, d, BaseRegs, tainted, force, &orb);
      // MOVQMI takes a 32-bit integer that gets signed extended to a 64-bit
      // value.
      MOVQMI(rb, d, int32_t(c));
      adjustBaseRegForBlinding(rb, orb);
    } else {
      Register rr, rb, orb;
      getBaseReg2WithBlinding(GpRegs, value, rr, BaseRegs, base, rb, d, tainted,
                              force, &orb);
...
void Assembler::asm_store32(LOpcode op, LIns *value, int d, LIns *base,
  ...
  if (value->isImmI() && !(value->isTainted() && shouldBlind(value->immI()))) {
    ...
    const RegisterMask SrcRegs =
        (op == LIR_sti2c) ? SingleByteStoreRegs : GpRegs;
    ...
    Register b = getBaseRegWithBlinding(base, d, BaseRegs & ~rmask(r), tainted,
                                        force, &ob);
    switch (op) {
    case LIR_sti2c:
      MOVBMR(r, d, b);
...
void Assembler::asm_immq(Register r, uint64_t v, bool canClobberCCs,
  ...
  if (isU32(v)) {
    ...
    // Value is within +/- 2GB from RIP, thus we can use LEA with RIP-relative
    // disp32. Don't use this pattern for blinded constants, as an attacker
    // might know where the code is loaded.
...
void Assembler::asm_param(LIns *ins) {
  ...
  if (kind == 0) {
    // Ordinary param.  First four or six args always in registers for x86_64
    // ABI.
...
void Assembler::beginOp1Regs(LIns *ins, RegisterMask allow, Register &rr,
                             Register &ra) {
  LIns *a = ins->oprnd1();
...
// Pairs with endOpRegs().
void Assembler::beginOp2Regs(LIns *ins, RegisterMask allow, Register &rr,
...
static const AVMPLUS_ALIGN16(int32_t) negateMaskF4[] = {0x80000000, 0x80000000,
                                                        0x80000000, 0x80000000};

void Assembler::asm_neg_abs(LIns *ins) {
  Register rr, ra;
  NanoAssert(ins->isop(LIR_negf) || ins->isop(LIR_negf4) ||
             ins->isop(LIR_negd));
  ...
  switch (ins->opcode()) {
  default:
    NanoAssert(!"bad opcode for asm_neg_abs");
    mask = 0;
    break;
  case LIR_negf:
    mask = (uintptr_t)negateMaskF;
    break;
  case LIR_negf4:
    mask = (uintptr_t)negateMaskF4;
    break;
  case LIR_negd:
    mask = (uintptr_t)negateMaskD;
    break;
  }
  ...
  if (isS32(mask)) {
    ...
    // AND all the called functions (asm_immq, asm_immi) don't alloc/inspect the
    // regstate But this is arguably dangerous (some called function may change
    // in the future),
    ...
    if (ins->isop(LIR_negf) || ins->isop(LIR_negf4)) {
      if (ins->isop(LIR_negf4))
        PSHUFD(rt, rt,
               PSHUFD_MASK(
                   0, 0, 0,
                   0)); // copy mask in all 4 components of the float4 vector
      MOVDXR(rt, gt);
...
void Assembler::asm_recip_sqrt(LIns *) { NanoAssert(!"not implemented"); }
...
NIns *Assembler::genPrologue() {
  ...
  uint32_t stackPushed = sizeof(void *) + // returnaddr
                         sizeof(void *);  // ebp
  ...
  // Alternatively, we can add this NOPs in front of function prologue but if we
  // do, a debugger (i.e Visueal Studio) would get lost callstack information so
  // it would make us difficult to debug. ToDo:
  ...
  verbose_only(asm_output("[patch entry]");) NIns *patchEntry = _nIns;
...
void Assembler::nPatchBranch(NIns *patch, NIns *target) {
  ...
  if (patch[0] == 0xE9) {
    ...
    // jmp 64bit target
    // This uses RIP-relative addressing, the 4 bytes after FF 25 is an offset
    // of 0.
    next = patch + 6;
  ...
  // Guards can result in a valid branch being patched again later, so don't
  // assert that the old value is poison.
...
void Assembler::nBeginAssembly() { max_stk_used = 0; }

// This should only be called from within emit() et al.
void Assembler::underrunProtect(ptrdiff_t bytes) {
  NanoAssertMsg(bytes <= LARGEST_UNDERRUN_PROT,
                "constant LARGEST_UNDERRUN_PROT is too small");
  NIns *pc = _nIns;
  NIns *top =
      codeStart; // this may be in a normal code chunk or an exit code chunk

#if PEDANTIC
  // pedanticTop is based on the last call to underrunProtect; any time we call
  // underrunProtect and would use more than what's already protected, then
  // insert a page break jump.  Sometimes, this will be to a new page, usually
  // it's just the next instruction

  NanoAssert(pedanticTop >= top);
  if (pc - bytes < pedanticTop) {
    // no page break required, but insert a far branch anyway just to be
    // difficult
...
RegisterMask RegAlloc::nHint(LIns *ins) {
  static RegisterMask
      Hints[LIR_sentinel + 1]; // effectively const, save for the initialization
...
verbose_only(void Assembler::asm_inc_m32(uint32_t *pCtr) {
  ...
  asm_immq(RAX, (uint64_t)pCtr, /*canClobberCCs*/ true,
           /*blind*/ false); // movabsq $pCtr, %rax
  ...
  if (isS32((intptr_t)table)) {
    ...
    // don't use R13 for base because we want to use mod=00, i.e. [index*8+base
    // + 0]
    Register tablereg =
        _allocator.allocTempReg(GpRegs & ~(rmask(indexreg) | rmask(R13)));
