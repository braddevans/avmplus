...
const char *cname[16] = {"f",   "un",  "eq", "ueq",  "olt", "ult",
...
const char *fname[32] = {
    ...
    "resv", "resv", "resv", "resv", "resv", "resv", "resv", "resv",
    "s",    "d",    "resv", "resv", "w",    "l",    "ps",   "resv",
    "resv", "resv", "resv", "resv", "resv", "resv", "resv", "resv",
};

const char *oname[64] = {
    "special",  "regimm", "j",     "jal",      "beq",   "bne",   "blez",
    "bgtz",     "addi",   "addiu", "slti",     "sltiu", "andi",  "ori",
    "xori",     "lui",    "cop0",  "cop1",     "cop2",  "cop1x", "beql",
    "bnel",     "blezl",  "bgtzl", "resv",     "resv",  "resv",  "resv",
    "special2", "jalx",   "resv",  "special3", "lb",    "lh",    "lwl",
    "lw",       "lbu",    "lhu",   "lwr",      "resv",  "sb",    "sh",
    "swl",      "sw",     "resv",  "resv",     "swr",   "cache", "ll",
    "lwc1",     "lwc2",   "pref",  "resv",     "ldc1",  "ldc2",  "resv",
    "sc",       "swc1",   "swc2",  "resv",     "resv",  "sdc1",  "sdc2",
    "resv",
};
#endif
...
const Register RegAlloc::savedRegs[] = {S0,  S1,  S2,  S3,  S4,  S5, S6, S7,
#ifdef FPCALLEESAVED
...
#define USE(x) (void)x
#define BADOPCODE(op)                                                          \
  NanoAssertMsgf(false, "unexpected opcode %s", lirNames[op])
...
static inline int mswoff(void) { return isLittleEndian() ? 4 : 0; }

static inline int lswoff(void) { return isLittleEndian() ? 0 : 4; }
...
debug_only(
    ...
    static NIns *breakAddr; static void codegenBreak(NIns *genAddr) {
...
void Assembler::asm_move(Register d, Register s) { MOVE(d, s); }
...
void Assembler::asm_stkarg(LIns *arg, int stkd) {
  ...
  Register rr;
  if (arg->isExtant() &&
      (rr = arg->deprecated_getReg(), deprecated_isKnownReg(rr))) {
...
void Assembler::asm_arg_64(LIns *arg, Register &r, Register &fr, int &stkd) {
  ...
  if (stkd < 16) {
    ...
    if (fr == FA0 || fr == FA1)
      ...
      Register intpair = fr;
      MFC1(mswregpair(intpair),
           fpupair + 1); // Odd fpu register contains sign,expt,manthi
...
void Assembler::asm_store32(LOpcode op, LIns *value, int dr, LIns *base) {
  ...
  TAG("asm_store32(value=%p{%s}, dr=%d, base=%p{%s})", value,
      lirNames[value->opcode()], dr, base, lirNames[base->opcode()]);
}

void Assembler::asm_ui2d(LIns *ins) {
  Register fr = deprecated_prepResultReg(ins, FpRegs);
  Register v = findRegFor(ins->oprnd1(), GpRegs);
  Register ft = _allocator.allocTempReg(
      FpRegs & ~(rmask(fr))); // allocate temporary register for constant
...
void Assembler::asm_neg_abs(LIns *ins) {
  ...
  if (cpu_has_fpu) {
    ...
    Register rr = deprecated_prepResultReg(ins, FpRegs);
    Register sr =
        (!lhs->isInReg() ? findRegFor(lhs, FpRegs) : lhs->deprecated_getReg());
    NEG_D(rr, sr);
  }
...
void Assembler::asm_neg_not(LIns *ins) {
  ...
  Register ra =
      !lhs->isInReg() ? findSpecificRegFor(lhs, rr) : lhs->deprecated_getReg();
  if (op == LIR_noti)
    NOT(rr, ra);
  else
    NEGU(rr, ra);
...
void Assembler::asm_condd(LIns *ins) {
  ...
  if (cpu_has_fpu) {
    ...
    if (cpu_has_cmov) {
      ...
      // 1:
      NIns *here = _nIns;
      verbose_only(verbose_outputf("%p:", here);) underrunProtect(3 * 4);
      MOVE(r, ZERO);
...
void Assembler::asm_arith(LIns *ins) {
  ...
  case LIR_subi:
    ...
    break;
  case LIR_lshi:
    // SLLV uses the low-order 5 bits of rb for the shift amount so no masking
    // required
    SLLV(rr, ra, rb);
    break;
  case LIR_rshi:
    // SRAV uses the low-order 5 bits of rb for the shift amount so no masking
    // required
    SRAV(rr, ra, rb);
    break;
  case LIR_rshui:
    // SRLV uses the low-order 5 bits of rb for the shift amount so no masking
    // required
    SRLV(rr, ra, rb);
    break;
  case LIR_mulxovi:
...
void Assembler::asm_store64(LOpcode op, LIns *value, int dr, LIns *base) {
  ...
  case LIR_std2f:
    if (cpu_has_fpu) {
      Register fr =
          value->isInReg() ? value->getReg() : findRegFor(value, FpRegs);
  ...
  TAG("asm_store64(value=%p{%s}, dr=%d, base=%p{%s})", value,
      lirNames[value->opcode()], dr, base, lirNames[base->opcode()]);
}
...
void Assembler::asm_cmp(LOpcode condop, LIns *a, LIns *b, Register cr) {
  ...
  case LIR_ged:
    ...
    break;
  default:
    debug_only(outputf("%s", lirNames[condop]);) TODO(asm_cond);
  }
}
...
NIns *Assembler::asm_bxx(bool branchOnFalse, LOpcode condop, Register ra,
                         Register rb, NIns *const targ) {
  NIns *patch = NULL;
...
Branches Assembler::asm_branch(bool branchOnFalse, LIns *cond,
                               NIns *const targ) {
...
void Assembler::asm_j(NIns *const targ, bool bdelay) {
  if (targ == NULL) {
    ...
    //  [nop]
    underrunProtect(
        4 * 4); // worst case to prevent underrunProtect from reinvoking asm_j
    if (bdelay)
      NOP();
    JR(RA);
...
void Assembler::asm_nongp_copy(Register dst, Register src) {
 ...
 *   on the stack.
 */
void Assembler::asm_arg(ArgType ty, LIns *arg, Register &r, Register &fr,
                        int &stkd) {
...
void Assembler::asm_pushstate(void) { NanoAssert(false); }

void Assembler::asm_popstate(void) { NanoAssert(false); }

void Assembler::asm_savepc(void) { NanoAssert(false); }

void Assembler::asm_restorepc(void) { NanoAssert(false); }

void Assembler::asm_discardpc(void) { NanoAssert(false); }
...
verbose_only(void Assembler::asm_inc_m32(uint32_t * /*pCtr*/){
    ...
    void Assembler::nativePageReset(void) {
  _nSlot = 0;
  _nExitSlot = 0;
  TAG("nativePageReset()");
}

void Assembler::nativePageSetup(void) {
  NanoAssert(!_inExit);
  if (!_nIns)
...
NIns *Assembler::genPrologue(void) {
  /*
  ...
  NIns *patchEntry =
      _nIns; // FIXME: who uses this value and where should it point?
...
NIns *Assembler::genEpilogue(void) {
  /*
   * move    $sp,$fp
   * lw      $fp,FP_OFFSET($sp)
   * lw      $ra,RA_OFFSET($sp)
