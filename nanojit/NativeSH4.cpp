...
const char *regNames[] = {"r0", "r1",  "r2",  "r3",  "r4",  "r5",  "r6", "r7",
...
void Assembler::JMP(NIns *target, bool from_underrunProtect) {
  ...
  if (target != NULL && FITS_SH4_bra(SH4_LABEL(target + 1 * sizeof(NIns)))) {
    ...
    //
    //     1.   mov.l @(pc, ZZZ), Rtemp
    //          mov.l @(pc, TARGET), Rtemp    # Technically Rtemp is now the
    //          address of 2a. jmp @Rtemp                    # Jump to 2a.
    //     2a.  add FP, Rtemp
    //     2b.  mov.l Rsrc, @Rtemp
    //
...
void Assembler::asm_arith(LIns *inst) {
  ...
  NanoAssert(result_reg != Rtemp && operand1_reg != Rtemp &&
             operand2_reg != Rtemp);
  switch (opcode) {
  case LIR_rshi:
  ...
  case LIR_muljovi:
  case LIR_mulxovi:
    // 3. Now the T-bit is equal to "sign_of(result_reg) == sign_of(operand1)
    // XOR sign_of(operand2)".
    SH4_shll(Rtemp);
  ...
  case LIR_lshi:
    SH4_shad(operand2_reg, result_reg);
    break;
  case LIR_ori:
    SH4_or(operand2_reg, result_reg);
    break;
  case LIR_subi:
    SH4_sub(operand2_reg, result_reg);
    break;
  case LIR_addi:
    SH4_add(operand2_reg, result_reg);
    break;
  case LIR_andi:
    SH4_and(operand2_reg, result_reg);
    break;
  case LIR_xori:
    SH4_xor(operand2_reg, result_reg);
    break;

  case LIR_subjovi:
  case LIR_subxovi:
    SH4_subv(operand2_reg, result_reg);
    break;

  case LIR_addjovi:
  case LIR_addxovi:
    SH4_addv(operand2_reg, result_reg);
    break;
  default:
...
void Assembler::asm_condd(LIns *inst) { asm_cond(inst); }
...
void Assembler::asm_fop(LIns *inst) {
  ...
  switch (opcode) {
  case LIR_addd:
    SH4_fadd_double(operand2_reg, result_reg);
    break;
  case LIR_subd:
    SH4_fsub_double(operand2_reg, result_reg);
    break;
  case LIR_muld:
    SH4_fmul_double(operand2_reg, result_reg);
    break;
  case LIR_divd:
    SH4_fdiv_double(operand2_reg, result_reg);
    break;
  default:
...
void Assembler::asm_load64d(int offset, Register base_reg,
                            Register result_reg) {
  underrunProtect(2 * sizeof(NIns));
...
void Assembler::asm_load32d(int offset, Register base_reg,
                            Register result_reg) {
  underrunProtect(3 * sizeof(NIns));
...
void Assembler::asm_load32i(int offset, Register base_reg,
                            Register result_reg) {
...
#define gen_asm_loadXXi(size, length)                                          \
  void Assembler::asm_load##size##i(int offset, Register base_reg,             \
                                    Register result_reg, bool sign_extend) {   \
    ...
    if (offset == 0) {                                                         \
      ...
      if (result_reg == R0) {                                                  \
        ...
        SH4_mov##length##_dispRy_R0(offset,                                    \
                                    base_reg); /* @(base + offset) -> R0 */    \
      } else {                                                                 \
        /* We have to save and restore R0. */                                  \
        SH4_mov(Rtemp, R0);      /* 4. Rtemp -> R0            */               \
        SH4_mov(R0, result_reg); /* 3. R0 -> result           */               \
        SH4_mov##length##_dispRy_R0(offset,                                    \
                                    base_reg); /* 2. @(base + offset) -> R0 */ \
        SH4_mov(R0, Rtemp);                    /* 1. R0 -> Rtemp            */ \
      }                                                                        \
    } else {                                                                   \
      /* Worst case. */                                                        \
      SH4_mov##length##_indRy(Rtemp,                                           \
                              result_reg); /* 3. @(temp)     -> result */      \
      SH4_add(base_reg, Rtemp);            /* 2. base + temp -> temp   */      \
      asm_immi(offset, Rtemp);             /* 1. offset      -> temp   */      \
    }                                                                          \
  }

gen_asm_loadXXi(16, w) gen_asm_loadXXi(8, b)
...
void Assembler::asm_immi_nochk(int constant, Register reg, bool force) {
  ...
  if (!force && constant >= 2 * MIN_IMM && constant <= 2 * MAX_IMM) {
    int tmp;
...
void Assembler::asm_store32i(Register value_reg, int offset,
                             Register base_reg) {
  // Rtemp is mandatory to handle the "worst" case.
...
#define gen_asm_storeXXi(size, length)                                         \
  void Assembler::asm_store##size##i(Register value_reg, int offset,           \
                                     Register base_reg) {                      \
    if (base_reg == FP)                                                        \
      offset = FP_RELATIVE(offset);                                            \
                                                                               \
    if (offset == 0) {                                                         \
      ...
      if (value_reg == R0) {                                                   \
        ...
        underrunProtect(1 * sizeof(NIns));                                     \
        SH4_mov##length##_R0_dispRx(offset,                                    \
                                    base_reg); /* R0 -> @(base + offset) */    \
      } else {                                                                 \
        /* We have to save and restore R0. */                                  \
        underrunProtect(4 * sizeof(NIns));                                     \
        SH4_mov(Rtemp, R0); /* 4. Rtemp -> R0            */                    \
        SH4_mov##length##_R0_dispRx(offset,                                    \
                                    base_reg); /* 3. R0 -> @(base + offset) */ \
...
gen_asm_storeXXi(16, w) gen_asm_storeXXi(8, b)

    void Assembler::asm_store32(LOpcode opcode, LIns *value, int offset,
                                LIns *base) {
  Register value_reg;
  Register base_reg;
...
void Assembler::asm_store64d(Register value_reg, int offset,
                             Register base_reg) {
  underrunProtect(2 * sizeof(NIns));
...
void Assembler::asm_store32d(Register value_reg, int offset,
                             Register base_reg) {
  underrunProtect(3 * sizeof(NIns));
...
void Assembler::asm_store64(LOpcode opcode, LIns *value, int offset,
                            LIns *base) {
  Register value_reg;
  Register base_reg;
...
void Assembler::asm_cmp(LOpcode simplified_opcode, LIns *condition) {
  ...
  case LIR_eqi:
    SH4_cmpeq(operand2_reg, operand1_reg);
    break;
  case LIR_gti:
    SH4_cmpgt(operand2_reg, operand1_reg);
    break;
  case LIR_gei:
    SH4_cmpge(operand2_reg, operand1_reg);
    break;
  case LIR_gtui:
    SH4_cmphi(operand2_reg, operand1_reg);
    break;
  case LIR_geui:
    SH4_cmphs(operand2_reg, operand1_reg);
    break;
...
Branches Assembler::asm_branch(bool branchOnFalse, LIns *condition,
                               NIns *target) {
  NanoAssert(condition->isCmp());
  ...
  NIns *patch_target =
      asm_branch(negated ? branchOnFalse : !branchOnFalse, target);
...
void Assembler::nBeginAssembly() { max_stack_args = 0; }
...
void Assembler::nInit() {
  ...
  // Only the 'simple' transfer size mode is currently supported within
  // Nano/SH4.
...
NIns *Assembler::genPrologue() {
  int adjusted_stack_size = max_stack_args +
                            _activation.stackSlotsNeeded() * STACK_GRANULARITY -
                            STATIC_FP;
...
void Assembler::underrunProtect(int nb_bytes) {
  NanoAssertMsg(nb_bytes <= LARGEST_UNDERRUN_PROT,
...
bool Assembler::asm_generate_pool(int reserved_bytes, int nb_slots) {
  ...
  if (instruction_y < code_start || instruction_y > code_end)
  ...
  if (scalar_xy > 0 && scalar_xy <= scalar_mn * instructions_per_constant) {
    verbose_only(verbose_outputf(
        "pool not emitted: 0 < scalar_xy [%d] <= %d * scalar_mn [%d]",
  ...
  while (instruction_x - nb_slots * (sizeof(int) +
                                     instructions_per_constant * sizeof(NIns)) <
         code_start)
    nb_slots--;

  if (nb_slots <= 2) {
    verbose_only(verbose_outputf(
        "pool not emitted: not enough space in the current code buffer."));
    return false;
  }
  ...
  verbose_only(verbose_outputf("pool emitted: 0 >= scalar_xy [%d] > %d * "
                               "scalar_mn [%d]; address = %p, size = %d",
                               (int)scalar_xy, instructions_per_constant,
                               scalar_mn, current_pool.slots,
                               current_pool.nb_slots));
  return true;
}
...
try_again:
  ...
  uintptr_t farther_slot =
      MIN((((uintptr_t)_nIns) & ~0x3) + MOVL_PCREL_RANGE,
          (uintptr_t)&current_pool.slots[current_pool.nb_slots - 1]);

  verbose_only(verbose_outputf(
      "pool load: _nIns = %p, constant = %d, farther_slot = 0x%x", _nIns,
      constant, farther_slot));
  ...
  for (slot = (int *)farther_slot; slot >= current_pool.slots; slot--) {
    ...
    if (*slot == constant) {
      verbose_only(
          verbose_outputf("pool load: found same constant in slot @ %p", slot));
