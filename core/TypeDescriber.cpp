...
namespace avmplus {
TypeDescriber::TypeDescriber(Toplevel *toplevel) : m_toplevel(toplevel) {
...
void TypeDescriber::setpropmulti(ScriptObject *o,
                                 const TypeDescriber::KVPair *kv,
                                 uint32_t count) {
  while (count--) {
...
Stringp TypeDescriber::str(StringId i) {
  static const char *const k_strs[maxStringId] = {
      "access",      "accessors",  "API",        "*",          "bases",
      "constructor", "declaredBy", "",           "interfaces", "isDynamic",
      "isFinal",     "isStatic",   "key",        "metadata",   "method",
      "methods",     "name",       "native",     "optional",   "parameters",
      "readonly",    "readwrite",  "returnType", "traits",     "type",
      "uri",         "value",      "variables",  "writeonly"};
  if (!m_strs[i])
...
static inline Atom boolAtom(bool b) { return b ? trueAtom : falseAtom; }

ArrayObject *TypeDescriber::describeParams(MethodInfo * /*mi*/,
                                           MethodSignaturep ms) {
...
void TypeDescriber::addBindings(AvmCore *core,
                                MultinameBindingHashtable *bindings,
                                TraitsBindingsp tb, uint32_t flags) {
  if (!tb)
    return;
  if ((flags & TypeDescriber::HIDE_OBJECT) && !tb->base &&
      !tb->owner->isInterface())
    return;
...
ScriptObject *TypeDescriber::describeTraits(Traitsp traits, uint32_t flags,
  ...
  if (flags & (INCLUDE_ACCESSORS | INCLUDE_METHODS | INCLUDE_VARIABLES)) {
    ...
    // make a flattened set of bindings so we don't have to check for overrides
    // as we go. This is not terribly efficient, but doesn't need to be.
    ...
    while (iter.next()) {
      ...
      // We only display public members -- exposing private namespaces could
      // compromise security.
      ...
      case BKIND_VAR: {
        ...
        const KVPair props[] = {
            {kstrid_access, strAtom(str(bk == BKIND_CONST ? kstrid_readonly
                                                          : kstrid_readwrite))},
            {kstrid_type,
             strAtom(describeClassName(tb->getSlotTraits(slotID)))},
        };
        setpropmulti(v, props, elem_count(props));
        if (!variables)
          variables = new_array();
      ...
      case BKIND_GETSET: {
        ...
        setpropmulti(v, props, elem_count(props));
        if (AvmCore::hasGetterBinding(binding))
          md1 = tm->getMethodMetadataPos(AvmCore::bindingToGetterId(binding),
                                         md1pool);
        if (AvmCore::hasSetterBinding(binding))
          md2 = tm->getMethodMetadataPos(AvmCore::bindingToSetterId(binding),
                                         md2pool);
        if (!accessors)
          accessors = new_array();
        pushobj(accessors, v);
...
void TypeDescriber::addDescribeMetadata(ArrayObject *a, PoolObject *pool,
...
static void read_u30_list(DataList<uint32_t> &list, uint32_t val_count,
                          const uint8_t *&pos) {
...
ScriptObject *TypeDescriber::describeMetadataInfo(PoolObject *pool,
                                                  uint32_t metadata_index) {
  AvmCore *core = m_toplevel->core();
  const uint8_t *metadata_pos = pool->metadata_infos[metadata_index];

  const uint32_t name_index =
      (metadata_pos) ? AvmCore::readU32(metadata_pos) : 0;
  // A bit of a hack: if the pool is builtin, always omit metadata chunks with
  // names of "Version" or "native" or "API", since these are used for reserved
  // purposes internally.
  Stringp name = poolstr(pool, name_index);
  AvmAssert(name->isInterned() && core->kVersion->isInterned() &&
            str(kstrid_native)->isInterned() && str(kstrid_api)->isInterned());
  if (pool->isBuiltin &&
      (name == core->kVersion || name == str(kstrid_native) ||
       name == str(kstrid_api)))
    return NULL;
...
Stringp TypeDescriber::poolstr(PoolObject *pool, uint32_t index) {
  return index < pool->constantStringCount ? pool->getString(index)
                                           : str(kstrid_emptyString);
}

// bug comaptibility note: FP10 and earlier sniffed the Atom type and return
// 'int' for all integer atoms. This is wrong, but we should preserve the
// behavior. Note that we compare the numeric range: doesn't matter for 32-bit
// builds, but 64-builds have larger possible int atoms, so this preserves the
// existing behavior of "29-bit int -> int" even on 64-bit systems.
...
Stringp TypeDescriber::getQualifiedSuperclassName(Atom value) {
  ...
  return traits->base &&
                 traits->base != m_toplevel->core()->traits.class_itraits
