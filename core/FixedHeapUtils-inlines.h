...
FixedHeapArray<TYPE>::FixedHeapArray(TYPE *values, int length)
...
bool FixedHeapArray<TYPE>::allocate(int length, bool canFail) {
  ...
  this->length = length;
  this->values =
      mmfx_new_array_opt(TYPE, length, canFail ? MMgc::kCanFail : MMgc::kNone);
...
bool FixedHeapArray<TYPE>::resize(int newLength, bool canFail) {
  ...
  this->values = mmfx_new_array_opt(TYPE, newLength,
                                    canFail ? MMgc::kCanFail : MMgc::kNone);
...
template <typename TYPE> void FixedHeapArray<TYPE>::deallocate() {
  if (values) {
...
template <class T, int TAG> void FixedHeapRef<T, TAG>::operator=(T *tNew) {
  set(tNew);
}

template <class T, int TAG> FixedHeapRef<T, TAG>::operator T *() const {
  return t;
}

template <class T, int TAG> T *FixedHeapRef<T, TAG>::operator->() const {
  return t;
}
...
template <class T, int TAG> void FixedHeapRef<T, TAG>::set(T *tNew) {
  if (tNew) {
  ...
  T *tOld = this->t;
  // inc/dec membars must force ordering, so that when t == tNew, rc doesn't go
  // to zero
  this->t = tNew;
  if (tOld) {
...
template <typename K, typename V> void FixedHeapHashTable<K, V>::Destroy() {
...
template <typename K, typename V> void FixedHeapHashTable<K, V>::Clear() {
  if (m_numItems) {
...
template <typename K, typename V> bool FixedHeapHashTable<K, V>::Expand() {
  if (!m_buckets) {
    return false;
  }
...
template <typename K, typename V>
void FixedHeapHashTable<K, V>::ForEach(
    TYPENAME_KLUDGE FixedHeapHashTable<K, V>::Iterator &iterator) {
  if (m_numItems) {
#ifdef DEBUG
    m_inIteration = true;
#endif
...
template <typename K, typename V>
/*virtual*/ typename FixedHeapHashTable<K, V>::Bucket *
FixedHeapHashTable<K, V>::BucketAlloc() {
...
template <typename K, typename V>
/*virtual*/ typename FixedHeapHashTable<K, V>::Bucket **
FixedHeapHashTable<K, V>::BucketArrayAlloc(size_t size) {
...
template <typename K, typename V>
/*virtual*/ bool FixedHeapHashTable<K, V>::KeysEqual(K key1,
                                                     const K key2) const {
  return key1 == key2;
}
