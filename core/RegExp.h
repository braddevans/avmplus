...
public:
  ...
  bool compile(RegExpSptrType pattern, int patternLength, int options,
               const char **errorptr, int *erroroffset,
               const unsigned char *tables);
  int exec(RegExpSptrType subject, int length, int start_offset, int options,
           RegExpSizeType &matchVector);
  int fullinfo(int what, void *where);
  int getString(const char *subject, RegExpSizeType &matchVector,
