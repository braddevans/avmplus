...
// greater than 4k off the starting pointer.  This will cause GetGC(this) to
// fail. WARNING: WARNING: WARNING:
...
void InlineHashtable::destroy() {
  ...
  if (atoms) {
    ...
    if (getAtomContainer()) {
      GC *gc = GC::GetGC(getAtomContainer());
      if (gc && gc->GetAttachedSampler()) {
        ((IMemorySampler *)gc->GetAttachedSampler())
            ->recordDeallocation(this, 0);
      }
    }
#endif
    // deliberately ignore the final two entries if hasIterIndex(), since they
    // aren't Atoms
...
// new entry.  Return false if we've simply updated the value of an existing
// entry. If a new entry is added, the caller is responsible expanding the table
// to maintain at least one free slot, and to prune any weak entries that are
// dead.
...
Atom InlineHashtable::get(Atom name) const {
  ...
  return atoms[i = find(name, atoms, getCapacity())] == name ? atoms[i + 1]
                                                             : undefinedAtom;
}
...
int InlineHashtable::find(Atom x, const Atom *t, uint32_t m) const {
  ...
  // this is a quadratic probe but we only hit even numbered slots since those
  // hold keys.
  int n = 7 << 1;
#ifdef _DEBUG
...
int InlineHashtable::rehash(const Atom *oldAtoms, int oldlen, Atom *newAtoms,
                            int newlen) const {
  int newSize = 0;
...
bool InlineHashtable::isFull() const {
  // This seems to work very well and the Intel compiler converts both the
  // multiplies into shift+add operations.
...
void InlineHashtable::grow(Toplevel *toplevel) {
  ...
  const uint32_t newCapacity = hasDeletedItems()
                                   ? oldCapacity
                                   : MathUtils::nextPowerOfTwo(oldCapacity + 1);
  ...
  // WE#2621977 -- the behavior of growing-during-enumeration has always been
  // ill-defined, but this change caused that to (effectively) terminate the
  // enumeration. This is a hack: if we have an iter index, copy it over to
  // preserve the previous oddball behavior. (srj)
...
// this function is slow -- however, for "normal", well-mannered ABC code, it
// should be called only once per iteration, at the start. (For handcrafted ABC
// it could be called repeatedly, which would dramatically slow such cases, but
// that's acceptable.)
int InlineHashtable::publicIterIndexToRealIndex(int publicIndex) {
  // keep in mind that a "public" index goes from 1...N (0 is the
  // start-and-finish value)
  AvmAssert(publicIndex > 0);

  // for "well-formed" (ie, generated with ASC or other reasonable compiler),
  // we should only end up calling this function once per iteration, with
  // publicIndex == 1 .... however, we don't assert here because it *is* legal
  // to call it with other values (in fact, our abcasm tests do so), but such
  // usage is now dramatically slower than it used to be. AvmAssert(publicIndex
  // == 1);
  ...
  for (int i = 0; i < cap; i += 2) {
    ...
    // don't need to check against EMPTY and DELETED since they won't be
    // kIntptrType
    MMGC_STATIC_ASSERT(atomKind(EMPTY) != kIntptrType);
    MMGC_STATIC_ASSERT(atomKind(DELETED) != kIntptrType);
    // don't need to check for dontEnumMask because it will make kIntptrType
    // appear as kDoubleType
  ...
  for (int i = 0; i < cap; i += 2) {
    Atom const a = atoms[i];
    if (a != EMPTY && a != DELETED && atomKind(a) != kIntptrType &&
        !(uintptr_t(a) & dontEnumMask)) {
      if (--publicIndex == 0)
        return i;
    }
  }
...
int InlineHashtable::next(int index) {
  ...
  if (!index) {
    ...
    // the first enumerable int atom (or first enumerable non-int if we have no
    // ints) in this case we ignore whatever is currently in iterIndices
    publicIndex = 1;
    ...
    // we'll never return an index larger than cap, but malicious ABC could try
    // to feed us one
    if (index > cap) {
    ...
    if (publicIndex == 0 || publicIndex != index) {
      // if publicIndex == 0, it's an uninitialized value (eg post-grow()),and
      // realIndex is garbage.
      //
    ...
    if (realIndex & kIterIndexIsIntptr) {
      ...
      for (realIndex += 2; realIndex < cap; realIndex += 2) {
        ...
        // don't need to check against EMPTY and DELETED since they won't be
        // kIntptrType. don't need to check for dontEnumMask because it will
        // make kIntptrType appear as kDoubleType
        if (atomKind(a) == kIntptrType) {
          realIndex |= kIterIndexIsIntptr;
          goto done;
        }
...
Atom InlineHashtable::keyAt(int i) {
  ...
  realIndex &= ~kIterIndexIsIntptr;
  return realIndex < cap ? removeDontEnumMask(atoms[realIndex])
                         : nullStringAtom;
}
...
/*virtual*/ HeapHashtableRC::~HeapHashtableRC() { ht.destroy(); }

/*virtual*/ HeapHashtable::~HeapHashtable() { ht.destroy(); }

/*virtual*/ int HeapHashtable::next(int index) { return ht.next(index); }

/*virtual*/ void HeapHashtable::add(Atom name, Atom value,
                                    Toplevel *toplevel /*=NULL*/) {
  ht.add(name, value, toplevel);
}

/*virtual*/ Atom HeapHashtable::get(Atom name) { return ht.get(name); }

/*virtual*/ Atom HeapHashtable::remove(Atom name) { return ht.remove(name); }

/*virtual*/ bool HeapHashtable::contains(Atom name) const {
  return ht.contains(name);
}

/*virtual*/ bool HeapHashtable::weakKeys() const { return false; }

/*virtual*/ bool HeapHashtable::weakValues() const { return false; }
...
/*virtual*/ Atom WeakKeyHashtable::get(Atom key) { return ht.get(getKey(key)); }
...
/*virtual*/ bool WeakKeyHashtable::weakKeys() const { return true; }
...
/*virtual*/ void WeakKeyHashtable::add(Atom key, Atom value,
                                       Toplevel *toplevel) {
...
/*virtual*/ void WeakValueHashtable::add(Atom key, Atom value,
                                         Toplevel *toplevel) {
...
/*virtual*/ bool WeakValueHashtable::weakValues() const { return true; }
