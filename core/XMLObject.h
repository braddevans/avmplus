...
protected:
  XMLObject(XMLClass *type, Stringp s = NULL,
            Namespace *defaultNamespace = NULL);
  XMLObject(XMLClass *type, E4XNode *node);

public:
  REALLY_INLINE static XMLObject *create(MMgc::GC *gc, XMLClass *type,
                                         Stringp s = NULL,
                                         Namespacep defaultNamespace = NULL) {
...
private:
  XMLClass *xmlClass() const { return toplevel()->xmlClass(); }
...
public:
  ...
  Atom AS3_setNotification(
      FunctionObject *f); // AS3 declaration sez this returns an Atom

  Atom AS3_localName();
  Atom AS3_name();
  Atom
  _namespace(Atom prefix,
             int argc); // prefix is optional (was previously called namespace)
  ...
  inline XMLObject *addNamespace(Atom _namespace) {
    return AS3_addNamespace(_namespace);
  }
  ...
  inline XMLListObject *child(Atom propertyName) {
    return AS3_child(propertyName);
  }
  ...
  inline XMLListObject *descendants(Atom PropertyName) const {
    return AS3_descendants(PropertyName);
  }
  ...
  inline Atom insertChildAfter(Atom child1, Atom child2) {
    return AS3_insertChildAfter(child1, child2);
  }
  inline Atom insertChildBefore(Atom child1, Atom child2) {
    return AS3_insertChildBefore(child1, child2);
  }
  ...
  inline ArrayObject *namespaceDeclarations() {
    return AS3_namespaceDeclarations();
  }
  ...
  inline XMLListObject *processingInstructions(Atom name) {
    return AS3_processingInstructions(name);
  }
  inline XMLObject *prependChild(Atom value) { return AS3_prependChild(value); }
  inline bool propertyIsEnumerable(Atom P) {
    return XML_AS3_propertyIsEnumerable(P);
  }
  inline XMLObject *removeNamespace(Atom _namespace) {
    return AS3_removeNamespace(_namespace);
  }
  inline XMLObject *replace(Atom propertyName, Atom value) {
    return AS3_replace(propertyName, value);
  }
  ...
  void __toXMLString(PrintWriter &s, NamespaceList &AncestorNamespace,
                     int indentLevel = 0, bool includeChildren = true) const;
  XMLObject *_deepCopy() const;

  Namespace *GenerateUniquePrefix(Namespace *ns,
                                  const NamespaceList &namespaces) const;

  static bool notifyNeeded(E4XNode *target);
  void issueNotifications(AvmCore *core, Toplevel *top, E4XNode *initialTarget,
                          Atom target, Stringp type, Atom value,
  ...
  Namespace *GetNamespace(const Multiname &mn,
...
public:
  ...
  PrintWriter &printUsingAncestors(PrintWriter &prw,
...
protected:
  ...
  QNameObject(QNameClass *type, Namespace *nameSpace, Atom name,
              bool bAttribute = false);

public:
  REALLY_INLINE static QNameObject *create(MMgc::GC *gc, QNameClass *cls,
                                           const Multiname &mn) {
    return new (gc, MMgc::kExact, cls->ivtable()->getExtraSize())
        QNameObject(cls, mn);
  }
  ...
  REALLY_INLINE static QNameObject *create(MMgc::GC *gc, QNameClass *cls,
                                           ...
                                           bool bAttribute = false) {
    return new (gc, MMgc::kExact, cls->ivtable()->getExtraSize())
        QNameObject(cls, attributeName, bAttribute);
  }

  REALLY_INLINE static QNameObject *create(MMgc::GC *gc, QNameClass *cls,
                                           Namespace *ns, Atom attributeName,
                                           bool bAttribute = false) {
    return new (gc, MMgc::kExact, cls->ivtable()->getExtraSize())
        QNameObject(cls, ns, attributeName, bAttribute);
  }
...
public:
  ...
  void getMultiname(Multiname &name) const { name = m_mn; }
