...
protected:
  ...
  RegExpObject(VTable *ivtable, ScriptObject *delegate, Stringp pattern,
               Stringp options);
...
public:
  REALLY_INLINE static RegExpObject *create(MMgc::GC *gc, RegExpClass *cls,
                                            RegExpObject *toCopy) {
    return new (gc, MMgc::kExact, cls->ivtable()->getExtraSize())
        RegExpObject(toCopy);
  }

  REALLY_INLINE static RegExpObject *create(MMgc::GC *gc, VTable *ivtable,
                                            ScriptObject *delegate) {
    return new (gc, MMgc::kExact, ivtable->getExtraSize())
        RegExpObject(ivtable, delegate);
  }
  ...
  REALLY_INLINE static RegExpObject *create(MMgc::GC *gc, VTable *ivtable,
    ...
    return new (gc, MMgc::kExact, ivtable->getExtraSize())
        RegExpObject(ivtable, delegate, pattern, options);
  }
...
private:
  ...
  ArrayObject *_exec(Stringp subject, StIndexableUTF8String &utf8Subject,
                     int startIndex, int &matchIndex, int &matchLen);

  void fixReplaceLastIndex(const char *src, int subjectLength, int matchLen,
                           int &newLastIndex, StringBuffer &resultBuffer);
