...
enum WordOpcode {
  ...
  WOP_lix8 = 0x135,
  WOP_lix16 = 0x136,
  WOP_float4 = 0x137, // Needs four arguments but does not pop any - an
                      // abomination brought on by LIR_ffff2f4 and writeNip()
...
struct WordOpcodeAttr {
  ...
  unsigned terminates : 1; // True if the instruction terminates control flow
                           // (return, throw)
  unsigned
      throws : 1; // True if the instruction may throw an exception (not
                  // including timer interrupts and other unhandleable errors)
  unsigned calls : 1;  // True if the last argument is arg_count
                       // An 'arg_count' number of extra parameters are pop from
                       // the stack
  unsigned pushes : 3; // Number of items pushed on the stack
  unsigned pops : 3;   // Number of items popped from the stack
  unsigned pops_extra : 1; // True if the instruction has extra stack pops,
                           // determined by verifier
