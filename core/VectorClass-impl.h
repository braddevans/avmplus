...
Atom TypedVectorClassBase::call(int argc, Atom *argv) {
  ...
  if (argc != 1)
    toplevel->throwArgumentError(kCoerceArgumentCountError,
  ...
  if (!AvmCore::isObject(argv[1]))
    toplevel->throwTypeError(kCheckTypeFailedError,
                             core->atomToErrorString(argv[1]),
                             core->toErrorString(itraits));
...
void TypedVectorClassBase::_forEach(Atom thisAtom, ScriptObject *callback,
                                    Atom thisObject) {
  return ArrayClass::generic_forEach(toplevel(), thisAtom, callback,
                                     thisObject);
}

bool TypedVectorClassBase::_every(Atom thisAtom, ScriptObject *callback,
                                  Atom thisObject) {
  return ArrayClass::generic_every(toplevel(), thisAtom, callback, thisObject);
}

bool TypedVectorClassBase::_some(Atom thisAtom, ScriptObject *callback,
                                 Atom thisObject) {
...
template <class OBJ>
Atom TypedVectorClass<OBJ>::createAndInitVectorFromObject(ScriptObject *so_args,
                                                          uint32_t len) {
  OBJ *v = newVector();
...
VectorBaseObject::VectorBaseObject(VTable *ivtable, ScriptObject *delegate)
    : ScriptObject(ivtable, delegate), m_vecClass(NULL), m_fixed(false) {}

VectorBaseObject *VectorBaseObject::_newVector() {
  AvmAssert(m_vecClass != NULL);
  Atom args[1] = {nullObjectAtom};
  return (VectorBaseObject *)AvmCore::atomToScriptObject(
      m_vecClass->construct(0, args));
}
...
Atom VectorBaseObject::_mapImpl(ScriptObject *callback, Atom thisObject,
                                VectorBaseObject *r, uint32_t len) {
...
Atom VectorBaseObject::_filterImpl(ScriptObject *callback, Atom thisObject,
                                   VectorBaseObject *r, uint32_t len) {
  AvmCore *core = this->core();
  if (callback) {
    ScriptObject *d = this;
...
TypedVectorObject<TLIST>::TypedVectorObject(VTable *ivtable,
...
bool TypedVectorObject<TLIST>::hasAtomProperty(Atom name) const {
  ...
  if (status == kValidNumber) {
    ...
    if (core()->currentBugCompatibility()->bugzilla456852b)
      ...
      // Prior to SWF11, isNumber was incorrectly set false for negative
      // numbers, so we fall through and allow searching the prototype chain for
      // these. See getAtomProperty() below.
      return false;
  }

  // status == kNotNumber, or index is negative and compatibility is earlier
  // than SWF11.
...
void TypedVectorObject<TLIST>::setAtomProperty(Atom name, Atom value) {
  ...
  if (status == kValidNumber) {
    setUintProperty(index, value);
             ...
             core()->currentBugCompatibility()->bugzilla456852b) {
    throwRangeError_a(name);
  } else {
    toplevel()->throwReferenceError(kWriteSealedError, core()->string(name),
                                    traits());
  }
}
...
Atom TypedVectorObject<TLIST>::getAtomProperty(Atom name) const {
  ...
  if (status == kValidNumber) {
    ...
    if (core()->currentBugCompatibility()->bugzilla456852b) {
      ...
      // Prior to SWF11, isNumber was incorrectly set false for negative
      // numbers.
      toplevel()->throwReferenceError(kReadSealedError, core()->string(name),
                                      traits());
      return undefinedAtom; // not reached
    }
  }
  ...
  // status == kNotNumber, or index is negative and compatibility is earlier
  // than SWF11. Check the prototype chain - that will throw if there is no
  // match.
...
void TypedVectorObject<TLIST>::set_length(uint32_t newLength) {
  checkFixed();
  // this is what the old code did, so we'll do it too: set the capacity
  // explicitly if we grow
...
template <class TLIST> Atom TypedVectorObject<TLIST>::nextName(int index) {
  AvmAssert(index > 0);
...
template <class TLIST> Atom TypedVectorObject<TLIST>::nextValue(int index) {
  AvmAssert(index > 0);
  if (uint32_t(index) <= m_list.length()) {
...
template <class TLIST> int TypedVectorObject<TLIST>::nextNameIndex(int index) {
  if (uint32_t(index) < m_list.length()) {
    return index + 1;
  } else {
    return 0;
...
template <class TLIST>
Atom TypedVectorObject<TLIST>::_filter(ScriptObject *callback,
                                       Atom thisObject) {
  TypedVectorObject<TLIST> *r = (TypedVectorObject<TLIST> *)_newVector();
  uint32_t const len = m_list.length();
...
template <class TLIST>
void TypedVectorObject<TLIST>::_spliceHelper_so(uint32_t insertPoint,
                                                uint32_t insertCount,
                                                uint32_t deleteCount,
                                                ScriptObject *so_args,
                                                uint32_t offset) {
  ...
  // passing NULL for args inserts the proper space, but leaves the inserted
  // entries empty
...
template <class TLIST>
void TypedVectorObject<TLIST>::_spliceHelper(uint32_t insertPoint,
                                             uint32_t insertCount,
                                             uint32_t deleteCount, Atom args,
                                             uint32_t offset) {
  if (AvmCore::istype(args, vtable->traits)) {
    TypedVectorObject<TLIST> *vec_args =
        (TypedVectorObject<TLIST> *)AvmCore::atomToScriptObject(args);
    m_list.splice(insertPoint, insertCount, deleteCount, vec_args->m_list,
                  offset);
...
template <class TLIST>
void TypedVectorObject<TLIST>::_splice(uint32_t insertPoint,
                                       uint32_t insertCount,
                                       uint32_t deleteCount, const Atom *argv) {
  // passing NULL for args inserts the proper space, but leaves the inserted
  // entries empty
  m_list.splice(insertPoint, insertCount, deleteCount, NULL);
  for (uint32_t i = 0; i < insertCount; i++) {
...
template <class TLIST>
void TypedVectorObject<TLIST>::AS3_insertAt(int32_t index,
                                            typename TLIST::TYPE element) {
  // Throw exception for fixed Vector.  Note that splice() fails to do so,
  // contrary to documentation. This is a bug, which we choose not to propagate
  // here.  Fixing splice() will require versioning.
  checkFixed();
  // Convert index argument to unsigned index into the underlying list. Negative
  // values count backwards from last element. Resulting list index is clamped
  // to interval (0, length), so it can never be out-of-bounds.
...
void TypedVectorObject<AtomList>::AS3_insertAt(int32_t index, Atom element) {
  ...
  AvmAssert(m_vecClass != NULL);
  Atom value =
      avmplus::coerce(toplevel(), element, m_vecClass->getTypeTraits());
...
typename TLIST::TYPE TypedVectorObject<TLIST>::AS3_removeAt(int32_t index) {
  ...
  checkFixed();
  // Convert index argument to unsigned index into the underlying list. Negative
  // values count backwards from last element. Resulting list index must point
  // to valid element, otherwise out-of-bounds.
  ...
  // Curiously, vec.splice(n, 1) for empty vector v does not throw, instead
  // returning an empty vector, but vec.removeAt(n) is defined to be equal to
  // vec.splice(n, 1)[0], so we will throw in this case as well.  The diagnostic
  // message, however, will reflect the actual index, however, rather than 0.
...
typename TLIST::TYPE TypedVectorObject<TLIST>::AS3_pop() {
  checkFixed();
  return m_list.isEmpty() ? (typename TLIST::TYPE)TypedVectorConstants<
                                typename TLIST::OPAQUE_TYPE>::undefinedValue()
                          : m_list.removeLast();
}
...
void TypedVectorObject<TLIST>::AS3_pop(typename TLIST::TYPE *retval) {
  ...
  *retval = m_list.isEmpty()
                ? (typename TLIST::TYPE)TypedVectorConstants<
                      typename TLIST::OPAQUE_TYPE>::undefinedValue()
                : m_list.removeLast();
}
#endif
...
template <class TLIST>
REALLY_INLINE typename TLIST::TYPE
TypedVectorObject<TLIST>::_getNativeIntProperty(int32_t index_i) const {
  AvmAssertMsg(!IS_FLOAT4_TYPE(typename TLIST::TYPE),
               "wrong _getNativeIntProperty helper used for Vector.<float4>");
  uint32_t const index = checkReadIndex_i(index_i);
  return m_list.get(index);
}

template <class TLIST>
void TypedVectorObject<TLIST>::_setNativeIntProperty(
    int32_t index_i, typename TLIST::TYPE value) {
  AvmAssertMsg(!IS_FLOAT4_TYPE(typename TLIST::TYPE),
               "wrong _setNativeIntProperty helper used for Vector.<float4>");
...
#ifdef VMCFG_AOT
template <class TLIST>
NO_INLINE void TypedVectorObject<TLIST>::_setNativeIntPropertySlow(
    int32_t index_i, typename TLIST::TYPE value) {
...
template <class TLIST>
typename TLIST::TYPE
TypedVectorObject<TLIST>::_getNativeUintProperty(uint32_t index) const {
  AvmAssertMsg(!IS_FLOAT4_TYPE(typename TLIST::TYPE),
               "wrong _getNativeUIntProperty helper used for Vector.<float4>");
  checkReadIndex_u(index);
  return m_list.get(index);
}

template <class TLIST>
void TypedVectorObject<TLIST>::_setNativeUintProperty(
    uint32_t index, typename TLIST::TYPE value) {
  AvmAssertMsg(!IS_FLOAT4_TYPE(typename TLIST::TYPE),
               "wrong _setNativeUIntProperty helper used for Vector.<float4>");
...
template <class TLIST>
typename TLIST::TYPE
TypedVectorObject<TLIST>::_getNativeDoubleProperty(double index_d) const {
  AvmAssertMsg(
      !IS_FLOAT4_TYPE(typename TLIST::TYPE),
      "wrong _getNativeDoubleProperty helper used for Vector.<float4>");
  uint32_t const index = checkReadIndex_d(index_d);
  return m_list.get(index);
}

template <class TLIST>
void TypedVectorObject<TLIST>::_setNativeDoubleProperty(
    double index_d, typename TLIST::TYPE value) {
  AvmAssertMsg(
      !IS_FLOAT4_TYPE(typename TLIST::TYPE),
      "wrong _setNativeDoubleProperty helper used for Vector.<float4>");
...
#ifdef VMCFG_FLOAT
template <class TLIST>
void TypedVectorObject<TLIST>::_getFloat4IntProperty(
    typename TLIST::TYPE *result, int32_t index_i) const {
  AvmAssertMsg(IS_FLOAT4_TYPE(typename TLIST::TYPE),
               "wrong _getNativeIntProperty helper used for Vector.<*>");
  uint32_t const index = checkReadIndex_i(index_i);
  if (result)
...
template <class TLIST>
void TypedVectorObject<TLIST>::_setFloat4IntProperty(
    int32_t index_i, const typename TLIST::TYPE &value) {
  AvmAssertMsg(IS_FLOAT4_TYPE(typename TLIST::TYPE),
               "wrong _setNativeIntProperty helper used for Vector.<*>");
  uint32_t const index = checkWriteIndex_i(index_i);
  m_list.set(index, value);
}

template <class TLIST>
void TypedVectorObject<TLIST>::_getFloat4UintProperty(
    typename TLIST::TYPE *result, uint32_t index) const {
  AvmAssertMsg(IS_FLOAT4_TYPE(typename TLIST::TYPE),
               "wrong _getNativeUIntProperty helper used for Vector.<*>");
  checkReadIndex_u(index);
  if (result)
    *result = m_list.get(index);
  return;
...
template <class TLIST>
void TypedVectorObject<TLIST>::_setFloat4UintProperty(
    uint32_t index, const typename TLIST::TYPE &value) {
  AvmAssertMsg(IS_FLOAT4_TYPE(typename TLIST::TYPE),
               "wrong _setNativeUIntProperty helper used for Vector.<*>");
  checkWriteIndex_u(index);
  m_list.set(index, value);
}

template <class TLIST>
void TypedVectorObject<TLIST>::_getFloat4DoubleProperty(
    typename TLIST::TYPE *result, double index_d) const {
  AvmAssertMsg(IS_FLOAT4_TYPE(typename TLIST::TYPE),
               "wrong _getNativeDoubleProperty helper used for Vector.<*>");
  uint32_t const index = checkReadIndex_d(index_d);
  if (result)
    *result = m_list.get(index);
  return;
...
template <class TLIST>
void TypedVectorObject<TLIST>::_setFloat4DoubleProperty(
    double index_d, const typename TLIST::TYPE &value) {
  AvmAssertMsg(IS_FLOAT4_TYPE(typename TLIST::TYPE),
               "wrong _setNativeDoubleProperty helper used for Vector.<*>");
  uint32_t const index = checkWriteIndex_d(index_d);
  m_list.set(index, value); // needed to compile for most types; hopefully the
                            // useless call will be optimized-away.
}
#endif
...
template <class TLIST>
void TypedVectorObject<TLIST>::_setKnownUintProperty(uint32_t index,
                                                     Atom value) {
  typename TLIST::OPAQUE_TYPE tmp;
...
template <class TLIST>
void TypedVectorObject<TLIST>::_setKnownIntProperty(int32_t index_i,
                                                    Atom value) {
  typename TLIST::OPAQUE_TYPE tmp;
  atomToValueKnown(value, tmp);
  uint32_t index = checkWriteIndex_i(index_i);
...
template <class TLIST>
void TypedVectorObject<TLIST>::_setKnownDoubleProperty(double index_d,
                                                       Atom value) {
  typename TLIST::OPAQUE_TYPE tmp;
  atomToValueKnown(value, tmp);
  uint32_t index = checkWriteIndex_d(index_d);
...
template <class TLIST>
void FASTCALL
TypedVectorObject<TLIST>::throwRangeError_u(uint32_t index) const {
  toplevel()->throwRangeError(kOutOfRangeError, core()->uintToString(index),
                              core()->uintToString(m_list.length()));
}

template <class TLIST>
void FASTCALL TypedVectorObject<TLIST>::throwRangeError_i(int32_t index) const {
  toplevel()->throwRangeError(kOutOfRangeError, core()->intToString(index),
                              core()->uintToString(m_list.length()));
}

template <class TLIST>
void FASTCALL TypedVectorObject<TLIST>::throwRangeError_a(Atom index) const {
  toplevel()->throwRangeError(kOutOfRangeError, core()->string(index),
                              core()->uintToString(m_list.length()));
}
