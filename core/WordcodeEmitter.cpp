...
WordcodeEmitter::WordcodeEmitter(MethodInfo *info, Toplevel *avm_toplevel)
    : WordcodeTranslator(), info(info), core(info->pool()->core),
      avm_toplevel(avm_toplevel), backpatches(NULL), labels(NULL),
      exception_fixes(NULL), buffers(NULL), buffer_offset(0), spare_buffer(NULL)
#ifdef VMCFG_DIRECT_THREADED
      ,
      opcode_labels(interpGetOpcodeLabels())
#endif
      ,
      pool(NULL), code_start(NULL), exceptions_consumed(false), dest(NULL),
      dest_limit(NULL) {
  AvmAssert(info != NULL);

  const uint8_t *pos = info->abc_body_pos();
  AvmCore::skipU32(pos, 5); // max_stack, local_count, init_scope_depth,
                            // max_scope_depth, code_length
  code_start = pos;
  pool = info->pool();
  boot();
}
...
WordcodeEmitter::WordcodeEmitter(AvmCore *core, uint8_t *code_start)
    : WordcodeTranslator(), info(NULL), core(core), backpatches(NULL),
      labels(NULL), exception_fixes(NULL), buffers(NULL), buffer_offset(0),
      spare_buffer(NULL)
#ifdef VMCFG_DIRECT_THREADED
      ,
      opcode_labels(interpGetOpcodeLabels())
#endif
      ,
      pool(NULL), code_start(code_start), exceptions_consumed(false),
      dest(NULL), dest_limit(NULL) {
  boot();
}
...
WordcodeEmitter::~WordcodeEmitter() { cleanup(); }
...
void WordcodeEmitter::emitRelativeOffset(uintptr_t base_offset,
                                         const uint8_t *base_pc,
                                         intptr_t offset) {
  if (offset < 0) {
    ...
    while (l != NULL && l->old_offset != old_offset)
      l = l->next;
    // See https://bugzilla.mozilla.org/show_bug.cgi?id=481171.  Verifier should
    // have caught the invalid target.
    AvmAssert(l != NULL);
...
void WordcodeEmitter::makeAndInsertBackpatch(const uint8_t *target_pc,
...
void WordcodeEmitter::computeExceptionFixups() {
  ...
  ExceptionHandlerTable *new_table =
...
void WordcodeEmitter::fixExceptionsAndLabels(const uint8_t *pc) {
#ifdef VMCFG_WORDCODE_PEEPHOLE
  // Do not optimize across control flow targets, so flush the peephole window
  // here
  ...
  while (backpatches != NULL && backpatches->target_pc <= pc) {
    ...
    *backpatches->patch_loc = (intptr_t)buffer_offset +
                              (intptr_t)(dest - buffers->data) -
                              (intptr_t)backpatches->patch_offset;
...
void WordcodeEmitter::writePrologue(const FrameState *, const uint8_t *pc,
                                    CodegenDriver *) {
#if defined DEBUGGER
  if (core->debugger())
    emitOp0(pc, WOP_debugenter);
#else
  (void)pc;
#endif
  computeExceptionFixups();
...
void WordcodeEmitter::writeOpcodeVerified(const FrameState *, const uint8_t *,
                                          AbcOpcode) {}

void WordcodeEmitter::writeFixExceptionsAndLabels(const FrameState *,
                                                  const uint8_t *pc) {
  fixExceptionsAndLabels(pc);
}

void WordcodeEmitter::writeOp1(const FrameState *state, const uint8_t *pc,
...
void WordcodeEmitter::writeMethodCall(const FrameState *, const uint8_t *pc,
...
void WordcodeEmitter::writeOp2(const FrameState *, const uint8_t *pc,
...
void WordcodeEmitter::write(const FrameState *, const uint8_t *pc,
                            AbcOpcode opcode, Traits *) {
...
// These take one U30 argument, and the argument is explicitly passed here
...
void WordcodeEmitter::emitRelativeJump(const uint8_t *pc, WordOpcode opcode) {
  ...
  *dest++ = NEW_OPCODE(opcode);
  ...
  peep(opcode, dest - 2);
  AvmAssert(state ==
            0); // Never allow a jump instruction to be in the middle of a match
#endif
}

void WordcodeEmitter::emitLabel(const uint8_t *pc) {
#ifdef VMCFG_WORDCODE_PEEPHOLE
  // Do not optimize across control control flow targets, so flush the peephole
  // window here.
  peepFlush();
#endif
...
#ifdef VMCFG_WORDCODE_PEEPHOLE
  // Avoid a lot of hair by flushing before LOOKUPSWITCH and not peepholing
  // after.
  peepFlush();
#endif
...
//
//
// DESCRIPTION.  This is a deterministic state machine driven peephole
// optimizer.  The machine attempts to match a set of patterns against a prefix
// of the instruction stream, replacing matched patterns by new strings of
// instructions (usually shorter or faster ones) -- a "reduction".  A reduction
// is possible when the machine enters a final state.  However, the machine is
// greedy and may leave the final state looking for a longer match.  As the
// longer match may fail, the machine maintains a stack of final states it may
// backtrack to.  A match may fail in two ways, either because a state is
// reached from which there is no move to a final state on the actual input, or
// if a final state is reached but the guard condition for the state is not
// satisfied.  The guard is only tested when the machine is ready to commit; for
// that reason, a stack of backtrack states is required (instead of a single
// backtrack state).  The guard is mixed in with the commit code in order to
// keep code size down, though it probably does not matter much.
//
// Instructions inserted by reductions are not subject to repeated matching
// (though they could be).
//
// In order to handle patterns that are subpatterns but not prefixes of other
// patterns, a special failure transition is encoded in each state and used if
// the matcher fails at that state.
//
// The peephole optimizer function peep() /must/ be called every time an
// instruction has been emitted to the instruction stream, as the state machine
// in the peephole optimizer tracks the emitted instruction stream (it does not
// inspect it repeatedly). The operands to peep() are the symbolic opcode that
// was just emitted and the address at which that opcode was emitted.  The
// function peep() drives the state machine.
//
// If optimization must not cross some instruction boundary (for example it must
// not merge the instruction at a control flow target with the previous
// instruction) then peepFlush() must be called before instructions are emitted
// for the point beyond the boundary.  This must also be called at the end of
// the instruction stream, and for instructions that must not appear in the
// peephole window (currently only LOOKUPSWITCH).
//
// The tables 'states', 'transitions', and 'toplevel' are generated by the
// program utils/peephole.as based on the patterns described in
// core/peephole.tbl, which are in turn hand-selected with aid of the dynamic
// instruction profiling infrastructure built into Tamarin - see comments in
...
//
// where toplevel_index is computed from the opcode, maybe worth simplifying
// that in order to make this test faster.  Anyhow, if the test is true then
// peep() need not be called as there will not be a state transition.  This
// factoid may be useful if emitOp0, emitOp1, and emitOp2 are in-lined into the
// verifier.
//
//
// OPTIMALITY.  We wish to establish that if there is a stream of instructions
// s1, s2, ... and patterns p1, p2, ... then (a) if some p matches a prefix of
// the stream then the algorithm finds the match; (b) if several p match a
// prefix of the stream then (one of) the longest p is chosen, subject to guard
// evaluation; (c) the properties (a) and (b) hold for the remainder of the
// stream when the instructions matching the pattern are removed from the
// stream; and (d) the properties (a) and (b) hold for the remainder of the
// stream if no patterns match the prefix and the first instruction is removed
// from the stream.
//
// Note that the optimality result - if established - does not imply that the
// "best" match (the one that produces the fewest instructions, or the one that
// minimizes stack pointer updates, or maximizes intra-instruction optimization
// in the C compiler, or is best according to any criteria like that) is found,
// since a best match can sometimes be obtained by choosing a shorter match
// against a prefix in order to make a longer match possible subsequently.  It
// only implies that we do as well as we can with a greedy matching strategy.
//
// Sketch of proof of optimality:
//
// To establish (a) and (b), note that the automaton tracks the prefix of the
// instruction stream and stacks all accepting states.  If it reaches a state
// without a successor (whether it's successor-less or the input has no
// transition from the current state) then the stacked states are tried in
// longest-first (reverse) order, and the longest match wins.  (The failure
// state is not visited if there are accepting stacked states, and does not
// interfere.)
//
// To establish (c), note that the automaton consumes exactly the instructions
// that were matched, then sets state=0 and re-emits the non-matched
// instructions, running the automaton by calling peep() for each non-matched
// instruction.  If this succeeds then (a) and (b) apply; if it fails, (d)
// applies.  Either way the automaton eventually consumes all of the originally
// non-matching instructions, after which it is once again driven by
// instructions pushed into the peephole window.
//
// To establish (d), note that the automaton makes a failure transition if no
// match could be made, and that the failure transition by construction (see
// utils/peephole.as) discards the fewest possible instructions from the prefix
// of the stream: those that will not match any pattern, given that the present
// pattern failed.  Thus matching will start at the earlies possible point, and
// (a), (b), (c), or (d) apply.
//
// Then, observe that peepFlush() commits to the longest match possible at the
// time it is called, and then skips the remaining instructions.  By definition,
// the skipped instructions cannot be matched (or they would have been part of
// the "longest match possible").  Thus peepFlush() preserve (a) and (b).
//
// Finally, observe that peepFlush() is called following the last instruction
// emitted in order to commit to the longest prefix of the instructions still in
// the peephole window.

void WordcodeEmitter::peepInit() { state = 0; }

// Replace old instructions with new words of code.  This is tail called from
// the generated commit() function (see peephole.cpp).
//
// Invariants here:
//
...
//     be the positive absolute ABC uint8_t offset of the branch target; a
//     backpatch structure will be created in the latter case.

bool WordcodeEmitter::replace(uint32_t old_instr, uint32_t new_words,
                              bool jump_has_been_translated) {
  ...
  // a reserve (there can only ever be one free).  We know I[nextI-1] points
  // into the current buffer, so check if I[0] is between the start of the
  // buffer and the last instruction.
  ...
  if (!(buffers->data <= I[0] && I[0] <= I[nextI - 1])) {
    ...
    dest_limit =
        buffers->data + sizeof(buffers->data) / sizeof(buffers->data[0]);
    buffer_offset -= buffers->entries_used;
  }
  dest = I[0];
      ...
      *dest++ = R[i++];
      ...
      if (offset >= 0) {
        ...
        makeAndInsertBackpatch(
            code_start + offset,
            uint32_t(buffer_offset + (dest + (width - 1) - buffers->data)));
      } else {
        // Backward jump
        // Compute new jump offset
        *dest = -int32_t(buffer_offset + (dest + (width - 1) - buffers->data) +
                         offset);
        dest++;
      }
      if (width >= 3)
...
void WordcodeEmitter::undoRelativeOffsetInJump() {
  ...
  if (offset == 0x80000000U) {
    ...
    if (b2 == NULL)
      ...
      b2->next = b->next;
    // b is unlinked
    // Install the ABC uint8_t offset from the backpatch structure (will be
    // positive)
    ...
    I[nextI - 1][1] = -int32_t(buffer_offset + (dest - buffers->data) +
                               (int32_t)I[nextI - 1][1]);
  }
}
...
void WordcodeEmitter::peep(uint32_t opcode, uintptr_t *loc) {
  ...
  // The transition lists can get quite long for popular instructions like
  // GETLOCAL; binary search if it that might be profitable.
...
void WordcodeEmitter::shiftBuffers(uint32_t shift) {
  ...
  nextI -= shift;
  backtrack_idx =
      0; // We could do better if we knew how many final states to discard
}
