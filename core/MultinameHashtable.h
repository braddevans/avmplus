...
// acceptable go GCC.
template <class VALUE_TYPE> class Quad {
public:
  Stringp name;
  Namespacep ns;
  VALUE_TYPE value;
  ...
  // 'multiNs' is 1 if the given name exists elsewhere with a different
  // namespace. 'api' is the minimum API version.
  //
  ...
  REALLY_INLINE uint32_t multiNS() const {
    return (uint32_t)(apiAndMultiNS & 1);
  }
  REALLY_INLINE ApiVersion apiVersion() const {
    return (ApiVersion)(apiAndMultiNS >> 1);
  }

  // return true iff a lookup with the given ns is compatible with the one in
  // this Quad.
...
public:
  ...
  uint32_t padding;
#endif
  // Now it should at least be word-aligned.  FIXME - do we want better than
  // that?
...
class MultinameHashtable : public MMgc::GCTraceableObject {
  template <class MNHT> friend class StMNHTIterator;
...
private:
   ...
   * quads.
   */
  static int find(Stringp name, Namespacep ns, const Quad<VALUE_TYPE> *t,
                  unsigned tLen);
  void rehash(const Quad<VALUE_TYPE> *oldAtoms, int oldlen,
              Quad<VALUE_TYPE> *newAtoms, int newlen);
...
public:
   ...
   */
  REALLY_INLINE static MultinameHashtable *
  create(MMgc::GC *gc, int capacity = kDefaultCapacity) {
...
protected:
  ...
  // ------------------------ DATA SECTION BEGIN
private:
  QuadContainer<VALUE_TYPE>
      *m_quads; // property hashtable (written with explicit WB)
public:
  int size; // no. of properties
public:
  int numQuads; // size of hashtable
  // ------------------------ DATA SECTION END
};
...
typedef MultinameHashtable<MethodInfo *, GCObjectType>
    MultinameMethodInfoHashtable;
...
//      }
//
template <class MNHT> class StMNHTIterator {
private:
  MNHT *const volatile m_mnht; // kept just to ensure it doesn't get collected
                               // -- must be volatile!
