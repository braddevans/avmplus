...
ScriptObject *Toplevel::toPrototype(Atom atom) {
  if (!AvmCore::isNullOrUndefined(atom)) {
    ...
    // TypeError in ECMA
    throwTypeError((atom == undefinedAtom) ? kConvertUndefinedToObjectError
    ...
    case kObjectType:
      ...
      if (AvmCore::isQName(attributeName)) {
        ...
        if (q->isAttr())
          ...
          return QNameObject::create(core->GetGC(), qnameClass(), attributeName,
                                     true);
      } else {
...
void Toplevel::CoerceE4XMultiname(const Multiname *m, Multiname &out) {
  ...
  if (m->isQName()) {
    ...
    if (m->isAnyNamespace()) {
      ...
      // For an unqualified reference, we need to add in the default xml
      // namespace since we did not find a match for it in our namespace set
      if (!bMatch) {
        int newNameCount = m->namespaceCount() + 1;
        NamespaceSet *nsset =
            NamespaceSet::_create(core->GetGC(), newNameCount);
        for (int i = 0, n = m->namespaceCount(); i < n; i++) {
...
Atom Toplevel:: instanceof (Atom atom, Atom ctor) {
  ...
  // check for null before tryign to call toPrototype(atom), which will throw an
  // error for null.
...
Atom Toplevel::in_operator(Atom nameatom, Atom obj) {
  ...
  if (!AvmCore::isDictionaryLookup(nameatom, obj)) {
    if (avmplus::atomIsIntptr(nameatom) && avmplus::atomCanBeUint32(nameatom)) {
#ifdef VMCFG_FLOAT
      ...
      return o->hasUintProperty((uint32_t)atomGetIntptr(nameatom)) ? trueAtom
                                                                   : falseAtom;
    }
    ...
    // ISSUE should we try this on each object on the proto chain or just the
    // first?
    TraitsBindingsp td = t->getTraitsBindings();
    // NOTE looking for a name that matches the caller's version, so use
    // caller's public
...
// Keep in sync with related methods.  See comments at Toplevel::getproperty.
bool Toplevel::hasproperty(Atom obj, const Multiname *multiname,
                           VTable *vtable) {
...
// Keep in sync with related methods.  See comments at Toplevel::getproperty.
bool Toplevel::deleteproperty(Atom obj, const Multiname *multiname,
                              VTable *vtable) const {
  const Binding propBinding = getBinding(vtable->traits, multiname);
  bool result = false;
  switch (AvmCore::bindingKind(propBinding)) {
 ...
 */
Atom Toplevel::getproperty(Atom obj, const Multiname *multiname,
                           VTable *vtable) {
  ...
  case BKIND_NONE:
    if (atomKind(obj) == kObjectType) {
      ...
      // converted to string, so we cannot distinguish between 1.5 and "1.5",
      // yet the float4 spec requires that we disallow the former and allow the
      // latter.
      //
      // The decision is not local: the JIT will sometimes call
      // getpropertylate_d if the property is a double, but other times call
      // getproperty (or other helpers); the interpreter on the other hand calls
      // getproperty in all cases when it does not call getUintProperty directly
      // on the object.
      //
      // Of course, if the argument is a float then the jit will go through
      // getproperty, yet we must handle 1f as well as we handle 1.
      //
      // For now, then, we handle strings that parse as integer
      // (getIndexFromAtom is actually too restrictive, because it caps the
      // index value at 2^32-2, but at least it can be applied uniformly) and
      // let other strings pass through to prototype lookup.  This is not
      // correct but it's a minor issue in practice.
      ...
      if (AvmCore::isFloat4(obj)) {
        if (multiname->isValidDynamicName()) {
          ...
          if (AvmCore::getIndexFromAtom(multiname->getName()->atom(), &index)) {
            ...
            throwRangeError(kOutOfRangeError, core()->uintToString(index),
                            core()->uintToString(4));
          }
        }
      }
      ...
      if (multiname->isValidDynamicName()) {
        ...
        return delegate->ScriptObject::getStringPropertyFromProtoChain(
            multiname->getName(), delegate, toTraits(obj));
      } else {
...
void Toplevel::setproperty(Atom obj, const Multiname *multiname, Atom value,
                           VTable *vtable) const {
  Binding b = getBinding(vtable->traits, multiname);
...
// Keep in sync with related methods.  See comments at Toplevel::getproperty.
void Toplevel::setproperty_b(Atom obj, const Multiname *multiname, Atom value,
                             VTable *vtable, Binding b) const {
  switch (AvmCore::bindingKind(b)) {
  case BKIND_METHOD: {
    if (AvmCore::isXMLorXMLList(obj) &&
        multiname->containsAnyPublicNamespace()) {
  ...
  case BKIND_NONE: {
    if (AvmCore::isObject(obj)) {
      ...
      // obj represents a primitive Number, Boolean, float, float4, int, or
      // String, and primitives are sealed and final.  Cannot add dynamic vars
      // to them.
...
Binding Toplevel::getBindingAndDeclarer(Traits *traits, const Multiname &ref,
...
Stringp Toplevel::decodeURI(ScriptObject *self, Stringp uri) {
  ...
  if (!out) {
    self->toplevel()->uriErrorClass()->throwError(
        kInvalidURIError, core->toErrorString("decodeURI"));
  }
  return out;
}
...
Stringp Toplevel::decodeURIComponent(ScriptObject *self, Stringp uri) {
  ...
  if (!out) {
    self->toplevel()->uriErrorClass()->throwError(
        kInvalidURIError, core->toErrorString("decodeURIComponent"));
  }
  return out;
}
...
Stringp Toplevel::encodeURI(ScriptObject *self, Stringp uri) {
  ...
  if (!out) {
    self->toplevel()->uriErrorClass()->throwError(
        kInvalidURIError, core->toErrorString("encodeURI"));
  }
  return out;
}
...
Stringp Toplevel::encodeURIComponent(ScriptObject *self, Stringp uri) {
  ...
  if (!out) {
    self->toplevel()->uriErrorClass()->throwError(
        kInvalidURIError, core->toErrorString("encodeURIComponent"));
  }
  return out;
}

bool Toplevel::isNaN(ScriptObject *, double n) { return MathUtils::isNaN(n); }

/**
 * isFinite(number) in section 15.1.2.5 of ecma 262 edition 3
 *
 * Applies ToNumber to argument then returns false if the result is NaN,
 * Negative Infinity, or Positive Infinity, and true otherwise
 *
 * Special case - if isFinite is called with no args, should return false based
 * on section 10.1.3 that states if a function is called with less arguments
 * then params, the variables get assigned 'undefined'.  ToNumber(undefined)
 * returns NaN
 *
...
Atom Toplevel::eval(ScriptObject *self, Atom input) {
  ...
  if (AvmCore::isString(input)) {
    ...
    NativeInitializer *nativeInitializer =
        NULL; // "native" not supported for eval code
    ...
    return core->handleActionSource(newsrc, filename, toplevel,
                                    nativeInitializer, codeContext, apiVersion);
  }
#endif // VMCFG_EVAL
  return undefinedAtom;
}
...
Stringp Toplevel::encode(AvmCore *core, Stringp in,
  ...
  while (len--) {
    ...
    if (contains(uriUnescaped, ch) ||
        (!encodeURIComponentFlag && contains(uriReservedPlusPound, ch))) {
      out << (char)ch;
      src++;
    } else {
      ...
      if (ch >= 0xD800 && ch < 0xDC00) {
        ...
        if (bugzilla609416) {
          ...
          src += 2;
          len--; // need to reduce the len to account for the surrogate byte we
                 // just read
        } else {
          V = (ch - 0xD800) * 0x400 + (src[1] - 0xDC00) * 0x10000;
          src += 2;
...
Stringp Toplevel::decode(AvmCore *core, Stringp in,
                         bool decodeURIComponentFlag) {
  StringIndexer chars(in);
  int length = in->length();
  wchar *out = (wchar *)core->GetGC()->Alloc(
      length * 2 + 1, 0,
      MMgc::kStringObjectContentPartition); // decoded result is at most length
                                            // wchar chars long
  int outLen = 0;
 ...
 */
const uint32_t Toplevel::uriUnescaped[] = {0x00000000, 0x03ff6782, 0x87fffffe,
                                           0x47fffffe};
 ...
 */
const uint32_t Toplevel::uriReservedPlusPound[] = {0x00000000, 0xac009858,
 ...
 */
const uint32_t Toplevel::unescaped[] = {0x00000000, 0x03ffec00, 0x87ffffff,
                                        0x07fffffe};
...
void Toplevel::throwVerifyError(int id, Stringp arg1, Stringp arg2,
                                Stringp arg3) const {
...
void Toplevel::throwTypeErrorWithName(int id, const char *namestr) const {
  ...
  Multiname name(core()->findPublicNamespace(),
                 core()->internString(core()->newStringLatin1(namestr)));
  throwTypeError(id, core()->toErrorString(&name));
}

void Toplevel::throwError(int id) const { errorClass()->throwError(id); }
...
void Toplevel::throwRangeError(int id, Stringp arg1, Stringp arg2,
                               Stringp arg3) const {
  rangeErrorClass()->throwError(id, arg1, arg2, arg3);
}

void Toplevel::throwReferenceError(int id, const Multiname *multiname,
                                   const Traits *traits) const {
  referenceErrorClass()->throwError(id, core()->toErrorString(multiname),
                                    core()->toErrorString((Traits *)traits));
}
...
void FASTCALL Toplevel::throwIOError(int id) { ioErrorClass()->throwError(id); }
...
void Toplevel::throwReferenceError(int id, String *name,
                                   const Traits *traits) const {
...
// virtual
bool Toplevel::sampler_trusted(ScriptObject * /*sampler*/) { return true; }

// Toplevel::bugzilla implements a Script, not a Method, so "self" is a plain
// ScriptObject, not a Toplevel
/*static*/ bool Toplevel::bugzilla(ScriptObject *self, int32_t bug) {
  const BugCompatibility *bugCompatibility =
      self->core()->currentBugCompatibility();

  // This is ad-hoc, we only need to introduce cases here for
  // bugs that are actually worked around in AS3 code.
#define HANDLE_BUG(n)                                                          \
  case n:                                                                      \
    return bugCompatibility->bugzilla##n;
...
//  -------------------------------------------------------
uint32_t Toplevel::charsetToCodepage(String * /*charset*/) { return 0; }

String *Toplevel::readMultiByte(uint32_t /*codepage*/, uint32_t /*length*/,
                                DataInput * /*input*/) {
  AvmAssert(0);
  return core()->kEmptyString;
}

void Toplevel::writeMultiByte(uint32_t /*codepage*/, String * /*str*/,
                              DataOutput * /*output*/) {
  AvmAssert(0);
}
...
void Toplevel::writeObject(ObjectEncoding encoding, DataOutput *output,
                           Atom a) {
  if (encoding >= kAMF3) {
...
void Toplevel::addAliasedClassClosure(Atom name, Atom context, ClassClosure *cc,
                                      bool isDomainEnv) {
  ...
  if (val == undefinedAtom) {
    ...
    val = AvmCore::genericObjectToAtom(
        WeakValueHashtable::create(core()->GetGC()));
    map->add(name, val);
  }

  map = (HeapHashtable *)AvmCore::atomToGenericObject(
      val); // the DomainEnv => ClassClosure WeakValueHashtable
  val = cc->atom();
  Atom key = context;
  if (isDomainEnv) {
...
Atom Toplevel::getClassClosureAtomFromAlias(Atom name,
                                            bool checkContextDomainOnly) {
  GCRef<HeapHashtable> map = _aliasToClassClosureMap;
  Atom val = map->get(name);
  if (val == undefinedAtom)
  ...
  // or we didn't find a match. So we fall back to old-school rules which did
  // not distinguish between DomainEnv's and do the lookup with
  // key=(name,'this') which picks up the most recent registration (see
  // registerClassByAlias)
...
Stringp Toplevel::getAliasFromTraits(Traitsp traits) {
  ...
  // By making using genericObjectToAtom the write barrier will be applied
  // correctly
...
ClassClosure *Toplevel::getClassClosureFromAlias(Stringp name) {
  ...
  Atom val = getClassClosureAtomFromAlias(name->atom(),
                                          /*checkContextDomainOnly*/ false);
  if (val != undefinedAtom) {
...
/*static*/ void Toplevel::registerClassAlias(ScriptObject *script,
                                             String *aliasName,
                                             ClassClosure *cc) {
  ...
  if (core->internString(aliasName) == core->kEmptyString) {
    toplevel->argumentErrorClass()->throwError(
        kEmptyStringError, core->toErrorString("aliasName"));
  }

  // Registration should normally occur in the toplevel where the class is
  // defined namely, cc->ivtable()->base->toplevel(), otherwise the caller may
  // bind the class from one toplevel to another toplevel.   We didn't prevent
  // this in prior players and there are some convincing use cases for this so
  // we continue to let it occur.
  // if (toplevel != cc_tl) {
  //    toplevel->argumentErrorClass()->throwError("can't register a class
  //    outside your ApplicationDomain family")
  //}
  core = (AvmCore *)cc->core();
  Atom name = core->internString(aliasName)->atom();
  Atom val = toplevel->getClassClosureAtomFromAlias(
      name,
      /*checkContextDomainOnly*/ true); // restrict lookup to current DomainEnv
  if (val != undefinedAtom) {
    // remove traits mapping , we don't need to remove name mapping since we'll
    // overwrite it
    ClassClosure *reg_cc = (ClassClosure *)AvmCore::atomToScriptObject(val);
    toplevel->_traitsToAliasMap.remove(
        AvmCore::genericObjectToAtom(reg_cc->traits()->itraits));
  }

  // add key=(name,domainEnv) value=cc tuple to table.
  toplevel->addAliasedClassClosure(
      name,
      AvmCore::genericObjectToAtom(toplevel->getDomainEnvOverridableHook()), cc,
      /*isDomainEnv*/ true);
  toplevel->_traitsToAliasMap.add(
      AvmCore::genericObjectToAtom(cc->traits()->itraits), name);

  // add key=(name,'this') value=cc tuple to be used when code context is null
  // (see getClassClosureAtomFromAlias)
  toplevel->addAliasedClassClosure(name, AvmCore::genericObjectToAtom(toplevel),
                                   cc, /*isDomainEnv*/ false);
}

DomainEnv *Toplevel::getDomainEnvOverridableHook() { return domainEnv(); }

/*static*/ ClassClosure *Toplevel::getClassByAlias(ScriptObject *script,
                                                   String *aliasName) {
  AvmCore *core = (AvmCore *)script->core();
  Toplevel *toplevel = (Toplevel *)script->toplevel();
  ...
  if (aliasName == NULL) {
    toplevel->argumentErrorClass()->throwError(
        kNullArgumentError, core->toErrorString("aliasName"));
  }

  String *ialiasName = core->internString(aliasName);
  Atom atom = toplevel->getClassClosureAtomFromAlias(
      ialiasName->atom(), /*checkContextDomainOnly*/ false);
  ...
  toplevel->referenceErrorClass()->throwError(kClassNotFoundError,
                                              core->toErrorString(&multiname));
  return NULL; // just to make the compiler happy.
}

void Toplevel::internObject(const FixedHeapRCObject *rep,
...
GCRef<ScriptObject>
Toplevel::getInternedObject(const FixedHeapRCObject *rep) const {
  Atom key = (Atom)((intptr_t)rep | kIntptrType);
  Atom value = _isolateInternedObjects->get(key);
  if (value == undefinedAtom) {
    return NULL;
