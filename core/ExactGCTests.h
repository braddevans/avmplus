...
//
// The file is compiled and compilation errors will be signaled for various
// kinds of problems.  Additionally, visual inspection of the generated tracing
// code is recommended. That is not ideal, but it's better than nothing.
//
// These tests are for corner cases / hard cases, not for mainstream cases -
// those are tested well enough by all the Tamarin and Flash Player code that's
// annotated
...
public:
  ...
  // Testing if, elif, else.  When compiling, we'll hit the third case,
  // !CASE3NEG. For more elaborate testing we could set up something that hits
  // each of the arms in separate compilations (or through multiple inclusions
  // of this file with different type names, maybe)
  ...
  ScriptObject *GC_POINTER(case1_and_3_w); // From above; we allow dupes in
                                           // different arms of the switch
#else
