...
#ifdef AVMPLUS_ARM
#ifdef _MSC_VER
#define RETURN_METHOD_PTR(_class, _method) return *((int *)&_method);
#else
...
#define RETURN_METHOD_PTR(_class, _method)                                     \
  ...
  return foo;
#else
#define RETURN_METHOD_PTR(_class, _method) return *((intptr_t *)&_method);
#endif
...
#define VECTORDOUBLEADDR(f)                                                    \
  vectorDoubleAddr((int (DoubleVectorObject::*)())(&f))
#define VECTORFLOATADDR(f) vectorFloatAddr((int (FloatVectorObject::*)())(&f))
#define VECTORFLOAT4ADDR(f)                                                    \
  vectorFloat4Addr((int (Float4VectorObject::*)())(&f))
...
// There is an optimization issue with Xcode 6.3 clang that makes AIR crash at
// launch deep in the VM This source code file is the culprit and the following
// #pragma clang optimize solve the problem.
#if defined(__clang__)
#pragma clang optimize off
#endif // defined __clang__

intptr_t coreAddr(int (AvmCore::*f)()) { RETURN_METHOD_PTR(AvmCore, f); }

#if defined(__clang__)
#pragma clang optimize on
#endif // defined __clang__

intptr_t gcAddr(int (MMgc::GC::*f)()) { RETURN_METHOD_PTR(MMgc::GC, f); }

intptr_t envAddr(int (MethodEnv::*f)()) { RETURN_METHOD_PTR(MethodEnv, f); }

#ifdef DEBUGGER
intptr_t debuggerAddr(int (Debugger::*f)()) { RETURN_METHOD_PTR(Debugger, f); }
#endif /* DEBUGGER */
...
intptr_t stringAddr(int (String::*f)()) { RETURN_METHOD_PTR(String, f); }
...
static const MopsInfo kMopsStoreInfo[5] = {{1, LIR_sti2c, FUNCTIONID(mop_si8)},
...
private:
  ...
  static void extractConstantDisp(LIns *&mopAddr, int32_t size,
                                  int32_t &curDisp);
  LIns *safeIns2(LOpcode op, LIns *, int32_t);
  void safeRewrite(LIns *ins, int32_t);

public:
  MopsRangeCheckFilter(LirWriter *out, LirWriter *prolog_out,
                       LIns *env_domainenv);

  LIns *emitRangeCheck(LIns *&mopAddr, int32_t const size, int32_t *disp,
                       LIns *&br);
...
inline MopsRangeCheckFilter::MopsRangeCheckFilter(LirWriter *out,
                                                  LirWriter *prolog_out,
                                                  LIns *env_domainenv)
    : LirWriter(out), prolog_out(prolog_out), env_domainenv(env_domainenv),
      curMemBase(NULL), curMemSize(NULL), curMopAddr(NULL),
      curRangeCheckLHS(NULL), curRangeCheckRHS(NULL),
...
// These definitions assume 2's complement arithmetic, as we do pervasively in
// avmplus.
...
/*static*/ void MopsRangeCheckFilter::extractConstantDisp(LIns *&mopAddr,
                                                          int32_t size,
                                                          int32_t &curDisp) {
  ...
  for (;;) {
    ...
    // accumulated displacement to wrap, don't extract this and further
    // displacements.
...
LIns *MopsRangeCheckFilter::emitRangeCheck(LIns *&mopAddr, int32_t const size,
                                           int32_t *disp, LIns *&br) {
  int32_t offsetMin = 0;
  if (disp != NULL) {
    *disp = 0;
  ...
  if (curRangeCheckLHS != NULL && curMopAddr == mopAddr) {
    ...
    if (differenceFitsInInt32(n_curRangeCheckMax, n_curRangeCheckMin) &&
        ((n_curRangeCheckMax - n_curRangeCheckMin) <=
         DomainEnv::GLOBAL_MEMORY_MIN_SIZE)) {
      // Rewrite the previously-emitted check to encompass the wider range
      // required to cover the present case as well.  This is valid because the
      // mop addresss is denoted by the same expression as before, and thus must
      // necessarily have the same value.  Only a widening of the displacement
      // range (taking the access size into account) can cause an access which
      // passed the most recently emitted check to be invalid here.  By widening
      // the earlier check, we cause the failure to appear earlier.  Note that
      // this leads to imprecise exception behavior for the mops RangeError
      // exception, as it may be thrown earlier than we'd expect from the naive
      // semantics.  We do not hoist exceptions above branch targets, however.

      if (curRangeCheckMinValue != n_curRangeCheckMin)
        safeRewrite(curRangeCheckLHS, n_curRangeCheckMin);

      if ((n_curRangeCheckMax - n_curRangeCheckMin) !=
          (curRangeCheckMaxValue - curRangeCheckMinValue))
      ...
      // range-checking code can always assume the range is within
      // GLOBAL_MEMORY_MIN_SIZE. This guarantees, for example, that the
      // curRangeCheckRHS expression always evaluates to a non-negative number,
      // so that negative values of curRangeCheckLHS, when treated as unsigned,
      // compare larger than the RHS as required.
      flushRangeChecks();
    }
  } else {
    flushRangeChecks();
  ...
  if (!curMemBase) {
    ...
    curMemBase = out->insLoad(LIR_ldp, env_domainenv,
                              offsetof(DomainEnv, m_globalMemoryBase),
                              ACCSET_OTHER, LOAD_VOLATILE);
    curMemSize = out->insLoad(LIR_ldi, env_domainenv,
                              offsetof(DomainEnv, m_globalMemorySize),
                              ACCSET_OTHER, LOAD_VOLATILE);
  }
  ...
  if (!curRangeCheckLHS) {
    ...
    //
    //      (curMopAddr+curRangeCheckMin >= 0 && curMopAddr+curRangeCheckMax <=
    //      mopsMemorySize)
    //
    // which is the same as
    //
    //      (curMopAddr >= -curRangeCheckMin && curMopAddr <= mopsMemorySize -
    //      curRangeCheckMax)
    //
    // which is the same as
    //
    ...
    //      curRangeCheckMax + 1)
    //
    // and since (x >= min && x < max) is equivalent to (unsigned)(x-min) <
    // (unsigned)(max-min)
    //
    //      (unsigned(curMopAddr + curRangeCheckMin) < unsigned(mopsMemorySize -
    //      curRangeCheckMax + 1 + curRangeCheckMin))
    //
    // from there, you'd think you could do
    //
    //      (curMopAddr < mopsMemorySize - curRangeCheckMax + 1))
    //
    // but that is only valid if you are certain that curMopAddr>0, due to the
    // unsigned casting... and curMopAddr could be anything, which is really the
    // point of this whole exercise. Instead, settle for
    //
    //      (unsigned(curMopAddr + curRangeCheckMin) <= unsigned(mopsMemorySize
    //      - (curRangeCheckMax-curRangeCheckMin)))
    //
    ...
    curRangeCheckRHS = safeIns2(LIR_subi, curMemSize,
                                curRangeCheckMaxValue - curRangeCheckMinValue);
...
CodegenLIR::CodegenLIR(MethodInfo *i, MethodSignaturep ms, Toplevel *toplevel,
                       OSR *osr_state, JITNoise *noise)
    : LirHelper(i->pool()), info(i), ms(ms), toplevel(toplevel),
      pool(i->pool()), osr(osr_state), driver(NULL), state(NULL),
      mopsRangeCheckFilter(NULL), restArgc(NULL), restLocal(-1),
      interruptable(true), npe_label("npe"), upe_label("upe"),
      interrupt_label("interrupt"),
      mop_rangeCheckFailed_label("mop_rangeCheckFailed"), catch_label("catch"),
      call_error_label("call_error"), inlineFastpath(false),
      call_cache_builder(*alloc1, *initCodeMgr(pool)),
      get_cache_builder(*alloc1, *pool->codeMgr),
      set_cache_builder(*alloc1, *pool->codeMgr), prolog(NULL), skip_ins(NULL),
      specializedCallHashMap(NULL), builtinFunctionOptimizerHashMap(NULL),
      blockLabels(NULL), cseFilter(NULL), noise(noise),
      jit_debug_info(NULL)
  ...
  verbose_only(if (pool->isVerbose(VB_jit, i)) {
    core->console << "codegen " << i;
    core->console << " required=" << ms->requiredParamCount() << " optional="
                  << (ms->param_count() - ms->requiredParamCount()) << "\n";
  })
}

CodegenLIR::~CodegenLIR() { cleanup(); }

void CodegenLIR::cleanup() { LirHelper::cleanup(); }
...
asm int CodegenLIR::setjmpDummy(jmp_buf buf) { b __setjmp; }
...
bool isNullable(Traits *t) {
  ...
  return bt != BUILTIN_int && bt != BUILTIN_uint && bt != BUILTIN_boolean &&
         bt != BUILTIN_number FLOAT_ONLY(&&bt != BUILTIN_float &&
                                         bt != BUILTIN_float4);
}

/**
 * Eliminates redundant loads within a block, and tracks the nullability of
 * pointers within blocks and across edges.  CodegenLIR will inform VarTracker
 * that a pointer is not null by calling setNotNull(ptr, type) either when the
 * Verifier's FrameState.FrameValue is not null, in localGetp(), or after a null
 * check in emitNullCheck().
 *
 * Within a block, we track nullability of references to instructions; when
 * references are copied, we know the copies are not null.
 *
 * At block boundaries, different values may flow together, so we track
 * nullability in variable slots instead of specific instruction references.
 */
class VarTracker : public LirWriter {
  ...
  int restLocal; // -1 or, if it's lazily allocated, the local holding the rest
                 // array

  // false after an unconditional control flow instruction (jump, throw,
  // return), true from the start and after we start a block via trackLabel()
  bool reachable;
...
public:
  VarTracker(MethodInfo *info, Allocator &alloc, LirWriter *out, int nvar,
             int scopeBase, int stackBase, int restLocal, uint32_t code_len)
      : LirWriter(out), alloc(alloc), vars(NULL), tags(NULL), nvar(nvar),
        varShift(VARSHIFT(info)), scopeBase(scopeBase), stackBase(stackBase),
        restLocal(restLocal), reachable(true), has_backedges(false)
#ifdef DEBUGGER
        ,
...
#endif
  {
    ...
    // allocate a large value until
    // https://bugzilla.mozilla.org/show_bug.cgi?id=565489 is resolved
  ...
  bool hasBackedges() const { return has_backedges; }
  ...
  bool isNotNull(LIns *ins) { return checked->containsKey(ins); }

  void initNotNull(const FrameState *state) { syncNotNull(notnull, state); }
  ...
  void syncNotNull(nanojit::BitSet *bits, const FrameState *state) {
    ...
    if (state->targetOfBackwardsBranch || state->targetOfExceptionBranch) {
      ...
      // This is done at both targets of backward branches and at exception
      // handlers, as we cannot be assured of having seen all branches, thus the
      // varTracker info may not be complete.  This limitation arises from the
      // one-pass algorithm.
      for (int i = 0, n = nvar; i < n; i++) {
        const FrameValue &v = state->value(i);
        bool stateNotNull = v.notNull && isNullable(v.traits);
        if (!stateNotNull || (i >= scopeTop && i < stackBase) ||
            (i >= stackTop))
          bits->clear(i);
        else
          bits->set(i);
  ...
  void trackForwardEdge(CodegenLabel &target) {
    AvmAssert(target.labelIns ==
              NULL); // illegal to call trackForwardEdge on backedge

    // Merge varTracker/tagTracker state with state at target label.
    // Due to hidden internal control flow in exception dispatch, state may not
    // be propagated across exception edges.  Thus, if a label may be reached
    // from such an edge, we cannot determine accurate state at the label, and
    // must clear it. The state will remain cleared due to the monotonicity of
    // the merge.
  ...
  void printNotNull(nanojit::BitSet *bits, const char *title,
    ...
    if (0 && verbose) {
      if (bits) {
        ...
        printf("%s [0-%d,%d-%d] notnull = ", title, max_scope + 1, stackBase,
               max_stack - 1);
  ...
  void checkBackEdge(CodegenLabel &target, const FrameState *state) {
    ...
    if (target.notnull) {
      ...
      for (int i = 0, n = nvar; i < n; i++) {
        ...
        //  true      true      true
        bool currentNotNull =
            (varTracker[i] ? isNotNull(varTracker[i]) : false) ||
            notnull->get(i);
  ...
  void trackLabel(CodegenLabel &label, const FrameState *state) {
    ...
    // Exception branches are assumed to be backward, as lowering to LIR may
    // have introduced backward control flow that was not present in the
    // original ABC.
    if (!state->targetOfBackwardsBranch && label.varTracker &&
        !state->targetOfExceptionBranch) {
  ...
  // correctly must be done at the verifier level, and at that level, it must
  // always be done or never be done (can't be conditional on debugging).
  ...
  void trackTagStore(LIns *value, int i) { tagTracker[i] = value; }

  // The first time we see a load from variable i, remember it,
  // and if we know that slot is nonnull, add the load instruction to the
  // nonnull set.
  ...
  void trackTagLoad(LIns *value, int i) { tagTracker[i] = value; }
  ...
  LIns *ins0(LOpcode op) {
    AvmAssert(
        op !=
        LIR_label); // trackState must be called directly to generate a label.
  ...
  // set reachable = false after LIR_jtbl which has explicit targets for all
  // cases
  ...
  // assume any non-pure function can throw an exception, and that pure
  // functions cannot.
  bool canThrow(const CallInfo *call) { return !call->_isPure; }
  ...
  LIns *insCall(const CallInfo *call, LIns *args[]) {
    if (haveDebugger && canThrow(call))
      clearVarState(); // debugger might have modified locals, so make sure we
                       // reload after call.
    if (call->_address == (uintptr_t)&restargHelper) {
      // That helper has a by-reference argument which points into the vars
      // array
...
LIns *CodegenLIR::localGetp(int i) {
  ...
  if (exactlyOneBit(v.sst_mask)) {
    // pointer or atom
    AvmAssert(
        !(v.sst_mask == (1 << SST_int32) && v.traits == INT_TYPE) &&
        !(v.sst_mask == (1 << SST_uint32) && v.traits == UINT_TYPE) &&
        !(v.sst_mask == (1 << SST_bool32) && v.traits == BOOLEAN_TYPE) &&
        FLOAT_ONLY(
            !(v.sst_mask == (1 << SST_float) && v.traits == FLOAT_TYPE) &&
            !(v.sst_mask == (1 << SST_float4) &&
              v.traits == FLOAT4_TYPE) &&) !(v.sst_mask == (1 << SST_double) &&
                                             v.traits == NUMBER_TYPE));
    ...
    // more than one representation is possible: convert to atom using tag found
    // at runtime.
  ...
  // verifier believes the slot is non-null.  This is valid from the ABC
  // standpoint, as the rest arg array will be created on demand if the slot is
  // accessed, and the null sentinel can never be observed.  We do not want to
  // infer that 'ins' is itself non-null in this case, however, as it may be
  // used elsewhere.
...
public:
  ...
  PrologWriter(LirWriter *out)
      : LirWriter(out), lastIns(NULL), env_scope(NULL), env_vtable(NULL),
        env_abcenv(NULL), env_domainenv(NULL), env_toplevel(NULL),
        env_finddef_table(NULL) {}

  virtual LIns *ins0(LOpcode v) { return lastIns = out->ins0(v); }
  virtual LIns *ins1(LOpcode v, LIns *a) { return lastIns = out->ins1(v, a); }
  ...
  virtual LIns *insImmI(int32_t imm, bool tainted) {
    return lastIns = out->insImmI(imm, tainted);
  }
#ifdef AVMPLUS_64BIT
  virtual LIns *insImmQ(uint64_t imm, bool tainted) {
    return lastIns = out->insImmQ(imm, tainted);
  }
#endif
  virtual LIns *insImmD(double d, bool tainted) {
    return lastIns = out->insImmD(d, tainted);
  }
  virtual LIns *insImmF(float f, bool tainted) {
    return lastIns = out->insImmF(f, tainted);
  }
  virtual LIns *insImmF4(const float4_t &f, bool tainted) {
    return lastIns = out->insImmF4(f, tainted);
  }
  virtual LIns *insLoad(LOpcode op, LIns *base, int32_t d, AccSet accSet,
                        LoadQual loadQual) {
    return lastIns = out->insLoad(op, base, d, accSet, loadQual);
  }
  virtual LIns *insStore(LOpcode op, LIns *value, LIns *base, int32_t d,
                         AccSet accSet) {
...
#ifdef DEBUG
FUNCTION(FUNCADDR(OSR::checkBugCompatibility), SIG1(V, P),
         osr_check_bugcompatibility)
#endif
...
// that provides access to the environment for this function and all vm
// services.
//
...
// arguments are packed.  doubles are sizeof(double), everything else is
// sizeof(Atom).
//
// Distinguished locals:
//
// methodFrame (LIR_allocp, MethodFrame*) is the current MethodFrame.  in the
// prolog we push this onto the call stack pointed to by
// AvmCore::currentMethodFrame, and in the epilog we pop it back off.
//
// coreAddr (LIR_immi|LIR_immq) constant address of AvmCore*.  used in lots of
// places. undefConst (LIR_immi|LIR_immq) constant value = undefinedAtom. used
// all over.
//
// vars (LIR_allocp) storage for ABC stack frame variables.  8 bytes per
// variable, always, laid out according to ABC param/local var numbering.  The
// total number is local_count + scope_depth + stack_depth, i.e. enough for the
// whole ABC frame. values at any given point in the jit code are are
// represented according to the statically known type of the variable at that
// point in the code.  (the type and representation may change at different
// points.  CodegenDriver maintains the known static types of variables and
// exposes them via FrameState.
//
// tags (LIR_allocp) SlotStorageType of each var in vars, one byte per variable.
//
// The contents of vars+tags are up-to-date at all labels and debugging safe
// points. Inbetween those points, the contents are stale; the JIT optimizes
// away stores and loads in straightline code.  Additional dead stores are
// elided by deadvars_analyze() and deadvars_kill().
//
...
// used by the debugger and also used for constructing human-readable stack
// traces.
//
// Locals for Exception-handling, only present when method has try/catch blocks:
//
// _save_eip (LIR_allocp, intptr_t) storage for the current ABC-based "pc", used
// by exception handling to determine which catch blocks are in scope.  The
// value is an ABC instruction offset, which is how catch handler records are
// indexed.
//
// _ef (LIR_allocp, ExceptionFrame) an instance of struct ExceptionFrame,
// including a jmp_buf holding our setjmp() state, a pointer to the next outer
// ExceptionFrame, and other junk.
//
// setjmpResult (LIR_call, int) result from calling setjmp; feeds a conditional
// branch that surrounds the whole function body; logic to pick a catch handler
// and jump to it is compiled after the function body.  if setjmp returns a
// nonzero result then we jump forward, pick a catch block, then jump backwards
// to the catch block.
//
...
void CodegenLIR::writePrologue(const FrameState *state, const uint8_t *pc,
  ...
  // analysis algorithms such as deadvars() that are stressed by the
  // introduction of many new labels and branches.  Our heuristic seeks to
  // confine the fastpath optimization to an "envelope" for we have good
  // empirical data, without attempting to establish its true outer limits.
  // Additionally, from first principles, we know that specific characteristics
  // of the flow graph are more relevant than length alone, and even a count of
  // the labels and branches would likely give a more precise result than
  // overall code lenth. The preferred long-term solution is to perform actual
  // acconting for JIT resource consumption (memory and time), and aborting when
  // reasonable limits are exceeded.
  //
  if (core->config.jitconfig.opt_inline) {
    if (codeLength < 50000 && framesize < 1000) {
      inlineFastpath = true;
    } else {
      // core->console << "disabling inline fastpaths, frame size " << framesize
  ...
  verbose_only(vbNames = 0; if (verbose()) {
    ...
    prolog_buf->printer = vbNames;
      ...
      debug_only(lirout = validate2 = new (*alloc1) ValidateWriter(
                     lirout, prolog_buf->printer, "writePrologue(prologue)");)
          verbose_only(vbWriter = 0;
                       if (verbose()) lirout = vbWriter = new (*alloc1)
                           VerboseWriter(*alloc1, lirout, vbNames,
                                         &pool->codeMgr->log, "PROLOG");)
              prolog = new (*alloc1) PrologWriter(lirout);
  redirectWriter = lirout = new (*lir_alloc) LirWriter(prolog);
  if (core->config.njconfig.cseopt)
    lirout = cseFilter = new (*alloc1)
        CseFilter(lirout, TR_NUM_USED_ACCS, *alloc1, core->config.njconfig);
  ...
  if (haveDebugger) {
    checker = new (*alloc1)
        DebuggerCheck(core, *alloc1, lirout, dbg_framesize, VARSHIFT(info));
    lirout = checker;
  }
#endif // DEBUG
#endif // DEBUGGER
  ...
  lirout = varTracker = new (*alloc1)
      VarTracker(info, *alloc1, lirout, framesize, ms->scope_base(),
                 ms->stack_base(), restLocal, codeLength);
  ...
  if (core->samplerEnabled) {
    LIns *sampleTicks = loadIns(LIR_ldi, offsetof(AvmCore, sampleTicks),
                                coreAddr, ACCSET_OTHER);
  ...
  LIns *currentMethodFrame = loadIns(
      LIR_ldp, offsetof(AvmCore, currentMethodFrame), coreAddr, ACCSET_OTHER);
  // save env in MethodFrame.envOrCodeContext
  //     explicitly leave IS_EXPLICIT_CODECONTEXT clear
  //     explicitly leave DXNS_NOT_NULL clear, dxns is effectively null without
  //     doing the store here.
  ...
  stp(InsConstPtr((void *)(uintptr_t)0xdeadbeef), methodFrame,
      offsetof(MethodFrame, dxns), ACCSET_OTHER);
#endif
  ...
  vars = insAlloc(
      framesize << VARSHIFT(
          info)); // room for double|Atom|int|pointer, and float4 if used
  ...
  verbose_only(if (prolog_buf->printer) {
    ...
    prolog_buf->printer->lirNameMap->addName(tags, "tags");
      ...
      debug_only(void **extras = new (*alloc1) void *[2]; extras[0] = vars;
                 extras[1] = tags; validate1->setCheckAccSetExtras(extras);
                 validate2->setCheckAccSetExtras(extras);)

      // stack overflow check - use methodFrame address as comparison
      LIns *d =
          loadIns(LIR_ldp, offsetof(AvmCore, minstack), coreAddr, ACCSET_OTHER);
  ...
  if (haveDebugger) {
    ...
    verbose_only(if (vbNames) { vbNames->lirNameMap->addName(csn, "csn"); })
  }
  ...
  if (info->hasOptional()) {
    ...
    for (int i = 0, n = optional_count; i < n; i++) {
      ...
      LIns *defaultVal = InsConstAtom(ms->getDefaultValue(i), /*tainted*/ true);
  ...
  // and the flag that determines whether to use the unconsed or the consed
  // array, are represented as follows:
  //
  //  - The unconsed array restArg is represented indirectly via ap_param and
  //    rest_offset.
  //  - The argument count restArgc is a LIR expression of type uint32 computed
  //  from
  //    argc_param and param_count.
  //  - The rest parameter local is an array, it is either null (no rest array
  //    consed yet) or a raw array pointer, so it doubles as the flag.  The
  //    offset of this variable is 1+param_count.
  //
  // The rest parameter local is passed by reference to restargHelper, which
  // may update it.
  //
  // The difference between a ...rest argument and an arguments array are that
  // in the former case,
  //
  ...
  if (info->needRest()) {
    if (info->lazyRest()) {
      ...
      LIns *rest = callIns(FUNCTIONID(createRestHelper), 3, env_param,
                           argc_param, apArg);
    ...
    if (info->lazyRest()) {
      ...
      LIns *arguments = callIns(FUNCTIONID(createArgumentsHelper), 3, env_param,
                                argc_param, apArg);
  ...
  debug_only(body = validate3 = new (*alloc1)
                 ValidateWriter(body, vbNames, "writePrologue(body)");
             validate3->setCheckAccSetExtras(extras);)
      verbose_only(if (verbose()) {
        AvmAssert(vbNames != NULL);
        body_buf->printer = vbNames;
        body = vbWriter = new (*alloc1)
            VerboseWriter(*alloc1, body, vbNames, &pool->codeMgr->log);
  ...
  if (driver->hasReachableExceptions()) {
    ...
    // If (setjmp() != 0) goto catch dispatcher, which we generate in the
    // epilog. Note that register contents following setjmp return via longjmp
    // are not predictable.
...
void CodegenLIR::emitOsrBranch() {
  ...
  callIns(FUNCTIONID(osr_adjust_frame), 4, methodFrame,
          haveDebugger ? csn : InsConstPtr(0), vars, tags);
...
#endif
      LIns *defaultVal = jit->InsConstAtom(value, /*tainted*/ true);
...
void CodegenLIR::emitDebugEnter() {
  ...
  callIns(FUNCTIONID(debugEnter), 5, env_param, tags, csn, vars,
...
void CodegenLIR::copyParam(int i, int &offset) {
  ...
  case BUILTIN_float:
    ...
    offset += sizeof(Atom); // float is 4 bytes; but on x64, it takes up 8 bytes
                            // too, just like any int.
    break;
  case BUILTIN_float4:
...
void CodegenLIR::emitGetscope(int scope_index, int dest) {
  ...
  LIns *scopeobj = loadIns(
      LIR_ldp, offsetof(ScopeChain, _scopes) + scope_index * sizeof(Atom),
      scope, ACCSET_OTHER, LOAD_CONST);
...
void CodegenLIR::emitKill(int i) { localSet(i, undefConst, NULL); }

#ifdef VMCFG_FLOAT
void CodegenLIR::emitNumericOp2(int32_t op1, int32_t op2,
                                const LIREmitter &emitIns) {
  static_assert(kIntptrType == 6,
                "You can't change kIntptrType just like that");
  static_assert(kDoubleType == 7,
                "You can't change kDoubleType just like that");

  const FrameValue &v1 = state->value(op1);
  (void)v1;
  const FrameValue &v2 = state->value(op2);
  (void)v2;
  AvmAssert(v1.traits == OBJECT_TYPE && v2.traits == OBJECT_TYPE &&
            v1.notNull && v2.notNull);
  ...
  LIns *tag =
      andp(binaryIns(LIR_andp, val1, val2), AtomConstants::kAtomTypeMask);
  // kIntptrType
  suspendCSE();
  /* NOTE: to make the fast path faster, we're playing with the bits to quickly
     determine if "both are double", "at least one is not Number" and "both are
     int" We rely on the Atom TAG scheme, i.e. "kIntPtrType is 6" and
     "kDoubleType is 7", see static asserts above.
  */
  ...
  LIns *xormask =
      andp(binaryIns(LIR_xorp, val1, val2), AtomConstants::kAtomTypeMask);

  branchToLabel(LIR_jf, eqp0(xormask), int_op_number);
  // both_integers:
  stp(nativeToAtom(emitIns(BUILTIN_number,
                           p2dIns(rshp(val1, AtomConstants::kAtomTypeSize)),
  ...
  stp(nativeToAtom(
          emitIns(BUILTIN_number,
                  ldd(subp(val1, AtomConstants::kDoubleType), 0, ACCSET_OTHER),
                  p2dIns(rshp(val2, AtomConstants::kAtomTypeSize))),
          NUMBER_TYPE),
      result, 0, ACCSET_OTHER);
...
void CodegenLIR::emitNumericOp1(int32_t op1, const LIREmitter &emitIns) {
  const FrameValue &v = state->value(op1);
  (void)v;
  ...
  stp(nativeToAtom(emitIns(BUILTIN_number,
                           p2dIns(rshp(val, AtomConstants::kAtomTypeSize))),
                   NUMBER_TYPE),
      result, 0, ACCSET_OTHER);
  ...
  stp(nativeToAtom(emitIns(BUILTIN_number,
                           ldd(val, -AtomConstants::kDoubleType, ACCSET_OTHER)),
                   NUMBER_TYPE),
      result, 0, ACCSET_OTHER);
...
// Branch to fallback label if rhs is not intptr, or result cannot be so
// represented due to overflow.
//
// 64-bit:
//
// FAST_ADD_INT_TO_ATOM(lhs, rhs, result, fallback)
//    if ((rhs & kAtomTypeMask) != kIntptrType) goto fallback;   # punt if atom
//    argument rhs is not intptr intptr_t lhsExtended = i; # extend int argument
//    lhs to atom size intptr_t lhsShifted = lhsExtended << (kAtomTypeSize+7);
//    # align lhs with 54-bit payload of rhs (see next step) intptr_t rhsShifted
//    = rhs << 7;                            # left-justify 54-bit payload of
//    rhs (note rhs is tagged) intptr_t sumShifted = lhsShifted + rhsShifted; #
//    add aligned values, producing 57-bit tagged sum, left-justified if
//    (OVERFLOW) goto fallback;                               # punt on
//    overflow, as sum will not fit in 57-bit field allotted intptr_t sum =
//    sumShifted >> 7;                            # right-justify 57-bit tagged
//    value (54 payload bits + 3 tag bits) result = sum; # result is intptr
//    atom, properly tagged
//
// 32-bit:
//
// FAST_ADD_INT_TO_ATOM(lhs, rhs, result, fallback)
//    if ((rhs & kAtomTypeMask) != kIntptrType) goto fallback;   # punt if atom
//    argument rhs is not intptr intptr_t lhsExtended = i; # extend int argument
//    lhs to atom size (a nop on 32-bit platforms) intptr_t lhsShifted =
//    lhsExtended << kAtomTypeSize;        # left-justify 29-bit payload
//    intptr_t lhsRestored = lhsShifted >> kAtomTypeSize;        # restore
//    if (lhsRestored != lhsExtended) goto fallback;             # high-order
//    bits were lost, value will not fit in 29-bit field allotted intptr_t sum =
//    lhsShifted + rhs;                           # add left-justified lhs to
//    tagged rhs, resulting in tagged result if (OVERFLOW) goto fallback; # punt
//    on overflow, as sum will not fit in 29-bit field allotted result = sum; #
//    result is intptr atom, properly tagged
//
// Note: If lhs will not fit in 29 bits, the fastpath will fail even if the
// result may fit.  We expect this to be an uncommon case.  Generating code as
// we do avoids extra tag manipulations that would be required if we did a full
// 32-bit addition followed by a range check.

void CodegenLIR::emitIntPlusAtomFastpath(int i, Traits *type, LIns *lhs,
                                         LIns *rhs, CodegenLabel &fallback) {
...
#ifdef AVMPLUS_64BIT
  // int argument is guaranteed to fit, but must restrict intptr result to 54
  // bit range
  // TODO: Consider maintaining 54-bit intptrs in pre-shifted form.
  LIns *lhsShifted = lshp(lhsExtended, atomSignExtendShift);
  LIns *rhsShifted =
      lshp(rhs, atomSignExtendShift - AtomConstants::kAtomTypeSize);
  LIns *sumShifted =
      branchJovToLabel(LIR_addjovp, lhsShifted, rhsShifted, fallback);
  LIns *sum =
      rshp(sumShifted, atomSignExtendShift - AtomConstants::kAtomTypeSize);
#else
...
// The usual helper function call may be bypassed with a fastpath for the int +
// intptr => intptr case:
//
//     intptr_t rhsa = CONVERT_TO_ATOM(rhs);                    # box atom if
//     needed FAST_ADD_INT_TO_ATOM(lhs, rhsa, result, fallback);       # handle
//     the fastpath, branching to fallback label on failure goto done; #
//     fastpath succeeded
// fallback:
//     result = op_add_a_ia(coreAddr, lhs, rhsa);               # fastpath
//     failed, fall back to helper function
// done:
...
void CodegenLIR::emitAddIntToAtom(int i, int j, Traits *type) {
  ...
  const CallInfo *addFunction = IFFLOAT(
      floatSupport ? FUNCTIONID(op_add_a_ia) : FUNCTIONID(op_add_a_ia_nofloat),
...
void CodegenLIR::emitAddDoubleToAtom(int i, int j, Traits *type) {
  ...
  const CallInfo *addFunction =
      floatSupport ? FUNCTIONID(op_add_a_da) : FUNCTIONID(op_add_a_da_nofloat);
#else
...
// The usual helper function call may be bypassed with a fastpath for the intptr
// + int => intptr case:
//
//     intptr_t lhsa = CONVERT_TO_ATOM(lhs);                    # box atom if
//     needed FAST_ADD_INT_TO_ATOM(rhs, lhsa, result, fallback);       # handle
//     the fastpath, note it is OK to commute arguments here goto done; #
//     fastpath succeeded
// fallback:
//     result = op_add_a_ai(coreAddr, lhsa, rhs);               # fastpath
//     failed, fall back to helper function
// done:
...
void CodegenLIR::emitAddAtomToInt(int i, int j, Traits *type) {
  ...
  const bool floatSupport = pool->hasFloatSupport();
  const CallInfo *addFunction =
      floatSupport ? FUNCTIONID(op_add_a_ai) : FUNCTIONID(op_add_a_ai_nofloat);
#else
...
void CodegenLIR::emitAddAtomToDouble(int i, int j, Traits *type) {
  ...
  const bool floatSupport = pool->hasFloatSupport();
  const CallInfo *addFunction =
      floatSupport ? FUNCTIONID(op_add_a_ad) : FUNCTIONID(op_add_a_ad_nofloat);
#else
...
//
//     intptr_t lhsa = CONVERT_TO_ATOM(lhs);                    # box atom if
//     needed intptr_t rhsa = CONVERT_TO_ATOM(rhs);                    # box
//     atom if needed if (((lhsa ^ kIntptrType) | (rhsa ^ kIntptrType)) &
//     kAtomTypeMask) goto fallback; # both arguments are intptr atoms intptr_t
//     lhsStripped = lhsa - kIntptrType;               # zero out tag bits on
//     lhs intptr_t lhsShifted = lhsStripped << 7;                  #
//     left-justify 54-bit payload (followed by 0s in tag position) intptr_t
//     rhsShifted = rhsa << 7;                         # align rhs payload and
//     tag with left-justified lhs intptr_t sumShifted = lhsShifted +
//     rhsShifted;           # add aligned values, producing 54-bit
//     left-justified sum followed by tag if (OVERFLOW) goto fallback; result =
//     sumShifted >> 7;                                # right-justify tagged
//     sum (54-bit payload + 3-bit tag) goto done;
// fallback:
//     result = op_add_a_aa(coreAddr, lhsa, rhsa);              # handle the
//     general case out-of-line
// done:
//
// 32-bit:
//
...
//     kAtomTypeMask) goto fallback; # both arguments are intptr atoms intptr_t
//     lhsStripped = lhsa - kIntptrType;               # zero out tag bits on
//     lhs (note rhs retains its tag) intptr_t sum = lhsStripped + rhs; # add,
//     producing 29-bit sum followed by 3-bit tag if (OVERFLOW) goto fallback;
//     result = sum;
//     goto done;
// fallback:
//     result = op_add_a_aa(coreAddr, lhsa, rhsa);              # handle the
...
void CodegenLIR::emitAddAtomToAtom(int i, int j, Traits *type) {
  ...
  const bool floatSupport = pool->hasFloatSupport();
  const CallInfo *addFunction =
      floatSupport ? FUNCTIONID(op_add_a_aa) : FUNCTIONID(op_add_a_aa_nofloat);
#else
  ...
  if (inlineFastpath) {
    ...
    // since 64-bit int atoms expect exactly 54 bits of precision, shift bit
    // 54+3 up into the sign bit
    LIns *lhsShifted =
        lshp(lhsStripped, atomSignExtendShift - AtomConstants::kAtomTypeSize);
    LIns *rhsShifted =
        lshp(rhs, atomSignExtendShift - AtomConstants::kAtomTypeSize);
    LIns *sumShifted =
        branchJovToLabel(LIR_addjovp, lhsShifted, rhsShifted, fallback);
...
void CodegenLIR::emitAddAtomToAtom(int i, int j, Traits *type) {
  ...
  const bool floatSupport = pool->hasFloatSupport();
  const CallInfo *addFunction =
      floatSupport ? FUNCTIONID(op_add) : FUNCTIONID(op_add_nofloat);
#else
...
void CodegenLIR::emitAdd(int i, int j, Traits *type) {
  ...
  if ((val1.traits == STRING_TYPE && val1.notNull) ||
      (val2.traits == STRING_TYPE && val2.notNull)) {
  ...
  } else if (val1.traits && val2.traits && val1.traits->isNumeric() &&
             val2.traits->isNumeric()) {
    // numeric + numeric
...
#ifdef VMCFG_FASTPATH_ADD
    // If we arrive here, at least one argument is not known to be of a numeric
    // type. Thus, having determined one argument to be of a known numeric type,
    // we will coerce the other to an atom.  We speculate that the other
    // argument will already be an atom of type kIntptrType or kDoubleType,
    // checking for these cases first.
    ...
    // Neither argument is known to be of a numeric type, so coerce both to
    // atoms.
    AvmAssert(type == OBJECT_TYPE);
...
void CodegenLIR::writeBlockStart(const FrameState *state) {
...
#if defined(VMCFG_INTERRUPT_SAFEPOINT_POLL) &&                                 \
    defined(NJ_SAFEPOINT_POLLING_SUPPORTED)
  ...
  if (interruptable && core->config.interrupts &&
      state->targetOfBackwardsBranch) {
    LIns *interrupted = loadIns(LIR_ldi, offsetof(AvmCore, interrupted),
                                coreAddr, ACCSET_OTHER, LOAD_VOLATILE);
    LIns *cond =
        binaryIns(LIR_eqi, interrupted, InsConst(AvmCore::NotInterrupted));
    branchToLabel(LIR_jf, cond, interrupt_label);
  }
#endif
}

void CodegenLIR::writeOpcodeVerified(const FrameState *state, const uint8_t *,
                                     AbcOpcode) {
  verbose_only(if (vbWriter) { vbWriter->flush(); })
#ifdef DEBUG
...
void CodegenLIR::writeFixExceptionsAndLabels(const FrameState *,
                                             const uint8_t *) {}

void CodegenLIR::write(const FrameState *state, const uint8_t *pc,
                       AbcOpcode opcode, Traits *type) {
  ...
  case OP_pushtrue:
    ...
    emitTaintedIntConst(sp + 1, 1, type);
    break;
  case OP_pushfalse:
    AvmAssert(type == BOOLEAN_TYPE);
    emitTaintedIntConst(sp + 1, 0, type);
    break;
  case OP_pushnull:
    // Null is coercible to numeric 0, which I don't think
    // is exploitable as an untainted value, but I'd give it
    // a bit more scrutiny if it were possible to do this
    // coercion at compile-time.   As it stands, conversions
    // of non-numeric to numeric values is done out-of-line.
    AvmAssert(type == NULL_TYPE);
    emitPtrConst(sp + 1, 0, type);
    break;
  case OP_pushundefined:
    // Through a chain of coercions undefined->false->0, there
    // is a plausible way in which an untainted 0 might be generated.
    // The comment above on OP_pushnull applies here as well; we are
    // relying on the fact that such conversions are done out-of-line
    // and will not reduce to constants.
    AvmAssert(type == VOID_TYPE);
    emitPtrConst(sp + 1, (void *)undefinedAtom, type);
    break;
  case OP_pushshort:
    AvmAssert(type == INT_TYPE);
    emitTaintedIntConst(sp + 1, (signed short)imm30, type);
    break;
  case OP_pushbyte:
    AvmAssert(type == INT_TYPE);
    emitTaintedIntConst(sp + 1, (signed char)imm8, type);
    break;
  case OP_pushstring:
    AvmAssert(type == STRING_TYPE);
  ...
  case OP_pushint:
    AvmAssert(type == INT_TYPE);
    emitTaintedIntConst(sp + 1, pool->cpool_int[imm30], type);
    break;
  case OP_pushuint:
    AvmAssert(type == UINT_TYPE);
    emitTaintedIntConst(sp + 1, pool->cpool_uint[imm30], type);
    break;
  case OP_pushdouble:
    AvmAssert(type == NUMBER_TYPE);
    emitTaintedDoubleConst(sp + 1, &pool->cpool_double[imm30]->value);
    break;
#ifdef VMCFG_FLOAT
  case OP_pushfloat:
    AvmAssert(type == FLOAT_TYPE);
    emitTaintedFloatConst(sp + 1, pool->cpool_float[imm30]->value);
    break;
  case OP_pushfloat4:
    AvmAssert(type == FLOAT4_TYPE);
    emitTaintedFloat4Const(sp + 1, (const float4_t *)pool->cpool_float4[imm30]);
    break;
#endif
  case OP_pushnan:
    AvmAssert(type == NUMBER_TYPE);
  ...
  case OP_debugfile: {
    if (haveDebugger || haveVTune || jit_observer) {
      String *filename = pool->getString(imm30);
      (void)filename;
#ifdef DEBUGGER
      if (haveDebugger) {
        LIns *debugger = loadIns(LIR_ldp, offsetof(AvmCore, _debugger),
                                 coreAddr, ACCSET_OTHER, LOAD_CONST);
        callIns(FUNCTIONID(debugFile), 2, debugger, InsConstPtr(filename));
      }
#endif // DEBUGGER
#ifdef VMCFG_VTUNE
      ...
      if (jit_observer) {
        ...
        JITDebugInfo::Info *info =
            new (*lir_alloc) JITDebugInfo::Info(JITDebugInfo::kFile);
        info->file = imm30;
        jdi->add(info);
  ...
  case OP_debugline: {
    if (haveDebugger || haveVTune || jit_observer) {
      ...
      if (jit_observer) {
        JITDebugInfo *jdi = initJitDebugInfo();
        JITDebugInfo::Info *info =
            new (*lir_alloc) JITDebugInfo::Info(JITDebugInfo::kLine);
  ...
  case OP_astype: {
    emit(OP_astype, (uintptr_t)type, sp,
         type && type->isMachineType() ? OBJECT_TYPE : type);
    break;
  }
  case OP_astypelate: {
  ...
  case OP_convert_f4:
    ...
    AvmAssert(FLOAT_ONLY((opcode == OP_convert_f && type == FLOAT_TYPE) ||
  ...
  case OP_istypelate: {
    ...
    // any side effects from loading T, even if we end up inlining T.itraits()
    // as a const.
    ...
    if (class_type && class_type->base == CLASS_TYPE) {
      ...
      istype_result =
          callIns(FUNCTIONID(istype), 2, obj, InsConstPtr(class_type->itraits));
    } else {
      // RHS is unknown, call general istype
      istype_result =
          callIns(FUNCTIONID(istypelate), 3, env_param, obj, loadAtomRep(sp));
    }
  ...
  case OP_getslot: {
    ...
    int index = imm30 - 1;
    Traits *slotTraits =
        obj.traits ? obj.traits->getTraitsBindings()->getSlotTraits(index)
                   : NULL;
  ...
  case OP_divide:
  case OP_multiply:
    AvmAssert(type == NUMBER_TYPE FLOAT_ONLY(|| type == OBJECT_TYPE ||
                                             type == FLOAT_TYPE ||
                                             type == FLOAT4_TYPE));
  ...
  case OP_lf64: {
    Traits *result =
        (opcode == OP_lf32 || opcode == OP_lf64) ? NUMBER_TYPE : INT_TYPE;
  ...
  case OP_restargc: {
    ...
    LIns *out =
        callIns(FUNCTIONID(restargcHelper), 2, localGetp(restLocal), restArgc);
...
// position is the class or function object and we haven't called newInstance
// yet. In this case, emitCall() will generate a call to newInstance, producing
// the new object, then call its init function with the coerced arguments.
void CodegenLIR::emitConstructCall(intptr_t method_id, int argc, LIns *ctor,
                                   Traits *ctraits) {
  Traits *itraits = ctraits->itraits;
  MethodInfo *m = itraits->init;
  MethodSignaturep mms = m->getMethodSignature();
  AvmAssert(
      mms->argcOk(argc)); // caller must check this before early binding to ctor
 ...
 * the driver did not.  It does the coersions using the signature of the callee,
 * and does not mutate FrameState.
 */
void CodegenLIR::emitCoerceCall(AbcOpcode opcode, intptr_t method_id, int argc,
                                MethodSignaturep mms) {
  AvmAssert(state->value(state->sp() - argc)
                .notNull); // make sure null check happened
...
void CodegenLIR::writeOp1(const FrameState *state, const uint8_t *pc,
                          AbcOpcode opcode, uint32_t opd1, Traits *type) {
  this->state = state;
  emitSetPc(pc);
  switch (opcode) {
  case OP_iflt:
  ...
  case OP_finddef: {
    ...
    LIns *obj_ptr = addp(table, offsetof(FinddefTable, elements) +
                                    sizeof(ScriptObject *) * name_id);
    LIns *obj = callIns(FUNCTIONID(finddef_cache), 2, obj_ptr, methodFrame);
...
#endif
    LIns *out = callIns(
        FUNCTIONID(restargHelper), 6, loadEnvToplevel(), InsConstPtr(multiname),
        loadAtomRep(state->sp()),
        // With a sane number of parameters, the displacement will never need to
        // be blinded.
        lea(restLocal << VARSHIFT(info), vars, /*tainted*/ true), restArgc,
        (info->needRest() ? binaryIns(LIR_addp, ap_param,
                                      InsConstPtr((void *)(ms->rest_offset())))
                          : binaryIns(LIR_addp, ap_param,
                                      InsConstPtr((void *)sizeof(Atom)))));
#ifdef __clang__
...
LIns *CodegenLIR::coerceToString(int index) {
  ...
  case BUILTIN_float4:
    return callIns(FUNCTIONID(float4ToString), 2, coreAddr,
                   localGetf4Addr(index));
#endif
  case BUILTIN_number:
  ...
  case BUILTIN_boolean: {
    ...
    LIns *offset =
        binaryIns(LIR_lshp, i2p(localGet(index)), InsConst(PTR_SCALE));
    LIns *arr = InsConstPtr(&core->booleanStrings);
    return loadIns(LIR_ldp, 0, binaryIns(LIR_addp, arr, offset), ACCSET_OTHER,
                   LOAD_CONST);
  }
  default:
    if (value.notNull) {
...
LIns *CodegenLIR::coerceToNumber(int index) {
  ...
  /* can't promote "Numeric" to "Number", we don't know the exact type at
   * compile time */
  if (in && (in->isNumeric() || in == BOOLEAN_TYPE)) {
    return promoteNumberIns(in, index);
  } else {
// * -> Number
// Note that another version of this code exists in
// InvokerCompiler::downcast_and_store, if we make changes here they might need
// to be propagated there too.
#ifdef VMCFG_FASTPATH_FROMATOM
    if (inlineFastpath) {
      //     double result;
      //     intptr_t val = CONVERT_TO_ATOM(arg);
      //     if ((val & kAtomTypeMask) != kIntptrType) goto not_intptr;   # test
      //     for kIntptrType tag # kIntptrType result = double(val >>
      //     kAtomTypeSize);                         # extract integer value and
      //     convert to double goto done;
      // not_intptr:
      //     if ((val & kAtomTypeMask) != kDoubleType) goto not_double;   # test
      //     for kDoubleType tag # kDoubleType result = *(val - kDoubleType); #
      //     remove tag and dereference goto done;
      // not_double:
      //     result = number(val);                                          #
      ...
      std(p2dIns(rshp(val, AtomConstants::kAtomTypeSize)), result, 0,
          ACCSET_OTHER);
      ...
      // kDoubleType
      branchToLabel(LIR_jf, eqp(tag, AtomConstants::kDoubleType), not_double);
      std(ldd(subp(val, AtomConstants::kDoubleType), 0, ACCSET_OTHER), result,
          0, ACCSET_OTHER);
...
LIns *CodegenLIR::coerceToNumeric(int index) {
  ...
  if (in && (in->isNumeric() || in == BOOLEAN_TYPE))
    ...
    const int32_t numericMask =
        ((1 << AtomConstants::kIntptrType) | (1 << AtomConstants::kDoubleType) |
         (1 << AtomConstants::kSpecialBibopType));
    branchToLabel(LIR_jt,
                  binaryIns(LIR_eqi,
                            binaryIns(LIR_andi, tagmask, InsConst(numericMask)),
                            InsConst(0)),
                  non_numeric);
    branchToLabel(
        LIR_jf, eqp(val, AtomConstants::undefinedAtom),
        already_numeric); // we still need to test for "undefined" before we
                          // conclude it's already a numeric type
    ...
    stp(callIns(FUNCTIONID(numericAtom), 2, coreAddr, val), result, 0,
        ACCSET_OTHER);
    ...
    branchToLabel(LIR_jf, eqp(tag, AtomConstants::kSpecialBibopType),
                  really_numeric);
    LIns *bibopTag =
        lirout->insLoad(LIR_lduc2ui, andp(val, (Atom)MMgc::GCHeap::kBlockMask),
                        0, ACCSET_OTHER);
    branchToLabel(
        LIR_jt,
        binaryIns(LIR_eqi, bibopTag, InsConst(AtomConstants::kBibopFloatType)),
        really_numeric);
    branchToLabel(
        LIR_jt,
        binaryIns(LIR_eqi, bibopTag, InsConst(AtomConstants::kBibopFloat4Type)),
        really_numeric);
...
LIns *CodegenLIR::emitStringCall(int index, const CallInfo *stringCall,
                                 bool preserveNull) {
  LIns *val = loadAtomRep(index);
  ...
  if (inlineFastpath) {
    ...
    if (!preserveNull) {
      ...
      branchToLabel(LIR_jt, eqp(val, AtomConstants::kStringType),
                    not_stringptr);
    }
...
void CodegenLIR::writeMethodCall(const FrameState *state, const uint8_t *pc,
                                 AbcOpcode opcode, MethodInfo *m,
                                 uintptr_t disp_id, uint32_t argc,
                                 Traits *type) {
  this->state = state;
  emitSetPc(pc);
  switch (opcode) {
...
void CodegenLIR::writeOp2(const FrameState *state, const uint8_t *pc,
                          AbcOpcode opcode, uint32_t opd1, uint32_t opd2,
                          Traits *type) {
  this->state = state;
  emitSetPc(pc);
  int sp = state->sp();
  ...
  case OP_getproperty: {
    ...
    // early bind accessor
    if (AvmCore::hasGetterBinding(b)) {
      MethodInfo *getter_info = obj.traits->getTraitsBindings()->getMethod(
      ...
      if (getter_info->pool() == core->builtinPool) {
        ...
        //
        //    These are statically known to produce float4, the swizzling
        //    compiles down to a single LIR instruction.
      ...
      vector_length : {
        LIns *arrayLen = loadIns(LIR_ldi, lenOffset, localGetp(sp),
  ...
  case OP_callstatic: {
    ...
    uint32_t argc = opd2;
    emitTypedCall(OP_callstatic, method_id, argc, type,
                  pool->getMethodInfo(method_id));
    break;
  }
...
void CodegenLIR::emitTaintedIntConst(int index, int32_t c, Traits *type) {
  localSet(index, lirout->insImmI(c, true), type);
}
...
void CodegenLIR::emitTaintedDoubleConst(int index, const double *pd) {
  localSet(index, lirout->insImmD(*pd, true), NUMBER_TYPE);
}

#ifdef VMCFG_FLOAT
void CodegenLIR::emitTaintedFloatConst(int index, const float f) {
  localSet(index, lirout->insImmF(f, true), FLOAT_TYPE);
}

void CodegenLIR::emitTaintedFloat4Const(int index, const float4_t *f4) {
  localSet(index, insImmF4(*(const float4_t *)f4, true), FLOAT4_TYPE);
}
...
void CodegenLIR::writeCoerceToNumeric(const FrameState *state, uint32_t loc) {
  this->state = state;
  emitSetPc(state->abc_pc);
  localSet(loc, IFFLOAT(coerceToNumeric(loc), coerceToNumber(loc)),
           OBJECT_TYPE);
}
...
void CodegenLIR::writeCoerceToFloat4(const FrameState *state, uint32_t index) {
  ...
  (void)index;
  (void)state;
  AvmAssert(!"coerceToFloat4 encountered in CodegenLIR, with VMCFG_FLOAT "
             "turned off!!!");
#endif
}

void CodegenLIR::writeCoerce(const FrameState *state, uint32_t loc,
                             Traits *result) {
  this->state = state;
  emitSetPc(state->abc_pc);
...
LIns *CodegenLIR::specializeIntCall(LIns *call, const Specialization *specs) {
  ...
  while (specs[i].oldFunc != NULL) {
    if (specs[i].oldFunc == ci) {
      ...
      LIns *specialization =
          callIns(nci, 2, call->arg(1), call->arg(0), INT_TYPE);
...
LIns *CodegenLIR::imm2Int(LIns *imm) {
  if (imm->isImmI())
    ...
    if (val == 0 || val == cvt)
      imm = InsConst((int32_t)cvt, imm->isTainted());
    else
      imm = 0; // can't convert
  } else {
    imm = 0; // non-imm
...
LIns *CodegenLIR::coerceNumberToInt(int loc) {
  ...
  // We cannot use LIR_divi if divisor is non-constant, as we need to generate a
  // NaN on division by zero.
  case LIR_divd: {
    LIns *a = arg->oprnd1();
    LOpcode aOpcode = a->opcode();
    // We should never arrive here at all if both operands are constants, as the
    // LIR_divd should have been folded previously.  Furthermore, Nanojit does
    // not permit both arguments of LIR_divi to be constant, and will assert if
    // this occurs, so let's be absolutely sure it will not by specifically not
    // attempting to optimize that case here.
      ...
      if (b) {
        ...
        if (intConst) {
          ...
          if (aOpcode == LIR_ui2d && intConst >= 0 && exactlyOneBit(intConst)) {
            return lirout->ins2(LIR_rshui, a,
                                lirout->insImmI(msbSet32(intConst)));
          }
  ...
  case LIR_immd:
    // const fold
    return InsConst(AvmCore::integer_d(arg->immD()), arg->isTainted());
...
LIns *CodegenLIR::coerceFloatToInt(int loc) {
  ...
  if (arg->isop(LIR_immf))
    return InsConst(AvmCore::integer_d((double)arg->immF()), arg->isTainted());
...
LIns *CodegenLIR::coerceToType(int loc, Traits *result) {
  ...
  if (result == NULL) {
    ...
    if (!Traits::canAssign(result, in)) {
      // coerceobj_obj() is void, but we mustn't optimize it out; we only call
      // it when required
      callIns(FUNCTIONID(coerceobj_obj), 3, env_param, localGetp(loc),
              InsConstPtr(result));
    }
    ...
    LIns *out =
        callIns(FUNCTIONID(coerce), 3, env_param, value, InsConstPtr(result));
...
void CodegenLIR::emitCheckNull(LIns *ptr, Traits *t) {
  ...
  if (valueStorageType(ty) == SST_atom) {
    ...
    if (ty != BUILTIN_object) {
      ...
      branchToLabel(
          LIR_jt, eqp(ptr, undefConst),
          upe_label); // if (p == undefined) throw undefined pointer error
    }
    branchToLabel(LIR_jt, ltup(ptr, undefConst),
                  npe_label); // if (p < undefined) throw null pointer error
  } else {
...
void CodegenLIR::emitIsNaN(Traits *result) {
  int op1 = state->sp();
  /* In practice, floats will be coerced to double before calling isNaN - so
  "singlePrecision" should be always false for now. But this is implemented as
  if isNaN was polymorphic - there's no reason why it won't eventually be
  polymorphic */
  ...
  if (isPromote(f->opcode())) {
    ...
    // LIR is required to follow IEEE floating point semantics, thus x is a NaN
    // iff x != x.
    localSet(op1 - 1, binaryIns(LIR_eqi, binaryIns(eqop, f, f), InsConst(0)),
             result);
  }
}
...
void CodegenLIR::emitStringLength(Traits *result) {
  int op1 = state->sp();
  LIns *ptr = loadIns(LIR_ldi, offsetof(String, m_length), localGetp(op1),
...
void CodegenLIR::emitFloatAbs(Traits *result) {
  emitFloatUnary(result, LIR_absf);
}
void CodegenLIR::emitFloatReciprocal(Traits *result) {
  emitFloatUnary(result, LIR_recipf);
}
void CodegenLIR::emitFloatRsqrt(Traits *result) {
  emitFloatUnary(result, LIR_rsqrtf);
}
void CodegenLIR::emitFloatSqrt(Traits *result) {
  emitFloatUnary(result, LIR_sqrtf);
}
...
void CodegenLIR::emitFloat4abs(Traits *result) {
  emitFloat4unary(result, LIR_absf4);
}
void CodegenLIR::emitFloat4reciprocal(Traits *result) {
  emitFloat4unary(result, LIR_recipf4);
}
void CodegenLIR::emitFloat4rsqrt(Traits *result) {
  emitFloat4unary(result, LIR_rsqrtf4);
}
void CodegenLIR::emitFloat4sqrt(Traits *result) {
  emitFloat4unary(result, LIR_sqrtf4);
}
...
LIns *CodegenLIR::emitDot(LOpcode dot_op, LIns *a, LIns *b) {
  ...
  LIns *sum = lirout->ins2(LIR_addf, lirout->ins1(LIR_f4x, c),
                           lirout->ins1(LIR_f4y, c));
...
void CodegenLIR::emitFloat4max(Traits *result) {
  emitFloat4binary(result, LIR_maxf4);
}
void CodegenLIR::emitFloat4min(Traits *result) {
  emitFloat4binary(result, LIR_minf4);
}
void CodegenLIR::emitFloat4Greater(Traits *result) {
  emitFloat4binary(result, LIR_cmpgtf4);
}
void CodegenLIR::emitFloat4GreaterOrEqual(Traits *result) {
  emitFloat4binary(result, LIR_cmpgef4);
}
void CodegenLIR::emitFloat4Less(Traits *result) {
  emitFloat4binary(result, LIR_cmpltf4);
}
void CodegenLIR::emitFloat4LessOrEqual(Traits *result) {
  emitFloat4binary(result, LIR_cmplef4);
}
void CodegenLIR::emitFloat4Equal(Traits *result) {
  emitFloat4binary(result, LIR_cmpeqf4);
}
void CodegenLIR::emitFloat4NotEqual(Traits *result) {
  emitFloat4binary(result, LIR_cmpnef4);
}
...
void CodegenLIR::emitFloat4dot(Traits *result) {
  emitFloat4dot(result, LIR_dotf4);
}
void CodegenLIR::emitFloat4dot2(Traits *result) {
  emitFloat4dot(result, LIR_dotf2);
}
void CodegenLIR::emitFloat4dot3(Traits *result) {
  emitFloat4dot(result, LIR_dotf3);
}
...
void CodegenLIR::emitFloat4magnitude(Traits *result) {
  emitMagnitude(result, LIR_dotf4);
}
void CodegenLIR::emitFloat4magnitude2(Traits *result) {
  emitMagnitude(result, LIR_dotf2);
}
void CodegenLIR::emitFloat4magnitude3(Traits *result) {
  emitMagnitude(result, LIR_dotf3);
}
...
void CodegenLIR::emitFloat4distance(Traits *result) {
  emitDistance(result, LIR_dotf4);
}
void CodegenLIR::emitFloat4distance2(Traits *result) {
  emitDistance(result, LIR_dotf2);
}
void CodegenLIR::emitFloat4distance3(Traits *result) {
  emitDistance(result, LIR_dotf3);
}
#endif

// Determine a mask for our argument that indicates to which types it may be
// trivially converted without insertion of additional instructions and without
// loss.  This information may allow us to substitute a numeric operation on a
// more specific type than that inferred by the verifier. For example:
//    A number that has been promoted from an integer will be marked as a
//    number|int. A constant number that is an unsigned integer will be marked
//    as a number|uint|int
...
// return a LIns that represents the argument as a value of the correct machine
// type.
LIns *CodegenLIR::getSpecializedArg(int argOffset, BuiltinType newBt) {
  ...
  if (oldBt == BUILTIN_number) {
    ...
    if (newBt == BUILTIN_int) {
      if (arg->isImmD())
        return InsConst((int32_t)arg->immD(), arg->isTainted());
      else if (arg->opcode() == LIR_i2d)
        return arg->oprnd1();
      else
        AvmAssert(0);
      ...
      if (arg->isImmD())
        return InsConstFlt((float)arg->immD(), arg->isTainted());
      else if (arg->opcode() == LIR_f2d)
        return arg->oprnd1();
      else
        AvmAssert(0);
    ...
    if (newBt == BUILTIN_int) {
      if (arg->isImmF())
        return InsConst((int32_t)arg->immF(), arg->isTainted());
      else if (arg->opcode() == LIR_i2f)
        return arg->oprnd1();
      else
        AvmAssert(0);
      ...
      if (arg->isImmD())
        ...
        AvmAssert(0);
               ...
               BUILTIN_number) { // not used right now, but maybe soon...
      if (arg->isImmF())
        return lirout->insImmD(arg->immF(), arg->isTainted());
      else
        return f2dIns(arg);
    }
...
#ifdef VMCFG_FLOAT
  case BUILTIN_number:
    AvmAssertMsg(false, "Case should be already handled; this means oldBt is "
                        "not number, but newBt is Number!");
    return localGetd(argOffset);
  case BUILTIN_float:
    AvmAssertMsg(false, "Case should be already handled; this means oldBt is "
                        "not float, but newBt is float!");
    return localGetf(argOffset);
  case BUILTIN_float4:
    AvmAssertMsg(false, "Case should be already handled; this means oldBt is "
                        "not float4, but newBt is float4!");
...
// For a given builtin function id and argument count, matching of argument
// lists occurs in the order in which the entries appear in the table below.
// More specialized variants that are expected to execute faster should be
// listed prior to the more general cases.  If no argument list matches,
// specialization is not performed.

const CodegenLIR::FunctionMatch CodegenLIR::specializedFunctions[] = {
    {0, /* dummy entry so 0 can be treated as HashMap miss*/ 1, {}, 0, 0},
    {avmplus::NativeID::native_script_function_isNaN,
     1,
     {BUILTIN_number, BUILTIN_none},
     0,
     &CodegenLIR::emitIsNaN},
    /*could theoretically also add {
       avmplus::NativeID::native_script_function_isNaN, 1, {BUILTIN_float,
       BUILTIN_none},   0, &CodegenLIR::emitIsNaN }, */

    {avmplus::NativeID::String_AS3_charCodeAt,
     1,
     {BUILTIN_uint, BUILTIN_none},
     FUNCTIONID(String_charCodeAtDU),
     0},
    {avmplus::NativeID::String_AS3_charCodeAt,
     1,
     {BUILTIN_int, BUILTIN_none},
     FUNCTIONID(String_charCodeAtDI),
     0},
    {avmplus::NativeID::String_AS3_charCodeAt,
     1,
     {BUILTIN_number, BUILTIN_none},
     FUNCTIONID(String_charCodeAtDD),
     0},
    {avmplus::NativeID::String_AS3_charAt,
     1,
     {BUILTIN_uint, BUILTIN_none},
     FUNCTIONID(String_charAtU),
     0},
    {avmplus::NativeID::String_AS3_charAt,
     1,
     {BUILTIN_int, BUILTIN_none},
     FUNCTIONID(String_charAtI),
     0},
    {avmplus::NativeID::String_AS3_charAt,
     1,
     {BUILTIN_number, BUILTIN_none},
     FUNCTIONID(String_charAtD),
     0},

    {avmplus::NativeID::String_length_get,
     0,
     {BUILTIN_none, BUILTIN_none},
     0,
     &CodegenLIR::emitStringLength},

    {avmplus::NativeID::Math_acos,
     1,
     {BUILTIN_number, BUILTIN_none},
     FUNCTIONID(Math_acos),
     0},
    {avmplus::NativeID::Math_asin,
     1,
     {BUILTIN_number, BUILTIN_none},
     FUNCTIONID(Math_asin),
     0},
    {avmplus::NativeID::Math_atan,
     1,
     {BUILTIN_number, BUILTIN_none},
     FUNCTIONID(Math_atan),
     0},
    {avmplus::NativeID::Math_ceil,
     1,
     {BUILTIN_number, BUILTIN_none},
     FUNCTIONID(Math_ceil),
     0},
    {avmplus::NativeID::Math_cos,
     1,
     {BUILTIN_number, BUILTIN_none},
     FUNCTIONID(Math_cos),
     0},
    {avmplus::NativeID::Math_exp,
     1,
     {BUILTIN_number, BUILTIN_none},
     FUNCTIONID(Math_exp),
     0},
    {avmplus::NativeID::Math_floor,
     1,
     {BUILTIN_number, BUILTIN_none},
     FUNCTIONID(Math_floor),
     0},
    {avmplus::NativeID::Math_log,
     1,
     {BUILTIN_number, BUILTIN_none},
     FUNCTIONID(Math_log),
     0},
    {avmplus::NativeID::Math_round,
     1,
     {BUILTIN_number, BUILTIN_none},
     FUNCTIONID(Math_round),
     0},
    {avmplus::NativeID::Math_sin,
     1,
     {BUILTIN_number, BUILTIN_none},
     FUNCTIONID(Math_sin),
     0},
    {avmplus::NativeID::Math_sqrt,
     1,
     {BUILTIN_number, BUILTIN_none},
     FUNCTIONID(Math_sqrt),
     0},
    {avmplus::NativeID::Math_tan,
     1,
     {BUILTIN_number, BUILTIN_none},
     FUNCTIONID(Math_tan),
     0},

    {avmplus::NativeID::Math_atan2,
     2,
     {BUILTIN_number, BUILTIN_number},
     FUNCTIONID(Math_atan2),
     0},
    {avmplus::NativeID::Math_pow,
     2,
     {BUILTIN_number, BUILTIN_number},
     FUNCTIONID(Math_pow),
     0},

    // We would like to inline abs(), but the obvious implementation does not
    // work, as both 0.0 and -0.0 compare as the same.
    {avmplus::NativeID::Math_abs,
     1,
     {BUILTIN_number, BUILTIN_none},
     FUNCTIONID(Math_abs),
     0},
    ...
    // float4
    {avmplus::NativeID::float4_abs,
     1,
     {BUILTIN_float4, BUILTIN_none},
     0,
     &CodegenLIR::emitFloat4abs},
    {avmplus::NativeID::float4_max,
     2,
     {BUILTIN_float4, BUILTIN_float4},
     0,
     &CodegenLIR::emitFloat4max},
    {avmplus::NativeID::float4_min,
     2,
     {BUILTIN_float4, BUILTIN_float4},
     0,
     &CodegenLIR::emitFloat4min},
    {avmplus::NativeID::float4_reciprocal,
     1,
     {BUILTIN_float4, BUILTIN_none},
     0,
     &CodegenLIR::emitFloat4reciprocal},
    {avmplus::NativeID::float4_rsqrt,
     1,
     {BUILTIN_float4, BUILTIN_none},
     0,
     &CodegenLIR::emitFloat4rsqrt},
    {avmplus::NativeID::float4_sqrt,
     1,
     {BUILTIN_float4, BUILTIN_none},
     0,
     &CodegenLIR::emitFloat4sqrt},
    {avmplus::NativeID::float4_cross,
     2,
     {BUILTIN_float4, BUILTIN_float4},
     0,
     &CodegenLIR::emitFloat4cross},
    {avmplus::NativeID::float4_normalize,
     1,
     {BUILTIN_float4, BUILTIN_none},
     0,
     &CodegenLIR::emitFloat4normalize},
    {avmplus::NativeID::float4_dot,
     2,
     {BUILTIN_float4, BUILTIN_float4},
     0,
     &CodegenLIR::emitFloat4dot},
    {avmplus::NativeID::float4_dot2,
     2,
     {BUILTIN_float4, BUILTIN_float4},
     0,
     &CodegenLIR::emitFloat4dot2},
    {avmplus::NativeID::float4_dot3,
     2,
     {BUILTIN_float4, BUILTIN_float4},
     0,
     &CodegenLIR::emitFloat4dot3},
    {avmplus::NativeID::float4_magnitude,
     1,
     {BUILTIN_float4, BUILTIN_none},
     0,
     &CodegenLIR::emitFloat4magnitude},
    {avmplus::NativeID::float4_magnitude3,
     1,
     {BUILTIN_float4, BUILTIN_none},
     0,
     &CodegenLIR::emitFloat4magnitude3},
    {avmplus::NativeID::float4_magnitude2,
     1,
     {BUILTIN_float4, BUILTIN_none},
     0,
     &CodegenLIR::emitFloat4magnitude2},
    {avmplus::NativeID::float4_distance,
     2,
     {BUILTIN_float4, BUILTIN_float4},
     0,
     &CodegenLIR::emitFloat4distance},
    {avmplus::NativeID::float4_distance2,
     2,
     {BUILTIN_float4, BUILTIN_float4},
     0,
     &CodegenLIR::emitFloat4distance2},
    {avmplus::NativeID::float4_distance3,
     2,
     {BUILTIN_float4, BUILTIN_float4},
     0,
     &CodegenLIR::emitFloat4distance3},
    {avmplus::NativeID::float4_isGreater,
     2,
     {BUILTIN_float4, BUILTIN_float4},
     0,
     &CodegenLIR::emitFloat4Greater},
    {avmplus::NativeID::float4_isLess,
     2,
     {BUILTIN_float4, BUILTIN_float4},
     0,
     &CodegenLIR::emitFloat4Less},
    {avmplus::NativeID::float4_isGreaterOrEqual,
     2,
     {BUILTIN_float4, BUILTIN_float4},
     0,
     &CodegenLIR::emitFloat4GreaterOrEqual},
    {avmplus::NativeID::float4_isLessOrEqual,
     2,
     {BUILTIN_float4, BUILTIN_float4},
     0,
     &CodegenLIR::emitFloat4LessOrEqual},
    {avmplus::NativeID::float4_isEqual,
     2,
     {BUILTIN_float4, BUILTIN_float4},
     0,
     &CodegenLIR::emitFloat4Equal},
    {avmplus::NativeID::float4_isNotEqual,
     2,
     {BUILTIN_float4, BUILTIN_float4},
     0,
     &CodegenLIR::emitFloat4NotEqual},
#endif

    {avmplus::NativeID::Math_min,
     2,
     {BUILTIN_int, BUILTIN_int},
     0,
     &CodegenLIR::emitIntMathMin},
    {avmplus::NativeID::Math_max,
     2,
     {BUILTIN_int, BUILTIN_int},
     0,
     &CodegenLIR::emitIntMathMax},
    {avmplus::NativeID::Math_private__min,
     2,
     {BUILTIN_int, BUILTIN_int},
     0,
     &CodegenLIR::emitIntMathMin},
    {avmplus::NativeID::Math_private__max,
     2,
     {BUILTIN_int, BUILTIN_int},
     0,
     &CodegenLIR::emitIntMathMax},

// We do not inline the non-integral cases of min and max due to the complexity
// of handling -0 correctly. See MathClass::_min() and MathClass::_max().

// Unsupported because it uses MathClass::seed
//{ avmplus::NativeID::Math_random,                  0, {BUILTIN_none,
//BUILTIN_none},   FUNCTIONID(Math_random), 0},

#ifdef VMCFG_FLOAT
    // float
    {avmplus::NativeID::float_acos,
     1,
     {BUILTIN_float, BUILTIN_none},
     FUNCTIONID(float_acos),
     0},
    {avmplus::NativeID::float_asin,
     1,
     {BUILTIN_float, BUILTIN_none},
     FUNCTIONID(float_asin),
     0},
    {avmplus::NativeID::float_atan,
     1,
     {BUILTIN_float, BUILTIN_none},
     FUNCTIONID(float_atan),
     0},
    {avmplus::NativeID::float_ceil,
     1,
     {BUILTIN_float, BUILTIN_none},
     FUNCTIONID(float_ceil),
     0},
    {avmplus::NativeID::float_cos,
     1,
     {BUILTIN_float, BUILTIN_none},
     FUNCTIONID(float_cos),
     0},
    {avmplus::NativeID::float_exp,
     1,
     {BUILTIN_float, BUILTIN_none},
     FUNCTIONID(float_exp),
     0},
    {avmplus::NativeID::float_floor,
     1,
     {BUILTIN_float, BUILTIN_none},
     FUNCTIONID(float_floor),
     0},
    {avmplus::NativeID::float_log,
     1,
     {BUILTIN_float, BUILTIN_none},
     FUNCTIONID(float_log),
     0},
    {avmplus::NativeID::float_sin,
     1,
     {BUILTIN_float, BUILTIN_none},
     FUNCTIONID(float_sin),
     0},
    {avmplus::NativeID::float_tan,
     1,
     {BUILTIN_float, BUILTIN_none},
     FUNCTIONID(float_tan),
     0},

    {avmplus::NativeID::float_atan2,
     2,
     {BUILTIN_float, BUILTIN_float},
     FUNCTIONID(float_atan2),
     0},
    {avmplus::NativeID::float_pow,
     2,
     {BUILTIN_float, BUILTIN_float},
     FUNCTIONID(float_pow),
     0},

    // We would like to inline this on x86, but need SSE 4.1 to use ROUNDSS. A
    // CVTSS2SI; CVTSI2SS sequence should work for SSE2, but only with the
    // correct rounding mode, which we cannot guarantee.  It would be possible
    // to inline the MathUtils::roundf function, which seems the best we can do.
    {avmplus::NativeID::float_round,
     1,
     {BUILTIN_float, BUILTIN_none},
     FUNCTIONID(float_round),
     0},
    // We would like to inline these on x86, but the MINSS and MAXSS
    // instructions do not handle signed zero as required.
    {avmplus::NativeID::float_max,
     2,
     {BUILTIN_float, BUILTIN_float},
     FUNCTIONID(float_max2),
     0},
    {avmplus::NativeID::float_min,
     2,
     {BUILTIN_float, BUILTIN_float},
     FUNCTIONID(float_min2),
     0},

#ifdef VMCFG_IA32
    {avmplus::NativeID::float_abs,
     1,
     {BUILTIN_float, BUILTIN_none},
     0,
     &CodegenLIR::emitFloatAbs},
    {avmplus::NativeID::float_reciprocal,
     1,
     {BUILTIN_float, BUILTIN_none},
     0,
     &CodegenLIR::emitFloatReciprocal},
    {avmplus::NativeID::float_rsqrt,
     1,
     {BUILTIN_float, BUILTIN_none},
     0,
     &CodegenLIR::emitFloatRsqrt},
    {avmplus::NativeID::float_sqrt,
     1,
     {BUILTIN_float, BUILTIN_none},
     0,
     &CodegenLIR::emitFloatSqrt},
#else
    {avmplus::NativeID::float_abs,
     1,
     {BUILTIN_float, BUILTIN_none},
     FUNCTIONID(float_abs),
     0},
    {avmplus::NativeID::float_reciprocal,
     1,
     {BUILTIN_float, BUILTIN_none},
     FUNCTIONID(float_reciprocal),
     0},
    {avmplus::NativeID::float_rsqrt,
     1,
     {BUILTIN_float, BUILTIN_none},
     FUNCTIONID(float_rsqrt),
     0},
    {avmplus::NativeID::float_sqrt,
     1,
     {BUILTIN_float, BUILTIN_none},
     FUNCTIONID(float_sqrt),
     0},
#endif
...
// Our builtinFunctionOptimizerHashMap maps from a [nativeId, argCount] key into
// the starting index in our specializedFunctions table.  The
// inlineBuiltinFunction function will then iterate over our
// specializedFunctions table for all matching [nativeId, argCount] pairs
// looking for a match to argument types.
void CodegenLIR::genBuiltinFunctionOptimizerHashMap() {
  builtinFunctionOptimizerHashMap =
      new (*alloc1) HashMap<uint32_t, uint32_t>(*alloc1, 100);

  uint32_t funcKey = 0;
  for (uint32_t i = 0; i < sizeof(specializedFunctions) / sizeof(FunctionMatch);
       i++) {
    uint32_t newFuncKey = genFunctionKey(specializedFunctions[i].methodId,
                                         specializedFunctions[i].argCount);
...
bool CodegenLIR::inlineBuiltinFunction(AbcOpcode, intptr_t, int argc,
                                       Traits *result, MethodInfo *mi) {
  if (haveDebugger)
    return false;
  ...
  for (int i = startingIndex; i < count; i++) {
    ...
    if (specializedFunctions[i].newFunction) {
      ...
      if (specializedFunctions[i].newFunction->count_args() == (uint32_t)argc) {
        if (argc == 1)
          localSet(sp - 1,
                   callIns(specializedFunctions[i].newFunction, 1,
                           getSpecializedArg(
                               sp, specializedFunctions[i].argType[0])),
                   result);
        else if (argc == 2)
          localSet(
              sp - 2,
              callIns(
                  specializedFunctions[i].newFunction, 2,
                  getSpecializedArg(sp - 1, specializedFunctions[i].argType[1]),
                  getSpecializedArg(sp, specializedFunctions[i].argType[0])),
              result);
      } else {
        if (argc == 1)
          localSet(
              sp - 1,
              callIns(
                  specializedFunctions[i].newFunction, 2, localGetp(sp - 1),
                  getSpecializedArg(sp, specializedFunctions[i].argType[0])),
              result);
        else if (argc == 2)
          localSet(
              sp - 2,
              callIns(
                  specializedFunctions[i].newFunction, 3, localGetp(sp - 2),
                  getSpecializedArg(sp - 1, specializedFunctions[i].argType[1]),
                  getSpecializedArg(sp, specializedFunctions[i].argType[0])),
              result);
      }
 ...
 * bind, and has already coerced arguments from whatever native type is
 * discovered, to the required types.  emitTypedCall() then just double-checks
 * (via assert) that the arg types are already correct.
 */
void CodegenLIR::emitTypedCall(AbcOpcode opcode, intptr_t method_id, int argc,
                               Traits *result, MethodInfo *mi) {
  AvmAssert(opcode != OP_construct);
  AvmAssert(state->value(state->sp() - argc)
                .notNull); // make sure null check happened
  ...
  for (int arg = 0; arg <= argc && arg <= ms->param_count(); arg++)
    AvmAssert(Traits::canAssign(state->value(objDisp + arg).traits,
                                ms->paramTraits(arg)));
...
#else
REALLY_INLINE void CodegenLIR::emitTypedCall(AbcOpcode opcode,
                                             intptr_t method_id, int argc,
                                             Traits *result, MethodInfo *mi) {
  if (inlineBuiltinFunction(opcode, method_id, argc, result, mi))
    return;
...
void CodegenLIR::emitCall(AbcOpcode opcode, intptr_t method_id, int argc,
                          Traits *result, MethodSignaturep ms) {
  int objDisp = state->sp() - argc;
...
void CodegenLIR::emitCall(AbcOpcode opcode, intptr_t method_id, int argc,
                          LIns *obj, Traits *objType, Traits *result,
                          MethodSignaturep ms) {
  int sp = state->sp();
  int dest = sp - argc;
  int objDisp = dest;
  ...
  case OP_constructsuper: {
    ...
    LIns *base = loadIns(LIR_ldp, offsetof(VTable, base), vtable, ACCSET_OTHER,
                         LOAD_CONST);
    method = loadIns(LIR_ldp, offsetof(VTable, init), base, ACCSET_OTHER,
                     LOAD_CONST);
    AvmAssert(result == VOID_TYPE);
    // The result returned by the init function will be ignored, but we must
    // receive the correct type to satisfy the ABI.
  ...
  case OP_callmethod: {
    ...
    method = loadIns(
        LIR_ldp,
        int32_t(offsetof(VTable, methods) + sizeof(MethodEnv *) * method_id),
        vtable, ACCSET_OTHER, LOAD_CONST);
    break;
  }
  case OP_callsuperid: {
    // stack in: obj arg1..N
    ...
    LIns *basevtable = loadIns(LIR_ldp, offsetof(VTable, base), declvtable,
                               ACCSET_OTHER, LOAD_CONST);
    method = loadIns(
        LIR_ldp,
        int32_t(offsetof(VTable, methods) + sizeof(MethodEnv *) * method_id),
        basevtable, ACCSET_OTHER, LOAD_CONST);
    break;
  }
  case OP_callstatic: {
    // stack in: obj arg1..N
    ...
    method = loadIns(
        LIR_ldp,
        int32_t(offsetof(AbcEnv, m_methods) + sizeof(MethodEnv *) * method_id),
        abcenv, ACCSET_OTHER, LOAD_CONST);
    break;
  }
  case OP_callinterface: {
    ...
    method = loadIns(
        LIR_ldp, offsetof(VTable, imt.entries) + index * sizeof(ImtThunkEnv *),
        vtable, ACCSET_OTHER, LOAD_CONST);
    // We don't need to blind method_id, as it is a MethodInfo* pointer, not a
    // method index.
  ...
  case OP_construct: {
    ...
    LIns *vtable = loadVTable(obj, objType);
    LIns *ivtable = loadIns(LIR_ldp, offsetof(VTable, ivtable), vtable,
                            ACCSET_OTHER, LOAD_CONST);
    method = loadIns(LIR_ldp, offsetof(VTable, init), ivtable, ACCSET_OTHER,
                     LOAD_CONST);
    LIns *createInstanceProc = loadIns(
        LIR_ldp, offsetof(VTable, createInstanceProc), ivtable, ACCSET_OTHER);
    obj = callIns(FUNCTIONID(createInstanceProc), 2, createInstanceProc, obj);
    objType = result;
    // The result of the expression is the newly-created object, not the result
    // of the function call as it is for other kinds of call instructions.  Save
    // the expression result now.
    localSet(dest, obj, result);
    // In the code below, 'result' is expected to be the result type of the
    // function to be called, which is the init function in this case.  The init
    // function should return void, which is actually represented at runtime by
    // a placeholder value, the atom 'undefined'.  This is not enforced,
    // however, and we must properly receive a value of the type actually
    // returned, which we then discard.
    result = ms->returnTraits();
    break;
  }
  default:
  ...
  for (int i = 0, index = objDisp; i <= argc; i++, index++) {
    ...
    case BUILTIN_float:
      v = (i == 0) ? obj
                   : lirout->insLoad(LIR_ldf, vars, index << VARSHIFT(info),
                                     ACCSET_VARS);
      stf(v, ap, disp, ACCSET_OTHER);
      disp +=
          sizeof(intptr_t); // only 4 bytes needed, but on X64 we use at least 8
                            // bytes anyway - same as "int" and "uint"
      break;
    case BUILTIN_float4:
      v = (i == 0) ? obj
                   : lirout->insLoad(LIR_ldf4, vars, index << VARSHIFT(info),
                                     ACCSET_VARS);
    ...
    case BUILTIN_number:
      v = (i == 0) ? obj
                   : lirout->insLoad(LIR_ldd, vars, index << VARSHIFT(info),
                                     ACCSET_VARS);
    ...
    case BUILTIN_int:
      v = (i == 0) ? obj
                   : lirout->insLoad(LIR_ldi, vars, index << VARSHIFT(info),
                                     ACCSET_VARS);
    ...
    default:
      v = (i == 0) ? obj
                   : lirout->insLoad(LIR_ldp, vars, index << VARSHIFT(info),
                                     ACCSET_VARS);
  ...
  LIns *target = loadIns(LIR_ldp, offsetof(MethodEnvProcHolder, _implGPR),
                         method, ACCSET_OTHER);
  ...
  if (!iid) {
    ...
    // It is doubtful that a tainted 'argc' can be usefully exploited due to the
    // density of surrounding code not under attacker control, but we expect
    // the values to be small enough that we'll not blind them anyhow, so why
    // take the risk?
    out = callIns(fid, 4, target, method, InsConst(argc, /*tainted*/ true),
                  apAddr);
  } else {
    const CallInfo *fid;
    switch (rbt) {
    ...
    out = callIns(fid, 5, target, method, InsConst(argc, /*tainted*/ true),
                  apAddr, iid);
  }
...
void CodegenLIR::emitSetslot(AbcOpcode opcode, int slot, int ptr_index,
  ...
  if (opcode == OP_setslot) {
    ...
    if (scopeTypes->size == 0) {
      ...
      LIns *scope = loadEnvScope();
      LIns *scopeobj =
          loadIns(LIR_ldp, offsetof(ScopeChain, _scopes) + 0 * sizeof(Atom),
                  scope, ACCSET_OTHER);
  ...
  if (!slotType || !slotType->isMachineType() || slotType == OBJECT_TYPE) {
    // slot type is Atom (for *, Object) or RCObject* (String, Namespace, or
    // other user types)
    ...
    callIns(wbAddr, 4, InsConstPtr(core->GetGC()), unoffsetPtr,
            lea(offset, ptr, /*tainted*/ true), value);
  }
#ifdef VMCFG_FLOAT
    ...
    AvmAssert(slotType == INT_TYPE || slotType == UINT_TYPE ||
              slotType == BOOLEAN_TYPE);
...
void CodegenLIR::emitConstruct(int argc, LIns *ctor, Traits *ctraits) {
  ...
  if (ctraits && (itraits = ctraits->itraits) != NULL &&
    ...
    // Cannot resolve signatures now because that could cause a premature
    // verification failure, one that should occur in the class's script-init.
    // If it's already resolved then we're good to go.
    if (itraits->init && itraits->init->isResolved() &&
        itraits->init->getMethodSignature()->argcOk(argc)) {
  ...
  LIns *args =
      storeAtomArgs(InsConstAtom(nullObjectAtom), argc, ctor_index + 1);
  LIns *newobj = callIns(FUNCTIONID(op_construct), 4, env_param, func,
                         InsConst(argc, /*tainted*/ true), args);
  liveAlloc(args);
...
static const CallInfo *getObjectVectorHelpers[VI_SIZE] = {
    ...
    FUNCTIONID(ObjectVectorObject_getDoubleProperty)};
...
static const CallInfo *getUIntVectorNativeHelpers[VI_SIZE] = {
    ...
    FUNCTIONID(UIntVectorObject_getNativeDoubleProperty)};
...
static const CallInfo *getUIntVectorHelpers[VI_SIZE] = {
    ...
    FUNCTIONID(UIntVectorObject_getDoubleProperty)};
...
static const CallInfo *getDoubleVectorNativeHelpers[VI_SIZE] = {
    ...
    FUNCTIONID(DoubleVectorObject_getNativeDoubleProperty)};
...
static const CallInfo *getDoubleVectorHelpers[VI_SIZE] = {
    ...
    FUNCTIONID(DoubleVectorObject_getDoubleProperty)};

#ifdef VMCFG_FLOAT
static const CallInfo *getFloatVectorNativeHelpers[VI_SIZE] = {
    FUNCTIONID(FloatVectorObject_getNativeUintProperty),
    FUNCTIONID(FloatVectorObject_getNativeIntProperty),
    FUNCTIONID(FloatVectorObject_getNativeDoubleProperty)};

static const CallInfo *getFloatVectorHelpers[VI_SIZE] = {
    FUNCTIONID(FloatVectorObject_getUintProperty),
    FUNCTIONID(FloatVectorObject_getIntProperty),
    FUNCTIONID(FloatVectorObject_getDoubleProperty)};

static const CallInfo *getFloat4VectorNativeHelpers[VI_SIZE] = {
    FUNCTIONID(Float4VectorObject_getNativeUintProperty),
    FUNCTIONID(Float4VectorObject_getNativeIntProperty),
    FUNCTIONID(Float4VectorObject_getNativeDoubleProperty)};

static const CallInfo *getFloat4VectorHelpers[VI_SIZE] = {
    FUNCTIONID(Float4VectorObject_getUintProperty),
    FUNCTIONID(Float4VectorObject_getIntProperty),
    FUNCTIONID(Float4VectorObject_getDoubleProperty)};
#endif

static const CallInfo *getGenericHelpers[VI_SIZE] = {
    FUNCTIONID(getpropertylate_u), FUNCTIONID(getpropertylate_i),
    FUNCTIONID(getpropertylate_d)};

LIns *CodegenLIR::emitInlineVectorRead(int objIndexOnStack, LIns *index,
                                       size_t arrayDataOffset, size_t lenOffset,
#ifdef VMCFG_VECTOR_SMASH_PROTECTION
                                       size_t cookieOffset,
#endif
                                       size_t entriesOffset, int scale,
                                       LOpcode load_item,
  ...
  // value = *(arrayData + (index<<scale) + entriesOffset) ; value =
  // array->data->entries[index]
  //
  // In the case of float4 the (arrayData + entriesOffset) has to be rounded up
  // to a 16-byte boundary at run-time to conform to alignment restrictions. It
  // would be better to have a different vector representation and a guarantee
  // about alignment from the storage manager, so that we could avoid the
  // adjustment, but that's a future improvement.
  //
  // The "(unsigned)index" compare trick works because vectors are shorter than
  // 2^31 elements, that fact is verified statically in the vector code, see eg
  // checkReadIndex_i.
  //
  // We also depend on vectors being shorter than 2^32 bytes in computing the
  // scaled offset for the vector.  The 4GB object limit is enforced by MMgc as
  // of September 2011.

  LIns *arrayPtr = localGetp(objIndexOnStack);
  LIns *arrayLen =
      loadIns(LIR_ldi, int32_t(lenOffset), arrayPtr, ACCSET_OTHER, LOAD_NORMAL);
  LIns *cmp = binaryIns(LIR_geui, index, arrayLen);
#ifdef VMCFG_VECTOR_SMASH_PROTECTION
  LIns *arrayData = loadIns(LIR_ldp, int32_t(arrayDataOffset), arrayPtr,
                            ACCSET_OTHER, LOAD_NORMAL);
  LIns *sessionSecret = InsConst(GCHeap::secret);
  LIns *testCookie = binaryIns(LIR_xori, arrayLen, sessionSecret);
  LIns *arrayCookie = loadIns(LIR_ldi, int32_t(cookieOffset), arrayData,
                              ACCSET_OTHER, LOAD_NORMAL);
  ...
  branchToLabel(LIR_jf, cmp, begin_label);
#endif
  // For read, the inlined code handles all non-error cases.  If we arrive here,
  // there is definitely a problem. We call the helper, but only to allow it to
  // handle the error.  The function is not expected to return, and we do not
  // receive a result.
  if (load_item == LIR_ldf4)
    callIns(
        helper, 3, arrayPtr, InsConstPtr(NULL),
        index); // we don't really need the returned arg, hence we pass null.
  else
  ...
  // If we are not using vector smash protection, we can load the data pointer a
  // bit later and avoid some register pressure.
  LIns *arrayData = loadIns(LIR_ldp, int32_t(arrayDataOffset), arrayPtr,
                            ACCSET_OTHER, LOAD_NORMAL);
#endif
  switch (load_item) {
  case LIR_ldf4: {
    LIns *base =
        binaryIns(LIR_addp, arrayData,
                  InsConstPtr((const void *)(uintptr_t(entriesOffset) +
                                             15))); // arrayData + entries + 15
    LIns *baseRounded = binaryIns(
        LIR_andp, base, InsConstPtr((const void *)~uintptr_t(15))); // & ~15
  ...
  case LIR_ldi:
  case LIR_ldf: {
    LIns *valOffset = binaryIns(
        LIR_addp, arrayData, ui2p(binaryIns(LIR_lshi, index, InsConst(scale))));
    return loadIns(load_item, int32_t(entriesOffset), valOffset, ACCSET_OTHER,
                   LOAD_NORMAL);
  }
  default:
...
LIns *CodegenLIR::emitInlineSpeculativeArrayRead(int objIndexOnStack,
  ...
  size_t denseArrayDataOffset =
      offsetof(ArrayObject, m_denseArray.m_header.data);
  ...
  LIns *arrayPtr = localGetp(objIndexOnStack);
  LIns *arraySimpleLen = loadIns(LIR_ldi, int32_t(lenIfSimpleOffset), arrayPtr,
                                 ACCSET_OTHER, LOAD_NORMAL);
...
#ifdef VMCFG_VECTOR_SMASH_PROTECTION
  LIns *arrayData = loadIns(LIR_ldp, int32_t(denseArrayDataOffset), arrayPtr,
                            ACCSET_OTHER, LOAD_NORMAL);
  LIns *sessionSecret = InsConst(GCHeap::secret);
  LIns *testCookie = binaryIns(LIR_xori, arraySimpleLen, sessionSecret);
  LIns *arrayCookie = loadIns(LIR_ldi, int32_t(denseArrayCookieOffset),
                              arrayData, ACCSET_OTHER, LOAD_NORMAL);
  LIns *cksumCmp = binaryIns(LIR_eqi, testCookie, arrayCookie);
...
// Generate code for get obj[index] where index is a signed or unsigned integer
// type, or double type.
LIns *CodegenLIR::emitGetIndexedProperty(int objIndexOnStack, LIns *index,
                                         Traits *result, IndexKind idxKind) {
  ...
  if (objType == ARRAY_TYPE) {
    ...
    if (core->config.jitconfig.opt_array_read_fastpath &&
        (idxKind == VI_INT || idxKind == VI_UINT)) {
      LIns *value =
          emitInlineSpeculativeArrayRead(objIndexOnStack, index, getter);
      return atomToNativeRep(result, value);
    }
  } else if (objType != NULL && objType->subtypeof(VECTOROBJ_TYPE)) {
    if (core->config.jitconfig.opt_inline_vector_access &&
        (idxKind == VI_INT || idxKind == VI_UINT)) {
      typedef TracedListData<Atom> STORAGE;
      return atomToNativeRep(
          result, emitInlineVectorRead(
                      objIndexOnStack, index,
                      offsetof(ObjectVectorObject, m_list.m_header.data),
                      offsetof(ObjectVectorObject, m_list.m_header.len),
#ifdef VMCFG_VECTOR_SMASH_PROTECTION
                      offsetof(STORAGE, cookie),
#endif
                      offsetof(STORAGE, entries), sizeof(void *) == 8 ? 3 : 2,
                      LIR_ldp, getObjectVectorHelpers[idxKind]));
    }
    ...
    if (result == INT_TYPE) {
      if (core->config.jitconfig.opt_inline_vector_access &&
        ...
        return emitInlineVectorRead(
            ...
            offsetof(STORAGE, cookie),
#endif
            offsetof(STORAGE, entries), 2, LIR_ldi,
    ...
    if (result == NUMBER_TYPE) {
      if (core->config.jitconfig.opt_inline_vector_access &&
        ...
        return emitInlineVectorRead(
            ...
            offsetof(STORAGE, cookie),
#endif
            offsetof(STORAGE, entries), 3, LIR_ldd,
    ...
    if (result == FLOAT_TYPE) {
      if (core->config.jitconfig.opt_inline_vector_access &&
        ...
        return emitInlineVectorRead(
            ...
            offsetof(STORAGE, cookie),
#endif
            offsetof(STORAGE, entries), 2, LIR_ldf,
    ...
    if (result == FLOAT4_TYPE) {
      if (core->config.jitconfig.opt_inline_vector_access &&
        ...
        return emitInlineVectorRead(
            ...
            offsetof(STORAGE, cookie),
#endif
            offsetof(STORAGE, entries), 4, LIR_ldf4,
            getFloat4VectorNativeHelpers[idxKind]);
      }
      /* handle here the case of native helpers, it is different than all the
       * others */
      LIns *retval = lirout->insAlloc(sizeof(float4_t));
      callIns(getFloat4VectorNativeHelpers[idxKind], 3,
              localGetp(objIndexOnStack), lea(0, retval), index);
  ...
  if (getter) {
    ...
    LIns *value = callIns(getGenericHelpers[idxKind], 3, env_param,
                          loadAtomRep(objIndexOnStack), index);
    return atomToNativeRep(result, value);
  }
}
...
static const CallInfo *setObjectVectorHelpers[VI_SIZE] = {
    ...
    FUNCTIONID(ObjectVectorObject_setDoubleProperty)};

static const CallInfo *setKnownTypeObjectVectorHelpers[VI_SIZE] = {
    FUNCTIONID(ObjectVectorObject_setKnownUintProperty),
    FUNCTIONID(ObjectVectorObject_setKnownIntProperty),
    FUNCTIONID(ObjectVectorObject_setKnownDoubleProperty)};

static const CallInfo *setKnownPointerTypeObjectVectorHelpers[VI_SIZE] = {
    FUNCTIONID(ObjectVectorObject_setKnownUintPropertyWithPointer),
    FUNCTIONID(ObjectVectorObject_setKnownIntPropertyWithPointer),
    FUNCTIONID(ObjectVectorObject_setKnownDoublePropertyWithPointer)};
...
static const CallInfo *setDoubleVectorNativeHelpers[VI_SIZE] = {
    ...
    FUNCTIONID(DoubleVectorObject_setNativeDoubleProperty)};
...
static const CallInfo *setDoubleVectorHelpers[VI_SIZE] = {
    ...
    FUNCTIONID(DoubleVectorObject_setDoubleProperty)};

#ifdef VMCFG_FLOAT
static const CallInfo *setFloatVectorNativeHelpers[VI_SIZE] = {
    FUNCTIONID(FloatVectorObject_setNativeUintProperty),
    FUNCTIONID(FloatVectorObject_setNativeIntProperty),
    FUNCTIONID(FloatVectorObject_setNativeDoubleProperty)};

static const CallInfo *setFloatVectorHelpers[VI_SIZE] = {
    FUNCTIONID(FloatVectorObject_setUintProperty),
    FUNCTIONID(FloatVectorObject_setIntProperty),
    FUNCTIONID(FloatVectorObject_setDoubleProperty)};

static const CallInfo *setFloat4VectorNativeHelpers[VI_SIZE] = {
    FUNCTIONID(Float4VectorObject_setNativeUintProperty),
    FUNCTIONID(Float4VectorObject_setNativeIntProperty),
    FUNCTIONID(Float4VectorObject_setNativeDoubleProperty)};

static const CallInfo *setFloat4VectorHelpers[VI_SIZE] = {
    FUNCTIONID(Float4VectorObject_setUintProperty),
    FUNCTIONID(Float4VectorObject_setIntProperty),
    FUNCTIONID(Float4VectorObject_setDoubleProperty)};
#endif

static const CallInfo *setGenericHelpers[VI_SIZE] = {
    FUNCTIONID(setpropertylate_u), FUNCTIONID(setpropertylate_i),
    FUNCTIONID(setpropertylate_d)};

// Not for values that require a write barrier!
void CodegenLIR::emitInlineVectorWrite(int objIndexOnStack, LIns *index,
                                       LIns *value, size_t arrayDataOffset,
                                       size_t lenOffset,
#ifdef VMCFG_VECTOR_SMASH_PROTECTION
                                       size_t cookieOffset,
#endif
  ...
  // else
  //   *(arrayData + (index<<scale) + entriesOffset) = value ;
  //   array->data->entries[index] = value
  //
  // In the case of float4 the (arrayData + entriesOffset) has to be rounded up
  // to a 16-byte boundary at run-time to conform to alignment restrictions. It
  // would be better to have a different vector representation and a guarantee
  ...
  // TODO: By placing the check here, we avoid a redundant check if we are
  // expanding the vector via the slow path. We are placing a lot of code
  // between the suspendCSE()/resumeCSE() pair, however, and potentially giving
  // up some CSE optimization.  Profile to see if it would be better to hoist
  // the loads and ALU ops above the suspendCSE. Possibly hoist just the load of
  // the session secret, as this load is eligible for CSE and likely to benefit.
  LIns *sessionSecret = InsConst(GCHeap::secret);
  LIns *testCookie = binaryIns(LIR_xori, arrayLen, sessionSecret);
  LIns *arrayData = loadIns(LIR_ldp, int32_t(arrayDataOffset), arrayPtr,
                            ACCSET_OTHER, LOAD_NORMAL);
  ...
  branchToLabel(LIR_jf, cmp, begin_label);
#endif
  // The inlined code is just a fastpath.  Handle the general case here. We may
  // need to grow the vector, otherwise, we'll throw an exception.
  ...
  case LIR_stf: {
    LIns *valOffset = binaryIns(
        LIR_addp, arrayData, ui2p(binaryIns(LIR_lshi, index, InsConst(scale))));
    storeIns(store_item, value, int32_t(entriesOffset), valOffset,
             ACCSET_OTHER);
    break;
  }
  default:
...
// Generate code for 'obj[index] = value' where index is a signed or unsigned
// integer type, or a double.
void CodegenLIR::emitSetIndexedProperty(int objIndexOnStack,
                                        int valIndexOnStack, LIns *index,
                                        IndexKind idxKind) {
  ...
  if (objType == ARRAY_TYPE) {
    ...
    if (valueType == INT_TYPE) {
      if (core->config.jitconfig.opt_inline_vector_access &&
          (idxKind == VI_INT || idxKind == VI_UINT)) {
        typedef ListData<int32_t> STORAGE;
        emitInlineVectorWrite(objIndexOnStack, index, localGet(valIndexOnStack),
                              offsetof(IntVectorObject, m_list.m_header.data),
                              offsetof(IntVectorObject, m_list.m_header.len),
#ifdef VMCFG_VECTOR_SMASH_PROTECTION
                              offsetof(STORAGE, cookie),
#endif
                              offsetof(STORAGE, entries), 2, LIR_sti,
    ...
    if (valueType == NUMBER_TYPE) {
      if (core->config.jitconfig.opt_inline_vector_access &&
          (idxKind == VI_INT || idxKind == VI_UINT)) {
        typedef ListData<double> STORAGE;
        emitInlineVectorWrite(
            objIndexOnStack, index, localGetd(valIndexOnStack),
            offsetof(DoubleVectorObject, m_list.m_header.data),
            offsetof(DoubleVectorObject, m_list.m_header.len),
#ifdef VMCFG_VECTOR_SMASH_PROTECTION
            offsetof(STORAGE, cookie),
#endif
            offsetof(STORAGE, entries), 3, LIR_std,
    ...
    if (valueType == FLOAT_TYPE) {
      if (core->config.jitconfig.opt_inline_vector_access &&
        ...
        emitInlineVectorWrite(objIndexOnStack, index,
                              ...
                              offsetof(STORAGE, cookie),
#endif
                              offsetof(STORAGE, entries), 2, LIR_stf,
    ...
    if (valueType == FLOAT4_TYPE) {
      if (core->config.jitconfig.opt_inline_vector_access &&
        ...
        emitInlineVectorWrite(
            objIndexOnStack, index, localGetf4Addr(valIndexOnStack),
            offsetof(Float4VectorObject, m_list.m_header.data),
            offsetof(Float4VectorObject, m_list.m_header.len),
#ifdef VMCFG_VECTOR_SMASH_PROTECTION
            offsetof(STORAGE, cookie),
#endif
            offsetof(STORAGE, entries), 4, LIR_stf4,
  ...
  if (setter) {
    ...
    value = loadAtomRep(valIndexOnStack);
    callIns(setGenericHelpers[idxKind], 4, env_param,
            loadAtomRep(objIndexOnStack), index, value);
  }
}
  ...
  // from int or uint, or if it is a constant in range for int or uint.
    ...
    if (index->opcode() == LIR_i2d FLOAT_ONLY(|| index->opcode() == LIR_i2f)) {
      ...
      *indexType = UINT_TYPE;
      ...
      if (double(i) == d) {
        *indexType = INT_TYPE;
        return InsConst(i, index->isTainted());
      }
    }
  }
...
void CodegenLIR::emit(AbcOpcode opcode, uintptr_t op1, uintptr_t op2,
                      Traits *result) {
  int sp = state->sp();
  ...
  case OP_li32: {
    ...
    int32_t disp = 0;
    LIns *realAddr =
        mopAddrToRangeCheckedRealAddrAndDisp(mopAddr, mi.size, &disp);
    LIns *i2 = loadIns(mi.op, disp, realAddr, ACCSET_OTHER);
    i2->setLoadTainted(true); // ISSUE: This may clobber performance of absolute
                              // addressing in Alchemy code!
#else
    LIns *realAddr =
        mopAddrToRangeCheckedRealAddrAndDisp(mopAddr, mi.size, NULL);
  ...
  case OP_lf32x4: {
    ...
    Ins(LIR_memfence);
#endif
    // TODO: inlining.  The appropriate condition is probably *not*
    // VMCFG_MOPS_USE_EXPANDED_LOADSTORE_FP.
    int32_t index = (int32_t)op1;
    LIns *mopAddr = binaryIns(LIR_andi, localGet(index), InsConst(~15U));
    LIns *realAddr =
        mopAddrToRangeCheckedRealAddrAndDisp(mopAddr, sizeof(float4_t), NULL);
  ...
  case OP_si32: {
    ...
    st->setStoreTainted(true); // ISSUE: This may clobber performance of
                               // absolute addressing in Alchemy code!
#else
    LIns *realAddr =
        mopAddrToRangeCheckedRealAddrAndDisp(mopAddr, mi.size, NULL);
  ...
  case OP_sf64: {
    bool singlePrecision =
        IFFLOAT(state->value(sp - 1).traits == FLOAT_TYPE, false);
  ...
  case OP_lookupswitch: {
    ...
    // Delete any trailing table entries that == default case (effective for asc
    // output)
    while (count > 0 &&
           targetpc == (state->abc_pc + AvmCore::readS24(pc + 3 * (count - 1))))
      count--;
    ...
    if (count > 0) {
      ...
      // count == 1 is equivalent to if (case) else (default), so don't bother
      // with jtbl
      if (NJ_JTBL_SUPPORTED && count > 1) {
        ...
        for (int i = 0; i < count; i++) {
          const uint8_t *target = state->abc_pc + AvmCore::readS24(pc + 3 * i);
          branchToAbcPos(LIR_jt, binaryIns(LIR_eqi, index, InsConst(i)),
                         target);
        }
      }
    } else {
  ...
  case OP_returnvalue: {
...
#ifdef DEBUGGER
    if (haveDebugger) {
      callIns(FUNCTIONID(debugExit), 2, env_param, csn);
    ...
    if (core->samplerEnabled) {
      LIns *sampleTicks = loadIns(LIR_ldi, offsetof(AvmCore, sampleTicks),
                                  coreAddr, ACCSET_OTHER);
      CodegenLabel &afterTakeSampleExit_label =
          createLabel("afterTakeSampleExit");
    ...
    LIns *nextMethodFrame = loadIns(LIR_ldp, offsetof(MethodFrame, next),
                                    methodFrame, ACCSET_OTHER);
    ...
    if (opcode == OP_returnvalue) {
      ...
      if (t && t != VOID_TYPE) {
        ...
        retvalue =
            callIns(FUNCTIONID(coerce), 3, env_param, retvalue, InsConstPtr(t));
  ...
  case OP_typeof: {
    ...
    LIns *i3 = callIns(FUNCTIONID(typeof), 2, coreAddr, value);
  ...
  case OP_negate: {
    ...
    if (vt == NUMBER_TYPE)
      ...
      emitNumericOp1(index,
                     BasicLIREmitter(this, LIR_negd, FUNCTIONID(op_negate)));
    }
#else
    localSet(index, Ins(LIR_negd, localGetd(index)), result);
#endif // VMCFG_FLOAT
  ...
  case OP_declocal: {
    ...
    LIns *addIns = NULL;
    // We do not blind the increment/decrement value, as it is always 1 or -1.
    AvmAssert(incr == 1 || incr == -1);
#ifdef VMCFG_FLOAT
    const Traits *vt = state->value(index).traits;
    AvmAssert(result == vt);
    if (vt == NUMBER_TYPE)
      addIns = binaryIns(LIR_addd, localGetd(index),
                         i2dIns(InsConst(incr, /*tainted*/ false)));
    else if (vt == FLOAT_TYPE)
      addIns = binaryIns(LIR_addf, localGetf(index),
                         InsConstFlt((float)incr, /*tainted*/ false));
    else if (vt == FLOAT4_TYPE)
      addIns =
          binaryIns(LIR_addf4, localGetf4(index),
                    Ins(LIR_f2f4, InsConstFlt((float)incr, /*tainted*/ false)));
    else {
      // NUMERIC - i.e., unknown
  ...
  case OP_decrement_i: {
    ...
    localSet(
        index,
        binaryIns(LIR_addi, localGet(index), InsConst(incr, /*tainted*/ false)),
        result);
    break;
  }
  ...
  case OP_modulo: {
    ...
    if (result == FLOAT_TYPE) {
      ...
      LIns *f4x = f2dIns(lirout->ins1(LIR_f4x, f4)),
           *g4x = f2dIns(lirout->ins1(LIR_f4x, g4));
      LIns *ox = callIns(FUNCTIONID(mod), 2, f4x, g4x);
      LIns *f4y = f2dIns(lirout->ins1(LIR_f4y, f4)),
           *g4y = f2dIns(lirout->ins1(LIR_f4y, g4));
      LIns *oy = callIns(FUNCTIONID(mod), 2, f4y, g4y);
      LIns *f4z = f2dIns(lirout->ins1(LIR_f4z, f4)),
           *g4z = f2dIns(lirout->ins1(LIR_f4z, g4));
      LIns *oz = callIns(FUNCTIONID(mod), 2, f4z, g4z);
      LIns *f4w = f2dIns(lirout->ins1(LIR_f4w, f4)),
           *g4w = f2dIns(lirout->ins1(LIR_f4w, g4));
      LIns *ow = callIns(FUNCTIONID(mod), 2, f4w, g4w);
      localSet(sp - 1,
               lirout->ins4(LIR_ffff2f4, d2fIns(ox), d2fIns(oy), d2fIns(oz),
                            d2fIns(ow)),
               result);
    } else if (result == NUMBER_TYPE) {
      ...
      AvmAssert(result == OBJECT_TYPE &&
                state->value(sp).traits == OBJECT_TYPE &&
                state->value(sp - 1).traits == OBJECT_TYPE &&
                state->value(sp).notNull && state->value(sp - 1).notNull);
  ...
  case OP_subtract: {
    ...
    AvmAssert(state->value(sp - 1).traits == result &&
              state->value(sp - 1).traits == result);

    switch (opcode) {
    default:
      AvmAssertMsg(false, "Can only handle OP_divide/OP_multiply/OP_subtract");
    case OP_divide:
      opf = LIR_divf;
      opf4 = LIR_divf4;
      opd = LIR_divd;
      interpCall = FUNCTIONID(op_divide);
      break;
    case OP_multiply:
      opf = LIR_mulf;
      opf4 = LIR_mulf4;
      opd = LIR_muld;
      interpCall = FUNCTIONID(op_multiply);
      break;
    case OP_subtract:
      opf = LIR_subf;
      opf4 = LIR_subf4;
      opd = LIR_subd;
      interpCall = FUNCTIONID(op_subtract);
      break;
    }

    if (result == FLOAT_TYPE) {
      localSet(sp - 1, binaryIns(opf, localGetf(sp - 1), localGetf(sp)),
               result);
    } else if (result == NUMBER_TYPE) {
      localSet(sp - 1, binaryIns(opd, localGetd(sp - 1), localGetd(sp)),
               result);
    } else if (result == FLOAT4_TYPE) {
      localSet(sp - 1, binaryIns(opf4, localGetf4(sp - 1), localGetf4(sp)),
               result);
    } else {
      AvmAssert(result == OBJECT_TYPE &&
                state->value(sp).traits == OBJECT_TYPE &&
  ...
  case OP_getsuper: {
    ...
    LIns *i3 = callIns(FUNCTIONID(getsuper), 3, env_param, obj, multi);
    liveAlloc(multi);
  ...
  case OP_setsuper: {
    ...
    callIns(FUNCTIONID(setsuper), 4, env_param, obj, multi, value);
    liveAlloc(multi);
    break;
  }
  ...
  case OP_nextvalue: {
    ...
    AvmAssert(state->value(sp).traits == INT_TYPE);
    LIns *index = localGet(sp);
    LIns *i1 = callIns((opcode == OP_nextname) ? FUNCTIONID(nextname)
                                               : FUNCTIONID(nextvalue),
                       3, env_param, obj, index);
  ...
  case OP_hasnext: {
    ...
    AvmAssert(state->value(sp).traits == INT_TYPE);
    LIns *index = localGet(sp);
    LIns *i1 = callIns(FUNCTIONID(hasnext), 3, env_param, obj, index);
  ...
  case OP_hasnext2: {
    ...
    LIns *i1 = callIns(FUNCTIONID(hasnextproto), 3, env_param, obj, index);
    localSet(obj_index, loadIns(LIR_ldp, 0, obj, ACCSET_LOAD_ANY),
             OBJECT_TYPE); // Atom obj
    localSet(index_index, loadIns(LIR_ldi, 0, index, ACCSET_LOAD_ANY),
             INT_TYPE); // int32_t index
  ...
  case OP_newfunction: {
    ...
    LIns *i3 =
        callIns(FUNCTIONID(newfunction), 3, env_param, InsConstPtr(func), ap);
    liveAlloc(ap);
  ...
  case OP_call: {
    ...
    case BUILTIN_methodClosure: {
      ...
      LIns *call_ptr =
          ldp(func, offsetof(FunctionObject, m_call_ptr), ACCSET_OTHER);
      result_ins = callIns(FUNCTIONID(call_ptr), 4, call_ptr, func,
                           InsConst(argc, /*tainted*/ true), ap);
      break;
    }
    default:
      // generic call
      ...
      ap = storeAtomArgs(loadAtomRep(funcDisp + 1), argc, funcDisp + 2);
      result_ins = callIns(FUNCTIONID(op_call_atom), 4, env_param, func,
                           InsConst(argc, /*tainted*/ true), ap);
      break;
    }
    liveAlloc(ap);
  ...
  case OP_callpropvoid: {
    ...
    LIns *receiver =
        opcode == OP_callproplex ? InsConstAtom(nullObjectAtom) : base;
    ...
    if (AvmCore::isSlotBinding(b)) {
      ...
      // todo if funcValue is already a ScriptObject then don't box it, use a
      // different helper.
      LIns *funcValue =
          loadFromSlot(baseDisp, AvmCore::bindingToSlotId(b), slotType);
      LIns *funcAtom = nativeToAtom(funcValue, slotType);
      out = callIns(FUNCTIONID(op_call_atom), 4, env_param, funcAtom,
                    InsConst(argc, /*tainted*/ true), ap);
      ...
      LIns *handler = loadIns(LIR_ldp, offsetof(CallCache, call_handler),
                              cacheAddr, ACCSET_OTHER);
      out = callIns(FUNCTIONID(call_cache_handler), 6, handler, cacheAddr, base,
                    InsConst(argc, /*tainted*/ true), ap, env_param);
    } else {
      // generic late bound call to anything
      out = callIns(FUNCTIONID(callprop_late), 5, env_param, base, multi,
                    InsConst(argc, /*tainted*/ true), ap);
      liveAlloc(multi);
    }
    liveAlloc(ap);
  ...
  case OP_constructprop: {
    ...
    LIns *i3 = callIns(FUNCTIONID(construct_late), 4, env_param, multi,
                       InsConst(argc, /*tainted*/ true), ap);
    liveAlloc(multi);
    liveAlloc(ap);
  ...
  case OP_callsupervoid: {
    ...
    LIns *i3 = callIns(FUNCTIONID(callsuper), 4, env_param, multi,
                       InsConst(argc, /*tainted*/ true), ap);
    liveAlloc(multi);
    liveAlloc(ap);
  ...
  case OP_applytype: {
    ...
    LIns *i3 = callIns(FUNCTIONID(op_applytype), 4, env_param, func,
                       InsConst(argc, /*tainted*/ true), ap);
    liveAlloc(ap);
  ...
  case OP_newobject: {
    ...
    LIns *i3 = callIns(FUNCTIONID(op_newobject), 3, env_param,
                       lea(sizeof(Atom) * (2 * argc - 1), ap, /*tainted*/ true),
                       InsConst(argc, /*tainted*/ true));
    liveAlloc(ap);
  ...
  case OP_newcatch: {
    ...
    LIns *activation =
        callIns(FUNCTIONID(newcatch), 2, env_param, InsConstPtr(result));
  ...
  case OP_newarray: {
    ...
    LIns *ap = storeAtomArgs(argc, arg0);
    LIns *i3 = callIns(FUNCTIONID(newarray), 3, env_param,
                       InsConst(argc, /*tainted*/ true), ap);
    liveAlloc(ap);
  ...
  case OP_newclass: {
    ...
    LIns *i3 = callIns(FUNCTIONID(newclass), 5, env_param, InsConstPtr(ctraits),
                       base, outer, ap);
    liveAlloc(ap);
  ...
  case OP_getdescendants: {
    ...
    LIns *out = callIns(FUNCTIONID(getdescendants), 3, env_param, obj, multi);
    liveAlloc(multi);
  ...
  case OP_findproperty: {
    ...
    LIns *i3 =
        callIns(FUNCTIONID(findproperty), 7, env_param, outer, ap,
                InsConst(extraScopes), multi,
                InsConst((int32_t)(opcode == OP_findpropstrict)), withBase);
    liveAlloc(multi);
    liveAlloc(ap);
  ...
  case OP_getproperty: {
    ...
    LIns *index = NULL;
    bool maybeIntegerIndex = !attr && multiname->isRtname() &&
    ...
    if (maybeIntegerIndex && indexType == INT_TYPE) {
...
#ifdef VMCFG_FASTPATH_ADD_INLINE
      if (inlineFastpath) {
        // TODO: Since a helper must be called anyhow, I conjecture that fused
        // add-and-index helpers may be more efficient than the inlining done
        // here.  On the other hand, it is plausible that we would go the other
        // way and completely inline the getNativeXXXProperty cases, in which
        ...
        if (op == LIR_addd || op == LIR_subd) {
          ...
          if (a && b) {
            ...
            LIns *addOp = branchJovToLabel(
                (op == LIR_addd) ? LIR_addjovi : LIR_subjovi, a, b, slow_path);
            branchToLabel(LIR_jt, binaryIns(LIR_lti, addOp, InsConst(0)),
                          slow_path);
            LIns *value0 =
                emitGetIndexedProperty(sp - 1, addOp, result, VI_UINT);
            stForTraits(result, value0, tempResult, 0, ACCSET_STORE_ANY);
            branchToLabel(LIR_j, NULL, done_path);

            emitLabel(slow_path);
            LIns *value1 =
                emitGetIndexedProperty(sp - 1, index, result, VI_DOUBLE);
            stForTraits(result, value1, tempResult, 0, ACCSET_STORE_ANY);

            emitLabel(done_path);
            localSet(sp - 1,
                     ldForTraits(result, tempResult, 0, ACCSET_LOAD_ANY),
                     result);
      ...
      LIns *obj = loadAtomRep(sp - 1);
      LIns *value =
          callIns(FUNCTIONID(getprop_index), 4, env_param, obj, multi, index);
      ...
      if (multiname->isRuntime()) {
        ...
        LIns *cacheAddr = InsConstPtr(cache);
        LIns *handler = loadIns(LIR_ldp, offsetof(GetCache, get_handler),
                                cacheAddr, ACCSET_OTHER);
        value = callIns(FUNCTIONID(get_cache_handler), 4, handler, cacheAddr,
                        env_param, obj);
      }
  ...
  case OP_setproperty: {
    ...
    if (maybeIntegerIndex && indexType == INT_TYPE) {
      ...
      if (inlineFastpath) {
        ...
        if (op == LIR_addd || op == LIR_subd) {
          ...
          if (a && b) {
            ...
            // (for subtraction, we emit (a-b) and call
            // set/initprop_index_subtract)
            LIns *addOp = branchJovToLabel(
                (op == LIR_addd) ? LIR_addjovi : LIR_subjovi, a, b, slow_path);
            branchToLabel(LIR_jt, binaryIns(LIR_lti, addOp, InsConst(0)),
                          slow_path);
      ...
      const CallInfo *func = (opcode == OP_setproperty)
                                 ? FUNCTIONID(setprop_index)
      ...
      if (opcode == OP_setproperty) {
        if (!multiname->isRuntime()) {
          ...
          LIns *cacheAddr = InsConstPtr(cache);
          LIns *handler = loadIns(LIR_ldp, offsetof(SetCache, set_handler),
                                  cacheAddr, ACCSET_OTHER);
          callIns(FUNCTIONID(set_cache_handler), 5, handler, cacheAddr, obj,
                  value, env_param);
        } else {
        ...
        // initproplate is rare in jit code because we typically interpret
        // static initializers, and constructor initializers tend to early-bind
        // successfully.
  ...
  case OP_deleteproperty: {
    ...
    if (!multiname->isRtname()) {
      ...
      LIns *i3 = callIns(FUNCTIONID(delproperty), 3, env_param, obj, multi);
      liveAlloc(multi);
      ...
      LIns *value = callIns(FUNCTIONID(delpropertyHelper), 4, env_param, obj,
                            _tempname, index);
  ...
  case OP_esc_xelem: // ToXMLString will call EscapeElementValue
  {
    ...
    int32_t index = (int32_t)op1;
    LIns *value = loadAtomRep(index);
    LIns *i3 = callIns(FUNCTIONID(ToXMLString), 2, coreAddr, value);
    AvmAssert(result == STRING_TYPE);
    localSet(index, i3, result);
    break;
  }
  ...
  case OP_esc_xattr: {
    ...
    int32_t index = (int32_t)op1;
    LIns *value = loadAtomRep(index);
    LIns *i3 = callIns(FUNCTIONID(EscapeAttributeValue), 2, coreAddr, value);
    AvmAssert(result == STRING_TYPE);
    localSet(index, i3, result);
    break;
  }
...
void CodegenLIR::emitIf(AbcOpcode opcode, const uint8_t *target, int a, int b) {
  ...
  //
  // compile instructions that cannot throw exceptions before we add exception
  // handling logic
  //
...
// Try to optimize comparison of a function call yielding NUMBER_TYPE with
// another expression of INT_TYPE.  We may be able to simplify the function
// and/or the comparison based on the type or value, if constant, of the other
// argument.  The function call is normally presumed to be the left-hand
// argument.  The swap parameter, if true, reverses this convention.
...
LIns *CodegenLIR::optimizeIntCmpWithNumberCall(int callIndex, int otherIndex,
                                               LOpcode icmp, bool swap) {
  LIns *numSide = localGetd(callIndex);
  const CallInfo *ci = numSide->callInfo();

  // Try to optimize charCodeAt to return an integer if possible. Because it can
  // return NaN for out of bounds access, we need to limit our support to
  // constant integer values that generate the same results for both NaN
  // (floating point result) and zero (NaN cast to integer result). These are
  // the six possibilities: String.CharCodeAt == int - any constant integer but
  // zero String.CharCodeAt < int  - zero or any negative integer constant
  ...
  if (ci == FUNCTIONID(String_charCodeAtDI) ||
      ci == FUNCTIONID(String_charCodeAtDU) ||
      ci == FUNCTIONID(String_charCodeAtDD)) {
...
LIns *CodegenLIR::optimizeStringCmpWithStringCall(int callIndex, int otherIndex,
                                                  LOpcode icmp, bool swap) {
  LIns *callSide = localGetp(callIndex);
  const CallInfo *ci = callSide->callInfo();
  if (ci == FUNCTIONID(String_charAtI) || ci == FUNCTIONID(String_charAtU) ||
      ci == FUNCTIONID(String_charAtD)) {
    ...
    if (s->length() == 1 && s->charAt(0) != 0) {
      ...
      // NOTE: Reconsider whether blinding is really needed here.
      strSide = InsConst(firstChar, /*tainted*/ true);
...
LIns *CodegenLIR::cmpOptimization(int lhsi, int rhsi, LOpcode icmp,
                                  LOpcode ucmp, LOpcode fcmp,
                                  bool strictOperation /*=false*/) {
  ...
  if (lht == rht && (lht == INT_TYPE || lht == BOOLEAN_TYPE)) {
    ...
    if (lht == NUMBER_TYPE && rht == INT_TYPE &&
        localGetd(lhsi)->opcode() == LIR_calld) {
      LIns *result = optimizeIntCmpWithNumberCall(lhsi, rhsi, icmp, false);
      if (result)
        return result;
    }
    if (rht == NUMBER_TYPE && lht == INT_TYPE &&
        localGetd(rhsi)->opcode() == LIR_calld) {
      LIns *result = optimizeIntCmpWithNumberCall(rhsi, lhsi, icmp, true);
      if (result)
        return result;
    }
    // Note: no optimizations needed above for "float" or "numeric".
    // Those optimize String.charCodeAt(), which returns a Number by language
    // definition.
...
LIns *CodegenLIR::cmpLt(int lhsi, int rhsi) {
  ...
  // TODO numeric comparisons can be optimized by inlining the fastpath
...
LIns *CodegenLIR::cmpEq(const CallInfo *fid, int lhsi, int rhsi) {
  ...
  LIns *result =
      cmpOptimization(lhsi, rhsi, LIR_eqi, LIR_eqi, LIR_eqd, isStrict);
  ...
  // There are various conditions we can check for that simplify our equality
  // check down to a ptr comparison:
  ...
  //  - both types do not have complex equality checks (non builtin derived
  //  Object types)
  ...
  if (((lht == NULL_TYPE) &&
       (rht && (!rht->hasComplexEqualityRules() || rht == STRING_TYPE))) ||
      ((rht == NULL_TYPE) &&
       (lht && (!lht->hasComplexEqualityRules() || lht == STRING_TYPE))) ||
      ((rht && !rht->hasComplexEqualityRules()) &&
       (lht && !lht->hasComplexEqualityRules()))) {
...
void CodegenLIR::writeEpilogue(const FrameState *state) {
  ...
  if (interrupt_label.unpatchedEdges) {
    ...
    Ins(LIR_restorepc);
#endif
    // LIR_restorepc is actually an unconditional branch, though it is not
    // marked as such for reachability.  Without safepoint polling, the
    // interrupt handler never returns. Therefore, we do not fall through to
  ...
  if (driver->hasReachableExceptions()) {
    ...
    LIns *handler_ordinal = callIns(FUNCTIONID(beginCatch), 6, coreAddr, _ef,
                                    InsConstPtr(info), pc, slotAddr, tagAddr);
  ...
  if (prolog->env_scope)
    livep(prolog->env_scope);
  if (prolog->env_vtable)
    livep(prolog->env_vtable);
  if (prolog->env_abcenv)
    livep(prolog->env_abcenv);
  if (prolog->env_domainenv)
    livep(prolog->env_domainenv);
  if (prolog->env_toplevel)
    livep(prolog->env_toplevel);
  if (prolog->env_finddef_table)
    livep(prolog->env_finddef_table);
  if (restArgc) {
...
LIns *CodegenLIR::initMultiname(const Multiname *multiname, int &csp,
                                bool isDelete /*=false*/) {
  if (!multiname->isRuntime()) {
  ...
  if (nameAtom) {
    if (isDelete) {
      callIns(FUNCTIONID(initMultinameLateForDelete), 3, env_param, _tempname,
              nameAtom);
    } else {
      callIns(FUNCTIONID(initMultinameLate), 3, coreAddr, _tempname, nameAtom);
    }
  }
...
LIns *CodegenLIR::mopAddrToRangeCheckedRealAddrAndDisp(LIns *mopAddr,
                                                       int32_t const size,
                                                       int32_t *disp) {
  AvmAssert(size > 0); // it's signed to help make the int promotion correct

  if (!mopsRangeCheckFilter) {
    // add a MopsRangeCheckFilter to the back end of the lirout pipeline, just
    // after CseFilter. fixme bug Bug 554030: We must put this after CseFilter
    // and ExprFilter so that the range-check expression using LIR_addi/LIR_subi
    // are not modified (by ExprFilter) and no not become referenced by other
    // unrelated code (by CseFilter).
    AvmAssert(lirout == varTracker);
    mopsRangeCheckFilter = new (*alloc1)
        MopsRangeCheckFilter(redirectWriter->out, prolog, loadEnvDomainEnv());
  ...
  LIns *br = NULL;
  LIns *mopsMemoryBase =
      mopsRangeCheckFilter->emitRangeCheck(mopAddr, size, disp, br);
  if (br)
    patchLater(br, mop_rangeCheckFailed_label);

  // if mopAddr is a compiletime constant, we still have to do the range-check
  // above (since globalMemorySize can vary at runtime), but we might be able to
  // encode the entire address into the displacement (if any)...
  if (mopAddr->isImmI() && disp != NULL &&
      sumFitsInInt32(*disp, mopAddr->immI())) {
    *disp += mopAddr->immI();
...
LIns *CodegenLIR::loadEnvScope() {
  ...
  if (!scope) {
    prolog->env_scope = scope =
        prolog->insLoad(LIR_ldp, env_param, offsetof(MethodEnv, _scope),
                        ACCSET_OTHER, LOAD_CONST);
    verbose_only(if (vbNames) {
...
LIns *CodegenLIR::loadEnvVTable() {
  ...
  if (!vtable) {
    LIns *scope = loadEnvScope();
    prolog->env_vtable = vtable =
        prolog->insLoad(LIR_ldp, scope, offsetof(ScopeChain, _vtable),
                        ACCSET_OTHER, LOAD_CONST);
    verbose_only(if (vbNames) {
...
LIns *CodegenLIR::loadEnvAbcEnv() {
  ...
  if (!abcenv) {
    LIns *scope = loadEnvScope();
    prolog->env_abcenv = abcenv =
        prolog->insLoad(LIR_ldp, scope, offsetof(ScopeChain, _abcEnv),
                        ACCSET_OTHER, LOAD_CONST);
    verbose_only(if (vbNames) {
...
LIns *CodegenLIR::loadEnvDomainEnv() {
  ...
  if (!domainenv) {
    LIns *abcenv = loadEnvAbcEnv();
    prolog->env_domainenv = domainenv =
        prolog->insLoad(LIR_ldp, abcenv, offsetof(AbcEnv, m_domainEnv),
                        ACCSET_OTHER, LOAD_CONST);
    verbose_only(if (vbNames) {
...
LIns *CodegenLIR::loadEnvToplevel() {
  ...
  if (!toplevel) {
    LIns *vtable = loadEnvVTable();
    prolog->env_toplevel = toplevel = prolog->insLoad(
        LIR_ldp, vtable, offsetof(VTable, _toplevel), ACCSET_OTHER, LOAD_CONST);
    verbose_only(if (vbNames) {
...
LIns *CodegenLIR::loadEnvFinddefTable() {
  ...
  if (!table) {
    ...
    prolog->env_finddef_table = table =
        prolog->insLoad(LIR_ldp, abc, offsetof(AbcEnv, m_finddef_table),
                        ACCSET_OTHER, LOAD_CONST);
    verbose_only(if (vbNames) {
...
LIns *CodegenLIR::loadVTable(LIns *obj, Traits *t) {
  if (t && !t->isMachineType() && t != STRING_TYPE && t != NAMESPACE_TYPE &&
      t != NULL_TYPE) {
    // must be a pointer to a scriptobject, and we've done the n
    // all other types are ScriptObject, and we've done the null check
    return loadIns(LIR_ldp, offsetof(ScriptObject, vtable), obj, ACCSET_OTHER,
                   LOAD_CONST);
  }
  ...
  int offset;
  if (t == NAMESPACE_TYPE)
    offset = offsetof(Toplevel, _namespaceClass);
  else if (t == STRING_TYPE)
    offset = offsetof(Toplevel, _stringClass);
  else if (t == BOOLEAN_TYPE)
    offset = offsetof(Toplevel, _booleanClass);
  else if (t == NUMBER_TYPE)
    offset = offsetof(Toplevel, _numberClass);
#ifdef VMCFG_FLOAT
  else if (t == FLOAT_TYPE)
    offset = offsetof(Toplevel, _floatClass);
  else if (t == FLOAT4_TYPE)
    offset = offsetof(Toplevel, _float4Class);
#endif
  else if (t == INT_TYPE)
    offset = offsetof(Toplevel, _intClass);
  else if (t == UINT_TYPE)
    offset = offsetof(Toplevel, _uintClass);
  else {
    // *, Object or Void
  ...
  LIns *cvtable = loadIns(LIR_ldp, offsetof(ClassClosure, vtable), cc,
                          ACCSET_OTHER, LOAD_CONST);
  return loadIns(LIR_ldp, offsetof(VTable, ivtable), cvtable, ACCSET_OTHER,
                 LOAD_CONST);
}
...
LIns *CodegenLIR::promoteFloat4Ins(Traits *t, int i) {
  ...
  // That is because if any operand is float4, the other needs to be coerced to
  // float4
  switch (bt(t)) {
  case BUILTIN_float:
  case BUILTIN_number:
...
#ifdef NJ_VERBOSE
bool CodegenLIR::verbose() { return pool->isVerbose(VB_jit, info); }
#endif
...
void CodegenLIR::branchToLabel(LOpcode op, LIns *cond, CodegenLabel &label) {
  ...
  if (br != NULL) {
    if (labelIns != NULL) {
      ...
      label.unpatchedEdges =
          new (*alloc1) Seq<InEdge>(InEdge(br), label.unpatchedEdges);
...
LIns *CodegenLIR::branchJovToLabel(LOpcode op, LIns *a, LIns *b,
                                   CodegenLabel &label) {
  LIns *labelIns = label.labelIns;
  ...
  if (result->isop(op)) {
    if (labelIns != NULL) {
      ...
      label.unpatchedEdges =
          new (*alloc1) Seq<InEdge>(InEdge(result), label.unpatchedEdges);
      varTracker->trackForwardEdge(label);
    }
  } else {
...
CodegenLabel &CodegenLIR::getCodegenLabel(const uint8_t *pc) {
  ...
  if (!blockLabels)
    blockLabels = new (*alloc1) HashMap<const uint8_t *, CodegenLabel *>(
        *alloc1, driver->getBlockCount());
...
void CodegenLIR::patchLater(LIns *jtbl, const uint8_t *pc, uint32_t index) {
  ...
  if (target.labelIns != 0) {
    ...
    target.unpatchedEdges =
        new (*alloc1) Seq<InEdge>(InEdge(jtbl, index), target.unpatchedEdges);
...
void CodegenLIR::patchLater(LIns *br, CodegenLabel &target) {
  if (!br)
    return; // occurs if branch was unconditional and thus never emitted.
  if (target.labelIns != 0) {
    br->setTarget(target.labelIns); // backwards edge
    varTracker->checkBackEdge(target, state);
  } else {
    target.unpatchedEdges =
        new (*alloc1) Seq<InEdge>(InEdge(br), target.unpatchedEdges);
    varTracker->trackForwardEdge(target);
  }
}
...
CodeMgr::CodeMgr(nanojit::Config *config)
    : codeAlloc(config), bindingCaches(NULL), jit_mgr(NULL) {
...
void CodeMgr::flushBindingCaches() {
  ...
  // but 0 is never a legal value there (and this is asserted when the tag is
  // set) so this should safely invalidate those as well (though we don't really
  // need to invalidate them)
  for (BindingCache *b = bindingCaches; b != NULL; b = b->next)
    b->vtable = NULL;
}

void analyze_edge(LIns *label, nanojit::BitSet &livein, LabelBitSet &labels,
                  InsList *looplabels) {
...
REALLY_INLINE void update_var_liveness(nanojit::BitSet *varlivein,
                                       nanojit::BitSet *taglivein, int varIdx,
                                       const bool isVarStore) {
  if (isVarStore) {
    varlivein->clear(varIdx);
    if (taglivein)
      taglivein->clear(varIdx);
  } else {
    varlivein->set(varIdx);
    if (taglivein)
      taglivein->set(varIdx);
  }
}

// Treat the calculated address of addp(vars, const) as the target
// of a load(or store) from/to the variable pointed to, as well as its
// associated tag (if a tag bitset is passed).
bool analyze_addp(LIns *ins, LIns *vars, nanojit::BitSet *varlivein,
                  nanojit::BitSet *taglivein, const size_t varShift,
                  const bool isStore) {
...
void analyze_call(LIns *ins, LIns *catcher, LIns *vars,
                  DEBUGGER_ONLY(bool haveDebugger, int dbg_framesize, )
  ...
  static const FuncLiveInfo varPtrFunctions[] = {
      // beginCatch(core, ef, info, pc, &vars[i], &tags[i]) => store to &vars[i]
      // and &tag[i]
      {FUNCTIONID(beginCatch), 1, true, true, true},
      // makeatom(core, &vars[index], tag[index]) => treat as load from
      // &vars[index]
      {FUNCTIONID(makeatom), 1, false, true, true},
      // restargHelper(Toplevel*, Multiname*, Atom, ArrayObject**, uint32_t,
      // Atom*) => arrayObject** might be reference to a var
      {FUNCTIONID(restargHelper), 2, false, false, false},
#ifdef VMCFG_FLOAT
      // float4(float4_t* result, Atom val) => result might point to a local
      // var.
      {FUNCTIONID(float4), 1, true, false, false},
      // Float4VectorObject::_getNativeUintProperty(this, result, index) =>
      // result might point to a local var.
      {FUNCTIONID(Float4VectorObject_getNativeUintProperty), 1, true, false,
       false},
      // Float4VectorObject::_getNativeIntProperty(this, result, index) =>
      // result might point to a local var.
      {FUNCTIONID(Float4VectorObject_getNativeIntProperty), 1, true, false,
       false},
      // Float4VectorObject::_getNativeDoubleProperty(this, result, index) =>
      // result might point to a local var.
      {FUNCTIONID(Float4VectorObject_getNativeDoubleProperty), 1, true, false,
       false},
      // Float4VectorObject::_setFloat4UintProperty(this, index, value) => value
      // might point to a local var.
      {FUNCTIONID(Float4VectorObject_setNativeUintProperty), 0, false, false,
       false},
      // Float4VectorObject::_setFloat4IntProperty(this, index, value) => value
      // might point to a local var.
      {FUNCTIONID(Float4VectorObject_setNativeIntProperty), 0, false, false,
       false},
      // Float4VectorObject::_setFloat4DoubleProperty(this, index, value) =>
      // value might point to a local var.
      {FUNCTIONID(Float4VectorObject_setNativeDoubleProperty), 0, false, false,
       false},
#endif
  };
  const int varPtrFunctionsNum =
      (int)(sizeof(varPtrFunctions) / sizeof(FuncLiveInfo));
  ...
  for (int i = 0; i < varPtrFunctionsNum; i++) {
    if (ci == varPtrFunctions[i].callinfo) {
      ...
      bool updated = false;
      LIns *varPtrArg =
          ins->arg(argIdx); // varPtrArg == vars, OR addp(vars, index)
      if (varPtrArg == vars) {
        update_var_liveness(&varlivein,
                            varPtrFunctions[i].tagAccess ? &taglivein : NULL, 0,
                            varPtrFunctions[i].isStore);
        updated = true;
      } else if (varPtrArg->isop(LIR_addp)) {
        updated = analyze_addp(varPtrArg, vars, &varlivein,
                               varPtrFunctions[i].tagAccess ? &taglivein : NULL,
                               varShift, varPtrFunctions[i].isStore);
      }
...
bool analyze_label(LIns *i, Allocator &alloc, bool again, int framesize,
                   InsList *looplabels, nanojit::BitSet &varlivein,
                   LabelBitSet &varlabels, nanojit::BitSet &taglivein,
                   LabelBitSet &taglabels) {
...
void CodegenLIR::deadvars_analyze(Allocator &alloc, nanojit::BitSet &varlivein,
                                  LabelBitSet &varlabels,
                                  nanojit::BitSet &taglivein,
                                  LabelBitSet &taglabels) {
  LIns *catcher = this->catch_label.labelIns;
  InsList looplabels(alloc);

  verbose_only(int iter = 0;) bool again;
  do {
    again = false;
    varlivein.reset();
    taglivein.reset();
    ...
    for (LIns *i = in.read(); !i->isop(LIR_start); i = in.read()) {
      ...
      case LIR_callf4:
      case LIR_callv:
        analyze_call(i, catcher, vars,
                     DEBUGGER_ONLY(haveDebugger, dbg_framesize, )
                         VARSHIFT(info),
                     varlivein, varlabels, taglivein, taglabels);
        break;
      default:
  ...
  // now make a final pass, modifying LIR to delete dead stores (make them
  // LIR_neartramps)
  verbose_only(if (pool->isVerbose(LC_Liveness, info)) AvmLog(
                   "killing dead stores after %d LA iterations.\n", iter);)
}
...
void CodegenLIR::deadvars_kill(Allocator &alloc, nanojit::BitSet &varlivein,
                               LabelBitSet &varlabels,
                               nanojit::BitSet &taglivein,
                               LabelBitSet &taglabels) {
#ifdef NJ_VERBOSE
  ...
  for (LIns *i = in.read(); !i->isop(LIR_start); i = in.read()) {
    ...
    case LIR_ldc2i:
      ...
      break;
    case LIR_label:
      analyze_label(i, alloc, true, framesize, 0, varlivein, varlabels,
                    taglivein, taglabels);
      break;
    case LIR_j:
      // the fallthrough path is unreachable, clear it.
    ...
    case LIR_callv:
      analyze_call(i, catcher, vars,
                   DEBUGGER_ONLY(haveDebugger, dbg_framesize, ) VARSHIFT(info),
                   varlivein, varlabels, taglivein, taglabels);
      break;
    default:
      break;
    }
    verbose_only(if (verbose) { AvmLog("  %s\n", printer->formatIns(&b, i)); })
  }
 ...
 *
 * once the live sets are stable, the DeadVars.kill flag is set to cause the
 * filter to not only drop dead stores, but overwrite them as tramps so they'll
 * be ignored by any later passes even without using this filter.
 */
...
void listing(const char *title, AvmLogControl &log, Fragment *frag) {
  ...
  ReverseLister lister(&reader, lister_alloc, frag->lirbuf->printer, &log,
                       title);
...
static const char *filenameFor(const char *path, const char *name,
                               const char *ext, Allocator &alloc) {
  size_t bufferLen =
      1 + VMPI_strlen(name) + VMPI_strlen(path) + VMPI_strlen(ext);
...
static void lircfg(FILE *f, const Fragment *frag, InsSet *ignore,
                   Allocator &alloc, CfgLister::CfgMode mode) {
  LirReader reader(frag->lastIns);
...
// return pointer to generated code on success, NULL on failure (frame size too
// large)
GprMethodProc CodegenLIR::emitMD() {
  deadvars(); // deadvars_kill() will add livep(vars) or livep(tags) if
              // necessary
  ...
  if (pool->isVerbose(VB_lircfg, info)) {
    ...
    CfgLister::CfgMode mode = pool->isVerbose(VB_lircfg_ins, info)
                                  ? CfgLister::CFG_INS
                                  : pool->isVerbose(VB_lircfg_bb, info)
                                        ? CfgLister::CFG_BB
                                        : CfgLister::CFG_EBB;
  ...
  // Use the 'active' log if we are in verbose output mode otherwise sink the
  // output
  LogControl *log = &(mgr->log);
  verbose_only(SinkLogControl sink;
               log = pool->isVerbose(VB_jit, info) ? log : &sink;)

      Assembler *assm =
          new (*lir_alloc) Assembler(mgr->codeAlloc, mgr->allocator, *lir_alloc,
                                     log, core->config.njconfig);
#ifdef VMCFG_VTUNE
  ...
  verbose_only(StringList asmOutput(*lir_alloc);
               if (!pool->isVerbose(VB_raw, info)) assm->_outputCache =
                   &asmOutput;);
  ...
  verbose_only(assm->_outputCache = 0;
               for (Seq<char *> *p = asmOutput.get(); p != NULL;
                    p = p->tail) { assm->outputf("%s", p->head); });
  ...
  if (keep) {
    ...
    verbose_only(if (pool->isVerbose(VB_execpolicy)) AvmLog(
                     "execpolicy revert to interp (%d) compiler error %d \n",
