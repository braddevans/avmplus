...
protected:
  ...
  ArrayObject(VTable *ivtable, ScriptObject *delegate, MethodEnv *env,
              ADT argDesc, uint32_t argc, va_list_wrapper ap,
              bool simple = false);
#endif

public:
  REALLY_INLINE static ArrayObject *create(MMgc::GC *gc, VTable *ivtable,
                                           ScriptObject *delegate,
                                           uint32_t capacity = 0) {
    return new (gc, MMgc::kExact, ivtable->getExtraSize())
        ArrayObject(ivtable, delegate, capacity);
  }

  REALLY_INLINE static ArrayObject *create(MMgc::GC *gc, VTable *ivtable,
                                           ScriptObject *delegate, Atom *argv,
                                           int argc) {
    return new (gc, MMgc::kExact, ivtable->getExtraSize())
        ArrayObject(ivtable, delegate, argv, argc);
  }

private:
  REALLY_INLINE static ArrayObject *createSimple(MMgc::GC *gc, VTable *ivtable,
                                                 ScriptObject *delegate,
                                                 uint32_t capacity = 0) {
    return new (gc, MMgc::kExact, ivtable->getExtraSize())
        ArrayObject(ivtable, delegate, capacity, true);
  }
  ...
  REALLY_INLINE static ArrayObject *createSimple(MMgc::GC *gc, VTable *ivtable,
                                                 ...
                                                 Atom *argv, int argc) {
    return new (gc, MMgc::kExact, ivtable->getExtraSize())
        ArrayObject(ivtable, delegate, argv, argc, true);
  }

#ifdef VMCFG_AOT
  template <typename ADT>
  static ArrayObject *
  createSimple(MMgc::GC *gc, VTable *ivtable, ScriptObject *delegate,
               MethodEnv *env, ADT argDesc, uint32_t argc, va_list_wrapper ap) {
    return new (gc, MMgc::kExact, ivtable->getExtraSize())
        ArrayObject(ivtable, delegate, env, argDesc, argc, ap, true);
  }
#endif
public:
  ~ArrayObject();
  ...
  // Virtual members so Array subclasses can treat length differently (in both
  // C++ and AS3)
...
protected:
  ...
  virtual ArrayObject *try_splice(uint32_t insertPoint, uint32_t insertCount,
                                  uint32_t deleteCount, const ArrayObject *that,
                                  uint32_t that_skip);
