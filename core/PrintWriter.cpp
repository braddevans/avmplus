...
// ensure that you're not performing unnecessary copies.  For example, if you
// have a null terminated string and are using a ConsoleOutputStream its best to
// call write() as opposed to writeN().  The latter copies to a intermediate
// buffer, while the former does not.

// ------------------------------------------------------------

void PrintWriter::write(const char *utf8) { m_stream.write(utf8); }
...
PrintWriter &PrintWriter::operator<<(int32_t value) {
  ...
  default: {
    ...
    char *p = MathUtils::convertIntegerToStringBuffer(
        (intptr_t)value, buffer, len, 10, MathUtils::kTreatAsSigned);
    return *this << p;
  }
  }
}
...
PrintWriter &PrintWriter::operator<<(uint64_t value) {
  ...
  default: {
    ...
    int32_t len = sizeof(buffer);
    char *p = MathUtils::convertIntegerToStringBuffer(
        (intptr_t)value, buffer, len, 10, MathUtils::kTreatAsUnsigned);
    return *this << p;
  }
  }
}
...
#endif
    char *p = MathUtils::convertIntegerToStringBuffer(
        wideVal, buffer, len, 10, MathUtils::kTreatAsUnsigned);
    return *this << p;
  }
  }
}
...
void PrintWriter::writeAtom(Atom atom) {
  if (!AvmCore::isNull(atom)) {
    ...
    case kStringType:
      *this << "\"";
      if (AvmCore::isString(atom))
        *this << AvmCore::atomToString(atom);
      *this << "\"";
      break;
    case kSpecialBibopType:
      ...
      if (bibopKind(atom) == kBibopFloat4Type) {
        ...
        *this << ((float)f4_x(v)) << "," << ((float)f4_y(v)) << ","
              << ((float)f4_z(v)) << "," << ((float)f4_w(v));
        break;
      }
...
void PrintWriter::writeAtomHex(Atom atom) {
  ...
  int32_t len = sizeof(buffer);
  char *p = MathUtils::convertIntegerToStringBuffer(
      (intptr_t)atom, buffer, len, 16, MathUtils::kTreatAsUnsigned);
  *this << p;
}
...
#define PRINT_STAR_OPERATOR_SUPPORT_NULL(x)                                    \
  PRINT_STAR_OPERATOR_SUPPORT(x, "null")
...
void PrintWriter::formatP(const char *format, Stringp arg1, Stringp arg2,
                          Stringp arg3) {
  while (*format) {
    if (*format == '%') {
      switch (*++format) {
