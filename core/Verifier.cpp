...
Verifier::Verifier(MethodInfo *info, MethodSignaturep ms, Toplevel *toplevel,
                   AbcEnv *abc_env
    ...
    : tryFrom(NULL), tryTo(NULL), info(info), ms(ms), worklist(NULL),
      blockStates(NULL), handlerIsReachable(false) {
#ifdef AVMPLUS_VERBOSE
...
public:
  ScopeWriter(CodeWriter *coder, MethodInfo *info, Toplevel *toplevel,
              Verifier *verifier)
      : NullWriter(coder), info(info), toplevel(toplevel), verifier(verifier) {}
  ...
  void writeOp1(const FrameState *state, const uint8_t *pc, AbcOpcode opcode,
    ...
    if (opcode == OP_newfunction) {
      ...
      const ScopeTypeChain *fscope = ScopeTypeChain::create(
          core->GetGC(), ftraits, scope, state, NULL, NULL);
      ...
      //    }
      //
      // could cause the second interior function ("functwo") to include a
      // bogus, unused OP_newfunction call to itself inside the body of functwo.
      // This caused the scope to get reinitialized and generally caused havok.
      // However, we want to allow existing code of this form to continue to
      // work, so check to see if we already have a declaringScope, and if so,
      // require that it match this one.
      ...
      if (curScope != NULL) {
        if (!curScope->equals(fscope)) {
          ...
          // just accept it. see
          // https://bugzilla.mozilla.org/show_bug.cgi?id=544370
...
#ifdef AVMPLUS_VERBOSE
            core->console << "ERROR in ScopeChain::create, mismatched traits "
                          << type << " " << curScope->traits() << "\n";
#endif
            toplevel->throwVerifyError(kCorruptABCError);
          }
      ...
      const ScopeTypeChain *cscope = ScopeTypeChain::create(
          core->GetGC(), ctraits, scope, state, NULL, ctraits);

      if (state->scopeDepth > 0) {
        // innermost scope must be the base class object or else
        // createInstance() will malfunction because it will use the wrong
        // [base] class object to construct the instance.  See
      ...
      const ScopeTypeChain *iscope = ScopeTypeChain::create(
          core->GetGC(), itraits, cscope, NULL, ctraits, itraits);
      ...
      if (!cur_cscope) {
        ...
        if (!cur_cscope->equals(cscope) || !cur_iscope ||
...
RestArgAnalyzer::RestArgAnalyzer()
    : NullWriter(NULL), optimize(true), core(NULL), info(NULL), pool(NULL),
      frameSize(0), restVar(0), isRestArray(NULL) {}

void RestArgAnalyzer::init(AvmCore *_core, MethodInfo *_info,
                           uint32_t _frameSize) {
  core = _core;
  info = _info;
  pool = _info->pool();
  ...
  if (_info->needRest() ||
      (_info->needArguments() && _info->onlyUntypedParameters()))
...
inline void RestArgAnalyzer::fail() { optimize = false; }
...
void RestArgAnalyzer::write(const FrameState *state, const uint8_t *pc,
...
void RestArgAnalyzer::writeOp1(const FrameState *state, const uint8_t *pc,
...
void RestArgAnalyzer::writeOp2(const FrameState *state, const uint8_t *pc,
...
void RestArgAnalyzer::writeMethodCall(const FrameState *state,
                                      const uint8_t *pc, AbcOpcode opcode,
                                      MethodInfo *m, uintptr_t disp_id,
...
bool RestArgAnalyzer::getProperty(const FrameState *state,
                                  const Multiname &multiname, int obj_offset) {
  if (!optimize)
    return false;
  ...
  if (isRestArray[sp - obj_offset + 1]) {
    ...
    if (multiname.isRtname() && multiname.containsAnyPublicNamespace()) {
      ...
      // containsAnyPublicNamespace should imply n==2 - the multiname must have
      // ns or nsset
...
void RestArgAnalyzer::writeOpcodeVerified(const FrameState *state,
                                          const uint8_t *pc, AbcOpcode opcode) {
  if (optimize) {
    switch (opcode) {
    case OP_getlocal: {
...
// This would be less painful if we had a table stating the number of operands
// read by most instructions.

void RestArgAnalyzer::operate(const FrameState *state, const uint8_t *pc,
                              AbcOpcode opcode) {
  uint32_t imm30 = 0, imm30b = 0;
  int32_t imm8 = 0, imm24 = 0;
  uint32_t numprops = 0;
  ...
  switch (opcode) {
  // Ignored because they are generated post-analysis when the analysis
  // succeeds.
  case OP_restarg:
  case OP_restargc:
    break;
...
void Verifier::checkFrameDefinition() {
  // validate rational bounds for frame definition parameters read directly from
  // the abc
  ...
  // also ensure that the computed size doesn't overflow (see
  // MethodInfo._buildMethodSignature)
...
const uint8_t *Verifier::verifyBlock(const uint8_t *start_pos) {
  ...
  for (const uint8_t *pc = start_pos, *nextpc = pc; pc < code_end;
       pc = nextpc) {
    ...
    const AbcOpcodeInfo *opcodeInfoPtr =
        FLOAT_ONLY(!pool->hasFloatSupport() ? opcodeInfoNoFloats :) opcodeInfo;
    if (opcodeInfoPtr[opcode].operandCount == -1)
      verifyFailed(kIllegalOpcodeError, core->toErrorString(info),
                   core->toErrorString(opcode),
                   core->toErrorString((int)(pc - code_pos)));
    ...
    if (pc < tryTo && pc >= tryFrom &&
      ...
      for (int i = 0, n = exTable->exception_count; i < n; i++) {
        ...
        if (pc >= code_pos + handler->from && pc < code_pos + handler->to) {
          ...
          FrameValue stackEntryZero =
              saveStackDepth > 0 ? state->stackValue(0) : state->value(0);
          state->stackDepth = 0;
          state->scopeDepth = 0;

          // PSIRT 3037: For checkTarget() to succeed, we need push something
          // onto the stack, so also check there is room for a push. Add
          // checkStack(0,1) to do the check.
          //
          // The security report of PSIRT 3037 shows it is valid to have a
          // method with a 0-sized stack (max_stack is 0), but it is not valid
          // to have 0-sized stack method that can throw, this will cause heap
          // overflow.
          checkStack(0, 1);
...
// fixme: bugzilla 552988: remove ifdef.
    ...
    case OP_pushint:
      ...
      if (imm30 == 0 || imm30 >= pool->constantIntCount)
        verifyFailed(kCpoolIndexRangeError, core->toErrorString(imm30),
                     core->toErrorString(pool->constantIntCount));
      coder->write(state, pc, opcode, INT_TYPE);
      state->push(INT_TYPE, true);
      break;
    ...
    case OP_pushuint:
      ...
      if (imm30 == 0 || imm30 >= pool->constantUIntCount)
        verifyFailed(kCpoolIndexRangeError, core->toErrorString(imm30),
                     core->toErrorString(pool->constantUIntCount));
    ...
    case OP_pushdouble:
      ...
      if (imm30 == 0 || imm30 >= pool->constantDoubleCount)
        verifyFailed(kCpoolIndexRangeError, core->toErrorString(imm30),
                     core->toErrorString(pool->constantDoubleCount));
      coder->write(state, pc, opcode, NUMBER_TYPE);
      state->push(NUMBER_TYPE, true);
      break;
    ...
    case OP_pushfloat:
      ...
      if (imm30 == 0 || imm30 >= pool->constantFloatCount)
        verifyFailed(kCpoolIndexRangeError, core->toErrorString(imm30),
                     core->toErrorString(pool->constantFloatCount));
    ...
    case OP_pushfloat4:
      ...
      if (imm30 == 0 || imm30 >= pool->constantFloat4Count)
        verifyFailed(kCpoolIndexRangeError, core->toErrorString(imm30),
                     core->toErrorString(pool->constantFloat4Count));
    ...
    case OP_pushnamespace:
      ...
      if (imm30 == 0 || imm30 >= pool->constantNsCount)
        verifyFailed(kCpoolIndexRangeError, core->toErrorString(imm30),
                     core->toErrorString(pool->constantNsCount));
    ...
    case OP_getlex: {
      ...
      if (multiname.isRuntime())
        verifyFailed(kIllegalOpMultinameError, core->toErrorString(opcode),
    ...
    case OP_finddef: {
      ...
      MethodInfo *script =
          core->domainMgr()->findScriptInPoolByMultiname(pool, multiname);
      Traits *resultType;
      if (script != (MethodInfo *)BIND_NONE &&
          script != (MethodInfo *)BIND_AMBIGUOUS) {
    ...
    case OP_initproperty: {
      ...
      if (AvmCore::isSlotBinding(b) &&
        ...
        coder->writeOp2(state, pc, OP_setslot,
                        (uint32_t)AvmCore::bindingToSlotId(b), sp - (n - 1),
                        propTraits);
        state->pop(n);
        break;
      }
      ...
      if (obj.traits == VECTORINT_TYPE || obj.traits == VECTORUINT_TYPE ||
...
#ifdef VMCFG_FLOAT
          || obj.traits == VECTORFLOAT_TYPE || obj.traits == VECTORFLOAT4_TYPE
#endif
      ) {
        ...
        if (maybeIntegerIndex &&
            (indexType == UINT_TYPE || indexType == INT_TYPE ||
             indexType == NUMBER_TYPE
#ifdef VMCFG_FLOAT
             ||
             indexType ==
                 FLOAT_TYPE // float indices are always absorbed by the vector
#endif
             )) {
    ...
    case OP_getproperty: {
      ...
      if (emitPass && emitOptimizedRestArg) {
        ...
        if (multiname.isRtname()) {
          // restarg assumes the property name is an atom, so we must coerce it
          // to an atom on input
          ...
          // restargc produces an uint, so we must coerce to the target type on
          // return
    ...
    case OP_istypelate:
      checkStack(2, 1);
      coder->write(state, pc, opcode);
      // TODO if the only common base type of lhs,rhs is Object, then result is
      // always false
    ...
    case OP_convert_o:
      ...
      // ToObject throws an exception on null and undefined, so after this runs
      // we know the value is safe to dereference.
      emitCheckNull(sp);
      coder->write(state, pc, opcode);
      break;
    ...
    case OP_esc_xattr:
      checkStack(1, 1);
      // this is the ECMA ToString and ToXMLString operators, so the result must
      // not be null (ToXMLString is split into two variants - escaping elements
      // and attributes)
      coder->write(state, pc, opcode);
    ...
    case OP_callstatic: {
      ...
      if (!mms->paramTraits(0)) {
        verifyFailed(kDanglingFunctionError, core->toErrorString(m),
      ...
      coder->writeOp2(state, pc, OP_callstatic, (uint32_t)m->method_id(), argc,
                      resultType);
    ...
    case OP_call: {
      ...
      checkStack(argc + 2, 1);
      // don't need null check, AvmCore::call() uses toFunction() for null
      // check.

      /*
          TODO optimizations
              - if this is a class closure for a non-native type, call == coerce
              - if this is a function closure, try early binding using the
         traits->call sig
    ...
    case OP_construct: {
      ...
      // don't need null check, AvmCore::construct() uses toFunction() for null
      // check.
    ...
    case OP_callmethod: {
      /*
         ...
         it's a last minute change before we shipped FP9 and was necessary when
         we added methods to class Object.

          since then we realized that OP_callmethod need only have failed when
         used outside of the builtin abc, but it's a moot point now.  We dont
      ...
      */
      ...
      if (disp_id >= 0) {
        ...
        if (!obj.traits)
          ...
          verifyFailed(kIllegalEarlyBindingError,
                       core->toErrorString(obj.traits));
      } else {
    ...
    case OP_pushscope: {
      ...
      if (scope->fullsize > (scope->size + state->scopeDepth)) {
        ...
        Traits *requiredType =
            scope->getScopeTraitsAt(scope->size + state->scopeDepth);
        if (!scopeTraits || !scopeTraits->subtypeof(requiredType)) {
          verifyFailed(kIllegalOperandTypeError,
                       core->toErrorString(scopeTraits),
                       core->toErrorString(requiredType));
        }
      }
      ...
      state->pop();
      state->setType(ms->scope_base() + state->scopeDepth, scopeTraits, true,
                     false);
      state->scopeDepth++;
      break;
    }
    ...
    case OP_newcatch: {
      ...
      if (!info->abc_exceptions() ||
          imm30 >= (uint32_t)info->abc_exceptions()->exception_count)
        verifyFailed(kInvalidNewActivationError);
...
#ifdef _DEBUG
        if (pool->isBuiltin)
          core->console << "getouterscope >= depth (" << index
                        << " >= " << state->scopeDepth << ")\n";
#endif
    ...
    case OP_getglobalscope:
      ...
      checkGetGlobalScope(); // after coder->write because mutates stack that
                             // coder depends on
      break;
    ...
    case OP_getglobalslot: {
      ...
      checkEarlySlotBinding(globalTraits); // sets state->value(sp).traits so
                                           // CodeWriter can see type
    ...
    case OP_setglobalslot: {
      ...
      Traits *globalTraits = scope->size > 0 ? scope->getScopeTraitsAt(0)
                                             : state->scopeValue(0).traits;
      checkStack(1, 0);
    ...
    case OP_greaterequals: {
      ...
      if (lhst && rhst) {
        // We assume is that number-to-number comparison yields the same result
        // as float-to-float comparison Also, float4 will be coerced to number,
        // and this is ok.
    ...
    case OP_add: {
      ...
      Traits *lhst = lhs.traits;
      Traits *rhst = rhs.traits;
      if ((lhst == STRING_TYPE && lhs.notNull) ||
          (rhst == STRING_TYPE && rhs.notNull)) {
        ...
        if (pool->hasFloatSupport()) {
          ...
          else if (rhst->isNumberType() || lhst->isNumberType())
            type = NUMBER_TYPE;
          else if (rhst == FLOAT_TYPE && lhst == FLOAT_TYPE)
            type = FLOAT_TYPE;
          else {
            // the result is "NUMERIC" because at least one is NUMERIC and the
            // other is NUMERIC or FLOAT.
            emitCoerceToNumeric(sp);
    ...
    case OP_multiply: {
      ...
      if (pool->hasFloatSupport()) {
        ...
        if (lhst == BUILTIN_float && rhst == BUILTIN_float)
          ...
          type = FLOAT4_TYPE;
        else if (lhst != BUILTIN_none && lhst != BUILTIN_object &&
                 lhst != BUILTIN_any && rhst != BUILTIN_none &&
                 rhst != BUILTIN_object && rhst != BUILTIN_any)
          type = NUMBER_TYPE;
        else {
          emitCoerceToNumeric(sp - 1);
          emitCoerceToNumeric(sp);
    ...
    case OP_hasnext2: {
      ...
      if (v.traits != INT_TYPE)
        verifyFailed(kIllegalOperandTypeError, core->toErrorString(v.traits),
                     core->toErrorString(INT_TYPE));
      coder->write(state, pc, opcode);
      state->setType(imm30, NULL, false);
      state->push(BOOLEAN_TYPE);
      break;
    ...
    case OP_li8:
    case OP_li16:
      if (pc + 1 < code_end && ((opcode == OP_li8 && pc[1] == OP_sxi8) ||
                                (opcode == OP_li16 && pc[1] == OP_sxi16))) {
        checkStack(1, 1);
        emitCoerce(INT_TYPE, sp);
    ...
    case OP_sf64:
      checkStack(2, 0);
      emitCoerce((opcode == OP_sf32 || opcode == OP_sf64) ? NUMBER_TYPE
                                                          : INT_TYPE,
                 sp - 1);
      emitCoerce(INT_TYPE, sp);
      coder->write(state, pc, opcode);
    ...
    default:
      // size was nonzero, but no case handled the opcode.  someone asleep at
      // the wheel!
...
void Verifier::emitCallproperty(AbcOpcode opcode, int &sp, Multiname &multiname,
                                uint32_t multiname_index, uint32_t argc,
                                const uint8_t *pc) {
  uint32_t n = argc + 1;
  checkPropertyMultiname(n, multiname);
  ...
  // If early binding then the state will have been updated, so this will be
  // skipped
  state->pop_push(n, NULL);
...
bool Verifier::emitCallpropertyMethod(AbcOpcode opcode, Traits *t, Binding b,
                                      Multiname &multiname,
                                      uint32_t multiname_index, uint32_t argc,
                                      const uint8_t *pc) {
  ...
  emitCoerceArgs(m, argc);
  if (!t->isInterface()) {
    coder->writeMethodCall(state, pc, OP_callmethod, m, disp_id, argc,
                           resultType);
    if (opcode == OP_callpropvoid)
...
bool Verifier::emitCallpropertySlot(AbcOpcode opcode, int &sp, Traits *t,
                                    Binding b, uint32_t argc,
                                    const uint8_t *pc) {
  if (!AvmCore::isSlotBinding(b))
    return false;
  if ((argc != 1) && (argc != 4))
  ...
  bool argCheck =
      ((argc == 4) FLOAT_ONLY(&&(slotType != core->traits.float4_ctraits)));
  if (argCheck)
    return false;
  ...
  if (slotType == core->traits.int_ctraits) {
  ...
  } else if (slotType == core->traits.float4_ctraits) {
    if (argc == 1) {
    ...
    state->setType(sp, STRING_TYPE, true);
      ...
      // NOTE the following has been refactored so that both lir and wc coerce.
      // previously wc would be skipped and fall back on the method call the the
      // class converter
      if (slotType && slotType->base == CLASS_TYPE &&
          slotType->getCreateClassClosureProc() == NULL) {
...
void Verifier::emitFindProperty(AbcOpcode opcode, Multiname &multiname,
                                uint32_t imm30, const uint8_t *pc) {
      ...
      if (index <= 0) {
        ...
        if (script != (MethodInfo *)BIND_NONE &&
          ...
          if (script->declaringTraits() == NULL)
            verifyFailed(kCorruptABCError);
          if (script == info) {
            // ISSUE what if there is an ambiguity at runtime? is VT too early
            // to bind? its defined here, use getscopeobject 0
            if (scope->size > 0) {
          ...
          {
            coder->writeOp1(state, pc, OP_finddef, imm30,
  ...
  checkPropertyMultiname(n, multiname);
  if (!skip_translation)
    coder->writeOp1(state, pc, opcode, imm30, OBJECT_TYPE);
...
void Verifier::emitGetProperty(Multiname &multiname, int n, uint32_t imm30,
  ...
  if (AvmCore::isSlotBinding(b)) {
    ...
    if (obj.traits->pool == core->builtinPool) {
      ...
      if (FLOAT_ONLY(nm == core->kfloat4 || nm == core->kfloat ||)
                  nm == core->kMath ||
          nm == core->kNumber) {
    ...
    coder->writeOp1(state, pc, OP_getslot, AvmCore::bindingToSlotId(b),
                    propType);
  ...
  if (!propType) {
    if (obj.traits == VECTORINT_TYPE || obj.traits == VECTORUINT_TYPE ||
      ...
      // know the compatibility settings for the code, not for the calling
      // context. If the ABC was compiled for SWFn then it gets SWFn verifier
      // treatment in every context.  Builtin ABC code is compiled with the
      // latest SWF version and always gets the optimization; that that is
      // correct requires some careful argumentation, again see Bugzilla 678952.
...
const uint8_t *Verifier::getTryFrom() const { return tryFrom; }

const uint8_t *Verifier::getTryTo() const { return tryTo; }
...
Traits *Verifier::peekType(Traits *requiredType, int n) {
  Traits *t = state->peek(n).traits;
  if (t != requiredType) {
    verifyFailed(kIllegalOperandTypeError, core->toErrorString(t),
                 core->toErrorString(requiredType));
  }
  return t;
}
...
void Verifier::emitCoerceArgs(MethodInfo *m, int argc) {
  ...
  MethodSignaturep mms = m->getMethodSignature();
  if (!mms->argcOk(argc)) {
    verifyFailed(kWrongArgumentCountError, core->toErrorString(m),
                 core->toErrorString(mms->requiredParamCount()),
  ...
  while (argc > 0) {
    Traits *target =
        (argc <= mms->param_count()) ? mms->paramTraits(argc) : NULL;
...
Traits *Verifier::checkSlot(Traits *traits, int imm30) {
  ...
  if (!traits || slot >= count) {
    verifyFailed(kSlotExceedsCountError, core->toErrorString(slot + 1),
                 core->toErrorString(count), core->toErrorString(traits));
  }
...
MethodInfo *Verifier::checkMethodInfo(uint32_t id) {
  ...
  if (id >= c) {
    verifyFailed(kMethodInfoExceedsCountError, core->toErrorString(id),
                 core->toErrorString(c));
  }
...
Traits *Verifier::checkClassInfo(uint32_t id) {
  ...
  if (id >= c) {
    verifyFailed(kClassInfoExceedsCountError, core->toErrorString(id),
                 core->toErrorString(c));
  }
...
Traits *Verifier::checkTypeName(uint32_t index) {
  ...
  if (name.isParameterizedType()) {
    ...
    Traits *param_traits =
        name.getTypeParameter() ? checkTypeName(name.getTypeParameter()) : NULL;
    t = pool->resolveParameterizedType(toplevel, t, param_traits);
  }
  return t;
}

void Verifier::verifyFailed(int errorID, Stringp arg1, Stringp arg2,
                            Stringp arg3) const {
...
// throws lots of output. Which eventually crashes the compiler on windows.
  ...
  if (!secondTry && !verbose) {
    ...
    TRY(core, kCatchAction_Ignore) { v2.verify(&stubWriter); }
    CATCH(Exception * ignored) { (void)ignored; }
    END_CATCH
    END_TRY
  }
#endif
...
void Verifier::checkTarget(const uint8_t *current, const uint8_t *target,
  ...
  if (!targetState) {
    if (blockStates != NULL && target > current) {
      ...
      //
      // if (target is new) and (branch < nearest < target) and (nearest not
      // queued) then queue(nearest)
      //
      // this is suboptimal in that we may guess wrong and requeue blocks that
      // don't need reverification, but this is theoretically harmless, just
      // extra work.
      //
      int i = blockStates->map.findNear(target);
      // i too large should be impossible, but i < 0 is possible, if the
      // insertion point was before the first block. In that case, just ignore
      // it, since we only need to requeue if it's possible that an existing
      // block has already been verified once; in this case no such block could
      // exist.
      ...
      if (i >= 0 && i < blockStates->map.length()) {
        ...
        if (current < existingState->abc_pc && existingState->abc_pc < target &&
            !existingState->wl_pending) {
#ifdef AVMPLUS_VERBOSE
          if (verbose) {
            core->console << "------------------------------------\n";
            core->console << "RE-QUEUE B"
                          << int(existingState->abc_pc - code_pos) << ":";
          }
#endif
          existingState->wl_pending = true;
          existingState->wl_next = worklist;
          worklist = existingState;
          // no return: we want to fall thru and create the new blockstate as
          // well.
        }
      }
    }
  ...
  bool targetOfExceptionBranch =
      targetState->targetOfExceptionBranch || isExceptionEdge;
...
bool Verifier::mergeState(FrameState *targetState) {
  ...
  if (state->stackDepth != targetState->stackDepth)
    verifyFailed(kStackDepthUnbalancedError,
                 core->toErrorString((int)state->stackDepth),
                 core->toErrorString((int)targetState->stackDepth));

  // check matching scope chain depth
  if (state->scopeDepth != targetState->scopeDepth)
    verifyFailed(kScopeDepthUnbalancedError,
                 core->toErrorString(state->scopeDepth),
                 core->toErrorString(targetState->scopeDepth));
  ...
  for (int i = 0, n = stackTop; i < n; i++) {
    ...
    if (curValue.isWith != targetValue.isWith) {
      ...
      verifyFailed(kCannotMergeTypesError,
                   core->toErrorString(targetValue.traits),
                   core->toErrorString(curValue.traits));
    }
    ...
    if (targetValue.traits != merged_traits ||
    ...
    // which will leave two bits set in merged_sst.  Erase the SST_scriptobject
    // bit. Any other type merged with null already works, as does
    // String|Namespace merged with any other type.
    ...
    if ((merged_traits == STRING_TYPE && merged_sst == str_or_obj) ||
      ...
      AvmAssert(targetValue.traits == NULL_TYPE ||
                curValue.traits == NULL_TYPE);
...
Binding Verifier::findMathFunction(TraitsBindingsp math,
                                   const Multiname &multiname, Binding b,
                                   int argc) {
  Stringp newname = core->internString(
      core->concatStrings(core->kUnderscore, multiname.getName()));
  ...
  if (AvmCore::isMethodBinding(newb)) {
    ...
    if (argc == param_count) {
      for (int i = state->stackDepth - argc, n = state->stackDepth; i < n;
           i++) {
...
Binding Verifier::findStringFunction(TraitsBindingsp str,
                                     const Multiname &multiname, Binding b,
                                     int argc) {
  Stringp newname = core->internString(
      core->concatStrings(core->kUnderscore, multiname.getName()));
  ...
  if (AvmCore::isMethodBinding(newb)) {
    ...
    if ((argc >= (param_count - optional_count)) && (argc <= param_count)) {
      for (int i = state->stackDepth - argc, k = 1, n = state->stackDepth;
           i < n; i++, k++) {
        Traits *t = state->stackValue(i).traits;
...
void Verifier::parseExceptionHandlers() {
  if (info->abc_exceptions()) {
    ...
    // Execption parsing was happening twice and duplicate scope traits were
    // generated. Which led to verify error for follwing sample action script
    // code
    //    function f1:void {
    //        try {
    //            //<code inside try>
    ...
    // and fill tryFrom and tryTo with existing exception handler table
    // information.
    if (!tryFrom || !tryTo) {
  ...
  int exception_count =
      toplevel->readU30(pos); // will be nonnegative and less than 0xC0000000

  if (exception_count != 0) {
    if (exception_count == 0 ||
        (size_t)(exception_count - 1) > SIZE_T_MAX / sizeof(ExceptionHandler))
      verifyFailed(kIllegalExceptionHandlerError);

    ExceptionHandlerTable *table =
        ExceptionHandlerTable::create(core->GetGC(), exception_count);
    ExceptionHandler *handler = table->exceptions;
    for (int i = 0; i < exception_count; i++, handler++) {
      ...
      if (name_index != 0) {
        ...
        if (!qn.isBinding()) {
          // abc docs specify that the name is a string but asc generates
          // QNames. Multinames with no namespaces could be supported, but
          // Tamarin currently can't handle them in this context.
          verifyFailed(
              kCorruptABCError); // the error code could be more precise
        }
      }

#ifdef AVMPLUS_VERBOSE
      if (verbose) {
        core->console << "            exception[" << i
                      << "] from=" << handler->from << " to=" << handler->to
                      << " target=" << handler->target << " type=" << t
                      << " name=";
        if (name_index != 0)
          core->console << qn;
        else
      ...
      if (handler->from < 0 || handler->to < handler->from ||
          handler->target < handler->to || handler->target >= code_length) {
      ...
      Traits *scopeTraits =
          name_index == 0
              ? OBJECT_TYPE
              : Traits::newCatchTraits(toplevel, pool, scopePosInPool,
                                       qn.getName(), qn.getNamespace());
...
void Verifier::printState(StringBuffer &prefix, FrameState *state) {
  ...
  for (int i = stackStart, n = ms->stack_base() + state->stackDepth; i < n;
       i++) {
    printValue(state->value(i));
    if (i + 1 < n)
      out << ' ';
  ...
  if (state->targetOfBackwardsBranch)
    out << " B";
  if (state->targetOfExceptionBranch)
    out << " E";
...
void Verifier::printValue(FrameValue &v) {
  ...
  PrintWriter &out = core->console;
  out << t;
  if (!t || (!t->isNumeric() && t != BOOLEAN_TYPE && t != NULL_TYPE &&
             t != VOID_TYPE))
    out << (v.notNull ? "~" : "");
  ...
  if (v.sst_mask) {
    out << '[';
    if (v.sst_mask & (1 << SST_atom))
      out << 'A';
    if (v.sst_mask & (1 << SST_string))
      out << 'S';
    if (v.sst_mask & (1 << SST_namespace))
      out << 'N';
    if (v.sst_mask & (1 << SST_scriptobject))
      out << 'O';
    if (v.sst_mask & (1 << SST_int32))
      out << 'I';
    if (v.sst_mask & (1 << SST_uint32))
      out << 'U';
    if (v.sst_mask & (1 << SST_bool32))
      out << 'B';
    if (v.sst_mask & (1 << SST_double))
      out << 'D';
#ifdef VMCFG_FLOAT
    if (v.sst_mask & (1 << SST_float))
      out << 'F';
    if (v.sst_mask & (1 << SST_float4))
      out << "F4";
#endif
    out << ']';
  }
