 ...
 * MEMORY MANAGEMENT:
 *
 * SortedMap can be instantiated on the stack or embedded as a field in another
 * class. However, it cannot be allocated dynamically (operator new is
 ...
 * one, use HeapSortedMap to wrap an instance.
 *
 ...
 */
template <class KLIST, class TLIST> class SortedMap {
private:
...
public:
  ...
  bool isEmpty() const { return keys.length() == 0; }
  int length() const { return keys.length(); }
  void clear() {
    keys.clear();
    values.clear();
  }
...
private:
  SortedMap<KLIST, TLIST> &
  operator=(const SortedMap<KLIST, TLIST> &other);          // unimplemented
