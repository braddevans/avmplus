...
REALLY_INLINE bool Quad<VALUE_TYPE>::matchNS(Namespacep ns) const {
  ...
  return u1 == u2 &&
         (u1 & 7) ==
             Namespace::NS_Public && // implies both are public, since u1 == u2
         this->apiVersion() <= ns->getApiVersion();
}

template <class VALUE_TYPE, class VALUE_WRITER>
REALLY_INLINE VALUE_TYPE MultinameHashtable<VALUE_TYPE, VALUE_WRITER>::get(
    Stringp name, NamespaceSetp nsset) const {
  return getNSSet(name, nsset)->value;
}

template <class VALUE_TYPE, class VALUE_WRITER>
REALLY_INLINE VALUE_TYPE MultinameHashtable<VALUE_TYPE, VALUE_WRITER>::getMulti(
...
MultinameHashtable<VALUE_TYPE, VALUE_WRITER>::keyAt(int index) const {
  AvmAssert(m_quads->quads[index - 1].name != NULL);
  return m_quads->quads[index - 1].name;
}

template <class VALUE_TYPE, class VALUE_WRITER>
REALLY_INLINE Namespacep
MultinameHashtable<VALUE_TYPE, VALUE_WRITER>::nsAt(int index) const {
  return m_quads->quads[index - 1].ns;
}

template <class VALUE_TYPE, class VALUE_WRITER>
REALLY_INLINE VALUE_TYPE
MultinameHashtable<VALUE_TYPE, VALUE_WRITER>::valueAt(int index) const {
  return m_quads->quads[index - 1].value;
}

template <class VALUE_TYPE, class VALUE_WRITER>
REALLY_INLINE size_t
MultinameHashtable<VALUE_TYPE, VALUE_WRITER>::allocatedSize() const {
  return numQuads * sizeof(Quad<VALUE_TYPE>);
}

template <class MNHT>
REALLY_INLINE StMNHTIterator<MNHT>::StMNHTIterator(MNHT *mnht)
    : m_mnht(mnht), m_cur(mnht->m_quads->quads - 1),
      m_end(mnht->m_quads->quads + mnht->numQuads) {}

template <class MNHT> REALLY_INLINE bool StMNHTIterator<MNHT>::next() {
  return ++m_cur < m_end;
}

template <class MNHT> REALLY_INLINE Stringp StMNHTIterator<MNHT>::key() const {
...
/*static*/
REALLY_INLINE void BindingType::store(MMgc::GC *gc, void *, void **addr,
                                      void *value) {
...
/*static*/
REALLY_INLINE void GCObjectType::store(MMgc::GC *gc, void *container,
