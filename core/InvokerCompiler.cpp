...
bool InvokerCompiler::canCompileInvoker(MethodInfo *method) {
  ...
  if (rest_offset > int32_t((param_count + 1) * sizeof(Atom)) &&
    ...
    // of space since we don't know (at compile time) how many extra varags are
    // present.
    //
    ...
    //   plus the extra ones.  (the unboxed copies are still available in the
    //   callee).
    //
...
InvokerCompiler::InvokerCompiler(MethodInfo *method)
    : LirHelper(method->pool()), method(method),
      ms(method->getMethodSignature()), maxargs_br(NULL), minargs_br(NULL) {
  this->method = method;
  ...
  verbose_only(if (verbose()) {
    ...
    lirbuf->printer->addrNameMap->addAddrRange(pool->core, sizeof(AvmCore), 0,
                                               "core");
  }) debug_only(lirout = validate2 = new (*alloc1)
                    ValidateWriter(lirout, lirbuf->printer, "InvokerCompiler");)
      verbose_only(if (verbose()) {
        CodeMgr *codeMgr = method->pool()->codeMgr;
        core->console << "compileInvoker " << method << "\n";
        core->console << " required=" << ms->requiredParamCount()
                      << " optional="
                      << (ms->param_count() - ms->requiredParamCount()) << "\n";
        lirout = new (*alloc1)
            VerboseWriter(*alloc1, lirout, lirbuf->printer, &codeMgr->log);
      })
...
void InvokerCompiler::generate_lir() {
  ...
  if (minargs_br || maxargs_br) {
    LIns *errlabel = label();
    if (minargs_br)
      minargs_br->setTarget(errlabel);
    if (maxargs_br)
      maxargs_br->setTarget(errlabel);
    callIns(FUNCTIONID(argcError), 2, env_param, argc_param);
  }

  // mark the endpoint of generated LIR with an instruction the Assembler allows
  // at the end
...
void InvokerCompiler::downcast_arg(int i, int offset, LIns *env_param,
                                   LIns *args_param) {
...
void InvokerCompiler::downcast_args(LIns *env_param, LIns *argc_param,
                                    LIns *args_param) {
  // the receiver arg (arg0) only needs to be unboxed, not coerced
  verbose_only(if (verbose()) core->console << "unbox arg 0 "
                                            << ms->paramTraits(0) << "\n";)
  ...
  for (i = 1; i <= required_count; i++) {
    verbose_only(if (verbose()) core->console << "arg " << i << " "
                                              << ms->paramTraits(i) << "\n";)
  ...
  if (required_count < param_count) {
    ...
    for (; i <= param_count; i++) {
      verbose_only(if (verbose()) core->console << "optional arg " << i << " "
                                                << ms->paramTraits(i) << "\n";)
          // if (argc < i) { goto done }
...
void *InvokerCompiler::assemble() {
  ...
  cfg.harden_blind_constants = false;
  ...
  verbose_only(SinkLogControl sink;
      ...
      Assembler *assm = new (*lir_alloc) Assembler(
          codeMgr->codeAlloc, codeMgr->allocator, *lir_alloc, log, cfg);
  verbose_only(StringList asmOutput(*lir_alloc);)
      verbose_only(if (!pool->isVerbose(VB_raw, method)) assm->_outputCache =
                       &asmOutput;) LirReader bufreader(frag->lastIns);
...
void InvokerCompiler::call_method(LIns *env_param, LIns *argc_param) {
  // We know we've called the method at least once, so method->implGPR pointer
  // is correct.
...
void InvokerCompiler::downcast_and_store(LIns *atom, Traits *t, LIns *env,
                                         int offset) {
  LIns *native;
  BuiltinType b = bt(t);
  switch (b) {
  ...
  case BUILTIN_uint: {
    ...
    LIns *slow_br = lirout->insBranch(
        LIR_jf, eqp(tag, InsConstAtom(AtomConstants::kIntptrType)), NULL);
    ...
    native = BUILTIN_int == b ? i2p(callIns(FUNCTIONID(integer), 1, atom))
                              : ui2p(callIns(FUNCTIONID(toUInt32), 1, atom));
    stp(native, args_out, offset, ACCSET_OTHER);
  ...
  case BUILTIN_number: {
    ...
    // kIntptrType
    LIns *not_intptr =
        lirout->insBranch(LIR_jf, eqp(tag, AtomConstants::kIntptrType), NULL);
    ...
    std(p2dIns(rshp(atom, AtomConstants::kAtomTypeSize)), args_out, offset,
        ACCSET_OTHER);
    LIns *fast_path_exit1 = lirout->insBranch(LIR_j, NULL, NULL);
    not_intptr->setTarget(label());
    // kDoubleType
    LIns *not_double =
        lirout->insBranch(LIR_jf, eqp(tag, AtomConstants::kDoubleType), NULL);
    std(ldd(subp(atom, AtomConstants::kDoubleType), 0, ACCSET_OTHER), args_out,
        offset, ACCSET_OTHER);
...
#ifdef VMCFG_FLOAT
  case BUILTIN_float: // TODO: I assume we don't need to inline here since we
                      // don't care about untyped performance; check assumption
  ...
  case BUILTIN_boolean: {
    LIns *tag = andp(atom, AtomConstants::kAtomTypeMask);
    LIns *not_bool =
        lirout->insBranch(LIR_jf, eqp(tag, AtomConstants::kBooleanType), NULL);
    stp(rshp(atom, AtomConstants::kAtomTypeSize), args_out, offset,
        ACCSET_OTHER);
    LIns *fast_path_exit = lirout->insBranch(LIR_j, NULL, NULL);
    not_bool->setTarget(label());
    stp(ui2p(callIns(FUNCTIONID(boolean), 1, atom)), args_out, offset,
        ACCSET_OTHER);
  ...
  case BUILTIN_namespace:
    native = andp(callIns(FUNCTIONID(coerce), 3, env, atom, InsConstPtr(t)),
                  ~AtomConstants::kAtomTypeMask);
    break;
  case BUILTIN_void:
  case BUILTIN_null:
  case BUILTIN_any:
