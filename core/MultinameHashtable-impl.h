...
bool MultinameHashtable<VALUE_TYPE, VALUE_WRITER>::gcTrace(MMgc::GC *gc,
                                                           size_t cursor) {
  (void)cursor;
...
void MultinameHashtable<VALUE_TYPE, VALUE_WRITER>::grow() {
  ...
  MMGC_MEM_TYPE(this);
  QuadContainer<VALUE_TYPE> *newAtoms =
      QuadContainer<VALUE_TYPE>::create(gc, capacity);
...
template <class VALUE_TYPE, class VALUE_WRITER>
VALUE_TYPE MultinameHashtable<VALUE_TYPE, VALUE_WRITER>::getMulti(
    const Multiname *mname) const {
...
// return the NS that unambigously matches in "match" (or null for
// none/ambiguous)
template <class VALUE_TYPE, class VALUE_WRITER>
VALUE_TYPE MultinameHashtable<VALUE_TYPE, VALUE_WRITER>::getMulti(
    const Multiname &mname, Namespacep &match) const {
  // multiname must not be an attr name, have wildcards, or have runtime parts.
  AvmAssert(mname.isBinding() && !mname.isAnyName());
...
MultinameHashtable<VALUE_TYPE, VALUE_WRITER>::MultinameHashtable(int capacity)
    : m_quads(NULL), size(0), numQuads(0) {
  ...
  // Code near the end of MultinameHashtable::put assumes that API will fit into
  // 31 bits
...
void MultinameHashtable<VALUE_TYPE, VALUE_WRITER>::Init(int capacity) {
  if (capacity) {
    ...
    MMGC_MEM_TYPE(this);
    QuadContainer<VALUE_TYPE> *newAtoms =
        QuadContainer<VALUE_TYPE>::create(gc, numQuads);
    WB(gc, this, &m_quads, newAtoms);
  }
}
...
template <class VALUE_TYPE, class VALUE_WRITER>
int MultinameHashtable<VALUE_TYPE, VALUE_WRITER>::find(
    Stringp name, Namespacep ns, const Quad<VALUE_TYPE> *t, unsigned m) {
  AvmAssert(ns->getURI()->isInterned());
  AvmAssert(name != NULL && ns != NULL);
  AvmAssert(name->isInterned());

  // this is a quadratic probe but we only hit every third slot since those hold
  // keys.
  int n = 7;
...
REALLY_INLINE QuadContainer<VALUE_TYPE> *
QuadContainer<VALUE_TYPE>::create(MMgc::GC *gc, uint32_t capacity) {
  return new (gc, MMgc::kExact, sizeof(Quad<VALUE_TYPE>) * (capacity - 1))
      QuadContainer(capacity);
}
...
template <class VALUE_TYPE, class VALUE_WRITER>
const Quad<VALUE_TYPE> *MultinameHashtable<VALUE_TYPE, VALUE_WRITER>::getNSSet(
    Stringp mnameName, NamespaceSetp nsset) const {
  static const Quad<VALUE_TYPE> kBindNone = {NULL, NULL, NULL, 0};
  static const Quad<VALUE_TYPE> kBindAmbiguous = {NULL, NULL, (VALUE_TYPE)-1,
                                                  0};
...
template <class VALUE_TYPE, class VALUE_WRITER>
VALUE_TYPE
MultinameHashtable<VALUE_TYPE, VALUE_WRITER>::get(Stringp name,
                                                  Namespacep ns) const {
  AvmAssert(ns->getURI()->isInterned());
  const Quad<VALUE_TYPE> *t = m_quads->quads;
...
template <class VALUE_TYPE, class VALUE_WRITER>
VALUE_TYPE MultinameHashtable<VALUE_TYPE, VALUE_WRITER>::getName(
    Stringp name, Namespacep *nsFoundOut) const {
  const Quad<VALUE_TYPE> *t = m_quads->quads;
...
template <class VALUE_TYPE, class VALUE_WRITER>
void MultinameHashtable<VALUE_TYPE, VALUE_WRITER>::rehash(
    const Quad<VALUE_TYPE> *oldAtoms, int oldTriplet,
    Quad<VALUE_TYPE> *newAtoms, int newTriplet) {
...
template <class VALUE_TYPE, class VALUE_WRITER>
int FASTCALL
MultinameHashtable<VALUE_TYPE, VALUE_WRITER>::next(int index) const {
...
template <class VALUE_TYPE, class VALUE_WRITER>
void MultinameHashtable<VALUE_TYPE, VALUE_WRITER>::add(Stringp name,
                                                       Namespacep ns,
                                                       VALUE_TYPE value) {
  if (isFull()) {
    grow();
...
template <class VALUE_TYPE, class VALUE_WRITER>
void MultinameHashtable<VALUE_TYPE, VALUE_WRITER>::put(Stringp name,
                                                       Namespacep ns,
                                                       VALUE_TYPE value) {
  AvmAssert(!isFull());
  AvmAssert(name->isInterned());
  ...
  Quad<VALUE_TYPE> *const quadbase = m_quads->quads;
  {
    ...
    for (;;) {
      ...
      if (probeName == name) {
        ...
        // it's not the one we're looking for, thus we are now multiNS on this
        // name.
  ...
  size++;
  // OPTIMIZEME: we know the entries are zero, so we could use
  // WriteBarrierRC_ctor here... except that it will call GetGC() on the
  // address, which will be wrong for > 4k. Need a version of
  // WriteBarrierRC_ctor that takes explicit GC*.
  WBRC(gc, m_quads, &cur->name, name);
  WBRC(gc, m_quads, &cur->ns, ns);
  // Set the "ApiVersion" section to an impossibly large value, so that we
  // always set it to the proper value the first time through.
...
write_value:
  ...
  if (v < (cur->apiAndMultiNS & ~1)) {
    ...
    // this should always be the case as we build them that way, but let's
    // double-check.
    cur->apiAndMultiNS = v | (cur->apiAndMultiNS & 1);
    AvmAssert((kApiVersionSeriesMembership[ns->getApiVersion()] &
               kApiVersionSeriesMembership[cur->apiVersion()]) != 0);
  }
}
} // namespace avmplus
