...
REALLY_INLINE PoolObject *PoolObject::create(AvmCore *core, ScriptBuffer &sb,
                                             const uint8_t *startpos,
                                             ApiVersion apiVersion) {
  return new (core->GetGC(), MMgc::kExact)
...
REALLY_INLINE ApiVersion PoolObject::getApiVersion() { return _apiVersion; }

REALLY_INLINE const uint8_t *PoolObject::getMetadataInfoPos(uint32_t index) {
  return metadata_infos[index];
}

REALLY_INLINE Traits *PoolObject::resolveTypeName(const uint8_t *&pc,
