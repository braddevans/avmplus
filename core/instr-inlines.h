...
bool isObjectPtr(Atom a) {
  // copy of AvmCore::isObject, but without the null check since we know we have
  // nonnull
  AssertNotNull(a);
...
bool isStringPtr(Atom a) {
  // copy of AvmCore::isString, but without the null check since we know we have
  // nonnull
  AssertNotNull(a);
...
REALLY_INLINE
Atom call_method_binding(Atom base, VTable *vtable, Binding b, int argc,
                         Atom *atomv) {
  // force receiver == base.  if caller used OP_callproplex then receiver was
  // null. todo: split out case for callproplex.
  atomv[0] = base;
...
template <class E>
REALLY_INLINE Atom call_slot_binding(E env, Atom base, Binding b, int argc,
                                     Atom *atomv) {
  ScriptObject *method = AvmCore::atomToScriptObject(base)->getSlotObject(
...
template <class E>
REALLY_INLINE Atom call_prim_dynamic(E env, Atom val, const Multiname *name,
                                     int argc, Atom *args) {
#ifdef VMCFG_FLOAT
 ...
 */
template <class E>
REALLY_INLINE Atom op_call(E env, Atom func, int argc, Atom *atomv) {
 ...
 */
template <class E>
REALLY_INLINE Atom op_call(E env, ScriptObject *func, int argc, Atom *atomv) {
  if (func)
 ...
 */
template <class E>
REALLY_INLINE Atom op_construct(E env, Atom ctor, int argc, Atom *atomv) {
...
template <class E>
REALLY_INLINE Binding getBinding(E env, VTable *vtable, const Multiname *ref) {
  return getBinding(env, vtable->traits, ref);
}

template <class E>
REALLY_INLINE ArrayObject *newarray(E caller_env, int argc, Atom *ap) {
...
template <class E>
REALLY_INLINE Atom constructprop(E env, const Multiname *multiname, int argc,
                                 Atom *atomv) {
  Toplevel *toplevel = env->toplevel();
  return constructprop(toplevel, multiname, argc, atomv,
                       toVTable(toplevel, atomv[0]));
}

template <class E>
REALLY_INLINE Atom coerce(E env, Atom atom, Traits *expected) {
  // do a couple of quick checks to see if we can bail early, since it's often
  // the case that the type is already what we expect (and we can determine that
  // quickly by checks against the Traits BuiltinType)
  if (!expected)
    return atom;
