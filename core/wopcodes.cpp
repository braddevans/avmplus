...
const WordOpcodeAttr wopAttrs[] = {
    ...
    // width=0 means the instruction is not used in the word-code interpreter;
    // other
    //    attrs should be 0.  Otherwise it's the width in words of the
    //    instruction including the opcode.  Current limit is 7.  Only
    //    lookupswitch is variable length
    // jumps=1 means a branch relative to the end of the instruction, offset is
    // 2nd word always term=1  means the instruction terminates control flow
    // (return, throw, lookupswitch) thro=1  means the instruction may throw an
    // exception calls=1 means the topmost value on the stack is an argument
    // count push=n means the instruction pushes n values (after popping).
    // Current limit is 7 pop=n means the instruction pops n values.  Current
    // limit is 7 pop_ex=1 means the instruction may pop more values than shown
    // in the pop column,
    //    there is custom logic elsewhere to handle it.  Usually it means there
    //    is a multiname operand
    // use_l=1 means the instruction reads a local slot
    // def_l=1 means the instruction writes a local slot

    // You may assume that the instructions that may invoke user code comprise a
    // subset of the ones that are marked as thro=1.  A few of the latter will
    // not call user code, but at this point we have no formal definition of
    // whether e.g. embedders may or may not subclass MethodEnv in such a way
    // that MethodEnv::findproperty() may call user code.  So there is no
    // separate column for whether such a call may take place.
    ...
    {3, 1, 1, 0, 0, 0, 1, 0, 0,
     0 N("lookupswitch")}, // NOTE, variable length beyond 3rd word
