...
void BigInteger::setFromDouble(double value) {
  int e;
  uint64_t mantissa = MathUtils::frexp(
      value, &e); // value = mantissa*2^e, mantissa and e are integers.
...
void BigInteger::setFromBigInteger(const BigInteger *from, int32_t offset,
                                   int32_t amount) {
  numWords = amount;
  AvmAssert(numWords <= kMaxBigIntegerBufferSize);
  VMPI_memcpy((uint8_t *)wordBuffer, (uint8_t *)&(from->wordBuffer[offset]),
...
double BigInteger::doubleValueOf() const {
  ...
  // start result with top word.  We will accumulate the most significant 53
  // bits of data into it.
  ...
  if (pos > 0) {
    ...
    if (bits == 32) {
      // last extract was exactly 32 bits, so rest of bits are in next word if
      // there is one
      if (nextWord > -1) {
        w = wordBuffer[nextWord--];
      ...
      // we know bit54 is in this word but the rest of the data may be in the
      // next
  ...
  double result = 0;
  int32_t expBase2 =
      lg2() + 1 - 53; // if negative, then we've already captured all the data
                      // in the mantissaResult and we can ignore.
...
// Compare this vs other, return -1 if this < other, 0 if this == other, or 1 if
// this > other.
//  (todo: optimization for D2A would be to check if comparison was larger than
//  an argument value currently, we add the argument to this and compare the
//  result with other).
int32_t BigInteger::compare(const BigInteger *other) const {
  // if a is bigger than other, subtract has to be positive (assuming unsigned
  // value)
  int result = 0;
  if (numWords > other->numWords)
    ...
    // otherwise, they are they same number of uint32_t words.  need to check
    // numWords by numWords
...
// Multiply by an integer factor and add an integer increment.  The addition is
// essentially free,
...
// Multiply by another BigInteger.  If optional arg result is not null, reuse it
// for
//  result, else allocate a new result.
//  (note, despite the name "smallerNum", argument does not need to be smaller
//  than this).
BigInteger *BigInteger::mult(const BigInteger *smallerNum,
  ...
  // wipe entire buffer (initToZero flag in setNumWords only sets new elements
  // to zero)
  for (int x = 0; x < maxNewNumWords; x++)
    result->wordBuffer[x] = 0;

  // do the math like gradeschool.  To optimize, use an FFT
  ...
  for (int x = 0; x < smallerNum->numWords; x++) {
    ...
    {
      uint32_t *pResult =
          result->wordBuffer + x; // each pass we rewrite elements of result
                                  // offset by the pass iteration
      uint64_t product;
      uint64_t carry = 0;
...
// Divide this by divisor, put the remainder (i.e. this % divisor) into
// residual.  If optional third argument result
//  is not null, use it to store the result of the div, else allocate a new
//  BigInteger for the result.
//   Note: this has been hard to get right.  If bugs do show up, use
//   divideByReciprocalMethod instead Note2: this is optimized for the types of
//   numerator/denominators generated by D2A.  It will not work when
//    the result would be a value bigger than 9.  For general purpose BigInteger
//    division, use divideByReciprocalMethod.
BigInteger *BigInteger::quickDivMod(const BigInteger *divisor,
                                    BigInteger *residual, BigInteger *result) {
  ...
  /*  this section only necessary for true division instead of special case
  division needed by D2A.  We are assuming the result is a single digit value <
  10 and > -1 int next   = this->numWords - dWords; residual->copyFrom(this,
  next, dWords); // residual holds a divisor->numWords sized chunk of this.
  */
  ...
  // do // need to loop over dword chunks of residual to make this handle
  // division of any arbitrary bigIntegers
  {
    ...
    if (((factor <= 0) || (factor > 10)) // over estimate of 9 could be 10
      ...
      uint64_t bigR =
          (((uint64_t)residual->wordBuffer[residual->numWords - 1]) << 32) +
          (residual->wordBuffer[residual->numWords - 2]);
      factor = bigR / divisor->wordBuffer[dWords - 1];
      if (factor >
          9) { //  Note:  This only works because of the relative size of the
               //  two operands
               //   which the D2A class produces.  todo: try generating a
               //   numerator out of the the top 32 significant bits of residual
               //   (may have to get bits from two seperate words) and a
               //   denominator out of the top 24 significant bits of divisor
               //   and use them for the factor guess.  Same principal as above
               //   applied to 8 bit units.
        factor = 9;
        /*
    ...
    if (factor) {
      ...
      // reduce dividend (aka residual) by factor*divisor, leave remainder in
      // residual
    ...
    /* The above works for the division requirements of D2A, where divisor is
    always around 10 larger than the dividend and the result is always a digit
    1-9. To make this routine work for general division, the following would
    need to be fleshed out /debugged. residual->trimLeadingZeros();

        // While we have more words to divide by and the residual has less words
    than the
        //  divisor,
        if (--next >= 0)
        {
            do
            {
                ...
                residual->wordBuffer[0] = this->wordBuffer[next]; // drop next
    word of "this" into bottom word of remainder
            }
...
/*  Was hoping dividing via a Newton's approximation of the reciprocal would be
   faster, but its not (makes D2A about twice as slow!).  Its not the 32bit
   divide above that's slow, its how many times you have to iterate over entire
   BigIntegers.  Below uses a shift, and three multiplies:
*/
BigInteger *BigInteger::divideByReciprocalMethod(const BigInteger *divisor,
                                                 BigInteger *residual,
                                                 BigInteger *result) {
  // handle easy cases where divisor is >= this
  int compareTo = this->compare(divisor);
  ...
  while (1) {
    ...
    int32_t usq2 = 4 + ag;
    ush2 -=
        usq2; // BigInteger* diff = ush->subtract(usq); // ush -= usq;  ush > 0
  ...
  this->subtract(&temp,
                 residual); //  todo: should be a more optimal way of doing this

  // ... doesn't work, e is the wrong scale for this (too large)....
  // residual = this->lshift(e,residual); // residual = (this*2^e - result) /
  // 2^e residual->decrementBy(result); residual->rshiftBy(e); // remove scaling
  return (result);
}

//  q = this->divBy(divisor)  is equilvalent to:  q =
//  this->quickDivMod(divisor,remainderResult); this = remainderResult;
BigInteger *BigInteger::divBy(const BigInteger *divisor,
  ...
  quickDivMod(
      divisor, &tempInt,
      divResult); // this has been hard to get right.  If bugs do show up,
  // divResult = divideByReciprocalMethod(divisor,tempInt,divResult); //  try
  // this version instead  Its slower, but less likely to be buggy
...
BigInteger *BigInteger::lshift(uint32_t shiftBy, BigInteger *result) const {
  ...
  int totalWords = numNewWords + numWords +
                   1; // overallocate by 1, trim if necessary at end.
  ...
  for (int x = 0; x < numNewWords; x++)
    *pResultBuff++ = 0;
  // move bits from wordBuffer into result's wordBuffer shifted by (shiftBy %
  // 32)
  shiftBy &= 0x1f;
  if (shiftBy) {
    uint32_t carry = 0;
    ...
    *pResultBuff = carry; // final carry may add a word
    ...
    for (int x = 0; x < numWords;
         x++) // shift was by a clean multiple of 32, just move words
    {
...
//  with argument specifying which direction to shift, but result might be
//  messy/harder to get right)
...
//  Add or subtract two BigIntegers.  If subtract, we assume the argument is
//  smaller than this since we
//   don't support negative numbers.  If optional third argument result is not
//   null, reuse it for result else allocate a new BigInteger for the result.
BigInteger *BigInteger::addOrSubtract(const BigInteger *smallerNum, bool isAdd,
                                      BigInteger *result) const {
  ...
  if (comparedTo < 0) {
    ...
    AvmAssert(isAdd ||
              comparedTo >=
                  0); // the d2a/a2d code should never subtract a larger from a
                      // smaller.
                      //  all of the BigInteger code assumes positive numbers.
                      //  Proper handling would be to create a negative result
                      //  here (and check for negative results everywhere).
  }
  ...
  // do the math: loop over common words of both numbers, performing - or + and
  // carrying the
  ...
  for (index = 0; index < smallerNum->numWords; index++) {
    if (isAdd)
      x = ((uint64_t)biggerNum->wordBuffer[index]) +
          smallerNum->wordBuffer[index] + borrow;
    else
      x = ((uint64_t)biggerNum->wordBuffer[index]) -
          smallerNum->wordBuffer[index] -
          borrow; // note x is unsigned.  Ok even if result would be negative
                  // however
    borrow = x >> 32 & (uint32_t)1;
    result->wordBuffer[index] = (uint32_t)(x & 0xffffffff);
  }

  // loop over remaining words of the larger number, carying borrow/overflow
  // forward
  ...
  AvmAssert(index >= 0); // If this fires then there's something wrong with the
                         // zero check above
