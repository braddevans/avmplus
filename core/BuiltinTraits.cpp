...
namespace avmplus {
BuiltinTraits::BuiltinTraits() { VMPI_memset(this, 0, sizeof(BuiltinTraits)); }
...
#define DO_BUILTIN(nm, clsnm)                                                  \
  do {                                                                         \
    nm##_itraits = core->domainMgr()->findBuiltinTraitsByName(                 \
        pool, core->internConstantStringLatin1(clsnm));                        \
...
#undef DO_BUILTIN
  ...
  vectordouble_itraits->set_names(vectordouble_itraits->ns(),
                                  core->kVectorNumber);
#ifdef VMCFG_FLOAT
  vectorfloat_itraits->set_names(vectorfloat_itraits->ns(), core->kVectorFloat);
  vectorfloat4_itraits->set_names(vectorfloat4_itraits->ns(),
                                  core->kVectorFloat4);
#endif
