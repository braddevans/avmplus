...
REALLY_INLINE Atom invoke_cached_method(CallCache &c, Atom obj, int argc,
                                        Atom *args) {
...
template <class T> REALLY_INLINE T load_cached_slot(BindingCache &c, Atom obj) {
...
REALLY_INLINE Atom boxslot(AvmCore *core, int32_t i) {
  return core->intToAtom(i);
}
REALLY_INLINE Atom boxslot(AvmCore *core, uint32_t u) {
  return core->uintToAtom(u);
}
REALLY_INLINE Atom boxslot(AvmCore *, Bool32 b) {
  return ((b != 0) << 3) | kBooleanType;
}
REALLY_INLINE Atom boxslot(AvmCore *core, double d) {
...
SSE2_ONLY(
    ...
    Atom getprop_obj_slot_double_sse2(GetCache &c, MethodEnv *env, Atom obj) {
      PROF_IF("getprop_obj_slot hit", OBJ_HIT(obj, c)) {
        return atomObj(obj)->core()->doubleToAtom_sse2(
...
Atom getprop_miss(GetCache &c, MethodEnv *env, Atom obj) {
  ...
  if (isObjectPtr(obj)) {
    ...
    if (AvmCore::isSlotBinding(b)) {
      ...
      SSE2_ONLY(if (toplevel->core()->config.njconfig.i386_sse2 &&
                    sst == SST_double) {
      ...
      if (AvmCore::isMethodBinding(b) && AvmCore::isXMLorXMLList(obj) &&
...
void set_slot_from_atom(Atom atom, Traits *traits, AnyVal *slotPtr,
                        uint8_t *tagPtr) {
...
int32_t beginCatch(AvmCore *core, ExceptionFrame *ef, MethodInfo *info,
                   intptr_t pc, AnyVal *slotPtr, uint8_t *tagPtr) {
  int32_t ordinal;
  ef->beginCatch();
