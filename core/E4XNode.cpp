...
E4XNodeAux::E4XNodeAux(Stringp s, Namespace *ns, FunctionObject *notify)
    : m_name(s), m_ns(ns), m_notification(notify) {}
...
AttributeE4XNode::AttributeE4XNode(E4XNode *parent, String *value)
    : E4XNode(parent), m_value(value) {
#if defined(VMCFG_TELEMETRY_SAMPLER) && defined(DEBUGGER)
  IMemorySampler *memSampler = (IMemorySampler *)gc()->GetAttachedSampler();
  if (memSampler) {
    memSampler->recordNewObjectAllocation(this, ".mem.xml.attribute",
                                          (size_t)bytesUsedShallow());
  }
#endif
}

CDATAE4XNode::CDATAE4XNode(E4XNode *parent, String *value)
    : E4XNode(parent), m_value(value) {
#if defined(VMCFG_TELEMETRY_SAMPLER) && defined(DEBUGGER)
  IMemorySampler *memSampler = (IMemorySampler *)gc()->GetAttachedSampler();
  if (memSampler) {
    memSampler->recordNewObjectAllocation(this, ".mem.xml.cdata",
                                          (size_t)bytesUsedShallow());
  }
#endif
}

CommentE4XNode::CommentE4XNode(E4XNode *parent, String *value)
    : E4XNode(parent), m_value(value) {
#if defined(VMCFG_TELEMETRY_SAMPLER) && defined(DEBUGGER)
  IMemorySampler *memSampler = (IMemorySampler *)gc()->GetAttachedSampler();
  if (memSampler) {
    memSampler->recordNewObjectAllocation(this, ".mem.xml.comment",
                                          (size_t)bytesUsedShallow());
  }
#endif
}

PIE4XNode::PIE4XNode(E4XNode *parent, String *value)
    : E4XNode(parent), m_value(value) {
#if defined(VMCFG_TELEMETRY_SAMPLER) && defined(DEBUGGER)
  IMemorySampler *memSampler = (IMemorySampler *)gc()->GetAttachedSampler();
  if (memSampler) {
    memSampler->recordNewObjectAllocation(
        this, ".mem.xml.processinginstruction", (size_t)bytesUsedShallow());
  }
#endif
}

TextE4XNode::TextE4XNode(E4XNode *parent, String *value)
    : E4XNode(parent), m_value(value) {
#if defined(VMCFG_TELEMETRY_SAMPLER) && defined(DEBUGGER)
  IMemorySampler *memSampler = (IMemorySampler *)gc()->GetAttachedSampler();
  if (memSampler) {
    memSampler->recordNewObjectAllocation(this, ".mem.xml.text",
                                          (size_t)bytesUsedShallow());
  }
#endif
}
...
ElementE4XNode::ElementE4XNode(E4XNode *parent) : E4XNode(parent) {
  ...
  IMemorySampler *memSampler = (IMemorySampler *)gc()->GetAttachedSampler();
  if (memSampler) {
    memSampler->recordNewObjectAllocation(this, ".mem.xml.element",
                                          (size_t)bytesUsedShallow());
  }
#endif
}
...
bool E4XNode::getQNameForeign(AvmCore *core, Multiname *mn,
                              Namespacep publicNS) const {
  if (!m_nameOrAux)
    return false;
...
void E4XNode::setQName(AvmCore *core, Stringp name, Namespace *ns) {
  ...
  if (!ns || ns->isPublic() ||
      (ns->getPrefix() == core->kEmptyString->atom() &&
       ns->getURI()->isEmpty())) {
...
void ElementE4XNode::_addInScopeNamespace(AvmCore *core, Namespace *ns,
                                          Namespacep publicNS) {
  //      if (getClass() & (kText | kCDATA | kComment | kProcessingInstruction |
  //      kAttribute))
  //          return;
  ...
  if (!m.isAnyNamespace() &&
      (m.getNamespace()->getPrefix() == ns->getPrefix())) {
    setQName(core, m.getName(),
             core->newNamespace(m.getNamespace()->getURI(),
                                Namespace::NS_Public,
  ...
  for (unsigned int i = 0, na = numAttributes(); i < na; i++) {
    ...
    if (!ma.isAnyNamespace() &&
        ma.getNamespace()->getPrefix() == ns->getPrefix()) {
      curAttr->setQName(core, ma.getName(),
                        core->newNamespace(ma.getNamespace()->getURI(),
                                           Namespace::NS_Public,
                                           kApiVersion_VM_ALLVERSIONS));
    }
  }
...
Namespace *E4XNode::FindNamespace(AvmCore *core, Toplevel *toplevel,
                                  Stringp &tagName, bool bAttribute) {
  ...
  // An attribute without a prefix is unqualified and does not inherit a
  // namespace from its parent.
  ...
  if (prefix == core->kxml) {
    Namespacep nsXML = core->newNamespace(core->kEmptyString->atom(),
                                          core->kXML1998NS->atom());
    return nsXML;
  }
...
void E4XNode::BuildInScopeNamespaceList(AvmCore * /*core*/,
                                        NamespaceList &inScopeNS) const {
...
void E4XNode::addAttribute(E4XNode * /*x*/) { AvmAssert(0); }
...
void ElementE4XNode::CopyAttributesAndNamespaces(AvmCore *core,
                                                 Toplevel *toplevel,
                                                 XMLTag &tag,
                                                 Namespacep publicNS) {
  m_attributes = 0;
  ...
  uint32_t numAttr = 0;
  int32_t len;
  // We first handle namespaces because the a attribute tag can reference a
  // namespace defined farther on in the same node... <ns2:table2
  // ns2:bar=\"last\" xmlns:ns2=\"http://www.macromedia.com/home\">...
  uint32_t index = 0;
    ...
    if (len >= 5) {
      ...
      if (attributeName->matchesLatin1_caseless("xmlns", 5, 0)) {
        ...
        if ((len > 5) && attributeName->charAt(5) == ':') {
          ...
          // xmlns=uri
          ns = core->newNamespace(core->kEmptyString->atom(),
                                  attributeValue->atom());
        }
  ...
  while (tag.nextAttribute(index, attributeName, attributeValue)) {
    ...
    E4XNode *attrObj =
        AttributeE4XNode::create(core->GetGC(), this, attributeValue);
    ...
    // check for a duplicate attribute here and throw a kXMLDuplicateAttribute
    // if found
    ...
    for (unsigned int i = 0, na = numAttributes(); i < na; i++) {
      ...
      if (m.matches(&m2)) {
        toplevel->typeErrorClass()->throwError(
            kXMLDuplicateAttribute, attributeName, tag.text,
            core->toErrorString(tag.text->length()));
      }
    }
...
// E4X 9.1.1.7, page 16
E4XNode *E4XNode::_deepCopy(AvmCore *core, Toplevel *toplevel,
                            Namespacep publicNS) const {
  ...
  if (x->getClass() == kElement) {
    ...
    if (numChildren()) {
      ...
      WB(gc(), y, &y->m_children,
         uintptr_t(HeapE4XNodeList::create(core->GetGC(), numChildren())));
      for (uint32_t k = 0; k < _length(); k++) {
        E4XNode *child = _getAt(k);
        if (((child->getClass() == E4XNode::kComment) &&
             toplevel->xmlClass()->get_ignoreComments()) ||
            ((child->getClass() == E4XNode::kProcessingInstruction) &&
             toplevel->xmlClass()->get_ignoreProcessingInstructions())) {
          continue;
        }
...
E4XNode *E4XNode::_deepCopyForeign(AvmCore *core, Toplevel *toplevel,
                                   Namespacep publicNS) const {
  core->stackCheck(toplevel);
  ...
  if (x->getClass() == kElement) {
    ...
    if (numNamespaces()) {
      y->m_namespaces =
          HeapNamespaceList::create(core->GetGC(), numNamespaces());
      uint32_t i;
      for (i = 0; i < numNamespaces(); i++) {
        y->m_namespaces->list.add(
            core->cloneNamespace(getNamespaces()->list.get(i)));
      }
    }
...
// Not enabled after discussion with JeffD.  If the namespaces are important,
// they're used in the node names themselves.
#if 0
  ...
  if (this->getValue() != v->getValue() &&
      (this->getValue() == NULL || v->getValue() == NULL ||
       *getValue() != *v->getValue()))
    return false;
...
void E4XNode::_insert(AvmCore * /*core*/, Toplevel * /*toplevel*/,
                      uint32_t /*entry*/, Atom /*value*/) {
  return;
}

void ElementE4XNode::_insert(AvmCore *core, Toplevel *toplevel, uint32_t entry,
                             Atom value) {
  //      //step 1
  //      if (m_class & (kText | kCDATA | kComment | kProcessingInstruction |
  //      kAttribute))
  //          return;
  ...
  if (!m_children) {
    WB(gc(), this, &m_children,
       uintptr_t(HeapE4XNodeList::create(core->GetGC(), n)));
  }
...
E4XNode *E4XNode::_replace(AvmCore * /*core*/, Toplevel * /*toplevel*/,
                           uint32_t /*i*/, Atom /*V*/, Atom /*pastValue*/) {
  return 0;
}

E4XNode *ElementE4XNode::_replace(AvmCore *core, Toplevel *toplevel, uint32_t i,
                                  Atom V, Atom pastValue) {
  // step 1
  // if (getClass() & (kText | kCDATA | kComment | kProcessingInstruction |
  // kAttribute))
  //  return;
  ...
  if (i >= _length()) {
    ...
    if (!m_children)
      WB(gc(), this, &m_children,
         uintptr_t(HeapE4XNodeList::create(core->GetGC(), 1)));
    convertToE4XNodeList();
    HeapE4XNodeList *aa = ((HeapE4XNodeList *)(uintptr_t)m_children);
    aa->list.add(NULL);
  }
  ...
  if (xml && (xml->getClass() & (kElement | kComment | kProcessingInstruction |
                                 kText | kCDATA))) {
    // a.    If V.[[Class]] is "element" and (V is x or an ancestor of x) throw
    // an Error exception
    ...
    if (XMLObject::notifyNeeded(newXML)) {
      ...
      if (!detail)
        detail = pastValue;
      XMLObject *target =
          XMLObject::create(core->GetGC(), toplevel->xmlClass(), newXML);
      target->nonChildChanges(core->ktextSet, newXML->getValue()->atom(),
                              detail);
    }
  }
...
int E4XNode::childIndex() const {
  ...
  AvmAssert(
      m_parent->_length()); // this child's parent does not contain itself???
...
void E4XNode::dispose() {
  ...
  IMemorySampler *memSampler = (IMemorySampler *)gc()->GetAttachedSampler();
  if (memSampler) {
    memSampler->recordDeallocation(this, GC::Size(this));
  }
#endif
}

void ElementE4XNode::dispose() {
  for (uint32_t i = 0, na = numAttributes(); i < na; ++i)
    getAttribute(i)->dispose();
...
uint64_t E4XNode::bytesUsedShallow() const {
  uint64_t size = GC::Size(this);
  HeapE4XNodeList *attr = getAttributes();
  if (attr)
    size += GC::Size(attr);
  HeapNamespaceList *nslist = getNamespaces();
  if (nslist)
    size += GC::Size(nslist);
