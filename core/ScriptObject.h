...
public:
  REALLY_INLINE static ScriptObject *create(MMgc::GC *gc, VTable *vtable,
                                            ScriptObject *delegate);
  REALLY_INLINE static ScriptObject *
  create(MMgc::GC *gc, VTable *vtable, ScriptObject *delegate, int htCapacity);
  ...
  // common set/set/has/delete/etc virtual methods renamed to explicitly name
  // the expected arg types, to avoid potentially hidden virtual functions
  ...
  Atom getStringPropertyFromProtoChain(Stringp name, ScriptObject *protochain,
   ...
   *       unless you make sure to reinitialize the contents of the argument
   ...
   *       construct="override" or construct="instance" is specified in the AS3
   * file.
   */
  ...
  // This is a fast way to implement "istype(MethodClosure)"; it returns
  // non-null if this is an MethodClosure (or subclass thereof)
  ...
  // this really shouldn't exist here, but AIR currently plays some games with
  // subclassing "Function" that use C++ classes that don't descend from
  // FunctionObject. Easier to fix by rooting this method here than by fixing
  // AIR at this time.
  ...
  // This gets the property named "length" (if any) on the object, coerces it to
  // a uint, and returns the value. (Since uint(undefined) == 0, undefined
  // length is zero.)
...
protected:
  Atom getAtomPropertyFromProtoChain(Atom name, ScriptObject *protochain,
                                     Traits *origObjTraits) const;
...
private:
  GCMember<ScriptObject> delegate; // __proto__ in AS2, archetype in semantics
  // ------------------------ DATA SECTION END
};
} // namespace avmplus
