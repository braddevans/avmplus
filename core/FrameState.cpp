...
FrameState::FrameState(MethodSignaturep ms, MethodInfo *mi)
    : info(mi), wl_next(NULL), abc_pc(NULL), scopeDepth(0), stackDepth(0),
      withBase(-1), frameSize(ms->frame_size()), scopeBase(ms->scope_base()),
      stackBase(ms->stack_base()), targetOfBackwardsBranch(false),
      targetOfExceptionBranch(false), wl_pending(false) {
  locals =
      (FrameValue *)mmfx_alloc_opt(sizeof(FrameValue) * frameSize, MMgc::kZero);
}

FrameState::~FrameState() { mmfx_free(locals); }
