...
BaseExecMgr::BaseExecMgr(AvmCore *core)
    : core(core), config(core->config)
      ...
      ,
      verifyFunctionQueue(core->gc, 0), verifyTraitsQueue(core->gc, 0)
#endif
#ifdef VMCFG_NANOJIT
      ,
      current_osr(NULL), jit_observer(NULL),
      noise(MMgc::GCHeap::secret) // TODO: Generate this from a random number
                                  // provider held by AvmCore
#endif
{
...
//    MMGC_STATIC_ASSERT(offsetof(MethodInfo, _implGPR) == 0);
  ...
  if (!m->isNative())
    m->setOSR((config.runmode == RM_mixed && config.osr_enabled)
                  ? config.osr_threshold
                  : 0);
...
uintptr_t BaseExecMgr::delegateInvoke(MethodEnv *env, int32_t argc,
                                      uint32_t *ap) {
...
// Called when MethodInfo is constructed.
void BaseExecMgr::init(MethodEnv *env) { env->_implGPR = delegateInvoke; }

#ifdef VMCFG_FLOAT
typedef enum _eRetTypeKinds {
  kIntegerRetType =
      0, // return type for ints & co; also used for pointers, typically
...
Atom BaseExecMgr::initInvokeInterpNoCoerce(MethodEnv *env, int argc,
                                           Atom *args) {
  initObj(env, (ScriptObject *)atomPtr(args[0]));
...
void BaseExecMgr::setInterp(MethodInfo *m, MethodSignaturep ms, bool isOsr) {
  ...
  if (isJitEnabled()) {
    ...
    // * if OSR is enabled, choose a stub that counts invocations to trigger
    // JIT.
    // * if the method is a constructor, choose a stub that initializes
    //   the object's fields before executing (init_).
    ...
    int rtype =
        IFFLOAT(ReturnType(ms), ms->returnTraitsBT() == BUILTIN_number ? 1 : 0);
...
uintptr_t BaseExecMgr::verifyEnterGPR(MethodEnv *env, int32_t argc,
                                      uint32_t *ap) {
  verifyOnCall(env);
...
#ifdef VMCFG_FLOAT
float4_t BaseExecMgr::verifyEnterVECR(MethodEnv *env, int32_t argc,
                                      uint32_t *ap) {
  verifyOnCall(env);
  STACKADJUST(); // align stack for 32-bit Windows and MSVC compiler
  float4_t f4 =
      thunkEnterVECR_adapter((void *)env->method->_implVECR, env, argc, ap);
  STACKRESTORE();
  return f4;
}
#endif // VMCFG_FLOAT
...
void BaseExecMgr::verifyOnCall(MethodEnv *env) {
  ...
  // We got here by calling env->_implGPR, which was pointing to
  // verifyEnterGPR/FPR, but next time we want to call the real code, not
  // verifyEnter again. All other MethodEnv's in their default state will call
  // the target method directly and never go through verifyEnter().  Update the
  // copy in MethodEnv.
  env->_implGPR = env->method->_implGPR;
}
...
void BaseExecMgr::verifyMethod(MethodInfo *m, Toplevel *toplevel,
                               AbcEnv *abc_env) {
 ...
 */
void BaseExecMgr::verifyInterp(MethodInfo *m, MethodSignaturep ms,
  ...
  if (m->pool()->isVerbose(VB_execpolicy))
    core->console << "execpolicy interp (" << m->unique_method_id() << ") " << m
                  << " jit-available\n";
#endif
...
template <typename T, typename CALLT>
T debugEnterExitWrapper(MethodEnv *env, GprMethodProc thunker, int32_t argc,
                        uint32_t *argv) {
...
/*static*/
uintptr_t BaseExecMgr::debugEnterExitWrapper32(MethodEnv *env, int32_t argc,
                                               uint32_t *argv) {
#if defined(VMCFG_TELEMETRY_SAMPLER) && !defined(VMCFG_AOT)
  return debugEnterExitWrapper<uintptr_t, GprMethodProc>(
      env,
      env->core()->samplerEnabled ? env->method->_native.samplerThunker
                                  : env->method->_native.thunker,
      argc, argv);
#else
  return debugEnterExitWrapper<uintptr_t, GprMethodProc>(
      env, env->method->_native.thunker, argc, argv);
#endif
}

/*static*/
double BaseExecMgr::debugEnterExitWrapperN(MethodEnv *env, int32_t argc,
                                           uint32_t *argv) {
#if defined(VMCFG_TELEMETRY_SAMPLER) && !defined(VMCFG_AOT)
  return debugEnterExitWrapper<double, FprMethodProc>(
      env,
      env->core()->samplerEnabled ? env->method->_native.samplerThunker
                                  : env->method->_native.thunker,
      argc, argv);
...
#ifdef VMCFG_FLOAT
/*static*/
float4_t BaseExecMgr::debugEnterExitWrapperV(MethodEnv *env, int32_t argc,
                                             uint32_t *argv) {
  CallStackNode csn(CallStackNode::kEmpty);
  env->debugEnter(/*frame_sst*/ NULL, &csn, /*framep*/ NULL, /*eip*/ NULL);
#if defined(VMCFG_TELEMETRY_SAMPLER) && !defined(VMCFG_AOT)
  const float4_t result = thunkEnterVECR_adapter(
      (void *)env->core()->samplerEnabled ? env->method->_native.samplerThunker
                                          : env->method->_native.thunker,
      env, argc, argv);
#else
  const float4_t result = thunkEnterVECR_adapter(
      (void *)env->method->_native.thunker, env, argc, argv);
#endif
  env->debugExit(&csn);
  return result;
}
...
// FIXME: Bug 529832 - We could specialize several common cases without JIT
// compiling.
...
bool BaseExecMgr::isJitEnabled() const { return false; }
...
// Note that some of these have (partial) guts of avmplus::coerce replicated
// here, for efficiency. If you find bugs here, you might need to update
// avmplus::coerce as well (and vice versa).
Atom *FASTCALL BaseExecMgr::coerceUnbox1(MethodEnv *env, Atom atom, Traits *t,
                                         Atom *args) {
  ...
  case BUILTIN_string:
    atom = AvmCore::isNullOrUndefined(atom) ? NULL
                                            : (Atom)env->core()->string(atom);
    break;
...
inline Atom coerceAtom(AvmCore *core, Atom atom, Traits *t,
                       Toplevel *toplevel) {
  switch (Traits::getBuiltinType(t)) {
  case BUILTIN_number:
  ...
  case BUILTIN_uint:
    return (atomKind(atom) == kIntptrType && atom >= 0) ? atom
                                                        : core->uintAtom(atom);
  case BUILTIN_boolean:
...
Atom BaseExecMgr::endCoerce(MethodEnv *env, int32_t argc, uint32_t *ap,
  ...
  case BUILTIN_float: {
    STACKADJUST(); // align stack for 32-bit Windows and MSVC compiler
    // WARNING: This assumes that the calling conventions of
    // SinglePrecisionFprMethodProc and FprMethodProc are identical, or at least
    // compatible. I.e. the register used to return the "double" value is
    // identical with (or a superset of) the register used to return a float
    // value. This assumption is true for ARM (softfloat, hardfloat ABIs) x86
    // and x64 (Mac, Windows, Linux) at least. Must double-check for other
    // platforms!! (most notably, TODO: MIPS)
    typedef float (*SinglePrecisionFprMethodProc)(MethodEnv *, int32_t,
                                                  uint32_t *);
    float f = reinterpret_cast<SinglePrecisionFprMethodProc>(
        *env->method->_implFPR)(env, argc, ap);
    STACKRESTORE();
...
inline void checkArgc(MethodEnv *env, int32_t argc, MethodSignaturep ms) {
  ...
  // Should no longer be possible to have mismatched scopes; we should reject
  // any such functions in makeIntoPrototypeFunction().
...
REALLY_INLINE size_t calcAtomAllocSize(int32_t argc, int32_t extra = 0) {
  // We need to check for integer overflow here; it's possible for someone to
  // use Function.apply() to pass an argument Array with a huge value for length
  // (but sparsely populated). But since "extra" is int32 and sizeof(Atom) is
  // known here, we can do an efficient check without a full 64-bit multiply, as
  // CheckForCallocSizeOverflow() does. (Note: yes, I know that a good compiler
  // should optimize unsigned-divide-by-power-of-2 into a shift, but this is a
  // hot path, so let's be skeptical and force the issue.)
#ifdef VMCFG_64BIT
  ...
  if (uint32_t(argc) >
      ((MMgc::GCHeap::kMaxObjectSize - size_t(extra)) >> kAtomSizeLog2))
    MMgc::GCHeap::SignalObjectTooLarge();
  return (argc << kAtomSizeLog2) + extra;
}

// static
inline size_t BaseExecMgr::startCoerce(MethodEnv *env, int32_t argc,
                                       MethodSignaturep ms) {
...
Atom BaseExecMgr::apply(MethodEnv *env, Atom thisArg, ArrayObject *a) {
  ...
  // An integer overflow while calling Function.apply can lead to enter an
  // ActionScript function without correctly validating the supplied arguments.
  ...
  AvmAssertMsgCanThrow(thisArg ==
                           coerce(env, thisArg, env->get_ms()->paramTraits(0)),
                       "", env->core());
...
// note that GCC typically restricts tailcalls to functions with similar
// signatures
// ("sibcalls") -- see http://www.ddj.com/architect/184401756 for a useful
// explanation. anyway, since we really want interpBoxed to be a tailcall from
// here, be sure to keep it using a compatible signature...
Atom BaseExecMgr::invokeInterp(MethodEnv *env, int32_t argc, Atom *atomv) {
  // The tail call to interpBoxed is important in order to keep stack
  // consumption down in an interpreter-only configuration, but it's good
  // always.
  ...
  // Caller will coerce instance if necessary, so make sure it was done.
  AvmAssertMsgCanThrow(atomv[0] == coerce(env, atomv[0], ms->paramTraits(0)),
                       "", env->core());
...
Atom BaseExecMgr::invokeInterpNoCoerce(MethodEnv *env, int32_t argc,
                                       Atom *atomv) {
  // The tail call to interpBoxed is important in order to keep stack
  // consumption down in an interpreter-only configuration, but it's good
  // always.
...
void BaseExecMgr::unboxCoerceArgs(MethodEnv *env, int32_t argc, Atom *in,
...
void BaseExecMgr::unboxCoerceArgs(MethodEnv *env, Atom thisArg, ArrayObject *a,
                                  uint32_t *argv, MethodSignaturep ms) {
  int32_t argc = a->getLength();
  ...
  for (int32_t i = 0; i < end; i++)
    args =
        coerceUnbox1(env, a->getUintProperty(i), ms->paramTraits(i + 1), args);
  while (end < argc)
    *args++ = a->getUintProperty(end++);
}

// Specialized for Function.call().
void BaseExecMgr::unboxCoerceArgs(MethodEnv *env, Atom thisArg, int32_t argc,
                                  Atom *in, uint32_t *argv,
                                  MethodSignaturep ms) {
  Atom *args = unbox1(thisArg, ms->paramTraits(0), (Atom *)argv);
