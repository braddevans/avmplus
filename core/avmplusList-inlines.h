...
REALLY_INLINE /*static*/ void
DataListHelper<T, align>::wbData(const void * /*container*/, LISTHEADER &header,
                                 LISTDATA *data) {
  header.data = data;
}

template <class T, uintptr_t align>
REALLY_INLINE /*static*/ typename DataListHelper<T, align>::TYPE
DataListHelper<T, align>::load(LISTDATA *data, uint32_t index) {
  typedef typename DataListHelper<T, align>::TYPE VAL;
  AvmAssert(data != NULL);
  if (align == 0)
    return data->entries[index];
  else
    return ((VAL *)((uintptr_t(data->entries) + (align - 1)) &
                    ~(align - 1)))[index];
}

template <class T, uintptr_t align>
REALLY_INLINE /*static*/ void
DataListHelper<T, align>::store(LISTDATA *data, uint32_t index, TYPE value) {
  typedef typename DataListHelper<T, align>::TYPE VAL;
  AvmAssert(data != NULL);
  if (align == 0)
    data->entries[index] = value;
  else
    ((VAL *)((uintptr_t(data->entries) + (align - 1)) & ~(align - 1)))[index] =
        value;
}

template <class T, uintptr_t align>
REALLY_INLINE /*static*/ void
DataListHelper<T, align>::storeInEmpty(LISTDATA *data, uint32_t index,
                                       TYPE value) {
  typedef typename DataListHelper<T, align>::TYPE VAL;
  // This is commented out because it's not necessary for safety of a DataList,
  // and it can generate an assert in List::insert(), which uses storeInEmpty()
  // for efficiency; since DataListHelper::moveRange doesn't zero out the
  // cleared space we could get this assertion firing. Better to neuter this
  // assertion than add a unnecessary zeroing-out to moveRange() just to pacify
  // it. AvmAssert(data->entries[index] == 0);
  AvmAssert(data != NULL);
  if (align == 0)
    data->entries[index] = value;
  else
...
template <class T, uintptr_t align>
REALLY_INLINE /*static*/ void
DataListHelper<T, align>::clearRange(LISTDATA *data, uint32_t start,
                                     uint32_t count) {
  typedef typename DataListHelper<T, align>::TYPE VAL;
  AvmAssert(count > 0);
  VAL *base;
...
template <class T, uintptr_t align>
REALLY_INLINE /*static*/ void
DataListHelper<T, align>::moveRange(LISTDATA *data, uint32_t srcStart,
                                    uint32_t dstStart, uint32_t count) {
  typedef typename DataListHelper<T, align>::TYPE VAL;
  VAL *base;
  if (align == 0)
    base = data->entries;
...
REALLY_INLINE /*static*/ void GCListHelper::wbData(const void *container,
                                                   LISTHEADER &header,
                                                   LISTDATA *data) {
...
REALLY_INLINE /*static*/ GCListHelper::TYPE GCListHelper::load(LISTDATA *data,
                                                               uint32_t index) {
  AvmAssert(data != NULL);
  return data->entries[index];
}

REALLY_INLINE /*static*/ void GCListHelper::store(LISTDATA *data,
                                                  uint32_t index, TYPE value) {
  AvmAssert(data != NULL);
  WB(MMgc::GC::GetGC(data), data, &data->entries[index], value);
}

REALLY_INLINE /*static*/ void
GCListHelper::storeInEmpty(LISTDATA *data, uint32_t index, TYPE value) {
  AvmAssert(data != NULL);
...
REALLY_INLINE /*static*/ void
GCListHelper::clearRange(LISTDATA *data, uint32_t start, uint32_t count) {
  AvmAssert(count > 0);
  VMPI_memset(&data->entries[start], 0, count * sizeof(MMgc::GCObject *));
}

REALLY_INLINE /*static*/ void GCListHelper::moveRange(LISTDATA *data,
                                                      uint32_t srcStart,
                                                      uint32_t dstStart,
                                                      uint32_t count) {
  AvmAssert(data != NULL);
  MMgc::GC::GetGC(data)->movePointersWithinBlock(
      (void **)data,
      uint32_t((char *)(&data->entries[dstStart]) - (char *)data),
      uint32_t((char *)(&data->entries[srcStart]) - (char *)data), count,
      /*zeroEmptied*/ true);
}

// ----------------------------

REALLY_INLINE /*static*/ void RCListHelper::wbData(const void *container,
                                                   LISTHEADER &header,
                                                   LISTDATA *data) {
  MMgc::GC *const gc = MMgc::GC::GetGC(data);
  if (gc->IsPointerToGCPage(container)) {
...
REALLY_INLINE /*static*/ RCListHelper::TYPE RCListHelper::load(LISTDATA *data,
                                                               uint32_t index) {
  AvmAssert(data != NULL);
  return data->entries[index];
}

REALLY_INLINE /*static*/ void RCListHelper::store(LISTDATA *data,
                                                  uint32_t index, TYPE value) {
  AvmAssert(data != NULL);
...
REALLY_INLINE /*static*/ void
RCListHelper::storeInEmpty(LISTDATA *data, uint32_t index, TYPE value) {
  AvmAssert(data != NULL);
  AvmAssert(data->entries[index] == 0);
  // We can't use WriteBarrierRC_ctor here, as it will call GetGC() on the
  // address, which might be wrong if the allocation is large. Instead,
  // replicate the necessary code inline (ugh) in the tradition of
  // AvmCore::atomWriteBarrier_ctor...
  if (value) {
    MMgc::GC *const gc = MMgc::GC::GetGC(data);
    if (gc->BarrierActive())
...
REALLY_INLINE /*static*/ void
RCListHelper::clearRange(LISTDATA *data, uint32_t start, uint32_t count) {
  AvmAssert(count > 0);
...
REALLY_INLINE /*static*/ void RCListHelper::moveRange(LISTDATA *data,
                                                      uint32_t srcStart,
                                                      uint32_t dstStart,
                                                      uint32_t count) {
  AvmAssert(data != NULL);
...
REALLY_INLINE AtomList::AtomList(MMgc::GC *gc, uint32_t capacity,
                                 const Atom *args)
...
REALLY_INLINE /*static*/ void AtomListHelper::wbData(const void *container,
                                                     LISTHEADER &header,
                                                     LISTDATA *data) {
  MMgc::GC *const gc = MMgc::GC::GetGC(data);
  if (gc->IsPointerToGCPage(container)) {
...
REALLY_INLINE /*static*/ AtomListHelper::TYPE
AtomListHelper::load(LISTDATA *data, uint32_t index) {
  AvmAssert(data != NULL);
  return data->entries[index];
}

REALLY_INLINE /*static*/ void
AtomListHelper::store(LISTDATA *data, uint32_t index, TYPE value) {
  AvmAssert(data != NULL);
  AvmCore::atomWriteBarrier(MMgc::GC::GetGC(data), data, &data->entries[index],
                            value);
}

REALLY_INLINE /*static*/ void
AtomListHelper::storePointer(LISTDATA *data, uint32_t index, TYPE value) {
  AvmAssert(data != NULL);
#ifdef _DEBUG
  // Never-used slots will be all-bits-zero, used slots will always be tagged
  // properly
  int oldtag = atomKind(data->entries[index]);
  AvmAssert(oldtag == kObjectType || oldtag == kStringType ||
            oldtag == kNamespaceType || data->entries[index] == 0);
  int newtag = atomKind(value);
  AvmAssert(newtag == kObjectType || newtag == kStringType ||
            newtag == kNamespaceType);
#endif
  // This is atomWriteBarrier specialized to the case of known-pointers and
  // inlined.
...
REALLY_INLINE /*static*/ void
AtomListHelper::storeInEmpty(LISTDATA *data, uint32_t index, TYPE value) {
  AvmAssert(data != NULL);
  // newly-allocated space clears to 0; clearRange() clears to nullObjectAtom.
  // Both are "empty" as far as AtomList is concerned.
  AvmAssert(data->entries[index] == 0 ||
            data->entries[index] == nullObjectAtom);
  AvmCore::atomWriteBarrier_ctor(MMgc::GC::GetGC(data), data,
                                 &data->entries[index], value);
}

REALLY_INLINE /*static*/ void
AtomListHelper::clearRange(LISTDATA *data, uint32_t start, uint32_t count) {
  AvmAssert(count > 0);
  AvmCore::decrementAtomRegion_null(&data->entries[start], count);
}

REALLY_INLINE /*static*/ void AtomListHelper::moveRange(LISTDATA *data,
                                                        uint32_t srcStart,
                                                        uint32_t dstStart,
                                                        uint32_t count) {
  AvmAssert(data != NULL);
...
REALLY_INLINE /*static*/ void WeakRefListHelper::wbData(const void *container,
                                                        LISTHEADER &header,
                                                        LISTDATA *data) {
  MMgc::GC *const gc = MMgc::GC::GetGC(data);
  if (gc->IsPointerToGCPage(container)) {
...
REALLY_INLINE /*static*/ WeakRefListHelper::TYPE
WeakRefListHelper::load(LISTDATA *data, uint32_t index) {
  AvmAssert(data != NULL);
...
REALLY_INLINE /*static*/ void
WeakRefListHelper::store(LISTDATA *data, uint32_t index, TYPE value) {
  AvmAssert(data != NULL);
...
REALLY_INLINE /*static*/ void
WeakRefListHelper::storeInEmpty(LISTDATA *data, uint32_t index, TYPE value) {
  AvmAssert(data != NULL);
  AvmAssert(data->entries[index] == 0);
...
REALLY_INLINE /*static*/ void
WeakRefListHelper::clearRange(LISTDATA *data, uint32_t start, uint32_t count) {
  AvmAssert(count > 0);
  VMPI_memset(&data->entries[start], 0, count * sizeof(STORAGE));
}

REALLY_INLINE /*static*/ void WeakRefListHelper::moveRange(LISTDATA *data,
                                                           uint32_t srcStart,
                                                           uint32_t dstStart,
                                                           uint32_t count) {
  AvmAssert(data != NULL);
...
template <class T, class ListHelper>
REALLY_INLINE void
ListImpl<T, ListHelper>::ensureCapacityExtra(uint32_t cap, uint32_t extra) {
  uint32_t const ncap =
      (cap > UINT32_T_MAX - extra)
          ? // if true, cap + extra will overflow a uint32_t...
          UINT32_T_MAX
          : // ...in that case, choose a size that will definitely fail in
            // ensureCapacityImpl.
          (cap + extra);
  if (ncap > capacity()) {
...
template <class T>
REALLY_INLINE GCList<T>::GCList(MMgc::GC *gc, uint32_t capacity,
                                const TYPE *args)
...
template <class T> REALLY_INLINE bool GCList<T>::isEmpty() const {
  return m_list.isEmpty();
}

template <class T> REALLY_INLINE uint32_t GCList<T>::length() const {
  return m_list.length();
}

template <class T> REALLY_INLINE uint32_t GCList<T>::capacity() const {
...
template <class T> REALLY_INLINE void GCList<T>::add(TYPE value) {
  m_list.add(reinterpret_cast<MMgc::GCObject *>(value));
}

template <class T> REALLY_INLINE void GCList<T>::add(const GCList<T> &that) {
...
template <class T>
REALLY_INLINE void GCList<T>::insert(uint32_t index, const TYPE *args,
                                     uint32_t argc) {
  m_list.insert(index, args, argc);
}

template <class T>
REALLY_INLINE void GCList<T>::splice(uint32_t insertPoint, uint32_t insertCount,
                                     uint32_t deleteCount, const TYPE *args) {
...
template <class T> REALLY_INLINE void GCList<T>::reverse() { m_list.reverse(); }

template <class T> REALLY_INLINE void GCList<T>::clear() { m_list.clear(); }

template <class T> REALLY_INLINE int32_t GCList<T>::indexOf(TYPE value) const {
...
template <class T>
REALLY_INLINE typename GCList<T>::TYPE
GCList<T>::operator[](uint32_t index) const {
  return reinterpret_cast<TYPE>(m_list[index]);
}

template <class T> REALLY_INLINE void GCList<T>::ensureCapacity(uint32_t cap) {
  m_list.ensureCapacity(cap);
}

template <class T> REALLY_INLINE uint64_t GCList<T>::bytesUsed() const {
  return m_list.bytesUsed();
}

template <class T> REALLY_INLINE void GCList<T>::destroy() { m_list.destroy(); }

template <class T> REALLY_INLINE bool GCList<T>::isDestroyed() const {
  return m_list.isDestroyed();
}

template <class T> REALLY_INLINE void GCList<T>::gcTrace(MMgc::GC *gc) {
  m_list.gcTrace(gc);
}
...
template <class T>
REALLY_INLINE RCList<T>::RCList(MMgc::GC *gc, uint32_t capacity,
                                const TYPE *args)
...
template <class T> REALLY_INLINE bool RCList<T>::isEmpty() const {
  return m_list.isEmpty();
}

template <class T> REALLY_INLINE uint32_t RCList<T>::length() const {
  return m_list.length();
}

template <class T> REALLY_INLINE uint32_t RCList<T>::capacity() const {
  return m_list.capacity();
}
...
template <class T> REALLY_INLINE void RCList<T>::add(TYPE value) {
  m_list.add(value);
}

template <class T> REALLY_INLINE void RCList<T>::add(const RCList<T> &that) {
  m_list.add(that.m_list);
}
...
template <class T>
REALLY_INLINE void RCList<T>::insert(uint32_t index, const TYPE *args,
                                     uint32_t argc) {
  m_list.insert(index, args, argc);
}

template <class T>
REALLY_INLINE void RCList<T>::splice(uint32_t insertPoint, uint32_t insertCount,
                                     uint32_t deleteCount, const TYPE *args) {
  m_list.splice(insertPoint, insertCount, deleteCount, args);
}
...
template <class T> REALLY_INLINE void RCList<T>::reverse() { m_list.reverse(); }

template <class T> REALLY_INLINE void RCList<T>::clear() { m_list.clear(); }

template <class T> REALLY_INLINE int32_t RCList<T>::indexOf(TYPE value) const {
...
template <class T>
REALLY_INLINE typename RCList<T>::TYPE
RCList<T>::operator[](uint32_t index) const {
  return reinterpret_cast<TYPE>(m_list[index]);
}

template <class T> REALLY_INLINE void RCList<T>::ensureCapacity(uint32_t cap) {
  m_list.ensureCapacity(cap);
}

template <class T> REALLY_INLINE uint64_t RCList<T>::bytesUsed() const {
  return m_list.bytesUsed();
}

template <class T> REALLY_INLINE void RCList<T>::destroy() { m_list.destroy(); }

template <class T> REALLY_INLINE bool RCList<T>::isDestroyed() const {
  return m_list.isDestroyed();
}

template <class T> REALLY_INLINE void RCList<T>::gcTrace(MMgc::GC *gc) {
  m_list.gcTrace(gc);
}
...
template <class T>
REALLY_INLINE UnmanagedPointerList<T>::UnmanagedPointerList(MMgc::GC *gc,
                                                            uint32_t capacity,
                                                            const T *args)
...
template <class T> REALLY_INLINE bool UnmanagedPointerList<T>::isEmpty() const {
  return m_list.isEmpty();
}
...
template <class T> REALLY_INLINE T UnmanagedPointerList<T>::first() const {
  return (T)m_list.first();
}

template <class T> REALLY_INLINE T UnmanagedPointerList<T>::last() const {
...
template <class T> REALLY_INLINE void UnmanagedPointerList<T>::add(T value) {
  m_list.add((UnmanagedPointer)value);
}

template <class T>
REALLY_INLINE void
UnmanagedPointerList<T>::add(const UnmanagedPointerList<T> &that) {
  m_list.add(that.m_list);
}
...
template <class T>
REALLY_INLINE void
UnmanagedPointerList<T>::insert(uint32_t index, const T *args, uint32_t argc) {
  m_list.insert(index, args, argc);
}

template <class T>
REALLY_INLINE void
UnmanagedPointerList<T>::splice(uint32_t insertPoint, uint32_t insertCount,
                                uint32_t deleteCount, const T *args) {
  m_list.splice(insertPoint, insertCount, deleteCount, args);
}

template <class T>
REALLY_INLINE void UnmanagedPointerList<T>::splice(
    uint32_t insertPoint, uint32_t insertCount, uint32_t deleteCount,
    const UnmanagedPointerList<T> &args, uint32_t argsOffset) {
  m_list.splice(insertPoint, insertCount, deleteCount, args, argsOffset);
}

template <class T> REALLY_INLINE void UnmanagedPointerList<T>::reverse() {
  m_list.reverse();
}

template <class T> REALLY_INLINE void UnmanagedPointerList<T>::clear() {
  m_list.clear();
}
...
template <class T> REALLY_INLINE T UnmanagedPointerList<T>::removeFirst() {
  return (T)m_list.removeFirst();
}

template <class T> REALLY_INLINE T UnmanagedPointerList<T>::removeLast() {
...
template <class T> REALLY_INLINE void UnmanagedPointerList<T>::destroy() {
  m_list.destroy();
}
...
template <class T>
REALLY_INLINE WeakRefList<T>::WeakRefList(MMgc::GC *gc, uint32_t capacity,
                                          const TYPE *args)
    : m_list(gc, capacity, (MMgc::GCObject *const *)args) {
...
template <class T> REALLY_INLINE bool WeakRefList<T>::isEmpty() const {
  return m_list.isEmpty();
}

template <class T> REALLY_INLINE uint32_t WeakRefList<T>::length() const {
  return m_list.length();
}

template <class T> REALLY_INLINE uint32_t WeakRefList<T>::capacity() const {
  return m_list.capacity();
}

template <class T>
REALLY_INLINE typename WeakRefList<T>::TYPE
WeakRefList<T>::get(uint32_t index) const {
  return reinterpret_cast<TYPE>(m_list.get(index));
}
...
template <class T> REALLY_INLINE void WeakRefList<T>::add(TYPE value) {
  m_list.add(reinterpret_cast<MMgc::GCObject *>(value));
}
...
template <class T>
REALLY_INLINE void WeakRefList<T>::insert(uint32_t index, const TYPE *args,
                                          uint32_t argc) {
  m_list.insert(index, args, argc);
}
...
template <class T>
REALLY_INLINE void
WeakRefList<T>::splice(uint32_t insertPoint, uint32_t insertCount,
                       uint32_t deleteCount, const TYPE *args) {
  m_list.splice(insertPoint, insertCount, deleteCount, args);
}
...
REALLY_INLINE void
WeakRefList<T>::splice(uint32_t insertPoint, uint32_t insertCount,
                       uint32_t deleteCount, const WeakRefList<T> &args,
                       uint32_t argsOffset) {
  m_list.splice(insertPoint, insertCount, deleteCount, args, argsOffset);
}

template <class T> REALLY_INLINE void WeakRefList<T>::reverse() {
  m_list.reverse();
}

template <class T> REALLY_INLINE void WeakRefList<T>::clear() {
  m_list.clear();
}
...
template <class T>
REALLY_INLINE typename WeakRefList<T>::TYPE
WeakRefList<T>::removeAt(uint32_t index) {
  return reinterpret_cast<TYPE>(m_list.removeAt(index));
}
...
template <class T>
REALLY_INLINE typename WeakRefList<T>::TYPE
WeakRefList<T>::operator[](uint32_t index) const {
  return reinterpret_cast<TYPE>(m_list[index]);
}
...
template <class T> REALLY_INLINE uint64_t WeakRefList<T>::bytesUsed() const {
  return m_list.bytesUsed();
}

template <class T> REALLY_INLINE void WeakRefList<T>::destroy() {
  m_list.destroy();
}

template <class T> REALLY_INLINE bool WeakRefList<T>::isDestroyed() const {
  return m_list.isDestroyed();
}
...
template <class T> REALLY_INLINE void WeakRefList<T>::gcTrace(MMgc::GC *gc) {
  m_list.gcTrace(gc);
}
...
template <class T, uintptr_t align>
REALLY_INLINE DataList<T, align>::DataList(MMgc::GC *gc, uint32_t capacity,
                                           const T *args)
...
template <class T, uintptr_t align>
REALLY_INLINE
DataListAccessor<T, align>::DataListAccessor(DataList<T, align> *v)
    : m_list(v) {}
...
REALLY_INLINE T *DataListAccessor<T, align>::addr() {
  ...
  return (T *)((uintptr_t(m_list->m_header.data->entries) + (align - 1)) &
               ~(align - 1));
}
...
template <class T>
REALLY_INLINE HeapList<T>::HeapList(MMgc::GC *gc, uint32_t capacity,
                                    const typename T::TYPE *args)
