...
private:
  ...
  static const ScopeTypeChain *create(MMgc::GC *gc, Traits *traits,
                                      const ScopeTypeChain *outer,
                                      const FrameValue *values, int32_t nValues,
                                      Traits *append, Traits *extra);

public:
  static const ScopeTypeChain *create(MMgc::GC *gc, Traits *traits,
                                      const ScopeTypeChain *outer,
                                      const FrameState *state, Traits *append,
                                      Traits *extra);
#ifdef VMCFG_AOT
  static const ScopeTypeChain *
  create(MMgc::GC *gc, Traits *traits, const ScopeTypeChain *outer,
         Traits *const *stateTraits, uint32_t nStateTraits,
         uint32_t nStateWithTraits, Traits *append, Traits *extra);
#endif
  static const ScopeTypeChain *createEmpty(MMgc::GC *gc, Traits *traits);
  static const ScopeTypeChain *createExplicit(MMgc::GC *gc, Traits *traits,
                                              Traits **types);
...
private:
  ...
  // Traits are MMgc-allocated, thus always 8-byte-aligned, so the low 3 bits
...
private:
  ...
  uintptr_t GC_POINTERS_SMALL(_scopes[1],
                              fullsize); // Tagged pointers to Traits objects.
...
#endif
private:
  ScopeChain(VTable *vtable, AbcEnv *abcEnv, const ScopeTypeChain *scopeTraits,
             Namespacep dxns);
...
public:
  ...
  */
  ...
  static ScopeChain *create(MMgc::GC *gc, VTable *vtable, AbcEnv *abcEnv,
                            const ScopeTypeChain *scopeTraits,
                            const ScopeChain *outer, Namespacep dxns);

  ScopeChain *cloneWithNewVTable(MMgc::GC *gc, VTable *vtable, AbcEnv *abcEnv,
                                 const ScopeTypeChain *scopeTraits = NULL);
