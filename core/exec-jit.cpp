...
// Attempt to match rules based on MethodInfo and the # of the currently
// processed method Process rules until a match, this means that conflicting
// rules are resolved by first match wins.
...
bool BaseExecMgr::shouldJitFirst(const AbcEnv *abc_env, const MethodInfo *m,
  ...
  if (_ruleSet) {
    // if we're planning to jit, make sure we don't match a jit rule (i.e user
    // specifically wants to jit the method) before searching the interp
    // matches. Similarly if we're planning to interp, search jit rules provided
    // no interp rule matches
    if (willJit) {
...
void BaseExecMgr::setJit(MethodInfo *m, GprMethodProc p) {
  ...
  m->_implGPR = p;
  m->_invoker =
      InvokerCompiler::canCompileInvoker(m) ? jitInvokerNext : invokeGeneric;
#ifdef AVMPLUS_VERBOSE
...
void BaseExecMgr::verifyJit(MethodInfo *m, MethodSignaturep ms,
  ...
  if (verifyOptimizeJit(m, ms, toplevel, abc_env, osr))
    ...
    // New Comment - Fixed it properly by populating Verifier's data members (
    // tryFrom, tryTo ) using existing execption handler table
...
uintptr_t BaseExecMgr::interpGPR(MethodEnv *env, int argc, uint32_t *ap) {
  ...
  const uint32_t ATOM_MASK =
      (1U << BUILTIN_object) | (1U << BUILTIN_void) | (1U << BUILTIN_any);
  if ((1U << bt) & ATOM_MASK)
    return a;
  if (bt == BUILTIN_int)
...
double BaseExecMgr::interpFPR(MethodEnv *env, int argc, uint32_t *ap) {
  ...
  ms->boxArgs(env->core(), argc, (uint32_t *)ap, atomv);
  Atom a = interpBoxed(env, argc, atomv);
  // Note: since we reuse the same trampoline for float & double, the atom may
  // actually be a float
  return FLOAT_ONLY(AvmCore::isFloat(a) ? AvmCore::atomToFloat(a) :)
      AvmCore::number_d(a);
}
...
// uses these to search for and then invoke an early bound interface method
// call.
struct ImtThunkEntry {
  uintptr_t iid;
  uintptr_t
      disp_id; // only needs to be uint32_t, is this size for alignment purposes
};
...
static void sortImtThunkEntries(ImtThunkEntry *map, uint32_t count) {
  ...
  // We'll never get anywhere near the upper limits but it's probably cheaper
  // than extending algorithmically.
  static const uint32_t k_shellsort_intervals[] = {
      1,        4,         9,         20,        46,        103,     233,
  ...
  // Simple ShellSort -- the list rarely gets larger than 100 or so, so this is
...
REALLY_INLINE ImtThunkEnv *ImtThunkEnv::create(MMgc::GC *gc,
                                               uint32_t imtMapCount) {
  return new (gc, MMgc::kExact, imtMapCount * sizeof(ImtThunkEntry))
      ImtThunkEnv(imtMapCount);
}

REALLY_INLINE ImtThunkEnv::ImtThunkEnv(VTable *v) : vtable(v) {}

REALLY_INLINE ImtThunkEnv::ImtThunkEnv(uint32_t c) : imtMapCount(c) {}
...
public:
  inline ImtEntry(ImtEntry *n, uintptr_t iid, uint32_t d)
      : next(n), iid(iid), disp_id(d) {}
  ImtEntry *const next;
  uintptr_t const iid;
  uintptr_t const disp_id;
};

ImtEntry *BaseExecMgr::buildImtEntries(VTable *vtable, uint32_t slot,
                                       uint32_t &count) {
  count = 0;
  ...
  // First check to see if we added any interfaces.  If not, we can just
  // inherit. do this first, since it's pretty common and allows us to skip lots
  // of hashtable lookups and memory allocations.
  ...
  for (InterfaceIterator ifc_iter(traits); ifc_iter.hasNext();) {
    ...
    while (iter.next()) {
      ...
      MethodInfo *v0 = ((1 << ibk) & ((1 << BKIND_METHOD) | (1 << BKIND_GET) |
                                      (1 << BKIND_GETSET)))
      ...
      if (v0 && ImtHolder::hashIID(v0) == slot) {
        ...
        AvmAssert(
            (1 << AvmCore::bindingKind(cBinding)) &
            ((1 << BKIND_METHOD) | (1 << BKIND_GET) | (1 << BKIND_GETSET)));
        map = new (gc)
            ImtEntry(map, ImtHolder::getIID(v0), getRawDispID(cBinding));
        ++count;
      }
      ...
      if (v1 && ImtHolder::hashIID(v1) == slot) {
        if (cBinding == BIND_NONE)
          cBinding = tb->findBinding(name, ns);
        AvmAssert(cBinding != BIND_NONE);
        AvmAssert((1 << AvmCore::bindingKind(cBinding)) &
                  ((1 << BKIND_SET) | (1 << BKIND_GETSET)));
        map = new (gc)
            ImtEntry(map, ImtHolder::getIID(v1), getRawDispID(cBinding) + 1);
        ++count;
      }
    } // while iter.next()
...
REALLY_INLINE MethodEnv *findMethod(ImtThunkEnv *ite, uintptr_t iid,
                                    VTable *vtable) {
...
/*static*/ uintptr_t BaseExecMgr::resolveImt(ImtThunkEnv *ite, int argc,
                                             uint32_t *ap, uintptr_t iid) {
...
/*static*/ uintptr_t BaseExecMgr::dispatchImt(ImtThunkEnv *ite, int argc,
                                              uint32_t *ap, uintptr_t iid) {
...
// or resolveImtSlotSelf must have already been called successfully on the base
// vtable.
void BaseExecMgr::resolveImtSlotFromBase(VTable *vtable, uint32_t slot) {
  ...
  // we want to use our own MethodEnv at the same disp-id. Alas, we don't have
  // the disp-id handy at this point, since we didn't build the ImtEntry chain.
  // Rather than build it in all cases, let's compare vs our parent's method
  // list; linear search, but I'm guessing that's still a net win vs. doing the
  // ImtEntry chain building. (Our base's TB is most likely still in the cache
  // since we used it inside buildImtEntries.)
...
bool BaseExecMgr::resolveImtSlotSelf(VTable *vtable, uint32_t slot) {
  ...
  AvmAssert(vtable->base != NULL); // Only Object and activation objects have
                                   // null base, and neither has interfaces
...
MethodEnv *BaseExecMgr::resolveImtToConcreteMethodEnv(ImtThunkEnv *ite,
                                                      VTable *vtable,
                                                      MethodInfo *info) {
  ...
  // b) more than one method hashed to this slot.  imt[] is updated to
  //    point to ::dispatchImt(), which uses the incoming iid parameter
  //    to choose which concrete MethodEnv to call. For this case we case
  //    findMethodEnv to
  //	find the correct method which is hashed.

  if ((reinterpret_cast<MethodEnv *>(ite))->_implGPR ==
      (GprMethodProc)BaseExecMgr::resolveImt)
    ite = resolveImtSlot(ite, iid);

  if ((reinterpret_cast<MethodEnv *>(ite))->_implGPR ==
      (GprMethodProc)BaseExecMgr::dispatchImt)
...
uint32_t JITNoise::getValue32() {
  // This is less than ideal, as the high order bit will never be set.
  // It is good enough for our purposes, however.
  return uint32_t(MathUtils::GenerateRandomNumber(&randomSeed));
}
...
JITLoggingObserver::JITLoggingObserver(AvmCore *core, int logLevel)
    : _log(NULL), _core(core), _logLevel(logLevel), _curBase(NULL) {
 ...
 */
void JITLoggingObserver::log(uint32_t tag, size_t byte_count,
                             const void *data) {
...
void JITLoggingObserver::flush() { fflush(_log); }
...
void JITLoggingObserver::notifyMethodJITed(MethodInfo *method,
  ...
  for (JITDebugInfo::Info *info = debug_info ? debug_info->info : 0;
    ...
    case JITDebugInfo::kFile: {
      ...
      log('file', sizeof(fileInfo) - sizeof(fileInfo.fileName) + len,
          &fileInfo);
      break;
    }
    default:
...
void JITLoggingObserver::notifyInvokerJITed(MethodInfo *method,
  ...
  if (!name) {
    ...
    // named method
    StUTF8String name8(name);
    len = snprintf(nameBuf, sizeof(nameBuf), "invoke-%s {pool=%d:method=%d}",
                   name8.c_str(), method->pool()->uniqueId(),
                   method->method_id());
  }
...
void JITLoggingObserver::logMethod(const char *nameBuf, int len,
  ...
  for (; !r.empty(); r.popFront()) {
    ...
    struct {
      uint64_t start, end;
    } block = {(uint64_t)r.frontStart(), (uint64_t)r.frontEnd()};
