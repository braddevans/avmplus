...
public:
  ...
  bool isWith;
#ifdef VMCFG_NANOJIT
  // One bit for each of 16 possible SlotStorageTypes for the native
  // representation of this value.  The JIT uses this mask to handle
  // control-flow merges of incompatible values (e.g. int and String*).  At
  // merge points, masks are OR-ed together. If more than one bit is set, the
  // type will be Object or *, and the JIT will use a separate tag byte to
  // convert the native representation to Atom. See CodegenLIR::localGetp().
  uint16_t sst_mask;
#endif
};
...
class FrameState {
  ...
  // Length is frameConsts->frameSize, one entry per local, scope, and stack
  // operand.
private:
  FrameValue *locals;
  MethodInfo *info;
...
public:
  FrameState *wl_next;   // next block in verifier->worklist.  ideally this is
                         // only accessed by Verifier.
...
public:
  bool targetOfBackwardsBranch; // true if this block is reachable from later
                                // code (in linear ABC order)
  bool targetOfExceptionBranch; // true if this block is reachable from
                                // exception dispatch
  bool wl_pending;              // true if this is in verifier->worklist.
                                // Verifier::checkTarget() sets to true.
