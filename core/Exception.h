...
public:
  static ExceptionHandlerTable *create(MMgc::GC *gc, int exception_count) {
    size_t extra = exception_count == 0
                       ? 0
                       : sizeof(ExceptionHandler) * (exception_count - 1);
 ...
 */
enum CatchAction {
  // It is not known what the CATCH block will do.  This should almost never be
  // used.
  kCatchAction_Unknown,

  // The CATCH block will silently consume any exception that occurs, and will
  // not treat it as an error, so exceptions should not be reported to the
  // debugger.
  kCatchAction_Ignore,
  ...
  // 'continue', which will take us back to the 'for' loop to keep going up the
  // exception stack, until we find a frame with some other value.
  kCatchAction_Rethrow,

  // The CATCH block will walk up the stack of ActionScript exception frames,
  // looking for an ActionScript try/catch block which will catch it.
...
public:
  // The interpreter sometimes allocates the exception frame inside a larger
  // data structure and needs the placement new operator.
 ...
 *
 * TRY_UNLESS is to support the optimization that if there are no exception
 * handlers in this frame, we don't need to create the exception frame at all.
 * If expr is true, the exception frame is not created.
 *
 * TRY_UNLESS_HEAPMEM is like TRY_UNLESS but the address at which to allocate
 * the ExceptionFrame is passed explicitly (it is inside some larger
 * heap-allocated block, useful for short-stack systems)
 */
...
#define TRY_UNLESS(core, expr, CATCH_ACTION)                                   \
  {                                                                            \
    ...
    int _setjmpVal = 0;                                                        \
    if ((expr) || (_ef.beginTry(core), _ef.catchAction = (CATCH_ACTION),       \
                   _setjmpVal = VMPI_setjmpNoUnwind(_ef.jmpbuf),               \
                   _ee = core->exceptionAddr, (_setjmpVal == 0)))
