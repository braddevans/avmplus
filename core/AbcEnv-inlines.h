...
REALLY_INLINE AbcEnv *AbcEnv::create(MMgc::GC *gc, PoolObject *builtinPool,
                                     CodeContext *builtinCodeContext) {
  return new (gc, MMgc::kExact, AbcEnv::calcExtra(builtinPool))
      AbcEnv(builtinPool, builtinCodeContext);
}

REALLY_INLINE AvmCore *AbcEnv::core() const { return m_core; }

REALLY_INLINE PoolObject *AbcEnv::pool() const { return m_pool; }

REALLY_INLINE DomainEnv *AbcEnv::domainEnv() const { return m_domainEnv; }

REALLY_INLINE CodeContext *AbcEnv::codeContext() const { return m_codeContext; }
