...
/**
 *  Execution manager pure virtual interface.  An execution manager
 * implementation is responsible for all aspects of AS3 execution, including
 * invocation, policy decisions for how to execute, when to verify, and when to
 * translate, if needed.
 *
...
typedef uintptr_t (*GprImtThunkProc)(class ImtThunkEnv *, int argc,
                                     uint32_t *args, uintptr_t idd);
typedef double (*FprImtThunkProc)(class ImtThunkEnv *, int argc, uint32_t *args,
                                  uintptr_t idd);
 ...
 * Note: for VMCFG_FLOAT we use a function call, to decide the size of a
 * variable per method (if method uses float4, then 16 bytes; otherwise, 8
 * bytes) Also: instead of actually defining VARSIZE, we define VARSHIFT (either
 * 3 or 4) because shifts are faster than multiplications/divisions, when the
 * second operand is a variable (and it's trivial to derive 'size' from 'shift',
 * and we only actually need VARSIZE in asserts - everywhere else we just
 * shift).
 */
#ifdef VMCFG_FLOAT
...
typedef float4_t (*VecrThunkProc)(void *thunk, MethodEnv *env, int32_t argc,
                                  uint32_t *argv);
...
/**
 * The JITNoise class implements a source of random numbers for JIT hardening.
 */
...
public:
  ...
  static float4_t debugEnterExitWrapperV(MethodEnv *env, int32_t argc,
                                         uint32_t *argv);
...
private:
  ...
  static uintptr_t debugEnterExitWrapper32(MethodEnv *env, int32_t argc,
                                           uint32_t *argv);
  static double debugEnterExitWrapperN(MethodEnv *env, int32_t argc,
                                       uint32_t *argv);
  ...
  static void unboxCoerceArgs(MethodEnv *, int32_t argc, Atom *in, uint32_t *ap,
                              MethodSignaturep ms);
  ...
  static void unboxCoerceArgs(MethodEnv *, Atom thisArg, int32_t argc, Atom *in,
                              uint32_t *argv, MethodSignaturep ms);

  /** Coerce and unbox a single argument. */
  static Atom *FASTCALL coerceUnbox1(MethodEnv *, Atom atom, Traits *t,
                                     Atom *args);
...
protected: // Used by AOTExecMgr subclass
   ...
   */
  void verifyCommon(MethodInfo *, MethodSignaturep, Toplevel *, AbcEnv *,
                    CodeWriter *);
...
private:
  ...
  bool shouldJitFirst(const AbcEnv *, const MethodInfo *,
                      MethodSignaturep) const;
  ...
  // are hashed into the IMT by a unique interface method id (IID).  If an IMT
  // slot has no collisions, then the concrete MethodEnv* for the method
  // implementing that interface method is installed.  Otherwise we install
  // dispatchImt().
  //
  // Dispatching: JIT'd call sites pass the IID of the method they early bound
  // to.  If the called slot contains just a concrete MethodEnv, the IID
  // parameter is ignored.  Otherwise we search a table for the correct concrete
   ...
   */
public:
  static MethodEnv *resolveImtToConcreteMethodEnv(ImtThunkEnv *ite,
                                                  VTable *vtable,
                                                  MethodInfo *info);
...
private:
  ...
  static class ImtEntry *buildImtEntries(VTable *vtable, uint32_t slot,
                                         uint32_t &count);

#ifndef VMCFG_HALFMOON_AOT_RUNTIME
  /** Trampoline to resolve this IMT slot then invoke the proper handler. */
  static uintptr_t resolveImt(class ImtThunkEnv *ite, int argc, uint32_t *ap,
                              uintptr_t iid);

#else
  static uintptr_t resolveImt(class ImtThunkEnv *ite, int argc, Atom *args,
                              MethodInfo *methodInfo);
#endif
  /** Trampoline which searches for the method with a matching IID. */
  static uintptr_t dispatchImt(class ImtThunkEnv *ite, int argc, uint32_t *ap,
                               uintptr_t iid);
...
public:
  ...
  bool verifyOptimizeJit(MethodInfo *m, MethodSignaturep ms, Toplevel *,
                         AbcEnv *, OSR *);
...
private:
  ...
  JITNoise noise;            // Random number source for JIT hardening
#endif
};
...
public:
  ...
  void writeOp1(const FrameState *, const uint8_t *pc, AbcOpcode, uint32_t opd1,
                Traits *);
};
...
class ImtHolder : public MMgc::GCInlineObject {
  ...
  // IMT_SIZE should be a number that is relatively prime to
  // sizeof(MethodInfo)/8 since we use the MethodInfo pointer as the interface
  // method id (IID). smaller = dense table, few large conflict tables larger  =
  // sparse table, many small conflict tables

#ifdef _DEBUG
  static const uint32_t IMT_SIZE = 3; // Good for testing all code paths.
#else
  static const uint32_t IMT_SIZE =
      7; // Marginally better for speed/size tradeoff.
#endif

  static uintptr_t
  getIID(MethodInfo *); // Return a full IID for the given interface method.
  static uint32_t
  hashIID(uintptr_t iid); // Hash the IID into an IMT slot number.
  static uint32_t
  hashIID(MethodInfo *); // Hash the method's IID into an IMT slot number.
  class ImtThunkEnv *entries[IMT_SIZE];

public:
  void gcTrace(MMgc::GC *gc) { gc->TraceLocations((void **)entries, IMT_SIZE); }
};
