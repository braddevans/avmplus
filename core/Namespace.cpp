...
Namespace::Namespace(Atom prefix, Stringp uri, NamespaceType flags,
...
#endif // DEBUGGER
      m_prefix(prefix), m_apiVersion(apiVersion) {
  ...
  AvmAssert(AvmCore::isString(prefix)
                ? (AvmCore::atomToString(prefix))->isInterned()
                : 1);
  setUri(uri, flags);
  // ApiVersion should be ALLVERSIONS for all Namespaces that are nonpublic.
  AvmAssert(flags != NS_Public ? apiVersion == kApiVersion_VM_ALLVERSIONS
                               : true);
}
...
bool Namespace::hasPrefix() const {
  return AvmCore::isName(m_prefix) &&
         AvmCore::atomToString(m_prefix)->length() > 0;
}
...
bool Namespace::EqualTo(const Namespace *other) const {
  if (isPrivate() || other->isPrivate()) {
    ...
    return m_uriAndType == other->m_uriAndType &&
