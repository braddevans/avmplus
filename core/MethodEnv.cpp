...
#ifdef DEBUGGER
void MethodEnv::debugEnter(const uint8_t *frame_sst, CallStackNode *callstack,
                           FramePtr framep, volatile intptr_t *eip) {
...
#pragma clang diagnostic pop
  ...
  // method_id can legitimately be -1 for activations, but we don't care about
  // those here, so just ignore them.
  const int32_t method_id = this->method->method_id();
  if (method_id >= 0) {
    this->abcEnv()->invocationCount(method_id) +=
        1; // returns a reference, so this works
  }
}
...
#pragma clang diagnostic pop
  ...
  // trigger a faked "line number changed" since we exited the function and are
  // now back on the old line (well, almost)
...
void FASTCALL MethodEnv::nullcheckfail(Atom atom) {
  ...
  if (error) {
    error->throwError((atom == undefinedAtom) ? kConvertUndefinedToObjectError
...
Atom MethodEnv::getpropertyHelper(Atom obj, /* not const */ Multiname *multi,
                                  VTable *vtable, Atom index) {
...
void MethodEnv::initpropertyHelper(Atom obj, /* not const */ Multiname *multi,
                                   Atom value, VTable *vtable, Atom index) {
  if (atomIsIntptr(index) && atomCanBeInt32(index)) {
...
void MethodEnv::setpropertyHelper(Atom obj, /* not const */ Multiname *multi,
                                  Atom value, VTable *vtable, Atom index) {
...
Atom MethodEnv::delpropertyHelper(Atom obj, /* not const */ Multiname *multi,
                                  Atom index) {
  AvmCore *core = this->core();
  ...
  if (AvmCore::isObject(obj) && AvmCore::isObject(index)) {
    if (AvmCore::isXMLList(index)) {
      ...
      toplevel()->throwTypeError(
          kDeleteTypeError, core->toErrorString(toplevel()->toTraits(index)));
    }
    ScriptObject *i = AvmCore::atomToScriptObject(index);
    if (i->traits() == core->traits.qName_itraits) {
      QNameObject *qname = (QNameObject *)i;
...
ScriptObject *MethodEnv::newcatch(Traits *traits) {
  ...
  if (traits == core->traits.object_itraits) {
    ...
    ScopeChain *catchScope =
        this->scope()->cloneWithNewVTable(core->GetGC(), vt, this->abcEnv());
...
ArrayObject *MethodEnv::createArgumentsHelper(int argc, uint32_t *ap) {
  ...
  // Even tho E3 says create an Object, E4 says create an Array so thats what we
  // will do.
  AvmAssert(argc >= 0);
  MMgc::GC::AllocaAutoPtr _atomv;
  Atom *atomv =
      (Atom *)avmStackAllocArray(core(), _atomv, (argc + 1), sizeof(Atom));
...
/*bool*/ int32_t MethodEnv::haspropertylate_u(Atom obj, uint32_t index) const {
  ...
  if (AvmCore::isFloat4(obj)) {
    // See FIXME in Toplevel::getproperty() - do not absorb non-integer values
    // outside the range [0,2^32-2], but let them pass through to the prototype
    // lookup.
...
// Numeric property name.  Except for float4, no fixed bindings have numeric
// names. Keep in sync with related methods.  See comments at
...
Atom MethodEnv::getpropertylate_i(Atom obj, int32_t index) const {
  if (atomKind(obj) == kObjectType) {
    if (index >= 0) {
      ...
      return AvmCore::atomToScriptObject(obj)->getAtomProperty(
          this->core()->internInt(index)->atom());
    }
  }
#ifdef VMCFG_FLOAT
    ...
    // lookup.
    if (index >= 0)
      toplevel()->throwRangeError(kOutOfRangeError,
                                  this->core()->internInt(index));
  }
#endif
  // *No* else clause
  {
    ...
    return protoObject->ScriptObject::getStringPropertyFromProtoChain(
        core->internInt(index), protoObject, toplevel->toTraits(obj));
  }
}
...
Atom MethodEnv::getpropertylate_u(Atom obj, uint32_t index) const {
  ...
  // *No* else clause
  {
    ...
    ScriptObject *protoObject = toplevel->toPrototype(obj);
    return protoObject->ScriptObject::getStringPropertyFromProtoChain(
        core->internUint32(index), protoObject, toplevel->toTraits(obj));
  }
}
...
Atom MethodEnv::getpropertylate_d(Atom obj, double index) const {
  if (atomKind(obj) == kObjectType) {
    ...
    if (double(index_i) == index && index_i >= 0) {
      ...
      // Sufficiently large non-negative numbers will also take this slower
      // path, but we know an exception will be thrown (Vector) or a dense
      // representation will not be used (Array, others).
      return AvmCore::atomToScriptObject(obj)->getAtomProperty(
          this->core()->internDouble(index)->atom());
    }
  }
#ifdef VMCFG_FLOAT
  else if (AvmCore::isFloat4(obj)) {
    ...
    float *pf4 = reinterpret_cast<float *>(&f4);
    if (index == 0.0)
      return core()->floatToAtom(pf4[0]);
    if (index == 1.0)
      return core()->floatToAtom(pf4[1]);
    if (index == 2.0)
      return core()->floatToAtom(pf4[2]);
    if (index == 3.0)
      return core()->floatToAtom(pf4[3]);
    // See FIXME in Toplevel::getproperty() - do not absorb non-integer values
    // outside the range [0,2^32-2], but let them pass through to the prototype
    // lookup.
  ...
  // *No* else clause
  {
    ...
    ScriptObject *protoObject = toplevel->toPrototype(obj);
    return protoObject->ScriptObject::getStringPropertyFromProtoChain(
        core->internDouble(index), protoObject, toplevel->toTraits(obj));
  }
}

ScriptObject *MethodEnv::finddef(const Multiname *multiname) const {
  // getting toplevel() is slightly more expensive than it used to be (more
  // indirection)... so only extract in the (rare) event of an exception

  AbcEnv *abcEnv = this->abcEnv();
  AvmCore *core = abcEnv->core();
  ScriptEnv *script =
      core->domainMgr()->findScriptEnvInAbcEnvByMultiname(abcEnv, *multiname);
...
/*static*/ ScopeChain *ScriptEnv::createScriptScope(const ScopeTypeChain *stc,
                                                    VTable *_vtable,
                                                    AbcEnv *_abcEnv) {
  AvmCore *core = _vtable->core();
  // NOTE the namespace here is the default xml namespaces. we use the caller's
  // public
  return ScopeChain::create(core->GetGC(), _vtable, _abcEnv, stc, NULL,
                            core->findPublicNamespace());
}
...
ScriptObject *ScriptEnv::initGlobal() {
  ...
  ScriptObject *const delegate =
      objectClass ? objectClass->prototypePtr() : NULL;
...
ScriptObject *MethodEnv::op_newobject(Atom *sp, int argc) const {
  ...
  ScriptObject *o =
      ScriptObject::create(core->GetGC(), object_ivtable,
                           toplevel->objectClass->prototypePtr(), argc);
...
Atom MethodEnv::nextname(Atom objAtom, int index) const {
  ...
  if (AvmCore::isNullOrUndefined(objAtom)) {
    toplevel()->throwTypeError((objAtom == undefinedAtom)
...
#endif // VMCFG_FLOAT
  default:
    ScriptObject *proto = toplevel()->toPrototype(
        objAtom); // cn: types like Number are sealed, but their prototype could
...
ClassClosure *MethodEnv::newfunction(MethodInfo *function, Atom *scopes) const {
  ...
  FunctionClass *functionClass =
      toplevel->_functionClass; // can't use functionClass(), might not be
                                // inited yet
  ...
  ScopeChain *fscope =
      ScopeChain::create(gc, fvtable, abcEnv(), fstc, outer, core->dxns());
...
/**
 * given a classInfo, create a new ClassClosure object and return it on the
 * stack.
 */

ClassClosure *MethodEnv::newclass(Traits *ctraits, ClassClosure *base,
                                  ScopeChain *outer, Atom *scopes) const {
  AvmCore *core = this->core();
  ...
  if (!((base == NULL && itraits->base == NULL) ||
        (base != NULL && itraitsBase == baseIvtableTraits))) {
  ...
  VTable *ivtable =
      core->newVTable(itraits, base ? base->ivtable() : NULL, toplevel);
  ...
  ScopeChain *cscope =
      ScopeChain::create(gc, cvtable, abcEnv, ctraits->init->declaringScope(),
                         outer, core->dxns());
  int i = outer->getSize();
  ...
  ScopeChain *iscope =
      ScopeChain::create(gc, ivtable, abcEnv, itraits->init->declaringScope(),
                         cscope, core->dxns());
  ...
  CreateClassClosureProc createClassClosure =
      cvtable->traits->getCreateClassClosureProc();
...
void MethodEnv::initproperty(Atom obj, const Multiname *multiname, Atom value,
                             VTable *vtable) const {
  Toplevel *toplevel = this->toplevel();

  Traitsp declarer = NULL;
  Binding b =
      toplevel->getBindingAndDeclarer(vtable->traits, *multiname, declarer);

  if (AvmCore::bindingKind(b) == BKIND_CONST) {
    if (this->method != declarer->init)
      toplevel->throwReferenceError(kConstWriteError, multiname,
                                    vtable->traits);
...
void MethodEnv::setpropertylate_i(Atom obj, int32_t index, Atom value) const {
  if (AvmCore::isObject(obj)) {
    ...
    // obj represents a primitive Number, Boolean, int, or String, and
    // primitives are sealed and final.  Cannot add dynamic vars to them.

    // throw a ReferenceError exception  Property not found and could not be
    // created.
    Multiname tempname(core()->getPublicNamespace(method->pool()),
                       core()->internInt(index));
    toplevel()->throwReferenceError(kWriteSealedError, &tempname,
                                    toplevel()->toTraits(obj));
  }
}
...
void MethodEnv::setpropertylate_d(Atom obj, double index, Atom value) const {
  if (AvmCore::isObject(obj)) {
    ...
    // but we know an exception will be thrown (Vector) or a dense
    // representation will not be used (Array, others).
...
void MethodEnv::setpropertylate_f(Atom obj, float index_f, Atom value) const {
  if (AvmCore::isObject(obj)) {
    ...
    // obj represents a primitive Number, Boolean, int, float or String, and
    // primitives are sealed and final.  Cannot add dynamic vars to them.

    // throw a ReferenceError exception  Property not found and could not be
    // created.
    Multiname tempname(core()->getPublicNamespace(method->pool()),
                       core()->internFloat(index_f));
    toplevel()->throwReferenceError(kWriteSealedError, &tempname,
                                    toplevel()->toTraits(obj));
  }
}
...
Atom MethodEnv::callsuper(const Multiname *multiname, int argc,
                          Atom *atomv) const {
  ...
  case BKIND_CONST: {
    ...
    ScriptObject *function =
        AvmCore::atomToScriptObject(atomv[0])->getSlotObject(slot);
...
Atom MethodEnv::delproperty(Atom obj, const Multiname *multiname) const {
  ...
  if (AvmCore::isObject(obj)) {
    ...
    if (b == BIND_NONE) {
      bool b =
          AvmCore::atomToScriptObject(obj)->deleteMultinameProperty(multiname);
...
Atom MethodEnv::getsuper(Atom obj, const Multiname *multiname) const {
  ...
  case BKIND_VAR:
  case BKIND_CONST:
    return AvmCore::atomToScriptObject(obj)->getSlotAtom(
...
void MethodEnv::setsuper(Atom obj, const Multiname *multiname,
                         Atom value) const {
  VTable *vtable = this->vtable()->base;
  Toplevel *toplevel = this->toplevel();
  Binding b = toplevel->getBinding(vtable->traits, multiname);
  ...
  case BKIND_METHOD:
    toplevel->throwReferenceError(kCannotAssignToMethodError, multiname,
                                  vtable->traits);
  ...
  case BKIND_VAR:
    // @todo, this does a redundant coercion -- no harm done, just slightly more
    // work than necessary
    AvmCore::atomToScriptObject(obj)->coerceAndSetSlotAtom(
        AvmCore::bindingToSlotId(b), value);
    return;
...
Atom MethodEnv::findWithProperty(Atom atom, const Multiname *multiname) {
  ...
  if (atomKind(atom) == kObjectType) {
    ...
    do {
      // ISSUE it is incorrect to return a reference to an object on the
      // prototype chain, we should only return the scopechain object; the next
      // operation could be a setproperty, and we don't want to mutate prototype
      // objects this way.
 ...
 */
Atom MethodEnv::findproperty(ScopeChain *outer, Atom *scopes, int extraScopes,
                             const Multiname *multiname, bool strict,
                             Atom *withBase) {
  Toplevel *toplevel = this->toplevel();
  ...
  // if we're in global$init (outer_depth==0), don't check "this" scope just
  // yet.
  ...
  if (outer_depth > 0 && scopep >= scopes) {
    ...
    if (outerTraits->fullsize > outerTraits->size) {
      // scope traits has extra constraint for "this" scope, see OP_newclass in
      // verifier
  ...
  for (int i = outer_depth - 1; i > 0; i--) {
    if (outerTraits->getScopeIsWithAt(i)) {
      ...
      // only look at the properties on the captured (verify time) type, not the
      // actual type, of the outer scope object.
...
Atom MethodEnv::findglobalproperty(Atom target_global,
...
Atom MethodEnv::getdescendants(Atom obj, const Multiname *multiname) {
  if (AvmCore::isObject(obj)) {
    ...
    toplevel()->throwTypeError(
        kDescendentsError, core()->toErrorString(toplevel()->toTraits(obj)));
...
void MethodEnv::checkfilter(Atom obj) {
  if (!AvmCore::isXMLorXMLList(obj)) {
    Toplevel *toplevel = this->toplevel();
    toplevel->throwTypeError(kFilterError,
                             core()->toErrorString(toplevel->toTraits(obj)));
  }
}
...
VTable *MethodEnv::buildActivationVTable() {
  ...
  // The problem: at the ABC level, activation objects can be coerced into plain
  // Objects, but they aren't plain Objects (they are distinct types). If a call
  // to one of Object's methods (e.g. AS3::hasOwnProperty) is early bound to
  // this, we'll attempt to call a method using a non-existent MethodEnv on the
  // activation object's VTable -- crashamundo. The *proper* fix is (arguably)
  // to smarten Verifier to ensure that activation objects can't be coerced, or
  // leaked into a different structure/function that might coerce them, but
  // that's tricky to do and a Quick Fix is needed for Wasabi. Hence, the
  // pragmatic approach: artificially graft the MethodEnvs from Object onto each
  // activation VTable, so that if such a dubious call happens, it quietly calls
  // the proper Object method. (Arguably, a better approach would be to call a
  // method that throws a VerifyError, but since this is intended as a temporary
  // fix for fuzzed/malformed ABC in the first place, I don't think it's worth
  // the extra effort.)
  MMgc::GC *gc = core->GetGC();
  VTable *object_ivtable = toplevel->objectClass->ivtable();
  Traits *object_itraits = object_ivtable->traits;
  uint32_t const object_method_count =
      object_itraits->getTraitsBindings()->methodCount;
  AvmAssert(object_method_count > 0);

  // activations shouldn't ever have methods.
  uint32_t const activation_method_count =
      activationTraits->getTraitsBindings()->methodCount;
  ...
  VTable *activation = new (gc, MMgc::kExact, extraSize)
      VTable(activationTraits, NULL, toplevel);

  ScopeChain *activationScope = this->scope()->cloneWithNewVTable(
      core->GetGC(), activation, this->abcEnv(), activationScopeTraits);
...
VTable *MethodEnv::getActivationVTable() {
  ...
  if (!activationOrMCTable) {
    ...
    activation = buildActivationVTable();
    ActivationMethodTablePair *amtp = new (core()->GetGC())
        ActivationMethodTablePair(activation, getMethodClosureTable());
...
ScriptObject *MethodEnv::newUninitializedActivation() {
  ...
  AvmAssert(obj->getDelegate() ==
            NULL); // activation objects must not have a prototype pointer.
  return obj;
}
...
WeakKeyHashtable *MethodEnv::getMethodClosureTable() {
  ...
  if (!activationOrMCTable) {
    ...
    WeakKeyHashtable *wkh = WeakKeyHashtable::create(core()->GetGC());
    ActivationMethodTablePair *amtp = new (core()->GetGC())
        ActivationMethodTablePair(getActivationVTable(), wkh);
    setActivationOrMCTable(amtp, kActivationMethodTablePair);
    return wkh;
  } else if (type == kActivationMethodTablePair) {
