...
// accurately", ACM PLDI 1990.
//
// Robert G Burger and R Kent Dybvig, "Printing floating-point numbers quickly
// and accurately", ACM PLDI 1996.
//
// The implementation in this file follows the latter paper, which builds on the
// former.
...
const int32_t maxBase2Precision =
    53; // Max number of bits of precision needed to encode this mantissa
const uint64_t two_pow_maxBase2PrecisionMinus1 =
    uint64_t(1) << (maxBase2Precision - 1); // 2^(maxBase2Precision-1)
...
D2A::D2A(double avalue, bool fixedPrecision, int32_t minPrecision)
    : value(avalue), finished(false), bFastEstimateOk(false) {
  ...
  // Represent this double as a ratio of two integers.  Use infinitely precise
  // integers if bFastEstimateOk is false.  mPlus and mMinus represent the range
  // of doubles around this value which would round to this same value when
  // converted from string back to number via atod().
  ...
  if (bFastEstimateOk) {
    ...
    // Adjust stopping conditions for fixed-precision formatting, Burger &
    // Dybvig section 4.
    //
    // Compatibility note:
    ...
    // Dybvig section 4.
    //
    //  Note: this is a cheap approximation of the correct adjustment.  It will
    //  likely
    //   lead to occasional off by one errors in the final digit generated for
    //   toPrecision() when a truncation might be required.  This mode is
    //   actually less accurate than normal mode anyway. Only normal mode is
    //   gauranteed to generate a string which will result in the same value
    //   when converted back from string (base 10) to double (base 2).  Extra
    //   digits generated by toPrecision will often be junk (but will display a
    //   value more similar to what you would see in a code debugger, where
    //   0.012 can
...
// nextDigit() returns the next relevant digit in the number being converted,
// else -1 if there are no more relevant digits.
...
int32_t D2A::nextDigit() {
  ...
  if (bFastEstimateOk) {
    ...
    // check if remaining ratio r/s is within the range of floats which would
    // round to the value we have output
    //  so far when read in from a string.
    withinLowEndRoundRange = (lowOk ? (dr <= dMMinus) : (dr < dMMinus));
    withinHighEndRoundRange =
        (highOk ? (dr + dMPlus >= ds) : (dr + dMPlus > ds));
  } else {
    ...
    quotient = (int32_t)(
        bigQuotient
            .wordBuffer[0]); // todo: optimize away need for BigInteger result?
                             // We know it should always be a single digit r <=
                             // mMinus               :  r < rMinus
    withinLowEndRoundRange =
        (lowOk ? (r.compare(&mMinus) != 1) : (r.compare(&mMinus) == -1));
    // r+mPlus >= s                     :  r+mPlus > s
    withinHighEndRoundRange = (highOk ? (r.compareOffset(&s, &mPlus) != -1)
                                      : (r.compareOffset(&s, &mPlus) == 1));
  }
  ...
  if (!withinLowEndRoundRange) {
    if (!withinHighEndRoundRange) // if not within either error range, set up to
    ...
    if ((bFastEstimateOk && (dr * 2 < ds)) ||
        (!bFastEstimateOk &&
         (r.compareOffset(&s, &r) ==
          -1))) // if (r*2 < s)  todo: faster to do lshift and compare?
    {
      finished = true;
    } else {
      quotient++;
...
int32_t D2A::fixup_ExponentEstimate(int32_t exponentEstimate) {
  ...
  if (bFastEstimateOk) {
    ...
    if (highOk ? (r.compareOffset(&s, &mPlus) != -1)
               : (r.compareOffset(&s, &mPlus) == 1)) {
      correctedEstimate = exponentEstimate + 1;
    } else {
      r.multBy((int32_t)10);
...
int32_t D2A::scale() {
  ...
  int32_t exponentEstimate =
      (int32_t)MathUtils::ceil((base2Exponent * kLog2_10) - 0.0000000001);

  if (bFastEstimateOk) {
    double scale = quickPowTen((exponentEstimate > 0) ? exponentEstimate
                                                      : -exponentEstimate);
    ...
    quickBigPowTen(
        (exponentEstimate > 0) ? exponentEstimate : -exponentEstimate, &scale);
