...
CodeContext *getFunctionCodeContext(const ScriptObject *so) {
  ...
  //  CodeContext* cc =
  //  ((avmplus::FunctionObject*)so)->getFunctionCodeContext();
  // getFunctionCodeContext() has been temporarily relocated to ScriptObject, as
  // AIR defines some classes that are subclasses of Function (in AS3) but not
  // of FunctionObject (in C++)...
  avmplus::CodeContext *cc =
      ((avmplus::ScriptObject *)so)->getFunctionCodeContext();
  return cc;
}
...
Atom callFunction(ScriptObject *f, int argc, Atom *args) {
  AvmAssertMsg(
      f != NULL,
      "f must not be null. Use callFunction(toplevel, atom, ...) form");
