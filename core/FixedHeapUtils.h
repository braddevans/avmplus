...
/*
 * Utility collection, allocated on the stack or as a member field, passed by
 * value.
 */
template <typename TYPE> struct FixedHeapArray {
  TYPE *values;
  int length;
...
template <typename K, typename V> class FixedHeapHashTable {
public:
  ...
  // helper methods so subclasses can build/tear down with their vtables (and
  // hence allocator functions in tact)
protected:
...
template <class T, int TAG = 0> class FixedHeapRef {
public:
