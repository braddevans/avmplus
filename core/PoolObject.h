...
public:
  REALLY_INLINE static ConstantStringContainer *
  create(MMgc::GC *gc, size_t extra, PoolObject *pool) {
...
class GC_CPP_EXACT(PoolObject, MMgc::GCFinalizedObject) {
  ...
  PoolObject(AvmCore *core, ScriptBuffer &sb, const uint8_t *startpos,
             ApiVersion apiVersion);

public:
  static PoolObject *create(AvmCore *core, ScriptBuffer &sb,
                            const uint8_t *startpos, ApiVersion apiVersion);
  ...
  static void
  destroyPrecomputedMultinames(ExactStructContainer<HeapMultiname> *self);
  ...
  Traits *resolveTypeName(uint32_t index, const Toplevel *toplevel,
                          bool allowVoid = false);
  Traits *resolveTypeName(const uint8_t *&pc, const Toplevel *toplevel,
                          bool allowVoid = false);

  void resolveBindingNameNoCheck(uint32_t index, Multiname &m,
                                 const Toplevel *toplevel) const;
  void resolveBindingNameNoCheck(const uint8_t *&p, Multiname &m,
                                 const Toplevel *toplevel) const;

  Traits *resolveParameterizedType(const Toplevel *toplevel, Traits *base,
                                   Traits *type_param) const;
  ...
  Atom getLegalDefaultValue(const Toplevel *toplevel, uint32_t index,
                            CPoolKind kind, Traits *t);
  static bool isLegalDefaultValue(BuiltinType bt, Atom value);

#ifdef VMCFG_FLOAT
  bool hasFloatSupport() const {
    return version >=
           0x002F0010; /* (47<<16|16), i.e. Cyril, major v.47, minor v.16 */
  }
#endif
  bool hasExceptionSupport() const {
    return version != 0x002E000F; /* (46<<16|15), i.e. major v.46, minor v.15 */
  }
...
#ifdef VMCFG_FLOAT
  GCList<GCFloat>
      GC_STRUCTURE(cpool_float); // We allocate 8 bytes/float in heap.
...
private:
  GCMember<ExactStructContainer<HeapMultiname>>
      GC_POINTER(precompNames); // a GCFinalizedObject
...
private:
  ...
  GCMember<ConstantStringContainer>
      GC_POINTER(_abcStrings); // The length is constantStringCount
  ...
  // Indexed by MethodInfo::_method_id, if the value is positive, it's an index
  // into cpool_string; if negative, an index into cpool_mn. Always safe because
  // those indices are limited to 30 bits.
  DataList<int32_t> GC_STRUCTURE(_method_name_indices);
  ApiVersion const _apiVersion;
  int32_t
      _uniqueId; // _uniqueId + mi->method_id() produces a unique id for methods
...
public:
  ...
  bool isBuiltin; // true if this pool is baked into the player.  used to
                  // control whether callees will set their context.
...
#ifdef VMCFG_HALFMOON_AOT_COMPILER
  compile_abc::PoolInfo
      *poolInfo; // Things the AOT compiler tracks about this pool
#endif
  // ------------------------ DATA SECTION END
};
} // namespace avmplus
