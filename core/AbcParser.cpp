...
PoolObject *AbcParser::decodeAbc(AvmCore *core, ScriptBuffer code,
  ...
  if (core->isVerbose(VB_parse))
    core->console << "major=" << (version & 0xFFFF)
                  << " minor=" << (version >> 16) << "\n";
#endif
  ...
  if (result != 0) {
    ...
    case kInvalidMagicError:
      toplevel->throwVerifyError(kInvalidMagicError,
                                 core->toErrorString(version >> 16),
                                 core->toErrorString(version & 0xFFFF));
  ...
  {
    if (version == 0 &&
        code.getSize() == 24) // only ABC "references" -- these are 0.0 version
                              // followed by SHA1
    {
      ...
      for (uint32_t i = 0; i < nAOTInfos; i++) {
        ...
        const AOTInfo *aotInfo =
            aotInfos[i]; // in halfmoon, array of pointers to AOTInfo
#else
        ...
        if (!memcmp(aotInfo->origABCSHA1, sha1, 20)) {
          ScriptBuffer code =
              ScriptBuffer(new (core->GetGC()) ReadOnlyScriptBufferImpl(
                  aotInfo->abcBytes, aotInfo->nABCBytes));

          NativeInitializer ninit(core, NULL, aotInfo, 0, 0);
          PoolObject *pool =
              decodeAbc(core, code, toplevel, domain, &ninit, apiVersion);
          AvmAssert(pool != NULL);
          AvmAssert(!pool->isBuiltin);
          AvmAssert(pool->aotInfo == aotInfo);
#ifdef DEBUGGER
          // TODO: is there a better pace to do this for both builtins and user
          // scripts?
    ...
    toplevel->throwVerifyError(
        kCorruptABCError); // not a reference or not found
    return NULL;
  }
#endif
  ...
  if (debugger) {
    ...
    if (methodDebugInfos != NULL) {
      ...
      for (int n = 0; n < (int)aotInfo->nABCMethods; n++) {
        ...
        DebuggerMethodInfo *dmi = DebuggerMethodInfo::create(
            core, info.local_count, -1, info.scope_count);
      ...
      // hook the ABC in
      debugger->pool2abcIndex.add(pool,
                                  (const void *)debugger->abcList.length());
...
AbcParser::AbcParser(AvmCore *core, ScriptBuffer code, Toplevel *toplevel,
                     Domain *domain, const NativeInitializer *natives)
    : instances(core->GetGC(), 0), toplevel(toplevel), domain(domain),
      metaNames(core->GetGC(), 0), cachedOsrThresholdString(NULL),
...
Namespacep AbcParser::parseNsRef(const uint8_t *&pc) const {
  ...
  if (index >= pool->constantNsCount)
    toplevel->throwVerifyError(kCpoolIndexRangeError,
                               core->toErrorString(index),
...
void AbcParser::parseTypeName(const uint8_t *&pc, Multiname &m) const {
  ...
  if (index >= pool->cpool_mn_offsets.length())
    toplevel->throwVerifyError(
        kCpoolIndexRangeError, core->toErrorString(index),
...
uint32_t AbcParser::resolveBindingName(const uint8_t *&p, Multiname &m) const {
  ...
  // Only builtin traits names can have multiple namespaces, which are used as
  // an implementation mechanism of the versioning API, and express multiple
  // introducing versions.
  if (m.isBinding() && (m.isQName() || pool->isBuiltin))
    return index;
  else {
    toplevel->throwVerifyError(kCpoolEntryWrongTypeError,
                               core->toErrorString(index));
...
MethodInfo *AbcParser::resolveMethodInfo(uint32_t index) const {
  ...
  if (index >= c)
    toplevel->throwVerifyError(kMethodInfoExceedsCountError,
                               core->toErrorString(index),
                               core->toErrorString(c));

  MethodInfo *f = pool->getMethodInfo(index);
  if (!f)
    toplevel->throwVerifyError(kMethodInfoOrderError,
                               core->toErrorString(index));
...
PoolObject *AbcParser::parse(ApiVersion apiVersion) {
  ...
  // parse all methodInfos in one pass.  Nested functions must come before outer
  // functions
  parseMethodInfos();

  // parse all metadataInfos - AVM+ doesn't care about this, so we are really
  // just skipping them
  parseMetadataInfos();

  // parse classes.  base classes must come first.
  if (!parseInstanceInfos())
    return NULL;
  ...
  if (CLASS_TYPE == NULL) {
    ...
    // register "void"
    addNamedTraits(core->getPublicNamespace(kApiVersion_VM_ALLVERSIONS),
...
void AbcParser::parseExecPolicyAttributes(const uint8_t *metadata,
                                          MethodInfo *m) {
...
/**
 * setting up a traits that extends another one.  Two parser passes are
 * required, one for sizing the traits object and the other for allocating it
 * and filling it in.
 */
Traits *AbcParser::parseTraits(uint16_t sizeofInstance, uint16_t offsetofSlots,
                               Traits *base, Namespacep ns, Stringp name,
                               MethodInfo *script, TraitsPosPtr traitsPos,
                               TraitsPosType posType,
                               Namespacep protectedNamespace, bool makeFinal) {
  ...
  if (posType == TRAITSTYPE_INTERFACE && base != NULL)
    toplevel->throwVerifyError(kCannotExtendError, name,
  ...
  Traits *traits = Traits::newTraits(pool, base, sizeofInstance, offsetofSlots,
                                     traitsPos, posType);
  ...
  for (uint32_t i = 0; i < nameCount; i++) {
    ...
    default:
      ...
      toplevel->throwVerifyError(kUnsupportedTraitsKindError,
                                 core->toErrorString(kind));
    }
    ...
    case TRAIT_Class: {
      ...
      if (kind == TRAIT_Class) {
        ...
        if (class_index >= classCount)
          toplevel->throwVerifyError(kClassInfoExceedsCountError,
                                     core->toErrorString(class_index),
                                     core->toErrorString(classCount));

        // class_index could be legal (<classCount) but pointing to
        // to a class not yet parsed (>= pool->classCount()), so handle that
        // case.
        Traits *ctraits = class_index < pool->classCount()
                              ? pool->getClassTraits(class_index)
                              : NULL;
        if (!ctraits)
          toplevel->throwVerifyError(kClassInfoOrderError,
                                     core->toErrorString(class_index));
...
void AbcParser::parseMethodInfos() {
  ...
  for (int i = 0; i < methodCount; i++) {
    ...
    NativeMethodInfo compiledMethodInfo = {{0}, 0, 0};
    {
      ...
      if (!ni && natives->getCompiledInfo(&compiledMethodInfo, &handler,
                                          returnTypeName, i)) {
    ...
    const int optional_count =
        (abcFlags & abcMethod_HAS_OPTIONAL) ? readU30(pos) : 0;

    MethodInfo *info =
        MethodInfo::create(core->GetGC(), i, pool, info_pos, abcFlags, ni);
...
#ifdef DEBUGGER
    // don't create the corresponding DebuggerMethodInfo yet, we'll do that in
    // parseMethodBodies
#endif
  }
}
...
void AbcParser::parseMethodBodies() {
  ...
  for (int i = 0; i < bodyCount; i++) {
    ...
    if (code_length <= 0) {
      toplevel->throwVerifyError(kInvalidCodeLengthError,
                                 core->toErrorString(code_length));
    }
#endif
...
#ifdef AVMPLUS_VERBOSE
    if (pool->isVerbose(VB_parse)) {
      core->console << "    " << offset << ":method[" << method_index
                    << "] max_stack=" << max_stack
      ...
      for (int i = 0; i < exception_count; i++) {
        ...
        if (name_index != 0) {
          if (name_index >= pool->cpool_mn_offsets.length())
            toplevel->throwVerifyError(
                kCpoolIndexRangeError, core->toErrorString(name_index),
                core->toErrorString(pool->constantCount));
    ...
    if (info->hasMethodBody()) {
      ...
      if (declaringTraits && declaringTraits->isInterface()) {
        toplevel->throwVerifyError(kIllegalInterfaceMethodBodyError,
      ...
      if (core->debugger()) {
        DebuggerMethodInfo *dmi = DebuggerMethodInfo::create(
            core, local_count, code_length, max_scope_depth - init_scope_depth);
      ...
      if (info->abc_body_pos()) {
        toplevel->throwVerifyError(kDuplicateMethodBodyError,
                                   core->toErrorString(info));
      }
      ...
      // there will be a traits_count here, even if abcMethod_NEED_ACTIVATION is
      // not set.  So we parse the same way all the time.  We could reduce file
      // size and memory by omitting the count + traits completely.
      ...
      if (info->needActivation() || nameCount > 0) {
        ...
        if (core->config.methodNames) {
          ...
          name = core->internString(
              info->getMethodNameWithTraits(declaringTraits));
        }
#endif
        ...
        Traits *act =
            parseTraits(sizeof(ScriptObject), sizeof(ScriptObject), NULL, ns,
                        name, NULL, traits_pos, TRAITSTYPE_ACTIVATION, NULL);
      ...
      if (info->needActivation()
#ifdef VMCFG_HALFMOON_AOT_RUNTIME // In the testcase abcasm/nullCheck/D.abs,
                                  // nameCount equals 1. So, the
                                  // AvmAssert(namecount==0) may not be valid
          || nameCount > 0
#endif
      ) {
        pos = traits_pos;
        ...
        if (core->config.methodNames) {
          ns = core->getPublicNamespace(pool);
          name = core->internString(
              info->getMethodNameWithTraits(info->declaringTraits()));
        }
#endif
        // activation traits are raw types, not subclasses of object.  this is
        // okay because they aren't accessable to the programming model.
        Traits *act =
            parseTraits(sizeof(ScriptObject), sizeof(ScriptObject), NULL, ns,
                        name, NULL, pos, TRAITSTYPE_ACTIVATION, NULL);
        info->init_activationTraits(act);
      } else
#endif
      {
#ifndef VMCFG_HALFMOON_AOT_RUNTIME
        toplevel->throwVerifyError(kIllegalNativeMethodBodyError,
                                   core->toErrorString(info));
#endif
      }
    }
...
void AbcParser::parseCpool(ApiVersion apiVersion) {
...
#ifdef VMCFG_HALFMOON_AOT_RUNTIME
  const_cast<AOTInfo *>(pool->aotInfo)
      ->pools.push_back(std::make_pair(pool, pool->core));
#endif
#else
  pool->isBuiltin = (natives != NULL);
#endif

  verbose_only(pool->verbose_vb =
                   core->config.verbose_vb); // pool picks up global settings
  ...
  // In principle, if the numeric pools are sized to make element zero available
  // then initialize that element.
  //
  // Note that for int, uint, double, float, and float4 we still throw
  // VerifyError in the Verifier if element zero is referenced from an
  // instruction, but that really should change, the documentation has
  // traditionally said that they're available.
  //
  // In practice, the Debugger may actually access these values if they are
  // referenced even in content that does not verify, so make them available
  // always.  Bugzilla 520888.
  ...
  for (uint32_t i = 1; i < string_count; ++i) {
    ...
    // don't call readU30(), as it can throw. call readU32 and sanity-check
    // ourself here.
    uint32_t len = AvmCore::readU32(pos);

    // check to see if we are trying to read past the file end or the beginning.
    // FIXME: bug #545652, should check if UTF8 is valid, skipping for
    // compatibility.
    if ((len & 0xc0000000) || pos < abcStart || pos + len >= abcEnd) {
      // if we throw a verify error here, _abcStringEnd will never be set, and
      // _abcStrings will be left in an inconsistent state. having
      // _abcStringStart set but not _abcStringEnd can cause dynamicizeStrings
      // to make poor decisions. So clean up before throwing.
      pool->_abcStringStart = NULL;
      pool->_abcStringEnd = NULL;
      VMPI_memset(pool->_abcStrings->data, 0,
                  string_count * sizeof(ConstantStringData *));
      toplevel->throwVerifyError(kCorruptABCError);
    }
  ...
  for (uint32_t i = 1; i < ns_count; ++i) {
    ...
    case CONSTANT_StaticProtectedNs: {
      ...
      if (index) {
        ...
        bool const hasVersionMark =
            core->getBaseURIAndApiVersion(uri, apiVersion);
        if (pool->isBuiltin) {
          // Finally, builtin public namespaces that are in the versioned-uri
          // list, but don't have an explicit version mark, get whisked off into
          // the VM_INTERNAL hidey hole.
          if (!hasVersionMark && type == Namespace::NS_Public &&
              core->isVersionedURI(uri))
        ...
        Namespacep ns =
            core->internNamespace(core->newNamespace(uri, type, apiVersion));
        cpool_ns.set(i, ns);
      } else {
    ...
    case CONSTANT_PrivateNs: {
      ...
      Namespacep ns =
          Namespace::create(core->GetGC(), nullStringAtom, uri,
                            Namespace::NS_Private, kApiVersion_VM_ALLVERSIONS);
      cpool_ns.set(i, ns);
      break;
    }
    default: {
      toplevel->throwVerifyError(kCpoolEntryWrongTypeError,
                                 core->toErrorString(i));
    }
    }
#ifdef AVMPLUS_VERBOSE
    if (pool->isVerbose(VB_parse)) {
      core->console << "    " << offset << ":"
                    << "cpool_ns[" << i << "]=" << constantNames[kind] << " "
                    << cpool_ns[i] << "\n";
    }
#endif
  }
  ...
  for (uint32_t i = 1; i < ns_set_count; ++i) {
    ...
    NamespaceSet *namespace_set =
        NamespaceSet::_create(core->GetGC(), ns_count);
  ...
  for (uint32_t i = 1; i < mn_count; ++i) {
    ...
    case CONSTANT_MultinameA: {
      ...
      if (!index || index >= pool->constantNsSetCount)
        toplevel->throwVerifyError(
            kCpoolIndexRangeError, core->toErrorString(index),
            core->toErrorString(pool->constantNsSetCount));

      // If it is in the range of Namespace Sets then it must be a namespace
      // set/
      break;
    }
    ...
    case CONSTANT_TypeName: {
      ...
      // compare index against mn_count, *not* cpool_mn_offsets.length(), as the
      // latter is still being built... it's ok to forward-reference here.
      if (!index || index >= mn_count)
        toplevel->throwVerifyError(kCpoolIndexRangeError,
                                   core->toErrorString(index),
                                   core->toErrorString(mn_count));
...

  {
    ...
    for (uint32_t i = 1; i < mn_count; ++i) {
      ...
      uint32_t basetype_index =
          readU30(p); // type being parameterized (currently, always Vector)
      ...
      const uint8_t *p_basetype =
          pool->_abcStart + cpool_mn_offsets[basetype_index];
      ...
      // now check for cycles. we can nest arbitrarily deep (eg
      // Vector<Vector<Vector<uint>>>) but we can't have a cycle.
      seen.reset();
      for (;;) {
        if (pt_index == 0) // Vector<*> is a special case
          break;
        seen.set(pt_index);
        const uint8_t *p_paramtype =
            pool->_abcStart + cpool_mn_offsets[pt_index];
        if (*p_paramtype++ != CONSTANT_TypeName)
          break;
        readU30(p_paramtype); // skip type being parameterized (currently,
                              // always Vector)
        readU30(p_paramtype); // skip param count (currently always 1)
        pt_index =
            readU30(p_paramtype); // param type(s) (currently exactly one)
...
void AbcParser::addNamedTraits(NamespaceSetp nss, Stringp name,
                               Traits *itraits) {
...
void AbcParser::addNamedScript(NamespaceSetp nss, Stringp name,
                               MethodInfo *script) {
...
bool AbcParser::parseScriptInfos() {
  /*
      ...
      ScriptInfo[script_count]
      {
          ...
          Traits script_traits    // traits for the global object of this
     package
      }
  */
  ...
  for (uint32_t i = 0; i < count; i++) {
    ...
    if (declaringTraits != NULL) {
      // method has already been bound to a different type.  Can't bind it twice
      // because it can only have one environment, for its scope chain and super
      // references.
      toplevel->throwVerifyError(kAlreadyBoundError,
                                 core->toErrorString(script),
                                 core->toErrorString(declaringTraits));
    }

    Traits *traits =
        parseTraits(sizeof(ScriptObject), sizeof(ScriptObject),
                    core->traits.object_itraits, core->getPublicNamespace(pool),
                    core->kglobal, script, script_pos, TRAITSTYPE_SCRIPT, NULL);

    if (!traits)
      return false; // parseTraits failed
...
// error - attempt to extend final class
...
#endif
      toplevel->throwVerifyError(kCannotExtendFinalClass,
                                 core->toErrorString(&mn));
    }

    if (baseTraits && baseTraits->isInterface()) {
      // error, can't extend interface
      toplevel->throwVerifyError(kCannotExtendError, core->toErrorString(&mn),
                                 core->toErrorString(baseTraits));
    }
    ...
    if (interfaceCount) {
      ...
      for (int x = 0; x < interfaceCount; ++x) {
        ...
        if (!t || !t->isInterface()) {
          ...
          toplevel->throwVerifyError(kCannotImplementError,
                                     core->toErrorString(&mn),
    ...
    if (pool->isVerbose(VB_parse)) {
      ...
      core->console << "    " << (int)(instancepos - startpos) << ":instance["
                    << i << "]"
                    << " " << mn;
      ...
      core->console << " interface_count=" << interfaceCount
                    << " iinit_index=" << iinit_index << "\n";
    }
#endif
    TraitsPosType postype =
        (flags & 4) ? TRAITSTYPE_INTERFACE : TRAITSTYPE_INSTANCE;
    uint16_t sizeofInstance, offsetofSlots;

    // If this is a native class, use the stated instance size and offset to
    // slot area.
    const NativeClassInfo *nativeEntry;
    if (natives && (nativeEntry = natives->get_class(i)) != NULL &&
        nativeEntry->sizeofInstance) {
      ...
      // Since nativegen.py emits "synthetic" C++ classes for all builtin
      // classes (except interfaces), only non-builtin classes should hit this
      // code; since they don't have C++ representations, "overlap" of fields
      // between parent and child isn't possibly so we can safely declare the
      // offsetOfSlots to be an the end of the base.
#ifdef VMCFG_VERIFYALL
      // (Don't bother with this assert in verifyonly mode, as avmglue.abc (etc)
      // will appear to be non-builtin (since we specify them as arguments,
      ...
      for (Traits *b = baseTraits; b != NULL; b = b->base) {
        if (b->getSizeOfInstance() > sizeof(ScriptObject)) {
          // non-Object base class uses a subclass of ScriptObject, so use that
          // size.
    ...
    Traits *itraits =
        parseTraits(sizeofInstance, offsetofSlots, baseTraits, ns, name, 0,
                    instancepos, postype, protectedNamespace, (flags & 2) != 0);
    if (!itraits)
      return false;
    ...
    if (declaringTraits != NULL) {
      ...
      // because it can only have one environment, for its scope chain and super
      // references.
      toplevel->throwVerifyError(kAlreadyBoundError, core->toErrorString(iinit),
                                 core->toErrorString(declaringTraits));
    }
...
void AbcParser::parseClassInfos() {
  ...
  for (uint32_t i = 0; i < classCount; i++) {
...
#ifdef AVMPLUS_VERBOSE
    if (pool->isVerbose(VB_parse)) {
      core->console << "    " << (int)(class_pos - startpos) << ":class[" << i
                    << "]"
                    << " " << ns << "::" << name;

      core->console << " cinit_index=" << cinit_index << "\n";
    }
#endif
    ...
    Traits *ctraits = parseTraits(
        haveClassNativeInfo ? nativeEntry->sizeofClass : sizeof(ClassClosure),
        haveClassNativeInfo ? nativeEntry->offsetofSlotsClass
                            : sizeof(ClassClosure),
        CLASS_TYPE, ns,
        core->internString(
            core->concatStrings(name, core->newConstantStringLatin1("$"))),
        NULL, class_pos, TRAITSTYPE_CLASS, itraits->protectedNamespace);
    ...
    ctraits->setCreateClassClosureProc(haveClassNativeInfo
                                           ? nativeEntry->createClassClosure
                                           : ClassClosure::createClassClosure);
    ...
    if (declaringTraits != NULL) {
      ...
      // because it can only have one environment, for its scope chain and super
      // references.
      toplevel->throwVerifyError(kAlreadyBoundError, core->toErrorString(cinit),
                                 core->toErrorString(declaringTraits));
    }
