...
void WorkerObjectBase<T>::throwIllegalOperationError(int errorID) {
  ...
  toplevel->builtinClasses()->get_IllegalOperationErrorClass()->throwError(
      errorID);
}

template <class T> WorkerObjectBase<T>::WorkerObjectBase() : m_isolate(NULL) {}

template <class T> T *WorkerObjectBase<T>::self() {
  return static_cast<T *>(this);
}

template <class T> void WorkerObjectBase<T>::initialize(Isolate *isolate) {
  if (isolate == NULL) {
    ...
    // (you'd need > 2 billion active workers), memory, or aggregate is in
    // shutdown
...
GCRef<ScriptObject> WorkerObjectBase<T>::setIsolate(Isolate *isolate) {
  ...
  AvmAssert(self()->toplevel()->getInternedObject(m_isolate) ==
            NULL); // always check the intern table first
...
template <class T> WorkerObjectBase<T>::~WorkerObjectBase() {
  m_isolate = NULL;
}
...
template <class T> bool WorkerObjectBase<T>::isPrimordial() const {
  // the isolate can be NULL in the situation where a
  // worker is busy creating workers during a shutdown cycle,
  // as the logic for creating an isolate will return NULL
  // in that situation.
...
template <class T> void WorkerObjectBase<T>::start() {
  // because AS thunkers cannot call virtual methods on all
  // platforms one level of indirection is used.
  internalStart();
}

template <class T> Stringp WorkerObjectBase<T>::get_state() {
  // the isolate can be NULL in the situation where a
  // worker is busy creating workers during a shutdown cycle,
  // as the logic for creating an isolate will return NULL
  // in that situation.
...
template <class T> bool WorkerObjectBase<T>::internalStop() {
  // the isolate can be NULL in the situation where a
  // worker is busy creating workers during a shutdown cycle,
  // as the logic for creating an isolate will return NULL
  // in that situation.
  if (m_isolate) {
    ...
    if (aggregate->isPrimordial(m_isolate->getDesc())) {
      ...
      return aggregate->requestIsolateExit(m_isolate->getDesc(),
                                           self()->toplevel());
    }
  } else {
    return false;
...
template <class T> Atom WorkerObjectBase<T>::getSharedProperty(String *key) {
  // the isolate can be NULL in the situation where a
  // worker is busy creating workers during a shutdown cycle,
  // as the logic for creating an isolate will return NULL
  // in that situation.
...
template <class T> void WorkerObjectBase<T>::internalStart() {
  // the isolate can be NULL in the situation where a
  // worker is busy creating workers during a shutdown cycle,
  // as the logic for creating an isolate will return NULL
  // in that situation.
  if (m_isolate) {
    ...
    if (m_isolate->getParentDesc() !=
        AvmCore::getActiveCore()->getIsolate()->getDesc())
...
template <class T> T *WorkerDomainObjectBase<T>::self() {
  return static_cast<T *>(this);
}

template <class T>
ObjectVectorObject *WorkerDomainObjectBase<T>::listWorkers() {
  return self()->core()->getIsolate()->getAggregate()->listWorkers(
      self()->toplevel());
}
