...
REALLY_INLINE uint32_t InlineHashtable::getSize() const { return m_size; }
...
REALLY_INLINE InlineHashtable::AtomContainer *
InlineHashtable::createAtoms(MMgc::GC *gc, int capacity) const {
  size_t extra = 0;
  if (capacity > 0)
    extra =
...
REALLY_INLINE HeapHashtableRC *HeapHashtableRC::create(
...
REALLY_INLINE uint32_t HeapHashtableRC::getSize() const { return ht.getSize(); }

REALLY_INLINE int HeapHashtableRC::next(int index) { return ht.next(index); }

REALLY_INLINE Atom HeapHashtableRC::keyAt(int index) { return ht.keyAt(index); }

REALLY_INLINE MMgc::RCObject *HeapHashtableRC::valueAt(int index) {
  return untagAtom(ht.valueAt(index));
}

REALLY_INLINE void HeapHashtableRC::add(Atom name, MMgc::RCObject *value,
...
REALLY_INLINE HeapHashtable *HeapHashtable::create(
    MMgc::GC *gc, int32_t capacity /*= InlineHashtable::kDefaultCapacity*/) {
...
REALLY_INLINE InlineHashtable *HeapHashtable::get_ht() { return &ht; }

REALLY_INLINE void HeapHashtable::reset() { ht.reset(); }

REALLY_INLINE uint32_t HeapHashtable::getSize() const { return ht.getSize(); }

REALLY_INLINE Atom HeapHashtable::keyAt(int index) { return ht.keyAt(index); }
