...
ScriptObject::create(MMgc::GC *gc, VTable *vtable, ScriptObject *delegate) {
...
REALLY_INLINE /*virtual*/ Atom ScriptObject::toAtom() const { return atom(); }

REALLY_INLINE Traits *ScriptObject::traits() const { return vtable->traits; }

REALLY_INLINE AvmCore *ScriptObject::core() const {
  return vtable->traits->core;
}

REALLY_INLINE MMgc::GC *ScriptObject::gc() const { return core()->GetGC(); }
...
REALLY_INLINE Atom ScriptObject::getStringPropertyFromProtoChain(
    Stringp name, ScriptObject *protochain, Traits *origObjTraits) const {
...
REALLY_INLINE bool
ScriptObject::getStringPropertyIsEnumerable(Stringp name) const {
  AvmAssert(name != NULL && name->isInterned());
  return getAtomPropertyIsEnumerable(name->atom());
}

REALLY_INLINE void
ScriptObject::setStringPropertyIsEnumerable(Stringp name, bool enumerable) {
