...
TraitsBindings::calcSlotAddrAndSST(uint32_t i, void *pin, void *&pout) const {
  AvmAssert(m_typesValid);
  AvmAssert(i < slotCount);
...
REALLY_INLINE void TraitsBindings::setSlotInfo(uint32_t i, Traits *t,
                                               SlotStorageType sst,
                                               uint32_t offset) {
  AvmAssert(m_typesValid);
  AvmAssert(i < slotCount);
...
TraitsMetadata::getMetadataPos(PoolObject *&residingPool) const {
...
REALLY_INLINE Traits *Traits::newParameterizedITraits(Stringp name,
                                                      Namespacep ns,
                                                      Traits *param_traits) {
  return _newParameterizedTraits(name, ns, this, param_traits);
}

REALLY_INLINE Traits *Traits::newParameterizedCTraits(Stringp name,
                                                      Namespacep ns) {
...
REALLY_INLINE Namespacep Traits::ns() const { return _ns; }

REALLY_INLINE Stringp Traits::name() const { return _name; }
