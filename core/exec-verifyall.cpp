...
#ifdef VMCFG_VERIFYALL
VerifyallWriter::VerifyallWriter(MethodInfo *m, BaseExecMgr *exec,
                                 CodeWriter *coder)
    : NullWriter(coder), pool(m->pool()), exec(exec) {}

void VerifyallWriter::write(const FrameState *state, const uint8_t *pc,
...
void VerifyallWriter::writeOp1(const FrameState *state, const uint8_t *pc,
...
Atom verifyOnlyInvoker(MethodEnv *, int32_t, Atom *) { return undefinedAtom; }
