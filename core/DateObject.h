...
protected:
  ...
  DateObject(VTable *ivtable, ScriptObject *objectPrototype)
    ...
    date.setTime(
        MathUtils::kNaN); // Date.prototype should be the "Invalid Date"
  }
...
public:
  REALLY_INLINE static DateObject *create(MMgc::GC *gc, VTable *ivtable,
                                          ScriptObject *prototype) {
    return new (gc, MMgc::kExact, ivtable->getExtraSize())
        DateObject(ivtable, prototype);
  }

  REALLY_INLINE static DateObject *create(MMgc::GC *gc, DateClass *type,
                                          const Date &date) {
    return new (gc, MMgc::kExact, type->ivtable()->getExtraSize())
  ...
  double _setUTCMilliseconds(Atom *argv, int argc) {
    return _set(-7, argv, argc);
  }
