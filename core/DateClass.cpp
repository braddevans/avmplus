...
namespace avmplus {
DateClass::DateClass(VTable *cvtable) : ClassClosure(cvtable) {
  ScriptObject *object_prototype = toplevel()->objectClass->prototypePtr();
  setPrototypePtr(
      DateObject::create(core()->GetGC(), ivtable(), object_prototype));
}
...
Atom DateClass::construct(int argc, Atom *argv) {
  ...
  if (argc == 1) {
    double dateAsDouble =
        (core->isString(argv[1]) ? stringToDateDouble(core->string(argv[1]))
    ...
    Date date(num[0], num[1], num[2], num[3], num[4], num[5], num[6], false);
...
double DateClass::UTC(Atom year, Atom month, Atom date, Atom hours,
                      Atom minutes, Atom seconds, Atom ms, Atom * /*argv*/,
                      int /*argc*/) // rest
{
  Date d(AvmCore::number(year), AvmCore::number(month), AvmCore::number(date),
         AvmCore::number(hours), AvmCore::number(minutes),
         AvmCore::number(seconds), AvmCore::number(ms), true);
  return d.getTime();
}
...
inline bool parseDateKeyword(const StringIndexer &s, int offset, int count,
                             int &hour, int &month, double &timeZoneOffset) {
  static const char kDayAndMonthKeywords[] =
      "JanFebMarAprMayJunJulAugSepOctNovDecSunMonTueWedThuFriSatGMTUTC";
  ...
  // string case must match.  Case insensitivity is not necessary for
  // compliance.
  ...
  if (count == 3) {
    for (int x = 0; x < 7 + 12 + 2; x++) {
      ...
      if ((key[0] == subString[0]) && (key[1] == subString[1]) &&
          (key[2] == subString[2])) {
        validKeyWord = true;
        if (x < 12) // its a month
          month = x;
          ...
          timeZoneOffset = 0;
        // else its a day or 'GMT'.  Ignore it: GMT is always followed by + or
        // -, and we identify
        //  it from there.  day must always be specified numerically, name of
        //  day is optional.
...
// Parses a number out of the string and updates
// year/month/day/hour/min/timeZoneOffset as
//  appropriate (based on whatever has already been parsed).  Assumes that s[i]
//  is an integer character.  (broken out into a seperate function from
//  stringToDateDouble() for readability)
static inline bool parseDateNumber(const StringIndexer &s, int sLength, int &i,
                                   wchar &c, wchar prevc,
                                   double &timeZoneOffset, int &year,
  ...
  // Supported examples:  "Mon Jan 1 00:00:00 GMT-0800 1900",  '1/1/1999 13:30
  // PM'
  ...
  // else check for year value
    ...
    if (year >= 0)
      numberIsValid = false;
    else if (c <= ((wchar)' ') || c == ((wchar)',') || c == ((wchar)'/') ||
             i >= sLength)
  ...
  else if (i < sLength && c != ((wchar)',') && c > ((wchar)' ') &&
           c != ((wchar)'-')) {
    numberIsValid = false;
  }
...
double DateClass::stringToDateDouble(Stringp _s) {
  ...
  //  Note:  compliance with ECMAScript 3 only requires that we can parse what
  //  our toString() method produces.  We do support some other simple formats
  //  just to pass Spidermonkey test suite, however (for instance "1/1/1999
  //  13:30 PM"), but we don't handle timezone keywords or instance.
  ...
  while (i < sLength) {
    ...
    if (c <= ((wchar)' ') || c == ((wchar)',') || c == ((wchar)'-')) {
      if (i < sLength) {
        wchar nextc = s[i];
        // if number follows '-' save c in prevc for use in parseDateNumber for
        // detecting GMT offset
        if (c == ((wchar)'-') && ((wchar)'0') <= nextc &&
            nextc <= ((wchar)'9')) {
          prevc = c;
        }
      }
    ...
    // parse numeric value.
      ...
      if (parseDateNumber(s, sLength, i, c, prevc, timeZoneOffset, year, month,
                          day, hour, min, sec) == false)
    ...
    // parse keyword
      ...
      // check keyword substring against known keywords, modify
      // hour/month/timeZoneOffset as appropriate
  ...
  if (timeZoneOffset == -1) { /* no time zone specified, have to use local */
    Date date(year, month, day, hour, min, sec, 0, false);

    return date.getTime();
  } else {
    Date date(year, month, day, hour, min, sec, 0, true);
    return date.getTime() +
           (timeZoneOffset * 60000); // millesecondsPerMinute = 60000
  }
}
