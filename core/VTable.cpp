...
VTable::VTable(Traits *traits, VTable *_base, Toplevel *toplevel)
    : _toplevel(toplevel), init(NULL), base(_base), ivtable(NULL),
      traits(traits),
      createInstanceProc(_base ? _base->createInstanceProc
...
void VTable::resolveSignatures(ScopeChain *scope) {
  ...
  AvmAssert((base == NULL && traits->base == NULL) ||
            (base != NULL && traitsBase == baseTraits));
#endif // DEBUG
  ...
  for (uint32_t i = 0, n = td->methodCount; i < n; i++) {
    ...
    if (traits->pool->isVerbose(VB_traits)) {
      ...
      traits->core->console << "WARNING: empty disp_id " << i << " on "
                            << traits << "\n";
    }
#endif
  }
...
MethodEnv *VTable::makeMethodEnv(MethodInfo *func, ScopeChain *scope) {
  ...
  if (method_id != -1) {
    ...
    if (abcEnv->getMethod(method_id) == NULL) {
      ...
      core->console << "WARNING: tried to re-register global MethodEnv for "
                    << func << "\n";
    }
#endif
  }
...
VTable *VTable::newParameterizedVTable(Traits *param_traits, Stringp fullname) {
  ...
  GCRef<ObjectVectorClass> vectorobj_cls =
      builtinClasses->get_Vector_objectClass();
  ...
  // See comments in VectorClass.h: the lookup of the ctraits under the class
  // name is redundant, because the ctraits are never registered.  That's mostly
  // OK: the call to newParameterizedVTable is guarded by its caller,
  // getTypedVectorClass, and is never invoked more than once on any
  // param_traits for the same factory.

  Stringp classname = core->internString(fullname->appendLatin1("$"));
  Traits *ctraits = core->domainMgr()->findTraitsInPoolByNameAndNS(
      traitsPool, classname, traitsNs);
  Traits *itraits;
  if (!ctraits) {
    // important: base the new ctraits on objVecVTable->traits, *not*
    // this->traits; we want the result to be based off ObjectVectorClass, not
    // VectorClass (otherwise sizeofInstance would be too small and we'd be
    // crashy)
    ctraits =
        objVecVTable->traits->newParameterizedCTraits(classname, traitsNs);
    ctraits->verifyBindings(toplevel);

    itraits = traitsPool->resolveParameterizedType(
        toplevel, this->ivtable->traits, param_traits);
  ...
  ScopeChain *cvtab_cscope =
      vectorobj_cscope->cloneWithNewVTable(core->GetGC(), cvtab, objVecAbcEnv);

  VTable *ivtab = core->newVTable(itraits, objVecIVTable, objVecToplevel);
  ScopeChain *ivtab_iscope =
      vectorobj_iscope->cloneWithNewVTable(core->GetGC(), ivtab, objVecAbcEnv);
  cvtab->ivtable = ivtab;
...
#ifdef AVMPLUS_VERBOSE
PrintWriter &VTable::print(PrintWriter &prw) const { return prw << traits; }
#endif
} // namespace avmplus
