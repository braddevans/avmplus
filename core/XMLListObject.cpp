...
namespace avmplus {
XMLListObject::XMLListObject(XMLListClass *type, Atom tObject,
                             const Multiname *tProperty)
...
#endif
      ,
      m_targetObject(tObject), m_appended(false), m_children(type->gc(), 0) {
  if (tProperty)
    m_targetProperty = *tProperty;
    // unnecessary: the default ctor for HeapMultiname (and Multiname) init to
    // an anyName
...
// arg1 = argv[1]
// argN = argv[argc]
Atom XMLListObject::callProperty(const Multiname *multiname, int argc,
                                 Atom *argv) {
  ...
  if (f == undefinedAtom) {
    ...
    // find a matching property for this method name.  In this case, if our
    // XMLList has one child, we get the child and callproperty on the child
    // object. This allows node elements to be treated as simple strings even if
    // they are XML or XMLList objects.  See 11.2.2.1 in the E4X spec for
    // CallMethod.
...
Atom XMLListObject::getMultinameProperty(const Multiname *m) const {
  ...
  // step 2
  // QNameObject* qname = new (core->GetGC())
  // QNameObject(toplevel->qnameClass(), *m);
  XMLListObject *l = XMLListObject::create(
      core->GetGC(), toplevel->xmlListClass(), this->atom(), m);
...
void XMLListObject::setUintProperty(uint32_t i, Atom V) {
  ...
  if (i >= _length()) {
    ...
    if (m_targetProperty.isAttr()) {
      ...
      if (AvmCore::isXMLList(attributesExist) &&
          (AvmCore::atomToXMLList(attributesExist)->_length() > 0))
        return;
      ...
      rx->nonChildChanges(core->kattributeAdded,
                          this->m_targetProperty.getName()->atom());
    }
    // if targetProperty is null or targetProperty.localName = "*"
    else if (m_targetProperty.isAnyName() ||
             // Added because of bug 145184 - appendChild with a text node not
             // working right Also handle attribute nodes
             ((AvmCore::isXML(V) &&
               AvmCore::atomToXML(V)->getClass() == E4XNode::kAttribute)) ||
             ((AvmCore::isXML(V) &&
               AvmCore::atomToXML(V)->getClass() == E4XNode::kText))) {
    ...
    if (y->getClass() != E4XNode::kAttribute) {
      ...
      if (y->getParent() != NULL) {
        ...
        if (i > 0) {
          // step 2 c vii 1 ii
          // spec says "y.[[parent]][j] is not the same object as x[i-1]" and
          // I'm assuming that means pointer equality and not "equals"
          // comparison
          while ((j < (parent->_length() - 1)) &&
                 parent->_getAt(j) != _getNodeAt(i - 1)) {
            j++;
          }

          j = j + 1; // slightly different than spec but our insert call does
                     // not have a + 1 on it
        } else {
          j = parent->_length(); // slightly different than spec but our insert
                                 // call does not have a + 1 on it
        }
      ...
      if (AvmCore::isXML(V)) {
        ...
        // ERRATA : 9.2.1.2 step 2(c)(vii)(3) what is V.[[PropertyName]]? s.b.
        // [[TargetProperty]]
  ...
  if (AvmCore::atomToXMLList(V)) {
    ...
    if ((src->_length() == 1) && src->_getNodeAt(0)->getClass() &
                                     (E4XNode::kText | E4XNode::kAttribute)) {
  ...
  // step 2f
      ...
      while (q < parent->numChildren()) {
        if (parent->_getAt(q) == _getNodeAt(i)) {
          ...
          for (uint32_t j = 0; j < c->_length(); j++) {
            c->m_children.set(
                j, AvmCore::genericObjectToAtom(parent->_getAt(q + j)));
          }
    ...
    XMLObject *target =
        (notify)
            ? XMLObject::create(core->GetGC(), toplevel->xmlClass(), parent)
            : 0;
      ...
      if (notify && (parent == node->getParent())) {
        ...
        if (i2 == 0) {
          ...
          if (node != prior->getNode())
            target->childChanges(core->knodeChanged, obj->atom(),
                                 prior->getNode());
        } else {
  ...
  // step 2g
  // !!@ always going into this step supports how Rhino acts with setting
  // XMLList props
  // var y = new XMLList("<alpha>one</alpha><bravo>two</bravo>");
  // y[0] = "five";
  // print(y);
  // <alpha>one</alpha> which is an element node is converted to a text node
  // with "five" as it's label
  // !!@ but in this case, the node's contents are replaced and not the node
  // itself... o = <o><i id="1">A</i><i id="2">B</i><i id="3">C</i><i
  // id="4">D</i></o>; o.i[1] = "BB"; print(o);
  else if (AvmCore::isXML(V) ||
           xi->getClass() &
               (E4XNode::kText | E4XNode::kCDATA | E4XNode::kComment |
    ...
    if (parent) {
      ...
      while (q < parent->numChildren()) {
        if (parent->_getAt(q) == _getNodeAt(i)) {
          ...
          XMLObject *xo = XMLObject::create(core->GetGC(), toplevel->xmlClass(),
                                            parent->_getAt(q));
          V = xo->atom();
    ...
    // From E4X errrata
    // 9.2.1.2 step 2(g)(iii) erratum: _V_ may not be of type XML, but all
    // index-named
    //      properties _x[i]_ in an XMLList _x_ must be of type XML, according
    //      to 9.2.1.1 Overview and other places in the spec.
  ...
  if (index >= _length()) {
    ...
    if (px != NULL) {
      if (xi->getClass() == E4XNode::kAttribute) {
        ...
        // let q be the property of parent, where parent[q] is the same object
        // as x[i].
        int q = xi->childIndex();
        ...
        if (XMLObject::notifyNeeded(px->getNode()) &&
            x->getClass() == E4XNode::kElement) {
          AvmCore *core = this->core();
    ...
    // The problem is with the notifications for removal being sent in
    // childChanges() before the call to removeAt() and the event handler for
    // the notifications deleting the children. But since changing the order
    // might break exisiting content, placing a range check for the index before
    // the call to removeAt()
  ...
  if (m_appended && numChildren() > 0) {
    ...
    if (v->getParent()) {
      ...
      if (target && target->getNode() != v->getParent()) {
        ...
        XMLObject *p = XMLObject::create(core->GetGC(), toplevel()->xmlClass(),
                                         v->getParent());
...
XMLListObject *XMLListObject::_deepCopy() const {
  ...
  XMLListObject *l =
      XMLListObject::create(core->GetGC(), toplevel()->xmlListClass(),
                            m_targetObject, m_targetProperty);
...
Atom XMLListObject::_equals(Atom V) const {
  ...
  if (AvmCore::isXMLList(V)) {
    ...
    for (uint32_t i = 0; i < _length(); i++) {
      ...
      if ((a1 != a2) &&
          core->equals(_getAt(i)->atom(), v->_getAt(i)->atom()) == falseAtom)
        return falseAtom;
    }
...
Atom XMLListObject::_resolveValue() {
  ...
  fixTargetObject();
  if (AvmCore::isNull(m_targetObject) || (m_targetProperty.isAttr()) ||
  ...
  if (AvmCore::isXML(base)) {
    target = AvmCore::atomToXMLList(
        AvmCore::atomToXMLObject(base)->getMultinameProperty(m_targetProperty));
  } else if (AvmCore::isXMLList(base)) {
    target = AvmCore::atomToXMLList(
        AvmCore::atomToXMLList(base)->getMultinameProperty(m_targetProperty));
  } else {
  ...
  if (!target->_length()) {
    if (AvmCore::isXMLList(base) &&
        AvmCore::atomToXMLList(base)->_length() > 1) {
      toplevel()->throwTypeError(kXMLAssigmentOneItemLists);
      return nullObjectAtom;
    }

    if (AvmCore::isXML(base)) {
      AvmCore::atomToXMLObject(base)->setMultinameProperty(
          m_targetProperty, core->kEmptyString->atom());
      return AvmCore::atomToXMLObject(base)->getMultinameProperty(
          m_targetProperty);
    } else if (AvmCore::isXMLList(base)) {
      AvmCore::atomToXMLList(base)->setMultinameProperty(
          m_targetProperty, core->kEmptyString->atom());
      return AvmCore::atomToXMLList(base)->getMultinameProperty(
          m_targetProperty);
    } else {
      AvmAssert(0); // b should be one or the other at this point since target
                    // is non-null
    }
  }
...
XMLObject *XMLListObject::_getAt(uint32_t i) const {
  ...
  if (!obj) {
    obj = XMLObject::create(core()->GetGC(), toplevel()->xmlClass(),
                            (E4XNode *)AvmCore::atomToGenericObject(a));
...
void XMLListObject::__toXMLString(PrintWriter &output, Atom,
                                  int /*indentLevel*/) {
  AvmCore *core = this->core();
...
XMLListObject *XMLListObject::AS3_child(Atom propertyName) {
  ...
  XMLListObject *m = XMLListObject::create(
...
XMLListObject *XMLListObject::AS3_copy() { return _deepCopy(); }
...
uint32_t XMLListObject::AS3_length() const { return _length(); }
...
XMLListObject *XMLListObject::AS3_normalize() {
  ...
  while (i < _length()) {
    ...
    if (xn->getClass() == E4XNode::kElement) {
      ...
      while (((i + 1) < _length()) && ((_getNodeAt(i + 1)->getClass() &
                                        (E4XNode::kText | E4XNode::kCDATA)))) {
        xn->setValue(
            core->concatStrings(xn->getValue(), _getNodeAt(i + 1)->getValue()));
...
XMLListObject *
XMLListObject::AS3_processingInstructions(Atom name) // name defaults to '*'
{
  AvmCore *core = this->core();
  XMLListObject *m = XMLListObject::create(
      core->GetGC(), toplevel()->xmlListClass(), this->atom());
...
bool XMLListObject::XMLList_AS3_propertyIsEnumerable(
...
Stringp XMLListObject::toString() {
  ...
  if (hasSimpleContent()) {
    ...
    for (uint32_t i = 0; i < _length(); i++) {
      E4XNode *xn = _getNodeAt(i);
      if ((xn->getClass() != E4XNode::kComment) &&
          (xn->getClass() != E4XNode::kProcessingInstruction)) {
        XMLObject *xm = _getAt(i);
...
Stringp XMLListObject::AS3_toString() { return toString(); }
...
XMLObject *XMLListObject::AS3_addNamespace(Atom _namespace) {
  if (_length() == 1) {
    ...
    // throw type error
    toplevel()->throwTypeError(kXMLOnlyWorksWithOneItemLists,
                               core()->toErrorString("addNamespace"));
    return 0;
  }
}
...
XMLObject *XMLListObject::AS3_appendChild(Atom child) {
  if (_length() == 1) {
    ...
    // throw type error
    toplevel()->throwTypeError(kXMLOnlyWorksWithOneItemLists,
                               core()->toErrorString("appendChild"));
    return 0;
  }
}
...
ArrayObject *XMLListObject::AS3_inScopeNamespaces() {
  if (_length() == 1) {
    ...
    // throw type error
    toplevel()->throwTypeError(kXMLOnlyWorksWithOneItemLists,
                               core()->toErrorString("inScopeNamespaces"));
    return 0;
  }
}
...
Atom XMLListObject::AS3_insertChildAfter(Atom child1, Atom child2) {
  if (_length() == 1) {
    ...
    // throw type error
    toplevel()->throwTypeError(kXMLOnlyWorksWithOneItemLists,
                               core()->toErrorString("insertChildAfter"));
    return undefinedAtom;
  }
}
...
Atom XMLListObject::AS3_insertChildBefore(Atom child1, Atom child2) {
  if (_length() == 1) {
    ...
    // throw type error
    toplevel()->throwTypeError(kXMLOnlyWorksWithOneItemLists,
                               core()->toErrorString("insertChildBefore"));
    return undefinedAtom;
  }
}
...
ArrayObject *XMLListObject::AS3_namespaceDeclarations() {
  if (_length() == 1) {
    ...
    // throw type error
    toplevel()->throwTypeError(kXMLOnlyWorksWithOneItemLists,
                               core()->toErrorString("namespaceDeclarations"));
    return 0;
  }
}
...
XMLObject *XMLListObject::AS3_prependChild(Atom value) {
  if (_length() == 1) {
    ...
    // throw type error
    toplevel()->throwTypeError(kXMLOnlyWorksWithOneItemLists,
                               core()->toErrorString("prependChild"));
    return 0;
  }
}
...
XMLObject *XMLListObject::AS3_removeNamespace(Atom _namespace) {
  if (_length() == 1) {
    ...
    // throw type error
    toplevel()->throwTypeError(kXMLOnlyWorksWithOneItemLists,
                               core()->toErrorString("removeNamespace"));
    return 0;
  }
}
...
XMLObject *XMLListObject::AS3_setChildren(Atom value) {
  if (_length() == 1) {
    ...
    // throw type error
    toplevel()->throwTypeError(kXMLOnlyWorksWithOneItemLists,
                               core()->toErrorString("setChildren"));
    return 0;
  }
}
...
void XMLListObject::AS3_setLocalName(Atom name) {
  if (_length() == 1) {
    ...
    // throw type error
    toplevel()->throwTypeError(kXMLOnlyWorksWithOneItemLists,
                               core()->toErrorString("setLocalName"));
  }
}
...
void XMLListObject::AS3_setNamespace(Atom ns) {
  if (_length() == 1) {
    ...
    // throw type error
    toplevel()->throwTypeError(kXMLOnlyWorksWithOneItemLists,
                               core()->toErrorString("setNamespace"));
  }
}
...
XMLListObject *XMLListObject::filter(Atom propertyName, Atom value) {
  ...
  XMLListObject *l =
      XMLListObject::create(core()->gc, toplevel()->xmlListClass());
  for (uint32_t i = 0; i < _length(); i++) {
    XMLObject *xm = _getAt(i);
...
ScriptObject *XMLListObject::getNotification() { return 0; }
...
uint64_t XMLListObject::bytesUsed() const {
  if (getSampler()->getSamplerType() == Sampler::SAMPLER_TYPE_AVMPLUS) {
    ...
    // For newer samplers, we report the bytes used only for this XMLListObject,
    // not for the underlying E4XNodes. We will report the memory used for
    // E4XNodes individually, eliminating problems with reporting the same
    // memory more than once as described above.
