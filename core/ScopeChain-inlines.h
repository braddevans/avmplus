...
ScopeTypeChain::createEmpty(MMgc::GC *gc, Traits *traits) {
  return create(gc, traits, NULL, NULL, NULL, NULL);
}

REALLY_INLINE Traits *ScopeTypeChain::traits() const { return _traits; }
...
// elimination
REALLY_INLINE VTable *ScopeChain::vtable() const { return _vtable; }
#endif

REALLY_INLINE AbcEnv *ScopeChain::abcEnv() const { return _abcEnv; }

REALLY_INLINE const ScopeTypeChain *ScopeChain::scopeTraits() const {
  return _scopeTraits;
}

REALLY_INLINE int32_t ScopeChain::getSize() const { return _scopeTraits->size; }
