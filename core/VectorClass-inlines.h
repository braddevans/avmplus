...
template <class T> REALLY_INLINE T TypedVectorConstants<T>::nullValue() {
  return 0;
}
...
template <class T> REALLY_INLINE T TypedVectorConstants<T>::undefinedValue() {
  return 0;
}
...
REALLY_INLINE void VectorBaseObject::atomToValueKnown(Atom atom,
  ...
  AvmAssert(m_vecClass != NULL);
  AvmAssert(atom ==
            avmplus::coerce(toplevel(), atom, m_vecClass->getTypeTraits()));
  value = (OpaqueAtom)atom;
}
...
TypedVectorObject<TLIST>::getUintPropertyFast(uint32_t index) const {
  return m_list.get(index);
}

template <class TLIST>
REALLY_INLINE void
TypedVectorObject<TLIST>::setUintPropertyFast(uint32_t index,
...
template <class TLIST> REALLY_INLINE void TypedVectorObject<TLIST>::_reverse() {
  m_list.reverse();
}

template <class TLIST>
REALLY_INLINE bool
TypedVectorObject<TLIST>::hasUintProperty(uint32_t index) const {
  return _hasUintProperty(index);
}

template <class TLIST>
REALLY_INLINE Atom
TypedVectorObject<TLIST>::getUintProperty(uint32_t index) const {
  return _getUintProperty(index);
}

template <class TLIST>
REALLY_INLINE void TypedVectorObject<TLIST>::setUintProperty(uint32_t index,
                                                             Atom value) {
...
template <class TLIST>
REALLY_INLINE void
TypedVectorObject<TLIST>::checkReadIndex_u(uint32_t index) const {
...
TypedVectorObject<TLIST>::checkReadIndex_i(int32_t index) const {
...
template <class TLIST>
REALLY_INLINE void
TypedVectorObject<TLIST>::checkWriteIndex_u(uint32_t index) const {
...
template <class TLIST>
REALLY_INLINE uint32_t
TypedVectorObject<TLIST>::checkWriteIndex_i(int32_t index) const {
  // If we are 'fixed', we can't write past the end.
  // If we are not 'fixed', legal to write exactly 1 past the end, growing as
  // needed. Common case is we're in bounds, so fast-path that.  This matters.
  uint32_t const length = m_list.length();
