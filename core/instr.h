 ...
 */
template <class E> Atom op_call(E env, Atom func, int argc, Atom *atomv);
 ...
 * values of "ctor"
 */
template <class E> Atom op_construct(E env, Atom ctor, int argc, Atom *atomv);
 ...
 */
template <class E>
Atom callprop_b(E env, Atom base, const Multiname *name, int argc, Atom *atomv,
                VTable *vtable, Binding b);
 ...
 */
template <class E> ScriptObject *toPrototype(Atom obj);
 ...
 */
template <class E> Atom op_applytype(E env, Atom factory, int argc, Atom *args);

/**
 * implementaion of OP_newarray for creating array literals
 */
template <class E> ArrayObject *newarray(E caller_env, int argc, Atom *ap);
...
template <class E> Atom coerce(E env, Atom atom, Traits *expected);
...
/**
 * generic coerceobj accepting any value and coercing to one of the
 * ScriptObject* types.  If this method returns, caller can safely use
 * (ScriptObject*)atomPtr(atom). (note: result can still be null).
 */
