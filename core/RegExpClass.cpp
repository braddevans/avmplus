...
namespace avmplus {
RegExpClass::RegExpClass(VTable *cvtable) : ClassClosure(cvtable) {
  ...
  setPrototypePtr(RegExpObject::create(core->GetGC(), cvtable->ivtable,
                                       object_prototype, pattern,
                                       core->kEmptyString));
}
...
Atom RegExpClass::call(int argc, Atom *argv) {
  ...
  if (argc > 0) {
    ...
    if (AvmCore::istype(argv[1], traits()->itraits) &&
        flagsAtom == undefinedAtom) {
      return argv[1];
    }
  }
...
Atom RegExpClass::construct(int argc, Atom *argv) {
  ...
  if (AvmCore::istype(patternAtom, traits()->itraits)) {
    ...
    RegExpObject *regExpObject =
        (RegExpObject *)AvmCore::atomToScriptObject(patternAtom);
    return RegExpObject::create(core->GetGC(), this, regExpObject)->atom();
  } else {
    if (patternAtom != undefinedAtom) {
      pattern = core->string(argv[1]);
    } else {
      // cn:  disable this, breaking ecma3 tests.   was: todo look into this.
      // it's what SpiderMonkey does.
  ...
  RegExpObject *inst = RegExpObject::create(
      core->GetGC(), this->ivtable(), this->prototypePtr(), pattern, options);
  return inst->atom();
}
} // namespace avmplus
