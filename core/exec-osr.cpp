...
TODO
- (done) factor out patch to look up FrameState and CodegenLabel by byte* pc,
not int pc
...
From testing
- Why is osr repo slower than tr-tip with osr not in use?  See
sunspider/math-cordic.as,math-partial-sums.as,math-spectral-norm.as,s3d-raytrace.as
- Not sure why osr repo seems faster with -osr not in use on regexp-dna.as?
- With -osr=50 jsbench/typed/FFT.as,LUFact.as,SOR.as are 5-15% slower than
without -osr parameter set
- With -osr=50 jsbench/SparseMatmult.as is 20% faster than without -osr
parameter set
...
#endif
  if (VMPI_sscanf(str, "%d", &threshold) != 1 || threshold < 0)
    return -1;
#if defined(_MSC_VER)
...
OSR::OSR(const uint8_t *osr_pc, Atom *interp_frame)
    : osr_pc(osr_pc), interp_frame(interp_frame), jit_frame_state(NULL) {}

void OSR::setFrameState(FrameState *fs) { jit_frame_state = fs; }

const uint8_t *OSR::osrPc() const { return osr_pc; }
...
// the method belongs, which will thus remain invariant. We can therefore only
// OSR non-builtin methods.
bool OSR::isSupported(const AbcEnv *abc_env, const MethodInfo *m,
                      MethodSignaturep ms) {
  ...
  return (m->osrEnabled() &&     // OSR allowed by policy (global or ExecPolicy
                                 // attribute)
          !m->hasExceptions() && // method does not have a try block
          !m->hasFailedJit() && // no previous attempt to compile the method has
                                // failed
          !CodegenLIR::jitWillFail(
              ms) && // fast-fail predictor says JIT success is possible
          !m->pool()->isBuiltin && // the method is not a builtin (ABC baked
                                   // into application)
          abc_env->codeContext()
              ->bugCompatibility()
              ->bugzilla539094); // bug compatibility permits OSR
}
 ...
 */
bool OSR::execute(MethodEnv *env, Atom *interp_frame, MethodSignaturep ms,
                  const uint8_t *osr_pc, Atom *result) {
  BaseExecMgr *exec = BaseExecMgr::exec(env);
  OSR osr(osr_pc, interp_frame);

#ifdef AVMPLUS_VERBOSE
  if (env->method->pool()->isVerbose(VB_execpolicy)) {
    env->core()->console << "execpolicy jit hot-loop " << env->method
                         << " osr_pc=" << int(osr_pc - ms->abc_code_start())
                         << "\n";
  }
#endif
...
void OSR::adjustFrame(MethodFrame *jitMethodFrame, CallStackNode *callStack,
  ...
  if (env->method->pool()->isVerbose(VB_interp)) {
    env->core()->console << "osr-adjust_frame " << env->method->method_id()
                         << " " << env->method << " scopeTop=" << scopeTop
                         << " stackTop=" << stackTop << "\n";
  }
#endif

  // Patch the JIT frame local variable slots and the scope slots in use to
  // match the interpreter state.
  for (int i = 0; i < scopeTop; i++)
    unboxSlot(frameState, env, interpFramePointer, jitFramePointer,
              jitFrameTags, i);
...
void OSR::unboxSlot(FrameState *frameState, MethodEnv *env,
                    Atom *interpFramePointer, FramePtr jitFramePointer,
  ...
  AvmAssert(isValidAtom(value, atom));
  Atom *addr =
      (Atom *)((char *)jitFramePointer + (index << VARSHIFT(env->method)));
  Traits *type = value.traits;
...
REALLY_INLINE bool OSR::countInvoke(MethodEnv *env) {
  ...
  if (m->isInterpreted()) {
    ...
    BaseExecMgr *exec = BaseExecMgr::exec(env);
    exec->verifyJit(m, m->getMethodSignature(), env->toplevel(), env->abcEnv(),
                    NULL);
    if (m->hasFailedJit())
      return false;
  }
...
#ifdef DEBUG
// Verify that currentBugCompatibility() is as OSR expected when method is
// called.
void OSR::checkBugCompatibility(MethodEnv *env) {
  ...
  if (!env->method->pool()->isBuiltin) {
    const BugCompatibility *abcBugCompatibility =
        env->abcEnv()->codeContext()->bugCompatibility();
    const BugCompatibility *dynamicBugCompatibility =
        env->core()->currentBugCompatibility();
    // Verify that currentBugCompatibility() agrees with what the JIT would have
    // assumed.
