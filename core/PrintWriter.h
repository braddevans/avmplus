...
private:
  static void *operator new(
      size_t size,
      MMgc::GC *gc); // private and not implemented, in order to catch errors

public:
  PrintWriter(AvmCore *core) { m_core = core; }
  PrintWriter(AvmCore *core, GCOutputStream *stream) {
    m_core = core;
    m_stream.set(stream);
  }
  PrintWriter(AvmCore *core, NonGCOutputStream *stream) {
    m_core = core;
    m_stream.set(stream);
  }
  ...
  void formatP(const char *format, Stringp arg1 = 0, Stringp arg2 = 0,
               Stringp arg3 = 0);
