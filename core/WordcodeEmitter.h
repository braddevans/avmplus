...
private:
  // 'backpatches' represent target addresses of forward jumps in the original
  // code, along with locations in the translated code that must be patched when
  // the target address in the new code is known.  There can be multiple
  // backpatch structures per target, one for each location that must be
  // patched.  The backpatches are sorted in address order: lowest address
  // first.  Once a backpatch has been consumed it can be deleted; when we're
  // done, the list of backpatches should be empty or there's an error.
  // Backpatch objects are managed by new/delete.
  //
  ...
  //
  // 'exception_fixes' represent addresses in the original code, along with
  // locations to be updated with corresponding offsets in the translated code.
  // The exception fixes are sorted in address order: lowest address first. Once
  // an exception fix has been consumed it can be deleted; when we're done, the
  // list of exception fixes should be empty.  Exception fix objects are managed
  // by new/delete.
  //
  // 'buffers' are sorted in reverse creation order: current buffer segment
  // first. The list of buffers is merged into a collectable object at the end
  // of translation, at which point buffers can be freed.  Buffer objects are
  // managed by new/delete.

  struct backpatch_info {
    const uint8_t *target_pc; // the instruction in the old code that is the
                              // target of a forward control transfer
    uintptr_t *patch_loc;   // location in the new code into which to write the
                            // new offset
  ...
  buffer_info *spare_buffer;   // may be populated during peephole optimization;
                               // reused by refill
  ...
  void refill();
  void emitRelativeOffset(uintptr_t base_offset, const uint8_t *pc,
                          intptr_t offset);
  ...
  // The structures are laid out so as to improve packing and conserve space.
  // The included initialization code below knows the order of fields.

  struct peep_state_t {
    uint8_t numTransitions; // Number of consecutive in the transitions[] array
                            // starting at transitionPtr
    uint8_t failShift;      // Initial tokens to discard on a failure transition
    uint16_t transitionPtr; // Location in transitions[] for our transitions,
                            // sorted in increasing token order
    uint16_t guardAndAction; // 0 if this is not a final state, otherwise an
                             // identifier for a case in 'commit()'
    uint16_t
        fail; // 0 if there is no failure transition, otherwise a state number
  };
  ...
  static const peep_transition_t
      transitions[]; // Compact transition representation
  ...
  uintptr_t *I[10]; // longest window 10 instructions, not a problem now,
                    // generator can generate constant later
  ...
  bool replace(uint32_t old_instr, uint32_t new_words,
               bool jump_has_been_translated = false);
  void undoRelativeOffsetInJump();
  void shiftBuffers(uint32_t shift);

  bool isJumpInstruction(uintptr_t opcode) { return wopAttrs[opcode].jumps; }
