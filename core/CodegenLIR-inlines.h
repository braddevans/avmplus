...
REALLY_INLINE LIns *CodegenLIR::loadIns(LOpcode op, int32_t disp, LIns *base,
                                        AccSet accSet, LoadQual loadQual) {
  return lirout->insLoad(op, base, disp, accSet, loadQual);
}

REALLY_INLINE LIns *CodegenLIR::storeIns(LOpcode op, LIns *val, int32_t disp,
                                         LIns *base, AccSet accSet) {
  return lirout->insStore(op, val, base, disp, accSet);
}

REALLY_INLINE LIns *CodegenLIR::Ins(LOpcode op) { return lirout->ins0(op); }
