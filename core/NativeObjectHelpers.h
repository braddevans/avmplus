...
// CN:  ES3 compliance requires uint32_t array and string indicies.  The
// arguments to several
//  array methods are integral values held in a double (to allow for negative
//  offsets from the array length).  To deal with this, ClampIndex replaces
//  ClampA
...
//
//  do this:
//  uint32_t start = ClampArrayIndex( AvmCore::toInteger(argv[0]),
//  a->getLength() ); uint32_t end   = ClampArrayIndex(
//  AvmCore::toInteger(argv[1]), a->getLength() ); if (end < start)
//    end == start;
inline uint32_t ClampIndex(double intValue, uint32_t length) {
  ...
  if (intValue < 0.0) {
    ...
    result = length;
  else if (intValue != intValue) // lookout for NaN.  It converts to a 0 int
                                 // value on Win, but as 0xffffffff on Mac
    result = 0;
  else
