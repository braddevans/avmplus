...
protected:
  XMLListObject(XMLListClass *type, Atom targetObject,
                const Multiname *targetProperty);

public:
  REALLY_INLINE static XMLListObject *
  create(MMgc::GC *gc, XMLListClass *cls, Atom targetObject = nullObjectAtom,
         const Multiname *targetProperty = 0) {
    return new (gc, MMgc::kExact)
        XMLListObject(cls, targetObject, targetProperty);
  }
...
public:
  ...
  void __toXMLString(PrintWriter &output, Atom AncestorNamespace,
                     int indentLevel = 0);
  ...
  inline bool propertyIsEnumerable(Atom P) {
    return XMLList_AS3_propertyIsEnumerable(P);
  }
...
private:
  ...
  void setTargetObject(Atom a) const {
    m_targetObject.set(MMgc::GC::GetGC(this), this, a);
  }
...
private:
#ifdef DEBUGGER
  bool m_created;
  mutable bool m_flushBeforeConstructionComplete;
#endif
  ...
  // An array of XMLObjects,or E4XNodes; mutable because E4XNodes may be
  // converted to XMLObjects
