...
extern "C" void deoptFrameUponReturnWithInt32(uint8_t *fp, int32_t result) {
  MethodEnv *env = (MethodEnv *)framelib::getPointerArgument0(fp);
  env->method->_armed_deoptimizers->deoptFrameUponReturnWithResult(
      fp, env, (void *)&result);
}

extern "C" void deoptFrameUponReturnWithInt64(uint8_t *fp, int64_t result) {
  MethodEnv *env = (MethodEnv *)framelib::getPointerArgument0(fp);
  env->method->_armed_deoptimizers->deoptFrameUponReturnWithResult(
      fp, env, (void *)&result);
}

extern "C" void deoptFrameUponReturnWithDouble(uint8_t *fp, double result) {
  MethodEnv *env = (MethodEnv *)framelib::getPointerArgument0(fp);
  env->method->_armed_deoptimizers->deoptFrameUponReturnWithResult(
      fp, env, (void *)&result);
}
...
// Return a trampoline to capture the frame pointer and return value from a
// returning function and transfer into the appropriate static bridge above.  We
// overwrite the return address of the frame we wish to "hook" in order to take
// control when it returns. We maintain 16-byte stack alignment, which is
...
// We arrange for activations of the method to be deoptimized as the stack
// unwinds. Here, we do not actually revert the state of the method to
// interpreted, but only arrange for the deoptimization of all pending
// activations.
// TODO:  When unwinding for exceptions, we may skip over an armed frame, and
// should then re-arm a frame that is still active.  Otherwise, the
// deoptimize-and-rearm chain will be broken, and we will not deoptimize all
// activations.
...
void Deoptimizer::armForLazyDeoptimization(DeoptContext *ctx,
                                           MethodFrame *victim) {
  ...
  if (deopt_trace) {
    ...
    core->console << "$$$ deopt armed " << hexAddr((uintptr_t)this) << " @ "
                  << hexAddr((uintptr_t)victim) << "\n";
  }
#endif
...
void Deoptimizer::deoptFrameUponReturnWithResult(DeoptContext *ctx, uint8_t *fp,
                                                 MethodEnv *env,
                                                 void *resultPtr) {
  ...
  // The frame to deoptimize must be an activation of the method to which we
  // belong.
  ...
  Atom returnVal =
      nativeLocalToAtom(core, resultPtr, ctx->safepointReturnValueSST());
  ...
  // Search for the previous activation of the method, and arm for lazy
  // deoptimization.
  ...
  // Return from lazily-deoptimized frame with the unboxed value from the
  // interpreter.
...
// The frame must be an activation of the AS3 method to which this member
// belongs.
...
void Deoptimizer::enterInterpreter(DeoptContext *ctx, MethodFrame *frame,
                                   int32_t vpc) {
  MethodEnv *env = frame->env();
  AvmCore *core = env->core();
...
#ifdef DEOPT_TRACE
  if (deopt_trace) {
    core->console << "$$$ deopt interp " << hexAddr((uintptr_t)this) << " @ "
                  << hexAddr((uintptr_t)frame) << "\n";
  }
#endif
...
extern void set_slot_from_atom(Atom atom, Traits *traits, AnyVal *slotPtr,
                               uint8_t *tagPtr);
#endif

// Attempt to handle an exception in the method activation to which 'ef' and
// 'handlerFrame' belong.  If no handler exists, we re-throw.  We do not simply
// search up the ExceptionFrame chain, as each frame may follow a different
// protocol (e.g., interpreter vs. JIT-ed code). See beginCatch() in jit-calls.h

void Deoptimizer::handleException(DeoptContext *ctx, AvmCore *core,
                                  ExceptionFrame *ef,
                                  MethodFrame *handlerFrame) {
  ...
  uint8_t *nativePc =
      framelib::getReturnAddress(ctx->calleeFrameBase(frameBase));
  ctx->setSafepointFromNativePc(nativePc);
  int32_t exnVpc = ctx->safepointVpc();
#else
  // Extract the virtual ABC pc from the frame of the handling method
  // activation.
  int32_t exnVpc = ctx->saveEip(frameBase);
#endif

  // Find handler for throw at location exnPc.  Rethrows if no applicable
  // handler exists.
  ExceptionHandler *handler =
      core->findExceptionHandler(methodInfo, exnVpc, exception);

#ifdef VMCFG_DEOPT_DIRECT_EXN
  //### This depends on CodegenLIR!
  // Re-establish exception-handling context for possible exceptions raised in
  // handler.
  ...
  set_slot_from_atom(exception->atom, handler->traits,
                     ((AnyVal *)((uint8_t *)vars + stack_base * 8)),
                     (tags + stack_base));

  // Reset the stack and the callee-saved registers using explicitly-saved
  // state. A proper unwinder that understands callee-saved registers would be
  // more efficient.
  framelib::resumeFrameAtLocation((uint8_t *)&ef->jmpbuf,
                                  handler->target_address);
#else
  ...
  if (deopt_trace) {
    core->console << "$$$ deopt interp " << hexAddr((uintptr_t)this) << " @ "
                  << hexAddr((uintptr_t)handlerFrame) << "\n";
  }
#endif
  // Unlink the intervening frames, including the victim.
  ...
  Atom result = ctx->interpretFromThrowSafepoint(
      frameBase, env, handler->target, exception->atom);
...
void Deoptimizer::showFrames(AvmCore *core) {
  ...
  int limit = 5;
  for (MethodFrame *frame = core->methodFrame(); frame != NULL;
       frame = frame->next) {
    if (limit-- < 0) {
      core->console << "...";
      break;
...
void Deoptimizer::showArmed(AvmCore *core) {
  ...
  int limit = 5;
  for (Deoptimizer *deopt = methodInfo->_armed_deoptimizers; deopt != NULL;
       deopt = deopt->nextArmed) {
    if (limit-- < 0) {
      core->console << "...";
      break;
    }
    core->console << " {" << hexAddr((uintptr_t)deopt) << " @ "
                  << hexAddr((uintptr_t)deopt->armedFrame) << "}";
  }
  core->console << "\n";
}
...
bool Deoptimizer::deoptAncestor(AvmCore *core, uint32_t k) {
  for (MethodFrame *frame = methodFrame(core); frame != NULL;
       frame = frame->next) {
    MethodEnv *env = frame->env();
    if (!env)
      continue; // MethodFrame is holder for CodeContext, so skip it.
    if (k-- == 0) {
      if (frame->isInterp)
        return true;
...
// Target architecture and backend-dependent library routines for manipulating
// stack frames. Currently aimed at supporting deoptimization in Tamarin, though
// debugging suggests other extensions to the API.
//
// TODO: This stuff doesn't belong in the code generator source files, and
// probably shouldn't be in the nanojit namespace either.  Most likely, we
// should have a separate support library in a directory alongside 'nanojit'.

#if _MSC_VER
#pragma warning(push)
#pragma warning(                                                               \
    disable : 4731) // Don't complain about changing FP inside function.
#endif

// We assume eager saving of the callee-saved registers, i.e., all are spilled
// to consecutive slots at method entry regardless of whether they are actually
// used.
...
uint8_t *getReturnAddress(uint8_t *frame) { return *((uint8_t **)(frame + 4)); }
