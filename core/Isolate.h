...
namespace avmplus {
/*
 * Represents an isolate in various stages of its lifecycle. There's only one
 * Isolate object per isolate.
 */
...
public:
  ...
  // Isolate objects are created by client code (e.g., the shell), before their
  // AvmCores are instantiated. If the isolate is successfully started, the
  // Isolate object will delete itself upon isolate termination. Otherwise, the
  // AS glue object (IsolateObject) destructor will delete Isolate objects that
  // have not been started or have failed.
  Isolate(descriptor_t desc, descriptor_t parentDesc, Aggregate *aggregate);

  static Isolate *newIsolate(descriptor_t desc, descriptor_t parentDesc,
                             Aggregate *aggregate);
  ...
  AvmCore *getAvmCore() const { return m_core; }

  Aggregate *getAggregate() const { return m_aggregate; }
...
protected:
  FixedHeapArray<uint8_t> getByteCode() const { return m_code.values[0]; }

  virtual void stopRunLoop() {
    // This is the default impl for AvmShellIsolate and any other Isolate
    // derived classes that do not have a run loop.
  }
...
protected:
  ...
  class SharedPropertyMap
      : public FixedHeapHashTable<SharedPropertyNamep, ChannelItem *> {
  public:
    SharedPropertyMap();
  ...
  protected:
    ...
    virtual bool KeysEqual(SharedPropertyNamep key1,
                           const SharedPropertyNamep key2) const;
...
public:
  ...
  public:
    ...
    public:
      ...
      WaitRecord *waitListHead() const { return m_state->m_head; }
      void notify();
      bool interrupted;
      bool result;
    ...
    private:
      ...
      public:
        ActiveInterruptibleStateHelper(Isolate *isolate,
...
/* An aggregate is a collection of isolates that have been transitively created
 * from a single isolate (the primordial isolate). In the avm shell build there
 * is only one of these but in the Flash Player there would be one for each
 * Player instance (for each <object/> or <embed/> tag).
 */
...
private:
  ...
  public:
    Globals()
        : m_nextGlobalIsolateId(1), m_idsWrapped(false), m_isolateMap(16) {}
    ...
    void orphanFor(Isolate::descriptor_t giid) {
      ...
      public:
        Orphanize(Isolate::descriptor_t target) : m_TargetID(target) {}

        virtual void each(Isolate::descriptor_t,
                          FixedHeapRef<Isolate> isolate) {
      ...
      SCOPE_LOCK(m_isolateMap.m_lock) { m_isolateMap.ForEach(makeOrphansFor); }
    }

  private:
    Isolate *
    getIsolateForID(Isolate::descriptor_t giid); // cannot be const because
                                                 // m_isolateMap is ill defined;

    vmbase::WaitNotifyMonitor
        m_lock; // protects m_globals and all Aggregate instances (curently).

    Isolate::descriptor_t m_nextGlobalIsolateId;
    bool m_idsWrapped;

    class IsolateMap : public FixedHeapHashTable<Isolate::descriptor_t,
                                                 FixedHeapRef<Isolate>> {
    public:
  ...
  //
  class ActiveIsolateThreadMap
      : public FixedHeapHashTable<Isolate::descriptor_t, vmbase::VMThread *> {
  public:
...
public:
  ...
  bool requestIsolateExit(Isolate::descriptor_t desc,
  ...
  vmbase::SafepointManager *safepointManager() { return &m_safepointMgr; }

  bool inShutdown() const { return m_inShutdown; }
...
private:
  ...
  vmbase::SafepointManager
      m_safepointMgr; // Currently for shared byte array only.
  bool m_inShutdown;
...
private:
  FixedHeapRef<Aggregate>
      m_aggregate; // to keep the safepoint mgr alive during shutdown, etc.
  vmbase::SafepointManager *m_safepointMgr;
  vmbase::SafepointRecord m_spRecord;
};

template <class T> class WorkerDomainObjectBase {
public:
  T *self();
  ObjectVectorObject *listWorkers();
};

template <class T> class WorkerObjectBase {
  T *self();

public:
  WorkerObjectBase();
  //  If "thisIsolate == NULL" then create a new isolate.  Otherwise, use
  //  thisIsolate to construct the worker
