...
/*
    ...
    Arrays are (mostly) dense in actual usage. Thus we now start with the
   assumption that all Arrays are dense until proven otherwise. The heuristics
   are largely based on SpiderMonkey.
    ...
    In dense mode, the Array has a (possibly zero-length) AtomList
   (m_denseArray) containing all its values; the first item in m_denseArray
   corresponds to index m_denseStart. There can be "holes" in m_denseArray;
   these are set to the special value atomNotFound and indicate an entry that
   has never been set.

    All properties that are less than the dense area start, or greater than the
   dense area end, are implicitly considered to be atomNotFound.

    It's perfectly legal (and not unusual) for a dense array to have storage for
   only the "middle" portion of itself; consider a code fragment like
    ...
    (m_denseStart+m_denseArray.length) and m_length, are implicitly considered
   "atomNotFound".

    Arrays are converted to sparse (m_denseArray = 0, all properties in the ht
   area) when any of these conditions are met:
     - the load factor (m_denseUsed / m_denseArray.length()) is less than 0.25,
   and there are more than MIN_SPARSE_INDEX slots total
     - a property is set that is non-numeric (and not "length"); or
     - a hole is filled below m_denseArray.length() (possibly implicitly through
   methods like |reverse| or |splice|).

    We currently never attempt to reconvert a sparse Array to a dense Array,
   except in the degenerate case of every property of the Array being removed
   (in which case it becomes a "dense" but empty Array).

    Note that, as before, we do not preserve enumeration-in-insertion-order
    for either dense or sparse arrays.
*/

// Arrays <= this in length are always dense, regardless of how many slots are
// used
...
/*virtual*/
REALLY_INLINE uint32_t ArrayObject::getLength() const { return m_length; }
...
/*static*/ REALLY_INLINE bool shouldBeSparse(const uint32_t denseLen,
                                             const uint32_t denseUsed) {
  // We don't allow dense indices to be negative when interpreted as a signed
  // int32, so let's constrain denseLen to <= 0x7FFFFFFF to ensure that.
...
Atom ArrayObject::indexToName(uint32_t index) const {
  // This is carefully designed to replicate the logic in
  // ScriptObject::getUintProperty; in particular, we must ensure that we create
  // kIntptrType Atoms for indices less than MAX_INTEGER_MASK (rather than
  // interned Strings)
  return (!(index & MAX_INTEGER_MASK)) ? atomFromIntptrValue_u(index)
...
#ifdef DEBUGGER
      ,
      m_created(false), m_flushBeforeConstructionComplete(false)
#endif
      ,
      m_denseArray(vtable->core()->GetGC(), capacity),
      m_denseStart(traits()->needsHashtable() ? 0 : IS_SEALED), m_denseUsed(0),
      m_lengthIfSimple(0), m_canBeSimple(simple) {
...
ArrayObject::ArrayObject(VTable *vtable, ScriptObject *proto, Atom *argv,
                         int argc, bool simple /*=false*/)
    : ScriptObject(vtable, proto, 0)
#ifdef DEBUGGER
      ,
      m_created(false), m_flushBeforeConstructionComplete(false)
      ...
      m_denseUsed(argc), m_length(argc), m_lengthIfSimple(simple ? argc : 0),
...
ArrayObject::ArrayObject(VTable *vtable, ScriptObject *proto, MethodEnv *env,
                         ADT argDesc, uint32_t argc, va_list_wrapper ap,
                         bool simple /*=false*/)
    : ScriptObject(vtable, proto, 0)
#ifdef DEBUGGER
      ,
...
#endif
      ,
      m_denseArray(vtable->core()->GetGC(), argc), m_length(argc),
      m_lengthIfSimple(simple ? argc : 0), m_canBeSimple(simple) {
  if (m_length > 0)
...
template ArrayObject::ArrayObject(VTable *, ScriptObject *, MethodEnv *,
                                  uint32_t, uint32_t, va_list_wrapper,
                                  bool simple /*=false*/);
template ArrayObject::ArrayObject(VTable *, ScriptObject *, MethodEnv *, char *,
                                  uint32_t, va_list_wrapper,
                                  bool simple /*=false*/);
#endif // VMCFG_AOT
...
Atom ArrayObject::_getDoubleProperty(double d) const {
  ...
  if (double(index) == d)
    ...
    // Number is non-integral, negative, or too large to be a valid index, so
    // intern it.
...
Atom *ArrayObject::getDenseCopy() const {
  ...
  Atom *result =
      (Atom *)gc->Calloc(len, sizeof(Atom), MMgc::GC::kContainsPointers,
                         MMgc::kAOTDenseCopyPartition);
...
uint32_t ArrayObject::getDenseLength() const { return m_denseArray.length(); }
#endif // VMCFG_AOT
...
void ArrayObject::_setUintProperty(uint32_t index, Atom value) {
  ...
  if (denseIdx < denseLen) {
    ...
    AvmAssert(index >= m_denseStart &&
              index < m_denseStart + m_denseArray.length());
    ...
    // Since we've already verified index, we can use replace() rather than
    // set()... it's minor but will show up on microbenchmarks.
    m_denseArray.replace(denseIdx, value);
  } else if (int32_t(index) >= 0 &&
             int32_t(m_denseStart) >=
                 0) // denseIdx >= denseLen and possibly-dense index
  {
    if (index >= m_length) {
  ...
  sparse_or_sealed:
    ...
    // always revert to sparse. (Note that per ES3 spec, 0xffffffff is not a
    // legal array index, so doesn't affect length, but does get stored as a
    // dynamic prop)
    ...
    if (isDense()) {
      ...
      if (oldDenseLength == 0 && oldLength == 0) {
        ...
        if (newLength > m_denseStart) {
          // wmaddox 2015.06.22
          // If we delete the last (non-hole) item from the dense area, we set
          // its length to zero and reset m_denseStart -- see
          // ArrayObject::delDenseUintProperty().  We can thus arrive here
          // attempting to *expand* the dense area, which fills the new slots
          // with nullObjecAtom instead of atomNotFound, and which also breaks
          // the expectation (asserted below) that no sparseness check is
          // needed.  Make sure we are actually shrinking the dense area,
          // otherwise, leave it alone.  This issue was discovered by
          // temporarily disabling the dense fastpath to stress the general case
          // when testing insertAt/removeAt.
        ...
        m_canBeSimple = false;
        m_lengthIfSimple = 0;
        // Check for sparseness before set_length, so giant requests don't
        // trigger OOM
      ...
      // in theory we need to call delUintProperty on every one of these, but in
      // practice, user AS3 code can't override delUintProperty, and no existing
      // VM/Flash/AIR classes subclass ArrayObject, so we can (and should)
      // short-circuit this and just process the items actually present. (this
      // is MUCH faster if the sole item was at index 0xfffffff0...)
      InlineHashtable *ht = this->getTable();
      for (int i = ht->next(0); i != 0; i = ht->next(i)) {
        Atom k = ht->keyAt(i);
        uint32_t index;
  ...
  if (argc > 0) {
    if (isDense()) {
      ...
      if (m_length != curDenseEnd) {
        ...
        if (newDenseLen < holesToAppend ||
            shouldBeSparse(newDenseLen, newDenseUsed)) {
          convertToSparse();
          goto push_sparse;
        }
  ...
  if (argc > 0) {
    if (isDense()) {
      if (m_denseStart > 0) {
        ...
        if (shouldBeSparse(m_denseArray.length() + m_denseStart + argc,
                           m_denseUsed + argc)) {
          convertToSparse();
          goto unshift_sparse;
        }
...
void ArrayObject::registerDependentObjects(IMemorySampler *memSampler) {
  if (!m_created) {
    m_flushBeforeConstructionComplete = true;
    return;
  }

  // Add the dense array as a dependent object. Even if this is not a dense
  // array, we track this in case it becomes one later.
  memSampler->addDependentObject(this, &m_denseArray);

  // Call the base class to also track sparse array hash map.
...
bool ArrayObject::try_concat(ArrayObject *that) {
  ...
  if (this->isDense() && that->isDense() &&
    ...
    // but m_length could, since they could be arbitrary large, so check for
    // overflow.
    uint64_t const totLen = uint64_t(this->m_length) + uint64_t(that->m_length);
    if (totLen != uint32_t(totLen) ||
        shouldBeSparse(uint32_t(totLen),
    ...
    if (extraHoles > 0) {
      this->m_denseArray.insert(this->m_denseArray.length(), atomNotFound,
                                extraHoles);
      this->m_canBeSimple = false;
      this->m_lengthIfSimple = 0;
    }
...
ArrayObject *ArrayObject::try_splice(uint32_t insertPoint, uint32_t insertCount,
                                     uint32_t deleteCount,
                                     const ArrayObject *that,
                                     uint32_t that_skip) {
  verify();

  // OPTIMIZEME, probably other dense cases could be handled too
  if (this->isDense() && that != NULL && that->isDense() &&
      insertPoint >= this->m_denseStart &&
      insertPoint <= this->m_denseStart + this->m_denseArray.length() &&
      insertPoint + deleteCount <=
          this->m_denseStart + this->m_denseArray.length() &&
      that->m_denseStart == 0) {
    ...
    deletedItems->m_denseArray.splice(0, deleteCount, 0, this->m_denseArray,
                                      insertPoint);
    ...
    this->m_denseArray.splice(insertPoint, insertCount, deleteCount,
                              that->m_denseArray, that_skip);
...
bool ArrayObject::try_unshift(ArrayObject *that) {
  ...
  if (this->isDense() && that->isDense() &&
    ...
    uint32_t thatExtraHolesBack =
        that_length - (that->m_denseStart + that->m_denseArray.length());
    uint32_t extraHoles = thisExtraHolesFront + thatExtraHolesBack;
    if (extraHoles > 0) {
      // We only need to check for sparseness if we are inserting holes;
      // inserting non-holes can't make us any "less dense".
      if (shouldBeSparse(this->m_denseArray.length() + extraHoles,
                         this->m_denseUsed + that->m_denseUsed)) {
        convertToSparse();
        verify();
        return false;
    ...
    m_denseArray.insert(0, atomNotFound,
                        that->m_denseArray.length() + extraHoles);
...
void ArrayObject::AS3_insertAt(int32_t index, Atom element) {
  ...
  if (isDense() && insertPoint >= m_denseStart &&
...
Atom ArrayObject::AS3_removeAt(int32_t index) {
  ...
  // This is important, as we rely on setUintProperty() to
  // enforce the restrictions.
  if (isDense() && removePoint >= m_denseStart &&
...
bool SemiSealedArrayObject::deleteAtomProperty(Atom) { return false; }

bool SemiSealedArrayObject::hasAtomProperty(Atom) const { return false; }
...
bool SemiSealedArrayObject::delUintProperty(uint32_t) { return false; }

bool SemiSealedArrayObject::hasUintProperty(uint32_t) const { return false; }
...
ArrayObject *SemiSealedArrayObject::try_splice(uint32_t insertPoint,
                                               uint32_t insertCount,
                                               uint32_t deleteCount,
                                               const ArrayObject *that,
                                               uint32_t that_skip) {
  if (deleteCount > 0)
    toplevel()->throwReferenceError(kReadSealedError,
                                    core()->toErrorString(insertPoint));
  return ArrayObject::try_splice(insertPoint, insertCount, deleteCount, that,
                                 that_skip);
}

/*static*/ ScriptObject *FASTCALL
SemiSealedArrayObject::createInstanceProc(avmplus::ClassClosure *cls) {
  return new (cls->gc(), MMgc::kExact, cls->getExtraSize())
      SemiSealedArrayObject(cls->ivtable(), cls->prototypePtr());
}
} // namespace avmplus
