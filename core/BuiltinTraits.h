...
namespace avmplus {
// Note: we rely on this being <= 32 entries. None of the enumeration values are
// magic, so we just keep them in alphabetical order for simplicity.
enum BuiltinType {
  BUILTIN_any, // this is the "*" type in AS3, corresponds to a NULL Traits in
               // C++ (not the "null" value in AS3")
  BUILTIN_array,
  BUILTIN_boolean,
  ...
  BUILTIN_none, // "none of the above" (ie it's not any of the rest of this
                // enum)
