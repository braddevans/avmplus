...
MethodInfo::MethodInfo(InitMethodStub, Traits *declTraits)
    : _msref(declTraits->pool->core->GetGC()->emptyWeakRef),
      _declarer(declTraits), _activation(NULL), _pool(declTraits->pool),
      _abc_info_pos(NULL), _method_id(-1), _hasMethodBody(1), _isResolved(1)
#ifdef VMCFG_FLOAT
      ,
      _has128bitLocals(0)
#endif // VMCFG_FLOAT
...
MethodInfo::MethodInfo(InitMethodStub, Traits *declTraits,
                       ...
                       const AvmThunkNativeHandler &handler, int32_t method_id)
    : MethodInfoProcHolder(),
      _msref(declTraits->pool->core->GetGC()->emptyWeakRef),
      _declarer(declTraits), _activation(NULL), _pool(declTraits->pool),
      _abc_info_pos(NULL), _method_id(method_id), _hasMethodBody(1),
      _isResolved(1)
#ifdef VMCFG_FLOAT
      ,
      _has128bitLocals(0)
 ...
 */
MethodInfo::MethodInfo(int32_t method_id, PoolObject *pool,
                       const uint8_t *abc_info_pos, uint8_t abcFlags,
                       const NativeMethodInfo *native_info)
    : _msref(pool->core->GetGC()->emptyWeakRef), _declarer(NULL),
      _activation(NULL), _pool(pool), _abc_info_pos(abc_info_pos),
...
Traits *MethodInfo::activationTraits() const { return _activation.getTraits(); }
...
// this only makes a difference on big-endian 64-bit systems (eg PPC64) which
// makes it a hard bug to notice.
...
/*static*/ DebuggerMethodInfo *DebuggerMethodInfo::create(AvmCore *core,
                                                          int32_t local_count,
                                                          uint32_t codeSize,
                                                          int32_t max_scopes) {
  MMgc::GC *gc = core->GetGC();
  const uint32_t extra =
      (local_count <= 1) ? 0 : (sizeof(Stringp) * (local_count - 1));

  DebuggerMethodInfo *dmi = new (gc, MMgc::kExact, extra)
      DebuggerMethodInfo(local_count, codeSize, max_scopes);
...
Stringp MethodInfo::getArgName(int32_t index) { return getRegName(index); }
...
// Note: dmi() can legitimately return NULL (for MethodInfo's that were
// synthesized by Traits::genInitBody, or for MethodInfo's that have no body,
// e.g. interface methods).
...
// note that the "local" can be a true local (0..local_count-1)
// or an entry on the scopechain (local_count...(local_count+max_scope)-1)
Atom MethodInfo::boxOneLocal(FramePtr src, int32_t srcPos,
  ...
  if (_isJitImpl) {
    ...
    return nativeLocalToAtom(this->pool()->core, src,
                             (SlotStorageType)sstArr[srcPos]);
  } else {
    AvmAssert(sstArr == NULL);
 ...
 *
 * The traitArr is used to determine the type of conversion that should take
 * place. traitArr[start]...traitArr[start+count-1] are used.
 *
 * If the method is interpreted then we just copy the Atom, no conversion is
 * needed.
 */
void MethodInfo::boxLocals(FramePtr src, int32_t srcPos, const uint8_t *sstArr,
                           Atom *dest, int32_t destPos, int32_t length) {
...
// note that the "local" can be a true local (0..local_count-1)
// or an entry on the scopechain (local_count...(local_count+max_scope)-1)
void MethodInfo::unboxOneLocal(Atom src, FramePtr dst, int32_t dstPos,
                               const uint8_t *sstArr) {
  if (_isJitImpl) {
    AvmAssert(sstArr != NULL);
 ...
 * needed.
 */
void MethodInfo::unboxLocals(const Atom *src, int32_t srcPos,
                             const uint8_t *sstArr, FramePtr dest,
                             int32_t destPos, int32_t length) {
...
bool MethodInfo::makeMethodOf(Traits *traits) {
  ...
  if (_declarer.getTraits() == NULL) {
    ...
    if (pool()->isVerbose(VB_parse))
      pool()->core->console << "WARNING: method " << this
                            << " was already bound to " << declaringTraits()
                            << "\n";
#endif
 ...
 */
void MethodSignature::boxArgs(AvmCore *core, int32_t argc, const uint32_t *ap,
                              Atom *out) const {
  MMGC_STATIC_ASSERT(
      sizeof(Atom) ==
  ...
  for (int32_t i = 0; i <= argc; i++) {
    const BuiltinType bt =
        (i <= param_count) ? this->paramTraitsBT(i) : BUILTIN_any;
    out[i] = nativeArgToAtom(core, (void *)ap, bt);
    ap += (bt == BUILTIN_number ? sizeof(double) : sizeof(Atom)) /
          sizeof(int32_t);
  }
}

MethodSignature *FASTCALL
MethodInfo::_buildMethodSignature(const Toplevel *toplevel) {
  PoolObject *pool = this->pool();
  AvmCore *core = pool->core;
  GC *gc = core->GetGC();
  ...
  // optional_count yet, so over-allocate for the worst case. (we know
  // optional_count<=param_count). this wastes space, but since we only cache a
  // few dozen of these, it's preferable to walking the data twice to get an
  // exact count.
  //
  // It is /critical/ to correct exact tracing of the MethodSignature that
  // param_count is written into the object before any types are written into
  // _args, and that optional_count is written into the object before any
  // default values are written into _args.

  const uint32_t extra = sizeof(MethodSignature::AtomOrType) *
                         (param_count + (hasOptional() ? param_count : 0));
  ...
  if (pos) {
    ...
    receiverType =
        _needClosure ? declaringTraits() : core->traits.object_itraits;
    ...
    // toplevel!=NULL check is so we only check when resolveSignature calls us
    // (not subsequently)
    if (toplevel != NULL && _isProtoFunc) {
      ...
      // if all params and return types are Object then make all params
      // optional=undefined
      if (param_count == 0)
        _ignoreRest = 1;
      if (!hasOptional() && returnType == NULL && param_count > 0 &&
          untyped_args == param_count) {
        _hasOptional = 1;
        _ignoreRest = 1;
        _isUnchecked = 1;
        // oops -- the ms we allocated is too small, has no space for optional
        // values but it's easy to re-create, especially since we know the types
        // are all null
        const uint32_t extra =
            sizeof(MethodSignature::AtomOrType) * (param_count + param_count);
        ms = MethodSignature::create(gc, extra, param_count);
        // don't need to re-set paramTypes: they are inited to NULL which is the
        // right value
      }
    }
    if (_isUnchecked) {
      ...
      for (uint32_t j = 0; j < optional_count; j++) {
        // WBATOM(gc, ms, &ms->_args[param_count+1+j].defaultValue,
        // undefinedAtom);
        // since we know we're going from NULL->undefinedAtom we can skip the
        // WBATOM call
      ...
      for (uint32_t j = 0; j < optional_count; j++) {
        ...
        const Atom value =
            pool->getLegalDefaultValue(toplevel, index, kind, argType);
...
#endif
  // _param_count and _optional_count were written earlier for reasons of GC
  // correctness
  ...
  ms->_allowExtraArgs =
      this->_needRest || this->_needArguments || this->_ignoreRest;
...
MethodSignature *FASTCALL MethodInfo::_getMethodSignature() {
  ...
  // note: MethodSignaturep are always built the first time in resolveSignature;
  // this is only executed for subsequent re-buildings. Thus we pass NULL for
  // toplevel (it's only used for verification errors, but those will have been
  // caught prior to this) and for abcGen and imtBuilder (since those only need
  // to be done once).
...
bool MethodInfo::computeIsTrivial() {
  ...
  if (_needActivation || _needRest || _needArguments || _isNative ||
      _setsDxns || !_hasMethodBody)
    return false;
...
#ifdef AVMPLUS_SAMPLER
    Sampler *sampler =
        declaringTraits ? declaringTraits->core->get_sampler() : NULL;
...
Stringp MethodInfo::getMethodNameWithTraits(Traits *t,
                                            bool includeAllNamespaces) const {
  Stringp name = NULL;
  ...
  // if config.methodNames isn't set, might as well still return a non-null
  // result
  if (name == NULL)
    name = core->concatStrings(core->newConstantStringLatin1("MethodInfo-"),
                               core->intToString(method_id));
...
PrintWriter &MethodRecognizer::print(PrintWriter &prw) const {
  AvmAssert(0);
  return prw;
}
...
MethodNameRecognizer::MethodNameRecognizer(const char *pattern, size_t length)
    : _pattern(pattern), _length(length) {}
...
MethodNameRegExRecognizer::MethodNameRegExRecognizer(const char *pattern,
                                                     size_t length) {
  ...
  if (!((RegExp *)_regex)
           ->compile(_pattern, length, options, &errStr, &errOffset, NULL)) {
    delete[] _pattern;
    ...
    size_t bufferLen =
        VMPI_strlen(msg) + errLen + VMPI_strlen(msgcont) + length + 1;
...
bool MethodNameRegExRecognizer::matches(const MethodInfo *m) const {
  ...
  int matches = (_invalid) ? -1
                           : ((RegExp *)_regex)
                                 ->exec(nm.c_str(), nm.length(), 0,
                                        RegExp::kNoUtfOption, offset);
  return (matches >= 0);
}
