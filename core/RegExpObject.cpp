...
RegExpObject::RegExpObject(VTable *ivtable, ScriptObject *objectPrototype)
    : ScriptObject(ivtable, objectPrototype), m_source(core()->kEmptyString),
      m_lastIndex(0), m_optionFlags(RegExp::kUtfOption), m_global(false),
      m_hasNamedGroups(false) {
  completeInitialization(core()->kEmptyString);
}

// This variant is used for "new RegExp( re )" where "re" is a RegExp - we copy
// it.

RegExpObject::RegExpObject(RegExpObject *toCopy)
    : ScriptObject(toCopy->vtable, toCopy->getDelegate()),
      m_source(toCopy->m_source), m_pcreInst(toCopy->m_pcreInst),
      m_lastIndex(0), m_optionFlags(toCopy->m_optionFlags),
      m_global(toCopy->m_global), m_hasNamedGroups(toCopy->m_hasNamedGroups),
      m_regexp(toCopy->m_regexp) {}

// This variant is used for "new RegExp(s)" and "new RegExp(s,f)" where s is not
// a RegExp object.  In the one-argument case "s" may have a trailing flags
// string.

RegExpObject::RegExpObject(VTable *ivtable, ScriptObject *delegate,
                           Stringp pattern, Stringp options)
    : ScriptObject(ivtable, delegate), m_source(pattern), m_lastIndex(0),
      m_optionFlags(RegExp::kUtfOption), m_global(false),
      m_hasNamedGroups(false) {
  // Skip the precompilation if the pattern and options hit the cache.

  if (!core()->m_regexCache.testCachedRegex(pattern, options)) {
    // Precompilation: check for named groups, and split out the options if no
    // options were passed in, as for new RegExp( regex.toString() ).
    ...
    while (pos < length) {
      ...
      if (c == 0)
        break;
      if (c == '(' && pos + 3 < length && pattern->charAt(pos + 1) == '?' &&
          pattern->charAt(pos + 2) == 'P' && pattern->charAt(pos + 3) == '<') {
        m_hasNamedGroups = true;
      } else if (options == NULL && c == '/' &&
                 (pos == 0 || pattern->charAt(pos - 1) != '\\') &&
                 numSlashSeen++ > 0) {
        options = pattern;
        optionpos = pos + 1;
      }
...
void RegExpObject::completeInitialization(String *options) {
  ...
  bool found = false;
  RegexCacheEntry &r =
      core()->m_regexCache.findCachedRegex(found, m_source, options);
  ...
  if (found) {
    ...
    ((RegExp *)m_regexp)
        ->compile(patternz.c_str(), patternz.length(), m_optionFlags, &error,
                  &errptr, NULL);
...
Atom RegExpObject::call(int argc, Atom *argv) {
  // this call occurs when a regexp object is invoked directly as a function ala
  // "/a|b/('dcab')"
  AvmCore *core = this->core();
  Atom inAtom =
      argc ? core->string(argv[1])->atom() : core->kEmptyString->atom();
...
ArrayObject *RegExpObject::split(Stringp subject, uint32_t limit) {
  ...
  while ((matchArray = _exec(subject, utf8Subject, startIndex, matchIndex,
                             matchLen)) != NULL) {
    // [cn 11/22/04] when match is made, but is length 0 we've matched the empty
    //  position between characters.  Although we've "matched", its zero length
    //  so just break out.
    if (matchLen == 0) {
      matchLen = 0;
      matchIndex =
          startIndex + numBytesInUtf8Character(
                           (uint8_t *)(utf8Subject.c_str()) +
                           startIndex); // +1char  will advance startIndex,
                                        // extract just one char
      if (!isEmptyRE) {
        // don't break if we're processing an empty regex - then we want to
        // split the string into each character so we want the loop to continue
        break;
      }
    }

    //[ed 8/10/04] don't go past end of string. not sure why pcre doesn't return
    //null for a match starting past the end. [cn 12/3/04] because a regular
    //expression which matches an empty position (space between characters)
    //  will match the empty position just past the last character.  This test
    //  is correct, though it needs to come before we do any setProperties to
    //  avoid a bogus xtra result.
    if (matchIndex + matchLen > utf8Subject.length()) {
      startIndex = matchIndex + matchLen;
      break;
    } else {
      out->setUintProperty(n++, stringFromUTF8(utf8Subject.c_str() + startIndex,
  ...
  // If we found no match, or we did find a match and are still under limit, and
  // there is a remainder left, add it
  if ((unsigned)n < limit && startIndex <= utf8Subject.length()) {
    out->setUintProperty(n++,
                         stringFromUTF8(utf8Subject.c_str() + startIndex,
                                        utf8Subject.length() - startIndex));
  }
...
ArrayObject *RegExpObject::_exec(Stringp subject,
  ...
  ArrayObject *result =
      _exec(subject, utf8Subject, startIndex, matchIndex, matchLen);
  if (get_global()) {
...
ArrayObject *RegExpObject::_exec(Stringp subject,
  ...
  if (startIndex < 0 || startIndex > subjectLength ||
      (results = ((RegExp *)m_regexp)
                     ->exec(utf8Subject.c_str(), subjectLength, startIndex,
                            RegExp::kNoUtfOption, ovector)) < 0) {
    matchIndex = 0;
    matchLen = 0;
    return NULL;
  }
  ...
  a->setAtomProperty(core->kinput->atom(), subject->atom());
  a->setLength(results);
  ...
  for (int i = 0; i < results; i++) {
    if ((int)ovector.getIndex(i * 2) > -1) {
      ...
      Atom match =
          stringFromUTF8(utf8Subject.c_str() + ovector.getIndex(i * 2), length);
  ...
  if (m_hasNamedGroups) {
    ...
    for (int i = 0; i < nameCount; i++) {
      ...
      // use to store the matching results, where OVECTOR_SIZE is 99. The
      // following check prevents the user-supplied RegEx from exploiting that
      // case. The maximum vale for nameIndex here is 48 or
      // ((OVECTOR_SIZE-2)/2)) to satisfy the calculation of length in, length =
      // ovector[nameIndex * 2 + 1] - ovector[ nameIndex * 2 ];
      ...
      length =
          ovector.getIndex(nameIndex * 2 + 1) - ovector.getIndex(nameIndex * 2);

      Atom name =
          stringFromUTF8((nameTable + 2), (uint32_t)VMPI_strlen(nameTable + 2));
      name = core->internString(name)->atom();

      Atom value = stringFromUTF8(
          utf8Subject.c_str() + ovector.getIndex(nameIndex * 2), length);
...
Atom RegExpObject::replace(Stringp subject, Stringp replacement) {
  ...
  while (lastIndex <= subjectLength &&
         (matchCount = ((RegExp *)m_regexp)
                           ->exec(src, subjectLength, lastIndex,
                                  RegExp::kNoUtfOption, ovector)) > 0) {
        ...
        case '9': {
          ...
          if (i >= 1 && i <= captureCount) {
            resultBuffer.writeN(src + ovector.getIndex(i * 2),
                                ovector.getIndex(i * 2 + 1) -
                                    ovector.getIndex(i * 2));
            ptr += (i >= 10) ? 3 : 2;
          } else {
            resultBuffer << *ptr++;
          }
    ...
    int newLastIndex =
        ovector.getIndex(0) + (ovector.getIndex(1) - ovector.getIndex(0));

    // prevents infinite looping in certain cases
    fixReplaceLastIndex(src, subjectLength, matchLen, newLastIndex,
                        resultBuffer);
  ...
  return stringFromUTF8((const char *)resultBuffer.c_str(),
                        resultBuffer.length());
}

Atom RegExpObject::replace(Stringp subject, ScriptObject *replaceFunction) {
  StUTF8String utf8Subject(subject);
  ...
  while (lastIndex < subjectLength &&
    ...
    for (int i = 1; i <= captureCount; i++) {
      argv[i + 1] =
          stringFromUTF8(src + ovector.getIndex(i * 2),
    ...
    resultBuffer << core()->string(
        avmplus::op_call(toplevel(), replaceFunction, argc, argv));
...
void RegExpObject::fixReplaceLastIndex(const char *src, int subjectLength,
                                       int matchLen, int &newLastIndex,
                                       StringBuffer &resultBuffer) {
  if (matchLen == 0 && get_global()) {
    // Advance one character
    if (newLastIndex < subjectLength) {
      uint32_t ch;
      int n = UnicodeUtils::Utf8ToUcs4(
          (const uint8_t *)src + newLastIndex, subjectLength - newLastIndex,
          &ch, core()->currentBugCompatibility()->bugzilla609416);
      if (n <= 0) {
...
bool RegExpObject::get_ignoreCase() {
  return hasOption(RegExp::kCaselessOption);
}
bool RegExpObject::get_multiline() {
  return hasOption(RegExp::kMultilineOption);
}
