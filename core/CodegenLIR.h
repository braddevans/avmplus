...
public:
  ...
  nanojit::BitSet
      *notnull; // merged nullability information for vars at this label
  ...
  const char *name;
  CodegenLabel()
      : labelIns(0), notnull(0), unpatchedEdges(0), varTracker(0),
        tagTracker(0), name(0) {}
  CodegenLabel(const char *name)
      : labelIns(0), notnull(0), unpatchedEdges(0), varTracker(0),
        tagTracker(0), name(name) {}
#else
  CodegenLabel()
      : labelIns(0), notnull(0), unpatchedEdges(0), varTracker(0),
        tagTracker(0) {}
  CodegenLabel(const char *)
      : labelIns(0), notnull(0), unpatchedEdges(0), varTracker(0),
        tagTracker(0) {}
#endif
};
...
//
// When we don't know the type of the base object at a point we access a
// property, we must look up the property at runtime./ We cache the results of
// this lookup in a BindingCache instance, and install a handler specialized for
// the object type and binding kind. Each BindingCache consists of:
//
...
//                      handlers quickly check this and "miss" when they dont
//                      match the object seen at runtime.
//
//    slot_offset or    precomputed offset for a slot, for fast loads, or a
//    preloaded method            MethodEnv* for a method, for fast calls
//
//    name              Multiname* for this dynamic access.  Used on a cache
//    miss,
...
//   * decl_type:  when an object type doesn't match the cached type, sometimes
//   the
...
//     method.  the subsequent misses can be handled faster if we saved the
//     method_id, and checked that the actual type & cached type are related by
//     inheritance. This doesn't help for interface methods and the fast case
//     didn't occur often enough for the extra cache size and code complexity.
//   * vtable: When a single type (Traits*) is used in different environments
//   we'll
//     have distinct VTable*'s that point to the same Traits*.  the cache might
//     hit more often if we stored Traits*.  however, specializing the cache on
//     VTable* lest us pre-load the MethodEnv* for calls, saving one load. also,
//     comparing vtable pointers instead of traits pointers saves one more load.
//     (5% median speedup)
//   * we pass the base object to CallCache::Handler instead of accessing it
//   indirectly
//     via args[], to save a load on the fast path.  Worth 2-3%, and enables the
//     same code to handle OP_callproperty and OP_callproplex.
//   * we put Multiname* in the cache instead of passing it as a constant
//   parameter,
//     because the increase in cache size is smaller than the savings in code
//     size, less parameters is faster, and the multiname is only used on
//     relatively slow paths.
//   * we allocate cache instances while generating code, which lets us embed
//     the cache address in code instead of loading from methodinfo.  some stats
//     on ESC:
//        call cache entries: 489
//        bytes of call cache entries: 7,824
//        instructions saved by early allocation (static): 1,445 (loads, movs,
//        adds) MethodInfo instances: 2436 bytes saved by eliminating 2 cache
//        pointers on MethodInfo: 19,448
//
//   (footnote: Times are from the tests in tamarin/test/performance as well as
//   a selection of Flash/Flex benchmark apps)
//
// Alternatives not yet investigated:
//
//   * we could put the whole Multiname in the cache, and maybe eliminate
//     PoolObject::precomputedMultinames[].  Not all multinames are used in late
//     bound call sites, so this could save some memory.  precomputed multinames
//     have been measured at over 10% of code size.  Need to study cache size
//     increase vs pool memory decrease.  Later when we have a code cache, this
//     could be more compelling.
//   * we could specialize BKIND_METHOD handlers on method return type, enabling
//   us
...
//     allowing us to inline boxing & unboxing code from coerceEnter AND
//     endCoerce
//   * the MethodEnv* passed to each handler is only used on slow paths.  Could
//   we
//     put it somewhere else?  maybe core->currentMethodFrame->env?
//   * on x86, FASTCALL might be faster, if it doesn't inhibit tail calls in
//   handlers
//   * other cache instance groupings:
//       * one per call site instead of per-unique-multiname?
//       * share them between methods?
//   * handlers that access primitive dynamic properites have to call
//   toplevel->toPrototype(val),
//     maybe we could save the result and check its validity with
//     (env->toplevel() == saved_proto->toplevel())
//   * we could specialize on primitive type too, inlining just the
//   toPrototype() path we need,
//     instead of using a single set of handlers for all primitives types
//   * OP_initproperty is rarely late bound in jit code; if this evidence
//   changes
...
public:
  ...
  const Multiname
      *const name; // multiname for this entry, saved when cache created.
...
public:
  typedef Atom (*Handler)(CallCache &, Atom base, int argc, Atom *args,
                          MethodEnv *);
...
template <class C> class CacheBuilder {
...
public:
   ...
   *  codeMgr.allocator is used to allocate memory for the method's caches, with
   * method lifetime */
...
// is presently used only for an argument to
// CodegenLIR::emitGetIndexedProperty() and
// CodegenLIR::emitSetIndexedProperty().
...
public:
  ...
  LineNumberRecord *jitAddRecord(uintptr_t pos, uint32_t filename,
                                 uint32_t line, bool pending = false);
...
protected:
  ...
  LIns *csn;
  LIns *undefConst;
  LIns *restArgc; // NULL or the expression that computes the number of rest
                  // arguments
...
#endif
  int labelCount;
  bool inlineFastpath; // Allow aggressive or speculative inlining, e.g.,
  ...
  CseFilter
      *cseFilter; // The CseFilter instance for this method, or NULL if none.
  JITNoise *noise;
  verbose_only(VerboseWriter *vbWriter;)
      verbose_only(LInsPrinter *vbNames;) JITDebugInfo *jit_debug_info;
  ...
  LIns *mopAddrToRangeCheckedRealAddrAndDisp(LIns *mopAddr, int32_t const size,
                                             int32_t *disp);
  LIns *loadEnvScope();
  LIns *loadEnvVTable();
  LIns *loadEnvAbcEnv();
  ...
  LIns *initMultiname(const Multiname *multiname, int &csp,
                      bool isDelete = false);
  ...
  LIns *cmpOptimization(int lhsi, int rhsi, LOpcode icmp, LOpcode ucmp,
                        LOpcode fcmp, bool strictOperation = false);
  debug_only(bool isPointer(int i);) void emitSetPc(const uint8_t *pc);
  ...
  void deadvars();
  void deadvars_analyze(Allocator &alloc, nanojit::BitSet &varlivein,
                        LabelBitSet &varlabels, nanojit::BitSet &taglivein,
                        LabelBitSet &taglabels);
  void deadvars_kill(Allocator &alloc, nanojit::BitSet &varlivein,
                     LabelBitSet &varlabels, nanojit::BitSet &taglivein,
                     LabelBitSet &taglabels);
  ...
  LIns *loadIns(LOpcode op, int32_t disp, LIns *base, AccSet accSet,
                LoadQual loadQual = LOAD_NORMAL);
  LIns *storeIns(LOpcode op, LIns *val, int32_t disp, LIns *base,
                 AccSet accSet);
  LIns *Ins(LOpcode op);
...
#ifdef VMCFG_FLOAT
  /* Handle 1- and 2-operand numeric operations. Assumption: "number"/"int" is
     the "fastpath", the rest may be slow (should really be typed if the user
     wants speed)   */
  ...
  void emitCall(AbcOpcode opcode, intptr_t method_id, int argc, Traits *result,
                MethodSignaturep);
  void emitCall(AbcOpcode opcode, intptr_t method_id, int argc, LIns *obj,
                Traits *objType, Traits *result, MethodSignaturep);

  /** Verifier has already coerced args and emitted the null check; JIT just
   * double checks and emits code */
  void emitTypedCall(AbcOpcode opcode, intptr_t method_id, int argc,
                     Traits *result, MethodInfo *);

  /** emit a JIT-discovered early bound call.  JIT must coerce args and receiver
   * to the proper type */
  void emitCoerceCall(AbcOpcode opcode, intptr_t method_id, int argc,
                      MethodSignaturep mms);

  /** emit a JIT-discovered early bound call to newInstance and invoke the
   * initializer function */
  void emitConstructCall(intptr_t method_id, int argc, LIns *ctor,
                         Traits *ctraits);
  ...
  void emit(AbcOpcode opcode, uintptr_t op1 = 0, uintptr_t op2 = 0,
            Traits *result = NULL);
  ...
  void emitIntPlusAtomFastpath(int i, Traits *type, LIns *lhs, LIns *rhs,
                               CodegenLabel &fallback);
#endif
  ...
  void emitTaintedIntConst(int index, int32_t c, Traits *type);
  void emitPtrConst(int index, void *c, Traits *type);
  void emitDoubleConst(int index, const double *pd);
  void emitTaintedDoubleConst(int index, const double *pd);
#ifdef VMCFG_FLOAT
  void emitTaintedFloatConst(int index, const float f);
  void emitTaintedFloat4Const(int index, const float4_t *pf4);
#endif // VMCFG_FLOAT
  ...
  LIns *emitGetIndexedProperty(int objIndex, LIns *index, Traits *result,
                               IndexKind idxKind);
  LIns *emitInlineVectorRead(int objIndexOnStack, LIns *index,
  ...
  LIns *emitInlineSpeculativeArrayRead(int objIndexOnStack, LIns *index,
                                       const CallInfo *helper);

  void emitInlineVectorWrite(int objIndexOnStack, LIns *index, LIns *value,
                             size_t arrayDataOffset, size_t lenOffset,
#ifdef VMCFG_VECTOR_SMASH_PROTECTION
                             size_t checksumOffset,
#endif
                             size_t entriesOffset, int scale,
                             LOpcode store_item, const CallInfo *helper);
  void emitSetIndexedProperty(int objIndex, int valIndex, LIns *index,
                              IndexKind idxKind);
  ...
  bool inlineBuiltinFunction(AbcOpcode opcode, intptr_t method_id, int argc,
                             Traits *result, MethodInfo *mi);
  LIns *optimizeIntCmpWithNumberCall(int callIndex, int otherIndex,
                                     LOpcode icmp, bool swap);
  LIns *optimizeStringCmpWithStringCall(int callIndex, int otherIndex,
                                        LOpcode icmp, bool swap);
  ...
  LIns *emitStringCall(int index, const CallInfo *stringCall,
                       bool preserveNull);
...
public:
  ...
  // May return true if JIT will always fail based on information known prior to
  // invocation.
  ...
  void writeOpcodeVerified(const FrameState *state, const uint8_t *pc,
                           AbcOpcode opcode);
