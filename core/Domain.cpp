...
ClassClosure *Domain::getParameterizedType(ClassClosure *type) {
  ...
  if (type) {
    ...
    if (AvmCore::isGenericObject(value)) {
      MMgc::GCWeakRef *wr =
          (MMgc::GCWeakRef *)AvmCore::atomToGenericObject(value);
      if (wr) {
...
void Domain::addParameterizedType(ClassClosure *type,
  ...
  if (type && parameterizedType) {
    ...
    // Note that what we really need here is fixing the problems with our weak
    // tables. If there exists a cycle in the weak table ('value' indirectly
    // pointing to the weak 'key'), the weak key will not get collected even if
    // there are no references to it from elsewhere and hence the table entry
    // will not be deleted, which will in turn lead to value not getting
    // collected.
    //
    // As a short term fix, we'll just use WeakKeyHashtable with explicit
    // GCWeakRef usage for the value.
    MMgc::GCWeakRef *wr =
        parameterizedType ? parameterizedType->GetWeakRef() : NULL;
