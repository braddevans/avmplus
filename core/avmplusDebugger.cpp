...
Debugger::Debugger(AvmCore *core, TraceLevel tracelevel)
    : astrace_console(tracelevel), astrace_callback(TRACE_OFF), in_trace(false),
      astraceStartTime(VMPI_getTime()), core(core),
      abcList(core->GetGC(), kListInitialCapacity), pool2abcIndex() {}

Debugger::~Debugger() { disableAllTracing(); }
...
void Debugger::stepInto() {
  ...
  stepState.startingDepth =
      (core && core->callStack) ? core->callStack->depth() : 0;
}
...
void Debugger::debugLine(int linenum) {
  ...
  bool exited =
      (prev == -1) ? true
                   : false; // are we being called as a result of function exit?
  if (!changed && !exited)
  ...
  bool stop = false;
  if (stepState.flag) {
    if (stepState.startingDepth != -1 &&
        core->callStack->depth() < stepState.startingDepth) {
      ...
      stop = true;
    } else if (!exited && (stepState.depth == -1 ||
                           core->callStack->depth() <= stepState.depth)) {
  ...
  if (stop) {
    ...
    StepState oldOldStepState =
        oldStepState; // save oldStepState in case of reentrancy
    oldStepState =
        stepState;     // save stepState so that stepContinue() can find it
...
void Debugger::traceMethod(MethodInfo *fnc, bool ignoreArgs) {
  ...
  if (astrace_console > TRACE_OFF) {
    if (fnc) {
      // WARNING: don't change the format of output since outside utils depend
      // on it
      ...
      if (!ignoreArgs && core->callStack &&
          (astrace_console == TRACE_METHODS_WITH_ARGS ||
           astrace_console == TRACE_METHODS_AND_LINES_WITH_ARGS))
...
void Debugger::traceLine(int line) {
  ...
  if (astrace_console >= TRACE_METHODS_AND_LINES) {
    ...
    // WARNING: don't change the format of output since outside utils depend on
    // it
    uint64_t delta = VMPI_getTime() - astraceStartTime;
...
void Debugger::traceCallback(int line) {
  ...
  Stringp file = (core->callStack->filename())
                     ? Stringp(core->callStack->filename())
                     : Stringp(core->kEmptyString);
  ...
  if (env->method) {
    ...
    if ((line == 0) &&
        (astrace_callback == TRACE_METHODS_WITH_ARGS ||
         astrace_callback == TRACE_METHODS_AND_LINES_WITH_ARGS)) {
  ...
  Atom argv[5] = {trace_callback->atom(), file->atom(), core->intToAtom(line),
                  name->atom(), args->atom()};
  int argc = 4;

  TRY(core, kCatchAction_ReportAsError) { trace_callback->call(argc, argv); }
...
bool Debugger::scanCode(AbcFile *file, PoolObject *pool, MethodInfo *m) {
  ...
  AvmCore::skipU32(
      pos, 4); // max_stack; local_count; init_stack_depth; max_stack_depth;
  uint32_t code_len =
...
DebugFrame *Debugger::frameAt(int frameNbr) {
  ...
  if (frameNbr >= 0) {
    ...
    if (trace)
      frame = new (core->GetGC()) DebugStackFrame(
          frameNbr, trace,
          this); //### FIXME: should be allocated in kDebugStackFramePartition
  }
  return frame;
}
 ...
 */
int Debugger::abcCount() const { return abcList.length(); }
...
// interactive
Atom Debugger::autoAtomKindAt(DebugFrame *frame, int autoIndex,
                              AutoVarKind kind) {
  if (!frame)
    return unreachableAtom;
  else
    return atomKind(autoAtomAt(frame, autoIndex, kind));
}

ScriptObject *Debugger::autoVarAsObject(DebugFrame *frame, int index,
                                        AutoVarKind kind) {
  if (!frame)
    return NULL; // should have tested for error earlier
  return AvmCore::atomToScriptObject(autoAtomAt(frame, index, kind));
}

Stringp Debugger::autoVarAsString(DebugFrame *frame, int index,
                                  AutoVarKind kind) {
  if (!frame)
    return NULL; // should have tested for error earlier
...
bool Debugger::autoVarAsBoolean(DebugFrame *frame, int index,
                                AutoVarKind kind) {
  if (!frame)
...
double Debugger::autoVarAsInteger(DebugFrame *frame, int index,
                                  AutoVarKind kind) {
  if (!frame)
    return MathUtils::kNaN; // should have tested for error earlier
  return AvmCore::number_d(autoAtomAt(frame, index, kind));
}

double Debugger::autoVarAsDouble(DebugFrame *frame, int index,
                                 AutoVarKind kind) {
  if (!frame)
    return MathUtils::kNaN; // should have tested for error earlier
...
float4_t Debugger::autoVarAsFloat4(DebugFrame *frame, int index,
                                   AutoVarKind kind) {
  float4_t res;
  if (!frame) {
...
Stringp Debugger::autoVarName(DebugStackFrame *frame, int index,
                              AutoVarKind kind) {
  if (frame == NULL)
    return NULL;
  int line;
...
MethodInfo *Debugger::functionFor(SourceInfo *src, int line,
 ...
 * have been loaded.
 */
AbcInfo *Debugger::abcAt(int index) const { return abcList.get(index); }
...
AbcFile::AbcFile(AvmCore *core, int size)
    : core(core), source(core->GetGC(), kListInitialCapacity), byteCount(size) {
  sourcemap = HeapHashtable::create(core->GetGC());
}

int AbcFile::sourceCount() const { return source.length(); }

SourceInfo *AbcFile::sourceAt(int index) const { return source.get(index); }

int AbcFile::size() const { return byteCount; }
...
SourceFile::SourceFile(MMgc::GC *gc, Stringp name)
    : named(name), functions(gc, kListInitialCapacity) {}

Stringp SourceFile::name() const { return named; }

/**
 * A line - offset pair should be recorded
 */
void SourceFile::addLine(int linenum, MethodInfo *func, int offset) {
  // Add the function to our list if it doesn't exist.  Use lastIndexOf()
  // instead of indexOf(), because this will be faster in the very common case
  // where the function already exists at the end of the list.
...
int SourceFile::functionCount() const { return functions.length(); }
...
bool SourceFile::hasBreakpoint(int linenum) { return breakpoints.get(linenum); }
...
DebugStackFrame::DebugStackFrame(int nbr, CallStackNode *tr, Debugger *debug)
    : trace(tr), debugger(debug), frameNbr(nbr) {
  AvmAssert(tr != NULL);
...
bool DebugStackFrame::sourceLocation(SourceInfo *&source, int &linenum) {
  ...
  if (trace->info() && trace->filename() && debugger) {
    uintptr_t index =
        (uintptr_t)debugger->pool2abcIndex.get(Atom(trace->info()->pool()));
...
bool DebugStackFrame::dhis(Atom &a) {
  ...
  if (trace->framep() && trace->info()) {
    trace->info()->boxLocals(trace->framep(), 0, trace->types(), &a, 0,
                             1); // pull framep[0] = [this]
    worked = true;
  } else {
    a = undefinedAtom;
  }
...
bool DebugStackFrame::methodName(Stringp &result) {
  if (trace->info()) {
    ...
    // return
    // trace->info()->pool()->getMethodInfoName(trace->info()->method_id());
...
bool DebugStackFrame::arguments(Atom *&ar, int &count) {
  ...
  if (trace->framep() && trace->info()) {
    ...
    if ((count > 0) && debugger) {
      ...
      ar = (Atom *)debugger->core->GetGC()->Calloc(
          count, sizeof(Atom), GC::kContainsPointers | GC::kZero,
          MMgc::kDebugStackFramePartition);
      MethodInfo *info = trace->info();
      info->boxLocals(trace->framep(), firstArgument, trace->types(), ar, 0,
                      count);
    }
  } else {
    worked = false;
...
bool DebugStackFrame::setArgument(int which, Atom &val) {
  ...
  if (trace->framep() && trace->info()) {
    ...
    if (count > 0 && which < count) {
      ...
      MethodInfo *info = trace->info();
      info->unboxLocals(&val, 0, trace->types(), trace->framep(),
                        firstArgument + which, 1);
      worked = true;
    }
  }
  return worked;
  ...
  if (trace->framep() && trace->info()) {
    ...
    if ((count > 0) && debugger) {
      ...
      ar = (Atom *)debugger->core->GetGC()->Calloc(
          ...
          MMgc::kDebugStackFramePartition);
      MethodInfo *info = trace->info();
      info->boxLocals(trace->framep(), firstLocal, trace->types(), ar, 0,
                      count);

      // If abcMethod_NEED_REST or abcMethod_NEED_ARGUMENTS is set, and the jit
      // is being used, then the first local is actually not an atom at all --
      // it is an ArrayObject*.  So, we need to convert it to an atom.  (If the
      // interpreter is being used instead of the jit, then it is stored as an
      // atom.)
  ...
  if (trace->framep() && trace->info()) {
    ...
    if (count > 0 && which < count) {
      ...
      if (which == 0 && info->needRestOrArguments()) {
        // They are trying to modify the first local, but that is actually the
        // special array for "...rest" or for "arguments".  That is too
        // complicated to allow right now.  We're just going to fail the
        // request.
      } else {
        // copy the single arg over
        info->unboxLocals(&val, 0, trace->types(), trace->framep(),
...
void DebugStackFrame::argumentBounds(int *firstArgument,
