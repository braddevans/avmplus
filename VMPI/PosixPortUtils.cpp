...
uint64_t VMPI_getTime() {
  ...
  // typecast tv.tv_sec to uint64_t to prevent overflow when multiplying by
  // 1000.
...
uintptr_t VMPI_startTimer(uint32_t micros, VMPI_TimerClient *client) {
  pthread_t p;
  VMPI_PosixTimerData *data =
      (VMPI_PosixTimerData *)VMPI_alloc(sizeof(VMPI_PosixTimerData));
...
void *VMPI_alloc(size_t size) { return malloc(size); }
...
void VMPI_callWithRegistersSaved(void (*fn)(void *stackPointer, void *arg),
  ...
  void *buf = NULL;
  __builtin_unwind_init(); // Save registers - GCC intrinsic.  Not reliable
                           // on 10.4 PPC or 64-bit
#else
  ...
  assert((intptr_t(&buf) & (sizeof(void *) - 1)) == 0);
  (void)force_align;
...
void VMPI_callWithRegistersSaved(void (*fn)(void *stackPointer, void *arg),
  ...
  CallWithRegistersSaved2(fn, arg,
                          NULL); // Computes the stack pointer, calls fn
  CallWithRegistersSaved3(
      fn, &arg,
      NULL); // Probably prevents the previous call from being a tail call
}
