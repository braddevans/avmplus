...
function cgExpr(ctx, e) {
    ...
    switch type(e) {
        ...
        {
            cgGetCogenTemp(ctx, e)
        }
        case (e:
            EvalScopeInitExpr
        )
        {
            cgEvalScopeInitExpr(ctx, e)
        }
        case (e:
            *
        )
...
function cgEvalPrefix(ctx, evalTmp, nargs, strict) {
    ...
    = ctx;
    let
    id_ESC = new Ast::Identifier(Token::sym_ESC, Ast::publicNSSL);
    let
    id_evaluateInScopeArray = new Ast::QualifiedIdentifier(id_ESC, Token::sym_evaluateInScopeArray);

    // Check it: Is this *really* the eval operator?

    let
    L_normalcall = undefined;
...
function cgEvalScopeInitExpr(ctx, {
    index, how
}
...
function cgDynamicOverrideExpr(ctx, {
    names, exprs, body
}
...
function cgSetExpr(ctx, e) {
    switch type(e.le) {
        ...
        {
            cgSetObjectRefExpr(ctx, e)
        }
        case (ident:
            Ast::IdentExpr
        )
        {
            cgSetIdentExpr(ctx, e)
        }
        case (x:
            *
        )
        {
            Gen::syntaxError(ctx, "Illegal lvalue " + x)
        }
    }
}

function cgSetIdentExpr(ctx, {
    op, le, re
}
...
function cgSetObjectRefExpr(ctx, {
    op, le, re
}
...
)
{
    ...
    if (opr == assignOp) {
        let
        name = cgIdentExpr(ctx, le.ident);   // order matters if it's a ComputedName
        cgExpr(ctx, re);
        asm.I_dup();
        asm.I_setlocal(t);
        ...
        let
        subtmp = null;     // stores indexing expression value
        let
        subname = null;    // multiname to store under
...
function cgOperate(ctx, expr, op, is_strict) {
    ...
    if (op == assignLogicalAndOp || op == assignLogicalOrOp) {
        ...
        asm.I_convert_b();
        let
        L0 = (op == assignLogicalAndOp) ? asm.I_iffalse(undefined) : asm.I_iftrue(undefined);
        asm.I_pop();
        cgExpr(ctx, expr);
        asm.I_coerce_a();
        asm.I_label(L0);
        ...
        switch (op) {
            ...
            case assignDivideOp:
                asm.I_divide();
                break;
            case assignRemainderOp:
                asm.I_modulo();
                break;
            case assignLeftShiftOp:
                asm.I_lshift();
                break;
            case assignRightShiftOp:
                asm.I_rshift();
                break;
            case assignRightShiftUnsignedOp:
                asm.I_urshift();
                break;
            case assignBitwiseAndOp:
                asm.I_bitand();
                break;
            case assignBitwiseOrOp:
                asm.I_bitor();
                break;
            case assignBitwiseXorOp:
                asm.I_bitxor();
                break;
            default:
...
function cgLiteralExpr(ctx, e) {
    ...
    function cgArrayInitializer(ctx, {
        exprs, spread
    }
    ...
    function cgObjectInitializer(ctx, {
        fields
    }
...
)
    {
        ...
        t = asm.getTemp();
        asm.I_setlocal(t);
        for (let i = 0, limit = fields.length;
        i < limit;
        i++
    )
        {
    ...
    switch type(e) {
        ...
        {
            cgArrayInitializer(ctx, e)
        }
        case (e:
            LiteralObject
        )
        {
            cgObjectInitializer(ctx, e)
        }
        case (e:
            LiteralRegExp
        )
        {
            cgRegExpLiteral(ctx, e)
        }
...
)
{
    ...
    = ctx;
    let
    id = new Ast::Identifier(Token::intern("$t" + n), Ast::publicNSSL);
    cgFindPropStrict(ctx, id);
    cgGetProp(ctx, id);
}

function cgGetParamExpr( {
    asm
}
