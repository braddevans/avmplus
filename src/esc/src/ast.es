...
type
IDENT = Token::Tok; // hm, how does this serialize??
...
class Fixture extends ASTNode implements Serializable {
    ...
    function Fixture(name, data): name
...
class InitBinding extends ASTNode implements Serializable {
    ...
    function InitBinding(name, expr): name
...
function nsEquals(ns1, ns2) {
    ...
    switch type(ns1) {
        ...
        {
            return ns2 is UnforgeableNamespace;
        }
        case (x:
            ForgeableNamespace
        )
        ...
        {
            return ns2 is PrivateNamespace;
        }
        case (x:
            ProtectedNamespace
        )
        {
            return ns2 is ProtectedNamespace;
        }
        case (x:
            PublicNamespace
        )
        {
            return ns2 is PublicNamespace;
        }
        case (x:
            InternalNamespace
        )
        {
            return ns2 is InternalNamespace;
        }
        case (x:
            *
        )
        {
            return ns1.hash() == ns2.hash();
        }
    }
}

class PrivateNamespace extends ASTNode implements Ast
...
class ProtectedNamespace extends ASTNode implements Ast
::Namespace, Serializable
{
    const name: IDENT;
...
class PublicNamespace extends ASTNode implements Ast
::Namespace, Serializable
{
    const name: IDENT;
...
class InternalNamespace extends ASTNode implements Ast
::Namespace, Serializable
{
    const name: IDENT;
...
class ForgeableNamespace extends ASTNode implements Ast
::Namespace, Serializable
{
    const name: IDENT;
...
class UnforgeableNamespace extends ASTNode implements Ast
::Namespace, Serializable
{
    const name: IDENT;
...
final class NamespaceSet extends ASTNode implements Serializable {
    ...
    function NamespaceSet(ns, link): ns
...
final class NamespaceSetList extends ASTNode implements Serializable {
    ...
    function NamespaceSetList(nsset, link): nsset
...
class TernaryExpr extends Expr implements Serializable {
    ...
    function TernaryExpr(e1, e2, e3): e1
...
class BinaryExpr extends Expr implements Serializable {
    ...
    function BinaryExpr(op, e1, e2): op
...
class BinaryTypeExpr extends Expr implements Serializable {
    ...
    function BinaryTypeExpr(op, e1, e2): op
...
class ThisExpr extends Expr implements Serializable {
    ...
    function ThisExpr(strict): strict
...
class FuncName extends ASTNode implements Serializable {
    ...
    function FuncName(kind, ident): kind
...
...
    Expr
       ...
       // Will be null if there is no 'super' call in the settings
    const superSpread: ?
    Expr const func: Func;
...
class ClassFixtureFwd extends FixtureFwd implements Serializable {
    function ClassFixtureFwd(params, nonnullable):

    super(params, nonnullable)
    {
    }
...
class InterfaceFixtureFwd extends FixtureFwd implements Serializable {
    function InterfaceFixtureFwd(params):
...
class TypeFixtureFwd extends FixtureFwd implements Serializable {
    function TypeFixtureFwd(params, nonnullable):
...
const [
anyType, nullType, undefinedType, voidType
]
= specialTypes;
...
internal;
    ...
    public function Serializer(compact = false): compact
    ...
    function serializeArray(obj, linebreak) {
        ...
        s = "[";
        let
        separator = !compact && linebreak ? "\n," : ",";
        let
        lastWasOK = true;
        for (let i = 0, limit = obj.length;
        i < limit;
        i++
    )
        {
...
internal;
    ...
    function decode(x) {
        ...
        if (x.hasOwnProperty("ast_layout")) {
            ...
            tag = desc[0];
            desc.shift(1);
            return (getConstructor(tag, desc.length)).apply(null, Util::map(function (n) {
                return decode(x[n])
            }, desc));
        }
