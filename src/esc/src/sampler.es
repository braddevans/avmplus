...
internal function frame2name(s) {
    ...
    if (!profile_data.hasOwnProperty(name))
        profile_data[name] = {name: name, callers: {toString: printCallers}, self: 0, children: 0,
    new
:
    0, toString
:
...
function snapshotProfile() {
    ...
    let
    samples = "flash.sampler"::getSamples();
    for each (let sample in samples
)
    {
        if (sample.stack) {
            let
            name = frame2name(sample.stack[0]);
            switch type(sample) {
                case (s:
                ...
                )
                {
                    ...
                    for (let i = 1;
                    i < sample.stack.length;
                    i++
                )
                    {
                        let
                        parent_name = frame2name(sample.stack[i]);
...
function dumpProfile(filename) {
    ...
    s += "" + right(tmp[i].new, 8) + " " + right((100 * tmp[i].new
    /total).toFixed(1), 5) + "% " + tmp[i].name + "\n";
    ...
    function timeProfile(val, bywhat) {
        ...
        for (let i = 0;
        i < tmp.length && val(tmp[i]) > 0;
        i++
    )
        total += tmp[i].self;  // [sic]
        for (let i = 0;
        i < tmp.length && (v = val(tmp[i])) > 0;
        i++
    )
        {
