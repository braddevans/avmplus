...
class ABCConstantPool {
    function ABCConstantPool() {
        ...
        function hash_name(n: Token

        ::Tok
    )
        n.hash;

        function eq_names(n1: Token
    ...
    function namespace(kind/*:uint*/, name/*:uint*/) {
        ...
        let
        probe = namespace_map.read(tmp_namespace);
        if (probe == 0) {
    ...
    function namespaceset(namespaces: Array) {
        let
        probe = namespaceset_map.read(namespaces);
        if (probe == 0) {
            ...
            for (let i = 0, limit = namespaces.length;
            i < limit;
            i++
        )
    ...
    function multinameLookup(kind, name, ns) {
        ...
        tmp_multiname.ns = ns;
        let
        probe = multiname_map.read(tmp_multiname);
        if (probe != 0)
            return probe;
        ...
        let
        entry = {"kind": tmp_multiname.kind, "name": tmp_multiname.name, "ns": tmp_multiname.ns}
    ...
    function QName(ns/*: uint*/, name/*: uint*/, is_attr: Boolean) {
        let
        kind = is_attr ? CONSTANT_QNameA : CONSTANT_QName;
        let
        idx = multinameLookup(kind, name, ns);
        if (idx < 0) {
    ...
    function RTQName(name/*: uint*/, is_attr: Boolean) {
        let
        kind = is_attr ? CONSTANT_RTQNameA : CONSTANT_RTQName;
        let
        idx = multinameLookup(kind, name, 0);
        if (idx < 0) {
            multiname_bytes.uint8(kind);
            multiname_bytes.uint30(name);
            idx = -idx;
    ...
    function RTQNameL(is_attr: Boolean) {
        let
        kind = is_attr ? CONSTANT_RTQNameLA : CONSTANT_RTQNameL;
        let
        idx = multinameLookup(kind, 0, 0);
        if (idx < 0) {
            multiname_bytes.uint8(kind);
            idx = -idx;
        }
    ...
    function Multiname(nsset/*: uint*/, name/*: uint*/, is_attr: Boolean) {
        let
        kind = is_attr ? CONSTANT_MultinameA : CONSTANT_Multiname;
        let
        idx = multinameLookup(kind, name, nsset);
        if (idx < 0) {
            multiname_bytes.uint8(kind);
            multiname_bytes.uint30(name);
    ...
    function MultinameL(nsset/*: uint*/, is_attr: Boolean) {
        let
        kind = is_attr ? CONSTANT_MultinameLA : CONSTANT_MultinameL;
        let
        idx = multinameLookup(kind, 0, nsset);
        if (idx < 0) {
            multiname_bytes.uint8(kind);
            multiname_bytes.uint30(nsset);
            idx = -idx;
    ...
    function hasRTNS(index) {
        let
        kind = multiname_pool[index].kind;
        let
        result;
        switch (kind) {
    ...
    var utf8_count = 1;
    /*private*/
    var namespace_count = 1;
    /*private*/
    var namespaceset_count = 1;
    /*private*/
    const multiname_pool = []; // its length is the count

    /*private*/
    const int_bytes = new ABCByteStream;
    /*private*/
    const uint_bytes = new ABCByteStream;
    /*private*/
    const double_bytes = new ABCByteStream;
    /*private*/
    const utf8_bytes = new ABCByteStream;
    /*private*/
    const namespace_bytes = new ABCByteStream;
    /*private*/
    const namespaceset_bytes = new ABCByteStream;
    /*private*/
    const multiname_bytes = new ABCByteStream;
...
class ABCMethodInfo {
    ...
    // abc-parse.es grubs around here.
    /*private*/
    var name, param_types, return_type, flags, options, param_names;
}
...
class ABCMetadataInfo {
    ...
    function serialize(bs) {
        ...
        for (let i = 0, limit = items.length;
        i < limit;
        i++
    )
        {
...
class ABCInstanceInfo {
    ...
    // abc-parse.es grubs around here.
    /*private*/
    var name, super_name, flags, protectedNS, interfaces, iinit, traits;
}
...
class ABCSlotTrait /// extends ABCTrait
{
    ...
    // abc-parse.es grubs around here.
    /*private*/
    var slot_id, type_name, vindex, vkind;
    ...
    function serialize(bs) {
        ...
        if (metadata.length > 0) {
            ...
            for (let i = 0, limits = metadata.length;
            i < limit;
            i++
        )
...
class ABCOtherTrait /// extends ABCTrait  // removed for esc
{
    ...
    function serialize(bs) {
        ...
        if (metadata.length > 0) {
            bs.uint30(metadata.length);
            for (let i = 0, limit = metadata.length;
            i < limit;
            i++
        )
            bs.uint30(metadata[i]);
...
class ABCClassInfo {
    ...
    function serialize(bs) {
        ...
        bs.uint30(cinit);
        bs.uint30(traits.length);
        for (let i = 0, limit = traits.length;
        i < limit;
        i++
    )
        traits[i].serialize(bs);
...
class ABCMethodBodyInfo {
    ...
    function setMaxStack(ms) {
        max_stack = ms
    }

    function setLocalCount(lc) {
        local_count = lc
    }

    function setInitScopeDepth(sd) {
        init_scope_depth = sd
    }

    function setMaxScopeDepth(msd) {
        max_scope_depth = msd
    }

    function setCode(insns) {
        code = insns
    }
    ...
    function serialize(bs) {
        ...
        for (let i = 0, limit = exceptions.length;
        i < limit;
        i++
    )
    ...
    // abc-parse.es grubs around here.
    /*private*/
    var init_scope_depth = 0, exceptions = [], traits = [];
    /*private*/
    var method, max_stack, local_count, max_scope_depth, code;
}
...
class ABCException {
    ...
    // abc-parse.es grubs around here.
    /*private*/
    var first_pc, last_pc, target_pc, exc_type, var_name;
}
