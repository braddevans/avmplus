...
type
NamespaceExpr = (Ast::IdentExpr | Ast::LiteralString);
...
final class Parser {
    ...
    function makeIdentifier(id: Token
    ...
    Ast::Identifier {
        ...
        let
        ident = new Ast::Identifier(id, nss, pos);
    ...
    */
    ...
    function desugarAssignmentPattern(p: Pattern, t: Ast
...
:
    ...
    function desugarBindingPattern(p: Pattern, t: Ast
...
:
    ...
    [[Ast::Fixture], Ast::Expr]
    {
        ...
        function identExprFromExpr(e: Ast
        ...
        function desugarSubPattern(p: Pattern, t: Ast
    ...
    :
        ...
        [[Ast::Fixture], Ast::Expr]
        {
            ...
            switch type(p) {
                ...
                {
                    let
                    nm = new Ast::PropName(new Ast::Name(ns, p.ident));
                    let
                    fx = new Ast::ValFixture(t, ro);
                    fxtrs = [new Ast::Fixture(nm, fx)];
                    let
                    inits = (e !== null) ? ([new Ast::InitBinding(nm, e)]) : []; // FIXME (fix what??)
                ...
                )
                {
                    ...
                    if (it != null) { // we have an init target so must be an init
                        let
                        ie = identExprFromExpr(p.expr);
                        let
                        nm = cx.resolveIdentExpr(ie, it);
                ...
                )
                {
                    ...
                    let
                    ptrns = p.ptrns;
                    for (let i = 0, limit = ptrns.length;
                    i < limit;
                    ++i
                )
                    {
                    ...
                    let
                    head = new Ast::Head([new Ast::Fixture(tn, new Ast::ValFixture(Ast::anyType, false))],
    ...
    function namespaceExpression() {
        if (hd() == Token::StringLiteral)
            return let(expr = new Ast::LiteralString(lexeme(), position()))
        next(), expr;
        let
        name = nameExpression();
    ...
    ::IdentExpr
    {
        ...
        pos = position();
        if (hd() == Token::StringLiteral)
            name = let(s = new Ast::LiteralString(lexeme(), pos))
        next(), s;
    else
        if (hd() == Token::Identifier)
    ...
    function parenExpression(): Ast

    ::Expr
    {
        eat(Token::LeftParen);
        let
        expr = fullCommaExpression();
    ...
    function objectInitializer(): Ast

    ::TypeExpr
    {
        let [var_prefix, const_prefix] = initializerAttribute();
        let
        pos = position();             // Record source location of initial left brace
        eat(Token::LeftBrace);
        let
        fields = fieldList(var_prefix, const_prefix);
        ...
        let
        texpr = alpha == allowColon && match(Token::Colon) ? typeExpression() : Ast::anyType;

        return new Ast::LiteralObject(fields, texpr, pos);
    }

    function fieldList(var_prefix, const_prefix):

    [(Ast::LiteralField | Ast::ProtoField | Ast::VirtualField)]
    {
        let
        fields = []
    :
    ...
    function literalField(var_prefix, const_prefix): Ast
    ...
    ::FieldType
    {
        ...
        if (fn is Ast::Identifier) {
            if (fn.ident == Token::sym_get) {
                let
                getter = getterOrSetterFieldMaybe(tag, Ast::getterFunction);
                if (getter)
                    return getter;
            } else if (fn.ident == Token::sym_set) {
                let
                setter = getterOrSetterFieldMaybe(tag, Ast::setterFunction);
                if (setter)
                    return setter;
            }
        }
        ...
        let
        expr = match(Token::Colon) ? fullAssignmentExpression() : null;
    ...
    function getterOrSetterFieldMaybe(tag, kind) {
        ...
        name = fieldName();
        let
        fn = functionExpression(kind);

        return new Ast::VirtualField(tag, name, kind, fn);
    }

    function fieldName(): Ast
        ...
        switch (hd()) {
            case Token::StringLiteral:
                if (hd2() == Token::DoubleColon)
                    break;
                return let(name = new Ast::Identifier(lexeme(), Ast::publicNSSL))
                next(), name;

            case Token::IntLiteral:
            case Token::UIntLiteral:
                return let(name = new Ast::Identifier(Token::intern(parseInt(lexeme().text)), Ast::publicNSSL))
                next(), name;

            case Token::DoubleLiteral:
                return let(name = new Ast::Identifier(Token::intern(parseFloat(lexeme().text)), Ast::publicNSSL))
                next(), name;

            case Token::DecimalLiteral:
                return let(name = new Ast::Identifier(Token::intern(new decimal(lexeme().text)), Ast::publicNSSL))
                next(), name;
    ...
    function arrayInitializer(): Ast

    ::Expr
    {
        let [var_prefix, const_prefix] = initializerAttribute();
        let
        pos = position();   // Record source location of initial left bracket
        ...
        if (elts.length == 1 && spread == null && (hd() == Token::For || hd() == Token::Let || hd() == Token::If)) {
            ...
            let
            t = alpha == allowColon && match(Token::Colon) ? typeExpression() : Ast::anyType;
    ...
    function comprehensionExpression() {
        switch (hd()) {
            ...
            case Token::For: {
                eat(Token::For);
                let
                is_each = matchToken(Token::id_each);
                eat(Token::LeftParen);
                let [pattern, annotation] = dynamic
            override
                (beta = noIn)
                typedPattern();
                ...
                eat(Token::In);
                let
                iterator = fullCommaExpression();
                eat(Token::RightParen);
                let
                subclause = comprehensionExpression();
    ...
    function primaryExpression(): Ast
    ...
    ::Expr
    {
        ...
        let
        pos = position();   // Record the source location before consuming the token
        switch (hd()) {
            case Token::Null:
                return let(expr = new Ast::LiteralNull(pos))
                next(), expr;

            case Token::True:
            case Token::False:
                return let(expr = new Ast::LiteralBoolean(hd() == Token::True, pos))
                next(), expr;

            case Token::IntLiteral:
                return let(expr = new Ast::LiteralInt(parseInt(lexeme().text), pos))
                next(), expr;

            case Token::UIntLiteral:
                return let(expr = new Ast::LiteralUInt(parseInt(lexeme().text), pos))
                next(), expr;

            case Token::DoubleLiteral:
                return let(expr = new Ast::LiteralDouble(parseFloat(lexeme().text), pos))
                next(), expr;

            case Token::DecimalLiteral:
                return let(expr = new Ast::LiteralDecimal(new decimal(lexeme().text), pos))
                next(), expr;

            case Token::StringLiteral:
                if (hd2() == Token::DoubleColon)
                    break;
                return let(expr = new Ast::LiteralString(lexeme(), pos))
                next(), expr;

            case Token::RegexpLiteral:
                return let(expr = new Ast::LiteralRegExp(lexeme(), pos))
                next(), expr;
    ...
    function propertyOperator(obj: Ast
    ...
    ::Expr
    {
        switch (hd()) {
            ...
            case Token::Super:
                ...
                let
                property_expr = propertyOperator(super_expr);
    ...
    function memberExpressionPrime(expr: Ast
    ...
    // shared among many
    function callExpressionPrime(call_expr: Ast
    ...
    function newExpression(new_count = 0): Ast

    ::Expr
    {
        let
        call_expression, is_new;
    ...
    function leftHandSideExpression(): Ast

    ::Expr
    {
        let
        operator = (hd() == Token::New) ? newExpression(0) : memberExpression();
        if (hd() == Token::LeftParen) {
            let
            pos = position();
    ...
    function postfixExpression(): Ast
    ...
    function multiplicativeExpression(): Ast
    ...
    function additiveExpression(): Ast
    ...
    function relationalExpression(): Ast
    ...
    function equalityExpression(): Ast
    ...
    function bitwiseAndExpression(): Ast
    ...
    function bitwiseXorExpression(): Ast
    ...
    function bitwiseOrExpression(): Ast
    ...
    function logicalAndExpression(): Ast
    ...
    function logicalOrExpression(): Ast
    ...
    function dynamicOverrideExpression() {
        ...
        eat(Token::RightParen);
        let
        body = commaExpression();
        return new Ast::DynamicOverrideExpr(names, exprs, body);
    }

    function nonAssignmentExpression(): Ast
        ...
        switch (hd()) {
            ...
            default: {
                ...
                override
                    ...
                    eat(Token::Colon);
                    let
                    alternate = nonAssignmentExpression();
    ...
    function fullAssignmentExpression(): Ast

    ::Expr
dynamic override
    (alpha = allowColon, beta = allowIn)
    assignmentExpression();

    function assignmentExpression(): Ast
    ...
    function patternFromExpr(e: Ast
...
)
    {
        ...
        if (e is Ast::LiteralObject)
            return new ObjectPattern(Util::map(function (f) {
                    return new FieldPattern(f.ident, f.expr || f.ident)
                },
                e.fields));
    ...
    function fullCommaExpression(): Ast
...
dynamic override
    ...
    function simplePattern(): Pattern {
        switch (gamma) {
            ...
            case allowExpr:
                return new SimplePattern(leftHandSideExpression());
        }
    }

    function arrayPattern(): Ast
...
dynamic override
    (beta = allowIn)
    arrayElements(pattern);

    function objectPattern(): Ast
    ...
    function typeName(): Ast

    ::TypeExpr
    {
        let
        name = new Ast::TypeName(nameExpression());
    ...
    function basicTypeExpression(): Ast

    ::TypeExpr
    {
        switch (hd()) {
            case Token::Mult:
                return next(), Ast::anyType;
            case Token::Null:
                return next(), Ast::nullType;
            case Token::Function:
                return functionType();
            case Token::LeftParen:
                return unionType();
    ...
    function unionType(): Ast

    ::TypeExpr
    {
        eat(Token::LeftParen);
        let
        type_exprs = typeExpressionListBar();
        eat(Token::RightParen);

        return new Ast::UnionType(type_exprs);
    }

    function objectType(): Ast

    ::TypeExpr
    {
        eat(Token::LeftBrace);
        let
        field_types = fieldTypeList();
        eat(Token::RightBrace);
    ...
    function typeExpressionListBar():
    ...
    ::Stmt
    {
        switch (hd()) {
            case Token::SemiColon:
                return let(stmt = new Ast::EmptyStmt)
                next(), stmt;
            case Token::LeftBrace:
                return new Ast::BlockStmt(statementBlock());
            case Token::Break:
                return let(stmt = breakStatement())
                semicolon(omega), stmt;
            case Token::Continue:
                return let(stmt = continueStatement())
                semicolon(omega), stmt;
            case Token::Do:
                return let(stmt = doStatement())
                semicolon(omega), stmt;
            case Token::For:
                return forStatement();
            case Token::If:
                return ifStatement();
            case Token::Return:
                return let(stmt = returnStatement())
                semicolon(omega), stmt;
            case Token::Switch:
                return (hd2() == Token::Type) ? switchTypeStatement() : switchStatement();
            case Token::Throw:
                return let(stmt = throwStatement())
                semicolon(omega), stmt;
            case Token::Try:
                return tryStatement();
            case Token::While:
            ...
            default:
                ...
                return let(stmt = new Ast::ExprStmt(fullCommaExpression()))
                semicolon(omega), stmt;
        }
    }

    function substatement(): Ast
    ...
    function statementBlock(): Ast

    ::Block
    {
        eat(Token::LeftBrace);
        cx.enterBlock();
        let [directive_list, _] = dynamic
    override
        (omega = fullStmt, tau = localBlk)
        directivesLocal();
        let
        head = cx.exitBlock();
        eat(Token::RightBrace);
    ...
    function breakStatement(): Ast

    ::Stmt
    new Ast::BreakStmt(breakOrContinueLabel(Token::Break));

    function continueStatement(): Ast
    ...
    ::Stmt
    {
        ...
        init = null;
        let
        bindings = null, it = null, ro = false;
        ...
        switch (hd()) {
            ...
            case Token::Let:
                cx.enterBlock();
            [it, ro, bindings]
                = bindingForInitialiser();
                break;
            default:
                init = dynamic
            override
                (alpha = allowColon, beta = noIn)
                commaExpression();
        }
        ...
        if (match(Token::In)) {
            ...
            eat(Token::SemiColon);
            let
            test = hd() == Token::SemiColon ? null : fullCommaExpression();
            eat(Token::SemiColon);
            let
            update = hd() == Token::RightParen ? null : fullCommaExpression();
            eat(Token::RightParen);
            let
            body = substatement();
    ...
    function collapseBindings(it, ro, bindings) {
        let
        init = null;
        for (let i = 0, limit = bindings.length;
        i < limit;
        i++
    )
        {
        ...
        loop:
            for (; ;) {
                switch (hd()) {
                    ...
                    override
                        (tau = localBlk)
                        directivesLocal(1);            // 'abbrev' is handled by RightBrace check in head
    ...
    function catchClause(require_type = false): Ast

    ::Catch
    {
        eat(Token::LeftParen);
        let [pattern, ty] = dynamic
    override
        (alpha = allowColon, beta = allowIn)
        typedPattern();
    ...
    function switchTypeStatement(): Ast
    ...
    function variableDefinition(attrs): ?
    ...
    function variableBindingList(ns: Ast
...
:
    ...
    function functionDefinition(attrs: ATTRS):

    [Ast::Stmt] {
        let
        is_const = false;     // FIXME: not currently used
        ...
        let
        name = functionName(tau == globalBlk || tau == classBlk || tau == interfaceBlk);
        let
        signature = functionSignature(name.kind);
        let
        body, strict;
    ...
    override
        ...
        let
        {
            params, defaults, resultType, thisType, numparams
        }
        = signature;
        let
        func = new Ast::Func(name, body, params, numparams, vars, defaults, resultType, attr, strict);

        if (strict)
            checkStrictFunction(func);

        let
        name = new Ast::PropName(new Ast::Name(attrs.ns, name.ident));
        let
        fxtr = new Ast::MethodFixture(func, Ast::anyType, true, attrs.override, attrs.final);
        cx.addFixture(tau == localBlk ? Ast::letInit : Ast::varInit, name, fxtr, attrs.static);
    }

    function functionExpression(kind): Ast
    ...
    ::Expr
    {
        ...
        let
        {
            params, numparams, defaults, resultType
        }
        = signature;
        let
        ...
        let
        fnexpr = new Ast::LiteralFunction(func, pos);

        return fnexpr;
    }

    function constructorDefinition(ns, attrs):
    ...
    [Ast::Stmt] {
        ...
        [body, strict]
            = dynamic
        override
            (omega = fullStmt, tau = constructorBlk)
            directivesLocal();
            eat(Token::RightBrace);
        }
        ...
        let
        {
            params, defaults, settings, superArgs, numparams, superSpread
        }
        = signature;
        let
        func = new Ast::Func(new Ast::FuncName(Ast::ordinaryFunction, name),
            body,
            params,
            numparams,
            vars,
...
static type
    ...
    function constructorSignature(): CTOR_SIG {
        ...
        let
        {
            fixtures, inits, exprs, types, numparams
        }
        = ps;
    ...
    */
    ...
    function constructorInitialiser():
    ...
    */
    ...
    ::Expr
    {
        ...
        lhs = dynamic
    override
        (beta = allowIn, gamma = allowExpr)
        pattern();
        eat(Token::Assign);
        let
        rhs = fullAssignmentExpression();
    ...
    function functionName(allow_getter_setter): Ast

    ::FUNC_NAME
    {
        let
        kind = Ast::ordinaryFunction;
        let
        ident = null;
...
static type
    ...
    function parameters(): PARAMETERS {
        let
        params = {fixtures: [], inits: [], exprs: [], types: [], numparams: 0};
        let
        initRequired = false;
...
[Ast::VAR_DEFN_TAG, [Pattern, Ast::TypeExpr]]
    {
        let
        kind = match(Token::Const) ? Ast::constTag : Ast::varTag;
        let
        pattern = dynamic
    override
        (beta = allowIn)
    ...
    function parameterToFixturesAndInitAndType(param, n):

    [[Ast::Fixture], Ast::Expr, Ast::TypeExpr]
    {
        let [k, [p, t]] = param;
    ...
    function classDefinitionOrDeclaration(attrs): void {
        ...
        classid = identifier();
        let
        cname = new Ast::Name(attrs.ns, classid);
        let [typeformals, nonnullable] = typeSignature();
        let
        superclass = extendsClause();
        let
        superinterfaces = interfaceList(Token::id_implements);
        let
        fx;

        if (superclass != null || superinterfaces.length != 0 || hd() == Token::LeftBrace) {
            let
            protectedNs = new Ast::ProtectedNamespace(classid);  // FIXME: needs to use attrs.ns too!
            let
            privateNs = new Ast::PrivateNamespace(classid);      // FIXME: needs to use attrs.ns too!
            ...
            let
            ihead = cx.exitClassInstance();
            let
            chead = cx.exitClassStatic();
            ...
            let
            ctype = Ast::anyType;                                 // FIXME: proper class type
            let
            itype = Ast::anyType;                                 // FIXME: proper instance type
            let
            cls = new Ast::Cls(cname,
                typeformals,
                nonnullable,
                superclass,
                superinterfaces,
    ...
    function interfaceDefinitionOrDeclaration(attrs): void {
        ...
        let
        iname = new Ast::Name(attrs.ns, interfaceid);
        let [typeformals, nonnullable] = typeSignature();
        let
        superinterfaces = interfaceList(Token::id_extends);
        let
        fx;
        ...
        if (superinterfaces.length != 0 || hd() == Token::LeftBrace) {
            ...
            let
            iface = new Ast::Interface(iname, typeformals, superinterfaces, ihead);
    ...
    function makeDefaultCtor(classname) {
        ...
        ty = Ast::anyType;
        let
        attr = cx.exitFunction();
        let
        func = new Ast::Func(new Ast::FuncName(Ast::ordinaryFunction, classname),
            ctorbody,
            params,
            numparams,
            vars,
    ...
    [[Ast::IDENT], boolean]
    {
        ...
        type_params = typeParameters();
        let
        is_nonnullable = match(Token::Not);
    ...
    function namespaceDefinition(attrs): void {
        ...
        let
        fxname = new Ast::PropName(new Ast::Name(attrs.ns, name));
        let
        fxtr = new Ast::NamespaceFixture(nsVal);
    ...
    [Ast::Stmt] {
        ...
        while (hd() != Token::RightBrace && hd() != Token::EOS) {
            ...
            override
                (beta = allowIn)
                variableDefinition(makeAttrs(ns, attrs));
                semicolon(fullStmt);
                if (e != null) {
            ...
            dynamic override
                (omega = fullStmt)
                functionDefinition(makeAttrs(ns, attrs));
            ...
            dynamic override
                (tau = classBlk)
                classDefinitionOrDeclaration(makeAttrs(ns, attrs));
                continue;
            }
            ...
            dynamic override
                (tau = interfaceBlk)
                interfaceDefinitionOrDeclaration(makeAttrs(ns, attrs));
                continue;
            }
    ...
    [Ast::Stmt, Ast::Ctor] {
        ...
        while (hd() != Token::RightBrace && hd() != Token::EOS) {
            ...
            if (t == Token::Function) {
                ...
                override
                    (omega = fullStmt)
                    constructorDefinition(cx.getDefaultNamespace(), makeAttrs(ns, attrs));
                } else
                dynamic override (omega = fullStmt)
                functionDefinition(makeAttrs(ns, attrs));
                continue;
            }
    ...
    function directivesInterface(): void {
        ...
        while (hd() != Token::RightBrace && hd() != Token::EOS) {
            ...
            dynamic override
                (omega = fullStmt)
                functionDefinition(makeAttrs(ns, 0));
                continue;
            }
    ...
    function directivesLocal(n = 1e500):
    ...
    [Ast::Stmt, Boolean] {
        ...
        while (hd() != Token::RightBrace && hd() != Token::EOS && n-- != 0) {
            ...
            if (t == Token::Var || t == Token::Let || (t == Token::Const && hd2() != Token::Function)) {
                ...
                e = dynamic
            override
                (beta = allowIn, tau = localBlk)
                variableDefinition(makeAttrs(Ast::publicNS, attrs));
                semicolon(fullStmt);
                if (e != null)
                    stmts.push(new Ast::ExprStmt(e));
                continue;
            ...
            dynamic override
                (omega = fullStmt, tau = localBlk)
                functionDefinition(makeAttrs(Ast::publicNS, attrs));
                continue;
            }
    ...
    function computeAttributes(f: FuncAttr) {
        ...
        for (let i = 0, limit = f.children.length;
        i < limit;
        i++
    )
        {
    ...
    function program(define_namespaces = true): Ast
    ...
    ::Program
    {
        ...
        let
        internalNamespace = new Ast::InternalNamespace(Token::intern("<#internal " + filename + ">"));
        ...
        let
        global_directives = directivesGlobal();
        let
        vhead = cx.exitGlobal();
