...
function cgEval(tree: PROGRAM, name: Token
...
function cgFixtures(ctx, fixtures) {
    ...
    let
    {
        slots, use_regs, scope_reg
    }
    = getVariableDefinitionScope(ctx);

    if (use_regs) {
        for (let i = 0, limit = fixtures.length;
        i < limit;
        i++
    )
        {
    ...
    )
        {
            ...
            if (fx is ValFixture) {
                ...
                checkTrait(fxname, name, TRAIT_Slot, true);
                let
                ifaceidx = cgInterface(ctx, fx.iface);
...
function cgStmts(ctx, stmts) {
    for (let i = 0, limit = stmts.length;
    i < limit;
    i++
)
    cgStmt(ctx, stmts[i]);
...
function cgClass(ctx, c) {
    ...
    let
    classname = emitter.qname(c.name, false);
    let
    basename = cgTypeExpr(ctx, c.baseName);
    let
    interfacenames = Util::map((function (n)
    cgTypeExpr(ctx, n)
),
    c.interfaceNames
)
    ...
    let
    cls = script.newClass(classname, basename, interfacenames, flags, c.protectedns);

    let
    class_ctx = pushClass(ctx, cls);
    ...
    let
    instance_ctx = pushInstance(ctx, inst);
    ...
    cinit = cls.getCInit();
    let
    cinit_ctx = pushCInit(ctx, cinit);  // shares asm with ctx
...
function cgInterface(ctx, c) {
    ...
    let
    ifacename = emitter.qname(c.name, false);
    let
    interfacenames = Util::map((function (n)
    cgTypeExpr(ctx, n)
),
    ...
    let
    iface = script.newInterface(ifacename, ctx.cp.symbolUtf8(c.name.id), interfacenames);

    let
    ifaceidx = iface.finalize();
...
function cgCtor(ctx, c, instanceInits) {
    ...
    f = c.func;
    let
    formals_type = extractFormalTypes(ctx, f);
    let
    method = new Method(ctx.script.e, formals_type, ctx.cp.stringUtf8("$construct"), false, f.attr);

    let
    defaults = extractDefaultValues(ctx, f);
    ...
    if (!f.attr.is_native) {
        ...
        let
        ctor_ctx = pushIInit(ctx, t, method);
        ...
        for (let i = 0, limit = c.settings.length;
        i < limit;
        i++
    )
        {
...
emitter.fixtureTypeToType(data)
),
...
function cgFunc(ctx0, f: Func) {
    ...
    = ctx0;
    let
    use_regs = early_binding && !f.attr.reify_activation;
    let
    fntype = ctx0.stk != null && (ctx0.stk.tag == "instance" || ctx0.stk.tag == "class") ? "method" : "function";  // brittle as hell
    let
    formals_types = extractFormalTypes({emitter: emitter, script: script}, f);
    let
    name = null;
    if (f.name == null) {
        ...
        name = f.name.ident;
    let
    method = new Method(emitter, formals_types, cp.symbolUtf8(name), fntype != "function", f.attr);

    let
    defaults = extractDefaultValues({emitter: emitter, script: script}, f);
    if (defaults.length > 0)
        method.setDefaults(defaults);
    ...
    if (!f.attr.is_native) {
        ...
        let
        fnctx = pushFunction(ctx0, fntype, scope_reg, capture_reg, (fntype != "function"), use_regs, method);
        cgDebugFile(fnctx);
...
function cgOptimizedParams(fnctx, f: Func) {
    ...
    for (let i = 0, limit = params.length;
    i < limit;
    ++i
)
    {
...
function cgHead(ctx, head) {
    ...
    let
    named_fixtures = extractNamedFixtures(head.fixtures);
    cgFixtures(ctx, named_fixtures);
    for (let i = 0, limit = head.exprs.length;
    i < limit;
    i++
)
    {
...
function cgInits(ctx, inits, baseOnStk) {
    ...
    for (let i = 0, limit = inits.length;
    i < limit;
    ++i
)
    {
        ...
        let
        bind = findBinding(ctx, n, new Ast::NamespaceSetList(new Ast::NamespaceSet(ns, null), null));
...
)
{
    ...
    while (stk != null) {
        ...
        if (stk.slots) {
            //print(stk.slots);
            for (let tmp_nssl = nssl;
            tmp_nssl != null;
            tmp_nssl = tmp_nssl.link
        )
            {
                for (let nss = tmp_nssl.nsset;
                nss != null;
                nss = nss.link
            )
                {
                    let
                    ns = slots.getBinding(name, nss.ns);
                    if (ns != null) {
                        if (bind == null)
                            bind = ns;
                        else if (ns != bind)
...
,
function_type, scope_reg, capture_reg, push_this, use_regs, func
)
new CTX(func.asm,
    {
        tag: "function",
...
new CTX(asm,
    {tag: "finally", label: label, returnreg: returnreg, returnAddresses: [], nextReturn: 0, link: stk},
    target);
