...
internal final class Rib {
    ...
    (Ast::Expr | Ast::Namespace);
    var strict: Boolean;
    var open_namespaces: Ast
    ::NamespaceSetList;
    const head: Ast
    ::Head;
    const link: Rib;
...
final class Definer {
    ...
    function enterFunction(attrs): void {
        let
        node = new Ast::FuncAttr(top_function);
     ...
     */
    function resolveNamespaceExpr(ident: NamespaceExpr): Ast
    ...
    function resolveTypeNameExpr(ident: Ast
    ...
    function resolveIdentExpr(ident: Ast
...
:
    ...
    Ast::FixtureName {
        if (ident is Ast::Identifier) {
            let
            fx = findFixtureWithIdentifier(ident.ident, it);
            if (fx == null)
    ...
    private function findFixtureWithIdentifier(id: Ast
...
:
    ...
    {
        for (let nssl = getOpenNamespaces();
        nssl != null;
        nssl = nssl.link
    )
        {
            let
            fx = findFixtureWithNames(id, nssl.nsset, it);
            if (fx != null)
                return fx;
        }
        return null;
    ...
    private function findFixtureWithNames(id, nss, it: ?
...
)
    {
        ...
        while (env != null) {
            let
            result = findFixtureWithNamesInHead(env.head, id, nss);
            if (result != null)
                return result;
            env = env.link;
        }
    ...
    private function findFixtureWithNamesInHead(head, id, nss: Ast
    ...
    (function (a, b)
    a.id === b.id && Ast::nsEquals(a.ns, b.ns)
),
    null
)
    ...
    private function extendTable(cache, fxtrs) {
        for (let i = cache.length, limit = fxtrs.length;
        i < limit;
        i++
    )
        {
    ...
    private function hasName(head, id, ns) {
        ...
        if (cache == null) {
            let
            fxtrs = head.fixtures;
            for (let i = 0, limit = fxtrs.length;
            i < limit;
            i++
        )
            {
