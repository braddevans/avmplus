...
class AbcEncoderCpp {
    ...
    function AbcEncoderCpp(filename: string, v: Boolean = false): fname
    ...
    = filename, verbose = v
    {
        ...
        let
        path_bits = fname.split(/[\/\\]/);
        basename = path_bits[path_bits.length - 1];
    }

    function encode() {
        let
        items = process_abc(abc);
        ...
        for each (var item in items) {
            ...
            bits = bits.filter(function (elt, ndx) {
                return elt.length != 0;
            })
            // convert dots or colons to underscores.
            bits.every(function (elt, ndx) {
                bits[ndx] = elt.replace(/[\.:]/g, "_");
                return true;
            });
    ...
    function process_traits(prefixes: Array, traits) {
    ...
    )
        {
            ...
            switch (kind) {
                ...
                case TRAIT_Setter:
                    let
                    suffix = kind == TRAIT_Getter ? 'get' : (kind == TRAIT_Setter ? 'set' : '');
                    let
                    mi = abc.methods[t.val];
                    if (mi.flags & METHOD_Native) {
                        // 'private' names get special treatment apparently!
                        let
                        is_private = constants.getConstantNameType(t.name) == CONSTANT_PrivateNamespace;
                        if (is_private) {
                ...
                case TRAIT_Class:
                    ...
                    let
                    ABCClassInfo = abc.classes[t.val];
                    let
                    new_prefixes = prefixes.concat([trait_ns, trait_name])
                    let
                    Array = process_traits(new_prefixes, klass.traits)
                    ret = ret.concat(sub);
                    break;
    ...
    function process_abc(abc: ABCFile): Array {
    ...
    )
        {
            ...
            let
            iprefixes = constants.getConstantName(i.name);

            // do the "instance init" function if its native.
            let
            iinit = abc.methods[i.iinit]
...

{
    ...
    let
    encoder = new AbcEncoderCpp(fname, verbose);

    let [h_data, cpp_data] = encoder.encode();
    let
    prefix = "// This is a GENERATED file - do not edit\n";
