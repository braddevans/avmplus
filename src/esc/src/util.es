...
function hash_string(s: String) {
    ...
    for (var i = 0, limit = s.length; i < limit; i++)
        h = ((h << 5) + h) + s.
    "http://adobe.com/AS3/2006/builtin"::charCodeAt(i);
    return uint(h);
}
...
class Names {
    ...
    function Names() {
        table = new Hashtable((function (sym)
        sym.hash
    ),
        (function (a, b)
        a === b
