...
)
{
    ...
    cgExpr(ctx, test);
    let
    L0 = asm.I_iffalse(undefined);
    cgStmt(ctx, consequent);
    if (alternate != null) {
        let
        L1 = asm.I_jump(undefined);
        asm.I_label(L0);
...
)
{
    ...
    Lbreak = asm.newLabel();
    let
    Lcont = asm.I_jump(undefined);
    let
    Ltop = asm.I_label(undefined);
...
function cgForStmt(ctx, {
    init, cond, incr, stmt, labels
}
...
function cgForInStmt(ctx, {
    assignment, tmp, obj, stmt, is_each, labels
}
...
function cgSwitchStmt(ctx, s) {
    let
    fastswitch = analyzeSwitch(ctx, s);
    if (!fastswitch)
...
function analyzeSwitch(ctx, s) {
    ...
    let
    has_default = false;
    for (let i = 0, limit = cases.length;
    i < limit;
    i++
)
    {
...
function cgSwitchStmtFast(ctx, s, low, high, has_default) {
    ...
    Ldef = asm.newLabel();
    let
    Lcases = new Array(high - low + 1);
    let
    Lbreak = asm.newLabel();
    let
    nctx = pushBreak(ctx, Lbreak);
    let
    ldef_emitted = false;
    ...
    let
    Lhandled = new Array(Lcases.length);
    for (let i = 0, limit = cases.length;
    i < limit;
    i++
)
...
)
    {
        ...
        if (e == null) {
            ...
            ldef_emitted = true;
            for (let j = 0, jlimit = Lhandled.length;
            j < jlimit;
            j++
        )
            if (!Lhandled[j])
        ...
        let
        stmts = c.stmts;
        for (let j = 0, jlimit = stmts.length;
        j < jlimit;
        j++
    )
        cgStmt(nctx, stmts[j]);
...
)
{
    ...
    = ctx;
    let
    t = new Ast::GetCogenTemp();
    t.n = asm.getTemp();
    cgExpr(ctx, expr);
    asm.I_setlocal(t.n);
    let
    b = new Ast::Block(new Ast::Head([], []), [new Ast::ThrowStmt(t)]);
    let
    newcases = [];
    let
    hasDefault = false;
...
function cgTryStmtWithFinally(ctx, {
...
)
{
    let
    {
        asm, emitter, target, cp
    }
    = ctx;

    let
    returnreg = asm.getTemp();
    let
    Lfinally = asm.newLabel();
    let
    newctx = pushFinally(ctx, Lfinally, returnreg);
    let
    rib = newctx.stk;

    let
    myreturn = rib.nextReturn++;
    rib.returnAddresses[myreturn] = null; // aka "Lreturn"; will be initialized below

    let
    Lend = asm.newLabel();
    let
    myend = rib.nextReturn++;
    rib.returnAddresses[myend] = Lend;

    let
    code_start = asm.length;
    ...
    let
    savedExn = asm.getTemp();
    let
    catch_idx = target.addException(new ABCException(code_start, code_end, asm.length, 0, 0));
    ...
    let
    {
        numvisits, returnAddresses
    :
        visitors
    }
    = rib;
    let
    Lcases = new Array(numvisits);
...
function cgTryStmtNoFinally(ctx, {
    block, catches
}
...
)
{
    ...
    for (let i = 0, limit = catches.length;
    i < limit;
    ++i
)
...
function cgCatch(ctx, code_start, code_end, Lend, {
    param, block
}
...
)
{
    ...
    let
    param_name = emitter.fixtureNameToName(propname);
    let
    param_type = emitter.realTypeName(fix.ty);

    let
    catch_idx = target.addException(new ABCException(code_start, code_end, asm.length, param_type, param_name));
