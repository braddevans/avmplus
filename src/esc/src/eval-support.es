...
ESC function evaluateInScopeArray(args, scopes, scopedesc, strict) {
    if (args.length > 0) {
        if (args[0] is String) {
            ...
            file = "(EVAL CODE)";
            let
            name = Token::intern("$eval$" + id + "$");
            let
            parser = new Parse::Parser(args[0], ESC::getTopFixtures(), file);
            let
            prog = parser.program(false);

            // These checks could be moved into the parser, and might be, by and by.

            for (let fs = prog.head.fixtures, limit = fs.length, i = 0;
            i < limit;
            i++
        )
            {
