...
internal function compile(consume, produce, context, start_line) {
    ...
    input = consume();
    let
    parser = new Parse::Parser(input, getTopFixtures(), context, start_line);
    let
    prog = parser.program();
...
compile((function ()
Util::readStringFromFile(fname)
),
(function (abc)
Util::writeBytesToFile(abc.getBytes(), fname + ".abc")
),
fname,
    start_line
)
...
function parseFromStringAndEncodeAst(input, context = "string input"): String {
    let
    parser = new Parse::Parser(input, getTopFixtures(), context);
    let
...
function decodeAstFromString(input): Ast
