...
class ABCEmitter {
    ...
    function flattenNamespaceSet(nssl: Ast
...
,
    ...
    function defaultLiteralExpr(lit) {
        switch type(lit) {
            ...
            {
                let
                val = constants.float64(ld.doubleValue);
            ...
            )
            {
                ...
                let
                val = constants.float64(ld.decimalValue);
                return {val: val, kind: CONSTANT_Double};
            }
            case(li:
                Ast::LiteralInt
            ...
            {
                let
                val = constants.int32(li.intValue);
            ...
            {
                let
                val = constants.uint32(lu.uintValue);
            ...
            {
                let
                val = (lb.booleanValue ? CONSTANT_True : CONSTANT_False);
            ...
            {
                let
                val = constants.symbolUtf8(ls.strValue);
            ...
            {
                let
                val = constants.namespace(ln.namespaceValue);
...
class Class extends TraitsTable {
    ...
    function finalize() {
        let
        instidx = instance.finalize();
        let
        clsinfo = new ABCClassInfo();
        ...
        let
        clsidx = s.e.file.addClass(clsinfo);
