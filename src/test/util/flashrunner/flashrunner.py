...
ostype = \
{'CYGWIN_NT-5.1': 'win', 'CYGWIN_NT-5.2': 'win', 'CYGWIN_NT-6.0-WOW64': 'win', 'Windows': 'win', 'Darwin': 'mac',
 'Linux': 'lnx', 'Solaris': 'sol', }[platform.system()]
if ostype == 'win':
    import win32api
...
ff = \
{"win": "c:/Program Files/Mozilla Firefox/firefox.exe", "lnx": "/usr/bin/firefox", "mac": "/Applications/Firefox.app"}[
    ostype]
...
if os.path.exists(swffile) == False or os.path.getmtime(infile) > os.path.getmtime(swffile):
    ...
    ascline = "java -jar %s -import %s -import %s -swf %s,550,400 %s" % (
    environ['ASC'].strip(), environ['BUILTINABC'].strip(), environ['PLAYERGLOBALABC'].strip(), classname, outfile)
