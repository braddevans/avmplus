    ...
    if insertMeasurements:
        ...
        for measurement in measurements:
            ...
            for i in range(len(lines)):
                ...
                if re.search('<measurement>', lines[i]) and measurement[0] == currentclass and measurement[
                    1] == currentmetric:
                    lines.insert(i + 1, '<measurement><name>%s</name><value>%s</value></measurement>' % (
            ...
            if found == False:
                ...
                lines.insert(lasttest + 1, '<testcase classname="%s.%s" name="%s" time="0">' % (
                toplevel, measurement[0], measurement[1]))
                lines.insert(lasttest + 2, '<system-out>')
                lines.insert(lasttest + 3, '<![CDATA[')
                lines.insert(lasttest + 4, '<measurement><name>%s</name><value>%s</value></measurement>' % (
                measurement[2], measurement[3]))
        ...
        for measurement in measurements:
            ...
            for i in range(len(lines)):
                newcontents += lines[i] + '\n'
                if i > 2 and re.search('classname="%s.%s".* name="%s"' % (toplevel, measurement[0], measurement[1]),
                                       lines[i - 2]):
                    found = True
        ...
        fixForXmlCdata(systemout), fixForXmlCdata(systemerr))
...
def convertAotToJunit(infile, outfile, toplevel):
        ...
        if line.startswith('AOT compiling'):
            if test != None:
                if test in skiplist:
                    ...
                    skip = ''
                testresults.append(
                    {'test': test, 'time': time, 'out': out, 'skip': skip, 'fail': fail, 'class': classname,
                     'name': testname, 'failure': error})
            error = ''
            test = tokens[2]
    ...
    for testresult in testresults:
        contents += '  <testcase classname="%s.%s-%s" name="%s" time="%s">' % (
        properties['config'], properties['config'], testresult['class'], testresult['name'], testresult['time'])
...
def convertAcceptanceToJunit(infile, outfile, toplevel):
    ...
    for testresult in testresults:
        out = ''
        contents += '  <testcase classname="%s.%s" name="%s" time="%s">' % (
        properties['configuration'], testresult['class'], testresult['name'], testresult['time'])
        if 'out' in testresult:
            out += testresult['out'] + '\n'
        if 'skip' in testresult:
            if not 'failure' in testresult:
                contents += '<skipped message="%s" type="%s">%s</skipped>\n  ' % (
                fixForXmlEscape(testresult['skipmessage']), testresult['skiptype'],
                fixForXmlCdata(testresult['skipout']))
