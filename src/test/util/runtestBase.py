...
from .runtestUtils import walk, TimeOutException, join, \
    parseArgStringToList, splitList, genAtsArgs, moveAtsSwf, search_file, create_ats_swfversion_copy
...
class RuntestBase(object):
    ...
    def determineConfig(self):
        ...
        try:
            ...
            elif (re.search('Bourne-Again shell script', f) or re.search('python script', f)) and re.search('android',
                                                                                                            self.avm):
                cputype = 'arm'
                self.osName = 'android'
            ...
            try:
                cputype = {'AMD64': 'x86', 'x86': 'x86', 'i386': 'x86', 'i686': 'x86', 'x86_64': 'x64', 'i86pc': 'x86',
                           'Power Macintosh': 'ppc', 'sun4u': 'x86', 'mips': 'mips', 'armv7l': 'arm', 'sh4': 'sh4',
                           '': 'x86'}[platform.machine()]
                if cputype == 'x86' and splitext(self.avm)[0][-2:] == '64':
                    cputype == 'x64'
            except:
                print(
                    "ERROR: cpu_arch '%s' is unknown, expected values are (x86,ppc), use runtests.py --config x86-win-tvm-release to manually set the configuration" % (
                        platform.machine()))
                exit(1)
    ...
    def determineOS(self):
        ...
        if ostype == '':
            print(
                "ERROR: os %s is unknown, expected values are (win,mac,lnx,sol), use runtests.py --config x86-win-tvm-release to manually set the configuration" % (
                    platform.system()))
            exit(1)
    ...
    def createOutputFile(self):
        ...
        if not self.js_output:
            for i in count(1):
                self.js_output = '%d-%s-%s.%d.%s' % (
                now.year, str(now.month).zfill(2), str(now.day).zfill(2), i, self.logFileType)
    ...
    def compile_aot(self, abcfile, extraabcs=[]):
        ...
        cmd = '%s -Xshell -Xoutput %s %s %s %s' % (
        os.path.join(self.aotsdk, 'bin/adt'), output, self.aotextraargs, " ".join(copiedExtraAbcs), outabc)
    ...
    def compile_aot_hm(self, abcfile, extraabcs=[]):
        ...
        cmd = '%s/bin/compile-shell-abc.py %s --sdk=%s --arch=%s --output=%s %s %s %s' % (
        self.aotsdk, wrapper_verbose, self.aotsdk, arch_name, output_name, " ".join(self.harness_abcs),
        " ".join(extraabcs), abcfile)
        self.verbose_print(cmd)
    ...
    def compile_test(self, as_file, extraArgs=[], outputCalls=None):
        ...
        if as_file.endswith(self.sourceExt):
            if not isfile(builtinabc):
                exit(
                    'ERROR: builtin.abc (formerly global.abc) %s does not exist, BUILTINABC environment variable or --builtinabc must be set to builtin.abc' % builtinabc)
    ...
    def compileWithAsh(self, tests):
        ...
        if not pexpect_module_present or self.ascOutputOnly:
            ...
            if isfile(self.ash):
                ...
                child = pexpect.spawn(
                    '"%s" %s -classpath %s macromedia.asc.embedding.Shell' % (self.java, self.javaargs, self.asc))
                    ...
                    if javaArgList:
                        ...
                        if self.genAtsSwfs:
                            ...
                            if not exists(testdir + ".abc"):
                                ...
                                self.ashErrors.append(
                                    "abc file %s.abc not created, cmd used to compile: %s" % (testdir, cmd))
        ...
        for p, dirs, files in walk(support_dir):
            for f in files:
                if f.endswith(self.sourceExt):
                    f = join(p, f)
                    binFile = splitext(f)[0] + '.abc'
                    if exists(binFile) and (
                            self.forcerebuild or (self.timestampcheck and getmtime(f) > getmtime(binFile))):
                        os.unlink(binFile)
                    if not isfile(binFile):
    ...
    def parseAscArgs(self, currentArgList, ascArgFile, currentdir):
        ...
        if ascargs[1].find('$SHELLABC') != -1:
            if not isfile(self.shellabc):  # TODO: not the best place to check for this
                exit(
                    'ERROR: shell.abc %s does not exist, SHELLABC environment variable or --shellabc must be set to shell_toplevel.abc' % self.shellabc)
    ...
    def runTests(self, testList):
        ...
        if self.verify:
            ...
            if not isfile(self.abcdump + '.abc'):  # check that abcdump.abc is built
                self.run_pipe('"%s" -jar %s -import %s -import %s %s' % (
                self.java, self.asc, self.builtinabc, self.shellabc, self.abcdump + '.as'))
    ...
    def setupCEEmulators(self):
        ...
        if os.path.isfile(emulator) == False:
            print(
                "ERROR: device emulator does not exist '%s', set EMULATOR environment variable to the correct emulator" % emulator)
            sys.exit(1)
        if os.path.isfile(emulator_image) == False:
            print(
                "ERROR: device emulator image does not exist, '%s', set EMULATORIMAGE environment variable to the correct emulator image" % emulator_image)
            sys.exit(1)
        if os.path.isfile(cerunner) == False:
            print(
                "ERROR: cerunner tool does not exist '%s', set CERUNNER environment variable to the correct path to cerunner.exe" % cerunner)
            sys.exit(1)
        ...
        for num in range(emthreads):
            if os.path.isfile("%s/share%d/running.txt" % (shared, num)):
                ...
                args = [emulator] + emulator_args.split() + ['/sharedfolder', '%s/share%d' % (shared, num),
                                                             emulator_image]
