...
Assert.expectError("li16(Domain.MIN_DOMAIN_MEMORY_LENGTH) prior to initMemory()",
    ...
    function () {
        LI16(Domain.MIN_DOMAIN_MEMORY_LENGTH);
    });
...
Assert.expectError("li16(-1)", Utils.RANGEERROR + 1506, function () {
    LI16(-1);
});
Assert.expectError("li16(mem.length)", Utils.RANGEERROR + 1506, function () {
    LI16(mem.length);
});
Assert.expectError("li16(mem.length-1)", Utils.RANGEERROR + 1506, function () {
    LI16(mem.length - 1);
});
