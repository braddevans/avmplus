...
Assert.expectError("lf32(Domain.MIN_DOMAIN_MEMORY_LENGTH) prior to initMemory()",
    ...
    function () {
        LF32(Domain.MIN_DOMAIN_MEMORY_LENGTH);
    });
...
Assert.expectError("lf32(-1)", Utils.RANGEERROR + 1506, function () {
    LF32(-1);
});
Assert.expectError("lf32(mem.length)", Utils.RANGEERROR + 1506, function () {
    LF32(mem.length);
});
Assert.expectError("lf32(mem.length-1)", Utils.RANGEERROR + 1506, function () {
    LF32(mem.length - 1);
});
Assert.expectError("lf32(mem.length-2)", Utils.RANGEERROR + 1506, function () {
    LF32(mem.length - 2);
});
Assert.expectError("lf32(mem.length-3)", Utils.RANGEERROR + 1506, function () {
    LF32(mem.length - 3);
});
