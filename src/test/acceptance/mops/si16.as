...
Assert.expectError("si16(Domain.MIN_DOMAIN_MEMORY_LENGTH) prior to initMemory()",
    Utils.RANGEERROR + 1506,
    function () {
        SI16(0x0111, Domain.MIN_DOMAIN_MEMORY_LENGTH);
    });
...
clearMemory();
Assert.expectError("si16(0x0101, -1)", Utils.RANGEERROR + 1506, function () {
    SI16(0x0101, -1);
});
Assert.expectError("si16(0x0101, mem.length)", Utils.RANGEERROR + 1506, function () {
    SI16(0x0101, mem.length);
});
Assert.expectError("si16(0x0101, mem.length-1)", Utils.RANGEERROR + 1506, function () {
    SI16(0x0101, mem.length - 1);
});
