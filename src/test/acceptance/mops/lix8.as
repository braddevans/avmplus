...
Assert.expectError("lix8(Domain.MIN_DOMAIN_MEMORY_LENGTH) prior to initMemory()",
    ...
    function () {
        LIX8(Domain.MIN_DOMAIN_MEMORY_LENGTH);
    });
...
Assert.expectError("lix8(-1)", Utils.RANGEERROR + 1506, function () {
    LIX8(-1);
});
Assert.expectError("lix8(mem.length)", Utils.RANGEERROR + 1506, function () {
    LIX8(mem.length);
});
