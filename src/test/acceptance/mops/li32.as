...
Assert.expectError("li32(Domain.MIN_DOMAIN_MEMORY_LENGTH) prior to initMemory()",
    ...
    function () {
        LI32(Domain.MIN_DOMAIN_MEMORY_LENGTH);
    });
...
Assert.expectError("li32(-1)", Utils.RANGEERROR + 1506, function () {
    LI32(-1);
});
Assert.expectError("li32(mem.length)", Utils.RANGEERROR + 1506, function () {
    LI32(mem.length);
});
Assert.expectError("li32(mem.length-1)", Utils.RANGEERROR + 1506, function () {
    LI32(mem.length - 1);
});
Assert.expectError("li32(mem.length-2)", Utils.RANGEERROR + 1506, function () {
    LI32(mem.length - 2);
});
Assert.expectError("li32(mem.length-3)", Utils.RANGEERROR + 1506, function () {
    LI32(mem.length - 3);
});
