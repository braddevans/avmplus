...
Assert.expectError("li8(Domain.MIN_DOMAIN_MEMORY_LENGTH) prior to initMemory()",
    Utils.RANGEERROR + 1506,
    function () {
        LI8(Domain.MIN_DOMAIN_MEMORY_LENGTH);
    });
...
Assert.expectError("li8(-1)", Utils.RANGEERROR + 1506, function () {
    LI8(-1);
});
Assert.expectError("li8(mem.length)", Utils.RANGEERROR + 1506, function () {
    LI8(mem.length);
});
