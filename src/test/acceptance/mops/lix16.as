...
Assert.expectError("lix16(Domain.MIN_DOMAIN_MEMORY_LENGTH) prior to initMemory()",
    ...
    function () {
        LIX16(Domain.MIN_DOMAIN_MEMORY_LENGTH);
    });
...
Assert.expectError("lix16(-1)", Utils.RANGEERROR + 1506, function () {
    LIX16(-1);
});
Assert.expectError("lix16(mem.length)", Utils.RANGEERROR + 1506, function () {
    LIX16(mem.length);
});
Assert.expectError("lix16(mem.length-1)", Utils.RANGEERROR + 1506, function () {
    LIX16(mem.length - 1);
});
