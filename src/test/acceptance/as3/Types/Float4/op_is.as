...
var pi_float4: Object = new float4(3.14
f
)
...
Assert.expectEq("float(3.14f) is NOT float4", false, float(3.14
f
) is
float4
)
...
var three: float4 = float4(3
f
)

Assert.expectEq("float4(3f) is NOT int", false, three is int);

var minus_three: float4 = float4(-3
f
)
...
var zero: float4 = float4(0
f
)

var neg_zero: float4 = float4(-0
f
)
...
Assert.expectEq("Number(Infinity) is NOT float4", false, (Number(Infinity)) is float4);
