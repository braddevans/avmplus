...
Assert.expectError("float.floor() with no args", Utils.ARGUMENTERROR + 1063, function () {
    float.floor();
});
...
AddStrictTestCase("float.floor(false)", 0
f, float.floor(false)
)

AddStrictTestCase("float.floor('0')", 0
f, float.floor('0')
)
...
AddStrictTestCase("float.floor(true)", 1
f, float.floor(true)
)

AddStrictTestCase("float.floor('1')", 1
f, float.floor('1')
)
