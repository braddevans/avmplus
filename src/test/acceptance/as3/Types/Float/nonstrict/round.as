...
Assert.expectError("float.round() with no args", Utils.ARGUMENTERROR + 1063, function () {
    float.round();
});
...
AddStrictTestCase("float.round(false)", 0
f, float.round(false)
)


AddStrictTestCase("float.round(true)", 1
f, float.round(true)
)
