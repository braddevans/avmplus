...
Assert.expectError("float.ceil() with no args", Utils.ARGUMENTERROR + 1063, function () {
    float.ceil();
});
...
AddStrictTestCase("float.ceil(true)", 1
f, float.ceil(true)
)

AddStrictTestCase("float.ceil(false)", 0
f, float.ceil(false)
)


AddStrictTestCase("float.ceil('1')", 1
f, float.ceil('1')
)

AddStrictTestCase("float.ceil('0')", 0
f, float.ceil('0')
)
