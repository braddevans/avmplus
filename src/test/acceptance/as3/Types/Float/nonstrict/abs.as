...
Assert.expectError("float.abs() with no args", Utils.ARGUMENTERROR + 1063, function () {
    float.abs();
});

AddStrictTestCase("float.abs(undefined)", float.NaN, float.abs(undefined));
AddStrictTestCase("float.abs(true)", 1
f, float.abs(true)
)

AddStrictTestCase("float.abs(false)", 0
f, float.abs(false)
)


AddStrictTestCase("float.abs('1')", 1
f, float.abs('1')
)

AddStrictTestCase("float.abs('0')", 0
f, float.abs('0')
)
