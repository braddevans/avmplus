...
Assert.expectError("float.atan() with no args", Utils.ARGUMENTERROR + 1063, function () {
    float.atan();
});
...
AddStrictTestCase("float.atan(true)", float.PI / 4
f, float.atan(true)
)

AddStrictTestCase("float.atan(false)", 0
f, float.atan(false)
)



AddStrictTestCase("float.atan('1')", float.PI / 4
f, float.atan('1')
)

AddStrictTestCase("float.atan('0')", 0
f, float.atan('0')
)
