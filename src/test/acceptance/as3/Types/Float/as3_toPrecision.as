...
test_flt = 2.7182818284e5
f;
test_num = test_flt;
Assert.expectEq("test float.AS3::toPrecision(2) float literal", test_num.AS3::toPrecision(2), 2.7182818284e5
f.AS3::toPrecision(2)
)
...
try {
    test_flt.AS3::toPrecision(0);
} catch (e: RangeError) {
    expectedRangeErr = Utils.rangeError(e.toString());
}
Assert.expectError("float.AS3::toPrecision() == float.AS3::toPrecision(0)", expectedRangeErr, function () {
    return test_flt.AS3::toPrecision();
});
...
Assert.expectError("float.AS3::toPrecision(undefined)", Utils.RANGEERROR, function () {
    test_flt.AS3::toPrecision(undefined)
});
Assert.expectError("float.AS3::toPrecision(NaN): RangeError", Utils.RANGEERROR, function () {
    test_flt.AS3::toPrecision(NaN)
});
Assert.expectError("float.AS3::toPrecision(0): RangeError", Utils.RANGEERROR, function () {
    test_flt.AS3::toPrecision(0)
});
Assert.expectError("float.AS3::toPrecision(22): RangeError ", Utils.RANGEERROR, function () {
    test_flt.AS3::toPrecision(22)
});
