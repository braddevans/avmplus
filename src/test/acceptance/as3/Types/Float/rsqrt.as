...
function check(param: float): float {
    return float.rsqrt(param);
}

AddStrictTestCase("float.rsqrt() returns a float", "float", getQualifiedClassName(float.rsqrt(12.345
f
)))
...
AddStrictTestCase("float.rsqrt(-1.125f)", float.NaN, float.rsqrt(-1.125
f
))

AddStrictTestCase("float.rsqrt(-1.125f) check()", float.NaN, check(-1.125
f
))
...
AddStrictTestCase("float.rsqrt(0f)", float.POSITIVE_INFINITY, float.rsqrt(0
f
))

AddStrictTestCase("float.rsqrt(0f) check()", float.POSITIVE_INFINITY, check(0
f
))
...
// If x is -0, the result is -0. (1/-0 == -Infinity)
AddStrictTestCase("float.rsqrt(-0f)", float.NEGATIVE_INFINITY, float.rsqrt(-0
f
))

AddStrictTestCase("float.rsqrt(-0f) check()", float.NEGATIVE_INFINITY, check(-0
f
))


// If x is +Infinity, the result is +Infinity. (1/Infinity == 0)
AddStrictTestCase("float.rsqrt(float.POSITIVE_INFINITY)", 0
f, float.rsqrt(float.POSITIVE_INFINITY)
)

AddStrictTestCase("float.rsqrt(float.POSITIVE_INFINITY) check()", 0
...
var x: float = 3.124
f;
AddStrictTestCase("x=3.124f float.rsqrt(x) == 1f/float.sqrt(x)", 1
f / float.sqrt(x), float.rsqrt(x)
)
