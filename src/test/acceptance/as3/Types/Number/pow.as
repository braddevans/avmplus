...
function check(param1: Number, param2: Number): Number {
    return Number.pow(param1, param2);
}
...
Assert.expectError("Number.pow() with no args", Utils.ARGUMENTERROR + 1063, function () {
    Number.pow();
});
Assert.expectError("Number.pow() with one args", Utils.ARGUMENTERROR + 1063, function () {
    Number.pow(1);
});
