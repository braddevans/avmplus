...
Assert.expectEq("Set private var:int", -99, (OBJ.y = 23334, OBJ.y));
Assert.expectEq("Get private var:Array", "7,8,9", OBJ.x.toString());
Assert.expectEq("Set private var:Array", "7,8,9", (OBJ.x = [4, 5, 6], OBJ.x.toString()));
...
Assert.expectEq("Call a parent getter from a child getter with super", "4,5,6", OBJ.childX.toString());

// call a parent setter from a child getter
Assert.expectEq("Call a parent setter from a child getter with super", 1976, OBJ.childY);

// call a parent getter from a child setter
OBJ.childZ = "FAILED";
Assert.expectEq("Call a parent getter from a child setter", "new string", OBJ.childZ);

// call a parent setter from a child setter
OBJ.childW = "PASSED";
Assert.expectEq("Call a parent getter from a child getter", "PASSED", OBJ.childW);
