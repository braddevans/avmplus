...
function getTestCases() {
    ...
    function addNewTestCase(DateCase, DateString, ResultArray) {
        ...
        array[item++] = Assert.expectEq("Date.parse( + DateCase.toString() +)", Math.floor(ResultArray[TIME] / 1000) * 1000, Date.parse(DateCase.toString()));

        var mydate = new Date(0, 0, 0, 0, 0, 0, 0);
        array[item++] = Assert.expectEq("mydate.valueOf()==Date.parse(mydate.toString())", true, mydate.valueOf() == Date.parse(mydate.toString()));

        array[item++] = Assert.expectEq("mydate.valueOf()==Date.parse(mydate.toUTCString())", true, mydate.valueOf() == Date.parse(mydate.toUTCString()));

        array[item++] = Assert.expectEq("Date.parse(mydate.toString())==Date.parse(mydate.toUTCString())", true, Date.parse(mydate.toString()) == Date.parse(mydate.toUTCString()));


        //    array[item++] = Assert.expectEq(  "Date.parse(" + DateCase.toGMTString() +")", Math.floor(ResultArray[TIME]/1000)*1000,  Date.parse(DateCase.toGMTString()) );

        // Adding the test case vector for toUTCString instead of the toGMTString, if it is required later on.
        array[item++] = Assert.expectEq("Date.parse(" + DateCase.toUTCString() + ")", Math.floor(ResultArray[TIME] / 1000) * 1000, Date.parse(DateCase.toUTCString()));
