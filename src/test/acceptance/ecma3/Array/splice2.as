...
function getTestCases() {
    ...
    function exhaustiveSpliceTestWithArgs(testname, testArray) {
        ...
        for (var first = -(testArray.length + 2); first <= 2 + testArray.length; first++) {
            ...
            for (var len = 0; len < testArray.length + 2; len++) {
                ...
                actualRemoved = actualSpliced.splice(first, len, -97, String("test arg"), [], 9.8);
                expectedRemoved = mySplice(testArray, expectedSpliced, first, len, [-97, String("test arg"), [], 9.8]);
    ...
    var a = ['a', 'test string', 456, 9.34, String("string object"), [], ['h', 'i', 'j', 'k']];
