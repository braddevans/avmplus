...
function getTestCases() {
    ...
    array[item++] = Assert.expectEq("!(new String(''))", true, !(String("")));
    array[item++] = Assert.expectEq("!(new String('string'))", false, !(String("string")));
    array[item++] = Assert.expectEq("!(new String())", true, !(String()));
    array[item++] = Assert.expectEq("!(new Boolean(true))", false, !(Boolean(true)));
    array[item++] = Assert.expectEq("!(new Boolean(false))", true, !(Boolean(false)));
    array[item++] = Assert.expectEq("!(new Array())", false, !([]));
    array[item++] = Assert.expectEq("!(new Array(1,2,3)", false, !([1, 2, 3]));
    array[item++] = Assert.expectEq("!(new Number())", true, !(Number()));
    array[item++] = Assert.expectEq("!(new Number(0))", true, !(Number(0)));
    array[item++] = Assert.expectEq("!(new Number(NaN))", true, !(Number(Number.NaN)));
    array[item++] = Assert.expectEq("!(new Number(Infinity))", false, !(Number(Number.POSITIVE_INFINITY)));
