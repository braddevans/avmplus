...
Assert.expectEq("JSON.parse(1,reviver)", "44", JSON.parse("1", function (name, val) {
    return "44"
}));
Assert.expectEq("JSON.parse([1,-2,3],negate reviver)", "[-1,2,-3]", JSON.parse("[1,-2,3]", function (name, val) {
    if (val is Number) return -val; else return val;
}).toString());
Assert.expectEq("JSON.parse([1,-2,3],undef neg reviver)", JSON.parse("[1,-2,3]", function (name, val) {
    if (val is Number && val < 0) return undefined; else return val;
}).toString(), "[1,,3]");
// not sure why?
Assert.expectEq("JSON.parse([1,-2,3],undef 1 reviver)", "[\"1\",,3]", JSON.parse("[\"1\",-2,3]", function (name, val) {
    if (name == "1") return undefined; else return val;
}).toString());
Assert.expectEq("JSON.parse({a:{\"1\":20},c:[1,2,3]})", "{\"a\":{\"1\":20},\"c\":[[,\"2\",3]]}",
    sortObject(JSON.parse("{\"a\":{\"1\":20},\"c\":[1,\"2\",3]}", function (name, val) {
        if (val == "1") return undefined; else return val;
    })));
...
Assert.expectEq("JSON.stringify(array,replacer=function)", "[1,null,3,null,false,null]", JSON.stringify(JSON.parse("[1,2,3,true,false,null]"), function (key, value) {
    if (!(parseInt(key) & 1)) return value;
    return undefined;
}));
Assert.expectEq("JSON.stringify(object,replacer=function)", "{\"a\":[object Object],\"c\":[-1,2,-3]}", sortObject(JSON.parse(JSON.stringify(JSON.parse("{\"a\":{\"b\":20},\"c\":[1,\"2\",3]}")), function (key, value) {
