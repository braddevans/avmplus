...
Assert.expectEq("15.12.3-5-a-i-1: JSON.stringify converts Number wrapper object space aruguments to Number values", true, JSON.stringify(obj, null, Number(5)) === JSON.stringify(obj, null, 5));

Assert.expectEq("15.12.3-5-b-i-1: JSON.stringify converts String wrapper object space aruguments to String values", true, JSON.stringify(obj, null, String('xxx')) === JSON.stringify(obj, null, 'xxx'));
...
try {
    Assert.expectEq("15.12.3-8-a-4: JSON.stringify treats an Boolean wrapper space argument the same as a missing space argument.", true, JSON.stringify(obj) === JSON.stringify(obj, null, Boolean(true)));
} catch (e) {
...
try {
    Assert.expectEq("15.12.3-8-a-5: JSON.stringify treats non-Number or String object space arguments the same as a missing space argument.", true, JSON.stringify(obj) === JSON.stringify(obj, null, Boolean(true)));
} catch (e) {
...
// [fortytwo] ??
Assert.expectEq("15.12.3-2-3-a-1: JSON.stringify converts string wrapper objects returned from replacer functions to literal strings.", '["fortytwo"]', JSON.stringify([42], function (k, v) {
    return v === 42 ? String('fortytwo') : v
}));

Assert.expectEq("15.12.3-2-3-a-2: JSON.stringify converts Number wrapper objects returned from replacer functions to literal numbers.", '[84]', JSON.stringify([42], function (k, v) {
    return v === 42 ? Number(84) : v
}));

Assert.expectEq("15.12.3-2-3-a-3: JSON.stringify converts Boolean wrapper objects returned from replacer functions to literal numbers.", '[false]', JSON.stringify([42], function (k, v) {
    return v === 42 ? Boolean(false) : v
}));


// See Bugzilla 654574 for info on tests 15.12.3-4-b-1 through 15.12.3-4-b-10

Assert.expectEq("15.12.3-4-b-1: JSON.stringify replacer array has negative integer already converted to string.", '{"-1":18}', JSON.stringify({
    "-0": 17,
    "-1": 18,
    0: 19
}, ["-1"]));
Assert.expectEq("15.12.3-4-b-2: JSON.stringify replacer array has negative integer not yet converted to string.", '{"-1":18}', JSON.stringify({
    "-0": 17,
    "-1": 18,
    0: 19
}, [-1]));
Assert.expectEq("15.12.3-4-b-3: JSON.stringify replacer array has negative zero already converted to string.", '{"-0":17}', JSON.stringify({
    "-0": 17,
    "-1": 18,
    0: 19
}, ["-0"]));
Assert.expectEq("15.12.3-4-b-4: JSON.stringify replacer array has negative zero not yet converted to string.", '{"0":19}', JSON.stringify({
    "-0": 17,
    "-1": 18,
    0: 19
}, [-0]));
...
Assert.expectEq("15.12.3-4-b-7: JSON.stringify replacer array has repeated entries.", '{"a":1,"b":2}', JSON.stringify({
    a: 1,
    b: 2,
    c: 3
}, ["a", "b", "b", "a"]));
...
var objkey = {"key": 3};
Assert.expectEq("15.12.3-4-b-8: JSON.stringify replacer array has non-string non-numberentries.", '{"a":1,"b":2}', JSON.stringify({
    a: 1,
    b: 2,
    c: 3
}, ["a", "b", objkey]));
...
gappy_array[0] = 'a';
gappy_array[100] = 'b';
Assert.expectEq("15.12.3-4-b-9: JSON.stringify replacer array has gaps.", '{"a":1,"b":2}', JSON.stringify({
    a: 1,
    b: 2,
    c: 3
}, gappy_array));

Assert.expectEq("15.12.3-4-b-10: JSON.stringify replacer array nesting of keys.", '{"a":1,"b":{"b":3,"d":2}}', JSON.stringify({
    a: 1,
    b: {d: 2, b: 3},
    c: 4
}, ["a", "b", "d"]));
