...
public final class md5_t {
    ...
    private function F(x: uint, y: uint, z: uint): uint {
        return (x & y) | ((~x) & z);
    }

    private function G(x: uint, y: uint, z: uint): uint {
        return (x & z) | (y & (~z));
    }

    private function H(x: uint, y: uint, z: uint): uint {
        return (x ^ y ^ z);
    }

    private function I(x: uint, y: uint, z: uint): uint {
        return (y ^ (x | (~z)));
    }
