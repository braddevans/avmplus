...
class AcceptanceRuntest(RuntestBase):
    ...
    def run(self):
        ...
        if self.rebuildtests == False and (
                re.search('arm-winmobile-emulator', self.config) != None or self.osName == 'winmobile'):
            if re.search('^arm-winmobile-emulator', self.config) == None:
                print(
                    'ERROR: to use windows mobile build set --config arm-winmobile-emulator-tvm-release or install cygwin utility /usr/bin/file.exe')
                sys.exit(1)
    ...
    def runTest(self, ast, root, testName, testnum, settings, extraVmArgs='', abcargs='', multiabc=''):
        ...
        if self.aotsdk and self.aotout:
            ...
            if not exists(self.abcasmShell + '.abc'):  # compile abcasmShell with no additional args
                self.run_pipe(
                    '"%s" -jar %s -import %s %s' % (self.java, self.asc, self.builtinabc, self.abcasmShell + '.as'),
                    outputCalls=outputCalls)
            (f, err, exitcode) = self.run_pipe(
                '%s %s %s %s %s' % (self.avm, self.vmargs, extraVmArgs, self.abcasmShell + '.abc', testName),
                outputCalls=outputCalls, envVars=passByEnv)
        elif self.verify:
            ...
            if abcargs:
                (f, err, exitcode) = self.run_pipe(
                    '%s %s %s %s %s -- %s' % (self.avm, self.vmargs, extraVmArgs, multiabc, testName, abcargs),
                    outputCalls=outputCalls, envVars=passByEnv)
            elif multiabc:
                (f, err, exitcode) = self.run_pipe(
                    '%s %s %s %s %s' % (self.avm, self.vmargs, extraVmArgs, multiabc, testName),
                    outputCalls=outputCalls, envVars=passByEnv)
            else:
        ...
        if self.verifyonly:
            ...
            if isfile(root + '.out'):
                ...
                for line in f + err:
                    ...
                    if skipTestDesc:
                        outputCalls.append(
                            (self.js_print, ('  skipping "%s" ... reason: %s' % (line.strip(), skipTestDesc),)))
                        self.allskips += 1
                        continue
                    if 'PASSED!' in line:
                        ...
                        if res:
                            outputCalls.append((self.fail, (
                            testName, 'unexpected pass: ' + line.strip() + ' reason: ' + res, self.unpassmsgs)))
                            lunpass += 1
                        else:
                            lpass += 1
                    if 'FAILED!' in line:
                        ...
                        if res:
                            outputCalls.append((self.fail, (testName,
                                                            'expected failure: ' + line.strip().replace('FAILED',
                                                                                                        'failed') + ' reason: ' + res,
                                                            self.expfailmsgs)))
                            lexpfail += 1
                        else:
            ...
            # exitcode check
            ec_lfail, ec_lexpfail, expectedExitcode = self.check_exit_code(exitcode, root, testName, f, err, settings,
                                                                           outputCalls)
            if ec_lfail or ec_lexpfail:
                lfail += ec_lfail
                lexpfail += ec_lexpfail
                ...
                outputCalls.append(
                    (self.fail, (testName, "unexpected stderr expected:'%s' actual:'%s'" % ('', err), self.failmsgs)))
                ...
                if res:
                    outputCalls.append((self.fail, (
                    testName, 'expected failure: FAILED contained no testcase messages reason: %s' % res,
                    self.expfailmsgs)))
                    lexpfail += 1
                else:
                    ...
                    if expectedExitcode != 0 and exitcode == expectedExitcode:
                        ...
                        for l in outputLines:
                            outputmsg += l.strip() + '|'
                        outputCalls.append((self.fail, (
                        testName, '   FAILED contained no testcase messages - reason: %s' % outputmsg, self.failmsgs)))
        ...
        if self.show_time:
            if self.passthreadid:
                outputCalls.insert(0, (self.js_print, ('%s running %s %s %s threadid=%s time %.1f' % (
                testnum, ast, extraVmArgs, abcargs, passByEnv['threadid'], time.time() - starttime), '<b>',
                                                       '</b><br/>')));
            else:
                outputCalls.insert(0, (self.js_print, (
                '%s running %s %s %s time %.1f' % (testnum, ast, extraVmArgs, abcargs, time.time() - starttime), '<b>',
                '</b><br/>')));
        else:
            if self.passthreadid:
                outputCalls.insert(0, (self.js_print, (
                '%s running %s %s %s threadid=%s' % (testnum, ast, extraVmArgs, abcargs, passByEnv['threadid']), '<b>',
                '</b><br/>')));
            else:
                outputCalls.insert(0, (
                self.js_print, ('%s running %s %s %s' % (testnum, ast, extraVmArgs, abcargs), '<b>', '</b><br/>')));
    ...
    def check_exit_code(self, exitcode, root, testName, f, err, settings, outputCalls):
        ...
        if exitcode != expectedExitcode:
            ...
            if res2 == None and res:
                ...
                outputCalls.append(
                    (self.fail, (testName, 'unexpected exit code expected:%d actual:%d Signal Name: %s FAILED!'
