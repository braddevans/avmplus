...
class RunSmokes():
    ...
    def run(self):
        ...
        if self.timeout > 0:
            if time.time() - self.startTime < self.timeout:
                print('\ntests finished after %d seconds, did not exceed timeout of %d seconds' % (
                time.time() - self.startTime, self.timeout))
            else:
                print('\nexceeded timeout of %d seconds, actual time %d seconds' % (
                self.timeout, time.time() - self.startTime))
        else:
