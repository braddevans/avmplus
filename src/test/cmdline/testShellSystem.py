...
def run():
    r = RunTestLib()
    r.run_test('trace', '%s testdata/trace1.abc' % r.avm,
               expectedout=['single line', 'array line1,array line2', 'arg1 arg2 arg3', 'null'])
    ...
    r.run_test('argv n', '%s testdata/argv.abc -- zero one two"' % r.avm,
               expectedout=['argv.length=3', 'argv\[0\]=zero', 'argv\[1\]=one', 'argv\[2\]=two'])
    ...
    r.run_test('memlimit no value', '%s -memlimit' % r.avm, expectedout=['Invalid GC option: -memlimit'],
               expectedcode=1)
    ...
    r.run_test('cache_bindings no value', '%s -cache_bindings' % r.avm,
               expectedout=['Unrecognized option -cache_bindings'], expectedcode=1)
    r.run_test('cache_metadata no value', '%s -cache_metadata' % r.avm,
               expectedout=['Unrecognized option -cache_metadata'], expectedcode=1)
    r.run_test('cache_methods no value', '%s -cache_methods' % r.avm,
               expectedout=['Unrecognized option -cache_methods'], expectedcode=1)
    ...
    if r.avmrd != None:
        r.run_test('Dastrace no value', '%s -Dastrace' % r.avmrd, expectedout=['Unrecognized option -Dastrace'],
                   expectedcode=1)
        r.run_test('Dlanguage no value', '%s -Dlanguage' % r.avmrd, expectedout=['Unrecognized option -Dlanguage'],
                   expectedcode=1)
