...
def run():
    ...
    if os.name != 'nt':
        print(
            "pyspy     requires windows named pipes, does not work with cygwin python or non-windows operating systems")
    else:
        failed = False
        try:
            import win32event
        ...
        if failed == False:
            ...
            while True:
                try:
                    readHandle = win32file.CreateFile(pipe, win32file.GENERIC_READ, 0, None, win32file.OPEN_EXISTING, 0,
                                                      None)
