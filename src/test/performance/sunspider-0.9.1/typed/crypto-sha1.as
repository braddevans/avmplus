...
function hex_hmac_sha1(key: String, data) {
    return binb2hex(core_hmac_sha1(key, data));
}

function b64_hmac_sha1(key: String, data) {
    return binb2b64(core_hmac_sha1(key, data));
}

function str_hmac_sha1(key: String, data) {
 ...
 */
function core_sha1(x: Array, len: uint): Array {
    /* append padding */
...
function binb2b64(binarray: Array): String {
    ...
    for (var i: uint = 0; i < binarray.length * 4; i += 3) {
        var triplet: uint = (((binarray[i >> 2] >> 8 * (3 - i % 4)) & 0xFF) << 16)
