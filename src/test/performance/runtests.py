...
'''
    ...
    def usage(self, c):
        ...
        print(
            "    --logConfigAppend= string to append to the config string that is logged to the database along with vmargs")
        exit(c)
    ...
    def compile_test(self, as_file):
        if not isfile(self.shellabc):
            exit(
                "ERROR: shell.abc %s does not exist, SHELLLABC environment variable or --shellabc must be set to shell.abc" % self.shellabc)
        args = []
    ...
    def preProcessTests(self):
        ...
        if self.logresults:
            ...
            tmpvmargs = self.vmargs.replace("-AOTSIZE",
                                            "")  # This is a fake vmarg that is used for reporting filesize, do not log to db
        ...
        if self.avm2:
            ...
            if self.iterations == 1:
                ...
                if self.detail:
                    ...
                    self.js_print('%-*s  %7s :%7s  %7s %6s    %7s :%7s  %7s %6s %7s %8s' % (
                    self.testFieldLen, 'test', 'min', 'max', 'avg', 'stdev', 'min', 'max', 'avg', 'stdev', '%diff',
                    'sig  '))
                    self.js_print(
                        '%*s ---------------------------------   ---------------------------------   -----  --------' % (
                        self.testFieldLen, ''))
    ...
    def calculateSpeedup(self, testName, resultDict, resultDict2):
        ...
        for metric in metrics:
            ...
            # Store the results
            self.testData.setdefault(testName, {}).setdefault(metric, {}).setdefault('results1', []).extend(
                resultDict[metric])
            ...
            if resultDict2:
                ...
                # Store the results
                self.testData.setdefault(testName, {}).setdefault(metric, {}).setdefault('results2', []).extend(
                    resultDict2[metric])
                # calculate the best result
        ...
        for arg in scriptargs:
            ...
            for i in range(self.iterations):
                if self.aotsdk and self.aotout:
                    ...
                    self.debug_print(f1)
                if self.avm2:
                    (f2, err2, exitcode2) = self.run_pipe(
                        "%s %s %s %s" % (self.avm2, self.vmargs2 if self.vmargs2 else self.vmargs, abc, scriptArg))
                    self.debug_print(
                        "%s %s %s %s" % (self.avm2, self.vmargs2 if self.vmargs2 else self.vmargs, abc, scriptArg))
                    self.debug_print(f2)
                try:
                    ...
                    if self.avm2:
                        if exitcode2 != 0:
                            self.finalexitcode = 1
                            self.js_print(
                                "%-50s %7s %s" % (testName, 'Avm2 Error: Test Exited with exit code:', exitcode))
                            return
                        else:
        ...
        if self.avm2:
            if self.iterations == 1:
                if numMetrics == 1:
                    ...
                    for metric in testData[testName].keys():
                        self.printSingleIterationComparison('    %s' % self.metricInfo[metric].get('name', metric),
                                                            testName, metric)
            else:  # multiple iterations
                if numMetrics == 1:
                    ...
                    self.js_print(desc)
                    for metric in testData[testName].keys():
                        self.printMultiIterationComparison('    %s' % self.metricInfo[metric].get('name', metric),
                                                           testName, metric)
            ...
            if self.iterations == 1:
                if numMetrics == 1:
                    ...
                    for metric in testData[testName].keys():
                        self.js_print(
                            '    %-*s %*s' % (self.testFieldLen - 2, self.metricInfo[metric].get('name', metric),
                                              self.resultWidth,
                                              self.formatResult(testData[testName][metric]['best1'], metric=metric)))
                ...
                if numMetrics == 1:
                    self.js_print(('%-*s %*s %*s %4.1f%% %s') %
                                  (self.testFieldLen, self.truncateDescField(desc), self.resultWidth,
                                   ...
                                   [self.formatResult(x, metric=metric) for x in
                                    self.testData[testName][metric]['results1']] if self.raw else ''
                                   ))
                else:  # numMetrics > 1
                    self.js_print(desc)
                    for metric in testData[testName].keys():
    ...
    def printSingleIterationComparison(self, descStr, testName, metric):
        ...
        if pythonVersion26():
            ...
            self.js_print('%-*s %5s %7s %6.1f %6.1f' % (self.testFieldLen, self.truncateDescField(descStr),
                                                        self.formatResult(self.testData[testName][metric]['best1'],
                                                                          metric=metric),
                                                        self.formatResult(self.testData[testName][metric]['best2'],
                                                                          metric=metric),
                                                        spdup, avg_spdup))
            ...
            self.js_print('%-*s [%7s :%7s] %7s ±%4.1f%%   [%7s :%7s] %7s ±%4.1f%% %6.1f%% %6.1f %2s %s %s' %
                          (self.testFieldLen, self.truncateDescField(descStr),
                           ...
                           [self.formatResult(x, metric=metric) for x in
                            self.testData[testName][metric]['results1']] if self.raw else '',
                           [self.formatResult(x, metric=metric) for x in
                            self.testData[testName][metric]['results2']] if self.raw else ''
                           ))
        elif pythonVersion26():
                    ...
                    for iteration, value in enumerate(metricDict['results2']):
                        csvwriter.writerow(avm2Options + \
                                           [testname, metric,
                                            self.metricInfo[metric].get('unit', ''),
                                            iteration + 1,
                                            # use iteration+1 so that iteration values are not zero based
                                            value])
