...
var DESC = "Compute float.rsqrt of a large Vector.<float>";
include "driver.as"

function float_rsqrt(iter: int, a: Vector.<float>): float {
    ...
    for (var i: int = 0; i < iter; i++) {
        for (var j: int = 0; j < 1000;) {
            tmp1 = a[j];
            tmp1 = float.rsqrt(tmp1);
            j++;
            tmp2 = a[j];
            tmp2 = float.rsqrt(tmp2);
            j++;
            tmp3 = a[j];
            tmp3 = float.rsqrt(tmp3);
            j++;
            tmp4 = a[j];
            tmp4 = float.rsqrt(tmp4);
            j++;
            tmp5 = a[j];
            tmp5 = float.rsqrt(tmp5);
            j++;
        }
    }
