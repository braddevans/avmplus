...
var DESC = "Compute float4.distance3 of a large Vector.<float4>";
include "driver.as"

function float4_distance3(iter: int, a: Vector.<float4>): float4 {
    ...
    for (var i: int = 0; i < iter; i++) {
        ...
        for (var j: int = 0; j < 1000;) {
            tmp2 = a[j];
            tmp1 = float4.distance3(tmp1, tmp2);
            j++;
            tmp2 = a[j];
            tmp1 = float4.distance3(tmp1, tmp2);
            j++;
