...
var DESC = "Compute float4.rsqrt of a large Vector.<float4>";
include "driver.as"

function float4_rsqrt(iter: int, a: Vector.<float4>): float4 {
    ...
    for (var i: int = 0; i < iter; i++) {
        for (var j: int = 0; j < 1000;) {
            tmp1 = a[j];
            tmp1 = float4.rsqrt(tmp1);
            j++;
            tmp2 = a[j];
            tmp2 = float4.rsqrt(tmp2);
            j++;
            tmp3 = a[j];
            tmp3 = float4.rsqrt(tmp3);
            j++;
            tmp4 = a[j];
            tmp4 = float4.rsqrt(tmp4);
            j++;
            tmp5 = a[j];
            tmp5 = float4.rsqrt(tmp5);
            j++;
        }
    }
