...
function float_reciprocal(iter: int, a: Vector.<float>): float {
    ...
    for (var i: int = 0; i < iter; i++) {
        for (var j: int = 0; j < 1000;) {
            tmp1 = a[j];
            tmp1 = float.reciprocal(tmp1);
            j++;
            tmp2 = a[j];
            tmp2 = float.reciprocal(tmp2);
            j++;
            tmp3 = a[j];
            tmp3 = float.reciprocal(tmp3);
            j++;
            tmp4 = a[j];
            tmp4 = float.reciprocal(tmp4);
            j++;
            tmp5 = a[j];
            tmp5 = float.reciprocal(tmp5);
            j++;
        }
    }
