...
var DESC = "Read a small number of aligned bytes from ByteArray into ByteArray.  Compare bytearray-read-bytes-2.";
include "driver.as"
...
function bytearray_read_bytes(iter: int, a: ByteArray): uint {
    ...
    tmp3.length = 8000;
    var tmp4: ByteArray = new ByteArray;
    tmp4.length = 8000;
    var tmp5: ByteArray = new ByteArray;
    tmp5.length = 8000;
    for (var i: int = 0; i < iter; i++) {
        a.position = 0;
        var k: int = 0;
        for (var j: int = 0; j < 1000; j++) {
            a.readBytes(tmp1, k, 8);
            a.readBytes(tmp2, k, 8);
            a.readBytes(tmp3, k, 8);
            a.readBytes(tmp4, k, 8);
            a.readBytes(tmp5, k, 8);
            k += 8;
        }
    }
...
TEST(function () {
    bytearray_read_bytes(250, bai);
}, "bytearray-read-bytes-1");
