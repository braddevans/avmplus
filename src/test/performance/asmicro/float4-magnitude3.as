...
function float4_magnitude3(iter: int, a: Vector.<float4>): float4 {
    ...
    for (var i: int = 0; i < iter; i++) {
        for (var j: int = 0; j < 1000;) {
            tmp1 = a[j];
            tmp1 = float4.magnitude3(tmp1);
            j++;
            tmp2 = a[j];
            tmp2 = float4.magnitude3(tmp2);
            j++;
            tmp3 = a[j];
            tmp3 = float4.magnitude3(tmp3);
            j++;
            tmp4 = a[j];
            tmp4 = float4.magnitude3(tmp4);
            j++;
            tmp5 = a[j];
            tmp5 = float4.magnitude3(tmp5);
            j++;
        }
    }
