...
var DESC = "Compute float.sqrt of a large Vector.<float>";
include "driver.as"

function float_sqrt(iter: int, a: Vector.<float>): float {
    ...
    for (var i: int = 0; i < iter; i++) {
        for (var j: int = 0; j < 1000;) {
            tmp1 = a[j];
            tmp1 = float.sqrt(tmp1);
            j++;
            tmp2 = a[j];
            tmp2 = float.sqrt(tmp2);
            j++;
            tmp3 = a[j];
            tmp3 = float.sqrt(tmp3);
            j++;
            tmp4 = a[j];
            tmp4 = float.sqrt(tmp4);
            j++;
            tmp5 = a[j];
            tmp5 = float.sqrt(tmp5);
            j++;
        }
    }
