...
QualifiedName *Parser::nameExpression(bool is_attr) {
  ...
  if (match(T_DoubleColon)) {
    ...
    return ALLOC(QualifiedName,
                 (n, ALLOC(SimpleName, (identifier())), is_attr, pos));
  } else {
...
Expr *Parser::propertyOperator(Expr *obj) {
  ...
  case T_Dot: {
    ...
    if (hd() == T_Namespace) {
      eat(T_Namespace);
      return ALLOC(ObjectRef,
                   (obj,
                    ALLOC(QualifiedName,
                          (NULL, ALLOC(SimpleName, (compiler->SYM_namespace)),
                           false, pos)),
                    pos));
    }
    // End keyword hacks
  ...
  case T_LeftBracket: {
    ...
    return ALLOC(
        ObjectRef,
        (obj,
         ALLOC(QualifiedName, (NULL, ALLOC(ComputedName, (expr)), false, pos)),
         pos));
  }
  case T_LeftDotAngle: {
    ...
    if (obj->tag() == TAG_qualifiedName) {
      ...
      rightAngle();
      next();
      return ALLOC(InstantiatedTypeRef,
                   (ALLOC(InstantiatedType, (basename, t2)), pos));
    }
  ...
  default:
    compiler->internalError(
        position(), "Unexpected token in propertyOperator: %d", (int)hd());
    /*NOTREACHED*/
    return NULL;
  }
}
...
Expr *Parser::callExpression() {
  ...
  return callExpressionPrime(
      ALLOC(CallExpr, (object_expr, argument_exprs, pos)));
}
...
Expr *Parser::callExpressionPrime(Expr *call_expr) {
  ...
  case T_LeftParen: {
    ...
    Seq<Expr *> *argument_exprs = argumentList();
    return callExpressionPrime(
        ALLOC(CallExpr, (call_expr, argument_exprs, pos)));
  }
  case T_LeftBracket:
  case T_Dot:
  case T_LeftDotAngle:
...
Expr *Parser::newExpression(int new_count) {
  ...
  if (hd() ==
      T_LeftParen) { // No more new exprs so this paren must start a call expr
    Seq<Expr *> *argument_exprs = argumentList();
    if (new_count > 0)
      return ALLOC(NewExpr, (call_expression, argument_exprs));
    return callExpressionPrime(
        ALLOC(CallExpr, (call_expression, argument_exprs, pos)));
  }
...
Expr *Parser::multiplicativeExpression() {
  ...
  while (isMultiplicative(t = hd()) || t == T_BreakSlash) {
    ...
    next();
    expr =
        ALLOC(BinaryExpr, (tokenToBinaryOperator(t), expr, unaryExpression()));
  }
  return expr;
}
...
Expr *Parser::additiveExpression() {
  ...
  while (isAdditive(t = hd())) {
    next();
    expr = ALLOC(BinaryExpr,
                 (tokenToBinaryOperator(t), expr, multiplicativeExpression()));
  }
  return expr;
}
...
Expr *Parser::shiftExpression() {
  ...
  while (isShift(t = hd()) || t == T_BreakLeftAngle || t == T_BreakRightAngle) {
    ...
    next();
    expr = ALLOC(BinaryExpr,
                 (tokenToBinaryOperator(t), expr, additiveExpression()));
  }
...
Expr *Parser::relationalExpression(int flags) {
  ...
  for (;;) {
    if (!(isRelational(t = hd(), allowIn) || t == T_BreakLeftAngle ||
          t == T_BreakRightAngle))
      break;
    if (t == T_BreakLeftAngle) {
      leftShiftOrRelationalOperator();
    ...
    next();
    expr =
        ALLOC(BinaryExpr, (tokenToBinaryOperator(t), expr, shiftExpression()));
  }
...
Expr *Parser::equalityExpression(int flags) {
  ...
  while (isEquality(t = hd())) {
    next();
    expr = ALLOC(BinaryExpr,
                 (tokenToBinaryOperator(t), expr, relationalExpression(flags)));
  }
  return expr;
}
...
Expr *Parser::bitwiseXorExpression(int flags) {
  ...
  while (match(T_BitwiseXor))
    expr =
        ALLOC(BinaryExpr, (OPR_bitwiseXor, expr, bitwiseAndExpression(flags)));
  return expr;
}
...
Expr *Parser::bitwiseOrExpression(int flags) {
  ...
  while (match(T_BitwiseOr))
    expr =
        ALLOC(BinaryExpr, (OPR_bitwiseOr, expr, bitwiseXorExpression(flags)));
  return expr;
}
...
Expr *Parser::logicalAndExpression(int flags) {
  ...
  while (match(T_LogicalAnd))
    expr =
        ALLOC(BinaryExpr, (OPR_logicalAnd, expr, bitwiseOrExpression(flags)));
  return expr;
}
...
Expr *Parser::logicalOrExpression(int flags) {
  ...
  while (match(T_LogicalOr))
    expr =
        ALLOC(BinaryExpr, (OPR_logicalOr, expr, logicalAndExpression(flags)));
  return expr;
}
...
Expr *Parser::assignmentExpression(int flags) {
  ...
  if (!((t = hd()) == T_Assign || isOpAssign(t) || t == T_BreakLeftAngle ||
        t == T_BreakRightAngle))
    return lhs;
