...
void Cogen::unstructuredControlFlow(Ctx *ctx, bool(hit)(Ctx *, void *),
                                    void *package, bool jump, SyntaxError msg,
                                    uint32_t pos) {
  while (ctx != NULL) {
    if (hit(ctx, package)) {
      if (jump)
...
// Syntax analysis and code generation for goto are intertwined.  That may or
// may not be a great idea, but here's how it works:
//
// When we enter a Program body or Function body we create an initial set of
// legal goto targets, this set is empty and provides the base case for the
// analysis that occurs within the body. There will be a list of these sets
// chained through the goto contexts on the stack.
//
// When we get to a statement list (Block, Cases, Group, Program body, Function
// body) we walk the statement list and look for labeled statements in the list
// (never within statements in the list).  The labels on these statements will
// comprise the set of legal "top-level" (non-nesting) labels for that statement
// list.  We create a new goto context for the set, and we enter the labels into
// that context.  Each entry in the set has the label name, a Label pointer, and
// a flag saying whether the entry is ambiguous.  It is ambiguous if the label
// name is also found in one of the sets we inherit from nesting contexts (up to
// a program or function body).  The labeled statements we find are updated: we
// store the Label pointer in each of their "address" members.  This pointer
// will serve as a flag when code is generated for the labeled statement (next
// paragraph).
//
// When we process a labeled statement, one of two things happens.  If the
// "address" member is non-null then this is a top-level labeled statement and
// we do not create a new goto context, as one already exists.  Otherwise, we
// create a new goto context containing just the label for this labeled
// statement; the entry may again be ambiguous.  We create an "address" value
// too. In either case, we emit the label stored in the address node.
//
// When we process a goto statement, we search the goto stack for the context
// node that contains the label.  If the label is not found, we signal an error.
// If the label is found but marked ambiguous, we signal an error.  Otherwise we
// emit a nonlocal jump to the node (without control transfer) and then jump to
// the target address, which may not be emitted yet (but backpatching will take
// care of that).
//
// Goto contexts are chained through the next_goto field.  If a goto context is
// not found on the stack when we want to process a statement list then the
// function or program body does not use "goto" and the analysis can be skipped
// entirely.  There's a small cost to pay to discover that, but in general the
// overhead of "goto" on programs not using it is tiny, as context lists are
// normally quite short.
//
// (The cost of checking for duplicates is quadratic in the number of labels in
// a function, roughly: hardly ideal.  A hashtable of all known labels in the
// function with occurence counts could be created in linear time in the parser,
// and if the occurence count on a label is 1 we would never need to search.
// Such a table could probably even be created on-line in the code generator.)

// Search for the nearest "goto" context starting with c.

static GotoCtx *lookupGoto(Ctx *c) {
  while (c->tag != CTX_Program && c->tag != CTX_Function &&
         c->tag != CTX_ClassMethod && c->tag != CTX_Goto)
    c = c->next;
  if (c->tag == CTX_Goto)
    return (GotoCtx *)c;
  return NULL;
...
static bool searchForDuplicateLabel(Str *label, Seq<GotoLabel *> *inner_labels,
                                    GotoCtx *outer_goto) {
...
// If there are any labeled statements in the statement list then populate
// goto_ctx and return it, otherwise return outer_ctx.

static Ctx *maybePopulateGotoCtx(Cogen *cogen, Seq<Stmt *> *stmts,
                                 GotoCtx *goto_ctx, Ctx *outer_ctx) {
  ...
  for (Seq<Stmt *> *ss = stmts; ss != NULL; ss = ss->tl) {
    ...
    while (s->isLabeledStmt()) {
      ...
      labels =
          ALLOC(Seq<GotoLabel *>,
                (ALLOC(GotoLabel, (ls->label, address, ambiguous)), labels));
      ls->address = address;
      s = ls->stmt;
    }
  }
...
// If there are any labeled statements in the statement lists within the case
// clauses then populate goto_ctx and return it, otherwise return outer_ctx.

static Ctx *maybePopulateGotoCtx(Cogen *cogen, Seq<CaseClause *> *cases,
                                 GotoCtx *goto_ctx, Ctx *outer_ctx) {
  GotoCtx *outer_goto = lookupGoto(outer_ctx);
  if (outer_goto == NULL)
    return outer_ctx;
...
static Ctx *maybePopulateGotoCtx(Cogen *cogen, LabeledStmt *stmt,
                                 GotoCtx *goto_ctx, Ctx *outer_ctx) {
  GotoCtx *outer_goto = lookupGoto(outer_ctx);
  if (outer_goto == NULL)
    return outer_ctx;
  ...
  Allocator *allocator = cogen->allocator;
  goto_ctx->label_names =
      ALLOC(Seq<GotoLabel *>,
            (ALLOC(GotoLabel, (stmt->label, stmt->address, ambiguous))));
  goto_ctx->next_goto = outer_goto;
  return goto_ctx;
}

// Process labeled statements and generate code for the statements.  This must
// not be used to generate code for case bodies in switch, but is fine for
// blocks and function and program bodies.
...
void LabeledStmt::cogen(Cogen *cogen, Ctx *ctx) {
  ...
  if (!address)
    ctx2 =
        maybePopulateGotoCtx(cogen, this, &gctx, &ctx1); // may update address
  if (address)
...
void ExprStmt::cogen(Cogen *cogen, Ctx *ctx) {
  ...
  Ctx *c;
  for (c = ctx; c->tag != CTX_Program && c->tag != CTX_Function &&
                c->tag != CTX_ClassMethod;
       c = c->next)
    ;
...
void ForInStmt::cogen(Cogen *cogen, Ctx *ctx) {
  ...
  (ALLOC(AssignExpr, (OPR_assign, lhs, ALLOC(RefLocalExpr, (T_val)))))
      ->cogen(cogen, ctx);
  cogen->I_pop();
...
void BreakStmt::cogen(Cogen *cogen, Ctx *ctx) {
  cogen->unstructuredControlFlow(
      ctx, hitBreak, (void *)label, true,
...
void ContinueStmt::cogen(Cogen *cogen, Ctx *ctx) {
  cogen->unstructuredControlFlow(ctx, hitContinue, (void *)label, true,
                                 (label == NULL
...
static bool hitGoto(Ctx *ctx, void *_package) {
  ...
  return ctx->tag == CTX_Goto &&
         matchLabel(ctx, package->label, (void **)&(package->infoloc));
}
...
void GotoStmt::cogen(Cogen *cogen, Ctx *ctx) {
  ...
  cogen->unstructuredControlFlow(ctx, hitGoto, (void *)&package, false,
                                 SYNTAXERR_GOTO_LABEL_UNDEFINED, pos);
...
static bool hitFunction(Ctx *ctx, void *) { return ctx->tag == CTX_Function; }
...
void ReturnStmt::cogen(Cogen *cogen, Ctx *ctx) {
  ...
  cogen->unstructuredControlFlow(ctx, hitFunction, NULL, false,
...
// OPTIMIZEME: we can do better here for switches that are sparse overall (cover
// a large range) but which have significant dense segments.  Consider a scanner
// that handles unicode: it may have a lot of cases for values in the ASCII
// range and then a few cases to handle unicode outliers, like unicode space
// characters.  It will fail the 'fast' test but would benefit from being
// rewritten as a dense switch whose default case switches further on the
// outlying values.
...
bool SwitchStmt::analyze(int32_t *low, int32_t *high) {
  ...
  *high = (-0x7FFFFFFF - 1);
  ...
  for (Seq<CaseClause *> *cases = this->cases; cases != NULL;
       cases = cases->tl) {
...
void SwitchStmt::cogenFast(Cogen *cogen, Ctx *ctx, int32_t low, int32_t high) {
  ...
  // Ldefault is the default case (whether or not there is a default in the
  // switch)
  ...
  Lcase = (Label **)avmStackAllocArray(compiler->context->core, _Lcase, ncases,
                                       sizeof(Label *));
#endif
  ...
  //
  // OPTIMIZEME: avoid type tests if the type is known and if we've verified
  // that the JIT does not perform that optimization.  (It might, but it would
  // have to track the type from the original expression through tmp, and ignore
  // all the coerce_a instructions, so it's unlikely that it's currently
  // performing it.)
  ...
  cogen->I_getlocal(tmp);
  cogen->I_coerce_i(); // not redundant, the representation could have been
                       // Number
  ...
  for (Seq<CaseClause *> *cases = this->cases; cases != NULL;
    ...
    if (e == NULL) {
      ...
      // There might be duplicate case selector values, but only the first one
      // counts.
      if (Lcase[v] != NULL) {
...
//   records the fact that there is a finally block, so that exits to the
//   outside of the try/catch block by means of break/continue (labelled or not)
//   must visit the finally block (in inside-out order if there are several)
...
// code bloat it is generated out-of-line.  Visiting is done by setting a
// register to the "return" address, then jumping to the finally code, which
// ends with a switch statement that jumps back to all the possible return
// points.
//
...
// all the code that can visit it (represented as a list of id/labels in the ctx
// rib).
//
...
void TryStmt::cogenWithFinally(Cogen *cogen, Ctx *ctx) {
  uint32_t returnreg =
      cogen->getTemp(); // the local that records the index of the label this
                        // finally block should return to
  ...
  // Lreturn is the address in this finally block that visited, outer finally
  // blocks return to after executing to completion.
  Label *Lreturn = cogen->newLabel();
  uint32_t myreturn = fctx.addReturnLabel(Lreturn);

  // Lend is the address to branch to to get out of the finally block after
  // entering the finally block normally and executing it without throwing any
  // exceptions.
  ...
  // Fallthrough from try-catch: visit the finally block.  This code must not be
  // in the scope of the generated exception handler.
  ...
  /*uint32_t catch_idx =*/cogen->emitException(code_start, code_end,
                                               cogen->getCodeLength(), 0, 0);
  ...
  Seq<Label *> *labels;
  uint32_t i;
  for (i = 0, labels = fctx.returnLabels.get(); labels != NULL;
       i++, labels = labels->tl) {
...
void TryStmt::cogenNoFinally(Cogen *cogen, Ctx *ctx) {
  ...
  for (Seq<CatchClause *> *catches = this->catches; catches != NULL;
       catches = catches->tl)
...
void TryStmt::cgCatch(Cogen *cogen, Ctx *ctx, uint32_t code_start,
                      uint32_t code_end, Label *Lend,
                      CatchClause *catchClause) {
  Compiler *compiler = cogen->compiler;
  uint32_t catch_idx = cogen->emitException(
      code_start, code_end, cogen->getCodeLength(),
      cogen->emitTypeName(compiler, catchClause->type_name),
      cogen->abc->addQName(
          cogen->emitNamespace(compiler->parser.defaultNamespace()),
  ...
  cogen->I_dup();
  cogen->I_setlocal(
      t); // Store catch scope in register so it can be restored later
  cogen->I_dup();
  cogen->I_pushscope();

  // Store the exception object in the catch scope.
  cogen->I_swap();
  cogen->I_setproperty(cogen->abc->addQName(
      cogen->emitNamespace(compiler->parser.defaultNamespace()),
      cogen->emitString(catchClause->name)));
