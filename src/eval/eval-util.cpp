...
Allocator::Allocator(Compiler *compiler)
    : compiler(compiler), free_sbchunks(NULL), current_chunk(NULL),
      current_top(NULL), current_limit(NULL) {
...
StringBuilder::StringBuilder(Compiler *compiler)
    : allocator(compiler->allocator), chunk(NULL) {
  clear();
}
...
ByteBuffer::ByteBuffer(Allocator *allocator, uint32_t increment)
    : allocator(allocator), increment(increment), out(NULL), limit(NULL),
      first(NULL), last(NULL), size_rest(0) {}
