...
// Significant performance improvements for qualified names if the namespace can
// be resolved at compile time. See notes in the implementation of the
// 'namespace' definition; the machinery has to be implemented for 'use default
...
void Name::computeName(QualifiedName *qname, Ctx *ctx) {
  ...
  if (qname->qualifier != NULL) {
    ...
    case TAG_simpleName: {
      uint32_t id = cogen->abc->addQName(
          compiler->NS_public,
          cogen->emitString(((SimpleName *)(qname->qualifier))->name), false);
      nsreg = cogen->getTemp();
    ...
    case TAG_builtinNamespace:
      ...
      break;
    default:
      compiler->internalError(qname->pos,
  ...
  case TAG_simpleName:
    if (nsreg != 0) {
      ...
      sym = cogen->abc->addRTQName(
          cogen->emitString(((SimpleName *)name)->name), qname->is_attr);
    } else if (ns_builtin) {
      AvmAssert(!ns_wildcard);
      uint32_t ns = cogen->emitNamespace(qname->qualifier);
      sym = cogen->abc->addQName(
          ns, cogen->emitString(((SimpleName *)name)->name), qname->is_attr);
    } else {
      ...
      // every time so be sure to cache the nsset for the current scope
      // somewhere.
      if (vs->nsset != 0 && !ns_wildcard && !qname->is_attr)
        sym = cogen->abc->addMultiname(
            vs->nsset, cogen->emitString(((SimpleName *)name)->name), false);
      else
        sym = cogen->abc->addQName(
            (ns_wildcard ? 0 : compiler->NS_public),
            cogen->emitString(((SimpleName *)name)->name), qname->is_attr);
    }
    break;
  case TAG_wildcardName:
...
void SimpleType::cogen(Cogen *cogen, Ctx *ctx) { name->cogen(cogen, ctx); }
...
void ObjectRef::cogen(Cogen *cogen, Ctx *ctx) {
  obj->cogen(cogen, ctx);
  if ((name->qualifier == NULL || name->qualifier->tag() == TAG_wildcardName) &&
      name->name->tag() == TAG_wildcardName && !name->is_attr)
...
void AssignExpr::cogen(Cogen *cogen, Ctx *ctx) {
  ...
  // Compute the object onto the stack, and elements of the name into locals if
  // necessary
...
void UnaryExpr::cogen(Cogen *cogen, Ctx *ctx) {
  ...
  case OPR_delete: {
    if (expr->tag() == TAG_qualifiedName || expr->tag() == TAG_objectRef) {
      ...
      // FIXME: e4x requires that if the value computed here is an XMLList then
      // a TypeError (ID 1119) is thrown.
...
void LiteralFunction::cogen(Cogen *cogen, Ctx *ctx) {
  if (function->name != NULL) {
    ...
    body.bindings.addAtEnd(
        ALLOC(Binding, (ns, function->name, NULL, TAG_varBinding)));
    body.functionDefinitions.addAtEnd(function);
    body.stmts.addAtEnd(
        ALLOC(ReturnStmt,
              (0, ALLOC(QualifiedName,
                        (ns, ALLOC(SimpleName, (function->name)), false, 0)))));
    Expr *e =
        ALLOC(CallExpr,
              (ALLOC(LiteralFunction, (ALLOC(FunctionDefn, (signature, body)))),
               NULL, 0));
    e->cogen(cogen, ctx);
  } else {
...
void LiteralObject::cogen(Cogen *cogen, Ctx *ctx) {
  (void)ctx;
  uint32_t i = 0;
  for (Seq<LiteralField *> *fields = this->fields; fields != NULL;
       fields = fields->tl) {
    ...
    for (; exprs != NULL; exprs = exprs->tl, i++) {
      ...
      if (e != NULL) {
        ...
        cogen->I_setproperty(cogen->abc->addQName(
            compiler->NS_public, cogen->emitString(compiler->intern(i))));
...
void LiteralVector::cogen(Cogen *cogen, Ctx *ctx) {
  ...
  cogen->I_construct(1);
  int32_t offset = 0;
  for (Seq<Expr *> *elements = this->elements; elements != NULL;
...
void LiteralRegExp::cogen(Cogen *cogen, Ctx *ctx) {
  ...
  // value is "/.../flags"
  //
  // OPTIMIZEME: silly to recompile the regular expression every time it's
  // evaluated, even if ES3.1 allows it (not sure what AS3 allows / requires;
  // ES3 requires compilation once).
  const wchar *s = value->s;
  const wchar *t = s + value->length - 1;
  while (*t != '/')
    t--;

  // Creating a new RegExp object every time is not compatible with ES3, but it
  // is what ASC does, and with luck ES3.1 will change to match this behavior.
  //
  // FIXME: semantics: findpropstrict(""::RegExp) is not quite right here.
  // Doing so creates a spoofing hole / surprising trap.  We want an
  // OP_newregexp instruction.
  cogen->I_findpropstrict(compiler->ID_RegExp);
  cogen->I_pushstring(
      cogen->emitString(compiler->intern(s + 1, uint32_t(t - s - 1))));
  cogen->I_pushstring(cogen->emitString(
      compiler->intern(t + 1, uint32_t(value->length - (t - s + 1)))));
