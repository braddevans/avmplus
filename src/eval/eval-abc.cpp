...
// Never called, but gcc gets very annoyed if a class with virtual methods does
// not have a virtual destructor.
ABCChunk::~ABCChunk() {}

ABCFile::ABCFile(Compiler *compiler)
#ifdef VMCFG_FLOAT
    : major_version(47), minor_version(16)
#else
    : major_version(46), minor_version(16)
#endif
      ,
      NO_VALUE(~0U), compiler(compiler), allocator(compiler->allocator),
      ...
      ,
      methodCount(0), metadataCount(0), instanceCount(0), classCount(0),
      scriptCount(0), nonemptyMethodBodyCount(0), intBuf(allocator, 16),
      uintBuf(allocator, 16), doubleBuf(allocator, 32), floatBuf(allocator, 32),
      float4Buf(allocator, 32), stringBuf(allocator),
      namespaceBuf(allocator, 16), nssetBuf(allocator, 16),
      multinameBuf(allocator, 16), namespaces(allocator),
      namespaceSets(allocator), multinames(allocator), methods(allocator),
      metadatas(allocator), instances(allocator), classes(allocator),
      scripts(allocator), bodies(allocator) {
}
...
loop_end:
  ...
  ABCNamespaceSetInfo *info = new (allocator->alloc(
      sizeof(ABCNamespaceSetInfo) + (length - 1) * sizeof(uint32_t)))
      ABCNamespaceSetInfo(length);
...
uint32_t ABCFile::multinameLookup(uint8_t kind, uint32_t ns_or_nsset,
                                  uint32_t name) {
  Seq<ABCMultinameInfo *> *mns;
  uint32_t i;

  for (mns = multinames.get(), i = 1; mns != NULL; mns = mns->tl, i++)
    if (mns->hd->kind == kind && mns->hd->ns_or_nsset == ns_or_nsset &&
        mns->hd->name == name)
      return i;
  ...
  case CONSTANT_TypeName:
    multinameBuf.emitU30(
        ns_or_nsset); // really the parameterizedType, eg, __AS3__.vec::Vector
...
uint32_t ABCFile::size() {
  ...
  reported_size =
      (4 + lenU30(intCount) + intBuf.size() + lenU30(uintCount) +
       uintBuf.size() + lenU30(doubleCount) + doubleBuf.size() +
#ifdef VMCFG_FLOAT
       lenU30(floatCount) + floatBuf.size() + lenU30(float4Count) +
       float4Buf.size() +
#endif
       lenU30(stringCount) + stringBuf.size() + lenU30(namespaceCount) +
       namespaceBuf.size() + lenU30(nssetCount) + nssetBuf.size() +
       lenU30(multinameCount) + multinameBuf.size() + lenU30(methodCount) +
       method_size + lenU30(metadataCount) + metadata_size +
       lenU30(instanceCount) + instance_size + class_size +
       lenU30(scriptCount) + script_size + lenU30(nonemptyMethodBodyCount) +
       body_size);
...
ABCTraitsTable::ABCTraitsTable(Compiler *compiler)
    : traitsCount(0), traits(compiler->allocator) {}
...
uint32_t ABCTrait::size() {
  ...
  reported_size = (lenU30(name) + 1 /* kind */ + dataSize());
  return reported_size;
}
...
uint32_t ABCSlotTrait::dataSize() {
  ...
  return (lenU30(slot_id) + lenU30(type_name) + lenU30(vindex) +
          (vindex != 0 ? 1 : 0));
}
...
uint32_t ABCClassTrait::dataSize() { return (lenU30(slot_id) + lenU30(index)); }
...
ABCScriptInfo::ABCScriptInfo(Compiler *compiler, ABCMethodInfo *init_method,
                             ABCTraitsTable *traits)
    : index(compiler->abc.addScript(this)), init_method(init_method),
      traits(traits) {}
...
ABCMethodBodyInfo::ABCMethodBodyInfo(Compiler *compiler,
                                     ABCMethodInfo *method_info,
                                     ABCTraitsTable *traits,
                                     uint32_t first_temp, bool is_empty)
    : cogen(compiler, &compiler->abc, traits, this, first_temp),
      exceptions(compiler), method_info(method_info), traits(traits),
      is_empty(is_empty) {
...
uint32_t ABCMethodBodyInfo::size() {
  if (is_empty)
    ...
    reported_size =
        (lenU30(method_info->index) + lenU30(cogen.getMaxStack()) +
         lenU30(cogen.getLocalCount()) + lenU30(0) /* init_scope_depth */ +
         lenU30(cogen.getMaxScope()) + lenU30(cogen.getCodeLength()) +
         cogen.getCodeLength() + exceptions.size() +
...
ABCMethodInfo::ABCMethodInfo(Compiler *compiler, uint32_t name,
                             uint32_t param_count, Seq<uint32_t> *param_types,
                             uint32_t option_count,
                             Seq<DefaultValue *> *default_values,
                             uint32_t return_type)
    : index(compiler->abc.addMethod(this)), name(name),
      param_count(param_count), param_types(param_types),
      option_count(option_count), default_values(default_values),
      return_type(return_type), flags(~0U) {}

uint32_t ABCMethodInfo::size() {
  AvmAssert(flags != ~0U);
  uint32_t param_size = 0;
  for (Seq<uint32_t> *param_types = this->param_types; param_types != NULL;
       param_types = param_types->tl)
    param_size += lenU30(param_types->hd);
  uint32_t option_size = option_count > 0 ? lenU30(option_count) : 0;
  for (Seq<DefaultValue *> *default_values = this->default_values;
       default_values != NULL; default_values = default_values->tl) {
    option_size += lenU30(default_values->hd->index);
    option_size += 1;
  }
  reported_size =
      (lenU30(param_count) + lenU30(return_type) + param_size + lenU30(name) +
       1 /* flags */ + option_size + 0 /* param_names */);
  return reported_size;
}
...
ABCExceptionTable::ABCExceptionTable(Compiler *compiler)
    : exceptionCount(0), exceptions(compiler->allocator) {}

uint32_t ABCExceptionTable::size() {
  reported_size = lenU30(exceptionCount);
  for (Seq<ABCExceptionInfo *> *exceptions = this->exceptions.get();
       exceptions != NULL; exceptions = exceptions->tl)
...
ABCExceptionInfo::ABCExceptionInfo(uint32_t from, uint32_t to, uint32_t target,
                                   uint32_t exception_type, uint32_t var_name)
    : from(from), to(to), target(target), exception_type(exception_type),
      var_name(var_name) {}

uint32_t ABCExceptionInfo::size() {
  reported_size = (lenU30(from) + lenU30(to) + lenU30(target) +
                   lenU30(exception_type) + lenU30(var_name));
  return reported_size;
}
...
uint32_t ABCInstanceInfo::size() {
  reported_size =
      (lenU30(name) + lenU30(super_name) + 1 /* flags */ +
       ((flags & CONSTANT_ClassProtectedNs) ? lenU30(protectedNS) : 0) +
       lenU30(interface_count) + lenU30(iinit) + traits->size());
