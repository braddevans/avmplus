...
namespace avmplus {
    namespace RTC {
        ...
        public:
            ...
            Seq <uint32_t>
                    *backpatches; // not a SeqBuilder so that we don't have to spend space on
            // storing an allocator in every label object
        };
...
// Context management
        ...
        class Ctx {
        public:
            Ctx(CtxType tag, Ctx *next) : tag(tag), next(next) {}

            const CtxType tag;
            Ctx *const next;

            bool mustPushThis(); // context requires a 'this' value to be pushed onto the
            // scope chain
            bool mustPushScopeReg(); // context requires a scope object stored in a scope
            // register to be pushed onto the scope chain
        ...
        public:
            BreakCtx(Label *label, Ctx *ctx0, Str *label_name = NULL)
                    : ControlFlowCtx(CTX_Break, label, ctx0), label_name(label_name) {}
        ...
        public:
            ContinueCtx(Label *label, Seq<Str *> *label_names, Ctx *ctx0)
                    : ControlFlowCtx(CTX_Continue, label, ctx0), label_names(label_names) {}
        ...
        struct GotoLabel {
            GotoLabel(Str *label_name, Label *address, bool ambiguous)
                    : label_name(label_name), address(address), ambiguous(ambiguous) {}
        ...
        public:
            GotoCtx(Ctx *ctx0)
                    : Ctx(CTX_Goto, ctx0), next_goto(NULL), label_names(NULL) {}
...
// or 'catch' or reified activation objects, see subclasses.
        ...
        public:
            ScopeCtx(CtxType tag, uint32_t scope_reg, Ctx *ctx0)
                    : Ctx(tag, ctx0), scope_reg(scope_reg) {}
        ...
        public:
            FinallyCtx(Allocator *allocator, Label *Lfinally, uint32_t returnreg,
                       Ctx *ctx0)
                    : Ctx(CTX_Finally, ctx0), Lfinally(Lfinally), returnreg(returnreg),
                      nextLabel(0), returnLabels(allocator) {}
        ...
        public:
            VarScopeCtx(CtxType tag, uint32_t nsset, Seq<Namespace *> *openNamespaces,
                        Allocator *allocator, Ctx *prev)
                    : Ctx(tag, prev), nsset(nsset), openNamespaces(openNamespaces) {
                (void) allocator;
            }

            const uint32_t nsset; // Will be something other than 0 if this variable scope
            // has a multiname qualifier (namespace set)
            Seq<Namespace *> *const openNamespaces; // Not including ""
        };

        class FunctionCtx : public VarScopeCtx {
        public:
            FunctionCtx(Allocator *allocator, uint32_t nsset,
                        Seq<Namespace *> *openNamespaces, Ctx *prev)
                    : VarScopeCtx(CTX_Function, nsset, openNamespaces, allocator, prev) {}
        };

        class ProgramCtx : public VarScopeCtx {
        public:
            ProgramCtx(Allocator *allocator, uint32_t nsset,
                       Seq<Namespace *> *openNamespaces, uint32_t capture_reg)
 ...
 */
        class Cogen {
        public:
            Cogen(Compiler *compiler, ABCFile *abc, ABCTraitsTable *traits,
                  ABCMethodBodyInfo *body, uint32_t first_temp = 1);
            ...
            void I_debug(uint8_t debug_type, uint32_t index, uint8_t reg,
                         uint32_t extra = 0);
            ...
            void I_lookupswitch(Label *default_label, Label **case_labels,
                                uint32_t ncases);
            ...
            uint32_t emitException(uint32_t from, uint32_t to, uint32_t target,
            ...
            void unstructuredControlFlow(Ctx *ctx, bool(hit)(Ctx *, void *),
                                         void *package, bool jump, SyntaxError msg,
                                         uint32_t pos = 0);

            void standardMethod(bool is_function, Seq<Str *> *params,
                                Seq<FunctionDefn *> *functions, Seq<Str *> *vars,
                                Seq<Stmt *> *stmts);
        ...
        private:
            ...
            void stackMovement(AbcOpcode opcode, bool hasRTNS, bool hasRTName,
                               uint32_t argc);
