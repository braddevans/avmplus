...
Token Lexer::xmlAtomImpl() {
  ...
  case '<':
    switch (idx[1]) {
    case '!':
      if (idx[2] == '[' && idx[3] == 'C' && idx[4] == 'D' && idx[5] == 'A' &&
          idx[6] == 'T' && idx[7] == 'A' && idx[8] == '[') {
        idx += 9;
...
Token Lexer::xmlMarkup(Token token) {
  ...
  while (idx < limit) {
    if (idx[0] == '-' || idx[0] == '?' || idx[0] == ']') {
      ...
      case T_XmlComment:
        if (idx[0] == '-' && idx[1] == '-') {
          // Done; we require > to follow but it's not part of the stop
          // condition.
          if (idx[2] != '>')
...
bool Lexer::isXmlNameSubsequent(wchar c) {
  return isUnicodeLetter(c) || isUnicodeDigit(c) || c == '_' || c == ':' ||
         c == '.' || c == '-';
}
