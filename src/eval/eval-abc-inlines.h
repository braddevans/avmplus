...
namespace avmplus {
    namespace RTC {
        ...
        inline uint32_t ABCFile::addClassAndInstance(ABCClassInfo *c,
                                                     ABCInstanceInfo *i) {
...
// The offset must not be revealed - the body may not actually be serialized
        ...
        inline uint8_t ABCMethodBodyInfo::getFlags() { return cogen.getFlags(); }
        ...
        inline uint32_t ABCFile::addQName(uint32_t ns, uint32_t name, bool is_attr) {
            ...
            return multinameLookup((uint8_t)(is_attr ? CONSTANT_QnameA : CONSTANT_Qname),
                                   ns, name);
        }

        inline uint32_t ABCFile::addRTQName(uint32_t name, bool is_attr) {
            using namespace ActionBlockConstants;
            return multinameLookup(
                    (uint8_t)(is_attr ? CONSTANT_RTQnameA : CONSTANT_RTQname), NO_VALUE,
                    name);
        }
        ...
        inline uint32_t ABCFile::addRTQNameL(bool is_attr) {
            using namespace ActionBlockConstants;
            return multinameLookup(
                    (uint8_t)(is_attr ? CONSTANT_RTQnameLA : CONSTANT_RTQnameL), NO_VALUE,
                    NO_VALUE);
        }

        inline uint32_t ABCFile::addMultiname(uint32_t nsset, uint32_t name,
                                              bool is_attr) {
            using namespace ActionBlockConstants;
            return multinameLookup(
                    (uint8_t)(is_attr ? CONSTANT_MultinameA : CONSTANT_Multiname), nsset,
                    name);
        }
        ...
        inline uint32_t ABCFile::addMultinameL(uint32_t nsset, bool is_attr) {
            using namespace ActionBlockConstants;
            return multinameLookup(
                    (uint8_t)(is_attr ? CONSTANT_MultinameLA : CONSTANT_MultinameL), nsset,
                    NO_VALUE);
        }

        inline uint32_t ABCFile::addTypeName(uint32_t parameterizedType,
                                             uint32_t parameterType) {
            using namespace ActionBlockConstants;
            return multinameLookup(CONSTANT_TypeName, parameterizedType, parameterType);
        }

        inline void ABCMethodBodyInfo::clearTraits() { traits = NULL; }
