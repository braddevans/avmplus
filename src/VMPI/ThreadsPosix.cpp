...
bool VMPI_threadCreate(vmpi_thread_t *thread, vmpi_thread_attr_t *attr,
  ...
  // Bugzilla 656008: double-checking VMPI_nullThread; must not collide with id
  // of fresh thread
...
bool VMPI_condVarTimedWait(vmpi_condvar_t *condvar, vmpi_mutex_t *mutex,
...
#if AVMSYSTEM_MAC || defined(VMCFG_PEPPER_MAC)
  struct timespec timeSpec;
  timeSpec.tv_sec = timeout_millis / 1000;
  timeSpec.tv_nsec = (timeout_millis % 1000) * 1000000;
  return pthread_cond_timedwait_relative_np(condvar, mutex, &timeSpec) ==
         ETIMEDOUT;
#else
  struct timespec timeSpec;
  ...
  timeSpec.tv_nsec =
      (timeVal.tv_usec * 1000) + ((timeout_millis % 1000) * 1000000);
