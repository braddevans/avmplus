...
static TIME_ZONE_INFORMATION UpdateTimeZoneInfo() {
  ...
  //   We're relying on the fact that static initialization is OK for spin
  //   locks.
  ...
  if ((gmt.wMinute != gGmtCache.wMinute) || (gmt.wHour != gGmtCache.wHour) ||
      (gmt.wDay != gGmtCache.wDay) || (gmt.wMonth != gGmtCache.wMonth) ||
...
static double ConvertWin32DST(int year, SYSTEMTIME *st) {
  ...
  if (st->wYear != 0) {
    return avmplus::MakeDate(avmplus::MakeDay(year, st->wMonth - 1, st->wDay),
                             avmplus::MakeTime(st->wHour, st->wMinute,
                                               st->wSecond, st->wMilliseconds));
  }

  double timeValue =
      avmplus::MakeDate(avmplus::MakeDay(year, st->wMonth - 1, 1),
                        avmplus::MakeTime(st->wHour, st->wMinute, st->wSecond,
                                          st->wMilliseconds));
...
void *VMPI_alloc(size_t size) { return HeapAlloc(GetProcessHeap(), 0, size); }

void VMPI_free(void *ptr) { HeapFree(GetProcessHeap(), 0, ptr); }

size_t VMPI_size(void *ptr) { return HeapSize(GetProcessHeap(), 0, ptr); }
...
#if !defined(UNDER_CE) &&                                                      \
    (!defined(AVMSYSTEM_WINDOWSSTOREAPP) || AVMSYSTEM_WINDOWSSTOREAPP == 0)
// The MS compiler warns that getenv() is 'insecure', but as far as I can tell,
// the issue is simply that it returns a pointer to a static memory location.  I
// do not intend to change the cross-platform VMPI_getenv API, so using one of
// the MS-suggested alternatives would simply move the static variable to
// another location, raising the question of how much space to allocate for it,
// since we would no longer be referencing the environment in-place.
...
// Call VMPI_getPerformanceFrequency() once to initialize its cache; avoids
...
extern void CallWithRegistersSaved2(void (*fn)(void *stackPointer, void *arg),
                                    void *arg, void *buf);
extern void CallWithRegistersSaved3(void (*fn)(void *stackPointer, void *arg),
                                    void *arg, void *buf);

void VMPI_callWithRegistersSaved(void (*fn)(void *stackPointer, void *arg),
                                 void *arg) {
  jmp_buf buf;
  VMPI_setjmpNoUnwind(buf); // Save registers
  CallWithRegistersSaved2(fn, arg,
                          &buf); // Computes the stack pointer, calls fn
  CallWithRegistersSaved3(
      fn, &arg,
      &buf); // Probably prevents the previous call from being a tail call
}
...
// Private to AAVMPI_getVMPageSize (but initialized here to avoid threading
// concerns). DO NOT REFERENCE THIS VARIABLE ELSEWHERE.  Always call
// AAVMPI_getVMPageSize.

static size_t pagesize = computePagesize();

size_t VMPI_getVMPageSize() { return pagesize; }
...
uintptr_t VMPI_startTimer(uint32_t micros, VMPI_TimerClient *client) {
  VMPI_WinTimerData *data =
      (VMPI_WinTimerData *)VMPI_alloc(sizeof(VMPI_WinTimerData));
  ...
  data->timer = ThreadPoolTimer::CreatePeriodicTimer(
      ref new TimerElapsedHandler(
          [&](ThreadPoolTimer ^ timer) { data->client->tick(); }),
      timeSpan);
...
// The constant TIME_KILL_SYNCHRONOUS is only available if WINVER >= 0x0501 (==
// WinXP)
...
uintptr_t VMPI_startTimer(uint32_t micros, VMPI_TimerClient *client) {
  ...
  data->timerId = timeSetEvent(micros / 1000, micros / 1000,
                               (LPTIMECALLBACK)timerProc, (DWORD_PTR)data,
