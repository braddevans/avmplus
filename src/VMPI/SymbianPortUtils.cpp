...
double VMPI_getDaylightSavingsTA(double newtime) {
  ...
  // either daylight saving is not in effect, or we don't know (if tm_isdst is
  // negative).
  return 0;
}
...
void VMPI_free(void *ptr) { free(ptr); }

size_t VMPI_size(void *ptr) { return 0; }

void VMPI_log(const char *message) { DebugTraceString(message); }
...
void *VMPI_allocateCodeMemory(size_t nbytes) {
  // Our custom SymbianJITHeap class can allocate only one page a time (=4k) per
  // design. If NJ starts allocating more than 4k a time on ARM platforms, we
  // have to change this, probably just to use RHeap directly (and manually page
  // align the memory it returns).
  ...
  if (symbianJITHeap) {
    ...
    // FIXME https://bugzilla.mozilla.org/show_bug.cgi?id=571407
    // The documentation for VMPI_allocateCodeMemory states specifically that
    // the function never returns NULL, so this is wrong and must be fixed.
...
void VMPI_makeCodeMemoryExecutable(void *address, size_t nbytes,
                                   bool makeItSo) {
...
const char *VMPI_getenv(const char *name) { return getenv(name); }
...
void AVMPI_callWithRegistersSaved(void (*fn)(void *stackPointer, void *arg),
                                  void *arg) {
  jmp_buf buf;
