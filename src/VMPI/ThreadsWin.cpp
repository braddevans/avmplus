...
void VMPI_tlsDestroy(uintptr_t tlsId) { TlsFree((DWORD)tlsId); }

bool VMPI_threadCreate(vmpi_thread_t *thread, vmpi_thread_attr_t *attr,
                       vmpi_thread_start_t start_fn, vmpi_thread_arg_t arg) {
  DWORD id;
  SIZE_T stackSize =
      attr == NULL || attr->stackSize == VMPI_threadAttrDefaultStackSize()
          ? 0
          : attr->stackSize;
  ...
  if (threadHandle) {
    *thread = id;
    ...
    // The SetThreadStackGuarantee API is not available in all versions of
    // Windows we care about, so we can't honor any specific guard size request.
    return true;
  } else {
    return false;
  }
...
void VMPI_threadSleep(int32_t timeout_millis) { Sleep(timeout_millis); }
...
bool VMPI_condVarTimedWait(vmpi_condvar_t *condvar, vmpi_mutex_t *mutex,
...
void VMPI_threadJoin(vmpi_thread_t thread) {
  HANDLE threadHandle =
      OpenThread(STANDARD_RIGHTS_REQUIRED | SYNCHRONIZE | 0x3FF, false, thread);
  if (threadHandle) {
...
bool VMPI_condVarTimedWaitInternal(vmpi_condvar_t *condvar, vmpi_mutex_t *mutex,
                                   int32_t timeout_millis, bool infinite) {

  // We use the same emulation technique as FP.
  // Although post-notify scheduling is not exactly fair, the algorithm is
  // simple. If we want something fairer then use Alexander Terekhov's 8a
  // algorithm.

  // Add the calling thread to the condition variable's list of waiting threads.
  // Note that the linked list is implicitly guarded by the mutex paired with
  ...
  // Note that there should be *no* lost wakeups between the unlock and the
  // sleep, as all notifies are queued as APCs.
  ...
  // As per pthread condition variable semantics, we block until we can
  // re-acquire the mutex we released.
  EnterCriticalSection(mutex);

  if (!thisThread.notified) {
    // A spurious or time-out wake-up, so the thread must remove itself from the
    // condvar's wait list.
...
// A private, dummy callback for the QueueUserAPC implementation of
// VMPI_condVarBroadcast/Signal
...
void VMPI_condVarBroadcast(vmpi_condvar_t *condvar) {
  ...
  // Note that the list is implicitly guarded by the mutex paired with the
  // conditional variable (which the calling thread holds).
  WaitingThread *waiter = condvar->head;
  condvar->head = NULL;
  condvar->tail = NULL;
  while (waiter != NULL) {
    ...
    HANDLE threadHandle =
        OpenThread(STANDARD_RIGHTS_REQUIRED | SYNCHRONIZE | 0x3FF, false,
                   waiter->threadID);
...
void VMPI_condVarSignal(vmpi_condvar_t *condvar) {
  // Signal the head of the list. FIFO looks like it should be fair, but it's
  // not really. Note that the list is implicitly guarded by the mutex paired
  // with the conditional variable (which the calling thread holds).
  WaitingThread *waiter = condvar->head;
  if (waiter != NULL) {
    waiter->notified = true;
    condvar->head = waiter->next;
...
size_t VMPI_threadAttrDefaultStackSize() {
  // FIXME: bug 609822 Is it possible to implement
  // VMPI_threadAttrDefaultStackSize on win32?
  //
  ...
  // API: http://msdn.microsoft.com/en-us/library/ms686774%28v=VS.85%29.aspx
  return 1024 * 1024;
}
