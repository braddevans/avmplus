...
#else
/* The embedding host must provide this file in some directory that's included
 * in header search paths.  It must define a value (0 or 1) for every feature
...
// Catchall, though in general the platform files are really responsible for
// giving REALLY_INLINE and FASTCALL a definition.
...
/**
 * This method should return the difference in milliseconds between local time
 * and UTC
...
/**
 * This method should return the Daylight Savings time adjustment in
 * milliseconds
 * @return number of milliseconds corresponding to daylight savings adjustment
 ...
 * The implementation of this method could either return the time elapsed since
 * the system started or since epoc
 ...
 * This method is specifically for debugging purposes and is invoked to output
 * useful debug informaton The implementation of this method is
 * platform-defined.  For example, it could be used to output the message to an
 * IDE
 * @param message NULL-terminated UTF8-encoded string
 * @return none
 */
 ...
 * Upon a call to this method, the platform execution should interrupt the
 * program execution while maintaining the current call stack and associated
 * program information such as the registers and return the control back to the
 * user/developer This method is specifically for debugging purposes and is
 * invoked to enter a debug interrupt to examine information like the variable
 * values or call stack The implementation of this method is platform-defined.
 * On platforms that cannot support debug interrupts the program execution
 * should be aborted
 * @return none
 */
 ...
 * @return pointer to the start of memory block if allocation was successful,
 * NULL otherwise
 */
 ...
 * On platforms where no API to retrieve this information should return a number
 * that closely matches its timer frequency
 * @return 64-bit value indicating the frequency (i.e. counts per second) of the
 * system's performance counter or clock with highest resolution
 ...
 * Platforms that support a high performance counter should return its current
 * value Platforms that do not have a high performance counter should return the
 * current timer/clock value that was used as a basis to calculate the frequency
 * returned from VMPI_getPerformanceFrequency()
...
/**
 * Start a timer that will call into a client object periodically. You can
 * create as many timers as you like.
 * @param period the number of microseconds between calls. We don't guarantee
 * this, you might get called early or late, or the actual period may be
 * different from what you wanted.
 * @param client an object which contains a function that we will call on every
 * tick
 * @return an id for this timer. You can pass this id to VMPI_stopTimer when you
 * want to stop.
 */
 ...
 * It is allowed for the caller to have acquired the lock when this function is
 * called.
 ...
 * If this method returns true, MMgc assumes that the lock was acquired
 * successfully During a call to this method, if the lock was held by some other
 * thread then the implementation should wait until the lock becomes available
 * Return value of false is considered to be an error condition and should only
 * happen in exception situations
 ...
 * @param lock instance to release the lock on
 * @return true if lock was successfully aqcuired, false if another thread has
 * it
 */
...
/**
 * Save all registers into the stack and invoke 'fn' in a non-tail fashion,
 * passing it a conservative approximation to the true stack top (the lowest
 * live address) as well as 'arg'.  The hot part of the stack - where the
 * registers are saved - is volatile; once the function returns that part of the
 * stack does not reliably contain the saved registers any longer.  The size of
 * the hot part of the stack is platform-dependent.
 */
extern void VMPI_callWithRegistersSaved(void (*fn)(void *stackPointer,
                                                   void *arg),
                                        void *arg);
 ...
 */
extern bool VMPI_compareAndSwap32(int32_t oldValue, int32_t newValue,
 ...
 * @return true if the update was successful
 */
extern bool VMPI_compareAndSwap32WithBarrier(int32_t oldValue, int32_t newValue,
                                             volatile int32_t *address);

/**
 * Inserts the strongest hardware read/write memory barrier provided by the
 * platform. The minimum guarantee provided by this barrier will be:
 ...
 *
 * Note that this function inserts an equivalent compiler memory-barrier in
 * addition to the hardware barrier.
 */
 ...
 *
 * The thread will be created with attributes described by 'attr'. If attr is
 * NULL then the platform's default attributes will be used. Changing values
 * within attr after the thread has started execution has no effect. Note that
 * it is platform dependent if non-default attributes are honored or completely
 * ignored. As of Nov 2010, support should be: pthread     win32 (XP version
 * APIs) priority        no         yes stack size      yes        yes guard
 * size      yes        no
 *
 *
 * @param thread On successful thread creation, the thread's id is stored into
 * the location referenced by 'thread'
 * @param attr The attributes with which to start the thread. If NULL, then the
 * platform's default attributes will be used
 ...
 */
extern bool VMPI_threadCreate(vmpi_thread_t *thread, vmpi_thread_attr_t *attr,
                              vmpi_thread_start_t start_fn,
                              vmpi_thread_arg_t arg);

/**
 * Indicates to the thread implementation that resources for the given thread
 * can be reclaimed when the thread terminates.
 * After a thread has been detached, any attempt to join the thread has
 * undefined behavior. Multiple detachments of the same thread results in
 * undefined behavior.
 *
 ...
 * If the given thread has already terminated but is not detached then the
 * function immediately returns. If the given thread has already terminated and
 * been detached then the behavior is undefined. If the given thread is the
 * calling thread then the behavior is undefined. IMPORTANT: This function
 * should not be assumed to be thread safe; the behavior of racing threads
...
/**
 * Initializes the vmpi_mutex_t structure referenced by 'mutex' to be used as a
 * recursive mutex. Note that mutex should be allocated by the caller.
 *
 ...
 * If the mutex is currently held by another thread then the calling thread
 * blocks until it is available. The mutex is recursive whereby successive lock
 * operations by the same thread increments the mutex's lock-count. Each
 * subsequent unlock operation decrements the lock-count until reaching zeroing,
 * resulting in an unlocked mutex. The limit and effects of lock-count overflow
 * is dependent on the platform thread implementation.
 *
...
/**
 * Identical to VMPI_recursiveMutexLock, except if the mutex is locked by
 * another thread the call returns immediately.
 *
 * @param mutex The mutex to lock
 ...
 * If the mutex is recursively locked then unlocking decrements the lock-count.
 * On reaching a lock-count of zero, the mutex is unlocked. Attempting to unlock
 * a mutex not held by the calling thread results in undefined behaviour.
 *
...
/**
 * Initializes the vmpi_condvar_t structure referenced by 'condvar' to be used
 * as a condition variable. Note that condvar should be allocated by the caller.
 *
 ...
 * Note that destroying a condition variable which has waiting threads results
 ...
 * to re-acquire mutex. When the mutex is re-acquired, this function will
 * return.
 *
 ...
 * undefined behavior. Waiting on a condition variable with a recursively locked
 ...
 * When the thread is blocked, it releases its lock on the mutex.
 * The thread remains blocked until the condition variable is signaled
 * (either individually or via a broadcast), or the specified timeout period has
 * expired. Post-wait, the thread will attempt to re-acquire mutex. When the
 * mutex is re-acquired, this function will return.
 *
 * Note that a waiting thread may spuriously awaken before the timeout period
 * has ended without being signaled. Waiting on a single condition variable with
 * multiple mutexes results in undefined behavior. Waiting on a condition
 * variable with a recursively locked mutex results in undefined behavior.
 *
 * @param condvar The condition variable on which to block
 * @param mutex The mutex to release whilst waiting and re-acquire when signaled
 ...
 */
extern bool VMPI_condVarTimedWait(vmpi_condvar_t *condvar, vmpi_mutex_t *mutex,
 ...
 * If no threads are waiting then the function simply returns.
 * This function does not imply any fairness policy when selecting the thread to
 * wake.
 *
 * @param condvar The condition variable to signal
 * @see VMPI_condVarWait
...
/**
 * Initializes the vmpi_thread_attr_t structure referenced by 'attr' with the
...
/**
 * Returns the number of processors (inc. logical processors) that were
 * available at boot time. Note that the number of dynamically available
 * processors can change (e.g. via power management), so do not rely on this
 * function for algorithm correctness.
 */
...
/** Note: this class is only used by the VMPI implementations -
 GenericPortUtils, WinPortUtils, and PosixPortUtils. It doesn't need to be
 declared in a public header, and should be moved into a private header file. */
struct VMPI_TimerData {
  ...
  VMPI_TimerClient
      *client; // a object containing a function we must call on every tick
};

#if defined(_MSC_VER)
#if _MSC_VER >= 1900 // VS2015
// This assumes we don't ship a player built with VS2015 or later to run on
// Windows XP or Vista. The RaiseFailFastException() API was introduced in
// Windows 7.
#include <windows.h>
#define VMPI_failFast()                                                        \
  do {                                                                         \
    ::DebugBreak();                                                            \
    ::RaiseFailFastException(NULL, NULL,                                       \
...
#define VMPI_failFast()                                                        \
  do {                                                                         \
    ::DebugBreak();                                                            \
    volatile unsigned *crash = 0;                                              \
    *crash = 0xdeadbeef;                                                       \
  } while (0)
#endif
#define VMPI_DECLARE_FAILFAST(name) extern __declspec(noinline) void name();
#define VMPI_DEFINE_FAILFAST(name)                                             \
  __declspec(noinline) void name() { VMPI_failFast(); }
...
#define VMPI_DECLARE_FAILFAST(name)                                            \
  extern void name() __attribute__((noinline));
#define VMPI_DEFINE_FAILFAST(name)                                             \
  __attribute__((noinline)) void name() { VMPI_failFast(); }
#else
#define VMPI_failFast()                                                        \
  do {                                                                         \
    volatile unsigned *crash = 0;                                              \
