...
// We make an assumption that using fiber local storage is equivalent of using
...
void VMPI_tlsDestroy(uintptr_t tlsId) { FlsFree((DWORD)tlsId); }

/*
The std::thread API does not use thread ids in the API unlike when using
pthreads or win32 threads for example. Std::threads are objects. It is not
possible to get std::thread object for "current thread" for example. Once we
create a new std::thread, we get its id (which is an object as well) and then
use a special hash map (which is defined by std API) to get an unique identifier
for the thread, type if size_t. This is used as the thread type in the vmpi API.
A look up is done whenever we need to access a thread that the vm has created
earlier.

It is not possible to call join, detach for threads that the vm did not create
(for example the main thread)
- we don't know their ids. Luckily by reading the code, the vm does not try to
do such a thing.

To return something from VMPI_currentThread we return hash of
std::this_thread::get_id(). To return something from VMPI_nullThread we return
hash of an uninitialized std::thread::id object. Returning 0 might work too.
...
// Used by vmbase::VMThread::startInternal - in other words by threads created
// with VMPI_createThread.
bool VMPI_threadDetach(vmpi_thread_t thread) {
  // Note! For some reason they have not implemented detach on win32. However it
  // is implemented in ThreadsPosix.cpp.
...
// Actually VMPI_threadJoin is not called by the vm at the moment, so comment
// out:
/*
