...
TypeAnalyzer::TypeAnalyzer(InstrGraph *ir) : ir_(ir), lattice_(&ir->lattice) {}
  ...
  if (hasOutputSignature(instr)) {
    ...
    for (int i = 0; !d.empty(); d.popFront(), sig.popFront(), ++i) {
      ...
      if (isDataType(*result_type) && !submodelof(data_type, result_type))
        setType(&d.front(),
                lattice_->changeModel(data_type, model(result_type)));
    }
  }
}
...
void TypeAnalyzer::do_abc_finddef(BinaryStmt *instr) {
  ...
  if (isConst(name_type))
    return setStmtType(instr, lattice_->finddefType(caller->pool(), name_type));
...
void TypeAnalyzer::do_callstatic(CallStmt2 *instr) {
  ...
  const Type *return_type =
      lattice_->makeType(method->getMethodSignature()->returnTraits());
...
void TypeAnalyzer::do_getlocal(GetlocalStmt *instr) {
  ...
  // Setlocal coerces the object to an atom, modelfixer coerces the getlocal
  // back to the original type. But there is no model fixer for an integer type
  // that is both int and uint (both constraints). So, faut de mieux, we pick
  // one.
  if (isNumber(t)) {
...
const Type *callstub(Lattice *l, const Type *c1, const Type *c2,
                     Stub_II_I stub) {
...
template <typename STUB> void TypeAnalyzer::fold(BinaryExpr *instr, STUB stub) {
...
template <typename STUB> void TypeAnalyzer::fold(UnaryExpr *instr, STUB stub) {
...
void TypeAnalyzer::do_touint(UnaryStmt *instr) {
  ...
  setStmtType(instr, subtypeof(t, uint_type)
                         ? t
                         : isConst(t) && isInt(t)
                               ? lattice_->makeUIntConst(uint32_t(intVal(t)))
                               : isConst(t) && isNumber(t)
                                     ? lattice_->makeUIntConst(
                                           AvmCore::doubleToInt32(doubleVal(t)))
                                     : uint_type);
}
...
void TypeAnalyzer::do_label(LabelInstr *label) {
  for (int i = 0, n = label->paramc; i < n; ++i) {
    ...
    if (isDataType(*new_type))
      new_type =
          lattice_->changeModel(new_type, halfmoon::promoteModelKind(new_type));
    setType(d, new_type);
  }
}
...
void TypeAnalyzer::do_abc_constructprop(CallStmt2 *instr) {
  ...
  if (isSlot(b)) {
    const Type *class_type =
        lattice_->getSlotType(type(instr->object_in()), toSlot(b));
...
void TypeAnalyzer::do_cknull(UnaryStmt *instr) {
  ...
  //      // Don't allow this type to go to bottom, so make up something wide
  //      enough to hold it return setStmtType(instr,
  //      lattice_->makeNotNull(lattice_->makeUnion(value,
  //      lattice_->atom_type[kTypeNullable])));
  //  }
...
void TypeAnalyzer::do_newactivation(UnaryStmt *instr) {
  ...
  if (method)
    return setStmtType(instr,
                       lattice_->makeType(method->activationTraits(), false));
  do_default(instr);
}
...
bool isCastCall(Lattice *l, const Type *object_type, uint32_t slot,
  ...
  if (slot_traits && slot_traits->base == builtin.class_itraits &&
    ...
    AvmAssert(slot_traits->itraits != NULL &&
              "Class with unknown instance traits");
...
void TypeAnalyzer::do_abc_callprop(CallStmt2 *instr) {
  const Type *object_type = type(instr->object_in());
  CallAnalyzer b(object_type, type(instr->param_in()), lattice_,
                 instr->vararg_count());
