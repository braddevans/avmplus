...
/// worklist if so.
///
void AbcGraph::addAbcEdge(AbcBlock *fm, const uint8_t *target_pc,
                          int edge_index) {
  ...
  if (enable_verbose)
    method->pool()->core->console << "edge " << int(fm->start - code_pos_)
                                  << "->" << int(target_pc - code_pos_) << "\n";
}
...
void AbcGraph::analyzeBranch(AbcBlock *blk, AbcOpcode abcop,
  ...
  AvmAssert(isBranchOpcode(abcop) && abcop != OP_jump);
  (void)abcop;
