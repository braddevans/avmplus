...
// Anyway to include nanojit allocators / contains w/o including the header
// here? Only methodprofile.cpp should include this file as well
 ...
 * Runtime types of local variables that are declared the any type by the
 * verifier.
 */
class MethodProfile {
public:
  ...
  /// inputs: the starting abc pc. The opcode at the starting point must be a
  /// branch ABC opcode. target_abc_pc - the destination abc pc.
  ///
  ...
  ///
  void initializeBranchCounters(int start_pc, int true_target_pc,
                                int false_target_pc);
  void incrementBranchCounters(bool took_true_branch, int start_pc,
...
private:
  ...
  /// branch_counter_map == maps abc branch location -> map of all possible
  /// targets. possible targets map == target -> taken count
  ///
