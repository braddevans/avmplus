...
public:
  ...
  void writeMethodCall(const FrameState *state, const uint8_t *pc,
                       AbcOpcode opcode, MethodInfo *methodInfo,
  ...
  virtual void emitIf(AbcOpcode opcode, const uint8_t *target, int lhs,
                      int rhs);
  virtual bool isProfilerLirEmitter() { return true; }
  virtual void emitRecordCallTypes(MethodSignaturep method_signature, LIns *obj,
                                   Traits *object_type, int argc, LIns *apAddr,
                                   Traits *result, LIns *call_result,
                                   LIns *method);

private:
  void emitRecordMethodReceiverType();
  void emitRecordBinaryTypes(LIns *left_operand, LIns *right_operand,
                             const FrameValue &left_value,
                             const FrameValue &right_value, const uint8_t *pc,
                             AbcOpcode opcode);
  void emitRecordBinaryTypes(LIns *left_operand, LIns *right_operand, int sp,
                             const uint8_t *pc, AbcOpcode opcode, Traits *type);
