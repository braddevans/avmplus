...
ProfileLirEmitter::ProfileLirEmitter(MethodInfo *info, MethodSignaturep ms,
                                     Toplevel *toplevel, JITNoise *noise)
    : CodegenLIR(info, ms, toplevel, NULL, noise) {}

ProfileLirEmitter::~ProfileLirEmitter() {
  // Compiler should automatically call CodegenLIR's destructor
}

void ProfileLirEmitter::initializeBranchMaps(int abc_pc, int true_target,
                                             int false_target) {
...
// abcbuilder time.
void ProfileLirEmitter::emitIf(AbcOpcode opcode, const uint8_t *target, int lhs,
                               int rhs) {
  int fall_through_offset = 4; // ABC dictates fall through is current_pc + 4
  initializeBranchMaps(getVerboseAbcPc(state->abc_pc), getVerboseAbcPc(target),
...
void ProfileLirEmitter::branchToAbcPos(LOpcode op, LIns *condition,
                                       const uint8_t *target) {
  LIns *current_pc = InsConst(getVerboseAbcPc(state->abc_pc));
  LIns *true_target_pc = InsConst(getVerboseAbcPc(target));

  int fall_through_offset = 4; // ABC dictates fall through is current_pc + 4
  LIns *fall_through_pc =
      InsConst(getVerboseAbcPc(state->abc_pc + fall_through_offset));
  ...
  callIns(FUNCTIONID(updateBranchCounters), 6, env_param, current_pc,
          true_target_pc, fall_through_pc, recordCondition, InsConst(op));
  CodegenLIR::branchToAbcPos(op, condition, target);
}

void ProfileLirEmitter::emitRecordBinaryTypes(
    LIns *left_operand, LIns *right_operand, const FrameValue &left_value,
    const FrameValue &right_value, const uint8_t *pc, AbcOpcode opcode) {
  /***
   * We cannot record the output type of binary operations
   * because CodegenLIR uses the framestate types for various operations.
   * An opcode such as add stores the result value of a different type into a
   * stack slot eg add int + object = object. Stack slot in the framestate still
   * says the type is an int not an object, which messes up vartracker,
   * localcopy, etc.
   */
  switch (opcode) {
  case OP_add: {
    ...
    callIns(FUNCTIONID(recordBinaryInputTypes), 4, env_param, left_atom,
            right_atom, InsConst(getVerboseAbcPc(pc)));
    break;
  }
  default:
    break;
...
void ProfileLirEmitter::emitRecordCallTypes(MethodSignaturep callee_ms,
                                            LIns *obj, Traits *objectType,
                                            int argc, LIns *apAddr,
                                            Traits *result, LIns *call_result,
                                            LIns *method) {
  ...
  if (!allArgumentsTyped(callee_ms)) {
    callIns(FUNCTIONID(recordCallInputTypes), 7, env_param, obj, InsConst(argc),
            apAddr, method, call_result, InsConst(abc_pc));
  }
  (void)result;
}

// CodeWriter methods
void ProfileLirEmitter::write(const FrameState *state, const uint8_t *pc,
  ...
  /// If Either the left or right values are boxed, box both values and record
  /// the type We could choose to record only the untyped value, but then we'd
  /// have a combinatorial explosion of profiler-jit-methods for each case. We
  /// could also write a "isNotAtomType" and feed that instead of the actual
  /// value However, we don't have the type tag for that in the atom tag (only 3
  /// bits). Finally, no damage is done if we record extra information because
  /// halfmoon Should always ask the verifier first, what type is the value.
  /// Only in the untyped case do we actually need to speculate.
  if (isBinaryOpcode(opcode)) {
    ...
    if (isAnyType(left_value) || (isAnyType(right_value))) {
      ...
      emitRecordBinaryTypes(leftOperand, rightOperand, left_value, right_value,
                            pc, opcode);
      return;
    }
...
void ProfileLirEmitter::writeOp1(const FrameState *state, const uint8_t *pc,
                                 AbcOpcode opcode, uint32_t opd1,
                                 Traits *type) {
  CodegenLIR::writeOp1(state, pc, opcode, opd1, type);
}

void ProfileLirEmitter::writeOp2(const FrameState *state, const uint8_t *pc,
                                 AbcOpcode opcode, uint32_t opd1, uint32_t opd2,
                                 Traits *type) {
  CodegenLIR::writeOp2(state, pc, opcode, opd1, opd2, type);
}

void ProfileLirEmitter::writeMethodCall(const FrameState *state,
                                        const uint8_t *pc, AbcOpcode opcode,
                                        MethodInfo *methodInfo,
                                        uintptr_t disp_id, uint32_t argc,
                                        Traits *type) {
  CodegenLIR::writeMethodCall(state, pc, opcode, methodInfo, disp_id, argc,
                              type);
}

void ProfileLirEmitter::writeNip(const FrameState *state, const uint8_t *pc,
                                 uint32_t count) {
  CodegenLIR::writeNip(state, pc, count);
}

void ProfileLirEmitter::writeCheckNull(const FrameState *state,
                                       uint32_t index) {
  CodegenLIR::writeCheckNull(state, index);
}

void ProfileLirEmitter::writeCoerce(const FrameState *state, uint32_t index,
                                    Traits *type) {
...
void ProfileLirEmitter::boxArguments(LIns *args) {
  for (int i = 1; i < ms->param_count() + 1; i++) {
    ...
    lirout->insStore(LIR_stp, argument, args, i * sizeof(Atom),
                     ACCSET_STORE_ANY);
  }
}

void ProfileLirEmitter::writePrologue(const FrameState *state,
                                      const uint8_t *pc,
...
void ProfileLirEmitter::writeOpcodeVerified(const FrameState *state,
                                            const uint8_t *pc,
                                            AbcOpcode opcode) {
  CodegenLIR::writeOpcodeVerified(state, pc, opcode);
}

void ProfileLirEmitter::writeFixExceptionsAndLabels(const FrameState *state,
                                                    const uint8_t *pc) {
  CodegenLIR::writeFixExceptionsAndLabels(state, pc);
}

void ProfileLirEmitter::cleanup() { CodegenLIR::cleanup(); }
...
bool ProfileLirEmitter::shouldEmitMethodCounters() {
  ...
  return !info->isNative() &&        // Don't do native methods
         ...
         ms->optional_count() ==
             0 &&                // Don't profile methods with optional args
