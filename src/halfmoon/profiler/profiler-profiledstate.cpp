...
ProfiledState::ProfiledState(Allocator &allocator, int input_count,
                             int output_count)
...
RecordedType ProfiledState::getRecordedType(Atom value) {
  ...
  case kObjectType: {
    ...
    if (object == NULL)
      return kOBJECT; // undefined object
    if (object->toArrayObject() != NULL)
      return kARRAY;
    return kOBJECT;
  }
  case kUnusedAtomTag:
...
bool ProfiledState::isNumericType(RecordedType t) {
  return t == kINTEGER || t == kDOUBLE || t == kNUMERIC;
}
...
void ProfiledState::setInputRecordedType(int input_index,
                                         RecordedType new_type) {
  AvmAssert(hasData());
  AvmAssert(input_index < input_count_);
  RecordedType previous_type = profiled_types_[input_index];

  if ((new_type != previous_type) && (previous_type != kUNINITIALIZED)) {
    new_type = kVARIADIC;
  }
