 ...
 *
 */
PrintWriter &DataType::printConstQual(PrintWriter &console,
                                      AvmCore *core) const {
...
/**
 * for most but not all data types, the default model is determined by the
 * traits.
 */
...
template <class SELF_CLASS, TypeKind TYPE_KIND, ModelKind MODEL_KIND,
          typename VALUE_REP>
PrintWriter &
SimpleDataType<SELF_CLASS, TYPE_KIND, MODEL_KIND, VALUE_REP>::printName(
    PrintWriter &console, AvmCore *core) const {
...
template <class SELF_CLASS, TypeKind TYPE_KIND, ModelKind MODEL_KIND,
          typename VALUE_REP>
const Type *SimpleDataType<SELF_CLASS, TYPE_KIND, MODEL_KIND, VALUE_REP>::copy(
    Lattice &lat) const {
  Type *t = new (lat.alloc)
      SELF_CLASS(model, traits, is_nullable, is_const, const_value);
  AvmAssert(t->name == NULL);
  t->name = buildName(lat);
  return t;
}
...
template <class SELF_CLASS, TypeKind TYPE_KIND, ModelKind MODEL_KIND,
          typename VALUE_REP>
const DataType *
SimpleDataType<SELF_CLASS, TYPE_KIND, MODEL_KIND, VALUE_REP>::changeModel(
    Lattice &lat, ModelKind m) const {
  if (model == m) {
    return this;
  } else if (isCompatibleModel(m)) {
    return (DataType *)lat.makeType(
        SELF_CLASS(m, traits, is_nullable, is_const, const_value));
...
template <class SELF_CLASS, TypeKind TYPE_KIND, ModelKind MODEL_KIND,
          typename VALUE_REP>
const Type *
SimpleDataType<SELF_CLASS, TYPE_KIND, MODEL_KIND, VALUE_REP>::makeNotNull(
    Lattice &lat) const {
  if (!is_nullable)
    return this;
  ...
  return lat.makeType(SELF_CLASS(model, traits, false, is_const, const_value));
}

template <class SELF_CLASS, TypeKind TYPE_KIND, ModelKind MODEL_KIND,
          typename VALUE_REP>
const SELF_CLASS *
SimpleDataType<SELF_CLASS, TYPE_KIND, MODEL_KIND, VALUE_REP>::makeUnion(
    const SELF_CLASS &t, Lattice &lat) const {
  ...
  return (SELF_CLASS *)lat.makeType(SELF_CLASS(
      union_model, union_traits, union_nullable, false, (VALUE_REP)0));
}
...
const Type *
SimpleDataType<SELF_CLASS, TYPE_KIND, MODEL_KIND, VALUE_REP>::makeUnion(
    const Type &t, Lattice &lat) const {
  ...
  if (isDataType(t)) {
    ...
    // if we're null, reverse the test so we only need to handle the
    // other-is-null case
    ...
    case BUILTIN_any:
      if (union_traits == NULL) {
        ...
        if (builtinType(getTraits(&t)) == BUILTIN_null &&
            builtinType(traits) != BUILTIN_void) {
          return lat.makeType(
              SELF_CLASS(model, traits, true, false, (VALUE_REP)0));
        }
      }
    ...
    default:
      if (union_traits == traits) {
        ...
        AvmAssert(halfmoon::defaultModelKind(union_traits) ==
                  kModelScriptObject);
        return lat.makeType(
            ScriptObjectType(kModelScriptObject, union_traits, union_nullable));
      }
    }
  }
...
ScriptObjectType::ScriptObjectType(ModelKind model, Traits *traits,
                                   bool is_nullable)
    : SimpleDataType<ScriptObjectType, kTypeScriptObject, kModelScriptObject,
                     ScriptObject *>(traits, model, is_nullable, false, NULL) {
...
ScriptObjectType::ScriptObjectType(ModelKind model, Traits *traits,
    ...
    : SimpleDataType<ScriptObjectType, kTypeScriptObject, kModelScriptObject,
                     ScriptObject *>(traits, model, const_value == NULL, true,
                                     const_value) {
  AvmAssert(isCompatibleModel(model));
}
...
ScriptObjectType::ScriptObjectType(ModelKind model, Traits *traits,
                                   ...
                                   ScriptObject *const_value)
    : SimpleDataType<ScriptObjectType, kTypeScriptObject, kModelScriptObject,
                     ScriptObject *>(traits, model, is_nullable, is_const,
                                     const_value) {
  AvmAssert(isCompatibleModel(model));
}

PrintWriter &ScriptObjectType::printName(PrintWriter &console,
                                         AvmCore *core) const {
#ifdef AVMPLUS_VERBOSE
...
NamespaceType::NamespaceType(Lattice &lat, ModelKind model, bool is_nullable)
    : SimpleDataType<NamespaceType, kTypeNamespace, kModelNamespace,
                     Namespace *>(lat.namespace_traits, model, is_nullable,
                                  false, NULL) {
  AvmAssert(isCompatibleModel(model));
}

NamespaceType::NamespaceType(Lattice &lat, ModelKind model,
                             Namespace *const_value)
    : SimpleDataType<NamespaceType, kTypeNamespace, kModelNamespace,
                     Namespace *>(lat.namespace_traits, model,
                                  const_value == NULL, true, const_value) {
  AvmAssert(isCompatibleModel(model));
}
...
NamespaceType::NamespaceType(ModelKind model, Traits *traits, bool is_nullable,
    ...
    : SimpleDataType<NamespaceType, kTypeNamespace, kModelNamespace,
                     Namespace *>(traits, model, is_nullable, is_const,
                                  const_value) {
  AvmAssert(isCompatibleModel(model));
}
...
NumberType::NumberType(ModelKind model, Traits *traits,
...
const NumberType *NumberType::makeUnion(const NumberType &t,
                                        Lattice &lat) const {
  NumberConstraint union_constraint = constraint.makeUnion(t.constraint);
  ModelKind union_model =
      (model == kModelAtom || t.model == kModelAtom)
          ? kModelAtom
          : (union_constraint.isInt() || union_constraint.isUInt())
                ? kModelInt
                : kModelDouble;
  return (NumberType *)lat.makeType(
      NumberType(union_model, traits, union_constraint));
}
 ...
 */
bool subtypeof(const Type *t1, const Type *t2) { return t1->isSubtypeOf(*t2); }

bool subtypeof(const Type &t1, const Type &t2) { return t1.isSubtypeOf(t2); }

// CAUTION: wiggly, will change
bool submodelof(const Type *t1, const Type *t2) {
  return model(t1) == model(t2) || isBottom(t1) || isTop(t2) ||
...
bool maybeIndex(const Type *name_type, const Type *index_type) {
  if (isConst(name_type) && isNumber(index_type)) {
    ...
    return !name->isAttr() && name->isRuntime() &&
           name->containsAnyPublicNamespace();
  }
  return false;
}

/**
 * Lattice. Unlike map, The size here is fixed and on colision, a list if
 * created. so it affects get/put performance if items in map are beyond size.
 * hence keeping the size large enough.
 */
Lattice::Lattice(AvmCore *core, Allocator &alloc)
  ...
  scriptobject_type[kTypeNullable] = (ScriptObjectType *)makeType(
      ScriptObjectType(kModelScriptObject, NULL, true));
  scriptobject_type[kTypeNotNull] = (ScriptObjectType *)makeType(
      ScriptObjectType(kModelScriptObject, NULL, false));
}
...
const Type *Lattice::makeType(const Type &type) {
  ...
  AvmAssert(!type.is_const || !isDataType(type) ||
            model(&type) != kModelInvalid);
 ...
 * TODO final param
 */
const DataType *Lattice::makeType(Traits *traits, bool nullable,
                                  ModelKind model) {
...
const DataType *Lattice::makeAtomConst(Traits *traits, Atom val) {
  ...
  case BUILTIN_any: {
    ...
    return AvmCore::isNull(val)
               ? makeObjectConst(traits, (ScriptObject *)atomPtr(val),
                                 kModelAtom)
...
const ScriptObjectType *
Lattice::makeObjectConst(Traits *traits, ScriptObject *val, ModelKind m) {
...
avmplus::Binding Lattice::toBinding(Traits *traits,
                                    const Type *name_type) const {
  ...
  return name->isNsset()
             ? tb->findBinding(name->getName(), name->getNsset())
 ...
 * coercions from a -> I by stating nothing is a sub type of t if t is an
 * interface see check halfmoon/test/functionscripts/classes/interfaceMerge.as
 */
...
const DataType *Lattice::getInstanceType(const Type *class_type) {
  ...
  return class_traits && class_traits->itraits ? makeType(class_traits->itraits)
                                               : NULL;
}
