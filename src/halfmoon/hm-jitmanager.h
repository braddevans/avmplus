...
class JitManager {
  ...
  struct MethodData {
    ...
    GprMethodProc code;
    DeoptData
        *deopt_data; // Not to be confused with Mason's bailout stuff below.
    BailoutData *bailout_data;
    MethodData(BailoutData *deoptMetaData)
        : ir(0), interp_data(0), code(0), deopt_data(0),
          bailout_data(deoptMetaData) {}
  };
...
public:
  ...
  Allocator &mgr_alloc() { return mgr_alloc_; }

  MethodProfileMgr *profile_mgr() const { return profile_mgr_; }

  CodeAlloc &meta_alloc() { return meta_alloc_; }
...
private:
  ...
  Lattice lattice_;
  InfoManager infos_;
  CodeAlloc meta_alloc_; // allocator for deoptimization metadata (similar to
                         // code, but R/W)
