...
Cleaner::Cleaner(InstrGraph *ir)
    : arm_labels_(alloc_, ir->block_count()), ir_(ir), factory_(ir) {}
...
bool Cleaner::simplify(BlockEndInstr *end) {
  ...
  if (k == HR_goto) {
    ...
    //    //   ensureLabel() inserts labels congruent to the branch arms, but
    //    they need
    //    //   to be congruent with the label before the branch that we are
    //    bypassing.
    //    //   fixing phis in this case needs to call translate() on each use
    //    dominated
    //    //   by each arm.
    if (ir_->isEmptyBlock(target)) {
      ...
      switch (kind(succ_end)) {
        ...
        //        case HR_goto:
        //          // Eliminate empty block, degenerate case of hoisting
        //          branches (#2) return hoistGoto(go,
        //          cast<GotoInstr>(succ_end));
      default:
        break;
      }
    }
...
bool Cleaner::joinBlocks(BlockEndInstr *pred, BlockStartInstr *succ,
                         InstrGraph *ir) {
  if (enable_verbose)
    printf("clean: join %s %s%d->%s%d\n", name(pred), kInstrPrefix, pred->id,
           kInstrPrefix, succ->id);
  ...
  if (isConst(type(selector))) {
    ...
    CondInstr *branch2 =
        kind(branch) == HR_if
            ...
            : (CondInstr *)factory_.newSwitchInstr(
                  selector, cast<SwitchInstr>(branch)->num_cases(), argc, 0);
    for (int i = 0; i < argc; ++i)
