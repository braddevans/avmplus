...
LirBuffer *LirHelper2::createLirout(bool verbose, const char *title) {
  ...
  if (verbose || enable_gml) {
    lirbuf->printer =
        new (*lir_alloc) LInsPrinter(*lir_alloc, TR_NUM_USED_ACCS);
    lirbuf->printer->addrNameMap->addAddrRange(pool->core, sizeof(AvmCore), 0,
                                               "core");
  }
#else
  (void)verbose;
...
#ifdef DEBUG
  lirout = validate2 =
      new (*alloc1) ValidateWriter(lirout, lirbuf->printer, title);
#else
  (void)title;
#endif
...
#ifdef NJ_VERBOSE
  if (verbose) {
    lirout = verbose_writer = new (*alloc1) VerboseWriter(
        *alloc1, lirout, lirbuf->printer, &pool->codeMgr->log, title);
  }
#endif
  return lirbuf;
}
...
#ifdef DEBUG
  stp(InsConstPtr((void *)(uintptr_t)0xdeadbeef), new_frame,
      JitFriend::mf_dxns_offset, ACCSET_OTHER);
#endif
...
int32_t hmBeginCatch(AvmCore *core, ExceptionFrame *ef, MethodInfo *info,
                     intptr_t pc, AnyVal *slotPtr) {
  int32_t ordinal;
  ef->beginCatch();
...
void hmBeginTry(ExceptionFrame *ef, AvmCore *core) { ef->beginTry(core); }
void hmEndTry(ExceptionFrame *ef) { ef->endTry(); }
...
enum LirModel { kLirPtr, kLirInt, kLirDouble, kLirVoid };
...
LirEmitter::LirEmitter(Context *cxt, InstrGraph *ir,
    ...
    : LirHelper2(cxt->method->pool()), cxt(cxt),
      signature(cxt->method->getMethodSignature()), ir(ir), env_param(0),
      argc_param(0), ap_param(0), loop_live_(*alloc1), npe_label(0),
      upe_label(0), interrupt_label(0), safepoint_space_(NULL),
      ...
      profiled_info_(profiled_info), bailout_branches_(*alloc1),
      have_safepoints(false), have_catchblocks_(false),
      emittedBeginCatch(false), catchLabels(NULL) {
  allocateTables();
}
...
class EachHotBlock {
public:
  explicit EachHotBlock(InstrGraph *ir,
                        ProfiledInformation *profiled_info = NULL);
...
///
void analyzeEdgeLiveness(HashMap<Instr *, BlockInfo *> &livemap, BitSet &live,
                         Instr *from, Instr *target, Allocator &scratch,
                         int ir_size) {
  ...
  do {
    ...
    for (ArrayRange<BlockStartInstr *> b(blocks_, num_blocks_); !b.empty();
      ...
      case HR_goto: {
        ...
        analyzeEdgeLiveness(livemap, live, go, go->target, scratch, ir->size());
        break;
      }
      case HR_if:
      case HR_switch: {
        // Pick up live bits from each arm
        for (ArrayRange<ArmInstr *> a = armRange((CondInstr *)branch);
             !a.empty();)
...
void LirEmitter::allocateTables() {
  ...
  for (ArrayRange<BlockStartInstr *> b(blocks_, num_blocks_); !b.empty();
    ...
    if (kind(b.front()) == HR_catchblock) {
      ...
      if (catchLabels == NULL) {
        catchLabels =
            new (*lir_alloc) HashMap<intptr_t, CatchBlock *>(*lir_alloc, 4);
      }
...
///
inline LIns *LirEmitter::def_ins(const Def *d) { return def_ins_[defId(d)]; }

/// Get the LIns* for a specific Def, given any Use of that Def.
///
inline LIns *LirEmitter::def_ins(const Use &u) { return def_ins(def(u)); }
...
///
inline LIns *LirEmitter::ins(Instr *i) { return instr_ins[i->id]; }
...
void LirEmitter::emitBegin(bool has_reachable_exceptions) {
  ...
  if (has_reachable_exceptions) {
    ...
    safepoint_space_ =
        stackAlloc(alloc_size << VARSHIFT(cxt->method), "deopt locals");
  } else {
    _save_eip = NULL;
    _ef = NULL;
...
LIns *LirEmitter::emitAvmCall(const Use *args_in, int actual_argc,
  ...
  LIns *result =
      emitIndirectAvmCall(type(value_out), callee_env, actual_argc, args_);
  return result;
}
...
LIns *LirEmitter::emitInterfaceAvmCall(const Type *result_type,
                                       LIns *callee_env, int actual_argc,
                                       LIns *args, LIns *iid) {
  LIns *addr = ldp(callee_env, JitFriend::env_impl_offset, ACCSET_OTHER);
  // AVM calling convention doesn't count the receiver parameter.
  LIns *argc_ins = InsConst(actual_argc - 1);
...
LIns *LirEmitter::emitLoad(const Type *value_type, LIns *ptr, int32_t offset,
  ...
  default:
    AvmAssert(false && "bad lirtype");
  case kLirPtr:
    return ldp(ptr, offset, accSet, qual);
  case kLirInt:
    return ldi(ptr, offset, accSet, qual);
  case kLirDouble:
    return ldd(ptr, offset, accSet, qual);
  }
}
 ...
 */
LIns *LirEmitter::emitStore(const Use &value, const Type *constraint, LIns *ptr,
                            int32_t offset, AccSet acc_set) {
  ...
  case kLirPtr:
    return stp(def_ins(value), ptr, offset, acc_set);
  case kLirInt:
    return sti(def_ins(value), ptr, offset, acc_set);
  case kLirDouble:
    return std(def_ins(value), ptr, offset, acc_set);
  }
}
...
void LirEmitter::emitLiveHints(Instr *target_block) {
  ...
  for (SeqRange<Instr *> i(live_instrs); !i.empty();
       i.popFront()) // for each live instr
    for (ArrayRange<Def> d = defRange(i.front()); !d.empty();
         d.popFront()) //   for each def
      emitLive(&d.front());
}
...
GprMethodProc LirEmitter::finish(DeoptData **deopt_data) {
  ...
  if (enable_deopt) {
    ...
    mdwriter = new (*lir_alloc)
        HMDeoptDataWriter(meta_alloc, *lir_alloc, signature, def_ins_);
  }
  *deopt_data = 0;
...
void LirEmitter::do_stub(Instr *instr, const CallInfo *call) {
  ...
  for (ArrayRange<Use> u = useRange(instr); !u.empty(); s.popFront()) {
    ...
    if (argc < limit) {
      ...
      offset +=
          (model(sig_type) == kModelDouble) ? sizeof(double) : sizeof(Atom);
      varargc++;
    }
  }
  if (fixc != -1) {
...
uint32_t *allocDefaultParams(CodeMgr *mgr, MethodSignaturep signature) {
  ...
  for (int i = 0; !p.empty(); p.popFront(), ++i) {
    if (i <= required_count) {
      ...
      default:
        AvmAssert(false && "bad slotStorageType");
      case SST_int32:
...
void LirEmitter::do_loadsuperinitenv(UnaryExpr *instr) {
  ...
  LIns *scope =
      ldp(env_in, JitFriend::env_scope_offset, ACCSET_OTHER, LOAD_CONST);
  LIns *vtable =
      ldp(scope, JitFriend::scope_vtable_offset, ACCSET_OTHER, LOAD_CONST);
...
void LirEmitter::do_callmethod(CallStmt2 *call) {
  ...
  LIns *result = emitAvmCall(call->args(), call->arg_count(), callee_sig,
                             callee_env, call->value_out());
...
void LirEmitter::do_callinterface(CallStmt2 *call) {
  ...
  AvmAssert(isBottom(value_out_type) ||
            model(value_out_type) ==
                defaultModelKind(interface_sig->returnTraits()));
...
/// This goto is a fall-through path if it jumps to the next block in linear
/// order.
///
...
bool LirEmitter::isFallthruArm(ArmInstr *arm) {
  ...
  return kind(owner) == HR_if && current_block_ > 0 &&
         blocks_[current_block_ - 1] == owner_block;
}
  ...
  for (int i = 0, n = numUses(go); i < n; ++i) {
    ...
    if (!isLinear(type(arg)) && !isState(type(arg))) {
      emitStore(arg, type(go->target->params[i]), args_,
                i << VARSHIFT(cxt->method), ACCSET_OTHER);
    }
  }
...
void LirEmitter::do_label(LabelInstr *instr) {
  ...
  for (int i = 0, n = instr->paramc; i < n; ++i) {
    ...
    if (!isLinear(param_type) && !isState(param_type)) {
      set_def_ins(param, emitLoad(param_type, args_, i << VARSHIFT(cxt->method),
...
void LirEmitter::emitBeginCatch() {
  ...
  if (have_catchblocks_) {
    ...
    LIns *pc =
        lirout->insLoad(LIR_ldp, _save_eip, 0, ACCSET_OTHER, LOAD_NORMAL);
    LIns *slotAddr = lea(stackBase << VARSHIFT(info), safepoint_space_);
    LIns *handler_ordinal = callIns(FUNCTIONID(hmBeginCatch), 5, coreAddr, _ef,
                                    InsConstPtr(info), pc, slotAddr);
...
void LirEmitter::do_catchblock(CatchBlockInstr *instr) {
  ...
  for (int i = 0, n = instr->paramc; i < n; ++i) {
    ...
    const Type *param_type = type(param);
    if (!isLinear(param_type) && !isState(param_type)) {
      set_def_ins(param, emitLoad(param_type, safepoint_space_,
                                  i << VARSHIFT(cxt->method), ACCSET_OTHER,
                                  LOAD_NORMAL));
    }
  }
...
void LirEmitter::do_greaterthan(BinaryExpr *instr) {
  LIns *compare_atom =
      emitCompare(def_ins(instr->rhs_in()), def_ins(instr->lhs_in()));
...
void LirEmitter::do_lessthan(BinaryExpr *instr) {
  LIns *compare_atom =
      emitCompare(def_ins(instr->lhs_in()), def_ins(instr->rhs_in()));
  LIns *bool_out = eqp(compare_atom, InsConstAtom(trueAtom));
  set_def_ins(instr->value_out(), bool_out);
}
...
/// as the int type. If we speculated number, we'd deopt when its maybe better
/// to just upconvert to a number. boids_untyped is slower for now
void LirEmitter::do_speculate_numeric(BinaryExpr *instr) {
  ...
  bailout_branches_.add(typecheck_fail);
  set_def_ins(instr->value_out(),
              callIns(FUNCTIONID(number), 1, def_ins(instr->lhs_in())));
}

void LirEmitter::do_speculate_int(BinaryExpr *instr) {
  addSpeculativeTypeCheck(instr->lhs_in(), kIntptrType);
  set_def_ins(instr->value_out(),
              callIns(FUNCTIONID(integer_i), 1, def_ins(instr->lhs_in())));
}
...
void LirEmitter::do_speculate_array(BinaryExpr *instr) {
  ...
  LIns *array_object =
      callIns(FUNCTIONID(scriptObjectToArrayObject), 1, script_object);
...
LIns *LirEmitter::atomToScriptObject(LIns *object) { return andp(object, ~7); }

void LirEmitter::writeSlot(LIns *object, LIns *value_in, const Type *value_type,
                           uint32_t offset) {
...
void LirEmitter::do_newinstance(UnaryExpr *instr) {
  ...
  LIns *ivtable =
      ldp(vtable, offsetof(VTable, ivtable), ACCSET_OTHER, LOAD_CONST);
  LIns *funcptr =
      ldp(ivtable, offsetof(VTable, createInstanceProc), ACCSET_OTHER);
...
LIns *LirEmitter::emitNpeHandler() { return emitHandler(&npe_label, &ci_npe); }

/** Generate one handler for all kConvertUndefinedToObjectError errors. */
LIns *LirEmitter::emitUpeHandler() { return emitHandler(&upe_label, &ci_upe); }
...
void LirEmitter::do_abc_getprop(CallStmt2 *instr) {
  ...
  LIns *value =
      callIns(&ci_get_cache_handler, 4,
...
void LirEmitter::do_abc_setprop(CallStmt2 *instr) {
  ...
  callIns(&ci_set_cache_handler, 5,
          ...
          cache_addr, def_ins(instr->object_in()), value_ins, env_param);
}
...
void LirEmitter::do_abc_callprop(CallStmt2 *instr) {
  ...
  LIns *cache_addr = InsConstPtr(cache);
  LIns *value =
      callIns(&ci_call_cache_handler, 6,
              ldp(cache_addr, offsetof(CallCache, call_handler), ACCSET_OTHER),
              cache_addr, def_ins(instr->object_in()), InsConst(arg_count - 1),
              args_, env_param);
  set_def_ins(instr->value_out(), value);
}
...
void LirEmitter::do_abc_add(BinaryStmt *instr) {
  ...
  LIns *result =
      callIns(&ci_op_add_a_aa, 3, coreAddr, left_operand, right_operand);
  set_def_ins(instr->value_out(), result);
}
...
void LirEmitter::do_loadenv_interface(BinaryExpr *instr) {
  ...
  LIns *env = ldp(
      vtable, JitFriend::vtable_imt_offset + imt_index * sizeof(ImtThunkEnv *),
      ACCSET_OTHER, LOAD_CONST);
  set_def_ins(instr->value_out(), env);
}
...
void LirEmitter::do_setlocal(SetlocalInstr *instr) {
  ...
  emitStore(instr->value_in(), type(instr->value_in()), safepoint_space_,
            stackIndex << VARSHIFT(cxt->method), ACCSET_STORE_ANY);
  set_def_ins(instr->state_out(), def_ins(instr->state_in()));
...
void LirEmitter::do_abc_hasnext2(Hasnext2Stmt *instr) {
  ...
  set_def_ins(instr->object_out(), emitLoad(type(instr->object_in()), object, 0,
                                            ACCSET_OTHER, LOAD_NORMAL));

  set_def_ins(instr->counter_out(), emitLoad(type(instr->counter_in()), counter,
                                             0, ACCSET_OTHER, LOAD_NORMAL));
}
