...
InstrKind swap(InstrKind k) {
  ...
  switch (k) {
  default:
    AvmAssert(false && "bad kind. fix isInvertable() or add case");
  case HR_addi:
  case HR_muli:
  case HR_ori:
  case HR_andi:
...
const Type *fold(Lattice *l_, InstrKind k, const Type *t) {
  switch (k) {
  case HR_negi:
    return callstub(l_, t, Stubs::do_negi);
  case HR_negd:
    return callstub(l_, t, Stubs::do_negd);
  case HR_noti:
    return callstub(l_, t, Stubs::do_noti);
  case HR_u2i:
    return callstub(l_, t, Stubs::do_u2i);
  case HR_i2u:
    return callstub(l_, t, Stubs::do_i2u);
  case HR_d2b:
    return callstub(l_, t, Stubs::do_d2b);
  default:
    return 0;
  }
}
...
const Type *fold(Lattice *l_, InstrKind k, const Type *x, const Type *y) {
  switch (k) {
  case HR_addi:
    return callstub(l_, x, y, Stubs::do_addi);
  case HR_subi:
    return callstub(l_, x, y, Stubs::do_subi);
  case HR_muli:
    return callstub(l_, x, y, Stubs::do_muli);
  default:
    return 0;
  }
}
...
public:
  ...
  static size_t hash(UnaryKey &k) { return k.kind * defId(k.value); }
...
template <class K, class V> struct ScopedTable {
...
private:
  template <class K, class M> Def *lookup(Def *d, M *map, K k) {
...
bool isConst(Def *x) { return isConst(type(x)); }
...
class Simplifier {
public:
  Simplifier(Lattice *lattice, ExprTable *ht) : l_(lattice), ht(ht) {}
...
Def *Simplifier::simplify(Def *d, InstrKind k, Def *x) {
  ...
  case HR_u2i:
    if (match(x, HR_i2u, &t))
      return t;
    break;
  // Bug # 3727278: if two newinstances are in same block, we can't replace one
  // with other.
  case HR_newinstance:
    return d;
  }
...
Def *Simplifier::simplify(Def *d, InstrKind k, Def *x0, Def *y0) {
  ...
  case HR_subi:
    if (x == y)
      return ht->lookup(d, l_->makeIntConst(0)); // x-x = 0
    if (match(y, 0))
      return x; // x-0 = x
...
Def *Simplifier::simplify(Def *d, InstrKind k, int pos, Def *effect,
                          Def *value) {
...
void DVN::pass() {
  ...
  for (EachBlock b(ir_); !b.empty();) {
    ...
    ExprTable *idom_table =
        doms->hasIDom(block) ? tables[doms->idom(block)->blockid] : 0;
    AvmAssert((!doms->hasIDom(block) || idom_table) &&
              "visited block before its dominator");
