...
//
// TODO: We really need a way to distinguish the halfmoon runtime from the
// halfmoon optimizer/compiler
//
...
inline static AvmCore *core(MethodFrame *f) { return f->env()->core(); }

inline static MethodEnv *env(MethodFrame *f) { return f->env(); }
...
inline static BoolKind boolKind(bool b) { return b ? kBoolTrue : kBoolFalse; }
...
Atom Stubs::do_string2atom(MethodFrame *, String *s) { return s->atom(); }

ScriptObject *Stubs::do_abc_finddef(MethodFrame *, const Multiname *name,
                                    MethodEnv *env) {
...
MethodEnv *Stubs::do_loadenv_interface(MethodFrame *,
                                       MethodInfo *interface_info,
                                       ScriptObject *object) {
  return JitFriend::imtEntry(object->vtable, interface_info);
}

MethodEnv *Stubs::do_loadenv_string(MethodFrame *f, int disp_id,
                                    String *object) {
  (void)object;
  return toplevel(f)->stringClass()->ivtable()->methods[disp_id];
}

MethodEnv *Stubs::do_loadenv_number(MethodFrame *f, int disp_id,
                                    double object) {
  (void)object;
  return toplevel(f)->numberClass()->ivtable()->methods[disp_id];
}

MethodEnv *Stubs::do_loadenv_namespace(MethodFrame *f, int disp_id,
                                       Namespace *object) {
  (void)object;
  return toplevel(f)->namespaceClass()->ivtable()->methods[disp_id];
}

MethodEnv *Stubs::do_loadenv_boolean(MethodFrame *f, int disp_id,
                                     BoolKind object) {
  (void)object;
...
MethodEnv *Stubs::do_loadenv_env(avmplus::MethodFrame *, int method_id,
                                 avmplus::MethodEnv *env) {
...
double Stubs::do_tonumber(MethodFrame *, Atom x) { return AvmCore::number(x); }

int32_t Stubs::do_toint(MethodFrame *, Atom x) { return AvmCore::integer(x); }
...
double Stubs::do_addd(MethodFrame *, double x, double y) { return x + y; }

double Stubs::do_divd(MethodFrame *, double x, double y) { return x / y; }

double Stubs::do_modulo(MethodFrame *, double x, double y) {
  return MathUtils::mod(x, y);
}

double Stubs::do_subd(MethodFrame *, double x, double y) { return x - y; }

double Stubs::do_muld(MethodFrame *, double x, double y) { return x * y; }

double Stubs::do_negd(MethodFrame *, double x) { return -x; }

int32_t Stubs::do_addi(MethodFrame *, int32_t x, int32_t y) { return x + y; }

int32_t Stubs::do_subi(MethodFrame *, int32_t x, int32_t y) { return x - y; }

int32_t Stubs::do_muli(MethodFrame *, int32_t x, int32_t y) { return x * y; }

int32_t Stubs::do_ori(MethodFrame *, int32_t x, int32_t y) { return x | y; }

int32_t Stubs::do_andi(MethodFrame *, int32_t x, int32_t y) { return x & y; }

int32_t Stubs::do_xori(MethodFrame *, int32_t x, int32_t y) { return x ^ y; }
...
int32_t Stubs::do_noti(MethodFrame *, int32_t x) { return ~x; }

int32_t Stubs::do_negi(MethodFrame *, int32_t x) { return -x; }
...
Atom Stubs::do_makenullrefexception(MethodFrame *f, Atom x) {
  int errorID = x == undefinedAtom ? kConvertUndefinedToObjectError
...
BoolKind Stubs::do_not(MethodFrame *, BoolKind x) { return boolKind(!x); }
...
double Stubs::do_i2d(MethodFrame *, int32_t x) { return x; }

double Stubs::do_u2d(MethodFrame *, uint32_t x) { return x; }

int32_t Stubs::do_d2i(MethodFrame *, double d) { return (int32_t)d; }

uint32_t Stubs::do_d2u(MethodFrame *, double d) { return (uint32_t)d; }

uint32_t Stubs::do_i2u(MethodFrame *, int32_t x) { return x; }

int32_t Stubs::do_u2i(MethodFrame *, uint32_t x) { return x; }
...
Atom Stubs::do_ns2atom(MethodFrame *, Namespace *ns) { return ns->atom(); }
...
void Stubs::do_throw(MethodFrame *f, Atom e) { return core(f)->throwAtom(e); }
...
void Stubs::do_debugline(MethodFrame *f, int32_t line) {
  ...
  if (avmcore->debugger())
    avmcore->debugger()->debugLine(line);
#else
  (void)f;
  (void)line;
#endif
...
void Stubs::do_debugfile(MethodFrame *f, String *file) {
  ...
  AvmCore *avmcore = core(f);
  if (avmcore->debugger())
    avmcore->debugger()->debugFile(file);
#else
  (void)f;
  (void)file;
#endif
...
void Stubs::do_debug(avmplus::MethodFrame *f, avmplus::String *name,
                     int32_t local) {
#if defined DEBUGGER
  AvmCore *avmcore = core(f);
  if (!avmcore->debugger())
...
inline Atom abc_find(MethodEnv *env, const Multiname *name, Atom *scopes,
  ...
  return env->findproperty(env->scope(), scopes, scope_count, name, strict,
                           withbase);
}
...
inline Atom abc_findx(MethodEnv *env, const Multiname *name, Atom index,
  ...
  return env->findproperty(env->scope(), scopes, scope_count, &tempname, strict,
                           withbase);
}
...
Atom Stubs::do_abc_findproperty(MethodFrame *, const Multiname *name,
                                MethodEnv *env, int32_t withbase,
                                int scope_count, Atom *scopes) {
...
Atom Stubs::do_abc_findpropertyx(MethodFrame *, const Multiname *name,
                                 MethodEnv *env, int32_t withbase, Atom index,
                                 int scope_count, Atom *scopes) {
  AvmAssert(withbase >= -1 && withbase < scope_count);
  Atom *withbasep = (withbase == -1) ? NULL : scopes + withbase;
...
BoolKind Stubs::do_abc_istypelate(MethodFrame *f, Atom object, Atom atomType) {
  return BoolKind(
      AvmCore::istype(object, toplevel(f)->toClassITraits(atomType)));
}
...
ScriptObject *Stubs::do_newobject(MethodFrame *f, int argc, Atom *argv) {
  ...
  int numberOfProperties =
      argc / 2; // argc for newobject is actually the number of property pairs
...
void checkXMLList(MethodFrame *f, Atom index) {
  if (AvmCore::isXMLList(index)) {
    ...
    toplevel(f)->throwTypeError(
        kDeleteTypeError, core(f)->toErrorString(toplevel(f)->toTraits(index)));
  }
}
...
BoolKind Stubs::do_abc_deletepropx(MethodFrame *f, const Multiname *name,
  ...
  if (!AvmCore::isDictionaryLookup(index, object)) {
    ...
    return boolKind(
        AvmCore::atomToScriptObject(object)->deleteAtomProperty(index));
  }
}
...
Atom Stubs::do_abc_callproplexns(MethodFrame *f, const Multiname *name, Atom ns,
                                 int argc, Atom *args) {
  Multiname tempname;
  MethodEnv *e = env(f);
  initnamens(e, name, ns, &tempname);
  Atom base = getBase(args);
  return toplevel(f)->callproperty(base, name, argc - 1, args,
                                   toVTable(e, base));
}
...
void Stubs::do_constructsuper(MethodFrame *, MethodEnv *e, int argc,
                              Atom *argv) {
...
Traits *Stubs::do_slottype(MethodFrame *, ScriptObject *, int) {
  AvmAssert(false && "slottype not implemented");
  return 0;
}
int Stubs::do_toslot(MethodFrame *, ScriptObject *, const Multiname *) {
  AvmAssert(false && "toslot not implemented");
  return 0;
}
void Stubs::do_never(MethodFrame *) {
  AvmAssert(false && "never not implemented");
}
