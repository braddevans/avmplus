...
template <typename T> struct Range {
  ...
  Range<T> join(const Range<T> &r) const {
    return Range<T>(min(x, r.x), max(y, r.y));
  }
  Range<T> meet(const Range<T> &r) const {
    return Range<T>(max(x, r.x), min(y, r.y));
  }
};

/// RangeConstraint
///
template <typename T> class RangeConstraint {
...
public:
  ...
  bool implies(const RangeConstraint<T> &rc) const {
    return (sense == rc.sense) ? (sense ? rc.r.contains(r) : r.contains(rc.r))
                               : (sense && r.disjoint(rc.r));
  }

  static RangeConstraint<T> eq(const T x) {
    return RangeConstraint<T>(x, x, true);
  }
  static RangeConstraint<T> in(const T x, const T y) {
    return RangeConstraint<T>(x, y, true);
  }
};
