...
public: // dispatch() adapter methods.
  ...
  void do_atom2scriptobject(UnaryExpr *i) {
    doChangeModel(i, kModelScriptObject);
  }
  ...
  void do_caststring(UnaryStmt *i) {
    doCoerceInstr(i, lattice_->string_type[kTypeNullable]);
  }
  void do_castobject(UnaryExpr *i) {
    doCoerceInstr(i, lattice_->object_type[kTypeNullable]);
  }
