...
enum BoolKind { kBoolFalse, kBoolTrue };
...
///
InstrGraph *parseAbc(MethodInfo *, Lattice *, InfoManager *,
                     Allocator &ir_alloc, AbcGraph *, Toplevel *toplevel,
...
struct Context {
  Context(MethodInfo *m, PrintWriter &out, Toplevel *tl, AbcEnv *abc_env,
      ...
      : method(m), out(out), abc_env(abc_env), toplevel(tl), caller(caller),
        inline_depth(caller ? caller->inline_depth + 1 : 0) {}
  Context(MethodInfo *m)
      : method(m), out(method->pool()->core->console), caller(NULL),
        inline_depth(0) {}
  MethodInfo *method;
  PrintWriter &out;
  ...
  const Context
      *caller; // calling context. like stacktrace of inlines in progress.
...
public:
  ...
  ///
  template <class INSTR> INSTR *copy(INSTR *i) { return (INSTR *)deepCopy(i); }
