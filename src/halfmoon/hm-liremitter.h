...
public:
  LirHelper2(PoolObject *pool) : LirHelper(pool), verbose_writer(0) {}
  ...
  LIns *ldp(LIns *ptr, int32_t d, AccSet acc_set,
            LoadQual load_qual = LOAD_NORMAL) {
    return lirout->insLoad(LIR_ldp, ptr, d, acc_set, load_qual);
  }
  LIns *ldd(LIns *ptr, int32_t d, AccSet acc_set,
            LoadQual load_qual = LOAD_NORMAL) {
    return lirout->insLoad(LIR_ldd, ptr, d, acc_set, load_qual);
  }
  LIns *ldi(LIns *ptr, int32_t d, AccSet acc_set,
            LoadQual load_qual = LOAD_NORMAL) {
    return lirout->insLoad(LIR_ldi, ptr, d, acc_set, load_qual);
  }
  LIns *jgeui(LIns *a, uint32_t b, LIns *target) {
    return lirout->insBranch(LIR_jt, lirout->ins2(LIR_geui, a, InsConst(b)),
                             target);
  }
  LIns *retd(LIns *value) { return lirout->ins1(LIR_retd, value); }
  LIns *eqi(LIns *a, int b) { return lirout->ins2(LIR_eqi, a, InsConst(b)); }
...
public: // ADAPTER impl
  ...
  // fixme: re-implement-this using finddef_cache void
  // do_abc_finddef(BinaryStmt*);
...
private:
  ...
  void writeSlot(LIns *object, LIns *value, const Type *value_type,
                 uint32_t offset);
  ...
  LIns *emitIndirectAvmCall(const Type *result_type, LIns *callee_env, int argc,
                            LIns *args);
  LIns *emitInterfaceAvmCall(const Type *result_type, LIns *callee_env,
                             int argc, LIns *args, LIns *iid);
