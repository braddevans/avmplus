...
JitManager::JitManager(Allocator &mgr_alloc, PoolObject *pool)
    : mgr_alloc_(mgr_alloc), mgr_alloc0_(mgr_alloc),
      lattice_(pool->core, mgr_alloc), infos_(mgr_alloc),
...
GprMethodProc JitManager::getImpl(MethodInfo *method) {
  ...
  if (haveInterp(method_id))
    ...
    case BUILTIN_number:
      return (GprMethodProc)Interpreter::fprStub;
    case BUILTIN_int:
      return Interpreter::intStub;
    case BUILTIN_uint:
    case BUILTIN_boolean:
      return Interpreter::uintStub;
    default:
      return Interpreter::gprStub;
    }
  return 0;
}
...
JitManager::MethodData *JitManager::ensureMethodData(MethodInfo *method) {
  ...
  if (!method_data) {
    ...
    BailoutData *meta_data = new (mgr_alloc_)
        BailoutData(mgr_alloc_, method, scope_base - 1, operand_base - 1);
