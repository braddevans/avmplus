...
#define HASH(c) (fvn1aHash = FNV_prime * (fvn1aHash ^ (uint8_t)c))
  ...
  while (getline(in, linebuf)) {
    ...
    if (command == "") {
      ...
      // struct <size> <name> [nativeSlotTemplate] -- introduces a structure
      // with a given size and name
      unsigned size = 0;
      ...
      // field <offset> <type> <name> [slot] -- defines a field with the given
      // offset, type and name
      FieldInfo field;
      if (!currentStruct) {
      ...
      // Internally, slots are numbered from zero; in the ABC, they are numbered
      // from 1. We use -1 to signify that the field is not a slot.
      field.slot = slot - 1;
      if (field.name.empty()) {
        err << lineNum << ": missing info - " << linebuf << "\n";
        ++errors;
...
StructInfo::StructInfo(const StructInfo &s, LLVMModule &module)
    : name(s.name), size(s.size), traitsIndex(-1), type(0), traits(0),
      fields(s.fields) {
...
StructInfo *LLVMModule::getStructInfo(llvm::raw_ostream &err,
                                      const std::string &name,
                                      llvm::Type **typeP,
                                      llvm::PointerType **ptrTypeP,
                                      int *errCntp) {
...
FieldInfo *LLVMModule::getFieldInfo(llvm::raw_ostream &err, StructInfo *s,
                                    const std::string &name, int *errCntp) {
...
bool LLVMModule::initializeFieldInfos(llvm::raw_ostream &err) {
  ...
  if (StructInfo *s = getStructInfo(err, "avmplus::AvmCore", &avmCoreTy,
                                    &avmCorePtrTy, &errCnt)) {
    avmcore_methodframe_member =
        getFieldInfo(err, s, "currentMethodFrame", &errCnt);
    avmcore_exceptionAddr_member =
        getFieldInfo(err, s, "exceptionAddr", &errCnt);
    avmcore_gc_member = getFieldInfo(err, s, "gc", &errCnt);
  }

  if (StructInfo *s =
          getStructInfo(err, "avmplus::ExceptionFrame", &exceptionFrameTy,
                        &exceptionFramePtrTy, &errCnt)) {
    exceptionFrameStruct = s;
    exceptionFrame_jmpbuf_member = getFieldInfo(err, s, "jmpbuf", &errCnt);
  }

  if (StructInfo *s =
          getStructInfo(err, "AOTInfo", &aotInfoTy, &aotInfoPtrTy, &errCnt)) {
    aotInfoStruct = s;
    ...
    aotInfo_methodDebugInfos_member =
        getFieldInfo(err, s, "methodDebugInfos", &errCnt);
    ...
    aotInfo_methodFlagsArr_member =
        getFieldInfo(err, s, "methodFlagsArr", &errCnt);
    aotInfo_activationInfo_member =
        getFieldInfo(err, s, "activationInfo", &errCnt);
  }

  if (StructInfo *s =
          getStructInfo(err, "AOTExceptionDesc", &aotExceptionDescTy,
                        &aotExceptionDescPtrTy, &errCnt)) {
    ...
    aotExceptionDesc_nameIndex_member =
        getFieldInfo(err, s, "nameIndex", &errCnt);
    aotExceptionDesc_typeIndexU30_member =
        getFieldInfo(err, s, "typeIndexU30", &errCnt);
  }

  if (StructInfo *s = getStructInfo(err, "MethodDebugInfo", &methodDebugInfoTy,
                                    &methodDebugInfoPtrTy, &errCnt)) {
    methodDebugInfoStruct = s;
    methodDebugInfo_local_count_member =
        getFieldInfo(err, s, "local_count", &errCnt);
    methodDebugInfo_scope_count_member =
        getFieldInfo(err, s, "scope_count", &errCnt);
    methodDebugInfo_file_name_member =
        getFieldInfo(err, s, "file_name", &errCnt);
    methodDebugInfo_local_names_member =
        getFieldInfo(err, s, "local_names", &errCnt);
    methodDebugInfo_line_count_member =
        getFieldInfo(err, s, "line_count", &errCnt);
    methodDebugInfo_lines_member = getFieldInfo(err, s, "lines", &errCnt);
  }

  if (StructInfo *s =
          getStructInfo(err, "AOTActivationInfo", &aotActivationInfoTy,
                        &aotActivationInfoPtrTy, &errCnt)) {
    aotActivationInfoStruct = s;
    aotActivationInfo_initMethodId_member =
        getFieldInfo(err, s, "initMethodId", &errCnt);
    aotActivationInfo_initHandler_member =
        getFieldInfo(err, s, "initHandler", &errCnt);
  }

  if (StructInfo *s = getStructInfo(err, "avmplus::Exception", &exceptionTy,
                                    &exceptionPtrTy, &errCnt)) {
    exception_atom_member = getFieldInfo(err, s, "atom", &errCnt);
  }

  if (StructInfo *s = getStructInfo(err, "avmplus::String", &stringTy,
                                    &stringPtrTy, &errCnt)) {
    (void)s;
  }

  if (StructInfo *s = getStructInfo(err, "avmplus::MethodFrame", &methodFrameTy,
                                    &methodFramePtrTy, &errCnt)) {
    methodframe_next_member = getFieldInfo(err, s, "next", &errCnt);
    methodframe_env_member = getFieldInfo(err, s, "env", &errCnt);
  }

  if (StructInfo *s =
          getStructInfo(err, "avmplus::CallStackNode", &callStackNodeTy,
                        &callStackNodePtrTy, &errCnt)) {
    (void)s;
  }

  if (StructInfo *s =
          getStructInfo(err, "avmplus::MethodInfo::NativeInfo",
                        &methodInfoNativeTy, &methodInfoNativePtrTy, &errCnt)) {
    methodinfonative_handler_member = getFieldInfo(err, s, "handler", &errCnt);
  }

  if (StructInfo *s = getStructInfo(err, "avmplus::MethodInfo", &methodInfoTy,
                                    &methodInfoPtrTy, &errCnt)) {
    methodinfo_pool_member = getFieldInfo(err, s, "pool", &errCnt);
    methodinfo_native_member = getFieldInfo(err, s, "native", &errCnt);
  }

  if (StructInfo *s = getStructInfo(err, "avmplus::Multiname", &multinameTy,
                                    &multinametPtrTy, &errCnt)) {
    (void)s;
  }

  if (StructInfo *s =
          getStructInfo(err, "ESC_HeapMultiname", &escHeapMultinameTy,
                        &escHeapMultinamePtrTy, &errCnt)) {
    escHeapMultiname_elements_member =
        getFieldInfo(err, s, "elements", &errCnt);
  }

  if (StructInfo *s =
          getStructInfo(err, "avmplus::TracedListData<avmplus::MethodInfo*>",
                        &methodInfoListTy, &methodInfoListPtrTy, &errCnt)) {
    methodInfoList_entries_member = getFieldInfo(err, s, "entries", &errCnt);
  }

  if (StructInfo *s =
          getStructInfo(err, "ListAccess<avmplus::MethodInfo*>",
                        &methodInfoGCListTy, &methodInfoGCListPtrTy, &errCnt)) {
    methodInfoGCList_m_list_member = getFieldInfo(err, s, "m_list", &errCnt);
  }

  if (StructInfo *s =
          getStructInfo(err, "avmplus::TracedListData<avmplus::Namespace*>",
                        &namespaceListTy, &methodInfoListPtrTy, &errCnt)) {
    namespaceList_entries_member = getFieldInfo(err, s, "entries", &errCnt);
  }

  if (StructInfo *s =
          getStructInfo(err, "ListAccess<avmplus::Namespace*>",
                        &namespaceRCListTy, &namespaceRCListPtrTy, &errCnt)) {
    namespaceRCList_m_list_member = getFieldInfo(err, s, "m_list", &errCnt);
  }

  if (StructInfo *s =
          getStructInfo(err, "avmplus::TracedListData<avmplus::Traits*>",
                        &traitsTracedListTy, &traitsTracedListPtrTy, &errCnt)) {
    traitsTracedList_entries_member = getFieldInfo(err, s, "entries", &errCnt);
  }

  if (StructInfo *s = getStructInfo(err, "ListAccess<avmplus::Traits*>",
                                    &traitsListTy, &traitsListPtrTy, &errCnt)) {
    traitsList_m_list_member = getFieldInfo(err, s, "m_list", &errCnt);
  }

  if (StructInfo *s = getStructInfo(err, "avmplus::PoolObject", &poolObjectTy,
                                    &poolObjectPtrTy, &errCnt)) {
    poolobject_core_member = getFieldInfo(err, s, "core", &errCnt);
    poolobject_escHeapMultiname_member =
        getFieldInfo(err, s, "precompNames", &errCnt);
  ...
  if (StructInfo *s = getStructInfo(err, "avmplus::VTable", &vtableTy,
                                    &vtablePtrTy, &errCnt)) {
    ...
    vtable_createinstanceproc_member =
        getFieldInfo(err, s, "createInstanceProc", &errCnt);
    vtable_base_member = getFieldInfo(err, s, "base", &errCnt);
  }

  if (StructInfo *s = getStructInfo(err, "avmplus::Traits", &traitsTy,
                                    &traitsPtrTy, &errCnt)) {
    traits_itraits_member = getFieldInfo(err, s, "itraits", &errCnt);
  }

  if (StructInfo *s = getStructInfo(err, "avmplus::MethodEnv", &methodEnvTy,
                                    &methodEnvPtrTy, &errCnt)) {
  ...
  if (StructInfo *s =
          getStructInfo(err, "avmplus::ScriptObject", &scriptObjectTy,
                        &scriptObjectPtrTy, &errCnt)) {
    scriptObjectStruct = s;
    scriptobject_vtable_member = getFieldInfo(err, s, "vtable", &errCnt);
  }

  if (StructInfo *s = getStructInfo(err, "avmplus::ScopeChain", &scopeChainTy,
                                    &scopeChainPtrTy, &errCnt)) {
  ...
  if (StructInfo *s = getStructInfo(err, "avmplus::AbcEnv", &abcEnvTy,
                                    &abcEnvTPtrTy, &errCnt)) {
    abcenv_finddeftbl_member = getFieldInfo(err, s, "m_finddef_table", &errCnt);
  }

  if (StructInfo *s = getStructInfo(err, "FinddefTableType", &finddefTableTy,
                                    &finddefTablePtrTy, &errCnt)) {
