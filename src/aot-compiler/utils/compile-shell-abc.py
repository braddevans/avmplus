...
def aot_compile(self):
    base_options = "-mtriple=" + self.arch_name + "-apple-darwin -filetype=obj -sdk %s/shell_toplevel.abc -fields " \
                                                  "%s/shell-fields.txt" % (
        self.sdk_dir, self.sdk_dir)
    if self.debug:
    ...
    def aot_link(self):
        base_options = "-arch " + self.arch_name + " -L%s/lib -llzma -lzlib -lavmplus -lMMgc -lvmbase -lshell -lc++ " % (
            self.sdk_dir)

    # AOT Compile the ABC files
    cmd = "gcc -w %s %s/avmshellMac.o %s/builtin_abc.o *.o -o %s" % (
        base_options, self.sdk_dir, self.sdk_dir, self.executable_name)
    if self.debug or self.verbose:
        print("AOT linking ABC files with the following command:")
        print(cmd)

    stdout, _exit = self.run_pipe(cmd=cmd, cwd=self.work_dir)
...
def run_pipe(self, cmd, cwd=None, env=None):
    if cwd is None:
        cwd = "./"
    # run a command and return a tuple of (output, exit_code)
    if env is None:
