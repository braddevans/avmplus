...
//
// Each module generates a single object file. Each module has an LLVMContext
// and llvm::Module (actually, it extends these classes--perhaps just a bit too
// cleverly). The Module/Context is then populated with class definitions
// supplied by the fields parameter then it captures pointers to the specific
// StructInfo, FieldInfo, and llvm Types that are frequently used.
//
// In LLVM everything in the IR ultimately hangs off of the Context and the
// Context is the unit of concurrency: everything in a contect (that is, each
// entire object graph) must be accessed by a single thread at a time. However
// different graphs can be accessed in different threads.
//
// Because each ABC file can depend on things in prior ABC files and since
// halfmoon is single-threaded, the ABC parsing, verifying, and LLVM IR
// generation must be sequential and might as well be on the main thread.
// However the optimization and code generation phases can be parallelized on
// separate threads. Experience has shown that those are likely to be the
// long-poles in overall compile latency (the regster allocator in particular is
// problematic.)
//
...
static cl::opt<bool>
    saveRawBitcode("save-raw-bitcode",
                   cl::desc("save bitcode files before any optimizations"),
                   cl::init(false));
static cl::opt<bool> saveBitcode("save-bitcode", cl::desc("save bitcode files"),
                                 cl::init(false));
static cl::opt<bool>
    saveRawLL("save-raw-ll",
              cl::desc("save pre-optimization llvm bitcode assembly"),
              cl::init(false));
static cl::opt<bool> saveLL("save-ll", cl::desc("save llvm bitcode assembly"),
                            cl::init(false));
static cl::opt<bool> noVerify("no-verify",
                              cl::desc("Don't run bitcode verification"),
                              cl::init(false));
static cl::opt<bool> printLLIR("print-llir", cl::desc("Print LLVM IR"),
                               cl::init(false));
enum EmitVal { EMIT_ASM, EMIT_OBJ, EMIT_LL, EMIT_NONE };
static cl::opt<EmitVal> codeGenType(
    "filetype", cl::init(EMIT_OBJ), cl::desc("output file type:"),
    cl::values(clEnumValN(EMIT_ASM, "asm", "Emit assembly files"),
               clEnumValN(EMIT_OBJ, "obj", "Emit object files"),
               clEnumValN(EMIT_LL, "bitcode", "Emit bitcode files"),
               clEnumValN(EMIT_NONE, "null", "Emit nothing"), clEnumValEnd));
static cl::opt<bool> globalOpt("global-opt",
                               cl::desc("enable global optimizations"),
                               cl::init(false));
static cl::opt<bool> noInline("disable-inlining", cl::desc("disable inlining"),
                              cl::init(false));
static cl::list<const PassInfo *, bool, PassNameParser>
    optimizations(cl::desc("Optimizations available:"));
static cl::opt<bool> disableCFI("disable-cfi", cl::Hidden, cl::init(true),
                                cl::desc("Do not use .cfi_* directives"));
static cl::opt<bool> debugSlots(
    "debug-slots", cl::Hidden,
    cl::desc("display information useful for debugging slot issues"));
} // namespace options

// These flags are counterpart of Traits.cpp: finishSlotsAndMethods flags.
// Following function ( LLVMModule::getSlotLayout )
//    represent compile time version of Traits.cpp::finishSlotsAndMethods() and
//    should always have same logic for slot layout.
...
LLVMModuleBase::LLVMModuleBase(const llvm::StringRef name,
                               const std::string &targetTripleArg,
                               int splitIndex)
    : llvm::Module(name, *static_cast<llvm::LLVMContext *>(this)),
      targetTriple(targetTripleArg), splitIndex(splitIndex),
      optimizeModule(true), functionCount(0) {
...
bool LLVMModuleBase::emit() {
  ...
  const llvm::Target *target =
      llvm::TargetRegistry::lookupTarget(targetTriple, errStr);
  ...
  // targetOptions.NoExcessFPPrecision = true;   // ActionScript requires IEEE
  // FP semantics
  ...
  if (optimizeModule && options::optLevel > '0') {
    ...
    builder.OptLevel =
        1; // for opt > 1, llvm adds more passes and shell tests starts failing.
    builder.SizeLevel = 0;
  ...
  case options::EMIT_ASM: {
    ...
    toolOut.reset(new llvm::tool_output_file(outf.c_str(), errStr,
                                             llvm::sys::fs::F_None));
    toolOstream.reset(new llvm::formatted_raw_ostream(toolOut->os()));
    bool failed = targetMachine->addPassesToEmitFile(
        passManager, *toolOstream, llvm::TargetMachine::CGFT_AssemblyFile,
        options::noVerify);
    AvmAssert(!failed);
  ...
  case options::EMIT_OBJ: {
    ...
    toolOstream.reset(new llvm::formatted_raw_ostream(toolOut->os()));
    bool failed = targetMachine->addPassesToEmitFile(
        passManager, *toolOstream, llvm::TargetMachine::CGFT_ObjectFile,
        options::noVerify);
    AvmAssert(!failed);
    (void)failed; // unused in release builds
    break;
...
LLVMModule::LLVMModule(const llvm::StringRef name, CompilerCore *core,
                       const std::string &targetTripleArg, int splitIndex)
    : LLVMModuleBase(name, targetTripleArg, splitIndex), core(core) {
  ...
  classClosurePtrTy =
      llvm::cast<llvm::PointerType>(llvmType("*'avmplus::ClassClosure'"));
  namespacePtrTy =
      llvm::cast<llvm::PointerType>(llvmType("*'avmplus::Namespace'"));
  multinamePtrTy =
      llvm::cast<llvm::PointerType>(llvmType("*'avmplus::Multiname'"));

  arrayObjectPtrTy =
      llvm::cast<llvm::PointerType>(llvmType("*'avmplus::ArrayObject'"));
  intVectorObjectPtrTy =
      llvm::cast<llvm::PointerType>(llvmType("*'avmplus::IntVectorObject'"));
  uIntVectorObjectPtrTy =
      llvm::cast<llvm::PointerType>(llvmType("*'avmplus::UIntVectorObject'"));
  doubleVectorObjectPtrTy =
      llvm::cast<llvm::PointerType>(llvmType("*'avmplus::DoubleVectorObject'"));

  fn_acalli = llvmFunctionType("a;*'avmplus::MethodEnv';i;*c")->getPointerTo();
  fn_ocalli =
      llvmFunctionType("*'avmplus::ScriptObject';*'avmplus::MethodEnv';i;*c")
          ->getPointerTo();
  fn_ncalli =
      llvmFunctionType("*'avmplus::Namespace';*'avmplus::MethodEnv';i;*c")
          ->getPointerTo();
  fn_scalli = llvmFunctionType("*'avmplus::String';*'avmplus::MethodEnv';i;*c")
                  ->getPointerTo();
...
bool LLVMModule::initializeDefs(llvm::raw_ostream &err) {
  ...
  //
  // So for now, this isn't really TLS and it's not really important until we
  // support concurreanct anyway.
...
void LLVMModule::disableOptimizeModule() { optimizeModule = false; }

unsigned int LLVMModule::getFunctionCount() { return functionCount; }
void LLVMModule::incrementFunctionCount() { functionCount++; }
...
//
// In retrospect, this was a mistake and I should have used the llvm assembler's
// type parser. [stan 12/13/12]
//
// Govindag[08/11/14] Now the format is (operator)n(type) operator could be
// pointer(*) or Array [number] operator works as prefix notation.
llvm::Type *LLVMModule::parseLlvmType(const char *&cursor, bool noAssert) {
  ...
  case '\'': {
    ...
    if (*cursor == '\'') {
      ...
      ++cursor;
      if (name.equals(
              "BoolKind")) { // oops, this is an enum type, represented as int32
...
llvm::FunctionType *
LLVMModule::llvmFunctionType(const char *functionTypeDescriptor) {
...
llvm::FunctionType *LLVMModule::llvmTypeForStubInfo(const StubInfo *stubinfo) {
  // TODO (stan): cache the LLVM argument types and return types rather than
  // recompouting each time.
...
bool LLVMModule::computeStructLayout(StructInfo &structInfo,
                                     llvm::raw_ostream &err) {
  if (options::debugSlots)
    printf(">> struct %s\n", structInfo.name.c_str());

  // Sort the fields by offset
  sort(structInfo.fields.begin(), structInfo.fields.end(),
       FieldInfo::smallerOffset);
  unsigned pos = 0;
  ...
  for (unsigned fieldIx = 0; fieldIx < structInfo.fields.size(); ++fieldIx) {
    ...
    if (pos > field.offset) {
      err << "field " << structInfo.name << "." << field.name
          << " does not fit\n";
      return true;
    }
    ...
    if (pos < field.offset) {
      ...
      llvm::Type *pad =
          llvm::ArrayType::get(llvm::Type::getInt8Ty(getContext()), padSize);
    ...
    if (field.slot != -1) {
      ...
      // printf(">>> %s %s %d %d\n", structInfo.name.c_str(),
      // field.name.c_str(), field.slot, pos-size);
    }
    if (options::debugSlots)
      printf(">>   field %s %d %d\n", field.name.c_str(), field.slot,
             field.offset);
  }
...
static String *getSlotName(const TraitsBindings *tb, unsigned slot) {
  ...
  while (iter.next()) {
    ...
    if ((AvmCore::bindingKind(b) == BKIND_VAR ||
         AvmCore::bindingKind(b) == BKIND_CONST) &&
...
StructInfo *LLVMModule::getSlotLayout(Traits *t) {
  ...
  // baseStruct->name.c_str(), nextSlotOffset);

  // See if this class has native info
  if (StructInfo *nativeTemplate =
          core->nativeSlotTemplates.findStruct(structInfo.name)) {
    ...
    for (unsigned i = 0; i < nativeTemplate->fields.size(); ++i) {
      ...
      structInfo.fields.back().type =
          llvmType(structInfo.fields.back().typedesc.c_str());
    }
  ...
  // It was assumed that the layout always has slots in order of slot ids, for
  // the same slot type. But that's not true. There could be cases when slot 2
  // is before slot 1 for the same slot type. This happens when abc has entries
  // in that order. To be able to generate the correct order of slots in layout,
  // now ordering the slots by the order of their offset, as it would have been
  // in AVM (tb->getSlotOffset).
  ...
  // Slots are laid out as by kind. First 32-bit objects, then pointer-sized
  // objects, then 64-bit items
  // TODO: makes target size and alignment assumptions
  // Used alignPointersTo8Bytes,align8ByteSlots for alignment. Same as in
  // Traits.cpp
  ...
  for (unsigned slotType = 0; slotType < SLOT_TYPE_END; ++slotType) {
    // for (unsigned slot = firstNewSlot; slot < tb->slotCount; ++slot) {
    for (std::vector<SLOT_AND_OFFSET>::iterator it = slots.begin();
         it != slots.end(); ++it) {
      unsigned slot = it->slot;
      Traits *slotTraits = tb->getSlotTraits(slot);
      SlotStorageType sst =
          valueStorageType(Traits::getBuiltinType(slotTraits));
  ...
  structInfo.size =
      nextSlotOffset; // ensure it's padded to the right size if necessary
...
std::string LLVMModule::traitsName(Traits *traits) {
  ...
  // There could be multiple private classes with name. So add module identifier
  // with the traits name
  ...
  case TRAITSTYPE_INTERFACE:
    if (traits->ns()->getType() == Namespace::NS_Private) {
      buffer << getModuleIdentifier().c_str() << ":"
             << getNameSpaceIndex(traits->pool, traits->ns());
    } else {
      Traits *searchingTraits = traits->itraits ? traits->itraits : traits;
      Multiname searchQname(searchingTraits->ns(), searchingTraits->name());
      Traits *searchResult = core->domainMgr()->findTraitsInPoolByMultiname(
          traits->pool, searchQname);
      AvmAssert(searchResult == NULL ||
                searchResult != (Traits *)BIND_AMBIGUOUS);

      // If searchResult != searchingTraits then this is a redefinition of the
      // same class;
      // In that case, Rename the variable with module identifier,so that always
  ...
  case TRAITSTYPE_CATCH: {
    ...
    AvmAssert(poolInfo &&
              poolInfo->pool ==
                  pool); // no use referencing someone else's catch traits
    // Find the method this is a catch traits for
    // TODO: this is rather laborious!
    MethodInfo *method = 0;
    int ix = 0;
    for (int i = 0, end = (int)pool->methodCount(); i < end && method == 0;
         ++i) {
  ...
  if (traits->m_paramTraits) {
    buffer << "<" << traitsName(traits->m_paramTraits).c_str()
  ...
  if (traits) {
    ...
    case TRAITSTYPE_ACTIVATION: {
      AvmAssert(
          poolInfo &&
          poolInfo->pool ==
              pool); // no use referencing someone else's activation traits
    ...
    llvm::GlobalVariable *var =
        llvm::dyn_cast_or_null<llvm::GlobalVariable>(getNamedValue(name));
    if (!var) {
      var =
          new llvm::GlobalVariable(*this, traitsPtrTy, /*isConst*/ true,
                                   llvm::GlobalValue::ExternalLinkage, 0, name);
    }
    // underscore at end of name helps distinguish global traits var from the
    // loaded value
...
// with HMAOT on both 32 and 64 bit devices but not with GOAOT (Same behavior on
// M+2  builds). Reason : Known Issue #2 at
...
// Solution : Propogate a flag to indicate that AOT function signature will have
// argc, argv. The boundary is Builtins where we can't propogate as we ship
// builtin.o Flag inMethodInfo is needArgsArrInMethodSig
llvm::FunctionType *LLVMModule::getFunctionTy(MethodInfo *info) {
  ...
  llvm::FunctionType *methodType =
      llvm::FunctionType::get(resultTy, params, false);
  return methodType;
}
