...
//
// Instantiates just enough AVM machinery to instantiate an avmcore and run the
// parser and verifier. Next it creates a toplevel and a couple of DomainEnvs
// since the parser and verifier need them. The builtins are parsed as a
// side-effect of instantiating the toplevel. Then it iterates through the "sdk"
// abcs (in practice there will be just one) parsing them. This is where the
// shell or player APIs are added above the ActionScript language primitive
// defintions (String, Class, Object, etc.) Then, it iterates over the user abcs
// feeding them to the parser/veifier which feeds the method bodies to halfmoon
// and then to the LLVMEmitter.
//
// Code is then emitted for the builtins+SDK ABCs and then for each user ABC.
//
// Finally, AOTInfos are generated which create the bondings between the runtime
// libraries and the generated code.
//
// Uses llvm's  command-line processing facility for argument parsing. Among the
// advantages is response-file support (options from a file). This is useful
// because AOT compiles often have command lines that are too long for Windows.
//
#ifdef _WIN32
...
static cl::list<string> fileList(cl::Positional, cl::ZeroOrMore,
                                 cl::desc("<abc and bitcode files>"));
static cl::list<string> sdkFileList("sdk", cl::ZeroOrMore,
                                    cl::desc("<sdk abc file>"));
static cl::opt<string> fields("fields",
                              cl::desc("Specify structure definitions"),
                              cl::value_desc("filename"), cl::ZeroOrMore);
static cl::opt<string> targetTriple("mtriple", cl::desc("<llvm target triple>"),
                                    cl::Required);
static cl::opt<bool> printHMIR("print-hmir", cl::desc("Print Halfmoon IR"),
                               cl::init(false));
static cl::opt<bool> printGML("print-hmir-gml",
                              cl::desc("Print Halfmoon IR as GML"),
                              cl::init(false));
static cl::opt<bool> verbose("verbose", cl::desc("enable verbose output"),
                             cl::Hidden);
static cl::opt<bool> perfLog("perf-log", cl::desc("enable logging"),
                             cl::Hidden);
static cl::opt<bool> emitSdk("emit-sdk", cl::desc("emit code for SDK files"),
                             cl::init(false));
static cl::opt<bool> emitANEFnMap("emit-ane-map",
                                  cl::desc("emit map for ANE functions"),
                                  cl::init(false));
static cl::list<string> exportedSym("ane-symbol",
                                    cl::desc("symbol for ANE lookup"),
                                    cl::ZeroOrMore);
static cl::opt<bool> as3Debugger("as3debugger",
                                 cl::desc("enable action script debugging"),
                                 cl::init(false));
static cl::list<string> abcFileList("abc-file-list",
                                    cl::desc("File having abc filenames"),
                                    cl::ZeroOrMore);
static cl::opt<bool>
    telemetrySampler("telemetry-sampler",
                     cl::desc("enable telemetry advanced sampler "),
                     cl::init(false));
...
public:
  ...
  virtual CodeContext *create(DomainEnv *domainEnv,
                              const BugCompatibility *bugCompatibility) {
...
public:
  CompilationJob(LLVMModule *llvmModule_) : llvmModule(llvmModule_) {}
  void handler() {
    {
      llvm::NamedRegionTimer codeGenTimer("CompilationJob : LLVM IR to native",
                                          "Compile-abc",
                                          llvm::TimePassesIsEnabled);
      llvmModule->emit();
    }
    {
      llvm::NamedRegionTimer codeGenTimer("CompilationJob : Finalize Modules",
                                          "Compile-abc",
                                          llvm::TimePassesIsEnabled);
      delete llvmModule;
      llvmModule = NULL;
...
CompilerCore::CompilerCore(MMgc::GC *gc)
    : AvmCore(gc, kApiVersionSeries_AIR), summaryModule(0), firstUserPool(0),
      builtinPoolInfo(0), currentlyParsing(0), fieldsChecksum(0),
      compilerToplevel(0), parseAndVerify(false), bugCompatibility(0),
      sdkCodeContext(0), userCodeContext(0) {
  ...
  config.verbose_vb =
      options::verbose ? avmplus::AvmCore::DEFAULT_VERBOSE_ON : 0;
  if (options::verbose)
...
CompilerCore::~CompilerCore() { threadPool.stopAndWait(); }
...
int CompilerCore::initialize() {
  TRY(this, avmplus::kCatchAction_ReportAsError) {
    ...
    {
      llvm::NamedRegionTimer codeGenTimer("Load Fields", "Compile-abc",
                                          llvm::TimePassesIsEnabled);
      ...
      // Load the field information
      {
        ...
        if (parseFieldFile(errs, fldStream, *summaryModule, &structInfos,
                           &nativeSlotTemplates, &nativeMethods,
                           &fieldsChecksum))
          return EXIT_CODE_BADFILE;
      }
    ...
    {
      llvm::NamedRegionTimer codeGenTimer("Parse and verify builtins",
                                          "Compile-abc",
                                          llvm::TimePassesIsEnabled);
      poolInfos.push_back(new PoolInfo(GetGC(), "builtin.abc", "builtin"));
      PoolInfo &poolInfo = *poolInfos.back();
      builtinPoolInfo = &poolInfo;
      poolInfo.setAbcData(NativeID::builtin_abc_data,
                          NativeID::builtin_abc_length);
      ...
      DomainEnv *sdkDomainEnv = avmplus::DomainEnv::newDomainEnv(
          this, sdkDomain, compilerToplevel->domainEnv());
      sdkCodeContext =
          new (GetGC()) CompilerCodeContext(sdkDomainEnv, bugCompatibility);

      // Create a Domain/DomainEnv/CodeContext for the user code
      Domain *userDomain = avmplus::Domain::newDomain(this, sdkDomain);
      DomainEnv *userDomainEnv =
          avmplus::DomainEnv::newDomainEnv(this, userDomain, sdkDomainEnv);
      userCodeContext =
          new (GetGC()) CompilerCodeContext(userDomainEnv, bugCompatibility);
    }

    // get the number of cores for the machine and decide the number of threads
    // to strt dynamically.
...
int CompilerCore::run(int argc, char **argv) {
  ...
  MMgc::GC *gc = mmfx_new(MMgc::GC(MMgc::GCHeap::GetGCHeap(), gcconfig));
  {
    ...
    // This needs to be a separate group or else the detail timings will be
    // added to the total and double-counted.  The point of this timer is to
    // double-check that we didn't miss anything big in the detail timers
    llvm::NamedRegionTimer codeGenTimer("Total Time", "AOT Compiler Command",
                                        llvm::TimePassesIsEnabled);
    {
      llvm::NamedRegionTimer LLVMInitTimer("Initialize LLVM", "Compile-abc",
                                           llvm::TimePassesIsEnabled);
    ...
    CompilerCore *core = 0;
    {
      llvm::NamedRegionTimer LLVMInitTimer(
          "Initialize Compiler Core", "Compile-abc", llvm::TimePassesIsEnabled);
      core = new CompilerCore(gc);
      core->settings.target_triple =
          llvm::Triple::normalize(options::targetTriple.c_str());
      ...
      // TODO: Revisit this option during performance testing. Because of
      // scheduling pass ( early / middle ), halfmoon IR
      // is generated differently and llvm emit fails with error.(Instruction
    ...
    {
      llvm::NamedRegionTimer LLVMInitTimer(
          "Finalize Compiler Core", "Compile-abc", llvm::TimePassesIsEnabled);
      delete core;
      gc->Collect();
    }
  }
...
int CompilerCore::compile() {
  ...
  //
  {
    llvm::NamedRegionTimer codeGenTimer("Parse SDK ABC", "Compile-abc",
                                        llvm::TimePassesIsEnabled);
  ...
  for (int i = 0, end = options::abcFileList.size(); i < end; ++i) {
    ...
    for (std::string abcFileName; getline(infile, abcFileName);) {
      AvmAssert(abcFileName.c_str() &&
                std::strstr(abcFileName.c_str(), ".abc"));
  ...
  //
  {
    llvm::NamedRegionTimer codeGenTimer("Parse user ABCs", "Compile-abc",
                                        llvm::TimePassesIsEnabled);
    for (int i = 0, end = options::fileList.size(); i < end; ++i) {
      ...
      moduleName << fileName.substr(basenamePos, dotPos - basenamePos) << "_"
                 << i;
  ...
  //
  // Verify everything except ActionScript builtins (including the rest of the
  // SDK abcs)
  //
  {
    llvm::NamedRegionTimer codeGenTimer("Verify ABCs", "Compile-abc",
                                        llvm::TimePassesIsEnabled);
    AOTExecMgr *execMgr = (AOTExecMgr *)exec;

    // Bug 3862419: ArcGISMobile application is crashing on both 32 and 64 bit
    // devices. Bug 3863078: [HMAOT] "SimpleCase.ipa" is crashing while packaging
    // with HMAOT on both 32 and 64 bit devices but not with GOAOT (Same behavior
    // on  M+2  builds). Reason : Known Issue #2 at
    // http://helpx.adobe.com/air/kb/air-compiler-known-issues.html
    //    A sample case is
    //    class A {
    ...
    //    var obj:A = new B();
    //    obj.getFn();
    // in ll, method signature mismatch at the place of call and called
    // function.
    // Solution : Propogate a flag to indicate that AOT function signature will
    // have argc, argv. The boundary is Builtins where we can't propogate as we
    // ship builtin.o Flag inMethodInfo is needArgsArrInMethodSig
  ...
  if (options::emitSdk) {
    ...
    {
      llvm::NamedRegionTimer codeGenTimer("Initialize Modules", "Compile-abc",
                                          llvm::TimePassesIsEnabled);
      llvmModule = new LLVMModule("builtin_abc", this, settings.target_triple);
      bool failed = llvmModule->initializeDefs(llvm::errs());
      AvmAssert(!failed); // if this was going to fail, it would have failed in
                          // the summary module
      (void)failed;
    }
    ...
    {
      llvm::NamedRegionTimer codeGenTimer("LLVM IR to native", "Compile-abc",
                                          llvm::TimePassesIsEnabled);
      llvmModule->emit();
    }
    {
      llvm::NamedRegionTimer codeGenTimer("Finalize Modules", "Compile-abc",
                                          llvm::TimePassesIsEnabled);
      delete llvmModule;
    }
  }
  ...
  //
  // Based on dependency information (not yet) collected during the initial
  // verification pass, determine which of the modules need to be recompiled.
  //
  {
    llvm::NamedRegionTimer codeGenTimer("Determine out-of-date modules",
                                        "Compile-abc",
                                        llvm::TimePassesIsEnabled);
  ...
  //
  {
    llvm::NamedRegionTimer codeGenTimer("Emit Summary Module", "Compile-abc",
                                        llvm::TimePassesIsEnabled);
    emitSummaryModule();
  }
  ...
  if (options::perfLog) {
    ...
    long totalABCSize = 0;
    long nFunctions = 0;
    for (unsigned i = firstUserPool, nPoolInfos = poolInfos.size();
         i < nPoolInfos; ++i) {
      PoolInfo &poolInfo = *poolInfos[i];
      totalABCSize += poolInfo.totalABCSize;
      for (unsigned j = 0, nMethods = poolInfo.verifyInfos.size(); j < nMethods;
           ++j) {
    ...
    outs << "\n ------------------- ABC Statistics ------------------- ";
    outs << "\n#PerfLog: Number of ABC Files : "
         << poolInfos.size() - firstUserPool;
...
void CompilerCore::emitPoolToModule(PoolInfo &poolInfo,
                                    LLVMModule *llvmModule) {
  {
    llvm::NamedRegionTimer codeGenTimer("Initialize Modules", "Compile-abc",
                                        llvm::TimePassesIsEnabled);
  ...
  for (unsigned i = 0; i < poolInfo.verifyInfos.size(); ++i) {
    ...
    if (verifyInfo.verifySucceeded) {
      if (!method->isNative()) {
        ...
        if (options::verbose) {
          llvm::raw_ostream &outs = llvm::outs();
          outs << "[" << i + 1 << "/" << poolInfo.verifyInfos.size() << "] - "
               << poolInfo.getFunctionName(method) << "\n";
        }

        halfmoon::JitWriter *jit = 0;
        {
          llvm::NamedRegionTimer codeGenTimer("ABC to HM IR", "Compile-abc",
                                              llvm::TimePassesIsEnabled);
...

        {
          ...
          llvm::NamedRegionTimer codeGenTimer("HM IR to LLVM IR", "Compile-abc",
                                              llvm::TimePassesIsEnabled);
  ...
  {
    llvm::NamedRegionTimer codeGenTimer("Create AOTInfo", "Compile-abc",
                                        llvm::TimePassesIsEnabled);
    poolInfo.createAOTInfo();
    // builtins is the only module that doesn't depend on anything, use that
    // fact to detect the builtin pool, whose module deeds to define the fields
    // checksum
...
/*Logic for breaking :- We break the function list into chunks on following
   conditions: A) A mdoule should have maximum kMaxFunctionInModule. B) If we
   encounter a very large function, we break both before and after that
   function. Thus this creates a module with only the large function. We don't
   perform llvm optmization for this function. This was done to fix a few bugs
   related to very large functions. Bug#3913141 Bug#3913137
 */
...
void CompilerCore::emitPoolToSpilttedModules(PoolInfo &poolInfo) {
  ...
  for (unsigned i = 0, end = poolInfo.verifyInfos.size(); i < end; ++i) {
      ...
      if (!method->isNative()) {
...

        {
          ...
          if (size > kLargeFunctionThreshold) {
            if (llvmModule->getFunctionCount() !=
                0) // Not the first function in the current module
            {
              // This creates a module with whatever functions were processed
              // before the large function. We will not create this module in
              // case our large function is the first function in the current
              // module.
              poolInfo.disconnect();
    ...
    if (i == end - 1 ||
        llvmModule->getFunctionCount() == kMaxFunctionInModule ||
        splitForLargeFn) {
      poolInfo.disconnect();
      emitModule(llvmModule);
      createModule(poolInfo, llvmModule, ++splitIndex);
  ...
  // to detect the builtin pool, whose module deeds to define the fields
  // checksum
  bool defineFieldsChecksum = poolInfo.dependsOn.size() == 0;
  poolInfo.createFieldsChecksum(fieldsChecksum, defineFieldsChecksum);
  poolInfo.disconnect();
...
void CompilerCore::createModule(PoolInfo &poolInfo, LLVMModule *&llvmModule,
                                int splitIndex) {

  llvm::NamedRegionTimer codeGenTimer("Initialize Modules", "Compile-abc",
                                      llvm::TimePassesIsEnabled);
  llvmModule = new LLVMModule(poolInfo.infoName, this, settings.target_triple,
                              splitIndex);
  bool failed = llvmModule->initializeDefs(llvm::errs());
  AvmAssert(!failed); // if this was going to fail, it would have failed in the
                      // summary module
  (void)failed;
  poolInfo.connect(llvmModule);
}
...
// Function to emit just the g_com_adobe_air_fre_fmap with all the function
// pointers output will be extensionglue.o in current dir. This function is
// inspired from emitSummaryModule
void emitANEMap(int argc, char **argv) {
  llvm::llvm_shutdown_obj auto_shutdown;
  llvm::NamedRegionTimer codeGenTimer(
      "Total Time ANE MAP", "AOT Compiler Command", llvm::TimePassesIsEnabled);
  {
    llvm::NamedRegionTimer LLVMInitTimer("Initialize LLVM", "Compile-abc",
                                         llvm::TimePassesIsEnabled);
    llvm::InitializeAllTargetInfos();
  ...
  LLVMModuleBase *lModule = new LLVMModuleBase(
      "extensionglue", llvm::Triple::normalize(options::targetTriple.c_str()));
  ...
  llvm::StructType *nameToFunctionStructTy = llvm::StructType::create(
      llvm::ArrayRef<llvm::Type *>(elementTypes), "NameToFunction");

  std::vector<llvm::Constant *> entries;
  for (unsigned i = 0; i < options::exportedSym.size(); ++i) {
    std::string &funcName = options::exportedSym[i];
    llvm::Constant *nameConst =
        llvm::ConstantDataArray::getString(module.getContext(), funcName);
    llvm::GlobalVariable *nameVar = new llvm::GlobalVariable(
        module, nameConst->getType(), isConst,
        llvm::GlobalValue::InternalLinkage, nameConst, funcName + "_str");
    llvm::Constant *func = module.getOrInsertFunction(
        funcName, llvm::cast<llvm::FunctionType>(module.functionTy));
    llvm::Constant *indices[] = {llvm::ConstantInt::get(int32Ty, 0),
                                 llvm::ConstantInt::get(int32Ty, 0)};
    llvm::Constant *nameCharPtrConst =
        llvm::ConstantExpr::getGetElementPtr(nameVar, indices);
  ...
  llvm::Constant *nameToFunctionArrayConst = llvm::ConstantArray::get(
      llvm::ArrayType::get(nameToFunctionStructTy, entries.size()), entries);
  llvm::GlobalVariable *aneMap = new llvm::GlobalVariable(
      module, nameToFunctionArrayConst->getType(), isConst,
      llvm::GlobalValue::ExternalLinkage, nameToFunctionArrayConst,
      "g_com_adobe_air_fre_fmap");
  (void)aneMap; // unused
...
void CompilerCore::emitSummaryModule() {
  ...
  for (unsigned i = firstUserPool; i < poolInfos.size(); ++i) {
    ...
    llvm::GlobalVariable *gvar =
        new llvm::GlobalVariable(module, module.aotInfoTy, isConst,
                                 llvm::GlobalVariable::ExternalLinkage,
                                 unInitialized, infoName + aotInfoSuffix);
    aotInfoConstants.push_back(gvar);
  }

  llvm::Constant *aotInfosConstantArray = llvm::ConstantArray::get(
      llvm::ArrayType::get(module.aotInfoPtrTy, aotInfoConstants.size()),
      aotInfoConstants);

  llvm::GlobalVariable *aotInfos = new llvm::GlobalVariable(
      module, aotInfosConstantArray->getType(), isConst,
      llvm::GlobalValue::ExternalLinkage, aotInfosConstantArray, "aotInfos");
  (void)aotInfos; // unused

  llvm::GlobalVariable *nAotInfos = new llvm::GlobalVariable(
      module, llvm::Type::getInt32Ty(module.getContext()), isConst,
      llvm::GlobalValue::ExternalLinkage,
      llvm::ConstantInt::get(int32Ty, aotInfoConstants.size()), "nAOTInfos");
  (void)nAotInfos; // unused

  llvm::GlobalVariable *g_aotSamplerEnabled = new llvm::GlobalVariable(
      module, llvm::Type::getInt32Ty(module.getContext()), isConst,
      llvm::GlobalValue::ExternalLinkage,
      llvm::ConstantInt::get(int32Ty, settings.telemetry_sampler ? 1 : 0),
      "g_aotSamplerEnabled");
...
int CompilerCore::evaluateFile(const std::string &fileName, PoolInfo &poolInfo,
  ...
  TRY(this, kCatchAction_ReportAsError) {
    ...
    handleActionBlock(code, 0, compilerToplevel, NULL, codeContext,
                      getDefaultAPI());
  }
...
namespace avmplus {
Isolate *Isolate::newIsolate(int32_t, int32_t, Aggregate *) { return 0; }

/*static*/
bool MutexClass::getMutexSupported(GCRef<avmplus::Toplevel>) { return false; }

} // namespace avmplus

namespace halfmoon {
bool AOTIsDebugMode() { return compile_abc::options::as3Debugger; }
} // namespace halfmoon
