...
void PoolInfo::appendVerifierResult(
    AbcEnv *abc_env_param, const compile_abc::MethodVerifyInfo &verifierInfo) {
...
void PoolInfo::connect(LLVMModule *modulep) {
  ...
  llvm::Type *traitsPtrArrayTy =
      llvm::ArrayType::get(module.traitsPtrTy, pool->methodCount());
  activationTraits = new llvm::GlobalVariable(
      module, traitsPtrArrayTy,
      /*isConst*/ false, llvm::GlobalValue::ExternalLinkage, 0,
      infoName + "_activationTraits");
  traitsPtrArrayTy =
      llvm::ArrayType::get(module.traitsPtrTy, pool->scriptCount());
  scriptTraits = new llvm::GlobalVariable(module, traitsPtrArrayTy,
                                          /*isConst*/ false,
                                          llvm::GlobalValue::ExternalLinkage, 0,
                                          infoName + "_scriptTraits");
}
...
void PoolInfo::addMethod(MethodInfo *method, llvm::Function *func) {
  ...
  llvm::Constant *fpCasted =
      llvm::ConstantExpr::getBitCast(func, module.functionPtrTy);
  llvm::Constant *methodFlagsConst =
      llvm::ConstantInt::get(int32Ty, method->needArgsArrInMethodSig());
  ...
  if (method_id == -1) {
    ...
    for (int i = 0, end = (int)pool->methodCount(); i < end; ++i) {
      ...
      if (mi->activationTraits() == declarer) {
        // found the method whose activation traits are the declarer of the
        // init. So this is the method for the activation whose init we're
        // dealing with.
...
llvm::GlobalVariable *PoolInfo::makeVariable(llvm::Constant *init, bool isConst,
  ...
  if (name.isTriviallyEmpty())
    ...
    gvar = llvm::dyn_cast_or_null<llvm::GlobalVariable>(
        modulep->getNamedValue(name.str()));
  }
  if (gvar) {
    gvar->setInitializer(init);
  } else {
    gvar = new llvm::GlobalVariable(*modulep, init->getType(), isConst, linkage,
                                    init, name);
  }
  return gvar;
}
...
llvm::Constant *PoolInfo::const_gep(llvm::Constant *in, int i, int j) {
  ...
  llvm::Constant *indices[] = {llvm::ConstantInt::get(int32Ty, i),
...
void PoolInfo::populateAllMethodsDebugInfos() {
  // walk all methods
  llvm::Constant *nullmethodDebugInfo =
      llvm::Constant::getNullValue(modulep->methodDebugInfoTy);
...
llvm::Constant *PoolInfo::collectMethodDebugInfo(MethodInfo *m) {
  ...
  aotInfo.init(
      module.methodDebugInfo_scope_count_member,
  ...
  uint32_t code_len =
      AvmCore::readU32(pos); // checked earlier by AbcParser::parseMethodInfos()
  ...
  llvm::Constant *local_namesConst = llvm::ConstantDataArray::get(
      module.getContext(),
      llvm::ArrayRef<uint32_t>(localNamesArr, localNamesSize));

  aotInfo.init(module.methodDebugInfo_local_names_member,
               const_gep(makeVariable(local_namesConst, isConst,
                                      getFunctionName(m) + "_local_names"),
                         0, 0));
  ...
  llvm::Constant *linesConst = llvm::ConstantDataArray::get(
      module.getContext(), llvm::ArrayRef<uint32_t>(linesArr, linesSize));

  aotInfo.init(module.methodDebugInfo_lines_member,
               const_gep(makeVariable(linesConst, isConst,
                                      getFunctionName(m) + "_lines"),
                         0, 0));
...
void PoolInfo::populateMethodsArray() {
  ...
  for (unsigned i = 0; i < verifyInfos.size(); ++i) {
    ...
    // Add all Aoted function weather they are verified or not. VM Created
    // Methods (id == -1 ) are not aoted hence not adding in array.
...
void PoolInfo::createAOTInfo() {
  ...
  llvm::Constant *sha1Const = llvm::ConstantDataArray::get(
      module.getContext(), llvm::ArrayRef<uint8_t>(sha1hash));

  llvm::Constant *abcBytesConst = llvm::ConstantDataArray::get(
      module.getContext(), llvm::ArrayRef<uint8_t>(abcBytes, abcSize));

  // Populate Function pointer array
  populateMethodsArray();
  llvm::Constant *abcMethodsConst = llvm::ConstantArray::get(
      llvm::ArrayType::get(module.functionPtrTy, functions.size()), functions);
  llvm::Constant *methodFlagsArrConst = llvm::ConstantArray::get(
      llvm::ArrayType::get(int32Ty, methodFlagsArr.size()), methodFlagsArr);
  ...
  for (int i = 0, end = activationInfos.size(); i < end; ++i) {
    ...
    actInfo.init(module.aotActivationInfo_initMethodId_member,
                 llvm::ConstantInt::get(int32Ty, -1));
    actInfo.init(module.aotActivationInfo_initHandler_member,
                 activationInits[i]);
    activationInfos[i] = actInfo.create();
  }
  llvm::Constant *activationInfoConst = llvm::ConstantArray::get(
      llvm::ArrayType::get(module.aotActivationInfoTy, activationInfos.size()),
      activationInfos);
  llvm::Constant *activationTraitsConst = llvm::Constant::getNullValue(
      llvm::ArrayType::get(module.traitsPtrTy, activationInfos.size()));
  llvm::Constant *scriptTraitsConst = llvm::Constant::getNullValue(
      llvm::ArrayType::get(module.traitsPtrTy, pool->scriptCount()));
  ...
  aotInfo.init(module.aotInfo_origABCSHA1_member, sha1Const);
  aotInfo.init(
      module.aotInfo_abcbytes_member,
      const_gep(makeVariable(abcBytesConst, isConst, infoName + "_abcBytes"), 0,
                0));
  ...
  if (halfmoon::AOTIsDebugMode()) {
    ...
    llvm::Constant *methodDebugInfosConst = llvm::ConstantArray::get(
        llvm::ArrayType::get(module.methodDebugInfoTy, methodDebugInfos.size()),
        methodDebugInfos);
    aotInfo.init(module.aotInfo_methodDebugInfos_member,
                 const_gep(makeVariable(methodDebugInfosConst, isConst,
                                        infoName + "_methodDebugInfos"),
                           0, 0));
  }
#endif

  aotInfo.init(module.aotInfo_abcMethods_member,
               const_gep(makeVariable(abcMethodsConst, isConst,
                                      infoName + "_abcMethods"),
                         0, 0));
  ...
  aotInfo.init(module.aotInfo_methodFlagsArr_member,
               const_gep(makeVariable(methodFlagsArrConst, isConst,
                                      infoName + "_methodFlagsArr"),
                         0, 0));
  aotInfo.init(module.aotInfo_activationInfo_member,
               const_gep(makeVariable(activationInfoConst, isConst,
                                      infoName + "_activationInfo"),
                         0, 0));
  llvm::Constant *aotInfoConst = aotInfo.create();
  llvm::GlobalVariable *aotInfoVar =
      makeVariable(aotInfoConst, nonConst, infoName + aotInfoSuffix);
...
void PoolInfo::generateDependencyGlobals() {
  LLVMModule &module = *modulep;
  const llvm::GlobalValue::LinkageTypes extLinkage =
  ...
  //
  // This mechanism ensures that attempts to link incompatibly-compiled object
  // files will generate a linker error instead of difficult-to-diagnose runtime
  // errors.
  //
  ...
  llvm::GlobalVariable *myVersionGlob =
      new llvm::GlobalVariable(*modulep, zeroConst->getType(), isConst,
                               extLinkage, zeroConst, varName.c_str());
  ...
  for (Deps::iterator i = dependsOn.begin(); i != dependsOn.end(); ++i) {
    ...
    if (!gRef)
      gRef = new llvm::GlobalVariable(*modulep, zeroConst->getType(), isConst,
                                      extLinkage, 0, varName.c_str());
    std::string varRefName =
        infoName + "_requires_" + poolInfo->infoName + "_version";
    llvm::GlobalVariable *gRefVar = new llvm::GlobalVariable(
        *modulep, gRef->getType(), isConst, extLinkage, gRef, varRefName);
    (void)gRefVar; // unused
  }
}
...
inline uint32_t readU32(const uint8_t *&pos) { return AvmCore::readU32(pos); }
...
static void skipCpool(const uint8_t *&pos) {
  ...
  // double pool
  count = readU32(pos);
  // doubles are always 8 bytes so lets just advance pos to just past the end of
  // the double pool there are really only count-1 doubles in there
  if (count != 0)
    pos += (count - 1) * 8;
...
static void skipMethodInfos(const uint8_t *&pos) {
  ...
  for (uint32_t i = 0; i < count; i++) {
    ...
    if (flags & abcMethod_HAS_OPTIONAL) {
      ...
      for (uint32_t j = 0; j < optionals;
           ++j) // skip over optional_count ValueKinds
      {
...
static void skipTraits(const uint8_t *&pos) {
  ...
  for (uint32_t i = 0; i < count; i++) {
    ...
    pos++;
    uint8_t needToSkipMetadata =
        ((kind >> 4) & 0x04); // save important bit from high half of kind
    kind = kind & 0x0F;       // low four bits of kind
    if (kind <= 6) // these seven cases all start with two U30's (with all
                   // different names)
    {
    ...
    if (0 == kind || 6 == kind) // these two cases have additional data
    {
      ...
      if (valueIndex != 0)
        pos++; // skip over value_kind if it is there
    }
    if (needToSkipMetadata) {
      uint32_t metadatas = readU32(pos); // metadata_count
      for (uint32_t j = 0; j < metadatas;
           ++j) // skip over metadata_count metadatas
        readU32(pos);
    }
  }
}
...
static void skipInstanceAndClassInfos(const uint8_t *&pos) {
  ...
  for (uint32_t i = 0; i < count; i++) {
    ...
    for (uint32_t j = 0; j < interfaces;
         ++j) // skip over interfaces_count interfaces
      readU32(pos);
...
static void copyMethodBodies(const uint8_t *&from, uint8_t *&to) {
  ...
  for (uint32_t i = 0; i < count; i++) {
    ...
    *to = 0; // rewrite code length as 0
    ...
    for (uint32_t j = 0; j < exceptions;
         ++j) // skip over exceptions Exception bodies
    {
      readU32(pos); // start
      readU32(pos); // end
      readU32(pos); // target
 ...
 */
uint32_t copyAbcSkippingMethodBodies(const uint8_t *abcBytes,
                                     uint8_t *copiedBytes, unsigned nAbcBytes) {
