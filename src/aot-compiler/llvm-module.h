...
public:
  ...
  LLVMModuleBase(const llvm::StringRef name, const std::string &targetTriple,
                 int splitIndex = 0);
  ~LLVMModuleBase();
...
public:
  LLVMModule(const llvm::StringRef name, CompilerCore *core,
             const std::string &targetTriple, int splitIndex = 0);
  ~LLVMModule();
  ...
  StructInfo *getStructInfo(llvm::raw_ostream &err, const std::string &name,
                            llvm::Type **typeP, llvm::PointerType **ptrTypeP,
                            int *errCntp);
  FieldInfo *getFieldInfo(llvm::raw_ostream &err, StructInfo *s,
                          const std::string &name, int *errCntp);

  // TODO: This map assumes that Traits lifetimes exceed the lifetime of a
  // compile (the holder is not a GCRoot). If this turns out to be wrong, we'll
  // keep a separate table of Traits in the CompilerCore just to pin them down.
...
public:
  ConstStructInitializer(StructInfo *structInfo) : structInfo(structInfo) {
