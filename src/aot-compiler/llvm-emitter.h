...
class LLVMEmitter : public llvm::IRBuilder<>,
                    public KindAdapter<LLVMEmitter, void> {
public:
...
private:
  ...
  llvm::Value *castPtrToInt(llvm::Value *ptr, const llvm::Twine &name = "") {
    ...
    // AvmAssert(ptr->get_type()->canLosslesslyBitCastTo(toType));
    return CreatePtrToInt(ptr, toType,
                          name.isTriviallyEmpty() ? ptr->getName() : name);
  }
  ...
  llvm::Value *castIntToPtr(llvm::Value *val, const llvm::Twine &name = "") {
    ...
    // TODO (stan): reenable this AvmAssert when we're really cross-compiling.
    //     currently we're still using some jit paths and the host is 64 bits.
    // AvmAssert(val->get_type()->canLosslesslyBitCastTo(toType));
    return CreateIntToPtr(val, toType,
                          name.isTriviallyEmpty() ? val->getName() : name);
  }

  llvm::Value *offsetGenericPointer(llvm::Value *ptr, uint32_t offset) {
    AvmAssert(ptr->getType()->isPointerTy());
    // TODO: we should be using typed pointers, typically
    //   (and if it happens to be one, let's smack it down to generic anyway,
    //   "temporarily")
  ...
  llvm::Value *applyTag(llvm::Value *intVal, int tag, bool shift,
                        const llvm::Twine &name) {
    if (intVal->getType() == getInt1Ty() ||
        intVal->getType() ==
            getInt32Ty()) // If we have a boolean, extend to an atom
  ...
  llvm::Value *removeTagReturningPointer(llvm::Value *val, llvm::Type *type) {
    AvmAssert(val->getType()->isIntegerTy());
    llvm::Value *masked =
        CreateAnd(val, llvm::ConstantInt::get(val->getType(), kAtomPtrMask));
  ...
  llvm::Value *getStringConstant(Atom val) {
    return getStringConstant(AvmCore::isString(val) ? AvmCore::atomToString(val)
                                                    : NULL);
  }
  llvm::Value *getNamespaceConstant(Namespace *val);
  llvm::Value *getNamespaceConstant(Atom val) {
    return getNamespaceConstant(AvmCore::atomToNamespace(val));
  }
  llvm::Value *getMultinameConstant(const Multiname *val);
  llvm::Value *getDoubleConstant(double val) {
    return llvm::ConstantFP::get(Context, llvm::APFloat(val));
  }
  llvm::Value *getDoubleConstant(Atom val) {
    return getDoubleConstant(AvmCore::atomToDouble(val));
  }

  llvm::Value *getMethodInfoValue(MethodInfo *info);

  llvm::Instruction *getFirstNonPHI(llvm::BasicBlock *bb) {
    // Can't use BasicBlock::getFirstNonPHI() since that assumes that the
    // BasicBlock already has a terminator. And this one doesn't since it is
    // still being constructed. That's evidently a bug since the docs say it
  ...
  llvm::Value *emitStore(const Use &value, const Type *constraint,
  ...
  llvm::BasicBlock
      *upe_block;         // label to jump to for undefined pointer exceptions.
  ...
  std::map<int32_t, CatchBlockInstr *>
      catch_blocks_; // targets to reachable catch blocks
  const bool enable_verbose_;
  std::stack<llvm::BasicBlock *> basic_blocks;
  bool isWindowsTriple;

  // Bug 3909398: [iOS64]: [Graphics]: [RWL]: RealWorld apps "Solitaire 3 and
  // "Meteor Storm 2" from Graphic Suite Crashing on launch.(working fine on 32
  // Bit device). Reason: hasnext is called too many times from
  // flare.loaders::ColladaLoader/start, App going out of stack due to allocating
  // temp variables that many times Solution: Instead of allocating space for
  // each hasNext call, allocating at the start of fn.It will be removed if these
  ...
  llvm::Value *createMemberGEP(llvm::Value *ptr, FieldInfo *member,
                               const llvm::Twine &name = "");
  void storeVPC(int vpc, bool force = false);

  llvm::CallInst *callIns(const LLVMModule::StubInfo *ci, uint32_t argc, ...);
  llvm::CallInst *acallIns(const LLVMModule::StubInfo *ci, uint32_t argc,
                           llvm::Value **args);
  bool needsMethodFrame(const LLVMModule::StubInfo *stubinfo);

  llvm::BasicBlock *splitEdgeIfNecessary(llvm::BasicBlock *fromBlock,
                                         llvm::BasicBlock *toBlock);
  llvm::PHINode *insertPhi(llvm::BasicBlock *bb, llvm::Type *type,
  ...
  llvm::CallInst *emitCall(MethodInfo *callee, llvm::Value *func,
                           llvm::Value *callee_env, CallStmt2 *call);
  llvm::Value *getMethodFrame();
  llvm::Instruction *
  getNearestInsertionInsertPointForAllUses(llvm::DominatorTree &dt,
                                           llvm::Value *val);
  void removeIfNotUsedOtherwiseMoveNearestToFirstUse(llvm::DominatorTree &dt,
  ...
  llvm::LoadInst *loadMember(llvm::Value *ptr, FieldInfo *member,
                             const llvm::Twine &name = "");
  llvm::Value *storeMember(llvm::Value *val, llvm::Value *ptr,
                           FieldInfo *member);
  ...
  public:
    ...
    ~PushBuilder() { builder->restoreIP(where); }
