...
static cl::opt<bool> verifyWarnings("verify-warnings",
                                    cl::desc("print verify warnings"),
                                    cl::init(true));
}; // namespace options
...
public:
  AotPass1Writer(MethodInfo *method) { verifyInfo.method = method; }

  // There is an opportunity to capture lots of information here, but for now we
  // don't
  ...
  void writeOp1(const FrameState *, const uint8_t *, AbcOpcode, uint32_t,
                Traits *) {}
  void writeOp2(const FrameState *, const uint8_t *, AbcOpcode, uint32_t,
                uint32_t, Traits *) {}
  void writeMethodCall(const FrameState *, const uint8_t *, AbcOpcode,
                       MethodInfo *, uintptr_t, uint32_t, Traits *) {}
...
AOTExecMgr::AOTExecMgr(CompilerCore *core)
    : BaseExecMgr(core), core(core), propogateArgArrMethodQueue(core->gc, 0),
...
void AOTExecMgr::verifyMethod(MethodInfo *m, Toplevel *toplevel,
                              AbcEnv *abc_env) {
...
void AOTExecMgr::verifyAot(MethodInfo *method, MethodSignaturep ms,
  ...
  if (canCompile) {
    ...
    CATCH(avmplus::Exception * exception) { (void)exception; }
    END_CATCH
    END_TRY
  }
  ...
  if (options::verifyWarnings && !aotWriter.verifyInfo.verifySucceeded) {
    if (method->method_id() >=
        0) { // don't warn about not compiling internally-generated initializers
      const char *err = "Verify error";
      if (!canCompile)
        err = "Halfmoon can't compile";
      llvm::outs() << "warning: " << err << ": "
                   << poolInfo.getFunctionName(method).c_str() << "\n";
    }
  }
        ...
        if (method != baseMethod && !baseMethod->needRestOrArguments()) {
          ...
          if (baseMethod->pool()->isBuiltin) {
            llvm::errs()
                << "Error: Method \""
                << poolInfo.getFunctionName(method).c_str()
                << " have either rest or argument usage unlike it's base "
                   "class. \nAOT can not handle this scenario. Please refer to "
                   "http://helpx.adobe.com/air/kb/"
                   "air-compiler-known-issues.html for more information.\n";
            break;
          } else {
...
void AOTExecMgr::propogateArgArrFlagInMethodHierarchy(MethodInfo *method) {
  ...
  // if needRestOrArguments in base, then propogate it to derived function
  // (current)
  if (disp_id >= 0 && btd && disp_id < btd->methodCount &&
      btd->getMethod(disp_id)) {
    MethodInfo *baseMethod = btd->getMethod(disp_id);
    if (method != baseMethod && !method->needRestOrArguments() &&
        (baseMethod->needRestOrArguments() ||
         baseMethod->needArgsArrInMethodSig())) {
