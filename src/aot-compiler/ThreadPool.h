...
public:
  ...
  ThreadPool()
      : jobQueueBack_(0), maxQueue_(0), queueLength_(0), stop_(false),
        nRunning_(0) {}

  //
  // Starts the ThreadPool with the given number of threads and a maximum queue
  // size. Zero is a good length for the queue because you don't usually want to
  // get out ahead of the thread pool processing.
  //
  ...
  //
  // Signal the ThreadPool to shut down when processing is complete and wait for
  // completion.
  //
  void stopAndWait() {
...
private:
  ...
  vmpi_thread_rtn_t threadBody() {
    SCOPE_LOCK(monitor_) { nRunning_ += 1; }
    for (;;) {
      Job *job = 0;
      SCOPE_LOCK_NAMED(locker, monitor_) {
      ...
      SCOPE_LOCK_NAMED(locker2, monitor_) { locker2.notifyAll(); }
    }
    return 0;
  }
