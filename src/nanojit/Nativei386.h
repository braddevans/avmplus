...
const int32_t LARGEST_UNDERRUN_PROT =
...
static const Register
    ...
    XMM0 = {8}, XMM1 = {9}, XMM2 = {10}, XMM3 = {11}, XMM4 = {12}, XMM5 = {13},
    XMM6 = {14}, XMM7 = {15},
...
static const RegisterMask SavedRegs =
    1 << REGNUM(rEBX) | 1 << REGNUM(rEDI) | 1 << REGNUM(rESI);
static const RegisterMask GpRegs =
    SavedRegs | 1 << REGNUM(rEAX) | 1 << REGNUM(rECX) | 1 << REGNUM(rEDX);
static const RegisterMask XmmRegs = 1 << REGNUM(XMM0) | 1 << REGNUM(XMM1) |
                                    1 << REGNUM(XMM2) | 1 << REGNUM(XMM3) |
                                    1 << REGNUM(XMM4) | 1 << REGNUM(XMM5) |
...
static const RegisterMask ScratchRegs =
    1 << REGNUM(rEAX) | 1 << REGNUM(rECX) | 1 << REGNUM(rEDX) | FpRegs;
#define FpDRegs FpRegs
#define FpSRegs FpRegs
#define FpQRegs XmmRegs

static const RegisterMask AllowableByteRegs =
    1 << REGNUM(rEAX) | 1 << REGNUM(rECX) | 1 << REGNUM(rEDX) |
    1 << REGNUM(rEBX);
...
#define DECLARE_PLATFORM_ASSEMBLER()                                           \
  ...
  debug_only(int32_t _fpuStkDepth;) debug_only(int32_t _sv_fpuStkDepth;) void  \
  nativePageReset();                                                           \
  ...
  void ALUsib(int32_t c, Register r, Register base, Register index,            \
              int32_t scale, int32_t disp);                                    \
  void ALUsib16(int32_t c, Register r, Register base, Register index,          \
                int32_t scale, int32_t disp);                                  \
  ...
  void ALU2sib(int32_t c, Register r, Register base, Register index,           \
               int32_t scale, int32_t disp);                                   \
  ...
  void LDsib(Register reg, int32_t disp, Register base, Register index,        \
             int32_t scale);                                                   \
  void LD16S(Register r, int32_t d, Register b);                               \
  void LD16Sdm(Register r, int32_t addr);                                      \
  void LD16Ssib(Register r, int32_t disp, Register base, Register index,       \
                int32_t scale);                                                \
  void LD16Z(Register r, int32_t d, Register b);                               \
  void LD16Zdm(Register r, int32_t addr);                                      \
  void LD16Zsib(Register r, int32_t disp, Register base, Register index,       \
                int32_t scale);                                                \
  void LD8Z(Register r, int32_t d, Register b);                                \
  void LD8Zdm(Register r, int32_t addr);                                       \
  void LD8Zsib(Register r, int32_t disp, Register base, Register index,        \
               int32_t scale);                                                 \
  void LD8S(Register r, int32_t d, Register b);                                \
  void LD8Sdm(Register r, int32_t addr);                                       \
  void LD8Ssib(Register r, int32_t disp, Register base, Register index,        \
               int32_t scale);                                                 \
  void LDi(Register r, int32_t i);                                             \
  void ST8(Register base, int32_t disp, Register reg);                         \
  void ST8sib(int32_t disp, Register base, Register index, int32_t scale,      \
              Register reg);                                                   \
  void ST16(Register base, int32_t disp, Register reg);                        \
  void ST16sib(int32_t disp, Register base, Register index, int32_t scale,     \
               Register reg);                                                  \
  void ST(Register base, int32_t disp, Register reg);                          \
  void STsib(int32_t disp, Register base, Register index, int32_t scale,       \
             Register reg);                                                    \
  void ST8i(Register base, int32_t disp, int32_t imm);                         \
  void ST8isib(int32_t disp, Register base, Register index, int32_t scale,     \
               int32_t imm);                                                   \
  void ST16i(Register base, int32_t disp, int32_t imm);                        \
  void ST16isib(int32_t disp, Register base, Register index, int32_t scale,    \
                int32_t imm);                                                  \
  void STi(Register base, int32_t disp, int32_t imm);                          \
  void STisib(int32_t disp, Register base, Register index, int32_t scale,      \
              int32_t imm);                                                    \
  void RET();                                                                  \
  void NOP();                                                                  \
  void INT3();                                                                 \
  ...
  void SSEsib(int32_t c, Register rr, int32_t d, Register rb, Register ri,     \
              int32_t scale);                                                  \
  ...
  void SSEssib(int32_t c, Register rr, int32_t d, Register rb, Register ri,    \
               int32_t scale);                                                 \
  ...
  void SSE_LDUPSsib(Register r, int32_t d, Register rb, Register ri,           \
                    int32_t scale);                                            \
  ...
  void SSE_LDQsib(Register r, int32_t d, Register rb, Register ri,             \
                  int32_t scale);                                              \
  void SSE_LDSSsib(Register r, int32_t d, Register rb, Register ri,            \
                   int32_t scale);                                             \
  ...
  void SSE_STQsib(int32_t d, Register rb, Register ri, int32_t scale,          \
                  Register rv);                                                \
