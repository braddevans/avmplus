...
#define OP___(op, repKind, retType, isCse) LRK_##repKind,
#include "LIRopcode.tbl"
#undef OP___
    0};

const LTy retTypes[] = {
#define OP___(op, repKind, retType, isCse) LTy_##retType,
#include "LIRopcode.tbl"
#undef OP___
    LTy_V};

const uint8_t insSizes[] = {
#define OP___(op, repKind, retType, isCse) sizeof(LIns##repKind),
#include "LIRopcode.tbl"
#undef OP___
    0};

const int8_t isCses[] = {
#define OP___(op, repKind, retType, isCse) isCse,
#include "LIRopcode.tbl"
#undef OP___
    0};
...
const char *lirNames[] = {
#define OP___(op, repKind, retType, isCse) #op,
#include "LIRopcode.tbl"
#undef OP___
    NULL};
...
struct AllArgs {
  static bool doCount(ArgType) { return true; }
};
struct IntArgs {
  static bool doCount(ArgType a) { return a == ARGTYPE_I || a == ARGTYPE_UI; }
};
struct FloatArgs {
  static bool doCount(ArgType a) { return a == ARGTYPE_F; }
};
struct Float4Args {
  static bool doCount(ArgType a) { return a == ARGTYPE_F4; }
};

uint32_t CallInfo::count_args() const { return countArgs<AllArgs>(); }

uint32_t CallInfo::count_int_args() const { return countArgs<IntArgs>(); }

uint32_t CallInfo::count_float_args() const { return countArgs<FloatArgs>(); }

uint32_t CallInfo::count_float4_args() const { return countArgs<Float4Args>(); }
...
CfgLister::CfgLister(LirFilter *in, Allocator &alloc, CfgMode mode)
    : LirFilter(in), _alloc(alloc), _alt(alloc), _edges(alloc),
      _vertices(alloc), _ids(alloc), _mode(mode) {
...
LIns *CfgLister::read() {
  bool priorAsVertex =
      false; // true, implies that the last processed instruction is a vertex
  ...
  // each instruction is a vertex in mode CFG_INS (except imm's which are
  // ignored)
  priorAsVertex = (_prior && _mode == CFG_INS && !_prior->isImmAny())
                      ? true
                      : priorAsVertex;
...
void CfgLister::addEdge(LIns *from, LIns *to) {
  ...
  NanoAssert(_ids.containsKey(to) ||
             to->isop(LIR_label)); // we should have seen the instruction
                                   // already or its a backwards jump to label
  list->add(to);

  // identify both sides of edge to later allow them to be re-mapped to other
  // instructions
  _alt.put(from, from);
  _alt.put(to, to);
}
...
void CfgLister::printGmlCfg(FILE *f, LInsPrinter *printer,
                            InsSet *makeProxyNodesFor) {
  ...
  bool last =
      true; // true, when we're processing the last instruction of a block
  LIns *priorBlock = 0; // prior block that was just processed.

  LirReader rdr(finalIns());
  LIns *ins = rdr.read();
  for (; !ins->isop(LIR_start); ins = rdr.read()) {
    // if last instruction of the block is not terminal then add a fall-thru
    // edge
    if (priorBlock && last) {
      last = false;
      LOpcode op = ins->opcode();
    ...
    if (_vertices.containsKey(ins)) {
      // re-map all incoming edges to any instruction in the block, to the
      // blocks' first instruction
      ...
      // add the instruction to our running list and note any incoming edges to
      // the instruction
...
// prints a node that consists of a sequence of instructions, the first of which
// is used to populate the node attributes, such as shape.
void CfgLister::printNode(FILE *f, InsList *nodeIns, LInsPrinter *printer,
                          InsSet *pseudo) {
...
void CfgLister::printEdges(FILE *f, LInsPrinter *printer,
                           InsSet *makeProxyNodesFor) {
  ...
  while (ite.next()) {
    ...
    while (l) {
      ...
      // do we want to add a pseudo node for destination; removes clutter from
      // the graph
      if (proxyNode) {
        did = --pseudoId;
...
void CfgLister::gmlNode(FILE *f, uint32_t id, const char *shape,
                        const char *title) {
  fprintf(f, "  node [\n");
  fprintf(f, "    id %d\n", id);
  fprintf(f, "    graphics [\n");
...
void CfgLister::gmlEdge(FILE *f, uint32_t srcId, uint32_t dstId,
                        const char *style, const char *fill, const char *width,
                        const char *text) {
  fprintf(f, "  edge [\n");
  fprintf(f, "    source %d\n", srcId);
  fprintf(f, "    target %d\n", dstId);
...
void LirBuffer::chunkAlloc() {
  ...
  NanoAssert(_unused !=
             0); // Allocator.alloc() never returns null. See Allocator.h
  NanoAssert(0 == _unused % sizeof(void *)); // Require word alignment.
  _limit = _unused + CHUNK_SZB;
}

int32_t LirBuffer::insCount() { return _stats.lir; }
...
#ifdef NANOJIT_64BIT
  NanoAssert(sizeof(LIns) <= szB &&
             szB <= sizeof(LInsOp4)); // LInsOp4 is the biggest one
#else
  NanoAssert(sizeof(LIns) <= szB &&
             szB <= sizeof(LInsF4)); // LInsF4 is the biggest one
#endif
...
LIns *LirBufWriter::insStore(LOpcode op, LIns *val, LIns *base, int32_t d,
                             AccSet accSet) {
  if (isS16(d)) {
...
LIns *LirBufWriter::insLoad(LOpcode op, LIns *base, int32_t d, AccSet accSet,
  ...
  if (isS16(d)) {
    ...
    // If the displacement is more than 16 bits, put it in a separate
    // instruction. Note that CseFilter::insLoad() also does this, so this will
...
LIns *LirBufWriter::insGuard(LOpcode op, LIns *c, GuardRecord *gr) {
  debug_only(if (LIR_x == op || LIR_xbarrier == op)
                 NanoAssert(!c);) return ins2(op, c, (LIns *)gr);
}
...
LIns *LirBufWriter::insImmI(int32_t imm, bool tainted) {
  LInsIorF *insIorF = (LInsIorF *)_buf->makeRoom(sizeof(LInsIorF));
  LIns *ins = insIorF->getLIns();
  ins->initLInsIorF(LIR_immi, imm);
  ins->setTainted(tainted && _config.harden_blind_constants);
  return ins;
}

LIns *LirBufWriter::insImmF(float f, bool tainted) {
  union {
    float flt;
    int32_t i;
  } u;
...
#ifdef NANOJIT_64BIT
LIns *LirBufWriter::insImmQ(uint64_t imm, bool tainted) {
...
LIns *LirBufWriter::insImmD(double d, bool tainted) {
  LInsQorD *insQorD = (LInsQorD *)_buf->makeRoom(sizeof(LInsQorD));
  LIns *ins = insQorD->getLIns();
  union {
    double d;
...
LIns *LirBufWriter::insImmF4(const float4_t &f4, bool tainted) {
...
#define OP1OFFSET (offsetof(LInsOp1, ins) - offsetof(LInsOp1, oprnd_1))
  ...
  NanoStaticAssert(OP1OFFSET ==
                   (offsetof(LInsSt, ins) - offsetof(LInsSt, oprnd_1)));
  NanoStaticAssert(OP1OFFSET ==
                   (offsetof(LInsJtbl, ins) - offsetof(LInsJtbl, oprnd_1)));
...
#define OP2OFFSET (offsetof(LInsOp2, ins) - offsetof(LInsOp2, oprnd_2))
  NanoStaticAssert(OP2OFFSET ==
                   (offsetof(LInsOp3, ins) - offsetof(LInsOp3, oprnd_2)));
  NanoStaticAssert(OP2OFFSET ==
                   (offsetof(LInsSt, ins) - offsetof(LInsSt, oprnd_2)));
...
// We only propagate constaint taint for newly-created instructions.
...
LIns *ExprFilter::ins1(LOpcode v, LIns *oprnd) {
  ...
  case LIR_q2i:
    if (oprnd->isImmQ())
      return insImmI(oprnd->immQlo(), oprnd->isTainted());
    break;
  case LIR_i2q:
    if (oprnd->isImmI())
      return insImmQ(int64_t(int32_t(oprnd->immI())), oprnd->isTainted());
    break;
  case LIR_ui2uq:
    if (oprnd->isImmI())
      return insImmQ(uint64_t(uint32_t(oprnd->immI())), oprnd->isTainted());
    break;
  case LIR_dasq:
  ...
  case LIR_dlo2i:
    if (oprnd->isImmD())
      return insImmI(oprnd->immDlo(), oprnd->isTainted());
    if (oprnd->isop(LIR_ii2d))
      return oprnd->oprnd1();
    break;
  case LIR_dhi2i:
    if (oprnd->isImmD())
      return insImmI(oprnd->immDhi(), oprnd->isTainted());
    if (oprnd->isop(LIR_ii2d))
      return oprnd->oprnd2();
    break;
#endif
  ...
  case LIR_negf:
    if (oprnd->isImmF())
      return insImmF(-oprnd->immF(), oprnd->isTainted());
  ...
  case LIR_negf4:
    if (oprnd->isImmF4()) {
      ...
      return insImmF4(f4_sub(zero, oprnd->immF4()), oprnd->isTainted());
    }
  ...
  case LIR_i2d:
    if (oprnd->isImmI())
      return insImmD(oprnd->immI(), oprnd->isTainted());
    // Nb: i2d(d2i(x)) != x
    break;
#if defined NANOJIT_64BIT
  case LIR_q2d:
    if (oprnd->isImmQ())
      return insImmD(double(oprnd->immQ()), oprnd->isTainted());
    break;
#endif
  case LIR_d2i:
    if (oprnd->isImmD())
      return insImmI(int32_t(oprnd->immD()), oprnd->isTainted());
    if (oprnd->isop(LIR_i2d))
      return oprnd->oprnd1();
#if defined NANOJIT_64BIT
    if (oprnd->isop(LIR_q2d))
      return insImmI(int32_t(oprnd->immQ()), oprnd->isTainted());
#endif
    break;
  case LIR_ui2d:
    if (oprnd->isImmI())
      return insImmD(uint32_t(oprnd->immI()), oprnd->isTainted());
    break;
  case LIR_absd:
  case LIR_absf:
  case LIR_absf4:
...
// into ExprFilter::ins2(). See
// https://bugzilla.mozilla.org/show_bug.cgi?id=538504
...
LIns *ExprFilter::ins2(LOpcode v, LIns *oprnd1, LIns *oprnd2) {
  ...
  // This is an unfortunate case where an attacker can generate a chosen
  // constant without having to explicitly mention any constants in his code!
  // It is not clear how valuable this optimization is, and it is likely to
  // be counterproductive if any internally-generated expressions can trigger
  // it.
  ...
  if (oprnd1 == oprnd2) {
    ...
    case LIR_gei:
    case LIR_geui:
      return insImmI(1, /*tainted*/ true); // (x <= x) == 1; (x >= x) == 1
  ...
  // Note that we taint the result of a folded operation even when only one of
  // the operands is tainted.  While the attacker presumably cannot directly
  // control an untainted operand, its value may be predictable.  In this case,
  // even a shift count under attacker control is trivially exploitable.  We can
  // isolate and position a single bit with shifts, then add/or several such
  // expressions to construct any value desired.

  if (oprnd1->isImmI() && oprnd2->isImmI()) {
    // The operands are both int immediates.
    int32_t c1 = oprnd1->immI();
    int32_t c2 = oprnd2->immI();
    bool tainted = (oprnd1->isTainted() | oprnd2->isTainted());
    double d;
    int32_t r;
...
#if NJ_SOFTFLOAT_SUPPORTED
    case LIR_ii2d:
      return insImmD(do_join(c1, c2), tainted);
#endif
    case LIR_eqi:
      return insImmI(c1 == c2, tainted);
    case LIR_lti:
      return insImmI(c1 < c2, tainted);
    case LIR_gti:
      return insImmI(c1 > c2, tainted);
    ...
    case LIR_gei:
      return insImmI(c1 >= c2, tainted);
    case LIR_ltui:
      return insImmI(uint32_t(c1) < uint32_t(c2), tainted);
    case LIR_gtui:
      return insImmI(uint32_t(c1) > uint32_t(c2), tainted);
    case LIR_leui:
      return insImmI(uint32_t(c1) <= uint32_t(c2), tainted);
    ...
    case LIR_lshi:
      return insImmI(c1 << (c2 & 0x1f), tainted);
    case LIR_rshi:
      return insImmI(c1 >> (c2 & 0x1f), tainted);
    case LIR_rshui:
      return insImmI(uint32_t(c1) >> (c2 & 0x1f), tainted);
    ...
    case LIR_geq:
      return insImmI(c1 >= c2, tainted);
    case LIR_ltuq:
      return insImmI(uint64_t(c1) < uint64_t(c2), tainted);
    case LIR_gtuq:
      return insImmI(uint64_t(c1) > uint64_t(c2), tainted);
    case LIR_leuq:
      return insImmI(uint64_t(c1) <= uint64_t(c2), tainted);
    ...
    switch (v) {
    case LIR_lshq:
      return insImmQ(c1 << (c2 & 0x3f), tainted);
    case LIR_rshq:
      return insImmQ(c1 >> (c2 & 0x3f), tainted);
    case LIR_rshuq:
      return insImmQ(uint64_t(c1) >> (c2 & 0x3f), tainted);
    ...
    switch (v) {
    case LIR_eqf4:
      return insImmI(f4_eq_i(c1, c2), tainted);

    case LIR_addf4:
      return insImmF4(f4_add(c1, c2), tainted);
    case LIR_subf4:
      return insImmF4(f4_sub(c1, c2), tainted);
    case LIR_mulf4:
      return insImmF4(f4_mul(c1, c2), tainted);
    case LIR_divf4:
      return insImmF4(f4_div(c1, c2), tainted);
  ...
  if (oprnd2->isImmI()) {
    ...
    case LIR_addi:
      if (oprnd1->isop(LIR_addi) && oprnd1->oprnd2()->isImmI()) {
        ...
        oprnd2 =
            insImmI(c, (oprnd1->oprnd2()->isTainted() | oprnd2->isTainted()));
    ...
    case LIR_rshi:
      if (c == 16 && oprnd1->isop(LIR_lshi) && oprnd1->oprnd2()->isImmI(16) &&
    ...
    if (c == 0) {
      ...
      case LIR_geui: // unsigned >= 0 -> always true
        return insImmI(1, oprnd2->isTainted());

      case LIR_eqi:
        if (oprnd1->isop(LIR_ori) && oprnd1->oprnd2()->isImmI() &&
            oprnd1->oprnd2()->immI() != 0) {
          // (x or c) != 0 if c != 0
          return insImmI(0, oprnd2->isTainted());
        }
      ...
      case LIR_gtui:
        return insImmI(
            0, oprnd2->isTainted()); // u32 >  0xffffffff -> always false
      case LIR_leui:
        return insImmI(1,
                       oprnd2->isTainted()); // u32 <= 0xffffffff -> always true
      default:
        break;
      }
      ...
      if (oprnd1->isCmp()) {
        switch (v) {
        case LIR_ori:
          return oprnd2; // 0or1 | 1 = 1   (and oprnd2 == 1)
        case LIR_andi:
          return oprnd1; // 0or1 & 1 = 0or1
        case LIR_gtui:
          return insImmI(0, oprnd2->isTainted()); // 0or1 > 1 -> always false
        default:
          break;
        }
      } else if (v == LIR_muli) {
    ...
    if (c == 0) {
      ...
      case LIR_gtuq:
        return insImmI(
            0,
            oprnd2->isTainted()); // u64 >  0xffffffffffffffff -> always false
      case LIR_leuq:
        return insImmI(
            1, oprnd2->isTainted()); // u64 <= 0xffffffffffffffff -> always true
      default:
        break;
      }
...
LIns *ExprFilter::ins4(LOpcode v, LIns *oprnd1, LIns *oprnd2, LIns *oprnd3,
                       LIns *oprnd4) {
...
LIns *ExprFilter::insGuard(LOpcode v, LIns *c, GuardRecord *gr) {
  if (v == LIR_xt || v == LIR_xf) {
    if (c->isImmI()) {
      ...
      while (c->isop(LIR_eqi) && c->oprnd1()->isCmp() &&
             c->oprnd2()->isImmI(0)) {
...
LIns *ExprFilter::simplifyOverflowArith(LOpcode op, LIns **opnd1,
                                        LIns **opnd2) {
  LIns *oprnd1 = *opnd1;
  LIns *oprnd2 = *opnd2;
  ...
  if (oprnd1->isImmI() && oprnd2->isImmI()) {
    ...
    int32_t r = int32_t(d);
    if (r == d)
      return insImmI(r, (oprnd1->isTainted() | oprnd2->isTainted()));
...
LIns *ExprFilter::insGuardXov(LOpcode op, LIns *oprnd1, LIns *oprnd2,
                              GuardRecord *gr) {
...
LIns *ExprFilter::insBranchJov(LOpcode op, LIns *oprnd1, LIns *oprnd2,
                               LIns *target) {
  LIns *simplified = simplifyOverflowArith(op, &oprnd1, &oprnd2);
  if (simplified)
    return simplified;
...
LIns *LirWriter::insChoose(LIns *cond, LIns *iftrue, LIns *iffalse,
                           bool use_cmov) {
  ...
  return ins2(LIR_ori, ins2(LIR_andi, iftrue, ncond),
...
#ifdef NANOJIT_WIN_CFG
  if (ci->isIndirect()) {
    // Insert call to Control Flow Guard check function.  Argument is address of
    // indirect call.
    NanoAssert(argc > 0);
...
LIns *StackFilter::read() {
  for (;;) {
    ...
    /*
     * NB: If there is a backward branch other than the loop-restart branch,
     * this is going to be wrong. Unfortunately there doesn't seem to be an easy
     * way to detect such branches. Just do not create any.
     *
...
public:
  ...
  LiveTable(Allocator &alloc)
      : alloc(alloc), live(alloc), retired(alloc), retiredCount(0), maxlive(0) {
  }

  void add(LIns *ins, LIns *use) {
    if (!ins->isImmAny() && !live.containsKey(ins)) {
      NanoAssert(size_t(ins->opcode()) <
                 sizeof(lirNames) / sizeof(lirNames[0]));
      live.put(ins, use);
    }
  }
  ...
  bool contains(LIns *i) { return live.containsKey(i); }
};
...
void LirNameMap::addNameWithSuffix(LIns *ins, const char *name, int suffix,
  ...
  if (suffix == 1 && ignoreOneSuffix) {
    ...
    VMPI_snprintf(name2, N, "%s_%d", name,
                  suffix); // use '_' to avoid confusion
  } else {
...
const char *LirNameMap::createName(LIns *ins) {
  if (ins->isCall()) {
    ...
    {
      if (!names.containsKey(ins))
        addNameWithSuffix(ins, ins->callInfo()->_name,
                          funccounts.add(ins->callInfo()),
                          /*ignoreOneSuffix*/ false);
    }
  } else {
    if (!names.containsKey(ins))
      addNameWithSuffix(ins, lirNames[ins->opcode()],
                        lircounts.add(ins->opcode()),
                        /*ignoreOneSuffix*/ false);
  }
...
const char *LInsPrinter::formatTaint(LIns *ref) {
  return ref->isTainted() ? " !" : "";
}
...
char *LInsPrinter::formatRef(RefBuf *buf, LIns *ref, bool showImmValue) {
  ...
  if (ref->isImmI() && showImmValue) {
    VMPI_snprintf(buf->buf, buf->len, "%s/*%s*/%s", name,
                  formatImmI(&buf2, ref->immI()), formatTaint(ref));
  }
#ifdef NANOJIT_64BIT
  else if (ref->isImmQ() && showImmValue) {
    VMPI_snprintf(buf->buf, buf->len, "%s/*%s*/%s", name,
                  formatImmQ(&buf2, ref->immQ()), formatTaint(ref));
  }
#endif
  else if (ref->isImmD() && showImmValue) {
    VMPI_snprintf(buf->buf, buf->len, "%s/*%s*/%s", name,
                  formatImmD(&buf2, ref->immD()), formatTaint(ref));
  } else if (ref->isImmF() && showImmValue) { // float is printed as a double
    VMPI_snprintf(buf->buf, buf->len, "%s/*%s*/%s", name,
                  formatImmD(&buf2, ref->immF()), formatTaint(ref));
  } else if (ref->isImmF4() &&
             showImmValue) { // float4 is printed as an array of 4 double values
    RefBuf bufx, bufy, bufz, bufw;
    float4_t v = ref->immF4();
    VMPI_snprintf(buf->buf, buf->len, "%s/*%s,%s,%s,%s*/%s", name,
                  formatImmD(&bufx, f4_x(v)), formatImmD(&bufy, f4_y(v)),
                  formatImmD(&bufz, f4_z(v)), formatImmD(&bufw, f4_w(v)),
                  formatTaint(ref));
  } else {
...
char *LInsPrinter::formatIns(InsBuf *buf, LIns *i) {
  ...
  switch (op) {
  case LIR_immi:
    VMPI_snprintf(s, n, "%s = %s %s%s",
                  formatRef(&b1, i, /*showImmValue*/ false), lirNames[op],
                  formatImmI(&b2, i->immI()), formatTaint(i));
    break;
  ...
  case LIR_immq:
    VMPI_snprintf(s, n, "%s = %s %s%s",
                  formatRef(&b1, i, /*showImmValue*/ false), lirNames[op],
                  formatImmQ(&b2, i->immQ()), formatTaint(i));
    break;
#endif
  ...
  case LIR_immf:
    VMPI_snprintf(s, n, "%s = %s %s%s",
                  formatRef(&b1, i, /*showImmValue*/ false), lirNames[op],
                  formatImmD(&b2, i->immF()), formatTaint(i));
    break;

  case LIR_immf4: {
    RefBuf b5;
    float4_t v = i->immF4();
    VMPI_snprintf(s, n, "%s = %s %s,%s,%s,%s%s",
                  formatRef(&b1, i, /*showImmValue*/ false), lirNames[op],
                  formatImmD(&b2, f4_x(v)), formatImmD(&b3, f4_y(v)),
                  formatImmD(&b4, f4_z(v)), formatImmD(&b5, f4_w(v)),
                  formatTaint(i));
    break;
  }
  ...
  case LIR_immd:
    VMPI_snprintf(s, n, "%s = %s %s%s",
                  formatRef(&b1, i, /*showImmValue*/ false), lirNames[op],
                  formatImmD(&b2, i->immD()), formatTaint(i));
    break;

  case LIR_allocp:
    VMPI_snprintf(s, n, "%s = %s %d", formatRef(&b1, i), lirNames[op],
                  i->size());
    break;
  ...
  case LIR_callf4: {
    ...
    if (call->isIndirect())
      m -= VMPI_snprintf(s, m, "%s = %s%s [%s] ( ", formatRef(&b1, i),
                         lirNames[op], formatAccSet(&b2, call->_storeAccSet),
  ...
  case LIR_jtbl: {
    int32_t m = int32_t(n); // Windows doesn't have 'ssize_t'
    m -= VMPI_snprintf(s, m, "%s %s [ ", lirNames[op],
    ...
    for (uint32_t j = 0, sz = i->getTableSize(); j < sz; j++) {
      ...
      s += VMPI_strlen(s);
      m -= VMPI_snprintf(s, m, "%s ",
                         target ? formatRef(&b2, target) : "unpatched");
      if (m < 0)
        break;
    }
    s += VMPI_strlen(s);
  ...
  case LIR_paramp: {
    ...
    if (!i->paramKind()) {
      ...
      VMPI_snprintf(s, n, "%s = %s %d %s", formatRef(&b1, i), lirNames[op], arg,
                    gpn(RegAlloc::savedRegs[arg]));
    }
    break;
  }
  ...
  case LIR_jf:
  case LIR_brsavpc:
    VMPI_snprintf(s, n, "%s %s -> %s", lirNames[op],
                  formatRef(&b1, i->oprnd1()),
  ...
  case LIR_muljovi:
    CASE64(LIR_addjovq:)
    CASE64(LIR_subjovq:)
    VMPI_snprintf(s, n, "%s = %s %s, %s ; ovf -> %s", formatRef(&b1, i),
                  lirNames[op], formatRef(&b2, i->oprnd1()),
  ...
  case LIR_ii2d:
    ...
    VMPI_snprintf(s, n, "%s = %s %s, %s", formatRef(&b1, i), lirNames[op],
                  formatRef(&b2, i->oprnd1()), formatRef(&b3, i->oprnd2()));
    break;
  ...
  case LIR_ffff2f4:
    VMPI_snprintf(s, n, "%s =(%s)= %s %s %s %s", formatRef(&b1, i),
                  lirNames[op], formatRef(&b2, i->oprnd1()),
                  formatRef(&b3, i->oprnd2()), formatRef(&b4, i->oprnd3()),
  ...
  case LIR_ldf2d: {
    ...
    VMPI_snprintf(s, n, "%s = %s%s%s %s[%d]", formatRef(&b1, i), lirNames[op],
                  formatAccSet(&b2, i->accSet()), qualStr,
                  formatRef(&b3, i->oprnd1()), i->disp());
    break;
  }
  ...
  case LIR_std2f:
    VMPI_snprintf(s, n, "%s%s %s[%d] = %s", lirNames[op],
                  formatAccSet(&b1, i->accSet()), formatRef(&b2, i->oprnd2()),
                  i->disp(), formatRef(&b3, i->oprnd1()));
    break;
  ...
  case LIR_swzf4:
    VMPI_snprintf(s, n, "%s %s, %d|%d|%d|%d", lirNames[LIR_swzf4],
                  formatRef(&b1, i->oprnd1()), i->mask() >> 0 & 3,
                  i->mask() >> 2 & 3, i->mask() >> 4 & 3, i->mask() >> 6 & 3);
    break;
...
CseFilter::CseFilter(LirWriter *out, uint8_t embNumUsedAccs, Allocator &alloc,
                     const Config &config)
    : LirWriter(out), EMB_NUM_USED_ACCS(embNumUsedAccs),
      CSE_NUM_USED_ACCS(EMB_NUM_USED_ACCS + 2),
      CSE_ACC_CONST(EMB_NUM_USED_ACCS + 0),
      CSE_ACC_MULTIPLE(EMB_NUM_USED_ACCS + 1), storesSinceLastLoad(ACCSET_NONE),
      alloc(alloc), knownCmpValues(alloc), suspended(0), config(config) {
  ...
  m_capNL[NLImmISmall] =
      2 * 17; // covers 0..16, tainted/untainted (over 50% on Tracemonkey)
  ...
  for (NLKind nlkind = NLFirst; nlkind <= NLLast; nlkind = nextNLKind(nlkind)) {
    m_listNL[nlkind] = (LIns **)alloc.alloc(sizeof(LIns *) * m_capNL[nlkind]);
    NanoAssert(m_listNL[nlkind]);
  ...
  for (CseAcc a = 0; a < CSE_NUM_USED_ACCS; a++) {
    m_capL[a] = 16;
    m_listL[a] = (LIns **)alloc.alloc(sizeof(LIns *) * m_capL[a]);
    NanoAssert(m_listL[a]);
...
inline uint32_t CseFilter::hash4(LOpcode op, LIns *a, LIns *b, LIns *c,
                                 LIns *d) {
  uint32_t hash = hash8(0, uint8_t(op));
  hash = hashptr(hash, a);
  hash = hashptr(hash, b);
...
inline uint32_t CseFilter::hashCall(const CallInfo *ci, uint32_t argc,
                                    LIns *args[]) {
...
void CseFilter::growNL(NLKind nlkind) {
  ...
  m_capNL[nlkind] <<= 1;
  LIns **tmp = (LIns **)alloc.alloc(sizeof(LIns *) * m_capNL[nlkind]);
  NanoAssert(tmp);
...
void CseFilter::growL(CseAcc cseAcc) {
  ...
  m_capL[cseAcc] <<= 1;
  LIns **tmp = (LIns **)alloc.alloc(sizeof(LIns *) * m_capL[cseAcc]);
  NanoAssert(tmp);
...
inline LIns *CseFilter::findImmISmall(int32_t a, uint32_t &k, bool tainted) {
  // This one is a direct array lookup rather than a hashtable lookup.
  NLKind nlkind = NLImmISmall;
  k = (a * 2) + (unsigned(tainted) & 1);
...
uint32_t CseFilter::findImmISmall(LIns *ins) {
  uint32_t k;
  findImmISmall(ins->immI(), k, ins->isTainted());
  return k;
}

inline LIns *CseFilter::findImmILarge(int32_t a, uint32_t &k, bool tainted) {
  NLKind nlkind = NLImmILarge;
  const uint32_t bitmask = m_capNL[nlkind] - 1;
  k = (hashImmI(a) + (unsigned(tainted) & 1)) & bitmask;
  uint32_t n = 1;
  while (true) {
    LIns *ins = m_listNL[nlkind][k];
    if (!ins)
    ...
    if (ins->immI() == a && ins->isTainted() == tainted)
      return ins;
...
uint32_t CseFilter::findImmILarge(LIns *ins) {
  uint32_t k;
  findImmILarge(ins->immI(), k, ins->isTainted());
  return k;
}

#ifdef NANOJIT_64BIT
inline LIns *CseFilter::findImmQ(uint64_t a, uint32_t &k, bool tainted) {
  NLKind nlkind = NLImmQ;
  const uint32_t bitmask = m_capNL[nlkind] - 1;
  k = (hashImmQorD(a) + (unsigned(tainted) & 1)) & bitmask;
  uint32_t n = 1;
  while (true) {
    LIns *ins = m_listNL[nlkind][k];
    if (!ins)
    ...
    if (ins->immQ() == a && ins->isTainted() == tainted)
      return ins;
    k = (k + n) & bitmask;
    n += 1;
  }
...
uint32_t CseFilter::findImmQ(LIns *ins) {
  uint32_t k;
  findImmQ(ins->immQ(), k, ins->isTainted());
  return k;
}
#endif

inline LIns *CseFilter::findImmD(uint64_t a, uint32_t &k, bool tainted) {
  ...
  while (true) {
    ...
    if (ins->immDasQ() == a && ins->isTainted() == tainted)
      return ins;
    k = (k + n) & bitmask;
    n += 1;
  }
...
uint32_t CseFilter::findImmD(LIns *ins) {
  ...
  uint32_t k;
  findImmD(ins->immDasQ(), k, ins->isTainted());
  return k;
}

inline LIns *CseFilter::findImmF(int32_t a, uint32_t &k, bool tainted) {
  ...
  while (true) {
    ...
    if (ins->immFasI() == a && ins->isTainted() == tainted)
      return ins;
    k = (k + n) & bitmask;
    n += 1;
  }
...
uint32_t CseFilter::findImmF(LIns *ins) {
  ...
  // Use bitwise (integer) comparison.
  uint32_t k;
  findImmF(ins->immFasI(), k, ins->isTainted());
  return k;
}
...
inline LIns *CseFilter::findImmF4(const float4_t &a, uint32_t &k,
  ...
  const uint32_t bitmask = m_capNL[nlkind] - 1;
  uint32_t n = 1;
  k = (hashImmF4(a) + (unsigned(tainted) & 1)) & bitmask;
  while (true) {
    LIns *ins = m_listNL[nlkind][k];
    if (!ins)
      return NULL;
...
uint32_t CseFilter::findImmF4(LIns *ins) {
  ...
  uint32_t k;
  findImmF4(ins->immF4(), k, ins->isTainted());
  return k;
}
...
inline LIns *CseFilter::find3(LOpcode op, LIns *a, LIns *b, LIns *c,
                              uint32_t &k) {
  NLKind nlkind = NL3;
  const uint32_t bitmask = m_capNL[nlkind] - 1;
...
inline LIns *CseFilter::find4(LOpcode op, LIns *a, LIns *b, LIns *c, LIns *d,
                              uint32_t &k) {
  NLKind nlkind = NL4;
  const uint32_t bitmask = m_capNL[nlkind] - 1;
...
uint32_t CseFilter::find4(LIns *ins) {
  uint32_t k;
  find4(ins->opcode(), ins->oprnd1(), ins->oprnd2(), ins->oprnd3(),
        ins->oprnd4(), k);
  return k;
}

inline LIns *CseFilter::findLoad(LOpcode op, LIns *a, int32_t d,
                                 MiniAccSet miniAccSet, LoadQual loadQual,
                                 uint32_t &k) {
  ...
  while (true) {
    ...
    // loadQual.
    NanoAssert(miniAccSetToCseAcc(ins->miniAccSet(), ins->loadQual()) ==
                   cseAcc &&
...
uint32_t CseFilter::findLoad(LIns *ins) {
  uint32_t k;
  findLoad(ins->opcode(), ins->oprnd1(), ins->disp(), ins->miniAccSet(),
           ins->loadQual(), k);
  return k;
}
...
inline LIns *CseFilter::findCall(const CallInfo *ci, uint32_t argc,
                                 LIns *args[], uint32_t &k) {
...
// Tainted and non-tainted constants of equal value are treated
// as distinct by CSE, thus preserving the taint status.  We might
// like to share small constants that will not be blinded anyhow,
// but this would allow synthesis of arbitrary untainted constants
// through constant folding.  We could avoid this hazard by
// rewriting CSEs after all constant folding has been performed, but
// such a strategy is incompatible with our one-pass optimizer
// architecture, thus we sacrifice some CSE opportunities when
// constant blinding is enabled.
//
// In practice, we expect sharing between tainted and untainted
// expressions to be confined mostly to leaf constants, which we
// tend to rematerialize frequently in preference to long-term
// register residency.  In fact, the loss of leaf constant sharing
// may reduce register pressure, due to pragmatic restrictions on
// rematerialization of tainted constants that can force spills.

LIns *CseFilter::insImmI(int32_t imm, bool tainted) {
  uint32_t k;
  LIns *ins;
  const bool blind = tainted && config.harden_blind_constants;
  // Each value gets two slots in m_capNL: one for tainted and another for
  // untainted.
  if (0 <= imm && imm < int32_t(m_capNL[NLImmISmall]) / 2) {
    ins = findImmISmall(imm, k, blind);
    if (!ins) {
      ins = out->insImmI(imm, blind);
      addNLImmISmall(ins, k);
    }
  } else {
    ins = findImmILarge(imm, k, blind);
    if (!ins) {
      ins = out->insImmI(imm, blind);
...
#ifdef NANOJIT_64BIT
LIns *CseFilter::insImmQ(uint64_t q, bool tainted) {
  uint32_t k;
  const bool blind = tainted && config.harden_blind_constants;
  LIns *ins = findImmQ(q, k, blind);
  if (!ins) {
    ins = out->insImmQ(q, blind);
    addNL(NLImmQ, ins, k);
  }
...
LIns *CseFilter::insImmD(double d, bool tainted) {
  uint32_t k;
  ...
  u.d = d;
  const bool blind = tainted && config.harden_blind_constants;
  LIns *ins = findImmD(u.u64, k, blind);
  if (!ins) {
    ins = out->insImmD(d, blind);
    addNL(NLImmD, ins, k);
  }
...
LIns *CseFilter::insImmF(float f, bool tainted) {
  uint32_t k;
  ...
  u.flt = f;
  const bool blind = tainted && config.harden_blind_constants;
  LIns *ins = findImmF(u.i, k, blind);
  if (!ins) {
    ins = out->insImmF(f, blind);
    addNL(NLImmF, ins, k);
  }
...
LIns *CseFilter::insImmF4(const float4_t &f4, bool tainted) {
  uint32_t k;
  const bool blind = tainted && config.harden_blind_constants;
  LIns *ins = findImmF4(f4, k, blind);
  if (!ins) {
    ins = out->insImmF4(f4, blind);
    addNL(NLImmF4, ins, k);
  }
#ifdef DEBUG
...
LIns *CseFilter::ins2(LOpcode op, LIns *a, LIns *b) {
  ...
  if (!ins) {
    ...
    if (knownCmpValues.containsKey(ins)) {
      ...
      return insImmI((cmpValue ? 1 : 0), /*tainted*/ false);
    }
  }
...
LIns *CseFilter::insLoad(LOpcode op, LIns *base, int32_t disp, AccSet accSet,
                         LoadQual loadQual) {
  LIns *ins;
  if (isS16(disp)) {
    ...
    ins = insLoad(op, ins2(LIR_addp, base, insImmWord(disp)), 0, accSet,
                  loadQual);
  }
  return ins;
}

LIns *CseFilter::insStore(LOpcode op, LIns *value, LIns *base, int32_t disp,
                          AccSet accSet) {
  LIns *ins;
  if (isS16(disp)) {
    storesSinceLastLoad |= accSet;
    ins = out->insStore(op, value, base, disp, accSet);
    NanoAssert(ins->isop(op) && ins->oprnd1() == value &&
               ins->oprnd2() == base && ins->disp() == disp &&
               ins->accSet() == accSet);
  } else {
    // If the displacement is more than 16 bits, put it in a separate
    // instruction.  Nb: LirBufWriter also does this, we do it here
    // too because CseFilter relies on LirBufWriter not changing code.
    ins =
        insStore(op, value, ins2(LIR_addp, base, insImmWord(disp)), 0, accSet);
  }
  return ins;
}
...
LIns *CseFilter::insCall(const CallInfo *ci, LIns *args[]) {
  ...
  NanoAssert(ins->isCall() && ins->callInfo() == ci &&
             argsmatch(ins, argc, args));
  NanoAssert(!ins->isInReg());
  return ins;
}
...
Interval Interval::of(LIns *ins, int lim) {
  ...
  case LIR_rshi: {
    ...
    if (ins->oprnd2()->isImmI()) {
      ...
      int32_t y = ins->oprnd2()->immI() & 0x1f; // we only use the bottom 5 bits
      return Interval(-(1 << (31 - y)), (1 << (31 - y)) - 1);
    }
    goto worst_non_overflow;
  }
...
#define SF_CALLINFO(name, typesig)                                             \
  static const CallInfo name##_ci = {(intptr_t)&name, typesig, ABI_FASTCALL,   \
                                     /*isPure*/ 1,                             \
                                     ACCSET_NONE verbose_only(, #name)}
...
LIns *SoftFloatFilter::ins1(LOpcode op, LIns *a) {
  ...
  if (ci) {
    NanoAssertMsg(ci->returnType() != ARGTYPE_F,
                  "Softfloat not implemented yet for single precision floats.");
    if (ci->returnType() == ARGTYPE_D)
      return callD1(ci, a);
    else
      return callI1(ci, a);
  }
  NanoAssertMsg(op != LIR_retf,
                "Softfloat not implemented yet for single precision floats.");
  if (op == LIR_retd)
...
AddrNameMap::AddrNameMap(Allocator &a) : allocator(a), names(a) {}

void AddrNameMap::addAddrRange(const void *p, size_t size, size_t align,
                               const char *name) {
...
void ValidateWriter::typeCheckArgs(LOpcode op, int nArgs, LTy formals[],
                                   LIns *args[]) {
  NanoAssert(nArgs >= 0);
  ...
  for (int i = 0; i < nArgs; i++) {
    ...
    if (formal != actual) {
      ...
      NanoAssertMsgf(0,
                     ...
                     lirNames[args[i]->opcode()], type2string(actual),
                     type2string(formal));
    }
  }
}

void ValidateWriter::errorStructureShouldBe(LOpcode op, const char *argDesc,
                                            int argN, LIns *arg,
                                            const char *shouldBeDesc) {
  NanoAssertMsgf(
      0, "LIR structure error (%s): %s %d of '%s' is '%s' (expected %s)",
      whereInPipeline, argDesc, argN, lirNames[op], lirNames[arg->opcode()],
      shouldBeDesc);
}

void ValidateWriter::errorAccSet(const char *what, AccSet accSet,
                                 const char *shouldDesc) {
  RefBuf b;
  NanoAssertMsgf(0, "LIR AccSet error (%s): '%s' AccSet is '%s'; %s",
                 whereInPipeline, what, printer->formatAccSet(&b, accSet),
                 shouldDesc);
}

void ValidateWriter::errorLoadQual(const char *what, LoadQual loadQual) {
  NanoAssertMsgf(0, "LIR LoadQual error (%s): '%s' loadQual is '%d'",
...
void ValidateWriter::checkLInsIsACondOrConst(LOpcode op, int argN, LIns *ins) {
  ...
  if (!ins->isCmp() && !ins->isImmI())
    errorStructureShouldBe(op, "argument", argN, ins,
                           "a condition or 32-bit constant");
}
...
void ValidateWriter::checkLInsHasOpcode(LOpcode op, int argN, LIns *ins,
                                        LOpcode op2) {
  if (!ins->isop(op2))
    errorStructureShouldBe(op, "argument", argN, ins, lirNames[op2]);
}

ValidateWriter::ValidateWriter(LirWriter *out, LInsPrinter *printer,
                               const char *where)
...
LIns *ValidateWriter::insImmI(int32_t imm, bool tainted) {
  return out->insImmI(imm, tainted);
}
...
#ifdef NANOJIT_64BIT
LIns *ValidateWriter::insImmQ(uint64_t imm, bool tainted) {
  return out->insImmQ(imm, tainted);
}
#endif

LIns *ValidateWriter::insImmD(double d, bool tainted) {
  return out->insImmD(d, tainted);
}

LIns *ValidateWriter::insImmF(float f, bool tainted) {
  return out->insImmF(f, tainted);
}

LIns *ValidateWriter::insImmF4(const float4_t &f, bool tainted) {
  return out->insImmF4(f, tainted);
}
...
LIns *ValidateWriter::insCall(const CallInfo *ci, LIns *args0[]) {
  ...
  if ((op == LIR_callv) != (retType == ARGTYPE_V) ||
      (op == LIR_calli) != (retType == ARGTYPE_UI || retType == ARGTYPE_I) ||
#ifdef NANOJIT_64BIT
    ...
    NanoAssertMsgf(0,
                   "LIR structure error (%s): return type mismatch: opcode %s "
                   "with %s return type",
  ...
  if (op == LIR_callv && ci->_isPure) {
    ...
    NanoAssertMsgf(0,
                   "LIR structure error (%s): LIR_callv must only be used with "
                   "nonpure functions.",
                   whereInPipeline);
  }

  if (ci->_isPure && ci->_storeAccSet != ACCSET_NONE)
    errorAccSet(ci->_name, ci->_storeAccSet,
                "it should be ACCSET_NONE for pure functions");
...
LIns *ValidateWriter::insGuardXov(LOpcode op, LIns *a, LIns *b,
                                  GuardRecord *gr) {
  int nArgs = 2;
...
LIns *ValidateWriter::insAlloc(int32_t size) { return out->insAlloc(size); }
