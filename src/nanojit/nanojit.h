...
#define _NanoAssertMsgf(a, file_, line_, f, ...)                               \
  __NanoAssertMsgf(a, file_, line_, f, __VA_ARGS__)

#define NanoAssertMsgf(a, f, ...)                                              \
  do {                                                                         \
    __NanoAssertMsgf(a, __FILE__, __LINE__, f ": ", __VA_ARGS__);              \
  } while (0)
#define NanoAssertMsg(a, m)                                                    \
  do {                                                                         \
    __NanoAssertMsgf(a, __FILE__, __LINE__, "\"%s\": ", m);                    \
  } while (0)
#define NanoAssert(a)                                                          \
  do {                                                                         \
    __NanoAssertMsgf(a, __FILE__, __LINE__, "%s", "");                         \
  } while (0)
#else
#define NanoAssertMsgf(a, f, ...)                                              \
  do {                                                                         \
...
static inline bool isS32(intptr_t i) { return int32_t(i) == i; }

static inline bool isU32(uintptr_t i) { return uint32_t(i) == i; }

#define alignTo(x, s) ((((uintptr_t)(x))) & ~(((uintptr_t)s) - 1))
#define alignUp(x, s)                                                          \
  ((((uintptr_t)(x)) + (((uintptr_t)s) - 1)) & ~(((uintptr_t)s) - 1))
...
// Note that values in the range -256..255 are not blinded even when they will
// be represented in 4 or 8 bytes.  In all of the cases permitted, only one byte
// of the representation can be controlled by the value, while the others are
// always either 0x00 or 0xff.
static inline bool shouldBlind(int32_t val) { return val > 127 || val < -128; }
static inline bool shouldBlind(uint32_t val) {
  return shouldBlind(int32_t(val));
}
static inline bool shouldBlind(int64_t val) { return val > 127 || val < -128; }
static inline bool shouldBlind(uint64_t val) {
  return shouldBlind(int64_t(val));
}
static inline bool shouldBlindDisplacement(int32_t val) {
  return val > 4095 || val < -4096;
}
...
static inline unsigned msbSet32(uint32_t x) {
  unsigned long idx;
  _BitScanReverse(
      &idx, (unsigned long)(x | 1)); // the '| 1' ensures a 0 result when x==0
  return idx;
}
...
static inline unsigned lsbSet32(uint32_t x) {
  unsigned long idx;
  _BitScanForward(
      &idx, (unsigned long)(x | 0x80000000)); // the '| 0x80000000' ensures a 0
                                              // result when x==0
  return idx;
}
...
static inline unsigned msbSet64(uint64_t x) {
  ...
  _BitScanReverse64(
      &idx,
      (unsigned __int64)(x | 1)); // the '| 1' ensures a 0 result when x==0
  return idx;
}
...
static inline unsigned lsbSet64(uint64_t x) {
  ...
  _BitScanForward64(
      &idx,
      (unsigned __int64)(x |
                         0x8000000000000000LL)); // the '| 0x80000000' ensures a
                                                 // 0 result when x==0
  return idx;
}
#else
...
static unsigned msbSet64(uint64_t x) {
  return (x & 0xffffffff00000000LL) ? msbSet32(uint32_t(x >> 32)) + 32
                                    : msbSet32(uint32_t(x));
}
// Returns the index of the least significant bit that is set.
static unsigned lsbSet64(uint64_t x) {
  return (x & 0x00000000ffffffffLL) ? lsbSet32(uint32_t(x))
                                    : lsbSet32(uint32_t(x >> 32)) + 32;
}
#endif
