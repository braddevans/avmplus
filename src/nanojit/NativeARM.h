...
static const Register
    ...
    R8 = {8}, R9 = {9}, R10 = {10}, FP = {11}, IP = {12}, SP = {13}, LR = {14},
    PC = {15},

    // VFP regs
    S0 = {16}, S1 = S0 + 1, S2 = S0 + 2, S3 = S0 + 3, S4 = S0 + 4, S5 = S0 + 5,
    S6 = S0 + 6, S7 = S0 + 7, S8 = S0 + 8, S9 = S0 + 9, S10 = S0 + 10,
    S11 = S0 + 11, S12 = S0 + 12, S13 = S0 + 13, S14 = S0 + 14, S15 = S0 + 15,
    S16 = S0 + 16, S17 = S0 + 17, S18 = S0 + 18, S19 = S0 + 19, S20 = S0 + 20,
    S21 = S0 + 21, S22 = S0 + 22, S23 = S0 + 23, S24 = S0 + 24, S25 = S0 + 25,
    S26 = S0 + 26, S27 = S0 + 27, S28 = S0 + 28, S29 = S0 + 29, S30 = S0 + 30,
    S31 = S0 + 31,
    // We reverse the order of the Dn register banks: first, the Dn regs that
    // have no Sn subregs
    D16 = {48}, D17 = D16 + 1, D18 = D16 + 2, D19 = D16 + 3, D20 = D16 + 4,
    D21 = D16 + 5, D22 = D16 + 6, D23 = D16 + 7, D24 = D16 + 8, D25 = D16 + 9,
    D26 = D16 + 10, D27 = D16 + 11, D28 = D16 + 12, D29 = D16 + 13,
    D30 = D16 + 14, D31 = D16 + 15,
    // The following regs are just alternate names for gruops of registers
    // already listed above
    D0 = {64}, D1 = D0 + 1, D2 = D0 + 2, D3 = D0 + 3, D4 = D0 + 4, D5 = D0 + 5,
    D6 = D0 + 6, D7 = D0 + 7, D8 = D0 + 8, D9 = D0 + 9, D10 = D0 + 10,
    D11 = D0 + 11, D12 = D0 + 12, D13 = D0 + 13, D14 = D0 + 14, D15 = D0 + 15,

    Q0 = {80}, Q1 = {81}, Q2 = {82}, Q3 = {83}, Q4 = {84}, Q5 = {85}, Q6 = {86},
    Q7 = {87}, Q8 = {88}, Q9 = {89}, Q10 = {90}, Q11 = {91}, Q12 = {92},
    Q13 = {93}, Q14 = {94}, Q15 = {95},

    FirstDFloatReg = D16, FirstQFloatReg = Q0, FirstSFloatReg = S0,
...
// The saved FP register mask should include  S16-31, aliased to Q4-Q7 and
// D8-D15. Unfortunately, our current implementation of callee-saved registers
// cannot handle floating-point registers, so we simply restrict the set of
// allocatable FP registers to those that need not be saved.

static const RegisterMask SavedFpRegs = 0;
static const RegisterMask SavedRegs =
    1 << R4 | 1 << R5 | 1 << R6 | 1 << R7 | 1 << R8 | 1 << R9 | 1 << R10;
static const int NumSavedRegs = 7;

// Some VFP CPUs (non-NEON) lack the full complement of 32 FP registers,
// implementing only S0-S15. Since we do not dynamically configure for the ARM
// variant in use, we must compile for the lowest common denominator among
// supported targets, currently VFP-D16 (Tegra-2).
// FIXME: We should identify the architectural variant at runtime. See bugs
// 718811 and 704111.
...
inline bool IsFpSReg(Register _r) {
  return _r >= FirstSFloatReg && _r < FirstDFloatReg;
}
inline bool IsFpDReg(Register _r) {
  return _r >= FirstDFloatReg && _r < FirstQFloatReg;
}
...
#define FpDRegNum(_fpr)                                                        \
  ((REGNUM(_fpr) - FirstDFloatReg) ^ 16) // mind the reversed order of the banks
#define FpQRegNum(_fpr) (REGNUM(_fpr) - FirstQFloatReg)
#define SReg(num) (FirstSFloatReg + (num))
#define DReg(num)                                                              \
  (FirstDFloatReg +                                                            \
   ((num) ^ 16)) // mind the reversed order of the register banks
...
#define BX(_r)                                                                 \
  do {                                                                         \
    ...
    *(--_nIns) =                                                               \
        (NIns)(COND_AL | (0x12 << 20) | (0xFFF << 8) | (1 << 4) | (_r));       \
    asm_output("bx %s", gpn(_r));                                              \
  } while (0)
...
//  op2imm  - operand 2 immediate. Use encOp2Imm (from NativeARM.cpp) to
//  calculate this.
#define ALUi(cond, op, S, rd, rl, op2imm)                                      \
  ALUi_chk(cond, op, S, rd, rl, op2imm, 1)
#define ALUi_chk(cond, op, S, rd, rl, op2imm, chk)                             \
  do {                                                                         \
    if (chk)                                                                   \
      underrunProtect(4);                                                      \
    ...
    *(--_nIns) = (NIns)((cond) << 28 | OP_IMM | (ARM_##op) << 21 | (S) << 20 | \
                        (rl) << 16 | (rd) << 12 | (op2imm));                   \
    if (ARM_##op == ARM_mov || ARM_##op == ARM_mvn) {                          \
      asm_output("%s%s%s %s, #0x%X", #op, condNames[cond], (S) ? "s" : "",     \
                 gpn(rd), decOp2Imm(op2imm));                                  \
    } else if (ARM_##op >= ARM_tst && ARM_##op <= ARM_cmn) {                   \
      NanoAssert(S == 1);                                                      \
      asm_output("%s%s %s, #0x%X", #op, condNames[cond], gpn(rl),              \
                 decOp2Imm(op2imm));                                           \
    } else {                                                                   \
      asm_output("%s%s%s %s, %s, #0x%X", #op, condNames[cond], (S) ? "s" : "", \
                 gpn(rd), gpn(rl), decOp2Imm(op2imm));                         \
    }                                                                          \
  } while (0)
...
#define ALUr_chk(cond, op, S, rd, rl, rr, chk)                                 \
  do {                                                                         \
    if (chk)                                                                   \
      underrunProtect(4);                                                      \
    NanoAssert(IsCond(cond));                                                  \
    ...
    *(--_nIns) = (NIns)((cond) << 28 | (ARM_##op) << 21 | (S) << 20 |          \
                        (rl) << 16 | (rd) << 12 | (rr));                       \
    if (ARM_##op == ARM_mov || ARM_##op == ARM_mvn) {                          \
      asm_output("%s%s%s %s, %s", #op, condNames[cond], (S) ? "s" : "",        \
                 gpn(rd), gpn(rr));                                            \
    } else if (ARM_##op >= ARM_tst && ARM_##op <= ARM_cmn) {                   \
      NanoAssert(S == 1);                                                      \
      asm_output("%s%s  %s, %s", #op, condNames[cond], gpn(rl), gpn(rr));      \
    } else {                                                                   \
      asm_output("%s%s%s %s, %s, %s", #op, condNames[cond], (S) ? "s" : "",    \
                 gpn(rd), gpn(rl), gpn(rr));                                   \
    }                                                                          \
  } while (0)

// ALU operation with two register arguments, with rr operated on by a shift and
// shift immediate
//  S   - bit, 0 or 1, whether the CPSR register is updated
//  rd  - destination register
//  rl  - first (left) operand register
...
#define ALUr_shi(cond, op, S, rd, rl, rr, sh, imm)                             \
  do {                                                                         \
    underrunProtect(4);                                                        \
    NanoAssert(IsCond(cond));                                                  \
    NanoAssert(IsOp(op));                                                      \
    ...
    *(--_nIns) =                                                               \
        (NIns)((cond) << 28 | (ARM_##op) << 21 | (S) << 20 | (rl) << 16 |      \
               (rd) << 12 | (imm) << 7 | (sh) << 4 | (rr));                    \
    if (ARM_##op == ARM_mov || ARM_##op == ARM_mvn) {                          \
      NanoAssert(rl == 0);                                                     \
      asm_output("%s%s%s %s, %s, %s #%d", #op, condNames[cond],                \
                 (S) ? "s" : "", gpn(rd), gpn(rr), shiftNames[sh], (imm));     \
    } else if (ARM_##op >= ARM_tst && ARM_##op <= ARM_cmn) {                   \
      NanoAssert(S == 1);                                                      \
      NanoAssert(rd == 0);                                                     \
      asm_output("%s%s  %s, %s, %s #%d", #op, condNames[cond], gpn(rl),        \
                 gpn(rr), shiftNames[sh], (imm));                              \
    } else {                                                                   \
      asm_output("%s%s%s %s, %s, %s, %s #%d", #op, condNames[cond],            \
                 (S) ? "s" : "", gpn(rd), gpn(rl), gpn(rr), shiftNames[sh],    \
                 (imm));                                                       \
    }                                                                          \
  } while (0)
...
#define ALUr_shr(cond, op, S, rd, rl, rr, sh, rs)                              \
  do {                                                                         \
    underrunProtect(4);                                                        \
    NanoAssert(IsCond(cond));                                                  \
    NanoAssert(IsOp(op));                                                      \
    ...
    *(--_nIns) =                                                               \
        (NIns)((cond) << 28 | (ARM_##op) << 21 | (S) << 20 | (rl) << 16 |      \
               (rd) << 12 | (rs) << 8 | (sh) << 4 | (rr));                     \
    if (ARM_##op == ARM_mov || ARM_##op == ARM_mvn) {                          \
      asm_output("%s%s%s %s, %s, %s %s", #op, condNames[cond], (S) ? "s" : "", \
                 gpn(rd), gpn(rr), shiftNames[sh], gpn(rs));                   \
    } else if (ARM_##op >= ARM_tst && ARM_##op <= ARM_cmn) {                   \
      NanoAssert(S == 1);                                                      \
      asm_output("%s%s  %s, %s, %s %s", #op, condNames[cond], gpn(rl),         \
                 gpn(rr), shiftNames[sh], gpn(rs));                            \
    } else {                                                                   \
      asm_output("%s%s%s %s, %s, %s, %s %s", #op, condNames[cond],             \
                 (S) ? "s" : "", gpn(rd), gpn(rl), gpn(rr), shiftNames[sh],    \
                 gpn(rs));                                                     \
    }                                                                          \
  } while (0)
...
#define SMULL(_d, _d_hi, _l, _r)                                               \
  do {                                                                         \
    ...
    NanoAssert(((_d) != PC) && ((_d_hi) != PC) && ((_l) != PC) &&              \
               ((_r) != PC));                                                  \
    *(--_nIns) = (NIns)(COND_AL | 0xc00090 | (_d_hi) << 16 | (_d) << 12 |      \
                        (_r) << 8 | (_l));                                     \
...
#define MVNis_chk(_d, _op2imm, _stat, _chk)                                    \
  ALUi_chk(AL, mvn, _stat, _d, 0, op2imm, _chk)
...
#define LSRi(_d, _r, _imm)                                                     \
  ALUr_shi(AL, mov, 1, _d, 0, _r, LSR_imm, (_imm & 0x1f))
...
// _d = _r >> (_imm & 0x1f)
#define ASRi(_d, _r, _imm)                                                     \
  ALUr_shi(AL, mov, 1, _d, 0, _r, ASR_imm, (_imm & 0x1f))
...
// _d = _r << (_imm & 0x1f)
#define LSLi(_d, _r, _imm)                                                     \
  ALUr_shi(AL, mov, 1, _d, 0, _r, LSL_imm, (_imm & 0x1f))
...
// MOV
#define MOVis_chk(_d, _op2imm, _stat, _chk)                                    \
  ALUi_chk(AL, mov, _stat, _d, 0, op2imm, _chk)
...
// _d = [_b + _x<<_s]
#define LDR_scaled(_d, _b, _x, _s)                                             \
  do {                                                                         \
    NanoAssert(((_s)&31) == _s);                                               \
    NanoAssert(IsGpReg(_d) && IsGpReg(_b) && IsGpReg(_x));                     \
    underrunProtect(4);                                                        \
    *(--_nIns) = (NIns)(COND_AL | (0x79 << 20) | ((_b) << 16) | ((_d) << 12) | \
                        ((_s) << 7) | (_x));                                   \
...
#define MOVWi_cond_chk(_cond, _d, _imm, _chk)                                  \
  do {                                                                         \
    ...
    if (_chk)                                                                  \
      underrunProtect(4);                                                      \
    *(--_nIns) =                                                               \
        (NIns)((_cond) << 28 | 3 << 24 | 0 << 20 |                             \
               (((_imm) >> 12) & 0xf) << 16 | (_d) << 12 | ((_imm)&0xfff));    \
...
#define MOVTi_cond_chk(_cond, _d, _imm, _chk)                                  \
  do {                                                                         \
    NanoAssert(isU16(_imm));                                                   \
    NanoAssert(IsGpReg(_d));                                                   \
    NanoAssert(IsCond(_cond));                                                 \
...
#define LDRB(_d, _n, _off)                                                     \
  do {                                                                         \
    NanoAssert(IsGpReg(_d) && IsGpReg(_n));                                    \
    underrunProtect(4);                                                        \
    if (_off < 0) {                                                            \
      NanoAssert(isU12(-(_off)));                                              \
      *(--_nIns) = (NIns)(COND_AL | (0x55 << 20) | ((_n) << 16) |              \
                          ((_d) << 12) | ((-(_off)) & 0xfff));                 \
    } else {                                                                   \
      NanoAssert(isU12(_off));                                                 \
      *(--_nIns) = (NIns)(COND_AL | (0x5D << 20) | ((_n) << 16) |              \
                          ((_d) << 12) | ((_off)&0xfff));                      \
    }                                                                          \
...
#define LDRSB(_d, _n, _off)                                                    \
  do {                                                                         \
    NanoAssert(IsGpReg(_d) && IsGpReg(_n));                                    \
    underrunProtect(4);                                                        \
    if (_off < 0) {                                                            \
      NanoAssert(isU8(-(_off)));                                               \
      *(--_nIns) = (NIns)(COND_AL | (0x15 << 20) | ((_n) << 16) |              \
                          ((_d) << 12) | ((0xD) << 4) |                        \
                          (((-(_off)) & 0xf0) << 4) | ((-(_off)) & 0xf));      \
    } else {                                                                   \
      NanoAssert(isU8(_off));                                                  \
      *(--_nIns) =                                                             \
          (NIns)(COND_AL | (0x1D << 20) | ((_n) << 16) | ((_d) << 12) |        \
                 ((0xD) << 4) | (((_off)&0xf0) << 4) | ((_off)&0xf));          \
    }                                                                          \
...
#define STRH(_d, _n, _off)                                                     \
  do {                                                                         \
    ...
    if ((_off) < 0) {                                                          \
      NanoAssert(isU8(-(_off)));                                               \
      *(--_nIns) =                                                             \
          (NIns)(COND_AL | (0x14 << 20) | ((_n) << 16) | ((_d) << 12) |        \
                 (((-(_off)) & 0xF0) << 4) | (0xB << 4) | ((-(_off)) & 0xF));  \
    } else {                                                                   \
      NanoAssert(isU8(_off));                                                  \
      *(--_nIns) =                                                             \
          (NIns)(COND_AL | (0x1C << 20) | ((_n) << 16) | ((_d) << 12) |        \
                 (((_off)&0xF0) << 4) | (0xB << 4) | ((_off)&0xF));            \
    }                                                                          \
...
#define PUSHr(_r)                                                              \
  do {                                                                         \
    ...
    *(--_nIns) = (NIns)(COND_AL | (0x92 << 20) | (SP << 16) | rmask(_r));      \
    asm_output("push %s", gpn(_r));                                            \
  } while (0)

// STMFD SP!,{reglist}
#define PUSH_mask(_mask)                                                       \
  do {                                                                         \
    underrunProtect(4);                                                        \
    NanoAssert(isU16(_mask));                                                  \
    *(--_nIns) = (NIns)(COND_AL | (0x92 << 20) | (SP << 16) | (_mask));        \
    asm_output("push %x", (_mask));                                            \
  } while (0)
...
#define POPr(_r)                                                               \
  do {                                                                         \
    ...
    *(--_nIns) = (NIns)(COND_AL | (0x8B << 20) | (SP << 16) | rmask(_r));      \
    asm_output("pop %s", gpn(_r));                                             \
  } while (0)

// LDMFD SP!,{reglist}
#define POP_mask(_mask)                                                        \
  do {                                                                         \
    underrunProtect(4);                                                        \
    NanoAssert(isU16(_mask));                                                  \
    *(--_nIns) = (NIns)(COND_AL | (0x8B << 20) | (SP << 16) | (_mask));        \
    asm_output("pop %x", (_mask));                                             \
  } while (0)

// PC always points to current instruction + 8, so when calculating pc-relative
// offsets, use PC+8.
#define PC_OFFSET_FROM(target, frompc)                                         \
  ((intptr_t)(target) - ((intptr_t)(frompc) + 8))

#define B_cond(_c, _t) B_cond_chk(_c, _t, 1)

#define B_nochk(_t) B_cond_chk(AL, _t, 0)
...
#define MOVSX(_d, _off, _b)                                                    \
  do {                                                                         \
    if ((_off) >= 0) {                                                         \
      if ((_off) < 256) {                                                      \
        underrunProtect(4);                                                    \
        *(--_nIns) =                                                           \
            (NIns)(COND_AL | (0x1D << 20) | ((_b) << 16) | ((_d) << 12) |      \
                   ((((_off) >> 4) & 0xF) << 8) | (0xF << 4) | ((_off)&0xF));  \
      } else if ((_off) <= 510) {                                              \
        underrunProtect(8);                                                    \
        int rem = (_off)-255;                                                  \
        NanoAssert(rem < 256);                                                 \
        *(--_nIns) =                                                           \
            (NIns)(COND_AL | (0x1D << 20) | ((_d) << 16) | ((_d) << 12) |      \
                   ((((rem) >> 4) & 0xF) << 8) | (0xF << 4) | ((rem)&0xF));    \
        *(--_nIns) = (NIns)(COND_AL | OP_IMM | (1 << 23) | ((_b) << 16) |      \
                            ((_d) << 12) | (0xFF));                            \
      } else {                                                                 \
        underrunProtect(16);                                                   \
        int rem = (_off)&3;                                                    \
        *(--_nIns) = (NIns)(COND_AL | (0x19 << 20) | ((_b) << 16) |            \
                            ((_d) << 12) | (0xF << 4) | (_d));                 \
        asm_output("ldrsh %s,[%s, #%d]", gpn(_d), gpn(_b), (_off));            \
        *(--_nIns) = (NIns)(COND_AL | OP_IMM | (1 << 23) | ((_d) << 16) |      \
                            ((_d) << 12) | rem);                               \
        *(--_nIns) =                                                           \
            (NIns)(COND_AL | (0x1A << 20) | ((_d) << 12) | (2 << 7) | (_d));   \
        *(--_nIns) = (NIns)(COND_AL | (0x3B << 20) | ((_d) << 12) |            \
                            (((_off) >> 2) & 0xFF));                           \
      ...
      if ((_off) > -256) {                                                     \
        underrunProtect(4);                                                    \
        *(--_nIns) = (NIns)(COND_AL | (0x15 << 20) | ((_b) << 16) |            \
                            ((_d) << 12) | ((((-(_off)) >> 4) & 0xF) << 8) |   \
                            (0xF << 4) | ((-(_off)) & 0xF));                   \
        asm_output("ldrsh %s,[%s, #%d]", gpn(_d), gpn(_b), (_off));            \
      } else if ((_off) >= -510) {                                             \
        ...
        NanoAssert(rem < 256);                                                 \
        *(--_nIns) =                                                           \
            (NIns)(COND_AL | (0x15 << 20) | ((_d) << 16) | ((_d) << 12) |      \
                   ((((rem) >> 4) & 0xF) << 8) | (0xF << 4) | ((rem)&0xF));    \
        *(--_nIns) = (NIns)(COND_AL | OP_IMM | (1 << 22) | ((_b) << 16) |      \
                            ((_d) << 12) | (0xFF));                            \
      } else                                                                   \
        NanoAssert(0);                                                         \
    }                                                                          \
...
#define split_qreg_enc(_Qd, D, Vd)                                             \
  do {                                                                         \
    Vd = FpQRegNum(_Qd);                                                       \
    NanoAssert(Vd <= 15);                                                      \
    Vd *= 2;                                                                   \
...
#define split_dreg_enc(_Dd, D, Vd)                                             \
  do {                                                                         \
    Vd = FpDRegNum(_Dd);                                                       \
    NanoAssert(Vd <= 31);                                                      \
    D = (Vd & 0x10) >> 4;                                                      \
...
#define split_sreg_enc(_Sd, D, Vd)                                             \
  do {                                                                         \
    Vd = FpSRegNum(_Sd);                                                       \
    NanoAssert(Vd <= 31);                                                      \
    D = (Vd & 0x1);                                                            \
...
#define FMDRR(_Dm, _Rd, _Rn)                                                   \
  do {                                                                         \
    underrunProtect(4);                                                        \
    NanoAssert(ARM_VFP);                                                       \
    NanoAssert(IsGpReg(_Rd) && IsGpReg(_Rn));                                  \
    int DmEnc;                                                                 \
    encodeRegM(_Dm, false, true, false);                                       \
    *(--_nIns) = (NIns)(COND_AL | (0xC4 << 20) | ((_Rn) << 16) |               \
                        ((_Rd) << 12) | (0xB1 << 4) | DmEnc);                  \
    asm_output("fmdrr %s,%s,%s", gpn(_Dm), gpn(_Rd), gpn(_Rn));                \
  } while (0)

#define FMRRD(_Rd, _Rn, _Dm)                                                   \
  do {                                                                         \
    underrunProtect(4);                                                        \
    NanoAssert(ARM_VFP);                                                       \
    NanoAssert(IsGpReg(_Rd) && IsGpReg(_Rn));                                  \
    int DmEnc;                                                                 \
    encodeRegM(_Dm, false, true, false);                                       \
    *(--_nIns) = (NIns)(COND_AL | (0xC5 << 20) | ((_Rn) << 16) |               \
                        ((_Rd) << 12) | (0xB1 << 4) | DmEnc);                  \
    asm_output("fmrrd %s,%s,%s", gpn(_Rd), gpn(_Rn), gpn(_Dm));                \
  } while (0)

#define FMRDH(_Rd, _Dn)                                                        \
  do {                                                                         \
    underrunProtect(4);                                                        \
    NanoAssert(ARM_VFP);                                                       \
    NanoAssert(IsGpReg(_Rd));                                                  \
    int DnEnc;                                                                 \
    encodeRegN(_Dn, false, true, false);                                       \
    *(--_nIns) =                                                               \
        (NIns)(COND_AL | (0xE3 << 20) | DnEnc | ((_Rd) << 12) | 0xB10);        \
    asm_output("fmrdh %s,%s", gpn(_Rd), gpn(_Dn));                             \
  } while (0)

#define FMRDL(_Rd, _Dn)                                                        \
  do {                                                                         \
    underrunProtect(4);                                                        \
    NanoAssert(ARM_VFP);                                                       \
    NanoAssert(IsGpReg(_Rd));                                                  \
    int DnEnc;                                                                 \
    encodeRegN(_Dn, DnEnc, false, true, false);                                \
    *(--_nIns) =                                                               \
        (NIns)(COND_AL | (0xE1 << 20) | DnEnc | ((_Rd) << 12) | 0xB10);        \
    asm_output("fmrdh %s,%s", gpn(_Rd), gpn(_Dn));                             \
  } while (0)

#define VLDR_chk(_Dd, _Rn, _offs, _chk)                                        \
  do {                                                                         \
    if (_chk)                                                                  \
      underrunProtect(4);                                                      \
    NanoAssert(ARM_VFP);                                                       \
    ...
    int dflag = DFlag(_Dd);                                                    \
    int DdEnc;                                                                 \
    encodeRegD(_Dd, true, true, false);                                        \
    intptr_t offs = (_offs);                                                   \
    if (_offs < 0) {                                                           \
      negflag = 0 << 23;                                                       \
      offs = -(offs);                                                          \
    }                                                                          \
    *(--_nIns) = (NIns)(COND_AL | (0xD1 << 20) | ((_Rn) << 16) | DdEnc |       \
                        (0xA << 8) | dflag | negflag | ((offs >> 2) & 0xff));  \
    asm_output("fld%c %s,%s(%d)", dflag ? 'd' : 's', gpn(_Dd), gpn(_Rn),       \
               _offs);                                                         \
  } while (0)

#define VLDQR(_Qd, _Rn)                                                        \
  do {                                                                         \
    underrunProtect(4);                                                        \
    NanoAssert(ARM_VFP);                                                       \
    NanoAssert(IsGpReg(_Rn));                                                  \
    int DdEnc;                                                                 \
    encodeRegD(_Qd, false, false, true);                                       \
    *(--_nIns) =                                                               \
        (NIns)(COND_AL | (0xC9 << 20) | ((_Rn) << 16) | DdEnc | 0xB04);        \
    asm_output("vldm <d%d,d%d>,%s // i.e. %s", Vd + D * 16, Vd + D * 16 + 1,   \
               gpn(_Rn), gpn(_Qd));                                            \
  } while (0)
...
#define VSTR(_Dd, _Rn, _offs)                                                  \
  do {                                                                         \
    ...
    *(--_nIns) = (NIns)(COND_AL | (0xD0 << 20) | ((_Rn) << 16) | (DdEnc) |     \
                        (0xA << 8) | dflag | negflag | ((offs >> 2) & 0xff));  \
    asm_output("fst%c %s,%s(%d)", dflag ? 'd' : 's', gpn(_Dd), gpn(_Rn),       \
               _offs);                                                         \
  } while (0)

#define VSTQR(_Qd, _Rn)                                                        \
  do {                                                                         \
    underrunProtect(4);                                                        \
    NanoAssert(ARM_VFP);                                                       \
    NanoAssert(IsGpReg(_Rn));                                                  \
    ...
    encodeRegD(_Qd, false, false, true);                                       \
    *(--_nIns) =                                                               \
        (NIns)(COND_AL | (0xC8 << 20) | ((_Rn) << 16) | DdEnc | 0xB04);        \
    asm_output("vstm %s,<d%d,d%d> // i.e. %s", gpn(_Rn), Vd + D * 16,          \
               Vd + D * 16 + 1, gpn(_Qd));                                     \
  } while (0)
#define FSTS VSTR
#define FSTD VSTR

#define VNEGvfp(_Dd, _Dm)                                                      \
  do {                                                                         \
    underrunProtect(4);                                                        \
    NanoAssert(ARM_VFP);                                                       \
    int dflag = DFlag(_Dd);                                                    \
    NanoAssert(dflag == DFlag(_Dm));                                           \
    int DdEnc;                                                                 \
    encodeRegD(_Dd, true, true, false);                                        \
    int DmEnc;                                                                 \
    encodeRegM(_Dm, true, true, false);                                        \
    *(--_nIns) = (NIns)(COND_AL | (0xEB10A40) | DdEnc | DmEnc | dflag);        \
    asm_output("fneg%c %s,%s", dflag ? 'd' : 's', gpn(_Dd), gpn(_Dm));         \
  } while (0)

#define VNEGneon(_Qd, _Qm)                                                     \
  do {                                                                         \
    underrunProtect(4);                                                        \
    NanoAssert(ARM_VFP);                                                       \
    int DdEnc;                                                                 \
    encodeRegD(_Qd, false, false, true);                                       \
    int DmEnc;                                                                 \
    encodeRegM(_Qm, false, false, true);                                       \
    *(--_nIns) = (NIns)(0xF3B907C0 | DdEnc | DmEnc);                           \
    asm_output("fnegq %s,%s", gpn(_Qd), gpn(_Qm));                             \
  } while (0)

#define GenericVfp(NAME, _Dd, _Dn, _Dm, SPECIFIC_ENC)                          \
  do {                                                                         \
    underrunProtect(4);                                                        \
    NanoAssert(ARM_VFP);                                                       \
    int dflag = DFlag(_Dd);                                                    \
    NanoAssert(dflag == DFlag(_Dm));                                           \
    NanoAssert(dflag == DFlag(_Dn));                                           \
    int DdEnc;                                                                 \
    encodeRegD(_Dd, true, true, false);                                        \
    int DmEnc;                                                                 \
    encodeRegM(_Dm, true, true, false);                                        \
    int DnEnc;                                                                 \
    encodeRegN(_Dn, true, true, false);                                        \
    *(--_nIns) =                                                               \
        (NIns)(COND_AL | SPECIFIC_ENC | DnEnc | DdEnc | DmEnc | dflag);        \
    asm_output(#NAME "%c %s,%s,%s", dflag ? 'd' : 's', gpn(_Dd), gpn(_Dn),     \
               gpn(_Dm));                                                      \
  } while (0)

#define GenericNeon(NAME, _Qd, _Qn, _Qm, SPECIFIC_ENC)                         \
  do {                                                                         \
    underrunProtect(4);                                                        \
    NanoAssert(ARM_VFP);                                                       \
    int DdEnc;                                                                 \
    ...
    encodeRegM(_Qm, false, false, true);                                       \
    int DnEnc;                                                                 \
    encodeRegN(_Qn, false, false, true);                                       \
    *(--_nIns) =                                                               \
        (NIns)((0xF2 << 24) | (SPECIFIC_ENC) | DdEnc | DnEnc | DmEnc);         \
...
#define VADD(_Dd, _Dn, _Dm)                                                    \
  if (IsFpQReg(_Dd))                                                           \
    VADDneon(_Dd, _Dn, _Dm);                                                   \
  else                                                                         \
    VADDvfp(_Dd, _Dn, _Dm);
#define VSUB(_Dd, _Dn, _Dm)                                                    \
  if (IsFpQReg(_Dd))                                                           \
    VSUBneon(_Dd, _Dn, _Dm);                                                   \
  else                                                                         \
    VSUBvfp(_Dd, _Dn, _Dm);
#define VMUL(_Dd, _Dn, _Dm)                                                    \
  if (IsFpQReg(_Dd))                                                           \
    VMULneon(_Dd, _Dn, _Dm);                                                   \
  else                                                                         \
    VMULvfp(_Dd, _Dn, _Dm);
#define VDIV(_Dd, _Dn, _Dm)                                                    \
  if (IsFpQReg(_Dd))                                                           \
    VDIVneon(_Dd, _Dn, _Dm);                                                   \
  else                                                                         \
    VDIVvfp(_Dd, _Dn, _Dm);
#define VNEG(_Dd, _Dm)                                                         \
  if (IsFpQReg(_Dd))                                                           \
...
#define VMOVN(_Dd, _Qm)                                                        \
  do {                                                                         \
    underrunProtect(4);                                                        \
    NanoAssert(ARM_VFP);                                                       \
    int DdEnc;                                                                 \
    encodeRegD(_Dd, false, true, false);                                       \
    int DmEnc;                                                                 \
    encodeRegM(_Qm, false, false, true);                                       \
...
#define VCMP(_Dd, _Dm, _E)                                                     \
  do {                                                                         \
    ...
    *(--_nIns) = (NIns)(COND_AL | (0xEB4 << 16) | DdEnc | 0xA40 |              \
                        ((_E) << 7) | dflag | DmEnc);                          \
    asm_output("fcmp%s%c %s,%s", (((_E) == 1) ? "e" : ""), dflag ? 'd' : 's',  \
               gpn(_Dd), gpn(_Dm));                                            \
  } while (0)
...
#define VMOV_cond(_cond, _Dd, _Dm)                                             \
  do {                                                                         \
    underrunProtect(4);                                                        \
    NanoAssert(ARM_VFP);                                                       \
    NanoAssert(IsCond(_cond));                                                 \
    ...
    int DmEnc;                                                                 \
    encodeRegM(_Dm, true, true, false);                                        \
    *(--_nIns) = (NIns)(((_cond) << 28) | (0xEB0 << 16) | DdEnc | 0xA40 |      \
                        DmEnc | dflag);                                        \
    asm_output("fcpy%c%s %s,%s", dflag ? 'd' : 's', condNames[_cond],          \
               gpn(_Dd), gpn(_Dm));                                            \
  } while (0)
...
#define FCPYQ(_Qd, _Qm)                                                        \
  do {                                                                         \
    ...
    encodeRegM(_Qm, false, false, true);                                       \
    int DnEnc;                                                                 \
    encodeRegN(_Qm, false, false, true);                                       \
...
#define FMSR(_Sn, _Rd)                                                         \
  do {                                                                         \
    ...
    NanoAssert(IsGpReg(_Rd));                                                  \
    int DnEnc;                                                                 \
    encodeRegN(_Sn, true, false, false);                                       \
    *(--_nIns) =                                                               \
        (NIns)(COND_AL | (0xE0 << 20) | ((_Rd) << 12) | 0xA10 | DnEnc);        \
...
#define FMRS(_Rd, _Sn)                                                         \
  do {                                                                         \
    ...
    encodeRegN(_Sn, true, false, false);                                       \
    *(--_nIns) =                                                               \
        (NIns)(COND_AL | (0xE1 << 20) | ((_Rd) << 12) | 0xA10 | DnEnc);        \
...
#define VCVTx(NAME, _Rd, _Rm, SPECIFIC_ENC, check_dst)                         \
  do {                                                                         \
    underrunProtect(4);                                                        \
    NanoAssert(ARM_VFP);                                                       \
    int dflag = DFlag((check_dst ? _Rd : _Rm));                                \
    int DdEnc;                                                                 \
    encodeRegD(_Rd, true, true, false);                                        \
    int DmEnc;                                                                 \
    encodeRegM(_Rm, true, true, false);                                        \
    *(--_nIns) =                                                               \
        (NIns)(COND_AL | (0xEB << 20) | SPECIFIC_ENC | dflag | DdEnc | DmEnc); \
