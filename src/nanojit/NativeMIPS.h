...
typedef uint64_t
    RegisterMask; // REQ: Large enough to hold LastRegNum-FirstRegNum bits
#define _rmask_(r) (1LL << (r))
#define REGMASK(r) _rmask_(REGNUM(r))

// Register numbers for Native code generator
static const Register ZERO = {0}, AT = {1}, V0 = {2}, V1 = {3}, A0 = {4},
                      A1 = {5}, A2 = {6}, A3 = {7},
                      ...
                      F0 = {32}, F1 = {33}, F2 = {34}, F3 = {35}, F4 = {36},
                      F5 = {37}, F6 = {38}, F7 = {39},

                      F8 = {40}, F9 = {41}, F10 = {42}, F11 = {43}, F12 = {44},
                      F13 = {45}, F14 = {46}, F15 = {47},
                      ...
                      // FP register aliases
    FV0 = F0, FV1 = F2, FA0 = F12, FA1 = F14, FT0 = F4, FT1 = F6, FT2 = F8,
                      FT3 = F10, FT4 = F16, FT5 = F18, FS0 = F20, FS1 = F22,
                      FS2 = F24, FS3 = F26, FS4 = F28, FS5 = F30,

                      UnspecifiedReg = {127},
                      deprecated_UnknownReg = {
                          127}; // XXX: remove eventually, see bug 538924
...
#ifdef FPCALLEESAVED
    REGMASK(FS0) | REGMASK(FS1) | REGMASK(FS2) | REGMASK(FS3) | REGMASK(FS4) |
    REGMASK(FS5) |
#endif
    REGMASK(S0) | REGMASK(S1) | REGMASK(S2) | REGMASK(S3) | REGMASK(S4) |
    REGMASK(S5) | REGMASK(S6) | REGMASK(S7);

// REQ: General purpose registers
static const RegisterMask GpRegs =
    REGMASK(V0) | REGMASK(V1) | REGMASK(A0) | REGMASK(A1) | REGMASK(A2) |
    REGMASK(A3) | REGMASK(S0) | REGMASK(S1) | REGMASK(S2) | REGMASK(S3) |
    REGMASK(S4) | REGMASK(S5) | REGMASK(S6) | REGMASK(S7) | REGMASK(T0) |
    REGMASK(T1) | REGMASK(T2) | REGMASK(T3) | REGMASK(T4) | REGMASK(T5) |
    REGMASK(T6) | REGMASK(T7) | REGMASK(T8) | REGMASK(T9);
...
static const RegisterMask FpRegs =
    ...
    REGMASK(FS5) |
#endif
    REGMASK(FV0) | REGMASK(FV1) | REGMASK(FA0) | REGMASK(FA1) | REGMASK(FT0) |
    REGMASK(FT1) | REGMASK(FT2) | REGMASK(FT3) | REGMASK(FT4) | REGMASK(FT5);

#define FpDRegs FpRegs
#define FpSRegs 0x0 // not implemented
#define FpQRegs 0x0 // not implemented

static inline bool IsGpReg(Register r) { return (REGMASK(r) & GpRegs) != 0; }

static inline bool IsFpReg(Register r) { return (REGMASK(r) & FpRegs) != 0; }

static inline bool IsGPR(Register r) { return (r >= ZERO && r <= RA); }

static inline bool IsFPR(Register r) { return (r >= F0 && r <= F31); }
...
#define firstAvailableReg(i, c, m)                                             \
  nRegisterAllocFromSet(m) // custom "firstAvailableReg" implementation
...
#define TODO(x)                                                                \
  do {                                                                         \
    verbose_only(avmplus::AvmLog(#x);) NanoAssertMsgf(false, "%s", #x);        \
  } while (0)
#ifdef MIPSDEBUG
#define TAG(fmt, ...)                                                          \
  do {                                                                         \
    debug_only(verbose_outputf("             # MIPS: " fmt, ##__VA_ARGS__);)   \
  } while (0)
#else
#define TAG(fmt, ...)                                                          \
  do {                                                                         \
...
#define EMIT(ins, fmt, ...)                                                    \
  do {                                                                         \
    underrunProtect(4);                                                        \
    *(--_nIns) = (NIns)(ins);                                                  \
    debug_only(codegenBreak(_nIns);) asm_output(fmt, ##__VA_ARGS__);           \
  } while (0)

// Emit code in trampoline/literal area
// Assume that underrunProtect has already been called
// This is a bit hacky...
#define TRAMP(ins, fmt, ...)                                                   \
  do {                                                                         \
    verbose_only(NIns *save_nIns = _nIns; _nIns = _nSlot;                      \
                 NIns *save_nInsAfter = _nInsAfter;                            \
                 _nInsAfter = _nSlot + 1;) *_nSlot = (NIns)ins;                \
    debug_only(codegenBreak(_nSlot);) _nSlot++;                                \
    verbose_only(setOutputForEOL("<= trampoline");)                            \
        asm_output(fmt, ##__VA_ARGS__);                                        \
    verbose_only(_nIns = save_nIns; _nInsAfter = save_nInsAfter;)              \
  } while (0)

#define MR(d, s) asm_move(d, s)

// underrun guarantees that there is always room to insert a jump and branch
// delay slot
...
#define LDSTGPR(op, rt, offset, base)                                          \
  do {                                                                         \
    count_misc();                                                              \
    EMIT(I_FORMAT(op, GPR(base), GPR(rt), offset), "%s %s, %d(%s)", oname[op], \
         gpn(rt), offset, gpn(base));                                          \
  } while (0)

#define LDSTFPR(op, ft, offset, base)                                          \
  do {                                                                         \
    count_misc();                                                              \
    EMIT(I_FORMAT(op, GPR(base), FPR(ft), offset), "%s %s, %d(%s)", oname[op], \
         fpn(ft), offset, gpn(base));                                          \
  } while (0)
...
#define ADDIU(rt, rs, simm)                                                    \
  do {                                                                         \
    count_alu();                                                               \
    EMIT(I_FORMAT(OP_ADDIU, GPR(rs), GPR(rt), simm), "addiu %s, %s, %d",       \
         gpn(rt), gpn(rs), simm);                                              \
  } while (0)
...
#define trampADDIU(rt, rs, simm)                                               \
  do {                                                                         \
    count_alu();                                                               \
    TRAMP(I_FORMAT(OP_ADDIU, GPR(rs), GPR(rt), simm), "addiu %s, %s, %d",      \
          gpn(rt), gpn(rs), simm);                                             \
  } while (0)
...
#define ADDU(rd, rs, rt)                                                       \
  do {                                                                         \
    count_alu();                                                               \
    EMIT(R_FORMAT(OP_SPECIAL, GPR(rs), GPR(rt), GPR(rd), 0, SPECIAL_ADDU),     \
         "addu %s, %s, %s", gpn(rd), gpn(rs), gpn(rt));                        \
  } while (0)

#define AND(rd, rs, rt)                                                        \
  do {                                                                         \
    count_alu();                                                               \
    EMIT(R_FORMAT(OP_SPECIAL, GPR(rs), GPR(rt), GPR(rd), 0, SPECIAL_AND),      \
         "and %s, %s, %s", gpn(rd), gpn(rs), gpn(rt));                         \
  } while (0)

#define ANDI(rt, rs, uimm)                                                     \
  do {                                                                         \
    count_alu();                                                               \
    EMIT(U_FORMAT(OP_ANDI, GPR(rs), GPR(rt), uimm), "andi %s, %s, 0x%x",       \
         gpn(rt), gpn(rs), ((uimm)&0xffff));                                   \
  } while (0)

#define BC1F(targ)                                                             \
  do {                                                                         \
    count_br();                                                                \
    EMIT(I_FORMAT(OP_COP1, COP1_BC, 0, BOFFSET(targ)), "bc1f %p", targ);       \
  } while (0)

#define BC1T(targ)                                                             \
  do {                                                                         \
    count_br();                                                                \
    EMIT(I_FORMAT(OP_COP1, COP1_BC, 1, BOFFSET(targ)), "bc1t %p", targ);       \
  } while (0)

#define B(targ)                                                                \
  do {                                                                         \
    count_br();                                                                \
    EMIT(I_FORMAT(OP_BEQ, GPR(ZERO), GPR(ZERO), BOFFSET(targ)), "b %p", targ); \
  } while (0)

#define BEQ(rs, rt, targ)                                                      \
  do {                                                                         \
    count_br();                                                                \
    EMIT(I_FORMAT(OP_BEQ, GPR(rs), GPR(rt), BOFFSET(targ)), "%s %s, %s, %p",   \
         oname[OP_BEQ], gpn(rs), gpn(rt), targ);                               \
  } while (0)

#define BNE(rs, rt, targ)                                                      \
  do {                                                                         \
    count_br();                                                                \
    EMIT(I_FORMAT(OP_BNE, GPR(rs), GPR(rt), BOFFSET(targ)), "%s %s, %s, %p",   \
         oname[OP_BNE], gpn(rs), gpn(rt), targ);                               \
  } while (0)

#define BLEZ(rs, targ)                                                         \
  do {                                                                         \
    count_br();                                                                \
    EMIT(I_FORMAT(OP_BLEZ, GPR(rs), 0, BOFFSET(targ)), "%s %s, %p",            \
         oname[OP_BLEZ], gpn(rs), targ);                                       \
  } while (0)

#define BGTZ(rs, targ)                                                         \
  do {                                                                         \
    count_br();                                                                \
    EMIT(I_FORMAT(OP_BGTZ, GPR(rs), 0, BOFFSET(targ)), "%s %s, %p",            \
         oname[OP_BGTZ], gpn(rs), targ);                                       \
  } while (0)

#define BGEZ(rs, targ)                                                         \
  do {                                                                         \
    count_br();                                                                \
    EMIT(I_FORMAT(OP_REGIMM, GPR(rs), REGIMM_BGEZ, BOFFSET(targ)),             \
         "bgez %s, %p", gpn(rs), targ);                                        \
  } while (0)

#define BLTZ(rs, targ)                                                         \
  do {                                                                         \
    count_br();                                                                \
    EMIT(I_FORMAT(OP_REGIMM, GPR(rs), REGIMM_BLTZ, BOFFSET(targ)),             \
         "bltz %s, %p", gpn(rs), targ);                                        \
  } while (0)
...
#define J(dest)                                                                \
  do {                                                                         \
    count_jmp();                                                               \
    EMIT(J_FORMAT(OP_J, JINDEX(dest)), "j %p", dest);                          \
  } while (0)

#define trampJ(dest)                                                           \
  do {                                                                         \
    count_jmp();                                                               \
    TRAMP(J_FORMAT(OP_J, JINDEX(dest)), "j %p", dest);                         \
  } while (0)

#define JAL(dest)                                                              \
  do {                                                                         \
    count_jmp();                                                               \
    EMIT(J_FORMAT(OP_JAL, JINDEX(dest)), "jal %p", dest);                      \
  } while (0)

#define JALR(rs)                                                               \
  do {                                                                         \
    count_jmp();                                                               \
    EMIT(R_FORMAT(OP_SPECIAL, GPR(rs), 0, GPR(RA), 0, SPECIAL_JALR),           \
         "jalr %s", gpn(rs));                                                  \
  } while (0)
...
#define JR(rs)                                                                 \
  do {                                                                         \
    count_jmp();                                                               \
    EMIT(R_FORMAT(OP_SPECIAL, GPR(rs), 0, 0, 0, SPECIAL_JR), "jr %s",          \
         gpn(rs));                                                             \
  } while (0)
#define trampJR(rs)                                                            \
  do {                                                                         \
    count_jmp();                                                               \
    TRAMP(R_FORMAT(OP_SPECIAL, GPR(rs), 0, 0, 0, SPECIAL_JR), "jr %s",         \
          gpn(rs));                                                            \
  } while (0)

#define LB(rt, offset, base) LDSTGPR(OP_LB, rt, offset, base)

#define LH(rt, offset, base) LDSTGPR(OP_LH, rt, offset, base)

#define LUI(rt, uimm)                                                          \
  do {                                                                         \
    count_alu();                                                               \
    EMIT(U_FORMAT(OP_LUI, 0, GPR(rt), uimm), "lui %s, 0x%x", gpn(rt),          \
         ((uimm)&0xffff));                                                     \
  } while (0)

#define LW(rt, offset, base) LDSTGPR(OP_LW, rt, offset, base)

#define MFHI(rd)                                                               \
  do {                                                                         \
    count_alu();                                                               \
    EMIT(R_FORMAT(OP_SPECIAL, 0, 0, GPR(rd), 0, SPECIAL_MFHI), "mfhi %s",      \
         gpn(rd));                                                             \
  } while (0)
...
#define MFLO(rd)                                                               \
  do {                                                                         \
    count_alu();                                                               \
    EMIT(R_FORMAT(OP_SPECIAL, 0, 0, GPR(rd), 0, SPECIAL_MFLO), "mflo %s",      \
         gpn(rd));                                                             \
  } while (0)
...
#define MUL(rd, rs, rt)                                                        \
  do {                                                                         \
    count_alu();                                                               \
    EMIT(R_FORMAT(OP_SPECIAL2, GPR(rs), GPR(rt), GPR(rd), 0, SPECIAL2_MUL),    \
         "mul %s, %s, %s", gpn(rd), gpn(rs), gpn(rt));                         \
  } while (0)

#define MULT(rs, rt)                                                           \
  do {                                                                         \
    count_alu();                                                               \
    EMIT(R_FORMAT(OP_SPECIAL, GPR(rs), GPR(rt), 0, 0, SPECIAL_MULT),           \
         "mult %s, %s", gpn(rs), gpn(rt));                                     \
  } while (0)

#define MOVE(rd, rs)                                                           \
  do {                                                                         \
    count_alu();                                                               \
    EMIT(R_FORMAT(OP_SPECIAL, GPR(rs), GPR(ZERO), GPR(rd), 0, SPECIAL_ADDU),   \
         "move %s, %s", gpn(rd), gpn(rs));                                     \
  } while (0)

#define MOVN(rd, rs, rt)                                                       \
  do {                                                                         \
    count_alu();                                                               \
    EMIT(R_FORMAT(OP_SPECIAL, GPR(rs), GPR(rt), GPR(rd), 0, SPECIAL_MOVN),     \
         "movn %s, %s, %s", gpn(rd), gpn(rs), gpn(rt));                        \
  } while (0)

#define MOVZ(rd, rs, rt)                                                       \
  do {                                                                         \
    count_alu();                                                               \
    EMIT(R_FORMAT(OP_SPECIAL, GPR(rs), GPR(rt), GPR(rd), 0, SPECIAL_MOVZ),     \
         "movz %s, %s, %s", gpn(rd), gpn(rs), gpn(rt));                        \
  } while (0)

#define NEGU(rd, rt)                                                           \
  do {                                                                         \
    count_alu();                                                               \
    EMIT(R_FORMAT(OP_SPECIAL, GPR(ZERO), GPR(rt), GPR(rd), 0, SPECIAL_SUBU),   \
         "negu %s, %s", gpn(rd), gpn(rt));                                     \
  } while (0)
...
#define NOP()                                                                  \
  do {                                                                         \
    count_misc();                                                              \
    EMIT(                                                                      \
        R_FORMAT(OP_SPECIAL, GPR(ZERO), GPR(ZERO), GPR(ZERO), 0, SPECIAL_SLL), \
        "nop");                                                                \
  } while (0)
...
#define NOR(rd, rs, rt)                                                        \
  do {                                                                         \
    count_alu();                                                               \
    EMIT(R_FORMAT(OP_SPECIAL, GPR(rs), GPR(rt), GPR(rd), 0, SPECIAL_NOR),      \
         "nor %s, %s, %s", gpn(rd), gpn(rs), gpn(rt));                         \
  } while (0)

#define NOT(rd, rs)                                                            \
  do {                                                                         \
    count_alu();                                                               \
    EMIT(R_FORMAT(OP_SPECIAL, GPR(rs), GPR(ZERO), GPR(rd), 0, SPECIAL_NOR),    \
         "not %s, %s", gpn(rd), gpn(rs));                                      \
  } while (0)

#define OR(rd, rs, rt)                                                         \
  do {                                                                         \
    count_alu();                                                               \
    EMIT(R_FORMAT(OP_SPECIAL, GPR(rs), GPR(rt), GPR(rd), 0, SPECIAL_OR),       \
         "or %s, %s, %s", gpn(rd), gpn(rs), gpn(rt));                          \
  } while (0)

#define ORI(rt, rs, uimm)                                                      \
  do {                                                                         \
    count_alu();                                                               \
    EMIT(U_FORMAT(OP_ORI, GPR(rs), GPR(rt), uimm), "ori %s, %s, 0x%x",         \
         gpn(rt), gpn(rs), ((uimm)&0xffff));                                   \
  } while (0)
...
#define SLTIU(rt, rs, simm)                                                    \
  do {                                                                         \
    count_alu();                                                               \
    EMIT(I_FORMAT(OP_SLTIU, GPR(rs), GPR(rt), simm), "sltiu %s, %s, %d",       \
         gpn(rt), gpn(rs), simm);                                              \
  } while (0)
...
#define SLT(rd, rs, rt)                                                        \
  do {                                                                         \
    count_alu();                                                               \
    EMIT(R_FORMAT(OP_SPECIAL, GPR(rs), GPR(rt), GPR(rd), 0, SPECIAL_SLT),      \
         "slt %s, %s, %s", gpn(rd), gpn(rs), gpn(rt));                         \
  } while (0)

#define SLTU(rd, rs, rt)                                                       \
  do {                                                                         \
    count_alu();                                                               \
    EMIT(R_FORMAT(OP_SPECIAL, GPR(rs), GPR(rt), GPR(rd), 0, SPECIAL_SLTU),     \
         "sltu %s, %s, %s", gpn(rd), gpn(rs), gpn(rt));                        \
  } while (0)

#define SLL(rd, rt, sa)                                                        \
  do {                                                                         \
    count_alu();                                                               \
    EMIT(R_FORMAT(OP_SPECIAL, 0, GPR(rt), GPR(rd), sa, SPECIAL_SLL),           \
         "sll %s, %s, %d", gpn(rd), gpn(rt), sa);                              \
  } while (0)

#define SLLV(rd, rt, rs)                                                       \
  do {                                                                         \
    count_misc();                                                              \
    EMIT(R_FORMAT(OP_SPECIAL, GPR(rs), GPR(rt), GPR(rd), 0, SPECIAL_SLLV),     \
         "sllv %s, %s, %s", gpn(rd), gpn(rt), gpn(rs));                        \
  } while (0)

#define SRA(rd, rt, sa)                                                        \
  do {                                                                         \
    count_alu();                                                               \
    EMIT(R_FORMAT(OP_SPECIAL, 0, GPR(rt), GPR(rd), sa, SPECIAL_SRA),           \
         "sra %s, %s, %d", gpn(rd), gpn(rt), sa);                              \
  } while (0)

#define SRAV(rd, rt, rs)                                                       \
  do {                                                                         \
    count_alu();                                                               \
    EMIT(R_FORMAT(OP_SPECIAL, GPR(rs), GPR(rt), GPR(rd), 0, SPECIAL_SRAV),     \
         "srav %s, %s, %s", gpn(rd), gpn(rt), gpn(rs));                        \
  } while (0)

#define SRL(rd, rt, sa)                                                        \
  do {                                                                         \
    count_alu();                                                               \
    EMIT(R_FORMAT(OP_SPECIAL, 0, GPR(rt), GPR(rd), sa, SPECIAL_SRL),           \
         "srl %s, %s, %d", gpn(rd), gpn(rt), sa);                              \
  } while (0)

#define SRLV(rd, rt, rs)                                                       \
  do {                                                                         \
    count_alu();                                                               \
    EMIT(R_FORMAT(OP_SPECIAL, GPR(rs), GPR(rt), GPR(rd), 0, SPECIAL_SRLV),     \
         "srlv %s, %s, %s", gpn(rd), gpn(rt), gpn(rs));                        \
  } while (0)

#define SUBU(rd, rs, rt)                                                       \
  do {                                                                         \
    count_alu();                                                               \
    EMIT(R_FORMAT(OP_SPECIAL, GPR(rs), GPR(rt), GPR(rd), 0, SPECIAL_SUBU),     \
         "subu %s, %s, %s", gpn(rd), gpn(rs), gpn(rt));                        \
  } while (0)

#define SW(rt, offset, base) LDSTGPR(OP_SW, rt, offset, base)

#define XOR(rd, rs, rt)                                                        \
  do {                                                                         \
    count_alu();                                                               \
    EMIT(R_FORMAT(OP_SPECIAL, GPR(rs), GPR(rt), GPR(rd), 0, SPECIAL_XOR),      \
         "xor %s, %s, %s", gpn(rd), gpn(rs), gpn(rt));                         \
  } while (0)

#define XORI(rt, rs, uimm)                                                     \
  do {                                                                         \
    count_alu();                                                               \
    EMIT(U_FORMAT(OP_XORI, GPR(rs), GPR(rt), uimm), "xori %s, %s, 0x%x",       \
         gpn(rt), gpn(rs), ((uimm)&0xffff));                                   \
  } while (0)
...
#define FOP_FMT2(ffmt, fd, fs, func, name)                                     \
  do {                                                                         \
    count_fpu();                                                               \
    EMIT(F_FORMAT(OP_COP1, ffmt, FPR(F0), FPR(fs), FPR(fd), func),             \
         "%s.%s %s, %s", name, fname[ffmt], fpn(fd), fpn(fs));                 \
  } while (0)

#define FOP_FMT3(ffmt, fd, fs, ft, func, name)                                 \
  do {                                                                         \
    count_fpu();                                                               \
    EMIT(F_FORMAT(OP_COP1, ffmt, FPR(ft), FPR(fs), FPR(fd), func),             \
         "%s.%s %s, %s, %s", name, fname[ffmt], fpn(fd), fpn(fs), fpn(ft));    \
  } while (0)

#define C_COND_FMT(cond, ffmt, fs, ft)                                         \
  do {                                                                         \
    count_fpu();                                                               \
    EMIT(F_FORMAT(OP_COP1, ffmt, FPR(ft), FPR(fs), FPR(F0), 0x30 | (cond)),    \
         "c.%s.%s %s, %s", cname[cond], fname[ffmt], fpn(fs), fpn(ft));        \
  } while (0)

#define MFC1(rt, fs)                                                           \
  do {                                                                         \
    count_fpu();                                                               \
    EMIT(F_FORMAT(OP_COP1, 0, GPR(rt), FPR(fs), FPR(F0), 0), "mfc1 %s, %s",    \
         gpn(rt), fpn(fs));                                                    \
  } while (0)
...
#define MTC1(rt, fs)                                                           \
  do {                                                                         \
    count_fpu();                                                               \
    EMIT(F_FORMAT(OP_COP1, 4, GPR(rt), FPR(fs), FPR(F0), 0), "mtc1 %s, %s",    \
         gpn(rt), fpn(fs));                                                    \
  } while (0)
...
#define MOVF(rd, rs, cc)                                                       \
  do {                                                                         \
    count_fpu();                                                               \
    EMIT(R_FORMAT(OP_SPECIAL, GPR(rs), (cc) << 2, GPR(rd), 0, SPECIAL_MOVCI),  \
         "movf %s, %s, $fcc%d", gpn(rd), gpn(rs), cc);                         \
  } while (0)

#define MOVF_D(fs, fd, cc)                                                     \
  do {                                                                         \
    count_fpu();                                                               \
    EMIT(F_FORMAT(OP_COP1, FMT_D, ((cc) << 2) | 0, FPR(fs), FPR(fd),           \
                  COP1X_MOVCF),                                                \
         "movf.d %s, %s, $fcc%d", fpn(fd), fpn(fs), cc);                       \
  } while (0)

#define MOVT_D(fs, fd, cc)                                                     \
  do {                                                                         \
    count_fpu();                                                               \
    EMIT(F_FORMAT(OP_COP1, FMT_D, ((cc) << 2) | 1, FPR(fs), FPR(fd),           \
                  COP1X_MOVCF),                                                \
         "movt.d %s, %s, $fcc%d", fpn(fd), fpn(fs), cc);                       \
  } while (0)

#define CVT_D_S(fd, fs)                                                        \
  do {                                                                         \
    count_fpu();                                                               \
    EMIT(F_FORMAT(OP_COP1, FMT_S, FPR(F0), FPR(fs), FPR(fd), COP1_CVTD),       \
         "cvt.d.s %s, %s", fpn(fd), fpn(fs));                                  \
  } while (0)

#define CVT_D_W(fd, fs)                                                        \
  do {                                                                         \
    count_fpu();                                                               \
    EMIT(F_FORMAT(OP_COP1, FMT_W, FPR(F0), FPR(fs), FPR(fd), COP1_CVTD),       \
         "cvt.d.w %s, %s", fpn(fd), fpn(fs));                                  \
  } while (0)

#define CVT_S_D(fd, fs)                                                        \
  do {                                                                         \
    count_fpu();                                                               \
    EMIT(F_FORMAT(OP_COP1, FMT_D, FPR(F0), FPR(fs), FPR(fd), COP1_CVTS),       \
         "cvt.s.d %s, %s", fpn(fd), fpn(fs));                                  \
  } while (0)

#define TRUNC_W_D(fd, fs)                                                      \
  do {                                                                         \
    count_fpu();                                                               \
    EMIT(F_FORMAT(OP_COP1, FMT_D, FPR(F0), FPR(fs), FPR(fd), COP1_TRUNCW),     \
         "trunc.w.d %s, %s", fpn(fd), fpn(fs));                                \
  } while (0)
...
#define LDXC1(fd, index, base)                                                 \
  do {                                                                         \
    count_fpu();                                                               \
    EMIT(X_FORMAT(OP_COP1X, GPR(base), GPR(index), FPR(F0), FPR(fd),           \
                  COP1X_LDXC1),                                                \
         "ldxc1 %s, %s(%s)", fpn(fd), gpn(index), gpn(base));                  \
  } while (0)
#define SDXC1(fs, index, base)                                                 \
  do {                                                                         \
    count_fpu();                                                               \
    EMIT(X_FORMAT(OP_COP1X, GPR(base), GPR(index), FPR(fs), FPR(F0),           \
                  COP1X_SDXC1),                                                \
         "sdxc1 %s, %s(%s)", fpn(fs), gpn(index), gpn(base));                  \
  } while (0)
