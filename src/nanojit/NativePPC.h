...
static const Register
    ...
    F13 = {45}, // param
    F14 = {46}, // F14-31 saved
    F15 = {47}, F16 = {48}, F17 = {49}, F18 = {50}, F19 = {51}, F20 = {52},
    F21 = {53}, F22 = {54}, F23 = {55}, F24 = {56}, F25 = {57}, F26 = {58},
    F27 = {59}, F28 = {60}, F29 = {61}, F30 = {62}, F31 = {63},
...
static inline bool IsGpReg(Register r) { return r <= R31; }
static inline bool IsFpReg(Register r) { return r >= F0; }
...
#ifdef NANOJIT_64BIT
#define DECL_PPC64() void asm_qbinop(LIns *);
#else
#define DECL_PPC64()
#endif
...
#define DECLARE_PLATFORM_ASSEMBLER()                                           \
  ...
  DECL_PPC64()
    ...
    const
    int LARGEST_UNDERRUN_PROT = 9 *
                                4; // largest value passed to underrunProtect
...
#define EMIT1(op, r1, r2, r3, ins, fmt, ...)                                   \
  do {                                                                         \
    underrunProtect(4);                                                        \
    NIns swapme = (NIns)0;                                                     \
    ...
    if (!_doNotSwap && _lastOpcode.reg1 != NoSwap && r1 != NoSwap &&           \
      ...
      if (op == PPC_stb || op == PPC_stbx || op == PPC_stw ||                  \
          op == PPC_stwu || op == PPC_stwux || op == PPC_std ||                \
          op == PPC_stdu || op == PPC_stdux || op == PPC_stfd ||               \
          op == PPC_stfdx || op == PPC_stdx || op == PPC_stwx) {               \
        if (_lastOpcode.opcode == PPC_cmp || _lastOpcode.opcode == PPC_cmpi || \
      ...
      if (!doubleswap &&                                                       \
          (_lastOpcode.reg1 == NoReg ||                                        \
           (_lastOpcode.reg1 != r1 && _lastOpcode.reg1 != r2 &&                \
            _lastOpcode.reg1 != r3)) &&                                        \
          (_lastOpcode.reg2 == NoReg ||                                        \
           (_lastOpcode.reg2 != r1 && _lastOpcode.reg2 != r2 &&                \
            _lastOpcode.reg2 != r3)) &&                                        \
          (_lastOpcode.reg3 == NoReg ||                                        \
           (_lastOpcode.reg3 != r1 && _lastOpcode.reg3 != r2 &&                \
            _lastOpcode.reg3 != r3))) {                                        \
...
#define EMIT1S(op, r1, r2, r3, ins, fmt, ...)                                  \
  do {                                                                         \
    underrunProtect(4);                                                        \
    *(--_nIns) = (NIns)(ins);                                                  \
    asm_output(fmt, ##__VA_ARGS__);                                            \
...
#define Bx(li, aa, lk)                                                         \
  EMIT1(PPC_b, NoSwap, NoReg, NoReg,                                           \
        PPC_b | ((li)&0xffffff) << 2 | (aa) << 1 | (lk), "b%s%s %p",           \
        (lk) ? "l" : "", (aa) ? "a" : "", _nIns + (li))
...
#define BCx(op, bo, bit, cr, bd, aa, lk, likely)                               \
  EMIT1(PPC_bc, NoSwap, NoReg, NoReg,                                          \
        PPC_bc | (bo | likely) << 21 | (4 * (cr) + COND_##bit) << 16 |         \
            ((bd)&0x3fff) << 2 | (aa) << 1 | (lk),                             \
        "%s%s%s%s cr%d,%p", #op, (lk) ? "l" : "", (aa) ? "a" : "",             \
        (likely) ? "+" : "", (cr), _nIns + (bd))
...
#define BCCTRx(op, bo, bit, cr, lk, likely)                                    \
  EMIT1(PPC_bcctr, NoSwap, NoReg, NoReg,                                       \
        PPC_bcctr | (bo | likely) << 21 | (4 * (cr) + COND_##bit) << 16 |      \
            (lk)&1,                                                            \
...
#define BCTR(link)                                                             \
  EMIT1(0x4E800420, NoSwap, NoReg, NoReg, 0x4E800420 | (link), "bctr%s",       \
        (link) ? "l" : "")
#define BCTRL() BCTR(1)
...
#define ALU1(op, ra, rs, rc)                                                   \
  EMIT1(PPC_##op, ra, rs, NoReg,                                               \
...
#define EXTSB(rd, rs)                                                          \
  EMIT1(PPC_extsb, rd, rs, NoReg, PPC_extsb | GPR(rs) << 21 | GPR(rd) << 16,   \
        "extsb %s,%s", gpn(rd), gpn(rs))
#define EXTSH(rd, rs)                                                          \
  EMIT1(PPC_extsh, rd, rs, NoReg, PPC_extsh | GPR(rs) << 21 | GPR(rd) << 16,   \
        "extsh %s,%s", gpn(rd), gpn(rs))
#define EXTSW(rd, rs)                                                          \
  EMIT1(PPC_extsw, rd, rs, NoReg, PPC_extsw | GPR(rs) << 21 | GPR(rd) << 16,   \
        "extsw %s,%s", gpn(rd), gpn(rs))

#define NEG(rd, rs)                                                            \
  EMIT1(PPC_neg, rd, rs, NoReg, PPC_neg | GPR(rd) << 21 | GPR(rs) << 16,       \
        "neg %s,%s", gpn(rd), gpn(rs))
#define FNEG(rd, rs)                                                           \
  EMIT1(PPC_fneg, rd, rs, NoReg, PPC_fneg | FPR(rd) << 21 | FPR(rs) << 11,     \
        "fneg %s,%s", gpn(rd), gpn(rs))
#define FMR(rd, rb)                                                            \
  EMIT1(PPC_fmr, rd, rb, NoReg, PPC_fmr | FPR(rd) << 21 | FPR(rb) << 11,       \
        "fmr %s,%s", gpn(rd), gpn(rb))
#define FCFID(rd, rs)                                                          \
  EMIT1(PPC_fcfid, rd, rs, NoReg, PPC_fcfid | FPR(rd) << 21 | FPR(rs) << 11,   \
        "fcfid %s,%s", gpn(rd), gpn(rs))
#define FCTIW(rd, rs)                                                          \
  EMIT1(PPC_fctiw, rd, rs, NoReg, PPC_fctiw | FPR(rd) << 21 | FPR(rs) << 11,   \
        "fctiw %s,%s", gpn(rd), gpn(rs))
#define FRSP(rd, rb)                                                           \
  EMIT1(PPC_frsp, rd, rb, NoReg, PPC_frsp | FPR(rd) << 21 | FPR(rb) << 11,     \
        "frsp %s,%s", gpn(rd), gpn(rb))
...
#define MTSPR(spr, rs)                                                         \
  EMIT1(PPC_mtspr, NoSwap, rs, NoReg,                                          \
        PPC_mtspr | GPR(rs) << 21 | SPR(spr) << 11, "mt%s %s", #spr, gpn(rs))
#define MFSPR(rd, spr)                                                         \
  EMIT1(PPC_mfspr, NoSwap, rd, NoReg,                                          \
        PPC_mfspr | GPR(rd) << 21 | SPR(spr) << 11, "mf%s %s", #spr, gpn(rd))
...
#define MEMx(op, r, a, b)                                                      \
  EMIT1(PPC_##op, r, a, b,                                                     \
        PPC_##op | GPR(r) << 21 | GPR(a) << 16 | GPR(b) << 11, "%s %s,%s,%s",  \
        #op, gpn(r), gpn(a), gpn(b))
#define FMEMx(op, r, a, b)                                                     \
  EMIT1(PPC_##op, r, a, b,                                                     \
        PPC_##op | FPR(r) << 21 | GPR(a) << 16 | GPR(b) << 11, "%s %s,%s,%s",  \
        #op, gpn(r), gpn(a), gpn(b))

#define MEMux(op, rs, ra, rb)                                                  \
  EMIT1(PPC_##op, rs, ra, rb,                                                  \
...
#define ALUI(op, rd, ra, d)                                                    \
  EMIT1(PPC_##op, rd, ra, NoReg,                                               \
...
#define SUBI(rd, ra, d)                                                        \
  EMIT1(PPC_addi, rd, ra, NoReg,                                               \
        PPC_addi | GPR(rd) << 21 | GPR(ra) << 16 | uint16_t(-(d)),             \
        "subi %s,%s,%d", gpn(rd), gpn(ra), (d))

#define LI(rd, v)                                                              \
  EMIT1(PPC_addi, rd, NoReg, NoReg, PPC_addi | GPR(rd) << 21 | uint16_t(v),    \
        "li %s,%d (0x%x)", gpn(rd), int16_t(v), int16_t(v)) /* addi rd,0,v */

#define LIS(rd, v)                                                             \
  EMIT1(PPC_addis, rd, NoReg, NoReg, PPC_addis | GPR(rd) << 21 | uint16_t(v),  \
        "lis %s,%d (0x%x)", gpn(rd), int16_t(v),                               \
        int16_t(v) << 16) /* addis, rd,0,v */

#define MTCRF(mask, rs)                                                        \
  EMIT1(PPC_mtcrf, NoSwap, rs, NoReg,                                          \
        PPC_mtcrf | (GPR(rs) << 21) | (uint16_t(mask) << 12), "mtcrf %d,%s",   \
        gpn(rs), uint16_t(mask))
#define MTCR(rs) MTCRF(0xff, rs)
#define MFCR(rd)                                                               \
  EMIT1(PPC_mfcr, NoSwap, rd, NoReg, PPC_mfcr | GPR(rd) << 21, "mfcr %s",      \
        gpn(rd))
/* Not supported on POWER4 and later. G5 Macs emulate in software. */
#define MCRXR(cr)                                                              \
  EMIT1(PPC_mcrxr, NoSwap, NoReg, NoReg, PPC_mcrxr | (cr) << 23, "mcrxr cr%d", \
        (cr))

#define CMPx(op, crfd, ra, rb, l)                                              \
  EMIT1(PPC_##op, ra, rb, NoReg,                                               \
...
#define CROR(cr, d, a, b)                                                      \
  EMIT1(PPC_cror, NoSwap, NoReg, NoReg,                                        \
        PPC_cror | (4 * (cr) + COND_##d) << 21 | (4 * (cr) + COND_##a) << 16 | \
            (4 * (cr) + COND_##b) << 11,                                       \
        "cror %d,%d,%d", 4 * (cr) + COND_##d, 4 * (cr) + COND_##a,             \
        4 * (cr) + COND_##b)

#define RLWIMI(rd, rs, sh, mb, me)                                             \
  EMIT1(PPC_rlwimi, rd, rs, NoReg,                                             \
        PPC_rlwimi | GPR(rs) << 21 | GPR(rd) << 16 | (sh) << 11 | (mb) << 6 |  \
            (me) << 1,                                                         \
        "rlwimi %s,%s,%d,%d,%d", gpn(rd), gpn(rs), (sh), (mb), (me))

#define RLWINM(rd, rs, sh, mb, me)                                             \
  EMIT1(PPC_rlwinm, rd, rs, NoReg,                                             \
        PPC_rlwinm | GPR(rs) << 21 | GPR(rd) << 16 | (sh) << 11 | (mb) << 6 |  \
            (me) << 1,                                                         \
...
#define RLDICL(rd, rs, sh, mb)                                                 \
  EMIT1(PPC_rldicl, rd, rs, NoReg,                                             \
        PPC_rldicl | GPR(rs) << 21 | GPR(rd) << 16 | LO5(sh) << 11 |           \
            SPLITMB(mb) << 5 | BIT6(sh) << 1,                                  \
...
#define RLDIMI(rd, rs, sh, mb)                                                 \
  EMIT1(PPC_rldimi, rd, rs, NoReg,                                             \
        PPC_rldimi | GPR(rs) << 21 | GPR(rd) << 16 | LO5(sh) << 11 |           \
            SPLITMB(mb) << 5 | BIT6(sh) << 1,                                  \
        "rldimi %s,%s,%d,%d", gpn(rd), gpn(rs), (sh), (mb))

// insrdi rD,rS,n,b => rldimi rD,rS,64-(b+n),b: insert n bit value into rD
// starting at b
#define INSRDI(rd, rs, n, b)                                                   \
  EMIT1(PPC_rldimi, rd, rs, NoReg,                                             \
        PPC_rldimi | GPR(rs) << 21 | GPR(rd) << 16 |                           \
            LO5(64 - ((b) + (n))) << 11 | SPLITMB(b) << 5 |                    \
            BIT6(64 - ((b) + (n))) << 1,                                       \
        "insrdi %s,%s,%d,%d", gpn(rd), gpn(rs), (n), (b))

#define EXTRWI(rd, rs, n, b)                                                   \
  EMIT1(PPC_rlwinm, rd, rs, NoReg,                                             \
        PPC_rlwinm | GPR(rs) << 21 | GPR(rd) << 16 | ((n) + (b)) << 11 |       \
            (32 - (n)) << 6 | 31 << 1,                                         \
        "extrwi %s,%s,%d,%d", gpn(rd), gpn(rs), (n), (b))

// sldi rd,rs,n (n<64) => rldicr rd,rs,n,63-n
#define SLDI(rd, rs, n)                                                        \
  EMIT1(PPC_rldicr, rd, rs, NoReg,                                             \
        PPC_rldicr | GPR(rs) << 21 | GPR(rd) << 16 | LO5(n) << 11 |            \
            SPLITMB(63 - (n)) << 5 | BIT6(n) << 1,                             \
        "sldi %s,%s,%d", gpn(rd), gpn(rs), (n))

#define SLWI(rd, rs, n)                                                        \
  EMIT1(PPC_rlwinm, rd, rs, NoReg,                                             \
        PPC_rlwinm | GPR(rs) << 21 | GPR(rd) << 16 | (n) << 11 | 0 << 6 |      \
            (31 - (n)) << 1,                                                   \
        "slwi %s,%s,%d", gpn(rd), gpn(rs), (n))
#define SRWI(rd, rs, n)                                                        \
  EMIT1(PPC_rlwinm, rd, rs, NoReg,                                             \
        PPC_rlwinm | GPR(rs) << 21 | GPR(rd) << 16 | (32 - (n)) << 11 |        \
            (n) << 6 | 31 << 1,                                                \
        "slwi %s,%s,%d", gpn(rd), gpn(rs), (n))
#define SRAWI(rd, rs, n)                                                       \
  EMIT1(PPC_srawi, rd, rs, NoReg,                                              \
