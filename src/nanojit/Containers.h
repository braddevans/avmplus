...
class BitSet {
  ...
  inline int bitnum2word(int i) const { return i >> 6; }
  inline int64_t bitnum2mask(int i) const { return ONE << (i & 63); }
...
public:
  SeqBuilder(Allocator &allocator)
      : allocator(allocator), items(NULL), last(NULL) {}
  ...
  Seq<T> *get() const { return items; }

  /** self explanitory */
  bool isEmpty() const { return items == NULL; }

  /** de-reference all items */
  void clear() { items = last = NULL; }
...
template <class K> struct UseOperEqual {
  static bool keysEqual(const K &x, const K &y) { return x == y; }
};

template <class K> struct DefaultKeysEqual<K *> {
  static bool keysEqual(K *x, K *y) { return x == y; }
};

template <> struct DefaultKeysEqual<int32_t> {
  static bool keysEqual(int32_t x, int32_t y) { return x == y; }
};

template <> struct DefaultKeysEqual<uint32_t> {
  static bool keysEqual(uint32_t x, uint32_t y) { return x == y; }
};

template <> struct DefaultKeysEqual<int64_t> {
  static bool keysEqual(int64_t x, int64_t y) { return x == y; }
};

template <> struct DefaultKeysEqual<uint64_t> {
  static bool keysEqual(uint64_t x, uint64_t y) { return x == y; }
};

/** Bucket hashtable with a fixed # of buckets (never rehash)
 *  Intended for use when a reasonable # of buckets can be estimated ahead of
 * time. Note that operator== is used to compare keys.
 */
template <class K, class T, class H = DefaultHash<K>,
          class E = DefaultKeysEqual<K>>
class HashMap {
  Allocator &allocator;
  size_t nbuckets;
  class Node {
...
public:
  ...
  void clear() { VMPI_memset(buckets, 0, sizeof(Seq<Node> *) * nbuckets); }
...
#else
    buckets[i] =
        new (allocator, alignof1<K>()) Seq<Node>(Node(k, v), buckets[i]);
#endif
  }
  ...
  public:
    Iter(MapType &map)
        : map(map), bucket((int)map.nbuckets - 1), current(NULL) {}
...
public:
  ...
  void put(K k, T v) { insert(root, k, v); }
