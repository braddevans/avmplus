 ...
 * Need the following:
 *
 *    - merging paths ( build a graph? ), possibly use external rep to drive
 * codegen
 */
Assembler::Assembler(CodeAlloc &codeAlloc, Allocator &dataAlloc,
                     Allocator &alloc, LogControl *logc, const Config &config,
                     MetaDataWriter *mdWriter)
    : alloc(alloc), _codeAlloc(codeAlloc), _dataAlloc(dataAlloc),
      _thisfrag(NULL), _branchStateMap(alloc), _patches(alloc), _labels(alloc),
      _noise(NULL)
...
#endif
      ,
      codeList(NULL), _epilogue(NULL), _err(None)
#if PEDANTIC
      ,
      pedanticTop(NULL)
#endif
      ...
      _config(config) {
  (void)logc;
  verbose_only(_logc = logc;) verbose_only(_outputCache = 0;)
      verbose_only(outline[0] = '\0';) verbose_only(outlineEOL[0] = '\0';)
...
void AR::validateQuick() {
  ...
  uint32_t const lo =
      (_highWaterMark > 1 + RADIUS ? _highWaterMark - RADIUS : 1);
  uint32_t const hi = (_highWaterMark + 1 + RADIUS < NJ_MAX_STACK_ENTRY
                           ? _highWaterMark + 1 + RADIUS
                           : NJ_MAX_STACK_ENTRY);
...
void Assembler::codeAlloc(NIns *&start, NIns *&end,
  ...
  verbose_only(nBytes += (end - start) * sizeof(NIns);) NanoAssert(
      uintptr_t(end) - uintptr_t(start) >= (size_t)LARGEST_UNDERRUN_PROT);
  eip = end;
...
void Assembler::pageValidate() {
  ...
  NanoAssertMsg(codeStart <= _nIns && _nIns <= codeEnd,
                "Native instruction pointer overstep paging bounds; check "
                "overrideProtect for last instruction");
}
#endif
...
void Assembler::registerConsistencyCheck() {
  ...
  for (Register r = lsReg(managed); managed; r = nextLsReg(managed, r)) {
    ...
    if (_allocator.isFree(r)) {
      ...
      /* note: registers may overlap; but if they do, the primary reg. defined
         by the instruction must completely contain all the overlapping
         registers. */
      NanoAssertMsg((rmask(r) & rmask(ins->getReg())) == rmask(r),
                    "Register record mismatch");
    }
  }

  RegisterMask not_managed = ~_allocator.getManagedSet();
  for (Register r = lsReg(not_managed); not_managed;
       r = nextLsReg(not_managed, r)) {
...
// For debugging: Define this symbol to force blinding of all tainted
// displacements.
//#define NANOJIT_STRESS_BLINDING
// For debugging: Always use a temporary, e.g., to diagnose cases where we fail
// to do so improperly.
...
Register Assembler::getBaseRegWithBlinding(LIns *base, int &d,
                                           RegisterMask allow, bool tainted,
                                           bool force, Register *rbp) {
  ...
  if (force || (tainted && shouldBlindDisplacement(d))) {
    d += _blindMask32;
    // If rb is live after this insn, we need a temporary that we can clobber
    // for blinding.
    *rbp = rb;
    // Note: If rb is not in GpRegs, it may be a special register such as FP and
    // may be live even if lastUseOfBase is true.
    return (lastUseOfBase && !(rmask(rb) & SpecialRegs))
               ? rb
               : _allocator.allocTempReg(allow & ~rmask(rb));
  } else {
    *rbp = UnspecifiedReg;
    return rb;
  }
...
Register Assembler::getBaseRegWithBlindingUsingSpecifiedTemp(
    LIns *base, int &d, RegisterMask allow, bool tainted, bool force,
    Register *rbp, Register temp) {
...
void Assembler::getBaseReg2WithBlinding(RegisterMask allowValue, LIns *value,
                                        Register &rv, RegisterMask allowBase,
                                        LIns *base, Register &rb, int &d,
                                        bool tainted, bool force,
                                        Register *rbp) {
#ifdef NANOJIT_BLINDING_FORCE_TEMPORARY
  bool lastUseOfBase = false;
#else
  ...
  if (force || (tainted && shouldBlindDisplacement(d))) {
    ...
    *rbp = rb;
    // Note: If rb is not in GpRegs, it may be a special register such as FP and
    // may be live even if lastUseOfBase is true.
    rb = (lastUseOfBase && !(rmask(rb) & SpecialRegs))
             ? rb
             : _allocator.allocTempReg(allowBase & ~rmask(rb) & ~rmask(rv));
  } else {
    *rbp = UnspecifiedReg;
    // leave rb alone
  }
...
void Assembler::adjustBaseRegForBlinding(Register trb, Register rb) {
  debug_only(disp_blind_getbase++;) if (rb != UnspecifiedReg) {
    ...
    if (trb != rb) {
      debug_only(disp_blind_needmove++;) MR(trb, rb);
    }
  }
}
#endif
...
void Assembler::findRegFor2(RegisterMask allowa, LIns *ia, Register &ra,
  ...
  if (ia == ib) {
    ra = rb =
        findRegFor(ia, allowa & allowb); // use intersection(allowa, allowb)
...
void Assembler::getBaseReg2(RegisterMask allowValue, LIns *value, Register &rv,
                            RegisterMask allowBase, LIns *base, Register &rb,
                            int &d) {
#if !PEDANTIC
  if (base->isop(LIR_allocp)) {
    rb = FP;
    ...
    if ((rm & allow) == rm) {
      ...
      if (copyCandidates) {
        ...
        if ((rmask(s) & GpRegs) && (rmask(r) & GpRegs)) {
          MR(s, r); // move 'ins' from its pre-state reg (r) to its post-state
                    // reg (s)
        } else {
          asm_nongp_copy(s, r);
        }
...
bool Assembler::asm_maybe_spill(LIns *ins, bool pop) {
  if (ins->isInAr()) {
    ...
    verbose_only(RefBuf b; if (_logc->lcbits & LC_Native) {
      setOutputForEOL("  <= spill %s",
                      _thisfrag->lirbuf->printer->formatRef(&b, ins));
    }) int8_t nWords = ins->isF4() ? 4 : (ins->isQorD() ? 2 : 1);
#ifdef NANOJIT_IA32
...
//
void Assembler::getBaseIndexScale(LIns *addp, LIns **base, LIns **index,
                                  int *scale) {
...
NIns *Assembler::asm_leave_trace(LIns *guard) {
  ...
  // we are done producing the exit logic for the guard so demark where our exit
  // block code begins
  NIns *jmpTarget =
      _nIns; // target in exit path for our mainline conditional jump

  // swap back pointers, effectively storing the last location used in the exit
  // path
  swapCodeChunks();
  _inExit = false;
  verbose_only(_nInsAfter = _nIns;)

      // verbose_only( verbose_outputf("         LIR_xt/xf swapCodeChunks, _nIns
      // is now %08X(%08X), _nExitIns is now %08X(%08X)",_nIns,
      // *_nIns,_nExitIns,*_nExitIns) );
      verbose_only(verbose_outputf("%p:", jmpTarget);)
          verbose_only(verbose_outputf("----------------------------------- ## "
                                       "BEGIN exit block (LIR_xt|LIR_xf)"));

#ifdef NANOJIT_IA32
  NanoAssertMsgf(_fpuStkDepth == _sv_fpuStkDepth,
...
void Assembler::compile(Fragment *frag, Allocator &alloc,
                        bool optimize verbose_only(, LInsPrinter *printer)) {
  verbose_only(bool anyVerb = (_logc->lcbits & 0xFFFF & ~LC_FragProfile) > 0;
      ...
      verbose_only(if (liveVerb) {
        ...
        if (optimize) {
          StackFilter *sf =
              new (alloc) StackFilter(lir, alloc, frag->lirbuf->sp);
          lir = sf;
        }
  ...
  verbose_only(if (anyVerb) {
    _logc->printf("=== Translating LIR fragments into assembly:\n");
      ...
      verbose_only(RefBuf b;) verbose_only(if (anyVerb) {
        _logc->printf("=== -- Compile trunk %s: begin\n",
                      printer->formatAddr(&b, frag));
      })

      // Used for debug printing, if needed
      debug_only(ValidateReader *validate = NULL;) verbose_only(
          ReverseLister *pp_init = NULL; ReverseLister *pp_after_sf = NULL;)
  ...
  verbose_only(if (_logc->lcbits & LC_ReadLIR) {
    pp_init = new (alloc)
        ReverseLister(lir, alloc, frag->lirbuf->printer, _logc, "Initial LIR");
    lir = pp_init;
  })
  ...
  verbose_only(if (_logc->lcbits & LC_AfterSF) {
    pp_after_sf = new (alloc) ReverseLister(lir, alloc, frag->lirbuf->printer,
                                            _logc, "After StackFilter");
    lir = pp_after_sf;
  })
  ...
  verbose_only(if (pp_init) pp_init->finish();
...
void Assembler::beginAssembly(Fragment *frag) {
  verbose_only(codeBytes = 0;) verbose_only(exitBytes = 0;)
...
#ifdef NANOJIT_64BIT
    _blindMask64 =
        uint64_t(_noise->getValue32()) << 32 | uint64_t(_noise->getValue32());
#endif
  }
#endif
  ...
  NanoAssert(!(_noise == NULL && (_config.harden_blind_constants ||
                                  _config.harden_nop_insertion ||
                                  _config.harden_function_alignment)));
          ...
          verbose_only(NanoAssert(frag->nExitBytes == 0);)
              verbose_only(NanoAssert(frag->profCount == 0);)
                  verbose_only(if (_logc->lcbits & LC_FragProfile)
                               ...
                               else NanoAssert(frag->profFragID == 0);)
...
CodeList *Assembler::endAssembly(Fragment *frag) {
  // don't try to patch code if we are in an error state since we might have
  // partially overwritten the code cache already
  if (error()) {
  ...
  if (_nExitIns) {
    _codeAlloc.addRemainder(codeList, exitStart, exitEnd, _nExitSlot,
                            _nExitIns);
  ...
  debug_only(pageValidate();) NanoAssert(_branchStateMap.isEmpty());
...
#ifdef RA_REGISTERS_OVERLAP
    active &=
        _allocator.activeMask(); // retiring one reg may actually retire several
                                 // at once if the registers overlap
#endif
  }
}
...
void Assembler::asm_jmp(LIns *ins, InsList &pending_lives) {
  ...
  debug_only(_fpuStkDepth = 0;)
    ...
    /// Halfmoon uses a lived, not livei instruction across an edge to keep a
    /// value alive
    ///
    /// test case:
    ...
    ///
    /// the lived, on x86-32 platforms, always uses FST0 because its hinted in
    /// nativei386.cpp::nInit() CodegenLIR does not keep numbers alive on edges,
    /// and instead stores, so you won't see this without halfmoon. the lived on
    /// x86-64 uses XMMO instead of FST0, perhaps that's a better fix? make the
    /// hint for calld on native386 use XMM0?
    ///
    /// If we don't have the _fpuStkDepth check, a different assert fails.
    /// (assembler.cpp:366). Leaving this check here, boids.abc passes no
    /// problems... so is this unique to halfmoon?
#ifdef NANOJIT_IA32
    debug_only(_fpuStkDepth = (_allocator.getActive(FST0) ? -1 : 0);)
#endif
        if (!label) {
...
void Assembler::asm_jcc(LIns *ins, InsList &pending_lives) {
  ...
  // Changes to the logic below will likely need to be propagated to
  // Assembler::asm_jov().
...
void Assembler::asm_x(LIns *ins) {
  verbose_only(_thisfrag->nStaticExits++;) countlir_x();
...
void Assembler::asm_xcc(LIns *ins) {
  LIns *cond = ins->oprnd1();
  if (cond->isImmI()) {
    if ((ins->isop(LIR_xt) && !cond->immI()) ||
        (ins->isop(LIR_xf) && cond->immI())) {
  ...
  verbose_only(_thisfrag->nStaticExits++;) countlir_xcc();
...
void Assembler::gen(LirFilter *reader) {
  ...
  int32_t nopInsertTrigger =
      hardenNopInsertion(_config) ? noiseForNopInsertion(_noise) : 0;
  ...
  NanoAssert(reader->finalIns()->isop(LIR_x) || reader->finalIns()->isRet() ||
             isLiveOpcode(reader->finalIns()->opcode()));

  for (currIns = reader->read(); !currIns->isop(LIR_start);
       currIns = reader->read()) {
    ...
    // compiler hardening technique that inserts no-op instructions in the
    // compiled method when nopInsertTrigger < 0
    if (hardenNopInsertion(_config)) {
      size_t delta =
          (uintptr_t)priorIns -
          (uintptr_t)
              _nIns; // # bytes that have been emitted since last go-around

      // if no codeList then we know priorIns and _nIns are on same page,
      // otherwise make sure priorIns was not in the previous code block
    ...
    case LIR_retf4:
      ...
      // The halfmoon emitter may emit it's blocks in a different order which
      // means that a ret may be the last thing emitted, so a call to
      // handleLoopCarriedExprs is required here.  It's probably not a bad idea
      // to emit it in general but but it's ifdef'ed for now.
      handleLoopCarriedExprs(pending_lives, 0);
#endif
      asm_ret(ins);
      break;
    ...
    case LIR_hcalli: {
      ...
      if (ins->isExtant()) {
        ...
        // since it may break on future platforms with overlapping register
        // files
    ...
    case LIR_ldd:
    case LIR_ldf2d:
    case LIR_ldf: // Ok, ldf is not really 64-bits, but it's still more natural
                  // to handle it in load64 than in load32 (which basically
                  // deals with ints)
      countlir_ldq();
      ins->oprnd1()->setResultLive();
      if (ins->isExtant()) {
    ...
    case LIR_jtbl: {
      ...
      for (uint32_t i = count; i-- > 0;) {
        ...
        if (lstate) {
          ...
          verbose_only(RefBuf b;) asm_output(
              "   %u: [&%s]", i, _thisfrag->lirbuf->printer->formatRef(&b, to));
        } else {
          has_back_edges = true;
        }
      }
      ...
      // with two existing edges that have conflicting register assignments,
      // unlike a conditional branch where code can be inserted on the
      // fall-through path to reconcile registers.  So, frontends *must* insert
      // LIR_regfence at labels of forward jtbl jumps.  Check here to make sure
      // no registers were picked up from any forward edges.
      NanoAssert(_allocator.activeMask() == 0);

      // Unlike the case of LIR_jt/LIR_jf, where we can separate the comparison
      // that consumes the registers from the branch itself, we will have no
      // opportunity to restore evicted registers between the last use of the
      // jump index and the actual transfer of control.  Unlike the case for
      // forward branches, however, code has not yet been emitted at the target,
      // and it should simply be sufficient to make sure that the captured state
      // reflects the allocation for the index.
      ...
      if (has_back_edges) {
        ...
        // save merged (empty) register state at target labels we haven't seen
        // yet
        for (uint32_t i = count; i-- > 0;) {
          LIns *to = ins->getTarget(i);
          LabelState *lstate = _labels.get(to);
          if (!lstate) {
            ...
            verbose_only(RefBuf b;)
                asm_output("   %u: [&%s]", i,
                           _thisfrag->lirbuf->printer->formatRef(&b, to));
          }
        }
        asm_output("backward edges");
      }

      // Emit the jump instruction, which allocates 1 register for the jump
      // index.
    ...
    case LIR_label: {
      ...
      verbose_only(if (_logc->lcbits & LC_FragProfile) {
        ...
        // label seen first, normal target of forward jump, save addr &
        // allocator
    ...
    if (_logc->lcbits & LC_AfterDCE) {
      ...
      if (ins->isop(LIR_comment))
        ...
        outputf("    %s (in reg? %d)", printer->formatIns(&b, ins),
                ins->isInReg() ? REGNUM(ins->getReg()) : -1);
    }
#endif
    ...
    // check that all is well (don't check in exit paths since its more
    // complicated)
    debug_only(pageValidate();) debug_only(resourceConsistencyCheck();)
  }
}
...
void Assembler::assignParamRegs() {
  ...
  if (param1)
    findSpecificRegForUnallocated(param1,
                                  RegAlloc::argRegs[param1->paramArg()]);
}

void Assembler::handleLoopCarriedExprs(InsList &pending_lives,
                                       RegisterMask reserved) {
// ensure that exprs spanning the loop are marked live at the end of the loop
  ...
  for (Seq<LIns *> *p = pending_lives.get(); p != NULL; p = p->tail) {
    ...
    if (!op1->isImmAny()) {
      ...
      switch (ins->opcode()) {
      default:
        NanoAssert(!"bad LIR_live opcode");
      case LIR_lived:
        allowed = FpDRegs;
        break;
      case LIR_livef:
...
void Assembler::printActivationState() {
  ...
  for (AR::Iter iter(_activation); iter.next(ins, nStackSlots, arIndex);) {
    ...
    const char *n = _thisfrag->lirbuf->printer->formatRef(&b, ins);
    if (nStackSlots > 1) {
      VMPI_sprintf(s, " %d-%d(%s)", 4 * arIndex,
                   4 * (arIndex + nStackSlots - 1), n);
    } else {
...
uint32_t AR::reserveEntry(LIns *ins) {
  ...
  if (nStackSlots == 1) {
    ...
    uint32_t const extraStackSlots = ins->isF4() ? ((4 - (nStackSlots & 3)) & 3)
                                                 :          // 16-byte align
    ...
    // Be sure to account for any 8/16-byte-round-up when calculating
    // spaceNeeded.
    ...
    uint32_t const extraSpaceForAlignment =
        ins->isF4() ? padding16byteAlign : padding8byteAlign;
...
void Assembler::arFree(LIns *ins) {
  ...
  _activation.freeEntryAt(
      arIndex); // free any stack stack space associated with entry
}
...
void Assembler::evictScratchRegsExcept(RegisterMask ignore) {
  ...
  for (Register r = lsReg(evict_set); evict_set; r = nextLsReg(evict_set, r)) {
    ...
    Register r1 = ins->getReg();
    NanoAssert((rmask(r1) & rmask(r)) ==
               rmask(r)); // r must be strictly included in r1
    r = r1;
  ...
  while (allow && len > 0) {
    ...
    if (allow && --len > 0) {
      ...
      while (j + 1 < len) {
        int child = j + 1;
        if (j + 2 < len &&
            regs->getPriority(tosave[j + 2]) > regs->getPriority(tosave[j + 1]))
          child++;
 ...
 *  current & !saved                    evict current (unionRegisterState does
 * nothing) current &  saved & current==saved current &  saved & current!=saved
 * evict current, add saved
 */
void Assembler::intersectRegisterState(RegAlloc &saved) {
  ...
  for (Register r = msReg(reg_set); reg_set; r = nextMsReg(reg_set, r)) {
    ...
    if (curins != savedins) {
#ifdef RA_REGISTERS_OVERLAP
      Register r1 = getFatherReg(
          r, curins ? curins : savedins); // at leasst one is non-null
      NanoAssert(((rmask(r) & rmask(r1)) == rmask(r)));
      ...
      if (curins) {
        ...
        verbose_only(shouldMention = true;) NanoAssert(curins->getReg() == r);
        evict(curins);
      }

#ifdef NANOJIT_IA32
      if (savedins && r == FST0) {
        verbose_only(shouldMention = true;) FSTP(FST0);
      }
#endif
    }
  }
  ...
  verbose_only(if (shouldMention) verbose_outputf(
 ...
 * !current & !saved                    none
 * !current &  saved                    add saved
 *  current & !saved                    none (intersectRegisterState evicts
 * current) current &  saved & current==saved   none current &  saved &
 * current!=saved   evict current, add saved
 */
void Assembler::unionRegisterState(RegAlloc &saved) {
  ...
  // Do evictions and pops first.
  verbose_only(bool shouldMention = false;) RegisterMask reg_set =
      _allocator.activeMask() | saved.activeMask();
...
LabelState *LabelStateMap::get(LIns *label) { return labels.get(label); }
