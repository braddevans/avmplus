...
static const int pagesPerAlloc = 256; // 1MB
    ...
    defined(VMCFG_STRESS_SPARSE_CODE_MEM)
...
CodeAlloc::CodeAlloc(const Config *config)
    : heapblocks(0), availblocks(0), totalAllocated(0),
      bytesPerPage(VMPI_getVMPageSize()),
      bytesPerAlloc(pagesPerAlloc * bytesPerPage), _config(config) {}

CodeAlloc::~CodeAlloc() { reset(); }
...
CodeList *CodeAlloc::firstBlock(CodeList *term) {
  // use uintptr_t, rather than char*, to avoid "increases required alignment"
  // warning
  uintptr_t end = (uintptr_t)alignUp(term, bytesPerPage);
  return (CodeList *)(end - (uintptr_t)bytesPerAlloc);
}

static int round(size_t x) { return (int)((x + 512) >> 10); }
...
void CodeAlloc::alloc(NIns *&start, NIns *&end, size_t byteLimit) {
  ...
  // grab a block
  NanoAssert(!byteLimit ||
             byteLimit >
                 blkSpaceFor(2)); // if a limit is imposed it must be bigger
                                  // than 2x minimum block size (see below)
  markBlockWrite(availblocks);
  CodeList *b = removeBlock(availblocks);

  // limit imposed (byteLimit > 0) and the block is too big?  then break it
  // apart
  if (byteLimit > 0 && b->size() > byteLimit) {

    size_t consume; // # bytes to extract from the free block

    // enough space to carve out a perfectly sized blk?  (leaving at least a
    // full free blk)
    if (b->size() >= byteLimit + headerSpaceFor(1) + blkSpaceFor(1)) {
      ...
      NanoAssert(b->size() - consume >
                 blkSpaceFor(1)); // thus, we know that at least 1 blk left.
    }
  ...
  NanoAssert(b->size() >= minAllocSize);
  b->next =
      0; // not technically needed (except for debug builds), but good hygiene.
  b->isFree = false;
  start = b->start();
  end = b->end;
  if (verbose)
...
void CodeAlloc::flushICache(void *, size_t) { VMPI_debugBreak(); }
...
// see
// http://developer.apple.com/documentation/Carbon/Reference/Memory_Manag_nt_Utilities/Reference/reference.html#//apple_ref/c/func/MakeDataExecutable
void CodeAlloc::flushICache(void *start, size_t len) {
...
void CodeAlloc::addBlock(CodeList *&blocks, CodeList *b) {
  NanoAssert(b->terminator !=
             NULL); // should not be mucking with terminator blocks
  b->next = blocks;
  blocks = b;
}
...
/**
 * split a block by freeing the hole in the middle defined by
 * [holeStart,holeEnd), and adding the used prefix and suffix parts to the
 * blocks CodeList.
 */
void CodeAlloc::addRemainder(CodeList *&blocks, NIns *start, NIns *end,
                             NIns *holeStart, NIns *holeEnd) {
  NanoAssert(start < end && start <= holeStart && holeStart <= holeEnd &&
             holeEnd <= end);
  // shrink the hole by aligning holeStart forward and holeEnd backward
  holeStart = (NIns *)((uintptr_t(holeStart) + sizeof(NIns *) - 1) &
                       ~(sizeof(NIns *) - 1));
  holeEnd = (NIns *)(uintptr_t(holeEnd) & ~(sizeof(NIns *) - 1));
  // hole needs to be big enough for 2 headers + 1 block of free space
  // (subtraction not used in check to avoid wraparound)
  size_t minHole = headerSpaceFor(2) + blkSpaceFor(1);
  if (uintptr_t(holeEnd) < minHole + uintptr_t(holeStart)) {
    // the hole is too small to make a new free block and a new used block. just
    // keep the whole original block and don't free anything.
    ...
    b2->higher->lower = b2;
    b1->higher = b2;
    debug_only(sanity_check();) this->free(b1->start(), b1->end);
    addBlock(blocks, b2);
  } else if (holeEnd == end) {
    ...
    b2->higher = b3;
    b2->isFree =
        false; // redundant, since we're about to free, but good hygiene
    b2->terminator = b1->terminator;
    b3->lower = b2;
    b3->end = end;
    b3->isFree = false;
    ...
    b2->next = 0;
    b3->next = 0;
    debug_only(sanity_check();) this->free(b2->start(), b2->end);
    addBlock(blocks, b3);
    addBlock(blocks, b1);
  }
}
...
size_t CodeAlloc::size() { return totalAllocated; }
...
void CodeAlloc::sanity_check() {
  ...
  for (CodeList *term = heapblocks; term; term = term->next) {
    for (CodeList *hb = term->lower; hb; hb = hb->lower) {
      if (hb->isFree && hb->size() >= minAllocSize) {
        ...
        for (CodeList *avail = this->availblocks; !found_on_avail && avail;
             avail = avail->next) {
  ...
  for (CodeList *avail = this->availblocks; avail; avail = avail->next) {
    ...
    for (CodeList *term = heapblocks; !found_in_heapblocks && term;
         term = term->next) {
      for (CodeList *hb = term->lower; !found_in_heapblocks && hb;
           hb = hb->lower) {
...
// Variant of markExec(CodeList*) that walks all heapblocks (i.e. chunks)
// marking each one executable.   On systems where bytesPerAlloc is low (i.e.
// have lots of elements in the list) this can be expensive.
...
#endif
{
  ...
  if (!term->isExec) {
    ...
    if (address <= (char *)pFnc &&
        (char *)pFnc < ((char *)address + bytesPerAlloc))
