...
#ifdef _MSC_VER
// disable some specific warnings which are normally useful, but pervasive in
// the code-gen macros
...
#ifdef NJ_VERBOSE
const char *regNames[] = {"eax",  "ecx",  "edx",  "ebx",  "esp",  "ebp",
                          "esi",  "edi",  "xmm0", "xmm1", "xmm2", "xmm3",
                          "xmm4", "xmm5", "xmm6", "xmm7", "f0"};
...
#define TODO(x)                                                                \
  do {                                                                         \
    verbose_only(outputf(#x);) NanoAssertMsgf(false, "%s", #x);                \
  } while (0)

const Register argRegsF4[] = {
    XMM0, XMM1,
    XMM2}; // don't make it part of the assembler, it's specific to i386
...
inline void Assembler::ALUsib(I32 opc, R r, R base, R index, I32 scale,
                              I32 disp) {
  underrunProtect(7);
...
inline void Assembler::ALUsib16(I32 opc, R r, R base, R index, I32 scale,
                                I32 disp) {
  underrunProtect(8);
  MODRMsib(REGNUM(r), base, index, scale, disp);
  OPCODE(opc);
...
inline void Assembler::ALU2sib(I32 opc2, Register r, R base, R index, I32 scale,
                               I32 disp) {
  underrunProtect(8);
  MODRMsib(REGNUM(r), base, index, scale, disp);
  OPCODE2(opc2);
...
inline void Assembler::OR(R l, R r) {
  count_alu();
  ALU(0x0b, REGNUM(l), r);
  asm_output("or %s,%s", gpn(l), gpn(r));
}
inline void Assembler::AND(R l, R r) {
  count_alu();
  ALU(0x23, REGNUM(l), r);
  asm_output("and %s,%s", gpn(l), gpn(r));
}
inline void Assembler::XOR(R l, R r) {
  count_alu();
  ALU(0x33, REGNUM(l), r);
  asm_output("xor %s,%s", gpn(l), gpn(r));
}
inline void Assembler::ADD(R l, R r) {
  count_alu();
  ALU(0x03, REGNUM(l), r);
  asm_output("add %s,%s", gpn(l), gpn(r));
}
inline void Assembler::SUB(R l, R r) {
  count_alu();
  ALU(0x2b, REGNUM(l), r);
  asm_output("sub %s,%s", gpn(l), gpn(r));
}
inline void Assembler::IMUL(R l, R r) {
  count_alu();
  ALU2(0x0faf, l, r);
  asm_output("imul %s,%s", gpn(l), gpn(r));
}
inline void Assembler::DIV(R r) {
  count_alu();
  ALU(0xf7, 7, r);
  asm_output("idiv  edx:eax, %s", gpn(r));
}
inline void Assembler::NOT(R r) {
  count_alu();
  ALU(0xf7, 2, r);
  asm_output("not %s", gpn(r));
}
inline void Assembler::NEG(R r) {
  count_alu();
  ALU(0xf7, 3, r);
  asm_output("neg %s", gpn(r));
}
inline void Assembler::AND8R(R r) {
  count_alu();
  ALU(0x22, REGNUM(r), AL2AHReg(r));
...
inline void Assembler::SHLi(R r, I32 i) {
  count_alu();
  SHIFTi(4, r, i);
  asm_output("shl %s,%d", gpn(r), i);
}
inline void Assembler::SHRi(R r, I32 i) {
  count_alu();
  SHIFTi(5, r, i);
  asm_output("shr %s,%d", gpn(r), i);
}
inline void Assembler::SARi(R r, I32 i) {
  count_alu();
  SHIFTi(7, r, i);
  asm_output("sar %s,%d", gpn(r), i);
}

inline void Assembler::MOVZX8(R d, R s) {
  count_alu();
  ALU2(0x0fb6, d, s);
  asm_output("movzx %s,%s", gpn(d), gpn(s));
}

// NOTE: SUBi moved to Nativei386.h
inline void Assembler::ADDi(R r, I32 i) {
  count_alu();
  ALUi(0x05, r, i);
  asm_output("add %s,%d", gpn(r), i);
}
inline void Assembler::ANDi(R r, I32 i) {
  count_alu();
  ALUi(0x25, r, i);
  asm_output("and %s,%d", gpn(r), i);
}
inline void Assembler::ORi(R r, I32 i) {
  count_alu();
  ALUi(0x0d, r, i);
  asm_output("or %s,%d", gpn(r), i);
}
inline void Assembler::XORi(R r, I32 i) {
  count_alu();
  ALUi(0x35, r, i);
  asm_output("xor %s,%d", gpn(r), i);
}

inline void Assembler::ADDmi(I32 d, R b, I32 i) {
  count_alust();
  ALUmi(0x05, d, b, i);
  asm_output("add %d(%s), %d", d, gpn(b), i);
}

inline void Assembler::TEST(R d, R s) {
  count_alu();
  ALU(0x85, REGNUM(d), s);
  asm_output("test %s,%s", gpn(d), gpn(s));
}
inline void Assembler::CMP(R l, R r) {
  count_alu();
  ALU(0x3b, REGNUM(l), r);
  asm_output("cmp %s,%s", gpn(l), gpn(r));
}
inline void Assembler::CMPi(R r, I32 i) {
  count_alu();
  ALUi(0x3d, r, i);
  asm_output("cmp %s,%d", gpn(r), i);
}

inline void Assembler::LEA(R r, I32 d, R b) {
  count_alu();
  ALUm(0x8d, REGNUM(r), d, b);
  asm_output("lea %s,%d(%s)", gpn(r), d, gpn(b));
}

inline void Assembler::CDQ() {
  SARi(rEDX, 31);
  MR(rEDX, rEAX);
}
...
inline void Assembler::SETE(R r) {
  count_alu();
  ALU2(0x0f94, r, r);
  asm_output("sete %s", gpn(r));
}
inline void Assembler::SETNP(R r) {
  count_alu();
  ALU2(0x0f9B, r, r);
  asm_output("setnp %s", gpn(r));
}
inline void Assembler::SETNPH(R r) {
  count_alu();
  ALU2(0x0f9B, AL2AHReg(r), AL2AHReg(r));
  asm_output("setnp %s", RBhi(r));
}
inline void Assembler::SETL(R r) {
  count_alu();
  ALU2(0x0f9C, r, r);
  asm_output("setl %s", gpn(r));
}
inline void Assembler::SETLE(R r) {
  count_alu();
  ALU2(0x0f9E, r, r);
  asm_output("setle %s", gpn(r));
}
inline void Assembler::SETG(R r) {
  count_alu();
  ALU2(0x0f9F, r, r);
  asm_output("setg %s", gpn(r));
}
inline void Assembler::SETGE(R r) {
  count_alu();
  ALU2(0x0f9D, r, r);
  asm_output("setge %s", gpn(r));
}
inline void Assembler::SETB(R r) {
  count_alu();
  ALU2(0x0f92, r, r);
  asm_output("setb %s", gpn(r));
}
inline void Assembler::SETBE(R r) {
  count_alu();
  ALU2(0x0f96, r, r);
  asm_output("setbe %s", gpn(r));
}
inline void Assembler::SETA(R r) {
  count_alu();
  ALU2(0x0f97, r, r);
  asm_output("seta %s", gpn(r));
}
inline void Assembler::SETAE(R r) {
  count_alu();
  ALU2(0x0f93, r, r);
  asm_output("setae %s", gpn(r));
}
inline void Assembler::SETO(R r) {
  count_alu();
  ALU2(0x0f92, r, r);
  asm_output("seto %s", gpn(r));
}

inline void Assembler::MREQ(R d, R s) {
  count_alu();
  ALU2(0x0f44, d, s);
  asm_output("cmove %s,%s", gpn(d), gpn(s));
}
inline void Assembler::MRNE(R d, R s) {
  count_alu();
  ALU2(0x0f45, d, s);
  asm_output("cmovne %s,%s", gpn(d), gpn(s));
}
inline void Assembler::MRL(R d, R s) {
  count_alu();
  ALU2(0x0f4C, d, s);
  asm_output("cmovl %s,%s", gpn(d), gpn(s));
}
inline void Assembler::MRLE(R d, R s) {
  count_alu();
  ALU2(0x0f4E, d, s);
  asm_output("cmovle %s,%s", gpn(d), gpn(s));
}
inline void Assembler::MRG(R d, R s) {
  count_alu();
  ALU2(0x0f4F, d, s);
  asm_output("cmovg %s,%s", gpn(d), gpn(s));
}
inline void Assembler::MRGE(R d, R s) {
  count_alu();
  ALU2(0x0f4D, d, s);
  asm_output("cmovge %s,%s", gpn(d), gpn(s));
}
inline void Assembler::MRB(R d, R s) {
  count_alu();
  ALU2(0x0f42, d, s);
  asm_output("cmovb %s,%s", gpn(d), gpn(s));
}
inline void Assembler::MRBE(R d, R s) {
  count_alu();
  ALU2(0x0f46, d, s);
  asm_output("cmovbe %s,%s", gpn(d), gpn(s));
}
inline void Assembler::MRA(R d, R s) {
  count_alu();
  ALU2(0x0f47, d, s);
  asm_output("cmova %s,%s", gpn(d), gpn(s));
}
inline void Assembler::MRAE(R d, R s) {
  count_alu();
  ALU2(0x0f43, d, s);
  asm_output("cmovae %s,%s", gpn(d), gpn(s));
}
inline void Assembler::MRNO(R d, R s) {
  count_alu();
  ALU2(0x0f41, d, s);
  asm_output("cmovno %s,%s", gpn(d), gpn(s));
}

// these aren't currently used but left in for reference
//#define LDEQ(r,d,b) do { ALU2m(0x0f44,r,d,b); asm_output("cmove %s,%d(%s)",
//gpn(r),d,gpn(b)); } while(0) #define LDNEQ(r,d,b) do { ALU2m(0x0f45,r,d,b);
//asm_output("cmovne %s,%d(%s)", gpn(r),d,gpn(b)); } while(0)
...
inline void Assembler::LDsib(R reg, I32 disp, R base, R index, I32 scale) {
  ...
  asm_output("mov %s,%d(%s+%s*%c)", gpn(reg), disp, gpn(base), gpn(index),
             SIBIDX(scale));
}
...
inline void Assembler::LD16Ssib(R r, I32 disp, R base, R index, I32 scale) {
  ...
  asm_output("movsx16 %s,%d(%s+%s*%c)", gpn(r), disp, gpn(base), gpn(index),
             SIBIDX(scale));
}
...
inline void Assembler::LD16Zsib(R r, I32 disp, R base, R index, I32 scale) {
  ...
  asm_output("movzx16 %s,%d(%s+%s*%c)", gpn(r), disp, gpn(base), gpn(index),
             SIBIDX(scale));
}
...
inline void Assembler::LD8Zsib(R r, I32 disp, R base, R index, I32 scale) {
  ...
  asm_output("movzx8 %s,%d(%s+%s*%c)", gpn(r), disp, gpn(base), gpn(index),
             SIBIDX(scale));
}
...
inline void Assembler::LD8Ssib(R r, I32 disp, R base, R index, I32 scale) {
  ...
  asm_output("movsx8 %s,%d(%s+%s*%c)", gpn(r), disp, gpn(base), gpn(index),
             SIBIDX(scale));
}
...
inline void Assembler::ST8(R base, I32 disp, R reg) {
  ...
  asm_output("mov8 %d(%s),%s", disp, base == UnspecifiedReg ? "0" : gpn(base),
             gpn(reg));
}
...
inline void Assembler::ST8sib(I32 disp, R base, R index, I32 scale, R reg) {
  ...
  asm_output("mov8 %d(%s+%s*%c),%s", disp,
             base == UnspecifiedReg ? "0" : gpn(base), gpn(index),
             SIBIDX(scale), gpn(reg));
}

inline void Assembler::ST16(R base, I32 disp, R reg) {
  count_st();
  ALUm16(0x89, REGNUM(reg), disp, base);
  asm_output("mov16 %d(%s),%s", disp, base == UnspecifiedReg ? "0" : gpn(base),
             gpn(reg));
}
...
inline void Assembler::ST16sib(I32 disp, R base, R index, I32 scale, R reg) {
  ...
  asm_output("mov16 %d(%s+%s*%c),%s", disp,
             base == UnspecifiedReg ? "0" : gpn(base), gpn(index),
             SIBIDX(scale), gpn(reg));
}
...
inline void Assembler::ST(R base, I32 disp, R reg) {
  ...
  asm_output("mov %d(%s),%s", disp, base == UnspecifiedReg ? "0" : gpn(base),
             gpn(reg));
}
...
inline void Assembler::ST8isib(I32 disp, R base, R index, I32 scale, I32 imm) {
  ...
  OPCODE(0xc6);
  asm_output("mov8 %d(%s+%s*%c),%d", disp, gpn(base), gpn(index), SIBIDX(scale),
             imm);
}
...
inline void Assembler::ST16isib(I32 disp, R base, R index, I32 scale, I32 imm) {
  ...
  OPCODE(0xc7);
  OPCODE(0x66);
  asm_output("mov16 %d(%s+%s*%c),%d", disp, gpn(base), gpn(index),
             SIBIDX(scale), imm);
}
...
inline void Assembler::STisib(I32 disp, R base, R index, I32 scale, I32 imm) {
  ...
  MODRMsib(0, base, index, scale, disp);
  OPCODE(0xc7);
  asm_output("mov %d(%s+%s*%c),%d", disp, gpn(base), gpn(index), SIBIDX(scale),
             imm);
}
...
inline void Assembler::LDPSm(R r, const float4_t *addr) {
  ...
  *(--_nIns) = 0x0f;
  // *addr is a constant, so we can print it here.
  asm_output("mov%cps %s,(%p) // =%f,%f,%f,%f", is_aligned ? 'a' : 'u', gpn(r),
             (void *)addr, f4_x(*addr), f4_y(*addr), f4_z(*addr), f4_w(*addr));
}

inline void Assembler::SSE_LDUPS(R r, I32 d, R b) {
  count_ld();
  SSEsm(0x0f10, r, d, b);
  asm_output("movups %s,%d(%s)", gpn(r), d, gpn(b));
}

inline void Assembler::SSE_LDUPSsib(R rr, I32 d, R rb, R ri, I32 scale) {
  count_ld();
  SSEssib(0x0f10, rr, d, rb, ri, scale);
  asm_output("movups %s,%d(%s+%s*%c)", gpn(rr), d, gpn(rb), gpn(ri),
             SIBIDX(scale));
}
...
inline void Assembler::SSE_STUPS(I32 d, R b, R r) {
  count_stf4();
  SSEsm(0x0f11, r, d, b);
  asm_output("movups %d(%s),%s", d, gpn(b), gpn(r));
}
inline void Assembler::SSE_CMPNEQPS(R rd, R rs) {
  count_fpu();
  SSEu8_2(0x0fc2, rd, rs, 4);
  asm_output("cmpneqps %s,%s", gpn(rd), gpn(rs));
}
inline void Assembler::SSE_CVTSI2SS(R xr, R gr) {
  count_fpu();
  SSE(0xf30f2a, xr, gr);
  asm_output("cvtsi2ss %s,%s", gpn(xr), gpn(gr));
}
inline void Assembler::SSE_CVTTSS2SI(R gr, R xr) {
  count_fpu();
  SSE(0xf30f2c, gr, xr);
  asm_output("cvttss2si %s,%s", gpn(gr), gpn(xr));
}
...
inline void Assembler::SSE_CMPPS(R rd, R rs, uint8_t imm) {
  count_fpu();
  static const char *ckind[8] = {"eq",  "lt",  "le",  "unord",
                                 "neq", "nlt", "nle", "ord"};
...
inline void Assembler::FLD32sm(const float *dm) {
  count_ldq();
  FPUdm(0xd900, (const double *)dm);
  asm_output("fld32 (%p)", (void *)dm);
  fpu_push();
}

inline void Assembler::FADD32(I32 d, R b) {
  count_fpu();
  FPUm(0xd800, d, b);
  asm_output("fadd32 %d(%s)", d, gpn(b));
}
inline void Assembler::FSUB32(I32 d, R b) {
  count_fpu();
  FPUm(0xd804, d, b);
  asm_output("fsub32 %d(%s)", d, gpn(b));
}
inline void Assembler::FSUBR32(I32 d, R b) {
  count_fpu();
  FPUm(0xd805, d, b);
  asm_output("fsubr %d(%s)", d, gpn(b));
}
inline void Assembler::FMUL32(I32 d, R b) {
  count_fpu();
  FPUm(0xd801, d, b);
  asm_output("fmul32 %d(%s)", d, gpn(b));
}
inline void Assembler::FDIV32(I32 d, R b) {
  count_fpu();
  FPUm(0xd806, d, b);
  asm_output("fdiv32 %d(%s)", d, gpn(b));
}
inline void Assembler::FDIVR32(I32 d, R b) {
  count_fpu();
  FPUm(0xd807, d, b);
  asm_output("fdivr32 %d(%s)", d, gpn(b));
}

inline void Assembler::FADD32dm(const float *dm) {
  count_ldq();
  FPUdm(0xd800, (const double *const)dm);
  asm_output("fadd32 (%p)", (void *)dm);
}
inline void Assembler::FSUBR32dm(const float *dm) {
  count_ldq();
  FPUdm(0xd805, (const double *const)dm);
  asm_output("fsubr32 (%p)", (void *)dm);
}
inline void Assembler::FMUL32dm(const float *dm) {
  count_ldq();
  FPUdm(0xd801, (const double *const)dm);
  asm_output("fmul32 (%p)", (void *)dm);
}
inline void Assembler::FDIVR32dm(const float *dm) {
  count_ldq();
  FPUdm(0xd807, (const double *const)dm);
  asm_output("fdivr32 (%p)", (void *)dm);
}

inline void Assembler::SSE_LDQ(R r, I32 d, R b) {
  count_ldq();
  SSEm(0xf30f7e, r, d, b);
  asm_output("movq %s,%d(%s)", gpn(r), d, gpn(b));
}
inline void Assembler::SSE_LDSS(R r, I32 d, R b) {
  count_ld();
  SSEm(0xf30f10, r, d, b);
...
inline void Assembler::SSE_LDQsib(R rr, I32 d, R rb, R ri, I32 scale) {
  ...
  asm_output("movq %s,%d(%s+%s*%c)", gpn(rr), d, gpn(rb), gpn(ri),
             SIBIDX(scale));
}
...
inline void Assembler::SSE_LDSSsib(R rr, I32 d, R rb, R ri, I32 scale) {
  ...
  asm_output("movss %s,%d(%s+%s*%c)", gpn(rr), d, gpn(rb), gpn(ri),
             SIBIDX(scale));
}

inline void Assembler::SSE_STSD(I32 d, R b, R r) {
  count_stq();
  SSEm(0xf20f11, r, d, b);
  asm_output("movsd %d(%s),%s", d, gpn(b), gpn(r));
}
inline void Assembler::SSE_STQ(I32 d, R b, R r) {
  count_stq();
  SSEm(0x660fd6, r, d, b);
  asm_output("movq %d(%s),%s", d, gpn(b), gpn(r));
}
inline void Assembler::SSE_STSS(I32 d, R b, R r) {
  count_st();
  SSEm(0xf30f11, r, d, b);
...
inline void Assembler::SSE_STQsib(I32 d, R rb, R ri, I32 scale, R rv) {
  ...
  asm_output("movq %d(%s+%s*%c),%s", d, gpn(rb), gpn(ri), SIBIDX(scale),
             gpn(rv));
}

inline void Assembler::SSE_CVTSI2SD(R xr, R gr) {
  count_fpu();
  SSE(0xf20f2a, xr, gr);
  asm_output("cvtsi2sd %s,%s", gpn(xr), gpn(gr));
}
inline void Assembler::SSE_CVTSD2SI(R gr, R xr) {
  count_fpu();
  SSE(0xf20f2d, gr, xr);
  asm_output("cvtsd2si %s,%s", gpn(gr), gpn(xr));
}
inline void Assembler::SSE_CVTTSD2SI(R gr, R xr) {
  count_fpu();
  SSE(0xf20f2c, gr, xr);
  asm_output("cvttsd2si %s,%s", gpn(gr), gpn(xr));
}
inline void Assembler::SSE_CVTSD2SS(R xr, R gr) {
  count_fpu();
  SSE(0xf20f5a, xr, gr);
  asm_output("cvtsd2ss %s,%s", gpn(xr), gpn(gr));
}
inline void Assembler::SSE_CVTSS2SD(R xr, R gr) {
  count_fpu();
  SSE(0xf30f5a, xr, gr);
  asm_output("cvtss2sd %s,%s", gpn(xr), gpn(gr));
}
inline void Assembler::SSE_CVTDQ2PD(R d, R r) {
  count_fpu();
  SSE(0xf30fe6, d, r);
  asm_output("cvtdq2pd %s,%s", gpn(d), gpn(r));
}
...
// Since we generate backwards, we track the FPU stack depth as a negative
// number. We use the top of the x87 stack as the single allocatable FP
// register, FST0. Thus, between LIR instructions, the depth of the FPU stack
// must be either 0 or -1, depending on whether FST0 is in use.  Within the
// expansion of a single LIR instruction, however, deeper levels of the stack
// may be used as unmanaged temporaries.  Hence, we allow for all eight levels
...
inline void Assembler::FNSTSW_AX() {
  count_fpu();
  FPUc(0xdfe0);
  asm_output("fnstsw_ax");
}
inline void Assembler::FCHS() {
  count_fpu();
  FPUc(0xd9e0);
  asm_output("fchs");
}
inline void Assembler::FLD1() {
  count_fpu();
  FPUc(0xd9e8);
  asm_output("fld1");
  fpu_push();
}
inline void Assembler::FLDZ() {
  count_fpu();
  FPUc(0xd9ee);
  asm_output("fldz");
  fpu_push();
...
inline void Assembler::FST32(bool p, I32 d, R b) {
  count_stq();
  FPUm(0xd902 | (p ? 1 : 0), d, b);
  asm_output("fst%s32 %d(%s)", (p ? "p" : ""), d, gpn(b));
  if (p)
    fpu_pop();
}
inline void Assembler::FSTQ(bool p, I32 d, R b) {
  count_stq();
  FPUm(0xdd02 | (p ? 1 : 0), d, b);
  asm_output("fst%sq %d(%s)", (p ? "p" : ""), d, gpn(b));
  if (p)
    fpu_pop();
}
...
inline void Assembler::FLD32(I32 d, R b) {
  count_ldq();
  FPUm(0xd900, d, b);
  asm_output("fld32 %d(%s)", d, gpn(b));
  fpu_push();
}
inline void Assembler::FLDQ(I32 d, R b) {
  count_ldq();
  FPUm(0xdd00, d, b);
  asm_output("fldq %d(%s)", d, gpn(b));
  fpu_push();
}
inline void Assembler::FLDQdm(const double *dm) {
  count_ldq();
  FPUdm(0xdd00, dm);
  asm_output("fldq (%p)", (void *)dm);
  fpu_push();
}
inline void Assembler::FILDQ(I32 d, R b) {
  count_fpuld();
  FPUm(0xdf05, d, b);
  asm_output("fildq %d(%s)", d, gpn(b));
  fpu_push();
}
inline void Assembler::FILD(I32 d, R b) {
  count_fpuld();
  FPUm(0xdb00, d, b);
  asm_output("fild %d(%s)", d, gpn(b));
  fpu_push();
}
...
inline void Assembler::FADD(I32 d, R b) {
  count_fpu();
  FPUm(0xdc00, d, b);
  asm_output("fadd %d(%s)", d, gpn(b));
}
inline void Assembler::FSUB(I32 d, R b) {
  count_fpu();
  FPUm(0xdc04, d, b);
  asm_output("fsub %d(%s)", d, gpn(b));
}
inline void Assembler::FSUBR(I32 d, R b) {
  count_fpu();
  FPUm(0xdc05, d, b);
  asm_output("fsubr %d(%s)", d, gpn(b));
}
inline void Assembler::FMUL(I32 d, R b) {
  count_fpu();
  FPUm(0xdc01, d, b);
  asm_output("fmul %d(%s)", d, gpn(b));
}
inline void Assembler::FDIV(I32 d, R b) {
  count_fpu();
  FPUm(0xdc06, d, b);
  asm_output("fdiv %d(%s)", d, gpn(b));
}
inline void Assembler::FDIVR(I32 d, R b) {
  count_fpu();
  FPUm(0xdc07, d, b);
  asm_output("fdivr %d(%s)", d, gpn(b));
}

inline void Assembler::FADDdm(const double *dm) {
  count_ldq();
  FPUdm(0xdc00, dm);
  asm_output("fadd (%p)", (void *)dm);
}
inline void Assembler::FSUBRdm(const double *dm) {
  count_ldq();
  FPUdm(0xdc05, dm);
  asm_output("fsubr (%p)", (void *)dm);
}
inline void Assembler::FMULdm(const double *dm) {
  count_ldq();
  FPUdm(0xdc01, dm);
  asm_output("fmul (%p)", (void *)dm);
}
inline void Assembler::FDIVRdm(const double *dm) {
  count_ldq();
  FPUdm(0xdc07, dm);
  asm_output("fdivr (%p)", (void *)dm);
}

inline void Assembler::FCOMP() {
  count_fpu();
  FPUc(0xD8D9);
  asm_output("fcomp");
  fpu_pop();
}
inline void Assembler::FCOMPP() {
  count_fpu();
  FPUc(0xDED9);
  asm_output("fcompp");
  fpu_pop();
...
inline void Assembler::FLDr(R r) {
  count_ldq();
  FPU(0xd9c0, r);
  asm_output("fld %s", gpn(r));
  fpu_push();
}
inline void Assembler::EMMS() {
  count_fpu();
  FPUc(0x0f77);
  asm_output("emms");
}
...
// Init per-opcode register hint table.  Defaults to no hints for all
// instructions (initialized to 0 )
...
void Assembler::nBeginAssembly() { max_stk_args = 0; }

NIns *Assembler::genPrologue() {
  // Prologue
  uint32_t stackNeeded =
      max_stk_args + STACK_GRANULARITY * _activation.stackSlotsNeeded();

  uint32_t stackPushed = STACK_GRANULARITY + // returnaddr
  ...
  verbose_only(asm_output("[frag entry]");) NIns *fragEntry = _nIns;
...
void Assembler::nFragExit(LIns *guard) {
  ...
  if (destKnown) {
    JMP(frag->fragEntry);
    lr = 0;
  } else { // Target doesn't exist. Jump to an epilogue for now. This can be
           // patched later.
    if (!_epilogue)
  ...
  asm_immi(rEAX, int(lr), /*canClobberCCs*/ true, /*blind*/ false);
}
...
static inline uint32_t compute_pushsize(const CallInfo *ci,
                                        uint32_t max_iregs) {
  ...
  if (f4args <= NJ_MAX_F4ARGS_IN_REGS) {
    if (indirect) {
      // target arg isn't pushed, its consumed in the call - i.e one less iarg
      // on stack (potentially)
      iargs--;
    }
    if (max_iregs > iargs)
      max_iregs = iargs;
    int32_t istack = iargs - max_iregs; // first 2 4B args are in registers, on
                                        // _fastcall calling convention
    return 4 * istack + 4 * fargs + 8 * dargs; // actual stack space used
  } else {
    /* this is tricky; since float4s on stack *must* be aligned, we can't just
     * add values - it depends on the parameter order */
    ArgType argTypes[MAXARGS];
    uint32_t argc = ci->getArgTypes(argTypes);
    if (argc && indirect) {
      // target arg isn't pushed, its consumed in the call - i.e ignore the last
      // argument when computing stack space
      argc--;
      NanoAssert(argTypes[argc] ==
                 ARGTYPE_P); // should be a pointer to the called function;
...
1. For int:
  ...
  - in FASTCALL, 2 parameters are passed in registers (ECX/EDX), the rest on
stack.
2. For float/double: in all calling conventions, they are transferred on stack.
3. Flor float4: the first three parameters of type __m128 are transferred in
registers XMM0 - XMM2. Any additional parameters of type __m128 are transferred
on the stack aligned by 16. This alignment is accomplished as follows: If there
are more than three parameters of type __m128 then the stack must be aligned by
16 before the call instruction. Consequently, the value of the stack pointer is
12 modulo 16 at the entry of the called function.
...
void Assembler::asm_call(LIns *ins) {
  if (!ins->isop(LIR_callv)) {
    // Float4 returned in XMM0; float, double returned in FST0; ints,pointers
    // returned in RAX
    Register rr = ins->isop(LIR_callf4)
                      ? XMM0
                      : ins->isop(LIR_callf) || ins->isop(LIR_calld)
                            ? FST0
                            : RegAlloc::retRegs[0];
  ...
  if (pushsize) {
    if (_config.i386_fixed_esp) {
      // In case of fastcall, stdcall and thiscall the callee cleans up the
      // stack, and since we reserve max_stk_args words in the prolog to call
      // functions and don't adjust the stack pointer individually for each call
      // we have to undo here any changes the callee just did to the stack.
      if (abi != ABI_CDECL)
        SUBi(SP, pushsize);
    } else {
      // stack re-alignment
  ...
  NanoAssert(ins->isop(LIR_callv) || ins->isop(LIR_callp) ||
             ins->isop(LIR_calld) || ins->isop(LIR_callf) ||
             ins->isop(LIR_callf4));
  if (!indirect) {
    CALL(call);
  } else {
  ...
  // Pre-assign registers to the first N 4B args based on the calling
  // convention.
  uint32_t n = 0;
  ...
  uint32_t nf4 = 0;
  /* Careful here!
     If we take the normal flow, the instructions that (temporarily) load
     float/double parameters in regs may inadvertently overwrite our float4
     registers. So we need to assign first the registers to float4 args, and
     only then assign the rest of the arguments
  */
...
bool canRematLEA(LIns *ins) {
  // We cannot rematerialize blinded integer literals, as the XOR
  // instruction used to synthesize the constant value may alter the CCs.
  // TODO: Contrary to the comment above, we can blind imm32 using LEA, which
  // does not affect the CCs.
...
bool RegAlloc::canRemat(LIns *ins) {
  // We cannot rematerialize blinded integer literals, as the XOR instruction
  // used to synthesize the constant value may alter the CCs.  See asm_restore()
  // below.
  // TODO: Contrary to the comment above, we can blind imm32 using LEA, which
  // does not affect the CCs.
  return (ins->isImmAny() &&
...
void Assembler::asm_restore(LIns *ins, Register r) {
  ...
  if (ins->isop(LIR_allocp)) {
    ...
    // We cannot rematerialize blinded integer literals, as the XOR
    // instruction used to synthesize the constant value may alter the CCs.
    // Rematerialization of other types of blinded literals is possible, because
    // we have provided for loading them from a pool.  We could likewise provide
    // a pool for integers, which would be more efficient than a spill/reload.
    ...
    asm_immf4(r, ins->immF4(), /*canClobberCCs*/ false, ins->isTainted());
             ...
             (arg = ins->paramArg()) >=
                 (abi_regcount = max_abi_regs[_thisfrag->lirbuf->abi])) {
...
void Assembler::asm_store32(LOpcode op, LIns *value, int dr, LIns *base,
  ...
  if (value->isImmI() && !(value->isTainted() && shouldBlind(value->immI()))) {
    ...
    if (base->opcode() == LIR_addp && !force &&
        !(tainted && shouldBlindDisplacement(dr))) {
      LIns *index;
      int scale;
      ...
      switch (op) {
      case LIR_sti2c:
        ST8isib(dr, rb, ri, scale, c);
        break;
      case LIR_sti2s:
        ST16isib(dr, rb, ri, scale, c);
        break;
      case LIR_sti:
        STisib(dr, rb, ri, scale, c);
        break;
      default:
        NanoAssert(0);
        break;
      ...
      // If the store is tainted and the displacement should be blinded,
      // we forgo folding the scaling into the store, because we need to
      // adjust both the final scaled address in its register as well as
      // the immediate displacement field in the instruction.
      int c = value->immI();
      Register orb;
      Register rb =
          getBaseRegWithBlinding(base, dr, GpRegs, tainted, force, &orb);
      switch (op) {
      case LIR_sti2c:
        ST8i(rb, dr, c);
        break;
    ...
    if (base->opcode() == LIR_addp && !force &&
      ...
      switch (op) {
      case LIR_sti2c:
        ST8sib(dr, rb, ri, scale, rv);
        break;
      case LIR_sti2s:
        ST16sib(dr, rb, ri, scale, rv);
        break;
      case LIR_sti:
        STsib(dr, rb, ri, scale, rv);
        break;
      default:
        NanoAssert(0);
        break;
      ...
      if (base->isImmI()) {
        ...
        Register orb;
        getBaseReg2WithBlinding(SrcRegs, value, rv, GpRegs, base, rb, dr,
                                tainted, force, &orb);
        switch (op) {
        case LIR_sti2c:
          ST8(rb, dr, rv);
...
void Assembler::asm_spill(Register rr, int d, bool pop, int8_t nWords) {
  ...
  if (rmask(rr) & GpRegs) {
    ...
    case 2:
      ...
      break;
    default:
      NanoAssertMsgf(false, "nWords is %d, expected 1,2,or 4",
                     nWords); // fall thru
    case 1:
      SSE_STSS(d, FP, rr);
    }
...
void Assembler::asm_load128(LIns *ins) {
  ...
  if (base->opcode() == LIR_addp && !force &&
      !(ins->isTainted() && shouldBlindDisplacement(d))) {
    LIns *index;
    int scale;
    getBaseIndexScale(base, &base, &index, &scale);
    ...
    Register orb;
    Register rb =
        getBaseRegWithBlinding(base, d, GpRegs, ins->isTainted(), force, &orb);
    SSE_LDUPS(rr, d, rb);
    SSE_XORPDr(rr, rr);
    adjustBaseRegForBlinding(rb, orb);
  }
...
void Assembler::asm_load64(LIns *ins) {
  ...
  if (ins->isInReg()) {
    ...
    if (base->opcode() == LIR_addp && (rmask(rr) & XmmRegs) && !force &&
        !(ins->isTainted() && shouldBlindDisplacement(d))) {
      LIns *index;
      int scale;
      getBaseIndexScale(base, &base, &index, &scale);
      ...
      // or 'ins' with the result because it has a different type.
      Register orb;
      Register rb = getBaseRegWithBlinding(base, d, GpRegs, ins->isTainted(),
                                           force, &orb);
      if (rmask(rr) & XmmRegs) {
        switch (ins->opcode()) {
        case LIR_ldd:
    ...
    case LIR_ldd:
      if (_config.i386_sse2) {
        ...
        Register rb = getBaseRegWithBlinding(base, d, GpRegs, ins->isTainted(),
                                             force, &orb);
        NanoAssert((GpRegs & XmmRegs) ==
                   0); // correctness of allocTempReg below relies on this
        ...
        if (force || (ins->isTainted() && shouldBlindDisplacement(d))) {
          d += _blindMask32;
          // We avoid copying via the FP stack because it's slow and likely to
          // cause spills.
          Register t, trb;
          _allocator.allocTempReg2(GpRegs & ~rmask(rb), t, GpRegs & ~rmask(rb),
                                   trb);
          ST(FP, dr + 4, t);
          LD(t, d + 4, trb);
          ST(FP, dr, t);
          ...
          Register t = _allocator.allocTempReg(GpRegs & ~rmask(rb));
          ST(FP, dr + 4, t);
          LD(t, d + 4, rb);
          ST(FP, dr, t);
          LD(t, d, rb);
...
// NOTE: Since float and double constants will almost certainly originate in
// user-controlled literals, the attempt to optmize their treatment below will
// almost assuredly be ineffective when constant blinding is used.

void Assembler::asm_store64(LOpcode op, LIns *value, int d, LIns *base,
                            bool tainted) {
  bool force = forceDisplacementBlinding(tainted);
  if (op == LIR_std2f ||
      (op == LIR_stf && (!value->isImmF() || value->isInReg()))) {
    bool pop = !value->isInReg();
    Register rv = (pop ? findRegFor(value, _config.i386_sse2 ? XmmRegs : FpRegs)
                       : value->getReg());
    Register orb;
    Register rb = getBaseRegWithBlinding(base, d, GpRegs, tainted, force, &orb);
    ...
    adjustBaseRegForBlinding(rb, orb);
  } else if (value->isImmD() && !value->isTainted()) {
    // If constant value should be blinded, fall through to the general case,
    // which will load it into a register from a literal pool.
    force =
        force || tainted; // If the store is tainted, and we are not going to
                          // blind the immediate, then blind the displacement.
    Register orb;
    ...
    adjustBaseRegForBlinding(rb, orb);
  } else if (value->isImmF() && !value->isTainted()) {
    // If constant value should be blinded, fall through to the general case,
    // which will load it into a register from a literal pool.]
    force =
        force || tainted; // If the store is tainted, and we are not going to
    ...
    adjustBaseRegForBlinding(rb, orb);
  } else if (base->opcode() == LIR_addp && _config.i386_sse2 && !force &&
             !(tainted && shouldBlindDisplacement(d))) {
    LIns *index;
    int scale;
    getBaseIndexScale(base, &base, &index, &scale);
    ...
    Register rv =
        value->isInReg() ? value->getReg() : findRegFor(value, XmmRegs);
    ...
    if (force || (tainted && shouldBlindDisplacement(d))) {
      d += _blindMask32;
      // We avoid copying via the FP stack because it's slow and likely to cause
      // spills. see asm_mmq
      Register t, trb;
      _allocator.allocTempReg2(GpRegs & ~rmask(rb), t, GpRegs & ~rmask(rb),
                               trb);
...
void Assembler::asm_store128(LOpcode op, LIns *value, int d, LIns *base,
  ...
  Register b = getBaseRegWithBlinding(base, d, GpRegs, tainted, force, &ob);
  SSE_STUPS(d, b, r);
...
Branches Assembler::asm_branch_helper(bool branchOnFalse, LIns *cond,
                                      NIns *targ) {
...
Branches Assembler::asm_branchi_helper(bool branchOnFalse, LIns *cond,
                                       NIns *targ) {
  if (branchOnFalse) {
    // op == LIR_xf/LIR_jf
    switch (cond->opcode()) {
...
void Assembler::asm_cmpi(LIns *cond) {
  ...
  if (rhs->isImmI() && !(rhs->isTainted() && shouldBlind(rhs->immI()))) {
    int c = rhs->immI();
    // findRegFor() can call asm_restore() -- asm_restore() better not disturb
    // the CCs!
    ...
    if (c == 0 && cond->isop(LIR_eqi)) {
      ...
      if (canSkipTest) {
        ...
        canSkipTest = currIns == lookahead.read() && cond == lookahead.read() &&
                      lhs == lookahead.read();
      }
      if (canSkipTest) {
...
void Assembler::asm_arith(LIns *ins) {
  ...
  if (op == LIR_addi && lhs->isop(LIR_allocp) && rhs->isImmI() &&
    ...
    // We assume that the address returned by findMemFor() is predictable,
    // and thus disallow this optimization for tainted constants.
  ...
  case LIR_rshui:
    isConstRhs =
        rhs->isImmI() && !(rhs->isTainted() && shouldBlind(rhs->immI()));
    if (!isConstRhs) {
  ...
  case LIR_ori:
  case LIR_xori:
    // For these operations, we can blind an immediate RHS operand without
    // an extra register, so we won't allocate one if the RHS is a constant.
  ...
  if (!isConstRhs) {
    ...
    switch (op) {
    case LIR_lshi:
      NanoAssert(!rhs->isTainted() || !shouldBlind(rhs->immI()));
      SHLi(rr, c);
      break;
    case LIR_rshi:
      NanoAssert(!rhs->isTainted() || !shouldBlind(rhs->immI()));
    ...
    case LIR_subxovi:
      ...
      // in the non-blinded case, so we must be careful to use asm_arith()
      // only in cases where the CC's are not required.  The ADDi->LEA
      // optimization has already imposed this constraint, however.

    case LIR_addi:
      if (rhs->isTainted() && shouldBlind(c)) {
        ADDi(rr, _blindMask32);
        LEA(rr, c - _blindMask32, ra);
      } else {
    ...
    case LIR_subi:
      if (rhs->isTainted() && shouldBlind(c)) {
        SUBi(rr, _blindMask32);
        SUBi(rr, c - _blindMask32);
      } else {
        SUBi(rr, c);
      }
      break;
    case LIR_andi:
      if (rhs->isTainted() && shouldBlind(c)) {
        ANDi(rr, (c & _blindMask32) | ~_blindMask32);
        ANDi(rr, (c & ~_blindMask32) | _blindMask32);
      } else {
        ANDi(rr, c);
      }
    ...
    case LIR_xori:
      if (rhs->isTainted() && shouldBlind(c)) {
        XORi(rr, _blindMask32);
        XORi(rr, c ^ _blindMask32);
      } else {
        XORi(rr, c);
      }
      break;
...
void Assembler::asm_div_mod(LIns *mod) {
  ...
  // LIR_modi expects the LIR_divi to be near (no interference from the register
  // allocator).
...
void Assembler::asm_load32(LIns *ins) {
  ...
  if (base->isImmI() && !force &&
      !(ins->isTainted() && shouldBlindDisplacement(d))) {
    // We assume that the base address is never tainted, but might be
    // predictable.
    NanoAssert(!base->isTainted());
  ...
  } else if (base->opcode() == LIR_addp && !force &&
             !(ins->isTainted() && shouldBlindDisplacement(d))) {
    LIns *index;
    int scale;
    getBaseIndexScale(base, &base, &index, &scale);
    ...
    Register rb, ri;
    // @todo -- If base and/or index is const, we could eliminate a register
    // use.
    if (!base->isInReg()) {
      rb = rr;
    ...
    switch (op) {
    case LIR_lduc2ui:
      LD8Zsib(rr, d, rb, ri, scale);
      break;
    case LIR_ldc2i:
      LD8Ssib(rr, d, rb, ri, scale);
      break;
    case LIR_ldus2ui:
      LD16Zsib(rr, d, rb, ri, scale);
      break;
    case LIR_lds2i:
      LD16Ssib(rr, d, rb, ri, scale);
      break;
    case LIR_ldi:
      LDsib(rr, d, rb, ri, scale);
      break;
    default:
      NanoAssert(0);
      break;
    ...
    Register ora;
    Register ra = getBaseRegWithBlindingUsingSpecifiedTemp(
        base, d, GpRegs, ins->isTainted(), force, &ora, rr);
    switch (op) {
    case LIR_lduc2ui:
      LD8Z(rr, d, ra);
      break;
    ...
    adjustBaseRegForBlinding(ra, ora);
...
void Assembler::asm_cmov(LIns *ins) {
  ...
  if (!_config.i386_sse2 && (ins->isop(LIR_cmovd) || ins->isop(LIR_cmovf))) {
    ...
    if (iffalse->isImmD()) {
      asm_immd(FST0, iffalse->immDasQ(), iffalse->immD(),
               /*canClobberCCs*/ false, iffalse->isTainted());
    } else if (iffalse->isImmF()) {

      asm_immf(FST0, iffalse->immFasI(), iffalse->immF(),
               /*canClobberCCs*/ false, iffalse->isTainted());
    } else {
  ...
  RegisterMask allow =
      (ins->isD() || ins->isF() || ins->isF4()) ? XmmRegs : GpRegs;
  Register rr = prepareResultReg(ins, allow);
...
void Assembler::asm_param(LIns *ins) {
  ...
  if (kind == 0) {
    ...
    NanoAssert(abi_regcount <=
               sizeof(RegAlloc::argRegs) / sizeof(RegAlloc::argRegs[0]));
    if (arg < abi_regcount) {
...
void Assembler::asm_immi(Register r, int32_t val, bool canClobberCCs,
  ...
  if (val == 0 && canClobberCCs) {
    XOR(r, r);
  } else if (blind && shouldBlind(val)) {
    NanoAssert(canClobberCCs);
    XORi(r, _blindMask32);
    LDi(r, val ^ _blindMask32);
  } else {
    LDi(r, val);
  }
}
...
void Assembler::asm_immf(Register r, int32_t i, float f, bool canClobberCCs,
  ...
  // be rematerialized from const data (or inline instructions in the special
  // cases). Special double values (zero, one) and those loaded from pools need
  // not be blinded.
  ...
  if (rmask(r) & XmmRegs) {
    if (i == 0) {
      ...
      SSE_CVTSI2SS(r, tr);
      // zero r to ensure no dependency stalls
      // pedantic: should be XORPSr; but XORPDr shoudl work too.
      SSE_XORPDr(r, r);
      // Blinding of an integer is possible because we can clobber CCs,
      // though it may be worth determining whether loading from a pool
      // might be faster when constant blinding is used.
      asm_immi(tr, (int)f, true, blind);
    } else {
      const uint64_t *p = findImmDFromPool(i);
      LDSSm(r, (const float *)
                   p); // ok to cast to "float*" since i386 is little-endian
    }
  } else {
    NanoAssert(r == FST0);
    if (i == 0) {
...
void Assembler::asm_immf4(Register r, const float4_t &f4, bool canClobberCCs,
                          bool blind) {
  (void)blind; // Float4 values are always loaded from a pool.
  NanoAssert(_config.i386_sse2);
  (void)canClobberCCs;
...
void Assembler::asm_immf4(LIns *ins) {
  ...
  if (ins->isInReg()) {
    Register rr = ins->getReg();
    NanoAssert(rmask(rr) & XmmRegs);
    asm_immf4(rr, ins->immF4(), /*canClobberCCs*/ true, ins->isTainted());
  } else {
    // Do nothing, will be rematerialized when necessary.
  }
...
// From Sun Studio C++ Readme: #pragma align inside namespace requires mangled
// names. Initialize here to avoid multithreading contention issues during
// initialization. each array must be big enough to hold a 128 bit 16-byte
// aligned value inside. static writable arrays start as zerod memory.
...
static uint32_t *initMaskD(uint32_t mask_temp[], uint32_t upper,
                           uint32_t lower) {
...
static const AVMPLUS_ALIGN16(uint32_t) negateMaskF4[] = {
    0x80000000, 0x80000000, 0x80000000, 0x80000000};
static const AVMPLUS_ALIGN16(uint32_t) absMaskD[] = {0xFFFFFFFF, 0x7FFFFFFF, 0,
                                                     0};
static const AVMPLUS_ALIGN16(uint32_t) absMaskF[] = {0x7FFFFFFF, 0, 0, 0};
static const AVMPLUS_ALIGN16(uint32_t) absMaskF4[] = {0x7FFFFFFF, 0x7FFFFFFF,
                                                      0x7FFFFFFF, 0x7FFFFFFF};
static const AVMPLUS_ALIGN16(uint32_t) onesMaskF4[] = {0x3F800000, 0x3F800000,
...
void Assembler::asm_neg_abs(LIns *ins) {
  ...
  if (_config.i386_sse2) {
    ...
    if (ins->retType() == LTy_D) {
      switch (ins->opcode()) {
      default:
        NanoAssert(!"bad opcode");
      case LIR_negd:
        SSE_XORPD(rr, negateMaskD);
        break;
      case LIR_absd:
        SSE_ANDPD(rr, absMaskD);
        break;
      }
      if (rr != ra)
        SSE_MOVSD(rr, ra);
      ...
      default:
        NanoAssert(!"bad opcode");
      case LIR_negf:
        SSE_XORPS(rr, negateMaskF);
        break;
      case LIR_negf4:
        SSE_XORPS(rr, negateMaskF4);
        break;
      case LIR_absf:
        SSE_ANDPS(rr, absMaskF);
        break;
      case LIR_absf4:
        SSE_ANDPS(rr, absMaskF4);
        break;
      }
      if (rr != ra)
        SSE_MOVAPS(rr, ra);
  ...
  if (ty == ARGTYPE_I || ty == ARGTYPE_UI) {
    if (r != UnspecifiedReg) {
      if (ins->isImmI()) {
        ...
        asm_immi(r, ins->immI(), /*canClobberCCs*/ true, ins->isTainted());
      } else if (ins->isInReg()) {
        if (r != ins->getReg())
          MR(r, ins->getReg());
      } else if (ins->isInAr()) {
...
void Assembler::asm_pusharg(LIns *ins) {
  ...
  if (!ins->isExtant() && ins->isImmI() &&
      !(ins->isTainted() && shouldBlind(ins->immI()))) {
...
void Assembler::asm_farg(LIns *ins, int32_t &stkd) {
  ...
  if (rmask(r) & XmmRegs) {
    ...
    // It's possible that the same LIns* with r=FST0 will appear in the argument
    // list more than once.  In this case FST0 will not have been evicted and
    // the multiple pop actions will unbalance the FPU stack.  A quick fix is to
    // always evict FST0 manually.
    NanoAssert(r == FST0);
...
void Assembler::asm_fop(LIns *ins) {
  ...
  if (_config.i386_sse2) {
    ...
    switch (op) {
    default:
      NanoAssertMsgf(false, "Unhandled op %d", op);
    case LIR_addd:
      SSE_ADDSD(rr, rb);
      break;
    case LIR_subd:
    ...
    case LIR_dotf2: {
      ...
      // dotf2 = ax*bx + ay*by
      static const uint8_t masks[] = {/*dot4*/ 0xF1, /*dot3*/ 0x71,
                                      /*dot2*/ 0x31};
    ...
    if (rhs->isImmD()) {
      // We do not need to blind the literal, because it is loaded from a pool.
      ...
      switch (op) {
      case LIR_addf:
        FADD32dm((const float *)p);
        break;
      case LIR_subf:
        FSUBR32dm((const float *)p);
        break;
      case LIR_mulf:
        FMUL32dm((const float *)p);
        break;
      case LIR_divf:
        FDIVR32dm((const float *)p);
        break;
      default:
        NanoAssert(0);
      }
...
RegisterMask RegAlloc::nRegCopyCandidates(Register r, RegisterMask allow) {
  if (rmask(r) & GpRegs)
    return allow & GpRegs; // theoretically we could allow XmmRegs too, but
                           // asm_nongp_copy is not done.

  if (rmask(r) & XmmRegs) // we can't allow GpRegs, even though nongp_copy
                          // works, because XMM may need the full 128bits
...
Branches Assembler::asm_branchd_helper(bool branchOnFalse, LIns *cond,
                                       NIns *targ) {
  NIns *patch1 = NULL;
  NIns *patch2 = NULL;
    ...
    if (branchOnFalse) {
      ...
      case LIR_eqd:
        if (cond->oprnd1() == cond->oprnd2()) {
          ...
          // skip: ...
          underrunProtect(
              16); // underrun of 7 needed but we write 2 instr --> 16
          NIns *skip = _nIns;
          JE(targ);
          patch1 = _nIns;
          JP(skip); // unordered
...
void Assembler::asm_cmpd(LIns *cond) {
  ...
  NanoAssert(singlePrecision ? (lhs->isF() && rhs->isF())
                             : (lhs->isD() && rhs->isD()));
  condop = singlePrecision
               ? getCmpDOpcode(condop)
               : condop; // force double precision, to simplify
                         // testing/conditions. Will revert the opcode to
                         // single-precision just before using it
  ...
  if (_config.i386_sse2) {
    ...
    if (lhs == rhs) {
      ...
      FNSTSW_AX(); // requires rEAX to be free
      if (rhs->isImmAny()) {
        // We do not need to blind the literal, because it is loaded from a
        // pool.
        const uint64_t *p;
        if (singlePrecision) {
          ...
          u.d = rhs->immF(); //  convert to double, to avoid adding a
                             //  findImmFFromPool + a FCOMdm32 instruction
...
void Assembler::asm_ffff2f4(LIns *ins) {
  ...
  freeResourcesOf(ins);
  Register rx = x->isInReg() ? findRegFor(x, XmmRegs)
...
void Assembler::asm_f4comp(LIns *ins) {
  ...
  case LIR_f4x:
    ...
    break;
  case LIR_f4y:
    SSE_PSHUFD(rr, rb, PSHUFD_COMPONENT_MASK(1));
    break;
  case LIR_f4z:
    SSE_PSHUFD(rr, rb, PSHUFD_COMPONENT_MASK(2));
    break;
  case LIR_f4w:
    SSE_PSHUFD(rr, rb, PSHUFD_COMPONENT_MASK(3));
    break;
  case LIR_swzf4: {
...
void Assembler::underrunProtect(int n) {
  NIns *eip = _nIns;
  NanoAssertMsg(n <= LARGEST_UNDERRUN_PROT,
  ...
  if (eip - n < codeStart) {
    ...
    JMP(eip);
    if (_mdWriter)
      _mdWriter->setNativePc((uint8_t *)eip);
  }
}
