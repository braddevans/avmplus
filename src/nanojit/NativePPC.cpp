...
NIns *Assembler::genPrologue() {
  ...
  uint32_t param_area = (max_param_size > min_param_area_size)
                            ? max_param_size
                            : min_param_area_size;
  ...
  uint32_t stackNeeded =
      param_area + linkage_size + _activation.stackSlotsNeeded() * 4;
...
void Assembler::asm_load64(LIns *ins) {
  ...
  if (ins->opcode() == LIR_ldf2d) {
    NanoAssertMsg(0, "NJ_EXPANDED_LOADSTORE_SUPPORTED not yet supported for "
                     "this architecture");
    return;
  }
  if (rmask(rr) & GpRegs) {
...
void Assembler::asm_store64(LOpcode op, LIns *value, int32_t dr, LIns *base) {
  ...
  Register rs = (!value->isInReg() ? findRegFor(value, GpRegs & ~rmask(ra))
...
void Assembler::asm_cond(LIns *ins) {
  ...
  case LIR_led:
    CASE64(LIR_ltq:)
    CASE64(LIR_ltuq:) EXTRWI(r, r, 1, 4 * cr + COND_lt); // extract CR0.lt
    MFCR(r);
    break;
  case LIR_gti:
  case LIR_gtui:
  ...
  case LIR_ged:
    CASE64(LIR_gtq:)
    CASE64(LIR_gtuq:) EXTRWI(r, r, 1, 4 * cr + COND_gt); // extract CR0.gt
    MFCR(r);
    break;
  case LIR_lei:
  case LIR_leui:
    CASE64(LIR_leq:)
    CASE64(LIR_leuq:) EXTRWI(r, r, 1, 4 * cr + COND_eq); // extract CR0.eq
    MFCR(r);
    CROR(CR0, eq, lt, eq);
    break;
  case LIR_gei:
  case LIR_geui:
    CASE64(LIR_geq:)
    CASE64(LIR_geuq:) EXTRWI(r, r, 1, 4 * cr + COND_eq); // select CR0.eq
    MFCR(r);
    CROR(CR0, eq, gt, eq);
    break;
  default:
    debug_only(outputf("%s", lirNames[ins->opcode()]);) TODO(asm_cond);
    break;
  }
  SwapEnable();
  asm_cmp(op, a, b, cr);
...
void Assembler::asm_condd(LIns *ins) { asm_cond(ins); }
...
NIns *Assembler::asm_branch_near(bool onfalse, LIns *cond, NIns *const targ) {
  ...
  if (!isS14(bd)) {
    ...
    if (isS24(bd)) {
      ...
      verbose_only(verbose_outputf("%p:", _nIns);) NIns *skip = _nIns;
      NOP();
      NOP(); // two only -- we have the branch too
      B(bd);
  ...
  // S14 branch
  verbose_only(verbose_outputf("branch14");) if (!patch) {
    NOP();
    NOP();
    NOP(); // to be patched later if needed
  }
  ...
  case LIR_ltd:
  case LIR_led:
    CASE64(LIR_ltq:) CASE64(LIR_ltuq:) if (onfalse) BNL(cr, bd, likely);
    else BLT(cr, bd, likely);
    break;
  case LIR_lei:
  case LIR_leui:
    CASE64(LIR_leq:) CASE64(LIR_leuq:) if (onfalse) BGT(cr, bd, likely);
    else BLE(cr, bd, likely);
    break;
  case LIR_gti:
  case LIR_gtui:
  case LIR_gtd:
  case LIR_ged:
    CASE64(LIR_gtq:) CASE64(LIR_gtuq:) if (onfalse) BNG(cr, bd, likely);
    else BGT(cr, bd, likely);
    break;
  case LIR_gei:
  case LIR_geui:
    CASE64(LIR_geq:) CASE64(LIR_geuq:) if (onfalse) BLT(cr, bd, likely);
    else BGE(cr, bd, likely);
    break;
  default:
    debug_only(outputf("%s", lirNames[cond->opcode()]);) TODO(unknown_cond);
  }
  if (!patch)
    patch = _nIns;
  SwapEnable();
...
NIns *Assembler::asm_branch_far(bool onfalse, LIns *cond, NIns *const targ) {
  ...
  case LIR_ltui:
    CASE64(LIR_ltq:)
    CASE64(LIR_ltuq:) case LIR_ltd : case LIR_led : if (onfalse)
                                                        BNLCTR(cr, likely);
    else BLTCTR(cr, likely);
    break;
  case LIR_lei:
  case LIR_leui:
    CASE64(LIR_leq:) CASE64(LIR_leuq:) if (onfalse) BGTCTR(cr, likely);
    else BLECTR(cr, likely);
    break;
  case LIR_gti:
  case LIR_gtui:
    CASE64(LIR_gtq:)
    CASE64(LIR_gtuq:) case LIR_gtd : case LIR_ged : if (onfalse)
                                                        BNGCTR(cr, likely);
    else BGTCTR(cr, likely);
    break;
  case LIR_gei:
  case LIR_geui:
    CASE64(LIR_geq:) CASE64(LIR_geuq:) if (onfalse) BLTCTR(cr, likely);
    else BGECTR(cr, likely);
    break;
  default:
    debug_only(outputf("%s", lirNames[condop]);) TODO(unknown_cond);
  }
...
void Assembler::asm_cmp(LOpcode condop, LIns *a, LIns *b,
...
void Assembler::asm_arith(LIns *ins) {
  ...
  if (rhs->isImmI() && op != LIR_mulxovi && op != LIR_muljovi &&
      op != LIR_addxovi && op != LIR_addjovi && op != LIR_subjovi &&
      op != LIR_subxovi) {
...
#endif
  default:
    debug_only(outputf("%s", lirNames[op]);) TODO(asm_arith);
  }
}
...
void Assembler::asm_fop(LIns *ins) {
  ...
  case LIR_divd:
    ...
    break;
  default:
    debug_only(outputf("%s", lirNames[op]);) TODO(asm_fop);
  }
}
...
#if defined NANOJIT_64BIT
// XXX: this is sub-optimal, see
// https://bugzilla.mozilla.org/show_bug.cgi?id=540368#c7.
...
void Assembler::asm_dasq(LIns *) { TODO(asm_dasq); }

void Assembler::asm_qasd(LIns *) { TODO(asm_qasd); }
...
void Assembler::br_far(NIns *addr, int link) {
  ...
  // to the register allocator, and we use R0 everywhere else as scratch, so
  // using R2 here avoids clobbering anything else besides CTR.
...
void Assembler::underrunProtect(int bytes) {
  ...
  // it's just the next instruction and the only effect is to clobber R2 & CTR
...
void Assembler::nBeginAssembly() { max_param_size = 0; }
...
static inline void fast_flush_icache2(NIns *branch) {
  register uint32_t j = (uint32_t)branch;
  register uint32_t z = 0;
  asm("sync\n\t" ICBI INCI ICBI "isync\n\t" : : "r"(z), "r"(j));
}

static inline void fast_flush_icache4(NIns *branch) {
  register uint32_t j = (uint32_t)branch;
  register uint32_t z = 0;
  asm("sync\n\t" ICBI INCI ICBI INCI ICBI INCI ICBI "isync\n\t"
      :
      : "r"(z), "r"(j));
}
...
static void demote_to_far_branch(NIns *branch, NIns *target) {
  ...
  branch[0] =
      PPC_addis | GPR(rd) << 21 | uint16_t(imm >> 16); // lis rd, imm >> 16
  branch[1] = PPC_ori | GPR(rd) << 21 | GPR(rd) << 16 |
              uint16_t(imm); // ori rd, rd, imm & 0xffff
...
void Assembler::nPatchBranch(NIns *branch, NIns *target) {
  ...
  if ((branch[0] & (63 << 25)) == PPC_b) {
    ...
    // jump would be (lis,ori,mtctr,bcctr) and we'd be patching the lis,ori
    // below.
    if (!isS14(bd)) {
      demote_to_far_branch(branch, target);
      return;
