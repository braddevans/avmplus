...
static const Register deprecated_UnknownReg = {
...
#endif
{
  ...
  X64_cmovqno =
      0xC0410F4800000004LL, // 64bit conditional mov if (no overflow) r = b
  X64_cmovqnae =
      0xC0420F4800000004LL, // 64bit conditional mov if (uint <)  r = b
  X64_cmovqnb =
      0xC0430F4800000004LL, // 64bit conditional mov if (uint >=) r = b
  X64_cmovqne =
      0xC0450F4800000004LL, // 64bit conditional mov if (c)       r = b
  X64_cmovqna =
      0xC0460F4800000004LL, // 64bit conditional mov if (uint <=) r = b
  X64_cmovqnbe =
      0xC0470F4800000004LL, // 64bit conditional mov if (uint >)  r = b
  X64_cmovqnge =
      0xC04C0F4800000004LL, // 64bit conditional mov if (int <)   r = b
  X64_cmovqnl =
      0xC04D0F4800000004LL, // 64bit conditional mov if (int >=)  r = b
  X64_cmovqng =
      0xC04E0F4800000004LL, // 64bit conditional mov if (int <=)  r = b
  X64_cmovqnle =
      0xC04F0F4800000004LL, // 64bit conditional mov if (int >)   r = b
  X64_cmovno =
      0xC0410F4000000004LL, // 32bit conditional mov if (no overflow) r = b
  X64_cmovnae =
      0xC0420F4000000004LL,          // 32bit conditional mov if (uint <)  r = b
  ...
  X64_cmovnbe =
      0xC0470F4000000004LL, // 32bit conditional mov if (uint >)  r = b
  X64_cmovnge =
      0xC04C0F4000000004LL,          // 32bit conditional mov if (int <)   r = b
  ...
  X64_cmovnle =
      0xC04F0F4000000004LL,         // 32bit conditional mov if (int >)   r = b
  X64_cmplr = 0xC03B400000000003LL, // 32bit compare r,b
  X64_cmpqr = 0xC03B480000000003LL, // 64bit compare r,b
  X64_cmppsr =
      0xC0C20F4000000004LL, // 128bit compare r,b; requires an immediate to
                            // specify what kind of comparison
  ...
  X64_cvtsd2si = 0xC02D0F40F2000005LL, // convert double to int32 with rounding
                                       // r = (int32) b
  ...
  X64_divps = 0xC05E0F4000000004LL, // divide float4 vector single-precision
                                    // r[i] /= b[i]
  X64_mulps = 0xC0590F4000000004LL, // multiply float4 vector single-precision
                                    // r[i] *= b[i]
  X64_addps =
      0xC0580F4000000004LL, // add float4 vector single-precision r[i] += b[i]
  X64_idiv =
      0xF8F7400000000003LL, // 32bit signed div (rax = rdx:rax/r, rdx=rdx:rax%r)
  ...
  X64_jneg8 =
      0x0001000000000000LL, // xor with this mask to negate the condition
  X64_leaqrm =
      0x00000000808D4807LL, // 64bit load effective addr reg <- disp32+base
  X64_lealrm =
      0x00000000808D4007LL, // 32bit load effective addr reg <- disp32+base
  X64_learip = 0x00000000058D4807LL, // 64bit RIP-relative lea. reg <-
                                     // disp32+rip (modrm = 00rrr101 = 05)
  ...
  X64_movqspr =
      0x0024448948000005LL, // 64bit store gpr -> [rsp+d8] (sib required)
  X64_movqspx =
      0x002444110F40F207LL, // 64bit store xmm -> [rsp+d8] (sib required)
  ...
  X64_movqrx = 0xC07E0F4866000005LL, // 64bit mov b <- xmm-r (reverses the usual
                                     // r/b order)
  ...
  X64_movsdrr =
      0xC0100F40F2000005LL, // 64bit mov xmm-r <- xmm-b (upper 64bits unchanged)
  X64_movupsrm = 0x80100F4000000004LL,  // 128bit load xmm-r <- [b+d32]
  X64_movupspr = 0x2484110F48000005LL,  // 128bit float store xmm -> [rsp+d32]
                                        // (sib required)
  ...
  X64_movlhps = 0xC0160F4000000004LL,   // 64bit mov r[64:127] <- l[0:63] (the
                                        // rest unmodified)
  X64_pmovmskb = 0xC0D70F4066000005LL,  // move byte mask, r = (first bit from
                                        // every byte of xmm)
  X64_movsdrm =
      0x80100F40F2000005LL, // 64bit load xmm-r <- [b+d32] (upper 64 cleared)
  X64_movsdmr = 0x80110F40F2000005LL, // 64bit store xmm-r -> [b+d32]
  X64_movssrm =
      0x80100F40F3000005LL, // 32bit load xmm-r <- [b+d32] (upper 96 cleared)
  X64_movssmr = 0x80110F40F3000005LL, // 32bit store xmm-r -> [b+d32]
  X64_movssspr =
      0x2484110F48F30006LL, // 32bit float store xmm -> [rsp+d32] (sib required)
  X64_movsdspr =
      0x2484110F48F20006LL, // 64bit float store xmm -> [rsp+d32] (sib required)
  X64_movsxdr =
      0xC063480000000003LL, // sign extend i32 to i64 r = (int64)(int32) b
  X64_movzx8 =
      0xC0B60F4000000004LL, // zero extend i8 to i64 r = (uint64)(uint8) b
  X64_movzx8m = 0x80B60F4000000004LL, // zero extend i8 load to i32 r <- [b+d32]
  X64_movzx16m =
      0x80B70F4000000004LL, // zero extend i16 load to i32 r <- [b+d32]
  X64_movsx8m = 0x80BE0F4000000004LL, // sign extend i8 load to i32 r <- [b+d32]
  X64_movsx16m =
      0x80BF0F4000000004LL,          // sign extend i16 load to i32 r <- [b+d32]
  ...
  X64_setle = 0xC09E0F4000000004LL, // set byte if less or equal (int <=) (ZF ==
                                    // 1 || SF != OF)
  X64_setg =
      0xC09F0F4000000004LL, // set byte if greater (int >) (ZF == 0 && SF == OF)
  X64_setge =
      0xC09D0F4000000004LL, // set byte if greater or equal (int >=) (SF == OF)
  X64_seta =
      0xC0970F4000000004LL, // set byte if above (uint >) (CF == 0 && ZF == 0)
  X64_setae =
      0xC0930F4000000004LL, // set byte if above or equal (uint >=) (CF == 0)
  X64_setb = 0xC0920F4000000004LL,   // set byte if below (uint <) (CF == 1)
  X64_setbe = 0xC0960F4000000004LL,  // set byte if below or equal (uint <=) (ZF
                                     // == 1 || CF == 1)
  ...
  X64_subps = 0xC05C0F4000000004LL,  // subtract float4 vector single-precision
                                     // r[i] -= b[i]
  ...
  X64_ucomiss =
      0xC02E0F4000000004LL, // unordered compare scalar single-precision float
  ...
  X64_xorps = 0xC0570F4000000004LL, // 128bit xor xmm (four packed singles), one
                                    // byte shorter
  ...
  X64_movqmi = 0x80C7480000000003LL, // 32bit signed extended to 64-bit store
                                     // imm -> qword ptr[b+disp32]
  ...
  X86_setnp = 0xC09B0F0000000003LL  // no-rex set byte if odd parity (ordered
                                    // fcmp result) (PF == 0)
};
...
static const RegisterMask SavedRegs =
    1 << REGNUM(RBX) | 1 << REGNUM(RSI) | 1 << REGNUM(RDI) | 1 << REGNUM(R12) |
    1 << REGNUM(R13) | 1 << REGNUM(R14) | 1 << REGNUM(R15);
static const int NumSavedRegs = 7; // rbx, rsi, rdi, r12-15
static const int NumArgRegs = 4;
#else
static const RegisterMask SavedRegs = 1 << REGNUM(RBX) | 1 << REGNUM(R12) |
                                      1 << REGNUM(R13) | 1 << REGNUM(R14) |
                                      1 << REGNUM(R15);
...
static const RegisterMask SingleByteStoreRegs =
    GpRegs & ~(1 << REGNUM(RSP) | 1 << REGNUM(RBP) | 1 << REGNUM(RSI) |
               1 << REGNUM(RDI));

static const RegisterMask SpecialRegs =
    1 << REGNUM(FP) |
    1 << REGNUM(SP); // These are GpRegs that may be assumed live.
...
#define DECLARE_PLATFORM_REGALLOC() const static Register argRegs[NumArgRegs];
...
#define DECLARE_PLATFORM_ASSEMBLER()                                           \
  ...
  void emitxb(uint64_t op, Register x, Register b) {                           \
    emitrxb(op, RZero, x, b);                                                  \
  }                                                                            \
  ...
  void emitrxb_imm(uint64_t op, Register r, Register x, Register b,            \
                   int32_t imm);                                               \
  void emitr_imm(uint64_t op, Register r, int32_t imm) {                       \
    emitrr_imm(op, RZero, r, imm);                                             \
  }                                                                            \
  ...
  void asm_immi(Register r, int32_t v, bool canClobberCCs, bool blind);        \
  void asm_immq(Register r, uint64_t v, bool canClobberCCs, bool blind);       \
  void asm_immd(Register r, uint64_t v, bool canClobberCCs, bool blind);       \
  void asm_regarg(ArgType, LIns *, Register);                                  \
  void asm_stkarg(ArgType, LIns *, int);                                       \
  void asm_shift(LIns *);                                                      \
  void asm_shift_imm(LIns *);                                                  \
  void asm_arith_imm(LIns *);                                                  \
  bool asm_arith_imm_blind(LIns *);                                            \
  void beginOp1Regs(LIns *ins, RegisterMask allow, Register &rr,               \
                    Register &ra);                                             \
  void beginOp2Regs(LIns *ins, RegisterMask allow, Register &rr, Register &ra, \
                    Register &rb);                                             \
  void endOpRegs(LIns *ins, Register rr, Register ra);                         \
  void beginLoadRegs(LIns *ins, RegisterMask allow, Register &rr, int32_t &d,  \
                     Register &rb, Register &orb);                             \
  ...
  void asm_immf4(Register r, float4_t v, bool canClobberCCs, bool blind);
                ...
                const
    int LARGEST_UNDERRUN_PROT = 38; // largest value passed to underrunProtect
