...
//
Register RegAlloc::allocReg(LIns *ins, RegisterMask setA___,
                            Register regClass) {
  Register r;
  ...
  if (setA_F_) {
    ...
    r = firstAvailableReg(
        ins, regClass, set); // Note: some platforms prefer lsReg, others msReg
#ifdef RA_REGISTERS_OVERLAP
    if (r == UnspecifiedReg) {
      // we may get here for composite regs, if the "prefered" set leaves only
      // single regs; try again
...
LIns *RegAlloc::findVictim(RegisterMask allow, LIns *forIns /*= NULL*/,
  ...
  for (Register r = lsReg(vic_set); vic_set; r = nextLsReg(vic_set, r)) {
    ...
    if (!ins) {
      NanoAssert(
          ins); // ins should really be non-null here, but let's be defensive
      continue;
    }
    ...
    if (forIns &&
        firstAvailableReg(forIns, regClass, (_free | rmask(r1)) & allow) ==
            UnspecifiedReg) {
