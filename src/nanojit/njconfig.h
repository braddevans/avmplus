 ...
 * ARM-specific fields are declared everywhere. This reduces build dependencies
 * (so that this files does not require nanojit.h to be included beforehand) and
 * also reduces clutter in this file; the extra storage space required is
 * trivial since most fields are single bits.
 */
struct Config {
public:
  ...
  // ARM architecture to assume when generate instructions for (currently, 4 <=
  // arm_arch <= 7)
  uint8_t arm_arch;
  ...
  // If true, use full-range addressing for branches even when a short branch
  // will suffice (x86-64 only)
  ...
  // whether or not an FPU is present. (ARM only for now, but might also
  // includes MIPS in the future)
  uint32_t soft_float : 1;

  // If true, compiler will insert a random amount of space in between functions
  // (x86-32 only)
  uint32_t harden_function_alignment : 1;

  // If true, compiler will insert randomly choosen no-op instructions at random
  // locations within a compiled method (x86-32 only)
  uint32_t harden_nop_insertion : 1;

  // If true, compiler will attempt to minimize the ability of an attacker to
  // control literal constants appearing in the code
...
public:
  virtual ~Noise() {}
  // Produce a random signed integer in the range 0..maxValue, where maxValue >
  // 0.
