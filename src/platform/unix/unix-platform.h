...
// Note, this used to be #ifndef SYMBIAN, but Symbian code belongs in the
// Symbian platform file. Do *NOT* unfix this.
#include <stdint.h>
...
#ifdef SOLARIS
// "NP" = non-portable.  Origin of code unclear, may be old.  Consider removing
// it, it's not used by current Solaris builds.
...
REALLY_INLINE void X86_Pause() { __asm__ volatile("pause"); }
...
REALLY_INLINE bool VMPI_compareAndSwap32WithBarrier(int32_t oldValue,
                                                    ...
                                                    volatile int32_t *address) {
  __asm__ __volatile__("" : : : "memory");
  int32_t result = __atomic_cmpxchg((int)oldValue, (int)newValue,
...
REALLY_INLINE bool VMPI_compareAndSwap32(int32_t oldValue, int32_t newValue,
                                         volatile int32_t *address) {
  return __atomic_cmpxchg((int)oldValue, (int)newValue,
                          (volatile int *)address) == 0;
}
...
#elif 1 // FIXME:here the test should be for VMCFG_ARM; but see comment on the
        // "else" branch, and bug 701638.
// FIXME: bug 609810 VMPI atomic primitives for ARM require inline-asm
// implementations
#define EMULATE_ATOMICS_WITH_PTHREAD_MUTEX

#else /* FIXME: the code below appears to not have been tested; it is supposed \
        to be compiled if !defined(ANDROID) && !defined(VMCFG_ARM). However,                            \
        enabling this branch results in lots of build errors. See bug                                                         \
        https://bugzilla.mozilla.org/show_bug.cgi?id=701638 */

#if !defined(SOLARIS)
#if defined(__GNUC__) && defined(__GNUC_MINOR__) && (__GNUC__ >= 4)
#if (__GNUC_MINOR__ >= 1) // _GLIBCXX_ATOMIC_BUILTINS_4 was added after the
                          // intrinsic was available
...
REALLY_INLINE bool VMPI_compareAndSwap32WithBarrier(int32_t oldValue,
                                                    ...
                                                    volatile int32_t *address) {
#if defined(SOLARIS)
  bool result = atomic_cas_32((volatile uint32_t *)address, (uint32_t)oldValue,
  ...
  return __sync_bool_compare_and_swap(
      address, oldValue, newValue); // This requires -march=i486 or better
#else
  int32_t currentValue;
  __asm__ __volatile__("lock cmpxchgl %4, %0"
                       : "=m"(*address), "=a"(currentValue)
                       : "m"(*address), "1"(oldValue), "r"(newValue)
                       : "memory");
...
REALLY_INLINE bool VMPI_compareAndSwap32(int32_t oldValue, int32_t newValue,
                                         volatile int32_t *address) {
#if defined(SOLARIS)
  return atomic_cas_32((volatile uint32_t *)address, (uint32_t)oldValue,
                       (uint32_t)newValue) == (uint32_t)oldValue;
#else
  // No barrier-less version implemented
