...
void vtuneEnd(void *, NIns *end) { endAddr = end; }
...
void vtuneLine(void *, int line, NIns *pos) {
  if (!line_cap) {
    line_cap = 4;
    ML.line_number_table =
        (pLineNumberInfo)VMPI_alloc(line_cap * sizeof(LineNumberInfo));
  } else if (ML.line_number_size == line_cap) {
    line_cap *= 2;
    pLineNumberInfo table =
        (pLineNumberInfo)VMPI_alloc(line_cap * sizeof(LineNumberInfo));
    memcpy(table, ML.line_number_table,
           ML.line_number_size * sizeof(LineNumberInfo));
    VMPI_free(ML.line_number_table);
