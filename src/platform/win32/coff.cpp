...
void Coff::addSection(const char *ar, int numBytes, int startAddr,
                      int /*endAddr*/) {
  int alignedSize = (numBytes + 3) & ~3;

  SectionHeader_t *section = (SectionHeader_t *)mmfx_new_array(
      char, sizeof(SectionHeader_t) + alignedSize);
...
void Coff::addSymbol(const char *s, int value) {
  ...
  int allocSize =
      sizeof(int) + size + 1; // string length + its content + null terminator

  SymbolEntry_t *symbol =
      (SymbolEntry_t *)mmfx_new_array(char, sizeof(SymbolEntry_t) + allocSize);
...
/**
 * We dump out the coff file in the following order.  Where * denotes 1 or more
 * entries CoffHeader_t *SectionHeader_t
 *   *<text>
 *   <symboltable>
 *   <stringtable>
 */
void Coff::done() {
  ...
  for (unsigned int i = 0; i < header.f_nsyms; i++) {
    symbols[i]->name.table.offset =
        pos - strSize;                   // name's offset into string table
    int *size = (int *)(symbols[i] + 1); // the string is stored pascal style
                                         // (i.e length leads content)
    pos += *size;
