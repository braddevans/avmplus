...
static bool isFiniteAndNonzerof(float x) {
  float_overlay u(x);
  return ((u.word & 0x7fffffffL) != 0x7F800000L) &&
         ((u.word & 0x7FFFFFFFL) != 0);
}

#ifdef UNDER_CE

// sin, cos, tan all function incorrectly when called with really large values
// on windows mobile they all start failing at different values, but all start
// failing somewhere with values greater than 210 million.
#define AVMPLUS_TRIG_FUNC_MAX 210000000

const static bool broken_trig_funcs =
    MathUtils::isNaN(MathUtils::cos(250000000));

// Helper function to adjust a value for sin, cos, or tan into an equivalent
// value in the range that works correctly.  This works because these functions
// all have a period of 2*PI or PI, so there are many equivalent values.
...
double MathUtils::cos(double value) {
  if (broken_trig_funcs &&
      (value > AVMPLUS_TRIG_FUNC_MAX || value < -AVMPLUS_TRIG_FUNC_MAX)) {
 ...
 * big-endian systems.
 */
#define EXTRACT_WORDS(hx, lx, x)                                               \
  {                                                                            \
    DWORD *ptr = (DWORD *)&x;                                                  \
    hx = ptr[1];                                                               \
    lx = ptr[0];                                                               \
  }
#define SET_HIGH_WORD(x, hx)                                                   \
  {                                                                            \
    DWORD *ptr = (DWORD *)&x;                                                  \
    ptr[1] = hx;                                                               \
  }
#define GET_HIGH_WORD(hx, x)                                                   \
  {                                                                            \
    DWORD *ptr = (DWORD *)&x;                                                  \
    hx = ptr[1];                                                               \
  }
...
#pragma warning(disable : 4740) // flow in or out of inline asm code suppresses
                                // global optimization
