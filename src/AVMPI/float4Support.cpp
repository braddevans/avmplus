...
namespace avmplus {
/* Note: This file must be compiled with NEON turned on even for targets that
  don't have NEON, in order for the adapters to work. But VMCFG_NEON may be
  undefined, indicating that we don't want fast intrinsic implementation */
#ifdef VMCFG_FLOAT
#ifdef VMCFG_ARM
#include <arm_neon.h>
...
float4_ret_t verifyEnterVECR_adapter_impl(avmplus::MethodEnv *env, int32_t argc,
                                          uint32_t *ap) {
...
float4_t thunkEnterVECR_adapter_impl(void *thunk_p, MethodEnv *env,
                                     int32_t argc, uint32_t *argv) {
  typedef float4_ret_t (*VecrThunk)(avmplus::MethodEnv * env, int32_t argc,
                                    uint32_t * argv);
  DECLARE_ALIGNED_FLOAT4_PTR(retval);
...
#ifdef DEBUGGER
float4_ret_t debugEnterVECR_adapter_impl(avmplus::MethodEnv *env, int32_t argc,
                                         uint32_t *ap) {
  DECLARE_ALIGNED_FLOAT4_PTR(retval);
  retval->f4 = avmplus::BaseExecMgr::debugEnterExitWrapperV(env, argc, ap);
  return retval->f4_jit;
}
const VecrMethodProc debugEnterVECR_adapter =
    (VecrMethodProc)debugEnterVECR_adapter_impl;
#endif

const VecrMethodProc verifyEnterVECR_adapter =
    (VecrMethodProc)verifyEnterVECR_adapter_impl;
const VecrThunkProc thunkEnterVECR_adapter = thunkEnterVECR_adapter_impl;

} // namespace avmplus
#endif
// TODO: leave these for "#if !defined(VMCFG_NEON) && !defined(VMCFG_SSE2)", add
// hardware-accellerated versions for NEON and SSE2
