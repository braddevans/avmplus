...
bool AVMPI_canCommitAlreadyCommittedMemory() { return false; }

bool AVMPI_useVirtualMemory() { return true; }

bool AVMPI_areNewPagesDirty() { return true; }

static SymbianHeap *symbianHeapListRoot = 0;

void *AVMPI_reserveMemoryRegion(void *address, size_t size) {
  // After reading the RChunk API it looks like we can not allocate contiguous
  // memory. Create a new heap when address is null. Otherwise return null which
  // MMgc should handle.

  // Open question is could we instead just create one SymbianHeap instance and
  // always allocate from there and ignore reserverMemoryRegion and
  // releaseMemoryRegion. However as each region should be contiguous it's maybe
  // easier just to create multiple regions = heaps.
  void *ptr = 0;
  if (address == NULL) {
...
bool AVMPI_releaseMemoryRegion(void *address, size_t size) {
  SymbianHeap *heap =
      SymbianHeap::FindHeap((TUint)address, symbianHeapListRoot);
  if (heap) {
    // remove from list
...
bool AVMPI_commitMemory(void *address, size_t size) {
  ...
  if (heap) {
    ...
    // TEMPORARY FIX: MMgc should zero initialize the memory, but it does not
    // seem to do it in all cases.
    if (result) {
...
void *AVMPI_allocateAlignedMemory(size_t) { return 0; }
...
void AVMPI_cleanStack(size_t amt) {
  // TODO - According to Tommy it does not kill if we don't implement on
  // Symbian. It could be possible to implement using ARM assembler.
}
