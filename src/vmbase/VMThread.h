 ...
 * ensures that any threads blocked on the mutex are considered 'safe' with
 * respect to their current safepoint context. I.e. lock acquisition is
 * performed within the RAII scope of a SafepointGate to create an implicit
 * safepoint.
 *
 ...
 * of safepoints. The default macro for clients, SCOPE_LOCK, is therefore
 * defined as SCOPE_LOCK_SP. Conservatively, SCOPE_LOCK_NO_SP should only be
 * used in the following scenarios: a) For critical sections known only to be
 * entered by threads that have no current safepointing context. b) For mutexes
 * that can never be contended by threads that have entered a common
 * safepointing context. c) For mutexes that protect only 'uninterruptible'
 * critical sections, i.e. critical sections that have none of the following
 * reachable from within their scope: a safepoint (either implicit or explicit),
 * a request to a SafepointManager to execute a SafepointTask.
 *
 * Note that any type of lock in VM, GC or avm-host code that is acquired
 * without a MutexLocker will have equivalent acquire semantics to that of
 * SCOPE_LOCK_NO_SP.
 */
#ifdef VMCFG_SAFEPOINTS
#define SCOPE_LOCK(_m_) SCOPE_LOCK_SP(_m_)
#define SCOPE_LOCK_NO_SP(_m_)                                                  \
  if (vmbase::MutexLocker<vmbase::NO_SAFEPOINT> __locker = _m_) {              \
  } else
#define SCOPE_LOCK_SP(_m_)                                                     \
  if (vmbase::MutexLocker<vmbase::IMPLICIT_SAFEPOINT> __locker = _m_) {        \
  } else
#else
#define SCOPE_LOCK(_m_)                                                        \
  if (vmbase::MutexLocker __locker = _m_) {                                    \
  } else
 ...
 *     }
 *
 * When VMCFG_SAFEPOINTS is defined, the SCOPE_LOCK_SP_NAMED version of the
 * macro defines an implicit safepoint for threads blocked on the monitor
 * (identical to SCOPE_LOCK_SP). Additionally, SCOPE_LOCK_SP_NAMED defines an
 * implicit safepoint for threads waiting and timed-waiting on the monitor. The
 * rules for using SCOPE_LOCK_NO_SP_NAMED instead of SCOPE_LOCK_SP_NAMED are the
 * same as those for SCOPE_LOCK_NO_SP described above.
 */
#ifdef VMCFG_SAFEPOINTS
#define SCOPE_LOCK_NAMED(_name_, _m_) SCOPE_LOCK_SP_NAMED(_name_, _m_)
#define SCOPE_LOCK_NO_SP_NAMED(_name_, _m_)                                    \
  if (vmbase::MonitorLocker<vmbase::NO_SAFEPOINT> _name_ = _m_) {              \
  } else
#define SCOPE_LOCK_SP_NAMED(_name_, _m_)                                       \
  if (vmbase::MonitorLocker<vmbase::IMPLICIT_SAFEPOINT> _name_ = _m_) {        \
  } else
#else
#define SCOPE_LOCK_NAMED(_name_, _m_)                                          \
  if (vmbase::MonitorLocker _name_ = _m_) {                                    \
  } else
...
#ifdef VMCFG_SAFEPOINTS
enum BlockingMode {
  IMPLICIT_SAFEPOINT, // When blocked, a thread should be considered 'safe' with
                      // respect to its current safepoint context
...
class ConditionVariable {
  ...
  // ConditionVariables should only to be used as part of a WaitNotifyMonitor,
  // so keep everything private.
private:
  ConditionVariable();

  /**
   * Signals a single thread waiting on this condition variable.
   * This function does not imply any fairness policy when selecting the thread
   * to signal. This function does not block.
   */
  void signal();
   ...
   * (asserted in debug builds). When the thread is blocked, it releases its
   * lock on the mutex. The thread remains blocked until the condition variable
   * is signaled (either individually or via a broadcast), or the specified
   * timeout period has expired. Post-wait, the thread will attempt to
   * re-acquire mutex. When the mutex is re-acquired, this function will return.
   *
   * Note:
   * - A waiting thread may spuriously awaken without being signaled.
   * - Waiting on a condition variable with a recursively locked mutex results
   * in undefined behavior (asserted in debug builds).
   * - Waiting on a single condition variable with multiple mutexes results in
   *   undefined behavior.
   *
   * @param mutex The mutex to release whilst waiting and re-acquire when
   * signaled
  ...
  /**
   * Identical to the timeout version except the thread will wait forever to be
   * signaled.
   */
...
/**
 * Base class for objects which require monitor synchronization semantics with a
 * single implicit condition for wait/notify.
 *
 * The general abstraction is that a monitor contains two thread sets: the
 * blocked set and the wait set. Threads in the blocked set compete for the
 * monitor's mutex. The monitor's 'owner' thread (i.e. the one which owns the
 * mutex) can move itself to the wait set by calling one of the wait()
 * functions. After an owner thread has entered the wait set the monitor's mutex
 * is released; the blocked set then compete to acquire the mutex. Threads in
 * the wait set may only continue execution by moving back to the blocked set
 * and competing with other threads to acquire the mutex. Moving from the
 * waiting to the block set occurs with the notify() and notifyAll() operations.
 * Calling notify() on the monitor causes a single thread from the wait set to
 * be moved to the blocked set (any thread could be chosen). Calling notifyAll()
 * on the monitor causes all threads from the waiting set to be moved to the
 * blocked set. When a thread acquires the mutex after moving from the wait set
 * to the blocked set it continues execution by returning from its call to
 * wait(). Note that only the owner of the monitor is permitted to call notify()
 * or notifyAll().
 */
...
private:
  ...
  /**
   * Notifies a single thread waiting on this monitor. The notified thread will
   * be moved to the blocked set. This function does not imply any fairness
   * policy when selecting the thread to notify. This function does not block.
   * Only the monitor's owner thread may call this function (asserted in debug
   * builds).
   */
  void notify();
...
private:
  // No copying allowed: undefined semantics
#ifdef VMCFG_SAFEPOINTS
  const MutexLocker<BLOCKING_MODE> &
  operator=(const MutexLocker<BLOCKING_MODE> &locker);
#else
 ...
 *     void bar(int val)
 *     {
 *         SCOPE_LOCK_NAMED(locker, m_monitor) { // Must give a name if the
 * locker will be referenced while (m_sharedState < val) { m_sharedState += val;
...
private:
  // No copying allowed: undefined semantics
#ifdef VMCFG_SAFEPOINTS
  const MonitorLocker<BLOCKING_MODE> &
  operator=(const MonitorLocker<BLOCKING_MODE> &locker);
#else
...
public:
   ...
   *   - All load and store operations executed before the barrier will appear
   * to complete before all load and store operations after the barrier.
   *
...
public:
  ...
  static bool compareAndSwap32WithBarrier(int32_t oldValue, int32_t newValue,
                                          volatile int32_t *address);

  static int32_t compareAndSwap32WithBarrierPrev(int32_t oldValue,
                                                 int32_t newValue,
                                                 volatile int32_t *address);
   ...
   * @return the updated value following the OR. The 'Prev' suffixed versions
   * return the value before the OR.
   */
   ...
   * @return the updated value following the AND. The 'Prev' suffixed versions
   * return the value before the AND.
   */
 ...
 */
template <typename T> class VMThreadLocal {
public:
  VMThreadLocal() : m_isInitialized(false) {
  ...
  REALLY_INLINE operator T() const { return get(); }

  REALLY_INLINE T operator->() const { return get(); }
 ...
 * putting VMThreads to sleep, and querying the identity of the running
 * VMThread.
 *
...
private:
  enum ThreadState {
    NOT_STARTED, // The VMThread has been constructed, but it does not yet
                 // represent a running native thread.
    RUNNABLE,    // The VMThread's native thread is running, waiting, blocked or
                 // sleeping
    TERMINATED,  // The VMThread's native thread has exited
  };

public:
  enum ThreadPriority { LOW_PRIORITY, NORMAL_PRIORITY, HIGH_PRIORITY };
   ...
   * Calling VMThread::currentThread() from the new thread will return this
   * VMThread.
   *
  ...
  /**
   * Identical to start(ThreadPriority priority), but starts the thread with the
   * given stack and guard area sizes. Note that if the platform does not
   * support either parameter then any supplied arguments will be ignored. As of
   * Nov' 2010, support should be: POSIX     win32 (XP version APIs) priority no
   * yes stack size      yes        yes guard size      yes        no
   *
   * @param priority The thread's priority (low/normal/high)
  ...
  /**
   * Causes the calling thread to block until this VMThread's native thread has
   * exited. This function is thread safe, i.e. multiple thread can concurrently
   * join a thread.
   *
   * If this VMThread's native thread has already exited, then the function
   * immediately returns.
   */
  void join();
...
private:
  ...
  static VMThreadLocal<VMThread *>
      m_currentThread; // Keep each thread's VMThread in TLS.
};
