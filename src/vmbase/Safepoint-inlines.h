...
/**
 * Wraps calls to WaitNotifyMonitor::wait with a register flush and safepoint
 * gate
 */
...
private:
  struct TimedWaitRecord {
    TimedWaitRecord(WaitNotifyMonitor *monitor, int32_t timeout_millis)
        : monitor(monitor), timeout_millis(timeout_millis), returnVal(false) {}
  ...
  static void timedWaitInSafepointGate(void *stackPointer,
                                       void *timedWaitRecord) {
    SafepointGate gate(stackPointer);
    TimedWaitRecord *const _timedWaitRecord =
        (TimedWaitRecord *)(timedWaitRecord);
    _timedWaitRecord->returnVal =
        _timedWaitRecord->monitor->wait(_timedWaitRecord->timeout_millis);
  }
...
public:
  ...
  static bool wait(WaitNotifyMonitor &monitor, int32_t timeout_millis) {
    ...
    VMPI_callWithRegistersSaved(timedWaitInSafepointGate,
                                (void *)&timedWaitRecord);
    ...
    if (!m_mutex.tryLock()) {
      ...
      if (current && current->m_status == SafepointRecord::SP_UNSAFE) {
        ...
        assert(!SafepointRecord::hasCurrent() ||
               SafepointRecord::current()->m_manager->inSafepointTask());
        m_mutex.lock();
      }
    }
  } else {
...
template <BlockingMode BLOCKING_MODE>
REALLY_INLINE
MonitorLocker<BLOCKING_MODE>::MonitorLocker(WaitNotifyMonitor &monitor)
    : m_monitor(monitor) {
  if (BLOCKING_MODE == IMPLICIT_SAFEPOINT) {
    // We do not need an implicit safepoint if the thread
    // acquires the lock without blocking.
...
REALLY_INLINE SafepointRecord::SafepointRecord()
    : m_status(SP_UNSAFE), m_safeRegionEnd(NULL), m_stackPrev(NULL),
      m_managerPrev(NULL), m_managerNext(NULL), m_manager(NULL),
      m_interruptLocation(NULL), m_isolateDesc(0) {}
...
SafepointRecord::setCurrent(SafepointRecord *record) {
...
REALLY_INLINE void SafepointRecord::setLocationAndDesc(int32_t *location,
                                                       int desc) {
  assert(m_manager ==
         NULL); // this record should not be on a SafepointManager list
...
REALLY_INLINE
SafepointManager::RecordIterator::RecordIterator(SafepointManager &manager)
