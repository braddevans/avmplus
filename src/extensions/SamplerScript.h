...
class SamplerScript {
private:
  static bool trusted(ScriptObject *self) {
    return self->toplevel()->sampler_trusted(self);
  }
...
public:
  ...
  static double _getInvocationCount(ScriptObject *self, Atom a,
                                    QNameObject *qname, uint32_t type);
  static bool isGetterSetter(ScriptObject *self, Atom a, QNameObject *name);

  static ArrayObject *getLexicalScopes(ScriptObject *self,
...
#ifdef DEBUGGER
private:
  static ClassClosure *getType(Toplevel *toplevel, SamplerObjectType sot,
                               const void *obj);

  friend class SampleIterator;
  static ScriptObject *makeSample(ScriptObject *self, ClassFactoryClass *cf,
                                  const Sample &sample);
  static bool set_stack(ScriptObject *self, ClassFactoryClass *cf,
