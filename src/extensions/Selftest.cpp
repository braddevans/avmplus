...
// The logging infrastructure uses stdio for the time being, this needs to be
// fixed.
#ifdef AVMPLUS_WIN32
#include <stdio.h>
#endif
...
void selftests(AvmCore *core, const char *component_glob,
               const char *category_glob, const char *name_glob) {
...
SelftestRunner::SelftestRunner(AvmCore *core)
    : MMgc::GCRoot(core->GetGC()), core(core), tests(NULL), last(NULL),
...
// print summary at the end, we have total number of tests that _should_ have
// run, can print number that failed, that didn't run, that succeeded

void SelftestRunner::run(const char *component_glob, const char *category_glob,
      ...
      TRY(core, kCatchAction_Ignore) {
        ...
        for (volatile int n = 0;; n++) {
          ...
          if (name_glob == NULL || match(name_glob, test_name)) {
            if (!isExplicit || (component_glob != NULL &&
                                category_glob != NULL && name_glob != NULL)) {
              TRY(core, kCatchAction_Ignore) {
                logTest();
                t->run(n);
                logPass();
...
void SelftestRunner::logFailure(const char *text, const char *file, int line) {
  AvmLog("['failure', '%s', '%s', '%s', '%s', '%s', %d]\n", test_component,
         test_category, test_name, text, file, line);
}
...
Selftest::Selftest(AvmCore *core, const char *component, const char *category,
                   const char **names, const bool *explicits)
    : core(core), component(component), category(category), names(names),
      explicits(explicits), next(NULL) {
...
/* final */ void Selftest::verifyPass(int expr, const char *text_expr,
                                      const char *file, int line) {
  if (!expr) {
    selftestRunner->logFailure(text_expr, file, line);
    core->throwException(new (core->GetGC())
                             Exception(core, selftestRunner->token));
  }
}
