...
const AOTInfo sdk_aotInfo = EMPTY_AOTInfo;
#else
...
LLVMVectorInstance abcVectorInstances[] = {};
...
extern "C" __attribute__((visibility("default"))) const struct {
  ...
  InterfaceBindingFunction m_InterfaceBindingFunction;
    ...
    "MultinameIndex",
    0,
    "MultinameIndexMaybeInt",
    0,
    "MultinamePtrMaybeInt",
    0,
    "MethodDebugInfo",
    0,
    "char",
...
AOTLazyEvalGuard::AOTLazyEvalGuard(const avmplus::AbcEnv *abcEnv)
    : LoadRedirGuard(abcEnv->pool()->aotInfo->nLazyEvals), m_abcEnv(abcEnv) {
  PoolObject *pool = m_abcEnv->pool();
  // printf("pool=%p, AOTLazyEvalGuard=%p m_abcEnv=%p \n", pool, this,
  // m_abcEnv);

  if (pool->aotRoot == NULL) {
    const AOTInfo *info = pool->aotInfo;
    AvmAssert(info != NULL);
    pool->aotRoot = new MMgc::GCRoot(
        pool->core->gc, m_abcEnv->pool()->aotInfo->lazyEvalVals,
        m_abcEnv->pool()->aotInfo->nLazyEvals * sizeof(uintptr_t));
    // printf("nLazyEvals=%d", info->nLazyEvals);
...
uintptr_t *AOTLazyEvalGuard::redirectLoad(uintptr_t *faultAddr) {
  ...
  if (info->lazyEvalValPtrs[lazyEvalNum] != redir) {
    ...
    // printf("m_abcEnv=%p, methodId: %d leNum: %d (%p) %p\n", m_abcEnv,
...
// Then LLVM can recognize the opportunity to CSE them up to the top of the
// function and then reuse the values rather than fetching them from scratch
// each time. A final pass marks them inline and inlines them.
//
...
__attribute__((noinline)) PoolObject *MethodInfo::pool() const { return _pool; }

__attribute__((noinline)) MMgc::GC *VTable::gc() const {
  return traits->core->GetGC();
}

__attribute__((noinline)) AvmCore *VTable::core() const { return traits->core; }

__attribute__((noinline)) Toplevel *VTable::toplevel() const {
  return _toplevel;
}

__attribute__((noinline)) VTable *ScopeChain::vtable() const { return _vtable; }

__attribute__((noinline)) MMgc::GC *MMgc::GCRoot::GetGC() const { return gc; }
#endif
...
__attribute__((noinline)) String *aotGetString(const MethodEnv *env,
                                               int32_t index) {
