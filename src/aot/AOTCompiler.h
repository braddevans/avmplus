...
#ifndef VMCFG_AOT // TEMP hack to enable building field offset files for
                  // ordinary shell
...
// LLVM type hacks
//------------------------------------------------------------------------------
// Because we can't specialise templates on typedefs we hide Atoms, bool, etc.
// as pointers to these opaque structs. The pointers are never dereferenced,
// they just get cast to whatever their real type is.
//------------------------------------------------------------------------------
...
struct MethodDebugInfo {
  ...
  MethodDebugInfo()
      : local_count(0), scope_count(0), file_name(0), local_names(NULL),
        line_count(0), lines(NULL) {}
};

typedef uintptr_t (*AOTLazyEvalFunc)(avmplus::MethodEnv *env,
...
struct AOTInfo {
  ...
  const MethodDebugInfo *methodDebugInfos; // may be NULL if not debugging but
                                           // it's always in the struct
...
#ifndef VMCFG_HALFMOON_AOT_RUNTIME // Halfmoon AOT accesses precomputed
                                   // multinames directly
  // static multinames
  avmplus::Multiname *
      *multinames; // TODO could probably be an array of multinames
...
struct AOTExceptionDesc {
  ...
  uint8_t typeIndexU30[5]; // where to store the u30 for the match traits type;
                           // newCatchTraits needs this!
} ALIGN8BYTE;
#endif

#ifdef VMCFG_BUFFER_GUARD
// Note that this assumes that there is only one AbcEnv/PoolObject per AOTInfo!
// I.e., a given ABC is only instantiated once!
...
#ifndef VMCFG_HALFMOON_AOT_RUNTIME // Halfmoon AOT accesses precomputed
                                   // multinames directly
#define EMPTY_AOTInfo                                                          \
  {                                                                            \
    {}, NULL, 0, NULL, NULL, 0, NULL, NULL, 0, NULL, NULL, 0, NULL, NULL,      \
...
public:
  ...
  inline void exit(avmplus::AvmCore *core) { m_frame.exit(core); }

  inline void set_dxns(avmplus::Namespace *dxns) { m_frame.setDxns(dxns); }
};
...
#ifdef VMCFG_HALFMOON_AOT_RUNTIME // halfmoon AOT doesn't care whether it's
                                  // compiling for shell or player
...
avmplus::String *aotGetString(const avmplus::MethodEnv *env,
                              int32_t index) ATTR_CONST;
