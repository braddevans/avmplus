...
struct DebugInfo {
  ...
  uint64_t pc() const { return ((uint64_t *)info->data)[0]; }

  uint32_t tag() const { return info->tag; }
...
struct MethBlock {
  ...
  uint64_t start() const { return ((uint64_t *)block->data)[0]; }

  uint64_t end() const { return ((uint64_t *)block->data)[1]; }

  std::string name() const { return std::string(meth->data, meth->size); }

  const void *codeBytes() const { return code ? code->data : NULL; }

  const uint32_t codeLen() const { return code ? code->size : 0; }
...
int main(int argc, char **argv) {
  ...
  if (base->size > sizeof(uint64_t)) {
    ...
    if (len >= sizeof(path))
      ...
      if (!hdl)
        warn("couldn't get info from base library %s (using defaults)", path);
      else {
        ...
        if (!start)
          ...
          if (dladdr(start, &dlInfo)) {
            if (((char *)start - (char *)dlInfo.dli_fbase) != textOffs)
...
static void emit_so_bfd(const Chunk *base, uint64_t textOffs, uint64_t textLen,
  ...
  bfd_set_section_flags(abfd, textSection,
  ...
  asymbol **symbols =
      (asymbol **)malloc(sizeof(asymbol *) * (6 + blocks.size()));
  ...
  symbols[n++] = make_glob_symbol(abfd, "_jitUnused", textSection,
                                  blocks.back().end() - start + textAdj);
  symbols[n++] =
      make_glob_symbol(abfd, "__jitEnd", textSection, textLen + textAdj);
  symbols[n++] =
      make_glob_symbol(abfd, "_jitEnd", textSection, textLen + textAdj);
