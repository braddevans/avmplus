...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow) {
  ...
  // initialization to initialize any of the device specific controls such as
  // CAPEDIT and SIPPREF.
  SHInitExtraControls();
  ...
  if (hWnd) {
    ...
    // The "| 0x00000001" is used to bring any owned windows to the foreground
    // and activate them.
  ...
  // menubar (if one is created is not taken into account). So we resize the
...
BOOL checkForCommand(HWND hWnd) {
  HANDLE handle = CreateFile(nextvm, GENERIC_READ, FILE_SHARE_READ, NULL,
  ...
  if (handle == INVALID_HANDLE_VALUE) {
    ...
    if (count == 0) {
      ...
      convert =
          MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, data, -1, params, 500);
      char *pch;
      char filename[500];
      pch = strtok(data, " ");
      while (pch != NULL) {
      ...
      if (convert == 0) {
        ...
        i_exitcode = -1;
        CreateProcess(shell, params, NULL, NULL, FALSE, CREATE_NEW_CONSOLE,
                      NULL, NULL, NULL, &pi);
        ...
        while (true) {
          ...
          if (res == WAIT_TIMEOUT) {
            ...
            DWORD dwBytes;
            HANDLE ceExitFile =
                CreateFile(szExitCodeFile, GENERIC_WRITE, FILE_SHARE_READ, NULL,
                           CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
            WriteFile(ceExitFile, (void *)szExit, strlen(szExit), &dwBytes,
                      NULL);
            ...
            wsprintf(szError, TEXT("ERROR: %s wait returned failure %d"), last,
                     res);
            wsprintf(szMessage, TEXT("ERROR: %s wait returned failure %d"),
                     last, res);
            SetRect(&r, 10, 10, 220, 230);
            InvalidateRect(hWnd, &r, TRUE);
            UpdateWindow(hWnd);
