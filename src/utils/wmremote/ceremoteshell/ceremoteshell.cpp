...
void *OpenAndConnectToNamedPipe(const char *pipeName) {
  ...
  HANDLE pipe = CreateNamedPipeA((LPCSTR)name, PIPE_ACCESS_OUTBOUND,
                                 PIPE_TYPE_BYTE, 1, 4096, 4096, 100, NULL);
...
void DoMemProfiler() {
  ...
  DWORD t;
  BYTE *b;
  CeRapiInvoke(L"\\Windows\\avmremote.dll", L"RunMemProfiler", 0, NULL, &t, &b,
               NULL, 0);
  ...
  if (dwAttr == -1)
    ...
    HANDLE statsFile =
        CeCreateFile(L"\\Temp\\gcstats.txt", GENERIC_READ, FILE_SHARE_READ,
...
DWORD copyFileToDevice(char *source, char *dest) {
  ...
  if (source == NULL) {
    DWORD dwLen =
        MultiByteToWideChar(CP_ACP, MB_COMPOSITE, dest, -1, wDest, 256);
    HANDLE hEmpty = CeCreateFile(wDest, GENERIC_WRITE, FILE_SHARE_READ, NULL,
                                 CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
    CeCloseHandle(hEmpty);
    return 0;
  }
  HANDLE hSource = CreateFile(source, GENERIC_READ, FILE_SHARE_READ, NULL,
  ...
  HANDLE hDest = CeCreateFile(wDest, GENERIC_WRITE, FILE_SHARE_READ, NULL,
                              CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
  ...
  while (ReadFile(hSource, localBuf, dwBufsize, &dwRead, NULL)) {
    BOOL bWrite =
        CeWriteFile(hDest, (void *)localBuf, dwRead, &dwCeBytes, NULL);
    if (!bWrite) {
...
int _tmain(int argc, _TCHAR *argv[]) {
  ...
  if (argc < 2) {
    printf("usage:\n --shell=avmshell_arm.exe [params] test.abc\n runs the abc "
           "file in the shell and prints the output to stdout\n");
    return -1;
  }
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (argv[i][0] != '-') {
      hFile = CreateFile(argv[i], GENERIC_READ, FILE_SHARE_READ, NULL,
                         OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
  ...
  // check for avmshell
  hFile = ::CeCreateFile(wShell, GENERIC_READ, FILE_SHARE_READ, NULL,
                         OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
  if (hFile == INVALID_HANDLE_VALUE) {
  ...
  strcat(pIn, "\t");
  strcat(pIn, cmdline);
  hr = CeRapiInvoke(L"\\Windows\\avmremote.dll", L"StartAVMShell", strlen(pIn),
                    (BYTE *)pIn, &cbOut, (BYTE **)&pOut, NULL, 0);

  HANDLE hProcess = *(HANDLE *)pOut;
  LocalFree((HLOCAL)*pOut);
  if (pIn)
    LocalFree((HLOCAL)pIn);
  ...
  while (CeRapiInvoke(L"\\Windows\\avmremote.dll", L"WaitForAVMShell",
                      sizeof(wait_info), (BYTE *)&info, &cbOut, (BYTE **)&pOut,
                      NULL, 0) == S_OK) {
    ...
    if (dwCode == STILL_ACTIVE) {
      ...
      if (dwTimeout >= PROCESS_TIMEOUT) {
        // Kill the process if we don't end in a decent amount of time (e.g. 60
        // seconds)
        CeRapiInvoke(L"\\Windows\\avmremote.dll", L"KillAVMShell",
                     sizeof(HANDLE), (BYTE *)&hProcess, &cbOut, (BYTE **)&pOut,
                     NULL, 0);
        break;
      }
      continue;
  ...
  while (1) {
    ...
    if (dwAttr == -1)
      continue;
    ceFile =
        CeCreateFile(L"\\Temp\\remoteshell.log", GENERIC_READ, FILE_SHARE_READ,
                     NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
