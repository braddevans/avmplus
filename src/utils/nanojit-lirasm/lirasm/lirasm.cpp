...
#define print(x)                                                               \
  (_isnan(x)                                                                   \
       ? cout << "NAN"                                                         \
       : _finite(x) ? cout << x : (x > 0) ? cout << "INF" : cout << "-INF")
#else
#define print(x)                                                               \
  (isnan(x)                                                                    \
       ? cout << "NAN"                                                         \
       : (x == INFINITY) ? cout << "INF"                                       \
                         : (x == -INFINITY) ? cout << "-INF" : print_float(x))
#endif

/* Allocator SPI implementation. */

void *nanojit::Allocator::allocChunk(size_t nbytes, bool /*fallible*/) {
...
void nanojit::Allocator::freeChunk(void *p) { free(p); }
...
int nanojit::StackFilter::getTop(LIns *) { return 0; }
...
void nanojit::LInsPrinter::formatGuard(InsBuf *buf, LIns *ins) {
  RefBuf b1, b2;
  LasmSideExit *x = (LasmSideExit *)ins->record()->exit;
  VMPI_snprintf(buf->buf, buf->len, "%s: %s %s -> line=%ld (GuardID=%03d)",
                formatRef(&b1, ins), lirNames[ins->opcode()],
                ins->oprnd1() ? formatRef(&b2, ins->oprnd1()) : "",
                (long)x->line, ins->record()->profGuardID);
}

void nanojit::LInsPrinter::formatGuardXov(InsBuf *buf, LIns *ins) {
  RefBuf b1, b2, b3;
  LasmSideExit *x = (LasmSideExit *)ins->record()->exit;
  VMPI_snprintf(buf->buf, buf->len, "%s = %s %s, %s -> line=%ld (GuardID=%03d)",
                formatRef(&b1, ins), lirNames[ins->opcode()],
                formatRef(&b2, ins->oprnd1()), formatRef(&b3, ins->oprnd2()),
                (long)x->line, ins->record()->profGuardID);
}

const char *nanojit::LInsPrinter::accNames[] = {
    "o", // (1 << 0) == ACCSET_OTHER
    "?", "?", "?", "?", "?", "?", "?", "?", "?", "?", //  1..10 (unused)
    "?", "?", "?", "?", "?", "?", "?", "?", "?", "?", // 11..20 (unused)
    "?", "?", "?", "?", "?", "?", "?", "?", "?", "?", // 21..30 (unused)
...
#ifdef DEBUG
void ValidateWriter::checkAccSet(LOpcode op, LIns *base, int32_t disp,
                                 AccSet accSet) {
  (void)op;
  (void)base;
  (void)disp;
...
#define CI(name, args)                                                         \
  {                                                                            \
    (uintptr_t)(&name), args, nanojit::ABI_CDECL, /*isPure*/ 0,                \
        ACCSET_STORE_ANY DEBUG_ONLY_NAME(name)                                 \
  }

#define FN(name, args)                                                         \
  { #name, CI(name, args) }

enum LirTokenType { NAME, NUMBER, PUNCT, NEWLINE };
...
public:
  ...
  bool get(LirToken &token) {
    ...
    char c = mLine[0];
    size_t e = mLine.find_first_not_of(
    ...
    if (startsWith(mLine, "->")) {
      ...
      cerr << "line " << mLineno << ": error: Unrecognized character in file."
           << endl;
      return false;
    }
  ...
  bool eat(LirTokenType type, const char *exact = NULL) {
    LirToken token;
    return (get(token) && token.type == type &&
            (exact == NULL || token.data == exact));
  }
...
public:
  ...
  void assembleFragment(LirTokenStream &in, bool implicitBegin,
...
double sinFn(double d) { return sin(d); }
#define sin sinFn

double calld1(double x, double i, double y, double l, double x1, double i1,
              double y1, double l1) {
  return x + i * y - l + x1 / i1 - y1 * l1;
}

float callf1(float x, float i, float y, float l, float x1, float i1, float y1,
             float l1) {
  return x + i * y - l + x1 / i1 - y1 * l1;
}

// The calling tests with mixed argument types are sensible for all platforms,
// but they highlight the differences between the supported ABIs on ARM.
...
float callif2(int i, int j, int k, float x) { return x / (float)(i + j + k); }
...
float4_t callf4_1(float4_t x, float4_t i, float4_t y, float4_t &l, float4_t &x1,
                  float4_t &i1, float4_t &y1, float4_t &l1) {
#else
float4_t callf4_1(float4_t x, float4_t i, float4_t y, float4_t l, float4_t x1,
                  float4_t i1, float4_t y1, float4_t l1) {
#endif
  return f4_add(f4_sub(f4_add(x, f4_mul(i, y)), l),
...
Function functions[] = {
    ...
    FN(calld1,
       CallInfo::typeSig8(ARGTYPE_D, ARGTYPE_D, ARGTYPE_D, ARGTYPE_D, ARGTYPE_D,
                          ARGTYPE_D, ARGTYPE_D, ARGTYPE_D, ARGTYPE_D)),
    FN(callf1,
       CallInfo::typeSig8(ARGTYPE_F, ARGTYPE_F, ARGTYPE_F, ARGTYPE_F, ARGTYPE_F,
                          ARGTYPE_F, ARGTYPE_F, ARGTYPE_F, ARGTYPE_F)),
    ...
    FN(callif4_1,
       CallInfo::typeSig6(ARGTYPE_F4, ARGTYPE_I, ARGTYPE_F4, ARGTYPE_F4,
                          ARGTYPE_I, ARGTYPE_I, ARGTYPE_F4)),
    FN(callid2, CallInfo::typeSig4(ARGTYPE_D, ARGTYPE_I, ARGTYPE_I, ARGTYPE_I,
                                   ARGTYPE_D)),
    FN(callif2, CallInfo::typeSig4(ARGTYPE_F, ARGTYPE_I, ARGTYPE_I, ARGTYPE_I,
                                   ARGTYPE_F)),
    FN(callif4_2, CallInfo::typeSig4(ARGTYPE_F4, ARGTYPE_I, ARGTYPE_I,
                                     ARGTYPE_I, ARGTYPE_F4)),
    ...
    FN(callif4_3,
       CallInfo::typeSig6(ARGTYPE_F4, ARGTYPE_I, ARGTYPE_I, ARGTYPE_F4,
                          ARGTYPE_I, ARGTYPE_F4, ARGTYPE_F4)),
    FN(callf4_sqrt, CallInfo::typeSig1(ARGTYPE_F4, ARGTYPE_F4)),
    FN(callf4_mt, CallInfo::typeSig8(ARGTYPE_F4, ARGTYPE_F, ARGTYPE_I,
                                     ARGTYPE_D, ARGTYPE_F4, ARGTYPE_I,
                                     ARGTYPE_D, ARGTYPE_F, ARGTYPE_F4)),
    FN(printi, CallInfo::typeSig1(ARGTYPE_V, ARGTYPE_I)),
};

template <typename out, typename in> out lexical_cast(in arg) {
  stringstream tmp;
  out ret;
...
/* we do this because on Mac stringstream can't handle scientinfic notation
 * properly*/
template <> float lexical_cast<float, const string &>(const string &arg) {
  float retval;
...
double immD(const string &s) { return lexical_cast<double>(s); }

float immF(const string &s) { return lexical_cast<float, const string &>(s); }

float4_t immF4(const string &sx, const string &sy, const string &sz,
               const string &sw) {
  float4_t ret = {lexical_cast<float, const string &>(sx),
                  lexical_cast<float, const string &>(sy),
                  lexical_cast<float, const string &>(sz),
                  lexical_cast<float, const string &>(sw)};
  return ret;
}

template <typename t> t pop_front(vector<t> &vec) {
  if (vec.empty()) {
...
void dep_u8(char *&buf, uint8_t byte, uint32_t &cksum) {
  sprintf(buf, "%2.2X", byte);
  cksum += byte;
  buf += 2;
}

void dep_u32(char *&buf, uint32_t word, uint32_t &cksum) {
...
void dump_srecords(ostream &, Fragment *) {
  ...
  /*
      ...
      for (Page *page = frag->pages(); page; page = page->next) {
          ...
          for (uintptr_t p = p0; p < p0 + sizeof(page->code); p += step) {
              ...
              //
              //     +2 char code 'S3'.
              //     +2 char / 1 byte count of remaining bytes (37 = addr,
     payload, cksum).
              //     +8 char / 4 byte addr.
              //    ---
              ...
              uint8_t *c = (uint8_t*) p;
              for (size_t i = 0; i < step; ++i) { // Actual object code being
     emitted dep_u8(b, c[i], cksum);
              }
...
uint32_t FragmentAssembler::sProfId = 0;
...
FragmentAssembler::FragmentAssembler(Lirasm &parent, const string &fragmentName,
    ...
    : mParent(parent), mFragName(fragmentName), optimize(optimize),
      mBufWriter(NULL), mCseFilter(NULL), mExprFilter(NULL),
      mSoftFloatFilter(NULL), mVerboseWriter(NULL), mValidateWriter1(NULL),
      mValidateWriter2(NULL) {
  mFragment = new Fragment(NULL verbose_only(
      , (mParent.mLogc.lcbits & nanojit::LC_FragProfile) ? sProfId++ : 0));
  ...
  if (optimize) { // don't re-validate if no optimization has taken place
    mLir = mValidateWriter2 = new ValidateWriter(
        mLir, mFragment->lirbuf->printer, "end of writer pipeline");
  }
#endif
#ifdef DEBUG
  if (mParent.mVerbose) {
    mLir = mVerboseWriter = new VerboseWriter(
        mParent.mAlloc, mLir, mParent.mLirbuf->printer, &mParent.mLogc);
  }
#endif
  if (optimize) {
    mLir = mCseFilter =
        new CseFilter(mLir, LIRASM_NUM_USED_ACCS, mParent.mAlloc);
  }
...
#ifdef DEBUG
  mLir = mValidateWriter1 = new ValidateWriter(mLir, mFragment->lirbuf->printer,
...
void FragmentAssembler::bad(const string &msg) {
  cerr << "line " << mLineno << ": " << msg << endl;
  exit(1);
}

void FragmentAssembler::nyi(const string &opname) {
  cerr << "line " << mLineno << ": '" << opname
       << "' not yet implemented, sorry" << endl;
  exit(1);
}

void FragmentAssembler::need(size_t n) {
  if (mTokens.size() != n) {
    bad("need " + lexical_cast<string>(n) + " tokens, have " +
        lexical_cast<string>(mTokens.size()));
  }
}

LIns *FragmentAssembler::ref(const string &lab) {
  if (mLabels.find(lab) == mLabels.end())
    bad("unknown label '" + lab + "'");
  return mLabels.find(lab)->second;
}

LIns *FragmentAssembler::assemble_jump(bool isCond) {
  LIns *condition;
...
LIns *FragmentAssembler::assemble_load() {
  ...
  need(2);
  if (mTokens[1].find("0x") == 0 || mTokens[1].find("0X") == 0 ||
      mTokens[1].find_first_of("0123456789") == 0) {
    return mLir->insLoad(mOpcode, ref(mTokens[0]), immI(mTokens[1]),
                         ACCSET_OTHER);
  }
...
LIns *FragmentAssembler::assemble_call(const string &op) {
  ...
  if (isBuiltin) {
    ...
    if (mOpcode == LIR_callv)
      retType = ARGTYPE_V;
    else if (mOpcode == LIR_calli)
      retType = ARGTYPE_I;
#ifdef NANOJIT_64BIT
    else if (mOpcode == LIR_callq)
      retType = ARGTYPE_Q;
#endif
    else if (mOpcode == LIR_calld)
      retType = ARGTYPE_D;
    else if (mOpcode == LIR_callf)
      retType = ARGTYPE_F;
    else if (mOpcode == LIR_callf4)
      retType = ARGTYPE_F4;
    else
      nyi("callh");
...
LIns *FragmentAssembler::assemble_ret(ReturnType rt) {
  need(1);
  mReturnTypeBits |= rt;
  return mLir->ins1(mOpcode, ref(mTokens[0]));
}

LasmSideExit *FragmentAssembler::createSideExit() {
...
GuardRecord *FragmentAssembler::createGuardRecord(LasmSideExit *exit) {
...
LIns *FragmentAssembler::assemble_guard(bool isCond) {
...
LIns *FragmentAssembler::assemble_guard_xov() {
  GuardRecord *guard = createGuardRecord(createSideExit());
...
LIns *FragmentAssembler::assemble_jump_jov() {
  need(3);
...
void FragmentAssembler::endFragment() {
  // Resolve all of the jumps in this fragment.
  resolve_jumps();

  if (mReturnTypeBits == 0) {
    cerr << "warning: no return type in fragment '" << mFragName << "'" << endl;

  } else if (mReturnTypeBits != RT_INT &&
#ifdef NANOJIT_64BIT
             mReturnTypeBits != RT_QUAD &&
#endif
             mReturnTypeBits != RT_DOUBLE && mReturnTypeBits != RT_FLOAT &&
             mReturnTypeBits != RT_FLOAT4 && mReturnTypeBits != RT_GUARD) {
    cerr << "warning: multiple return types in fragment '" << mFragName << "'"
         << endl;
  }
  ...
  mParent.mAssm.compile(mFragment, mParent.mAlloc,
                        optimize verbose_only(, mParent.mLirbuf->printer));
...
void FragmentAssembler::tokenizeLine(LirTokenStream &in, LirToken &token) {
  mTokens.clear();
...
void FragmentAssembler::extract_any_label(string &lab, char lab_delim) {
  if (mTokens.size() > 2 && mTokens[1].size() == 1 &&
      mTokens[1][0] == lab_delim) {
...
void FragmentAssembler::resolve_jumps() {
...
void FragmentAssembler::add_jump_label(const string &lab, LIns *ins) {
  if (mJumpLabels.find(lab) != mJumpLabels.end())
    bad("Label '" + lab + "' found at multiple locations.");
  mJumpLabels[lab] = ins;
}

void FragmentAssembler::assembleFragment(LirTokenStream &in, bool implicitBegin,
                                         const LirToken *firstToken) {
  LirToken token;
  while (true) {
    if (firstToken) {
...
#endif
      need(1);
      ins = mLir->ins1(mOpcode, ref(mTokens[0]));
      break;
    ...
    case LIR_gef:
      ...
      CASESF(LIR_ii2d:)
      need(2);
      ins = mLir->ins2(mOpcode, ref(mTokens[0]), ref(mTokens[1]));
      break;
    ...
    case LIR_cmovf4:
      need(3);
      ins = mLir->ins3(mOpcode, ref(mTokens[0]), ref(mTokens[1]),
                       ref(mTokens[2]));
      break;
    ...
    case LIR_immf4:
      ...
      // DebugBreak();
      ins =
          mLir->insImmF4(immF4(mTokens[0], mTokens[1], mTokens[2], mTokens[3]));
      break;
    ...
    case LIR_stf4:
      need(3);
      ins = mLir->insStore(mOpcode, ref(mTokens[0]), ref(mTokens[1]),
    ...
    case LIR_paramp:
      need(2);
      ins = mLir->insParam(immI(mTokens[0]), immI(mTokens[1]));
      break;
...
static inline size_t rnd(size_t lim) {
  size_t i = size_t(rand());
  return i % lim;
}

// Returns an int32_t in the range -RAND_MAX..RAND_MAX.
static inline int32_t rndI32() { return (rnd(2) ? 1 : -1) * rand(); }
...
static inline uint32_t rndU32() {
  return uint32_t(rnd(2) ? 0 : RAND_MAX) + uint32_t(rand());
}

template <typename t> t rndPick(vector<t> &v) {
  assert(!v.empty());
  return v[rnd(v.size())];
}

bool min16bytes(const LIns *i) { return i->size() >= 16; }

template <typename t> t *rndPickCond(vector<t *> &v, bool (*cond)(const t *v)) {
  size_t idx = 0;
  ...
  NanoAssert(false); // should've found at least an element that satisfies the
                     // condition
  return v[idx];
}

// Add the operand, and retire an old one if we have too many.
template <typename t> void addOrReplace(vector<t> &v, t x) {
...
static int32_t rndOffset32(size_t szB) { return int32_t(rnd(szB)) & ~3; }

// Returns an 8-aligned address within the give size.
static int32_t rndOffset64(size_t szB) { return int32_t(rnd(szB)) & ~7; }
// Returns an 16-aligned address within the give size.
static int32_t rndOffset128(size_t szB) { return int32_t(rnd(szB)) & ~0xf; }

static int32_t f_I_I1(int32_t a) { return a; }

static int32_t f_I_I6(int32_t a, int32_t b, int32_t c, int32_t d, int32_t e,
                      int32_t f) {
  return a + b + c + d + e + f;
}

#ifdef NANOJIT_64BIT
static uint64_t f_Q_Q2(uint64_t a, uint64_t b) { return a + b; }
...
static double f_F_F3(double a, double b, double c) { return a + b + c; }

static double f_F_F8(double a, double b, double c, double d, double e, double f,
                     double g, double h) {
...
const CallInfo ci_I_I6 =
    CI(f_I_I6, CallInfo::typeSig6(ARGTYPE_I, ARGTYPE_I, ARGTYPE_I, ARGTYPE_I,
                                  ARGTYPE_I, ARGTYPE_I, ARGTYPE_I));

#ifdef NANOJIT_64BIT
const CallInfo ci_Q_Q2 =
    CI(f_Q_Q2, CallInfo::typeSig2(ARGTYPE_Q, ARGTYPE_Q, ARGTYPE_Q));
const CallInfo ci_Q_Q7 =
    CI(f_Q_Q7, CallInfo::typeSig7(ARGTYPE_Q, ARGTYPE_Q, ARGTYPE_Q, ARGTYPE_Q,
                                  ARGTYPE_Q, ARGTYPE_Q, ARGTYPE_Q, ARGTYPE_Q));
#endif

const CallInfo ci_F_F3 =
    CI(f_F_F3, CallInfo::typeSig3(ARGTYPE_D, ARGTYPE_D, ARGTYPE_D, ARGTYPE_D));
const CallInfo ci_F_F8 =
    CI(f_F_F8,
       CallInfo::typeSig8(ARGTYPE_D, ARGTYPE_D, ARGTYPE_D, ARGTYPE_D, ARGTYPE_D,
                          ARGTYPE_D, ARGTYPE_D, ARGTYPE_D, ARGTYPE_D));

#ifdef NANOJIT_64BIT
const CallInfo ci_V_IQF =
    CI(f_V_IQF, CallInfo::typeSig3(ARGTYPE_V, ARGTYPE_I, ARGTYPE_Q, ARGTYPE_D));
#endif
...
// - LIR_parami/LIR_paramq (hard to test beyond what is auto-generated in
// fragment
//   prologues)
...
// - LIR_reti/LIR_retq/LIR_retd/LIR_retf (hard to test without having multiple
// fragments)
...
//
void FragmentAssembler::assembleRandomFragment(int nIns) {
...
#if !NJ_SOFTFLOAT_SUPPORTED
  // Don't emit LIR_{ui,i}2d for soft-float platforms because the soft-float
  // filter removes them.
...
#endif
#if !NJ_SOFTFLOAT_SUPPORTED
  // Don't emit LIR_d2i for soft-float platforms because the soft-float filter
  // removes it.
  ...
  while (n < nIns) {
    ...
    case LIMM_I: {
      ...
      case 1:
        ...
        break;
      case 2:
        immI = (int32_t)(4 * (rnd(256) + 1));
        break; // 4, 8, ..., 1024
      case 3:
        immI = (int32_t)(rnd(19999) - 9999);
        break; // -9999..9999
      case 4:
        immI = rndI32();
        break; // -RAND_MAX..RAND_MAX
      }
    ...
    case LIMM_Q: {
      ...
      case 2:
        ...
        break; // 4, 8, ..., 1024
      case 3:
        imm64 = rnd(19999) - 9999;
        break; // -9999..9999
      case 4:
        imm64 = uint64_t(rndU32()) << 32 | rndU32();
        break; // possibly big!
      }
      ...
      if (!Is.empty()) {
        ...
        if (op == LIR_divi || op == LIR_modi) {
          ...
          if (!lhs->isImmI() || !rhs->isImmI()) {
            ...
            // exception.  If it's -1 and the dividend is -2147483648 (-2^31) we
            // get an exception (and this has been encountered in practice).  So
            // we only allow positive divisors, ie. compute:  lhs / (rhs > 0 ?
            // rhs : -k), where k is a random number in the range 2..100 (this
            // ensures we have some negative divisors).
            LIns *gt0 = mLir->ins2ImmI(LIR_gti, rhs, 0);
            LIns *rhs2 = mLir->ins3(LIR_cmovi, gt0, rhs,
                                    mLir->insImmI(-((int32_t)rnd(99)) - 2));
            ...
            if (op == LIR_divi) {
              ...
              // Add 'div' to the operands too so it might be used again,
              // because the code generated is different as compared to the case
              // where 'div' isn't used again.
              addOrReplace(Is, div);
              addOrReplace(Is, ins);
              n += 6;
            }
    ...
    case LOP_I_BII:
      if (!Bs.empty() && !Is.empty()) {
        ins = mLir->ins3(rndPick(I_BII_ops), rndPick(Bs), rndPick(Is),
                         rndPick(Is));
        addOrReplace(Is, ins);
        n++;
      }
    ...
    case LOP_Q_BQQ:
      if (!Bs.empty() && !Qs.empty()) {
        ins = mLir->ins3(rndPick(Q_BQQ_ops), rndPick(Bs), rndPick(Qs),
                         rndPick(Qs));
        addOrReplace(Qs, ins);
        n++;
      }
    ...
    case LOP_D_BDD:
      if (!Bs.empty() && !Ds.empty()) {
        ins = mLir->ins3(rndPick(D_BDD_ops), rndPick(Bs), rndPick(Ds),
                         rndPick(Ds));
        addOrReplace(Ds, ins);
        n++;
      }
    ...
    case LOP_F_BFF:
      if (!Bs.empty() && !Fs.empty()) {
        ins = mLir->ins3(rndPick(F_BFF_ops), rndPick(Bs), rndPick(Fs),
                         rndPick(Fs));
        addOrReplace(Fs, ins);
        n++;
      }
    ...
    case LOP_F4_BF4F4:
      if (!Bs.empty() && !F4s.empty()) {
        ins = mLir->ins3(rndPick(F4_BF4F4_ops), rndPick(Bs), rndPick(F4s),
                         rndPick(F4s));
        addOrReplace(F4s, ins);
        n++;
      }
...
// XXX: NativeX64 doesn't implement qhi yet (and it may not need to).
    ...
    case LLD_D:
      if (!M8ps.empty()) {
        LIns *base = rndPick(M8ps);
        ins = mLir->insLoad(rndPick(D_loads), base, rndOffset64(base->size()),
                            ACCSET_OTHER);
        addOrReplace(Ds, ins);
        n++;
      }
    ...
    case LLD_F: {
      ...
      if (!Ms.empty()) {
        LIns *base = rndPick(Ms);
        ins = mLir->insLoad(rndPick(F_loads), base, rndOffset32(base->size()),
                            ACCSET_OTHER);
        addOrReplace(Fs, ins);
        n++;
      }
    ...
    case LLD_F4: {
      if (!M8ps.empty()) {
        ...
        ins = mLir->insLoad(rndPick(F4_loads), base, rndOffset128(base->size()),
                            ACCSET_OTHER);
        addOrReplace(F4s, ins);
        n++;
      }
    ...
    case LST_I: {
      ...
      if (!Ms.empty() && !Is.empty()) {
        LIns *base = rndPick(Ms);
        mLir->insStore(rndPick(Is), base, rndOffset32(base->size()),
                       ACCSET_OTHER);
        n++;
      }
      break;
    ...
    case LST_Q:
      if (!M8ps.empty() && !Qs.empty()) {
        LIns *base = rndPick(M8ps);
        mLir->insStore(rndPick(Qs), base, rndOffset64(base->size()),
                       ACCSET_OTHER);
        n++;
      }
      break;
    ...
    case LST_D:
      if (!M8ps.empty() && !Ds.empty()) {
        LIns *base = rndPick(M8ps);
        mLir->insStore(rndPick(Ds), base, rndOffset64(base->size()),
                       ACCSET_OTHER);
        n++;
      }
      break;
    ...
    case LST_F: {
      ...
      if (!Ms.empty() && !Fs.empty()) {
        LIns *base = rndPick(Ms);
        mLir->insStore(rndPick(Fs), base, rndOffset32(base->size()),
                       ACCSET_OTHER);
        n++;
      }
      break;
    ...
    case LST_F4: {
      if (!M8ps.empty() && !F4s.empty()) {
        LIns *base = rndPick(M8ps);
        mLir->insStore(rndPick(F4s), base, rndOffset128(base->size()),
                       ACCSET_OTHER);
        n++;
      }
      break;
...
Lirasm::Lirasm(bool verbose, Config &config)
    : mConfig(config), mAssm(mCodeAlloc, mAlloc, mAlloc, &mLogc, mConfig) {
  mVerbose = verbose;
  mLogc.lcbits = 0;
...
#ifdef DEBUG
  if (mVerbose) {
    mLogc.lcbits = LC_ReadLIR | LC_AfterDCE | LC_Native | LC_RegAlloc |
...
#define OP___(op, repKind, retType, isCse) mOpMap[#op] = LIR_##op;
...
bool Lirasm::lookupFunction(const string &name, CallInfo *&ci) {
  ...
  if (func != mFragments.end()) {
    ...
    } else if (func->second.mReturnType == RT_FLOAT4) {
      CallInfo target = {(uintptr_t)func->second.rfloat4, 0, ABI_FASTCALL,
                         /*isPure*/ 0,
                         ACCSET_STORE_ANY verbose_only(, func->first.c_str())};
      *ci = target;
...
void Lirasm::assemble(istream &in, bool optimize) {
...
void Lirasm::assembleRandom(int nIns, bool optimize) {
  string name = "main";
  FragmentAssembler assembler(*this, name, optimize);
  assembler.assembleRandomFragment(nIns);
}

void Lirasm::handlePatch(LirTokenStream &in) {
...
void usageAndQuit(const string &progname) {
  cout
      << "usage: " << progname
      << " [options] [filename]\n"
         "Options:\n"
         ...
         "  --execute         execute LIR\n"
         "  --[no-]optimize   enable or disable optimization of the LIR "
         "(default=off)\n"
         "  --random [N]      generate a random LIR block of size N "
         "(default=100)\n"
         "  --stkskip [N]     push approximately N Kbytes of stack before "
         "execution (default=100)\n"
         "\n"
         "Build query options (these print a value for this build of lirasm "
         "and exit)\n"
         "  --show-arch       show the architecture ('i386', 'X64', 'arm', "
         "'ppc',\n"
         "                    'sparc', 'mips', or 'sh4')\n"
         "  --show-word-size  show the word size ('32' or '64')\n"
         "  --show-endianness show the endianness ('little-endian' or "
         "'big-endian')\n"
         "\n"
         ...
         "  --arch N          use ARM architecture version N instructions "
         "(default=7)\n"
...
void errMsgAndQuit(const string &progname, const string &msg) {
...
bool parseOptionalInt(int argc, char **argv, int *i, int *value,
                      int defaultValue) {
  if (*i == argc - 1) {
...
static void processCmdLine(int argc, char **argv, CmdLineOptions &opts) {
    ...
    if (arg == "-h" || arg == "--help")
      ...
      if (!parseOptionalInt(argc, argv, &i, &opts.random, 100))
        errMsgAndQuit(opts.progname,
                      "--random argument must be greater than zero");
    } else if (arg == "--stkskip") {
      if (!parseOptionalInt(argc, argv, &i, &opts.stkskip, 100))
        errMsgAndQuit(opts.progname,
                      "--stkskip argument must be greater than zero");
...
#error "unknown arch"
  ...
  if ((!opts.random && opts.filename.empty()) ||
      (opts.random && !opts.filename.empty()))
    errMsgAndQuit(
        opts.progname,
  ...
  if (((arm_arch == 5) && (arm_vfp)) || ((arm_arch >= 6) && (!arm_vfp))) {
    ...
    cerr << "Warning: This configuration (ARMv" << arm_arch << ", "
         << vfp_string << ") "
...
void executeFragment(const LirasmFragment &fragment, int skip) {
...
int main(int argc, char **argv) {
  ...
  if (opts.execute) {
    ...
    if (i == lasm.mFragments.end())
      errMsgAndQuit(opts.progname,
                    "error: at least one fragment must be named 'main'");
