...
void *nanojit::CodeAlloc::allocCodeChunk(size_t nbytes) {
  return VirtualAlloc(NULL, nbytes, MEM_COMMIT | MEM_RESERVE,
                      PAGE_EXECUTE_READWRITE);
}

void nanojit::CodeAlloc::freeCodeChunk(void *p, size_t) {
...
void *nanojit::CodeAlloc::allocCodeChunk(size_t nbytes) {
  ...
  if (DosAllocMem(&addr, nbytes,
                  OBJ_ANY | PAG_COMMIT | PAG_READ | PAG_WRITE | PAG_EXECUTE)) {
    if (DosAllocMem(&addr, nbytes,
...
void nanojit::CodeAlloc::freeCodeChunk(void *p, size_t nbytes) {
  DosFreeMem(p);
}
...
void *nanojit::CodeAlloc::allocCodeChunk(size_t nbytes) {
  return mmap(NULL, nbytes, PROT_READ | PROT_WRITE | PROT_EXEC,
...
void nanojit::CodeAlloc::markCodeChunkWrite(void *, size_t) {}

void nanojit::CodeAlloc::markCodeChunkExec(void *, size_t) {}

bool nanojit::CodeAlloc::checkChunkMark(void * /*addr*/, size_t /*nbytes*/,
                                        bool /*isExec*/) {
