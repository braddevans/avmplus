...
public:
  ...
  const wchar *readFileForEval(const wchar *basename, const wchar *filename,
                               uint32_t *inputlen);
...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    // FIXME: want an option to control public_by_default (which is false, by
    // default)
    if (options) {
    ...
    fseek(fp, 0, SEEK_END) == 0 ||
        fail("Could not seek on input file: %s", filename);
    ...
    fread(text, 1, len, fp) == len ||
        fail("Could not read from file: %s\n", filename);
    fclose(fp);
    ...
    if (!context.stopAfterParse) {
      ...
      fwrite(context.result, 1, context.result_size, fp) ==
              context.result_size ||
          fail("Could not write to output: %s\n", outfn);
      fclose(fp);
    }
...
bool MathUtils::isNaN(double d) { return isnan(d); }

bool MathUtils::isInfinite(double d) { return isinf(d); }

AvmcHostContext::AvmcHostContext()
    : result(NULL), result_size(0), include_nesting(0) {}
...
const wchar *AvmcHostContext::constructName(const wchar *basename,
  ...
  if (basename != NULL && filename[0] != '/'
    ...
    VMPI_memcpy(canonical_filename, basename,
                sizeof(wchar) * (basesep - basename + 1));
    VMPI_memcpy(canonical_filename + (basesep - basename + 1), filename,
                sizeof(wchar) * (fnlen + 1));
...
// circularities in the include files) we depend on the parser calling freeInput
// as soon as it's finished consuming the input for a particular file, and not
// batching the freeInput calls at the end.
//
// The responsibility for freeing input buffers when errors are signaled falls
// to the host context; since this is a batch compiler and we just abort we
// don't need to do anything.

const wchar *AvmcHostContext::readFileForEval(const wchar *basename,
                                              const wchar *filename,
                                              uint32_t *inputlen) {
  ...
  const wchar *canonical_filename =
      constructName(basename, filename); // canonical_filename will be either
                                         // 'filename' or a heap-allocated array
  ...
  // Terminate and return.  freeInput will be called to free the buffer after
  // parsing.
