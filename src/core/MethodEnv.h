...
namespace avmplus {
    ...
    *method,
    ...
    ArrayObject *createRest(Atom * argv, int32_t
    argc);
    Atom getpropertylate_i(Atom
    obj,
    int32_t index
    ) const;
    Atom getpropertylate_u(Atom
    obj,
    uint32_t index
    ) const;
    Atom getpropertylate_d(Atom
    obj,
    double index
    ) const;
    /*bool*/ int32_t haspropertylate_i(Atom
    obj,
    int32_t index
    ) const;
    /*bool*/ int32_t haspropertylate_u(Atom
    obj,
    uint32_t index
    ) const;

#if defined VMCFG_NANOJIT || defined VMCFG_AOT
    void setpropertyHelper(Atom obj, /* not const */ Multiname *multi, Atom value,
                           VTable *vtable, Atom index);
    void initpropertyHelper(Atom obj, /* not const */ Multiname *multi,
                            Atom value, VTable *vtable, Atom index);
    Atom getpropertyHelper(Atom obj, /* not const */ Multiname *multi,
                           VTable *vtable, Atom index);
    Atom delpropertyHelper(Atom obj, /* not const */ Multiname *multi,
                           Atom index);
     ...
     */
    ScriptObject *op_newobject(Atom * sp, int32_t
    argc) const;
    ...
    int32_t hasnext(Atom
    objAtom,
    int32_t index
    ) const;

    /** Implementation of OP_hasnext2 */
    int32_t hasnextproto(Atom & objAtom, int & index)
    const;
     ...
     */
    ClassClosure *newfunction(MethodInfo * function, Atom * scopes)
    const;
    ...
    ClassClosure *newclass(Traits *ctraits, ClassClosure *base, ScopeChain *outer,
                           Atom *scopes) const;

    void initproperty(Atom
    obj,
    const Multiname *multiname, Atom
    value,
    VTable *vtable
    ) const;
    void setpropertylate_i(Atom
    obj,
    int32_t index, Atom
    value) const;
    void setpropertylate_u(Atom
    obj,
    uint32_t index, Atom
    value) const;
    void setpropertylate_d(Atom
    obj,
    double index, Atom
    value) const;
#ifdef VMCFG_FLOAT
    ...
    void setsuper(Atom
    obj,
    const Multiname *name, Atom
    value) const;

    /** Implementation of OP_findproperty */
    Atom findproperty(ScopeChain *outer, Atom *scopes, int32_t extraScopes,
                      const Multiname *multiname, bool strict, Atom *withBase);

    /** Like findproperty, but ignoring all lexical and 'this' scopes.  Returns
     * NULL if property could not be found; caller should signal strict error or
     * return the target_global as appropriate.
     */
    Atom findglobalproperty(Atom
    target_global,
    const Multiname *multiname
    );

    Namespace *internRtns(Atom
    ns);

    /** Creates the arguments array */
    ArrayObject *createArguments(Atom * atomv, int32_t
    argc);

    /**
     * E4X descendants operator (..)
     */
    Atom getdescendants(Atom
    obj,
    const Multiname *multiname
    );
     ...
     * E4X filter operator
     */
    void checkfilter(Atom
    obj);

#ifdef DEBUGGER
    void debugEnter(const uint8_t *frame_sst, CallStackNode *callstack,
    ...
    enum {
        kActivation = 0, kMethodTable, kActivationMethodTablePair
    };
...
ScriptEnv(MethodInfo
*_method,
ScopeChain *_scope
);

public:
REALLY_INLINE static ScriptEnv
*
create(MMgc::GC
*gc,
MethodInfo *_method,
...
ScriptEnv(_method, _scope
);
}

ScriptObject *initGlobal();

static ScopeChain *createScriptScope(const ScopeTypeChain *stc,
                                     VTable *_vtable, AbcEnv *_abcEnv);

// ------------------------ DATA SECTION BEGIN
GC_DATA_BEGIN(ScriptEnv)

public:
GCMember <ScriptObject>
        GC_POINTER(global); // initially null, set after initialization
...
FunctionEnv(MethodInfo
*_method,
ScopeChain *_scope
);

public:
REALLY_INLINE static FunctionEnv
*
create(MMgc::GC
*gc,
MethodInfo *_method,
