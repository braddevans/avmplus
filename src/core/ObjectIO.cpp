...
void ObjectOutputObject::writeBytes(ByteArrayObject *bytes, uint32_t offset,
                                    uint32_t length) {
...
void ObjectOutputObject::writeObject(Atom value) {
  ...
  // WriteAtom() explicitly. This assumes AMF3 is the only version for this
  // code.
...
void ObjectOutputObject::set_objectEncoding(uint32_t objectEncoding) {
  if ((objectEncoding == kAMF3) || (objectEncoding == kAMF0)) {
    ...
    toplevel()->argumentErrorClass()->throwError(
        kInvalidEnumError, core()->toErrorString("objectEncoding"));
  }
}

Stringp ObjectOutputObject::get_endian() {
  return (m_out->GetEndian() == kBigEndian) ? core()->kbigEndian
                                            : core()->klittleEndian;
}
...
void ObjectOutputObject::set_endian(Stringp type) {
  ...
  if (type == core()->kbigEndian) {
    ...
    toplevel()->argumentErrorClass()->throwError(
        kInvalidEnumError, core()->toErrorString(kStrTypeValue));
  }
}
...
void ObjectInputObject::readBytes(ByteArrayObject *bytes, uint32_t offset,
                                  uint32_t length) {
  toplevel()->checkNull(bytes, "bytes");
...
bool ObjectInputObject::readBoolean() { return m_in->ReadBoolean(); }

int ObjectInputObject::readByte() { return (int8_t)m_in->ReadU8(); }

int ObjectInputObject::readUnsignedByte() { return m_in->ReadU8(); }

int ObjectInputObject::readShort() { return (int16_t)m_in->ReadU16(); }

int ObjectInputObject::readUnsignedShort() { return m_in->ReadU16(); }

int ObjectInputObject::readInt() { return (int32_t)m_in->ReadU32(); }

uint32_t ObjectInputObject::readUnsignedInt() { return m_in->ReadU32(); }

double ObjectInputObject::readFloat() { return m_in->ReadFloat(); }

double ObjectInputObject::readDouble() { return m_in->ReadDouble(); }
...
String *ObjectInputObject::readUTF() { return m_in->ReadUTF(); }

String *ObjectInputObject::readUTFBytes(uint32_t length) {
  return m_in->ReadUTFBytes(length);
}

int ObjectInputObject::get_bytesAvailable() { return m_in->Available(); }
...
Stringp ObjectInputObject::get_endian() {
  return (m_in->GetEndian() == kBigEndian) ? core()->kbigEndian
                                           : core()->klittleEndian;
}
...
DynamicPropertyOutputClass::DynamicPropertyOutputClass(VTable *vtable)
    : ClassClosure(vtable) {
  setPrototypePtr(toplevel()->objectClass->construct());
}

void DynamicPropertyOutputObject::writeDynamicProperty(Stringp name,
                                                       Atom value) {
  AvmAssert(NULL !=
            m_out); // this should have been set after constructing the object
  if (NULL != m_out) {
...
ObjectEncodingClass::ObjectEncodingClass(VTable *vtable)
    : ClassClosure(vtable),
      m_writer(GCREF_NULL(IDynamicPropertyWriterInterface)) {
  setPrototypePtr(toplevel()->objectClass->construct());
}

ObjectEncodingClass::~ObjectEncodingClass() { m_writer = NULL; }
} // namespace avmplus
