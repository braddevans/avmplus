...
// The macro VMCFG_WORDCODE is true if the representation of ABC code is as an
// array of words and not an array of bytes.

namespace avmplus {
// an auto-ptr for managing MethodFrame calls.
class EnterMethodEnv {
public:
  inline explicit EnterMethodEnv(AvmCore *core, MethodEnv *env,
                                 MethodFrame &frame)
      : m_core(core), m_frame(frame) {
    m_frame.enter(m_core, env);
  }

  inline ~EnterMethodEnv() { m_frame.exit(m_core); }

  void setCurrent() { m_core->currentMethodFrame = &m_frame; }
...
REALLY_INLINE Atom CHECK_INT_ATOM(Atom a) {
  AvmAssert(atomKind(a) == kIntptrType &&
            atomIsValidIntptrValue(atomGetIntptr(a)));
  return a;
}
...
#ifdef VMCFG_FLOAT
#define IS_FLOAT(v)                                                            \
  ((((v)&7) == kSpecialBibopType) && ((v) != AtomConstants::undefinedAtom) &&  \
   bibopKind(v) == kBibopFloatType)
#define IS_FLOAT4(v)                                                           \
  ((((v)&7) == kSpecialBibopType) && ((v) != AtomConstants::undefinedAtom) &&  \
   bibopKind(v) == kBibopFloat4Type)
#endif // VMCFG_FLOAT
// note that the argument to SIGN_EXTEND is expected to be upshifted 3 bits (not
// a "raw" intptr), but it doesn't expect or require the tag bits to be set
// properly.
#ifdef AVMPLUS_64BIT
// since 64-bit int atoms expect exactly 54 bits of precision, we want to shift
// bit 54+3 up into the sign bit and back down
#define SIGN_EXTEND(v)                                                         \
  ((intptr_t(v) << (atomSignExtendShift - AtomConstants::kAtomTypeSize)) >>    \
   (atomSignExtendShift - AtomConstants::kAtomTypeSize))
#else
#define SIGN_EXTEND(v) (intptr_t(v))
#endif

// CLAMP_32 is equivalent to running an int atom thru AvmCore::integer (ie,
// truncate to int32_t using the right rules), but, like SIGN_EXTEND, it expects
// the argument to be upshifted 3 bit.
#ifdef AVMPLUS_64BIT
#define CLAMP_32(v)                                                            \
  ((intptr_t(v) << (32 - AtomConstants::kAtomTypeSize)) >>                     \
   (32 - AtomConstants::kAtomTypeSize))
#else
...
#endif
#define IS_BOTH_KIND(a, b, k)                                                  \
  (((((a) ^ (k)) | ((b) ^ (k))) & 7) ==                                        \
   0) // less control flow but more registers -- which is better?
#define IS_BOTH_INTEGER(a, b) IS_BOTH_KIND(a, b, kIntptrType)
#define IS_BOTH_DOUBLE(a, b) IS_BOTH_KIND(a, b, kDoubleType)
#ifdef VMCFG_FLOAT
#define IS_BOTH_BIBOP(a, b, k)                                                 \
  IS_BOTH_KIND(a, b, kSpecialBibopType) &&                                     \
      ((a) != AtomConstants::undefinedAtom) &&                                 \
      ((b) != AtomConstants::undefinedAtom) &&                                 \
...
inline intptr_t readS24(const uint8_t *pc) { return AvmCore::readS24(pc); }

inline uintptr_t readU30(const uint8_t *&pc) { return AvmCore::readU32(pc); }
#endif
...
static Traits *getTraits(const Multiname *name, PoolObject *pool,
                         Toplevel *toplevel, AvmCore *core);
#ifdef AVMPLUS_VERBOSE
// display contents of current stack frame only
static void showState(MethodInfo *info, const bytecode_t *code_start,
                      const bytecode_t *pc, Atom *framep, Atom *spp,
                      int scopeDepth, Atom *scopebasep, int max_scope);
#endif
 ...
 */
REALLY_INLINE void branchCheck(AvmCore *core, MethodEnv *env,
                               bool interruptable) {
#ifdef DEBUGGER
  core->sampleCheck();
#endif
...
#ifdef MSVC_X86_ASM_THREADING
#pragma warning(                                                               \
    disable : 4740) // "inline assembler suppresses global optimization"
#endif
#endif // _MSC_VER
...
void **interpGetOpcodeLabels() {
  ...
  //
  // If we ever enable direct threading for a platform where the thread jump
  // targets cannot be constants in a vector but must be written into a vector
  // the first time interpGetOpcodeLabels() is called, then the following call
  // must be within a critical section (or some startup code must make an
  // initial call to make sure the vector is initialized in a thread safe way).
  // At this time, Visual C++ requires this, but we're not using direct
  // threading with Visual C++, so there is no critical section here.
  //
  // The startup code to make the initial call can simply call this function and
  // assign the result to a dummy static global variable; that trick is not
  // portable but it can be used for many platforms (and is used elsewhere).

#if defined MSVC_X86_ASM_THREADING || defined MSVC_X86_REWRITE_THREADING
#error                                                                         \
    "interpGetOpcodeLabels needs a critical section or eager initialization for this compiler / platform combination"
#endif
 ...
 * |initMultiname()|
 *
 * The problem only happens on Pepper linux x86 build, and it consistently
 * crashes when running testID=18467 in ATS10AS3, refer to bug# 3831468. Bug#
 * 3831882 is also related to this issue.
 *
 * We found that GCC 4.4.2/4.4.3/4.4.4 all have this problem, while GCC 4.4.5
 * and later do not have this issue.
 *
 * We should remove this __attribute__ once we upgrade GCC compiler to GCC 4.4.5
 * and later.
 */
#if defined(PEPPER_PLUGIN) && defined(AVMPLUS_UNIX) && !defined(PEPPER_MAC) && \
    !defined(OS_CHROMEOS) && defined(VMCFG_32BIT) && !defined(DEBUG)
Atom __attribute__((optimize("O1")))
interpBoxed(register MethodEnv *env, register int _argc, register Atom *_atomv)
#else
Atom interpBoxed(register MethodEnv *env, register int _argc,
                 register Atom *_atomv)
#endif
{
#ifdef VMCFG_DIRECT_THREADED

  // If env is NULL return the jump table.  Optionally initialize it here on
  // those platforms where compile-time initialization is not possible or
  // practical.
...
#define IID(a, b) XXX(a)
#endif
      XXX(0x00) XXX(0x01) /* OP_bkpt */
      III(0x02, L_nop) III(0x03, L_throw) III(0x04, L_getsuper)
          III(0x05, L_setsuper) III(0x06, L_dxns) III(0x07, L_dxnslate)
              III(0x08, L_kill) XXX(0x09) /* OP_label */
#ifdef VMCFG_FLOAT
      III(0x0A, L_lf32x4) III(0x0B, L_sf32x4)
#else
      XXX(0x0A)
      XXX(0x0B)
#endif
          III(0x0C, L_ifnlt) III(0x0D, L_ifnle) III(0x0E, L_ifngt) III(0x0F,
                                                                       L_ifnge)
              III(0x10, L_jump) III(0x11, L_iftrue) III(0x12, L_iffalse)
                  III(0x13, L_ifeq) III(0x14, L_ifne) III(0x15, L_iflt)
                      III(0x16, L_ifle) III(0x17, L_ifgt) III(0x18, L_ifge)
                          III(0x19, L_ifstricteq) III(0x1A, L_ifstrictne)
                              III(0x1B, L_lookupswitch) III(0x1C, L_pushwith)
                                  III(0x1D, L_popscope) III(0x1E, L_nextname)
                                      III(0x1F, L_hasnext) III(0x20, L_pushnull)
      ...
      XXX(0x22)
#endif
                                                  III(0x23, L_nextvalue) XXX(
                                                      0x24) /* OP_pushbyte */
      XXX(0x25)                                             /* OP_pushshort */
      III(0x26, L_pushtrue) III(0x27, L_pushfalse) III(0x28, L_pushnan)
          III(0x29, L_pop) III(0x2A, L_dup) III(0x2B, L_swap)
              III(0x2C, L_pushstring) XXX(0x2D) /* OP_pushint */
      XXX(0x2E)                                 /* OP_pushuint */
      III(0x2F, L_pushdouble) III(0x30, L_pushscope) III(0x31, L_pushnamespace)
          III(0x32, L_hasnext2) XXX(0x33) XXX(0x34) IIM(0x35, L_li8)
              IIM(0x36, L_li16) IIM(0x37, L_li32) IIM(0x38, L_lf32)
                  IIM(0x39, L_lf64) IIM(0x3A, L_si8) IIM(0x3B, L_si16)
                      IIM(0x3C, L_si32) IIM(0x3D, L_sf32) IIM(0x3E, L_sf64) XXX(
                          0x3F) III(0x40, L_newfunction) III(0x41, L_call)
                          III(0x42, L_construct) III(0x43, L_callmethod) III(
                              0x44, L_callstatic) III(0x45, L_callsuper)
                              III(0x46, L_callproperty) III(0x47, L_returnvoid)
                                  III(0x48, L_returnvalue)
                                      III(0x49, L_constructsuper)
                                          III(0x4A, L_constructprop)
                                              XXX(0x4B) /* OP_callsuperid */
      III(0x4C, L_callproplex) XXX(0x4D)                /* OP_callinterface */
      III(0x4E, L_callsupervoid) III(0x4F, L_callpropvoid) IIM(
          0x50, L_sxi1) IIM(0x51, L_sxi8) IIM(0x52, L_sxi16) III(0x53,
                                                                 L_applytype)
#ifdef VMCFG_FLOAT
      ...
      XXX(0x54)
#endif
              III(0x55, L_newobject) III(0x56, L_newarray) III(
                  0x57, L_newactivation) III(0x58, L_newclass)
                  III(0x59, L_getdescendants) III(0x5A, L_newcatch) XXX(
                      0x5B) XXX(0x5C) III(0x5D, L_findpropstrict)
                      III(0x5E, L_findproperty) III(0x5F, L_finddef) III(
                          0x60, L_getlex) III(0x61, L_setproperty)
                          III(0x62, L_getlocal) III(0x63, L_setlocal) III(
                              0x64,
                              L_getglobalscope) III(0x65, L_getscopeobject)
                              III(0x66, L_getproperty) III(
                                  0x67, L_getouterscope) III(0x68,
                                                             L_initproperty)
                                  XXX(0x69) III(0x6A, L_deleteproperty) XXX(
                                      0x6B) III(0x6C, L_getslot)
                                      III(0x6D, L_setslot) III(
                                          0x6E,
                                          L_getglobalslot) III(0x6F,
                                                               L_setglobalslot)
                                          III(0x70, L_convert_s) III(
                                              0x71,
                                              L_esc_xelem) III(0x72,
                                                               L_esc_xattr)
                                              III(0x73, L_convert_i) III(
                                                  0x74,
                                                  L_convert_u) III(0x75,
                                                                   L_convert_d)
                                                  III(0x76, L_convert_b) III(
                                                      0x77,
                                                      L_convert_o) III(0x78,
                                                                       L_checkfilter)
#ifdef VMCFG_FLOAT
                                                      III(0x79, L_convert_f) III(
                                                          0x7A, L_unplus)
                                                          III(0x7B,
                                                              L_convert_f4)
#else
      ...
      XXX(0x7B)
                                                              ...
                                                              XXX(0x7C) XXX(
                                                                  ...
                                                                  XXX(0x7F) III(
                                                                      ...
                                                                      L_coerce)
                                                                      III(0x81,
                                                                          L_convert_b) // coerce_b -> convert_b, they are the same
      ...
      III(0x85, L_coerce_s) III(0x86, L_astype) III(0x87, L_astypelate)
          III(0x88, L_convert_u) // coerce_u -> convert_u, they are the same
      III(0x89, L_coerce_o) XXX(0x8A) XXX(0x8B) XXX(0x8C) XXX(0x8D) XXX(
          0x8E) XXX(0x8F) III(0x90, L_negate) III(0x91, L_increment) III(0x92,
                                                                         L_inclocal)
          III(0x93, L_decrement) III(0x94, L_declocal) III(0x95, L_typeof) III(
              0x96,
              L_not) III(0x97,
                         L_bitnot) XXX(0x98) XXX(0x99) XXX(0x9A) XXX(0x9B) XXX(0x9C) XXX(0x9D) XXX(0x9E)
              XXX(0x9F) III(
                  0xA0,
                  L_add) III(0xA1,
                             L_subtract) III(0xA2,
                                             L_multiply) III(0xA3,
                                                             L_divide) III(0xA4,
                                                                           L_modulo) III(0xA5,
                                                                                         L_lshift) III(0xA6,
                                                                                                       L_rshift)
                  III(0xA7, L_urshift) III(0xA8, L_bitand) III(
                      0xA9,
                      L_bitor) III(0xAA,
                                   L_bitxor) III(0xAB,
                                                 L_equals) III(0xAC,
                                                               L_strictequals)
                      III(0xAD, L_lessthan) III(0xAE, L_lessequals) III(0xAF, L_greaterthan) III(
                          0xB0,
                          L_greaterequals) III(0xB1,
                                               L_instanceof) III(0xB2,
                                                                 L_istype) III(0xB3,
                                                                               L_istypelate) III(0xB4,
                                                                                                 L_in) XXX(0xB5) XXX(0xB6)
                          XXX(0xB7) XXX(
                              0xB8) XXX(0xB9) XXX(0xBA)
                              XXX(0xBB) XXX(
                                  0xBC) XXX(0xBD) XXX(0xBE)
                                  XXX(0xBF) III(0xC0, L_increment_i) III(
                                      0xC1,
                                      L_decrement_i) III(0xC2,
                                                         L_inclocal_i) III(0xC3,
                                                                           L_declocal_i) III(0xC4,
                                                                                             L_negate_i)
                                      III(0xC5, L_add_i) III(0xC6, L_subtract_i) III(0xC7, L_multiply_i) XXX(
                                          0xC8) XXX(0xC9) XXX(0xCA)
                                          XXX(0xCB) XXX(
                                              0xCC) XXX(0xCD) XXX(0xCE)
                                              XXX(0xCF) III(0xD0, L_getlocal0) III(
                                                  0xD1,
                                                  L_getlocal1) III(0xD2,
                                                                   L_getlocal2) III(0xD3, L_getlocal3)
                                                  III(0xD4, L_setlocal0) III(
                                                      0xD5,
                                                      L_setlocal1) III(0xD6,
                                                                       L_setlocal2)
                                                      III(0xD7, L_setlocal3) XXX(0xD8) XXX(
                                                          0xD9) XXX(0xDA) XXX(0xDB)
                                                          XXX(0xDC) XXX(
                                                              0xDD) XXX(0xDE) XXX(0xDF)
                                                              XXX(0xE0) XXX(
                                                                  0xE1) XXX(0xE2) XXX(0xE3)
                                                                  XXX(0xE4) XXX(
                                                                      0xE5) XXX(0xE6) XXX(0xE7)
                                                                      XXX(0xE8) XXX(
                                                                          0xE9) XXX(0xEA)
                                                                          XXX(0xEB) XXX(
                                                                              0xEC) XXX(0xED)
      ...
      XXX(0xF4) XXX(0xF5) XXX(0xF6) XXX(0xF7) XXX(0xF8) XXX(0xF9) XXX(0xFA)
          XXX(0xFB) XXX(0xFC) XXX(0xFD) XXX(0xFE) XXX(0xFF) /* OP_ext */
      XXX(0x100) III(0x101, L_pushbits) III(0x102, L_push_doublebits) IIP(
          0x103, L_get2locals) IIP(0x104, L_get3locals) IIP(0x105, L_get4locals)
          IIP(0x106, L_get5locals) IIP(0x107, L_storelocal) IIP(
              0x108, L_add_ll) IIP(0x109, L_add_set_lll) IIP(0x10A,
                                                             L_subtract_ll)
              IIP(0x10B, L_multiply_ll) IIP(0x10C, L_divide_ll) IIP(
                  0x10D,
                  L_modulo_ll) IIP(0x10E,
                                   L_bitand_ll) IIP(0x10F,
                                                    L_bitor_ll) IIP(0x110,
                                                                    L_bitxor_ll)
                  IIP(0x111, L_add_lb) IIP(0x112, L_subtract_lb) IIP(
                      0x113, L_multiply_lb) IIP(0x114, L_divide_lb) IIP(0x115,
                                                                        L_bitand_lb)
                      IIP(0x116, L_bitor_lb) IIP(0x117, L_bitxor_lb) IIP(
                          0x118, L_iflt_ll) IIP(0x119,
                                                L_ifnlt_ll) IIP(0x11A,
                                                                L_ifle_ll)
                          IIP(0x11B, L_ifnle_ll) IIP(0x11C, L_ifgt_ll) IIP(
                              0x11D, L_ifngt_ll) IIP(0x11E, L_ifge_ll) IIP(0x11F,
                                                                           L_ifnge_ll)
                              IIP(0x120, L_ifeq_ll) IIP(0x121, L_ifne_ll) IIP(
                                  0x122,
                                  L_ifstricteq_ll) IIP(0x123, L_ifstrictne_ll)
                                  IIP(0x124, L_iflt_lb) IIP(
                                      0x125, L_ifnlt_lb) IIP(0x126, L_ifle_lb)
                                      IIP(0x127, L_ifnle_lb) IIP(
                                          0x128, L_ifgt_lb) IIP(0x129,
                                                                L_ifngt_lb)
                                          IIP(0x12A, L_ifge_lb) IIP(
                                              0x12B,
                                              L_ifnge_lb) IIP(0x12C, L_ifeq_lb)
                                              IIP(0x12D, L_ifne_lb) IIP(
                                                  0x12E,
                                                  L_ifstricteq_lb) IIP(0x12F,
                                                                       L_ifstrictne_lb)
                                                  IIP(0x130, L_swap_pop) III(
                                                      0x131, L_findpropglobal)
                                                      III(0x132,
                                                          L_findpropglobalstrict)
#if defined DEBUGGER && defined VMCFG_WORDCODE
                                                          IID(0x133, L_debugenter) IID(
                                                              0x134,
                                                              L_debugexit)
#else
      XXX(0x133)
  ...
  // required to invoke the destructor.  The ExceptionFrame gets such a
  // mechanism when the TRY_UNLESS_HEAPMEM macro is used to initialize it.
  ...
  // programs that use them tend to be quite a bit slower if they're not cached.
  // (In the ABC interpreter we should also cache cpool_int and cpool_uint.) But
...
#ifdef DEBUGGER
  const size_t kAuxFrameSize = core->debugger()
                                   ? sizeof(InterpreterAuxiliaryFrameWithCSN)
...
#ifdef AVMPLUS_64BIT
  // Allocation is guaranteed on an 8-byte boundary, but we need 16 for
  // _setjmpex. So allocate 8 bytes extra, then round up to a 16-byte boundary.
  register Atom *const framep = (Atom *)avmStackAlloc(
      core, _framep, sizeof(Atom) * (ms->frame_size()) + 8 + kAuxFrameSize);
  register InterpreterAuxiliaryFrame *const aux_memory =
      (InterpreterAuxiliaryFrame *)(((uintptr_t)(framep + ms->frame_size()) +
                                     15) &
                                    ~15);
#else
  register Atom *const framep = (Atom *)avmStackAlloc(
      core, _framep, sizeof(Atom) * (ms->frame_size()) + kAuxFrameSize);
  union {
    Atom *fa;
  ...
  Atom /* NOT VOLATILE */ globalScopeAtom =
      (scope->getSize() > 0) ? scope->getScope(0) : nullObjectAtom;
...

  {
    ...
    // Set optional param values.  these not aliased to arguments[] since
    // arguments[] only present with traditional prototype functions (no
    // optional args)
    if (info->hasOptional()) {
      if (_argc < param_count) {
        // initialize default values
        for (int i = _argc + 1, o = _argc + ms->optional_count() - param_count,
                 n = param_count;
             i <= n; i++, o++)
    ...
    // Set remaining locals to undefined.  Don't have to init scope or stack
    // because our conservative GC scan knows how to ignore garbage.
    ...
    if (info->needRest()) {
      ...
      // Even tho E3 says create an Object, E4 says create an Array so thats
      // what we will do.
      framep[param_count + 1] =
          env->createArguments(_atomv, arguments_argc)->atom();
    }

#ifdef DEBUGGER
    // in debugger builds, ensure that non-active scope entries are nulled out
    // (really only need to do so if there's a debugger active, but not really
    // worth checking for)
...
#ifndef VMCFG_WORDCODE
  if (core->debugger()) {
    callStackNode = new ((char *)aux_memory +
                         offsetof(InterpreterAuxiliaryFrameWithCSN, cs))
        CallStackNode(env, (FramePtr)framep, /*frameTraits*/ 0, &expc);
    env->debugEnterInner();
  }
#endif
#endif
  ...
  //
  // U30ARG picks up a variable-length unsigned integer argument from the
  // instruction stream and advances the PC.
  //
  // U8ARG picks up a fixed-length unsigned byte argument from the instruction
  // stream and advances the PC.
  //
  // S24ARG picks up a fixed-length signed integer argument from the instruction
  // stream and advances the PC.
  //
  // SAVE_EXPC and variants saves the address of the current opcode in the local
  // 'expc'. Used in the case of exceptions.

#ifdef AVMPLUS_VERBOSE
#define VERBOSE                                                                \
  if (pool->isVerbose(VB_interp, info))                                        \
  showState(info, codeStart, pc - 1, framep, sp, scopeDepth, scopeBase,        \
            ms->max_scope())
#else
#define VERBOSE
#endif
...
#else
  // Adjusted on demand in the CATCH clause.  Reduces size of interpreter
  // function by 2.5KB of object code (x86 / gcc4.0 / -O3).
...
MainLoop:
#ifdef VMCFG_WORDCODE
  TRY_UNLESS_HEAPMEM((char *)aux_memory +
                         offsetof(InterpreterAuxiliaryFrame, ef),
                     core, !info->word_code_exceptions(),
                     kCatchAction_SearchForActionScriptExceptionHandler) {
#else
  TRY_UNLESS_HEAPMEM((char *)aux_memory +
                         offsetof(InterpreterAuxiliaryFrame, ef),
                     core, !info->abc_exceptions(),
                     kCatchAction_SearchForActionScriptExceptionHandler) {
#endif
    ...
    INSTR(debugline) {
      ...
      u1 = U30ARG;
      DEBUGGER_ONLY(
          if (core->debugger()) core->debugger()->debugLine((int32_t)u1);)
      NEXT;
    }
#endif
    ...
    INSTR(debugfile) {
      SAVE_EXPC;
      u1 = U30ARG;
      DEBUGGER_ONLY(if (core->debugger()) core->debugger()->debugFile(
          pool->getString((int32_t)u1));)
      NEXT;
    }
#endif
...
#ifdef VMCFG_FLOAT
    INSTR(pushfloat) {
      AvmAssertMsg(pool->hasFloatSupport(),
                   "float instruction(pushfloat), but float support disabled");
      *(++sp) =
          kSpecialBibopType | (uintptr_t)pool->cpool_float[(uint32_t)U30ARG];
      NEXT;
    }

    INSTR(pushfloat4) {
      AvmAssertMsg(pool->hasFloatSupport(),
                   "float instruction(pushfloat4), but float support disabled");
      *(++sp) =
          kSpecialBibopType | (uintptr_t)pool->cpool_float4[(uint32_t)U30ARG];
      NEXT;
    }
#endif
    INSTR(pushnamespace) {
    ...
    INSTR(coerce_d) { goto convert_d_impl; }
#endif
    ...
    INSTR(coerce_b) { goto convert_b_impl; }
#endif
  ...
  NEXT
    ...
    mod_two_values_and_next:
      ...
      if (IS_BOTH_INTEGER(a1, a2) && a2 != zeroIntAtom) {
        ...
        sp[0] = core->doubleToAtom(
            MathUtils::mod(DOUBLE_VALUE(a1), DOUBLE_VALUE(a2)));
        NEXT;
      }
      ...
      if (pool->hasFloatSupport()) {
        ...
        a2 = core->numericAtom(a2);
        if (IS_BOTH_FLOAT(a1, a2)) {
          sp[0] = core->floatToAtom(
              (float)MathUtils::mod(FLOAT_VALUE(a1), FLOAT_VALUE(a2)));
          NEXT;
        }
        if (AvmCore::isFloat4(a1) || AvmCore::isFloat4(a2)) {
          float4_t x;
  ...
  NEXT
    ...
    INSTR(lookupswitch) {
      ...
      if (index <= case_count)
        ...
        pc = base + default_offset;
          ...
          pc =
              base + readS24(index < case_count ? (switch_pc + 3 * index) : pc);
#endif
      if (pc <= base) {
    ...
    branch_on_boolean:
      ...
      if (IS_BOOLEAN(a1))
        ...
        a1 = AvmCore::booleanAtom(
            a1); // does not throw or change the XML namespace
      i1 = S24ARG;
      if (a1 == a2) {
        if (i1 < 0) {
          SAVE_EXPC_TARGET(i1);
...
#define GENERATE_FLAG(numeric_cmp, generic_cmp, a1, a2)                        \
  ...
  FLOAT_ONLY(else if (IS_BOTH_FLOAT(a1, a2) && pool->hasFloatSupport()) b1 =   \
                 FLOAT_VALUE(a1) numeric_cmp FLOAT_VALUE(a2);)                 \
  else b1 = generic_cmp
...
#define CMP2(numeric_cmp, generic_cmp)                                         \
  ...
  sp[0] = b1 ? trueAtom : falseAtom;
    ...
    // handle common cases.
    //
    // But does it matter? getproperty, findproperty are heavyweight operations,
    // and opportunities for early binding are resolved during translation.
    // Still, it may make a small difference in connection with avoiding
    // multiname copying (now implemented), since most fields of the multiname
    // data structure will not be accessed.

    // OPTIMIZEME - inline fast case for toVTable?
    // Presumably it will very often be a non-null object; now we have to make a
    // call and a test and a switch to get to that.  Should we in-line? How will
    // RTTI caching affect that?

#define GET_MULTINAME_PTR(decl, arg)                                           \
  decl = pool->precomputedMultiname(uint32_t(arg))
    ...
    INSTR(getlex) {
      ...
      a1 = env->findproperty(scope, scopeBase, scopeDepth, multiname, true,
                             withBase);
    ...
    INSTR(getproperty) {
      ...
      } else if (!multiname->isRtns() && IS_INTEGER(*sp) &&
                 atomCanBeUint32(*sp) && AvmCore::isObject(sp[-1])) {
        a2 = *(sp--); // key
        *sp =
            AvmCore::atomToScriptObject(*sp)->getUintProperty(UINT32_VALUE(a2));
      } else if (multiname->isRtns() ||
                 !AvmCore::isDictionaryLookup(*sp, *(sp - 1))) {
        aux_memory->multiname2 = *multiname;
        sp = initMultiname(env, aux_memory->multiname2, sp);
        *sp = toplevel->getproperty(*sp, &aux_memory->multiname2,
                                    toplevel->toVTable(*sp));
      } else {
        a2 = *(sp--); // key
    ...
    INSTR(setproperty) {
      ...
      if (!multiname->isRuntime()) {
        ...
        sp = initMultiname(env, aux_memory->multiname2, sp);
        a3 = *(sp--); // object
        toplevel->setproperty(a3, &aux_memory->multiname2, a1,
                              toplevel->toVTable(a3));
      } else {
        a2 = *(sp--); // key
        a3 = *(sp--); // object
    ...
    findproperty_impl:
      ...
      *(++sp) = env->findproperty(scope, scopeBase, scopeDepth, multiname, b1,
                                  withBase);
      NEXT;
    }
    ...
    INSTR(finddef) {
      ...
      u1 = U30ARG;
      o1 = finddef_cache(
          (ScriptObject **)&(*env->abcEnv()->finddefTable())[(uint32_t)u1],
    ...
    // sign extends -- results always fit into an atom, no need to call
    // intToAtom since we are downshifting anyway, integrate final upshift-by-3
    // into downshift rather than using MAKE_INTEGER macro.
    INSTR(sxi1) {
      i1 = AvmCore::integer(sp[0]);
      sp[0] = CHECK_INT_ATOM(Atom(
          ((i1 << (8 * sizeof(Atom) - 1)) >> ((8 * sizeof(Atom) - 1) - 3)) |
          kIntptrType));
      NEXT;
    }
    ...
    INSTR(sxi16) {
      i1 = AvmCore::integer(sp[0]);
      sp[0] = CHECK_INT_ATOM(Atom(
          ((i1 << (8 * (sizeof(Atom) - 2))) >> ((8 * (sizeof(Atom) - 2)) - 3)) |
          kIntptrType));
      NEXT;
    }

    // note that the mops "addr" (offset from globalMemoryBase) is in fact a
    // signed int, so we have to check for it being < 0 ... but we can get by
    // with a single unsigned compare since all values < 0 will be > size
#define MOPS_RANGE_CHECK(addr, type)                                           \
  if (uint32_t(addr) > (envDomain->globalMemorySize() - sizeof(type))) {       \
...
#define MOPS_STORE_F4(addr, call, value)                                       \
  ...
  avmplus::mop_##call(envDomain->globalMemoryBase() + (addr), value);
    ...
    INSTR(deleteproperty) {
      ...
      if (!multiname->isRuntime()) {
        ...
        a2 = *(sp--); // key
        *sp =
            AvmCore::atomToScriptObject(*sp)->delUintProperty(UINT32_VALUE(a2))
                ? trueAtom
                : falseAtom;
      } else if (multiname->isRtns() ||
                 !AvmCore::isDictionaryLookup(*sp, *(sp - 1))) {
        ...
        a2 = *(sp--); // key
        sp[0] = AvmCore::atomToScriptObject(sp[0])->deleteAtomProperty(a2)
                    ? trueAtom
                    : falseAtom;
      }
      NEXT;
    ...
    INSTR(getslot) {
      ...
      // Perform the -1 adjustment in the bytecode translator, not here every
      // time.
      sp[0] =
          AvmCore::atomToScriptObject(sp[0])->getSlotAtom((uint32_t)U30ARG - 1);
      NEXT;
    }
    ...
    INSTR(setglobalslot) {
      ...
      a1 = sp[0];      // value
      sp--;
      AvmCore::atomToScriptObject(globalScopeAtom)
          ->coerceAndSetSlotAtom((uint32_t)u1, a1);
      NEXT;
    }
    ...
    INSTR(getglobalslot) {
      ...
      // time.
      sp++;
      sp[0] = AvmCore::atomToScriptObject(globalScopeAtom)
                  ->getSlotAtom((uint32_t)(U30ARG - 1));
      NEXT;
    }

    // OPTIMIZEME - presumably there are many ways in which the call opcodes may
    // be specialized to avoid the full function prologue?
    ...
    INSTR(call) {
      ...
      // stack out: result
      a1 = avmplus::op_call(toplevel, sp[-i1 - 1] /*function*/, (int32_t)i1,
                            sp - i1);
      *(sp = sp - i1 - 1) = a1;
      NEXT;
    }
    ...
    INSTR(newclass) {
      ...
      sp[0] =
          env->newclass(ctraits, (ClassClosure *)o1, scope, scopeBase)->atom();
      NEXT;
    }
    ...
    INSTR(callstatic) {
      ...
      sp[-i2] = toplevel->coerce(
          sp[-i2], f->method->getMethodSignature()->paramTraits(0));
    ...
    INSTR(callmethod) {
      ...
      // must be a real class instance for this to be used.  primitives that
      // have methods will only have final bindings and no dispatch table.
    ...
    callproperty_impl:
      ...
      *sp = toplevel->callproperty(a1, multiname, (int32_t)i1, a2p,
                                   toplevel->toVTable(a1));
    ...
    INSTR(constructprop) {
      ...
      *sp = toplevel->constructprop(multiname, (int32_t)i1, a2p,
                                    toplevel->toVTable(a2p[0]));
      NEXT;
    }
    ...
    callsuper_impl:
      ...
      *sp = env->callsuper(multiname, (int32_t)i1, a2p);
      if (u1 == WORD_CODE_ONLY(WOP_callsupervoid)
                    ABC_CODE_ONLY(OP_callsupervoid))
        sp--;
      NEXT;
    }
    ...
    // An possible optimization is to split instructions that have a runtime
    // multiname into two: one that initializes aux_memory->multiname2, and
    // another that simply picks up that value.  That way there will be less
    // code in the interpreter, and the common case will be faster.
    ...
    INSTR(astype) {
      ...
      GET_MULTINAME_PTR(multiname, U30ARG);
      sp[0] =
          AvmCore::astype(sp[0], getTraits(multiname, pool, toplevel, core));
      NEXT;
    }
    ...
    INSTR(coerce) {
      ...
      GET_MULTINAME_PTR(multiname, U30ARG);
      sp[0] =
          toplevel->coerce(sp[0], getTraits(multiname, pool, toplevel, core));
      NEXT;
    }
    ...
    INSTR(coerce_s) {
      ...
      if (!IS_STRING(a1)) {
        SAVE_EXPC;
        sp[0] = AvmCore::isNullOrUndefined(a1) ? nullStringAtom
                                               : core->string(a1)->atom();
      }
      NEXT;
    }
    ...
    INSTR(pushwith) {
      ...
      // is it possible to have pushWith for scope 0? not sure, let's do this
      // just in case
      if (scopeDepth == 0 && AvmCore::isNullOrUndefined(globalScopeAtom)) {
        AvmAssert(scope->getSize() == 0);
        globalScopeAtom = a1;
...
#ifndef VMCFG_WORDCODE
    INSTR(coerce_i) { goto convert_i_impl; }
#endif
...
#ifndef VMCFG_WORDCODE
    INSTR(coerce_u) { goto convert_u_impl; }
#endif
    ...
    INSTR(dxns) {
      ...
      SAVE_EXPC;
      core->setDxns(&aux_memory->methodFrame,
                    pool->getString((uint32_t)U30ARG));
      NEXT;
    }
  ...
  default:
    ...
    // OPTIMIZEME - push_doublebits should probably not cons up a new atom every
    // time, it would be better to keep it in the constant pool.
    //
    // OPTIMIZEME - on 64-bit systems we don't need two operand words here
    //
    // OPTIMIZEME - we can do better if unaligned int64 loads or unaligned fp
    // loads are allowed on the platform.
...
#define LOAD_OFFSET_AND_FETCH_LB(a1, a2, i1)                                   \
  ...
  FETCH_LB(a1, a2)
    ...
    // driven peephole optimizer, see comments and code in
...
CATCH(Exception *exception) {
  ...
  ExceptionHandler *handler = core->findExceptionHandler(
      info, (uintptr_t *)expc - 1 - info->word_code_start(), exception);
#else
        ExceptionHandler *handler =
            core->findExceptionHandler(info, expc, exception);
#endif
  // handler found in current method
#ifdef DEBUGGER
  // This is a little hokey, see
  // https://bugzilla.mozilla.org/show_bug.cgi?id=470954.
  //
  // The debugenter instruction sets up core->callStack, we do this lazily to
  // save time in builds where the debugger is enabled at compile time but not
  // present at run time.
  //
  // The problem is that CATCH restores core->callStack to its old value, saved
  // by TRY. So we force it to the new value here if there is a new value.  Then
  // TRY will save the value again (the new value this time) which we restore
  // redundantly the next time there is an exception, if any.  The debugexit
  // instruction will take care of restoring the actual old value.
...
L_illegal_op : AvmAssert(!"Illegal operation!");
goto L_illegal_op;
} // end interpBoxed()

// Note, this function is not on the hot path normally, so optimizing it is not
// urgent.
//
// OPTIMIZEME - statically knowable if name isRtname or isRtns; exploit this
// somehow? OPTIMIZEME - often knowable whether the TOS is an object or
// something simple; exploit this?

// Refer to comments for |interpBoxed()|
#if defined(PEPPER_PLUGIN) && defined(AVMPLUS_UNIX) && !defined(PEPPER_MAC) && \
    !defined(OS_CHROMEOS) && defined(VMCFG_32BIT) && !defined(DEBUG)
Atom *__attribute__((optimize("O1")))
initMultiname(MethodEnv *env, Multiname &name, Atom *sp)
#else
...
Atom *initMultinameNoXMLList(MethodEnv *env, Multiname &name, Atom *sp) {
  if (name.isRtname() && AvmCore::isXMLList(sp[0])) {
    ...
    env->toplevel()->throwTypeError(
        kDeleteTypeError,
        env->core()->toErrorString(env->toplevel()->toTraits(sp[0])));
  }
  return initMultiname(env, name, sp);
}

Traits *getTraits(const Multiname *name, PoolObject *pool, Toplevel *toplevel,
                  AvmCore *core) {
  ...
  if (name->isParameterizedType()) {
    ...
    Traits *param_traits = name->getTypeParameter()
                               ? getTraits(param_name, pool, toplevel, core)
                               : NULL;
 ...
 * display contents of current stack frame only.
 */
void showState(MethodInfo *info, const bytecode_t *code_start,
               const bytecode_t *pc, Atom *framep, Atom *spp, int scopeDepth,
               Atom *scopebasep, int max_scope) {
#ifdef VMCFG_WORDCODE
#ifdef VMCFG_DIRECT_THREADED
  ...
  //          core->console << " 0x" <<
  //          MathUtils::convertDoubleToStringRadix(core, framep[i]>>32, 16) <<
  //                                  MathUtils::convertDoubleToStringRadix(core,
  //                                  framep[i]&0xffffffff, 16);
  //      }
  //      core->console << '\n';
  //#endif
  ...
  const size_t stackLimit =
      20; // don't display more than this, to reduce verbosity
  ...
  if (core->debugger() && core->callStack && core->callStack->filename()) {
    core->console << '[' << core->callStack->filename() << ':'
                  << (uint32_t)core->callStack->linenum() << "] ";
  }
#endif
  core->console << (int)off << ':';
  // Using largest-possible-pointer here for code_end; this is obviously not
  // correct, but extracting the proper code_end requires diving into the
  // method's ABC bytecode (and may not be easily accessible at all for
  // wordcode), and more importantly, is only used for range-checking of fuzzed
  // opcodes... all of which should have been caught by Verifier, thus checking
  // should be unnecessary here.
  const bytecode_t *const code_end = (const bytecode_t *)uintptr_t(-1);
#ifdef VMCFG_WORDCODE
  core->formatOpcode(core->console, pc, code_end,
                     (WordOpcode)((int32_t)opcode & 0xffff), off, pool);
#else
