...
namespace avmplus {
    ...
    REALLY_INLINE DomainMgr
    *

    AvmCore::domainMgr() const { return m_domainMgr; }

#ifdef DEBUGGER
    REALLY_INLINE Debugger *AvmCore::debugger() const { return _debugger; }

    REALLY_INLINE Profiler *AvmCore::profiler() const { return _profiler; }

    REALLY_INLINE Sampler *AvmCore::get_sampler() const { return _sampler; }
    ...
    REALLY_INLINE QCache
    *

    AvmCore::tbCache() { return m_tbCache; }

    REALLY_INLINE QCache
    *

    AvmCore::tmCache() { return m_tmCache; }

    REALLY_INLINE QCache
    *

    AvmCore::msCache() { return m_msCache; }
    ...
    REALLY_INLINE /*static*/ bool AvmCore::isPointer(Atom atom) {
        // kDouble, all kTypes less than kSpecialType, and kSpecialType if it's not
        // "null" - i.e. not undefinedAtom, but a bibopType.
        return (atomKind(atom + 1) <= (kSpecialBibopType + 1)) &&
               atom != undefinedAtom;
    }
    ...
    REALLY_INLINE /*static*/ BindingKind
    AvmCore::bindingKind(Binding
    b) {
    return

    BindingKind (uintptr_t(b) &
...
REALLY_INLINE /*static*/ Binding
AvmCore::makeSlotBinding(uintptr_t
id,
BindingKind kind
) {
AvmAssert(kind
== BKIND_VAR || kind == BKIND_CONST);
return
Binding((id
<< 3) | kind);
}
...
REALLY_INLINE /*static*/ Binding
AvmCore::makeMGSBinding(uintptr_t
id,
BindingKind kind
) {
AvmAssert(kind
== BKIND_METHOD || kind == BKIND_GET || kind == BKIND_SET);
return
Binding((id
<< 3) | kind);
}
...
REALLY_INLINE /*static*/ Binding
AvmCore::makeGetSetBinding(Binding
b) {
AvmAssert (bindingKind(b)
...
REALLY_INLINE /*static*/ int AvmCore::isVarBinding(Binding
b) {
return
bindingKind(b)
==
...
REALLY_INLINE /*static*/ int AvmCore::isConstBinding(Binding
b) {
return
bindingKind(b)
==
...
REALLY_INLINE /*static*/ bool AvmCore::isFloat(Atom atom) {
    return atomKind(atom) == kSpecialBibopType &&
           atom != AtomConstants::undefinedAtom &&
           bibopKind(atom) == kBibopFloatType;
}
...
REALLY_INLINE /*static*/ bool AvmCore::isFloat4(Atom atom) {
    return atomKind(atom) == kSpecialBibopType &&
           atom != AtomConstants::undefinedAtom &&
           bibopKind(atom) == kBibopFloat4Type;
}
...
REALLY_INLINE /*static*/ bool AvmCore::isNumeric(Atom atom) {
    MMGC_STATIC_ASSERT(kIntptrType == 6 && kDoubleType == 7 &&
                       kSpecialBibopType == 4 && kBooleanType == 5);
    ...
    /* the second assumption is that all kSpecialBibopTypes, except undefinedAtom,
     * are numeric */
    /* Finally, the third assumption is that the Boolean type has only two values:
     * trueAtom and falseAtom */
    return isNumber(atom) || (((uintptr_t)
    atom > (uintptr_t)
    trueAtom) &&
    (atom & kSpecialBibopType));
}

REALLY_INLINE /*static*/ bool AvmCore::isNumberOrFloat(Atom atom) {
    MMGC_STATIC_ASSERT(kIntptrType == 6 && kDoubleType == 7);
    return (atom & 6) == kIntptrType || (atomKind(atom) == kSpecialBibopType &&
                                         atom != AtomConstants::undefinedAtom &&
                                         bibopKind(atom) == kBibopFloatType);
}
...
REALLY_INLINE /*static*/ uint32_t
AvmCore::toUInt32(Atom
atom) {
return (uint32_t)
integer(atom);
}

REALLY_INLINE /*static*/ double AvmCore::toInteger(Atom
atom) {
if (
atomIsIntptr(atom)
) {
...
return
MathUtils::toInt(AvmCore::number(atom)
);
}
}

REALLY_INLINE /*static*/ int32_t
AvmCore::integer_i(Atom
a) {
if (
atomIsIntptr(a)
) {
...
REALLY_INLINE /*static*/ double AvmCore::number_d(Atom
a) {
...
REALLY_INLINE Atom
AvmCore::intAtom(Atom
atom) {
return
intToAtom(AvmCore::integer(atom)
);
}
...
REALLY_INLINE Atom
AvmCore::uintAtom(Atom
atom) {
return
uintToAtom(AvmCore::toUInt32(atom)
);
}

REALLY_INLINE /*static*/ bool AvmCore::isString(Atom
atom) {
return
atomKind(atom)
== kStringType && !
...
REALLY_INLINE /*static*/ bool AvmCore::isName(Atom
atom) {
return
isString(atom)
&&
...
*
AvmCore::newObject(VTable
*vtable,
ScriptObject *delegate
) {
return

ScriptObject::create(GetGC(), vtable, delegate
...
/**
 * Returns the size of the instruction + all it's operands.  For OP_lookupswitch
 * the size will not include the size for the case targets.
 */
REALLY_INLINE /*static*/ int
AvmCore::calculateInstructionWidth(const uint8_t *p) {
    int a, b;
    unsigned int c, d;
    const uint8_t *p2 = p;
...
AvmCore::readU16(const uint8_t *pc) {
#ifdef VMCFG_UNALIGNED_INT_ACCESS
    // unaligned short access still faster than 2 byte accesses
    return *((uint16_t *)pc);
#else
...
REALLY_INLINE /*static*/ bool AvmCore::isXML(Atom
atm) {
return
isBuiltinType(atm, BUILTIN_xml
);
}

REALLY_INLINE /*static*/ bool AvmCore::isXMLList(Atom
atm) {
return
isBuiltinType(atm, BUILTIN_xmlList
);
}

REALLY_INLINE /*static*/ bool AvmCore::isXMLorXMLList(Atom
atm) {
return
isBuiltinTypeMask(atm,
(1 << BUILTIN_xml) | (1 << BUILTIN_xmlList));
}

REALLY_INLINE /*static*/ bool AvmCore::isDate(Atom
atm) {
return
isBuiltinType(atm, BUILTIN_date
);
}

REALLY_INLINE /*static*/ bool AvmCore::isQName(Atom
atm) {
return
isBuiltinType(atm, BUILTIN_qName
);
}

REALLY_INLINE /*static*/ bool AvmCore::isDictionaryLookup(Atom
key,
Atom obj
) {
return
...
REALLY_INLINE /*static*/ Namespacep
AvmCore::atomToNamespace(Atom
atom) {
...
REALLY_INLINE /*static*/ double AvmCore::atomToDouble(Atom
atom) {
AvmAssert(atomKind(atom)
== kDoubleType);
return *(const double *)
 ...
 */
REALLY_INLINE /*static*/ Stringp
AvmCore::atomToString(Atom
atom) {
AvmAssert(atomKind(atom)
== kStringType);
return (Stringp)
...
REALLY_INLINE /*static*/ ScriptObject
*

AvmCore::atomToScriptObject(const Atom atom) {
...
REALLY_INLINE /*static*/ const void *AvmCore::atomToGenericObject(Atom
a) {
return
atomPtr(a);
}

REALLY_INLINE /*static*/ bool AvmCore::isGenericObject(Atom
a) {
return
atomKind(a)
==
...
REALLY_INLINE /*static*/ bool AvmCore::getIndexFromAtom(Atom
a,
uint32_t *result
) {
if (
...
AvmCore::allocDouble(double n) {
    double *d = (double *) GetGC()->AllocDouble();
    *d = n;
    return kDoubleType | (uintptr_t)
    d;
}
...
REALLY_INLINE bool AvmCore::isValidApiVersion(ApiVersion apiVersion) {
    ...
    return (kApiVersionSeriesMembership[apiVersion] &
            m_activeApiVersionSeriesMask) != 0;
}

REALLY_INLINE ApiVersion
AvmCore::getValidApiVersion(ApiVersion
apiVersion) {
AvmAssert((int)apiVersion >= 0 && (int)apiVersion<kApiVersion_count);
return kApiVersionSeriesTransfer[apiVersion][m_activeApiVersionSeries];
}

REALLY_INLINE Namespacep
AvmCore::getPublicNamespace(PoolObject
*pool) {
AvmAssert(pool
!= NULL);
...
REALLY_INLINE Namespacep
AvmCore::getPublicNamespace(ApiVersion
apiVersion) {
...
REALLY_INLINE CodeContext
*

MethodFrame::cc() const {
    return (envOrCodeContext & IS_EXPLICIT_CODECONTEXT)
           ? (CodeContext * )(envOrCodeContext & ~FLAGS_MASK)
           : NULL;
}

REALLY_INLINE MethodEnv
*

MethodFrame::env() const {
    return (envOrCodeContext & IS_EXPLICIT_CODECONTEXT)
           ? NULL
...
AvmCore::getIsolate() const { return m_isolate; }
...
REALLY_INLINE void AvmCore::assignPoolId(PoolObject * pool) {
    // each pool is assigned an id that is based on a running total of
    // MethodInfo's this is so that MethodInfo.unique_method_id() can return a
    // AvmCore-wide unique identifier for any given method.
