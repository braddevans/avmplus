...
/*static*/ TraitsBindings *
TraitsBindings::alloc(GC *gc, Traits *_owner, TraitsBindingsp _base,
                      MultinameBindingHashtable *_bindings, uint32_t slotCount,
                      uint32_t methodCount, bool typesValid) {
  const uint32_t extra = typesValid ? slotCount * sizeof(SlotInfo) +
                                          methodCount * sizeof(MethodInfo)
                                    : 0;

  TraitsBindings *tb = new (gc, MMgc::kExact, extra) TraitsBindings(
      _owner, _base, _bindings, slotCount, methodCount, typesValid);
  if (_base && typesValid) {
    if (_base->slotCount) {
      ...
      AvmAssert(
          ((_owner->isMachineType()) ||
           (tb->owner->m_sizeofInstance >= _base->owner->m_sizeofInstance)));
    }
    if (_base->methodCount)
      VMPI_memcpy(&tb->getMethods()[0], &_base->getMethods()[0],
                  _base->methodCount * sizeof(MethodInfo));
  }
  return tb;
}
...
Binding TraitsBindings::findBindingAndDeclarer(const Multiname &mn,
                                               Traitsp &declarer) const {
  if (mn.isBinding()) {
    for (TraitsBindingsp self = this; self; self = self->base) {
      ...
      if (b != BIND_NONE) {
        ...
        while (foundns == declarer->protectedNamespace) {
          ...
          Binding const bp = declParent->getTraitsBindings()->findBinding(
              mn.getName(), declParent->protectedNamespace);
          if (bp != b)
            break;

          // self->owner->core->console<<"bounce "<<declarer<<" to
          // "<<declParent<<"\n";
          declarer = declParent;
...
void TraitsBindings::buildSlotDestroyInfo(MMgc::GC *gc,
                                          FixedBitSet &slotDestroyInfo,
                                          uint32_t slotAreaCount,
                                          uint32_t slotAreaSize) const {
  MMGC_STATIC_ASSERT(kUnusedAtomTag == 0 && kObjectType == 1 &&
                     kStringType == 2 && kNamespaceType == 3);
  ...
  const uint32_t bitsNeeded =
      slotAreaSize / sizeof(uint32_t); // not sizeof(Atom)!
  AvmAssert(bitsNeeded * sizeof(uint32_t) ==
            slotAreaSize); // should be even multiple!
  ...
  if (slotAreaSize > 0) {
    ...
    const TraitsBindings::SlotInfo *tbs =
        getSlots() + (slotCount - slotAreaCount);
    ...
    for (; tbs < tbs_end; ++tbs) {
      ...
      if (isAtomOrRCObjectSlot(tbs->sst())) {
        // owner->core->console<<"SDI "<<owner<<" "<<sizeofInstance<<"
        // "<<tbs->type<<" "<<tbs->offset()<<"\n";
        AvmAssert(tbs->offset() >= sizeofInstance);
        const uint32_t off = tbs->offset() - sizeofInstance;
        AvmAssert((off % 4) == 0);
        // if slot is "big" then this is the bit of the first 4 bytes. that's
        // fine.
  ...
  // if nothing set, blow away what we built and realloc as single clear bit --
  // smaller and faster
...
bool TraitsBindings::checkOverride(AvmCore *core, MethodInfo *virt,
                                   MethodInfo *over) const {
  if (over == virt)
    return true;
  ...
  if (overms->param_count() != virtms->param_count() ||
    ...
    core->console << "   virt params=" << virtms->param_count()
                  << " optional=" << virtms->optional_count() << " " << virt
                  << "\n";
    core->console << "   over params=" << overms->param_count()
                  << " optional=" << overms->optional_count() << " " << virt
                  << "\n";
#endif
    return false;
  }
  ...
  if (!owner->subtypeof(virtTraits) ||
      !Traits::isMachineCompatible(this->owner, virtTraits)) {
    if (!this->owner->isMachineType() &&
        virtTraits == core->traits.object_itraits) {
      over->setUnboxThis();
    } else {
#ifdef AVMPLUS_VERBOSE
      core->console << "\n";
...
static bool isCompatibleOverrideKind(BindingKind baseKind,
                                     BindingKind overKind) {
...
bool TraitsBindings::checkLegalInterfaces(AvmCore *core) const {
  ...
  for (InterfaceIterator ifc_iter(this); ifc_iter.hasNext();) {
    ...
    // Checking only that an interface was once correctly implemented is not
    // sufficient.
    ...
    while (iter.next()) {
      ...
      // The second lookup that we used to do here is unnecessary. The name
      // would have been brought out for this namespace by
      ...
      //
      // here, presumably on the theory that if the bindings are the same, we
      // don't need to check any further. THIS IS WRONG: we must check the
      // actual argument types in all cases.

      if (!isCompatibleOverrideKind(iBindingKind,
                                    AvmCore::bindingKind(cBinding)))
        return false;

      switch (iBindingKind) {
      default: {
        AvmAssert(0); // interfaces shouldn't have anything but methods,
                      // getters, and setters
        return false;
      }
      case BKIND_METHOD: {
        MethodInfo *virt =
            ifcd->getMethods()[AvmCore::bindingToMethodId(iBinding)].f;
        MethodInfo *over =
            this->getMethods()[AvmCore::bindingToMethodId(cBinding)].f;
      ...
      case BKIND_GETSET: {
        ...
        if (AvmCore::hasGetterBinding(iBinding)) {
          ...
          MethodInfo *virt =
              ifcd->getMethods()[AvmCore::bindingToGetterId(iBinding)].f;
          MethodInfo *over =
              this->getMethods()[AvmCore::bindingToGetterId(cBinding)].f;
          if (!checkOverride(core, virt, over))
            return false;
        }
        ...
        if (AvmCore::hasSetterBinding(iBinding)) {
          ...
          MethodInfo *virt =
              ifcd->getMethod(AvmCore::bindingToSetterId(iBinding));
          MethodInfo *over =
              this->getMethod(AvmCore::bindingToSetterId(cBinding));
          if (!checkOverride(core, virt, over))
            return false;
        }
      } break;
...
void TraitsBindings::fixOneInterfaceBindings(Traitsp ifc) {
  ...
  while (iter.next()) {
    ...
    if (!isCompatibleOverrideKind(iBindingKind,
                                  AvmCore::bindingKind(cBinding))) {
      // Try again with public namespace that matches the version of the current
      // traits
      const Binding pBinding =
          this->findBinding(name, ifc->core->getPublicNamespace(owner->pool));
      if (isCompatibleOverrideKind(iBindingKind,
                                   AvmCore::bindingKind(pBinding))) {
...
void Traits::addVersionedBindings(MultinameBindingHashtable *bindings,
                                  Stringp name, NamespaceSetp nss,
...
TraitsMetadata::MetadataPtr
TraitsMetadata::getSlotMetadataPos(uint32_t i,
                                   PoolObject *&residingPool) const {
  AvmAssert(i < slotCount);
  residingPool = NULL;
  for (TraitsMetadatap self = this; self && (i < self->slotCount);
       self = self->base) {
...
TraitsMetadata::MetadataPtr
TraitsMetadata::getMethodMetadataPos(uint32_t i,
                                     PoolObject *&residingPool) const {
  AvmAssert(i < methodCount);
  residingPool = NULL;
  for (TraitsMetadatap self = this; self && (i < self->methodCount);
       self = self->base) {
...
Traits::Traits(PoolObject *_pool, Traits *_base, uint16_t _sizeofInstance,
               uint16_t _offsetofSlots, TraitsPosPtr traitsPos,
               TraitsPosType posType)
    : core(_pool->core), base(_base), m_paramTraits(NULL), pool(_pool),
      m_traitsPos(traitsPos), m_tbref(_pool->core->GetGC()->emptyWeakRef),
      m_tmref(_pool->core->GetGC()->emptyWeakRef),
      m_sizeofInstance(_sizeofInstance), m_offsetofSlots(_offsetofSlots),
      builtinType(BUILTIN_none), m_posType(uint8_t(posType))
...
/*static*/ Traits *Traits::newTraits(PoolObject *pool, Traits *base,
                                     uint16_t objectSize,
  ...
  AvmAssert(pool != NULL);
  Traits *traits = new (pool->core->GetGC(), MMgc::kExact)
      Traits(pool, base, objectSize, offsetOfSlots, traitsPos, posType);
  return traits;
}

/*static*/ Traits *Traits::newCatchTraits(const Toplevel *toplevel,
                                          PoolObject *pool,
                                          TraitsPosPtr traitsPos, Stringp name,
                                          Namespacep ns) {
  AvmAssert(pool != NULL);
  Traits *traits = new (pool->core->GetGC(), MMgc::kExact)
      Traits(pool, NULL, sizeof(ScriptObject), sizeof(ScriptObject), traitsPos,
             TRAITSTYPE_CATCH);
  traits->final = true;
...
Traits *Traits::_newParameterizedTraits(Stringp name, Namespacep ns,
                                        Traits *_base, Traits *_param_traits) {
  Traits *newtraits =
      Traits::newTraits(this->pool, _base, this->getSizeOfInstance(),
                        this->m_offsetofSlots, NULL, TRAITSTYPE_RT);
...
#ifdef AVMPLUS_64BIT
  const uint32_t BIG_TYPE_MASK =
      (~(NUMERIC_TYPE_MASK | (1U << BUILTIN_boolean))) |
      (1U << BUILTIN_number); // number, plus all non-numeric, non-boolean types
                              // (i.e. pointers, essentially)
#else
...
SlotStorageType valueStorageType(BuiltinType bt) {
  ...
  case BUILTIN_namespace:
    return SST_namespace;
  default:
    return SST_scriptobject;
  }
}
...
// the logic for assigning slot id's is used in several places, so it's now
// collapsed here rather than redundantly sprinkled thru several bits of code.
class SlotIdCalcer {
private:
...
public:
  SlotIdCalcer(uint32_t _baseSlotCount, bool _earlySlotBinding)
      : m_slotCount(_baseSlotCount), m_earlySlotBinding(_earlySlotBinding) {}
...
void Traits::buildBindings(TraitsBindingsp basetb,
  ...
  for (uint32_t i = 0; i < nameCount; i++) {
    ...
    case TRAIT_Class: {
      ...
      if (toplevel) {
        ...
        // when we resolve, we must do some additional verification checks...
        // these were formerly done in AbcParser::parseTraits but require the
        // base class to be resolved first, so we now defer it to here.

        // illegal raw slot id -- reject IF we are early binding. (theoretically
        // it's always illegal, but won't cause any problems if
        // earlySlotBinding=false, and rejecting in this case will break
        // existing content.)
        ...
        if (basetb && slot_id < basetb->slotCount)
          toplevel->throwVerifyError(kIllegalOverrideError,
                                     core->toErrorString(mn),
                                     core->toErrorString(base));
        ...
        // in practice we don't, as it causes problems with some existing
        // content
        // if (basetb->findBinding(name, ns) != BIND_NONE)
        //  toplevel->throwVerifyError(kIllegalOverrideError,
        //  toplevel->core()->toErrorString(qn),
        //  toplevel->core()->toErrorString(this));

        // Interfaces cannot have slots.
        if (this->isInterface())
          toplevel->throwVerifyError(kIllegalSlotError,
                                     core->toErrorString(this));
      }
      ...
      // AvmAssert(!(ne.id > nameCount));                     // unhandled
      // verify error AvmAssert(!(basetb && slot_id < basetb->slotCount)); //
      // unhandled verify error AvmAssert(!(bindings->get(name, ns) !=
      // BIND_NONE));      // unhandled verify error
      addVersionedBindings(
          bindings, name, compat_nss,
          AvmCore::makeSlotBinding(
              slot_id, ne.kind == TRAIT_Slot ? BKIND_VAR : BKIND_CONST));
      if (slotSizeInfo != NULL) {
        // only do this if we need it -- doing it too early could cause us to
        // reference a not-yet-loaded type which we don't need yet, causing an
        // unnecessary verification failure
    ...
    case TRAIT_Method: {
      Binding baseBinding =
          this->getOverride(basetb, ns, name, ne.tag, toplevel);
    ...
    case TRAIT_Getter:
    case TRAIT_Setter: {
      // if nothing already is defined in this class, use base class in case
      // getter/setter has already been defined.
      ...
      const BindingKind them =
          (ne.kind == TRAIT_Getter) ? BKIND_SET : BKIND_GET;
      if (baseBinding == BIND_NONE) {
        addVersionedBindings(bindings, name, compat_nss,
                             AvmCore::makeMGSBinding(methodCount, us));
        // accessors require 2 vtable slots, methods only need 1.
        methodCount += 2;
    ...
    default:
      // unsupported traits type -- can't happen, caught in
  ...
  if (slotSizeInfo) {
    uint32_t const c = slotSizeInfo->nonPointer32BitSlotCount +
                       slotSizeInfo->nonPointer64BitSlotCount + baseSlotCount +
                       IFFLOAT(slotSizeInfo->nonPointer128BitSlotCount, 0);
    // We calculate this at the end because we might have a sparse slot table...
    // the unused slots will be, well, unused, but we still have to reserve
    // space for them to guard against pathological code.
    slotSizeInfo->pointerSlotCount = slotCount - c;
    // Don't do this assertion: if this situation occurs, we'll catch it (and
    // throw a VerifyError) in finishSlotsAndMethods; asserting here will just
    // spoil acceptance test runs. AvmAssert(slotSizeInfo->pointerSlotCount >=
    // pointerSlots);
  }
}
...
static const bool align8ByteSlots =
    (offsetof(GlueClassTest_Slots, m_numberSlot) == 8);
static const bool alignPointersTo8Bytes =
...
static const bool align16ByteSlots =
...
struct AlmostStaticAsserts {
  AlmostStaticAsserts() {
    ...
    AvmAssert((offsetof(GlueClassTest128_Slots, m_float4Slot) == 4) ||
              ...
              (offsetof(GlueClassTest128_Slots, m_float4Slot) == 8));
    /* Is it EVER the case that pointers are 8-byte aligned, but doubles are
       not? We currently assume "no", but let's double-test this assumption */
    struct GlueClassTestPtr_Slots {
      int32_t m_intSlot;
      void *m_ptrSlot;
    };
    AvmAssert(((offsetof(GlueClassTestPtr_Slots, m_ptrSlot) == 8) &&
               alignPointersTo8Bytes) ||
              ((offsetof(GlueClassTestPtr_Slots, m_ptrSlot) == 4) &&
               !alignPointersTo8Bytes));
  }
} __unused_var;
#endif
...
// Sun compilers don't allow static and REALLY_INLINE
#ifdef VMCFG_FLOAT
/*static*/ REALLY_INLINE int32_t
computeSlotOffset(Traits *slotType, int32_t &next32BitSlotOffset,
                  int32_t &nextPointerSlotOffset, int32_t &next64BitSlotOffset,
                  int32_t &next128BitSlotOffset)
#else
/*static*/ REALLY_INLINE int32_t
computeSlotOffset(Traits *slotType, int32_t &next32BitSlotOffset,
                  int32_t &nextPointerSlotOffset, int32_t &next64BitSlotOffset)
...
void Traits::computeNonNativeSlotAreaCountAndSize(TraitsBindings *tb,
                                                  uint32_t &slotCount,
                                                  uint32_t &size) const {
  const TraitsBindings *prevBindings = tb;
  const TraitsBindings *currBindings = tb->base;
  uint32_t thisSize = getSizeOfInstance();
  while ((currBindings != NULL) &&
         (currBindings->owner->getSizeOfInstance() == thisSize)) {
    const TraitsBindings *baseBindings = currBindings->base;
    AvmAssert((baseBindings == 0) ||
              (baseBindings->owner->getSizeOfInstance() <= thisSize));
    (void)baseBindings;
    prevBindings = currBindings;
    currBindings = currBindings->base;
  }

  // currBindings is first ancestor class that is native with at least one slot
  // or native member variable or null if there is no native ancenstor class
  ...
  if (currBindings == NULL) {
    ...
    AvmAssert((tb->base->slotCount == 0) ||
              (tb->base->owner->getSizeOfInstance() <= thisSize));
...
uint32_t Traits::finishSlotsAndMethods(TraitsBindingsp basetb,
  ...
  /*
      ...
      (Note that this behavior is theoretically possible on 32-bit builds as
     well, but since the slots only contain 32-bit-or-larger fields, there can't
     ever be enough properly-aligned padding for this to happen.)

      Anyway, the point of this digression is to point out that you can't ever
     assume that our slotArea starts "after" our parent; there might be overlap.
     m_offsetofSlots is computed for all native classes using C++ offsetof(),
     and thus correctly accounts for any overlap of this sort. See bugzilla
     655300 for more info.
  ...
  */
  ...
  if (slotAreaStart == 0 && base != NULL) {
    ...
    slotAreaStart = base->getHashtableOffset() ? base->getHashtableOffset()
                                               : base->getTotalSize();
  }

  int32_t next32BitSlotOffset = slotAreaStart;
  int32_t endOf32BitSlots =
      next32BitSlotOffset + (sizeInfo.nonPointer32BitSlotCount * 4);
  int32_t nextPointerSlotOffset =
      alignPointersTo8Bytes && (sizeInfo.pointerSlotCount != 0)
          ? pad8(endOf32BitSlots)
          : endOf32BitSlots;
  int32_t endOfPointerSlots =
      nextPointerSlotOffset + (sizeInfo.pointerSlotCount * sizeof(void *));
  int32_t next64BitSlotOffset =
      align8ByteSlots && (sizeInfo.nonPointer64BitSlotCount != 0)
          ? pad8(endOfPointerSlots)
          : endOfPointerSlots;
  int32_t endOf64BitSlots =
      next64BitSlotOffset + (sizeInfo.nonPointer64BitSlotCount * 8);
#ifdef VMCFG_FLOAT
  int32_t next128BitSlotOffset =
      align16ByteSlots && (sizeInfo.nonPointer128BitSlotCount != 0)
          ? pad16(endOf64BitSlots)
          : endOf64BitSlots;
  int32_t endOf128BitSlots =
      next128BitSlotOffset + (sizeInfo.nonPointer128BitSlotCount * 16);
#endif // VMCFG_FLOAT
  ...
  for (uint32_t i = 0; i < nameCount; i++) {
    ...
    case TRAIT_Class: {
      ...
      // note, for TRAIT_Class, AbcParser::parseTraits has already verified that
      // pool->cinits[ne.info] is not null
      Traitsp slotType = (ne.kind == TRAIT_Class)
                             ? pool->getClassTraits(ne.info)
                             : pool->resolveTypeName(ne.info, toplevel);
      uint32_t slotOffset = computeSlotOffset(
          slotType, next32BitSlotOffset, nextPointerSlotOffset,
          next64BitSlotOffset FLOAT_ONLY(, next128BitSlotOffset));
      AvmAssert(slotOffset >= sizeof(ScriptObject));
      tb->setSlotInfo(slotid, slotType,
                      slotStorageType(getBuiltinType(slotType)), slotOffset);
      break;
    }
    case TRAIT_Getter:
    case TRAIT_Setter:
    case TRAIT_Method: {
      ...
      if (b != BIND_NONE) {
        const uint32_t disp_id =
            uint32_t(uintptr_t(b) >> 3) + (ne.kind == TRAIT_Setter);
        ...
        // This should only be BIND_NONE if the attribute was elided via Api
        // Versioning.
  ...
  // check for sparse slot table -- anything not specified will default to *
  // (but we must allocate space for it)
  for (uint32_t i = 0; i < tb->slotCount; i++) {
    ...
    if (pool->isVerbose(VB_traits)) {
      core->console << "WARNING: slot " << i + 1 << " on " << this
                    << " not defined by compiler.  Using *\n";
    }
#endif
  ...
  if (!(next32BitSlotOffset == endOf32BitSlots &&
    ...
    // have a fuzzed file that duplicate slots in certain orders. This is
    // unacceptable, as it may allow access to the same memory location with
    // different type expectations.
    if (toplevel)
      toplevel->throwVerifyError(kCorruptABCError);
    AvmAssert(!"unhandled verify error");
  }
...
uint32_t Traits::countNewInterfaces(GCList<Traits> &seen) {
  ...
  while (!pending.isEmpty()) {
    ...
    for (uint32_t j = 0; j < interfaceCount; j++) {
      ...
      // an interface can "extend" multiple other interfaces, so we must recurse
      // here.
...
TraitsBindings *Traits::_buildTraitsBindings(const Toplevel *toplevel,
                                             bool includeTypes) {
#ifdef AVMPLUS_VERBOSE
  if (pool->isVerbose(VB_traits)) {
  ...
  // if we know the cap we need, go there right away, otherwise start at small
  // power of 2 this saves tremendously on subsequent builds of this set of
  // bindings since we don't have to waste time growing the MNHT as we build it
  const int32_t bindingCap = m_bindingCapLog2 ? (1 << m_bindingCapLog2) : 2;

  MMgc::GC *gc = core->GetGC();
  MultinameBindingHashtable *bindings =
      MultinameBindingHashtable::create(gc, bindingCap);
  ...
  if (this->posType() == TRAITSTYPE_CATCH) {
    ...
    addVersionedBindings(bindings, this->name(), compat_nss,
                         AvmCore::makeSlotBinding(0, BKIND_VAR));
    // bindings just need room for one slot binding
    thisData = TraitsBindings::alloc(gc, this, /*base*/ NULL, bindings,
                                     /*slotCount*/ 1, /*methodCount*/ 0, true);
    thisData->setSlotInfo(0, t, slotStorageType(getBuiltinType(t)),
                          this->m_sizeofInstance);
    thisData->m_slotSize = is8ByteSlot(t) ? 8 : 4;
  } else {
    TraitsBindingsp basetb =
        this->base ? this->base->getTraitsBindings() : NULL;
    ...
    buildBindings(basetb, bindings, slotCount, methodCount, slotSizeInfo,
                  toplevel);

    thisData = TraitsBindings::alloc(gc, this, basetb, bindings, slotCount,
                                     methodCount, includeTypes);
    if (slotSizeInfo) {
      thisData->m_slotSize =
          finishSlotsAndMethods(basetb, thisData, toplevel, *slotSizeInfo);
      if (basetb)
  ...
  // allocate in 8-byte increments anyway, so we're not really losing any
  // space.) (only really necessary to do this if we calced slotSizeInfo, but
  // simpler & harmless to just do it unconditionally)
  thisData->m_slotSize = (thisData->m_slotSize + (sizeof(uintptr_t) - 1)) &
                         ~(sizeof(uintptr_t) - 1);

  // remember the cap we need
  if (m_bindingCapLog2 == 0)
    m_bindingCapLog2 = calcLog2(
        thisData->m_bindings->numQuads); // remember capacity, not count
...
TraitsMetadata *Traits::_buildTraitsMetadata() {
  ...
  const uint32_t extra = td->slotCount * sizeof(TraitsMetadata::MetadataPtr) +
                         td->methodCount * sizeof(TraitsMetadata::MetadataPtr);

  TraitsMetadata *tm =
      TraitsMetadata::create(gc, extra, basetm, this->pool, this->metadata_pos,
                             td->slotCount, td->methodCount);
  tm->slotMetadataPos = (TraitsMetadata::MetadataPtr *)(tm + 1);
  tm->methodMetadataPos =
      (TraitsMetadata::MetadataPtr *)(tm->slotMetadataPos + tm->slotCount);

  const uint8_t *pos = traitsPosStart();
  const uint32_t nameCount = pos ? AvmCore::readU32(pos) : 0;
  SlotIdCalcer sic(td->base ? td->base->slotCount : 0,
                   this->allowEarlyBinding());
  NameEntry ne;
  for (uint32_t i = 0; i < nameCount; i++) {
    ne.readNameEntry(pos);
    switch (ne.kind) {
    ...
    case TRAIT_Method: {
      if (ne.tag & ATTR_metadata) {
        Multiname qn;
        // passing NULL for toplevel here, since it's only used if a
        // verification error occurs -- but if there was one, we would have
        // encountered it during AbcParser::parseTraits.
...
void Traits::verifyBindings(const Toplevel *toplevel) {
  ...
  // make sure our supertypes have verified their bindings. (must be done before
  // calling _buildTraitsBindings)
...
void Traits::resolveSignaturesSelf(const Toplevel *toplevel) {
  ...
  // make sure our supertypes are resolved. (must be done before calling
  ...
  if (m_needsHashtable ||
      (base && base->base && base->m_hashTableOffset && !isXMLType())) {
    // round up total size to multiple of pointer size.
    m_totalSize =
        ((m_totalSize + (sizeof(uintptr_t) - 1)) & ~(sizeof(uintptr_t) - 1));
    m_hashTableOffset = m_totalSize;
    m_totalSize += sizeof(InlineHashtable);
    AvmAssert(builtinType == BUILTIN_boolean ? true
                                             : (m_hashTableOffset & 3) == 0);
  ...
  if (tb->methodCount) {
    ...
    for (; tbm < tbm_end; ++tbm) {
      // don't assert: could be null if only one of a get/set pair is
      // implemented
  ...
  if (tbbase && tbbase->methodCount > 0) {
    ...
    const TraitsBindings::BindingMethodInfo *basetbm_end =
        basetbm + tbbase->methodCount;
    const TraitsBindings::BindingMethodInfo *tbm = tb->getMethods();
  ...
  tb->buildSlotDestroyInfo(core->GetGC(), m_slotDestroyInfo, slotAreaCount,
                           slotAreaSize);
...
PrintWriter &Traits::printExtended(PrintWriter &pw) {
  ...
  if (domainString) {
    ...
    // not a well known domain, print the address to help distinguish distinct
    // domains
...
#ifdef VMCFG_AOT
static inline void
hookUpActivationTraitsInitMethodForTraitsMethod(AvmCore *core, MethodInfo *m) {
  ...
  // See comment in initActivationTraits about why this can be called more than
  // once per Traits
  if (activationTraits->init == NULL) {
    if (aotInfo->activationInfo[m->method_id()].initHandler) {
      ...
      activationTraits->init = MethodInfo::create(
          ...
          aotInfo->activationInfo[m->method_id()].initMethodId);
    }
    // The following comes from Verifier::write() TODO: refactor so we can share
    // this code
    const ScopeTypeChain *scope = m->activationScope();
    if (scope == NULL) {
      scope = m->declaringScope()->cloneWithNewTraits(core->GetGC(),
                                                      activationTraits);
...
void Traits::initActivationTraits() {
  // Note: this can be called multiple times per Traits from initScript, which
  // must call this in case it's needed but is itself called once per Toplevel
  const uint8_t *pos = traitsPosStart();
...
bool Traits::isMachineCompatible(const Traits *a, const Traits *b) {
  return (a == b) ||
         ...
         ((!a || a->builtinType == BUILTIN_object ||
           a->builtinType == BUILTIN_void) &&
          (!b || b->builtinType == BUILTIN_object ||
           b->builtinType == BUILTIN_void)) ||
...
void Traits::genInitBody(const Toplevel *toplevel, TraitsBindings *tb) {
  ...
  struct ValidateInitVisitor : public InitVisitor {
    ...
    void defaultVal(Atom, uint32_t, Traits *) { hasDefaults = true; }
  };
  ValidateInitVisitor visitor;
  visitInitBody(&visitor, toplevel, tb);

  // if initialization code gen is required, create a new method body and write
  // it to traits->init->body_pos
  if (visitor.hasDefaults && !this->init) {
    // Make a do-nothing init method.
    this->init =
        MethodInfo::create(core->gc, MethodInfo::kInitMethodStub, this);
    ...
    static const uint8_t body[] = {
        ...
        0                       // exception_count
        // the verifier and interpreter don't read the activation traits so stop
        // here
    };
...
void Traits::visitInitBody(InitVisitor *visitor, const Toplevel *toplevel,
                           const TraitsBindings *tb) {
  const uint8_t *pos = traitsPosStart();
  SlotIdCalcer sic(tb->base ? tb->base->slotCount : 0,
                   this->allowEarlyBinding());
  NameEntry ne;
  const uint32_t nameCount = pos ? AvmCore::readU32(pos) : 0;
  for (uint32_t i = 0; i < nameCount; i++) {
    ...
    case TRAIT_Class: {
      ...
      Atom value =
          pool->getLegalDefaultValue(toplevel, value_index, kind, slotType);
...
void Traits::destroyInstance(ScriptObject *obj) const {
  ...
  if (!m_slotDestroyInfo.test(0)) {
    ...
    AvmAssert(m_totalSize >= (sizeof(AvmPlusScriptableObject) +
                              (ht ? sizeof(InlineHashtable) : 0)));
    uint32_t sizeToZero = m_totalSize - (sizeof(AvmPlusScriptableObject) +
                                         (ht ? sizeof(InlineHashtable) : 0));
    AvmAssert((sizeToZero % sizeof(uint32_t)) ==
              0); // we assume all sizes are multiples of sizeof(uint32_t)

    // no RCObjects, so just zero it all... my, that was easy
    VMPI_memset(p, 0, sizeToZero);
  } else {
    uint32_t sizeToZero =
        m_sizeofInstance - uint32_t(sizeof(AvmPlusScriptableObject));
    AvmAssert((sizeToZero % sizeof(uint32_t)) ==
              0); // we assume all sizes are multiples of sizeof(uint32_t)
    VMPI_memset(p, 0, sizeToZero);
...
#ifdef AVMPLUS_64BIT
        AvmAssert((uintptr_t(p) & 7) ==
                  0); // we had better be on an 8-byte boundary...
#endif
        Atom a = *(const Atom *)p;
        RCObject *rc = NULL;
...
void Traits::traceSlotsFromBitmap(MMgc::GC *gc, uint32_t *p,
                                  const FixedBitSet &traceInfo,
                                  uint32_t bitsUsed) const {
  for (uint32_t bit = 1; bit <= bitsUsed; bit++) {
    if (traceInfo.test(bit)) {
      ...
      if (atomKind(a) <= avmplus::AtomConstants::kNamespaceType) {
        ...
        gc->TracePointer(atomPtr(a) HEAP_GRAPH_ARG((uintptr_t *)p));
      } else if (atomKind(a) == avmplus::AtomConstants::kSpecialBibopType &&
                 a != undefinedAtom) {
        AvmAssert(bibopKind(a) == avmplus::AtomConstants::kBibopFloatType ||
                  bibopKind(a) == avmplus::AtomConstants::kBibopFloat4Type);
        gc->TracePointer(atomPtr(a) HEAP_GRAPH_ARG((uintptr_t *)p));
      }
    }
    p++;
...
Binding Traits::getOverride(TraitsBindingsp basetb, Namespacep ns, Stringp name,
                            int tag, const Toplevel *toplevel) const {
  Binding baseBinding = BIND_NONE;
  if (base) {
    const Namespacep lookupNS =
        (protectedNamespace == ns && base->protectedNamespace)
  ...
  if (((kOverrideRequired[baseBindingKind] & dkMask) ? ATTR_override : 0) !=
      (tag & ATTR_override))
    goto failure;
...
failure:
  ...
  if (pool->isVerbose(VB_traits) || pool->core->config.verifyonly)
    core->console << "illegal override in " << this << ": "
                  << Multiname(ns, name) << "\n";
#endif
  if (toplevel)
    toplevel->throwVerifyError(
        kIllegalOverrideError,
        toplevel->core()->toErrorString(Multiname(ns, name)),
        toplevel->core()->toErrorString(this));
  AvmAssert(!"unhandled verify error");
  return BIND_NONE;
}
...
#endif
  toplevel->throwVerifyError(kIllegalOverrideError,
                             toplevel->core()->toErrorString(minfo),
                             toplevel->core()->toErrorString(this));
  AvmAssert(!"unhandled verify error");
}
...
TraitsBindings *FASTCALL Traits::_getTraitsBindings() {
  AvmAssert(m_bindingsVerified);
  TraitsBindings *tb =
      _buildTraitsBindings(/*toplevel*/ NULL, /*includeTypes*/ m_resolved);
  return tb;
}
...
void Traits::build_primary_supertypes() {
  MMGC_STATIC_ASSERT(offsetof(Traits, m_primary_supertypes) +
                         sizeof(m_primary_supertypes) <
                     256);
  ...
  if (!base) {
    ...
    m_supertype_offset = isInterface() ? offsetof(Traits, m_supertype_cache)
                                       : offsetof(Traits, m_primary_supertypes);
    ...
    for (int i = 0; i < MAX_PRIMARY_SUPERTYPE; i++)
      WB(core->gc, this, &m_primary_supertypes[i],
         base->m_primary_supertypes[i]);
    size_t off = base->m_supertype_offset;
    if (off != offsetof(Traits, m_supertype_cache) &&
        (off += sizeof(Traits *)) - offsetof(Traits, m_primary_supertypes) <
            sizeof(m_primary_supertypes)) {
...
void Traits::build_secondary_supertypes() {
  ...
  if (!isInstanceType() || (count = countNewInterfaces(seen)) == 0) {
    ...
    if (base && !base->isPrimary() &&
        base->get_secondary_supertypes()[0] != base) {
      seen.add(base);
      count++;
    }
    UnscannedTraitsArray *list;
    uint32_t baseCount =
        base ? countSupertypes(base->get_secondary_supertypes()) : 0;
    if (baseCount > 0) {
      uint32_t total = count + baseCount;
      list = allocSupertypeList(gc, total);
      for (Traits **d = list->AsArray(), **s = base->get_secondary_supertypes();
           *s != NULL; s++, d++)
        WB_SKIP(gc, list, d, *s);
    } else {
...
Traits *Traits::readBinding(Traits *traits, Binding b) {
  ...
  case BKIND_VAR:
  case BKIND_CONST:
    return traits->getTraitsBindings()->getSlotTraits(
        AvmCore::bindingToSlotId(b));
  }
}
