...
//
// class ClassInfo
//      for output, it converts a (Traits *) into a list of multinames to
//      output->write(obj->get(mn)) for input, it holds the list of multinames
//      to obj->set(input->read())
//
// class ObjectOutput
//      functionality common to writing avmplus::ScriptObject using AMF0 or AMF3
//      encoding
//
...
//
// class ObjectInput
//      functionality common to reading avmplus::ScriptObject using AMF0 or AMF3
//      encoding
//
...
// WARNING: These do not match the AtomConstant types
    ...
    protected:
        ...
        void TraitsTableAdd(Traits *traits) {
            TableAdd(m_htTraits, AvmCore::genericObjectToAtom(traits));
        }

        void ObjectTableAdd(GCRef <ScriptObject> obj) {
            TableAdd(m_htObject, obj->atom());
        }

        void ClassInfoListAdd(ClassInfo *info) { m_listClassInfo.add(info); }

        int32_t StringTableFind(String *str) {
            return TableFind(m_htString, str->atom());
        }

        int32_t TraitsTableFind(Traits *traits) {
            return TableFind(m_htTraits, AvmCore::genericObjectToAtom(traits));
        }

        int32_t ObjectTableFind(GCRef <ScriptObject> obj) {
            return TableFind(m_htObject, obj->atom());
        }

        ClassInfo *ClassInfoListFind(uint32_t i) {
            AvmAssert(i < m_listClassInfo.length());
    ...
    private:
        ...
        int32_t TableFind(HeapHashtable *ht,
                          Atom atom); // returns -1 for atom not found
    ...
    protected:
        virtual bool handleAdditionalType(uint8_t /*type*/, Atom & /*value*/) {
            return false;
        }
    ...
    public:
        ...
        static ClassInfo *Read(Toplevel *tl, AvmPlusObjectInput *input, bool dynamic,
                               bool externalizable, int count);
    ...
    private:
        ClassInfo(Toplevel *tl)
                : m_toplevel(tl), m_sealed(tl->core()->GetGC(), kListInitialCapacity) {
        } // static Read() will use this, and it initializes members
    ...
    public:
        AvmOutputWrapper(Toplevel *tl, DataOutput *out)
                : AvmPlusObjectOutput(tl), m_toplevel(tl), m_out(out) {}

        virtual void Write(const void *buffer, uint32_t count) {
            m_out->Write(buffer, count);
        }
    ...
    public:
        AvmInputWrapper(Toplevel *tl, DataInput *in)
                : AvmPlusObjectInput(tl), m_toplevel(tl), m_in(in) {}
