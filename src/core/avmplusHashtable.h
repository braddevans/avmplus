...
// NOTE NOTE NOTE NOTE
//
// InlineHashtable is designed to never have its ctors or dtors called directly;
// it can't be allocated either on the stack or the heap. For a standalong heap
// allocation, use HeapHashtable instead.
//
// The reason for this oddity is that InlineHashtable can be grafted "inline" to
// a ScriptObject, and will be manually initialized and destroyed (but not via
// its ctor/dtor). We also don't want to descend from any class that might force
// us to have a vtable (eg by way of a virtual dtor) because we neither need one
// nor want to account for the extra pointer-sized space in every instance of
// ScriptObject. (It's not large, but it adds up...)
//
// NOTE NOTE NOTE NOTE
// NOTE NOTE NOTE NOTE
// NOTE NOTE NOTE NOTE
// NOTE NOTE NOTE NOTE
    ...
    void initializeWithDontEnumSupport(MMgc::GC *gc,
    ...
    // rather than undefinedAtom. This is due to the regettable fact that
    // undefinedAtom is a legal value for a hashtable value (while EMPTY is not).
    // (Note that we can't actually use "EMPTY" here due to compiler issues, see
    // isEmpty() for explanation)
    ...
    /**
     * The next()/keyAt()/valueAt() methods allow a caller to enumerate all
     * entries in the table. Note that the value passed in is not an index, per
     * se; it's an integral value that indicates that next entry, but is not
     * guaranteed to increase monotonically, or allow for random access: instead,
     * it should be treated as a "magic cookie". Proper usage is (conceptually) as
     * follows:
     *
     ...
     *
     * All nonzero values of the next() int arg are reserved. Don't assume what
     * they mean.
     */
    ...
    // kDontEnumBit is or'd into atoms to indicate that the property is
    // {DontEnum}, and ALSO or'd into m_atomsAndFlags to indicate that the
    // InlineHashtable as a whole supports DontEnum.
    ...
    // if kHasIterIndex is set, there are two extra Atoms allocated at the end of
    // our array (giving it capacity+2) that are used to cache the current value
    // of next()
    static const uintptr_t kHasIterIndex = 0x04;
    static const uintptr_t kAtomFlags =
            (kDontEnumBit | kHasDeletedItems | kHasIterIndex);

    // if an iter index has a key of kIntptrType, we set this bit; this allows us
    // to continue iteration in the expected order even if the current item is
    // deleted during iteration. Since iter indices can't be negative or >2^31,
    // this is a safe value.
    static const int kIterIndexIsIntptr = 0x80000000;

    //
    // "capacity" is the total number of atoms we allocate.
    // we use that capacity as name-value pairs, thus the maximum size at any time
    // is always half the capacity. for 32-bit builds, we want to limit the
    // maximum number of entries to (1<<27)-1, thus we the max capacity we need is
    // (1<<28)-2. but since capacity is always limited to a power of two, we'll
    // actually limit capacity to 1<<27... which in turn will limit maximum number
    // of entries to (1<<26)-1. this has the downside of halving our maximum size,
    // but the upside of avoiding the need to check m_size for overflow on every
    // put (we only need to check capacity for overflow on every grow).
    //
    // (note: for consistency between 32 and 64-bit builds, we'll artificially
    // limit 64-bit systems to the same size, even though the m_size field can
    // hold more.)
    //
    // How does this compare with pre-existing behavior on 32-bit systems?
    // theoretically, capacity could have been an allocation of 1<<32 == 4GB max
    // (it's always a power of two)... but some memory is of course used for other
    // purposes, thus effectively 2GB max. divide by sizeof(Atom) == 4 to find we
    // had an actual max-capacity of 1<<29 entries. So it is mathematically
    // possible that a hashtable that was previously possible will now prematurely
    // run out of memory... but extraordinarily unlikely.
    //
    // (In practice, Win32 limits each process to 2GB, so we can halve the above
    // for those systems, thus "portable" ABC/SWF code could only rely on a max
    // capacity of 1<<28 entries.)
    //
    //
    ...
    InlineHashtable();
    // do NOT make this virtual; we want InlineHashtable to NOT have ANY virtual
    // methods, not even a dtor
    ~
...
int rehash(const Atom *oldAtoms, int oldlen, Atom *newAtoms,
           int newlen) const;
...
uintptr_t GC_POINTER(
        m_atomsAndFlags); /** property hashtable, this has no GCMember<> on
                           purpose, setAtoms contains the WB */
GC_DATA_END(InlineHashtable);
#ifdef AVMPLUS_64BIT
// on 64-bit systems, padding will force us to 16 bytes here anyway, so let's
// just use unpacked ints
uint32_t m_size;        // number of properties
uint32_t m_logCapacity; // (log2 of capacity) + 1
#else
uint32_t m_size
: 27;       // number of properties
uint32_t m_logCapacity
: 5; // (log2 of capacity) + 1 -- gives us enough space
// for 2^32 entries
#endif
// ------------------------ DATA SECTION END
};
 ...
 */
HeapHashtable(MMgc::GC
*gc,
int32_t capacity = InlineHashtable::kDefaultCapacity
);

static HeapHashtable *
create(MMgc::GC *gc, int32_t capacity = InlineHashtable::kDefaultCapacity);
