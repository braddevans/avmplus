...
// !!@ I'm not sure what this was supposed to do originally but I've rewritten
// it to remove the leading and trailing white space for text elements. "     5
// 4     3     " becomes "5     4     3" This is to simulate the E4X XML parser
  ...
  for (;;) {
    ...
    while (ch != delimiter) {
      if (atEnd() || ch == '<')
        ...
        // Changed this from kMalformedElement to kUnterminatedAttributeValue
        // for bug 117058(105422)
...
Stringp XMLParser::unescape(int32_t start, int32_t last, bool intern) {
  ...
  if (bgn < 0) {
    return intern ? core->internSubstring(m_str.str(), start, last)
...
XMLParser::XMLParser(AvmCore *core, Stringp str)
    : m_str(truncateAtFirstNullChar(core, str)), m_pos(0) {
  this->core = core;
  ...
  if (!core->xmlEntities) {
    ...
    while (*entities) {
      core->xmlEntities->add(
          core->internConstantStringLatin1(entities + 1)->atom(),
...
bool XMLTag::nextAttribute(uint32_t &index, Stringp &name, Stringp &value) {
