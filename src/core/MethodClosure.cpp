...
MethodClosure *MethodClosureClass::create(MethodEnv *m, Atom obj) {
  ...
  if (!ref || ref->isNull()) {
    ...
    // since MC inherits from CC, we must explicitly set the prototype and
    // delegate since the ctor will leave those null (and without delegate set,
    // apply() and friends won't be found in pure ES3 code)
...
// [jd] The slot table of a method closure is used to hold the value of the
// length property for the method. Dont know how important this is, but there
// you go. [ed] 9/15/04 we're implementing length as a getter instead of a slot.
...
PrintWriter &MethodClosure::print(PrintWriter &prw) const {
  return prw << "MC{" << asAtom(get_savedThis()) << " " << get_callEnv()->method
             << "}@" << asAtomHex(atom());
}
#endif

/*virtual*/ bool MethodClosure::isValid() const { return true; }

#ifdef _DEBUG
/*virtual*/ bool MethodClosure::isWeak() const { return false; }
#endif

/*virtual*/ Atom MethodClosure::get_savedThis() const { return m_savedThis; }
...
/*virtual*/ MethodClosure *MethodClosure::weaken() const {
  ...
  if (!AvmCore::isObject(m_savedThis))
    return const_cast<MethodClosure *>(this);
  return WeakMethodClosure::create(gc(), this->vtable, get_callEnv(),
                                   m_savedThis);
}
...
bool MethodClosure::equals(const MethodClosure *that) const {
  ...
  return that != NULL && this->get_callEnv() == that->get_callEnv() &&
...
uintptr_t MethodClosure::hashKey() const {
  ...
  // AvmAssert(isValid());
  return ((uintptr_t((MethodEnv *)get_callEnv()) << 8) |
          (uintptr_t(get_savedThisOrNull()) >> 3));
}
...
Atom MethodClosure::construct(int /*argc*/, Atom * /*argv*/) {
  ...
  toplevel()->throwTypeError(kCannotCallMethodAsConstructor,
                             core()->toErrorString(get_callEnv()->method));
 ...
 * Call entry point.
 */
/*static*/ Atom MethodClosure::callMethodClosure(MethodClosure *f, int argc,
                                                 Atom *argv) {
...
#ifdef _DEBUG
/*virtual*/ bool WeakMethodClosure::isWeak() const { return true; }
#endif
...
/*static*/ Atom WeakMethodClosure::callWeakMethodClosure(WeakMethodClosure *f,
                                                         int argc, Atom *argv) {
