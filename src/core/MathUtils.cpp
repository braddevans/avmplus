...
static int32_t skipSpaces(const StringIndexer &s, int32_t index) {
  while (index < s->length()) {
    uint32_t ch = s[index];
    if (!(ch == ' ' || ch == '\t' || ch == '\r' || ch == '\n' || ch == '\f' ||
          ch == '\v' || (ch >= 0x2000 && ch <= 0x200b) || ch == 0x2028 ||
...
double MathUtils::parseInt(Stringp inStr, int32_t radix /*=10*/,
                           bool strict /*=false*/) {
  bool negate;
  ...
  if (radix >= 2 && radix <= 36 && index < s->length()) {
    ...
    if (result >= 0x20000000000000LL && // i.e. if the result may need at least
                                        // 54 bits of mantissa
        (radix == 2 || radix == 4 || radix == 8 || radix == 16 ||
         radix == 32)) {

      // CN:  we're here because we may have incurred roundoff error with the
      // above.
      ...
      for (next = 0; next * powOf2 <= 52;
           next++) { // read first 52 non-zero digits.  Once
                     // charPosition*log2(radix) is > 53, we can have rounding
                     // issues
      ...
      if (next * powOf2 >
          52) { // If number contains more than 53 bits of precision, may need
                // to roundUp last digit processed.
        bool roundUp = false;
        int32_t bit53 = 0;
        int32_t bit54 = 0;
        ...
        case 16: // last word read contained digits 50,51,52,53
          ...
          break;
        case 8: // last work read contained digits 49,50,51, next word contains
                // 52,53,54
          v = parseIntDigit(s[end]);
          if (v == -1 || v >= radix) {
            v = 0;
        ...
        case 2: // 52 - 53 - 54
          /*
          v = parseIntDigit(s[end++]);
          result = result * radix;  // add factor before round-off adjustment
          for 52 bit
          */
          bit53 = v & (1 << 0);
          v = parseIntDigit(s[end]);
          if (v != -1 && v < radix) {
            factor *= radix;
            bit54 = (v != -1 ? (v & (1 << 0))
                             : 0); // Might be there are only 53 digits.
          }
        ...
        while (++end < s->length()) {
          ...
          roundUp |=
              (v != 0); // any trailing positive bit causes us to round up
          factor *= radix;
        }
    ...
    // if there are more than 15 digits, roundoff error may affect us.  Need to
    use exact integer rep instead of float
...
// used by AvmCore::number() and numberAtom() for converting arbitrary string to
// a number.
double MathUtils::convertStringToNumber(Stringp inStr) {
  ...
  if (inStr->length() == 0) { // toNumber("") should be 0, not NaN
    ...
    value = MathUtils::parseInt(
        inStr, 0, true); // 0 radix means figure it out from the string.
  }
  return value;
}
...
static double powInt(double base, int32_t exponent) {
  ...
  if (MathUtils::isInfinite(base)) {
    ...
    // if base is -Infinity, return Infinity or -Infinity depending whether the
    // exponent is even or not.
    if (base < 0)
  ...
  if (exponent < 0) {
    ...
    while (exponent) {
      if (exponent & 1) {
        value /= base;
        // cn: max double value is magnitude 1e308 (base 10), but min double
        // value is magnitude 1e-324 That means we can't invert the exponent
        // when the base ten value exponent is < 307.
        //  We could first check if powerOfTwo(base)*exp > 1074, but that would
        //  slow down all powInts calls.  This limits the xtra work to just very
        //  small numbers.
        if (value == 0 && base != 0) // double check by calling the real thing
          return MathUtils::powInternal(original_base,
                                        (double)original_exponent);
      }
      exponent >>= 1;
      base *= base;
    }
...
Stringp MathUtils::convertIntegerToStringRadix(AvmCore *core, intptr_t value,
                                               int32_t radix,
                                               UnsignedTreatment treatAs) {
  MMgc::GC::AllocaAutoPtr _buffer;
  char *buffer =
      (char *)avmStackAlloc(core, _buffer, kMinSizeForInt64_t_toString);
...
Stringp MathUtils::convertIntegerToStringBase10(AvmCore *core, int32_t value,
                                                UnsignedTreatment treatAs) {
  char buffer[kMinSizeForInt32_t_base10_toString];
  int32_t len = kMinSizeForInt32_t_base10_toString;
#ifdef AVMPLUS_64BIT
  intptr_t wideVal =
      treatAs == kTreatAsUnsigned ? (intptr_t)(uint32_t)value : (intptr_t)value;
#else
...
char *MathUtils::convertIntegerToStringBuffer(intptr_t value, char *buffer,
                                              int32_t &len, int32_t radix,
                                              UnsignedTreatment treatAs) {
  ...
  // MathUtils::convertIntegerToString doesn't deal with this number because you
  // can't negate it.
  {
    AvmAssert(len >= 12);
    if (len < 12)
...
Stringp MathUtils::convertDoubleToStringRadix(AvmCore *core, double value,
                                              int32_t radix) {
  if (radix < 2 || radix > 36) {
    AvmAssert(0);
    return NULL;
  ...
  char *tmp =
      (char *)avmStackAlloc(core, _tmp, kMinSizeForDouble_base2_toString);
  ...
  if (value < 1) {
    *src-- = '0';
    ...
    while (uVal != 0) {
      ...
      *src-- =
          (char)((j < 10) ? ((int32_t)j + '0') : ((int32_t)j + ('a' - 10)));
...
Stringp MathUtils::convertDoubleToString(AvmCore *core, double value,
                                         int32_t mode, int32_t precision) {
  // FIXME: Bugzilla 442974: The test for infinity and NaN really needs to be in
  // the AS3 code, /or/ the range check on precision needs to be in this
  // function, because the former is supposed to happen before the latter for
  // toExponential and toPrecision. (As this function is used more generally it
  // seems probable that the correct fix is to add the infinity and NaN checks
  // to the AS3 code /and/ leave them in here.)

  AvmAssert(mode == DTOSTR_NORMAL || mode == DTOSTR_FIXED ||
            mode == DTOSTR_PRECISION || mode == DTOSTR_EXPONENTIAL);
  ...
  MMgc::GC::AllocaAutoPtr _buffer;
  char *const buffer =
      (char *)avmStackAlloc(core, _buffer, kMinSizeForDouble_base10_toString);
  const bool negative = value < 0.0;
  const bool zero = value == 0.0;
  const bool noFraction = (precision == 0);
  const bool bugzilla513039 =
      core->currentBugCompatibility()
          ->bugzilla513039; // Number.toFixed(0) returns incorrect numbers,
                            // rounding issues
  bool round = true;
  ...
  enum FormatType { kNormal, kExponential, kFraction, kFixedFraction };
  ...
  if (round && (d2a->bFastEstimateOk || mode == DTOSTR_FIXED ||
  ...
  if (exp10) {
    ...
    // FIXME: Bugzilla 442974: This code is broken, it will find the decimal
    // point.
    ...
    if (s == firstNonZero) {
      ...
      // if the value is zero, and I don't have any proof that that can't
      // happen.
      *s++ = '1';
      exp10++;
    } else if (!zero) {
      ...
      if (firstNonZero == lastNonZero) {
        exp10 += (int32_t)(s - firstNonZero -
                           1); // What if exp10 is already the negative of that?
        s = lastNonZero + 1;
      }
    }
    ...
    *s++ = 'e';
    ...
    int32_t len = kMinSizeForInt32_t_base10_toString;
    char *t =
        convertIntegerToStringBuffer(exp10, expstr, len, 10, kTreatAsSigned);
    while (*t) {
      *s++ = *t++;
    }
  }
  ...
  // Deal with the sentinel introduced for the kFraction case: we might have a
  // leading '00.'
...
//
bool MathUtils::convertStringToDouble(Stringp inStr, double *value,
                                      bool strict /*=false*/) {
  bool negate;
  int32_t numDigits = 0;
  int32_t exp10 = 0;
  ...
  if (index >= s->length()) // empty string or string of spaces == 0
  {
    *value = 0;
    return (strict ? true
                   : false); // odd use of strict, but Number('  ') is 0 and
                             // that uses strict == true.  parseFloat('  ') is
                             // NaN and that uses strict == false
  }
  ...
  while (index < slength) {
    ...
    if (ch >= '0' && ch <= '9') {
      ...
      // https://bugzilla.mozilla.org/show_bug.cgi?id=564839 -- stop parsing at
      // null char
      if (ch == 0) {
        slength = index;
      }
  ...
  if (ch == '.') {
    while (++index < slength) {
      ...
      if (ch >= '0' && ch <= '9') {
        ...
        // https://bugzilla.mozilla.org/show_bug.cgi?id=564839 -- stop parsing
        // at null char
  ...
  if (index < slength && (s[index] == 'e' || s[index] == 'E')) {
    ...
    if (negexp && index >= slength) // fail if string ends in "e-" with no
                                    // exponent value specified.
      return false;
  ...
  */
  const int32_t limit = inStr->core()->currentBugCompatibility()->bugzilla513018
                            ? index
                            : slength;
  index = start;
  if (numDigits > 15) // after 15 digits, we can run into roundoff error
  {
    ...
    //  This is an approximation which is at most off by 1.  To gain complete
    //  accuracy, we need to implement
    //   the algorithm commented out below
    result = exactInt.doubleValueOf();
    if (exp10 < 0) {
      if (exp10 < -307) { // max positive value is e308. min neg value is e-324
                          // Avoid overflow
        int32_t diff = exp10 + 307;
        result /= quickPowTen(-diff);
        exp10 -= diff;
      }
      result /= quickPowTen(-exp10); // actually more accurate than multiplying
                                     // by negative power of 10
    }
  } else // we can use double
  {
    ...
    if (exp10 >= 0) {
      ...
      if (exp10 < -307) { // max positive value is e308.  min neg value is
                          // e-324. Avoid overflow
        int32_t diff = exp10 + 307;
        result /= quickPowTen(-diff);
        exp10 -= diff;
      }
...
// cn: The following Alg is based on William Clinger's paper "How to Read
// Floating Point Numbers Accurately"
//
//  Using BigIntegers gives us a result which is either the best approximation
//  or the next best approximation.
//   The rest of this algorithm should be completed to adjust the result in
//   order to make sure its the best. Held off on finishing it as there are
//   bigger fish to fry, the ecmaScript number tests pass with the above and
//   perhaps that's good enough.  To finish
//      1) BigInteger class needs to be extended to support negative numbers
//      2) The section under findClosestFloat after the comment "Compare" needs
//      to be fleshed out with the
//         details from Clinger's paper to figure out if the result,
//         nextFloat(), or previousFloat() should be returned.  Note that no
//         loop needs to be completed because the BigInteger calclulation
//         gaurantees we are at worst one estimate off the true value and the
//         loop will only require a single iteration.
//      3) Implement nextFloat() / previousFloat()
/*
// Check if both the mantissa and exponent are guaranteed to accurately fit into
a double
//  (mantissa is <= 2^53, exponent is <= 10^23)   If so, we can get away with
double math. if ( (exactInt->numWords == 1) || (exactInt->numWords == 2 &&
wordBuffer[1] < 0x00200000) )  { // i.e. its < 0x0020 0000 0000 0000 == 2^53

    if (exp10 >= 0 && exp10 < 23) { // log5-of-two^53 = ceil( log(pow(2,53)) /
log(5) ) = ceil(22.82585758) = 23 double f = exactInt->doubleValueOf(); result =
f * quickPowTen(exp10);
    }
...
} else if (exactInt->numWords == 2) { // i.e. its < 0x0000 0001 0000 0000 0000
0000 == 2^64 if ( (exp10 >= 0) && (exp10 < 23) { result =
Bellerophon_MultiplyAndTest(exactInt, exp10, 0); } else { // exp10 < 0 || exp10
> 23) result = Bellerophon_MultiplyAndTest(exactInt, exp10, 3);
    }
} else {
    if ( (exp10 >= 0) && (exp10 < 23) {
...
MathUtils::Bellerophon_MultiplyAndTest(BigInteger* f, int32_t exp10, int32_t
slop)
{
...
int64_t lowBits = (int64_t)(mantissaEstimate % 2048); // two^p-n = 2^(64-53) =
2^11 = 2048
...
// compare
BigInteger* diff = compX->subtract(compY); // need to modify BigInteger to
...
static const uint32_t Random_Xor_Masks[31] = {
    /* First mask is for generating sequences of 3 (2^2 - 1) random numbers,
       ...
       /  etc.  Numbers to the right are number of bits of random number
       sequence. /  It generates 2^n - 1 numbers. */
...
#define RandomFastNext(_pRandomFast)                                           \
  (((_pRandomFast)->uValue & 1L)                                               \
       ? ((_pRandomFast)->uValue =                                             \
              ((_pRandomFast)->uValue >> 1) ^ (_pRandomFast)->uXorMask)        \
...
void MathUtils::initRandom(TRandomFast *seed) { RandomFastInit(seed); }
