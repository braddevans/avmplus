...
//   On PPC there is supposedly a three-operation sequence that byte swaps
//   32-bit
//     data [Warren, Hacker's Delight].
//   On MIPS a two-operation sequence of WSBH and ROTR can byte swap a 32-bit
//   word,
//     WSBH can byte swap a halfword by itself.
//   On SH4 a combination of SWAP.B (twice) and SWAP.H should byte swap a 32-bit
//   word,
//     while SWAP.B by itself should byte swap a 16-bit halfword.
//   On ARM there are the REV (32-bit) and REV16 (16-bit) instructions since
//   ARMv6. To my knowledge SPARC has no byte swap instructions.
//
// Survey of compilers:
//
//   MSVC starting with Visual Studio 2005 have _byteswap_ulong,
//   _byteswap_ushort, _byteswap_uint64. GCC starting with 4.3 have
//   __builtin_bswap32 and __builtin_bswap64, must include <byteswap.h>

#if defined VMCFG_IA32 || defined VMCFG_AMD64 || defined VMCFG_PPC ||          \
    defined VMCFG_MIPS || defined VMCFG_SH4 || defined VMCFG_ARM

#if defined _MSC_VER
#define HAVE_BYTESWAP32
#define HAVE_BYTESWAP16
REALLY_INLINE uint32_t byteSwap32(uint32_t x) { return _byteswap_ulong(x); }

REALLY_INLINE uint16_t byteSwap16(uint16_t x) { return _byteswap_ushort(x); }
#endif // _MSC_VER
...
REALLY_INLINE uint32_t byteSwap32(uint32_t x) { return __builtin_bswap32(x); }
#endif // __GNUC__

#endif // Various architectures with byteswap idioms backed by intrinsics

// MAX_BYTEARRAY_STORE_LENGTH is the largest value that can be taken on by
// m_length, strictly less than 2^32.  See constraint in
// ByteArray::Grower::ReallocBackingStore().
//
// MAX_BYTEARRAY_SHORT_ACCESS_LENGTH is a value no smaller than 4095 and no
// greater than 2^32-1-MAX_BYTEARRAY_STORE_LENGTH.
//
// The purpose of these two limits is to allow relatively efficient bounds
// checking on common ByteArray accesses, see comments on
// ByteArray::requestBytesForShortRead() and
// ByteArray::requestBytesForShortWrite() for more information.

#define MAX_BYTEARRAY_STORE_LENGTH                                             \
  (MMgc::GCHeap::kMaxObjectSize - MMgc::GCHeap::kBlockSize * 2)
#define MAX_BYTEARRAY_SHORT_ACCESS_LENGTH (MMgc::GCHeap::kBlockSize * 2 - 1)

namespace avmplus {
VMPI_DEFINE_FAILFAST(ByteArrayValidationError)

template <typename T> REALLY_INLINE T min(T a, T b) { return (a < b) ? a : b; }
 ...
 * NB. memcpy is allowed to copy backwards, in big gulps, etc. No overlap is
 * acceptable. I (matz) wonder if the extra checking done by this method washes
 * out the benefit of calling memcopy? Prompted by (google)fuzzing bug 3303673.
 */
inline bool non_overlapping_memcpy_args(const void *dst, const void *src,
  ...
  // do the subtraction such that result is positive and can't overflow
  // anything. if the difference is > uint32_t, it's okay, compare to count will
  // be right.
...
ByteArray::ByteArray(Toplevel *toplevel)
    : DataIOBase(), DataInput(), DataOutput(), m_toplevel(toplevel),
      m_gc(toplevel->core()->GetGC()), m_subscribers(m_gc, 0),
      m_copyOnWriteOwner(NULL), m_position(0), m_buffer(mmfx_new(Buffer())),
      m_isShareable(false), m_isShareablePropReadOnly(false),
      m_isLinkWrapper(false) {
  static_assert(uint64_t(MAX_BYTEARRAY_STORE_LENGTH) < 0x100000000ULL,
                "Constraint on MAX_BYTEARRAY_STORE_LENGTH");
  static_assert(MAX_BYTEARRAY_SHORT_ACCESS_LENGTH >= 4095,
                "Constraint on MAX_BYTEARRAY_SHORT_ACCESS_LENGTH");
  static_assert(uint64_t(MAX_BYTEARRAY_STORE_LENGTH) +
                        uint64_t(MAX_BYTEARRAY_SHORT_ACCESS_LENGTH) <
                    0x100000000ULL,
                "Constraints on internal ByteArray constants");
  AvmAssert(m_gc != NULL);

  m_buffer->initialize(/*array*/ NULL, /*capacity*/ 0, /*length*/ 0,
                       /*copyOnWrite*/ false);
}
...
ByteArray::ByteArray(Toplevel *toplevel, const ByteArray &other)
    : DataIOBase(), DataInput(), DataOutput(), m_toplevel(toplevel),
      m_gc(toplevel->core()->GetGC()), m_subscribers(m_gc, 0),
      m_copyOnWriteOwner(NULL), m_position(0),
      m_buffer(other.m_isShareable ? other.m_buffer : mmfx_new(Buffer)),
      m_isShareable(other.m_isShareable), m_isShareablePropReadOnly(false),
      m_isLinkWrapper(false) {
  AvmAssert(m_gc != NULL);
  if (m_isShareable) {
    return;
  }
  m_buffer->setCapacity(other.m_buffer->getCapacity());
  m_buffer->setLength(other.m_buffer->getLength());
  m_buffer->setCopyOnWrite(false);
  // only allocate a new array if the other bytearray has data in it
  if (other.m_buffer->getArray()) {
    uint8_t *newArray =
        ByteArrayBuffer_alloc_CanFailAndZero(m_buffer->getCapacity());
    m_buffer->setArray(newArray);

    if (!newArray)
      ThrowMemoryError();

    TellGcNewBufferMemory(newArray, m_buffer->getCapacity());
    VMPI_memcpy(newArray, other.m_buffer->getArray(), m_buffer->getLength());
  } else {
    m_buffer->setArray(NULL);
  }
}

ByteArray::ByteArray(Toplevel *toplevel, ByteArray::Buffer *source,
                     bool shareable)
    : DataIOBase(), DataInput(), DataOutput(), m_toplevel(toplevel),
      m_gc(toplevel->core()->GetGC()), m_subscribers(m_gc, 0),
      m_copyOnWriteOwner(NULL), m_position(0), m_buffer(source),
      m_isShareable(shareable), m_isShareablePropReadOnly(false),
      m_isLinkWrapper(false) {
  // Note that this constructor is only used when receiving a ByteArray from
  // another worker
  // We should not receive a buffer containing copy-on-write data from another
  // worker. It is our caller's responsibility to assure that the Buffer object
  // owns its data array.
  AvmAssert(!source->getCopyOnWrite());
  if (!m_isShareable) {
    // If we made a copy of the ByteArray (i.e. it's not sharable), then we
    // should account for the memory that we're about to receive. (until now,
    // it's been sitting in the message queue, not assigned to any GC)
    TellGcNewBufferMemory(m_buffer->getArray(), m_buffer->getCapacity());
  }
}
...
/* virtual */ void ByteArray::Buffer::destroy() {
  if (array && !copyOnWrite) {
    ...
    // FIXME: the lack of the following appears to be accidentally correct,
    // because for normal uses array will be NULL in this->destroy() and the
    // Isolate use doesn't signal dependent allocation
...
ByteArray::~ByteArray() {
  ...
  if (!m_isShareable) {
    ...
    // m_subscribers.clear(); If this is the only ByteArray referencing this
    // buffer then it is okay to clear it.
...
void ByteArray::UnprotectedClear() {
  if (m_buffer->getArray() && !m_buffer->getCopyOnWrite()) {
    AvmAssert(m_buffer->getCapacity() > 0);
    // Note that TellGcXXX always expects capacity, not (logical) length.
    uint8_t *array = m_buffer->getArray();
    TellGcDeleteBufferMemory(array, m_buffer->getCapacity());
    ByteArrayBuffer_free(array);
  }
  m_buffer->initialize(NULL, 0, 0, false);
#if defined(VMCFG_TELEMETRY_SAMPLER) && defined(DEBUGGER)
  if (m_gc->GetAttachedSampler()) {
    ((IMemorySampler *)m_gc->GetAttachedSampler())
        ->recordObjectReallocation(this);
  }
#endif
  m_copyOnWriteOwner = NULL;
}
...
void ByteArray::SetCopyOnWriteData(MMgc::GCObject *owner, const uint8_t *data,
                                   uint32_t length) {
  Clear();
  m_buffer->initialize(const_cast<uint8_t *>(data), length, length, true);
  SetCopyOnWriteOwner(owner);
}

REALLY_INLINE bool ByteArray::Grower::RequestWillReallocBackingStore() const {
  return m_minimumCapacity > m_owner->m_buffer->getCapacity() ||
         m_owner->m_buffer->getCopyOnWrite();
}

REALLY_INLINE bool ByteArray::Grower::RequestExceedsMemoryAvailable() const {
  return m_minimumCapacity >
         (MMgc::GCHeap::kMaxObjectSize - MMgc::GCHeap::kBlockSize * 2);
}

void FASTCALL
ByteArray::Grower::EnsureWritableCapacity(bool calledFromLengthSetter) {
  if (RequestExceedsMemoryAvailable())
    m_owner->ThrowMemoryError();

  if (RequestWillReallocBackingStore()) {
    uint32_t newCapacity = m_owner->m_buffer->getCapacity() << 1;
    ...
    // do not set capacity to kGrowthIncr, but let it remain as set by the
    // setter.
    if (!(m_owner->m_buffer->getCapacity() == 0 && calledFromLengthSetter)) {
...
void FASTCALL ByteArray::Grower::ReallocBackingStore(
    uint32_t newCapacity, bool calledFromLengthSetter) {
  ...
  static_assert(MAX_BYTEARRAY_STORE_LENGTH == (MMgc::GCHeap::kMaxObjectSize -
  ...
  if (!(newCapacity != m_owner->m_buffer->getCapacity() ||
        m_owner->m_buffer->getCopyOnWrite())) {
    return;
  }
  ...
  m_oldArray = m_owner->m_buffer->getArray();
  m_oldLength = m_owner->m_buffer->getLength();
  m_oldCapacity = m_owner->m_buffer->getCapacity();
  m_oldCopyOnWrite = m_owner->m_buffer->getCopyOnWrite();
  ...
  m_owner->m_buffer->setArray(newArray);
  m_owner->m_buffer->setCapacity(newCapacity);
  m_owner->m_buffer->setCopyOnWrite(false);
  if (m_oldCopyOnWrite) {
  ...
  if (m_owner->m_gc->GetAttachedSampler()) {
    ((IMemorySampler *)m_owner->m_gc->GetAttachedSampler())
        ->recordObjectReallocation(m_owner);
  }
#endif
}
...
ByteArray::Grower::~Grower() {
  if (m_oldArray != m_owner->m_buffer->getArray() ||
      m_oldLength != m_owner->m_buffer->getLength()) {
    m_owner->NotifySubscribers();
  }
  // m_oldArray could be NULL if we grew a copy-on-write ByteArray.
  if (m_oldArray != NULL && m_oldArray != m_owner->m_buffer->getArray()) {
    // Note that TellGcXXX always expects capacity, not (logical) length.
...
public:
  ByteArrayTask(ByteArray *ba)
      : m_byteArray(ba), m_core(ba->m_toplevel->core()), m_exception(NULL) {}
...
public:
  ByteArraySetLengthTask(ByteArray *ba, uint32_t newLength,
                         bool calledFromLengthSetter)
      : ByteArrayTask(ba), m_newLength(newLength),
        m_calledFromLengthSetter(calledFromLengthSetter) {}

  void run() {
    // safepoints cannot survive exceptions
    TRY(m_core, kCatchAction_Rethrow) {
      m_byteArray->UnprotectedSetLengthCommon(m_newLength,
                                              m_calledFromLengthSetter);
    }
    CATCH(Exception * e) { m_exception = e; }
    END_CATCH;
    END_TRY;
  }
...
public:
  ByteArrayCompareAndSwapLengthTask(ByteArray *ba, uint32_t expectedLength,
                                    uint32_t newLength)
      : ByteArrayTask(ba), result(0), m_expectedLength(expectedLength),
  ...
  void run() {
    ...
    TRY(m_core, kCatchAction_Rethrow) {
      ...
      result = m_byteArray->UnprotectedAtomicCompareAndSwapLength(
          m_expectedLength, m_newLength);
    }
    CATCH(Exception * e) { m_exception = e; }
    END_CATCH;
    END_TRY;
...
public:
  ByteArrayClearTask(ByteArray *ba) : ByteArrayTask(ba) {}
...
//
// this task ensures that EnsureCapacity-write operation will be performed in a
// safepoint.
//
class ByteArrayEnsureCapacityAndWriteTask : public ByteArrayTask {
public:
  ByteArrayEnsureCapacityAndWriteTask(ByteArray *ba, uint32_t capacity,
                                      const void *buffer, uint32_t count)
      : ByteArrayTask(ba), m_capacity(capacity), m_buffer(buffer),
        m_count(count) {}

  void run() {
    // safepoints cannot survive exceptions
    TRY(m_core, kCatchAction_Rethrow) {
      m_byteArray->UnprotectedEnsureCapacityAndWrite(m_capacity, m_buffer,
                                                     m_count);
      // notify all subscribers to this byte array data if it is being
      // shared between workers
      ByteArray::UpdateSubscribers();
...
public:
  ByteArraySwapBufferTask(ByteArray *ba, ByteArray::Buffer *dest)
      : ByteArrayTask(ba), m_destination(dest) {}

  void run() {
    ByteArray::Buffer *src = m_byteArray->getUnderlyingBuffer();

    AvmAssert((src != NULL) && (m_destination != NULL));
    m_destination->initialize(src->getArray(), src->getCapacity(),
                              src->getLength(), src->getCopyOnWrite());
...
uint8_t *FASTCALL ByteArray::GetWritableBuffer() {
  ...
  AvmAssert(!m_buffer->getCopyOnWrite());
  // But in case something goes crazy
  if (m_buffer->getCopyOnWrite()) {
    // Should never reach here
    EnsureCapacity(m_buffer->getCapacity());
  }

  return m_buffer->getArray();
}

uint8_t &ByteArray::operator[](uint32_t index) {
  if (index >= m_buffer->getLength()) {
  ...
  return m_buffer->getArray()[index];
}
...
void ByteArray::SetLengthCommon(uint32_t newLength,
                                bool calledFromLengthSetter) {
  if (IsSharedAndWorkerStarted()) {
...
void ByteArray::UnprotectedSetLengthCommon(uint32_t newLength,
                                           bool calledFromLengthSetter) {
  if (m_subscribers.length() > 0 &&
      newLength < DomainEnv::GLOBAL_MEMORY_MIN_SIZE) {
    m_toplevel->throwRangeError(kInvalidRangeError);
  }
...
void ByteArray::Grower::SetLengthCommon(uint32_t newLength,
                                        bool calledFromLengthSetter) {
  if (!calledFromLengthSetter ||
      (newLength < kHugeGrowthThreshold &&
       m_owner->m_buffer->getLength() < kHugeGrowthThreshold)) {
    if (newLength > m_owner->m_buffer->getCapacity()) {
    ...
    uint32_t newCap = ((newCapRoundedUp <= MAX_BYTEARRAY_STORE_LENGTH)
                           ? uint32_t(newCapRoundedUp)
                           : newLength);

    AvmAssert(newCap >= newLength);

    if (newCap != m_owner->m_buffer->getCapacity()) {
      ReallocBackingStore(newCap);
    }
  }

  m_owner->m_buffer->setLength(newLength);
...
void ByteArray::Read(void *buffer, uint32_t count) {
  CheckEOF(count);
  move_or_copy(buffer, m_buffer->getArray() + m_position, count);
  m_position += count;
}
...
void ByteArray::Write(const void *buffer, uint32_t count) {
  // Though m_length is limited to 2^32-8K by EnsureWritableCapacity() above,
  // m_position is not similarly limited - it can be set beyond length and can
  // be as large as 2^32-1. No callers to Write() check that m_position + count
  // does not overflow, so we check that here.  There's a question of whether
  // MemoryError or RangeError is the "best" error to throw; ReadByteArray
  // throws RangeError in the similar circumstance but as
  // EnsureWritableCapacity() always throws MemoryError, we choose that here.
  if (count >
      UINT32_T_MAX -
          m_position) // Do not rearrange, guards against 64-bit overflow
    ThrowMemoryError();

  uint32_t writeEnd = m_position + count;

  if (writeEnd > m_buffer->getCapacity() || m_buffer->getCopyOnWrite()) {
    // If the byte array is copy-on-write, we must break sharing with the
    // original data source.

    // The check for capacity is redundant but it prevents us from getting into
    // a safepoint task for operations which do not require reallocation of the
    // buffer. The duplicate check for this condition is in
    // RequestWillReallocBackingStore() after we reach EnsureWriteAbleCapacity()
    // when we are in the ByteArrayEnsureCapacityTask task.

    // https://watsonexp.corp.adobe.com/#bug=3936030
    // ByteArray::Write() reallocates the backing store through
    // EnsureCapacity(). According to the general rule any operation which
    // reallocates the backing store must be in a safepoint. But Write() cannot
    // be put in a safepoint because it might go against the public
    // documentation of how ByteArray writes work whe shared. However, one layer
    // down, EnsureCapacity() basically ensures whether we have enough capacity
    // or not and if not reallocates the backing store. Now EnsureCapacity()
    // here can be put in a safepoint which would prevent a shared ByteArray
    // from being written to at the same time. And we enter this safepoint task
    // when the required capacity is greater than the exisiting capacity or if
    // the buffer is copyOnWrite (ByteArray does not own the buffer is
    // question).

    EnsureCapacityAndWrite(writeEnd, buffer, count);
  } else {
    move_or_copy(m_buffer->getArray() + m_position, buffer, count);
  }
  m_position += count;
  if (m_buffer->getLength() < m_position)
    m_buffer->setLength(m_position);
}

void ByteArray::EnsureCapacityAndWrite(uint32_t capacity, const void *buffer,
                                       uint32_t count) {
  if (IsSharedAndWorkerStarted()) {
...
void ByteArray::UnprotectedEnsureCapacityAndWrite(uint32_t capacity,
                                                  const void *buffer,
                                                  uint32_t count) {
  Grower grower(this, capacity);
  grower.EnsureWritableCapacity();
  // In the case of Write(), it's legal to call Write() on your own buffer, so
  // if growth occurs, you must not discard the old buffer until copying takes
  // place.
  move_or_copy(m_buffer->getArray() + m_position, buffer, count);
}
...
void ByteArray::NotifySubscribers() {
  for (uint32_t i = 0, n = m_subscribers.length(); i < n; ++i) {
    ...
    if (subscriber) {
      subscriber->notifyGlobalMemoryChanged(m_buffer->getArray(),
                                            m_buffer->getLength());
    } else {
...
bool ByteArray::addSubscriber(DomainEnv *subscriber) {
  if (m_buffer->getLength() >= DomainEnv::GLOBAL_MEMORY_MIN_SIZE) {
...
uint64_t ByteArray::bytesUsed() const {
  ...
  return m_buffer ? (m_buffer->getCopyOnWrite() ? 0 : m_buffer->getCapacity())
                  : 0;
}
#endif

void ByteArray::TellGcNewBufferMemory(const uint8_t *buf,
...
void ByteArray::TellGcDeleteBufferMemory(const uint8_t *buf,
                                         uint32_t numberOfBytes) {
...
void ByteArray::RestoreOrigData(uint8_t *origData, bool origCopyOnWrite,
                                uint32_t origLen, uint32_t origCap,
                                uint32_t origPos,
                                MMgc::GCObject *origCopyOnWriteOwner) {
  m_buffer->initialize(origData, origCap, origLen, origCopyOnWrite);
  m_position = origPos;
...
void ByteArray::Compress(CompressionAlgorithm algorithm) {
  if (IsShared() || (m_subscribers.length() > 0)) {
    toplevel()->argumentErrorClass()->throwError(
        kAPICannotAcceptSharedByteArray);
  }
  ...
  if (origLen) {
    switch (algorithm) {
    case k_lzma:
      CompressViaLzma(origData, origCopyOnWrite, origLen, origCap, origPos,
                      origCopyOnWriteOwner);
      break;
    case k_zlib:
    default:
      CompressViaZlibVariant(algorithm, origData, origCopyOnWrite, origLen,
                             origCap, origPos, origCopyOnWriteOwner);
      break;
    }
  }
#if defined(VMCFG_TELEMETRY_SAMPLER) && defined(DEBUGGER)
...
void ByteArray::CompressViaLzma(uint8_t *origData, bool origCopyOnWrite,
                                uint32_t origLen, uint32_t origCap,
                                uint32_t origPos,
                                MMgc::GCObject *origCopyOnWriteOwner) {
  ...
  // since this worker is doing this op and cannot share it and it is not
  // already shared (e.g. placed in a MessaegChannel).
...
retry_compress:
  ...
  if (!ensured) {
    ...
    RestoreOrigData(origData, origCopyOnWrite, origLen, origCap, origPos,
                    origCopyOnWriteOwner);

    m_toplevel->core()->throwException(exn);
  }

  destOverlay = (struct lzma_compressed *)m_buffer->getArray();
  destLen = m_buffer->getCapacity() - lzmaHeaderSize;
  ...
  retcode = LzmaCompress(destOverlay->compressedPayload, &destLen, dataSnapshot,
                         origLen, destOverlay->lzmaProps, &lzmaPropsSize,
  ...
  switch (retcode) {
  case SZ_OK:
    if (destLen > (m_buffer->getCapacity() - lzmaHeaderSize)) {
    ...
    m_buffer->setLength(uint32_t(lzmaHeaderSize + destLen));
    break;
  ...
  if (origLen != m_buffer->getLength() || origData != m_buffer->getArray()) {
    NotifySubscribers();
  }

  if (origData && origData != m_buffer->getArray() && !origCopyOnWrite) {
    // Note that TellGcXXX always expects capacity, not (logical) length.
...
void ByteArray::CompressViaZlibVariant(CompressionAlgorithm algorithm,
                                       uint8_t *origData, bool origCopyOnWrite,
                                       uint32_t origLen, uint32_t origCap,
                                       uint32_t origPos,
                                       MMgc::GCObject *origCopyOnWriteOwner) {
  // UncompressViaLzma() and UncompressViaZlibVariant() overwrite a
  ...
  error = deflateInit2(&stream, Z_BEST_COMPRESSION, Z_DEFLATED,
                       algorithm == k_zlib ? MAX_WBITS : MAX_WINDOW_RAW_DEFLATE,
                       DEFAULT_MEMORY_USE, Z_DEFAULT_STRATEGY);
  ...
  stream.next_out = m_buffer->getArray();
  stream.avail_out = m_buffer->getCapacity();

  error = deflate(&stream, Z_FINISH);
  AvmAssert(error == Z_STREAM_END);

  m_buffer->setLength(stream.total_out);
  AvmAssert(m_buffer->getLength() <= m_buffer->getCapacity());
...
void ByteArray::Uncompress(CompressionAlgorithm algorithm) {
  ...
  if (origLen) { // empty buffer should give empty result
    switch (algorithm) {
    case k_lzma:
      UncompressViaLzma(origData, origCopyOnWrite, origLen, origCap, origPos,
                        origCopyOnWriteOwner);
      break;
    case k_zlib:
    default:
      UncompressViaZlibVariant(algorithm, origData, origCopyOnWrite, origLen,
                               origCap, origPos, origCopyOnWriteOwner);
      break;
    }
  }
...
void ByteArray::HandleErrorInUncompressFn(
    FixedHeapRef<Buffer> origBuffer, uint8_t *origData, bool origCopyOnWrite,
    uint32_t origLen, uint32_t origCap, uint32_t origPos,
    MMgc::GCObject *origCopyOnWriteOwner, const bool cShared) {
...
void ByteArray::UncompressViaLzma(uint8_t *origData, bool origCopyOnWrite,
                                  uint32_t origLen, uint32_t origCap,
                                  uint32_t origPos,
                                  MMgc::GCObject *origCopyOnWriteOwner) {
  // UncompressViaLzma() and UncompressViaZlibVariant() overwrite a
  ...
  if (!m_buffer->getArray() || m_buffer->getLength() < lzmaHeaderSize)
    return;
  ...
  Exception *exn;
  bool ensured = EnsureCapacityOrFail(unpackedLen, kCatchAction_Rethrow, &exn);
  if (!ensured) {
    // clean up when the EnsureCapacity call fails.
    if (cShared) {
      m_buffer = origBuffer;
  ...
  retcode = LzmaUncompress(m_buffer->getArray(), &destLen,
  ...
  case SZ_OK: // - OK
    ...
    m_buffer->setLength(uint32_t(destLen));
  ...
  error_cases:
    // In error cases:
    HandleErrorInUncompressFn(origBuffer, origData, origCopyOnWrite, origLen,
                              origCap, origPos, origCopyOnWriteOwner, cShared);
    break;
  }
}
...
void ByteArray::UncompressViaZlibVariant(CompressionAlgorithm algorithm,
                                         uint8_t *origData,
                                         bool origCopyOnWrite, uint32_t origLen,
                                         uint32_t origCap, uint32_t origPos,
                                         MMgc::GCObject *origCopyOnWriteOwner) {
  // UncompressViaLzma() and UncompressViaZlibVariant() overwrite a
  // compressed ByteArray with it's uncompressed counterparts.
...
// larger value for nbytes should not use this API.  The purpose of all these
// limits is to make the range check tractably small for inlining in jitted code
// without limiting the ByteArray size unreasonably.  (Requiring m_length < 2^31
// would lead to further optimizations but that seems unreasonably short.)
//
// Observe that ByteArray::Grower::ReallocBackingStore() implements the
// appropriate limit on m_capacity, and that is the only code that allocates
// memory for ByteArray. Everywhere else we ensure m_length <= m_capacity.

REALLY_INLINE uint8_t *ByteArray::requestBytesForShortRead(uint32_t nbytes) {
  AvmAssert(m_buffer->getLength() <= m_buffer->getCapacity());
  AvmAssert(m_buffer->getCapacity() <= MAX_BYTEARRAY_STORE_LENGTH);
  AvmAssert(nbytes > 0 && nbytes <= MAX_BYTEARRAY_SHORT_ACCESS_LENGTH);

  // m_position + nbytes does not overflow uint32_t in the second disjunct
  // because:
  //
  //   m_position < m_length <= m_capacity <= MAX_BYTEARRAY_STORE_LENGTH (by the
  //   first disjunct + global invariants) nbytes <=
  //   MAX_BYTEARRAY_SHORT_ACCESS_LENGTH                            (global
  //   invariant) MAX_BYTEARRAY_STORE_LENGTH + MAX_BYTEARRAY_SHORT_ACCESS_LENGTH
  //   < 2^32  (global invariant)

  uint32_t buflen = m_buffer->getLength();
  if (m_position >= buflen || m_position + nbytes > buflen)
    ThrowEOFError(); // Does not return
  uint8_t *b = m_buffer->getArray() + m_position;
  m_position += nbytes;
  return b;
}
...
REALLY_INLINE uint8_t *ByteArray::requestBytesForShortWrite(uint32_t nbytes) {
  ...
  // because:
  //
  //   m_position < m_length <= m_capacity <= MAX_BYTEARRAY_STORE_LENGTH     (by
  //   the first disjunct + global invariants) nbytes <=
  //   MAX_BYTEARRAY_SHORT_ACCESS_LENGTH                           (global
  //   invariant) MAX_BYTEARRAY_STORE_LENGTH + MAX_BYTEARRAY_SHORT_ACCESS_LENGTH
  //   < 2^32 (global invariant)
  ...
  if (m_position >= buflen || m_position + nbytes > buflen)
    SetLength(m_position,
              nbytes); // The addition would *not* be safe against overflow here

  AvmAssert(m_buffer->getLength() <= m_buffer->getCapacity());
  AvmAssert(m_buffer->getCapacity() <= MAX_BYTEARRAY_STORE_LENGTH);
  AvmAssert(m_buffer->getLength() >= nbytes &&
            m_position <= m_buffer->getLength() - nbytes);
...
int32_t ByteArray::CAS(uint32_t index, int32_t expected, int32_t next) {
  ...
  uint32_t buflen = m_buffer->getLength();
  if ((buflen < minLength) || (index > (buflen - minLength))) {
    m_toplevel->throwRangeError(kInvalidRangeError);
  }
  ...
  uint8_t *wordptr = &m_buffer->getArray()[index];
  return vmbase::AtomicOps::compareAndSwap32WithBarrierPrev(expected, next,
                                                            (int32_t *)wordptr);
}

bool ByteArray::isShareable() const { return m_isShareable; }
...
ByteArrayObject::ByteArrayObject(VTable *ivtable, ScriptObject *delegate)
  ...
  m_byteArray.SetObjectEncoding(
      (ObjectEncoding)cls->get_defaultObjectEncoding());
...
// Inspection of the object code (GCC 4.2) shows that the forced inlining of
// GetLength and operator[] lead to optimal code here: no redundant range check
// is being executed even if there is a redundant check in operator[].
...
String *ByteArrayObject::_toString() {
  ...
  if (len >= 3) {
    ...
    if ((c[0] == 0xef) && (c[1] == 0xbb) && (c[2] == 0xbf)) {
      ...
      c += 2;
      len = (len - 2) >> 1;
      return core->newStringEndianUTF16(/*littleEndian*/ false,
                                        (const wchar *)c, len);
    } else if ((c[0] == 0xff) && (c[1] == 0xfe)) {
      // UTF-16 little endian
      c += 2;
      len = (len - 2) >> 1;
      return core->newStringEndianUTF16(/*littleEndian*/ true, (const wchar *)c,
                                        len);
    }
  }
  ...
  // buggy behavior, where malformed UTF-8 sequences are stored as single
  // characters.
  return core->newStringUTF8((const char *)c, len, false);
}

Atom ByteArrayObject::readObject() { return m_byteArray.ReadObject(); }
...
REALLY_INLINE uint32_t ByteArrayObject::read16() {
  ...
  if (m_byteArray.GetEndian() ==
      m_byteArray.GetNativeEndian()) { // GetNativeEndian expands to a constant
...
int ByteArrayObject::readShort() { return (int)(int16_t)read16(); }

int ByteArrayObject::readUnsignedShort() { return (int)read16(); }
...
REALLY_INLINE uint32_t ByteArrayObject::read32() {
      ...
      m_byteArray.GetNativeEndian()) // GetNativeEndian expands to a constant
  {
    ...
    return ((uint32_t)b[0] << 24) | ((uint32_t)b[1] << 16) |
           ((uint32_t)b[2] << 8) | (uint32_t)b[3]; // read big-endian
#else
    return ((uint32_t)b[3] << 24) | ((uint32_t)b[2] << 16) |
           ((uint32_t)b[1] << 8) | (uint32_t)b[0]; // read little-endian
#endif
  } else {
...
int ByteArrayObject::readInt() { return (int)read32(); }

uint32_t ByteArrayObject::readUnsignedInt() { return read32(); }
...
REALLY_INLINE float ByteArrayObject::readFloatHelper(uint8_t *b) {
      ...
      m_byteArray.GetNativeEndian()) // GetNativeEndian expands to a constant
  {
    ...
    return *(float *)b;
#elif defined VMCFG_BIG_ENDIAN
    u.word = ((uint32_t)b[0] << 24) | ((uint32_t)b[1] << 16) |
             ((uint32_t)b[2] << 8) | (uint32_t)b[3]; // read big-endian
    return u.fval;
#else
    u.word = ((uint32_t)b[3] << 24) | ((uint32_t)b[2] << 16) |
             ((uint32_t)b[1] << 8) | (uint32_t)b[0]; // read little-endian
    return u.fval;
#endif
  } else {
...
double ByteArrayObject::readDouble() {
      ...
      m_byteArray.GetNativeEndian()) // GetNativeEndian expands to a constant
  {
    ...
    return *(double *)b;
#elif defined VMCFG_BIG_ENDIAN
    u.words[first] = ((uint32_t)b[0] << 24) | ((uint32_t)b[1] << 16) |
                     ((uint32_t)b[2] << 8) | (uint32_t)b[3]; // read
    u.words[second] = ((uint32_t)b[4] << 24) | ((uint32_t)b[5] << 16) |
                      ((uint32_t)b[6] << 8) | (uint32_t)b[7]; //   big-endian
    return u.dval;
#else
    u.words[first] = ((uint32_t)b[3] << 24) | ((uint32_t)b[2] << 16) |
                     ((uint32_t)b[1] << 8) | (uint32_t)b[0]; // read
    u.words[second] = ((uint32_t)b[7] << 24) | ((uint32_t)b[6] << 16) |
                      ((uint32_t)b[5] << 8) | (uint32_t)b[4]; //   little-endian
    return u.dval;
#endif
  } else {
#if defined VMCFG_UNALIGNED_INT_ACCESS && defined HAVE_BYTESWAP32
...
#elif defined VMCFG_BIG_ENDIAN
    u.words[first] = ((uint32_t)b[7] << 24) | ((uint32_t)b[6] << 16) |
                     ((uint32_t)b[5] << 8) | (uint32_t)b[4]; // read
    u.words[second] = ((uint32_t)b[3] << 24) | ((uint32_t)b[2] << 16) |
                      ((uint32_t)b[1] << 8) | (uint32_t)b[0]; //   little-endian
#else
    u.words[first] = ((uint32_t)b[4] << 24) | ((uint32_t)b[5] << 16) |
                     ((uint32_t)b[6] << 8) | (uint32_t)b[7]; // read
    u.words[second] = ((uint32_t)b[0] << 24) | ((uint32_t)b[1] << 16) |
...
void ByteArrayObject::writeInt(int value) { write32((uint32_t)value); }

void ByteArrayObject::writeUnsignedInt(uint32_t value) { write32(value); }
...
ByteArray::CompressionAlgorithm
ByteArrayObject::algorithmToEnum(String *algorithm) {
  Toplevel *toplevel = this->toplevel();
...
void ByteArrayObject::writeBytes(ByteArrayObject *bytes, uint32_t offset,
                                 uint32_t length) {
  ...
  m_byteArray.WriteByteArray(bytes->GetByteArray(), offset, length);
}

void ByteArrayObject::readBytes(ByteArrayObject *bytes, uint32_t offset,
                                uint32_t length) {
  toplevel()->checkNull(bytes, "bytes");
  ...
  m_byteArray.ReadByteArray(bytes->GetByteArray(), offset, length);
}
...
String *ByteArrayObject::readUTFBytes(uint32_t length) {
  ...
  const uint8_t *p = (const uint8_t *)m_byteArray.GetReadableBuffer() +
                     m_byteArray.GetPosition();
  ...
  String *result =
      toplevel()->core()->newStringUTF8((const char *)p, int(q - p));
...
Stringp ByteArrayObject::get_endian() {
  return (m_byteArray.GetEndian() == kBigEndian) ? core()->kbigEndian
                                                 : core()->klittleEndian;
}
...
int32_t ByteArrayObject::atomicCompareAndSwapIntAt(int32_t byteIndex,
                                                   int32_t expectedValue,
                                                   int32_t newValue) {
  return m_byteArray.CAS(byteIndex, expectedValue, newValue);
}

int32_t ByteArrayObject::atomicCompareAndSwapLength(int32_t expectedLength,
                                                    int32_t newLength) {
  // An extension to https://watsonexp.corp.adobe.com/#bug=3823565
  // When a Byte Array is used as Domain Memory, the length of the ByteArray
  // should never be less than 1024. Whereas normal use of ByteArrays can allow
  // smaller lengths.
  if (m_byteArray.m_subscribers.length() > 0 &&
      newLength < DomainEnv::GLOBAL_MEMORY_MIN_SIZE) {
    m_byteArray.m_toplevel->throwError(kInvalidRangeError);
  }

  if (m_byteArray.IsSharedAndWorkerStarted()) {
    ByteArrayCompareAndSwapLengthTask task(&m_byteArray, expectedLength,
                                           newLength);
    task.exec();
    return task.result;
  } else {
    return m_byteArray.UnprotectedAtomicCompareAndSwapLength(expectedLength,
                                                             newLength);
  }
}

int32_t ByteArray::UnprotectedAtomicCompareAndSwapLength(int32_t expectedLength,
                                                         int32_t newLength) {
...
void ByteArrayObject::set_shareable(bool val) {
  // https://watsonexp.corp.adobe.com/#bug=4063414
  // Loader.loadBytes does not allow shared ByteArrays, but it is possible
  // to bypass the check by initially having a non-shared bytearray but
  // then having the 'shareable' property set to true in a event handler
  // when an event is dispatched in the middle of loadBytes.

  if (m_byteArray.isShareablePropReadOnly())
    toplevel()->argumentErrorClass()->throwError(
        kAPICannotAcceptSharedByteArray);
  else
...
bool ByteArrayObject::get_shareable() { return m_byteArray.isShareable(); }
...
ChannelItem *ByteArrayObject::makeChannelItem() {
  ...
  public:
    ...
    Atom getAtom(Toplevel *toplevel) const {
      ByteArrayObject *baObject =
          toplevel->getInternedObject(m_value).staticCast<ByteArrayObject>();
      if (baObject == NULL) {
        ByteArrayClass *baClass = toplevel->byteArrayClass();
        ByteArray ba(toplevel, m_value, m_isShareable);
        baObject = new (toplevel->gc(), MMgc::kExact)
            ByteArrayObject(baClass->ivtable(), baClass->prototypePtr(), ba);
  ...
  if (!cIsShareable) {
    ...
    if (array) {
      ...
      if (newArray) {
        GetByteArray().TellGcNewBufferMemory(newArray, capacity);
    ...
    // If the byte array is copy-on-write, we must break sharing with the
    // original data source. We cannot otherwise guarantee the lifetime of the
    // underlying array since the receiving Isolate cannot hold a copy of the
    // m_copyOnWriteOwner member of the sender.
    uint8_t *array = buffer->getArray();
    uint32_t capacity = buffer->getCapacity();
    if (array && buffer->getCopyOnWrite()) {
      uint8_t *newArray = ByteArrayBuffer_alloc_CanFailAndZero(capacity);
      if (newArray) {
        GetByteArray().TellGcNewBufferMemory(array, capacity);
        VMPI_memcpy(newArray, array, buffer->getLength());
        buffer->setArray(newArray);
      }
    }
    buffer->setCopyOnWrite(false);
...
ByteArrayClass::ByteArrayClass(VTable *vtable) : ClassClosure(vtable) {
