...
public:
  ...
  virtual LIns *operator()(enum BuiltinType, LIns *, LIns *) const {
    return TODO("operator()(enum,LIns,LIns) NOT IMPLEMENTED");
  }
  virtual LIns *operator()(enum BuiltinType, LIns *) const {
    return TODO("operator()(enum,LIns) NOT IMPLEMENTED");
  }
  LIns *TODO(const char *msg) const {
    AvmAssertMsg(false, msg);
    (void)msg;
    return _lh->InsConstAtom(AtomConstants::undefinedAtom);
  }
...
public:
  BasicLIREmitter(LirHelper *lh, LOpcode op, const CallInfo *call)
      : LIREmitter(lh), _dblOp(op), _call(call) {}
...
public:
  ...
  virtual LIns *operator()(enum BuiltinType, LIns *) const {
    return TODO("ModuloEmitter must have two operands");
  }
};

class IncrementLIREmitter : public LIREmitter {
public:
  IncrementLIREmitter(LirHelper *lh, bool shouldDecrement = false)
      : LIREmitter(lh), _shouldDecrement(shouldDecrement) {}
  virtual LIns *operator()(enum BuiltinType, LIns *, LIns *) const {
    return TODO("IncrementLIREmitter can't handle two operands");
  }
  virtual LIns *operator()(enum BuiltinType bt, LIns *oper) const;
