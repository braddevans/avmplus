...
// runtime info associated with a pool
    ...
    *_pool,
    ...
    static AbcEnv *create(MMgc::GC *gc, PoolObject *builtinPool,
    ...
    DomainEnv *const
            GC_POINTER(m_domainEnv); // Same as m_codeContext->domainEnv(); replicated
    // here solely for efficiency in jitted code
    CodeContext *const GC_POINTER(m_codeContext);
#ifdef DEBUGGER
    GCMember<U64Array>
        m_invocationCounts; // Actual size will hold pool->methodCount methods,
                            // only allocated if debugger exists.
#endif
...
#endif
    MethodEnv *GC_POINTERS(
            m_methods[1],
            "m_pool ? m_pool->methodCount() : 0"); // actual size will hold
    // pool->methodCount methods.
    // Observe m_pool can be null
