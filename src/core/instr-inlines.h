...
#define AssertNotNull(a) AvmAssert(!AvmCore::isNullOrUndefined(a))
    ...
    REALLY_INLINE
            Atom
    call_method_binding(Atom
    base,
    VTable *vtable, Binding
    b,
    int argc,
            Atom
    *atomv) {
...
return
op_call(env, method, argc, atomv
);
}

REALLY_INLINE
        Atom
call_get_binding(Atom
base,
VTable *vtable, Binding
b) {
...
REALLY_INLINE
        Atom
call_obj_dynamic(Atom
base,
const Multiname *name,
int argc, Atom
*atomv) {
return
AvmCore::atomToScriptObject(base)
->
callProperty(name, argc, atomv
);
}
...
*atomv) {
if (
AvmCore::isObject(func)
)
return
AvmCore::atomToScriptObject(func)
->
call(argc, atomv
);
return
...
*atomv) {
if (
AvmCore::isObject(ctor)
)
return
AvmCore::atomToScriptObject(ctor)
->
construct(argc, atomv
);
...
return
getBinding(env, vtable
->traits, ref);
}

template<class E>
REALLY_INLINE ArrayObject
*
newarray(E
caller_env,
int argc, Atom
...
return
constructprop(toplevel, multiname, argc, atomv,
...
if (
AvmCore::atomDoesNotNeedCoerce(atom, BuiltinType(expected->builtinType)
))
return
atom;
