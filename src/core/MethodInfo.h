...
namespace avmplus {
    ...
    public:
        ...
        REALLY_INLINE void gcTrace(MMgc::GC *gc) {
            gc->TraceLocation(&_scopeOrTraits);
        }
    };
    ...
    private:
      explicit DebuggerMethodInfo(int32_t _local_count, uint32_t _codeSize,
                                  int32_t _max_scopes);

    public:
      static DebuggerMethodInfo *create(AvmCore *core, int32_t _local_count,
                                        uint32_t _codeSize, int32_t _max_scopes);

      GC_DATA_BEGIN(DebuggerMethodInfo)

      GCMember<AbcFile>
          GC_POINTER(file);      // the abc file from which this method came
      ...
      const int32_t local_count; // FIXME: merge with localCount above; this one may
                                 // be visible to a debugger?
      const int32_t
          max_scopes; // FIXME: merge with maxScopeDepth above; this one is not used
                      // by the VM but may be visible to a debugger?
      Stringp GC_POINTERS_SMALL(
          localNames[1],
          local_count); // array of names for args and locals in framep order,
                        // written with explicit WBRC (actually local_count)
 ...
 */
    ...
    // special ctor for the methodinfo generated for slot-initialization when no
    // init method is present
    MethodInfo(InitMethodStub, Traits
    *declTraits);
    ...
    public:
    REALLY_INLINE static MethodInfo
    *
    create(MMgc::GC
    *gc,
    int32_t _method_id,
    ...
    *pool,
    ...
    return new (gc, MMgc::kExact)
    MethodInfo(_method_id, pool, abc_info_pos, abcFlags, native_info
    );
}
...
*declTraits) {
return new (gc, MMgc::kExact)
MethodInfo(stub, declTraits
);
}

#ifdef VMCFG_AOT
REALLY_INLINE static MethodInfo *create(MMgc::GC *gc, InitMethodStub stub,
                                        Traits *declTraits,
                                        const NativeMethodInfo *native_info,
                                        const AvmThunkNativeHandler &handler,
                                        int32_t method_id) {
  return new (gc, MMgc::kExact)
      MethodInfo(stub, declTraits, native_info, handler, method_id);
}
#endif
...
public:
  ...
  void unboxOneLocal(Atom src, FramePtr dst, int32_t dstPos,
                     const uint8_t *sstArr);

  void boxLocals(FramePtr src, int32_t srcPos, const uint8_t *sstArr,
                 Atom *dest, int32_t destPos, int32_t length);
  void unboxLocals(const Atom *src, int32_t srcPos, const uint8_t *sstArr,
                   FramePtr dest, int32_t destPos, int32_t length);
...
// Halfmoon AOT need to propogate a flag in method hierarchy to indicate that
// method signature required argc, argv
...
getMethodName(bool includeAllNamespaces = false) const;
...
int32_t varShift(); // effectively, log2(varSize), i.e. 3 or 4. See exec.h for
                    // details.
#endif                // VMCFG_FLOAT

/**
 * Determine whether a function is trivial.  A function is (conservatively)
 * trivial if all these conditions hold:
 *
 ...
 *
 * Stronger definitions of triviality are possible - for example, the
 * condition on returning 'undefined' is a hack to avoid dealing with data
 * flow analysis and determining whether RETURNVALUE will throw an exception -
 * and some functions that are trivial by the above definition may still not
 * be recognized by the implementation, obviously.  The only condition is that
 * if isTrivial returns true then the function is trivial by the above
 * definition.
 */
bool isTrivial();
...
_buildMethodSignature(const Toplevel *toplevel);
...
struct AbcInfo {
    ...
    ExceptionHandlerTable *exceptions; // we write this once, in Verifier, with
    // an explicit WB.  so no GCMember<>.
    uint32_t countdown;                // Counts down calls plus backedges.
#ifdef VMCFG_WORDCODE
    struct {
      TranslatedCode
          *translated_code; // The object that contains the code pointed to by
                            // body_pos, written with explicit WB
      // We write this once, in WordcodeTranslator, with an explicit WB.  so no
      // GCMember<>. The contents are the same as the 'exceptions' structure
      // above, except the 'from', 'to', and 'target' fields.
...
{
    NativeInfo _native; // stuff used only for Native methods (formerly in
    // NativeMethod)
    AbcInfo
            _abc; // stuff used only for bytecode methods (formerly in MethodInfo)
};
...
uint32_t _needArguments
: 1;

// (Set in ABC) need activation object
uint32_t _needActivation
: 1;
...
uint32_t _isOverride
: 1;

uint32_t _isNonInterruptible
: 1;
uint32_t _unboxThis
: 1;
uint32_t _needsCodeContext
: 1;
uint32_t _needsDxns
: 1;
uint32_t _hasExceptions
: 1;
...
uint32_t _hasFailedJit
: 1;

// true if execution mechanism is the interpreter
uint32_t _isInterpImpl
: 1;

// see isTrivial() for definition of these two flags
uint32_t _isTrivial
: 1;
uint32_t _isNonTrivial
: 1;

// Set by setLazyRest when the verifier discovers that the method's rest array
// or arguments array can be allocated lazily (usually not at all).  Only
// makes sense in combination with _needRest or _needArguments.
uint32_t _lazyRest
: 1;
...
uint32_t _onlyUntypedParameters
: 1;
...
uint32_t _apply_fastpath
: 1;

// If true, execution policy permits OSR for this method.
// This does not guarantee that OSR will apply, however.  See
// OSR::isSupported() for details.
uint32_t _osr_enabled
: 1;

#ifdef VMCFG_FLOAT
// if true, the function operates on float4_t values, and needs a double
// VARSIZE
...
public:
REALLY_INLINE static MethodSignature
*
create(MMgc::GC
*gc,
size_t extra,
...
MethodSignature(param_count);
}

private:
MethodSignature(int32_t
param_count);
...
Traits *paramTraits(int32_t
i) const;
BuiltinType paramTraitsBT(int32_t
i) const;
Atom getDefaultValue(int32_t
i) const;
...
void boxArgs(AvmCore *core, int32_t argc, const uint32_t *ap,
             Atom *out) const;
...
int _rest_offset;    // offset to first rest arg, including the instance
// parameter. this is sum(sizeof(paramType(0..N)))
...
int _frame_size; // abc-only: total size of frame in number of Atoms, derived
// from other values above
// Note that these two flags are "bool" because we will have to pad the struct
// anyway; might as well avoid per-bit access as long as we have the space.
bool _isNative;       // dupe of owner's flag of same name.
bool _allowExtraArgs; // == _needRest | _needArguments | _ignoreRest
AtomOrType _args[1];  // lying, actually 1+param_count+optional_count.  Traced
// by gcTraceHook().
...
/**
 * A MethodRecognizer that performs an exact match comparision against a method
 * name
 */
...
/**
 * A MethodRecognizer that performs an Perl Regular Expression comparision
...
/**
 * A MethodRecognizer that performs compares a method's unique id against a
 * range of id's.
 */
