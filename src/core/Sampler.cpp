...
// Adobe patent application tracking #P721, entitled Application Profiling,
// inventors: T. Reilly
...
// sadly, declaring this "static" produces spurious warnings in some GCC
// versions
template <class T>
...
// aligns to an 8-byte boundary -- apparently assumes input is at least 4-byte
// aligned.
...
Sampler::Sampler(AvmCore *_core)
    : GCRoot(_core->GetGC()), IMemorySampler(), sampleIteratorVTable(NULL),
      slotIteratorVTable(NULL), core(_core),
      fakeMethodNames(_core->GetGC(), kListInitialCapacity), allocId(1),
      sampleBufferId(0), samples(NULL), currentSample(NULL),
      lastAllocSample(NULL), lastSampleCheckMicros(0), callback(NULL),
      timerHandle(0), uids(1024), ptrSamples(1024), takeSample(0),
      numSamples(0), samples_size(0), samplingNow(false),
      samplingAllAllocs(false), runningCallback(false),
      autoStartSampling(false), _sampling(true) {
...
void Sampler::sample() {
  ...
  if (takeSample) {
    if (core->callStack) {
      // We may want to write more than one sample.  E.g. if 5.5 milliseconds
      // have passed, we'll write 5 samples.
      int sampleCount = 0;
      if (lastSampleCheckMicros != 0)
        sampleCount =
            (int)((nowMicros - lastSampleCheckMicros) / sampleFrequencyMicros);
      if (sampleCount <= 0)
        sampleCount = 1;
      for (int sampleNum = sampleCount - 1; sampleNum >= 0; sampleNum--) {
        ...
        uint64_t sampleTimeMicros =
            nowMicros - (sampleNum * sampleFrequencyMicros);
  ...
  // Even if the callstack was empty, don't take another sample until the next
  // timer tick.
  takeSample = 0;

  // Don't just set lastSampleCheckMicros equal to nowMicros -- we want to keep
  // the sampling frequency as close to one per millisecond as we can.
  uint64_t elapsed = nowMicros - lastSampleCheckMicros;
  lastSampleCheckMicros +=
      (elapsed / sampleFrequencyMicros * sampleFrequencyMicros);
}
...
int Sampler::sampleSpaceCheck(bool callback_ok) {
  ...
  if (callback && callback_ok && !runningCallback &&
      currentSample + sampleSize + samples_size / 3 >
          (samples + samples_size) &&
      !core->GetGC()->Collecting() && !core->GetGC()->Reaping()) {
    runningCallback = true;
    pauseSampling();
    Atom args[1] = {nullObjectAtom};
    Atom ret = AtomConstants::falseAtom;
    TRY(core, kCatchAction_Ignore) { ret = callback->call(0, args); }
    CATCH(Exception * pExceptionToIgnore) { (void)pExceptionToIgnore; }
    END_CATCH
    END_TRY
...
void Sampler::writeRawSample(SampleType sampleType,
  ...
  if (sampleType != DELETED_OBJECT_SAMPLE) {
    // if(depth == 0)
    //  AvmDebugMsg(false, "Please add SAMPLE_FRAME's to give this allocation
    //  some context.");
    write(p, depth);
    while (csn) {
      VMPI_memset(p, 0, sizeof(StackTrace::Element));
      StackTrace::Element *e = (StackTrace::Element *)p;
      e->m_info = csn->isAS3Sample()
                      ? csn->info()
                      : (MethodInfo *)StackTrace::Element::EXTERNAL_CALL_FRAME;
...
void Sampler::readSample(uint8_t *&p, Sample &s) {
  ...
  if (s.sampleType != DELETED_OBJECT_SAMPLE) {
    ...
    AvmAssert(sizeof(StackTrace::Element) ==
              sizeof(MethodInfo *) + sizeof(Stringp) + sizeof(Stringp) +
                  sizeof(int32_t));
#else
  ...
  if (s.sampleType != Sampler::RAW_SAMPLE) {
    ...
    if (s.sampleType == Sampler::NEW_OBJECT_SAMPLE ||
        s.sampleType == Sampler::NEW_AUX_SAMPLE) {
      read(p, s.ptr);
      read(p, s.sot);
      read(p, s.alloc_size);

      if (s.ptr != NULL && sotGetKind(s.sot) != kSOT_Empty &&
          !GC::IsFinalized(s.ptr)) {
        ...
        // run and crash when trying to make the Sample object for the zombie
        // object. The Sampler will eventually be notified that the zombie is
        // deleted.
        s.ptr = NULL;
      }
...
void Sampler::recordNewObjectAllocation(AvmPlusScriptableObject *obj,
                                        avmplus::SamplerObjectType sot) {
  if (sampling())
    recordAllocationInfo(obj, sot);
}

uint64_t Sampler::recordAllocationSample(const void *item, uint64_t size,
                                         bool callback_ok, bool forceWrite) {
  AvmAssertMsg(sampling(), "How did we get here if sampling is disabled?");
  if (!samplingNow)
    return 0;
  ...
  AvmAssertMsg((uintptr_t)currentSample % 4 == 0,
               "Alignment should have occurred at end of raw sample.");
  numSamples++;

  return uid;
}

uint64_t Sampler::recordAllocationInfo(AvmPlusScriptableObject *obj,
  ...
  DomainEnv *domainEnv =
      core->codeContext() ? core->codeContext()->domainEnv() : NULL;
  Toplevel *toplevel = domainEnv ? domainEnv->toplevel() : NULL;
  sot = sotSetToplevel(sot, toplevel);

  AvmAssertMsg(s.sampleType == NEW_AUX_SAMPLE,
               "Sample stream corrupt - can only add info to an AUX sample.");
  AvmAssertMsg(s.ptr == (void *)obj,
               "Sample stream corrupt - last sample is not for same object.");
  ...
  AvmAssertMsg(ptrSamples.get(obj) == 0,
               "Missing dealloc sample - same memory alloc'ed twice.");
...
void Sampler::recordDeallocationSample(const void *item, uint64_t size) {
  ...
  // If we didn't find a UID then this wasn't memory that the sampler knew was
  // allocated
  ...
  if (samples) {
    ...
    if ((oldptr = (uint8_t *)ptrSamples.get(item)) != 0) {
      ...
      AvmAssertMsg(
          oldval == item,
          "Sample stream corrupt, dealloc doesn't point to correct address");
...
void Sampler::startSampling() {
  ...
  if (timerHandle == 0)
    timerHandle =
        OSDep::startIntWriteTimer(SAMPLE_FREQUENCY_MILLIS, &takeSample);
}
...
void Sampler::sampleInternalAllocs(bool b) { samplingAllAllocs = b; }
...
void Sampler::createFakeFunction(const char *name) {
  ...
  // save it in fakeMethodNames just to be sure it isn't cleared from the
  // intern-name list.
...
Stringp Sampler::getFakeFunctionName(const char *name) {
  // this can't make any allocations, it's called from sensitive areas (like
  // from the GC marking routines).  For one we'll recurse but also GC state can
  // get messed up if for instance the allocation triggers a collection
...
void Sampler::presweep() {
  ...
  for (uint32_t i = 0; i < num; i++) {
    ...
    if (s.sampleType == NEW_OBJECT_SAMPLE) {
      // keep all weak refs and type's live, in postsweep we'll erase our weak
      // refs to objects that were finalized.  we can't nuke them here b/c
      // pushing the types could cause currently unmarked things to become live
      void *ptr = sotGetGCPointer(s.sot);
      if (ptr != NULL && !GC::GetMark(ptr)) {
        // NOTE that Push_GCObject_MayFail can fail due to mark stack overflow
        // in tight memory situations.  It seems hard to compensate for it here,
        // so we don't.  The most credible workaround is likely to test the
        // overflow flag at the end of presweep and disable the sampler if it is
        // set.
