...
// helper method
// sets index to the uint32_t value of name, if it can be converted to a vector
// index isNumber is set to true if name was a number (whether it was a uint32_t
// value or not)
VectorBaseObject::VectorIndexStatus
VectorBaseObject::getVectorIndex(Atom name, uint32_t &index) const {
  // This is implicitly asserted by the call to AvmCore::getIndexFromAtom()
  // below. Make it explicit here.
...
// Assumes that getVectorIndex() has been previously invoked with argument
// 'name', and returned with the 'isNumber' flag true.
bool VectorBaseObject::isNegativeVectorIndexAtom(Atom name) {
  AvmAssert(atomIsIntptr(name) || (AvmCore::isString(name) &&
                                   AvmCore::atomToString(name)->length() > 0));

  return (atomIsIntptr(name) ? atomGetIntptr(name) < 0
                             : AvmCore::atomToString(name)->charAt(0) == '-');
}

// Helper for _getDoubleProperty and _getNativedoubleProperty
// Called when vector index of type double is non-integral or negative, or too
// large to fit in an int32.
void VectorBaseObject::throwGetDoubleException(double d,
                                               uint32_t length) const {
  // For SWF11 and later, throw RangeError.
  // For SWF10 and earlier, throw ReferenceError if the value is not legal
  // ECMAscript Array index, which must be in the range 0..2^32-2. Otherwise,
  // throw RangeError.
  if ((double(uint32_t(d)) == d && uint32_t(d) != 0xffffffff) ||
      core()->currentBugCompatibility()->bugzilla456852b) {
    toplevel()->throwRangeError(kOutOfRangeError, core()->doubleToString(d),
                                core()->uintToString(length));
  } else {
    // NOTE: Backward compatibility for SWF10 and earlier requires that we
    // search the prototype chain.  Unfortunately, that might result in a value
    // of type other than the declared element type of the vector, as assumed
    // here, so we punt and just throw.
...
// Called when vector index of type double is non-integral or negative, or too
// large to fit in an int32.
void VectorBaseObject::throwSetDoubleException(double d,
                                               uint32_t length) const {
  // For SWF11 and later, throw RangeError.
  // For SWF10 and earlier, throw ReferenceError if the value is not legal
  // ECMAscript Array index, or one greater than the largest legal index, i.e.,
  // in the range 0..2^32-1.  Othewrwise, throw RangeError.
...
VectorClass::VectorClass(VTable *vtable) : ClassClosure(vtable) {
...
/*static*/ Stringp VectorClass::makeVectorClassName(AvmCore *core, Traits *t) {
  ...
  // Reason: both have 'CharLocation' class in private namespace.but there is
  // just one vector
  // for them. hence there is mistach resulting in setting one traits variable
  // and reading other.
  // Solution: Add param traits' Private NameSpace Index (in Pool) to vector
  // name.
  if (t->ns()->getType() == Namespace::NS_Private) {
    s = s->append(core->newConstantStringLatin1("_PrivateNS_"));
    s = s->append(
        core->string(core->intToAtom(t->getTraitsNameSpaceIndexInPool())));
  }
#endif
...
ClassClosure *VectorClass::getTypedVectorClass(ClassClosure *typeClass) {
  ...
  if (typeClass == NULL) {
    ...
    // if we have an object, we must have an itraits (otherwise the typearg is
    // not a Class)
    ...
    if ((result = typeDomain->getParameterizedType(typeClass)) == NULL) {
      ...
      vt->ivtable->createInstanceProc =
          ClassClosure::impossibleCreateInstanceProc;
      ObjectVectorClass *parameterizedVector =
          ObjectVectorClass::create(vt->gc(), vt);
      parameterizedVector->m_typeTraits =
          typeClass ? (Traits *)(typeClass->traits()->itraits) : NULL;
      ...
      parameterizedVector->setPrototypePtr(
          toplevel->objectVectorClass()->prototypePtr());
...
Atom VectorClass::applyTypeArgs(int argc, Atom *argv) {
  ...
  if (argc != 1) {
    ...
    toplevel->typeErrorClass()->throwError(
        kWrongTypeArgCountError, traits()->formatClassName(),
        core()->toErrorString(1), core()->toErrorString(argc));
  }
...
ObjectVectorObject *VectorClass::newVector(ClassClosure *type,
                                           uint32_t length) {
  ClassClosure *vc = getTypedVectorClass(type);
  Atom args[2] = {nullObjectAtom, core()->uintToAtom(length)};
  return (ObjectVectorObject *)AvmCore::atomToScriptObject(
      vc->construct(1, args));
}
...
IntVectorClass::IntVectorClass(VTable *vtable)
  ...
  // This is an ugly hack: Vector<> doesn't work properly with the ClassManifest
  // setup, because the name it returns for itself isn't the name listed for
  // finddef. To work around this (and maintain legacy internal behavior), we
  // pre-emptively enter the Vector classes into the table upon first creation.
  // (Arguably the name lookup issue should be fixed.)
  toplevel()->builtinClasses()->fillInClass(
      avmplus::NativeID::abcclass___AS3___vec_Vector_int, this);
...
UIntVectorClass::UIntVectorClass(VTable *vtable)
  ...
  // (Arguably the name lookup issue should be fixed.)
  toplevel()->builtinClasses()->fillInClass(
      avmplus::NativeID::abcclass___AS3___vec_Vector_uint, this);
...
DoubleVectorClass::DoubleVectorClass(VTable *vtable)
  ...
  // (Arguably the name lookup issue should be fixed.)
  toplevel()->builtinClasses()->fillInClass(
      avmplus::NativeID::abcclass___AS3___vec_Vector_double, this);
...
FloatVectorClass::FloatVectorClass(VTable *vtable)
  ...
  toplevel()->builtinClasses()->fillInClass(
      avmplus::NativeID::abcclass___AS3___vec_Vector_float, this);
...
Float4VectorClass::Float4VectorClass(VTable *vtable)
  ...
  toplevel()->builtinClasses()->fillInClass(
      avmplus::NativeID::abcclass___AS3___vec_Vector_float4, this);
...
ObjectVectorClass::ObjectVectorClass(VTable *vtable)
  ...
  // (Arguably the name lookup issue should be fixed.)
  toplevel()->builtinClasses()->fillInClass(
      avmplus::NativeID::abcclass___AS3___vec_Vector_object, this);
