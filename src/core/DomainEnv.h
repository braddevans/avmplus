...
// proper Atom<->GenericObject transmutation is in place for this map.
    ...
    REALLY_INLINE ScriptEnvMap(MMgc::GC *gc) { ht.initialize(gc); }

    public:
    REALLY_INLINE static ScriptEnvMap
    *
    create(MMgc::GC
    *gc) {
...
*mi) {
return (ScriptEnv *)
AvmCore::atomToGenericObject(
        ht
.

get (AvmCore::genericObjectToAtom(mi))
...
public:
static DomainEnv *newDomainEnv(AvmCore * core, Domain * domain,
                               DomainEnv * base);
virtual ~
...
// see note in newDomainEnv about why this is always valid, even if
// m_baseCount == 1
inline DomainEnv *base() const {
#ifdef __clang__
...
enum {
    ...
    // But using larger values allows us to collapse a lot of range checks in
    // the JIT
...
globalMemoryBase() const { return m_globalMemoryBase; }

REALLY_INLINE uint32_t

globalMemorySize() const { return m_globalMemorySize; }

// global memory object accessor
ByteArrayObject *get_globalMemory() const {
    return m_globalMemoryProviderObject;
}

bool set_globalMemory(ByteArrayObject *providerObject);

void notifyGlobalMemoryChanged(uint8_t * newBase, uint32_t
newSize);
...
GCMember <Domain>
        GC_POINTER(m_domain); // Domain associated with this DomainEnv
