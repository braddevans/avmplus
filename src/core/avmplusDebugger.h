...
public:
  typedef enum _TraceLevel {
    ...
    TRACE_METHODS_AND_LINES_WITH_ARGS =
        4 // method entry, arguments and line numbers
  } TraceLevel;
...
public:
   ...
   * --------------------------------------------------
   */
  // typedef void (*TraceCallback_i)( Stringp fileName, int linenum, Stringp
  // methodName, Stringp methodArgs );
...
public:
   ...
   */
  static Atom autoAtomKindAt(DebugFrame *frame, int autoIndex,
                             AutoVarKind kind);

  // The following return the native type and rely the client
  //  (i.e., most likely the debugger) to use the right variant.
  //
  static ScriptObject *autoVarAsObject(DebugFrame *frame, int index,
                                       AutoVarKind kind);
  static bool autoVarAsBoolean(DebugFrame *frame, int index, AutoVarKind kind);
  static Stringp autoVarAsString(DebugFrame *frame, int index,
                                 AutoVarKind kind);
  // returns double, cf AvmCore::number_d, ok for interactive use
  static double autoVarAsInteger(DebugFrame *frame, int localIndex,
                                 AutoVarKind kind);
  static double autoVarAsDouble(DebugFrame *frame, int localIndex,
                                AutoVarKind kind);
#ifdef VMCFG_FLOAT
  static float autoVarAsFloat(DebugFrame *frame, int localIndex,
                              AutoVarKind kind);
  static float4_t autoVarAsFloat4(DebugFrame *frame, int localIndex,
                                  AutoVarKind kind);
#endif
  static Stringp autoVarName(DebugStackFrame *frame, int index,
                             AutoVarKind kind);
...
protected:
  static MethodInfo *functionFor(SourceInfo *src, int line,
                                 DebugStackFrame *frame); // protected
...
protected:
  AvmCore *core;
  GCMember<String>
      GC_POINTER(abcname); // filename of the ABC or abc name from SWF
  GCMember<HeapHashtable>
      GC_POINTER(sourcemap); // maps filename to that file's index in "sources"
  GCList<SourceFile>
      GC_STRUCTURE(source); // all source files used in this abc file
