...
MMGC_STATIC_ASSERT(sizeof(GCFloat4) == sizeof(float4_t) &&
                   sizeof(GCFloat4) % 16 == 0);
...
MMGC_STATIC_ASSERT(sizeof(GCFloat) % 8 == 0 &&
                   sizeof(GCFloat) >= sizeof(float) &&
                   offsetof(GCFloat, value) == 0);
#endif

PoolObject::PoolObject(AvmCore *core, ScriptBuffer &sb, const uint8_t *startPos,
                       ApiVersion apiVersion)
    : core(core), cpool_int(core->GetGC(), 0), cpool_uint(core->GetGC(), 0),
      cpool_double(core->GetGC(), 0),
#ifdef VMCFG_FLOAT
      cpool_float(core->GetGC(), 0), cpool_float4(core->GetGC(), 0),
#endif
      cpool_ns(core->GetGC(), 0), cpool_ns_set(core->GetGC(), 0),
#ifdef VMCFG_HALFMOON
      cpool_const_double(core->GetGC(), 0),
      cpool_const_string(core->GetGC(), 0),
#endif
#ifndef AVMPLUS_64BIT
      cpool_int_atoms(core->GetGC(), 0), cpool_uint_atoms(core->GetGC(), 0),
#endif
      cpool_mn_offsets(core->GetGC(), 0), metadata_infos(core->GetGC(), 0),
      ...
      _code(sb.getImpl()), _abcStart(startPos), _abcStringStart(NULL),
      _abcStringEnd(NULL), _abcStrings(NULL), _classes(core->GetGC(), 0),
      _scripts(core->GetGC(), 0), _methods(core->GetGC(), 0)
#ifdef DEBUGGER
      ,
      _method_dmi(core->GetGC(), 0)
#endif
      ,
      _method_name_indices(core->GetGC(), 0), _apiVersion(apiVersion),
      _uniqueId(-1)
#ifdef VMCFG_AOT
      ,
      aotInfo(NULL), aotRoot(NULL)
#endif
...
void PoolObject::dynamicizeStrings() {
  if (!MMgc::GC::GetGC(this)->Destroying()) {
    ...
    for (uint32_t i = 1; i < constantStringCount; i++) {
      ...
      // any index could be empty, and makeDynamic doesn't work on zero-length
      // strings. since that call doesn't have easy access to an AvmCore, do the
      // check here. (s can be null in the obscure case of a fuzzed ABC)
      if (!s || s->isEmpty()) {
      ...
      s->makeDynamic(_abcStringStart,
                     uint32_t(_abcStringEnd - _abcStringStart));
    }
  }
}
...
void PoolObject::setupConstantStrings(uint32_t count) {
  ...
  _abcStrings = ConstantStringContainer::create(
      core->gc, (count - 1) * sizeof(ConstantStringData), this);
...
Stringp PoolObject::getString(int32_t index) const {
  ...
  if (dataP->abcPtr >= _abcStringStart && dataP->abcPtr < _abcStringEnd) {
    ...
    // strict=false for bug-compatibility with swfs with incorrect utf8 encoding
    // of strings
    Stringp s =
        core->internStringUTF8((const char *)dataP->abcPtr, len, true, false);
    s->Stick(); // FIXME - Bugzilla 596918:  The Stick() call is dodgy.
    dataP->abcPtr = NULL; // Important to clear it - what's there is not an
                          // RCObject*, don't let WBRC see it
...
Atom PoolObject::getLegalDefaultValue(const Toplevel *toplevel, uint32_t index,
                                      CPoolKind kind, Traits *t) {
...
#else
      // LIR relies on the return values from this being "sticky" so it can
      // insert them inline. that's true for everything but int/uints that
      // overflow, so special-case them.
  ...
  if (toplevel) {
    if (t) {
      toplevel->throwVerifyError(
          ...
          core->toErrorString(Multiname(t->ns(), t->name())));
    } else {
...
void PoolObject::resolveBindingNameNoCheck(uint32_t index, Multiname &m,
                                           const Toplevel *toplevel) const {
  // FIXME: should only assert and not throw, pending correctness verification,
  // see bug https://bugzilla.mozilla.org/show_bug.cgi?id=557541

  if (index == 0 || index >= cpool_mn_offsets.length()) {
    if (toplevel)
      toplevel->throwVerifyError(
          kCpoolIndexRangeError, core->toErrorString(index),
          core->toErrorString(cpool_mn_offsets.length()));
    AvmAssert(!"unhandled verify error");
  }
...
Traits *PoolObject::resolveTypeName(uint32_t index, const Toplevel *toplevel,
  ...
  // check contents is a multiname.  in the cpool, and type system, kObjectType
  // means multiname.
...
Traits *PoolObject::resolveParameterizedType(const Toplevel *toplevel,
                                             Traits *base,
                                             Traits *param_traits) const {
  Traits *r = NULL;
  if (base == core->traits.vector_itraits) {
    ...
    default: {
      ...
      r = core->domainMgr()->findTraitsInPoolByNameAndNS(pool, fullname,
                                                         base->ns());
      if (!r) {
        r = core->traits.vectorobj_itraits->newParameterizedITraits(
            fullname, base->ns(), param_traits);
...
void PoolObject::initPrecomputedMultinames() {
  if (precompNames == NULL) {
    ...
    precompNames = ExactStructContainer<HeapMultiname>::create(
        gc, destroyPrecomputedMultinames, nNames);
...
void PoolObject::destroyPrecomputedMultinames(
    ExactStructContainer<HeapMultiname> *self) {
...
Stringp PoolObject::getMethodInfoName(uint32_t i) {
  ...
  if (core->config.methodNames &&
      (uint32_t(i) < uint32_t(this->_method_name_indices.length()))) {
    const int32_t index = this->_method_name_indices[i];
    if (index >= 0) {
      if (hasString(index))
        name = this->getString(index);
    } else {
      // Precomputed multinames may not be inited yet, but we'll need them
      // eventually, so go ahead and init them now
