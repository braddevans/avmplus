...
namespace avmplus {
StringClass::StringClass(VTable *cvtable) : ClassClosure(cvtable) {
...
// arg1 = argv[1]
// argN = argv[argc]
Atom StringClass::call(int argc, Atom *argv) { return construct(argc, argv); }
...
ArrayObject *StringClass::_match(Stringp in, Atom regexpAtom) {
  ...
  if (!AvmCore::istype(regexpAtom, core->traits.regexp_itraits)) {
    ...
    regexpAtom = core->newRegExp(toplevel()->regexpClass(),
                                 core->string(regexpAtom), core->kEmptyString)
                     ->atom();
  }
...
Stringp StringClass::_replace(Stringp subject, Atom pattern,
  ...
  if (AvmCore::istype(pattern, core->traits.regexp_itraits)) {
    ...
    if (replaceFunction) {
      ...
      // replacement string
      Atom argv[4] = {undefinedAtom, searchString->atom(),
                      core->uintToAtom(index), subject->atom()};
    ...
    out = String::concatStrings(
        out,
        subject->substring(index + searchString->length(), subject->length()));
    return out;
  }
}
...
ArrayObject *StringClass::_split(Stringp in, Atom delimAtom, uint32_t limit) {
  ...
  if (AvmCore::istype(delimAtom, core->traits.regexp_itraits)) {
    RegExpObject *reObj =
        (RegExpObject *)AvmCore::atomToScriptObject(delimAtom);
