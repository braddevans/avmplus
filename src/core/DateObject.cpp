...
double DateObject::AS3_valueOf() { return date.getTime(); }
...
double DateObject::_get(int index) { return date.getDateProperty(index); }
...
double DateObject::_set(int index, Atom *argv, int argc) {
  ...
  for (i = 0; i < argc; i++) {
    ...
    if (MathUtils::isNaN(
            num[j - 1])) // actually specifying NaN results in a NaN date. Don't
                         // pass Nan, however, because we use
  ...
  const int minTimeSetterIndex =
      4; // any setNames index >= 4 should call setTime() instead of setDate()
         //  setFullYear/setUTCFullYear/setMonth/setUTCMonth/setDay/setUTCDay
         //  are all in indices < 3
  if (index < minTimeSetterIndex) {
    date.setDate(num[0], num[1], num[2], utcFlag);
  } else {
    date.setTime(num[3], num[4], num[5], num[6], utcFlag);
  }
  return date.getTime();
}
