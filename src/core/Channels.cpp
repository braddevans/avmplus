...
MostlyNonBlockingChannel::MostlyNonBlockingChannel(MMgc::GC *gc)
    : eof(false), get_tries(0), put_tries(0), head(0), tail(0),
      fallback(NO_FB) {
...
bool MostlyNonBlockingChannel::isEmpty() { return (head == tail); }
...
MMgc::GC *MostlyNonBlockingChannel::gc() { return bufferData->GetGC(); }
...
/*virtual*/ bool MostlyNonBlockingChannel::BufferData::gcTrace(MMgc::GC *gc,
                                                               size_t cursor) {
  (void)cursor;
...
void MostlyNonBlockingChannel::createOverflowBuffer() {
  ...
  for (int i = 0; i < bufferData->length - 1; i++) {
    overflowBufferData->m_items[i] =
        bufferData->m_items[(fallback + i) % bufferData->length];
  }
...
NoSyncSingleItemBuffer::NoSyncSingleItemBuffer(MMgc::GC *gc)
    : m_hasData(false) {
  bufferData = new BufferData(gc, this);
}

NoSyncSingleItemBuffer::~NoSyncSingleItemBuffer() { delete bufferData; }

bool NoSyncSingleItemBuffer::isEmpty() { return !m_hasData; }

bool NoSyncSingleItemBuffer::isFull() { return m_hasData; }
...
MMgc::GC *NoSyncSingleItemBuffer::gc() { return bufferData->GetGC(); }

void NoSyncSingleItemBuffer::unregisterRoot() { bufferData->Destroy(); }

NoSyncSingleItemBuffer::BufferData::BufferData(MMgc::GC *gc,
                                               NoSyncSingleItemBuffer *buf)
    : GCRoot(gc, MMgc::kExact), m_item(0), buffer(buf) {}

/*virtual*/ bool NoSyncSingleItemBuffer::BufferData::gcTrace(MMgc::GC *gc,
                                                             size_t cursor) {
  (void)cursor;
  gc->TraceAtom(&m_item);
  return false;
...
bool NoSyncMultiItemBuffer::isEmpty() { return (head == tail) && !m_hasData; }

bool NoSyncMultiItemBuffer::isFull() { return (head == tail) && m_hasData; }
...
bool NoSyncMultiItemBuffer::put(const ChannelItem *in) {
  if (isFull()) {
    ...
    const ChannelItem **newBuf =
        mmfx_new_array_opt(const ChannelItem *, newLength, MMgc::kZero);
    uint32_t it = 0;
...
NonBlockingMultiItemBuffer::~NonBlockingMultiItemBuffer() { delete bufferData; }

bool NonBlockingMultiItemBuffer::isEmpty() { return (head == tail); }
...
MMgc::GC *NonBlockingMultiItemBuffer::gc() { return bufferData->GetGC(); }

void NonBlockingMultiItemBuffer::unregisterRoot() { bufferData->Destroy(); }

NonBlockingMultiItemBuffer::BufferData::BufferData(
    MMgc::GC *gc, NonBlockingMultiItemBuffer *buf)
...
/*virtual*/ bool
NonBlockingMultiItemBuffer::BufferData::gcTrace(MMgc::GC *gc, size_t cursor) {
  (void)cursor;
  for (int i = 0; i < BUF_LENGTH; i++) {
    gc->TraceAtom(&m_items[i]);
  }
