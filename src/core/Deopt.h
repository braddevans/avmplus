...
// these transient needs.
    ...
    public:
        ...
        // To deoptimize functions containing inlined calls, the context must
        // represent a stack of canonical (interpreter) frames, and we will need
        // methods to step through this stack, beginning with the innermost
        // invocation.

        // METHOD GLOBAL: These operations are always valid.

        // Given a pointer to the MethodFrame object for this activation, return the
        // native code frame base (FP).  This result is valid even if we have not yet
        // set a safepoint location.
        virtual uint8_t *frameBase(MethodFrame *methodFrame) = 0;

        // Given a native code frame base (FP) for this activation, return a pointer
        // to the MethodFrame object.  This result is valid even if we have not yet
        // set a safepoint location.
        virtual MethodFrame *methodFrame(uint8_t *frameBase) = 0;

        // Given a native code frame base (FP) for this activation, return the value
        // of the
        // '_save_eip' slot in the frame.
        // TODO: Should do something sensible if _save_eip was not defined for this
        // frame.
        ...
        // The location provided must be exactly the vpc for the safepoint
        // instruction.
        ...
        // If the current virtual pc is a call instruction, return the machine type of
        // the result of the call, else result is undefined.
        virtual SlotStorageType safepointReturnValueSST() = 0;

        // Restore the canonical interpreter frame from current context and interpret
        // from the current virtual pc to the end of the method.
        virtual Atom interpretFromSafepoint(uint8_t *fp, MethodEnv *env) = 0;

        // Valid only if the virtual pc of the current context is at a call
        // instruction. Like interpret, but adjust the operand stack to reflect the
        // completion of the call. Operands to the call are removed from the stack,
        // and the result is pushed onto it. Advances the virtual pc over the call
        // before resuming execution.
        virtual Atom interpretFromCallSafepoint(uint8_t *fp, MethodEnv *env,
                                                Atom returnVal) = 0;

        // Valid only if the virtual pc of the current context is at an instruction
        // that can throw, including a call instruction.  Restore the locals, but
        // clear the scope stack, and push a single value on the operand stack.
        // Interpret from the provided handler virtual pc to the end of the method.
        virtual Atom interpretFromThrowSafepoint(uint8_t *fp, MethodEnv *env,
                                                 int32_t handlerVpc, Atom exnVal) = 0;
    ...
    public:
        ...
        // for deoptimization, and arrange for the deoptimization of all earlier
        // activations as the stack unwinds.  The caller is responsible for changing
        // the execution strategy for subsequent invcations of the method.  Normally,
        // we should be called only from subclasses of ExecMgr.
        void deoptimize(AvmCore *core);

        // Enter the interpreter at the specified virtual pc within the method to
        // which the deoptimizer belongs, and in the given activation.  The virtual pc
        // must correspond to a safepoint, and the activation must be at that
        // safepoint.
        virtual void enterInterpreter(MethodFrame *frame, int32_t vpc) = 0;

        // Attempt to handle an exception within an activation of the method to which
        // this deoptimizer belongs.
        virtual void handleException(AvmCore *core, ExceptionFrame *ef,
                                     MethodFrame *handlerFrame) = 0;

        // Arm activation so that deoptTrameUponResult() will be invoked when it
        // returns.
        ...
        virtual void deoptFrameUponReturnWithResult(uint8_t *fp, MethodEnv *env,
                                                    void *resultPtr) = 0;
    ...
    protected:
        Deoptimizer(MethodInfo *info)
                : methodInfo(info), nextArmed(NULL), armedFrame(NULL), resumptionPc(0) {}

        // Shared code invoked by the virtual functions above.
        // Overrides simply allocate a context of the appriate type and call the
        // functions below.
        void enterInterpreter(DeoptContext *ctx, MethodFrame *frame, int32_t vpc);

        void handleException(DeoptContext *ctx, AvmCore *core, ExceptionFrame *ef,
                             MethodFrame *handlerFrame);

        void armForLazyDeoptimization(DeoptContext *ctx, MethodFrame *victim);

        void deoptFrameUponReturnWithResult(DeoptContext *ctx, uint8_t *fp,
                                            MethodEnv *env, void *resultPtr);

    private:
        // Internal helper functions.
        MethodFrame *nextActivation(MethodFrame *startFrame);

        static void returnFromFrameWithUnboxedValue(uint8_t *frameBase, Atom atom,
                                                    Traits *t);
...
// A concrete sublcass of Deoptimizer is based on an underlying type
// representing the raw deopt metadata, and a concrete subclass of DeoptContext
// that inteprets metadata in that format.  The argument to the constructor for
// the DeoptContext must match the type of the raw metadata.  The trampolines
// defined in this template allow us to avoid any further specialization of
// Deoptimizer itself.
    ...
    public:
        ...
        void handleException(AvmCore *core, ExceptionFrame *ef,
                             MethodFrame *handlerFrame) {
            DEOPTCONTEXT ctx(metaData);
        ...
        void deoptFrameUponReturnWithResult(uint8_t *fp, MethodEnv *env,
                                            void *resultPtr) {
            DEOPTCONTEXT ctx(metaData);
 ...
 */
    template<class T>
    T *addrInFrame(void *fp, int offset) {
