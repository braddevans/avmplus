...
ClassClosure::calcCreateInstanceProc(VTable *cvtable) {
...
/*static*/ CreateInstanceProc FASTCALL
ClassClosure::checkForRestrictedInheritance(VTable *ivtable,
  ...
  if (ivtable) {
    ...
    if (p == ArrayClass::createInstanceProc && !itraits->needsHashtable()) {
      ...
      // to see if instances of this class should behave as "semisealed"
      // instead.
    ...
    if (base != NULL && base->isRestrictedInheritance &&
        base->pool != itraits->pool) {
...
ClassClosure::ClassClosure(VTable *cvtable)
    : ScriptObject(cvtable, NULL),
      m_createInstanceProc(checkForRestrictedInheritance(
          cvtable->ivtable, cvtable->ivtable->createInstanceProc))
...
// to create any instances).
{
  ...
  cvtable->ivtable->createInstanceProc =
      ClassClosure::reinitNullPrototypeCreateInstanceProc;
...
ClassClosure::ClassClosure(VTable *cvtable,
    ...
    : ScriptObject(cvtable, NULL),
      m_createInstanceProc(
          checkForRestrictedInheritance(cvtable->ivtable, createInstanceProc))
// NB: prototype is null right now, but we expect our subclass to
// initialize it in their ctor (or, at a minimum, before it attempts
// to create any instances).
{
  ...
  // FunctionObject can legally have a null ivtable, and uses this ctor, so
  // check.
  VTable *const ivtable = cvtable->ivtable;
  if (ivtable != NULL) {
    ivtable->createInstanceProc =
        ClassClosure::reinitNullPrototypeCreateInstanceProc;
  }
  AvmAssert(m_createInstanceProc != reinitNullPrototypeCreateInstanceProc);
...
/*static*/ ClassClosure *FASTCALL
ClassClosure::createClassClosure(VTable *cvtable) {
  ClassClosure *cc = new (cvtable->gc(), MMgc::kExact, cvtable->getExtraSize())
      ClassClosure(cvtable, calcCreateInstanceProc(cvtable));
...
GCRef<ScriptObject> ClassClosure::constructObject() {
  ...
  // All of the autogenerated constructObject() stubs are guaranteed to never
  // return NULL, so let's honor that contract here too: in the unlikely event a
  // subclass returns NULL or a non-object Atom, we'll throw. (Generally, this
  // shouldn't ever happen, so we'll also assert.)
...
// This is used by construct="native" to special-case construction of some
// objects.
Atom ClassClosure::construct_native(CreateInstanceProc ciproc, int argc,
                                    Atom *argv) {
...
Atom ClassClosure::call(int argc, Atom *argv) {
  ...
  if (argc != 1) {
    toplevel->throwArgumentError(kCoerceArgumentCountError,
                                 toplevel->core()->toErrorString(argc));
  }
...
Stringp ClassClosure::implToString() const {
  ...
  Stringp s =
      core->concatStrings(core->newConstantStringLatin1("[class "), t->name());
  return core->concatStrings(s, core->newConstantStringLatin1("]"));
}

/*static*/
ScriptObject *FASTCALL
ClassClosure::reinitNullPrototypeCreateInstanceProc(ClassClosure *cls) {
  if (cls->m_prototype == NULL) {
    // ES3 spec, 13.2.2 (we've already ensured prototype is either an Object or
    // null)
    ScriptObject *prototype = AvmCore::atomToScriptObject(
        cls->toplevel()->objectClass->get_prototype());
...
ScriptObject *FASTCALL
ClassClosure::cantInstantiateCreateInstanceProc(ClassClosure *cls) {
  cls->throwCantInstantiateError();
  return NULL;
}

ScriptObject *FASTCALL
ClassClosure::impossibleCreateInstanceProc(ClassClosure *cls) {
...
void ClassClosure::throwError(int errorID, Stringp arg1, Stringp arg2,
                              Stringp arg3) {
