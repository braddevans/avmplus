...
namespace avmplus {
ObjectClass::ObjectClass(VTable *cvtable) : ClassClosure(cvtable) {
...
void ObjectClass::initPrototype() {
  ...
  toplevel->global()->setDelegate(
      prototypePtr()); // global.__proto__ = Object.prototype
  this->setDelegate(toplevel->classClass()
                        ->prototypePtr()); // Object.__proto__ = Class.prototype
}
...
/**
   ...
   false.
    4. Return true.
    NOTE Unlike [[HasProperty]] (section 8.6.2.4), this method does not consider
...
bool ObjectClass::_hasOwnProperty(Atom thisAtom, Stringp name) {
  ...
  case kObjectType: {
    ...
    // TODO
    // The change below is important as otherwise we will throw error in a call
    // to hasAtomProperty for ByteArrayObject. This gets us back to the
    // behaviour which we had in Marlin. A bugzilla bug [ 562224 ] has been
    // created to address this issue more cleanly in near future
    return obj->traits()->getTraitsBindings()->findBinding(
               name, core->findPublicNamespace()) != BIND_NONE ||
  ...
  return t->getTraitsBindings()->findBinding(
             name, core->findPublicNamespace()) != BIND_NONE;
}
...
/**
    ...
    When the isPrototypeOf method is called with argument V, the following steps
   are taken:
    1. Let O be this object.
    2. If V is not an object, return false.
    3. Let V be the value of the [[Prototype]] property of V.
    4. if V is null, return false
    5. If O and V refer to the same object or if they refer to objects joined to
...
Stringp ObjectClass::_toString(Atom thisAtom) {
  ...
  if (AvmCore::isObject(thisAtom)) {
    ...
    Stringp s = core->concatStrings(core->newConstantStringLatin1("[object "),
                                    t->name());
