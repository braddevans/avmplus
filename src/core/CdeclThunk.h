...
// TODO for x86-64, we probably want to split this into real 32-bit integer
// returning variant
...
// convert arguments to ap style argument block, returning "argc"
    int32_t argDescArgsToAp(void *args, uintptr_t argDesc, MethodEnv *env,
                            va_list_wrapper ap);

    int32_t argDescArgsToAp(void *args, char *argDesc, MethodEnv *env,
                            va_list_wrapper ap);
...
// convert arguments to AtomList
    void argDescArgsToAtomList(AtomList &args, uintptr_t argDesc, MethodEnv *env,
                               va_list_wrapper ap);

    void argDescArgsToAtomList(AtomList &args, char *argDesc, MethodEnv *env,
                               va_list_wrapper ap);
