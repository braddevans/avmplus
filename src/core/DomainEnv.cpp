...
DomainEnv::DomainEnv(AvmCore *core, Domain *domain, DomainEnv *base,
                     uint32_t baseCount)
    : m_scriptEnvMap(ScriptEnvMap::create(core->GetGC())), m_domain(domain),
      m_globalMemoryScratch(mmfx_new0(Scratch)),
      m_globalMemoryBase(m_globalMemoryScratch->scratch),
      m_globalMemorySize(GLOBAL_MEMORY_MIN_SIZE), m_baseCount(baseCount) {
...
DomainEnv *DomainEnv::newDomainEnv(AvmCore *core, Domain *domain,
                                   DomainEnv *base) {
  ...
  DomainEnv *domainEnv = new (core->GetGC(), MMgc::kExact, extra)
      DomainEnv(core, domain, base, baseCount);
...
bool DomainEnv::set_globalMemory(ByteArrayObject *providerObject) {
  if (!providerObject) {
    ...
    notifyGlobalMemoryChanged(m_globalMemoryScratch->scratch,
                              GLOBAL_MEMORY_MIN_SIZE);
  } else if (!globalMemorySubscribe(providerObject)) {
    // failed... do nothing
    return false;
  } else {
    // success on globalMemorySubscribe would have led to
    // notifyGlobalMemoryChanged success... unsubscribe from original, but only
    // if not the same as new provider
...
void DomainEnv::notifyGlobalMemoryChanged(uint8_t *newBase, uint32_t newSize) {
  ...
  TELEMETRY_UINT32(toplevel()->core()->getTelemetry(), ".mem.bytearray.alchemy",
                   m_globalMemorySize / 1024);
}
