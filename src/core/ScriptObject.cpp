...
#endif // DEBUGGER
      vtable(_vtable),
      // note that it's substantially more efficient to initialize this in the
      // ctor list vs. a later explicit call to setDelegate, as we don't have to
      // check for marking nor decrement an existing value...
      delegate(_delegate) {
...
void ScriptObject::initHashtable(
  ...
  AvmAssert(vtable->traits->isDictionary() ==
            0); // should not be called DictionaryObject uses HeapHashtable
...
InlineHashtable *ScriptObject::getTable() const {
  ...
  if (!vtable->traits->isDictionary()) {
    ...
    if (*hht == NULL) {
      ...
      // call the Dictionary ctor via super(), so our HeapHashtable was never
      // created. This is a horrible band-aid: lazily create it here. (Since the
      ...
      // sweeping rethinking of ScriptObject, so this will have to do
      // short-term.
      MMgc::GC *gc = this->gc();
    ...
    AvmAssertMsg(ihp != NULL,
                 "Illegal to call getTable before Dictionary init is called");
    return ihp;
  }
}
...
Atom ScriptObject::getAtomPropertyFromProtoChain(Atom name, ScriptObject *o,
                                                 Traits *origObjTraits) const {
  ...
  Multiname multiname(core()->getAnyPublicNamespace(),
                      AvmCore::atomToString(name));
...
void ScriptObject::throwWriteSealedError(Atom name) {
  ...
  throwWriteSealedError(
      Multiname(core->getAnyPublicNamespace(), core->intern(name)));
}

void ScriptObject::throwCantInstantiateError() {
  Multiname qname(traits()->ns(), traits()->name());
  toplevel()->argumentErrorClass()->throwError(kCantInstantiateError,
...
// arg1 = argv[1]
// argN = argv[argc]
Atom ScriptObject::callProperty(const Multiname *multiname, int argc,
                                Atom *argv) {
  Atom method = getMultinameProperty(multiname);
  if (!AvmCore::isObject(method))
    toplevel()->throwTypeError(kCallOfNonFunctionError,
...
Atom ScriptObject::getDescendants(const Multiname * /*name*/) const {
  toplevel()->throwTypeError(kDescendentsError,
...
PrintWriter &ScriptObject::print(PrintWriter &prw) const {
  (traits()->name() != NULL) ? prw << traits() : prw << "{}";
...
Atom ScriptObject::defaultValue() {
  ...
  toplevel->throwTypeError(kConvertToPrimitiveError,
                           core->toErrorString(traits()));
  return undefinedAtom;
}
...
Atom ScriptObject::getSlotAtom(uint32_t slot, AvmCore *core) {
  ...
  // repeated if-else is actually more performant than a switch statement in
  // this case. SST_atom is most common case, put it first
  void *p;
  ...
  if (sst == SST_atom) {
    ...
    return traits->core->float4ToAtom(
        AvmThunkUnbox_FLOAT4(float4_t, *((Atom *)p)));
  }
#endif // VMCFG_FLOAT
...
ScriptObject *ScriptObject::getSlotObject(uint32_t slot) {
  ...
  // based on profiling of Flex apps, it's *much* more common for the slot in
  // this case to have a type (vs "atom"), so check for that first...
  if (sst == SST_scriptobject) {
    return *((ScriptObject **)p);
  } else if (sst == SST_atom) {
    Atom const a = *((const Atom *)p);

    // don't call AvmCore::isObject(); it checks for null, which we don't care
    // about here
...
// of Toplevel::coerce. If you modify that code, you might need to modify this
// code.
void ScriptObject::coerceAndSetSlotAtom(uint32_t slot, Atom value) {
  ...
  // this case. SST_atom is most common case, put it first
  if (sst == SST_atom) {
    // no call to coerce() needed, since anything will fit here... with one
    // exception: BUILTIN_object needs to convert undefined->null (though
    // BUILTIN_any does not). it's cheaper to do that here than call out to
    // coerce().
    AvmAssert(td->getSlotTraits(slot) == NULL ||
              td->getSlotTraits(slot)->builtinType == BUILTIN_object);
    ...
    *((int32_t *)p) = AvmCore::boolean(value);
    ...
    if (AvmCore::isNullOrUndefined(value)) {
      ...
      value =
          (Atom)traits->core->string(value); // don't bother setting tag bits
    } else if (sst == SST_namespace) {
    ...
    {
      AvmAssert(sst == SST_scriptobject);
      if (atomKind(value) != kObjectType ||
          !AvmCore::atomToScriptObject(value)->traits()->subtypeof(
              td->getSlotTraits(slot)))
        goto failure;
    }
...
failure:
  toplevel()->throwTypeError(
      kCheckTypeFailedError, traits->core->atomToErrorString(value),
      traits->core->toErrorString(td->getSlotTraits(slot)));
  return;
}
...
Stringp ScriptObject::implToString() const {
  ...
  Stringp s =
      core->concatStrings(core->newConstantStringLatin1("[object "), t->name());
