...
private:
  ...
  double *dblPtr() { return &_d; }

  Atom dblAtom() { return kDoubleType | (Atom)dblPtr(); }

public:
  int kind() { return atomKind(_a); }
  ...
  double getDouble() { return AvmCore::number(_a); }
  int32_t getInt32() { return AvmCore::integer(_a); }
  uint32_t getUInt32() { return AvmCore::toUInt32(_a); }
  int32_t getBoolean() { return AvmCore::boolean(_a); }
...
public:
  MethodSigArgDescIter(MethodInfo *methInfo)
      : m_n(0), m_methInfo(methInfo),
        m_methSig(methInfo->getMethodSignature()) {}
  ...
  bool hasRest() { return m_methInfo->needRest() ? true : false; }

  bool needArguments() {
#ifdef VMCFG_HALFMOON_AOT_RUNTIME
    // For Halfmoon, we have one more flag to accomodate in arguments.
    return (m_methInfo->needArguments() || m_methInfo->needArgsArrInMethodSig())
               ? true
               : false;
#endif
  ...
  bool needOptionalArgs() { return true; }

  bool isVarArg() { return false; }
};

typedef struct _APType
    *APType; // type doesn't matter.. just don't clash w/ va_list
...
public:
  APArgDescIter(int argc, MethodInfo *mi)
      : MethodSigArgDescIter(mi), m_argc(argc) {
#if CDECL_VERBOSE
    if (argc >= 0 && !m_methSig->argcOk(argc)) {
      AvmCore *core = m_methInfo->pool()->core;

      core->console << "argc bad: ";
      m_methInfo->print(core->console) << " : " << argc << "\n";
    }
#endif
  ...
  bool hasRest() { return false; }

  bool needArguments() { return false; }

  bool needOptionalArgs() { return false; }
...
public:
  ImmArgDescIter(uintptr_t argDesc, AvmCore *core)
      : m_argDesc(argDesc), m_core(core) {}
  ...
  // end of arguments!
  Traits *nextType() { return argTraitsFromType(m_core, nextTypeKind()); }
...
public:
  PtrArgDescIter(void *argDesc, AvmCore *core)
      : m_p((unsigned char *)argDesc), m_bitBuf(0), m_bits(0), m_core(core) {}
...
// on ARM64, first 8 double words are v0-v7 floating point, then r0 -r7, then
// overflow registers with natural alignment. on x86-64, it will no doubt be
// more complicated...
class ArgDescLayout {
protected:
  void *m_dst;
...
public:
  ...
  ArgDescLayout(void *dst)
      :
        ...
        ,
        m_dst((void *)((uintptr_t)dst +
                       64)) // First 8 FP registers (v0-v7) always
        ,
        ...
        ,
        m_numIntReg(8), m_numFloatReg(8)
#elif defined(AVMPLUS_AMD64)
        m_dstFP(dst) // First 8 FP registers (v0-v7) always
        ,
        m_dst((void *)((uintptr_t)dst +
                       64)) // First 8 FP registers xmm0-xmm7 always
        ,
        m_minDst((void *)((uintptr_t)dst +
                          112)) // then (rdi,rsi,rdx,rcx,r8,r9) always
        ,
        m_fpParamCount(0) // count for first m_numFloatReg FP parameters
        ,
        m_paramCount(0) // count for first m_numIntReg non FP parameters
        ,
        m_numIntReg(6), m_numFloatReg(8)
#else
        m_dst(dst)
#ifdef AVMPLUS_ARM // 32 bit ARM
        ,
        m_minDst((void *)((uintptr_t)dst +
                          16)) // must make room for a1-a4(4*4) always
#endif
#endif
  {
#if defined(AVMPLUS_ARM) && !defined(AVMPLUS_64BIT)
  ...
  int32_t *int32Arg() {
    ...
    // First 8 Non FP params should go to generic registers x0-x7(8 byte).
    // memory starting from m_dst
  ...
  void **ptrArg() {
#if (defined(AVMPLUS_ARM) && defined(AVMPLUS_64BIT)) || defined(AVMPLUS_AMD64)
    // First 8 Non FP params should go to generic registers x0-x7. memory
    // starting from m_dst
    if (++m_paramCount <= m_numIntReg) {
...
// calculate the size required for the given set of argument types for a cdecl
// call including MethodEnv
template <class ARG_TYPE_ITER>
static int32_t argDescSize(ARG_TYPE_ITER calleeTypeIter, AvmCore *) {
  ...
  if (calleeTypeIter.needArguments() || calleeTypeIter.hasRest()) {
    ...
    // llCreateRestHelper/llCreateArgumentHelper and calling convention doesn't
    // accept ArrayObject* In GO, ArrayObject is created and send as parameter.
...
// calculate the size required for the given set of argument types passed "ap"
// style just figures out the size of the ap struct... doesn't include MethodEnv
template <class ARG_TYPE_ITER1, class ARG_TYPE_ITER2>
static int32_t apArgDescSize(ARG_TYPE_ITER1 callerTypeIter,
                             ARG_TYPE_ITER2 calleeTypeIter, AvmCore *core) {
  int32_t size = 0;
...
#if defined(VMCFG_HALFMOON_AOT_RUNTIME) &&                                     \
    defined(AVMPLUS_ARM) // Fixing HMAOT bug
typedef void *(*ArgCoercer)(void *callee, MethodEnv *env, Traits *retTraits,
                            uintptr_t callerArgDesc, void *callerAp,
                            void *calleeArgDescBuf);
extern "C" Atom coerce32CdeclShim(void *callee, unsigned calleeArgDescBufSize,
                                  ArgCoercer argCoercer, MethodEnv *env,
                                  Traits *retTraits, uintptr_t callerArgDesc,
                                  void *callerAp);
extern "C" double coerceNCdeclShim(void *callee, unsigned calleeArgDescBufSize,
                                   ArgCoercer argCoercer, MethodEnv *env,
                                   Traits *retTraits, uintptr_t callerArgDesc,
                                   void *callerAp);
...
// this stuff is hackery to help development -- gcc targets should have .s files
// for production
#ifdef AVMPLUS_AMD64
#define ASM_FUNC_BEGIN(R, N, A)                                                \
  typedef R(*N##_type) A;                                                      \
  static void *N##_container() {                                               \
    void *result;                                                              \
...
#define ASM_FUNC_END(N)                                                        \
  ...
  N##_type N = (N##_type)N##_container();
...
// callerArgDesc is an opaque description of the arguments to be found via
// callerArgDesc callerAp is the variadic args passed to
// coerce32CdeclImmArgDesc, et al calleeArgDescBuf is the argument buffer into
// which coerced arguments are writter returns a pointer to a function used for
// return value conversion or NULL is none is needed
typedef void *(*ArgCoercer)(void *callee, MethodEnv *env, Traits *retTraits,
                            uintptr_t callerArgDesc, void *callerAp,
                            void *calleeArgDescBuf);
...
// -- calleeArgDescBufSize is the amount of space to allocate for the coerced
// args (and possibly register alloc info for, say, x86/64)
// -- argCoercer is a callback that does the actual filling of the coerced
// argument buf
// -- env is the MethodEnv we're calling (used to get argument traits -- does
// not make callee redundant as env may have >1 impl)
// -- calleeArgDesc is an opaque description of the types of variadic arguments
// in callerAp
// -- callerAp represents the arguments to coerce
#ifndef FR_SDK_COCOA_TOUCH_SIMULATOR

ASM_FUNC_BEGIN(Atom, coerce32CdeclShim,
               (void *callee, unsigned calleeArgDescBufSize,
                ArgCoercer argCoercer, MethodEnv *env, Traits *retTraits,
                uintptr_t callerArgDesc, void *callerAp))
#ifdef AVMPLUS_ARM
__asm__("stmdb  sp!, {v1, v2, v3, v7, lr}");
__asm__("mov v7, sp");
// First four arguments are passed in registersa1-a4 and other are pushed in
// stack in reverse order. remember 5 registers are pushed in stack and after
// that sp is copied to v7
// a1 = callee
...
// Remember:First four arguments are passed in registersa1-a4 and other are
// pushed in stack in reverse order.
// a1 stays callee
...
__asm__("cmp v2, #0"); // maybe call returnCoercer -- a1 and a2 will be the
                       // double or a1 will be the 32 and a2 will be a dummy
...
__asm__("callq r12");         //  map args: argCoercer(callee, env, retTraits,
                              //  callerArgDesc, callerAp, callerArgDescBuf);
__asm__("movq r12, rax");     //  save result mapping func

// mov all the arguments from stack to the register -- 8 float, 6
// integer/pointer
...
ASM1(call[ebp + 16]) //  map args: argCoercer(callee, env, retTraits,
                     //  callerArgDesc, callerAp, callerArgDescBuf);
ASM2(add esp, 32)
...
ASM_FUNC_BEGIN(double, coerceNCdeclShim,
               (void *callee, unsigned calleeArgDescBufSize,
                ArgCoercer argCoercer, MethodEnv *env, Traits *retTraits,
                uintptr_t callerArgDesc, void *callerAp))
#ifdef _WIN32
...
// SDK target for simulator for x86 architecture failing bcoz of assembly code
// with new command line tools.
double coerceNCdeclShim(void *callee, unsigned calleeArgDescBufSize,
                        ArgCoercer argCoercer, MethodEnv *env,
                        Traits *retTraits, uintptr_t callerArgDesc,
                        void *callerAp) {
  return 0.0;
...
Atom coerce32CdeclShim(void *callee, unsigned calleeArgDescBufSize,
                       ArgCoercer argCoercer, MethodEnv *env, Traits *retTraits,
                       uintptr_t callerArgDesc, void *callerAp) {
  return 0;
}

Atom returnCoercerN(Traits *retTraits, MethodEnv *env) { return 0; }
double returnCoercerN32(Traits *retTraits, MethodEnv *env) { return 0; }
Atom returnCoercer32(Traits *retTraits, MethodEnv *env) { return 0; }

Atom returnCoercerNPop(Traits *retTraits, MethodEnv *env) { return 0; }
...
static int32_t arg32(APType &ap) {
  ...
  ap = (APType)((uintptr_t)ap + sizeof(Atom)); // ap is internally stored as
                                               // Atom
  return result;
}

static void *argPtr(va_list_wrapper &ap) { return va_arg(ap.m_list, void *); }
...
static double argN(va_list_wrapper &ap) { return va_arg(ap.m_list, double); }
...
template <class ARG_ITER>
static Atom coerceArgToAny(Toplevel *toplevel, ARG_ITER &ap, Traits *callerT) {
  AvmCore *core = toplevel->core();
  AvmValue v;
...
template <class ARG_ITER>
static void coerceArg(Toplevel *toplevel, ArgDescLayout &l, Traits *calleeT,
                      ARG_ITER &callerAp, Traits *callerT) {
  AvmCore *core = toplevel->core();

  if (calleeT == callerT &&
      calleeT != OBJECT_TYPE) // OBJECT_TYPE might be a naked ScriptObject...
                              // let AvmValue handle it
  {
    if (calleeT == NUMBER_TYPE)
      *l.doubleArg() = argN(callerAp);
    else if (calleeT == INT_TYPE || calleeT == UINT_TYPE ||
             calleeT == BOOLEAN_TYPE)
#if (defined(AVMPLUS_64BIT) && defined(AVMPLUS_ARM)) || defined(AVMPLUS_AMD64)
      // On ARM64 bit, only 4 bytes are overwritten,in some cases padding is
      // required so just clear next 4 bytes.
  ...
  // Handling cases where callerT != calleeT, for default arguments, callerT is
  // NULL
  else if (calleeT == INT_TYPE || calleeT == UINT_TYPE ||
           calleeT == BOOLEAN_TYPE) {
#if (defined(AVMPLUS_64BIT) && defined(AVMPLUS_ARM)) || defined(AVMPLUS_AMD64)
...
static void coerceArgAtom(Toplevel *toplevel, ArgDescLayout &l, Traits *calleeT,
                          Atom a) {
  APType ap = (APType)&a;
...
static void coerceArg(Toplevel *toplevel, AtomList &atoms, Traits *calleeT,
                      va_list_wrapper &callerAp, Traits *callerT) {
  AvmCore *core = toplevel->core();
...
static void coerceArgAtom(Toplevel *, AtomList &atoms, Traits * /*calleeT*/,
                          Atom a) {
  atoms.add(a);
}

// coerces a single argument and writes it to an "ap" style arg list
static void coerceArg(Toplevel *toplevel, APType &ap, Traits *calleeT,
                      va_list_wrapper &callerAp, Traits *callerT) {
  AvmCore *core = toplevel->core();
...
static void coerceArgAtom(Toplevel *toplevel, APType &ap, Traits *calleeT,
                          Atom a) {
...
template <class ARG_TYPE_ITER1, class ARG_ITER, class ARG_TYPE_ITER2,
          class ARG_WRITER>
static int32_t argCoerceLoop(MethodEnv *env, ARG_TYPE_ITER1 callerTypeIter,
                             ARG_ITER callerAp, ARG_TYPE_ITER2 calleeTypeIter,
                             ARG_WRITER &argDescWriter) {
  ...
  if (calleeTypeIter.needArguments()) {
    ...
    // In halfmoon, ArrayObject is created in function prologue by calling
    // llCreateArgumentHelper and 'this' is required to call
    // env->createArguments there. In GO, env->createArguments is called here
    // itself and ArrayObject is send as parameter.
#ifndef VMCFG_HALFMOON_AOT_RUNTIME
  ...
  if (calleeTypeIter.needOptionalArgs()) {
    ...
    while (argc < regArgs) // optional...
    {
      ...
      coerceArgAtom(toplevel, argDescWriter, calleeT,
                    ms->getDefaultValue(optNum++));
      argc++;
    }
  }
  ...
  if (calleeTypeIter.needArguments() || calleeTypeIter.hasRest()) {
    ...
    handleRest(toplevel, argDescWriter, argsOrRest,
               calleeTypeIter.needArguments());
  }
  return argc;
}

static void passBaseArgs(MethodEnv *env, va_list_wrapper &callerAp,
                         ArgDescLayout &l) {
  (void)callerAp;
  (void)l;
  (void)env;
...
static void passTailArgs(MethodEnv *env, va_list_wrapper &callerAp,
                         ArgDescLayout &l) {
  (void)callerAp;
  (void)l;
...
template <class ARG_TYPE_ITER, class AP_TYPE>
static void *argCoercer(void * /*callee*/, MethodEnv *env, Traits *callerRT,
                        ARG_TYPE_ITER callerTypeIter, AP_TYPE callerAp,
                        void *calleeArgDescBuf) {
...
#if CDECL_VERBOSE
  core->console << "argCoercer: ";
  info->print(core->console) << " ";
  if (callerRT)
...
static void *atomvArgDescCoercer(void *callee, MethodEnv *env,
                                 Traits *retTraits, uintptr_t callerArgDesc,
                                 void *callerAp, void *calleeArgDescBuf) {
  ...
  return argCoercer(callee, env, retTraits, callerTypeIter, (APType)callerAp,
                    calleeArgDescBuf);
}

// callerArgDesc is the number of args
static void *apArgDescCoercer(void *callee, MethodEnv *env, Traits *retTraits,
                              uintptr_t callerArgDesc, void *callerAp,
                              void *calleeArgDescBuf) {
...
static void *ptrArgDescCoercer(void *callee, MethodEnv *env, Traits *retTraits,
                               uintptr_t callerArgDesc, void *callerAp,
                               void *calleeArgDescBuf) {
  PtrArgDescIter callerTypeIter((void *)callerArgDesc, env->core());

  return argCoercer(callee, env, retTraits, callerTypeIter,
                    *(va_list_wrapper *)callerAp, calleeArgDescBuf);
}

// callerArgDesc is a value containing nybbles describing arg types
static void *immArgDescCoercer(void *callee, MethodEnv *env, Traits *retTraits,
                               uintptr_t callerArgDesc, void *callerAp,
                               void *calleeArgDescBuf) {
...
// calls "env" with supplied variadic arguments described by the "immediate"
// flavor of argument description in argDesc returns an int32_t value
Atom coerce32CdeclArgDescEnter(Traits *retTraits, uintptr_t argDesc,
                               MethodEnv *env, va_list_wrapper ap) {
  MethodInfo *info = env->method;
  Atom result =
      coerce32CdeclShim((void *)info->handler_function(), argDescSize(info),
                        immArgDescCoercer, env, retTraits, argDesc, &ap);
  return result;
}

// calls "env" with supplied variadic arguments described by the "pointer"
// flavor of argument description in argDesc returns an int32_t value
Atom coerce32CdeclArgDescEnter(Traits *retTraits, char *argDesc, MethodEnv *env,
                               va_list_wrapper ap) {
  MethodInfo *info = env->method;
  Atom result = coerce32CdeclShim((void *)info->handler_function(),
                                  argDescSize(info), ptrArgDescCoercer, env,
                                  retTraits, (uintptr_t)argDesc, &ap);
  return result;
}

Atom coerce32CdeclArgDescEnter(Traits *retTraits, MethodEnv *env, int argc,
                               Atom *argv) {
  MethodInfo *info = env->method;
  Atom result = coerce32CdeclShim((void *)info->handler_function(),
                                  argDescSize(info), atomvArgDescCoercer, env,
                                  retTraits, (uintptr_t)argc, (void *)argv);
  return result;
}
...
Atom coerce32CdeclArgDescEnter(Traits *retTraits, MethodEnv *env, int argc,
  ...
  MethodInfo *info = env->method;
  Atom result = coerce32CdeclShim((void *)info->handler_function(),
                                  argDescSize(info), apArgDescCoercer, env,
                                  retTraits, (uintptr_t)argc, (void *)ap);
  return result;
}

// calls "env" with supplied variadic arguments described by the "immediate"
// flavor of argument description in argDesc returns a double value
double coerceNCdeclArgDescEnter(uintptr_t argDesc, MethodEnv *env,
                                va_list_wrapper ap) {
  MethodInfo *info = env->method;
  AvmCore *core = env->core();

  double result =
      coerceNCdeclShim((void *)info->handler_function(), argDescSize(info),
...
// calls "env" with supplied variadic arguments described by the "pointer"
// flavor of argument description in argDesc returns a double value
double coerceNCdeclArgDescEnter(char *argDesc, MethodEnv *env,
                                va_list_wrapper ap) {
  MethodInfo *info = env->method;
  AvmCore *core = env->core();

  double result = coerceNCdeclShim((void *)info->handler_function(),
                                   argDescSize(info), ptrArgDescCoercer, env,
                                   NUMBER_TYPE, (uintptr_t)argDesc, &ap);
  return result;
}
...
double coerceNCdeclArgDescEnter(MethodEnv *env, int argc, Atom *argv) {
  ...
  double result = coerceNCdeclShim((void *)info->handler_function(),
                                   argDescSize(info), atomvArgDescCoercer, env,
                                   NUMBER_TYPE, (uintptr_t)argc, (void *)argv);
  return result;
}
...
double coerceNCdeclArgDescEnter(MethodEnv *env, int argc, uint32_t *ap) {
  ...
  double result = coerceNCdeclShim((void *)info->handler_function(),
                                   argDescSize(info), apArgDescCoercer, env,
                                   NUMBER_TYPE, (uintptr_t)argc, (void *)ap);
  return result;
}
...
int32_t argDescArgsToAp(void *calleeArgDescBuf, uintptr_t argDesc,
                        MethodEnv *env, va_list_wrapper ap) {
  APArgDescIter calleeTypeIter(-1, env->method);
  ImmArgDescIter callerTypeIter(argDesc, env->core());
...
int32_t argDescArgsToAp(void *calleeArgDescBuf, char *argDesc, MethodEnv *env,
                        va_list_wrapper ap) {
  APArgDescIter calleeTypeIter(-1, env->method);
  PtrArgDescIter callerTypeIter(argDesc, env->core());
  APType dst = (APType)calleeArgDescBuf;
...
void argDescArgsToAtomv(Atom *args, uintptr_t argDesc, MethodEnv *env,
                        va_list_wrapper ap) {
  AvmCore *core = env->core();
...
void argDescArgsToAtomv(Atom *args, char *argDesc, MethodEnv *env,
                        va_list_wrapper ap) {
  AvmCore *core = env->core();
...
void argDescArgsToAtomList(AtomList &dst, uintptr_t argDesc, MethodEnv *env,
                           va_list_wrapper ap) {
  AvmCore *core = env->core();
  ImmArgDescIter callerTypeIter(argDesc, core);
  AtomvArgDescIter calleeTypeIter(core);
...
void argDescArgsToAtomList(AtomList &dst, char *argDesc, MethodEnv *env,
                           va_list_wrapper ap) {
  AvmCore *core = env->core();
  PtrArgDescIter callerTypeIter(argDesc, core);
  AtomvArgDescIter calleeTypeIter(core);
