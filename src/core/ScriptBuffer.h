 ...
 */
    ...
    public:
      BasicScriptBufferImpl(size_t _size) : ScriptBufferImpl() {
        this->size = _size;
 ...
 */
    class ScriptBuffer {
    public:
        ScriptBuffer() { m_impl = NULL; }

        ScriptBuffer(ScriptBufferImpl *rep) { m_impl = rep; }

        ScriptBuffer(const ScriptBuffer &toCopy) { m_impl = toCopy.m_impl; }
        ...
        ScriptBufferImpl *getImpl() const { return m_impl; }

        operator ScriptBufferImpl *() const { return m_impl; }

        operator uint8_t *() const { return m_impl->getBuffer(); }

        size_t getSize() const { return m_impl->getSize(); }

        uint8_t *getBuffer() const { return m_impl->getBuffer(); }

        uint8_t operator[](int index) const { return (*m_impl)[index]; }

        uint8_t &operator[](int index) { return (*m_impl)[index]; }
