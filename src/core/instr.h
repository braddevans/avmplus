 ...
 */
    template<class E> NO_INLINE Atom
    op_call_error(E
    env);
 ...
 */
    template<class E> ScriptObject *toPrototype(Atom
    obj);
 ...
 */
    ...
    void FASTCALL
    mop_rangeCheckFailed(MethodEnv
    *env);
// load-and-sign-extend
    int32_t FASTCALL

    mop_lix8(const void *addr);

    int32_t FASTCALL

    mop_lix16(const void *addr);

// load-and-zero-extend
    int32_t FASTCALL

    mop_liz8(const void *addr);

    int32_t FASTCALL

    mop_liz16(const void *addr);

    int32_t FASTCALL

    mop_li32(const void *addr);

    double FASTCALL

    mop_lf32(const void *addr);

    double FASTCALL

    mop_lf64(const void *addr);

    void FASTCALL

    mop_si8(void *addr, int32_t value);

    void FASTCALL

    mop_si16(void *addr, int32_t value);

    void FASTCALL

    mop_si32(void *addr, int32_t value);
