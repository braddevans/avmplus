...
// When an AS3 method is called, the AS3 method will ensure that
// core->codeContext() will return its context. Note that CodeContext should not
// be instantiated directly (except in certain situations by AvmCore); client
// code should create a concrete subclass.
    ...
    protected:
        explicit CodeContext(DomainEnv *env, const BugCompatibility *bugCompatibility)
                : m_domainEnv(env), m_bugCompatibility(bugCompatibility) {
            AvmAssert(env != NULL);
        }
    ...
    public:
        ...
        *
        ...
        domainEnv() const { return m_domainEnv; }

        REALLY_INLINE const BugCompatibility
        *

        bugCompatibility() const {
            return m_bugCompatibility;
        }
