...
#define BUILTIN_TRAITS_(class_id)                                              \
  toplevel->core()                                                             \
      ->builtinPool->getClassTraits(avmplus::NativeID::abcclass_##class_id)    \
      ->itraits
...
void ObjectOutput::TableAdd(HeapHashtable *ht, Atom atom) {
  ...
  // just store the raw int value.  we encapsulate access to these tables so
  // it's okay that the keys are atoms and the values are raw ints.
...
void ObjectInput::SetObjectProperty(Atom objectAtom, Stringp name, Atom value){
    TRY(core(), kCatchAction_Ignore){Toplevel *toplevel = this->toplevel();
...
void AvmPlusObjectOutput::WriteAtom(Atom value) {
  ...
  if (AvmCore::isUndefined(value)) {
    ...
    // cn: Note we ignore XMLList type here, which is actually different from
    // the XML type.
    //     AMF is a published format, however, so we can't add a new datatype to
    //     correctly handle XMLList.  We'll write a generic Object type in that
    //     case (see default below), workaround is to wrap your XMLList value in
    //     an XML wrapper.
...
void AvmPlusObjectOutput::WriteXML(GCRef<ScriptObject> obj, bool isAS3XML) {
  ...
  String *str = NULL;
  // cn:  XML's toString() will skip the tags if the content is simple (i.e.
  // <f>hi</f>.toString() == "hi")
...
void AvmPlusObjectOutput::WriteDate(DateObject *obj) {
  ...
  // As the date is only a double, it's hard to re-use the surplus bits of the
  // reference.
  WriteReference(1);
...
void AvmPlusObjectOutput::WriteByteArray(GCRef<ByteArrayObject> obj) {
  ...
  //
  // NOTE: ByteArray::Write() uses
  // ByteArray::UnprotectedEnsureCapacityAndWrite(), which is effectively:
  //  {
  //      Grower grower(this, capacity);
  //      memcpy(new_ptr, old_ptr, size);
  //  }
  // where the memcpy() from old_ptr to new_ptr is safe because Grower()
  // allocates new_ptr, and ~Grower() deletes old_ptr, so both are valid while
  // the grower instance is in scope.
...
void AvmPlusObjectOutput::WriteArray(GCRef<ArrayObject> obj) {
  ...
  // holes, and emit that as a list, with any extra properties getting emitted
  // as a map. (Array used to have a "getDenseLength" call that would we'd use,
  // but that relied on the underlying representation, which has changed in the
  // name of efficiency; therefore we calculate it manually here. Note that this
  // could cause some arrays to be written in slightly different form (ie with
  // more name/value pairs), but that's OK, just suboptimal.)
  ...
  // Write an invalid reference. Repurpose the unused bits to store the number
  // of dense elements.
  WriteReference((len << 1) | 1);

  // When the entire Array is dense, the server will create a List.
  // When the Array has holes, or contains elements indexed with non-numeric
  // values, the server will create a Map.
  //
  // Therefore, it's easier for the server if we write out the non-dense portion
  // first.
  ...
  // Terminate the non-dense portion with an empty string (which is invalid as a
  // property name)
...
void AvmPlusObjectOutput::WriteDictionary(GCRef<DictionaryObject> obj) {
  ...
  {
    // 04apr14 pgrandma@adobe.com :
    // https://watsonexp.corp.adobe.com/#bug=3731340
    //
    // getSize() includes deleted items, but the below iteration skips deleted
    // items, which caused WriteDictionary() to write corrupted data.
    //
    // The fix is to count the number of items using the same iteration we'll
    // write them with.
  ...
  for (uint32_t c = h->next(0); c; c = h->next(c)) {
    ...
    if (atomIsIntptr(key)) {
      key = MathUtils::convertIntegerToStringRadix(
                core(), atomGetIntptr(key), 10, MathUtils::kTreatAsSigned)
                ->atom();
    }
    WriteAtom(key);
...
void AvmPlusObjectOutput::WriteScriptObject(GCRef<ScriptObject> obj) {
  ...
  if (ref >= 0) {
    // output already contains class info. Save a reference to previous
    // instance.
    ...
    WriteReference(3 | (externalizable ? 4 : 0) | (dynamic ? 8 : 0) |
                   (count << 4));
    info->Write(this);
  }

  if (externalizable) {
    // ClassInfo's constructor has already verified this, so we don't need to
    // repeat the error handling here
    AvmAssert(obj->traits()->containsInterface(
        BUILTIN_TRAITS_(flash_utils_IExternalizable)));
    ...
    GCRef<ObjectOutputObject> output =
        toplevel->builtinClasses()->get_ObjectOutputClass()->constructObject();
    output->m_out = this;
    argv[1] = output->atom();
    ...
    if (AvmCore::isMethodBinding(info->get_functionBinding())) {
      MethodEnv *method = obj->vtable->methods[AvmCore::bindingToMethodId(
    ...
    if (dynamic) {
      GCRef<ScriptObject> writer =
          toplevel->builtinClasses()->get_ObjectEncodingClass()->m_writer;
      ...
      if (!writer) {
        for (int index = 0; ((index = obj->nextNameIndex(index)) != 0);) {
          ...
          // Empty string is sentinel for end-of-data, so we CANNOT serialize
          // it.
        ...
        Atom propAtom = toplevel->getpropname(
            writer->atom(),
            core->internConstantStringLatin1("writeDynamicProperties"));
        // invoke ObjectEncoding.dynamicPropertyWriter

        // https://watsonexp.corp.adobe.com/#bug=4062733
        // Overriding writeDynamicProperties with something that is not a
        // function causes type confusion. The isObject() check here prevents
        // that.
        GCRef<ScriptObject> func = AvmCore::isObject(propAtom)
                                       ? AvmCore::atomToScriptObject(propAtom)
                                       : NULL;
        if (func) {
          GCRef<DynamicPropertyOutputObject> output =
              toplevel->builtinClasses()
...
Atom AvmPlusObjectInput::ReadAtom() {
  ...
  case kAvmMinusXmlAtomType: // Even XML minus atoms are read back as XML
                             // objects. However handleAdditionalType can
...
DateObject *AvmPlusObjectInput::ReadDate() {
  ...
  DateObject *obj = DateObject::create(dateClass->ivtable()->gc(), dateClass,
                                       Date(ReadDouble()));
  ObjectListAdd(obj);
...
String *AvmPlusObjectInput::ReadString() {
  ...
  if (len == 0) {
    // WriteString() special cases the empty string to avoid creating a
    // reference.
...
GCRef<DictionaryObject> AvmPlusObjectInput::ReadDictionary() {
  ...
  GCRef<DictionaryObject> obj =
      toplevel->builtinClasses()->get_DictionaryClass()->constructObject(
          weakKeys);
  ObjectListAdd(obj);
...
Atom AvmPlusObjectInput::ReadTypedVector(uint8_t type) {
  ...
  case kTypedVectorObject: {
    ...
    GCRef<ObjectVectorObject> v =
        toplevel->vectorClass()->newVector(closure, len);
    ObjectListAdd(v);
    v->set_fixed(fixed);
...
GCRef<ScriptObject>
AvmPlusObjectInput::ReadScriptObject(ClassClosure **closure) {
  uint32_t ref = ReadReference();
  ...
  if (info->externalizable()) {
    // ClassInfo::Read() has already verified that the class closure registered
    // for this class's alias implements IExternalizable, so there's no need to
    // repeat the error handling here.
    AvmAssert(obj->traits()->containsInterface(
        BUILTIN_TRAITS_(flash_utils_IExternalizable)));
    ...
    GCRef<ObjectInputObject> input =
        toplevel->builtinClasses()->get_ObjectInputClass()->constructObject();
    input->m_in = this;
    argv[1] = input->atom();
    ...
    // read in the value of each sealed property, and then assign it to the
    // object's member
...
GCRef<ArrayObject> AvmPlusObjectInput::ReadArray() {
  AvmCore *core = this->core(); // this->core() is necessary when there is a
                                // local variable called core.
...
bool ClassInfo::containsTransientMetadata(PoolObject *pool,
                                          const uint8_t *meta_pos) {
  AvmCore *core = m_toplevel->core();
  return meta_pos ? pool->hasMetadataName(
                        meta_pos, core->internConstantStringLatin1(kTransient))
                  : false;
}
...
bool ClassInfo::slotContainsTransientMetadata(Traits *traits, int index) {
  ...
  const uint8_t *pos =
      traits->getTraitsMetadata()->getSlotMetadataPos(index, pool);
...
bool ClassInfo::methodContainsTransientMetadata(Traits *traits, int index) {
  PoolObject *pool = NULL;
  const uint8_t *pos =
      traits->getTraitsMetadata()->getMethodMetadataPos(index, pool);
  return containsTransientMetadata(pool, pos);
}

bool ClassInfo::isSerializable(Traits *t, Namespace *ns, Binding b) {
  // ignore members that are not public and not compatible with the current
  // version
  if (!ns->isPublic() ||
      !m_toplevel->core()->isNamespaceVisibleToApiVersionFromCallStack(ns)) {
    return false;
  }
  ...
  if (AvmCore::isVarBinding(b)) {
    ...
    if (!AvmCore::hasGetterBinding(b) ||
        methodContainsTransientMetadata(t, AvmCore::bindingToGetterId(b))) {
      // must implement a non-transient get accessor
      return false;
    }

    if (!AvmCore::hasSetterBinding(b) ||
        methodContainsTransientMetadata(t, AvmCore::bindingToSetterId(b))) {
...
ClassInfo::ClassInfo(Toplevel *toplevel, Traits *t)
    : m_toplevel(toplevel), m_traits(t), m_dynamic(t->needsHashtable()),
...
void ClassInfo::Write(AvmPlusObjectOutput *output) {
  ...
  for (int i = 0; i < len; i++) {
    // write the name:String for each property (SealedPropCount() was encoded in
    // the invalid ref id
...
ClassInfo *ClassInfo::Read(Toplevel *toplevel, AvmPlusObjectInput *input,
                           bool dynamic, bool externalizable, int count) {
  AvmCore *core = toplevel->core();
  ...
  if (externalizable) {
    if (!info->m_closure->traits()->itraits->containsInterface(
            BUILTIN_TRAITS_(flash_utils_IExternalizable))) {
      // object was encoded with writeExternal, but current class does not
      // implement readExternal, so data is meaningless to us.
      toplevel->argumentErrorClass()->throwError(
          kReadExternalNotImplementedError, info->m_name);
    }

    Multiname mn(core->findPublicNamespace(),
                 core->internConstantStringLatin1(kReadExternal));
    info->m_functionBinding =
        toplevel->getBinding(info->m_closure->traits()->itraits, &mn);
  }
