...
namespace avmplus {
XMLClass::XMLClass(VTable *cvtable) : ClassClosure(cvtable) {
  ...
  // XML.prettyPrinting
  // XML.prettyIndent
  m_flags = kFlagIgnoreComments | kFlagIgnoreProcessingInstructions |
            kFlagIgnoreWhitespace | kFlagPrettyPrinting;
  m_prettyIndent = 2;
...
Atom XMLClass::construct(int argc, Atom *argv) {
  // We are no longer supporting this weird Rhino behavior.  According to the
  // spec and a discussion with the E4X developers, Rhino is buggy and a no
  // argument constructor should be equivalent to the empty string constructor.
...
Atom XMLClass::ToXML(Atom arg) {
  ...
  if (AvmCore::isNullOrUndefined(arg)) {
    toplevel->throwTypeError((arg == undefinedAtom)
    ...
    XMLObject *x = XMLObject::create(core->GetGC(), toplevel->xmlClass(),
                                     core->string(arg), defaultNamespace);
    ...
    // 7. else throw a syntaxError
      ...
      // check for multiple nodes where the first n-1 are PI/comment nodes and
      // the last one is an element node.  Just ignore the PI/comment nodes and
      // return the element node.  (bug 148526). Will also now ignore any text
      // nodes that just contain whitespace (leading or trailing) the one valid
      // element node.  If multiple element nodes are found, that is a failing
      // case as well. (bug 192355)
...
void XMLClass::set_prettyIndent(int printVal) { m_prettyIndent = printVal; }

int XMLClass::get_prettyIndent() { return m_prettyIndent; }

/////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////

QNameClass::QNameClass(VTable *cvtable) : ClassClosure(cvtable) {
...
Atom QNameClass::call(int argc, Atom *argv) {
  if (argc == 1) {
    AvmCore *core = this->core();
    if (core->isObject(argv[1]) &&
        AvmCore::istype(argv[1], core->traits.qName_itraits))
      return argv[1];
  }
...
Atom QNameClass::construct(int argc, Atom *argv) {
  ...
  if (argc == 1) {
    ...
    if (a == undefinedAtom) {
      ...
      // If we ever use this QName as a multiname, we need to preserve the exact
      // ns
