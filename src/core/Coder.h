 ...
 */
    ...
    public:
        ...
        virtual void write(const FrameState *state, const uint8_t *pc,
                           AbcOpcode opcode, Traits *type);

        virtual void writeOp1(const FrameState *state, const uint8_t *pc,
                              AbcOpcode opcode, uint32_t opd1, Traits *type);

        virtual void writeOp2(const FrameState *state, const uint8_t *pc,
                              AbcOpcode opcode, uint32_t opd1, uint32_t opd2,
                              Traits *type);

        virtual void writeMethodCall(const FrameState *state, const uint8_t *pc,
                                     AbcOpcode opcode, MethodInfo *,
                                     uintptr_t disp_id, uint32_t argc, Traits *type);
        ...
        virtual void writeCoerce(const FrameState *state, uint32_t index,
                                 Traits *type);
        ...
        virtual void writeOpcodeVerified(const FrameState *state, const uint8_t *pc,
                                         AbcOpcode opcode);

        virtual void writeFixExceptionsAndLabels(const FrameState *state,
                                                 const uint8_t *pc);
    ...
    public:
        ...
        /** Remove the count items under the stack top.  Equivalent to: while
         * (count--) {swap;pop} */
