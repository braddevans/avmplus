 ...
 */
    ...
    public:
        ...
        void init(MethodEnv *env, FramePtr framep, const uint8_t *frame_sst,
         ...
         */
        inline explicit CallStackNode(MethodEnv *env, FramePtr framep,
        ...
        inline explicit CallStackNode(AvmCore *core, uint64_t functionId,
                                      int32_t lineno) {
        ...
        inline explicit CallStackNode(MethodInfo *info) { init(info); }

        // dummy ctor we can use to construct an uninitalized version -- useful for
        // the thunks, which will construct one and immediately call initialize (via
        // debugEnter) so there's no need to redundantly fill in fields. (note that
        // the fields are uninitialized so you *must* call init afterwards when using
        // this!)
        enum Empty {
            kEmpty = 0
        };
        ...
        // WARNING, env() can return null if there are fake Sampler-only frames. You
        // must always check for null.
        inline MethodEnv *env() const { return m_env; }

        // WARNING, info() can return null if there are fake Sampler-only frames. You
        // must always check for null.
        ...
        //
        // The delete operator is required by some compilers in builds that compile
        // with exceptions enabled, but is never actually called.

        inline void *operator new(size_t, void *storage) { return storage; }

        inline void operator delete(void *) {}

        // enumerateScopeChainAtoms will list the active scopechain by repeatedly
        // calling the addScope of the interface.
        ...
        // ------------------------ DATA SECTION BEGIN
    private:
        uint64_t m_functionId; // int used to uniquely identify function calls in
        // external scripting languages
    private:
        AvmCore *m_core;

    private:
        MethodEnv *m_env; // will be NULL if the element is from a fake CallStackNode
    private:
        MethodInfo *m_info;

    private:
        CallStackNode *m_next;

    private:
        Stringp m_fakename; // NULL unless we are a fake CallStackNode
    private:
        intptr_t volatile *m_eip; // ptr to where the current pc is stored
    private:
        Stringp m_filename; // in the form
        // "C:\path\to\package\root;package/package;filename"
    private:
        FramePtr m_framep; // pointer to top of AS registers
    private:
        const uint8_t *m_frame_sst; // array of traits for AS registers and scopechain
    private:
        int32_t m_linenum;
    ...
    public:
    REALLY_INLINE static StackTrace
    *
    create(MMgc::GC
    *gc,
    int depth
...
struct Element {
    enum {
        EXTERNAL_CALL_FRAME = 1
    }; // used for CallStackNodes for external scripting languages
    ...
    union {
        struct {
            Stringp
                    m_fakename; // needed just for fake CallStackNodes, null otherwise
            Stringp
                    m_filename; // in the form
            // "C:\path\to\package\root;package/package;filename"
        } u;
    ...
    inline void set(const CallStackNode &csn) {
        if (csn.functionId() != 0) {
            ...
            m_info = csn.info(); // will be NULL if the element is from a fake
            // CallStackNode
    ...
    inline void set(MethodInfo *methodInfo) {
        ...
        m_info = methodInfo; // will be NULL if the element is from a fake
        // CallStackNode
        u.m_fakename = NULL;
        u.m_filename = NULL;
        m_linenum = 0;
    ...
    inline bool isAS3Sample() const {
        return m_info != (MethodInfo *) EXTERNAL_CALL_FRAME;
    }

    // WARNING, info() can return null if there are fake Sampler-only frames.
    ...
    inline Stringp fakename() const {
        return isAS3Sample() ? u.m_fakename : NULL;
    }

#endif

    inline Stringp name() const {
        return isAS3Sample()
               ? ((!u.m_fakename && m_info) ? m_info->getMethodName()
                                            : u.m_fakename)
               : NULL;
    }

    inline Stringp filename() const {
        return isAS3Sample() ? u.m_filename : NULL;
    }

    inline int32_t linenum() const { return m_linenum; }

    inline uint64_t functionId() const {
        return isAS3Sample() ? 0 : m_functionId;
    }
};
