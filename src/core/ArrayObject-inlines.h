...
namespace avmplus {
    ...
    ArrayObject::pop() { return AS3_pop(); }

    REALLY_INLINE uint32_t
    ArrayObject::push(Atom
    *args,
    int argc
    ) {
    return
...
REALLY_INLINE uint32_t
ArrayObject::unshift(Atom
*args,
int argc
) {
return
AS3_unshift(args, argc
);
}
...
// that the implementations of getUintProperty and
// _getUintProperty/_getIntProperty (used by the JIT) contain inlined code
// rather than a tail-call.
REALLY_INLINE Atom
ArrayObject::getUintPropertyImpl(uint32_t
index) const {
...
return
result;
// else, a hole in the dense area, must fall thru and search the
// hashtable/proto chain
}
...
/*virtual*/
REALLY_INLINE void ArrayObject::setUintProperty(uint32_t
index,
Atom value
) {
...
/*virtual*/
REALLY_INLINE Atom
ArrayObject::getUintProperty(uint32_t
index) const {
return
getUintPropertyImpl(index);
}

// Non-virtual members for ActionScript method implementation.
// Always calls thru to the virtual method to allow subclasses to override in
// C++.
REALLY_INLINE uint32_t

ArrayObject::get_length() const { return getLength(); }

REALLY_INLINE void ArrayObject::set_length(uint32_t
newLength) {
setLength(newLength);
}
