...
#define ARGTYPE_RF4                                                            \
  ARGTYPE_P // to return a float4, we receive an "output" pointer to a float4
    // value
...
#define PRIM_HIT(val, c) (atomKind(val) == c.tag)
    ...
    *args) {
    ...
    obj;
    return c.method->
    coerceEnter(argc, args
    );
}

template<class T> REALLY_INLINE T
load_cached_slot(BindingCache
&c,
Atom obj
) {
return *((T *)(
uintptr_t(atomObj(obj)
) + c.slot_offset));
}

// calling a declared method on a ScriptObject
Atom callprop_obj_method(CallCache &c, Atom obj, int argc, Atom *args,
                         MethodEnv *env) {
    PROF_IF("callprop_obj_method hit", OBJ_HIT(obj, c))return invoke_cached_method(c, obj, argc, args);
    return callprop_miss(c, obj, argc, args, env);
}

// calling a function in a declared slot, specialized for slot type
template<class T>
Atom callprop_obj_slot(CallCache &c, Atom obj, int argc, Atom *args,
                       MethodEnv *env) {
    PROF_IF("callprop_obj_slot hit", OBJ_HIT(obj, c))return op_call(env, load_cached_slot<T>(c, obj), argc, args);
    return callprop_miss(c, obj, argc, args, env);
}

// calling an unknown binding on an object, i.e. a dynamic property
Atom callprop_obj_none(CallCache &c, Atom obj, int argc, Atom *args,
                       MethodEnv *env) {
...
Atom callprop_prim_method(CallCache &c, Atom prim, int argc, Atom *args,
                          MethodEnv *env) {
    PROF_IF("callprop_prim_method hit", PRIM_HIT(prim, c))return invoke_cached_method(c, prim, argc, args);
    return callprop_miss(c, prim, argc, args, env);
}

// calling a dynamic property on a primitive's prototype
Atom callprop_prim_none(CallCache &c, Atom prim, int argc, Atom *args,
                        MethodEnv *env) {
...
Atom callprop_generic(CallCache &c, Atom obj, int argc, Atom *args,
                      MethodEnv *env) {
    ...
    tagprof("callprop_generic bind",
            toplevel->getBinding(vtable->traits, c.name));
...
Atom callprop_miss(CallCache &c, Atom obj, int argc, Atom *args,
                   MethodEnv *env) {
    AssertNotNull(obj);
    Toplevel *toplevel = env->toplevel();
    VTable *vtable = toplevel->toVTable(obj);
    ...
    if (isObjectPtr(obj)) {
        ...
        if (AvmCore::isSlotBinding(b)) {
            ...
            SlotStorageType sst = obj_type->getTraitsBindings()->calcSlotAddrAndSST(
                    AvmCore::bindingToSlotId(b), obj_ptr, slot_ptr);
...
Atom callprop_late(MethodEnv *caller_env, Atom base, const Multiname *name,
                   int argc, Atom *args) {
    CallCache c(
            name, NULL); // temporary cache, just so we can call the generic handler.
...
enum Bool32 {
}; // can't use bool when sizeof(bool) != sizeof(int32_t)
REALLY_INLINE Atom
boxslot(AvmCore
*,
OpaqueAtom a
...
#ifdef VMCFG_FLOAT
REALLY_INLINE Atom boxslot(AvmCore *core, float f) {
  return core->floatToAtom(f);
}
REALLY_INLINE Atom boxslot(AvmCore *core, const float4_t &f4) {
...
// Force this function to have a frame pointer so the double used is 8-byte
// aligned
...
SSE2_ONLY(
        ...
        Atom
getprop_obj_slot_double_sse2(GetCache
&c,
MethodEnv *env, Atom
obj) {
PROF_IF("getprop_obj_slot hit", OBJ_HIT(obj, c)) {
...
->core()->
doubleToAtom_sse2(
        load_cached_slot<double>(c, obj)
);
}
return
getprop_miss(c, env, obj
);
})
#if defined(_MSC_VER) && defined(AVMPLUS_IA32)
...
Atom getprop_obj_get(GetCache & c, MethodEnv * env, Atom
obj) {
...
Atom getprop_obj_none(GetCache & c, MethodEnv * env, Atom
obj) {
PROF_IF("getprop_obj_none hit", OBJ_HIT(obj, c)) {
return
atomObj(obj)
->
getMultinameProperty(c
.name);
}
return
getprop_miss(c, env, obj
...
Atom getprop_obj_method(GetCache & c, MethodEnv * env, Atom
obj) {
PROF_IF("getprop_obj_method hit", OBJ_HIT(obj, c)) {
return env->toplevel()->methodClosureClass()->
create(c
.method, obj)->
...
Atom getprop_prim_get(GetCache & c, MethodEnv * env, Atom
obj) {
...
Atom getprop_miss(GetCache & c, MethodEnv * env, Atom
obj) {
// cache handler when cache miss occurs
AvmAssert(!
AvmCore::isNullOrUndefined(obj)
);
Toplevel *toplevel = env->toplevel();
VTable *vtable = toplevel->toVTable(obj);
...
if (
AvmCore::isMethodBinding(b)
) {
c.
method = vtable->methods[AvmCore::bindingToMethodId(b)];
} else if (
AvmCore::hasGetterBinding(b)
) {
c.
method = vtable->methods[AvmCore::bindingToGetterId(b)];
}
if (
isObjectPtr(obj)
) {
...
vtable = vtable;
if (
AvmCore::isSlotBinding(b)
) {
// precompute the slot offset, then install a cache handler that's
// hardwired to read the slot and convert value to Atom.
void *slot_ptr, *obj_ptr = atomObj(obj);
const SlotStorageType sst =
        actual_type->getTraitsBindings()->calcSlotAddrAndSST(
                AvmCore::bindingToSlotId(b), (void *) obj_ptr, slot_ptr);
SSE2_ONLY(if (toplevel->core()->config.njconfig.
i386_sse2 &&
        sst
== SST_double) {
c.
get_handler = &getprop_obj_slot_double_sse2;
} else) {
c.
get_handler = getprop_slot_handlers[sst];
}
c.
slot_offset = uintptr_t(slot_ptr) - uintptr_t(obj_ptr);
} else {
c.
get_handler = getprop_obj_handlers[AvmCore::bindingKind(b)];
if (
AvmCore::isMethodBinding(b)
&&
...
c.
get_handler = getprop_obj_none;
}
}
} else {
// must be a primitive: int, bool, string, namespace, or number
c.
get_handler = getprop_prim_handlers[AvmCore::bindingKind(b)];
c.
tag = atomKind(obj);
AvmAssert(c
.tag != 0);
...
FASTFUNCTION(FUNCADDR(AvmCore::takeSampleWrapper), SIG1(V, P),
             takeSampleWrapper);
#endif
#ifdef VMCFG_STACK_METRICS
FASTFUNCTION(FUNCADDR(AvmCore::recordStackPointerWrapper), SIG1(V, P),
             recordStackPointerWrapper);
#endif

FUNCTION(FUNCADDR(AvmCore::handleInterruptMethodEnv), SIG1(V, P),
        handleInterruptMethodEnv

)

FASTFUNCTION(FUNCADDR(AvmCore::handleStackOverflowMethodEnv), SIG1(V, P),
...
// Coercing an atom to a numeric type may yield a side-effect via the valueOf
// property.
...
#ifdef VMCFG_FLOAT
FUNCTION(FUNCADDR(AvmCore::singlePrecisionFloat), SIG1(F, A),
         singlePrecisionFloat)
FUNCTION(FUNCADDR(AvmCore::float4), SIG2(V, RF4, A), float4)
#endif

// These conversions assume the argument is already a numeric atom, thus no
// side-effects. They are called in contexts where the type of the atom has been
// previously ascertained, either by preceding program logic or by the verifier.
...
PUREMETHOD(STRINGADDR(String::_charCodeAtDI), SIG2(D, P, I),
        String_charCodeAtDI

)

PUREMETHOD(STRINGADDR(String::_charCodeAtII), SIG2(I, P, I),
        String_charCodeAtII

)

PUREMETHOD(STRINGADDR(String::AS3_charCodeAt), SIG2(D, P, D),
        String_charCodeAtDD

)

PUREMETHOD(STRINGADDR(String::_charCodeAtID), SIG2(I, P, D),
        String_charCodeAtID

)

PUREMETHOD(STRINGADDR(String::_charCodeAtDU), SIG2(D, P, U),
        String_charCodeAtDU

)

PUREMETHOD(STRINGADDR(String::_charCodeAtIU), SIG2(I, P, U),
        String_charCodeAtIU

)

FASTFUNCTION(FUNCADDR(String::equalsWithNullChecks), SIG2(I, P, P),
        String_equals
...
// with MethodEnv::nullcheckfail(), which allows for a null return from
// typeErrorClass() and reports a different diagnostic.  It is unclear that this
// error is actually possible, but we should get to the bottom of it and do the
// same thing in both the interpreter and the JIT.
...
FUNCTION(FUNCADDR((op_call_error_MethodEnv) op_call_error < MethodEnv * > ),
        SIG1(A, P), op_call_error
)

typedef VTable *(*toVTable_Toplevel)(Toplevel *, Atom);

PUREFUNCTION(FUNCADDR((toVTable_Toplevel) &toVTable < Toplevel * > ), SIG2(P, P, A),
        toVTable
)
...
Atom makeatom(AvmCore * core, AnyVal * native, SlotStorageType
tag) {
switch (tag) {
default:
AvmAssert(false);
case SST_atom:
AvmAssert(atomKind(native->atom)
!= 0);
return native->
atom;
case SST_namespace:
...
AvmAssert(atomKind(native->atom)
== 0);
AvmAssert(uintptr_t(native->str)
> 1000 || native->str == 0);
return native->str->
...
// Superficially, makeatom appears to be a pure function.  The 'native'
// argument, however, represents a value passed by read-only reference, and
// which may change between identical calls.  These calls are not subject to
// CSE, and thus makeatom must not be marked pure.
FUNCTION(FUNCADDR(makeatom), SIG3(A, P, P, I), makeatom
)

void set_slot_from_atom(Atom
atom,
Traits *traits, AnyVal
*slotPtr,
uint8_t *tagPtr
) {
...
== kNamespaceType);
slotPtr->
ns = (Namespace *) atomPtr(atom);
break;
case SST_string:
AvmAssert(atomKind(atom)
== kStringType);
slotPtr->
str = (String *) atomPtr(atom);
break;
case SST_scriptobject:
AvmAssert(atomKind(atom)
== kObjectType);
slotPtr->
obj = (ScriptObject *) atomPtr(atom);
break;
case SST_int32:
AvmAssert(atomKind(atom)
== kIntptrType ||
...
== kDoubleType);
slotPtr->
u = AvmCore::integer_u(atom);
break;
case SST_bool32:
AvmAssert(atomKind(atom)
== kBooleanType);
slotPtr->
b = (int32_t) atomGetBoolean(atom);
break;
case SST_double:
...
int32_t beginCatch(AvmCore * core, ExceptionFrame * ef, MethodInfo * info,
                   intptr_t
pc,
AnyVal *slotPtr, uint8_t
*tagPtr) {
int32_t ordinal;
ef->
...
ExceptionHandler *handler =
        core->findExceptionHandlerNoRethrow(info, pc, exception, &ordinal);
if (!handler) {
...
ef->
beginTry(core);
set_slot_from_atom(exception
->atom, handler->traits, slotPtr, tagPtr);
return
ordinal;
}
...
REALLY_INLINE void store_cached_slot(SetCache &c, ScriptObject *obj,
                                     OpaqueAtom *slot_ptr, Atom val) {
    AvmAssert(c.gc == obj->gc());
    WBATOM(c.gc, obj, (Atom * )
    slot_ptr,
            (val != undefinedAtom ? val : nullObjectAtom));
}
...
void throw_checktype_error(SetCache &c, ScriptObject *obj_ptr, Atom val) {
    ...
    obj_ptr->toplevel()->throwTypeError(kCheckTypeFailedError,
                                        core->atomToErrorString(val),
                                        core->toErrorString(c.slot_type));
}

// coerce & store into ScriptObject subclass-typed slot
REALLY_INLINE void store_cached_slot(SetCache &c, ScriptObject *obj,
                                     ScriptObject **slot_ptr, Atom val) {
    if (AvmCore::isNullOrUndefined(val) ||
        (atomKind(val) == kObjectType &&
         atomObj(val)->traits()->subtypeof(c.slot_type))) {
...
static const SetCache::Handler setprop_slot_handlers[SST_MAX_VALUE] = {
        &setprop_slot<OpaqueAtom>, // SST_atom (for slot type Object.  type * is an
        // exception)
...
void setprop_miss(SetCache &c, Atom obj, Atom val, MethodEnv *env) {
    ...
    if (isObjectPtr(obj)) {
        ...
        if (AvmCore::isVarBinding(b)) {
            ...
            const SlotStorageType sst =
                    tb->calcSlotAddrAndSST(slot, obj_ptr, slot_ptr);
            ...
            if (sst == SST_atom) {
                ...
                // This line must come after the test of c.slot_type above, as c.gc
                // overlays it! See the union in class SetCache.
...
void setprop_late(MethodEnv *env, Atom obj, const Multiname *name, Atom val) {
    SetCache c(name,
               NULL); // temporary cache, just so we can call the generic handler.
...
void initprop_late(MethodEnv *env, Atom obj, const Multiname *name,
                   Atom value) {
...
// implements OP_getproperty with unknown base object or index type, but a
// multiname that includes public and therefore exposes dynamic properties
Atom getprop_index(MethodEnv *caller_env, Atom obj, const Multiname *name,
                   Atom index) {
    if (atomIsIntptr(index) && atomCanBeUint32(index)) {
        if (isObjectPtr(obj)) {
            _nvprof("getprop_index P-fast", 1);
            return AvmCore::atomToScriptObject(obj)->getUintProperty(
                    uint32_t(atomGetIntptr(index)));
        }
    }
...
// index expression that is public and therefore able to access dynamic
// properties. (typically this is late-bound array access)
void setprop_index(MethodEnv *caller_env, Atom obj, const Multiname *name,
                   Atom value, Atom index) {
    if (isObjectPtr(obj)) {
        if (atomIsIntptr(index) && atomCanBeUint32(index)) {
            // todo: obj is probably a dense array or vector
            AvmCore::atomToScriptObject(obj)->setUintProperty(
                    uint32_t(atomGetIntptr(index)), value);
    ...
    Multiname tempname = *name;
    caller_env->setpropertyHelper(obj, &tempname, value,
                                  toVTable(caller_env->toplevel(), obj), index);
}
...
// This is equivalent to setprop_index but performs addition of index1 and
// index2 to compute the index. This is only run when (index1+index2) overflows
// an int32_t value or is negative and is not time critical.
void setprop_index_add(MethodEnv *caller_env, Atom obj, const Multiname *name,
                       Atom value, int32_t index1, int32_t index2) {
  Atom index = caller_env->toplevel()->core()->doubleToAtom((double)index1 +
                                                            (double)index2);
  if (isObjectPtr(obj)) {
    if (atomIsIntptr(index) && atomCanBeUint32(index)) {
      // todo: obj is probably a dense array or vector
...
FUNCTION(FUNCADDR(setprop_index_add), SIG6(V, P, A, P, A, I, I),
         setprop_index_add)

// This is equivalent to setprop_index but performs subtraction of index1 and
// index2 to compute the index. This is only run when (index1-index2) overflows
// an int32_t value or is negative and is not time critical.
void setprop_index_subtract(MethodEnv *caller_env, Atom obj,
                            const Multiname *name, Atom value, int32_t index1,
                            int32_t index2) {
  Atom index = caller_env->toplevel()->core()->doubleToAtom((double)index1 -
                                                            (double)index2);
  if (isObjectPtr(obj)) {
...
FUNCTION(FUNCADDR(setprop_index_subtract), SIG6(V, P, A, P, A, I, I),
         setprop_index_subtract)
...
void initprop_index(MethodEnv *caller_env, Atom obj, const Multiname *name,
                    Atom value, Atom index) {
...
#ifdef VMCFG_FASTPATH_ADD_INLINE
// This is equivalent to initprop_index but performs addition of index1 and
// index2 to compute the index. This is only run when (index1+index2) overflows
// an int32_t value or is negative and is not time critical.
void initprop_index_add(MethodEnv *caller_env, Atom obj, const Multiname *name,
                        Atom value, int32_t index1, int32_t index2) {
  Atom index = caller_env->toplevel()->core()->doubleToAtom((double)index1 +
                                                            (double)index2);
...
FUNCTION(FUNCADDR(initprop_index_add), SIG6(V, P, A, P, A, I, I),
         initprop_index_add)

// This is equivalent to initprop_index but performs subtraction of index1 and
// index2 to compute the index. This is only run when (index1-index2) overflows
// an int32_t value or is negative and is not time critical.
void initprop_index_subtract(MethodEnv *caller_env, Atom obj,
                             const Multiname *name, Atom value, int32_t index1,
                             int32_t index2) {
  Atom index = caller_env->toplevel()->core()->doubleToAtom((double)index1 -
                                                            (double)index2);
...
FUNCTION(FUNCADDR(initprop_index_subtract), SIG6(V, P, A, P, A, I, I),
         initprop_index_subtract)
#endif // VMCFG_FASTPATH_ADD_INLINE
...
return
AvmCore::istype(val, t
); // implicitly cast bool -> int
}
...
Atom restargHelper(Toplevel *toplevel, const Multiname *m, Atom val,
                   ArrayObject **restLocal, uint32_t argc, Atom *restArea) {
    uint32_t index;
    ...
    if (atomIsIntptr(val) && atomCanBeUint32(val)) {
        ...
        // Non-int index, check for Number values that are in the int range and
        // nonnegative
        double d = AvmCore::atomToDouble(val);
        if (!(d >= 0 && d <= 0xFFFFFFFFU && MathUtils::floor(d) == d &&
              (index = (uint32_t) d) < argc))
            goto exception;
    }
#ifdef VMCFG_FLOAT
        else if (AvmCore::isFloat(val)) {
          // Non-int index, check for float values that are in the int range and
          // nonnegative
    ...
    if (atomIsIntptr(val) && atomCanBeUint32(val))
        return ((ArrayObject * ) * restLocal)
                ->getUintProperty((uint32_t) atomGetIntptr(val));
...
PUREMETHOD(COREADDR(AvmCore::EscapeAttributeValue), SIG2(P, P, A),
...
METHOD(ENVADDR(MethodEnv::delpropertyHelper), SIG4(A, P, A, P, A),
        delpropertyHelper
...
METHOD(ENVADDR(MethodEnv::setpropertylate_u), SIG4(V, P, A, U, A),
        setpropertylate_u

)

METHOD(ARRAYADDR(ArrayObject::_setUintProperty), SIG3(V, P, U, A),
        ArrayObject_setUintProperty

)

METHOD(VECTOROBJADDR(ObjectVectorObject::_setUintProperty), SIG3(V, P, U, A),
        ObjectVectorObject_setUintProperty

)

METHOD(VECTOROBJADDR(ObjectVectorObject::_setKnownUintProperty),
        SIG3(V, P, U, A), ObjectVectorObject_setKnownUintProperty

)

METHOD(VECTOROBJADDR(ObjectVectorObject::_setKnownUintPropertyWithPointer),
        SIG3(V, P, U, A), ObjectVectorObject_setKnownUintPropertyWithPointer

)

METHOD(VECTORUINTADDR(UIntVectorObject::_setUintProperty), SIG3(V, P, U, A),
        UIntVectorObject_setUintProperty

)

METHOD(VECTORUINTADDR(UIntVectorObject::_setNativeUintProperty),
        SIG3(V, P, U, U), UIntVectorObject_setNativeUintProperty

)

METHOD(VECTORINTADDR(IntVectorObject::_setUintProperty), SIG3(V, P, U, A),
        IntVectorObject_setUintProperty

)

METHOD(VECTORINTADDR(IntVectorObject::_setNativeUintProperty), SIG3(V, P, U, I),
        IntVectorObject_setNativeUintProperty

)

METHOD(VECTORDOUBLEADDR(DoubleVectorObject::_setUintProperty), SIG3(V, P, U, A),
        DoubleVectorObject_setUintProperty

)

METHOD(VECTORDOUBLEADDR(DoubleVectorObject::_setNativeUintProperty),
        SIG3(V, P, U, D), DoubleVectorObject_setNativeUintProperty

)
#ifdef VMCFG_FLOAT
METHOD(VECTORFLOATADDR(FloatVectorObject::_setUintProperty), SIG3(V, P, U, A),
       FloatVectorObject_setUintProperty)
METHOD(VECTORFLOATADDR(FloatVectorObject::_setNativeUintProperty),
       SIG3(V, P, U, F), FloatVectorObject_setNativeUintProperty)
METHOD(VECTORFLOAT4ADDR(Float4VectorObject::_setUintProperty), SIG3(V, P, U, A),
       Float4VectorObject_setUintProperty)
METHOD(VECTORFLOAT4ADDR(Float4VectorObject::_setFloat4UintProperty),
       SIG3(V, P, U, PF4), Float4VectorObject_setNativeUintProperty)
#endif

METHOD(ENVADDR(MethodEnv::setpropertylate_i), SIG4(V, P, A, I, A),
        setpropertylate_i

)

METHOD(ARRAYADDR(ArrayObject::_setIntProperty), SIG3(V, P, I, A),
        ArrayObject_setIntProperty

)

METHOD(VECTOROBJADDR(ObjectVectorObject::_setIntProperty), SIG3(V, P, I, A),
        ObjectVectorObject_setIntProperty

)

METHOD(VECTOROBJADDR(ObjectVectorObject::_setKnownIntProperty),
        SIG3(V, P, I, A), ObjectVectorObject_setKnownIntProperty

)

METHOD(VECTOROBJADDR(ObjectVectorObject::_setKnownIntPropertyWithPointer),
        SIG3(V, P, I, A), ObjectVectorObject_setKnownIntPropertyWithPointer

)

METHOD(VECTORUINTADDR(UIntVectorObject::_setIntProperty), SIG3(V, P, I, A),
        UIntVectorObject_setIntProperty

)

METHOD(VECTORUINTADDR(UIntVectorObject::_setNativeIntProperty),
        SIG3(V, P, I, U), UIntVectorObject_setNativeIntProperty

)

METHOD(VECTORINTADDR(IntVectorObject::_setIntProperty), SIG3(V, P, I, A),
        IntVectorObject_setIntProperty

)

METHOD(VECTORINTADDR(IntVectorObject::_setNativeIntProperty), SIG3(V, P, I, I),
        IntVectorObject_setNativeIntProperty

)

METHOD(VECTORDOUBLEADDR(DoubleVectorObject::_setIntProperty), SIG3(V, P, I, A),
        DoubleVectorObject_setIntProperty

)

METHOD(VECTORDOUBLEADDR(DoubleVectorObject::_setNativeIntProperty),
        SIG3(V, P, I, D), DoubleVectorObject_setNativeIntProperty

)
#ifdef VMCFG_FLOAT
METHOD(VECTORFLOATADDR(FloatVectorObject::_setIntProperty), SIG3(V, P, I, A),
       FloatVectorObject_setIntProperty)
METHOD(VECTORFLOATADDR(FloatVectorObject::_setNativeIntProperty),
       SIG3(V, P, I, F), FloatVectorObject_setNativeIntProperty)
METHOD(VECTORFLOAT4ADDR(Float4VectorObject::_setIntProperty), SIG3(V, P, I, A),
       Float4VectorObject_setIntProperty)
METHOD(VECTORFLOAT4ADDR(Float4VectorObject::_setFloat4IntProperty),
       SIG3(V, P, I, PF4), Float4VectorObject_setNativeIntProperty)
#endif

METHOD(ENVADDR(MethodEnv::setpropertylate_d), SIG4(V, P, A, D, A),
        setpropertylate_d

)

METHOD(ARRAYADDR(ArrayObject::_setDoubleProperty), SIG3(V, P, D, A),
        ArrayObject_setDoubleProperty

)

METHOD(VECTOROBJADDR(ObjectVectorObject::_setDoubleProperty), SIG3(V, P, D, A),
        ObjectVectorObject_setDoubleProperty

)

METHOD(VECTOROBJADDR(ObjectVectorObject::_setKnownDoubleProperty),
        SIG3(V, P, D, A), ObjectVectorObject_setKnownDoubleProperty

)

METHOD(VECTOROBJADDR(ObjectVectorObject::_setKnownDoublePropertyWithPointer),
        SIG3(V, P, D, A), ObjectVectorObject_setKnownDoublePropertyWithPointer

)

METHOD(VECTORUINTADDR(UIntVectorObject::_setDoubleProperty), SIG3(V, P, D, A),
        UIntVectorObject_setDoubleProperty

)

METHOD(VECTORUINTADDR(UIntVectorObject::_setNativeDoubleProperty),
        SIG3(V, P, D, U), UIntVectorObject_setNativeDoubleProperty

)

METHOD(VECTORINTADDR(IntVectorObject::_setDoubleProperty), SIG3(V, P, D, A),
        IntVectorObject_setDoubleProperty

)

METHOD(VECTORINTADDR(IntVectorObject::_setNativeDoubleProperty),
        SIG3(V, P, D, I), IntVectorObject_setNativeDoubleProperty

)

METHOD(VECTORDOUBLEADDR(DoubleVectorObject::_setDoubleProperty),
        SIG3(V, P, D, A), DoubleVectorObject_setDoubleProperty

)

METHOD(VECTORDOUBLEADDR(DoubleVectorObject::_setNativeDoubleProperty),
        SIG3(V, P, D, D), DoubleVectorObject_setNativeDoubleProperty

)
#ifdef VMCFG_FLOAT
METHOD(VECTORFLOATADDR(FloatVectorObject::_setDoubleProperty), SIG3(V, P, D, A),
       FloatVectorObject_setDoubleProperty)
METHOD(VECTORFLOATADDR(FloatVectorObject::_setNativeDoubleProperty),
       SIG3(V, P, D, F), FloatVectorObject_setNativeDoubleProperty)
METHOD(VECTORFLOAT4ADDR(Float4VectorObject::_setDoubleProperty),
       SIG3(V, P, D, A), Float4VectorObject_setDoubleProperty)
METHOD(VECTORFLOAT4ADDR(Float4VectorObject::_setFloat4DoubleProperty),
       SIG3(V, P, D, PF4), Float4VectorObject_setNativeDoubleProperty)
#endif

METHOD(ENVADDR(MethodEnv::getpropertylate_u), SIG3(A, P, A, U),
        getpropertylate_u

)

METHOD(ARRAYADDR(ArrayObject::_getUintProperty), SIG2(A, P, U),
        ArrayObject_getUintProperty

)

METHOD(VECTOROBJADDR(ObjectVectorObject::_getUintProperty), SIG2(A, P, U),
        ObjectVectorObject_getUintProperty

)

METHOD(VECTORUINTADDR(UIntVectorObject::_getUintProperty), SIG2(A, P, U),
        UIntVectorObject_getUintProperty

)

METHOD(VECTORUINTADDR(UIntVectorObject::_getNativeUintProperty), SIG2(U, P, U),
        UIntVectorObject_getNativeUintProperty

)

METHOD(VECTORINTADDR(IntVectorObject::_getUintProperty), SIG2(A, P, U),
        IntVectorObject_getUintProperty

)

METHOD(VECTORINTADDR(IntVectorObject::_getNativeUintProperty), SIG2(I, P, U),
        IntVectorObject_getNativeUintProperty

)

METHOD(VECTORDOUBLEADDR(DoubleVectorObject::_getUintProperty), SIG2(A, P, U),
        DoubleVectorObject_getUintProperty

)

METHOD(VECTORDOUBLEADDR(DoubleVectorObject::_getNativeUintProperty),
        SIG2(D, P, U), DoubleVectorObject_getNativeUintProperty

)
#ifdef VMCFG_FLOAT
METHOD(VECTORFLOATADDR(FloatVectorObject::_getUintProperty), SIG2(A, P, U),
       FloatVectorObject_getUintProperty)
METHOD(VECTORFLOATADDR(FloatVectorObject::_getNativeUintProperty),
       SIG2(F, P, U), FloatVectorObject_getNativeUintProperty)
METHOD(VECTORFLOAT4ADDR(Float4VectorObject::_getUintProperty), SIG2(A, P, U),
       Float4VectorObject_getUintProperty)
METHOD(VECTORFLOAT4ADDR(Float4VectorObject::_getFloat4UintProperty),
       SIG3(V, P, RF4, U), Float4VectorObject_getNativeUintProperty)
#endif

METHOD(ENVADDR(MethodEnv::getpropertylate_i), SIG3(A, P, A, I),
        getpropertylate_i

)

METHOD(ARRAYADDR(ArrayObject::_getIntProperty), SIG2(A, P, I),
        ArrayObject_getIntProperty

)

METHOD(VECTOROBJADDR(ObjectVectorObject::_getIntProperty), SIG2(A, P, I),
        ObjectVectorObject_getIntProperty

)

METHOD(VECTORUINTADDR(UIntVectorObject::_getIntProperty), SIG2(A, P, I),
        UIntVectorObject_getIntProperty

)

METHOD(VECTORUINTADDR(UIntVectorObject::_getNativeIntProperty), SIG2(U, P, I),
        UIntVectorObject_getNativeIntProperty

)

METHOD(VECTORINTADDR(IntVectorObject::_getIntProperty), SIG2(A, P, I),
        IntVectorObject_getIntProperty

)

METHOD(VECTORINTADDR(IntVectorObject::_getNativeIntProperty), SIG2(I, P, I),
        IntVectorObject_getNativeIntProperty

)

METHOD(VECTORDOUBLEADDR(DoubleVectorObject::_getIntProperty), SIG2(A, P, I),
        DoubleVectorObject_getIntProperty

)

METHOD(VECTORDOUBLEADDR(DoubleVectorObject::_getNativeIntProperty),
        SIG2(D, P, I), DoubleVectorObject_getNativeIntProperty

)
#ifdef VMCFG_FLOAT
METHOD(VECTORFLOATADDR(FloatVectorObject::_getIntProperty), SIG2(A, P, I),
       FloatVectorObject_getIntProperty)
METHOD(VECTORFLOATADDR(FloatVectorObject::_getNativeIntProperty), SIG2(F, P, I),
       FloatVectorObject_getNativeIntProperty)
METHOD(VECTORFLOAT4ADDR(Float4VectorObject::_getIntProperty), SIG2(A, P, I),
       Float4VectorObject_getIntProperty)
METHOD(VECTORFLOAT4ADDR(Float4VectorObject::_getFloat4IntProperty),
       SIG3(V, P, RF4, I), Float4VectorObject_getNativeIntProperty)
#endif

METHOD(ENVADDR(MethodEnv::getpropertylate_d), SIG3(A, P, A, D),
        getpropertylate_d

)

METHOD(ARRAYADDR(ArrayObject::_getDoubleProperty), SIG2(A, P, D),
        ArrayObject_getDoubleProperty

)

METHOD(VECTOROBJADDR(ObjectVectorObject::_getDoubleProperty), SIG2(A, P, D),
        ObjectVectorObject_getDoubleProperty

)

METHOD(VECTORINTADDR(IntVectorObject::_getDoubleProperty), SIG2(A, P, D),
        IntVectorObject_getDoubleProperty

)

METHOD(VECTORINTADDR(IntVectorObject::_getNativeDoubleProperty), SIG2(I, P, D),
        IntVectorObject_getNativeDoubleProperty

)

METHOD(VECTORUINTADDR(UIntVectorObject::_getDoubleProperty), SIG2(A, P, D),
        UIntVectorObject_getDoubleProperty

)

METHOD(VECTORUINTADDR(UIntVectorObject::_getNativeDoubleProperty),
        SIG2(U, P, D), UIntVectorObject_getNativeDoubleProperty

)

METHOD(VECTORDOUBLEADDR(DoubleVectorObject::_getDoubleProperty), SIG2(A, P, D),
        DoubleVectorObject_getDoubleProperty

)

METHOD(VECTORDOUBLEADDR(DoubleVectorObject::_getNativeDoubleProperty),
        SIG2(D, P, D), DoubleVectorObject_getNativeDoubleProperty

)
#ifdef VMCFG_FLOAT
METHOD(VECTORFLOATADDR(FloatVectorObject::_getDoubleProperty), SIG2(A, P, D),
       FloatVectorObject_getDoubleProperty)
METHOD(VECTORFLOATADDR(FloatVectorObject::_getNativeDoubleProperty),
       SIG2(F, P, D), FloatVectorObject_getNativeDoubleProperty)
METHOD(VECTORFLOAT4ADDR(Float4VectorObject::_getDoubleProperty), SIG2(A, P, D),
       Float4VectorObject_getDoubleProperty)
METHOD(VECTORFLOAT4ADDR(Float4VectorObject::_getFloat4DoubleProperty),
       SIG3(V, P, RF4, D), Float4VectorObject_getNativeDoubleProperty)
#endif

METHOD(ENVADDR(MethodEnv::haspropertylate_u), SIG3(I, P, A, U),
        haspropertylate_u

)

METHOD(ENVADDR(MethodEnv::haspropertylate_i), SIG3(I, P, A, I),
        haspropertylate_i

)

METHOD(ENVADDR(MethodEnv::findproperty), SIG7(A, P, P, P, I, P, B, P),
        findproperty
...
typedef Atom (*constructprop_MethodEnv)(MethodEnv *, const Multiname *, int,
                                        Atom *);

FUNCTION(FUNCADDR((constructprop_MethodEnv) &constructprop < MethodEnv * > ),
        SIG4(A, P, P, I, P), construct_late
)

typedef Atom (*op_construct_MethodEnv)(MethodEnv *, Atom, int, Atom *);

FUNCTION(FUNCADDR((op_construct_MethodEnv) &op_construct < MethodEnv * > ),
        SIG4(A, P, A, I, P), op_construct
)
...
PUREMETHOD(COREADDR(AvmCore::_typeof), SIG2(P, P, A),

typeof)

FUNCTION(FUNCADDR(AvmCore::atomWriteBarrier), SIG4(V, P, P, P, A),
        atomWriteBarrier

)

METHOD(GCADDR(GC::privateWriteBarrierRC), SIG4(V, P, P, P, P),
...
// While it would be incorrect to perform CSE on these, we emit only one call
// per function. By making them pure, we allow dead calls to be eliminated.  It
// is unfortunate that we do not distinguish the absence of observable
// side-effects from full referential transparency.
PUREMETHOD(ENVADDR(MethodEnv::createRestHelper), SIG3(P, P, I, P),
        createRestHelper

)

PUREMETHOD(ENVADDR(MethodEnv::createArgumentsHelper), SIG3(P, P, I, P),
...
void initMultinameLate(AvmCore * core, Multiname & name, Atom
index) {
if (
AvmCore::isObject(index)
) {
...
METHOD(ENVADDR(MethodEnv::initMultinameLateForDelete), SIG3(V, P, P, A),
...
SSE2_ONLY(PUREMETHOD(COREADDR(AvmCore::doubleToAtom_sse2), SIG2(A, P, D),
                     doubleToAtom_sse2)

PUREFUNCTION(FUNCADDR(AvmCore::integer_d_sse2), SIG1(I, D),
        integer_d_sse2
...
FUNCTION(FUNCADDR((op_call_MethodEnv_Atom) &avmplus::op_call<MethodEnv *>),
        SIG4(A, P, A, I, P), op_call_atom
)
...
FUNCTION(FUNCADDR((op_applytype_MethodEnv) &avmplus::op_applytype<MethodEnv *>),
        SIG4(A, P, A, I, P), op_applytype
)

typedef ArrayObject *(*newarray_MethodEnv)(MethodEnv *, int, Atom *);

FUNCTION(FUNCADDR((newarray_MethodEnv) &newarray < MethodEnv * > ), SIG3(P, P, I, P),
        newarray
)

typedef Atom (*astype_late_MethodEnv)(MethodEnv *, Atom, Atom);

PUREFUNCTION(FUNCADDR((astype_late_MethodEnv) &astype_late < MethodEnv * > ),
        SIG3(A, P, A, A), astype_late
)
...
FUNCTION(FUNCADDR((instanceof_MethodEnv) &instanceof < MethodEnv * > ),
        SIG3(A, P, A, A),
        instanceof
)
...
FUNCTION(FUNCADDR((op_in_MethodEnv) &op_in < MethodEnv * > ), SIG3(A, P, A, A),
        op_in
)
...
FUNCTION(FUNCADDR((coerce_MethodEnv) &coerce < MethodEnv * > ), SIG3(A, P, A, P),
        coerce
)
...
#ifdef DOPROF
void jitProfileEvent(void *id) { _jvprof(id, 1); }

void jitProfileValue32(void *id, int32_t value) { _jvprof(id, value); }

void jitProfileHist32(void *id, int32_t value) { _jhprof(id, value); }
