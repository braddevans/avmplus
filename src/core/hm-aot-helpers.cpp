...
extern "C" int32_t llBeginCatch(MethodEnv *env, ExceptionFrame *ef, int pc,
                                int exception_count,
  ...
  for (int i = 0; i < exception_count; ++i) {
    ...
    if (desc.from <= pc && pc < desc.to &&
      ...
      // if traits are zero and not the special any traits, it's an
      // uninitialized type global. Don't spurtiously match it as if it were a
      // real "any."
...
extern "C" Traits *llLoadCatchTraits(MethodEnv *env,
                                     AOTExceptionDesc *exceptionDescs) {
  ...
  if (desc.nameIndex) {
    ...
    scopeTraits = Traits::newCatchTraits(env->toplevel(), pool,
                                         (TraitsPosPtr)desc.typeIndexU30,
...
// Not usually used, but here as a backstop in case someone wants to turn off
...
#ifdef VMCFG_HALFMOON_AOT_RUNTIME // Halfmoon AOT accesses precomputed
                                  // multinames directly
  PoolObject *poolSameWorker = NULL;
  std::vector<std::pair<PoolObject *, AvmCore *>> pools = aotInfo->pools;
  for (std::vector<std::pair<PoolObject *, AvmCore *>>::iterator it =
           pools.begin();
       it != pools.end(); ++it) {
...
// Class creation.
//
// The tricky issue here is to initialize the scope type chain and the scope
// chain and also to retrieve the class and instanca traits for the class.
//
// Pointers to the locations where the class and instance traits should be saved
// are passed in as well as an array of pointers to pointers to the scope chait
// traits. The fist layer of indirection allows the table to be constructed
// statically; the second lavel of indirection is bacause the locations pointed
// to are (like the scope and instance traits pointers just mentioned) lazily
// filled in, as the values become known.
//
// Finally, we must do some initializations that would normally have been done
// by the verifier.
//
...
extern "C" ClassClosure *llNewClass(MethodEnv *env, int classIndex,
                                    Traits **scopeTraitsPtrs,
                                    ClassClosure *base, int nWithScopes,
  ...
  if (ctraits) {
    ...
    // Create a scope type chain; we don't really have the info we need but we
    // can assume that the traits of the actual scopes are in fact the correct
    // types.
    MMgc::GC::AllocaAutoPtr _scopeTraitsBuffer;
    Traits **scopeTraits = reinterpret_cast<Traits **>(
        VMPI_alloca(core, _scopeTraitsBuffer, sizeof(Traits *) * scopeCount));
    ...
    const ScopeTypeChain *cscope = ScopeTypeChain::create(
        core->GetGC(), ctraits, info->declaringScope(), scopeTraits, scopeCount,
        nWithScopes, NULL, ctraits);
...
//
// The tricky issue here is to initialize the scope type chain and the scope
// chain; this works the same as for llNewClass.
//
// Again, we must do some initializations that would normally have been done by
// the verifier.
//

extern "C" ClassClosure *llNewFunction(MethodEnv *env, uint32_t methodindex,
                                       Traits **scopeTraitsPtrs,
                                       int nWithScopes, int scopeCount,
                                       Atom *scopes) {
  PoolObject *pool = env->method->pool();
  ...
  if (!declTraits) {
    ...
    const ScopeTypeChain *scopeTypeChain =
        ScopeTypeChain::create(core->GetGC(), core->traits.function_itraits,
                               env->method->declaringScope(), scopeTraits,
                               scopeCount, nWithScopes, NULL, NULL);
    ...
    if (atraits) {
      ...
      if (handler != NULL) {
        ...
        atraits->init = MethodInfo::create(
            core->GetGC(), MethodInfo::kInitMethodStub,
      ...
      if (scope == NULL) {
        scope =
            body->declaringScope()->cloneWithNewTraits(core->GetGC(), atraits);
...
//
// calling createArguments because it creates ArrayObject and sets callee in
// arguments
//
extern "C" ArrayObject *llCreateArgumentHelper(MethodEnv *env, Atom *ap,
                                               int argc) {
...
//
// Throws either an undefined pointer exception or a null pointer exceptiom,
// depending on the value of the atom.
//
extern "C" void llUPE(MethodEnv *env, Atom atom) {
  ...
  // Call nullcheck, which will always throw because we already know that atom
...
extern "C" double llMod(double x, double y) { return MathUtils::mod(x, y); }
...
extern "C" Traits *llGetVectorTraits(Traits *param_traits, Namespace *ns) {
  ...
  Traits *traits = core->domainMgr()->findTraitsInPoolByNameAndNS(
      param_traits->pool, fullName, ns);
...
namespace avmplus {
// hm-stubs calls these, but we shouldn't be including jit-calls.h in the aot
// runtime so backfill them here.
