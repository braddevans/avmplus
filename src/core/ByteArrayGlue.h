...
static inline uint8_t *ByteArrayBuffer_alloc(size_t count) {
    size_t size = MMgc::GCHeap::CheckForNewSizeOverflow(
            count, sizeof(uint8_t), /*canFail*/ false); // abort on bad size
    MMgc::FixedMalloc *const fm =
            MMgc::FixedMalloc::GetFixedMalloc(MMgc::kByteArrayPartition);
    return (uint8_t *) fm->Alloc(size);
}

static inline uint8_t *ByteArrayBuffer_alloc_CanFail(size_t count) {
    size_t size = MMgc::GCHeap::CheckForNewSizeOverflow(
            count, sizeof(uint8_t), /*canFail*/ true); // return 0 on bad size
    if (!size) {
        return NULL;
    }
    MMgc::FixedMalloc *const fm =
...
namespace avmplus {
    ...
    class ByteArray : public DataInput, public DataOutput {
    ...
    public:
        ...
        public:
            ...
            REALLY_INLINE uint8_t
            *

            getArray() const {
#ifdef AVMPLUS_64BIT
                if ((uint32_t(uint64_t(array) >> 32) ^
                     uint32_t(uint64_t(array) & 0xffffffff) ^ MMgc::GCHeap::secret) !=
                    check_array)
                  ByteArrayValidationError();
#else
                if ((uint32_t(array) ^ MMgc::GCHeap::secret) != check_array)
                    ByteArrayValidationError();
#endif
                return array;
            }
            ...
            REALLY_INLINE void setArray(uint8_t *a) {
                array = a;
#ifdef AVMPLUS_64BIT
                check_array = uint32_t(uint64_t(array) >> 32) ^
                              uint32_t(uint64_t(array) & 0xffffffff) ^
                              MMgc::GCHeap::secret;
#else
            ...
            getCapacity() const {
                if ((capacity ^ MMgc::GCHeap::secret) != check_capacity)
                    ByteArrayValidationError();
                return capacity;
            }
            ...
            getLength() const {
                if ((length ^ MMgc::GCHeap::secret) != check_length)
                    ByteArrayValidationError();
                return length;
            }
            ...
            REALLY_INLINE bool getCopyOnWrite() const {
                if ((copyOnWrite ^ MMgc::GCHeap::secret) != check_copyOnWrite)
                    ByteArrayValidationError();
        ...
        private:
            ...
            uint32_t
                    copyOnWrite; // effectively bool: 0x00000000 false, 0xffffffff true.
            // We expect only a small number of Buffer objects to be in
            // use at a time, and for their storage cost to be
            // dominated by the buffer data itself. There is little
            // reason to be concerned with the size of the metadata
            // defined here, so we use a separate checksum for each
            // field to lower the time for each check.
        ...
        operator[](uint32_t index) const {
            AvmAssert(index < m_buffer->getLength());
            return (index < m_buffer->getLength()) ? m_buffer->getArray()[index] : 0;
        }
        ...
        GetLength() const { return m_buffer->getLength(); }

        REALLY_INLINE bool IsShared() const {
            return m_isShareable && (m_buffer->RefCount() > 1);
        }

        /* Normally at this point hasCurrent() should always return TRUE. However, if
        ...
        Now despite the worker not having started we would do a requestSafepointTask()
        to enter into a safepoint for any action changing the buffer or length of the
        bytearray. So, with this new function, we would not issue a request for a
        safepoint task, if the worker hasn't started already. */

        REALLY_INLINE bool IsSharedAndWorkerStarted() const {
            return (m_isShareable && (m_buffer->RefCount() > 1) &&
                    vmbase::SafepointRecord::hasCurrent());
        }
    ...
    private:
        ...
        int32_t UnprotectedAtomicCompareAndSwapLength(int32_t expectedLength,
                                                      int32_t newLength);

    public:
        // Set the length to x+y, with overflow check.  If x+y overflows a uint32_t
        // then throw a MemoryError (same error that the one-argument variety will
        // throw if trying to create a buffer larger than the buffer limit, which is
        // less than 2^32-1).
        void FASTCALL
        SetLength(uint32_t
        x,
        ...
        // The pointer starts at offset zero (regardless of the value of
        // GetPosition()) and the data is guaranteed to be valid for GetLength()
        // bytes.
        //
        // *** USE THIS METHOD WITH EXTREME CAUTION, AND ONLY WHEN ABSOLUTELY
        // NECESSARY ***
        //
        // In particular, you must not use this method and const_cast the result, as
        // you may overwrite ByteArrays that are copy-on-write; please use
        // GetWritableBuffer instead if the data must be written to.
        //
        ...
        //
        REALLY_INLINE const uint8_t
        *

        GetReadableBuffer() const {
        ...
        // Make this ByteArray a copy-on-write ByteArray. All existing content will be
        // discarded and the new data referenced instead. "owner", if non-null, is a
        // GCObject that owns the data (a reference will be kept to it to ensure the
        // data isn't collected); if owner is null, it's assumed that the data will
        // never become invalid.
        void SetCopyOnWriteData(MMgc::GCObject *owner, const uint8_t *data,
                                uint32_t length);
        ...
        // return false.
        bool EnsureCapacityOrFail(uint32_t capacity, enum CatchAction,
                                  Exception **exn_recv);

        // overrides from DataInput
        /*virtual*/ uint32_t Available() {
            uint32_t length = m_buffer->getLength();
            return (m_position <= length) ? (length - m_position) : 0;
        }
        ...
        void EnsureCapacityAndWrite(uint32_t capacity, const void *buffer,
                                    uint32_t count);

        void UnprotectedEnsureCapacityAndWrite(uint32_t capacity, const void *buffer,
                                               uint32_t count);
        ...
        void setShareablePropReadOnly(bool value) {
            m_isShareablePropReadOnly = value;
        }
        ...
        // For use by ByteArrayObject for optimized read functions: check that there
        // are nbytes available to read at the current position, and throw an EOF
        // exception if not.  Update the position to point beyond that area.  Return a
        // pointer to the start of the area.
        //
        // Precondition: 0 < nbytes < 4096
        uint8_t *requestBytesForShortRead(uint32_t nbytes);

        // For use by ByteArrayObject for optimized write functions: check that there
        // are nbytes available to write at the current position, and extend the array
        // if not. Throw MemoryError if the array cannot be extended.  Update the
        // position to point beyond the write area.  Return a pointer to the start of
        // the area.
        //
        // Precondition: 0 < nbytes < 4096
    ...
    private:
        ...
        void RestoreOrigData(uint8_t *origData, bool origCopyOnWrite,
                             uint32_t origLen, uint32_t origCap, uint32_t origPos,
                             MMgc::GCObject *origCopyOnWriteOwner);

        void HandleErrorInUncompressFn(FixedHeapRef <Buffer> origBuffer,
        ...
        void CompressViaZlibVariant(CompressionAlgorithm algorithm, uint8_t *origData,
        ...
        void CompressViaLzma(uint8_t *origData, bool origCopyOnWrite,
                             uint32_t origLen, uint32_t origCap, uint32_t origPos,
                             MMgc::GCObject *origCopyOnWriteOwner);

        void UncompressViaLzma(uint8_t *origData, bool origCopyOnWrite,
                               uint32_t origLen, uint32_t origCap, uint32_t origPos,
                               MMgc::GCObject *origCopyOnWriteOwner);
    ...
    public: // Tasks need it
        ...
        public:
            Grower(ByteArray *owner, uint32_t minimumCapacity)
                    : m_owner(owner), m_oldArray(owner->m_buffer->getArray()),
            ...
            void FASTCALL
            ReallocBackingStore(uint32_t
            newCapacity,
            bool calledFromLengthSetter = false
            );
            void FASTCALL

            EnsureWritableCapacity(bool calledFromLengthSetter = false);
    ...
    private: // end FIXME
        ...
        REALLY_INLINE bool IsCopyOnWrite() const {
            return m_buffer->getCopyOnWrite();
        }
    ...
    private:
        ...
        // currently requires the object to be GCObject or GCFinalizedObject, not
        // RCObject; that's fine for all extant use cases.) (Note that the pointer is
        // written with explicit WB calls.)
...
//
    ...
    *vtable);
    ...
    uint32_t get_defaultObjectEncoding() const {
        return get__defaultObjectEncoding();
    }

    void set_defaultObjectEncoding(uint32_t version) {
        set__defaultObjectEncoding(version);
    }
...
void readBytes(ByteArrayObject * bytes, uint32_t
offset,
uint32_t length
);
void writeBytes(ByteArrayObject * bytes, uint32_t
offset,
uint32_t length
);
...
int32_t atomicCompareAndSwapIntAt(int32_t byteIndex, int32_t expectedValue,
                                  int32_t newValue);
...
// REALLY_INLINE helpers for fast reading and writing, without code
// duplication.
uint32_t read32();
...
float
readFloatHelper(uint8_t *b); // ...Helper because AOT hates overloaded names
