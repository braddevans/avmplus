...
// Second argument is metric name (reverse DNS notation is the recommended
// notation to avoid
//      name conflicts and for native metrics (i.e. not from ActionScript) the
//      metric name should start with '.' -- for example, a metric in VM could
//      be ".vm.core.internString").
// Third argument, if applicable, is the value of metric of any supported type
// (for TELEMETRY_VALUE,
//      expected type of arguments is any type allowed by WriteValue function in
//      ITelemetry class or union of all other macros with same number of
//      arguments; for other macros, the type may be of the type indicated by
//      the name of the macro).
//  For example,
//   char* internedString = "abc";
//   TELEMETRY_STRING(GetTelemetry(), ".vm.core.internString", internedString);
//   // Assuming GetTelemetry returns ITelemetry*.
//
...
// A microsecond timestamp (corresponding to the time of the call) would be
// written as a time metric
#define TELEMETRY_TIME(t, x)                                                   \
  do {                                                                         \
    if (t && t->IsActive()) {                                                  \
      t->WriteTime(x);                                                         \
...
// Put this at the beginning of a block of code and it will transmit the
// starting timestamp and duration from this until the block exits (this goes
// out of scope).
#define TELEMETRY_METHOD(t, x) telemetry::TelemetryMethod _tm(t, x, false)

#define TELEMETRY_METHOD_NO_THRESHOLD(t, x)                                    \
  telemetry::TelemetryMethod _tm(t, x, true)

// Use this, if multiple TELEMETRY_METHOD macros need to be used per block
// (specify variable name as the last argument).
#define TELEMETRY_METHOD_VAR(t, x, v) telemetry::TelemetryMethod v(t, x)

namespace telemetry {
typedef const char *telemetryId;

// It is recommended to use the above macros -- they are sufficient for most use
// cases.
class ITelemetry {
public:
  ...
  virtual uint64_t
  GetElapsedTime() = 0; // Get a microsecond timestamp since Telemetry start up.
  int32_t GetSpanNestLevel() {
    return m_spanNestLevel;
  } // Get the current span nesting level.

  virtual void WriteValue(telemetryId id,
                          bool value) = 0; // Writes a boolean metric.
  ...
  virtual void WriteValue(telemetryId id, const uint8_t *value,
                          uint32_t len) = 0; // Writes bytearray.
  virtual void WriteValue(telemetryId id, const uint32_t value[],
                          uint32_t len) = 0; // Writes unsigned integer array
  virtual void WriteSpan(telemetryId id, uint64_t start, bool force,
                         bool fromAS = false) = 0;
  virtual void WriteTime(
      telemetryId) = 0; // Writes a time metric with microsecond timestamp.
protected:
...
// Declare this on the stack and when the block exits, the destructor will log
// the time delta.
