...
//          m_name = Multiname containing namespace:name pair (marked as an
//          attribute) m_value = text value of the attribute
...
//     uri
//
//  Properties of the XML object can be either qualified (one associated
//  namespace) or unqualified.
//
...
typedef bool (*MissingEndTagProc)(String *parentName, String *tagText,
                                  Namespace *ns, Namespace *defaultNS);

// We're trying to support paired nodes where the first node gets a namespace
// from the default namespace.

// used when bugzilla598683 == false
static bool missingEndTagOld(String *parentName, String *tagText, Namespace *ns,
                             Namespace *defaultNS) {
  return parentName->Compare(*tagText, NODE_NAME_START,
                             tagText->length() - NODE_NAME_START) != 0 &&
         ns->getURI() == defaultNS->getURI();
}

// used when bugzilla598683 == true
static bool missingEndTagNew(String *parentName, String *tagText, Namespace *ns,
                             Namespace *defaultNS) {
  return !(parentName->Compare(*tagText, NODE_NAME_START,
                               tagText->length() - NODE_NAME_START) == 0 &&
           ns->getURI() == defaultNS->getURI());
...
XMLObject::XMLObject(XMLClass *type, Stringp str, Namespace *defaultNamespace)
  ...
  if (defaultNamespace) {
    ...
    Namespace *ns = core->internNamespace(core->newNamespace(
        core->kEmptyString->atom(), defaultNamespace->getURI()->atom()));
  ...
  MissingEndTagProc missingEndTag =
      bugzilla598683 ? missingEndTagNew : missingEndTagOld;
  ...
  while ((m_status = parser.getNext(tag)) == XMLParser::kNoError) {
    ...
    case XMLTag::kElementType: {
      ...
      if (tag.text->charAt(0) == '/') {
        ...
        // OPTIMIZEME: can toplevel->getDefaultNamespace() ever change during
        // this call? If not, extract at top of loop.
        if (!NodeNameEquals(tag.text, parentName, ns) &&
            missingEndTag(parentName, tag.text, ns,
                          toplevel->getDefaultNamespace())) {
          // If p == m_node, we are at the top of our tree and we're parsing the
          // fake "parent" wrapper tags around our actual XML text.  Instead of
          // warning about a missing "</parent>" tag, we instead complain about
          // the XML markup not being well-formed. (Emulating Rhino behavior)
          if (p == m_node)
            toplevel->throwTypeError(kXMLMarkupMustBeWellFormed);
          else
            toplevel->throwTypeError(kXMLUnterminatedElementTag, parentName,
                                     parentName);
        } else {
          // Catch the case where our input string ends with a bogus <parent>
          // tag
      ...
      } else // an opening tag
      {
        ...
        // Needs to happen after setting m_name->name so throw error can use
        // name in routine
        e->CopyAttributesAndNamespaces(core, toplevel, tag, publicNS);

        // Find a namespace that matches this tag in our parent chain.  If this
        // name is a qualified name (ns:name), we search for a namespace with a
        // matching prefix.  If is an unqualified name, we find the first empty
        // prefix name.
        Namespace *ns =
            pNewElement->FindNamespace(core, toplevel, tag.text, false);
...
bool XMLObject::NodeNameEquals(Stringp nodeName, Stringp parentName,
                               Namespace *parentNs) {
  ...
  if (parentNs && parentNs->hasPrefix()) {
    ...
    int totalLen =
        prefixLen + 1 + parentName->length(); // + 1 for ':' separator
  ...
  return parentName->Compare(*nodeName, NODE_NAME_START + prefixLen,
                             nodeNameLength - prefixLen) == 0;
}
...
Atom XMLObject::callProperty(const Multiname *multiname, int argc, Atom *argv) {
  ...
  if (f == undefinedAtom) {
    ...
    if (AvmCore::isXMLList(f) && !AvmCore::atomToXMLList(f)->_length() &&
        (hasSimpleContent())) {
      Stringp r0 = core->string(this->atom());
      return toplevel()->callproperty(r0->atom(), multiname, argc, argv,
                                      toplevel()->stringClass()->vtable);
    }
  }
...
Atom XMLObject::getMultinameProperty(const Multiname *name_in) const {
  ...
  if (!name.isAnyName() && !name.isAttr()) {
    ...
    if (nameString->parseIndex(index)) {
      ...
      // property number for the new XMLList is 0 which just returns this node.
  ...
  XMLListObject *xl = XMLListObject::create(
      core->GetGC(), toplevel->xmlListClass(), this->atom(), &name);
  ...
  if (name.isAttr()) {
    ...
    for (uint32_t i = 0; i < m_node->numAttributes(); i++) {
      ...
      // if (((n.[[Name]].localName == "*") || (n.[[Name]].localName ==
      // a.[[Name]].localName)) &&
  ...
  // step 5 - look through all the children for a match - [[length]] implies
  // length of children n isn't an attributeName so it must be a qname??
  //      for k = 0 to x.[[length]]-1
  //      if (n.localName = "*" and this[k].class == "element" and
  //      (this[k].name.localName == n.localName)
  //          and (!n.uri) or (this[k].class == "element) and (n.uri ==
  //          this[k].name.uri))) xl->_append (x[k]);
  ...
  for (uint32_t i = 0; i < m_node->numChildren(); i++) {
    ...
    //  if (n.localName = "*" OR this[k].class == "element" and
    //  (this[k].name.localName == n.localName) and (!n.uri) or (this[k].class
    //  == "element) and (n.uri == this[k].name.uri)))
    //      xl->_append (x[k]);
    if (name.matches(m2)) {
...
void XMLObject::setMultinameProperty(const Multiname *name_in, Atom V) {
  ...
  // step 4
  if (getClass() & (E4XNode::kText | E4XNode::kCDATA | E4XNode::kComment |
                    E4XNode::kProcessingInstruction | E4XNode::kAttribute))
    return;
  ...
  if (AvmCore::atomToXMLList(V)) {
    ...
    if ((src->_length() == 1) &&
        src->_getAt(0)->getClass() & (E4XNode::kText | E4XNode::kAttribute)) {
  ...
  if (m.isAttr()) {
    ...
    } else // step 7g
    {
      ...
      nonChildChanges(core->kattributeChanged, nameAtom,
                      (prior) ? prior->atom() : undefinedAtom);
    }
  ...
  bool primitiveAssign =
      ((!AvmCore::isXML(c) && !AvmCore::isXMLList(c)) && (!m.isAnyName()));
    ...
    if (m.matches(m2)) {
      ...
      if (i != -1) {
        ...
        if (notify && (was->getClass() == E4XNode::kElement)) {
          XMLObject *nd =
              XMLObject::create(core->GetGC(), toplevel->xmlClass(), was);
  ...
  if (i == -1) {
    ...
    if (primitiveAssign) {
      ...
      // We use m->namespaceCount here to choose to use the default xml
      // namespace name here for an unqualified prop access. For a qualified
      // access, there will be only one namespace
      Stringp name = m.getName();
      Namespace *ns;
      if (m.namespaceCount() == 1)
        ns = m.getNamespace();
  ...
  if (primitiveAssign) {
    ...
    XMLObject *target =
        (notify) ? XMLObject::create(core->GetGC(), xmlClass(), xi) : 0;
    ...
    for (int r = 0; notify && (r < count); r++) {
      ...
      if (ild->getClass() == E4XNode::kElement) {
        XMLObject *nd =
            XMLObject::create(core->GetGC(), toplevel->xmlClass(), ild);
    ...
    if (notify && count > 0) {
      XMLObject *nd =
          XMLObject::create(core->GetGC(), toplevel->xmlClass(), xi->_getAt(0));
      prior = nd->atom();
    }
    ...
    if (notifyNeeded(getNode())) {
      // The above _replace call may be used to insert new nodes at the end.
      // However, if a null is inserted the effect is as though nothing was
      // inserted.  Test for this case.
      if (m_node->_length() > (uint32_t)i) {
        XMLObject *xml =
            XMLObject::create(core->GetGC(), xmlClass(), m_node->_getAt(i));
        childChanges((prior) ? core->knodeChanged : core->knodeAdded,
                     xml->atom(), prior);
      }
    }
  }
  ...
  if (m.isAttr()) {
    ...
    while (j < m_node->numAttributes()) {
      ...
      if (m.matches(&m2)) {
        ...
        nonChildChanges(core->kattributeRemoved,
                        (name) ? name->atom() : undefinedAtom,
                        (val) ? val->atom() : undefinedAtom);
      } else {
        j++;
      }
    }
...
Atom XMLObject::getDescendants(const Multiname *name_in) const {
  ...
  XMLListObject *l =
      XMLListObject::create(core->GetGC(), toplevel->xmlListClass());
  ...
  for (uint32_t k = 0; k < _length(); k++) {
    ...
    XMLObject *co =
        XMLObject::create(core->GetGC(), toplevel->xmlClass(), child);
...
bool XMLObject::hasUintProperty(uint32_t index) const { return (index == 0); }
...
Atom XMLObject::_resolveValue() { return this->atom(); }

Namespace *
XMLObject::GenerateUniquePrefix(Namespace *ns,
...
// E4X 10.2, pg 29
void XMLObject::__toXMLString(PrintWriter &s, NamespaceList &AncestorNamespaces,
                              int indentLevel, bool includeChildren) const {
  AvmCore *core = this->core();
  ...
  // step 9/10 - add in our namespaces into ancestorNamespaces if there are no
  // conflicts
  ...
  if (thisNodesNamespace->getPrefix() == undefinedAtom) {
    ...
    thisNodesNamespace =
        GenerateUniquePrefix(thisNodesNamespace, AncestorNamespaces);
    AncestorNamespaces.add(thisNodesNamespace);
  }

  String *nsPrefix = core->string(thisNodesNamespace->getPrefix());

  // If any of this node's attribute's namespaces have an undefined prefix,
  // generate a new one
  for (uint32_t i = 0; i < m_node->numAttributes(); i++) {
  ...
  // This adds any NS that were added to our ancestor namespace list (from
  // origLength on up)
  ...
  bool bIndentChildren =
      ((_length() > 1) ||
       (firstChild->getClass() & ~(E4XNode::kText | E4XNode::kCDATA)));
  ...
  // We need to prune any namespaces with duplicate prefixes in our
  // AncestorNamespace array to prevent shadowing of similar namespaces.  Bug
  // 153363.
  ...
  //  </order>;
  //
  // The namespace for menuName should be output even though the identical
  // namespace was output for the top node.  (Since the item node is using an
  // incompatible namespace with the same prefix.)
  ...
  for (uint32_t i = 0; i < _length(); i++) {
    ...
    // Our __toXMLString call might have added new namespace onto our list.  We
    // don't want to save these new namespaces so clear them out here.
...
/*static*/ bool XMLObject::fixBugzilla444630(AvmCore *core) {
  ...
  // if the current BugCompatibility has this flag set, fix the bug (no
  // escaping). otherwise, legacy behavior dictates that the root SWF/ABC
  // dictates the bug; this is encapsulate in core->bugzilla444630.
...
Atom XMLObject::maybeEscapeChild(Atom child) {
  ...
  const BugCompatibility *bugCompatibility = core()->currentBugCompatibility();
  if (bugCompatibility->watson3548918) {
    E4XNode *c1 = AvmCore::atomToXML(child);
    if (c1) {
      E4XNode *p1 = c1->getParent();
      int i = c1->childIndex();
      // E4XNode to have class 'kAttribute' represents its value.
      // So do not treat is as a node.
      if (p1 && (i != -1))
        p1->_deleteByIndex(i);
    }
  }
...
XMLListObject *XMLObject::AS3_attribute(Atom arg) {
  ...
  // return [[get]](name)
  return AvmCore::atomToXMLList(
      getAtomProperty(toplevel()->ToAttributeName(arg)->atom()));
}
...
XMLListObject *XMLObject::AS3_attributes() {
  ...
  return AvmCore::atomToXMLList(
      getAtomProperty(toplevel()->ToAttributeName(core()->kAsterisk)->atom()));
}
...
int XMLObject::AS3_childIndex() { return m_node->childIndex(); }
...
XMLListObject *XMLObject::AS3_comments() {
  ...
  XMLListObject *l = XMLListObject::create(
      core->GetGC(), toplevel()->xmlListClass(), this->atom());
...
// E4X 13.4.4.11, pg 76
XMLObject *XMLObject::AS3_copy() { return _deepCopy(); }
...
/*Bug : 3951226 : when XMLObject/XMLListObject's function are called with Object
type variable from actionscript side we get Object type ATOM as this pointer.
Object is always handled as Atom in AOT.
therefore using this check to correctly finding out the object and call the
function.*/
  ...
  // !!@ spec talks about prototype object being different from regular XML
  // object
...
bool XMLObject::AS3_hasComplexContent() { return m_node->hasComplexContent(); }

// E4X 13.4.4.16, page 77
bool XMLObject::AS3_hasSimpleContent() { return m_node->hasSimpleContent(); }
...
Atom XMLObject::AS3_insertChildAfter(Atom child1, Atom child2) {
  ...
  if (getClass() &
      (E4XNode::kText | E4XNode::kComment | E4XNode::kProcessingInstruction |
       E4XNode::kAttribute | E4XNode::kCDATA))
    return undefinedAtom;
...
Atom XMLObject::AS3_name() {
  ...
  return QNameObject::create(core->GetGC(), toplevel()->qnameClass(), m)
      ->atom();
}
...
Atom XMLObject::_namespace(Atom p_prefix, int argc) // prefix is optional
{
  ...
  if (!argc) {
    // step 5a
    if (getClass() & (E4XNode::kText | E4XNode::kComment | E4XNode::kCDATA |
                      E4XNode::kProcessingInstruction))
      return nullObjectAtom;
...
ArrayObject *XMLObject::AS3_namespaceDeclarations() {
  ...
  for (uint32_t i = 0; i < m_node->numNamespaces(); i++) {
    ...
    } else // ns.prefix is NOT empty
    {
      ...
      for (uint32_t j = 0; j < ancestorNS.length(); j++) {
        Namespace *ns2 = ancestorNS.get(j);
        if (ns->getPrefix() == ns2->getPrefix() &&
            ns->getURI() == ns2->getURI()) {
          bMatch = true;
          break;
        }
      }
...
String *XMLObject::AS3_nodeKind() const { return m_node->nodeKind(toplevel()); }
...
XMLObject *XMLObject::AS3_normalize() {
  ...
  while (i < _length()) {
    ...
    if (x->getClass() == E4XNode::kElement) {
      XMLObject *xo =
          XMLObject::create(core->GetGC(), toplevel()->xmlClass(), x);
      xo->normalize();
      delete xo;
      i++;
    } else if (x->getClass() & (E4XNode::kText | E4XNode::kCDATA)) {
      Stringp prior = x->getValue();
      while (((i + 1) < _length()) && (m_node->_getAt(i + 1)->getClass() &
                                       (E4XNode::kText | E4XNode::kCDATA))) {
      ...
      /// Need to check if string is "empty" - 0 length or filled with
      /// whitespace
      ...
      if ((current != prior) && notify) {
        ...
        xo->nonChildChanges(core->ktextSet, current->atom(),
                            (prior) ? prior->atom() : undefinedAtom);
      }
    } else {
      i++;
...
Atom XMLObject::AS3_parent() {
  if (m_node->getParent())
    return XMLObject::create(core()->GetGC(), toplevel()->xmlClass(),
                             m_node->getParent())
        ->atom();
  else
    return undefinedAtom;
...
XMLListObject *
XMLObject::AS3_processingInstructions(Atom name) // name defaults to '*'
{
  AvmCore *core = this->core();
...
bool XMLObject::XML_AS3_propertyIsEnumerable(
    Atom P) // NOT virtual, not an override
{
  /*Bug : 3951226 : when XMLObject/XMLListObject's function are called with
  Object type variable from actionscript side we get Object type ATOM as this
  pointer. Object is always handled as Atom in AOT. therefore using this check
  to correctly finding out the object and call the function.*/
  AvmCore *core = NULL;
#ifdef VMCFG_HALFMOON_AOT_RUNTIME
  Atom thisAtom = (Atom)this;
  if (atomKind(thisAtom) == kObjectType) {
...
XMLObject *XMLObject::AS3_removeNamespace(Atom nsAtom) {
  ...
  Namespace *ns = core->isNamespace(nsAtom) ? AvmCore::atomToNamespace(nsAtom)
                                            : core->newNamespace(nsAtom);
  ...
  for (uint32_t k = 0; k < _length(); k++) {
    ...
    if (p->getClass() == E4XNode::kElement) {
      XMLObject *xo =
          XMLObject::create(core->GetGC(), toplevel()->xmlClass(), p);
...
XMLObject *XMLObject::AS3_replace(Atom P, Atom value) {
  ...
  QNameObject *qn1 =
      QNameObject::create(core->GetGC(), toplevel->qnameClass(), P);
  Multiname m;
  qn1->getMultiname(m);
  bool notify = notifyNeeded(getNode());
  int i = -1;
...
void XMLObject::AS3_setLocalName(Atom name) {
  if (m_node->getClass() &
      (E4XNode::kText | E4XNode::kComment | E4XNode::kCDATA))
    return;
  ...
  if (this->getNode()->getQName(&m, publicNS)) {
    ...
    nonChildChanges(core->knameSet, m.getName()->atom(),
                    (prior) ? prior->atom() : undefinedAtom);
  }
  return;
}
...
void XMLObject::AS3_setName(Atom name) {
  ...
  QNameObject *n =
      QNameObject::create(core->GetGC(), toplevel()->qnameClass(), name);
...
void XMLObject::AS3_setNamespace(Atom ns) {
  ...
  if (m_node->getClass() & (E4XNode::kText | E4XNode::kComment |
                            E4XNode::kProcessingInstruction | E4XNode::kCDATA))
    return;
...
XMLListObject *XMLObject::AS3_text() {
  XMLListObject *l =
      XMLListObject::create(gc(), toplevel()->xmlListClass(), this->atom());
...
Stringp XMLObject::toString() {
  ...
  if (hasSimpleContent()) {
    ...
    for (uint32_t i = 0; i < _length(); i++) {
      E4XNode *child = m_node->_getAt(i);
      if ((child->getClass() != E4XNode::kComment) &&
          (child->getClass() != E4XNode::kProcessingInstruction)) {
        XMLObject *xo =
            XMLObject::create(core->GetGC(), toplevel()->xmlClass(), child);
...
Stringp XMLObject::AS3_toString() { return toString(); }
...
PrintWriter &
XMLObject::printUsingAncestors(PrintWriter &prw,
                               NamespaceList &AncestorNamespaces) const {
  //
...
int XMLObject::getClass() const { return m_node->getClass(); }

uint32_t XMLObject::_length() const { return m_node->_length(); }
...
void XMLObject::setValue(Stringp s) { m_node->setValue(s); }

Stringp XMLObject::getValue() { return m_node->getValue(); }

bool XMLObject::getQName(Multiname *m) { return m_node->getQName(m, publicNS); }
...
void XMLObject::childChanges(Stringp type, Atom value, E4XNode *prior) {
  ...
  if (notifyNeeded(initialTarget)) {
    XMLObject *target =
        XMLObject::create(core->GetGC(), top->xmlClass(), initialTarget);
    ...
    if (AvmCore::isXML(value)) {
      issueNotifications(core, top, initialTarget, target->atom(), type, value,
                         detail);
    } else if (AvmCore::isXMLList(value)) {
      // if its a list each element in the list is added.
      XMLListObject *xl = AvmCore::atomToXMLList(value);
      if (xl) {
        issueNotifications(core, top, initialTarget, target->atom(), type,
                           xl->atom(), detail);
      } else {
        AvmAssert(false);
      }
 ...
 */
void XMLObject::issueNotifications(AvmCore *core, Toplevel *top,
                                   E4XNode *initialTarget, Atom target,
                                   Stringp type, Atom value, Atom detail) {
  ...
  while (node) {
    ...
    if (methodObj) {
      XMLObject *currentTarget =
          XMLObject::create(core->GetGC(), top->xmlClass(), node);
      Atom argv[6] = {top->atom(),  currentTarget->atom(),
                      type->atom(), target,
                      value,        detail};
      int argc = 5;

      // EnterScriptTimeout enterScriptTimeout(core);
      TRY(core, kCatchAction_Rethrow) { methodObj->call(argc, argv); }
...
XMLListObject *XMLObject::filter(Atom propertyName, Atom value) {
  ...
  XMLListObject *l = XMLListObject::create(
      core()->gc, toplevel()->xmlListClass(), nullObjectAtom);
...
void XMLObject::_filter(XMLListObject *l, const Multiname &name, Atom value) {
  ...
  for (uint32_t i = 0; i < m_node->numChildren(); i++) {
    ...
    if (name.matches(m2)) {
      // If we're an element node, we do something more complicated than a
      // string compare
      if (child->getClass() == E4XNode::kElement) {
...
void XMLObject::dispose() { m_node->dispose(); }
...
/*override*/ uint64_t XMLObject::bytesUsed() const {
  if (getSampler()->getSamplerType() == Sampler::SAMPLER_TYPE_AVMPLUS) {
    ...
    // For newer samplers, we report the bytes used only for this XMLObject, not
    // for the underlying E4XNodes. We will report the memory used for E4XNodes
    // individually, eliminating problems with reporting the same memory more
    // than once as described above.
 ...
 * An AttributeName is simply a QName wrapper for finding properties that have a
 * leading @ sign. It's an internal class to the spec and the only difference
 * between a QName is the @.  Instead of having the overhead of an AttributeName
 * class that wraps the QName class, we just use a boolean inside the QName to
 * differentiate betweent the two types.
 */
QNameObject::QNameObject(QNameClass *factory, Namespace *ns, Atom nameatom,
                         bool bA)
...
QNameObject::QNameObject(QNameClass *factory, Atom nameatom, bool bA)
  ...
  if (AvmCore::isQName(nameatom)) {
    ...
    if (name == core->kAsterisk) {
      ...
      Namespacep ns = core->getVersionedNamespace(
          toplevel->getDefaultNamespace(), core->getApiVersionFromCallStack());
      mn.setNamespace(ns);
    }
  }
...
Atom QNameObject::get_uri() const { return getURI(); }

// E4X 13.3.5.4, pg 69
Namespace *XMLObject::GetNamespace(const Multiname &mn,
                                   const NamespaceList *nsArray) const {
  AvmCore *core = this->core();
