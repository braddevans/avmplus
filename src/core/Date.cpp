...
// TODO(vtl): Move stuff to a "glue" file, so that avmplus doesn't potentially
// depend on Flash core.
...
static double DayFromYear(double year) {
  return (365 * (year - 1970) + MathUtils::floor((year - 1969) / 4) -
...
// NOTE: this is used by the Player core code. Changing this will change legacy
// behavior.
int YearFromTime(double t) {
  ...
  while (lo < hi) {
    ...
    // 13may04 grandma :
    // This was pivot = (lo + hi) / 2, but bug 89715 inadvertantly calls this
    // with t = -6.5438017398347670e+019, which produces lo = -2075023950 and hi
    // = -2069354479, and (lo + hi) overflows. The below expression won't
    // overflow
...
double UTC(double t) {
#if defined(PEPPER_PLUGIN)
  // TODO(vtl): This is just a clone of the code below; we should probably
  // provide our own implementations of |VMPI_...()|.
  double adj = pepper::GetTimeZoneAdjustment();
  double dstAdjust = pepper::GetDaylightSavingTimeAdjustmentForTime(t - adj);
  if (dstAdjust != 0) {
    double dst2 =
        pepper::GetDaylightSavingTimeAdjustmentForTime(t - adj - kMsecPerHour);
    if (dst2 == 0)
      t += kMsecPerHour;
  }
...
static int HourFromTime(double t) {
  int result = (int)MathUtils::mod(MathUtils::floor((t + 0.5) / kMsecPerHour),
                                   kHoursPerDay);
  if (result < 0) {
    result += kHoursPerDay;
  }
...
static double DayFromMonth(double year, double month) {
  ...
  return DayFromYear((int)year) +
         kMonthOffset[(int)IsLeapYear((int)year)][iMonth];
}

static int MinFromTime(double time) {
  int result = (int)MathUtils::mod(MathUtils::floor(time / kMsecPerMinute),
                                   kMinutesPerHour);
  if (result < 0) {
...
static int SecFromTime(double time) {
  int result = (int)MathUtils::mod(MathUtils::floor(time / kMsecPerSecond),
                                   kSecondsPerMinute);
  if (result < 0) {
...
double MakeDate(double day, double time) {
  ...
  if (MathUtils::isInfinite(day) || MathUtils::isInfinite(time) || day != day ||
      time != time)
    return MathUtils::kNaN;
...
double MakeTime(double hour, double min, double sec, double ms) {
  // if any value is not finite, return NaN
  if (MathUtils::isInfinite(hour) || MathUtils::isInfinite(min) ||
      MathUtils::isInfinite(sec) || MathUtils::isInfinite(ms) || hour != hour ||
  ...
  return hour * (double)kMsecPerHour + min * (double)kMsecPerMinute +
         sec * (double)kMsecPerSecond + ms;
}

double MakeDay(double year, double month, double date) {
  // if any value is not finite, return NaN
  if (MathUtils::isInfinite(year) || MathUtils::isInfinite(month) ||
      MathUtils::isInfinite(date) || year != year || month != month ||
      date != date)
    return MathUtils::kNaN;
...
Date::Date() { m_time = VMPI_getDate(); }

Date::Date(double year, double month, double date, double hours, double min,
           double sec, double msec, bool utcFlag) {
  if (year < 100) {
    year += 1900;
  }
  m_time =
      MakeDate(MakeDay(year, month, date), MakeTime(hours, min, sec, msec));
  if (!utcFlag) {
    m_time = UTC(m_time);
  }
}

void Date::format(wchar *buffer, const char *format, ...) const {
  va_list ap;
  ...
  while (*format) {
    if (*format == '%') {
      ...
      case 'd': {
        ...
        char *intptr = MathUtils::convertIntegerToStringBuffer(
            value, intbuf, len, 10, MathUtils::kTreatAsSigned);
        while (*intptr) {
          *buffer++ = *intptr++;
        }
      } break;
...
bool Date::toString(wchar *buffer, int formatIndex, int &len) const {
  ...
  if (MathUtils::isNaN(m_time)) {
    UnicodeUtils::Utf8ToUtf16((const uint8_t *)"Invalid Date", 12, buffer, 12,
                              true);
    buffer[len = 12] = 0;
    return true;
  }
  ...
  if (month < 0 || month >= 12 || day < 0 || day >= 7) {
    ...
    // The bug demonstrates the scenario when we early without initializing
    // 'len' which might result in further actions being performed on the wrong
    // length.
    len = 0;
    return false;
  }
  ...
  switch (formatIndex) {
    ...
    /   format to make it easier to write code which parses dates in string
    format /   regardless of the implementation which produced it. /
    http://bugzilla.mozilla.org/show_bug.cgi?id=118266 /   We are not required
    by the standard to follow suit, but the one of the goals /   of compliance
    is easy porting of code / techniques from ecmascript.  This change / should
    be done in a AS2/AS3 conditional manner, however:

    // CN: 1/8/05 well, maybe this does break some existing user code (like the
    ATS).  Since
    //  we aren't required to match Spidermonkey by the ES3 spec, lets decide to
    break
    ...
    */
  case kToString:
    format(buffer, "%3 %3 %d %2:%2:%2 GMT%c%2%2 %d", dayOfWeekStr, monthStr,
           date, hour24, min, seconds, signChar, deltaH, deltaM, year);
    break;
  case kToLocaleString:
    format(buffer, "%3 %3 %d %d %2:%2:%2 %cM", dayOfWeekStr, monthStr, date,
           year, hour12, min, seconds, ampm);
    break;
  case kToUTCString:
    format(buffer, "%3 %3 %d %2:%2:%2 %d UTC", dayOfWeekStr, monthStr, date,
           hour24, min, seconds, year);
    break;

    /*  This would be the SpiderMonkey / IE format (well, logically at least in
    that /   toString() == toDateString() + toTimeString().  See long comment
    above...
  ...
  case kToLocaleDateString:
    format(buffer, "%3 %3 %d %d", dayOfWeekStr, monthStr,
           (int)DateFromTime(time), (int)YearFromTime(time));
    break;
  case kToTimeString:
    format(buffer, "%2:%2:%2 GMT%c%2%2", hour24, min, seconds, signChar, deltaH,
           deltaM);
    break;
  case kToLocaleTimeString:
    format(buffer, "%2:%2:%2 %cM", hour12, min, seconds, ampm);
    break;
  default:
    return false;
  }
...
void Date::setTime(double value) { m_time = TimeClip(value); }

// To not change a particular value, pass NaN.
void Date::setTime(double hours, double min, double sec, double msec,
                   bool utcFlag) {
...
void Date::setDate(double year, double month, double date, bool utcFlag) {
  double t = utcFlag ? m_time : LocalTime(m_time);
  ...
  // cn 2/14/06  Not sure whent this was added, but its not ECMAScript
  // compatible.
