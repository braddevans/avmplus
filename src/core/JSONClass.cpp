...
class JSONSerializer {
  ...
  JSONSerializer(Toplevel *toplevel, Namespace *as3ns, ArrayObject *proplist,
                 FunctionObject *replacer, String *gap);
...
private:
  ...
  ReturnCondition Str(String *key, ScriptObject *holder, String *pendingPrefix,
                      bool pendingPropnameColon);
  ReturnCondition Str(uint32_t key, ScriptObject *holder, String *pendingPrefix,
                      bool pendingPropnameColon);
  ReturnCondition Str(Multiname const *key, ScriptObject *holder,
                      String *pendingPrefix, bool pendingPropnameColon);
  ...
  ReturnCondition StrFoundValue(Atom val,
                                ...
                                ScriptObject *holder, String *pendingPrefix,
                                bool pendingPropnameColon);
  ...
  bool TryToplevelCall(Atom *method, int32_t argc, Atom *args, Atom *recv) {
  ...
  bool TryAS3Call(FunctionObject *f, Atom *thisAtom, Atom *args, int32_t argc,
                  Atom *recv) {
    TRY(core(), kCatchAction_Rethrow) {
  ...
  ReturnCondition JODescriptions(ScriptObject *names, ScriptObject *value,
                                 String *pendingPre, String *connective);
  ReturnCondition JOProp(Atom p, ScriptObject *value, String *pendingPrefix);
  ReturnCondition JOPropMultiname(Atom p, ScriptObject *value,
...
private:
  ...
  void committedToEmitFor(Atom key, String *pending, bool pendingPropnameColon);
  ...
  struct Rope {
    struct Chunk {
      ...
      REALLY_INLINE int32_t min(int32_t x, int32_t y) {
        return (x < y) ? x : y;
      }
      ...
      REALLY_INLINE int32_t emit(bufelem_t const *src, int32_t len) {
        ...
        memcpy(&m_payload[m_cursor], src, write_amt * sizeof(bufelem_t));
        m_cursor += write_amt;
        return write_amt;
      }
    };
    ...
    Rope(MMgc::FixedMalloc *fm) : m_fixedmalloc(fm), m_len(0) {
...
JSONClass::JSONClass(VTable *cvtable) : ClassClosure(cvtable) {
...
JSONParser::JSONParser(Toplevel *toplevel, String *text)
    : m_toplevel(toplevel), m_text(text), m_textUTF8(text), m_i(0),
      m_len(uint32_t(m_textUTF8.length())), m_indexValidForText(true),
      m_token('\0'), m_value(NULL) {}

// If the reviver is null then the parser returns null, Boolean,
// Number/int/uint, String, Array, or Object.
...
// ISSUE: AS3 has a richer notion of number than ECMAScript, and for numbers in
// the int range we could return an int value rather than a Number value.  There
// could be a performance impact in some cases.  At present we return a Number
// value (actually the result of parseFloat).
...
void JSONParser::advance() {
  ...
  while (m_i < m_len) {
    ...
    case '9':
      ...
      return;
    case 'n':
      if (m_i + 3 < m_len && m_textUTF8.c_str()[m_i + 1] == 'u' &&
    ...
    case 't':
      if (m_i + 3 < m_len && m_textUTF8.c_str()[m_i + 1] == 'r' &&
    ...
    case 'f':
      if (m_i + 4 < m_len && m_textUTF8.c_str()[m_i + 1] == 'a' &&
          m_textUTF8.c_str()[m_i + 2] == 'l' &&
...
void JSONParser::adv_number() {
  ...
  if (m_indexValidForText)
    ...
    m_value = m_toplevel->core()->newStringUTF8(&m_textUTF8.c_str()[start],
                                                m_i - start);
  ...
  while (m_i < m_len) {
    ...
    if (c == 92) { // backslash
      if (m_indexValidForText)
        ...
        buffer = buffer->append(m_toplevel->core()->newStringUTF8(
      ...
      case '\\':
        ...
        break;
      case 'b':
        buffer = buffer->appendLatin1("\b");
        break;
      case 'f':
        buffer = buffer->appendLatin1("\f");
        break;
      case 'n':
        buffer = buffer->appendLatin1("\n");
        break;
      case 'r':
        buffer = buffer->appendLatin1("\r");
        break;
      case 't':
        buffer = buffer->appendLatin1("\t");
        break;
      case 'u':
        m_i++;
        wchar d1, d2, d3, d4;
        if (m_i + 3 < m_len && isHexDigit(d1 = m_textUTF8.c_str()[m_i]) &&
            isHexDigit(d2 = m_textUTF8.c_str()[m_i + 1]) &&
            isHexDigit(d3 = m_textUTF8.c_str()[m_i + 2]) &&
            isHexDigit(d4 = m_textUTF8.c_str()[m_i + 3])) {
          wchar charcode =
              wchar((hexDigitValue(d1) << 12) + (hexDigitValue(d2) << 8) +
                    (hexDigitValue(d3) << 4) + (hexDigitValue(d4)));
...
JSONSerializer::JSONSerializer(Toplevel *toplevel, Namespace *as3ns,
                               ArrayObject *proplist, FunctionObject *replacer,
                               String *gap)
    : m_toplevel(toplevel), m_as3ns(as3ns), m_proplist(proplist),
      m_replacerFunction(replacer), m_gap(gap), m_exception(NULL),
      m_fixedmalloc(
          MMgc::FixedMalloc::GetFixedMalloc(MMgc::kJSONSerializerPartition)),
...
REALLY_INLINE void JSONSerializer::emit(char c) { m_rope.emit(c); }
...
REALLY_INLINE void
JSONSerializer::committedToEmitFor(Atom key, String *pending,
                                   bool pendingPropnameColon) {
  emit(pending);
...
bool JSONSerializer::isVectorInstance(ScriptObject *o) {
  ...
  return (traits->subtypeof(core()->traits.vectorobj_itraits) ||
...
ReturnCondition JSONSerializer::Str(uint32_t key, ScriptObject *holder,
                                    String *pendingPrefix,
                                    bool pendingPropnameColon) {
  Atom value;
  value = holder->getUintProperty(key);
  return StrFoundValue(value, core()->uintToAtom(key), holder, pendingPrefix,
                       pendingPropnameColon);
}

ReturnCondition JSONSerializer::Str(String *key, ScriptObject *holder,
                                    String *pendingPrefix,
                                    bool pendingPropnameColon) {
  Atom value;
  value = holder->getAtomProperty(key->atom());
  return StrFoundValue(value, key->atom(), holder, pendingPrefix,
                       pendingPropnameColon);
}

ReturnCondition JSONSerializer::Str(Multiname const *name, ScriptObject *holder,
                                    String *pendingPrefix,
                                    bool pendingPropnameColon) {
  ...
  return StrFoundValue(value, name->getName()->atom(), holder, pendingPrefix,
                       pendingPropnameColon);
}

bool JSONSerializer::stackOverflowCheck(){
    TRY(core(), kCatchAction_Rethrow){core()->stackCheck(m_toplevel);
return false;
} // namespace avmplus
CATCH(Exception *exception) {
  m_exception = exception;
...
ReturnCondition JSONSerializer::StrFoundValue(Atom value, Atom key,
                                              ScriptObject *holder,
                                              String *pendingPrefix,
                                              bool pendingPropnameColon) {
  ...
  case kSpecialBibopType:
    if (value != undefinedAtom) {
      if (AvmCore::isFloat(value)) {
        ...
        // OPTIMIZEME: It would be better if this didn't cons a new object with
        // a new dynamic property table every time it needed to format a float4.

        float4_t f = AvmCore::atomToFloat4(value);
        const float *floats = reinterpret_cast<const float *>(&f);
        ScriptObject *obj =
            m_toplevel->builtinClasses()->get_ObjectClass()->construct();
        const String *names[] = {core()->kx, core()->ky, core()->kz,
                                 core()->kw};
        for (int i = 0; i < 4; i++) {
          Atom val = m_toplevel->isFinite(NULL, floats[i])
                         ? core()->floatToAtom(floats[i])
                         : core()->knull->atom();
...
void JSONSerializer::Quote(String *value) {
  ...
  while (i < len) {
    ...
    default: // control character => \u####
      ...
      ptr = MathUtils::convertIntegerToStringBuffer(
          c + 0x10000, (char *)int_conv_buf, buflen, 16,
...
bool JSONSerializer::hasTransientMetadata(ScriptObject *description) {
  Atom metadata = description->getAtomProperty(propName("metadata"));
  ...
  if (array != NULL) {
    ...
    for (uint32_t i = 0; i < len; i++) {
      ...
      ScriptObject *entry = AvmCore::atomToScriptObject(entryAtom);
      Atom entryNameAtom = entry->getAtomProperty(propName("name"));
      if (AvmCore::isString(entryNameAtom)) {
        String *entryNameString = core()->atomToString(entryNameAtom);
...
ReturnCondition JSONSerializer::JODescriptions(ScriptObject *descs,
  ...
  if (descs != NULL) {
    ...
    for (index = descs->nextNameIndex(0); index != 0;
         index = descs->nextNameIndex(index)) {
      Atom nextValue = descs->nextValue(index);
      ScriptObject *description = AvmCore::atomToScriptObject(nextValue);
      Atom access = description->getAtomProperty(propName("access"));
      if (core()->atomToString(access)->charAt(0) == 'r') {
        if (hasTransientMetadata(description))
          continue;
        Atom name = description->getAtomProperty(propName("name"));
...
ReturnCondition JSONSerializer::JO(ScriptObject *value) {
  ...
  if (m_proplist != NULL) {
    ...
    uint32_t flags =
        (TypeDescriber::HIDE_OBJECT | TypeDescriber::INCLUDE_VARIABLES |
         TypeDescriber::INCLUDE_ACCESSORS | TypeDescriber::INCLUDE_TRAITS |
         TypeDescriber::INCLUDE_BASES | TypeDescriber::INCLUDE_METADATA);
    ScriptObject *valueDescription =
        typeDescriber.describeType(value->atom(), flags);

    if (valueDescription != NULL) {
      ScriptObject *traitsDescription = AvmCore::atomToScriptObject(
      ...
      ScriptObject *varsDescriptions = AvmCore::atomToScriptObject(
          traitsDescription->getAtomProperty(propName("variables")));
      ret = JODescriptions(varsDescriptions, value, pendingPre, connective);
      RET_EXN_CHECK(ret);
      if (ret == kSomeOutput) {
        pendingPre = connective;
        emittedSomething = true;
      ...
      ScriptObject *gettersDescriptions = AvmCore::atomToScriptObject(
          traitsDescription->getAtomProperty(propName("accessors")));
      ret = JODescriptions(gettersDescriptions, value, pendingPre, connective);
      RET_EXN_CHECK(ret);
      if (ret == kSomeOutput) {
        pendingPre = connective;
        emittedSomething = true;
...
ReturnCondition JSONSerializer::JOProp(Atom p, ScriptObject *value,
                                       String *pendingPrefix) {
  String *pName;
  ReturnCondition ret;
...
ReturnCondition JSONSerializer::JOPropMultiname(Atom p, ScriptObject *value,
                                                String *pendingPrefix) {
