...
ArrayClass::ArrayClass(VTable *cvtable) : ClassClosure(cvtable) {
  ...
  setPrototypePtr(
      ArrayObject::create(core->GetGC(), ivtable, objectPrototype, 0));

  // According to ECMAscript spec (ECMA-262.pdf)
  // generic support: concat, join, pop, push, reverse, shift, slice, sort,
  // splice, unshift NOT generic: toString, toLocaleString unknown: sortOn (our
  // own extension)
}

static ArrayObject *toArray(Atom instance) {
  return AvmCore::isObject(instance)
             ? AvmCore::atomToScriptObject(instance)->toArrayObject()
             : NULL;
}
...
it returns an array containing the array elements of the object followed by the
array elements of each argument in order. The following steps are taken:
...
this value be an Array object. Therefore it can be transferred to other kinds of
objects for use as a method. Whether the concat function can be applied
successfully to a host object is implementation-dependent.
*/

/*static*/ void ArrayClass::array_concat(Toplevel * /*toplevel*/,
                                         ArrayObject *a, ArrayObject *b) {
  if (!a->try_concat(b)) {
...
/*static*/ ArrayObject *ArrayClass::generic_concat(Toplevel *toplevel,
                                                   Atom thisAtom,
                                                   ArrayObject *args) {
  ScriptObject *d = AvmCore::isObject(thisAtom)
 ...
 */
/*static*/ Atom ArrayClass::generic_pop(Toplevel * /*toplevel*/,
                                        Atom thisAtom) {
  ArrayObject *a = toArray(thisAtom);
 ...
 * TRANSFERABLE: Needs to support generic objects as well as Array objects
 */
/*static*/ Atom ArrayClass::generic_reverse(Toplevel * /*toplevel*/,
                                            Atom thisAtom) {
  ArrayObject *a = toArray(thisAtom);
  if (a && a->try_reverse()) {
    return thisAtom;
 ...
 * TRANSFERABLE: Needs to support generic objects as well as Array objects
 */
/*static*/ Atom ArrayClass::generic_shift(Toplevel * /*toplevel*/,
                                          Atom thisAtom) {
  ArrayObject *a = toArray(thisAtom);
 ...
 * TRANSFERABLE: Needs to support generic objects as well as Array objects
 */
/*static*/ ArrayObject *ArrayClass::generic_slice(Toplevel *toplevel,
                                                  Atom thisAtom, double A,
                                                  double B) {
  if (!AvmCore::isObject(thisAtom))
    return 0;
...
public:
  ...
  /**
   * Array.sortOn() will pass an array of field names, which is converted to a
   * stack- allocated array of FieldName structures (hence no write barriers
   * here).
   */
  struct FieldName {
    String *name;
  ...
  static int StringCompareFunc(const ArraySort *s, uint32_t j, uint32_t k) {
    return s->StringCompare(j, k);
  }
  static int CaseInsensitiveStringCompareFunc(const ArraySort *s, uint32_t j,
                                              uint32_t k) {
    return s->CaseInsensitiveStringCompare(j, k);
  }
  static int ScriptCompareFuncCompatible(const ArraySort *s, uint32_t j,
                                         uint32_t k) {
    return s->ScriptCompareCompatible(j, k);
  }
  static int ScriptCompareFuncCorrect(const ArraySort *s, uint32_t j,
                                      uint32_t k) {
    return s->ScriptCompareCorrect(j, k);
  }
  static int NumericCompareFuncCompatible(const ArraySort *s, uint32_t j,
                                          uint32_t k) {
    return s->NumericCompareCompatible(j, k);
  }
  static int NumericCompareFuncCorrect(const ArraySort *s, uint32_t j,
                                       uint32_t k) {
    return s->NumericCompareCorrect(j, k);
  }
  static int DescendingCompareFunc(const ArraySort *s, uint32_t j, uint32_t k) {
    return s->altCmpFunc(s, k, j);
  }
  static int FieldCompareFunc(const ArraySort *s, uint32_t j, uint32_t k) {
    return s->FieldCompare(j, k);
  }
...
public:
  ...
  ArraySort(Atom &result, ArrayClass *f, ScriptObject *d, int options,
            CompareFuncPtr cmpFunc, CompareFuncPtr altCmpFunc,
            Atom cmpActionScript, uint32_t numFields = 0,
            FieldName *fields = NULL);
...
private:
  ...
  int compare(uint32_t lhs, uint32_t rhs) const {
    return cmpFunc(this, lhs, rhs);
  }
...
ArraySort::ArraySort(Atom &result, ArrayClass *f, ScriptObject *d, int options,
                     CompareFuncPtr cmpFunc, CompareFuncPtr altCmpFunc,
                     Atom cmpActionScript, uint32_t numFields,
                     FieldName *fields)
    : d(d), core(f->core()), toplevel(f->toplevel()), options(options),
      cmpFunc(cmpFunc), altCmpFunc(altCmpFunc),
      cmpActionScript(cmpActionScript), index_autoptr(), index(NULL),
      atoms(NULL), numFields(numFields), fields(fields), fieldatoms(NULL) {
  uint32_t len = d->getLengthProperty();
  uint32_t iFirstAbsent = len;

  // new class[n] compiles into code which tries to allocate n * sizeof(class).
  // unfortunately, the generated assembly does not protect against this product
  // overflowing int. So I limit the length -- for larger values, I expect new
  // will fail anyways.
  if ((len > 0) && (len < (0x10000000))) {
    index = (uint32_t *)avmStackAlloc(
        core, index_autoptr,
        GCHeap::CheckForCallocSizeOverflow(len, sizeof(uint32_t)));
  ...
  if (!index || !atoms) {
    ...
    // could also fallback to an unindexed sort, but with that many elements, it
    // won't finish for hours
    result = d->atom();
    return;
  }
  ...
  if (cmpFunc == ArraySort::FieldCompareFunc && numFields == 1) {
    ...
    // note, loop needs to go until i = -1, but i is unsigned. 0xffffffff is a
    // valid index, so check (i+1) != 0.
    ...
    bool isNumericCompare =
        (cmpFunc == ArraySort::NumericCompareFuncCompatible) ||
    ...
    for (i = (len - 1), j = len; (i + 1) != 0; i--) {
      ...
      // We want to throw if this is an Array.NUMERIC sort and any items are not
      // numbers, and not strings that can be converted into numbers
      if (isNumericCompare && !core->isNumberOrFloat(atoms->list.get(i))) {
        ...
        if (MathUtils::isNaN(val))
          ...
          toplevel->throwTypeError(
              kCheckTypeFailedError,
              core->atomToErrorString(atoms->list.get(i)),
              core->toErrorString(core->traits.number_itraits));
      }

      // getAtomProperty() returns undefined when that's the value of the
      // property. It also returns undefined when the object does not have the
      // property.
      //
      ...
      // come after that.
      //
      // To simplify our compare, partitition the array into { defined |
      // undefined | missing } Note that every missing element shrinks the
      // length -- we'll set this new length at the end of this routine when we
      // are done.
  ...
  // The portion of the array containing values undefined is now [j,
  // iFirstAbsent). The portion of the array containing absent values is now
  // [iFirstAbsent, len).
  ...
  if (options & kReturnIndexedArray) {
    ...
    // our atoms array so the below code works on the right data. Fieldatoms
    // contain our original objects while atoms contain our objects[field]
    // values for faster sorting.
...
void ArraySort::qsort(uint32_t lo, uint32_t hi) {
  ...
  // leave without doing anything if the array is empty (lo > hi) or only one
  // element (lo == hi)
  if (lo >= hi)
    return;
...
recurse:
  ...
  if (size < 4) {
    ...
    //  1. left partition   : i in [lo, right), elements less than or equal to
    //  pivot
    //  2. center partition : i in [right, left], elements equal to pivot
    //  3. right partition  : i in (left, hi], elements greater than pivot
    // NOTE : [ means the range includes the lower bounds, ( means it excludes
    // it, with the same for ] and ).
    ...
    // the left is empty, the center is just the pivot, and the right is
    // everything else.
    //
    // If you recurse into the smaller partition first, then the worst case is
    // an equal partitioning, which leads to a depth of log2(size).
...
int ArraySort::ScriptCompareCompatible(uint32_t j, uint32_t k) const {
  ScriptObject *o =
      (ScriptObject *)AvmCore::atomToScriptObject(cmpActionScript);
...
int ArraySort::ScriptCompareCorrect(uint32_t j, uint32_t k) const {
  ...
  //  ((value % 2^32) - 2^31), which could change the intended sign of the
  //  number.
...
int ArraySort::NumericCompareCompatible(uint32_t j, uint32_t k) const {
  ...
  if (atomIsBothIntptr(atmj, atmk)) {
    // This is incorrect, see bugzilla 524122.  NumericCompareCorrect, below,
    // fixes the bug.
...
inline int ArraySort::FieldCompare(uint32_t lhs, uint32_t rhs) const {
  ...
  for (uint32_t i = 0; i < numFields; i++) {
    ...
    opt =
        fields[i].options; // override the group defaults with the current field
    ...
    if (!(def_x && def_y)) {
      ...
      if (def_y) {
        ...
        bool has_x = (toplevel->getBinding(obj_j->vtable->traits, &mname) !=
                      BIND_NONE) ||
                     obj_j->hasStringProperty(name);
        bool has_y = (toplevel->getBinding(obj_k->vtable->traits, &mname) !=
                      BIND_NONE) ||
                     obj_k->hasStringProperty(name);
...
// This takes a args object because there is no way to distinguigh between
// sort() and sort(undefined, 0) if we take default parameters.
/*static*/ Atom ArrayClass::generic_sort(Toplevel *toplevel, Atom thisAtom,
                                         ArrayObject *args) {
    ...
    if (AvmCore::isObject(arg0)) {
      ...
      if (args->getLength() >= 2) {
        ...
        if (core->isNumberOrFloat(arg1)) {
          ...
          // throw exception (not a Number)
          toplevel->throwTypeError(
              kCheckTypeFailedError, core->atomToErrorString(arg1),
              core->toErrorString(core->traits.number_itraits));
        }
      }
      ...
      toplevel->throwTypeError(
          kCheckTypeFailedError, core->atomToErrorString(arg0),
          core->toErrorString(core->traits.function_itraits));
    }
  }
  ...
  Atom result;
  ArraySort sort(result, toplevel->arrayClass(), d, opt, compare, altCompare,
                 cmp);
 ...
 * TRANSFERABLE: Needs to support generic objects as well as Array objects
 */
/*static*/ Atom ArrayClass::generic_sortOn(Toplevel *toplevel, Atom thisAtom,
                                           Atom namesAtom, Atom optionsAtom) {
  AvmCore *core = toplevel->core();
  if (!AvmCore::isObject(thisAtom))
    return undefinedAtom;
  ScriptObject *d = AvmCore::atomToScriptObject(thisAtom);
  ...
  if (toplevel->builtinClasses()->get_StringClass()->isType(namesAtom)) {
    ...
    fn = (ArraySort::FieldName *)avmStackAllocArray(
        core, fn_autoptr, nFields, sizeof(ArraySort::FieldName));
    ...
    if (toplevel->builtinClasses()->get_ArrayClass()->isType(optionsAtom)) {
      ArrayObject *obj =
          (ArrayObject *)AvmCore::atomToScriptObject(optionsAtom);
      uint32_t nOptions = obj->getLength();
      if (nOptions == nFields) {
        // The first options are used for uniqueSort and returnIndexedArray
        // option
  ...
  Atom result;
  ArraySort sort(result, toplevel->arrayClass(), d, options,
                 ArraySort::FieldCompareFunc, NULL, undefinedAtom, nFields, fn);
  return result;
}
...
// splice(org arg) coerce the input to a number (otherwise it's zero) normal
// behavior splice (two args) - coerce both args to numbers (otherwise they are
// zero)
/*static*/ ArrayObject *ArrayClass::generic_splice(Toplevel *toplevel,
                                                   Atom thisAtom,
                                                   ArrayObject *args) {
  ...
  uint32_t insertPoint = NativeObjectHelpers::ClampIndex(
      AvmCore::toInteger(args->getUintProperty(0)), len);

  double d_deleteCount = args->getLength() > 1
                             ? AvmCore::toInteger(args->getUintProperty(1))
                             : (len - insertPoint);
  uint32_t deleteCount =
      (d_deleteCount < 0) ? 0 : AvmCore::integer_d(d_deleteCount);
  ...
  long l_shiftAmount =
      (long)insertCount -
      (long)deleteCount; // long because result could be negative
  uint32_t shiftAmount;

  ArrayObject *a = toArray(thisAtom);
  ArrayObject *out;
  if (a && (out = a->try_splice(insertPoint, insertCount, deleteCount, args,
                                2)) != NULL) {
    return out;
  }
  ...
  if (l_shiftAmount < 0) {
    ...
    // delete top elements here to match ECMAscript spec (generic object
    // support)
    ...
    for (uint32_t i = len;
         i > end;) { // Note: i is unsigned, can't check if --i >=0.
      --i;
      d->setUintProperty(i + shiftAmount, d->getUintProperty(i));
    }
  }
...
ArrayObject *ArrayClass::newarray(Atom *argv, int argc) {
  return ArrayObject::createSimple(core()->GetGC(), ivtable(), prototypePtr(),
                                   argv, argc);
}
...
ArrayObject *ArrayClass::newArray(MethodEnv *env, ADT argDesc,
                                  va_list_wrapper ap) {
  uint32_t argc = argDescArgCount(argDesc);
  // removed assert, argc is unsigned!
  return ArrayObject::createSimple<ADT>(core()->GetGC(), ivtable(),
                                        prototypePtr(), env, argDesc, argc, ap);
}

template ArrayObject *ArrayClass::newArray(MethodEnv *env, uint32_t argDesc,
                                           va_list_wrapper ap);
template ArrayObject *ArrayClass::newArray(MethodEnv *env, char *argDesc,
                                           va_list_wrapper ap);
#endif

/*static*/ int ArrayClass::generic_indexOf(Atom thisAtom, Atom searchElement,
                                           int startIndex) {
  if (!AvmCore::isObject(thisAtom))
    return -1;
...
/*static*/ int ArrayClass::generic_lastIndexOf(Atom thisAtom,
                                               Atom searchElement,
                                               int startIndex) {
  if (!AvmCore::isObject(thisAtom))
    return -1;
...
/*static*/ bool ArrayClass::generic_every(Toplevel *toplevel, Atom thisAtom,
  ...
  AvmCore *core = toplevel->core();
  for (uint32_t i = 0; i < len; i++) {
    // If thisObject is null, the call function will substitute the global
    // object. args are modified in place by callee
...
/*static*/ ArrayObject *ArrayClass::generic_filter(Toplevel *toplevel,
                                                   Atom thisAtom,
                                                   ScriptObject *callback,
                                                   Atom thisObject) {
  ...
  for (uint32_t i = 0; i < len; i++) {
    ...
    // If thisObject is null, the call function will substitute the global
    // object args are modified in place by callee
    Atom args[4] = {thisObject, element,
                    core->uintToAtom(i), // index
                    thisAtom};
    Atom result = callback->call(3, args);
    if (result == trueAtom)
...
/*static*/ void ArrayClass::generic_forEach(Toplevel *toplevel, Atom thisAtom,
                                            ScriptObject *callback,
                                            Atom thisObject) {
  if (!AvmCore::isObject(thisAtom) || !callback)
    return;
...
/*static*/ bool ArrayClass::generic_some(Toplevel *toplevel, Atom thisAtom,
                                         ScriptObject *callback,
                                         Atom thisObject) {
  if (!AvmCore::isObject(thisAtom) || !callback)
    return false;
...
/*static*/ ArrayObject *ArrayClass::generic_map(Toplevel *toplevel,
                                                Atom thisAtom,
                                                ScriptObject *callback,
                                                Atom thisObject) {
  ArrayObject *r = toplevel->arrayClass()->newArray();
...
/* static */ uint32_t ArrayClass::generic_unshift(Toplevel * /*toplevel*/,
                                                  Atom thisAtom,
                                                  ArrayObject *args) {
  ArrayObject *a = toArray(thisAtom);
...
/*static*/
ScriptObject *FASTCALL
ArrayClass::createUnsubclassedInstanceProc(ClassClosure *cls) {
  return new (cls->gc(), MMgc::kExact, cls->getExtraSize())
      ArrayObject(cls->ivtable(), cls->prototypePtr(),
                  /*capacity = */ 0,
                  /*simple   = */ true);
}
