 ...
 *
 *  46<<16|16   Content recognized by Flash Player "Spicy" and earlier (SWF <=
 * 11) 47<<16|12   Content recognized by Flash Player TBD and earlier (SWF <=
 * 12) 47<<16|13   Content recognized by Flash Player TBD and earlier (SWF <=
 * 13)
 *  ...
 *
 ...
 */
    class AbcParser {
    public:
        AbcParser(AvmCore *core, ScriptBuffer code, Toplevel *toplevel,
    ...
    protected:
        ...
        Traits *parseTraits(uint16_t sizeofInstance, uint16_t offsetofSlots,
                            Traits *base, Namespacep ns, Stringp name,
                            MethodInfo *script, TraitsPosPtr traitsPos,
                            TraitsPosType posType, Namespacep protectedNamespace,
                            bool makeFinal = true);
        ...
        /**
         * Reads in 2 bytes and turns them into a 16 bit number.  Always reads in 2
         * bytes.  Currently only used for version number of the ABC file and for
         * version 11 support.
         */
        int readU16(const uint8_t *p) const;

        /**
         * Read in a 32 bit number that is encoded with a variable number of bytes.
         * The value can take up 1-5 bytes depending on its value.  0-127 takes 1
         * byte, 128-16383 takes 2 bytes, etc. The scheme is that if the current
         * uint8_t has the high bit set, then the following uint8_t is also part of
         * the value.
         *
         * Returns the value, and the 2nd argument is set to the number of bytes that
         * were read to get that value.
         */
    ...
    private:
        ...
        // Since we expect most OSR threshold attributes to be the same on all
        // methods, cache the result of converting the string value to a number to
        // save conversions.
        Stringp cachedOsrThresholdString; // Pointer to interned string for threshold
        // value
        uint32_t cachedOsrThresholdValue; // Numeric value corresponding to the string
        // above
        // ------------------------ DATA SECTION END
    };
