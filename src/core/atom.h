...
namespace avmplus {
// Atom should really be an intptr_t, but doing so can cause problematic
// compiles because some platforms define intptr_t as an int64_t, and some as a
// long, which create different overload possibilities in a few cases. Ideally,
// Atom should be a unique pointer type (as it is in TT) but for now, avoid the
// code churn by defining it the "old" way
//
...
#else
    typedef AVMPLUS_TYPE_IS_POINTER_SIZED int32_t
    Atom;
#endif
...
/*@}*/
        ...
        const static uint32_t kBibopBlockSize =
                4096; // This must be the same as GCHeap::kBlockSize, we check that in
        // atom.cpp
...
/*@{*/
        ...
        const Atom kBibopTypePtrMask = ~(Atom)(
                kBibopBlockSize -
                1); // mask for bibop type pointer (char*, first char from the bibop page)
/*@}*/
...
// the VM will *never* return this value to calling code; the VM *may or may
// not* accept this value as a valid Atom if it is passed in from calling code.
// Caveat emptor.
        const Atom atomNotFound = Atom(0);
    } // namespace AtomConstants

// atomMinIntValue is the smallest negative (most negative) integer value
// representable in a native word, and atomMaxIntValue is the largest positive
// integer value thus representable, in both cases constrained such that
// interchanging the value to and from a 'double' can be done without loss of
// precision.
//
//  - On 32-bit systems an intptr_t is 32 bits wide so the range is -2^28 ..
//  2^28-1 (29 bit range).
//
//  - On 64-bit systems an intptr_t is 64 bits wide so the value is actually
//  constrained by
//    the available bits in a double.  There are 52 bits of unsigned mantissa
//    represented explicitly, an implicit leading 1 bit, and a sign bit.  This
//    gives a total of 54 bits in a signed-magnitude representation, which would
//    ordinarily allow a range of -2^53-1 .. 2^53-1.  Due to the implicit
//    leading 1 bit, however, larger powers of 2 can be represented with an
//    appropriate exponent. As a result, the contiguous range of
//    exactly-representable integers is actually extended to -2^53 .. 2^53. When
//    working with integer values in intptr_t form, however, we use a
//    twos-complement representation, in which 54 bits provides a range of -2^53
//    .. 2^53-1.  We represent all integers outside this range as doubles. Being
//    slightly conservative in this way allows us to verify that an integer
//    value is in range merely by verifying that it fits in 54 bits in the
//    native twos-complement form, without explicit comparison with the bounds.
//    These bounds should be used consistently, so that all integer values have
//    a canonical representation as atoms. NOTE: There is no absolute
//    requirement that integers be in canonical form, and correctness must never
//    depend on it.  For performance reasons, we sometimes roll over to the
//    double-float  representation at 32 bits in order to exploit a cheaper
//    test.  See https://bugzilla.mozilla.org/show_bug.cgi?id=601426#c6 .
//
// atomSignExtendShift is the number of bits to shift an untagged intptr_t left
// and then right in order to properly sign extend it.  (I don't like the 'atom'
// prefix but it fits in with the pattern of the others.)
...
// In debug builds bibopKind() is a more elaborate inline function in
// atom-inlines.h.
#ifndef DEBUG
...
// if the atom is not kIntptrType, assert
    bool atomCanBeUint32(Atom
    atom);

// getters to obtain the value.
// these getters require that the type of the Atom be *exactly* what is
// requested; no coercion is done (e.g., calling getDouble() on a Atom
// containing TypeInt will assert).
    intptr_t atomGetIntptr(Atom
    atom);

// if the given intptr can fit into an kIntptrType atom, return true.
// if not (the magnitude is too large/small), return false.
// WARNING: do not pass an (unsigned) uintptr_t value; you may get incorrect
// results.
    bool atomIsValidIntptrValue(const intptr_t i);

// if the given uintptr_t can fit into an kIntptrType atom, return true.
// if not (the magnitude is too large/small), return false.
// WARNING: do not pass a (signed) intptr_t value; you may get incorrect
// results.
