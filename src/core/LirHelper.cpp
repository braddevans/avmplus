...
LirHelper::LirHelper(PoolObject *pool)
    : pool(pool), core(pool->core), alloc1(mmfx_new(Allocator())),
...
#ifdef VMCFG_FLOAT
  case BUILTIN_float:
    // always allocFloat; we don't currently have the option to represent a
    // float as part of the atom, like doubles/numbers can
  ...
  case BUILTIN_int:
    if (native->isImmI()) {
      ...
      if (atomIsValidIntptrValue(val))
        return InsConstAtom(atomFromIntptrValue(val), native->isTainted());
    }
  ...
  case BUILTIN_uint:
    if (native->isImmI()) {
      ...
      if (atomIsValidIntptrValue_u(val))
        return InsConstAtom(atomFromIntptrValue_u(val), native->isTainted());
    }
...
LIns *LirHelper::atomToNative(BuiltinType bt, LIns *atom) {
  ...
  case BUILTIN_float4:
    if (atom->isImmP()) {
      ...
      return lirout->insImmF4(f4val, atom->isTainted());
    } else {
...
void LirHelper::emitStart(Allocator &alloc, LirBuffer *lirbuf,
                          LirWriter *&lirout) {
  (void)alloc;
  (void)lirbuf;
  debug_only(
      // catch problems before they hit the writer pipeline
      lirout = validate1 = new (alloc) ValidateWriter(
          lirout, lirbuf->printer, "emitStart");) lirout->ins0(LIR_start);

  // create params for saved regs -- processor specific
  for (int i = 0; i < NumSavedRegs; i++) {
    LIns *p = lirout->insParam(i, 1);
    (void)p;
    verbose_only(if (lirbuf->printer) lirbuf->printer->lirNameMap->addName(
        p, regNames[REGNUM(RegAlloc::savedRegs[i])]);)
  }
}
...
LIns *LirHelper::stForTraits(Traits *t, LIns *val, LIns *p, int32_t d,
                             AccSet accSet) {
  switch (bt(t)) {
  case BUILTIN_number:
...
#ifdef VMCFG_FLOAT
LIns *LirHelper::BasicLIREmitter::operator()(enum BuiltinType bt, LIns *oper1,
                                             LIns *oper2) const {
  switch (bt) {
  case BUILTIN_number:
    return _lh->binaryIns(_dblOp, oper1, oper2);
  case BUILTIN_any:
    if (_call)
      return _lh->callIns(_call, 3, _lh->coreAddr, oper1,
                          oper2); // else, fall through - a call is unexpected
  default:
    return TODO("Unexpected builtin type InstructionEmiter, binary op");
  }
}

LIns *LirHelper::BasicLIREmitter::operator()(enum BuiltinType bt,
                                             LIns *oper) const {
  switch (bt) {
  case BUILTIN_number:
    return _lh->lirout->ins1(_dblOp, oper);
  case BUILTIN_any:
    if (_call)
      return _lh->callIns(_call, 2, _lh->coreAddr,
                          oper); // else, fall through - a call is unexpected
  default:
    return TODO("Unexpected builtin type InstructionEmiter, unary op");
  }
}

LIns *LirHelper::ModuloLIREmitter::operator()(enum BuiltinType bt, LIns *op1,
                                              LIns *op2) const {
  switch (bt) {
  case BUILTIN_number:
    return _lh->callIns(FUNCTIONID(mod), 2, op1, op2);
  case BUILTIN_any:
    return _lh->callIns(FUNCTIONID(op_modulo), 3, _lh->coreAddr, op1, op2);
  default:
    return TODO("Unexpected builtin type in ModuloLIREmitter");
  }
}

LIns *LirHelper::IncrementLIREmitter::operator()(enum BuiltinType bt,
                                                 LIns *oper) const {
  switch (bt) {
  case BUILTIN_number:
    return _lh->binaryIns(LIR_addd, oper,
                          _lh->lirout->insImmD(_shouldDecrement ? -1 : 1));
  case BUILTIN_any: {
    /* Note: we add with 1.0f/-1.0f, to make sure that  the results stays float
     * if operand is float */
    const CallInfo *addfunc = _lh->pool->hasFloatSupport()
                                  ? FUNCTIONID(op_add)
                                  : FUNCTIONID(op_add_nofloat);
    return _lh->callIns(addfunc, 3, _lh->coreAddr, oper,
                        _lh->InsConstAtom(_shouldDecrement
                                              ? _lh->core->kFltMinusOne
                                              : _lh->core->kFltOne));
  }
  default:
    return TODO("Unexpected builtin type in IncrementLIREmitter");
  }
}
...
#ifdef NJ_VERBOSE
void LInsPrinter::formatGuard(InsBuf *, LIns *) { AvmAssert(false); }
void LInsPrinter::formatGuardXov(InsBuf *, LIns *) { AvmAssert(false); }
...
// Always returns requested storage upon normal return.
// OOM abort occurs if space is not available.
void *Allocator::allocChunk(size_t size) { return mmfx_alloc(size); }

void Allocator::freeChunk(void *p) { mmfx_free(p); }
...
void *CodeAlloc::allocCodeChunk(size_t nbytes) {
  if (!sHandle) {
    ...
    if (sHandle) {
      ...
      if (!mprotect(sStart, sEnd - sStart, PROT_READ | PROT_WRITE | PROT_EXEC))
        sCur = sStart;
    } else {
...
bool CodeAlloc::checkChunkMark(void *addr, size_t nbytes, bool isExec) {
  ...
  for (uintptr_t n = start; b && n <= last; n += psize) {
    ...
    kern_return_t err = vm_region_64(mach_task_self(), &vmaddr, &vmsize,
                                     VM_REGION_BASIC_INFO_64,
                                     (vm_region_info_t)&inf, &infoCnt, &port);
...
void ValidateWriter::checkAccSet(LOpcode op, LIns *base, int32_t disp,
  ...
  // not enough to check base == xxx , since cse or lirbuffer may split
  // (ld/st,b,d) into (ld/st,(addp,b,d),0)
  ...
  switch (accSet) {
  case avmplus::ACCSET_VARS:
    ok = isVars;
    break;
  case avmplus::ACCSET_TAGS:
    ok = isTags;
    break;
  case avmplus::ACCSET_OTHER:
    ok = isUnknown;
    break;
  default:
  ...
  if (!ok) {
    ...
    VMPI_snprintf(b2.buf, b2.len, "but the base pointer (%s) doesn't match",
                  b1.buf);
