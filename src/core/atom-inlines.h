...
// Macros for these are defined in atom.h.  Measurements show that macros have
// an edge over inline functions with Visual C++ 2008, at least.
//
// inline AtomConstants::AtomKind atomKind(Atom a) { return
// AtomConstants::AtomKind(uintptr_t(a) & 7); } inline void* atomPtr(Atom a) {
// return (void*)(uintptr_t(a) & ~7); }
    ...
    REALLY_INLINE bool atomIsBothIntptr(Atom a, Atom b) {
        return ((((uintptr_t(a) ^ kIntptrType) | (uintptr_t(b) ^ kIntptrType)) & 7) ==
                0);
    }
...
atomFromIntptrValue(const intptr_t i) {
...
atomFromIntptrValue_u(const uintptr_t u) {
...
REALLY_INLINE avmplus::ScriptObject
*
atomObj(Atom
a) {
AvmAssert (avmplus::testIsObject(a)); // proper type and not null or undefined
return (avmplus::ScriptObject *)(
uintptr_t(a)
- kObjectType);
}

REALLY_INLINE bool atomGetBoolean(Atom
a) {
AvmAssert(a
== trueAtom || a == falseAtom);
return (a >> 3) != 0;
}
