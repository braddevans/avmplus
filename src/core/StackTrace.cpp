...
namespace avmplus {
#ifdef DEBUGGER
void CallStackNode::init(MethodEnv *env, FramePtr framep,
                         const uint8_t *frame_sst, intptr_t volatile *eip) {
  AvmAssert(env != NULL);
  ...
  m_next = m_core->callStack;
  m_core->callStack = this;
  m_fakename = NULL;
