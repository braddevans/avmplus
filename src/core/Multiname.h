...
/**
 * Multiname is a reference to an identifier in 0 or more namespaces.  It
 * consists of the simple name and a list of namespaces.
 *
 * Multinames are always stack allocated.  Namespace and name values are
 * interened, since they are used as hashtable keys.
 */
    class Multiname {
        const static int32_t ATTR = 0x01; // attribute name
        const static int32_t QNAME =
                0x02; // qualified name (size==1, explicit in code)
    ...
    public:
        ...
        PrintWriter &print(PrintWriter &prw,
                           MultiFormat form = MULTI_FORMAT_FULL) const;

        PrintWriter &printName(PrintWriter &prw) const;

        static PrintWriter &print(PrintWriter &prw, Namespacep ns, Stringp name,
                                  bool attr = false,
                                  bool hideNonPublicNamespaces = true);
        ...
        public:
            ...
            Format(const Multiname *mn)
                    : _ns(mn->getNamespace()), _name(mn->getName()) {}
...
//
// NOTE NOTE NOTE
// Special care must be taken if this object is not embedded within the first 4K
// of a GC object, because write barriers must be handled differently.  See
// comments to setMultiname below and comments on the destructor in
// Multiname.cpp.
    ...
    public:
        ...
        // object is not within the first block of the parent object.  See bugzilla
        // 525875.
        void setMultiname(MMgc::GC *gc, const void *container,
                          const Multiname &other);
