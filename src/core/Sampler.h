...
namespace avmplus {
    ...
    //
    // note that for efficiency, the Toplevel* in the String/Namespace case is
    // filled in lazily downstream (via sotSetToplevel), to avoid having
    // String/Namespace ctors look up the proper Toplevel when not sampling.
    //
    // To reinforce the fact that this Isn't An Atom, it's now its own opaque type,
    // with lovely little accessors. Please use them now, rather than casting to
    // intptr and dancing on the bits directly.
    //
    ...
    REALLY_INLINE Toplevel *sotGetToplevel(SamplerObjectType sot) {
      AvmAssert(sotGetKind(sot) == kSOT_String ||
                sotGetKind(sot) == kSOT_Namespace);
      return (Toplevel *)(uintptr_t(sot) & ~7);
    }

    REALLY_INLINE SamplerObjectType sotSetToplevel(SamplerObjectType sot,
                                                   Toplevel *toplevel) {
    ...
    // return value will be a VTable* or a Toplevel* -- intended only for use in
    // adding GC work items
    ...
    struct Sample {
      ...
      union {
        ...
        struct {
          ...
          uint32_t depth;
          // Beginning of an array of StackTraceElement.  Basically, an MethodInfo*,
          // Stringp, Stringp, uint32_t for each entry.
          void *trace;
        } stack;
      ...
      // Following three fields are only filled in for sampleType==NEW_OBJECT_SAMPLE
      // or NEW_AUX_SAMPLE They are not present in the sample stream for other
      // sample types
    ...
    // The abstract interface for a memory sampler. Use the AttachSampler() method
    // to attach an IMemorySampler based class to MMgc for getting called back when
    // allocations/deallocations occur.
    class IMemorySampler {
    public:
      // A unique ID for identifying the different memory samplers.
      ...
      virtual void recordNewObjectAllocation(AvmPlusScriptableObject *obj,
                                             avmplus::SamplerObjectType sot) = 0;

      // Called when a ScriptObject based object has been reallocated (a back buffer
      // it owns has grown). The object passed in is the dependent object for the
      // ScriptObject. It can be of any type.
      virtual void recordObjectReallocation(const void *obj) = 0;

      // Call to add a dependent object. A dependent object is one that should be
      // watched for reallocations in recordObjectReallocation(). A dependent object
      // can be of any type.
      virtual void addDependentObject(const avmplus::AvmPlusScriptableObject *obj,
                                      const void *depObj) = 0;

      // Called when a new object of the given type has been allocated. For generic
      // recording of any object type.
      virtual void recordNewObjectAllocation(void *obj, const char *objectType,
                                             size_t size) = 0;

      virtual ~IMemorySampler() {}
    };

    // Set the current memory sampler. A memory sampler is tied to a particular
    // GC/core pair.
    void AttachSampler(IMemorySampler *sampler);

    // Get the current memory sampler. Global utility function, should only be used
    // when we don't already have a reference to the current GC. If the GC is known,
    // then GetAttachedSampler should be called on the GC object.
    IMemorySampler *GetSampler();

    class Sampler : public MMgc::GCRoot, public IMemorySampler {
    public:
      enum { SAMPLER_TYPE_AVMPLUS = 1 };
      ...
      int getSamplerType() { return SAMPLER_TYPE_AVMPLUS; }
      void recordAllocation(const void *item, size_t size);
      void recordDeallocation(const void *item, size_t size);
      void recordNewObjectAllocation(AvmPlusScriptableObject *obj,
                                     avmplus::SamplerObjectType sot);
      void recordObjectReallocation(const void *) {}
      void recordNewObjectAllocation(void *, const char *, size_t) {}
      void addDependentObject(const avmplus::AvmPlusScriptableObject *,
                              const void *){};
      uint64_t recordAllocationInfo(AvmPlusScriptableObject *obj,
                                    SamplerObjectType sot);
      uint64_t recordAllocationSample(const void *item, uint64_t size,
    ...
    private:
      ...
      void rewind(uint8_t *&b, uint32_t amount) { b -= amount; }
    ...
    private:
      ...
      uint64_t lastSampleCheckMicros; // the last time we considered writing a timer
                                      // sample
      GCMember<ScriptObject> callback;
      uintptr_t timerHandle;
      MMgc::GCHashtableBase<uint64_t, MMgc::GCHashtableKeyHandler,
                            MMgc::GCHashtableAllocHandler_VMPI>
          uids; // important to use the VMPI variant for non-MMGC-based memory
                // allocation.
      MMgc::GCHashtable_VMPI ptrSamples; // important to use the VMPI variant for
                                         // non-MMGC-based memory allocation.
      ...
      bool autoStartSampling; // if true we call startSampling as early as possible
                              // during startup
      bool _sampling;         // are we sampling at all
      // ------------------------ DATA SECTION END
    };

#define SAMPLE_FRAME(_strp, _core)                                             \
  avmplus::CallStackNode __fcsn((avmplus::AvmCore *)_core, _strp)
...
namespace telemetry {
/* An interface that lets the VM call into the host when it's time to take a
   sample. See the sampler description in AvmCore.h for more details. */
struct ISampler {
  virtual void takeSample() = 0;
};

/* An opaque handle that represents a function on a call stack. A given function
   should have the same handle for the entire duration of a run. */
