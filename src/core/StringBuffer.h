...
namespace avmplus {
/**
 * StringBuffer is a PrintWriter subclass which enables easy, cout-like output
 * of text to a string buffer in memory.
 *
 * NOTE!  This is *not* a GC class.  A StringBuffer instance *must* be allocated
 * on the stack, inside GCRoots, or inside GC'd objects (as a member, not a
 * pointer).
 *
 ...
 */
    ...
    public:
        StringBuffer(AvmCore *core) : PrintWriter(core), m_stream(core->gc) {
    ...
    private:
        /**
         * StringOutputStream accumulates any data written to the stream into an
         * in-memory buffer. The buffer can later be retrieved using the c_str method.
         *
         * NOTE!  This is *not* a GC object.  You *must* ensure that the destructor of
         * this object is called in order to properly deallocate resources.
         *
         * lhansen made StringOutputStream a private class of StringBuffer on
         * 2011-02-26 because StringBuffer is the only client of StringOutputStream in
         * the entire Flash Player source tree.
         */
