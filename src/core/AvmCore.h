...
#define VECTOROBJ_TYPE (core->traits.vectorobj_itraits)
    ...
    enum VB_Bits {
        ...
        VB_builtins =
        1 << 31 // display output for builtins (default is to ignore any builtins)
        ,
        ...
        ,
        VB_raw = 1 << 24 // native code (via nanojit) is displayed in raw (i.e
        // unbuffered bottom-up) fashion.
        ,
        VB_lircfg = 1 << 23 // produce a control-flow graph of any resultant lir (gml
        // format) in extended basic blocks
        ,
    ...
    struct JitConfig {
      // Generate inline fastpaths for arithmetic and conversions, introducing new
      // control flow. Distinct from non-speculative inlining and specialization.
      bool opt_inline;
      // Generate speculative fastpath for read of a simple array.
      // NOTE: Default is false, as this does not appear to be profitable when smash
      // protection checks are included.
      ...
      JitConfig()
          : opt_inline(true), opt_array_read_fastpath(false),
            opt_inline_vector_access(true) {}
    };
#endif
    ...
    struct Config {
         ...
         * the method or traversal of a backward branch therein, triggering
         * compilation when the counter reaches zero.
         *
         ...
         * overrides via the ExecPolicy attribute.  This provides a way to
         * definitively suppress OSR even for content that chooses to opt-in via
         * ExecPolicy, thus facilitating certain diagnostic scenarios.
         */
        bool osr_enabled;
...
// Use this for picking apart and putting together double values from
// constituent words.  "words.msw" is the most significant word, containing sign
// bit, exponent, and most significant bits of mantissa.  "words.lsw" is the
// least significant word, containing the least significant bits of mantissa.
// "bits" are two words you can use if you don't care about the word order or if
// you're constructing a double from a machine representation to which you only
// have a byte pointer. All words are native-endianness.
//
// (Note the structure tag 'words' is required, the Solaris C++ compiler does
// not support unnamed structures as of May, 2010.
...
// A single entry in the regex cache.  Sits inside a GCRoot.
    ...
    private:
        RegexCacheEntry()
                : timestamp(0), hits(0), optionFlags(0), global(false),
...
// Cache replacement is LRU based on a simple timestamping mechanism.  There's
// also a mechanism to disable the cache if the number of entries that are used
// just once significantly outweigh the uses of the entries that are used more
// than once, that mechanism cuts the overhead of cache maintenance for programs
// that don't benefit from the cache.
    ...
    public:
         ...
         */
        RegexCacheEntry &findCachedRegex(bool &found, String *source,
                                         String *options);
    ...
    private:
        ...
        static const uint64_t wastedWorkMultiplier =
                10; // Gut feeling: utilization should be above 10%
        static const uint64_t warmupTicks =
                1000; // Number of ticks before we start worrying about hit rate

        RegexCacheEntry m_entries[cacheSize]; // Cache storage
        uint64_t m_timestamp;                 // Timestamp value for LRU
        uint64_t
                m_wasted; // Utility of wasted cache entries (sum of usage counts == 1)
        uint64_t
                m_useful;    // Utility of useful cache entries (sum of usage counts > 1)
 ...
 *
 * NOTE the allocation code in AvmCore::createBugCompatibility assumes this
 * structure is pointer-free and allocates it as pointer-free memory (bugzilla
 * 596529). Subclassers should consider doing the same.
 */
    ...
    public:
        enum Version {
            ...
            kSWF19, // SWF19 (Flash Player 11.6 Folsom)
         ...
         *
         * A short description is usually good to have by each of these, eg the
         * subject line from the bug report.  If the subject line is not very good
        ...
        //
        // This is some ugly hackery, but a necessary evil for legacy content. Please
        // don't ever use this antipattern for new bugs.
        unsigned bugzilla444630: 1;

        unsigned bugzilla456852a: 1; // Different runtime error for invalid
        // constructor call when -Ojit set
        unsigned bugzilla456852b: 1; // Different runtime error for negative vector
        // index when -Ojit set
        unsigned
        ...
        unsigned bugzilla513039: 1; // Number.toFixed(0) returns incorrect numbers,
        // rounding issues
        unsigned
        ...
        unsigned bugzilla532454: 1; // Sort incorrectly converts result of
        // user-supplied comparison function to integer
        unsigned bugzilla539094: 1; // OSR (On-Stack replacement of interpreted
        // methods by JIT-ted methods)
        unsigned bugzilla551587: 1; // MathClass:_min() does not correctly handle -0
        unsigned bugzilla558863: 1; // in operator on bytearray throws exception for
        // non-natural number
        unsigned bugzilla574600: 1; // Vector.<>.AS3::map returns no value
        unsigned bugzilla585791: 1; // String.localeCompare with a null String object
        // returns 0
        unsigned bugzilla598683: 1; // Bad XML with unterminated node with namespace
        // not throwing correctly
        unsigned
                bugzilla609416: 1; // encodeURIComponent and decodeURIComponent give
        // wrong output when input contains surrogate pairs
        unsigned bugzilla654807: 1; // sealed subclasses of Array act sort-of-dynamic
        unsigned bugzilla661330: 1; // Array.length behavior doesn't follow ECMA262
        // near 2^32-1
        unsigned bugzilla678952: 1; // Operations on Vector.<C> do not make use of or
        // provide information about C
        unsigned bugzilla619148: 1; // Stack traces in release builds
        unsigned watson3548918: 1;  // For appendChild, if the newChild is already in
        // the tree, it is first removed.
        unsigned watson3573621: 1;  // HTTPStatusEvent is not sent when the request
        // has to have a crossdomain policy file checked.
    protected:
        friend class AvmCore;
...
// Internal to AvmCore, do not use for other purposes
    ...
    LivePoolNode(MMgc::GC
    *gc) :
    GCRoot(gc, MMgc::kExact
    ) {
}
};
 ...
 * ----- NOTE!! NOTE!! NOTE!! NOTE!! --------------------------
 *
 * AvmCore internal types and data fields are grouped at the beginning of the
 * class definition in order to give exact tracing a credible shot at staying
 * correct. PLEASE add data fields only in the correct group.
 *
 * ----- NOTE!! NOTE!! NOTE!! NOTE!! --------------------------
 */
...
private:
    enum {
        MIN_API_MARK =
        0xE294, // that's "0xE000 + 660", 660 being an odd legacy wart
        MAX_API_MARK = 0xF8FF
    };
...
public:
    ...
    public:
        ...
        void oom(MMgc::MemoryStatus status) {
            if (core)
                core->oom(status);
        }
    ...
    public:
        ...
        virtual CodeContext *create(DomainEnv *domainEnv,
                                    const BugCompatibility *bugCompatibility) = 0;
    };

    struct CacheSizes {
        enum {
            DEFAULT_BINDINGS = 32, DEFAULT_METADATA = 1, DEFAULT_METHODS = 32
        };
        ...
        inline CacheSizes()
                : bindings(DEFAULT_BINDINGS), metadata(DEFAULT_METADATA),
...
private:
    ...
    /**
     * If this field is not NotInterrupted, the host has requested that the
     * currently executing AS3 code stop and invoke handleInterrupt.  The field is
     * checked directly by executing code.
     *
    ...
    public:
      /*
        Sampler functionality - used by the host application to do periodic sampling
        of the actionscript call stack. It works like this:
        ...
        - from within takeSample(), the host can fetch the current call stack, in
        the form of an array of MethodInfo (which is an opaque token whose details
        are known only to the VM).
      ...
      */
      ...
      // provides the host's takeSample() function. Sampling becomes enabled if the
      // parameter is non-null. A null parameter disables sampling.
      void enableSampler(telemetry::ISampler *);

      // Request the VM to call ISampler::takeSample() as soon as possible, from
      // within the VM's execution thread. requestSample() can be called from any
      // thread. This function will only be called when sampling is enabled.
      // Multiple calls to this function have the same effect as a single call.
      void requestSample();

      // Clear the request that was set by requestSample(). The host will always
      // call this from within takeSample(), and perhaps at other times as well.
      void clearSampleRequest();

      // Find out if the current call stack is empty. This function will only ever
      // be called from within ISampler::takeSample(), as called from the VM.
      bool isCallStackEmpty();

      // Copy the current call stack into 'stackBuffer'. Returns the number of stack
      // items. This function will only ever be called from within
      // ISampler::takeSample(), as called from the VM. If the stack is deeper than
      // maxDepth, we fill stackBuffer to the maximum and then return maxDepth+1.
      // The deepest stack frame should appear first in the buffer.
      unsigned int recordCallStack(telemetry::FunctionHandle *stackBuffer,
      ...
      // This function can be called from the host at any time, not only from inside
      // takeSample().
      ...
      */
      ...
      /** if non-zero, it means that the VM should call takeSample() or
       * takeSampleWrapper() as soon as possible. */
      ...
      /** one of these functions is called by the executing actionscript code,
          whenever that code notices that sampleTicks != 0. They just call
         sampler->takeSample() */
...
private:
    ...
    // a running count of MethodInfos used to generate unique id's for the
    // PoolObjects
...
public:
    /**
     * Default values for the config parameters.  These need to be visible,
     * because the shell's command line parsing needs to know what they are.
     * Presumably other host environments might find them useful too.
     *
...
#ifdef _DEBUG
    // Only the thread used to create the AvmCore is allowed to modify
    // currentMethodFrame (and thus, use EnterCodeContext). We don't enforce this
    // in Release builds, but check for it and assert in Debug builds.
...
private:
    ...
    GCMember <UnscannedTraitsArray>
            _emptySupertypeList; // empty supertype list shared by many Traits
...
public:
    ...
    // From here at least the spelling mirrors exactly the spelling in the string
    // constant
...
public:
    ...
    // To be called post-PoolObjects' population to assign it a unique id amongst
    // pools
    ...
    virtual uintptr_t getStackBase() const { return 0; }

    /**
     * Restore the stack base and limit based on a base of p.
     * Restore the stack base when leaving a thread.
     */
    virtual void restoreStackBase(uintptr_t p) {
        (void) p;
        // NOTE: the shell currently does not implement setStackBase() or
        // restoreStackBase().
    }
...
protected:
    virtual BaseExecMgr *createExecMgr() { return new(gc) BaseExecMgr(this); }
...
public:
    ...
    // safe to call at any time, but calling tosses existing caches, thus has a
    // perf hit -- don't call cavalierly
...
public:
     ...
     * An inlined version of this code is also generated by
     * CodegenLIR::prologue().
     */
    ...
    handleStackOverflowMethodEnv(MethodEnv
    *env);
    ...
    *env);
     ...
     * back to the system. This is only used on platforms (e.g. Android) that do
     * not call the destructor of the static variable when the library is unloaded
     * (on Android this is an OS bug in Android 2.2, and probably earlier).
     */
    static void releasePCREContext();

private:
    ScriptEnv *initOneScript(Toplevel *toplevel, AbcEnv *abcEnv,
                             Traits *scriptTraits);
...
public:
     ...
     * @param codeContext CodeContext to use for execution (implicitly specifies
     * DomainEnv)
     ...
     *         in TRY/CATCH.
     */
    Atom handleActionPool(PoolObject *pool, Toplevel *toplevel,
                          CodeContext *codeContext);

    template<class CLASSMANIFEST>
    CLASSMANIFEST *prepareBuiltinActionPool(PoolObject *pool, Toplevel *toplevel,
                                            CodeContext *codeContext);
     ...
     *         in TRY/CATCH.
     */
    PoolObject *parseActionBlock(ScriptBuffer code, int start, Toplevel *toplevel,
                                 Domain *domain, const NativeInitializer *ninit,
                                 ApiVersion apiVersion, Stringp name = NULL);
    ...
    ScriptEnv *prepareActionBlock(ScriptBuffer code, Toplevel *toplevel,
     ...
     *         in TRY/CATCH.
     */
    Atom handleActionBlock(ScriptBuffer code, int start, Toplevel *toplevel,
     ...
     *         in TRY/CATCH.
     */
    Atom handleActionSource(String *code, String *filename, Toplevel *toplevel,
                            const NativeInitializer *ninit,
                            CodeContext *codeContext, ApiVersion apiVersion);
     ...
     * @return  A string representing the contents of the file named by
     * 'filename'. The string must NUL-terminated and the NUL is not considered
     * part of the input.  If the returned string is not in UTF-16 format then it
     * will be converted to UTF-16 format, so it is highly advisable that the
     * method has created an UTF-16 string.  If the file cannot be opened or read
     * then the return value should be NULL, an exception should not be thrown.
     */
    virtual String *readFileForEval(String *referencingFilename,
                                    String *filename) = 0;
#endif // VMCFG_EVAL
     ...
     * be flushed more aggressively to encourage faster collection of the
     * discarded ABC. Note that it is safe to call this "too often", but doing so
     * will hurt performance; you need call it only once per ABC unloading event.
     */
    ...
    void formatOpcode(PrintWriter &out, const uint8_t *pc,
                      const uint8_t *code_end, AbcOpcode opcode, ptrdiff_t off,
                      PoolObject *pool);
#ifdef VMCFG_WORDCODE
    void formatOpcode(PrintWriter &out, const uintptr_t *pc,
                      const uintptr_t *code_end, WordOpcode opcode, ptrdiff_t off,
                      PoolObject *pool);
    void formatBits(PrintWriter &buffer, uint32_t bits);
#endif
    static void formatMultiname(PrintWriter &out, uint32_t index,
                                PoolObject *pool);
#endif
     ...
     */
    virtual const BugCompatibility *
    createBugCompatibility(BugCompatibility::Version v);
    ...
    /**
     * Find the ApiVersion from the first non-builtin caller on the AS3 call
     * stack. If the AS3 call stack is empty, return getDefaultAPI().
     */
    ApiVersion getApiVersionFromCallStack();

    /**
     * If the given Namespace has a version that allows it to be used by
     * the ApiVersion active on the call stack, return true; otherwise return
     * false. This is equivalent to ns->getApiVersion() <=
     * getApiVersionFromCallStack(), but it's easy for client code to get the
     * comparison reversed, so this is useful syntactic sugar.
     */
...
#define float4_decl_v(var)                                                     \
  float4_t var##v;                                                             \
  AvmCore::float4(&var##v, var);
#endif

    // convert atom to integer when we know it is already a legal signed-32 bit
    // int value
    static int32_t integer_i(Atom a);

    // convert atom to integer when we know it is already a legal unsigned-32 bit
    // int value
...
public:
    ...
    /**
     * Returns the size of the instruction + all it's operands.  For
     * OP_lookupswitch the size will not include the size for the case targets.
     */
    static int calculateInstructionWidth(const uint8_t *p);

    /**
     * Read in some operands for the instruction located at *pc.
     * Returns the size of the instruction, but will not read in all the case
     * targets for an OP_lookupswitch, since there will be a variable number of
     * them.
     */
    static void readOperands(const uint8_t *&pc, unsigned int &imm32, int &imm24,
                             unsigned int &imm32b, int &imm8);
    ...
    static uint32_t readU32(const uint8_t *&p) {
        ...
        // but first we must determine whether it should be inline, REALLY_INLINE,
        // etc...
    ...
    static void skipU32(const uint8_t *&p, int count = 1) {
        ...
        while (count-- > 0) {
            ...
            // if (!(*p[4] & 0x80)) { p += 5; continue; }    // test should be
            // unnecessary
            AvmAssert(!(p[4] & 0x80));
            p += 5;
        }
...
public:
    ...
    // but return of false still means we *might* need to (ie, negating the result
    // of this function isn't "needscoerce")
    static bool atomDoesNotNeedCoerce(Atom a, BuiltinType bt);

    /**
     * this is the implementation of the actionscript "is" operator.  similar to
     * java's instanceof.  returns true/false according to AS rules.  in
     * particular, it will return false if value==null.
     */
    ...
    /**
     * implementation of OP_inclocal, declocal, increment, decrement which
     * correspond to
     * ++ and -- operators in AS.
     *
     * This function is now used only on the interpreter slowpath, and has been
     * restricted to apply only to kIntptrType and kDoubleType atoms.
     */
    Atom increment_number_d(Atom atom, int delta);

    /**
     * implementation of OP_inclocal_i, declocal_i, increment_i, decrement_i which
     * correspond to
     * ++ and -- operators in AS.
     */
    ...
    template<bool>
    Atom numericAtomImpl(Atom atom);
    ...
    // return true if there is a pending interrupt of the specific
    // InterruptReason.
     ...
     */
    void throwErrorV(ClassClosure *type, int errorID, Stringp arg1 = 0,
                     Stringp arg2 = 0, Stringp arg3 = 0);

    /**
     * formatErrorMessageV is a convenience function for
     * assembling an error message with varags.
     */
    String *formatErrorMessageV(int errorID, Stringp arg1 = 0, Stringp arg2 = 0,
                                Stringp arg3 = 0);
     ...
     */
    String *findErrorMessage(int errorID, int *mapTable, const char **errorTable,
                             int numErrors);
#endif
     ...
     */
    ExceptionHandler *findExceptionHandler(MethodInfo *info, intptr_t pc,
                                           Exception *exception);
     ...
     * found.
     */
    ExceptionHandler *findExceptionHandlerNoRethrow(MethodInfo *info, intptr_t pc,
                                                    Exception *exception,
                                                    int32_t *ordinal);
    ...
    Stringp EscapeElementValue(const Stringp s,
...
public:
    /**
     * If the uri is versioned, set apiVersion to the version and strip the uri
     * string, and return true. If the uri is unversioned, leave uri and
     * apiVersion unchanged and return false.
     */
     ...
     */
    static bool parseApiVersion(const char *p, ApiVersion &apiVersion,
    ...
    Stringp newStringEndianUTF16(bool littleEndian, const wchar *str,
                                 int len = -1, bool strict = false);
    ...
    RegExpObject *newRegExp(RegExpClass *regExpClass, Stringp pattern,
                            Stringp options);
    ...
    Namespacep newNamespace(Atom prefix, Atom uri,
                            Namespace::NamespaceType type = Namespace::NS_Public);

    Namespacep newNamespace(Atom uri,
                            Namespace::NamespaceType type = Namespace::NS_Public);

    Namespacep newNamespace(Stringp uri, Namespace::NamespaceType type,
                            ApiVersion apiVersion);

    // Like newNamespace(), but versioned such that it will match all versions of
    // the namespace with the same uri/type. This is useful for glue code that
    // (e.g.) needs to look up specific Bindings in the builtins. (Analogous to
    // "getAnyPublicNamespace" for arbitrary names.)
    ...
    static void atomWriteBarrier(MMgc::GC *gc, const void *container,
                                 Atom *address, Atom atomNew);

    static void atomWriteBarrier_ctor(MMgc::GC *gc, const void *container,
                                      Atom *address, Atom atomNew);
    ...
    private:
      telemetry::ITelemetry
          *m_telemetry; // Owned by the AvmCore subclass which calls SetTelemetry
#endif
...
public:
    ...
    virtual int evaluateScriptBuffer(ScriptBuffer &buffer,
...
/*
    ...
    -- native C++ code can override the current CodeContext by using
   EnterCodeContext(), which just pushes another MethodFrame onto the stack...
        it overrides the current CodeContext, but subsequent nested calls to
   non-builtin methods will in turn override this.
    -- The implementation is a bit convoluted, in the name of saving stack
   space. A single field can contain either a MethodEnv* (for a normal
   MethodFrame) or a CodeContext* (for one pushed by EnterCodeContext). This
   means that the top-of-stack may not have the current MethodEnv* handy, so
   walking down the stack is necessary to find it.
    -- Note that MethodFrame doesn't contain a pointer to AvmCore*; this is by
   design (as a stack-saving measure), as CodegenLIR doesn't need to save it (it
   can emit the proper constant value), and all other callers have ready access
   to one.
*/
class MethodFrame {
public:
    ...
    // Search for a frame that has a default namespace, starting on the given
    // frame.
...
private:
    ...
    enum {
        IS_EXPLICIT_CODECONTEXT = 0x1, DXNS_NOT_NULL = 0x2, FLAGS_MASK = 0x3
    };
    uintptr_t envOrCodeContext;
    Namespace
            *dxns; // NOTE: this struct is always stack-allocated (or via
    // avmStackAlloc, which is just as good), so no GCMember needed
};
