...
namespace avmplus {
    ...
    MethodClosureClass(VTable
    *cvtable);

    public:
    REALLY_INLINE static MethodClosureClass
    *
    create(MMgc::GC
    *gc,
    VTable *cvtable
...
public:
REALLY_INLINE static MethodClosure
*
create(MMgc::GC
*gc,
VTable *vtable,
...
MethodClosure(vtable, call, savedThis
);
}

/**
 * Implements OP_call and ScriptObject::call for concrete MethodClosures.  Can
 * be invoked directly by jit code or by FunctionObject::call; do not override
 * the latter.
 */
static Atom callMethodClosure(MethodClosure * , int
argc,
Atom *argv
);
...
virtual Atom
get_savedThisOrNull() const; // returns nullAtom if invalid, no assert
...
// Note that this class doesn't have an AS3 declaration; from AS3 it is
// indistinguishable from a normal MethodClosure. (In fact, this is currently
// never even exposed to AS3 code; it's used internally.) The only way to obtain
// one is to call weaken() on a normal MethodClosure.
...
REALLY_INLINE WeakMethodClosure(VTable *cvtable, MethodEnv *call,
                                Atom savedThis)
...
m_weakSavedThis(AvmCore::atomToScriptObject(savedThis)
->

GetWeakRef()

) {
AvmAssert(m_weakSavedThis
!= NULL);
m_call_ptr = (FunctionProc) callWeakMethodClosure;
}

REALLY_INLINE static WeakMethodClosure
*
create(MMgc::GC
*gc,
VTable *vtable, MethodEnv
*call,
Atom savedThis
) {
return new (gc, MMgc::kExact, vtable->
...
WeakMethodClosure(vtable, call, savedThis
);
}
 ...
 * the latter.
 */
static Atom callWeakMethodClosure(WeakMethodClosure * , int
argc,
Atom *argv
);
...
private:
GCMember <MMgc::GCWeakRef>
        GC_POINTER(m_weakCallEnv); // WeakRef to a MethodEnv*
GCMember <MMgc::GCWeakRef>
        GC_POINTER(m_weakSavedThis); // WeakRef to a ScriptObject*
