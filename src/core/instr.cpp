...
template <class E>
Atom callprop_b(E env, Atom base, const Multiname *multiname, int argc,
                Atom *atomv, VTable *vtable, Binding b) {
  AssertNotNull(base);
  ...
  case BKIND_SET:
    ...
    env->toplevel()->throwReferenceError(kWriteOnlyError, multiname,
                                         vtable->traits);
...
template Atom callprop_b(Toplevel *, Atom, const Multiname *, int, Atom *,
                         VTable *, Binding);
  ...
  if (traits && ref->isBinding()) {
    ...
    if (!ref->isNsset()) {
      ...
      if (b == BIND_AMBIGUOUS) {
        ...
        env->toplevel()->throwTypeError(kAmbiguousBindingError,
                                        env->core()->toErrorString(ref));
      }
    }
  }
  return b;
...
template <class E> VTable *toVTable(E env, Atom atom) {
  ...
  // TypeError in ECMA
  env->toplevel()->throwTypeError((atom == undefinedAtom)
...
template <class E> Atom astype_late(E caller_env, Atom value, Atom type) {
...
template <class E> Atom instanceof (E caller_env, Atom val, Atom ctor) {
  return caller_env->toplevel()->instanceof (val, ctor);
}
template Atom instanceof (MethodEnv *, Atom, Atom);

template <class E> Atom op_in(E caller_env, Atom name, Atom obj) {
 ...
 */
NO_INLINE
ScriptObject *FASTCALL finddef_miss(ScriptObject **obj_ptr,
                                    MethodFrame *frame) {
...
Atom constructprop(Toplevel *toplevel, const Multiname *multiname, int argc,
                   Atom *atomv, VTable *vtable) {
  ...
  case BKIND_METHOD: {
    ...
    toplevel->throwTypeError(kCannotCallMethodAsConstructor,
                             toplevel->core()->toErrorString(env->method));
  }
  case BKIND_VAR:
  case BKIND_CONST: {
    ScriptObject *ctor = AvmCore::atomToScriptObject(obj)->getSlotObject(
        AvmCore::bindingToSlotId(b));
    // This code is a bit subtle.  Note that ScriptObject::getSlotObject() will
    // return NULL if the slot is an SST_scriptobject and is NULL, or an
    // SST_atom that is null, undefined, or not an object.  These subsume the
    // test isObject(ctor) in op_construct(), which is inlined here. We once
    // included an extra check, resulting in throwing a different exception when
    // taking this path in interpreted code.  We must emulate this old behavior
    // for backward compatibility. It is unfortunate that CLASS_TYPE and
    // FUNCTION_TYPE expand into references to 'core', otherwise we could push
    // the next line down into the exceptional path.
    AvmCore *core = toplevel->core();
    if (!ctor || (!ctor->traits()->subtypeof(CLASS_TYPE) &&
                  !ctor->traits()->subtypeof(FUNCTION_TYPE))) {
      // We know that an exception will be thrown eventually, so now do the
      // version check.
      if (core->currentBugCompatibility()->bugzilla456852a) {
        // Correct behavior is to throw kConstructOfNonFunctionError as
        // op_construct would do.
        if (!ctor) {
          toplevel->throwTypeError(kConstructOfNonFunctionError);
        }
      } else {
        // Stay compatible with old behavior.
        toplevel->throwTypeError(kNotConstructorError,
 ...
 */
// NOTE: parts of this function have been explicitly inlined into
// BaseExecMgr::unbox1 for efficiency. If you change/fix this method, you may
// need to change/fix MethodEnv::unbox1 as well.
Atom coerceImpl(const Toplevel *toplevel, Atom atom, Traits *expected) {
  AvmAssert(expected != NULL);
  AvmAssert(!AvmCore::atomDoesNotNeedCoerce(
      atom, BuiltinType(expected->builtinType)));
...
#endif
  case BUILTIN_string:
    return AvmCore::isNullOrUndefined(atom) ? nullStringAtom
                                            : core->string(atom)->atom();
  case BUILTIN_int:
  ...
  if (AvmCore::isNullOrUndefined(atom))
    return (expected->builtinType == BUILTIN_void) ? undefinedAtom
                                                   : nullObjectAtom;
  ...
  if (!actual->subtypeof(expected)) {
    ...
    // "\n";
#endif
    toplevel->throwTypeError(kCheckTypeFailedError,
                             core->atomToErrorString(atom),
                             core->toErrorString(expected));
  }
...
template <> float4_t __multiply<float4_t>(float4_t a, float4_t b) {
  return f4_mul(a, b);
}
template <> float4_t __divide<float4_t>(float4_t a, float4_t b) {
  return f4_div(a, b);
}
template <> float4_t __subtract<float4_t>(float4_t a, float4_t b) {
  return f4_sub(a, b);
}
...
#define op_generic(name)                                                       \
  Atom op_##name(AvmCore *core, Atom lhs, Atom rhs) {                          \
    ...
    if (AvmCore::isFloat(lhs) && AvmCore::isFloat(rhs))                        \
      return core->floatToAtom(                                                \
          __##name(AvmCore::atomToFloat(lhs), AvmCore::atomToFloat(rhs)));     \
    else                                                                       \
      return core->doubleToAtom(                                               \
          __##name(AvmCore::number(lhs), AvmCore::number(rhs)));               \
  }
...
Atom op_negate(AvmCore *core, Atom val) {
  ...
  if (atomIsIntptr(val) && val != zeroIntAtom) {
    ...
    intptr_t res_int = intptr_t(res);
    // note: we can't negate "0" as integer, but we should've guarded against
    // that case with the zeroIntAtom test
    AvmAssert(res_int != 0);
    if (atomIsValidIntptrValue(res_int) && res == (double)res_int)
      return (res_int << 3) | kIntptrType;
  }
...
#ifdef VMCFG_FLOAT
template <bool float_enabled>
Atom op_add_impl(AvmCore *core, Atom lhs, Atom rhs)
#else
...
#define SIGN_EXTEND(v) (intptr_t(v))
  ...
  if (AvmCore::isNumeric(lhs) && AvmCore::isNumeric(rhs)) {
    // C++ porting note. if either side is undefined NaN then result must be
    // NaN, which is assumed to be taken care of by IEEE 748 double add.
    goto add_numbers;
  }

  if (AvmCore::isString(lhs) || AvmCore::isString(rhs) ||
      AvmCore::isDate(lhs) || AvmCore::isDate(rhs)) {
    goto concat_strings;
  }
  ...
  if (AvmCore::isXMLorXMLList(lhs) && AvmCore::isXMLorXMLList(rhs)) {
    XMLListObject *l = XMLListObject::create(
        core->GetGC(), atomObj(lhs)->toplevel()->xmlListClass());
    l->_append(lhs);
    l->_append(rhs);
    return l->atom();
  }
  ...
  // if L is String || R is String, concat, else add toNumber(lhs) to
  // toNumber(rhs)

  // ToPrimitive() will call [[DefaultValue]], which calls valueOf().  If the
  // result is
  //  a primitive, return that value else call toString() instead.

  // from E3:
  // NOTE No hint is provided in the calls to ToPrimitive in steps 5 and 6. All
  // native ECMAScript objects except Date objects handle the absence of a hint
  // as if the hint Number were given; Date objects handle the absence of a hint
  // as if the hint String were given. Host objects may handle the absence of a
  // hint in some other manner.

  lhs =
      AvmCore::primitive(lhs); // Date is handled above with the String argument
                               // case,  we don't have to check for it here.
  rhs = AvmCore::primitive(rhs);
...
#ifdef VMCFG_FLOAT
  // if both are floats, the addition is done on float; if any is float4,
  // addition is done on float4
  if (float_enabled) {
    if (AvmCore::isFloat4(lhs) || AvmCore::isFloat4(rhs)) {
      float4_decl_v(lhs);
      float4_decl_v(rhs);
    ...
    if (AvmCore::isFloat(lhs) && AvmCore::isFloat(rhs))
      return core->floatToAtom(AvmCore::singlePrecisionFloat(lhs) +
                               AvmCore::singlePrecisionFloat(rhs));
  }
#endif
...
// Specialized helper functions called by code generated for the OP_add
// instruction. These take into account operands with known specific types and
// invariants previously established by inline code.

// atom + atom => atom
#ifdef VMCFG_FLOAT
template <bool float_support>
Atom op_add_a_aa_impl(AvmCore *core, Atom lhs, Atom rhs)
#else
...
#endif // VMCFG_FLOAT
{
...
#else
  // Inline code should handle intptr+intptr case, except where result must be
  // double.
  AvmAssert(!atomIsBothIntptr(lhs, rhs) ||
            addIntptrOverflow(atomGetIntptr(lhs), atomGetIntptr(rhs)));
#endif

  if (atomIsIntptr(lhs)) {
    if (AvmCore::isDouble(rhs)) {
      return core->doubleToAtom(double(atomGetIntptr(lhs)) +
                                AvmCore::atomToDouble(rhs));
    } else if (atomIsIntptr(rhs)) {
      // We should arrive here only on overflow in inline fastpath.
      return core->doubleToAtom(
          double(atomGetIntptr(lhs) + double(atomGetIntptr(rhs))));
    }
  } else if (AvmCore::isDouble(lhs)) {
    if (atomIsIntptr(rhs)) {
      return core->doubleToAtom(AvmCore::atomToDouble(lhs) +
                                double(atomGetIntptr(rhs)));
  ...
  if (AvmCore::isString(lhs)) {
    if (AvmCore::isString(rhs)) {
      ...
      // Here, we know that neither lhs nor rhs are null, so we can concatenate
      // w/o the coercion.
      return String::concatStrings(AvmCore::atomToString(lhs),
                                   AvmCore::atomToString(rhs))
          ->atom();
    } else {
      goto concat_strings;
    }
  ...
  if (!(AvmCore::isString(lhs) || AvmCore::isString(rhs))) {
    ...
    if (float_support) {
      ...
      if (AvmCore::isFloat(lhs) && AvmCore::isFloat(rhs))
        return core->floatToAtom(AvmCore::atomToFloat(lhs) +
                                 AvmCore::atomToFloat(rhs));
    }
#endif
    return core->doubleToAtom(AvmCore::number(lhs) + AvmCore::number(rhs));
  }
...
#ifdef VMCFG_FLOAT
template <bool float_support>
Atom op_add_a_ai_impl(AvmCore *core, Atom lhs, int32_t rhs)
#else
...
// Fastpath for intptr+int with no overflow
#ifdef AVMPLUS_64BIT
    // A 32-bit integer is guaranteed convertible to an atom of type
    // kIntptrType.
    intptr_t lhsv = lhs;
...
#endif
#else
    // Inline code should handle intptr+int case, except where result must be
    // double. It is allowable for the inline code to punt if the 32-bit int
    // argument is itself too large for an intptr.
    AvmAssert(!atomIsValidIntptrValue(rhs) ||
              addIntptrOverflow(atomGetIntptr(lhs), rhs));
#endif
  ...
  if (AvmCore::isString(lhs) || AvmCore::isDate(lhs))
    goto concat_strings;
...
concat_strings:
  // See AvmCore::string()
  Stringp s = MathUtils::convertIntegerToStringBase10(
      core, rhs, MathUtils::kTreatAsSigned);
...
#ifdef VMCFG_FLOAT
template <bool float_support>
Atom op_add_a_ia_impl(AvmCore *core, int32_t lhs, Atom rhs)
#else
...
// Fastpath for int+intptr with no overflow
...
#endif
#else
    // Inline code should handle int+intptr case, except where result must be
    // double. It is allowable for the inline code to punt if the 32-bit int
    // argument is itself too large for an intptr.
    AvmAssert(!atomIsValidIntptrValue(lhs) ||
              addIntptrOverflow(lhs, atomGetIntptr(rhs)));
#endif
  ...
  if (AvmCore::isString(rhs) || AvmCore::isDate(rhs))
    goto concat_strings;
...
concat_strings:
  // See AvmCore::string()
  Stringp s = MathUtils::convertIntegerToStringBase10(
      core, lhs, MathUtils::kTreatAsSigned);
...
#ifdef VMCFG_FLOAT
template <bool float_support>
Atom op_add_a_ad_impl(AvmCore *core, Atom lhs, double rhs)
#else
...
#ifdef VMCFG_FLOAT
template <bool float_support>
Atom op_add_a_da_impl(AvmCore *core, double lhs, Atom rhs)
#else
...
double FASTCALL mop_lf32(const void *addr) {
  ...
  a = (uint32_t(u[3]) << 24) | (uint32_t(u[2]) << 16) | (uint32_t(u[1]) << 8) |
      uint32_t(u[0]);
...
double FASTCALL mop_lf64(const void *addr) {
  ...
  // is turned on.
  d.words.lsw = ((uint64_t(u[3]) << 24) | (uint64_t(u[2]) << 16) |
                 (uint64_t(u[1]) << 8) | (uint64_t(u[0])));
  d.words.msw = ((uint32_t(u[7]) << 24) | (uint32_t(u[6]) << 16) |
                 (uint32_t(u[5]) << 8) | (uint32_t(u[4])));
  return d.value;
#endif
}
...
void FASTCALL mop_lf32x4(float4_t *result, const void *addr) {
  ...
  *result = *(const float4_t *)addr;
  ...
  float4_t result;
  float_overlay f;
  f.word = ((uint32_t(u[3]) << 24) | (uint32_t(u[2]) << 16) |
            (uint32_t([1]) << 8) | uint32_t(u[0]));
  result->x = f.value;
  f.word = ((uint32_t(u[7]) << 24) | (uint32_t(u[6]) << 16) |
            (uint32_t([5]) << 8) | uint32_t(u[4]));
  result->y = f.value;
  f.word = ((uint32_t(u[11]) << 24) | (uint32_t(u[10]) << 16) |
            (uint32_t([9]) << 8) | uint32_t(u[8]));
  result->z = f.value;
  f.word = ((uint32_t(u[15]) << 24) | (uint32_t(u[14]) << 16) |
            (uint32_t([13]) << 8) | uint32_t(u[12]));
  result->w = f.value;
#endif
}
#endif
...
template <class E> NO_INLINE Atom op_call_error(E env) {
