...
namespace avmplus {
/*static*/ const ScopeTypeChain *
ScopeTypeChain::createExplicit(MMgc::GC *gc, Traits *owner, Traits **types) {
  uint32_t fullsize = 0;
  ...
  ScopeTypeChain *stc =
      new (gc, MMgc::kExact, padSize) ScopeTypeChain(size, fullsize, owner);
...
/* static */ const ScopeTypeChain *
ScopeTypeChain::create(MMgc::GC *gc, Traits *traits,
                       const ScopeTypeChain *outer, const FrameValue *values,
                       int32_t nValues, Traits *append, Traits *extra) {
  ...
  const size_t padSize =
      sizeof(uintptr_t) * (((pad > 0) ? (pad - 1) : 0) + outerSize);
  ScopeTypeChain *nscope = new (gc, MMgc::kExact, padSize) ScopeTypeChain(
      outerSize + capture, outerSize + capture + extraEntries, traits);
  int32_t j = 0;
...
/*static*/ const ScopeTypeChain *
ScopeTypeChain::create(MMgc::GC *gc, Traits *traits,
                       const ScopeTypeChain *outer, const FrameState *state,
                       Traits *append, Traits *extra) {
  if (state && state->scopeDepth > 0)
    return ScopeTypeChain::create(gc, traits, outer, &state->scopeValue(0),
                                  state->scopeDepth, append, extra);
  else
...
/*static*/ const ScopeTypeChain *
ScopeTypeChain::create(MMgc::GC *gc, Traits *traits,
                       const ScopeTypeChain *outer, Traits *const *stateTraits,
                       uint32_t nStateTraits, uint32_t nStateWithTraits,
                       Traits *append, Traits *extra) {
  MMgc::GC::AllocaAutoPtr valuesPtr;
  FrameValue *values = (FrameValue *)avmStackAllocGC(
      gc, valuesPtr, sizeof(FrameValue) * nStateTraits);
  ...
  return ScopeTypeChain::create(gc, traits, outer, values, nStateTraits, append,
                                extra);
}
#endif

const ScopeTypeChain *
ScopeTypeChain::cloneWithNewTraits(MMgc::GC *gc, Traits *p_traits) const {
  if (p_traits == this->traits())
    return this;

  const size_t padSize =
      sizeof(uintptr_t) * (this->fullsize ? this->fullsize - 1 : 0);
  ScopeTypeChain *nscope = new (gc, MMgc::kExact, padSize)
      ScopeTypeChain(this->size, this->fullsize, p_traits);
...
bool ScopeTypeChain::equals(const ScopeTypeChain *that) const {
  if (this != that) {
    ...
    if (this->size != that->size || this->fullsize != that->fullsize ||
...
/*static*/ ScopeChain *ScopeChain::create(MMgc::GC *gc, VTable *vtable,
                                          AbcEnv *abcEnv,
                                          const ScopeTypeChain *scopeTraits,
                                          const ScopeChain *outer,
                                          Namespacep dxns) {
  ...
  const size_t padSize =
      scopeTraitsSize > 0 ? sizeof(Atom) * (scopeTraitsSize - 1) : 0;
  ScopeChain *nscope = new (gc, MMgc::kExact, padSize)
      ScopeChain(vtable, abcEnv, scopeTraits, dxns);
  for (int32_t i = 0; i < outerSize; i++) {
...
ScopeChain *
ScopeChain::cloneWithNewVTable(MMgc::GC *gc, VTable *p_vtable, AbcEnv *p_abcEnv,
                               const ScopeTypeChain *p_scopeTraits) {
  if (p_vtable == this->vtable() && p_abcEnv == this->abcEnv())
    return this;

  const ScopeTypeChain *nstc =
      p_scopeTraits ? p_scopeTraits
                    : _scopeTraits->cloneWithNewTraits(gc, p_vtable->traits);
  AvmAssert(nstc->traits() == p_vtable->traits);
  const int32_t scopeTraitsSize = nstc->size;
  const size_t padSize =
      scopeTraitsSize > 0 ? sizeof(Atom) * (scopeTraitsSize - 1) : 0;
  ScopeChain *nscope = new (gc, MMgc::kExact, padSize)
      ScopeChain(p_vtable, p_abcEnv, nstc, _defaultXmlNamespace);
