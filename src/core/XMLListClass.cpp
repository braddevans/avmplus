...
namespace avmplus {
XMLListClass::XMLListClass(VTable *cvtable) : ClassClosure(cvtable) {
...
Atom XMLListClass::ToXMLList(Atom arg) {
  ...
  if (AvmCore::isXMLList(arg)) {
    ...
    XMLListObject *xl = XMLListObject::create(
        core->GetGC(), toplevel()->xmlListClass(), parent, bFound ? &m : 0);
    ...
    if (s->matchesLatin1("<>", 2, 0) &&
        s->matchesLatin1("</>", 3, s->length() - 3))
      s = s->substr(2, s->length() - 5);

    Namespace *defaultNamespace = toplevel->getDefaultNamespace();
    // We handle this step in the XMLObject constructor to avoid concatenation
    // huge strings together parentString = <parent xnlns=defaultNamespace> s
    // </parent>
    ...
    // parentString << s;
    // parentString << "</parent>";
    XMLObject *x = XMLObject::create(core->GetGC(), toplevel->xmlClass(), s,
                                     defaultNamespace);
