...
namespace avmplus {
    ...
    REALLY_INLINE FrameValue
    &
    FrameState::value(int32_t
    i) {
    AvmAssert(i
    >= 0 && i<frameSize);
    return locals[i];
...
REALLY_INLINE const FrameValue
&
FrameState::value(int32_t
i) const {
AvmAssert(i
...
REALLY_INLINE void FrameState::setType(int32_t
i,
Traits *t
) {
...
FLOAT_ONLY(if (bt == BUILTIN_float4) info->forceLargeVarSize();)
#endif
}

REALLY_INLINE void FrameState::pop(int32_t
n) {
stackDepth -=
n;
AvmAssert(stackDepth
>= 0);
}

REALLY_INLINE FrameValue
&
FrameState::peek(int32_t
n) {
return
value(stackBase
+ stackDepth - n);
...
REALLY_INLINE void FrameState::pop_push(int32_t
n,
Traits *t
) {
