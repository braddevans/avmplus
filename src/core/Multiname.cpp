...
PrintWriter &operator<<(PrintWriter &prw, const Multiname::Format &mnf) {
  if (mnf._ns->isPublic() || ( // backwards compatibility
...
PrintWriter &operator<<(PrintWriter &prw,
                        const Multiname::FormatNameOnly &mnf) {
  return mnf._mn->print(prw, Multiname::MULTI_FORMAT_NAME_ONLY);
}

// @TODO can we get rid of this ?!?
/*static*/ PrintWriter &Multiname::print(PrintWriter &prw, Namespacep ns,
                                         Stringp name, bool attr,
                                         bool hideNonPublicNamespaces) {
  if (ns->isPublic() || (hideNonPublicNamespaces && // backwards compatibility
...
PrintWriter &Multiname::print(PrintWriter &prw, MultiFormat form) const {
  ...
  if (isAny) {
    ...
    bool showName =
        (form == MULTI_FORMAT_FULL) || (form == MULTI_FORMAT_NAME_ONLY);
...
//
// NOTE NOTE NOTE
// This version is only safe if 'this' is on the first block of the object -
// because GC::GetGC() is not reliable otherwise.  See bugzilla 525875.  Use the
// more reliable version below if in doubt.
...
// This is always safe.  We must /not/ use WBRC_NULL here, only the full WBRC
// functions that take the gc and container explicitly will be safe.  See
// bugzilla 525875.
void HeapMultiname::setMultiname(MMgc::GC *gc, const void *container,
                                 const Multiname &that) {
...
// This is only safe if 'this' is on the first block of the object - because
// GC::GetGC() is not reliable otherwise.  See bugzilla 525875.  To destroy
// objects beyond the first block, use the safe version of setMultiname() with
// an empty Multiname argument.
