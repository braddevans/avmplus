...
#define DUMP_LIST(x)                                                           \
  {                                                                            \
    avmplus::Isolate::InterruptibleState::WaitRecord *t = x;                   \
    while (t) {                                                                \
      AvmLog("thread %d signaled %d\n", t->threadID, t->signaled);             \
      t = t->next;                                                             \
    }                                                                          \
  }
#else
...
Aggregate::Globals::IsolateMap::IsolateMap(int initialSize)
    : FixedHeapHashTable<Isolate::descriptor_t, FixedHeapRef<Isolate>>(
          initialSize) {}

Aggregate::Globals::IsolateMap::~IsolateMap() {
  Clear();
  // The destructor defined by the base class will call Clear(), but at that
  // time the object vtable will be that of the base class, so the overriden
  // DestroyItem() can't be seen and won't be called! We'll call Clear() here
  // first, the subsequent Clear() in the base class will have nothing to do.
}

Aggregate::Aggregate()
    : m_primordialGiid(
          Isolate::INVALID_DESC) // eventually there will be many of those
      ,
      m_activeIsolateCount(0), m_inShutdown(false),
      m_activeIsolateThreadMap(15) {}

Aggregate::~Aggregate() {}

void Aggregate::destroy() { mmfx_delete(this); }
...
bool Aggregate::requestIsolateExit(Isolate::descriptor_t desc,
  ...
  SCOPE_LOCK_NAMED(locker, m_globals.m_lock) {
    ...
    // if the isolate is in CANSTART, STARTING or RUNNING state it should be
    // interrupted
    if (isolate->m_state > Isolate::NEW &&
        isolate->m_state <= Isolate::RUNNING) {
      isolate->interrupt();
    }
    result = true;
    ...
    if (isolate->m_state <= Isolate::CANSTART ||
        isolate->m_state > Isolate::FINISHING) {
      return false;
    }
...
void Aggregate::requestAggregateExit() {
      ...
      if (m_aggregate == isolate->m_aggregate && !isolate->isInterrupted()) {
        if (isolate->m_state <= Isolate::CANSTART) {
          ...
          if (isolate->m_state < Isolate::FAILED) {
            AvmCore *core = isolate->m_core;
            // Ensure that the write to AvmCore::interrupted is visible to the
            // other thread.

            // in some shutdown cases the core is already NULL so we have to
  ...
  SCOPE_LOCK(m_globals.m_lock) {
    ...
    //  (1) needs to make sure that all cores are interrupted so that the very
    //  next
    //      thing they do is unwind, we don't want to continue executing if they
    //      could possibly be in an ideteriminate state (get unblocked from a
    //      Mutex or Condition when the condition hasn't been met.
    //  (2) needs to release resources (unlock any held Mutex or Condition, wake
    //  any blocked
...
void Aggregate::throwWorkerTerminatedException(Toplevel *currentToplevel) {
  ...
  GCRef<ErrorObject> error = currentToplevel->errorClass()->constructObject(
      errorMessage->atom(), core->intToAtom(kWorkerTerminated));
...
Isolate::Isolate(Isolate::descriptor_t desc, Isolate::descriptor_t parentDesc,
                 Aggregate *aggregate)
    : m_desc(desc), m_parentDesc(parentDesc), m_core(NULL),
      m_interruptibleState(NULL), m_aggregate(aggregate), m_state(Isolate::NEW),
      m_failed(false), m_interrupted(false) {
...
void Aggregate::stateTransition(Isolate *isolate, Isolate::State to) {
  SCOPE_LOCK_NAMED(locker, m_globals.m_lock) {
    ...
    if (to == Isolate::CANSTART) {
      ...
      AvmAssert(isolate->getDesc() == m_primordialGiid ||
                m_activeIsolateThreadMap.LookupItem(isolate->getDesc(), &t) ==
                    true);
      ...
      AvmAssert(from == Isolate::CANSTART || from == Isolate::RUNNING ||
                from == Isolate::STARTING || from == Isolate::FINISHING);
      AvmAssert(isolate->getDesc() == m_primordialGiid ||
                m_activeIsolateThreadMap.LookupItem(isolate->getDesc(), &t) ==
                    true);
      ...
      AvmAssert(from == Isolate::RUNNING || from == Isolate::STARTING ||
                from == Isolate::FINISHING || from == Isolate::NEW);
      setIsolateAsInterrupted(isolate);
      isolateInactive = true;
    }
...
void Aggregate::setIsolateAsInterrupted(Isolate *isolate) {
  Isolate::lockInSafepoint(&isolate->m_interruptibleStateMutex);
  {
    ...
    if (state) {
      SCOPE_LOCK(state->getMonitor()) { isolate->m_interrupted = true; }
    } else {
...
uintptr_t
Isolate::SharedPropertyMap::HashKey(Isolate::SharedPropertyNamep key) const {
  return String::hashCodeLatin1(key->values, key->length);
}

bool Isolate::SharedPropertyMap::KeysEqual(
    Isolate::SharedPropertyNamep k1,
    const Isolate::SharedPropertyNamep k2) const {
...
void Isolate::releaseActiveResources() {
  SCOPE_LOCK(m_sharedPropertyLock) {
    ...
    // resources from being collected and released, like a locked Mutex, waiting
    // Condition, or a reference to ourself
    m_sharedProperties.Clear();
  }

  // if we have any active Mutexes or Conditions release them.
  // this is safe because we have already interrupted our core and the very next
  // execution point should be to throw an external interrupt error
  signalInterruptibleState();
}

void Isolate::SharedPropertyMap::DestroyItem(Isolate::SharedPropertyNamep key,
                                             ChannelItem *item) {
  mmfx_delete(item);
  key->deallocate();
  mmfx_delete(key);
...
Atom Isolate::getSharedProperty(const StUTF8String &key, Toplevel *toplevel) {
  ...
  SCOPE_LOCK(m_sharedPropertyLock) {
    ...
    TRY(core, kCatchAction_Rethrow) {
      ...
      Isolate::SharedPropertyNamep keyInternal =
          mmfx_new(FixedHeapArray<char>());
    ...
    CATCH(Exception * e) {
      __locker.~MutexLocker(); // __locker is created in the SCOPE_LOCK macro
...
ChannelItem *Isolate::makeChannelItem(Toplevel *toplevel, Atom atom) {
  ...
  public:
    ScriptObjectChannelItem(Toplevel *toplevel, Atom value) {
      GCRef<ByteArrayObject> ba =
          toplevel->byteArrayClass()->constructByteArray();
      ba->writeObject(value);
      // FIXME: The ByteArray::Buffer class should be opaque outside of the
      // ByteArray class.
      // FIXME: The code below is essentially duplicated from
      // ByteArrayObject::makeChannelItem().
    ...
    Atom getAtom(Toplevel *toplevel) const {
      ...
      GCRef<ByteArrayObject> byteArray = new (toplevel->gc(), MMgc::kExact)
          ByteArrayObject(cls->ivtable(), cls->prototypePtr(), m_value);
  ...
  if (kind == kDoubleType) {
    ...
    // no value
  } else if (AvmCore::istype(atom,
                             toplevel->byteArrayClass()->ivtable()->traits)) {
    ByteArrayObject *ba = static_cast<ByteArrayObject *>(
        toplevel->core()->atomToScriptObject(atom));
    item = ba->makeChannelItem();
  } else if (AvmCore::istype(atom, toplevel->builtinClasses()
                                       ->get_MutexClass()
                                       ->ivtable()
                                       ->traits)) {
    MutexObject *mutexObj =
        static_cast<MutexObject *>(toplevel->core()->atomToScriptObject(atom));
    item = mutexObj->makeChannelItem();
  } else if (AvmCore::istype(atom, toplevel->builtinClasses()
                                       ->get_ConditionClass()
                                       ->ivtable()
                                       ->traits)) {
    ConditionObject *conditionObj = static_cast<ConditionObject *>(
        toplevel->core()->atomToScriptObject(atom));
...
bool Isolate::retryInterruptibleState() { return false; }
...
bool Isolate::hasFailed() const { return m_failed; }
...
bool Isolate::isInterrupted() const { return m_interrupted; }
...
bool Isolate::isPrimordial() const { return m_aggregate->isPrimordial(m_desc); }

void Isolate::run() {
  // Make sure the isolate survives for the duration of the doRun() call by
  // keeping a ref to it.
  FixedHeapRef<Isolate> handle(this);
  DecrementRef(); // Let go of the hand-over-hand-safety-ref set by
                  // spawnIsolateThread() now that handle is valid.
...
void Isolate::destroy() { mmfx_delete(this); }
  ...
  if (VMPI_recursiveMutexTryLock(mutex) == false) {
    ...
    if (current && !current->isSafe()) {
      ...
      assert(!vmbase::SafepointRecord::hasCurrent() ||
             vmbase::SafepointRecord::current()->manager()->inSafepointTask());
...
Isolate::InterruptibleState::Enter::ActiveInterruptibleStateHelper::
...
REALLY_INLINE void
Isolate::InterruptibleState::Enter::ActiveInterruptibleStateHelper::unlock()
    const {
  VMPI_recursiveMutexUnlock(m_mutex);
}

REALLY_INLINE Isolate *
Isolate::InterruptibleState::Enter::ActiveInterruptibleStateHelper::getIsolate()
    const {
  return m_isolate;
}
//
...
//
Isolate::InterruptibleState::Enter::Enter(WaitRecord &record,
                                          InterruptibleState *state,
                                          Isolate *isolate)
    : interrupted(false), result(false),
      ...
      ,
      m_state(state), m_waitRecord(&record) {
  ...
  do {
    if (millis == -1) {
      ...
      if (now < endTime) {
        ...
        // In order to ensure that the lock is released even though the wait
        // time has expired, a wait() for the minimum time must be done here.
    ...
    // if a script timeout has fired; script timeouts only happen for the
    // primordial isolate.
    interrupted =
        isolate->isInterrupted() ||
        isolate->getAvmCore()->interruptCheckReason(AvmCore::ScriptTimeout);
...
Isolate::InterruptibleState::InterruptibleState()
    : m_waiterCount(0), m_signaledWaiters(0), m_head(NULL), m_tail(NULL) {
#ifdef DEBUG_INTERRUPTIBLE_STATE
  gid = ++globalId;
#endif // DEBUG_INTERRUPTIBLE_STATE
}
...
bool Isolate::InterruptibleState::hasWaiters() {
  bool result =
      vmbase::AtomicOps::compareAndSwap32WithBarrierPrev(
          0, 0, reinterpret_cast<volatile int32_t *>(&m_waiterCount)) != 0;
  return result;
}
...
void Isolate::InterruptibleState::signal() {
  DEBUG_STATE(
      ("thread %d is calling signal on (%d)\n", VMPI_currentThread(), gid));
...
void Isolate::InterruptibleState::notify() {
  SCOPE_LOCK_NAMED(cond, m_condition) {
    ...
    DEBUG_STATE(
        ("thread %d is calling notify on (%d)\n", VMPI_currentThread(), gid));
    cond.notify();
  }
}
...
Aggregate::ActiveIsolateThreadMap::ActiveIsolateThreadMap(int initialSize)
    : FixedHeapHashTable<Isolate::descriptor_t, vmbase::VMThread *>(
          initialSize) {}
...
bool Aggregate::spawnIsolateThread(Isolate *isolate) {
  ...
  SCOPE_LOCK(m_globals.m_lock) {
    ...
    AvmAssert(m_activeIsolateThreadMap.LookupItem(isolate->getDesc(), &t) ==
              false);
    vmbase::VMThread *thread = mmfx_new(vmbase::VMThread(isolate));
    // increment the refcount on the isolate as a hand-over-hand-safety measure
    // to ensure that when the thread spins up the isolate will still be around
    // for the thread to take a ref to the isolate.
...
Isolate *Aggregate::getIsolate(Isolate::descriptor_t desc) {
  Isolate *result = NULL;
  SCOPE_LOCK(m_globals.m_lock) { result = m_globals.getIsolateForID(desc); }
  return result;
}
...
Isolate::State Aggregate::queryState(Isolate *isolate) {
  ...
  SCOPE_LOCK(m_globals.m_lock) { result = isolate->m_state; }
...
Isolate *Aggregate::newIsolate(Isolate *parent) {
  ...
  SCOPE_LOCK(m_globals.m_lock) {
    if (!m_inShutdown) {
      ...
      AvmAssert(giid !=
                Isolate::POISON_DESC); // this can only happen if there are
                                       // Isolate::POISON_DESC currently running
                                       // workers or a bug.

      newIsolate = Isolate::newIsolate(
          giid, parent ? parent->getDesc() : Isolate::INVALID_DESC, this);

      SCOPE_LOCK(m_globals.m_isolateMap.m_lock) {
        m_globals.m_isolateMap.InsertItem(giid,
                                          FixedHeapRef<Isolate>(newIsolate));
      }
#ifdef _DEBUG
      vmbase::VMThread *t;
#endif // _DEBUG
      AvmAssert(m_activeIsolateThreadMap.LookupItem(giid, &t) == false);
      AvmAssert(parent == NULL ||
                AvmCore::getActiveCore()->getIsolate() == parent);
      if (parent == NULL) {
...
void Aggregate::initialize(AvmCore *targetCore, Isolate *current) {
  ...
  SCOPE_LOCK_NAMED(locker, m_globals.m_lock) {
    ...
    AvmAssert(m_globals.getIsolateForID(current->getDesc()) == NULL ||
              m_globals.getIsolateForID(current->getDesc())->m_state >
                  Isolate::NEW);
  }
...
EnterSafepointManager::EnterSafepointManager(AvmCore *core)
  ...
  if (isolate) {
    ...
    m_spRecord.setLocationAndDesc((int32_t *)&core->interrupted,
                                  core->getIsolateDesc());
...
EnterSafepointManager::~EnterSafepointManager() { leaveSafepoint(); }
...
void Aggregate::beforeCoreDeletion(Isolate *current) {
  ...
  AvmAssert(AvmCore::getActiveCore() == NULL ||
            AvmCore::getActiveCore()->getIsolate() == current);
...
void Aggregate::afterGCDeletion(Isolate *current) {
  SCOPE_LOCK(m_globals.m_lock) {
    ...
    vmbase::VMThread *t;
#endif // _DEBUG
    AvmAssert(current->getDesc() == m_primordialGiid ||
              m_activeIsolateThreadMap.LookupItem(current->getDesc(), &t) ==
                  true);
    if (current->m_state != Isolate::EXCEPTION &&
        current->m_state != Isolate::FAILED) {
...
GCRef<ObjectVectorObject> Aggregate::listWorkers(Toplevel *toplevel) {
  ...
  GCRef<ObjectVectorObject> workerVector =
      toplevel->vectorClass()->newVector(workerClass);
  ...
  public:
    IsolateLister(GCRef<ObjectVectorObject> workerVector, Toplevel *toplevel)
        : m_workerVector(workerVector), m_toplevel(toplevel), m_index(0) {}
    virtual void each(Isolate::descriptor_t, FixedHeapRef<Isolate> isolate) {
...
void Aggregate::runSafepointTaskHoldingIsolateMapLock(
    vmbase::SafepointTask *task) {
  SCOPE_LOCK(m_globals.m_isolateMap.m_lock) {
