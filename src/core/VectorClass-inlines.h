...
// ----------------------------
    ...
    *
    ...
    TypedVectorClassBase::getTypeTraits() const {
        return m_typeTraits;
    }
...
// ----------------------------
    ...
    template<>
    REALLY_INLINE OpaqueAtom

    TypedVectorConstants<OpaqueAtom>::nullValue() {
    ...
    TypedVectorConstants<OpaqueAtom>::undefinedValue() {
        return (OpaqueAtom) undefinedAtom;
    }

    template<class T> REALLY_INLINE T

    TypedVectorConstants<T>::nullValue() {
        return 0;
    }
    ...
    TypedVectorConstants<T>::undefinedValue() {
        return 0;
    }

// ----------------------------

    REALLY_INLINE bool VectorBaseObject::get_fixed() const { return m_fixed; }

    REALLY_INLINE void VectorBaseObject::set_fixed(bool f) { m_fixed = f; }
    ...
    *
    ...
    VectorBaseObject::getTypeTraits() const {
    ...
    REALLY_INLINE void VectorBaseObject::atomToValueKnown(Atom atom,
                                                          int32_t &value) {
        value = AvmCore::integer(atom);
    }
    ...
    VectorBaseObject::valueToAtom(const int32_t &value) const {
        return core()->intToAtom(value);
    }

    REALLY_INLINE void VectorBaseObject::atomToValue(Atom
    atom,
    uint32_t &value
    ) {
...
VectorBaseObject::valueToAtom(const uint32_t &value) const {
...
REALLY_INLINE void VectorBaseObject::atomToValueKnown(Atom
atom,
double &value
) {
value = AvmCore::number(atom);
...
VectorBaseObject::valueToAtom(const double &value) const {
...
avmplus::coerce(toplevel(), atom, m_vecClass
...
VectorBaseObject::valueToAtom(const OpaqueAtom &value) const {
    return (Atom)
    value;
}
...
TypedVectorObject<TLIST>::getUintPropertyFast(uint32_t
index) const {
return m_list.
get(index);
}
...
template<class TLIST>
REALLY_INLINE void
        TypedVectorObject<TLIST>::setUintPropertyFast(uint32_t
index,
typename TLIST::TYPE value
) {
m_list.
set(index, value
);
...
TypedVectorObject<TLIST>::get_length() const {
...
template<class TLIST>
REALLY_INLINE bool
        TypedVectorObject<TLIST>::hasUintProperty(uint32_t
index) const {
return
_hasUintProperty(index);
}

template<class TLIST>
REALLY_INLINE Atom
TypedVectorObject<TLIST>::getUintProperty(uint32_t
index) const {
return
_getUintProperty(index);
}

template<class TLIST>
REALLY_INLINE void TypedVectorObject<TLIST>::setUintProperty(uint32_t
index,
Atom value
) {
...
template <class TLIST>
REALLY_INLINE void
TypedVectorObject<TLIST>::registerDependentObjects(IMemorySampler *memSampler) {
  if (!m_created) {
    m_flushBeforeConstructionComplete = true;
    return;
  }

  memSampler->addDependentObject(this, &m_list);

  VectorBaseObject::registerDependentObjects(memSampler);
}
#endif

template<class TLIST>
REALLY_INLINE void
        TypedVectorObject<TLIST>::checkReadIndex_u(uint32_t
index) const {
...
//
// We assert statically that the maximum object size divided by the element size
// will never exceed 2^31-1.  This should always be true at the time of writing
// (Aug 2011), because the smallest object to go into a Vector is four bytes and
// the largest object size supported by MMgc is 4GB.

template<class TLIST>
REALLY_INLINE uint32_t
TypedVectorObject<TLIST>::checkReadIndex_i(int32_t
index) const {
static_assert((MMgc::GCHeap::kMaxObjectSize / sizeof(typename TLIST::TYPE)) <=
              0x7FFFFFFFU,
              "Optimizations in the vector code depend on vectors being "
              "shorter than 2^31 elements");
uint32_t const limit = m_list.length();
// See comment above
if (
uint32_t(index)
...
TypedVectorObject<TLIST>::checkReadIndex_d(double index) const {
...
template<class TLIST>
REALLY_INLINE void
        TypedVectorObject<TLIST>::checkWriteIndex_u(uint32_t
index) const {
// If we are 'fixed', we can't write past the end.
// If we are not 'fixed', legal to write exactly 1 past the end, growing as
// needed. Common case is we're in bounds, so fast-path that.  This matters.
...
#ifdef VMCFG_AOT
template <class TLIST>
REALLY_INLINE void TypedVectorObject<TLIST>::_setNativeIntPropertyFast(
...
template <class TLIST>
REALLY_INLINE bool
TypedVectorObject<TLIST>::canTakeFastPath(int32_t index) const {
...
template<class TLIST>
REALLY_INLINE uint32_t
TypedVectorObject<TLIST>::checkWriteIndex_i(int32_t
index) const {
// If we are 'fixed', we can't write past the end.
// If we are not 'fixed', legal to write exactly 1 past the end, growing as
// needed. Common case is we're in bounds, so fast-path that.  This matters.
...
if (
uint32_t(index)
>= length) // See comment above checkReadIndex_i
{
uint32_t const limit = length + 1 - uint32_t(m_fixed);
if (
uint32_t(index)
>= limit) // See comment above checkReadIndex_i
throwRangeError_i(index);
}
return
uint32_t(index);
...
TypedVectorObject<TLIST>::checkWriteIndex_d(double index) const {
    // If we are 'fixed', we can't write past the end.
    // If we are not 'fixed', legal to write exactly 1 past the end, growing as
    // needed.
    int32_t const index_i = int32_t(index);
...
REALLY_INLINE
VectorAccessor<TLIST, align>::VectorAccessor(TypedVectorObject <TLIST> *v)
        : m_vector(v) {}
...
VectorAccessor<TLIST, align>::addr() {
    if (m_vector == NULL)
        return (typename TLIST::TYPE *) NULL;
    if (align == 0)
        return m_vector->m_list.m_header.data->entries;
    return (
            typename TLIST::TYPE *) ((uintptr_t(
                    m_vector->m_list.m_header.data->entries) +
                                      (align - 1)) &
                                     ~(align - 1));
}
...
VectorAccessor<TLIST, align>::length() {
