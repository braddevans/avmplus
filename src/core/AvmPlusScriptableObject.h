...
namespace avmplus {
    ...
    // This really ought to be defined in Sampler.h, but is here to simplify
    // header-file inclusion order issues.
    ...
    public:
        ...
        /**
         * Objects that use any backbuffer(s) for their memory (e.g. an array, byte
         * array etc) that they want included in their size calculation should
         * override this method and call addDependentObject() in the passed in
         * IMemorySampler with each data item here so that reallocations on those data
         * items are tracked. This method will get called after the initial object
         * allocation is recorded. If a dependent object gets created at a later time,
         * this object can call IMemorySampler::addDependentObject() to add/change the
         * dependent objects.
         */
