...
namespace avmplus {
    ...
    REALLY_INLINE SlotStorageType

    TraitsBindings::SlotInfo::sst() const {
        return SlotStorageType(offsetAndSST & 0xf);
    }

    REALLY_INLINE uint32_t

    TraitsBindings::SlotInfo::offset() const {
        return (offsetAndSST >> 4) << 2;
    }

    REALLY_INLINE TraitsBindings::TraitsBindings(
            Traits *_owner, TraitsBindingsp _base, MultinameBindingHashtable *_bindings,
            uint32_t _slotCount, uint32_t _methodCount, bool typesValid)
            : owner(_owner), base(_base), m_bindings(_bindings), slotCount(_slotCount),
              methodCount(_methodCount), m_slotSize(0),
              m_typesValid(typesValid ? 1 : 0) {}

    REALLY_INLINE Traitsp
    TraitsBindings::getSlotTraits(uint32_t
    i) const {
...
REALLY_INLINE uint32_t
TraitsBindings::getSlotOffset(uint32_t
i) const {
AvmAssert(m_typesValid);
AvmAssert(i<slotCount);
return
...
REALLY_INLINE SlotStorageType
TraitsBindings::calcSlotAddrAndSST(uint32_t
i,
void *pin,
void *&pout
) const {
...
pin) + (offsetAndSST >> 4));
return
SlotStorageType(offsetAndSST
& 0xf);
}

REALLY_INLINE MethodInfo
*
TraitsBindings::getMethod(uint32_t
i) const {
AvmAssert(m_typesValid);
...
TraitsBindings::valueAt(int32_t
index) const {
return m_bindings->
valueAt(index);
}

REALLY_INLINE TraitsBindings::SlotInfo
*

TraitsBindings::getSlots() {
    AvmAssert(m_typesValid);
    return (SlotInfo * )(this + 1);
}

REALLY_INLINE const TraitsBindings::SlotInfo
*

TraitsBindings::getSlots() const {
    AvmAssert(m_typesValid);
    return (const SlotInfo *) (this + 1);
}

REALLY_INLINE TraitsBindings::BindingMethodInfo
*

TraitsBindings::getMethods() {
    AvmAssert(m_typesValid);
    return (BindingMethodInfo * )(getSlots() + slotCount);
}

REALLY_INLINE const TraitsBindings::BindingMethodInfo
*

TraitsBindings::getMethods() const {
    AvmAssert(m_typesValid);
    return (const BindingMethodInfo *) (getSlots() + slotCount);
}

REALLY_INLINE void TraitsBindings::setSlotInfo(uint32_t
i,
Traits *t,
        SlotStorageType
sst,
...
.
offsetAndSST = (offset << 2) | uint32_t(sst);
}

REALLY_INLINE void TraitsBindings::setMethodInfo(uint32_t
i,
MethodInfo *f
) {
AvmAssert(m_typesValid);
...
REALLY_INLINE
StTraitsBindingsIterator::StTraitsBindingsIterator(TraitsBindingsp tb)
        : StMNHTBindingIterator(tb->m_bindings), _tb(tb) {}

REALLY_INLINE
TraitsMetadata::TraitsMetadata(TraitsMetadatap _base, PoolObject *_residingPool,
                               MetadataPtr _metadata_pos, uint32_t _slotCount,
                               uint32_t _methodCount)
        : base(_base), residingPool(_residingPool), slotCount(_slotCount),
          methodCount(_methodCount), metadataPos(_metadata_pos),
          slotMetadataPos(NULL), methodMetadataPos(NULL) {}

REALLY_INLINE TraitsMetadata::MetadataPtr
TraitsMetadata::getMetadataPos(PoolObject
*&residingPool) const {
...
Traits::getSlotAreaSize() const {
    AvmAssert(m_resolved);
    return getTotalSize() - m_sizeofInstance -
           (m_hashTableOffset ? sizeof(InlineHashtable) : 0);
}

// in bytes. includes size for all base classes too.
REALLY_INLINE uint32_t

Traits::getExtraSize() const {
    AvmAssert(m_resolved);
...
*
Traits::newParameterizedITraits(Stringp
name,
Namespacep ns,
        Traits
*param_traits) {
return
_newParameterizedTraits(name, ns,
this, param_traits);
}

REALLY_INLINE Traits
*
Traits::newParameterizedCTraits(Stringp
name,
Namespacep ns
) {
return
...
REALLY_INLINE TraitsBindingsp

Traits::getTraitsBindings() {
...
REALLY_INLINE TraitsMetadatap

Traits::getTraitsMetadata() {
...
REALLY_INLINE bool Traits::containsInterface(Traitsp t) { return subtypeof(t); }
...
Traits::getBuiltinType(const Traitsp t) {
...
REALLY_INLINE TraitsPosType

Traits::posType() const {
    return (TraitsPosType)(uint32_t)
    m_posType;
}

REALLY_INLINE bool Traits::isResolved() const { return m_resolved; }
...
REALLY_INLINE CreateClassClosureProc

Traits::getCreateClassClosureProc() const {
    return m_createClassClosure;
}

REALLY_INLINE Namespacep

Traits::ns() const { return _ns; }

REALLY_INLINE Stringp

Traits::name() const { return _name; }

REALLY_INLINE void Traits::set_names(Namespacep
p_ns,
Stringp p_name
) {
_ns = p_ns;
...
REALLY_INLINE bool Traits::isDictionary() const { return m_isDictionary != 0; }

REALLY_INLINE void Traits::set_isDictionary() { m_isDictionary = true; }
