...
namespace avmplus {
DictionaryClass::DictionaryClass(VTable *vtable) : ClassClosure(vtable) {
...
void DictionaryObject::init(bool weakKeys) {
  ...
  HeapHashtable *ht =
      weakKeys ? WeakKeyHashtable::create(gc) : HeapHashtable::create(gc);
...
int DictionaryObject::nextNameIndex(int index) {
  ...
  // hht could be null if you break in debugger in a subclasses constructor
  // before super has been called -- let's do it in all builds, it's better than
  // crashing.
  HeapHashtable *hht = getHeapHashtable();
