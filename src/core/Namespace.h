 ...
 */
    ...
    friend
    class MultinameHashtable;

    template<class VALUE_TYPE> friend
    class Quad;
    ...
    uintptr_t GC_POINTER(
            m_uriAndType); // Uses 3 bits for flags, but otherwise is really a Stringp
    ...
    Namespace(Atom
    prefix,
    Stringp uri, NamespaceType
    type,
    ApiVersion apiVersion
    );

    public:
    static REALLY_INLINE Namespace
    *
    create(MMgc::GC
    *gc,
    Atom prefix, Stringp
    ...
    Namespace(prefix, uri, type, apiVersion
    );
}
...
get_prefix() const { return m_prefix; }

REALLY_INLINE Stringp

get_uri() const {
    return (Stringp)
    atomPtr(m_uriAndType);
}

REALLY_INLINE Atom

getPrefix() const { return get_prefix(); }

REALLY_INLINE Stringp

getURI() const { return get_uri(); }

REALLY_INLINE ApiVersion

getApiVersion() { return m_apiVersion; }

Atom atom() const {
    return AtomConstants::kNamespaceType | (Atom)
    this;
}
...
REALLY_INLINE bool isPrivate() const { return ISNULL(m_prefix); }

REALLY_INLINE NamespaceType
