 ...
 */
    ...
    protected:
    FunctionClass(VTable
    *cvtable);

    public:
    REALLY_INLINE static FunctionClass
    *
    create(MMgc::GC
    *gc,
    VTable *cvtable
...
protected:
FunctionObject(VTable
*cvtable,
MethodEnv *call
);

public:
REALLY_INLINE static FunctionObject
*
create(MMgc::GC
*gc,
VTable *vtable,
...
FunctionObject(vtable, call
);
}

/** Implements ScriptObject::call called directly via ptr */
static Atom callFunction(FunctionObject * , int
argc,
Atom *argv
);
...
virtual Atom
call(int argc, Atom *argv); // overrides ClassClosure.call, ScriptObject.call
...
/** Implements get_coerced_receiver specifically for concrete FunctionObject
 * instances */
...
private:
GCMember <MethodEnv>
        GC_POINTER(m_callEnv); // can be null (e.g. WeakMethodClosure)
 ...
 * extend AS3 class Function.  Their C++ class must extend FunctionObject via
 * this wrapper.
 */
    ...
    public:
        ...
        /**
         * callHostFunction() intercepts calls via m_call_ptr, then delegates back to
         * the HostFunctionObject::call virtual function, because HostFunctionObjects
         * do not use the FunctionProc / m_call_ptr protocol.
         */
