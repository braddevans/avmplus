 ...
 *
 * Selects an architecture that allows load/store of unaligned 16- and 32-bit
 * ints.
 *
 * While it's OK for an unaligned int load or store to be slower than an aligned
 * load or store, we require that:
 *
 * - an unaligned load/store MUST NOT generate a run-time fault, and
 * - an unaligned load/store MUST be at least as efficient as separate
 * instructions to load and assemble the word one byte at a time / disassemble
 * and store one byte at a time.
 *
 * If you cannot guarantee that the requirements are met then DO NOT enable
 * this feature.  (For example, on Palm Pre unaligned loads/stores are allowed
 * but they're reportedly so slow that they're pointless.)
 *
 * Code that uses this feature MUST NOT use it as a license to load/store
 * floating-point data using integer instructions, since in general that will
 * not work.  In particular this classical pattern will not work:
 *
 ...
 * ...
 *
 * The reason it won't work is that some compilers (notably on ARM) will
 * generate code that uses a floating-point load into an FP register, so the
 * code actually needs unaligned floating-point loads to be supported,
 * AVMSYSTEM_UNALIGNED_FP_ACCESS. (Whether it is correct for the compiler to
 * generate that code is beside the point.)
 *
 * The prohibition applies to 64-bit loads/stores as well (expressed as pairs of
 * uint32_t loads/stores): ARM compilers as of October 2011 will rewrite a pair
 * of loads into a uint32_t array in a union with a return of a double from the
 * union as a single double load into the return register.  See comments
 * throughout the code as well as Bugzilla 569691 and 685441.
 */
#if !defined AVMSYSTEM_UNALIGNED_INT_ACCESS || \
    AVMSYSTEM_UNALIGNED_INT_ACCESS != 0 && AVMSYSTEM_UNALIGNED_INT_ACCESS != 1
#error "AVMSYSTEM_UNALIGNED_INT_ACCESS must be defined and 0 or 1 (only)."
#endif

/* AVMSYSTEM_UNALIGNED_FP_ACCESS
 *
 * Selects an architecture that allows load/store of unaligned 32- and 64-bit
 * floats.
 *
 * While it's OK for an unaligned floating-point load/store to be slower than an
 * aligned load/store, we require that:
 *
 * - an unaligned load/store MUST NOT generate a run-time fault, and
 * - an unaligned load/store MUST be at least as efficient as separate
 * instructions to load and assemble the datum one byte at a time / disassemble
 * and store one byte at a time.
 *
 * If you cannot guarantee that the requirements are met then DO NOT enable
 * this feature.
 *
 * Note that if AVMSYSTEM_UNALIGNED_FP_ACCESS is not set then it is assumed that
 * 64-bit floats require 8-byte alignment.
 *
 * Note that AVMSYSTEM_UNALIGNED_FP_ACCESS does not apply to float4 values. Some
 * SIMD units have different instructions for aligned and unaligned access; on
 * some systems the alignment requirement is 16 bytes, on others it's 8 bytes.
 * But as of November 2011 all C++ compilers we use will assume such alignment
 * when manipulating float4 values and will not use the instructions for
 * unaligned access even if they are available.  C++ code must never assume that
 * unaligned access is OK is appropriate for float4 data.
 */
#if !defined AVMSYSTEM_UNALIGNED_FP_ACCESS || \
    AVMSYSTEM_UNALIGNED_FP_ACCESS != 0 && AVMSYSTEM_UNALIGNED_FP_ACCESS != 1
 ...
 */
#if !defined AVMSYSTEM_BIG_ENDIAN || \
    AVMSYSTEM_BIG_ENDIAN != 0 && AVMSYSTEM_BIG_ENDIAN != 1
 ...
 * is stored at the lowest byte address of the word
 */
#if !defined AVMSYSTEM_LITTLE_ENDIAN || \
    AVMSYSTEM_LITTLE_ENDIAN != 0 && AVMSYSTEM_LITTLE_ENDIAN != 1
 ...
 */
#if !defined AVMSYSTEM_DOUBLE_MSW_FIRST || \
    AVMSYSTEM_DOUBLE_MSW_FIRST != 0 && AVMSYSTEM_DOUBLE_MSW_FIRST != 1
 ...
 * Selects Symbian
 */
#if !defined AVMSYSTEM_SYMBIAN || \
    AVMSYSTEM_SYMBIAN != 0 && AVMSYSTEM_SYMBIAN != 1
 ...
 */
#if !defined AVMFEATURE_DEBUGGER || \
    AVMFEATURE_DEBUGGER != 0 && AVMFEATURE_DEBUGGER != 1
 ...
 */
#if !defined AVMFEATURE_DEBUGGER_STUB || \
    AVMFEATURE_DEBUGGER_STUB != 0 && AVMFEATURE_DEBUGGER_STUB != 1
 ...
 */
#if !defined AVMFEATURE_ALLOCATION_SAMPLER || \
    AVMFEATURE_ALLOCATION_SAMPLER != 0 && AVMFEATURE_ALLOCATION_SAMPLER != 1
 ...
 */
#if !defined AVMFEATURE_HALFMOON || \
    AVMFEATURE_HALFMOON != 0 && AVMFEATURE_HALFMOON != 1
 ...
 *
 * Allows the default JIT compilation policy to be overriden with alternate
 * rules. In shell builds, this enables the -policy option which allows one to
 * specify which methods should be compiled and which should be interpreted.
 * There are currently three means of identifying a method to be controlled;
 * unique id, exact name match, and regular expression name match.
 */
#if !defined AVMFEATURE_COMPILEPOLICY || \
    AVMFEATURE_COMPILEPOLICY != 0 && AVMFEATURE_COMPILEPOLICY != 1
 ...
 */
#if !defined AVMFEATURE_HALFMOON_AOT_RUNTIME || \
    AVMFEATURE_HALFMOON_AOT_RUNTIME != 0 && \
        AVMFEATURE_HALFMOON_AOT_RUNTIME != 1
 ...
 */
#if !defined AVMFEATURE_HALFMOON_AOT_COMPILER || \
    AVMFEATURE_HALFMOON_AOT_COMPILER != 0 && \
        AVMFEATURE_HALFMOON_AOT_COMPILER != 1
 ...
 * exclusively by AOT.
 */
#if !defined AVMFEATURE_BUFFER_GUARD || \
    AVMFEATURE_BUFFER_GUARD != 0 && AVMFEATURE_BUFFER_GUARD != 1
 ...
 */
#if !defined AVMFEATURE_ABC_INTERP || \
    AVMFEATURE_ABC_INTERP != 0 && AVMFEATURE_ABC_INTERP != 1
 ...
 * interpreter for some or all methods and are not exceptionally
 * memory-constrained.
 */
#if !defined AVMFEATURE_WORDCODE_INTERP || \
    AVMFEATURE_WORDCODE_INTERP != 0 && AVMFEATURE_WORDCODE_INTERP != 1
 ...
 * compiler all do.
 */
#if !defined AVMFEATURE_THREADED_INTERP || \
    AVMFEATURE_THREADED_INTERP != 0 && AVMFEATURE_THREADED_INTERP != 1
#error "AVMFEATURE_THREADED_INTERP must be defined and 0 or 1 (only)."
#endif

/* AVMFEATURE_SELFTEST
 *
 * AVMFEATURE_SELFTEST enables the built-in selftests.  These can be run by
 * -Dselftest at the shell or by calling the global function
 * avmplus::selftests(), see extensions/Selftest.h. Mostly they are useful for
 * AVM development, not for embedders.
 *
 * Apart from code size considerations this can be enabled for release builds.
 */
#if !defined AVMFEATURE_SELFTEST || \
    AVMFEATURE_SELFTEST != 0 && AVMFEATURE_SELFTEST != 1
#error "AVMFEATURE_SELFTEST must be defined and 0 or 1 (only)."
#endif

/* AVMFEATURE_EVAL
 *
 * Select support for the AS3 run-time compiler.  NOT RECOMMENDED.  The run-time
 * compiler is still undergoing development.
 */
 ...
 */
#if !defined AVMFEATURE_TELEMETRY || \
    AVMFEATURE_TELEMETRY != 0 && AVMFEATURE_TELEMETRY != 1
#error "AVMFEATURE_TELEMETRY must be defined and 0 or 1 (only)."
#endif

/* AVMFEATURE_TELEMETRY_SAMPLER
 *
 * Select support for Telemetry based sampler, requires a Telemetry
 * implementation (to be used in host)
 */
#if !defined AVMFEATURE_TELEMETRY_SAMPLER || \
    AVMFEATURE_TELEMETRY_SAMPLER != 0 && AVMFEATURE_TELEMETRY_SAMPLER != 1
 ...
 */
#if !defined AVMFEATURE_PROTECT_JITMEM || \
    AVMFEATURE_PROTECT_JITMEM != 0 && AVMFEATURE_PROTECT_JITMEM != 1
#error "AVMFEATURE_PROTECT_JITMEM must be defined and 0 or 1 (only)."
#endif

/* AVMFEATURE_SHARED_GCHEAP
 *
 * Selects locking around calls to the memory block manager (GCHeap), allowing
 * multiple threads to share the block manager.  Any client with more than one
 * thread that uses MMgc either for garbage collected or manually managed memory
 * wants this; the Flash Player requires it.
 */
#if !defined AVMFEATURE_SHARED_GCHEAP || \
    AVMFEATURE_SHARED_GCHEAP != 0 && AVMFEATURE_SHARED_GCHEAP != 1
#error "AVMFEATURE_SHARED_GCHEAP must be defined and 0 or 1 (only)."
#endif

/* AVMFEATURE_USE_SYSTEM_MALLOC
 *
 * Make MMgc's overridden global new and delete operators delegate allocation
 * and deallocation to VMPI_alloc and VMPI_free instead of going to FixedMalloc.
 *
 * Whether you want this or not probably depends on the performance of the
 * underlying malloc and might depend on memory consumption patterns.  On
 * desktop systems you probably want this to be disabled.
 */
#if !defined AVMFEATURE_USE_SYSTEM_MALLOC || \
    AVMFEATURE_USE_SYSTEM_MALLOC != 0 && AVMFEATURE_USE_SYSTEM_MALLOC != 1
 ...
 * this means decorating the global new and delete operator with appropriate
 * 'throw' clauses.  It is unlikely to mean anything more, as AVM+ and MMgc do
 * not use and do not generally support C++ exceptions.
 *
 * Note that even if this is enabled, the global new and delete operators may
 * not throw exceptions when memory can't be allocated, because the
 * out-of-memory handling in MMgc may take precedence.
 *
 * FixedMalloc never throws an exception for a failed allocation.
 */
#if !defined AVMFEATURE_CPP_EXCEPTIONS || \
    AVMFEATURE_CPP_EXCEPTIONS != 0 && AVMFEATURE_CPP_EXCEPTIONS != 1
#error "AVMFEATURE_CPP_EXCEPTIONS must be defined and 0 or 1 (only)."
#endif

/* AVMFEATURE_INTERIOR_POINTERS
 *
 * Recognize a pointer or pointer-like value into anywhere in an object as
 * referencing that object during marking in the garbage collector.
 *
 * Enabling this tends to be increase GC cost but it can be a useful debugging
 * aid.
 */
#if !defined AVMFEATURE_INTERIOR_POINTERS || \
    AVMFEATURE_INTERIOR_POINTERS != 0 && AVMFEATURE_INTERIOR_POINTERS != 1
 ...
 * mainly a security issue, as alloca() will do strange things if given
 * sufficiently large requests.
 */
#if !defined AVMFEATURE_HEAP_ALLOCA || \
    AVMFEATURE_HEAP_ALLOCA != 0 && AVMFEATURE_HEAP_ALLOCA != 1
 ...
 * By default we use specialized new/delete operators and avoid global
 * new/delete.  However this requires some tricks to get multiple inheritance
 * and private destructors to work so some codebases may want to use the simpler
 * path of overriding global new/delete. Note that this feature works
 * independently of AVMFEATURE_USE_SYSTEM_MALLOC.
 */
#if !defined AVMFEATURE_OVERRIDE_GLOBAL_NEW || \
    AVMFEATURE_OVERRIDE_GLOBAL_NEW != 0 && AVMFEATURE_OVERRIDE_GLOBAL_NEW != 1
 ...
 * enabled at runtime.)
 */
#if !defined AVMFEATURE_MEMORY_PROFILER || \
    AVMFEATURE_MEMORY_PROFILER != 0 && AVMFEATURE_MEMORY_PROFILER != 1
 ...
 */
#if !defined AVMFEATURE_CACHE_GQCN || \
    AVMFEATURE_CACHE_GQCN != 0 && AVMFEATURE_CACHE_GQCN != 1
#error "AVMFEATURE_CACHE_GQCN must be defined and 0 or 1 (only)."
#endif

/* AVMFEATURE_VALGRIND
 *
 * Enabling this will compile in code to tell valgrind about how MMgc allocates
 * memory.
 */
#if !defined AVMFEATURE_VALGRIND || \
    AVMFEATURE_VALGRIND != 0 && AVMFEATURE_VALGRIND != 1
 ...
 */
#if !defined AVMFEATURE_SAFEPOINTS || \
    AVMFEATURE_SAFEPOINTS != 0 && AVMFEATURE_SAFEPOINTS != 1
 ...
 */
#if !defined AVMFEATURE_INTERRUPT_SAFEPOINT_POLL || \
    AVMFEATURE_INTERRUPT_SAFEPOINT_POLL != 0 && \
        AVMFEATURE_INTERRUPT_SAFEPOINT_POLL != 1
...
/* AVMTWEAK_PEPPER_MAC
 *
 * The Pepper plug-in environment on the Macintosh was set-up such that it
 * builds on the Mac as if it is a UNIX platform.  This is problematic as there
 * are special cases in the VM for the Mac that are required. Ideally, Pepper on
 * the Mac should be build as _MAC not UNIX. But that is not the case.
 */
...
#endif
#if AVMTWEAK_SELECTABLE_EXACT_TRACING != 0 && \
    AVMTWEAK_SELECTABLE_EXACT_TRACING != 1
...
#endif
#if AVMFEATURE_JIT
#if AVMSYSTEM_IA32 + AVMSYSTEM_AMD64 + AVMSYSTEM_ARM + AVMSYSTEM_PPC +         \
        AVMSYSTEM_SPARC + AVMSYSTEM_MIPS + AVMSYSTEM_SH4 !=                    \
    1
#error                                                                         \
    "Exactly one of AVMSYSTEM_IA32,AVMSYSTEM_AMD64,AVMSYSTEM_ARM,AVMSYSTEM_PPC,AVMSYSTEM_SPARC,AVMSYSTEM_MIPS,AVMSYSTEM_SH4 must be defined."
#endif
...
#error                                                                         \
    "Exactly one of AVMSYSTEM_AMD64, AVMSYSTEM_IA32,AVMSYSTEM_ARM must be defined."
#endif
...
#error                                                                         \
    "AVMFEATURE_SAFEPOINTS is required for AVMFEATURE_INTERRUPT_SAFEPOINT_POLL"
#endif
#endif
...
#error "AVMFEATURE_SWF19 is required for AVMFEATURE_SWF20"
    ...
    1
#error                                                                         \
    "At most one of AVMSYSTEM_IA32,AVMSYSTEM_AMD64,AVMSYSTEM_ARM,AVMSYSTEM_PPC,AVMSYSTEM_SPARC,AVMSYSTEM_MIPS,AVMSYSTEM_SH4 must be defined."
#endif
#if AVMFEATURE_HALFMOON_AOT_COMPILER + AVMFEATURE_AOT > 1
#error                                                                         \
    "At most one of AVMFEATURE_HALFMOON_AOT_COMPILER,AVMFEATURE_AOT must be defined."
#endif
#if AVMFEATURE_HALFMOON_AOT_COMPILER + AVMFEATURE_HALFMOON_AOT_RUNTIME > 1
#error                                                                         \
    "At most one of AVMFEATURE_HALFMOON_AOT_COMPILER,AVMFEATURE_HALFMOON_AOT_RUNTIME must be defined."
#endif
#if AVMFEATURE_WORDCODE_INTERP + AVMFEATURE_ABC_INTERP > 1
#error                                                                         \
    "At most one of AVMFEATURE_WORDCODE_INTERP,AVMFEATURE_ABC_INTERP must be defined."
#endif
#if AVMFEATURE_WORDCODE_INTERP + AVMFEATURE_JIT > 1
#error                                                                         \
    "At most one of AVMFEATURE_WORDCODE_INTERP,AVMFEATURE_JIT must be defined."
#endif
...
#error                                                                         \
    "At most one of AVMFEATURE_AOT,AVMFEATURE_WORDCODE_INTERP must be defined."
#endif
#if AVMTWEAK_EXACT_TRACING + AVMTWEAK_SELECTABLE_EXACT_TRACING > 1
#error                                                                         \
    "At most one of AVMTWEAK_EXACT_TRACING,AVMTWEAK_SELECTABLE_EXACT_TRACING must be defined."
#endif

#if AVMSYSTEM_UNIX + AVMSYSTEM_MAC + AVMSYSTEM_WIN32 + AVMSYSTEM_SYMBIAN != 1
#error                                                                         \
    "Exactly one of AVMSYSTEM_UNIX,AVMSYSTEM_MAC,AVMSYSTEM_WIN32,AVMSYSTEM_SYMBIAN must be defined."
#endif
