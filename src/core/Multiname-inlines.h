...
namespace avmplus {
    ...
    REALLY_INLINE void Multiname::setName(Stringp
    _name) {
    AvmAssert(_name
    && _name->
...
Multiname::namespaceCount() const {
...
REALLY_INLINE void Multiname::setNamespace(Namespacep
_ns) {
flags &= ~(NSSET | RTNS);
AvmAssert(_ns
!= NULL);
...
REALLY_INLINE void Multiname::setNsset(NamespaceSetp
_nsset) {
flags &= ~
RTNS;
flags |=
...
Multiname::getTypeParameter() const {
    AvmAssert(isParameterizedType());
    return next_index;
}

REALLY_INLINE void Multiname::setTypeParameter(uint32_t
index) {
flags |=
TYPEPARAM;
this->
...
REALLY_INLINE Multiname::Multiname(NamespaceSetp
nsset) {
this->
name = NULL;
this->
...
REALLY_INLINE Multiname::Multiname(const Multiname &other) { *this = other; }

REALLY_INLINE Multiname::Multiname(Namespacep
ns,
Stringp name
) {
AvmAssert(name
...
Multiname::isBinding() const {
    return !(flags & (ATTR | RTNS | RTNAME)) && name && ns;
}

REALLY_INLINE int32_t

Multiname::isRuntime() const {
    return flags & (RTNS | RTNAME);
}

REALLY_INLINE int32_t

Multiname::isRtns() const { return flags & RTNS; }

REALLY_INLINE int32_t

Multiname::isRtname() const { return flags & RTNAME; }

REALLY_INLINE int32_t

Multiname::isQName() const { return flags & QNAME; }

REALLY_INLINE bool Multiname::isAttr() const { return (bool) (flags & ATTR); }

REALLY_INLINE bool Multiname::isAnyName() const { return !isRtname() && !name; }
...
Multiname::isNsset() const { return flags & NSSET; }
...
REALLY_INLINE bool Multiname::isValidDynamicName() const {
    ...
    return ((flags & (ATTR | RTNAME)) == 0) && name != NULL &&
           containsAnyPublicNamespace();
}

REALLY_INLINE int32_t

Multiname::isParameterizedType() const {
...
REALLY_INLINE HeapMultiname::HeapMultiname() { /* our embedded Multiname inits
                                                  itself to all zero */
}
...
REALLY_INLINE HeapMultiname::operator const Multiname &() const { return name; }

REALLY_INLINE const HeapMultiname
&

HeapMultiname::operator=(const HeapMultiname &that) {
    if (this != &that) {
...
HeapMultiname::operator=(const Multiname &that) {
    setMultiname(that);
    return *this;
}

REALLY_INLINE void HeapMultiname::gcTrace(MMgc::GC *gc) { name.gcTrace(gc); }

REALLY_INLINE Stringp

HeapMultiname::getName() const { return name.getName(); }

REALLY_INLINE int32_t

HeapMultiname::namespaceCount() const {
    return name.namespaceCount();
}

REALLY_INLINE Namespacep
HeapMultiname::getNamespace(int32_t
i) const {
return name.
getNamespace(i);
}
...
REALLY_INLINE bool HeapMultiname::contains(Namespacep
ns) const {
return name.
contains(ns);
}
...
HeapMultiname::ctFlags() const { return name.ctFlags(); }

REALLY_INLINE int32_t

HeapMultiname::isBinding() const {
    return name.isBinding();
}

REALLY_INLINE int32_t

HeapMultiname::isRuntime() const {
    return name.isRuntime();
}

REALLY_INLINE int32_t

HeapMultiname::isRtns() const { return name.isRtns(); }

REALLY_INLINE int32_t

HeapMultiname::isRtname() const {
    return name.isRtname();
}

REALLY_INLINE int32_t

HeapMultiname::isQName() const { return name.isQName(); }

REALLY_INLINE bool HeapMultiname::isAttr() const { return name.isAttr(); }

REALLY_INLINE bool HeapMultiname::isAnyName() const { return name.isAnyName(); }
...
HeapMultiname::isNsset() const { return name.isNsset(); }
