...
// change now.)
    ...
    check_unbox(MethodEnv
    *env,
    bool u
    );

    REALLY_INLINE uintptr_t
    _AvmThunkUnbox_AvmReceiver(MethodEnv
    *env,
    uintptr_t r
    ) {
    check_unbox(env,
...
#define AvmThunkUnbox_AvmReceiver(t, r)                                        \
  ((t)(_AvmThunkUnbox_AvmReceiver(env, uintptr_t(r))))

REALLY_INLINE uintptr_t
_AvmThunkUnbox_AvmAtomReceiver(MethodEnv
*env,
uintptr_t r
) {
check_unbox(env,
...
#define AvmThunkUnbox_AvmAtomReceiver(t, r)                                    \
  ((t)(_AvmThunkUnbox_AvmAtomReceiver(env, uintptr_t(r))))
#else
...
// trick, since values are compile-time known we usually don't need to call
// intToAtom, can statically transform them good for ints and ints currently
...
#define AvmThunkCoerce_INT_ATOM(v)                                             \
  (AvmThunkCanBeSmallIntAtom(v) ? AvmThunkSmallIntAtom(v)                      \
                                : env->core()->intAtom(v))

#define AvmThunkCoerce_UINT_DOUBLE(v) double(v)
#define AvmThunkCoerce_UINT_INT(v) int32_t(v)
#define AvmThunkCoerce_UINT_ATOM(v)                                            \
  (AvmThunkCanBeSmallIntAtom(v) ? AvmThunkSmallIntAtom(v)                      \
                                : env->core()->intAtom(v))
...
template <typename TO, typename FROM> TO force_cast(FROM from) {
  typedef int checkSameSize[sizeof(TO) >= sizeof(FROM)
                                ? 1
                                : -1]; // simple static assert
  union {
    FROM from;
    TO to;
  } u = {from};
...
public:
    NativeInitializer(AvmCore *core, char const *const *versioned_uris,
#ifdef VMCFG_AOT
            const AOTInfo *aotInfo,
#else
                      const uint8_t *abcData, uint32_t abcDataLen,
#endif
                      uint32_t methodCount, uint32_t classCount);

    ~NativeInitializer();

    PoolObject *parseBuiltinABC(Domain *domain);

    const NativeMethodInfo *getNativeInfo(uint32_t i) const {
        return get_method(i);
    }

#ifdef VMCFG_AOT
    bool getCompiledInfo(NativeMethodInfo *info,
                         AvmThunkNativeHandler *handlerOut,
                         Multiname &returnTypeName, uint32_t i) const;
...
#else
    REALLY_INLINE const NativeClassInfo
    *
    get_class(uint32_t
    i) const {
        AvmAssert(i < classCount);
        return classes[i];
    }
#endif
...
private:
    ...
    REALLY_INLINE const NativeMethodInfo
    *
    get_method(uint32_t
    i) const {
...
#define _NATIVE_METHOD_CAST_PTR(CLS, PTR)                                      \
  reinterpret_cast<AvmThunkNativeMethodHandler>(                               \
      force_cast<void (CLS::*)()>(PTR))
...
#define _AVMTHUNK_NATIVE_METHOD(CLS, METHID, IMPL)                             \
  {{_NATIVE_METHOD_CAST_PTR(CLS, &IMPL)},                                      \
   (GprMethodProc)avmplus::NativeID::METHID##_thunk,                           \
   avmplus::NativeID::METHID},
#else
#ifdef VMCFG_TELEMETRY_SAMPLER
#define _AVMTHUNK_NATIVE_METHOD(CLS, METHID, IMPL)                             \
  {(GprMethodProc)avmplus::NativeID::METHID##_thunk,                           \
   (GprMethodProc)avmplus::NativeID::METHID##_sampler_thunk,                   \
   avmplus::NativeID::METHID},
#else
#define _AVMTHUNK_NATIVE_METHOD(CLS, METHID, IMPL)                             \
...
#define AVMTHUNK_NATIVE_FUNCTION(METHID, IMPL)                                 \
  {{force_cast<AvmThunkNativeMethodHandler>(IMPL)},                            \
   (GprMethodProc)avmplus::NativeID::METHID##_thunk,                           \
   avmplus::NativeID::METHID},
...
#define AVMTHUNK_NATIVE_CLASS(CLSID, CLS, FQCLS, OFFSETOFSLOTSCLS, INST, \
                              OFFSETOFSLOTSINST, CUSTOMCONSTRUCT, \
                              RESTRICTEDINHERITANCE, ABSTRACTBASE)             \
...
#define AVMTHUNK_DEFINE_NATIVE_INITIALIZER(NAME)                               \
  PoolObject *initBuiltinABC_##NAME(AvmCore *core, Domain *domain) {           \
    NativeInitializer ninit(core, avmplus::NativeID::NAME##_versioned_uris,    \
...
protected:
    ...
    avmplus::ClassClosure
            *_classes[1]; // lying, really [_count]. written with explicit WBRCs.
protected:
    REALLY_INLINE ClassManifestBase(uint32_t count, avmplus::ScriptEnv *e)
            : _count(count), _env(e) {}

    ClassClosure *FASTCALL
    lazyInitClass(uint32_t
    class_id);
