 ...
 */
    ...
    //
    // - to access the 'length' property with either the public namespace or a
    // public
    ...
    // present.
    //
    // The rest array is stored in a distinguished local slot in the ABC code, and
    // we know which slot this is.  We assume ASC does not do something clever with
    // the array, eg, by moving it to a different local slot. (It is possible to do
    // better.)
    //
    // The analysis needs to prove the following:
    //
    // * If the rest local is read by a GETLOCAL and the rest array value is
    // produced,
    //   and if that value is consumed by an instruction (rather than being
    //   discarded by an exception) then that value must be consumed by a
    //   GETPROPERTY instruction of one of two forms:
    //
    //   - It reads a "length" property with a public namespace or a public
    //   namespace
    ...
    //
    // A bit on each value in the FrameState, isRestArray, tracks whether a value is
    // a rest array.  This bit is set by GETLOCAL.  The bit is checked by every
    // other instruction: the operands to the instruction are checked.  If the bit
    // is set but the instruction is not a GETPROPERTY of the correct form then the
    // analysis fails.  (This means the bit does not propagate with the value as the
    // value is duplicated, moved into locals, etc.)
    //
    ...
    public:
      ...
      bool getProperty(const FrameState *state, const Multiname &multiname,
                       int obj_offset);
    ...
    public:
        Verifier(MethodInfo *info, MethodSignaturep ms, Toplevel *toplevel,
                 AbcEnv *abc_env
    ...
    private:
        ...
        Toplevel *toplevel;
        AbcEnv *abc_env;
        typedef HeapSortedMap <UnmanagedPointerList<const uint8_t *>,
        UnmanagedPointerList<FrameState *>>
                BlockStatesType;
    ...
    private:
        ...
        void emitCallproperty(AbcOpcode opcode, int &sp, Multiname &multiname,
                              uint32_t multiname_index, uint32_t argc,
                              const uint8_t *pc);

        bool emitCallpropertyMethod(AbcOpcode opcode, Traits *t, Binding b,
                                    Multiname &multiname, uint32_t multiname_index,
                                    uint32_t argc, const uint8_t *pc);

        bool emitCallpropertySlot(AbcOpcode opcode, int &sp, Traits *t, Binding b,
                                  uint32_t argc, const uint8_t *pc);
        ...
        Binding findMathFunction(TraitsBindingsp math, const Multiname &name,
                                 Binding b, int argc);

        Binding findStringFunction(TraitsBindingsp string, const Multiname &name,
                                   Binding b, int argc);
        ...
        void verifyFailed(int errorID, Stringp a1 = 0, Stringp a2 = 0,
                          Stringp a3 = 0) const;
