...
namespace avmplus {
    enum ObjectEncoding {
        kAMF0 = 0, kAMF3 = 3, kEncodeDefault = kAMF3
    };

    enum Endian {
        kBigEndian = 0, kLittleEndian = 1
    };
...
#else
        // OPTIMIZEME: GCC 4.3+ have intrinsics we should use
        a = ((a & 0x000000ffUL) << 24) | ((a & 0x0000ff00UL) << 8) |
            ((a & 0x00ff0000UL) >> 8) | ((a & 0xff000000UL) >> 24);
#endif
    }
    ...
    public:
        ...
        DataIOBase()
                : m_objectEncoding(kEncodeDefault), m_endian(kBigEndian) {}

        virtual ~DataIOBase() {}

        REALLY_INLINE Endian

        GetEndian() const { return m_endian; }

        REALLY_INLINE void SetEndian(Endian endian) { m_endian = endian; }

        REALLY_INLINE ObjectEncoding

        GetObjectEncoding() const {
            return m_objectEncoding;
        }

        REALLY_INLINE void SetObjectEncoding(ObjectEncoding objectEncoding) {
...
#error
    ...
    protected:
        ...
        REALLY_INLINE void gcTrace(MMgc::GC *gc) { DataIOBase::gcTrace(gc); }
    };
