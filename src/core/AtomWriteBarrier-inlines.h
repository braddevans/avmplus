...
namespace avmplus {
    ...
    REALLY_INLINE AtomWB::AtomWB(Atom t) // : AtomWBCore(t) -- not necessary,
    // atomWriteBarrier_ctor handles it
    {
    ...
    REALLY_INLINE AtomWB::~AtomWB() { AvmCore::atomWriteBarrier_dtor(&m_atom); }

    REALLY_INLINE Atom
    AtomWB::operator=(Atom
    tNew) {
    MMgc::GC *const gc = MMgc::GC::GetGC(this);
    void *const container = gc->FindBeginningFast(this);
...
REALLY_INLINE void AtomWB::set(MMgc::GC *gc, const void *container,
                               Atom atomNew) {
    if (m_atom != atomNew) {
...
REALLY_INLINE AtomWB::operator const Atom &() const { return m_atom; }
...
AvmCore::atomWriteBarrier(gc, gc
->FindBeginningFast(this), &m_atom,
atomNew);
}
return
atomNew;
