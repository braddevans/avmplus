...
#define kHalfTimeDomain 8.64e15
        ...
        static inline double TimeClip(double t) {
            if (MathUtils::isInfinite(t) || MathUtils::isNaN(t) ||
                ((t < 0 ? -t : t) > kHalfTimeDomain)) {
        ...
        Date(const Date &toCopy) { m_time = toCopy.m_time; }
        ...
        Date(double year, double month, double date, double hours, double min,
        ...
        void setDate(double year, double month, double date, bool utcFlag);

        void setTime(double hours, double min, double sec, double msec, bool utcFlag);
    ...
    private:
        ...
        void format(wchar *buffer, const char *format, ...) const;
    };
