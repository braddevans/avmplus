...
MMGC_STATIC_ASSERT((sizeof languageNames / sizeof languageNames[0]) ==
                   kLanguages);

int errorMappingTable[2 * kNumErrorConstants] = {
    1000, 0,   1001, 1,   1002, 2,   1003, 3,   1004, 4,   1005, 5,   1006, 6,
    1007, 7,   1008, 8,   1009, 9,   1010, 10,  1011, 11,  1012, 12,  1013, 13,
    1014, 14,  1015, 15,  1016, 16,  1017, 17,  1018, 18,  1019, 19,  1020, 20,
    1021, 21,  1022, 22,  1023, 23,  1024, 24,  1025, 25,  1026, 26,  1027, 27,
    1028, 28,  1029, 29,  1030, 30,  1031, 31,  1032, 32,  1033, 33,  1034, 34,
    1035, 35,  1037, 36,  1038, 37,  1039, 38,  1040, 39,  1041, 40,  1042, 41,
    1043, 42,  1044, 43,  1045, 44,  1046, 45,  1047, 46,  1049, 47,  1050, 48,
    1051, 49,  1052, 50,  1053, 51,  1054, 52,  1056, 53,  1057, 54,  1058, 55,
    1059, 56,  1060, 57,  1061, 58,  1063, 59,  1064, 60,  1065, 61,  1066, 62,
    1067, 63,  1068, 64,  1069, 65,  1070, 66,  1071, 67,  1072, 68,  1073, 69,
    1074, 70,  1075, 71,  1076, 72,  1077, 73,  1078, 74,  1079, 75,  1080, 76,
    1081, 77,  1082, 78,  1083, 79,  1084, 80,  1085, 81,  1086, 82,  1087, 83,
    1088, 84,  1089, 85,  1090, 86,  1091, 87,  1092, 88,  1093, 89,  1094, 90,
    1095, 91,  1096, 92,  1097, 93,  1098, 94,  1100, 95,  1101, 96,  1102, 97,
    1103, 98,  1104, 99,  1107, 100, 1108, 101, 1109, 102, 1110, 103, 1111, 104,
    1112, 105, 1113, 106, 1114, 107, 1115, 108, 1116, 109, 1117, 110, 1118, 111,
    1119, 112, 1120, 113, 1121, 114, 1122, 115, 1123, 116, 1124, 117, 1125, 118,
    1126, 119, 1127, 120, 1128, 121, 1129, 122, 1131, 123, 1132, 124, 1500, 125,
    1501, 126, 1502, 127, 1503, 128, 1504, 129, 1505, 130, 1506, 131, 1507, 132,
    1508, 133, 1510, 134, 1511, 135, 1512, 136, 1513, 137, 1514, 138, 1515, 139,
    1516, 140, 1517, 141, 1518, 142, 1519, 143, 1520, 144, 1521, 145, 2004, 146,
    2006, 147, 2007, 148, 2008, 149, 2012, 150, 2030, 151, 2058, 152, 2085, 153,
    2088, 154, 2089, 155, 2090, 156, 2091, 157, 2092, 158, 2093, 159, 2105, 160,
    2106, 161, 2107, 162, 2108, 163, 2173, 164, 3735, 165};
...
const char *errorConstants[][kNumErrorConstants] = {
    ...
    {/*1000,0*/ "The system is out of memory.",
     ...
     /*1002,2*/
     "Number.toPrecision has a range of 1 to 21. Number.toFixed and "
     "Number.toExponential have a range of 0 to 20. Specified value is not "
     ...
     /*1073,69*/
     "Non-override method %1 replaced because of duplicate disp_id %2.",
     ...
     /*1084,80*/
     "Element or attribute (\"%1\") does not match QName production: "
     "QName::=(NCName':')?NCName.",
     /*1085,81*/
     "The element type \"%1\" must be terminated by the matching end-tag "
     "\"</%2>\".",
     ...
     /*1109,102*/
     "Attempt to directly call unbound function %1 from method %2.",
     /*1110,103*/ "%1 cannot extend %2.",
     /*1111,104*/ "%1 cannot implement %2.",
     /*1112,105*/
     "Argument count mismatch on class coercion.  Expected 1, got %1.",
     /*1113,106*/
     ...
     /*1124,117*/
     "OP_hasnext2 requires object and index to be distinct registers.",
     ...
     /*1131,123*/
     "Replacer argument to JSON stringifier must be an array or a two "
     "parameter function.",
     /*1132,124*/ "Invalid JSON parse input.",
     /*1500,125*/ "Error occurred opening file %1.",
     /*1501,126*/ "Error occurred writing to file %1.",
     /*1502,127*/
     "A script has executed for longer than the default timeout period of 15 "
     "seconds.",
     /*1503,128*/
     "A script failed to exit after 30 seconds and was terminated.",
     ...
     /*1510,134*/
     "When the callback argument is a method of a class, the optional this "
     ...
     /*1517,141*/
     "Condition cannot notifyAll if associated mutex is not owned.",
     ...
     /*2088,154*/
     "The Proxy class does not implement getProperty. It must be overridden by "
     "a subclass.",
     /*2089,155*/
     "The Proxy class does not implement setProperty. It must be overridden by "
     "a subclass.",
     /*2090,156*/
     "The Proxy class does not implement callProperty. It must be overridden "
     "by a subclass.",
     /*2091,157*/
     "The Proxy class does not implement hasProperty. It must be overridden by "
     "a subclass.",
     /*2092,158*/
     "The Proxy class does not implement deleteProperty. It must be overridden "
     "by a subclass.",
     /*2093,159*/
     "The Proxy class does not implement getDescendants. It must be overridden "
     "by a subclass.",
     /*2105,160*/
     "The Proxy class does not implement nextNameIndex. It must be overridden "
     "by a subclass.",
     /*2106,161*/
     "The Proxy class does not implement nextName. It must be overridden by a "
     "subclass.",
     /*2107,162*/
     "The Proxy class does not implement nextValue. It must be overridden by a "
     "subclass.",
     /*2108,163*/ "The value %1 is not a valid Array length.",
     /*2173,164*/
     "Unable to read object in stream.  The class %1 does not implement "
    ...
    {/*1000,0*/ "La mémoire du système est saturée.",
     ...
     /*1002,2*/
     "L'argument precision doit être compris entre %2 et %3. %1 n'est pas "
     "valide.",
     ...
     /*1009,9*/
     "Il est impossible d'accéder à la propriété ou à la méthode d'une "
     ...
     /*1016,16*/
     "L'opérateur Descendants (..) n'est pas pris en charge sur le type %1.",
     ...
     /*1021,21*/
     "Au moins une cible branche ne figurait pas dans une instruction valide "
     "de la méthode.",
     ...
     /*1034,34*/
     "Echec de la contrainte de type : conversion de %1 en %2 impossible.",
     ...
     /*1040,39*/
     "L'expression à droite de instanceof doit être une classe ou une "
     "fonction.",
     ...
     /*1063,59*/
     "Non-correspondance du nombre d'arguments sur %1. %2 prévu(s), %3 "
     "détecté(s).",
     ...
     /*1069,65*/
     "La propriété %1 est introuvable sur %2 et il n'existe pas de valeur par "
     "défaut.",
     /*1070,66*/ "La méthode %1 est introuvable sur %2",
     /*1071,67*/ "La fonction %1 a déjà été liée à %2.",
     /*1072,68*/ "Disp_id 0 est illégal.",
     /*1073,69*/
     "La méthode de non-remplacement %1 a été supplantée en raison du doublon "
     "disp_id %2.",
     /*1074,70*/
     "Ecriture illégale dans une propriété en lecture seule %1 sur %2.",
     /*1075,71*/ "Math n'est pas une fonction.",
     /*1076,72*/ "Math n'est pas un constructeur.",
     /*1077,73*/
     "Lecture illégale d'une propriété en écriture seule %1 sur %2.",
     ...
     /*1084,80*/
     "L'élément ou l'attribut (\"%1\") ne correspond pas à la production QName "
     ": QName::=(NCName':')?NCName.",
     /*1085,81*/
     "Le type d'élément \"%1\" doit se terminer par la balise de fin "
     "correspondante \"</%2>\".",
     /*1086,82*/
     "La méthode %1 ne fonctionne qu'avec les listes composées d'un seul "
     "élément.",
     /*1087,83*/ "L'affectation à un élément XML indexé n'est pas autorisée.",
     /*1088,84*/
     "Le marquage du document après l'élément root doit être composé "
     "correctement.",
     /*1089,85*/
     "L'affectation à des listes composées de plus d'un élément n'est pas "
     "prise en charge.",
     /*1090,86*/
     ...
     /*1097,93*/
     "Echec de l'analyse XML : Instruction de traitement non terminée.",
     /*1098,94*/ "Préfixe %1 illégal sans espace de nom.",
     /*1100,95*/
     "Impossible de fournir des indicateurs lors de la construction d'un "
     "RegExp à partir d'un autre.",
     /*1101,96*/
     ...
     /*1107,100*/
     "Les données ABC sont corrompues, tentative de lecture en dehors des "
     "limites.",
     /*1108,101*/
     "L'élément opcode OP_newclass a été utilisé avec une classe de base "
     "incorrecte.",
     /*1109,102*/
     "Tentative d'appel direct d'une fonction non liée %1 à partir de la "
     "méthode %2.",
     /*1110,103*/ "%1 ne peut pas étendre %2.",
     /*1111,104*/ "%1 ne peut pas mettre en oeuvre %2.",
     /*1112,105*/
     "Non-concordance du nombre d'arguments pour contrainte de classe.  1 "
     "prévu, %1 détecté(s).",
     /*1113,106*/
     "OP_newactivation utilisé dans la méthode sans indicateur "
     "NEED_ACTIVATION.",
     /*1114,107*/
     "OP_getglobalslot ou OP_setglobalslot utilisé sans domaine global.",
     /*1115,108*/ "%1 n'est pas un constructeur.",
     /*1116,109*/
     "Le deuxième argument transmis à Function.prototype.apply doit être un "
     "tableau.",
     /*1117,110*/ "Nom XML non valide : %1.",
     /*1118,111*/ "Boucle cyclique illégale entre noeuds.",
     /*1119,112*/
     "L'opérateur delete n'est pas pris en charge avec une opérande de type "
     "%1.",
     /*1120,113*/ "Impossible de supprimer la propriété %1 sur %2.",
     /*1121,114*/ "La méthode %1 contient un corps de méthode dupliqué.",
     /*1122,115*/
     "La méthode d'interface %1 contient un corps de méthode illégal.",
     /*1123,116*/ "L'opérateur Filter n'est pas pris en charge sur le type %1.",
     /*1124,117*/
     "OP_hasnext2 requiert que l'objet et l'index soient des registres "
     "distincts.",
     ...
     /*1128,121*/
     "Nombre incorrect de paramètres type pour %1. %2 prévu(s), %3 détecté(s).",
     /*1129,122*/
     "Il est impossible de convertir la structure cyclique en chaîne JSON.",
     /*1131,123*/
     "L’argument Replacer vers le transformateur de chaînes JSON doit être un "
     "tableau ou une fonction à deux paramètres.",
     ...
     /*1502,127*/
     "La durée d'exécution d'un script excède le délai par défaut (15 "
     "secondes).",
     /*1503,128*/
     "Un script ne s'est pas terminé après 30 secondes et a été arrêté.",
     /*1504,129*/ "Fin du fichier.",
     /*1505,130*/
     "L'index de chaîne %1 sort des limites. Il doit être compris entre %2 et "
     "%3.",
     ...
     /*1510,134*/
     "Lorsque l’argument du rappel correspond à une méthode de classe, "
     "l’argument facultatif 'this' doit être null.",
     /*1511,135*/ "Le programme de travail a déjà démarré.",
     /*1512,136*/
     "Le démarrage d’un programme de travail ayant déjà échoué n’est pas pris "
     "en charge.",
     /*1513,137*/ "Le programme de travail est terminé.\"",
     /*1514,138*/
     "Méthode unlock() sans méthode lock() correspondante qui précède.",
     /*1515,139*/
     "Valeur de délai d’expiration de la condition non valide : %1.",
     /*1516,140*/
     "L’objet Condition ne peut pas utiliser la méthode notify si l’objet "
     "Mutex associé n’existe pas.",
     /*1517,141*/
     "L’objet Condition ne peut pas utiliser la méthode notifyAll si l’objet "
     "Mutex associé n’existe pas.",
     /*1518,142*/
     "L’objet Condition ne peut pas attendre si l’objet Mutex associé n’existe "
     "pas.",
     ...
     /*2088,154*/
     "La classe Proxy ne met pas en oeuvre getProperty. Elle doit être "
     "remplacée par une sous-classe.",
     /*2089,155*/
     "La classe Proxy ne met pas en oeuvre setProperty. Elle doit être "
     "remplacée par une sous-classe.",
     /*2090,156*/
     "La classe Proxy ne met pas en oeuvre callProperty. Elle doit être "
     "remplacée par une sous-classe.",
     /*2091,157*/
     "La classe Proxy ne met pas en oeuvre hasProperty. Elle doit être "
     "remplacée par une sous-classe.",
     /*2092,158*/
     "La classe Proxy ne met pas en oeuvre deleteProperty. Elle doit être "
     "remplacée par une sous-classe.",
     /*2093,159*/
     "La classe Proxy ne met pas en oeuvre getDescendants. Elle doit être "
     "remplacée par une sous-classe.",
     /*2105,160*/
     "La classe Proxy ne met pas en oeuvre nextNameIndex. Elle doit être "
     "remplacée par une sous-classe.",
     /*2106,161*/
     "La classe Proxy ne met pas en oeuvre nextName. Elle doit être remplacée "
     "par une sous-classe.",
     /*2107,162*/
     "La classe Proxy ne met pas en oeuvre nextValue. Elle doit être remplacée "
     "par une sous-classe.",
     /*2108,163*/ "La valeur %1 n’est pas une valeur de tableau valide.",
     /*2173,164*/
     "Impossible de lire l'objet dans le flux. La classe %1 n'implémente pas "
     "flash.utils.IExternalizable, mais est aliasée vers une classe "
     "externalisable.",
     /*3735,165*/
     "Cette API ne prend pas en charge les objets ByteArray partagés."},
#endif
#ifdef AVMPLUS_ERROR_LANG_ja
    // ja
    {/*1000,0*/ "システムのメモリ不足です。",
     ...
     /*1008,8*/
     "%1 があいまいです。一致するバインディングが複数見つかりました。",
     /*1009,9*/
     "null "
     "のオブジェクト参照のプロパティまたはメソッドにアクセスすることはできませ"
     "ん。",
     ...
     /*1063,59*/
     "%1 の引数の数が一致していません。%2 が必要ですが、%3 が指定されました。",
     /*1064,60*/
     ...
     /*1073,69*/
     "disp_id %2 が重複しているために、非オーバーライドメソッド %1 "
     "が置換されました。",
     ...
     /*1084,80*/
     "エレメントまたは属性 (\"%1\") が QName プロダクションと一致しません : "
     "QName::=(NCName':')?NCName。",
     /*1085,81*/
     "エレメント型 \"%1\" は対応する終了タグ \"</%2>\" "
     "で終了する必要があります。",
     /*1086,82*/
     "%1 メソッドは、単一のアイテムを含むリストに対してのみ使用できます。",
     /*1087,83*/ "インデックス付きの XML への代入は許可されません。",
     /*1088,84*/
     "ルートエレメントに続くドキュメントのマークアップは整形式でなければなりま"
     "せん。",
     ...
     /*1112,105*/
     "クラスの型変換に指定された引数の数が不正です。1 個必要ですが、%1 "
     "個指定されました。",
     /*1113,106*/
     "OP_newactivation が NEED_ACTIVATION "
     "フラグなしでメソッドで使用されました。",
     /*1114,107*/
     "グローバルスコープなしで OP_getglobalslot または OP_setglobalslot "
     "が使用されました。",
     /*1115,108*/ "%1 はコンストラクターではありません。",
     /*1116,109*/
     "Function.prototype.apply の 2 番目の引数は配列でなければなりません。",
     ...
     /*1124,117*/
     "OP_hasnext2 "
     "を明示的に登録するには、オブジェクトとインデックスが必要です。",
     /*1125,118*/ "インデックス %1 は %2 の範囲外です。",
     /*1126,119*/ "固定ベクターの長さは変更できません。",
     /*1127,120*/
     "非パラメーター化された型に対して型指定を実行しようとしました。",
     /*1128,121*/
     "%1 の型パラメーター数が正しくありません。%2 である必要がありますが、%3 "
     "が指定されました。",
     /*1129,122*/ "周期的な構成を JSON 文字列に変換できません。",
     /*1131,123*/
     "JSON stringifier への Replacer 引数は、1 つの配列または 2 "
     "つのパラメーターである必要があります。",
     ...
     /*1516,140*/
     "関連する mutex が所有されていない場合、条件は notify を実行できません。",
     /*1517,141*/
     "関連する mutex が所有されていない場合、条件は notifyAll "
     "を実行できません。",
     /*1518,142*/
     "関連する mutex が所有されていない場合、条件は wait を実行できません。",
     ...
     /*2088,154*/
     "Proxy クラスは、getProperty "
     "を実装しません。サブクラスでオーバーライドする必要があります。",
     /*2089,155*/
     "Proxy クラスは、setProperty "
     "を実装しません。サブクラスでオーバーライドする必要があります。",
     /*2090,156*/
     "Proxy クラスは、callProperty "
     "を実装しません。サブクラスでオーバーライドする必要があります。",
     /*2091,157*/
     "Proxy クラスは、hasProperty "
     "を実装しません。サブクラスでオーバーライドする必要があります。",
     /*2092,158*/
     "Proxy クラスは、deleteProperty "
     "を実装しません。サブクラスでオーバーライドする必要があります。",
     /*2093,159*/
     "Proxy クラスは、getDescendants "
     "を実装しません。サブクラスでオーバーライドする必要があります。",
     /*2105,160*/
     "Proxy クラスは、nextNameIndex "
     "を実装しません。サブクラスでオーバーライドする必要があります。",
     /*2106,161*/
     "Proxy クラスは、nextName "
     "を実装しません。サブクラスでオーバーライドする必要があります。",
     /*2107,162*/
     "Proxy クラスは、nextValue "
     "を実装しません。サブクラスでオーバーライドする必要があります。",
     /*2108,163*/ "値 %1 は有効な配列の長さではありません。",
     /*2173,164*/
     "ストリーム内のオブジェクトを読み取れません。クラス %1 は "
     "flash.utils.IExternalizable "
     "を実装しませんが、外部化可能なクラスにエイリアス処理されます。",
    ...
    {/*1000,0*/ "系统内存不足。",
     ...
     /*2173,164*/
     "无法读取流中的对象。类 %1 虽未实现 "
     "flash.utils.IExternalizable，但由其别名可得知它为 externalizable 类。",
     /*3735,165*/ "此 API 不接受共享的 ByteArray。"},
#endif
};

MMGC_STATIC_ASSERT((sizeof errorConstants / sizeof errorConstants[0]) ==
                   kLanguages);
