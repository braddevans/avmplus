...
MutexObject::State::State()
    : FixedHeapRCObject(), m_recursionCount(0),
      m_ownerThreadID(VMPI_nullThread()), m_lockWaitListHead(NULL),
...
void MutexObject::State::lock(Toplevel *toplevel) {
  ...
  if (isolate) {
    ...
    Isolate::InterruptibleState::Enter state(record, &m_interruptibleState,
                                             isolate);

    if (m_ownerThreadID != VMPI_currentThread()) {
      while (state.waitListHead() != &record ||
             VMPI_recursiveMutexTryLock(&m_mutex) == false) {
        state.wait();
...
bool MutexObject::State::unlock() {
  if (m_ownerThreadID != VMPI_currentThread()) {
    // Non-fenced read of the m_ownerThreadID field possibly outside of a
    // critical section. Writes to m_ownerThreadID occur only in a critical
    // section. If the current thread really holds the lock, then
    // m_ownerThreadID is set accurately. If the current thread doesn't hold the
    // lock, it can't see its own thread id in m_ownerThreadID, because either
    // it never held the lock and never wrote it, or it had held the lock, set
    // the thread id to null and unlocked (fenced). Only the current thread
    // writes the current thread id to m_ownerThreadID.
    return false;
  }
...
MutexObject::MutexObject(VTable *cvtable, ScriptObject *delegate)
    : ScriptObject(cvtable, delegate), m_state(NULL) {}
...
//    worker.setSharedProperty("foo", mutex) === worker.getSharedProperty("foo")
//    // always true
//
...
void MutexObject::lock() {
  DEBUG_STATE(("thread %d calling Mutex(%d).lock()\n", VMPI_currentThread(),
               m_state->m_interruptibleState.gid));
...
bool MutexObject::tryLock() {
  TELEMETRY_METHOD_NO_THRESHOLD(core()->getTelemetry(),
                                ".player.mutex.trylock");
...
ChannelItem *MutexObject::makeChannelItem() {
  ...
  public:
    MutexChannelItem(MutexObject::State *value) { m_value = value; }

    Atom getAtom(Toplevel *toplevel) const {
      MutexObject *mutexObj =
          toplevel->getInternedObject(m_value).staticCast<MutexObject>();
      if (mutexObj == NULL) {
        MutexClass *mutexClass = toplevel->builtinClasses()->get_MutexClass();
        mutexObj = new (toplevel->gc(), MMgc::kExact,
                        mutexClass->ivtable()->getExtraSize())
            MutexObject(mutexClass->ivtable(), mutexClass->prototypePtr());
...
MutexClass::MutexClass(VTable *cvtable) : ClassClosure(cvtable) {
...
ConditionObject::ConditionObject(VTable *cvtable, ScriptObject *delegate)
    : ScriptObject(cvtable, delegate), m_mutex(NULL), m_state(NULL) {}

ConditionObject::State::State(MutexObject::State *mutexState)
    : FixedHeapRCObject(), m_mutexState(mutexState) {}
...
bool ConditionObject::State::wait(int32_t millis, Isolate *isolate,
                                  Toplevel *toplevel) {
  ...
  if (isolate) {
    ...
    m_mutexState->m_recursionCount = 0;
    {
      ...
      Isolate::InterruptibleState::Enter state(record, &m_interruptibleState,
                                               isolate);
      AvmAssert(m_interruptibleState.getMonitor().isLockedByCurrentThread() ==
                true);
      DEBUG_STATE(("thread %d releasing Mutex(%d)\n", VMPI_currentThread(),
                   m_mutexState->m_interruptibleState.gid));
      ...
      result = state.result;
      // if we have been interrupted do not re-acquire the public lock, just
      // bail
      if (state.interrupted) {
...

    {
      ...
      Isolate::InterruptibleState::WaitRecord record;
      Isolate::InterruptibleState::Enter state(
          record, &m_mutexState->m_interruptibleState, isolate);

      while (state.waitListHead() != &record ||
             VMPI_recursiveMutexTryLock(&m_mutexState->m_mutex) == false) {
        state.wait();
        if (state.interrupted) {
          goto process_interrupt;
        }
...
process_interrupt:
  // because we should not long jump with a InterruptibleState::Enter on the
  // stack we need to destroy the RAII object before throwing
  isolate->getAggregate()->processWorkerInterrupt(toplevel);
  return result;
}

// There are two ways that a ConditionObject can be created
//  (1) directly from ActionScript - e.g. var condition:Condition= new
//  Condition(new Mutex()); (2) from native code as a result of deserialization
// The ctor() method was created to be called only in the first case where
// ActionScript is constructing the object directly.
// The second case occurs when a Condition is passed to another GC address space
// either using a MessageChannelObject or the WorkerObject.setSharedProperty
// and WorkerObject.getSharedProperty methods.  When using either of these
// two mechanisms the Condition and its associated mutex is serialized on a
// per-toplevel basis. Each toplevel contains a hash (see
// Toplevel::internObject) of the "backing state" of the ConditionObject, this
// hash is used to ensure that the "highlander" principle is adhered to (there
// can be only one!), ensuring that sending the same ActionScript object back
// and forth between GC address space returns the same object every time.
// Example:
//    var mutex:Mutex = new Mutex()
//    var cond:Condition = new Condition(mutex);
//    worker.setSharedProperty("foo", cond) === worker.getSharedProperty("foo");
//    // always true worker.getSharedProperty("foo").mutex === mutex; // always
//    true
//
void ConditionObject::ctor(GCRef<MutexObject> mutex) {
  ...
  if (mutex == NULL) {
    toplevel()->throwArgumentError(kNullPointerError,
                                   core()->newStringLatin1("mutex"));
  }
...
void ConditionObject::notify() {
  if (m_state->m_mutexState->m_ownerThreadID != VMPI_currentThread()) {
    DEBUG_STATE(("**ERROR*** thread %d calling Condition(%d).notify but "
                 "doesn't own mutex!\n",
...
void ConditionObject::notifyAll() {
  ...
  if (m_state->m_mutexState->m_ownerThreadID != VMPI_currentThread()) {
    DEBUG_STATE(("**ERROR*** thread %d calling Condition(%d).notifyAll but "
                 "doesn't own mutex!\n",
                 VMPI_currentThread(), m_state->m_interruptibleState.gid));
...
ChannelItem *ConditionObject::makeChannelItem() {
  ...
  public:
    ConditionChannelItem(ConditionObject::State *value) { m_value = value; }

    Atom getAtom(Toplevel *toplevel) const {
      ConditionObject *conditionObj =
          toplevel->getInternedObject(m_value).staticCast<ConditionObject>();
      if (conditionObj == NULL) {
        ConditionClass *conditionClass =
            toplevel->builtinClasses()->get_ConditionClass();
        conditionObj = new (toplevel->gc(), MMgc::kExact,
                            conditionClass->ivtable()->getExtraSize())
            ConditionObject(conditionClass->ivtable(),
                            conditionClass->prototypePtr());
        // will increment the refcount of m_state
...
GCRef<MutexObject> ConditionObject::get_mutex() {
  if (m_mutex == NULL) {
    ...
    if (prev) {
      ...
      MutexObject *mutex =
          new (gc(), MMgc::kExact, cls->ivtable()->getExtraSize())
              MutexObject(cls->ivtable(), cls->prototypePtr());
...
ConditionClass::ConditionClass(VTable *cvtable) : ClassClosure(cvtable) {
  createVanillaPrototype();
}
...
int32_t ConcurrentMemory::casi32(ScriptObject *obj, int32_t addr,
                                 int32_t expectedVal, int32_t newVal) {
  ...
  if (uint32_t(addr) > (domainMemSize - sizeof(int32_t)) ||
      domainMemSize < sizeof(int32_t))
    toplevel->throwRangeError(kInvalidRangeError);

  int32_t *p = (int32_t *)(domainEnv->globalMemoryBase() + uint32_t(addr));
  return vmbase::AtomicOps::compareAndSwap32WithBarrierPrev(expectedVal, newVal,
                                                            p);
}
