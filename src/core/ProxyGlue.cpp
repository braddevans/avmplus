...
ProxyClass::ProxyClass(VTable *vtable) : ClassClosure(vtable) {
  createVanillaPrototype();

  AvmCore *const core = this->core();
  Atom const uri = core->internConstantStringLatin1(
...
//
// Virtual methods from ScriptObject invoked by the VM to reference and/or
// modify members. Each of these is redirected to an ActionScript method of the
// same name, so a subclass of Proxy can handle them. If unhandled, a native
// method (of the same name) will get invoked, which forwards to our
// superclass's method.
//
...
Atom ProxyObject::getMultinameProperty(const Multiname *name) const {
  GCRef<Toplevel> toplevel = this->toplevel();
  GCRef<builtinClassManifest> builtinClasses = toplevel->builtinClasses();
  GCRef<QNameObject> q = QNameObject::create(
      core()->GetGC(), builtinClasses->get_QNameClass(), *name);
...
Atom ProxyObject::callProperty(const Multiname *name, int argc, Atom *argv) {
  ...
  Atom *proxy_argv = (Atom *)avmStackAllocArray(core(), proxy_argv_autoptr,
                                                proxy_argc + 1, sizeof(Atom));
