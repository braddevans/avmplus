 ...
 */
    ...
    public:
        enum {
            kUnit = 8 * sizeof(uintptr_t), kDefaultCapacity = 4
        };

        BitSet() : capacity(kDefaultCapacity) { reset(); }
...
// use WB if allocation is necessary.
    ...
    public:
        ...
        inline void resize(MMgc::GC *gc, uint32_t cap) {
            ...
            if (cap > MAX_INLINE_BITS) {
                // always allocate one more bit than we need to simplify the logic
                // elsewhere (bit 0 goes unused)
                const uint32_t count =
                        (cap + (BITS_PER_UINTPTR - 1) + 1) / BITS_PER_UINTPTR;
                const uint32_t sz = count * sizeof(uintptr_t);
                uintptr_t *v = (uintptr_t *) gc->Alloc(sz, MMgc::GC::kZero,
                                                       MMgc::kFixedBitSetPartition);
        ...
        inline void set(uint32_t bit) {
            ...
            v[nbit / BITS_PER_UINTPTR] |=
                    (uintptr_t(1) << (nbit & (BITS_PER_UINTPTR - 1)));
        }
    ...
    private:
        enum {
            BITS_PER_UINTPTR = sizeof(uintptr_t) * 8
        };
