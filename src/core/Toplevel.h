 ...
 */
    ...
    *abcEnv);
    ...
    void throwReferenceError(int id, const Multiname *multiname,
    ...
    throwIllegalOperationError(int id);
     ...
     * argN = atomv[argc]
     */
    Atom op_construct(Atom ctor, int argc, Atom *atomv);

    /** Implementation of OP_callproperty */
    Atom callproperty(Atom base, const Multiname *name, int argc, Atom *atomv,
                      VTable *vtable);

    /** Implementation of OP_constructprop */
    Atom constructprop(const Multiname *name, int argc, Atom *atomv,
                       VTable *vtable);
     ...
     * argN = atomv[argc-1]
     */
    Atom op_applytype(Atom obj, int argc, Atom *atomv);
    ...
    /**
     * Delete a specified property on a specified object,where the property is
     * specified by a multiname.
     * @param obj Object on which to look for the specified property.
     * @param multiname The name of the property
     * @param vtable The vtable of the object
     * @return  true if the property is deleted. false if the property cannot be
     * deleted.
     */
    bool deleteproperty(Atom obj, const Multiname *multiname,
                        VTable *vtable) const;

    void setproperty(Atom obj, const Multiname *multiname, Atom value,
                     VTable *vtable) const;

    void setproperty_b(Atom obj, const Multiname *multiname, Atom value,
    ...
    // like getBinding, but do extra work to find the initial declarer of the
    // member
    Binding getBindingAndDeclarer(Traits *traits, const Multiname &multiname,
    ...
    // non-Unicode text encodings. The default implementation understands nothing,
    // and always returns 0.
    //
    // Subclasses may implement a larger set of codepages (see
    // http://en.wikipedia.org/wiki/Code_page) but should ensure that any
    // recognized by charsetToCodepage() are also handled by the conversion
    // routines. They should also ensure to return zero for illegal input.
    //
    ...
    //
    // codepage can be assumed to be a value returned by the charsetToCodepage()
    // method.
    //
    // Note that the implementation is not allowed to call
    ...
    //
    virtual String *readMultiByte(uint32_t codepage, uint32_t length,
                                  DataInput *input);

    //
    // Convert the given (Unicode) String into the given codepage and write it to
    ...
    // in certain existing versions of Flash, so must be kept legal.)
    //
    virtual void writeMultiByte(uint32_t codepage, String *str,
                                DataOutput *output);

    //  -------------------------------------------------------

    //
    // If a ByteArray doesn't begin with a BOM, Flash may want
    // ByteArray.toString() to attempt a conversion from the system's default
    // codepage as though the data is MBCS. If such a conversion is desirable and
    // possible, you should return the result as a String. If the conversion is
    // either impossible or undesirable, return NULL.
    //
    ...
    // If the encoding is not valid, or the input stream is malformed, throw an
    // error.
    //
    ...
    //
    // Serialize the given Atom value into the output using the specified
    // encoding. If the encoding is not valid, throw an error.
    //
    // Note that the implementation is not allowed to call
    ...
    // mapping tables don't exist in the VM proper, so this is a no-op in current
    // Tamarin.
    //
    ...
    void addAliasedClassClosure(Atom key1, Atom key2, ClassClosure *cc,
                                bool isDomainEnv);
     ...
     */
    static void registerClassAlias(ScriptObject *script, String *aliasName,
                                   ClassClosure *cc);
    ...
    private:
    // Used to be a WeakHashtable but workers need to access it across worker
    // boundary, and weak accesses can allocate memory.
    ...
    GCMember <HeapHashtable>
            GC_POINTER(_aliasToClassClosureMap); // maps aliasName -> ClassClosure
