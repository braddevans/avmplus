...
def translateTraces(traces, addrs):
    for t in traces:
        ...
        for item in trace:
            ...
            if str(addrinfo[1]["func"]).startswith(
                    ("Alloc", "GetStackTrace", "LargeAlloc", "Calloc", "operator new", "RecordAlloc")) == False:
...
def printTrace(trace, total_size):
    ...
    print(
        "\t\t" + str(round(float(trace_size) / total_size * 100, 2)) + "% - " + str(trace_size / 1024) + " kb - " + str(
            trace_count) + " items");
...
def printSortedTraces(l):
    ...
    line = "Memory allocation report for " + str(allocs) + " allocations, totalling " + str(
        bytes / 1024) + " kb (" + str(bytes / allocs) + " ave) across " + str(len(l)) + " packages";
    print(line);
    ...
    for ns in l:
        ...
        ns_size = ns["size"];
        ns_count = ns["count"];
        line += " - " + str(round(float(ns_size) / bytes * 100, 2)) + "% - " + str(ns_size / 1024) + " kb " + str(
            ns_count) + " items, avg " + str(ns_size / ns_count) + " b";
        print(line);
        ...
        for t in traces:
            ...
            tg_size = t["size"];
            tg_count = t["count"];
            line = "\t" + t["name"] + " - " + str(round(float(tg_size) / bytes * 100, 2)) + "% - " + str(
                tg_size / 1024) + " kb " + str(tg_count) + " items, avg " + str(tg_size / tg_count) + " b";
            print(line);
            sub_traces = t["traces"];
            numtraces = 0;
            for f in sub_traces:
