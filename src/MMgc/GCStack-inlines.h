...
namespace MMgc {
    ...
    REALLY_INLINE GCMarkStack::TypeTag

    GCMarkStack::PeekTypetag() {
        GCAssert(!IsEmpty());
        uintptr_t w = m_top[-1];
        GCAssert(!(w & 1));
        if ((w & 3) == 0)
    ...
    GCMarkStack::InactiveSegments() {
        return m_hiddenSegments;
    }

#ifdef MMGC_MARKSTACK_DEPTH
    REALLY_INLINE uint32_t GCMarkStack::MaxCount() { return m_maxDepth; }
#endif

    REALLY_INLINE uintptr_t
    *
    GCMarkStack::allocSpace(size_t
    nwords) {
    GCAssert (Invariants());
