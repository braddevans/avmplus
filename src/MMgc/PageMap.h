...
// See: https://bugzilla.mozilla.org/show_bug.cgi?id=588878
        ...
        private:
          ...
          friend void SimpleExpandSetAll(PM *pagemap, GCHeap *heap, void *item,
...
// chance of assigning non-zero payloads to addrs in range.
        ...
        protected:
            ...
            static const uint32_t tier2_pages =
                    tier2_entries * sizeof(uint8_t) / GCHeap::kBlockSize;
            MMGC_STATIC_ASSERT(tier2_pages
            *GCHeap::kBlockSize ==
            tier2_entries * sizeof(uint8_t));
        ...
        protected:
          ...
          static const uint32_t tier1_pages =
              tier1_entries * sizeof(uint8_t **) / GCHeap::kBlockSize;
          static const uint32_t tier2_pages =
              tier2_entries * sizeof(uint8_t *) / GCHeap::kBlockSize;
          static const uint32_t tier3_pages =
              tier3_entries * sizeof(uint8_t) / GCHeap::kBlockSize;

          MMGC_STATIC_ASSERT(tier1_pages *GCHeap::kBlockSize ==
                             tier1_entries * sizeof(uint8_t **));
          MMGC_STATIC_ASSERT(tier2_pages *GCHeap::kBlockSize ==
                             tier2_entries * sizeof(uint8_t *));
          MMGC_STATIC_ASSERT(tier3_pages *GCHeap::kBlockSize ==
                             tier3_entries * sizeof(uint8_t));
        ...
        protected:
          ...
          friend void SimpleExpandSetAll(PM *, GCHeap *, void *, uint32_t, PageType);
          // (helper implements ClearAddrs via AddrClear)
          template <typename PM> friend void SimpleClearAddrs(PM *, void *, uint32_t);
        ...
        private:
          void SetupDelayedPagemap(GCHeap *heap, uintptr_t addr, uintptr_t addr_lim);
