...
// AVMPI.h includes avmfeatures.h, which detects platforms and sets up most
// MMGC_ names.
#include "AVMPI.h"
#include "atom.h"
...
// The GCHeap, GC and FixedMalloc allocators are instrumented for valgrind's
// purposes. All memory from the virtual memory API's is unknown to valgrind.
// Only when we tell valgrind about the memory using client requests does
// valgrind track it. See valgrind headers and online manual for client request
// details.
...
// MMGC_MARKSTACK_ALLOWANCE allows host code to limit the size of the mark
// stack, and thus to test the overflow handling.
...
//
// On MacOS X desktop MMGC_POLICY_PROFILING incurs a 5% execution overhead on
// the benchmark test/performance/mmgc/gcbench.as.
//
// Enabling this and MMGC_MEMORY_INFO at the same time is probably not a good
// idea.
//
// If AVMSHELL_BUILD is enabled then the output can be enabled/disabled with the
// -gcbehavior switch in the shell; the output is directed to whatever channel
// is used by GCLog.
//
// If AVMSHELL_BUILD is disabled then the output of policy info is unconditional
// and is directed to a file gcbehavior.txt in the working directory.
...
// Really for the specially interested!  These switches incur measurable
// overhead, so be careful when benchmarking.

// Profile the pointer density of scanned memory
//#define MMGC_POINTINESS_PROFILING

// Profile reference count traffic.  This feature adds about 8% execution time
// overhead to the overhead already added by MMGC_POLICY_PROFILING, on the
// gcbench benchmark, measured on MacOS X desktop.
...
// this might be good to enable in a heightened GCDEBUG build, it triggers bug
// 561402
//#define MMGC_POISON_MEMORY_FROM_OS

#ifdef MMGC_MEMORY_PROFILER
// For the following profilers you need source information:
// VMPI_getFunctionNameFromPC and VMPI_getFileAndLineInfoFromPC must work.

// Internal: profile uses of the conservative marker
//
// When MMGC_CONSERVATIVE_PROFILER is enabled and MMGC_PROFILE=1 is defined in
// the runtime environment the VM will record all uses of conservative marking
// and dump the allocation sites that result in conservatively marked storage
// when the GC is shut down.
...
// runtime environment the VM will record all uses of explicit deletion of
// managed storage, and dump a profile on GC shutdown.  These profiles are
// useful because we want to reduce the frequency of those deletions, and
// tightly control the ones that continue to exist.
...
// runtime environment the VM will record all allocations of weak references and
// dump the allocation sites on GC shutdown.

//#define MMGC_WEAKREF_PROFILER
#endif

#ifdef MMGC_WEAKREF_PROFILER
// Internal: measure the lookup traffic in GCHashtable, the statistics are
// dumped along with the statistics for MMGC_WEAKREF_PROFILER.
...
// MMGC_FASTBITS enables a potentially faster representation of the per-block
// bit table. The representation is fairly well hidden from most of the GC code;
// only GC::GetGCBits and GCAlloc / GCLargeAlloc take it into account.
//
// The new representation uses more space than the old because there are gaps in
// the bit table.
//
// Do not disable this.  The #define will be removed and the feature will always
// be enabled when we're certain that the effects of the feature are positive.
...
#define GC_SLICE_TO_PARTITION_MAP                                              \
  { 0 }

// Slices of the FixedMalloc arena.
const static int kNumFixedPartitions = 1;

#define FIXMEDMALLOC_SLICE_TO_PARTITION_MAP                                    \
  { 0 }

// Assign slices of the GC arena to allocation sites.
// The partition argument is a "slice" of the arena, not the entire heap. Slices
// are mapped to partitions via gcPartitionMap.
...
// The partition argument is a "slice" of the arena, not the entire heap. Slices
// are mapped to partitions via fixedPartitionMap.
