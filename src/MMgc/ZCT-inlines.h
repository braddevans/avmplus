...
namespace MMgc {
    REALLY_INLINE void
    ZCT::Add(RCObject *obj REFCOUNT_PROFILING_ARG(bool initial)) {
    ...
    REALLY_INLINE void
    ZCT::Remove(RCObject *obj REFCOUNT_PROFILING_ARG(bool final)) {
        REFCOUNT_PROFILING_ONLY(gc->policy.signalZCTRemove(final);)

        // Note:
        //  - We just clear out the entry, compaction takes care of NULL entries
        //  later.
    ...
    REALLY_INLINE bool ZCT::IsReaping() { return reaping; }

    REALLY_INLINE uint32_t
    ZCT::BlockNumber(uint32_t
    idx) {
    return idx /
    CAPACITY(RCObject
    **);
...
REALLY_INLINE uint32_t
ZCT::EntryNumber(uint32_t
idx) {
return

idx &(CAPACITY(RCObject *)
...
**
ZCT::PointerTo(uint32_t
idx) {
GCAssert(
(slowState
&& idx<slowTopIndex) ||
(!
slowState &&idx<topIndex
)); // Fascistic compiler settings compel us
// to add the inner parens here
return blocktable[
BlockNumber(idx)
] +
EntryNumber(idx);
...
REALLY_INLINE void ZCT::Put(uint32_t
idx,
RCObject *value
) {
*
