...
namespace MMgc {
FixedAlloc::FixedAlloc(uint32_t itemSize, GCHeap *heap, int partition,
                       bool isFixedAllocSafe)
...
FixedAlloc::FixedAlloc() : m_isFixedAllocSafe(true) {}
...
void FixedAlloc::CreateChunk(bool canFail) {
  ...
  FixedBlock *b =
      (FixedBlock *)m_heap->GetPartition(m_heapPartition)
          ->Alloc(1, GCHeap::kExpand | (canFail ? GCHeap::kCanFail : 0));
...
#ifdef GCDEBUG
  // Deleted and unused memory is poisoned, this is important for leak
  // diagnostics.
  if (!RUNNING_ON_VALGRIND)
    VMPI_memset(b->items, uint8_t(GCHeap::FXFreedPoison),
                m_itemSize * m_itemsPerBlock);
#endif
...
bool FixedAlloc::QueryOwnsObject(const void *item) {
  ...
  for (FixedBlock *fb = m_firstBlock; fb != NULL; fb = fb->next)
    if (ci >= (const char *)fb->items &&
        ci < (const char *)fb->items + m_itemsPerBlock * m_itemSize)
      return true;
  return false;
}
...
FixedAllocSafe::FixedAllocSafe() { VMPI_lockInit(&m_spinlock); }
...
void *FastAllocator::operator new[](size_t size) {
  return FixedMalloc::GetFixedMalloc(kFastAllocatorPartition)
      ->OutOfLineAlloc(size);
}
