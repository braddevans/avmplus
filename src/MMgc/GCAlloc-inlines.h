...
GCAlloc::GetObjectIndex(const GCBlock *block,
                        const void *item) {
    GCAlloc *alloc = (GCAlloc *) block->alloc;
    uint32_t index = (uint32_t)(
            (((char *) item - block->items) * alloc->multiple) >> alloc->shift);
#ifdef GCDEBUG
...
/*static*/
REALLY_INLINE uint32_t GCAlloc::GetBitsIndex(const GCBlock *block,
                                             const void *item) {
...
GCAlloc::GetBitsIndex(const GCBlock *block,
                      const void *item) {
...
REALLY_INLINE GCAllocIterator::GCAllocIterator(MMgc::GCAlloc *alloc)
        : alloc(alloc), block(alloc->m_firstBlock), idx(0),
          limit(alloc->m_itemsPerBlock), size(alloc->m_itemSize) {}
