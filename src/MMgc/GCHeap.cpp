...
#include <float.h>
    ...
    (defined(VMCFG_TELEMETRY_SAMPLER) && defined(DEBUGGER))
namespace avmplus {
...
uint32_t GCHeap::secret_uintptr = 0;

GCHeapConfig::GCHeapConfig()
    : initialSize(512), heapLimit(kDefaultHeapLimit), heapSoftLimit(0),
      dispersiveAdversarial(0), // 0 means dispersive allocation is off.
      OOMExitCode(0), useVirtualMemory(AVMPI_useVirtualMemory()),
      trimVirtualMemory(true),
      mergeContiguousRegions(AVMPI_canMergeContiguousRegions()),
      sloppyCommit(AVMPI_canCommitAlreadyCommittedMemory()), secret(0),
      verbose(false), returnMemory(true), gcstats(false), // tracking
...
#endif
      gcLoadCeiling(
          1.15), // Bug 619885: need > 1.0 to get belt loosening effect
      gcEfficiency(0.25),
      _checkFixedMemory(
          true) // See comment in GCHeap.h for why the default must be 'true'
{
  ...
  gcLoad[0] = 2.5;
  gcLoadCutoff[0] = 10; // Breathing room for warmup
  gcLoad[1] = 2.0;
  gcLoadCutoff[1] = 25; // Classical 2x factor
  gcLoad[2] = 1.75;
  gcLoadCutoff[2] = 50; // Tighten
  gcLoad[3] = 1.5;
  gcLoadCutoff[3] = 75; //   the
  gcLoad[4] = 1.25;
  gcLoadCutoff[4] = 150; //     screws
  gcLoad[5] = 1.2;
  gcLoadCutoff[5] = 300; // Large heaps are
  gcLoad[6] = 1.125;
  gcLoadCutoff[6] = DBL_MAX; //   controlled (very) tightly
...
bool GCHeapConfig::IsGCOptionWithParam(const char *arg) {
  if (!VMPI_strcmp(arg, "-memlimit") || !VMPI_strcmp(arg, "-load") ||
      !VMPI_strcmp(arg, "-loadCeiling") || !VMPI_strcmp(arg, "-gcwork") ||
...
static const char *useDefaultOrSkipForward(const char *arg, const char *key,
                                           const char *dflt) {
  size_t n = VMPI_strlen(key);
  if (VMPI_strcmp(arg, key) == 0) {
    return dflt;
  } else if (HasPrefix(arg, key) && VMPI_strcmp(arg, key) > 0) {
    AvmAssert(dflt == NULL);
...
bool GCHeapConfig::ParseAndApplyOption(const char *arg, bool &wrong,
  ...
  if (!VMPI_strcmp(arg, "-memstats")) {
    ...
    const char *param = useDefaultOrSkipForward(arg, "-load", successorString);
    if (param == NULL) {
      wrong = true;
      return true;
    }
...
#pragma warning(disable : 4996)
    ...
    if (VMPI_sscanf(val, "%lf%n", &ceiling, &nchar) == 1 &&
        size_t(nchar) == VMPI_strlen(val) && ceiling >= 1.0) {
...
#pragma warning(disable : 4996)
#endif
    if (VMPI_sscanf(val, "%lf%n", &work, &nchar) == 1 &&
        size_t(nchar) == VMPI_strlen(val) && work > 0.0 && work <= 1.0) {
      gcEfficiency = work;
      return true;
    } else {
      wrong = true;
...
void GCHeap::Init(const GCHeapConfig &config) {
...
#ifdef MMGC_64BIT
  // TODO: Do we need a true 64-bit secret?
  secret_uintptr = uintptr_t(config.secret) | (uintptr_t(config.secret) << 32);
#else
  secret_uintptr = config.secret;
#endif
  ...
  if (secret != 0) {
    ...
    GCAssertMsg(false, "Vector smash protection is configured but the secret "
                       "was not initialized.");
  }
#endif
}
...
GCHeap::GCHeap(const GCHeapConfig &c)
    : kNativePageSize(VMPI_getVMPageSize()), totalBlocksLen(0),
      totalNumDecommitted(0), totalLargeAllocs(0), totalNumAlloc(0),
      gcheapCodeMemory(0), externalCodeMemory(0), externalPressure(0),
      config(c), status(kMemNormal), enterCount(0), preventDestruct(0),
      m_oomHandling(true), m_notificationBeingSent(false),
#ifdef MMGC_MEMORY_PROFILER
      hasSpy(false),
#endif
      maxTotalHeapSize(0),
      ...
      hooksEnabled(false),
#endif
      entryChecksEnabled(true), abortStatusNotificationSent(false),
      nextDecommitPartition(0) {
  VMPI_lockInit(&m_spinlock);
  VMPI_lockInit(&gclog_spinlock);

  // ResetStatics should be called at the start here before using/initializing
  // any statics
  ResetStatics();

  // Initialize free lists
  for (uint32_t i = 0; i < kNumHeapPartitions; i++) {
    partitions[i].heap = this;
    partitions[i].index = i;

    HeapBlock *block = partitions[i].freelists;
    for (uint32_t j = 0; j < kNumFreeLists; j++) {
      block->FreelistInit();
      block++;
    }
    ...
    // Create the initial heap
    {
      ...
      if (!partitions[i].ExpandHeap((int)config.initialSize)) {
        Abort();
      }
    }
  }
...
void GCHeap::DestroyInstance() {
  ...
  leakedBytes = 0;
  for (int i = 0; i < kNumFixedPartitions; i++) {
    leakedBytes += GetFixedMalloc(i)->GetBytesInUse();
    fixedMallocs[i].DestroyInstance(i);
  }

  GCAssertMsg(leakedBytes == 0 || GetStatus() == kMemAbort, "Leaks!");

  for (uint32_t i = 0; i < kNumHeapPartitions; i++) {
    partitions[i].Destroy();
  }
...
void *GCHeap::Partition::Alloc(size_t size, uint32_t flags, size_t alignment) {
  GCAssert(size > 0);
  ...
  bool expand = (flags & kExpand) != 0;
  {
    ...
    bool saved_oomHandling = heap->m_oomHandling;
    heap->m_oomHandling = saved_oomHandling && (flags & kNoOOMHandling) == 0;
    ...
    if (!baseAddr) {
      heap->SendFreeMemorySignal(size);
      baseAddr = AllocHelper(size, expand, zero, alignment);
    }
    ...
    if (!baseAddr) {
      if (flags & kCanFail) {
        heap->m_oomHandling = saved_oomHandling;
        return NULL;
      } else {
        heap->Abort();
      }
    ...
    numAlloc += size;
    heap->totalNumAlloc += size;

#ifdef MMGC_MEMORY_PROFILER
    if ((flags & kProfile) && heap->HooksEnabled() && heap->profiler) {
      heap->profiler->RecordAllocation(baseAddr, size * kBlockSize,
                                       size * kBlockSize, /*managed=*/false);
    }
#endif

    //  Only check for memory limits if we're allowing OOM notifications
    if (heap->m_oomHandling) {
  ...
  // Fail the allocation if we're a "canFail" allocation that has pushed beyond
  // one of our limits.
  if ((flags & kCanFail) != 0 &&
      (heap->status == kMemSoftLimit || heap->SoftLimitExceeded() ||
       heap->HardLimitExceeded())) {
    FreeInternal(baseAddr, (flags & kProfile) != 0, heap->m_oomHandling);
    return NULL;
  }
  ...
  if ((flags & kProfile) != 0) {
    VALGRIND_MALLOCLIKE_BLOCK(baseAddr, size * kBlockSize, 0,
                              (flags & kZero) != 0);
  }
...
void *GCHeap::Partition::AllocHelper(size_t size, bool expand, bool &zero,
                                     size_t alignment) {
  ...
  if (!block && expand) {
    ...
    //  Look ahead at memory limits to see if we should trigger a free memory
    //  signal
    if ((heap->HardLimitExceeded(size) || heap->SoftLimitExceeded(size))) {
      heap->SendFreeMemorySignal(size);
    }

    // Don't allow our memory consumption to grow beyond hard limit
    if (heap->HardLimitExceeded(size))
      return NULL;

    if (size >= kOSAllocThreshold && heap->config.useVirtualMemory) {
...
#if defined(MMGC_MEMORY_PROFILER) && defined(MMGC_MEMORY_INFO)
  if (heap->profiler && block)
    block->allocTrace = heap->profiler->GetStackTrace();
#endif
...
void GCHeap::CheckForMemoryLimitsExceeded() {
  ...
  //  If we're already in the process of sending out memory notifications, don't
  //  bother verifying now.
  if (status == MMgc::kMemAbort || statusNotificationBeingSent())
    return;

  size_t overage = 0;
  if (SoftLimitExceeded()) {
    overage = GetTotalHeapSize() + externalPressure / kBlockSize -
              config.heapSoftLimit;
  } else if (HardLimitExceeded()) {
    overage = (GetTotalHeapSize() + externalPressure / kBlockSize) -
              config.heapLimit + (config.heapLimit / 10);
  }
...
void GCHeap::Partition::FreeInternal(const void *item, bool profile,
                                     bool oomHandling) {
  (void)profile;
  ...
  bool saved_oomHandling = heap->m_oomHandling;
  heap->m_oomHandling = saved_oomHandling && oomHandling;
  ...
  // Update metrics
  GCAssert(numAlloc >= size);
  numAlloc -= size;
  GCAssert(heap->totalNumAlloc >= size);
  heap->totalNumAlloc -= size;

#if defined(MMGC_MEMORY_PROFILER) && defined(MMGC_MEMORY_INFO)
  if (heap->profiler && block)
    block->freeTrace = heap->profiler->GetStackTrace();
#endif

#ifdef MMGC_MEMORY_PROFILER
  if (profile && heap->HooksEnabled() && heap->profiler) {
    heap->profiler->RecordDeallocation(item, size * kBlockSize);
  }
#endif
...
void GCHeap::Decommit() {
  ...
  // don't decommit if OOM handling is disabled; there's a guard in the OOM code
  // so this should never happen, but belt and suspenders...
  if (!m_oomHandling)
    return;
  ...
  if (FreeMemoryExceedsDecommitThreshold()) {
    decommitSize =
        int((freeSize * 100 - heapSize * kDecommitThresholdPercentage) / 100);
  }
  //  If we're over the heapLimit, attempt to decommit enough to get just under
  //  the limit
  else if ((heapSize > config.heapLimit) &&
           ((heapSize - freeSize) < config.heapLimit)) {
    decommitSize = heapSize - config.heapLimit + 1;

  }
  //  If we're over the SoftLimit, attempt to decommit enough to get just under
  //  the softLimit
  else if ((config.heapSoftLimit != 0) && (heapSize > config.heapSoftLimit) &&
           ((heapSize - freeSize) < config.heapSoftLimit)) {
  ...
  if ((decommitSize < (size_t)kMinHeapIncrement) &&
      (freeSize > (size_t)kMinHeapIncrement)) {
...
restart:
  ...
  int startingPartition = nextDecommitPartition;
  nextDecommitPartition = (nextDecommitPartition + 1) % kNumHeapPartitions;

  for (int count = 0; count < kNumHeapPartitions && decommitSize > 0; count++) {
    int part = (startingPartition + count) % kNumHeapPartitions;
    // fprintf(stderr, "$$$ Decommit: partition %d\n", part);

    // search from the end of the free list so we decommit big blocks
    HeapBlock *freelist = &partitions[part].freelists[kNumFreeLists - 1];

    HeapBlock *endOfBigFreelists =
        &partitions[part].freelists[partitions[part].GetFreeListIndex(1)];
    ...
    for (; freelist >= endOfBigFreelists && decommitSize > 0; freelist--) {
      ...
      if (!partitions[part].EnsureFreeRegion(/*expand=*/false))
        return;
#endif
      ...
      while ((block = block->prev) != freelist && decommitSize > 0) {
        ...
        if (config.useVirtualMemory) {
          partitions[part].RemoveFromList(block);
          if ((size_t)block->size > decommitSize) {
            HeapBlock *newBlock =
                partitions[part].Split(block, (int)decommitSize);
            partitions[part].AddToFreeList(newBlock);
          }

          Region *region = partitions[part].AddrToRegion(block->baseAddr);
          if (config.trimVirtualMemory &&
              freeSize * 100 > heapSize * kReleaseThresholdPercentage &&
              // if block is as big or bigger than a region, free the whole
              // region
              block->baseAddr <= region->baseAddr &&
              region->reserveTop <= block->endAddr()) {

            if (block->baseAddr < region->baseAddr) {
              HeapBlock *newBlock = partitions[part].Split(
                  block,
                  int((region->baseAddr - block->baseAddr) / kBlockSize));
              partitions[part].AddToFreeList(block);
              block = newBlock;
            }
            if (block->endAddr() > region->reserveTop) {
              HeapBlock *newBlock = partitions[part].Split(
                  block,
                  int((region->reserveTop - block->baseAddr) / kBlockSize));
              partitions[part].AddToFreeList(newBlock);
            }

            decommitSize -= block->size;
            partitions[part].RemoveBlock(block);
            goto restart;
          } else if (AVMPI_decommitMemory(block->baseAddr,
...
#ifdef MMGC_MAC
            // this can happen on mac where we release and re-reserve the memory
            // and another thread may steal it from us
            partitions[part].RemoveLostBlock(block);
            goto restart;
#else
          ...
          partitions[part].numDecommitted += block->size;
          totalNumDecommitted += block->size;
          ...
          if (block->sizePrevious != 0 && !prev->committed && !prev->inUse()) {
            partitions[part].RemoveFromList(prev);
          ...
          if (next->size != 0 && !next->committed && !next->inUse()) {
            partitions[part].RemoveFromList(next);
          ...
          // add this block to the back of the bus to make sure we consume
          // committed memory first
          HeapBlock *backOfTheBus =
              &partitions[part].freelists[kNumFreeLists - 1];
          HeapBlock *pointToInsert = backOfTheBus;
          while ((pointToInsert = pointToInsert->next) != backOfTheBus) {
            if (pointToInsert->size >= block->size &&
                !pointToInsert->committed) {
              break;
            }
          }
          partitions[part].AddToFreeList(block, pointToInsert);
          ...
          // if we aren't using mmap we can only do something if the block maps
          // to a region that is completely empty
          Region *region = partitions[part].AddrToRegion(block->baseAddr);
...
void GCHeap::CheckForSoftLimitExceeded(size_t request) {
  ...
  GCDebugMsg(false,
             "*** Alloc exceeded softlimit: ask for %u, usedheapsize =%u, "
             "totalHeap =%u, of which external =%u\n",
             unsigned(request), unsigned(GetUsedHeapSize() + externalBlocks),
  ...
  if (!statusNotificationBeingSent() && statusNotNormalOrAbort()) {
    ...
    if (config.heapSoftLimit != 0 && status == kMemSoftLimit) {
      if (!SoftLimitExceeded()) {
        ...
        GCDebugMsg(false,
                   "*** Alloc dropped below softlimit: usedheapsize =%u, "
                   "totalHeap =%u, of which external =%u\n",
                   unsigned(used), unsigned(total), unsigned(externalBlocks));
...
void GCHeap::Partition::RemoveLostBlock(HeapBlock *block) {
  if (heap->config.verbose) {
  ...
  if (block->baseAddr != region->baseAddr &&
      region->commitTop != block->endAddr()) {

    GCAssertMsg(HaveFreeRegion(), "Decommit was supposed to ensure this!");

    NewRegion(block->endAddr(), region->reserveTop,
              region->commitTop > block->endAddr() ? region->commitTop
                                                   : block->endAddr(),
              region->blockId +
                  (block->endAddr() - region->baseAddr) / kBlockSize);
  ...
  Region temp(this, block->baseAddr, block->endAddr(), block->endAddr(),
              regionBlockId + (block->baseAddr - regionBaseAddr) / kBlockSize);
...
void GCHeap::Partition::RemoveBlock(HeapBlock *block, bool release) {
  Region *region = AddrToRegion(block->baseAddr);
  ...
  if (block->sizePrevious && nextBlock->size) {
    ...
    // the block is not contigous with the block before or after it - we need to
    // remove a sentinel since there would already be one on each side.
  ...
  // Fix up the prev/next pointers of each freelist.  This is a little more
  // complicated than the similiar code in ExpandHeap because blocks after the
  // one we are free'ing are sliding down by block->size
  ...
  for (uint32_t i = 0; i < kNumFreeLists; i++) {
    ...
    do {
      if (temp->prev != fl) {
        if (temp->prev > block) {
          temp->prev =
              newBlocks + (temp->prev - blocks - blockSize) + sen_offset;
        }
      }
      if (temp->next != fl) {
        if (temp->next > block) {
          temp->next =
              newBlocks + (temp->next - blocks - blockSize) + sen_offset;
        }
      }
  ...
  heap->totalBlocksLen += (newBlocksLen - blocksLen);
...
void GCHeap::Partition::ValidateHeapBlocks() {
#ifdef GCDEBUG
  ...
  while (block - blocks < (intptr_t)blocksLen) {
    ...
    if (block->baseAddr) {
      ...
      // block is a sentinel
      GCAssert(block->size == 0);
      // FIXME: the following asserts are firing and we need to understand why,
      // could be bugs make sure last block ends at commitTop
      Region *prevRegion =
          AddrToRegion(prev->baseAddr + (prev->size * kBlockSize) - 1);
      GCAssert(prev->baseAddr + (prev->size * kBlockSize) ==
               prevRegion->commitTop);
      block++;
...
GCHeap::Region *GCHeap::Partition::AddrToRegion(const void *item) const {
  // Linear search of regions list to find this address.
  // The regions list should usually be pretty short.
  for (Region *region = lastRegion; region != NULL; region = region->prev) {
...
GCHeap::HeapBlock *GCHeap::Partition::BaseAddrToBlock(const void *item) const {
...
GCHeap::HeapBlock *
GCHeap::Partition::InteriorAddrToBlock(const void *item) const {
  Region *region = AddrToRegion(item);
  if (region) {
    if (region->blockId == kLargeItemBlockId)
      return NULL;
    ...
    GCAssert(item >= b->baseAddr &&
             item < b->baseAddr + b->size * GCHeap::kBlockSize);
    return b;
  }
  return NULL;
}

size_t GCHeap::Partition::SafeSize(const void *item) {
  MMGC_LOCK(heap->m_spinlock);
...
static inline size_t alignmentSlop(char *baseAddr, size_t alignment) {
  return (alignment - (size_t)(((uintptr_t)baseAddr >> GCHeap::kBlockShift) &
                               (alignment - 1))) &
         (alignment - 1);
}
...
REALLY_INLINE char *GCHeap::Partition::ReserveSomeRegion(size_t sizeInBytes) {
#ifdef GCDEBUG
  if (!heap->config.dispersiveAdversarial)
    return (char *)AVMPI_reserveMemoryRegion(NULL, sizeInBytes);
  else
    return reserveSomeRegionDispersively(heap->config.dispersiveAdversarial,
                                         sizeInBytes);
#else
  return (char *)AVMPI_reserveMemoryRegion(NULL, sizeInBytes);
#endif
...
void *GCHeap::Partition::LargeAlloc(size_t size, size_t alignment) {
  GCAssert(heap->config.useVirtualMemory);
  ...
  NewRegion(addr,
            ...
            kLargeItemBlockId);
  largeAllocation += size;
  heap->totalLargeAllocs += size;
  heap->CheckForNewMaxTotalHeapSize();

  return alignedAddr;
}

void GCHeap::Partition::LargeFree(const void *item) {
  GCAssert(heap->config.useVirtualMemory);
...
GCHeap::HeapBlock *GCHeap::Partition::AllocBlock(size_t size, bool &zero,
                                                 size_t alignment) {
  ...
  for (uint32_t i = startList; i < kNumFreeLists; i++) {
    ...
    while ((block = block->next) != freelist) {
      ...
      if (block->size >= size + alignmentSlop(block->baseAddr, alignment) &&
          block->committed) {
        RemoveFromList(block);
        return AllocCommittedBlock(block, size, zero, alignment);
      }

      // Look for a sequence of decommitted and committed blocks that together
      // would be large enough, in case a single committed block can't be found.

      if (heap->config.useVirtualMemory && decommittedSuitableBlock == NULL &&
          !block->committed) {
        // Note, 'block' should be invariant throughout this scope, it's the
        // block whose successors and predecessors we're inspecting
        ...
        HeapBlock *nextBlock = block + block->size;
        while (totalSize < size + firstSlop &&
               !(nextBlock->inUse() || nextBlock->size == 0)) {
  ...
  if (decommittedSuitableBlock != NULL)
    return AllocCommittedBlock(
        CreateCommittedBlock(decommittedSuitableBlock, size, alignment), size,
        zero, alignment);

  return NULL;
}

GCHeap::HeapBlock *GCHeap::Partition::AllocCommittedBlock(HeapBlock *block,
                                                          size_t size,
                                                          bool &zero,
                                                          size_t alignment) {
...
GCHeap::HeapBlock *GCHeap::Partition::CreateCommittedBlock(HeapBlock *block,
                                                           size_t size,
                                                           size_t alignment) {
  RemoveFromList(block);
  ...
  if (block->size < size) {
    ...
    if (!heap->config.sloppyCommit && !block->committed)
      Commit(block);
...
//
// 'available' is the amount of memory available including the memory in
// 'block', and 'request' is the amount of memory required.

void GCHeap::Partition::PruneDecommittedBlock(HeapBlock *block,
                                              size_t available,
                                              size_t request) {
...
GCHeap::HeapBlock *GCHeap::Partition::Split(HeapBlock *block, size_t size) {
  GCAssert(block->size > size);
  HeapBlock *newBlock = block + size;
  newBlock->Init(block->baseAddr + kBlockSize * size, block->size - size,
                 block->dirty);
...
void GCHeap::Partition::Commit(HeapBlock *block) {
  GCAssert(heap->config.sloppyCommit || !block->committed);
  ...
  heap->totalNumDecommitted -= block->size;
...
void GCHeap::Partition::CheckFreelist() {
  ...
  for (uint32_t i = 0; i < kNumFreeLists; i++) {
    ...
    while ((block = block->next) != freelist) {
      ...
      if (block->sizePrevious) {
        ...
        GCAssert(prev->inUse() || prev->size == 0 ||
                 prev->committed != block->committed);
      }
      {
        HeapBlock *next = block + block->size;
        GCAssert(next->inUse() || next->size == 0 ||
                 next->committed != block->committed);
      }
    }
    freelist++;
  }
...
bool GCHeap::Partition::BlocksAreContiguous(void *item1, void *item2) {
  Region *r1 = AddrToRegion(item1);
  Region *r2 = AddrToRegion(item2);
  return r1 == r2 || r1->reserveTop == r2->baseAddr;
}

void GCHeap::Partition::AddToFreeList(HeapBlock *block,
                                      HeapBlock *pointToInsert) {
  CheckFreelist();
...
void GCHeap::Partition::AddToFreeList(HeapBlock *block, bool makeDirty) {
  GCAssert(block->size != 0);

  // Try to coalesce a committed block with its committed non-sentinel
  // predecessor
...
void GCHeap::Partition::FreeBlock(HeapBlock *block) {
  GCAssert(block->inUse());

#ifdef GCDEBUG
  // trash it. fb == free block
  if (!RUNNING_ON_VALGRIND)
    VMPI_memset(block->baseAddr, uint8_t(MMFreedPoison),
                block->size * kBlockSize);
#endif
...
#ifdef MMGC_POLICY_PROFILING
    // The guard on instance being non-NULL is a hack, to be fixed later
    // (now=2009-07-20). Some VMPI layers (WinMo is at least one of them) try to
    // grab the GCHeap instance to get at the map of private pages.  But the
    // GCHeap instance is not available during the initial call to ExpandHeap.
    // So sidestep that problem here.
    //
    // Note that if CheckForNewMaxTotalHeapSize is only called once then
    // maxPrivateMemory will be out of sync with maxTotalHeapSize, see also
    // bugzilla 608684.
    if (instance != NULL)
      maxPrivateMemory =
          AVMPI_getPrivateResidentPageCount() * VMPI_getVMPageSize();
#endif
  }
}

bool GCHeap::Partition::ExpandHeap(size_t askSize) {
...
bool GCHeap::HardLimitExceeded(size_t additionalAllocationAmt) {
  return GetTotalHeapSize() + externalPressure / kBlockSize +
...
bool GCHeap::Partition::ExpandHeapInternal(size_t askSize) {
  size_t size = askSize;
  ...
  size_t defaultReserve =
      test_bridging ? (size + kMinHeapIncrement) : kDefaultReserve;
#else
  ...
  // when we allocate a new region the space needed for the HeapBlocks, if it
  // won't fit in existing space it must fit in new space so we may need to
  // increase the new space

  HeapBlock *newBlocks = blocks;

  if (blocksLen != 0 || // first time through just take what we need out of
                        // initialSize instead of adjusting
      heap->config.initialSize == 0) // unless initializeSize is zero of course
  {
    ...
    size_t newHeapBlocksSize =
        heap->numHeapBlocksToNumBlocks(blocksLen + size + extraBlocks);

    // size is based on newSize and vice versa, loop to settle (typically one
    // loop, sometimes two)
    while (newHeapBlocksSize > curHeapBlocksSize) {
      // use askSize so HeapBlock's can fit in rounding slop
      size =
          roundUp(askSize + newHeapBlocksSize + extraBlocks, kMinHeapIncrement);
  ...
  // At this point we have adjusted/calculated the final size that would need to
  // be committed. We need to check that against the hardlimit to see if we are
  // going to go above it.
  if (heap->HardLimitExceeded(size))
    return false;

  if (heap->config.useVirtualMemory) {
    Region *region = lastRegion;
    if (region != NULL) {
      commitAvail =
          (int)((region->reserveTop - region->commitTop) / kBlockSize);
      ...
      if (size <= commitAvail) {
        if (AVMPI_commitMemory(region->commitTop, size * kBlockSize)) {
          ...
          HeapBlock *last =
              &blocks[blocksLen - 1] - blocks[blocksLen - 1].sizePrevious;
      ...
      if (defaultReserve > size) {
        newRegionAddr = (char *)AVMPI_reserveMemoryRegion(
            region->reserveTop, defaultReserve * kBlockSize);
      ...
      if (newRegionAddr == NULL) {
        newRegionAddr = (char *)AVMPI_reserveMemoryRegion(
            region->reserveTop, (size - commitAvail) * kBlockSize);
        newRegionSize = size - commitAvail;

        // check for contiguity
        if (newRegionAddr && newRegionAddr != region->reserveTop) {
          // we can't use this space since we need commitAvail from prev region
          // to meet the size requested, toss it
      ...
      if (newRegionAddr ==
          region->reserveTop) // we'll use the region below as a separate region
                              // if its not contiguous
      {
        // We were able to reserve some space.

        // Commit available space from the existing region.
        if (commitAvail != 0) {
          if (!AVMPI_commitMemory(region->commitTop,
        ...
        if (!AVMPI_commitMemory(newRegionAddr,
          ...
          if (commitAvail != 0) {
            AVMPI_decommitMemory(region->commitTop, commitAvail * kBlockSize);
          }
          ReleaseMemory(newRegionAddr, (size - commitAvail) * kBlockSize);
          return false;
        }
    ...
    if (AVMPI_commitMemory(newRegionAddr, size * kBlockSize) == 0) {
...
gotMemory:
  ...
  block->Init(baseAddr, size, heap->newPagesDirty());
  ...
  if ((char *)newBlocks == baseAddr) {
    size_t numBlocksNeededForHeapBlocks =
        heap->numHeapBlocksToNumBlocks(newBlocksLen);
    ...
    heap->totalNumAlloc += numBlocksNeededForHeapBlocks;
    block = next;
  }
  ...
  // free old blocks space using new blocks (FreeBlock poisons blocks so can't
  // use old blocks)
  if (oldBlocks && oldBlocks != newBlocks) {
    HeapBlock *oldBlocksHB = BaseAddrToBlock(oldBlocks);
    numAlloc -= oldBlocksHB->size;
    heap->totalNumAlloc -= oldBlocksHB->size;
...
void GCHeap::Partition::RemoveRegion(Region *region, bool release) {
  Region **next = &lastRegion;
  while (*next != region)
    next = &((*next)->prev);
  *next = region->prev;
  if (release) {
    ReleaseMemory(region->baseAddr, region->reserveTop - region->baseAddr);
  }
  if (heap->config.verbose) {
    GCLog("unreserved region 0x%p - 0x%p (commitTop: %p)\n", region->baseAddr,
          region->reserveTop, region->commitTop);
    DumpPartitionRep();
  }
  FreeRegion(region);
}

void GCHeap::Partition::FreeAll() {
  ...
  while (lastRegion != NULL) {
    ...
    if (region->blockId == kLargeItemBlockId) {
      ...
      GCAssertMsg(heap->status == kMemAbort,
                  "Allocation of large object not freed");
      AVMPI_releaseMemoryRegion(region->baseAddr,
...
EnterFrame::EnterFrame()
    : m_heap(NULL), m_gc(NULL), m_abortUnwindList(NULL), m_previous(NULL),
      m_suspended(false) {
...
EnterFrame::~EnterFrame() {
  if (m_heap) {
    GCHeap *heap = m_heap;
    // this prevents us from doing multiple jumps in case leave results in more
    // allocations
    m_heap = NULL;
    heap->Leave();
  }
...
void EnterFrame::UnwindAllObjects() {
  while (m_abortUnwindList) {
    ...
    //  The unwind call may remove the handler or may leave it on this list.  If
    //  it leaves it, then make sure to advance the list, otherwise, the list
    //  will automatically advance if it's removed.
...
void EnterFrame::AddAbortUnwindObject(AbortUnwindObject *obj) {
  GCAssertMsg(!EnterFrame::IsAbortUnwindObjectInList(obj),
...
void EnterFrame::RemoveAbortUnwindObject(AbortUnwindObject *obj) {
  GCAssertMsg(obj == m_abortUnwindList || obj->previous != NULL,
              "Object not in list");
...
AbortUnwindObject::~AbortUnwindObject() {
  GCAssertMsg(!EnterFrame::IsAbortUnwindObjectInList(this),
              "RemoveAbortUnwindObject not called, dangling pointer in list.");
}
...
SuspendEnterFrame::~SuspendEnterFrame() {
  if (m_ef)
    m_ef->Resume();
  GCHeap *heap = GCHeap::GetGCHeap();
  GCAssertMsg(heap->GetEnterFrame() == m_ef,
              "EnterFrame's not unwound properly");
...
void GCHeap::SignalObjectTooLarge() {
  GCLog("Implementation limit exceeded: attempting to allocate too-large "
        "object\n");
  GetGCHeap()->Abort();
}
...
/*static*/
void GCHeap::SignalExternalFreeMemory(
    size_t minimumBytesToFree /*= kMaxObjectSize */) {
  GCHeap *heap = GetGCHeap();
  GCAssertMsg(heap != NULL, "GCHeap not valid!");

  MMGC_LOCK(heap->m_spinlock);

  // When calling SendFreeMemorySignal with kMaxObjectSize it will try to
  // release as much memory as possible. Otherwise it interprets the parameter
  // as number of blocks to be freed. This function uses bytes instead. The
  // value is converted to blocks here, except when kMaxObjectSize has been
  // passed in so that it will still trigger freeing maximum amount of memory.
  // The division may lose some precision, but SendFreeMemorySignal adds one
  // more block to the requested amount so that is ok.
  heap->SendFreeMemorySignal((minimumBytesToFree != kMaxObjectSize)
                                 ? minimumBytesToFree / GCHeap::kBlockSize
                                 : minimumBytesToFree);
}

// This can *always* be called.  It will clean up the state on the current
// thread if appropriate, otherwise do nothing.  It *must* be called by host
// code if the host code jumps past an MMGC_ENTER instance.  (The Flash player
...
void GCHeap::SignalImminentAbort() {
  ...
  // If the current thread is holding a lock for a GC that's not currently
  // active on the thread then break the lock: the current thread is collecting
  // in that GC, but the Abort has cancelled the collection.
  ef->UnwindAllObjects();
...
void GCHeap::Abort() {
  ...
  //  If we hit abort, we need to turn m_oomHandling back on so that listeners
  //  are guaranteed to get this signal We also need to set m_notoficationThread
  //  to NULL in case we hit abort while we were processing another memory
  //  status change
  m_oomHandling = true;
  ...
  if (ef != NULL) {
    // Guard against repeated jumps: ef->m_heap doubles as a flag.  We go
    // Abort->longjmp->~EnterFrame->Leave and Leave calls StatusChangeNotify and
    // the host code might do another allocation during shutdown in which case
    // we want to go to VMPI_abort instead.  At that point m_heap will be NULL
...
// longjmps are incorrect in C++ code, and will break the Pepper plugin.
  ...
  GCAssertMsg(
      false,
      "MMGC_ENTER missing or we allocated more memory trying to shutdown");
  VMPI_abort();
}
...
void GCHeap::Leave() {
  ...
  // need to check if enterFrame is valid, it might have been nulled out by
  // SignalImminentAbort
...
void GCHeap::log_percentage(const char *name, size_t bytes,
                            size_t bytes_compare) {
  bytes_compare = size_t((bytes * 100.0) / bytes_compare);
  if (bytes > 1 << 20) {
    GCLog("%s %u (%.1fM) %u%%\n", name,
          (unsigned int)(bytes / GCHeap::kBlockSize),
          bytes * 1.0 / (1024 * 1024), (unsigned int)(bytes_compare));
  } else {
    GCLog("%s %u (%uK) %u%%\n", name,
          (unsigned int)(bytes / GCHeap::kBlockSize),
          (unsigned int)(bytes / 1024), (unsigned int)(bytes_compare));
  }
}
...
void GCHeap::DumpMemoryInfo() {
  ...
  size_t unmanaged = 0;
  for (int i = 0; i < kNumFixedPartitions; i++) {
    unmanaged += GetFixedMalloc(i)->GetTotalSize() * GCHeap::kBlockSize;
  }
  size_t fixed_alloced = 0;
  size_t fixed_asksize = 0;
  for (int i = 0; i < kNumFixedPartitions; i++) {
    size_t asksize, alloced;
    GetFixedMalloc(i)->GetUsageInfo(asksize, alloced);
    fixed_asksize += asksize;
    fixed_alloced += alloced;
  }
  size_t gc_total = 0;
...
#ifdef MMGC_MEMORY_PROFILER
  for (int i = 0; i < kNumFixedPartitions; i++) {
    fixedMallocs[i].DumpMemoryInfo();
  }
#endif

  // Gross stats are not meaningful if the profiler is running, see bugzilla
  // 490014. Disabling their printing is just an expedient fix to avoid
  // misleading data being printed.  There are other, more complicated, fixes we
  // should adopt.
...
#endif
  {
    ...
    log_percentage("[mem]\t\t free",
                   (size_t)GetFreeHeapSize() * GCHeap::kBlockSize, priv);
    log_percentage("[mem]\t other", priv - mmgc, priv);
    log_percentage("[mem] \tunmanaged overhead ", unmanaged - fixed_alloced,
                   unmanaged);
    log_percentage("[mem] \tmanaged overhead ", gc_total - gc_allocated_total,
                   gc_total);
#ifdef MMGC_MEMORY_PROFILER
    if (HooksEnabled()) {
      log_percentage("[mem] \tunmanaged internal wastage",
                     fixed_alloced - fixed_asksize, fixed_alloced);
      log_percentage("[mem] \tmanaged internal wastage",
                     gc_allocated_total - gc_ask_total, gc_allocated_total);
    }
#endif
...
size_t GCHeap::Partition::DumpPartitionRep() {
  Region **regions = NULL;
  Region *r = lastRegion;
  int numRegions = 0;
  ...
  for (int i = 0; i < numRegions; i++) {
    r = regions[i];
    ptrdiff_t regionSize = (r->reserveTop - r->baseAddr);
    GCLog("0x%p -  0x%p/0x%p [%llu]\n", r->baseAddr, r->commitTop,
          r->reserveTop, (unsigned long long)regionSize);
    reserved += regionSize;
    ...
    if (spanningBlock) {
      GCAssert(spanningBlock->baseAddr + (spanningBlock->size * kBlockSize) >
               r->baseAddr);
      ...
      heap->LogChar(spanningBlock->inUse() ? '1' : '0',
                    (end - addr) / kBlockSize);
      addr = end;
    ...
    heap->LogChar('-', (r->reserveTop - addr) / kBlockSize);

    GCLog("\n");
  }
  VMPI_free(regions);
  GCLog("%llu bytes reserved\n", (unsigned long long)reserved);
  return reserved;
}
...
void GCHeap::DumpHeapRep() {
  ...
  for (int i = 0; i < kNumHeapPartitions; i++) {
    GCLog("PARTITION %d\n", i);
    reserved += GetPartition(i)->DumpPartitionRep();
  }

  GCLog("Total heap reservation: %llu bytes\n", (unsigned long long)reserved);
}
...
void GCHeap::Partition::ReleaseMemory(char *address, size_t size) {
  if (heap->config.useVirtualMemory) {
...
void GCManager::destroy() { collectors.Destroy(); }
...
void GCHeap::SendFreeMemorySignal(size_t minimumBlocksToFree) {
  //  If we're already in the process of sending out memory notifications, don't
  //  bother verifying now. Also, we only want to send the "free memory" signal
  //  while our memory is in a normal state.  Once we've entered softLimit or
  //  abort state, we want to allow the softlimit or abort processing to return
...
bool GCHeap::ShouldNotEnter() {
  // don't enter if the heap is already gone or we're aborting but not on the
  // aborting call stack in a nested enter call
  GCHeap *heap = GetGCHeap();
  if (heap == NULL ||
...
bool GCHeap::Partition::IsAddressInHeap(void *addr) {
...
bool GCHeap::Partition::EnsureFreeRegion(bool allowExpansion) {
  if (!HaveFreeRegion()) {
    bool zero = false;
...
GCHeap::Region *GCHeap::Partition::NewRegion(char *baseAddr, char *rTop,
                                             char *cTop, size_t blockId) {
  Region *r = freeRegion;
  if (r) {
    freeRegion = *(Region **)freeRegion;
  } else {
    r = nextRegion++;
    if (roundUp((uintptr_t)nextRegion, kBlockSize) - (uintptr_t)nextRegion <
        sizeof(Region))
...
void GCHeap::Partition::FreeRegion(Region *r) {
  if (r == lastRegion)
    lastRegion = r->prev;
  *(Region **)r = freeRegion;
  freeRegion = r;
...
GCHeap::Region::Region(GCHeap::Partition *partition, char *baseAddr, char *rTop,
                       char *cTop, size_t blockId)
    : prev(partition->lastRegion), baseAddr(baseAddr), reserveTop(rTop),
      commitTop(cTop), blockId(blockId) {
  partition->lastRegion = this;
}
...
void GCHeap::CheckForOOMAbortAllocation() {
  if (m_notificationBeingSent && status == kMemAbort)
    GCAssertMsg(
        false,
        "Its not legal to perform allocations during OOM kMemAbort callback");
}
#endif

bool GCHeap::QueryCanReturnToNormal() {
  // must be below soft limit _AND_ above decommit threshold
  return GetUsedHeapSize() + externalPressure / kBlockSize <
             config.heapSoftLimit &&
         FreeMemoryExceedsDecommitThreshold();
}

bool GCHeap::FreeMemoryExceedsDecommitThreshold() {
  return GetFreeHeapSize() * 100 >
         GetTotalHeapSize() * kDecommitThresholdPercentage;
}
} // namespace MMgc
