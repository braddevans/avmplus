...
namespace MMgc {
    ...
    REALLY_INLINE bool GCManager::tryAddGC(GC *gc) { return collectors.Add(gc); }

    REALLY_INLINE void GCManager::removeGC(GC *gc) { collectors.Remove(gc); }

    REALLY_INLINE BasicList<GC *>
    &

    GCManager::gcs() { return collectors; }

/* legacy API */
/* static */
    REALLY_INLINE void GCHeap::Init(GCMallocFuncPtr malloc, GCFreeFuncPtr free,
                                    int initialSize) {
        (void) malloc;
        (void) free;
        GCHeapConfig props;
...
/* static */
    ...
    REALLY_INLINE FixedMalloc
    *

    GCHeap::GetFixedMalloc(int partition) {
        return FixedMalloc::GetFixedMalloc(partition);
    }

    REALLY_INLINE GCHeap::Partition
    *

    GCHeap::GetPartition(int partition) {
    ...
    REALLY_INLINE void GCHeap::SignalCodeMemoryDeallocated(size_t size,
                                                           bool gcheap_memory) {
        if (gcheap_memory)
    ...
    GCHeap::GetUsedHeapSize() const { return totalNumAlloc; }

    REALLY_INLINE size_t

    GCHeap::GetFreeHeapSize() const {
        return GetTotalHeapSize() - totalNumAlloc;
    }

    REALLY_INLINE size_t

    GCHeap::GetTotalCodeSize() const {
        return gcheapCodeMemory + externalCodeMemory;
    }

    REALLY_INLINE size_t

    GCHeap::GetTotalGCHeapAllocatedCodeSize() const {
        return gcheapCodeMemory;
    }

#ifdef MMGC_POLICY_PROFILING
    REALLY_INLINE void GCHeap::GetMaxTotalHeapSize(size_t &heapBlocks,
                                                   size_t &privateBlocks) {
    ...
    GCHeap::Partition::GetPartitionSize() const {
        return blocksLen - numDecommitted + largeAllocation;
    }

    REALLY_INLINE size_t

    GCHeap::GetTotalHeapSize() const {
    ...
    REALLY_INLINE void GCHeap::PreventDestruct() { preventDestruct++; }
...
/* static */
    REALLY_INLINE size_t
    GCHeap::SizeToBlocks(size_t
    bytes) {
    return ((bytes + kBlockSize - 1) &
    size_t(kBlockMask)
    ) /
    kBlockSize;
}
...
#ifdef MMGC_HOOKS
REALLY_INLINE void GCHeap::EnableHooks() { hooksEnabled = true; }

REALLY_INLINE bool GCHeap::HooksEnabled() const { return hooksEnabled; }
#endif

#ifdef MMGC_MEMORY_PROFILER
REALLY_INLINE MemoryProfiler *GCHeap::GetProfiler() { return profiler; }

REALLY_INLINE void GCHeap::DumpFatties() { profiler->DumpFatties(); }
#endif

REALLY_INLINE MemoryStatus

GCHeap::GetStatus() { return status; }

/* static */
REALLY_INLINE size_t
GCHeap::CheckForAllocSizeOverflow(size_t
size,
size_t extra
) {
...
#else
// This is the 32-bit implementation, it avoids unnecessary checks for
// overflow.
...
/* static */
REALLY_INLINE size_t
GCHeap::CheckForCallocSizeOverflow(size_t
count,
size_t elsize
) {
...
#ifdef MMGC_64BIT
if (elsize > kMaxObjectSize || count >= kMaxObjectSize ||
...
/* static */
REALLY_INLINE size_t
GCHeap::CheckForNewSizeOverflow(size_t
count,
size_t elsize,
bool canFail
) {
...
// Verify that base address + offset does not overflow address range.
// It's not clear what error to report in this case, but we use it in situations
// where the only recourse by the user is to allocate a smaller object, even if
// the object size is less than kMaxObjectSize.  The problem is that
// kMaxObjectSize as presently defined on a 32-bit platform will already exceed
// available memory. See GC::allocaPush().

/* static */
REALLY_INLINE uintptr_t
GCHeap::AddToAddressWithOverflowCheck(uintptr_t
base,
size_t offset
) {
static const uintptr_t MAX_ADDR = ~uintptr_t(0);

if (base > (MAX_ADDR - offset))
...
REALLY_INLINE GCHeapConfig
&

GCHeap::Config() { return config; }

REALLY_INLINE void *GCHeap::GetStackEntryAddress() {
    return (void *) GetEnterFrame();
}

REALLY_INLINE EnterFrame
*

GCHeap::GetEnterFrame() { return enterFrame; }

REALLY_INLINE GC
*
GCHeap::SetActiveGC(GC
*gc) {
GCAssertMsg(IsStackEntered(),

"A MMGC_ENTER macro must exist to use GCHeap memory");
return GetEnterFrame()->
SetActiveGC(gc);
}
...
REALLY_INLINE void GCHeap::SetEntryChecks(bool to) { entryChecksEnabled = to; }
...
REALLY_INLINE bool GCHeap::HeapBlock::inUse() const { return prev == NULL; }
...
REALLY_INLINE void GCHeap::HeapBlock::Init(char *baseAddr, size_t size,
                                           bool dirty) {
    Clear();
...
GCHeap::Partition::LargeAllocSize(const void *item) {
...
GCHeap::Partition::Size(const void *item) {
...
GCHeap::Partition::GetFreeListIndex(size_t
size) {
...
return (uint32_t)((size - kUniqueThreshold) / kFreeListCompression +
kUniqueThreshold - 1);
}
}
...
REALLY_INLINE size_t
GCHeap::numHeapBlocksToNumBlocks(size_t
numBlocks) {
