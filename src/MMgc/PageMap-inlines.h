...
namespace MMgc {
    namespace PageMap {
        ...
        PageMapBase::MemStart() const { return memStart; }

        REALLY_INLINE uintptr_t

        PageMapBase::MemEnd() const { return memEnd; }

        REALLY_INLINE bool PageMapBase::AddrIsMappable(uintptr_t
        addr) const {
        return addr >=
        memStart &&addr<memEnd;
    }
    ...
    Tiered2::AddrToIndex1(uintptr_t
    addr) const {
...
REALLY_INLINE uint32_t
Tiered2::AddrToIndex2(uintptr_t
addr) const {
...
REALLY_INLINE uint32_t
Tiered2::AddrToByteShiftAmt(uintptr_t
addr) const {
// shift amount to determine position in the byte (times 2 b/c 2 bits per
// page)
return (
uint32_t(addr
>> kPageShift) & 0x3) * 2;
...
*
Tiered2::AddrToLeafBytes(uintptr_t
addr) const {
GCAssert(pageMap
!= NULL);
...
REALLY_INLINE PageType
Tiered2::LeafAddrToVal(uint8_t
*subMap,
uintptr_t addr
) const {
...
REALLY_INLINE void Tiered2::LeafAddrClear(uint8_t * subMap, uintptr_t
addr) {
uint32_t k = AddrToIndex2(addr);
...
REALLY_INLINE void Tiered2::LeafAddrSet(uint8_t * subMap, uintptr_t
addr,
PageType val
) {
uint32_t k = AddrToIndex2(addr);
...
REALLY_INLINE PageType
Tiered2::AddrToVal(uintptr_t
addr) const {
uint8_t *subMap = Tiered2::AddrToLeafBytes(addr);

MMGC_STATIC_ASSERT(kNonGC
== 0);
if (subMap == NULL)
return
kNonGC;
...
return
LeafAddrToVal(subMap, addr
);
}

REALLY_INLINE void Tiered2::AddrClear(uintptr_t
addr) {
uint8_t *subMap = Tiered2::AddrToLeafBytes(addr);
if (subMap == NULL)
return;
...
REALLY_INLINE void Tiered2::AddrSet(uintptr_t
addr,
PageType val
) {
uint8_t *subMap = Tiered2::AddrToLeafBytes(addr);
...
REALLY_INLINE PageType Tiered4::LeafAddrToVal(uint8_t *subsubMap,
...
REALLY_INLINE void Tiered4::LeafAddrSet(uint8_t *subsubMap, uintptr_t addr,
                                        PageType val) {
  uint32_t k = AddrToIndex3(addr);
