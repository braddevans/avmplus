...
// Avoid premature inlining for GO AOT; it prevents LLVM constant subexpression
// elimination
    REALLY_INLINE GC
    *

    GCRoot::GetGC() const { return gc; }
...
// GC
    ...
    *
    ...
    REALLY_INLINE void GC::QueueCollection() { policy.queueFullCollection(); }
...
// Normally extra will not be zero (overloaded 'new' operators take care of
// that) so the overflow check is not actually redundant.
...
// See comments around GC::Alloc that explain why the guard and table lookup for
// the small-allocator cases are correct.
    ...
    REALLY_INLINE void *GC::AllocPtrZero(size_t size, int partition) {
        ...
        if (size <= kLargestAlloc)
            return GetUserPointer(
                    containsPointersNonfinalizedAllocs[sizeClassIndex[(size - 1) >>
                                                                                 3]][partition]
                            ->Alloc(SIZEARG GC::kContainsPointers | GC::kZero));
#endif
    ...
    REALLY_INLINE void *GC::AllocPtrZeroFinalized(size_t size, int partition) {
        ...
        if (size <= kLargestAlloc)
            return GetUserPointer(
                    containsPointersFinalizedAllocs[sizeClassIndex[(size - 1) >> 3]]
                    [partition]
                            ->Alloc(
                                    SIZEARG GC::kContainsPointers |
                                            GC::kZero | GC::kFinalize));
#endif
...
#endif
        return Alloc(size,
                     GC::kContainsPointers | GC::kZero | GC::kFinalize |
                     GC::kInternalExact,
                     partition);
    }
    ...
    REALLY_INLINE void *GC::AllocRCObject(size_t size, int partition) {
        ...
        if (size <= kLargestAlloc)
            return GetUserPointer(
                    containsPointersRCAllocs[sizeClassIndex[(size - 1) >> 3]][partition]
                            ->Alloc(SIZEARG GC::kContainsPointers | GC::kZero | GC::kRCObject |
                                            GC::kFinalize));
#endif
        return Alloc(
                size, GC::kContainsPointers | GC::kZero | GC::kRCObject | GC::kFinalize,
                partition);
    }
    ...
    REALLY_INLINE void *GC::AllocRCObjectExact(size_t size, int partition) {
        ...
        if (size <= kLargestAlloc)
            return GetUserPointer(
                    containsPointersRCAllocs[sizeClassIndex[(size - 1) >> 3]][partition]
                            ->Alloc(SIZEARG GC::kContainsPointers | GC::kZero | GC::kRCObject |
                                            GC::kFinalize | GC::kInternalExact));
#endif
        return Alloc(size,
                     GC::kContainsPointers | GC::kZero | GC::kRCObject |
                     GC::kFinalize | GC::kInternalExact,
                     partition);
    }
...
#if !defined GCDEBUG && !defined AVMPLUS_SAMPLER && \
    !defined MMGC_MEMORY_PROFILER
        return GetUserPointer(
                noPointersNonfinalizedAllocs[0][kBoxedDoublePartition]->Alloc(
                        /*flags*/ 0));
#else
    ...
    template<>
    REALLY_INLINE void *
    GC::AllocBibopType<avmplus::AtomConstants::kBibopFloatType>() {
    ...
    (defined GCDEBUG || \
     0) /* Change 0 to 1 to enable the test in release builds */
        if (uintptr_t(p) & 15)
...
// For AllocExtra the trick is that we can compute (size|extra) quickly without
// risk of overflow and compare it to half the maximum small-alloc size (rounded
// down to 8 bytes), and if the guard passes then we can definitely take the
// quick path.  Most allocations are small.
//
// As 'extra' won't usually be known at compile time the fallback case won't
// usually compile away, though, so we risk bloating the code slightly here.
...
#endif
        return OutOfLineAllocExtra(
                size, extra, GC::kContainsPointers | GC::kZero | GC::kInternalExact,
                partition);
    }
...
#endif
        return OutOfLineAllocExtra(size, extra,
                                   GC::kContainsPointers | GC::kZero | GC::kFinalize,
                                   partition);
    }

    REALLY_INLINE void *
    GC::AllocExtraPtrZeroFinalizedExact(size_t size, size_t extra, int partition) {
#if !defined GCDEBUG && !defined AVMPLUS_SAMPLER
        GCAssert(partition < kNumGCPartitions);
        if ((size | extra) <= (kLargestAlloc / 2 & ~7)) {
            size += extra;
...
#undef SIZEARG
    ...
    REALLY_INLINE void GC::FreeNotNull(const void *item) {
        GCAssert(item != NULL);
        GCAssertMsg(onThread(), "GC called from a different thread or not associated "
                                "with a thread, missing MMGC_GCENTER macro perhaps.");
        // GCAssertMsg(!IsFinalized(item), "Finalizable storage must not be deleted
        // with GC::Free; the destructor will not be called.");
    ...
    REALLY_INLINE void GC::FreeFromDelete(const void *item) {
        if (item) {
            GCAssertMsg(onThread(),
                        "GC called from a different thread or not associated with a "
                        "thread, missing MMGC_GCENTER macro perhaps.");
...
/*static*/
    REALLY_INLINE size_t

    GC::Size(const void *ptr) {
...
/*static*/
    ...
    REALLY_INLINE GCHeap
    *

    GC::GetGCHeap() const { return heap; }

    REALLY_INLINE void GC::ReapZCT(bool scanStack) { zct.Reap(scanStack); }

    REALLY_INLINE bool GC::Reaping() { return zct.IsReaping(); }

    REALLY_INLINE bool GC::IncrementalMarking() { return marking; }
    ...
    REALLY_INLINE bool GC::Collecting() { return collecting; }

    REALLY_INLINE bool GC::Presweeping() { return presweeping; }
...
/*static*/
    REALLY_INLINE double GC::duration(uint64_t start) {
        return (double(VMPI_getPerformanceCounter() - start) * 1000) /
               VMPI_getPerformanceFrequency();
    }

/*static*/
    REALLY_INLINE uint64_t
    GC::ticksToMicros(uint64_t
    ticks) {
...
GC::bytesMarked() { return policy.bytesMarked(); }
...
GC::markIncrements() {
    return (uint32_t)
    policy.countIncrementalMark;
}

REALLY_INLINE bool GC::Destroying() { return destroying; }

REALLY_INLINE uintptr_t

GC::GetStackTop() const {
...
GC::GetStackEnter() const {
    return (uintptr_t)
    stackEnter;
}

REALLY_INLINE GCAutoEnter
*

GC::GetAutoEnter() { return stackEnter; }

REALLY_INLINE bool GC::onThread() { return VMPI_currentThread() == m_gcThread; }

REALLY_INLINE void GC::FreeBits(uint32_t * bits, int
sizeClass,
int partition
) {
#ifdef GCDEBUG
for (int i = 0, n = noPointersNonfinalizedAllocs[sizeClass][partition]
                        ->m_numBitmapBytes;
     i < n; i++)
...
m_bitsFreelists[sizeClass][partition] =
bits;
}
...
REALLY_INLINE void GC::TraceLocation(T *const *loc) {
    TracePointer((void *) *loc
    HEAP_GRAPH_ARG((uintptr_t * )
    loc));
}

REALLY_INLINE void GC::TraceLocation(uintptr_t * loc) {
    TracePointer((void *) (*loc & ~7)
    HEAP_GRAPH_ARG(loc));
}
...
REALLY_INLINE void GC::TraceLocation(MMgc::WriteBarrier<uintptr_t> const *loc) {
    TracePointer((void *) (loc->value() & ~7)
    HEAP_GRAPH_ARG(loc->location()));
}
...
template<>
REALLY_INLINE void
GC::TraceLocation(MMgc::WriteBarrierRC<uintptr_t> const *loc) {
    TracePointer((void *) (loc->value() & ~7)
    HEAP_GRAPH_ARG(loc->location()));
}
...
REALLY_INLINE void GC::TraceAtom(AtomWBCore *loc) {
    TraceAtomValue(loc->value()
    HEAP_GRAPH_ARG(loc->location()));
}

template<class T>
REALLY_INLINE void GC::TraceLocations(MMgc::GCMemberBase<T> *p,
                                      size_t numobjects) {
    for (size_t i = 0; i < numobjects; i++)
        TracePointer((void *) (p + i)->value()
    HEAP_GRAPH_ARG((uintptr_t * )(p + i)->location()));
}
...
REALLY_INLINE void GC::TraceLocations(T **p, size_t numobjects) {
    for (size_t i = 0; i < numobjects; i++)
        TracePointer(p[i]
    HEAP_GRAPH_ARG((uintptr_t * )(p + i)));
}

REALLY_INLINE void GC::TraceLocations(uintptr_t * p, size_t
numobjects) {
for (
size_t i = 0;
i<numobjects;
i++)
TracePointer((void *)(p[i] & ~7)
HEAP_GRAPH_ARG(p
+ i));
}
...
REALLY_INLINE void GC::TraceConservativeLocation(uintptr_t * loc) {
    TraceConservativePointer(*loc, false
    HEAP_GRAPH_ARG(loc));
}

REALLY_INLINE PageMap::PageType
GC::GetPageMapValue(uintptr_t
addr) const {
GCAssert(pageMap
.
AddrIsMappable(addr)
...
REALLY_INLINE PageMap::PageType
GC::GetPageMapValueGuarded(uintptr_t
addr) {
if (pageMap.
AddrIsMappable(addr)
)
return
GetPageMapValue(addr);
MMGC_STATIC_ASSERT(PageMap::kNonGC
== 0);
return
PageMap::kNonGC;
}
...
REALLY_INLINE void GC::AddToZCT(RCObject *obj) { zct.Add(obj); }

REALLY_INLINE void GC::RemoveFromZCT(RCObject *obj) { zct.Remove(obj); }
...
GC::GetNumBlocks() { return policy.blocksOwnedByGC(); }

REALLY_INLINE void *GC::allocaTop() { return stacktop; }
...
#ifdef DEBUGGER
REALLY_INLINE void *GC::GetAttachedSampler() { return m_sampler; }
...
REALLY_INLINE void GC::UpdateDependentAllocation(uint32_t bytes,
                                                 DependentMemoryType memType) {
  GCAssert(memType < typeCount);
  if (memType < typeCount)
...
REALLY_INLINE void
GC::UpdateDependentDeallocation(uint32_t bytes, DependentMemoryType memType) {
  GCAssert(memType < typeCount);
  if (memType < typeCount)
    m_dependentMemory[memType] -= bytes;
}

REALLY_INLINE telemetry::ITelemetry *GC::getTelemetry() { return m_telemetry; }
...
REALLY_INLINE GC::AllocaAutoPtr::AllocaAutoPtr() : gc(NULL), unwindPtr(NULL) {}
...
REALLY_INLINE bool GCAutoEnter::Entered() { return m_gc != NULL; }
