...
#endif
    : m_base(NULL), m_top(NULL), m_limit(NULL), m_topSegment(NULL),
      m_hiddenCount(0), m_hiddenSegments(0), m_extraSegment(NULL), m_deadItem(0)
#ifdef MMGC_MARKSTACK_ALLOWANCE
      ,
...
bool GCMarkStack::Push_StackMemory(const void *p, uint32_t size,
                                   const void *baseptr) {
...
void GCMarkStack::Pop_StackMemory(const void *&p, uint32_t &size,
                                  const void *&baseptr) {
...
bool GCMarkStack::Push_LargeObjectChunk(const void *p, uint32_t size,
                                        const void *baseptr) {
  uintptr_t *top = allocSpace(4);
  if (top == NULL)
    return false;
...
void GCMarkStack::Pop_LargeObjectChunk(const void *&p, uint32_t &size,
                                       const void *&baseptr) {
...
bool GCMarkStack::Push_LargeRootChunk(const void *p, uint32_t size,
                                      const void *baseptr) {
  uintptr_t *top = allocSpace(4);
  if (top == NULL)
    return false;
...
void GCMarkStack::Pop_LargeRootChunk(const void *&p, uint32_t &size,
                                     const void *&baseptr) {
...
void GCMarkStack::ClearRootProtectorAndChunkAbove(uintptr_t index,
                                                  const void *rootptr) {
#ifdef GCDEBUG
...
inline uintptr_t *GCMarkStack::limit(GCMarkStack::StackSegment *seg) {
  static const size_t kMarkStackItems =
      (GCHeap::kBlockSize - sizeof(StackSegment)) / sizeof(uintptr_t);
#ifdef GCDEBUG
...
void GCMarkStack::TransferOneInactiveSegmentFrom(GCMarkStack &other) {
  ...
  // If a segment was inserted into an empty stack then pop the now empty top
  // segment.
  if (m_top == m_base)
    PopSegment();
...
bool GCMarkStack::TransferEverythingFrom(GCMarkStack &other) {
  ...
  if (!MakeSpaceForSegments(incoming_segments))
    ...
    // Make sure we have a segment in the other stack with which to reestablish
    // invariants.
#ifdef MMGC_MARKSTACK_ALLOWANCE
  // Optimistically assume we'll succeed so that there will be an allowance for
  // the other stack to allocate in.
  other.m_allowance += incoming_segments;
  if (!other.PopulateExtraSegment(false)) {
    other.m_allowance -=
        incoming_segments; // Won't be moving anything after all
    return false;
  }
#else
  if (!other.PopulateExtraSegment(false))
  ...
  uint32_t subject_hc =
      other.m_hiddenCount +
      uint32_t(other.m_topSegment->m_savedTop - items(other.m_topSegment));
  ...
  // If the segments were inserted into an empty stack then pop the now empty
  // top segment.
  if (m_top == m_base)
    PopSegment();
...
//
// It's not strictly necessary to do it this way.  The two alternatives are to
// use a distinguished type of stack item (kDeadItem), or using a NULL pointer.
// A kDeadItem type could be used at zero cost, dead items would be handled on
// the slow path. However a kDeadItem would always be at least two words long
// and could not be used to clear out a GCObject.  Also, a kDeadItem would be
// variable-length, complicating code that parses stack items.  A NULL GCObject
// pointer is possible as the current marker architecture would just divert
// those items onto the slow path at zero cost. However, the invariant that
// there are no NULL pointers on the stack is worth more to me at present.
...
inline void *GCMarkStack::AllocStackSegment(bool mustSucceed) {
  ...
  if (mustSucceed)
    return GCHeap::GetGCHeap()
        ->GetPartition(kStackPartition)
        ->Alloc(1, GCHeap::flags_Alloc);
  else
    return GCHeap::GetGCHeap()
        ->GetPartition(kStackPartition)
...
bool GCMarkStack::Invariants() {
  ...
  uint32_t hc = 0;
  uint32_t ns = 0;
  for (StackSegment *seg = m_topSegment->m_prev; seg != NULL;
       seg = seg->m_prev) {
