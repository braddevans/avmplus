...
namespace MMgc {
/*static*/
    REALLY_INLINE GCLargeAlloc::LargeBlock
    *

    GCLargeAlloc::GetLargeBlock(const void *addr) {
...
GCLargeAlloc::GetGCBits(const void *realptr) {
...
REALLY_INLINE bool GCLargeAlloc::IsProtectedAgainstFree(const void *item) {
    ...
    return (block->flags[0] & kQueued) != 0 ||
           (block->flags[1] & kProtected) != 0;
}
...
REALLY_INLINE void *GCLargeAlloc::FindBeginning(const void *item) {
    LargeBlock *block = GetLargeBlock(item);
    GCAssertMsg(item >= block->GetObject(),
...
REALLY_INLINE
GCLargeAllocIterator::GCLargeAllocIterator(MMgc::GCLargeAlloc *alloc)
        : alloc(alloc), block(alloc->m_blocks) {}
