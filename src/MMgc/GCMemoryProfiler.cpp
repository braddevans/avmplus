...
// include total and swept memory totals in memory profiling dumps as opposed to
// just "live"
...
uintptr_t
    destructor_addresses[213]; // direct-mapped cache for destructor addresses
...
MemoryProfiler::MemoryProfiler()
    : stackTraceMap(128), stringsTable(4), nameTable(128), allocInfoTable(128) {
...
MemoryProfiler::~MemoryProfiler() {
  ...
  ObjectPopulationProfiler<AllocationSiteHandler>::NodeTable::Iterator
      nodesIter(&nodes);
  ...
  GCLog("GC allocation sites: %d, conservatively traced: %d (%d%% exact)\n",
        nodes.count(), numConserv,
        (100 * (nodes.count() - numConserv)) / nodes.count());
#endif
...
void MemoryProfiler::RecordAllocation(const void *item, size_t askSize,
                                      size_t gotSize, bool managed) {
  if (gotSize == 0) {
...
void MemoryProfiler::RecordDeallocation(const void *item, size_t size) {
  ...
  // When we fix the perf issues with GCHashtable::remove, we should change this
  // back to a remove.
  AllocInfo *info = (AllocInfo *)allocInfoTable.get(item);
...
size_t MemoryProfiler::GetAskSize(const void *item) {
  MMGC_LOCK(lock);
  AllocInfo *info = (AllocInfo *)allocInfoTable.get(item);
  // failing this assert means that either FinalizeHook() was called before
  // GetAsk() or this item is being double deleted
  GCAssert(info != NULL);
...
public:
  PackageGroup(const char *name)
      : name(name), size(0), count(0), categories(16) {}
  const char *name;
  size_t size;
  uint32_t count;
...
void MemoryProfiler::DumpFatties() {
    ...
    for (int j = 0; j < kNumTracesPerType; j++) {
      ...
      if (!tg->traces[j] || tg->traces[j]->size < size) {
        if (j != kNumTracesPerType - 1) {
          VMPI_memmove(&tg->traces[j + 1], &tg->traces[j],
                       (kNumTracesPerType - j - 1) * sizeof(void *));
        }
        tg->traces[j] = trace;
        break;
      }
  ...
  PackageGroup **packages =
      (PackageGroup **)VMPI_alloc(packageCount * sizeof(PackageGroup *));
  if (packages == NULL)
    return;
  ...
  while ((package = (const char *)pack_iter.nextKey()) != NULL) {
    ...
    for (unsigned j = 0; j < packageCount; j++) {
      if (packages[j] == NULL || packages[j]->size < pg->size) {
        if (j != packageCount - 1) {
          VMPI_memmove(&packages[j + 1], &packages[j],
                       (packageCount - j - 1) * sizeof(PackageGroup *));
        }
        packages[j] = pg;
        break;
      }
  ...
  GCLog("\n\nMemory allocation report for %llu allocations, totaling %llu kb "
        "(%llu ave) across %llu packages\n",
  ...
  for (unsigned i = 0; i < packageCount; i++) {
    ...
    CategoryGroup **residentFatties =
        (CategoryGroup **)VMPI_alloc(numTypes * sizeof(CategoryGroup *));
    ...
    while ((name = (const char *)iter.nextKey()) != NULL) {
      ...
      for (int j = 0; j < numTypes; j++) {
        ...
        if (residentFatties[j]->size < tg->size) {
          if (j != numTypes - 1) {
            VMPI_memmove(&residentFatties[j + 1], &residentFatties[j],
                         (numTypes - j - 1) * sizeof(CategoryGroup *));
          }
          residentFatties[j] = tg;
          break;
        }
    ...
    GCLog("%s - %3.1f%% - %llu kb %u items, avg %llu b\n", pg->name,
          PERCENT(residentSize, pg->size), (unsigned long long)(pg->size >> 10),
          (unsigned)pg->count,
    ...
    for (int i = 0; i < numTypes; i++) {
      ...
      GCLog("\t%s - %3.1f%% - %llu kb %u items, avg %llu b\n", tg->name,
            ...
            (unsigned long long)(tg->size >> 10), (unsigned)tg->count,
      ...
      for (int j = 0; j < kNumTracesPerType; j++) {
        ...
        if (trace) {
          ...
          GCLog("\t\t %3.1f%% - %llu kb - %u items - ", PERCENT(tg->size, size),
                (unsigned long long)(size >> 10), (unsigned)count);
...
void MemoryProfiler::DumpSimple() {
  ...
  // at runtime, and just need to dump the raw addresses (which makes sorting
  // impossible)
  GCStackTraceHashtable_VMPI::Iterator iter(&stackTraceMap);
  const void *obj;
  size_t num_traces = 0;
  // Get a stack trace with AVMPI_captureStackTrace as the top address - this
  // will be used to calculate the base address to translate the addresses into
  // relative addresses later
  uintptr_t trace[kMaxStackTrace];
  VMPI_memset(trace, 0, sizeof(trace));
...
void MemoryProfiler::DumpSimpleByTotal(uint32_t limit,
                                       MemoryProfiler::SortMode sort) {
  struct Item {
    uint64_t value;
    StackTrace *trace;
  };
  ...
  for (uint32_t i = 0; i < limit && traces[i].value > 0; i++) {
    GCLog("%llu b - %u items - ",
          (unsigned long long)traces[i].trace->totalSize,
          (unsigned)traces[i].trace->totalCount);
...
#endif
    if (VMPI_sscanf(x, "%u,%s%n", &limit, buf, &len) == 2 &&
        unsigned(len) == VMPI_strlen(x)) {
      if (VMPI_strcmp(buf, "count") == 0)
        s = MemoryProfiler::BY_COUNT;
      else
        s = MemoryProfiler::BY_VOLUME;
    } else if (VMPI_sscanf(x, "%u%n", &limit, &len) == 1 &&
               unsigned(len) == VMPI_strlen(x)) {
      s = MemoryProfiler::BY_VOLUME;
    } else {
      limit = 40;
...
void SetMemType(const void *s) {
  if (GCHeap::GetGCHeap()->GetProfiler() != NULL) {
    GCAssertMsg(s == NULL ||
                    GCHeap::GetGCHeap()->GetProfiler()->GetAllocationTrace(s) !=
                        NULL,
                "Unknown allocation");
...
void DumpStackTraceHelper(uintptr_t *trace, int limit = INT_MAX) {
  ...
  *tp++ = '\n';
  for (int i = 0; trace[i] != 0; i++) {
    ...
    bool found_name;
    if ((found_name = AVMPI_getFunctionNameFromPC(trace[i], buff,
                                                  sizeof(buff))) == false) {
    ...
    uint32_t lineNum;
    if (AVMPI_getFileAndLineInfoFromPC(trace[i], buff, sizeof(buff),
                                       &lineNum)) {
      // Don't bother with file, linenumber, and address if we're just printing
      // the address anyways
      if (found_name) {
...
void PrintAllocStackTrace(const void *item) {
  if (GCHeap::GetGCHeap()->GetProfiler()) {
    StackTrace *trace =
        GCHeap::GetGCHeap()->GetProfiler()->GetAllocationTrace(item);
...
void PrintDeleteStackTrace(const void *item) {
  if (GCHeap::GetGCHeap()->GetProfiler()) {
    StackTrace *trace =
        GCHeap::GetGCHeap()->GetProfiler()->GetDeletionTrace(item);
    GCAssertMsg(trace != NULL, "Trace was null");
    PrintStackTrace(trace);
  }
}
...
struct ObjectPopulationProfiler<T>::PopulationNode : public GCAllocObject {
  PopulationNode(StackTrace *trace)
      : trace(trace), numobjects(0), numbytes(0),
...
template <class T>
ObjectPopulationProfiler<T>::ObjectPopulationProfiler(GC *gc,
                                                      const char *profileName,
                                                      bool roots, bool stacks)
    : gc(gc), heap(gc->GetGCHeap()), profileName(profileName), root_info(roots),
      stack_info(stacks) {}

template <class T> ObjectPopulationProfiler<T>::~ObjectPopulationProfiler() {
...
template <class T>
void ObjectPopulationProfiler<T>::dumpTopBacktraces(int howmany,
                                                    DumpMode mode) {
  MMgc::GC::AllocaAutoPtr _popnode;
  PopulationNode **xs = (PopulationNode **)avmStackAllocArrayGC(
      gc, _popnode, howmany, sizeof(PopulationNode *));
  int live = 0;
  ...
  while ((obj = traceIter.nextKey()) != NULL) {
    ...
    for (int i = 0; i < live && !inserted; i++) {
      ...
      if (mode == BY_VOLUME)
        ...
        insert = l->numobjects > xs[i]->numobjects ||
                 (l->numobjects == xs[i]->numobjects &&
                  l->numbytes > xs[i]->numbytes);
      if (insert) {
    ...
    for (size_t i = 0; i < ARRAY_SIZE(rootHistogram); i++) {
      if (rootHistogram[i].numobjects > 0)
        GCLog("  Class %d-%d:  %llu bytes scanned  - %u objs scanned\n",
              (1 << i), (1 << (i + 1)) - 1,
              (unsigned long long)rootHistogram[i].numbytes,
              (unsigned)rootHistogram[i].numobjects);
    }
  }
  if (stack_info) {
    ...
    for (size_t i = 0; i < ARRAY_SIZE(stackHistogram); i++) {
      if (stackHistogram[i].numobjects > 0)
        GCLog("  Class %d-%d:  %llu bytes scanned  - %u objs scanned\n",
              (1 << i), (1 << (i + 1)) - 1,
              (unsigned long long)stackHistogram[i].numbytes,
              (unsigned)stackHistogram[i].numobjects);
    }
  }
  GCLog("Objects:\n");
  for (int i = 0; i < live; i++) {
    dumpObjectInfo((unsigned long long)xs[i]->numbytes,
                   (unsigned)xs[i]->numobjects);
...
template <class T>
void ObjectPopulationProfiler<T>::dumpObjectInfo(unsigned long long numbytes,
                                                 unsigned numobjects) {
  GCLog("  %llu bytes scanned - %u objs scanned", numbytes,
        (unsigned)numobjects);
}

DeletionProfiler::DeletionProfiler(GC *gc, const char *profileName)
    : ObjectPopulationProfiler<DeletionNodeHandler>(gc, profileName, false,
                                                    false) {}
...
void DeletionProfiler::dumpObjectInfo(unsigned long long numbytes,
                                      unsigned numobjects) {
  (void)numbytes;
  if (numobjects == 1)
...
WeakRefAllocationSiteProfiler::WeakRefAllocationSiteProfiler(
    GC *gc, const char *profileName)
    : ObjectPopulationProfiler<AllocationSiteHandler>(gc, profileName, false,
                                                      false),
      peakPopulation(0), scannedAtGC(0), removedAtGC(0), collections(0) {}
...
void WeakRefAllocationSiteProfiler::reportGCStats(uint32_t scanned,
                                                  uint32_t removed) {
  scannedAtGC += scanned;
  removedAtGC += removed;
  collections++;
...
void WeakRefAllocationSiteProfiler::dumpObjectInfo(unsigned long long numbytes,
                                                   unsigned numobjects) {
  (void)numbytes;
  if (numobjects == 1)
...
size_t DebugSize() {
  ...
  // 1 word of padding on 64-bit systems (ie round up to even number of 32-bit
  // words)
  //
  ...
  const size_t words =
      USER_POINTER_WORDS + poison_words + writeback_pointer_words;
  const size_t words_rounded = (words + 1) & ~1;
  return words_rounded * sizeof(int32_t);
}

/*
 * allocate the memory such that we can detect underwrites, overwrites and
 * remember the allocation stack in case of a leak.   Memory is laid out like
 * so:
 *
 ...
 * it enables the leak detection to work see ~FixedAlloc.  Underwrite detection
 * isn't perfect, an assert will be fired if the stack table index is invalid
 * (greater than the table size or to an unused table entry) or if the size gets
 * mangled and the end tag isn't at mem+size.
 */
...
void DebugFreeHelper(const void *item, int poison, size_t wholeSize,
  ...
  if (actualFree) {
    ...
    if (*endMarker != uint32_t(GCHeap::GCEndOfObjectPoison)) {
      ...
      // overrun, put a memory breakpoint on the location endMarker is pointing
      // to.
...
void ReportDeletedMemoryWrite(const void *item) {
  GCDebugMsg(
      false,
      "Object 0x%llx was written to after it was deleted, allocation trace:",
      (unsigned long long)(uintptr_t)item);
  PrintAllocStackTrace(GetUserPointer(item));
