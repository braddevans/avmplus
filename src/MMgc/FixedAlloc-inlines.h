...
namespace MMgc {
/*static*/
    REALLY_INLINE FixedAlloc
    *

    FixedAlloc::GetFixedAlloc(void *item) {
        return GetFixedBlock(item)->alloc;
    }

/*static*/
    REALLY_INLINE FixedAlloc::FixedBlock
    *

    FixedAlloc::GetFixedBlock(const void *item) {
...
/*static*/
    ...
    FixedAlloc::Size(const void *item) {
        return GetFixedBlock(item)->size;
    }

    REALLY_INLINE size_t

    FixedAlloc::GetBytesInUse() const {
    ...
    FixedAlloc::GetMaxAlloc() const {
        return m_numBlocks * m_itemsPerBlock;
    }

    REALLY_INLINE size_t

    FixedAlloc::GetNumBlocks() const { return m_numBlocks; }

    REALLY_INLINE bool FixedAlloc::IsFull(FixedBlock * b)
    const {
    return b->numAlloc ==
    m_itemsPerBlock;
}
...
/*static*/
REALLY_INLINE void FixedAlloc::InlineFreeHook(
    void *item MMGC_MEMORY_PROFILER_ARG(size_t &askSize)) {
  FixedBlock *b = FixedAlloc::GetFixedBlock(item);
...
REALLY_INLINE void *FixedAllocSafe::Alloc(size_t
size,
FixedMallocOpts flags
) {
void *item;
{
...
"NULL is only valid when kCanFail is set");
}
#ifdef MMGC_HOOKS
...
REALLY_INLINE void FixedAllocSafe::Free(void *ptr) {
    ...
    {
        MMGC_LOCK(m_spinlock);
        FixedAlloc::InlineFreeSansHook(ptr
        MMGC_MEMORY_PROFILER_ARG(askSize));
    }
}

REALLY_INLINE void *FixedAlloc::InlineAllocSansHook(size_t
size,
...
"MMGC_ENTER must be on the stack");
GCAssertMsg(((size_t)
m_itemSize >= size), "allocator itemsize too small");
...
// Assert that the freelist hasn't been tampered with (by writing to the
// first 4 bytes).
GCAssert(
        b
->firstFree == NULL ||
...
items &&
(((uintptr_t)
b->firstFree - (uintptr_t)b->items) % b->size) == 0 &&
(uintptr_t)b->firstFree <
((uintptr_t)
b &GCHeap::kBlockMask
) + GCHeap::kBlockSize));
...
VALGRIND_MEMPOOL_ALLOC(b, item, b
->size);
...
/*static*/
REALLY_INLINE void FixedAlloc::InlineFreeSansHook(
        void *item MMGC_MEMORY_PROFILER_ARG(size_t askSize)
...
//      GCAssertMsg(b->alloc->m_heap->IsAddressInHeap(item), "Bogus pointer
//      passed to free");
...
/*static*/
REALLY_INLINE FixedAllocSafe
*

FixedAllocSafe::GetFixedAllocSafe(void *item) {
    return (FixedAllocSafe * )
    FixedAlloc::GetFixedAlloc(item);
}

/*static*/
REALLY_INLINE void *FastAllocator::operator new(size_t
size,
FixedAlloc *alloc
) {
return alloc->
