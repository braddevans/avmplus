...
// A number of the inline functions for this class are currently in GC.cpp
    ...
    REALLY_INLINE void GCPolicyManager::signalZCTAdd(bool initial,
                                                     uint32_t population) {
    ...
    REALLY_INLINE void
    GCPolicyManager::signalDemographics(size_t candidate_words,
                                        size_t could_be_pointer,
...
// OPTIMIZEME: the byte counts are needed for policy, but the object counts are
// only for unspecified profiling and we could in principle enable object
// counting only in a profiling mode.
    ...
    GCPolicyManager::lowerLimitCollectionThreshold() {
