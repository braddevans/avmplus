 ...
 *
 * FixedMalloc is a singleton, obtainable by calling
 * FixedMalloc::GetFixedMalloc. The "owner" of the FixedMalloc is GCHeap, the
 * block manager.
 *
 ...
 */
    class FixedMalloc {
        ...
        friend class avmplus::ST_mmgc_fixedmalloc_findbeginning::
        ST_mmgc_fixedmalloc_findbeginning;
    ...
    public:
         ...
         *
         * @return  A pointer to the object.  The pointer may be NULL only if kCanFail
         * is part of flags.  The memory is zeroed only if kZero is part of flags.
         */
         ...
         *          The memory is /not/ necessarily zeroed; for zeroed memory, use
         * Alloc().
         *
         ...
         * is part of flags.  The memory is zeroed only if kZero is part of flags.
         *
         * @note Exactly like Alloc, but guaranteed not to be inlined - used by ::new
         * etc.
         */
        void *FASTCALL
        OutOfLineAlloc(size_t
        size,
        FixedMallocOpts flags = kNone
        );
         ...
         * @param flags  A bit vector of allocation options
         *
         * @return  A pointer to the aggregate object.  The pointer may be NULL only
         * if kCanFail is part of flags.  The memory is zeroed only if kZero is part
         *          of flags.
         *
         ...
         * @param item  The object to free.
         *
         * @note  Exactly like Free, but guaranteed not to be inlined - used by
         * ::delete etc.
         */
        void FASTCALL

        OutOfLineFree(void *p);
         ...
         *
         * @note  The returned value is the totalAllocated value returned from
         * GetUsageInfo.
         */
    ...
    private:
#ifdef GCDEBUG
        // Data type used for tracking live large objects, used by
        ...
        // Return the total number of blocks allocated by FixedMalloc for large-object
        // allocations.
        ...
        // @see FixedAlloc::FindBeginning
        bool FindBeginningAndSize(const void *addr, const void *&begin_recv,
                                  size_t &size_recv);
        ...
        // Return the allocated size (in bytes) of 'item', which must have been
        // returned from LargeAlloc.
        size_t LargeSize(const void *item);

    private:
        static FixedMalloc
                *instances[kNumFixedPartitions]; // A single FixedMalloc instance for each
        // partition
...
#endif
        const static int kNumSizeClasses =
                41; // The number of small-object size classes
    ...
    private:
        ...
        int m_largeAllocHeapPartition; // The heap partition in which we allocate
        // large objects, set in InitInstance
        FixedAllocSafe
                m_allocs[kNumSizeClasses]; // The array of size-segregated allocators for
        // small objects, set in InitInstance
        ...
        class FindBeginningRootsCache {
            ...
            bool Lookup(const void *addr, const void *&begin_recv, size_t &size_recv);
        ...
        vmpi_spin_lock_t m_largeAllocInfoLock; // Protects numLargeBlocks and
        // totalAskSizeLargeAllocs

        size_t
                numLargeBlocks; // Number of large-object blocks owned by this FixedMalloc
#ifdef MMGC_MEMORY_PROFILER
        size_t totalAskSizeLargeAllocs; // The current number of bytes requested for
                                        // large objects
#endif
#if defined GCDEBUG
        vmpi_spin_lock_t m_largeObjectLock; // Protects largeObjects
        LargeObject
            *largeObjects; // Data structure of live large objects, initially NULL
#endif
    };
} // namespace MMgc
#endif /* __Malloc__ */
