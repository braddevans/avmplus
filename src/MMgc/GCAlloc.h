 ...
 */
    struct GCBlockHeader {
        uint8_t bibopTag;  // *MUST* be the first byte.  0 means "not a bibop block."
        // For others, see core/atom.h.
        uint8_t bitsShift; // Right shift for lower 12 bits of a pointer into the
        // block to obtain the mark bit item for that pointer
        // bitsShift is only used if MMGC_FASTBITS is defined but
        // its always present to simplify header layout.
        uint8_t containsPointers; // nonzero if the block contains pointer-containing
        // objects
 ...
 */
    enum {
        ...
        kFinalizable =
        4, // object's destructor must be called when the object is destroyed
        kHasWeakRef = 8,     // there's an entry for the object in the weakRefs table
        kVirtualGCTrace = 16 // object derived from GCTraceableBase and has gcTrace
        // override(s), see GCObject.h
        // free: 32
        // free: 64
        // free: 128
    };
 ...
 */
    ...
    private:
        ...
        enum {
            kFreelist = kMark | kQueued
        };

        GCAlloc(GC *gc, int itemSize, bool containsPointers, bool isRC,
                bool isFinalized, int sizeClassIndex, int sliceIndex,
                int partitionIndex, uint8_t bibopTag);
        ...
        // Return the actual size of items managed by this allocator (includes
        // debugging overheads)
        REALLY_INLINE uint32_t

        GetItemSize() { return m_itemSize; }
    ...
    private:
        const static int kBlockSize =
                4096; // This must be the same as GCHeap::kBlockSize, we check it in
        // GCAlloc::GCAlloc

        const static short kFlagNeedsSweeping =
                1; // set if the block had finalized objects and needs to be swept
        const static short kFlagWeakRefs =
        ...
        struct GCBlock : public GCBlockHeader {
            GCBlock *prev; // the previous block on the list of all blocks ('next' is in
            // the block header)
            void *firstFree;   // first item on free list
            GCBlock *prevFree; // the previous block on the lists of blocks with free or
            // sweepable objects
            GCBlock *nextFree; // the next block on the lists of blocks with free or
            // sweepable objects
            short numFree;     // the number of free objects in this block
            uint8_t
                    slowFlags; // flags for special circumstances: kFlagNeedsSweeping, etc
            bool finalizeState: 1; // whether we've been visited during the Finalize
            // stage
        ...
        // Quick list of free objects.  See comment in GCAlloc.cpp for general
        // information.

        void *m_qList; // Linked list of some free objects for this allocator
        int m_qBudget; // Number of items we can yet free before obtaining a larger
        // budget
        int m_qBudgetObtained; // Quick list budget actually obtained from the GC for
        // this allocator
        ...
        // marking), and other hot code.  Code that walks the byte map (eg the
        // sweeper) should do so by stepping through objects one by one and computing
        // the byte index for each (this is cheap) rather than examining bytes in the
        // byte map in sequence.
        //
        ...
        //
        // The key is that a unique byte index will be assigned to each object
        // position in the block even for non-power-of-two object sizes.  I don't have
        // a mathematical proof for this, but it's easy to test it exhaustively (and
        // I've done so).
        //
        ...
        static int ConservativeGetMark(const void *item,
