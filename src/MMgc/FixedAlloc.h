 ...
 */
    ...
    public:
        ...
        /**
         * Free an object allocated with a FixedAlloc instance.  The object is
         * returned to the FixedAlloc that provided it.
         *
    ...
    public:
         ...
         *                          is FixedAllocSafe, and certain unlocking
         * operations will be performed in out-of-memory situations.
         */
        FixedAlloc(uint32_t itemSize, GCHeap *heap, int partition,
                   bool isFixedAllocSafe = false);
         ...
         *
         * @param size   The size of the object; this is used for debugging only, as
         * the size allocated is fixed by the allocator.
        ...
        /**
         * Obtain current (not running total) allocation information for this
         * FixedAlloc.
         *
    ...
    public:
        // This structure is really private to FixedAlloc, but the current Symbian
        // compiler requires this structure to be public (now == May, 2010).
        struct FixedBlock {
            void *firstFree;   // First object on the block's free list
            void *nextItem;    // First object free at the end of the block
            FixedBlock *next;  // Next block on the list of blocks (m_firstBlock list in
            // the allocator)
            ...
            FixedBlock
                    *prevFree;     // Previous block on the list of blocks with free items
    ...
    private:
        ...
        size_t m_totalAskSize; // Current total amount of memory requested from this
                               // allocator
#endif
        bool const m_isFixedAllocSafe; // true if this allocator's true type is
        // FixedAllocSafe
    ...
    private:
        ...
        static void
        InlineFreeSansHook(void *item MMGC_MEMORY_PROFILER_ARG(size_t askSize)
        ...
        static void
        InlineFreeHook(void *item MMGC_MEMORY_PROFILER_ARG(size_t &askSize));
#endif
 ...
 * manually freed objects using 'new' and 'delete' operators for object
 * management.
 */
    class FastAllocator {
    public:
