...
//#define ZCT_TESTING                   // Test the handling of a failure to
//extend the ZCT
 ...
 *   is ensured by GC::FinishIncrementalMark returning immediately if
 * zct.reaping is true, and by ZCT::Reap returning immediately if gc->collecting
 * is true.
 *
 ...
 *   below a block the block is removed from the ZCT and added to an empty
 * blocks pool.
 *
...
#ifdef ZCT_TESTING
// Max number less 1 of blocks the ZCT may use for the second level of the block
// table as well as the pinned table during reaping.
static uint32_t zct_allowance = 0;
#endif

ZCT::ZCT()
    : gc(NULL), blocktable(NULL), blocktop(NULL), reaping(false), budget(0),
      bottom(NULL), top(NULL), limit(NULL), topIndex(0), slowState(false),
      slowBottom(NULL), slowTop(NULL), slowLimit(NULL), slowTopIndex(0),
      pinTop(NULL), pinLimit(NULL), pinIndex(0), pinList(0), pinLast(0),
      freeList(0) {}
...
void ZCT::SetGC(GC *gc) {
  ...
  const uint32_t numblocks =
      RCObject::ZCT_CAPACITY / CAPACITY(RCObject **) / CAPACITY(RCObject ***);

  blocktable = (RCObject ***)GCHeap::GetGCHeap()
                   ->GetPartition(kZCTPartition)
                   ->Alloc(numblocks); // must succeed, so use default flags
  for (uint32_t i = 0; i < CAPACITY(RCObject **) * numblocks; i++)
    blocktable[i] = NULL;
  blocktable[0] = (RCObject **)GCHeap::GetGCHeap()
                      ->GetPartition(kZCTPartition)
                      ->Alloc(1); // must succeed, so use default flags
...
void ZCT::AddSlow(RCObject *obj) {
  ...
  if (gc->collecting) {
    // This is a vestige from FP8 to fix bug 165100, it has the effect of
    // delaying the deletion of some objects; this causes the site to work.
  ...
  // Overflow.
  // Expand or reap?  Sometimes we must grow even if the budget has been
  // exhausted.
  ...
  if (reaping)
    ...
    // 'obj' will not be reaped as it's on the stack; we'll add it to the ZCT
    // below.
    Reap();
...
bool ZCT::CanGrow() {
  return (slowState ? slowTopIndex : topIndex) + CAPACITY(RCObject *) <=
...
void ZCT::Reap(bool scanStack) {
  ...
  // Note that we must pin from root segments even if scanStack is false,
  // because the MMGC_GC_ROOT_THREAD creates one AutoRCRootSegment that is not
  // managed by avmStackAlloc. The root segment list should be very short if
  // scanStack==false so performance-wise this is not a big deal.
  //
  // It is not necessary to pin from the mark and barrier stacks because there
  // is a test in GC::Free that prevents queued objects from being deleted; we
  // have to pay for that check in any case and can depend on it here.
  //
  ...
  //
  // Popping an element off the end of the ZCT, it is either NULL, pinned, or
  // unpinned.
  ...
  //
  // Depth-first processing is desirable because object graphs will tend to be
  // wider than they are deep; going depth-first reduces ZCT growth during
  // reaping.
  //
  // Memory use is optimal to within a constant: space occupied by a pointer to
  // a reaped object is released immediately, and empty segments popped off the
  // ZCT are used for the list of replacement blocks.
  ...
  if (gc->heap->Config().gcstats && objects_reaped > 0) {
    ...
    gc->gclog("[mem] DRC reaped %u objects (%u kb) freeing %u pages (%u kb) in "
              "%.2f millis (%.4f s)\n",
              objects_reaped, unsigned(bytes_reaped / 1024),
...
#ifdef MMGC_POLICY_PROFILING
  gc->policy.signalReapWork(objects_reaped, uint32_t(bytes_reaped),
                            objects_pinned);
#endif
...
void ZCT::PinStackObjects(const void *start, size_t len) {
  ...
  while (p < end) {
    ...
    if (val && GC::IsRCObject(val)) {
      // We must pin all objects that are reachable from the stack whether
      // they're in the ZCT or not, because destroying an object not in the ZCT
      // may push additional references onto the ZCT, and if those are reachable
      // from the stack they must be pinned.  (Ergo adding objects during
      // reaping must not clear the ZCT flag.)
...
RCObject **ZCT::PleaseAllocBlock() {
  ...
  if (freeList != NULL) {
    ...
    block = (RCObject **)GCHeap::GetGCHeap()
                ->GetPartition(kZCTPartition)
                ->AllocNoOOM(1, GCHeap::flags_Alloc | GCHeap::kCanFail);
#ifdef ZCT_TESTING
  if (block != NULL)
    --zct_allowance;
#endif
