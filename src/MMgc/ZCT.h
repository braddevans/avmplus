 ...
 */
    ...
    public:
         ...
         * ZCT destruction must occur at a better defined time than GC destruction
         * allows for, typically while the GC is still operational.
         */
        void Destroy();
         ...
         *
         * To be called exactly once after the GC has been properly constructed:
         * SetGC() uses 'gc' to allocate memory.
         */
        void SetGC(GC *gc);
        ...
        /**
         * Will be called by the collector when the collector is notified of an
         * imminent abort
         */
        void SignalImminentAbort();

        /**
         * Add obj to the ZCT; it must not already be in the ZCT.  This method can
         * fail silently, leaving the GC to reap the object in case its reference
         * count stays zero.
         *
        ...
        /**
         * Reap the ZCT: destroy every non-pinned object in the ZCT.  If
         * scanNativeStack is true then the program stack is traversed and objects
         * conservatively referenced from it are pinned.  Auxiliary auto-pinning
         * memory (think of them as ZCT roots; see AllocaStackSegment in GC.h) is
         * always traversed and reachable objects pinned.
         *
    ...
    private:
        ...
        // Scan the AllocaStackSegments and pin all objects directly reachable from
        // them.
        void PinRootSegments();

        // Conservatively scan memory starting at 'start' for 'len' bytes, examining
        // each aligned word.  If the value stored can be interpreted as a (possibly
        // tagged) pointer to the start of an RCObject, then pin that RCObject.  The
        // value of 'start' must itself be aligned.
        void PinStackObjects(const void *start, size_t len);

        // The object was pinned, leave it in the ZCT by moving it to the pinning
        // memory.
