...
/*static*/
    ...
    *
    ...
    FixedMalloc::GetInstance(int partition) {
        GCAssert(partition < kNumFixedPartitions);
        return instances[partition];
    }
...
/*static*/
    ...
    *
    ...
    FixedMalloc::Size(const void *item) {
        size_t size;
        if (IsLargeAlloc(item))
            size = LargeSize(item);
        else
...
#ifdef GCDEBUG
    uint32_t const size8 =
        (uint32_t)((size + 7) & ~7); // Round up to multiple of 8
...
FixedMalloc::GetNumLargeBlocks() {
    MMGC_LOCK(m_largeAllocInfoLock);
    return numLargeBlocks;
}

REALLY_INLINE void FixedMalloc::UpdateLargeAllocStats(void *item,
                                                      size_t blocksNeeded) {
    (void) item;
    MMGC_LOCK(m_largeAllocInfoLock);
...
REALLY_INLINE void FixedMalloc::UpdateLargeFreeStats(void *item,
                                                     size_t blocksFreed) {
    (void) item;
    MMGC_LOCK(m_largeAllocInfoLock);
#if defined MMGC_HOOKS && defined MMGC_MEMORY_PROFILER
