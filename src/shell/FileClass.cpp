...
avmplus::Stringp FileClass::read(avmplus::Stringp filename) {
  ...
  // Avoid avmStackAlloc - the buffer can be large and the memory is
  // non-pointer-containing, but the GC will scan it conservatively.
  uint8_t *c = (uint8_t *)core->gc->Alloc(len + 1, MMgc::kNone,
                                          MMgc::kAVMShellGCPartition);

  len = (int)fp->read(
      c,
      len); // need to force since the string creation functions expect an int
  c[len] = 0;
  ...
  if (len >= 3) {
    ...
    if ((c[0] == 0xef) && (c[1] == 0xbb) && (c[2] == 0xbf)) {
      ...
      c += 2;
      len = (len - 2) >> 1;
      ret = core->newStringEndianUTF16(/*littleEndian*/ false,
                                       (wchar *)(void *)c, len);
    } else if ((c[0] == 0xff) && (c[1] == 0xfe)) {
      // UTF-16 little endian
      c += 2;
      len = (len - 2) >> 1;
      ret = core->newStringEndianUTF16(/*littleEndian*/ true,
                                       (wchar *)(void *)c, len);
    }
  }
...
void FileClass::write(avmplus::Stringp filename, avmplus::Stringp data) {
  avmplus::Toplevel *toplevel = this->toplevel();
  ...
  if ((int32_t)fp->write(dataUTF8.c_str(), dataUTF8.length()) !=
      dataUTF8.length()) {
    toplevel->throwError(kFileWriteError, filename);
  }
  fp->close();
...
bool FileClass::writeByteArray(avmplus::Stringp filename,
