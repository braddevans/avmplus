...
#ifndef UINT32_T_MAX
#define UINT32_T_MAX                                                           \
  (uint32_t(0xFFFFFFFFUL)) // max value for a 32-bit unsigned integer
#endif
...
#elif (defined AVMPLUS_WIN32 && (defined UNDER_CE || defined UNDER_RT)) ||     \
    defined VMCFG_SYMBIAN
...
// exit codes
enum { OUT_OF_MEMORY = 128 };

// swf support, impl code in swf.cpp
bool isSwf(avmplus::ScriptBuffer);
bool handleSwf(const char *, avmplus::ScriptBuffer, avmplus::Toplevel *,
               avmplus::CodeContext *, bool test_only);

#ifdef VMCFG_AOT
// AOT support, impl code in aot.cpp
void handleAOT(avmplus::AvmCore *, avmplus::Domain *, avmplus::DomainEnv *,
               avmplus::Toplevel *, avmplus::CodeContext *);
#endif
...
public:
  ...
  int repeats;
  uint32_t stackSize;
  char st_mem[200]; // Selftest scratch memory.  200 chars ought to be enough
                    // for anyone
};
...
public:
   ...
   * @param mainthread  True if this core is being used on the main thread and
   * not on a spawned thread.  It must be one or the other; a core created on a
   * spawned thread cannot be used on the main thread or vice versa.
   */
