 ...
 * This class needs to be derived and its methods implemented by platform to
 * enable the shell to perform file I/O
 */
class File {
public:
  /**
   ...
   * The implementation should open the file with the given name and attributes
   * for open mode
   * @param filename represents the path and name of the file to be opened.
   * filename is UTF-8
   * @param flags one of the values from OpenAttribute enumeration indicating
   * the  mode of file
   ...
   * This method can return 0 in event of a read error or if the file marker was
   * at end-of-file. So a return value of zero should not be treated as an error
   * condition by default. Instead method isEOF() should be used to detect a
   * end-of-file .
   * @param buffer buffer to read the data into
   * @param bytesToRead number of bytes to read
   * @return number of bytes actually read.  0 or less than bytesToRead in case
   * of error or end-of-file was reached
   */
   ...
   * @return number of bytes actually written.  If this value is less than
   * bytesToWrite then that indicates an error
   */
  virtual size_t write(const void *buffer, size_t bytesToWrite) = 0;

  /**
   * Method to get the current position of the file head
   * @return returns the current byte position of the file head, -1 if an error
   * occurred
   */
   ...
   * @param pos number indicating the byte position in file to move the file
   * head to
   * @return returns true if the operation was successful, false if an error
   * occurred
   */
