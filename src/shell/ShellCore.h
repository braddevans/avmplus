...
namespace avmshell {
/**
 * Settings for ShellCore.  The command line parser uses this, but the initial
 * state is set in ShellCore.cpp, and it's propagated throughout various parts
 * of the system.  Partly duplicates the functionality in AvmCore::config, and
 * some settings are copied from ShellCoreSettings into that structure when a
 * new core is created.
 *
 * We try to avoid conditional compilation here, as there's only ever one of
 * these structures floating around and the savings would be minuscule.
 *
 * FIXME: the naming of many fields could be much better, and they should be
 * documented.
 */
...
public:
  ...
  MMgc::GCConfig::GCMode gcMode() {
    if (nogc)
      return MMgc::GCConfig::kDisableGC;
    else if (greedy)
      return MMgc::GCConfig::kGreedyGC;
    else if (incremental)
      return MMgc::GCConfig::kIncrementalGC;
    else
      return MMgc::GCConfig::kNonincrementalGC;
  }
};

class ShellCodeContext : public avmplus::CodeContext {
public:
  inline ShellCodeContext(avmplus::DomainEnv *env,
...
class ShellCore : public avmplus::AvmCore {
  ...
  friend class avmplus::ST_avmplus_vector_accessors::
      ST_avmplus_vector_accessors;
...
public:
   ...
   * top level.  The file may contain ABC bytecode, a SWF containing a single
   * DoABC tag, or (if the run-time compiler is present) ActionScript source
   * code.
   *
   * Requires: MMGC_ENTER and MMGC_GCENTER(gc) on the stack.
   */
  ...
  int evaluateScriptBuffer(avmplus::ScriptBuffer &buffer,
                           bool enter_debugger_on_launch);
  ...
  /*virtual*/ inline avmplus::ApiVersion getDefaultAPI() {
    return this->defaultAPIVersion;
  }

  inline avmplus::BugCompatibility::Version
  getDefaultBugCompatibilityVersion() const {
    return defaultBugCompatibilityVersion;
  }
...
protected:
  ...
  virtual avmplus::Debugger *createDebugger(int tracelevel) {
    ...
    return allowDebugger
               ? DebugCLI::create(GetGC(), this,
...
#endif
#ifdef VMCFG_EVAL
  virtual avmplus::String *
  readFileForEval(avmplus::String *referencing_filename,
...
private:
  ...
  void initShellPool();
  int handleArbitraryExecutableContent(bool do_testSWFHasAS3,
                                       avmplus::ScriptBuffer &code,
                                       const char *filename);
#ifdef VMCFG_EVAL
  avmplus::String *decodeBytesAsUTF16String(uint8_t *bytes, uint32_t nbytes,
...
public:
  REALLY_INLINE static ShellToplevel *create(MMgc::GC *gc,
                                             avmplus::AbcEnv *abcEnv) {
    return new (gc, MMgc::kExact) ShellToplevel(abcEnv);
  }

  ShellCore *core() const { return (ShellCore *)Toplevel::core(); }
