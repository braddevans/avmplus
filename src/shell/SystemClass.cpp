...
SystemClass::~SystemClass() { initialTime = 0; }

void SystemClass::exit(int status) {
  if (core()->getIsolate()->getAggregate()->isPrimordial(
          core()->getIsolate()->getDesc())) {
    Platform::GetInstance()->exit(status);
  } else {
    GCRef<avmplus::ClassClosure> workerClass = toplevel()->workerClass();
    static_cast<ShellWorkerClass *>((avmplus::ClassClosure *)workerClass)
        ->getCurrentWorker()
        ->terminate();
  }
}
...
void SystemClass::sleep(int32_t ms) {
  // Or do we try to make the all the isolate threads sleep? In a safepoint? and
  // interruptibly?
  if (ms < 0)
    ms = 0;
...
avmplus::Stringp SystemClass::getAvmplusVersion() {
  return core()->newConstantStringLatin1(AVMPLUS_VERSION_USER
                                         " " AVMPLUS_BUILD_CODE);
}
...
int32_t SystemClass::get_swfVersion() {
  ShellCore *core = (ShellCore *)this->core();
  avmplus::BugCompatibility::Version v =
      core->getDefaultBugCompatibilityVersion();
...
void SystemClass::forceFullCollection() { core()->GetGC()->Collect(); }

void SystemClass::queueCollection() { core()->GetGC()->QueueCollection(); }

bool SystemClass::isGlobal(avmplus::Atom o) {
  return avmplus::AvmCore::isObject(o)
             ? avmplus::AvmCore::atomToScriptObject(o)->isGlobalObject()
             : false;
}
...
void SystemClass::smashObjectVectorSize(avmplus::ObjectVectorObject *vec,
                                        uint32_t size) {
...
void SystemClass::smashIntVectorSize(avmplus::IntVectorObject *vec,
                                     uint32_t size) {
...
void SystemClass::smashUIntVectorSize(avmplus::UIntVectorObject *vec,
                                      uint32_t size) {
...
void SystemClass::smashDoubleVectorSize(avmplus::DoubleVectorObject *vec,
                                        uint32_t size) {
...
void SystemClass::runInSafepoint(avmplus::FunctionObject *code) {
  ...
  core()
      ...
      ->getAggregate()
      ->safepointManager()
      ->requestSafepointTask(task);
}

/*static*/ void FASTCALL
CheckBaseClass::preCreateInstanceCheck(avmplus::ClassClosure *cls) {
  ...
  cls->toplevel()->argumentErrorClass()->throwError(
      kNotImplementedError, cls->core()->toErrorString(&qname));
}
...
avmplus::Atom SystemClass::canonicalizeNumber(avmplus::Atom a) {
  if (atomKind(a) == avmplus::AtomConstants::kDoubleType) {
    ...
    if (double(intval) == val &&
        !(val == 0 && avmplus::MathUtils::isNegZero(val))) {
