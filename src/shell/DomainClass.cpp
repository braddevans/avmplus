...
void DomainObject::init(DomainObject *parentDomain) {
  ...
  DomainEnv *baseDomainEnv =
      parentDomain ? (DomainEnv *)parentDomain->domainEnv : (DomainEnv *)NULL;
  Domain *baseDomain = baseDomainEnv ? baseDomainEnv->domain() : NULL;

  domainToplevel = parentDomain ? (Toplevel *)parentDomain->domainToplevel
                                : core->createShellToplevel();

  Domain *domain = Domain::newDomain(core, baseDomain);
  domainEnv = DomainEnv::newDomainEnv(
      core, domain,
      parentDomain ? (DomainEnv *)parentDomain->domainEnv : (DomainEnv *)NULL);
}
...
Atom DomainObject::loadBytes(ByteArrayObject *b, uint32_t swfVersion) {
  ...
  PoolObject *pool =
      core->parseActionBlock(code,
                             /*start*/ 0, toplevel, domainEnv->domain(),
                             /*ninit*/ NULL, apiVersion);

  // by default, use the same bugCompatibility as the builtins use
  const BugCompatibility *bugCompatibility =
      toplevel->abcEnv()->codeContext()->bugCompatibility();
  if (swfVersion != 0) {
    // ...unless specified otherwise.
    for (int j = 0; j < BugCompatibility::VersionCount; ++j) {
      if (BugCompatibility::kNames[j] == swfVersion) {
        bugCompatibility =
            core->createBugCompatibility((BugCompatibility::Version)j);
        goto done;
      }
    }
    // if we get here, didn't find a valid name
    toplevel->throwTypeError(kInvalidArgumentError,
                             core->toErrorString("swfVersion"));
  }
done:

  ShellCodeContext *codeContext =
      new (core->GetGC()) ShellCodeContext(domainEnv, bugCompatibility);
...
ScriptObject *DomainObject::finddef(const Multiname &multiname,
  ...
  ScriptEnv *script = core()->domainMgr()->findScriptEnvInDomainEnvByMultiname(
      domainEnv, multiname);
...
ClassClosure *DomainObject::getClass(Stringp name) {
  ...
  if (name == NULL) {
    toplevel->throwArgumentError(kNullArgumentError,
  ...
  if (dot >= 0) {
    ...
    ns = core->internNamespace(core->newNamespace(
        uri, Namespace::NS_Public, core->getApiVersionFromCallStack()));
  ...
  if (!container) {
    toplevel->throwTypeError(kClassNotFoundError,
                             core->toErrorString(&multiname));
  }
  Atom atom =
      toplevel->getproperty(container->atom(), &multiname, container->vtable);
