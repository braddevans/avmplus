...
SymbianJITHeap::~SymbianJITHeap() { m_chunk.Close(); }

bool SymbianJITHeap::IsInitialized() { return m_startAddr != 0; }

TInt SymbianJITHeap::GetNumAllocs() { return m_numAllocs; }
...
void *SymbianJITHeap::Alloc() {
  ...
  TUint maxMem =
      (chunkIndex + 1) * kPageSize; // could use shift in places like this but
                                    // this is easier to read
  if (maxMem > m_currentMaxMem) {
    AvmAssert(maxMem == ((m_numAllocs + 1) *
                         kPageSize)); // There should not be any empty slots
    maxMem += kHeapAdjustSize; // call Adjust for more than one chunk a time to
                               // improve performance
    if (m_chunk.Adjust(maxMem) != KErrNone) {
      // FIXME https://bugzilla.mozilla.org/show_bug.cgi?id=571407
      // It is really the task of VMPI_allocateCodeMemory to shut down the
      // Player in the event of a failure (not this function). shut down Flash
      // Player after this!
      return 0;
    }
...
void SymbianJITHeap::Free(void *ptr) {
  ...
  int chunkIndex = (((TUint)ptr) - m_startAddr) /
                   kPageSize; // // could use shift in places like this but this
                              // is easier to read
...
int SymbianJITHeap::GetEmptyBit(int arrayPos) {
  ...
  for (int i = 0; i < 4;
       i++) // max 4 iterations instead of 32 because of using a lookup
  {
    TUint8 b = v & 0xff;
    if (b != 0xff) {
...
int SymbianJITHeap::GetEmptyChunkIndex() {
  ...
  while (i < kMaxMap) {
    if (m_map[i] != 0xffffffff) // this loop can compare 128k a time because
                                // each bit = 4k chunk
      break;
    i++;
  }
