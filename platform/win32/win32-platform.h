...
#pragma warning(disable : 4946) // reinterpret_cast used between related classes
                                // (bugzilla 615964)
#pragma warning(disable : 4127) // conditional expression is constant - appears
                                // to be compiler noise primarily
#pragma warning(                                                               \
    disable : 4201) // nonstandard extension used : nameless struct/union
#pragma warning(disable : 4251) // X needs to have dll-interface to be used by
                                // clients of class Y
#pragma warning(disable : 4310) // cast truncates constant value
#pragma warning(disable : 4371) // better packing than in the previous version
                                // of the compiler
...
#pragma warning(                                                               \
    disable : 4725) // instruction may be inaccurate on some Pentiums
#pragma warning(                                                               \
    disable : 4324) // padding was added at the end of a structure because you
                    // specified a __declspec(align) value.
#pragma warning(disable : 4800) // forcing value to bool 'true' or 'false' -
                                // bogus performance warning

// enable some that are off even in /W4 mode, but are still handy
#pragma warning(default : 4242) // 'identifier' : conversion from 'type1' to
                                // 'type2', possible loss of data
#pragma warning(                                                               \
    default : 4263) // 'function' : member function does not override any base
                    // class virtual member function
#pragma warning(                                                               \
    default : 4264) // 'virtual_function' : no override available for virtual
                    // member function from base 'class'; function is hidden
#pragma warning(default : 4265) // 'class' : class has virtual functions, but
                                // destructor is not virtual
#pragma warning(                                                               \
    default : 4266) // 'function' : no override available for virtual member
                    // function from base 'type'; function is hidden
#pragma warning(                                                               \
    default : 4296) // expression is always true (false) (Generally, an unsigned
                    // variable was used in a comparison operation with zero.)
#pragma warning(default : 4905) // wide string literal cast to 'LPSTR'
#pragma warning(default : 4906) // string literal cast to 'LPWSTR'

// some that might be useful to turn on someday, but would require too much
// twiddly code tweaking right now
//  #pragma warning(error:4820) // 'bytes' bytes padding added after construct
//  'member_name' (MSFT system headers generate zillions of these, sadly)
...
// There is nothing wrong with using these in preference to preprocessor
// conditionalization.
#pragma warning(disable : 6237) // (<zero> && <expression>) is always zero
#pragma warning(disable : 6239) // (<non-zero constant> && <expression>) always
                                // evaluates to the result of <expression>

// This needs to be fixed, but reflects our generally cavalier treatment of
// stack overflow
#pragma warning(disable : 6255) // _alloca indicates failure by raising a stack
                                // overflow exception
#endif
#endif // _MSC_VER
...
#define VMPI_strncpy(d, nd, s, ns)                                             \
  ::strncpy_s(d, nd, s, ns) // change to secure API
...
#if defined(UNDER_CE)
// winmo complains if we try to include <new> and it complains if someone else
// includes new before us so...
...
#if _MSC_VER > 1500
#include <stdint.h>
#elif !defined(STDINT_TYPES_DEFINED)
...
REALLY_INLINE bool VMPI_lockAcquire(vmpi_spin_lock_t *lock) {
  ...
  while (::InterlockedCompareExchange((LPLONG)&lock->lock, 1, 0) != 0) {
    ++tries;
    // We used to reset to zero if we got to 100. This resets to 0 at 64
    // instead, with no branch.
    tries &= 63;
    // if tries == 0, we just rolled over 64, so we Sleep(1) to give other
    // threads a chance to run... otherwise we Sleep(0)
#if AVMSYSTEM_WINDOWSSTOREAPP
    if (tries == 0) {
...
#ifdef UNDER_CE
  return InterlockedIncrement(const_cast<LONG *>(
      (volatile LONG *)value)); // Barrier semantics are not actually documented
#else
...
#ifdef UNDER_CE
  return InterlockedDecrement(const_cast<LONG *>(
      (volatile LONG *)value)); // Barrier semantics are not actually documented
#else
...
REALLY_INLINE bool VMPI_compareAndSwap32WithBarrier(int32_t oldValue,
  ...
  return InterlockedCompareExchange(
             const_cast<LONG *>((volatile LONG *)address), newValue,
             oldValue) ==
         oldValue; // Barrier semantics are not actually documented
#else
  return InterlockedCompareExchange((volatile LONG *)address, newValue,
                                    oldValue) == oldValue;
#endif
}
...
#endif // AVMSYSTEM_WINDOWSSTOREAPP
  // dwNumberOfProcessors can be unreliable, but we know we have at least one
  // processor
  return systemInfo.dwNumberOfProcessors < 1 ? 1
