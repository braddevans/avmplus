...
namespace avmplus {
void CALLBACK intWriteTimerProc(UINT, UINT, DWORD_PTR dwUser, DWORD_PTR,
                                DWORD_PTR) {
...
uintptr_t OSDep::startIntWriteTimer(uint32_t millis, volatile int *addr) {
  return (uintptr_t)timeSetEvent(
      millis, millis, (LPTIMECALLBACK)intWriteTimerProc, (DWORD_PTR)addr,
...
void OSDep::stopTimer(uintptr_t handle) { timeKillEvent((UINT)handle); }
} // namespace avmplus
