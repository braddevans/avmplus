...
namespace avmplus {
TraceClass::TraceClass(VTable *cvtable) : ClassClosure(cvtable) {
  ...
  if (core->debugger()) {
    ...
    if (core->isNullOrUndefined(f->atom())) {
      ...
      toplevel()->argumentErrorClass()->throwError(
          kInvalidArgumentError, core->toErrorString("Function"));
      return undefinedAtom;
    }

    // MethodClosure* mc =
    // f->toplevel()->methodClosureClass->create(f->getCallMethodEnv(),
    // f->atom());
...
public:
  SampleIterator(ScriptObject *script, VTable *vt)
      : ScriptObject(vt, NULL), script(script) {
...
Atom SamplerScript::_getSamples(ScriptObject *self, ClassClosure *cf) {
  ...
  if (s->sampleIteratorVTable == NULL)
    s->sampleIteratorVTable =
        _newVT(self->toplevel(), self->traits()->pool, sizeof(SampleIterator));
  SampleIterator *iter =
      new (self->gc()) SampleIterator(self, s->sampleIteratorVTable);
...
#ifdef DEBUGGER
ClassClosure *SamplerScript::getType(Toplevel *ss_toplevel,
                                     SamplerObjectType sot, const void *ptr) {
  Toplevel *tl;
  ...
  if (obj && AvmCore::istype(obj->atom(), core->traits.class_itraits)) {
    type = tl->classClass();
             ...
             AvmCore::istype(obj->atom(), core->traits.function_itraits)) {
...
#ifdef DEBUGGER
bool SamplerScript::set_stack(ScriptObject *self, ClassFactoryClass *cf,
                              const Sample &sample, SampleObject *sam) {
  if (sample.stack.depth > 0) {
    ...
    for (uint32_t i = 0; i < sample.stack.depth; i++, e++) {
      ...
      // could be deleted the StackTrace::Element pointer is a raw pointer into
      // that buffer so we need to check that its still around before
      // dereferencing e
      uint32_t num;
      if (s->getSamples(num) == NULL)
        return false;

      sf->setconst_name(e->name()); // NOT e->info()->name() because e->name()
                                    // can be a fake name
...
ScriptObject *SamplerScript::makeSample(ScriptObject *self,
                                        ClassFactoryClass *cf,
                                        const Sample &sample) {
  Toplevel *toplevel = self->toplevel();
  AvmCore *core = toplevel->core();
  Sampler *s = core->get_sampler();
  ...
  case Sampler::DELETED_OBJECT_SAMPLE: {
    DeleteObjectSampleClass *cls =
        (DeleteObjectSampleClass *)cf->get_DeleteObjectSampleClass();
  ...
  case Sampler::NEW_OBJECT_SAMPLE: {
    NewObjectSampleClass *cls =
        (NewObjectSampleClass *)cf->get_NewObjectSampleClass();
...
Atom SamplerScript::getMemberNames(ScriptObject *self, Atom o,
                                   bool instanceNames) {
#ifdef DEBUGGER
  AvmCore *core = self->core();
  ...
  if (AvmCore::isObject(o)) {
    ...
    if (s->slotIteratorVTable == NULL)
      s->slotIteratorVTable =
          _newVT(self->toplevel(), self->traits()->pool, sizeof(SlotIterator));
...
void SamplerScript::_setSamplerCallback(ScriptObject *self,
...
double SamplerScript::_getInvocationCount(ScriptObject *self, Atom a,
                                          QNameObject *qname, uint32_t type) {
#ifdef DEBUGGER
  AvmCore *core = self->core();
  Sampler *s = core->get_sampler();
  if (!s || !trusted(self))
  ...
  if (!AvmCore::isObject(a)) {
    ...
    ScriptEnv *script =
        (ScriptEnv *)core->domainMgr()->findScriptEnvInDomainEnvByMultiname(
            domainEnv, multiname);
    if (script != (ScriptEnv *)BIND_NONE) {
      if (script == (ScriptEnv *)BIND_AMBIGUOUS)
        self->toplevel()->throwReferenceError(kAmbiguousBindingError,
                                              &multiname);
    ...
    if (AvmCore::istype(a, CLASS_TYPE) && !qname) {
      ...
      if (cc->vtable->init) // Vector related crash here, Tommy says: I didn't
                            // think a type could ever not have a constructor
                            // but I guess there's no reason it has to.
...
again:
  ...
  case BKIND_VAR:
  case BKIND_CONST: {
    // only look at slots for first pass, otherwise we're applying instance
    // traits to the Class
    if (v == object->vtable) {
...
bool SamplerScript::isGetterSetter(ScriptObject *self, Atom a,
                                   QNameObject *qname) {
#ifdef DEBUGGER
  AvmCore *core = self->core();
  Sampler *s = core->get_sampler();
...
NewObjectSampleObject::NewObjectSampleObject(VTable *vtable,
...
/*static*/
ArrayObject *SamplerScript::getLexicalScopes(ScriptObject *self,
