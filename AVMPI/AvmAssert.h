...
// AVMPLUS_CUSTOM_ASSERTION_HANDLER not yet (now == Apr-2009) exposed as a
// feature; awaiting cleanup of the debug code (merging MMgc and Avm debug
// macros, etc)
...
#define AvmAssertMsgCanThrow(condition, message, core)                         \
  do {                                                                         \
    ...
    CATCH(Exception *e) {                                                      \
      avmplus::_AvmAssertMsg(0, "Exception caught in assert!\n");              \
      (void)e;                                                                 \
    }                                                                          \
    END_CATCH                                                                  \
    END_TRY                                                                    \
