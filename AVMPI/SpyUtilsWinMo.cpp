...
DWORD WINAPI WaitForMemorySignal(LPVOID) {
  ...
  while (spyRunning) {
    ...
    // For some reason ReadMsgQueue does not clear the signal on the handle
    // (even though it should), and we end up setting the profilerAddr
    // constantly, which makes the VM hang, since all it is doing is writing out
    // profile data. so we have to call CloseMsgQueue, and then open a new
    // queue, and wait on that.
    // char buff[256];
    // DWORD bytesread;
...
void SpyLog(const char *message) { fprintf(spyStream, "%s", message); }
...
bool VMPI_hasSymbols() { return false; }
