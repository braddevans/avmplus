...
/**
 * Method to find whether the platform supports merging of contiguous memory
 * regions from heap
...
/**
 * @return true if AVMPI_commitMemory can operate on a memory range that
 * contains a mixture of decommitted and already-committed memory, /and/ if
 * AVMPI_decommitMemory similarly can operate on a memory range that contains a
 * mixture of committed and already-decommitted memory.
 *
...
/**
 * This method is used to reserve region(s) of memory, i.e. one or more memory
 * pages, in the system's virtual address space This method is recommended to be
 * supported on platforms with virtual memory
 * @param address optional argument, indicating the base address of the region
 * to be reserved. The system should treat this address as a hint and try to
 * reserve a region as close to this address as possible If NULL, the system can
 * determine the address of the region.
...
/**
 * This method is used to commit a pages(s) of memory in a previously reserved
 * region
...
/**
 * This method is used to de-commit a pages(s) of memory that were previously
 * commited
 ...
 * the VM page size (so long as the platform does not have constraints on how
 * many pages can be allocated at a time).
 *
 * On some platforms code for JIT memory is separate from other memory; on
 * others, it can be mixed into the heap memory.  On the latter platforms it is
 * explicitly allowed for AVMPI_allocCodeMemory to call GCHeap::Alloc to obtain
 * memory.
 *
 ...
 * @param size  The argument that was passed to AVMPI_allocCodeMemory when
 ...
 *
 * @param address  Pointer to start of an acceptable protection boundary
 * (typically a VM page) within the block.
 * @param size  Size in bytes of the area to be protected in some acceptable
 * granule (typically one or more VM pages).
 * @param  makeItSo  If true then memory is made executable, otherwise it is
 * made not-executable.
 *
 * @exceptions  May abort the process if its constraints are violated.
 *
 * @note If AVMPI_allocateCodeMemory uses GCHeap for its storage then it is
 * possible that the memory block pointed to by address could be acquired from
 * more than one call to AVMPI_reserveMemoryRegion.  The implementation of this
 * method is expected to handle such cases and identify the region boundaries if
 * the underlying system requires setting the protection flags on blocks
 * individually if they were allocated separately.
 */
extern void AVMPI_makeCodeMemoryExecutable(void *address, size_t size,
                                           bool makeItSo);

/**
 * This method is used to allocate a block of memory with the base address
 * aligned to the system page size This method should be implemented in systems
 * that do not have virtual memory in lieu of APIs to reserve and commit memory
 * regions
 * @param size size, in bytes, of the block of memory to allocate
 * @return pointer to start of the memory block if allocation was successful,
 * NULL otherwise
...
/**
 * This method is used to release a block of memory via
 * AVMPI_AllocateAlignedMemory
...
/**
 * This method is used to determind should MMgc zero initalize newly allocated
 * memory, either allocated with AVMPI_commitMemory or
 * AVMPI_allocateAlignedMemory.
 ...
 */
extern bool AVMPI_captureStackTrace(uintptr_t *buffer, size_t bufferSize,
                                    uint32_t framesToSkip);

/**
 * Method to retrieve the name of the method/function given a specific address
 * in code space Used by the MMgc memory profiler to get and display function
 * names This method is expected to write a null terminated string representing
 * the function name in to the buffer
 ...
 */
extern bool AVMPI_getFunctionNameFromPC(uintptr_t pc, char *buffer,
                                        size_t bufferSize);

/**
 * Method to retrieve the source filename and line number given a specific
 * address in a code space Used by the MMgc memory profiler to display location
 * info of source code This method is expected to write a null terminated string
 * representing the file name in to the buffer
 ...
 * @return true if the file and line number info was retrieved successfully,
 * false otherwise
 */
extern bool AVMPI_getFileAndLineInfoFromPC(uintptr_t pc, char *buffer,
                                           size_t bufferSize,
                                           uint32_t *lineNumber);

/**
 * can two consecutive calls to AVMPI_reserveMemoryRegion be freed with one
 * VMP_releaseMemoryRegion call?
 ...
 * If the platform intends to periodically retrieve information from
 * MMgc/avmplus then it can perform the necessary setup during this function
 ...
 * @param size  Size in bytes of the area to be protected in some acceptable
 * granule (typically one or more VM pages).
 * @param  offset  offset of call address in the region to allow being
 * executable
 */
