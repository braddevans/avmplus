...
const char *spy_socket_channel =
    "/tmp/MMgc_Spy"; // name of unix domain for spy connections

bool spy_connected = false;
bool spy_running = false;

pthread_cond_t spy_cond; // condition variable for synchronizing spy signalling
                         // and socket communication
pthread_mutex_t
    spy_mutex; // mutex for spy_signal and spy_cond condition variable
...
void *SpyConnectionLoop(void *) {
  ...
  while (spy_running) {
    ...
    if ((clientSocket =
             accept(serverSocket, (struct sockaddr *)&sockAddr, &len)) >= 0) {
      fd_set fds;
      FD_ZERO(&fds);
...
bool SetupSpyServer() {
  ...
  if ((bind(serverSocket, (struct sockaddr *)&sockAddr, len) < 0) ||
...
bool AVMPI_hasSymbols() { return true; }
