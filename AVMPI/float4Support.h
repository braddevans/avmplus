...
/* These may be hardware-accelerated and slighly imprecise (e.g. in handling of
   denormal numbers) The implementation stays in .cpp so it can be done in the
   same way that the CodegenLIR.cpp does it */
...
#define thunkEnterVECR_adapter(thunk, env, argc, argv)                         \
  ((VecrMethodProc)thunk)(env, argc, argv)
...
REALLY_INLINE float f4_y(float4_t v) {
  return _mm_cvtss_f32(_mm_shuffle_ps(v, v, 0x55));
}
REALLY_INLINE float f4_z(float4_t v) {
  return _mm_cvtss_f32(_mm_shuffle_ps(v, v, 0xAA));
}
REALLY_INLINE float f4_w(float4_t v) {
  return _mm_cvtss_f32(_mm_shuffle_ps(v, v, 0xFF));
}

#elif defined VMCFG_NEON // FIXME: nobody defines VMCFG_NEON currently... this
                         // should change.
#include <arm_neon.h>
...
REALLY_INLINE int32_t f4_eq_i(float4_t a, float4_t b) {
  ...
  return vget_lane_u32(res, 0) == 0xffffffff &&
         vget_lane_u32(res, 1) == 0xffffffff;
}
...
REALLY_INLINE float4_t f4_div(float4_t a, float4_t b) {
  float4_t result = {f4_x(a) / f4_x(b), f4_y(a) / f4_y(b), f4_z(a) / f4_z(b),
                     f4_w(a) / f4_w(b)};
  return result;
}
#else
