...
public:
  ...
  SwfParser(avmplus::ScriptBuffer swf)
      : bitPos(0), bitBuf(0), swf(swf), pos(0) {}
...
static void handleDoABC(int type, SwfParser &parser, int taglen,
  ...
  if (type == stagDoABC2) {
    ...
    // contain the following bits set: kDoAbcLazyInitializeFlag = 1: Indicates
    // that the ABC block should not be executed immediately, but only parsed. A
    // later finddef may cause its scripts to execute.
  ...
  // allocate a new buffer and copy abc into it; the abc buffer will be
  // referenced by PoolObject and can outlive the swf it came from.  Using a
  // ReadOnlyScriptBuffer avoids copying, but interior pointers to the swf data
  // do not pin the swf in memory.
  ...
  if (flags & kDoAbcLazyInitializeFlag) {
    avmplus::PoolObject *pool = core->parseActionBlock(
        code, 0, toplevel, codeContext->domainEnv()->domain(), NULL,
        apiVersion);
    deferred.add(pool);
    // defer: handleActionPool(pool/* result of parse */, domainEnv, toplevel,
    // codeContext);
  } else {
...
bool handleSwf(const char *filename, avmplus::ScriptBuffer swf,
  ...
  avmplus::AvmCore *core = toplevel->core();
  avmplus::GCList<avmplus::PoolObject> deferred(core->gc,
                                                avmplus::kListInitialCapacity);
  if (swf[0] == 'C') {
    // decompress the swf
    swflen -= 8;
    avmplus::ScriptBuffer newswf(core->newScriptBuffer(swflen));
    uLongf dlen = swflen;
    int e = uncompress((Bytef *)&newswf[0], &dlen, (Bytef *)&swf[8],
                       (uLongf)swf.getSize() - 8);
    if (e != Z_OK) {
      if (!test_only)
  ...
  while (parser.pos < swflen) {
    ...
    if (parser.pos <= oldpos) {
      has_abc = false; // broken file or broken parser, but either way we can't
                       // process it
      break;
    }
    oldpos = parser.pos;
