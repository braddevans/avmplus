...
ShellSettings::ShellSettings()
    : ShellCoreSettings(), programFilename(NULL), filenames(NULL), numfiles(-1),
      do_selftest(false), do_repl(false), do_log(false), do_projector(false),
      numthreads(1), numworkers(1), repeats(1), stackSize(0) {}

ShellCoreImpl::ShellCoreImpl(MMgc::GC *gc, ShellSettings &settings,
                             bool mainthread)
    : ShellCore(gc, settings.apiVersionSeries), settings(settings),
...
void ShellCoreImpl::setStackLimit() {
  ...
  if (settings.stackSize != 0) {
    ...
    // stack address is.
    minstack =
        uintptr_t(&minstack) - settings.stackSize + avmshell::kStackMargin;
  } else {
...
int Shell::run(int argc, char *argv[]) {
  ...
  // Note that output from the command line parser (usage messages, error
  // messages, and printed version number / feature list) will not go to the log
  // file.  We could fix this if it's a hardship.

  {
    ...
    if (instance->settings.do_log)
      initializeLogging(instance->settings.numfiles > 0
                            ? instance->settings.filenames[0]
                            : "AVMLOG");

#ifdef VMCFG_WORKERTHREADS
    if (instance->settings.numworkers == 1 &&
        instance->settings.numthreads == 1 && instance->settings.repeats == 1) {
...
ShellIsolate::ShellIsolate(int32_t desc, int32_t parentDesc,
                           avmplus::Aggregate *aggregate)
...
void ShellIsolate::copyByteCode(avmplus::ByteArrayObject *ba) {
  if (ba != NULL) {
    ...
    for (int i = 0; i < numfiles; i++) {
      ...
      const bool isValid =
          f.valid() && ((uint64_t)f.length() <
                        UINT32_T_MAX); // currently we cannot read files > 4GB
      if (isValid) {
        // parse new bytecode
        // avmplus::ScriptBuffer code =
        // targetCore()->newScriptBuffer((size_t)f.available()); (f.read(buf),
        //(size_t)f.available();
...
ShellIsolate::newWorkerObject(avmplus::Toplevel *toplevel) {
  GCRef<ShellWorkerObject> workerObject = toplevel->workerClass()
...
void ShellIsolate::evalCodeBlobs(bool enter_debugger_on_launch) {
  ...
  for (int i = 0; i < m_code.length; i++) {
    ...
    // parse new bytecode
    avmplus::ReadOnlyScriptBufferImpl *codeimpl = new (core->gc)
        avmplus::ReadOnlyScriptBufferImpl(m_code.values[i].values,
                                          m_code.values[i].length);
...
public:
  PrimordialShellIsolate(int32_t desc, int32_t parentDesc,
                         avmplus::Aggregate *aggregate)
...
struct CoreNode {
  CoreNode(ShellCore *core, int id) : core(core), id(id), next(NULL) {}
...
struct ThreadNode {
  ThreadNode(MultiworkerState &state, int id)
      : state(state), id(id), pendingWork(false), corenode(NULL),
        filename(NULL), next(NULL) {}
  ...
  const int id;
  bool pendingWork;
  CoreNode *corenode;   // The core running (or about to run, or just finished
...
struct MultiworkerState {
  MultiworkerState(ShellSettings &settings)
      : settings(settings), numthreads(settings.numthreads),
        numcores(settings.numworkers), free_threads(NULL),
        free_threads_last(NULL), free_cores(NULL), free_cores_last(NULL),
        num_free_threads(0) {}
...
void Shell::multiWorker(ShellSettings &settings) {
  AvmAssert(!settings.do_repl && !settings.do_projector &&
            !settings.do_selftest);
      ...
      for (;;) {
        ...
        while (state.getThreadAndCore(&threadnode, &corenode) && !finish) {
          LOGGING(avmplus::AvmLog("Scheduling %s on T%d with C%d\n",
                                  filenames[nextfile], threadnode->id,
                                  corenode->id);)
...
void SlaveThread::run() {
  ...
  for (;;) {
    // Signal that we're ready for more work: add self to the list of free
    // threads

    state.freeThread(self);

    // Obtain more work.  We have to hold self->thread_monitor here but the
    // master won't touch corenode and filename until we register for more work,
    // so they don't have to be copied out of the thread structure.

    SCOPE_LOCK_NAMED(locker, self->thread_monitor) {
      // Don't wait when pendingWork == true,
      // slave might have been already signalled but it didn't notice because it
      // wasn't waiting yet.
...
#endif
      self->corenode->core->evaluateFile(
          state.settings, self->filename); // Ignore the exit code for now
    }
    LOGGING(avmplus::AvmLog("T%d: Work completed\n", self->id);)

    SCOPE_LOCK(self->thread_monitor) { self->pendingWork = false; }
  }
  return;
}
...
void Shell::repl(ShellCore *shellCore) {
  ...
  for (;;) {
    ...
    if (Platform::GetInstance()->getUserInput(commandLine, kMaxCommandLine) ==
        NULL)
      return;
    ...
    if (VMPI_strncmp(commandLine, ".load", 5) == 0) {
      ...
      // Small amount of generalization of the code currently in the main loop
    ...
    if (VMPI_strncmp(commandLine, ".input", 6) == 0) {
      ...
      for (;;) {
        if (Platform::GetInstance()->getUserInput(commandLine,
                                                  kMaxCommandLine) == NULL)
          return;
        commandLine[kMaxCommandLine - 1] = 0;
...
void Shell::initializeLogging(const char *basename) {
  ...
  size_t logFilenameLen =
      (lastDot == NULL) ? VMPI_strlen(basename) : (lastDot - basename);

  char *logFilename =
      new char[logFilenameLen + 5]; // 5 bytes for ".log" + null char
...
void Shell::parseCommandLine(int argc, char *argv[]) {
    ...
    if (arg[0] == '-') {
      ...
      if (arg[1] == 'D') {
        if (!VMPI_strcmp(arg + 2, "timeout")) {
          ...
          // allow this option even in non-DEBUGGER builds to make test scripts
          // simpler
          ...
          if (arg[10] == '=') {
            VMPI_strncpy(settings.st_mem, sizeof(settings.st_mem), arg + 11,
                         sizeof(settings.st_mem) - 1);
          ...
          settings.verboseOnlyArg =
              argv[++i]; // capture the string process it later
        } else if (!VMPI_strncmp(arg + 2, "verbose", 7)) {
          settings.do_verbose =
              avmplus::AvmCore::DEFAULT_VERBOSE_ON; // all 'on' by default
          if (arg[9] == '=') {
            char *badFlag;
            settings.do_verbose =
                avmplus::AvmCore::parseVerboseFlags(&arg[10], badFlag);
            if (badFlag) {
        ...
        int32_t threshold;
        if (VMPI_sscanf(arg + 5, "%d", &threshold) != 1 || threshold < 0) {
        ...
        int32_t level;
        if (VMPI_sscanf(arg + 6, "%d", &level) != 1 || level < 0 || level > 2) {
...
#endif /* VMCFG_NANOJIT */
      else if (MMgc::GCHeap::GetGCHeap()->Config().IsGCOptionWithParam(arg) &&
               i + 1 < argc) {
        const char *val = argv[++i];
        if (MMgc::GCHeap::GetGCHeap()->Config().ParseAndApplyOption(
                arg, mmgcSaysArgIsWrong, val)) {
          ...
          AvmAssertMsg(
              false,
              "IsGCOptionWithParam inconsistent with ParseAndApplyOption.");
        }
      } else if (MMgc::GCHeap::GetGCHeap()->Config().ParseAndApplyOption(
                     arg, mmgcSaysArgIsWrong)) {
        if (mmgcSaysArgIsWrong) {
        ...
        int nchar;
        const char *val = argv[++i];
        if (VMPI_sscanf(val, "%u%n", &stack, &nchar) == 1 &&
            size_t(nchar) == VMPI_strlen(val) &&
            stack > avmshell::kStackMargin) {
        ...
        int nchar;
        const char *val = argv[++i];
        if (VMPI_sscanf(val, "%d%n", &stack, &nchar) == 1 &&
            size_t(nchar) == VMPI_strlen(val) && stack >= 0) {
        ...
        if (val == NULL)
          val = "";
        if (VMPI_sscanf(val, "%d,%d,%d%n", &settings.numworkers,
                        &settings.numthreads, &settings.repeats, &nchar) != 3)
          if (VMPI_sscanf(val, "%d,%d%n", &settings.numworkers,
                          &settings.numthreads, &nchar) != 2) {
            avmplus::AvmLog("Bad value to -workers: %s\n", val);
            usage();
          }
        if (settings.numthreads < 1 ||
            settings.numworkers < settings.numthreads || settings.repeats < 1 ||
            size_t(nchar) != VMPI_strlen(val)) {
          avmplus::AvmLog("Bad value to -workers: %s\n", val);
          usage();
        }
        ...
        if (!avmplus::AvmCore::parseApiVersion(argv[i + 1], settings.apiVersion,
                                               settings.apiVersionSeries)) {
        ...
        int nchar;
        const char *val = argv[++i];
        if (VMPI_sscanf(val, "%u%n", &swfVersion, &nchar) == 1 &&
            size_t(nchar) == VMPI_strlen(val)) {
...
#ifdef AVMSHELL_PROJECTOR_SUPPORT
  if (settings.programFilename != NULL &&
      ShellCore::isValidProjectorFile(settings.programFilename)) {
    if (settings.do_selftest || settings.do_repl || settings.numfiles > 0) {
      avmplus::AvmLog("Projector files can't be used with -repl, -Dselftest, "
                      "or program file arguments.\n");
      usage();
    }
    if (settings.numthreads > 1 || settings.numworkers > 1) {
      avmplus::AvmLog(
          "A projector requires exactly one worker on one thread.\n");
      usage();
    }
  ...
  if (settings.numfiles == 0) {
    ...
    if (!settings.do_selftest && !settings.do_repl) {
      avmplus::AvmLog("You must provide input files, -repl, or -Dselftest, or "
                      "the executable must be a projector file.\n");
      usage();
    }
  }
#endif
...
#ifdef VMCFG_SELFTEST
  if (settings.do_selftest) {
    // Presumably we'd want to use the selftest harness to test multiple workers
    // eventually.
    if (settings.numthreads > 1 || settings.numworkers > 1 ||
        settings.numfiles > 0) {
      avmplus::AvmLog("The selftest harness requires exactly one worker on one "
                      "thread, and no input files.\n");
      usage();
    }
  }
#endif
...
/*static*/
void Shell::usage() {
  avmplus::AvmLog("avmplus shell " AVMPLUS_VERSION_USER " " AVMPLUS_BIN_TYPE
  ...
  avmplus::AvmLog("          [-gcbehavior] summarize GC behavior and policy, "
                  "after every gc\n");
  avmplus::AvmLog("          [-gcsummary]  summarize GC behavior and policy, "
                  "at end only\n");
#endif
  avmplus::AvmLog(
      "          [-load L,B, ...\n"
      "                        GC load factor L up to a post-GC heap size of B "
      "megabytes.\n"
      "                        Up to %d pairs can be accommodated, the limit "
      "for the last pair\n"
  ...
  avmplus::AvmLog("          [-gcwork G]   Max fraction of time (default 0.25) "
                  "we're willing to spend in GC\n");
  avmplus::AvmLog("          [-stack N]    Stack size in bytes (will be "
                  "honored approximately).\n"
                  "                        Be aware of the stack margin: %u\n",
                  avmshell::kStackMargin);
#ifdef MMGC_MARKSTACK_ALLOWANCE
  avmplus::AvmLog("          [-gcstack N]  Mark stack size allowance (# of "
                  "segments), for testing.\n");
#endif
  avmplus::AvmLog("          [-cache_bindings N]   size of bindings cache (0 = "
                  "unlimited)\n");
  avmplus::AvmLog("          [-cache_metadata N]   size of metadata cache (0 = "
                  "unlimited)\n");
  avmplus::AvmLog(
  ...
  avmplus::AvmLog("          [-Ddrcvalidation]     Perform a mark before each "
                  "Reap to seek out reachable zero count items\n");
#ifdef VMCFG_SELECTABLE_EXACT_TRACING
  avmplus::AvmLog("          [-Dnoexactgc] disable exact tracing\n");
#endif
  avmplus::AvmLog("          [-Dnoincgc]   don't use incremental collection\n");
  avmplus::AvmLog("          [-Dnodebugger] do not initialize the debugger (in "
                  "DEBUGGER builds)\n");
  avmplus::AvmLog("          [-Dgcthreshold N] lower bound on allocation "
                  "budget, in blocks, between collection completions\n");
  avmplus::AvmLog("          [-Dnofixedcheck]  don't check FixedMalloc "
                  "deallocations for correctness (sometimes expensive)\n");
#ifdef DEBUGGER
  avmplus::AvmLog("          [-Dastrace N] display AS execution information, "
                  "where N is [1..4]\n");
  avmplus::AvmLog(
...
#ifdef AVMPLUS_VERBOSE
  avmplus::AvmLog("          "
                  "[-Dverbose[=[parse,verify,interp,traits,builtins,memstats,"
                  "sweep,occupancy,execpolicy"
#ifdef VMCFG_NANOJIT
                  ",jit,opt,regs,raw,bytes,lir,lircfg[-bb|-ins]"
#endif
                  "]]\n");
  avmplus::AvmLog("                        With no options, enables extreme! "
                  "output mode.  Otherwise the\n");
  avmplus::AvmLog("                        options are mostly self-descriptive "
                  "except for the following: \n");
  avmplus::AvmLog("                           builtins - includes output from "
                  "builtin methods\n");
  avmplus::AvmLog("                           memstats - generate statistics "
                  "on memory usage \n");
  avmplus::AvmLog("                           sweep - [memstats] include "
                  "detailed sweep information \n");
  avmplus::AvmLog("                           occupancy - [memstats] include "
                  "occupancy bit graph \n");
  avmplus::AvmLog(
      "                           execpolicy - shows which execution method "
      "(interpretation, compilation) was chosen and why \n");
#ifdef VMCFG_NANOJIT
  avmplus::AvmLog("                           jit - output LIR as it is "
                  "generated, and final assembly code\n");
  avmplus::AvmLog("                           lir - [jit] only write out the "
                  "LIR instructions\n");
  avmplus::AvmLog("                           opt - [jit] show details about "
                  "each optimization pass\n");
  avmplus::AvmLog("                           regs - [jit] show register "
                  "allocation state after each assembly instruction\n");
  avmplus::AvmLog("                           raw - [jit] assembly code is "
                  "displayed in raw (i.e unbuffered bottom-up) fashion. \n");
  avmplus::AvmLog("                           bytes - [jit] display the byte "
                  "values of the assembly code. \n");
  avmplus::AvmLog("                           lircfg - [jit] write a gml "
                  "representation of the LIR control-flow graph to a file.\n");
  avmplus::AvmLog("                           lircfg-bb - [jit] same as above, "
                  "but each vertex is a basic blocks rather than an EBBs.\n");
  avmplus::AvmLog("                           lircfg-ins - [jit] same as "
                  "above, but each vertex is an instruction.\n");
#endif

  avmplus::AvmLog("                        Note that ordering matters for "
                  "options with dependencies.  Dependencies \n");
  avmplus::AvmLog("                        are contained in [ ] For example, "
                  "'sweep' requires 'memstats' \n");
  avmplus::AvmLog("          [-Dverbose-only [{id|name|%%regex%%}]+ ]\n");
  avmplus::AvmLog("                        enable verbose output only for the "
                  "methods identified. Valid only for verify,interp and jit\n");
  avmplus::AvmLog("                        options.  The method identification "
                  "follows the same syntax as -policy (see below)\n");
  avmplus::AvmLog("                        e.g. -Dverbose-only "
                  "\"Function/prime_val.as\\$1:prime,%%global%%\" \n");
#endif
#ifdef VMCFG_NANOJIT
  avmplus::AvmLog("          [-Dinterp]    do not generate machine code, "
                  "interpret instead\n");
  avmplus::AvmLog("          [-Ojit]       use jit always, never interp "
                  "(except when the jit fails)\n");
  avmplus::AvmLog("          [-Dcheckjitpageflags] check page protection flags "
                  "on JIT memory allocation (sometimes expensive)\n");
#ifdef VMCFG_HALFMOON
  avmplus::AvmLog("          [-Dhalfmoon   use experimental 'halfmoon' jit "
                  "(development only)\n");
#endif
  avmplus::AvmLog(
      "          [-Djitordie]  use jit always, and abort when the jit fails\n");
  avmplus::AvmLog("          [-Dnocse]     disable CSE optimization\n");
  avmplus::AvmLog("          [-Dnoinline]  disable speculative inlining for "
                  "arithmetic and conversions\n");
  avmplus::AvmLog(
      "          [-Dnoinlinevector]  disable inlining of vector get/set\n");
  avmplus::AvmLog("          [-Darrayfastpath]  enable speculative inlining of "
                  "simple array reads\n");
  avmplus::AvmLog("          [-Dforcelongbranch]  force full-range branches "
                  "even if not required (x86-64 only)\n");
  avmplus::AvmLog("          [-jitharden]  enable jit hardening techniques\n");
  avmplus::AvmLog("          [-osr=T]      enable OSR with invocation "
                  "threshold T; disable with -osr=0; default is -osr=%d\n",
                  avmplus::AvmCore::osr_threshold_default);
  avmplus::AvmLog("          [-prof=L]     enable jit profile level L; default "
                  "0=disabled; 1=function ranges, 2=functions+native asm)\n");
#ifdef AVMPLUS_IA32
  avmplus::AvmLog(
      "          [-Dnosse]     use FPU stack instead of SSE2 instructions\n");
  avmplus::AvmLog("          [-Dfixedesp]  pre-decrement stack for all needed "
                  "call usage upon method entry\n");
#endif
#ifdef AVMPLUS_ARM
  avmplus::AvmLog("          [-Darm_arch N]  nanojit assumes ARMvN "
                  "architecture (default=5)\n");
  avmplus::AvmLog(
      "          [-Darm_vfp]     nanojit uses VFP rather than SoftFloat\n");
#endif
#ifdef VMCFG_COMPILEPOLICY
  avmplus::AvmLog("          [-policy [{interp|jit}={id|name|%%regex%%}]+ ]\n");
  avmplus::AvmLog("                        override the default policy using "
                  "method identifiers (see -Dverbose=execpolicy,builtins)\n");
  avmplus::AvmLog("                        and/or method names and/or regular "
                  "expressions. ids can be specified either\n");
  avmplus::AvmLog("                        singly or as a range, while names "
                  "will attempt to match exactly. regex follows\n");
  avmplus::AvmLog("                        perl regular expression syntax. "
                  "Note: rules are *not* checked for consistency\n");
  avmplus::AvmLog(
      "                        e.g. -Dverbose=execpolicy,builtins -policy "
      "\"jit=%%(avmshell)|(global)%%,interp=Function/"
      "prime\",jit=-3,interp=5-9,jit=42\n");
#endif /* VMCFG_COMPILEPOLICY */
#endif
#ifdef VMCFG_VERIFYALL
  avmplus::AvmLog(
  ...
  avmplus::AvmLog("          [-Dverifyquiet] verify greedily and don't execute "
                  "anything w/o re-queue msgs\n");
#endif
#ifdef VMCFG_SELFTEST
  ...
  avmplus::AvmLog("          [-Dversion]   print the version and the list of "
                  "compiled-in features and then exit\n");
#ifdef AVMPLUS_WIN32
  avmplus::AvmLog(
  ...
  avmplus::AvmLog("                        Spawn W worker VMs on T threads "
                  "where W >= T and T >= 1.\n");
  avmplus::AvmLog("                        The files provided are handed off "
                  "to the workers in the order given,\n");
  avmplus::AvmLog("                        as workers become available, and "
                  "these workers are scheduled onto threads\n");
  avmplus::AvmLog("                        in a deterministic order that "
                  "prevents them from having affinity to a thread.\n");
  avmplus::AvmLog("                        To test this functionality you want "
                  "many more files than workers and many more\n");
  avmplus::AvmLog("                        workers than threads, and at least "
                  "two threads.\n");
  avmplus::AvmLog("                        If R > 0 is provided then it is the "
                  "number of times the list of files is repeated.\n");
#endif
  avmplus::AvmLog("          [-swfHasAS3]  Exit with code 0 if the single file "
                  "argument is a swf that contains a DoABC or DoABC2 tag,\n");
  avmplus::AvmLog("                        otherwise exit with code 1.  Do not "
                  "execute or verify anything.\n");
  avmplus::AvmLog("          [-swfversion version]\n");
  avmplus::AvmLog("                        Run with a given bug-compatibility "
                  "version in use by default.\n");
  avmplus::AvmLog("                        (This can be overridden on a "
                  "per-ABC basis by embedded metadata.)\n");
...
namespace avmplus {
Isolate *Isolate::newIsolate(int32_t desc, int32_t parentDesc,
                             Aggregate *aggregate) {
  if (parentDesc == Isolate::INVALID_DESC) {
    return mmfx_new(
        avmshell::PrimordialShellIsolate(desc, parentDesc, aggregate));
  } else {
...
// don't break the shell builds. But this means we need to define symbols it
// uses.
...
// don't break the shell builds. But this means we need to define symbols it
// uses.
bool AOTIsDebugMode() { return false; }
} // namespace halfmoon
#endif
