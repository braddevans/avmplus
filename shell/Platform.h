...
public:
   ...
   * @return Pointer to singleton instance of the platform defined
   ...
   * @param code  a numeric value indicating the result of program execution or
   * an error code
   * @return none
   */
   ...
   * @return a handle to platform-specific file instance, NULL if an error
   * occurred
   */
  ...
  /**
   * Method to receive a NUL-terminated string of user data via console or other
   * source This method is useful while running shell in interactive mode and
   * for debugging purposes Currently the expected encoding of the data is
   * Latin-1
   * @param buffer buffer for storing the input data
   * @param bufferSize size of the buffer being passed indicating the max number
   * of bytes it can accept
   * @return pointer to the beginning of input data, NULL on end-of-file
   */
  virtual char *getUserInput(char *buffer, int bufferSize) = 0;

  /**
   * Method to retrieve the lowest address (the limit) of the program stack
   * pointer for the main program thread.  This function makes /no sense/ on any
   * other thread. The limit returned has been adjusted so that some amount of
   * space is available at addresses below the limit, for use by native code.
   *
   * The amount of space made available for native code will typically depend on
   * the platform class (64-bit, 32-bit, embedded), and is heuristic - we have
   * no true estimation at this time (now == Apr-2009) for how much space native
   * code might need.
   *
   * The returned value is used by the VM execution engines to check
   * heuristically (and conservatively) for stack overflow.
   *
   * @return address indicating the lower limit of the current program's
   * execution stack, less a safety margin.
   */
  virtual uintptr_t getMainThreadStackLimit() = 0;

  /**
   * Method to set a platform-specific timer for a callback after specific
   * interval of time This method should implement a one-shot timer
   ...
   * @param callbackData context data that will be passed as an argument during
   * "callback" invocation
   * @return none
   */
  virtual void setTimer(int seconds, AvmTimerCallback callback,
                        void *callbackData) = 0;
};
} // namespace avmshell
