...
ShellCoreSettings::ShellCoreSettings()
    : arguments(NULL), numargs(-1), nodebugger(false), astrace_console(0),
      do_verbose(0), verboseOnlyArg(NULL), enter_debugger_on_launch(false),
      ...
      verifyquiet(avmplus::AvmCore::verifyquiet_default), greedy(false),
      nogc(false), incremental(true), exactgc(true), drc(true),
      drcValidation(false), markstackAllowance(0), fixedcheck(true),
      gcthreshold(0), langID(-1), jitordie(avmplus::AvmCore::jitordie_default),
      do_testSWFHasAS3(false), runmode(avmplus::AvmCore::runmode_default)
#ifdef VMCFG_NANOJIT
      ,
      njconfig(), jitconfig(),
...
#endif
      ,
      st_component(NULL), st_category(NULL), st_name(NULL),
...
ShellToplevel::ShellToplevel(avmplus::AbcEnv *abcEnv)
    : avmplus::Toplevel(abcEnv) {}
...
void ShellCore::stackOverflow(avmplus::Toplevel *toplevel) {
  ...
  GCRef<avmplus::ErrorObject> error = toplevel->errorClass()->constructObject(
      errorMessage->atom(), this->intToAtom(0));
  avmplus::Exception *exception =
      new (GetGC()) avmplus::Exception(this, error->atom());
...
void ShellCore::interrupt(avmplus::Toplevel *toplevel, InterruptReason) {
  ...
  Platform::GetInstance()->setTimer(kScriptGracePeriod, interruptTimerCallback,
                                    this);
...
#ifdef VMCFG_AOT
  avmplus::NativeInitializer shellNInit(
      this, avmplus::NativeID::shell_toplevel_versioned_uris,
...
#else
  shellPool =
      AVM_INIT_BUILTIN_ABC_IN_DOMAIN(shell_toplevel, this, shell_domain);
#endif
}
...
ShellToplevel *ShellCore::createShellToplevel() {
  ...
  public:
    ...
    virtual avmplus::CodeContext *
    create(avmplus::DomainEnv *domainEnv,
  ...
  shell_domainEnv =
      avmplus::DomainEnv::newDomainEnv(this, shell_domain, builtinDomainEnv);

  // Initialize the shell builtins in the new Toplevel
  // use the same bugCompatibility that the base builtins use
  const avmplus::BugCompatibility *shell_bugCompatibility =
      shell_toplevel->abcEnv()->codeContext()->bugCompatibility();
  ShellCodeContext *shell_codeContext =
      new (GetGC()) ShellCodeContext(shell_domainEnv, shell_bugCompatibility);

  // handleActionPool(shellPool, shell_toplevel, shell_codeContext);
  shell_toplevel->shellClasses =
      prepareBuiltinActionPool<avmplus::shell_toplevelClassManifest>(
          shellPool, shell_toplevel, shell_codeContext);
...
#endif
  {
    ...
    AvmAssert(
        shell_toplevel->shellClasses->get_ShellCoreFriend1Class()->get_foo() ==
        42);
    AvmAssert(
        shell_toplevel->shellClasses->get_ShellCoreFriend2Class()->get_bar() ==
        101);
  }
...
/*virtual*/ avmplus::Toplevel *
ShellCore::createToplevel(avmplus::AbcEnv *abcEnv) {
...
//
//  - Does not try to determine whether input is Latin1, UTF8, or indeed,
//  already UTF16,
//    but assumes UTF8, which can be dangerous.  Falls back to latin1 if the
//    utf8 conversion fails, this seems ill-defined in the string layer though
//    so it's just one more hack.
//
//  - Does not create an UTF16 string.  The string layer is actually broken on
//  this count,
//    because requesting an empty UTF16 string returns a constant that is a
//    Latin1 string, and appending to it won't force the representation to UTF16
//    unless the data require that to happen.  See
//    <URL:https://bugzilla.mozilla.org/show_bug.cgi?id=473995>.
//
//  - May incur copying because the terminating NUL is not accounted for in the
//  original
//    creation
//
// Much useful guidance is found at http://www.unicode.org/faq/utf_bom.html.

avmplus::String *ShellCore::decodeBytesAsUTF16String(uint8_t *bytes,
                                                     uint32_t nbytes,
                                                     bool terminate) {
...
avmplus::String *ShellCore::readFileForEval(avmplus::String *referencingFile,
                                            avmplus::String *filename) {
  // FIXME, filename sanitazion is more complicated than this
  if (referencingFile != NULL && filename->charAt(0) != '/' &&
      filename->charAt(0) != '\\') {
  ...
  // FIXME, not obvious that UTF8 is correct for all operating systems (far from
  // it!)
...
void ShellCore::evaluateString(avmplus::String *input, bool record_time) {
  ...
  TRY(this, avmplus::kCatchAction_ReportAsError) {
    ...
    avmplus::Atom result =
        handleActionSource(input, /*filename*/ NULL, shell_toplevel,
                           /*ninit*/ NULL, user_codeContext, apiVersion);
    if (record_time)
      now = VMPI_getDate();
...
int ShellCore::executeProjector(char *executablePath) {
  ...
  return handleArbitraryExecutableContent(settings.do_testSWFHasAS3, code,
                                          executablePath);
}
...
void ShellCore::executeSelftest(ShellCoreSettings &settings) {
  setStackLimit();
  avmplus::selftests(this, settings.st_component, settings.st_category,
                     settings.st_name);
}
#endif
...
ShellToplevel *ShellCore::setup(const ShellCoreSettings &settings) {
#ifdef VMCFG_AOT
  if (nAOTInfos == 0) {
    console << "To run an AOT enabled avmshell you must link in some AOT "
               "compiled ABC blocks.\n";
    return NULL;
  }
#endif
  ...
  this->bugzilla444630 = (this->defaultBugCompatibilityVersion >=
                          avmplus::BugCompatibility::kSWF10);
  ...
  if (settings.do_verbose & avmplus::VB_builtins)
    config.verbose_vb =
        settings
            .do_verbose; // ~builtins then skip verbose settings during setup()
#endif
  config.runmode = settings.runmode;

  // verbose requires methodnames (in avmshell, anyway), before calling
  // initBuiltinPool.
  ...
  TRY(this, avmplus::kCatchAction_ReportAsError) {
    ...
    avmplus::Domain *user_domain =
        avmplus::Domain::newDomain(this, shell_domain);
    avmplus::DomainEnv *user_domainEnv =
        avmplus::DomainEnv::newDomainEnv(this, user_domain, shell_domainEnv);
    const avmplus::BugCompatibility *user_bugCompatibility =
        createBugCompatibility(defaultBugCompatibilityVersion);
    this->user_codeContext =
        new (GetGC()) ShellCodeContext(user_domainEnv, user_bugCompatibility);

#ifdef AVMPLUS_VERBOSE
    config.verbose_vb =
        settings.do_verbose; // builtins is done, so propagate verbose
#endif
...
const char *ShellCore::identifyDomain(avmplus::Domain *domain) {
  return domain == builtinDomain ? "builtin"
                                 : (domain == shell_domain ? "shell" : NULL);
}
#endif
...
int ShellCore::evaluateFile(ShellCoreSettings &settings, const char *filename) {
  ...
  return handleArbitraryExecutableContent(settings.do_testSWFHasAS3,
                                          dummyScriptBuffer, NULL);
#else // ndef VMCFG_AOT

  if (config.interrupts)
    Platform::GetInstance()->setTimer(kScriptTimeout, interruptTimerCallback,
                                      this);
...
int ShellCore::evaluateScriptBuffer(avmplus::ScriptBuffer &buffer,
...
int ShellCore::handleArbitraryExecutableContent(bool do_testSWFHasAS3,
  ...
  TRY(this, avmplus::kCatchAction_ReportAsError) {
    ...
    if (filename == NULL) {
      ...
      avmplus::ApiVersion apiVersion = this->getApiVersionFromCallStack();
      handleActionBlock(code, 0, shell_toplevel, NULL, user_codeContext,
                        apiVersion);
    } else if (isSwf(code)) {
#ifdef VMCFG_VERIFYALL
      if (config.verbose_vb & avmplus::VB_verify)
...
#endif
      bool result = handleSwf(filename, code, shell_toplevel, user_codeContext,
                              do_testSWFHasAS3);
      if (do_testSWFHasAS3)
        return int(!result); // "no abc" is "false" but translates to "1" for
                             // the exit code, and "true" becomes "0"
    } else {
#ifdef VMCFG_EVAL
      AvmCore *core = shell_toplevel->core();
      if (!core->config.verifyonly) {
        // FIXME: I'm assuming code is UTF8 - OK for now, but easy to go wrong;
        // it could be 8-bit ASCII
        avmplus::String *code_string = decodeBytesAsUTF16String(
            code.getBuffer(), (uint32_t)code.getSize(), true);
        avmplus::String *filename_string = decodeBytesAsUTF16String(
            (uint8_t *)filename, (uint32_t)VMPI_strlen(filename));
        avmplus::ScriptBuffer empty; // With luck: allow the
        code = empty;                //    buffer to be garbage collected
        avmplus::ApiVersion apiVersion = this->getApiVersionFromCallStack();
        handleActionSource(code_string, filename_string, shell_toplevel, NULL,
                           user_codeContext, apiVersion);
      }
#else
  ...
  CATCH(avmplus::Exception * exception) {
    if (!(exception->flags & avmplus::Exception::EXIT_EXCEPTION)) {
      ...
      getIsolate()->getAggregate()->stateTransition(
          getIsolate(), avmplus::Isolate::EXCEPTION);
    }
    ...
    TRY(this, avmplus::kCatchAction_ReportAsError) {
      if (!(exception->flags & avmplus::Exception::SUPPRESS_ERROR_REPORT)) {
        ...
        if (atomKind(payload) == kObjectType) {
          ...
          if (object) {
            avmplus::Atom code =
                object->getStringProperty(internStringLatin1("exitCode"));
