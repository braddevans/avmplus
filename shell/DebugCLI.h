...
public:
  ...
  BreakAction(avmplus::SourceFile *sourceFile, int id,
              avmplus::Stringp filename, int linenum) {
...
public:
  LocalValue(avmplus::DebugFrame *frame, int index)
  ...
  virtual void set(avmplus::Atom newValue) { frame->setLocal(index, newValue); }
...
public:
  ArgumentValue(avmplus::DebugFrame *frame, int index)
      : frame(frame), index(index) {}
...
public:
  ...
  virtual bool isLValue() { return true; }
  virtual avmplus::Atom get() {
    return parent->toplevel()->getproperty(parent->atom(), propertyname,
                                           parent->vtable);
  }
  virtual void set(avmplus::Atom newValue) {
    parent->toplevel()->setproperty(parent->atom(), propertyname, newValue,
                                    parent->vtable);
  }
...
public:
  ...
  REALLY_INLINE static DebugCLI *
  create(MMgc::GC *gc, avmplus::AvmCore *core,
...
private:
   ...
   */
  /*@{*/
  int determineCommand(StringInt cmdList[], const char *input, int defCmd);
  const char *commandNumberToCommandName(StringInt cmdList[], int cmdNumber);
...
public:
   ...
   */
  StPropertyFinder(avmplus::AvmCore *core, avmplus::Atom atom,
...
public:
  PropertyPrinter(avmplus::AvmCore *core, DebugCLI *debugger,
                  avmplus::Atom atom);
