...
void CALLBACK TimeoutProc(UINT /*uTimerID*/, UINT /*uMsg*/, DWORD_PTR dwUser,
...
void WinPlatform::exit(int code) { ::exit(code); }

File *WinPlatform::createFile() { return new WinFile(); }

void WinPlatform::destroyFile(File *file) { delete file; }
...
void WinPlatform::setTimer(int seconds, AvmTimerCallback callback,
                           void *callbackData) {
  TimerCallbackInfo *info = new TimerCallbackInfo;
  info->callbackFunc = callback;
  info->userData = callbackData;

  timeSetEvent(seconds * 1000, seconds * 1000, (LPTIMECALLBACK)TimeoutProc,
               (DWORD_PTR)info, TIME_ONESHOT);
}
...
unsigned long CrashFilter(LPEXCEPTION_POINTERS pException, int exceptionCode) {
  ...
  if ((result = UnhandledExceptionFilter(pException)) !=
  ...
  if (MiniDumpWriteDump_) {
    ...
    HANDLE hDumpFile = CreateFileW(DumpPath, GENERIC_WRITE, 0, NULL,
...
static int executeShell(int argc, char *argv[]) {
  ...
  __try {
    ...
    if (code == avmshell::OUT_OF_MEMORY)
      ...
      avmshell::CrashFilter(GetExceptionInformation(), GetExceptionCode())) {
    code = -1;
  }
