 ...
 */
DebugCLI::StringInt DebugCLI::commandArray[] = {{"break", CMD_BREAK},
                                                {"bt", INFO_STACK_CMD},
...
DebugCLI::DebugCLI(avmplus::AvmCore *core,
                   avmplus::Debugger::TraceLevel tracelevel)
...
char *DebugCLI::nextToken() {
  ...
  if (currentToken) {
    while (*currentToken) {
      if (*currentToken == ' ' || *currentToken == '\r' ||
          *currentToken == '\n' || *currentToken == '\t') {
        *currentToken++ = 0;
        break;
      }
 ...
 */
int DebugCLI::determineCommand(DebugCLI::StringInt cmdList[], const char *input,
                               int defCmd) {
  if (!input)
  ...
  else if (!ambiguous || inputLen == 1 ||
           !VMPI_strcmp(cmdList[match].text, input)) {
...
void DebugCLI::breakpoint(char *location) {
  ...
  if (breakpointSet(sourceFile, targetLine)) {
    ...
    core->console << "Breakpoint " << breakpointId << ": file " << filename
                  << ", " << (targetLine) << ".\n";

    BreakAction *breakAction = new (core->GetGC())
        BreakAction(sourceFile, breakpointId, filename, targetLine);
...
IValue *DebugCLI::getValue(const char *name) {
  ...
  if (name[0] == '\'' || name[0] == '"') // String literal
  {
    ...
    if (length >= 2 && namestr->charAt(length - 1) == namestr->charAt(0)) {
      ...
      return new (core->gc)
          ConstantValue(namestr->substr(1, length - 2)->atom());
    }
  }
  if (name[0] == '<') // XML or XMLList literal
  {
    if (avmplus::AvmCore::isObject(thisAtom)) {
      avmplus::Toplevel *toplevel =
          avmplus::AvmCore::atomToScriptObject(thisAtom)->toplevel();
      if (name[1] == '>')
        return new (core->gc)
            ConstantValue(toplevel->xmlListClass()->ToXMLList(namestr->atom()));
      else
        return new (core->gc)
            ConstantValue(toplevel->xmlClass()->ToXML(namestr->atom()));
    }
  }
  ...
  if (env) {
    ...
    avmplus::ScriptEnv *script =
        core->domainMgr()->findScriptEnvInDomainEnvByMultiname(env->domainEnv(),
                                                               mn);
    if (script != (avmplus::ScriptEnv *)avmplus::BIND_NONE &&
        script != (avmplus::ScriptEnv *)avmplus::BIND_AMBIGUOUS) {
        ...
        if (!value) {
          if (avmplus::AvmCore::isObject(parent)) {
            ...
            avmplus::Multiname mn(core->getAnyPublicNamespace(),
                                  core->internStringLatin1(name));
            value = new (core->gc)
                PropertyValue(avmplus::AvmCore::atomToScriptObject(parent), mn);
          } else {
            if (dot)
              *dot = '.'; // restore for our caller
            throwUndefinedVarError(name);
            return NULL; // unreachable
          }
        }
...
bool DebugCLI::filterException(avmplus::Exception *exception,
                               bool /*willBeCaught*/) {
...
void DebugCLI::setCurrentSource(avmplus::Stringp file) {
  ...
  if (f.valid() &&
      ((uint64_t)file->length() < UINT32_T_MAX)) { // cannot handle files > 4GB
    ...
    core->console << "Could not find '" << currentFile
...
avmplus::Stringp DebugCLI::formatValue(avmplus::Atom value) {
  // An experiment: Printing XML and XMLList variables with their full
  // toXMLString() form.
  //        if (core->isXMLorXMLList(value))
  //        {
  //            ScriptObject* object = AvmCore::atomToScriptObject(value);
  //            Multiname mn(core->getAnyPublicNamespace(),
  //            core->internStringLatin1("toXMLString")); Atom thisAtom =
  //            undefinedAtom; return
  //            core->string(object->toplevel()->callproperty(value, &mn, 1,
  //            &thisAtom, object->vtable));
  //        }
...
void PropertyIterator::iterate() {
  ...
  while ((index = object->nextNameIndex(index)) != 0) {
    avmplus::Multiname mn(core->getAnyPublicNamespace(),
                          core->string(object->nextName(index)));
    if (!process(&mn, avmplus::BKIND_VAR))
      return;
  }
}

StPropertyFinder::StPropertyFinder(avmplus::AvmCore *core, avmplus::Atom atom,
                                   const char *propertyname)
    : PropertyIterator(core, atom), value(NULL), propertyname(propertyname) {}

bool StPropertyFinder::process(avmplus::Multiname *key,
                               avmplus::BindingKind /*bk*/) {
...
PropertyPrinter::PropertyPrinter(avmplus::AvmCore *core, DebugCLI *debugger,
                                 avmplus::Atom atom)
    : PropertyIterator(core, atom), debugger(debugger) {}

bool PropertyPrinter::process(avmplus::Multiname *key,
                              avmplus::BindingKind bk) {
  if (bk == avmplus::BKIND_CONST || bk == avmplus::BKIND_VAR ||
      bk == avmplus::BKIND_GET || bk == avmplus::BKIND_GETSET) {
    avmplus::Atom value =
        object->toplevel()->getproperty(object->atom(), key, object->vtable);
    switch (bk) {
