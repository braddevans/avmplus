...
uintptr_t UnixPlatform::getMainThreadStackLimit() {
  ...
  // https://bugzilla.mozilla.org/show_bug.cgi?id=504976: setting the height to
  // kStackSizeFallbackValue is not ideal if the stack is meant to be unlimited
  // but is an OK workaround for the time being.
...
void UnixPlatform::setTimer(int seconds, AvmTimerCallback callback,
                            void *callbackData) {
...
void alarmProc(int /*signum*/) { pCallbackFunc(pCallbackData); }
