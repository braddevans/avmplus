...
llvm::Value *LLVMEmitter::createMemberGEP(llvm::Value *ptr, FieldInfo *member,
  ...
  if (ptr->getType() == module.scriptObjectPtrTy &&
      ptr->getType() != structPtrTy) {
...
llvm::Value *LLVMEmitter::storeMember(llvm::Value *val, llvm::Value *ptr,
  ...
  return CreateStore(canUpcastTo(val->getType(), member->type)
...
llvm::Value *LLVMEmitter::coerceArg(llvm::Value *val, llvm::Type *to) {
  AvmAssert(canUpcastTo(val->getType(), to) ||
            permissibleDowncast(val->getType(), to));
...
llvm::Value *LLVMEmitter::set_def_ins(Def *d, llvm::Value *ins) {
  ...
  if (!ins->getType()->isVoidTy() && ins->getName().empty() &&
...
void LLVMEmitter::InsertPrintf(const char *s) {
  ...
  llvm::GlobalVariable *gStr =
      new llvm::GlobalVariable(module, str->getType(), /*isConst*/ true,
...
void LLVMEmitter::do_abc_finddef(BinaryStmt *instr) {
  ...
  llvm::Value *cmp = CreateICmpEQ(
      targetObj, llvm::ConstantPointerNull::get(
                     llvm::cast<llvm::PointerType>(targetObj->getType())));
...
void LLVMEmitter::emitExceptionHandling() {
...

  {
    ...
    llvm::GlobalVariable *exceptionDescsVar = new llvm::GlobalVariable(
        module, exceptionDescsConst->getType(), /*isConst*/ true,
