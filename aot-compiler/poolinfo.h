...
//
// Manages per-Pool (per-ABC file) level things such as the creation of AOTInfo
// structures that bind AOT code to the runtime libries.
//
// The naming of certain things like functions is handled here since function
// names contain a pool identifier.
//
// IMPORTANT NOTE:
//
// Instances of this class persist from when a pool is loaded until the end of
// the compile. The class should not carry heavyweight data, and in particular,
// it should not contain any references to LLVM data after "disconnect()" is
// called. It should be possible to delete an LLVM Module and LLVMContext as
// soon as a module's code is generated. Also: continued references to LLVM data
// after "disconnect()" will put multi-threaded code generation at unnecessary
// risk.
//
...
public:
  PoolInfo(GC *gc, const std::string &fileName, const std::string &infoName)
      : GCRoot(gc), modulep(0), pool(0), abcBytes(0), abcSize(0),
        totalABCSize(0), fileName(fileName), infoName(infoName),
        lastDependency(0), outOfDate(false), abc_env(0),
        creatingAOTInfo(false) {}

  ~PoolInfo() { delete[] abcBytes; }
  ...
  void addDependency(MethodInfo *info) {
    addDependency(info->pool()->poolInfo);
  }
  ...
  void appendVerifierResult(AbcEnv *abc_env,
                            const compile_abc::MethodVerifyInfo &verifierInfo);
  void createAOTInfo();
...
private:
  ...
  llvm::GlobalVariable *makeVariable(llvm::Constant *init, bool isConst,
