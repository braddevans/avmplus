...
void SimpleExpandSetAll(PM *pm, GCHeap *heap, void *item, uint32_t numPages,
                        PageType val) {
...
PageMapBase::PageMapBase() : memStart(MAX_UINTPTR), memEnd(0) {}

#ifdef MMGC_USE_UNIFORM_PAGEMAP
Uniform::Uniform() : PageMapBase(), pageMap(NULL) {}
...
void Uniform::EnsureCapacity(GCHeap *heap, void *item, uint32_t numPages) {
  ...
  uint32_t numBlocksNeeded = uint32_t(
      ((memEnd - memStart) >> kByteOffsetShift) / GCHeap::kBlockSize + 1);
...
void Uniform::ExpandSetAll(GCHeap *heap, void *item, uint32_t numPages,
                           PageType val) {
...
void Tiered2::EnsureCapacity(GCHeap *heap, void *item, uint32_t numPages) {
  ...
  for (uint32_t i = AddrToIndex1(addr); i < i_lim; i++) {
    ...
    uint8_t *subMap = pageMap[i];
    if (subMap == NULL) {
      subMap = (uint8_t *)heap->GetPartition(kPageMapPartition)
...
void Tiered2::ExpandSetAll(GCHeap *heap, void *item, uint32_t numPages,
                           PageType val) {
  SimpleExpandSetAll(this, heap, item, numPages, val);
}
...
void Tiered4::InitPageMap(GCHeap *heap, uintptr_t start, uintptr_t limit) {
  ...
  uint32_t i = AddrToIndex0(start), j = AddrToIndex1(start),
           k = AddrToIndex2(start);
  ...
  for (; i < i_lim; i++, j = 0, k = 0) {
    uint8_t ***subMap1 = pageMap[i];
    uint32_t j_lim =
        (i + 1 < i_lim) ? tier1_entries : AddrToIndex1(limit - 1) + 1;
    if (subMap1 == NULL) {
      subMap1 = (uint8_t ***)heap->GetPartition(kPageMapPartition)
    ...
    for (; j < j_lim; j++, k = 0) {
      uint8_t **subMap2 = subMap1[j];
      uint32_t k_lim =
          (j + 1 < j_lim) ? tier2_entries : AddrToIndex2(limit - 1) + 1;
      if (subMap2 == NULL) {
        subMap2 = (uint8_t **)heap->GetPartition(kPageMapPartition)
                      ->AllocNoOOM(tier2_pages);
        subMap1[j] = subMap2;
      }
      for (; k < k_lim; k++) {
        uint8_t *subMap3 = subMap2[k];
        if (subMap3 == NULL) {
          subMap3 = (uint8_t *)heap->GetPartition(kPageMapPartition)
...
CacheT4::CacheT4()
    : Tiered4(), cached_addr_prefix(CacheT4::uncached),
      cached_leaf_bytes(NULL) {}

DelayT4::DelayT4() : CacheT4(), stillInitialDelay(true) {}
...
void DelayT4::SetupDelayedPagemap(GCHeap *heap, uintptr_t start,
                                  uintptr_t limit) {
...
void DelayT4::EnsureCapacity(GCHeap *heap, void *item, uint32_t numPages) {
  ...
  if (fits_in_one_leaf) {
    if (cached_addr_prefix == CacheT4::uncached) {
      ...
      cached_leaf_bytes = (uint8_t *)heap->GetPartition(kPageMapPartition)
                              ->AllocNoOOM(tier3_pages);
    } else {
...
void Tiered4::ExpandSetAll(GCHeap *heap, void *item, uint32_t numPages,
                           PageType val) {
  SimpleExpandSetAll(this, heap, item, numPages, val);
}
...
void CacheT4::ExpandSetAll(GCHeap *heap, void *item, uint32_t numPages,
                           PageType val) {
  SimpleExpandSetAll(this, heap, item, numPages, val);
}
...
void DelayT4::ExpandSetAll(GCHeap *heap, void *item, uint32_t numPages,
                           PageType val) {
  SimpleExpandSetAll(this, heap, item, numPages, val);
}
#endif // ! defined(MMGC_USE_UNIFORM_PAGEMAP) && defined(MMGC_64BIT)
