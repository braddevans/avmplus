 ...
 *
 * The policy manager centralizes policy decisions about how and when to run
 * garbage collection.  It is queried from the collector code, and collector
 * code signals to the policy manager at the occurrence of certain events.
 *
 * Typically the GCPolicyManager is simply embedded inside a GC instance, and
 * manages policy for that GC.
 *
 * ----
 *
 ...
 * documented extensively in GC.cpp and in doc/mmgc/policy.pdf.  The policy
 * improves on the first-cut policy by running the GC less often and having
 * lower pause times.
 *
...
public:
  ...
  /**
   * @return the number of objects reported marked by signalMarkWork, since
   * startup.
   */
  uint64_t objectsMarked();

  /**
   * @return the number of bytes reported marked by signalMarkWork, since
   * startup.
   */
  ...
  /**
   * Situation: a GC event corresponding to one of the PolicyEvent values
   * occurs. Tell the policy manager about it.
   */
  ...
  /**
   * Situation: 'numblocks' blocks have just been obtained by this GC from the
   * GCHeap.
   */
  void signalBlockAllocation(size_t numblocks);

  /**
   * Situation: 'numblocks' blocks have just been returned from this GC to the
   * GCHeap.
   */
  ...
  /**
   * Situation: external code has deallocated memory whose lifetime was tied to
   * a GC object; the memory must previously have been accounted for through a
   * call to signalDependentAllocation().
   */
  void signalDependentDeallocation(size_t nbytes);

  /**
   * Situation: the incremental marker has been started, and we need to know
   * whether to run another mark increment or push the conclusion to a finish
   * (because the total allocation budget for the collection cycle has been
   * exhausted).  This predicate returns true in the latter case.
   */
  bool queryEndOfCollectionCycle();

#ifdef MMGC_POLICY_PROFILING
  /**
   * Situation: signal that one write has been examined by the write barrier and
   * made it to the given stage of the barrier.   See the barrier code to find
   * out what the stages are.
   */
   ...
   */
  void signalReapWork(uint32_t objects_reaped, uint32_t bytes_reaped,
                      uint32_t objects_pinned);
#endif
#ifdef MMGC_POINTINESS_PROFILING
  /**
   * Situation: signal that 'words' words have been scanned; that
   * 'could_be_pointer' number of these words passed the initial heap range
   * checks; and that 'actually_is_pointer' number of these words were
   * conservatively found to be pointers to heap objects.
   */
  /*inline*/ void signalDemographics(size_t words, size_t could_be_pointer,
                                     size_t actually_is_pointer);
#endif
  ...
  /**
   * Situation: signal that one reference is being added to the ZCT.  If
   * 'initial' is true then the object is freshly allocated and the adding is
   * from RCObject's constructor.  'population' is the number of elements in the
   * table at the time ZCT::Add is entered.
   */
  /*REALLY_INLINE*/ void signalZCTAdd(bool initial, uint32_t population);

  /**
   * Situation: signal that one reference is being removed from the ZCT.  If
   * 'final' is true then the object is being deleted and the removal is from
   * RCObject's destructor.
   */
  /*REALLY_INLINE*/ void signalZCTRemove(bool final);
#endif

  /**
   * The collector 'gc' (which is not the collector for this manager) has
   * started a garbage collection, indicating perhaps some memory pressure in
   * that heap.
   */
  void signalStartCollection(GC *gc);

  /**
   * The collctor 'gc' (which is not the collector for this manager) has
   * completed a garbage collection.
   */
  ...
  // Elapsed time (in ticks) for various collection phases, and the maximum
  // phase time for each phase.  They are updated when an END event is signaled.
  // Client code must not modify these variables.
  ...
  // The total time doing collection work (sum of the variables above except the
  // ZCT reap time) and the elapsed time from the start of StartIncrementalMark
  // to the end of FinalizeAndSweep.  Together they provide a crude
  // approximation to a measure of pause clustering.
  uint64_t timeInLastCollection;
  uint64_t timeEndToEndLastCollection;

  // Time for ZCT reaping during the last collection cycle (end of one
  // FinalizeAndSweep to the end of the next one).
  ...
  // The maximum latcency for various collection phases during the previous
  // collection cycle
...
private:
  ...
  // @return true if we should print policy data after every GC and at the end
  // of the run
  ...
  // Various private methods for the GC policy follow.  See comment in GC.cpp
  // for details.

  // Amount of GC work to perform (bytes to scan) per byte allocated while the
  // GC is active
  double W();
  ...
  // Called from the policy event handler to start computing adjustments to R:
  // before any mark work is performed.
  void startAdjustingR();

  // Called from the policy event handler to finish computing adjustments to R:
  // after all mark work has been performed.
  void endAdjustingR();

  // Called from adjustPolicyForNextMajorCycle to compute the effective L for
  ...
  // major collection cycle (from the end of one FinishIncrementalMark to the
  // start of the next one)
  void adjustPolicyForNextMajorCycle(bool okToShrinkHeap);

  // Called from the policy event handler to compute the GC policy for the next
  // minor collection cycle (from the end of one IncrementalMark to the start of
  // the next one)
  ...
  // Temporaries for holding the start time / start event until the end event
  // arrives
  uint64_t start_time;
  PolicyEvent start_event;

  // Value returned by lowerLimitCollectionThreshold() and set by
  // setLowerLimitCollectionThreshold(): the heap size, in blocks, below which
  // we do not collect.
  uint32_t collectionThreshold;
...
#ifdef MMGC_POLICY_PROFILING
  // Records the heap population before we sweep.  Unit is blocks except where
  // noted.
  ...
  // Barrier stages hit in InlineWriteBarrier: examine mark, and hit the
  // barrier.
...
#endif
#ifdef MMGC_POINTINESS_PROFILING
  // Track the number of scannable words, the number that passes the initial
  // range check, and the number that turn out to be (conservative) pointers.
  //
  ...
  // gc trigger as fraction of allocation budget to use before triggering GC
  // [0,1]
  double T;
  ...
  // largest multiple of a selected L_ideal to which L_actual can grow (unless
  // 0, which means unlimited)
  double X;

  // Value of the heap size, H, as computed at the beginning of the last major
  // cycle
  double H_previous;

  // The allocation budget for the major GC cycle.  This variable remains
  // constant throughout the major cycle.
  double majorAllocationBudget;

  // the remaining allocation budget for the major GC cycle.  (This can go
  // negative and the variable must accomodate negative values.  It's not
  // frequently accessed.)
  double remainingMajorAllocationBudget;

  // the allocation budget for the minor GC cycle.  This variable remains
  // constant throughout the minor cycle, is measured in bytes, and must be
  // positive.
  int32_t minorAllocationBudget;

  // the remaining allocation budget for the minor GC cycle.  Initially this has
  // the value of minorAllocationBudget; the allocation request size is
  // subtracted for every allocation.  This variable can go negative because we
