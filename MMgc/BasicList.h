...
namespace MMgc {
template <typename T, int growthIncrement = 4> class BasicList {
public:
  BasicList()
      : count(0), capacity(0), items(NULL), iteratorCount(0), holes(false),
        cursor(0) {}
  ...
  void Remove(T item) {
    ...
    if (i == Limit()) {
      GCAssertMsg(
          false,
          "Bug: should not try to remove something that's not in the list");
      return;
    }

    //  If the item we deleted occured is our cursor, set our cursor to the next
    //  item
    if (i == cursor) {
      do {
        cursor++;
  ...
  //   Call "SetCursor" on the list to have future iterators begin iterating
  //   from that position in the list. The iterator will still iterate through
  //   all items in the list once by wrapping to the front when the end is hit.
  void SetCursor(uint32_t newCursor) { cursor = newCursor; }
  uint32_t GetCursor() { return cursor; }
  uint32_t Count() const { return count; }

  // iterator needs this to give complete iteration in the face of in-flight
  // mutation
...
//  The BasicListIterator will iterate through items in the BasicList starting
//  with the List's "cursor position" if "SetCursor" is never called on the
//  BasicList passed into this BasicListIterator, then the list will iterate
//  starting at position 0.  The iterator will still iterate through all items
//  in the list once by wrapping to the front when the end is hit.
template <typename T> class BasicListIterator {
public:
  BasicListIterator(BasicList<T> &bl) : bl(bl), bListEnd(false) {
    index = bl.GetCursor();
    bl.IteratorAttach();
  }
  ~BasicListIterator() { bl.IteratorDettach(); }
  T next() {
    if (bListEnd)
      return NULL;
