 ...
 *
 * @return  A pointer to a suitable memory area.  NULL only if kCanFail is part
 * of opts.  The memory is zeroed only of kZero is part of opts.
 *
 * @note If memory cannot be allocated and kCanFail is not part of opts then the
 * regular OOM handling is run, in the hope that this will free some memory on
 * the system heap as well.  If memory is not available on a second allocation
 * attempt then the system will enter the abort state.
 */
 ...
 * @return NULL only if ((opts & kCanFail) != 0), zeroed memory only if ((opts &
 * kZero) != 0).
 */
REALLY_INLINE void *AllocCallInline(size_t size, FixedMallocOpts opts,
                                    int partition) {
#ifdef MMGC_USE_SYSTEM_MALLOC
  (void)partition;
...
// Unmanaged (MMFX) memory allocations via 'new' may be partitioned only by
// whether they are array or scalar (non-array) objects. For more fine-grained
// control of partitioning, the allocation site must use FixedMalloc::Alloc() or
// FixedMalloc::CAlloc(), and the client is responsible for invoking
// constructors (via placement 'new') and destructors explicitly.  For scalar
// allocations, we can provide a customized implementation of 'new' as a class
// member and avoid modifying the allocation sites.

#ifdef MMGC_OVERRIDE_GLOBAL_NEW

// Global operator new and delete implementations to allocate/free from
// FixedMalloc. Using these may create a problem if compiled with system (OS)
// headers that also use the default new & delete. This happens for example in
// ATL of the Windows ActiveX builds. Also, out of memory handling will release
// everything allocated with FixedMalloc when abort due to OOM occurs. If things
// in the system headers/plugin interface/platform globals are allocated with
// these, it will result most likely in a crash. By using the GCAllocObject you
// can try to go around some of these issues, but at least to system headers may
// pose a problem.
#ifdef __clang__
...
REALLY_INLINE void *operator new(size_t size) MMGC_NEW_THROWS_CLAUSE {
  return MMgc::AllocCallInline(size, MMgc::kNone,
                               MMgc::kMMFXNewScalarPartition);
}

REALLY_INLINE void *
operator new(size_t size, MMgc::FixedMallocOpts opts) MMGC_NEW_THROWS_CLAUSE {
...
operator new[](size_t size, MMgc::FixedMallocOpts opts) MMGC_NEW_THROWS_CLAUSE {
  return ((size == 0) ? NULL
                      : MMgc::AllocCallInline(size, opts,
                                              MMgc::kMMFXNewArrayPartition));
}
...
// Defines allocation and deallocation functions used by allocation and
// deallocation macros. operator new and new[] have a specialized signature by
// having an ignored MMgc::kUseFixedMalloc parameter. Specialized operator
// delete and delete[] are provided to get rid of compiler warnings.
...
// has impl for solaris (maybe solaris is generating C++ exception based delete
// calls?)
REALLY_INLINE void operator delete(void *, MMgc::NewDummyOperand /* ignored */)
    MMGC_DELETE_THROWS_CLAUSE {
  GCAssert(false);
}

// b/c we need to use the fancy template routines for new[] these arent used
// void* operator new[](size_t size, size_t arraySize, MMgc::NewDummyOperand
// /*ignored*/) MMGC_NEW_THROWS_CLAUSE; void* operator new[](size_t size, size_t
// arraySize, MMgc::NewDummyOperand /*ignored*/, MMgc::FixedMallocOpts opts)
// NEW_THROWS_CLAUSE; void operator delete[](void *p, MMgc::NewDummyOperand /*
// ignored */) MMGC_DELETE_THROWS_CLAUSE;

// These values are '8' because some architectures (MIPS for sure, PPC probably)
// require that structures be aligned on the natural boundary for their largest
// element, which is typically a 'double', ie, 8 bytes.

#define MMGC_ARRAYHEADER_SIZE 8 // Space for array length before array
#define MMGC_GUARDCOOKIE_SIZE                                                  \
  8 // Space for cookie before object (precedes header)
 ...
 * The operators new are defined in GCGlobalNew.cpp; they inline NewTaggedScalar
 * as their entire function body, so NewTaggedScalar defines the semantics of
 * 'new'.
 *
 * @param size  The size in bytes of the object
 * @param opts  A bit vector of the usual MMgc options (kCanFail, kZero).
 *
 * @return A pointer to the object.  Returns NULL only if ((opts & kCanFail) !=
 * 0) and the object can't be allocated due to an out-of-memory condition or if
 * the object would exceed the maximum object size.
 *
 ...
 * @param opts         A bit vector of the usual MMgc options (kCanFail, kZero).
 * @param isPrimitive  A flag indicating whether the array needs a header
 * storing the value of count: if false, the header is created and the value of
 * count is stored; if true, it is not.
 *
 * @return  A pointer to element 0 of the array.  Returns NULL only if ((opts &
 * kCanFail) != 0) and the object can't be allocated due to an out-of-memory
 * condition or if the object would exceed the maximum object size.
 */
 ...
 *
 * @param p  A non-NULL pointer returned from NewTaggedArray with
 * isPrimitive=false.
 */
 ...
 *
 * @param p  A pointer returned from NewTaggedArray with isPrimitive=false, or
 * NULL.
 */
void DeleteTaggedArrayWithHeaderChecked(void *p, bool isPrimitive,
                                        int partition);
}; // namespace MMgc

template <class T>
T *MMgcConstructTaggedArray(T * /*dummy template arg*/, size_t count,
                            MMgc::FixedMallocOpts opts) {
  T *mem = (T *)MMgc::NewTaggedArray(count, sizeof(T), opts, false,
                                     MMgc::kMMFXNewArrayPartition);
  // Observe that mem can be NULL only if (opts & kCanFail) is true, so there is
  // no reason to redundantly test that condition here, and in fact adding the
  // condition will confuse tools like Coverity (bugzilla 655048).
  if (mem == NULL)
    return NULL;
  T *tp = mem;
...
#define DECLARE_PRIM_ARRAY_NEW(_x, _part)                                      \
  template <>                                                                  \
  REALLY_INLINE _x *MMgcConstructTaggedArray(_x *, size_t count,               \
                                             MMgc::FixedMallocOpts opts) {     \
    return (_x *)MMgc::NewTaggedArray(count, sizeof(_x), opts, true, _part);   \
  }

// Scalar delete mechanism. First calls the destructor and then deletefunc to
// release the memory.

template <typename T>
REALLY_INLINE void MMgcDestructTaggedScalarChecked(T *mem) {
  if (mem) {
...
template <typename T>
REALLY_INLINE void MMgcDestructTaggedArrayChecked(T *mem) {
  if (mem) {
    mmgc_debug_only(
        MMgc::VerifyTaggedArray(mem, false, MMgc::kMMFXNewArrayPartition);)
...
// msvc 2008 treats char and (u)int8_t as the same but msvc 2010+, as well as
// gcc and armcc don't.
DECLARE_PRIM_ARRAY(char, MMgc::kMMFXNewPrimitiveNonPointerArrayPartition)
#elif !defined(_MSC_VER) && !defined(__SUNPRO_CC)
#error                                                                         \
    "Unknown compiler, check whether your compiler compiles with the above declaration, if it does you probably want it"
#endif
...
template <>
REALLY_INLINE void *MMgcConstructTaggedArray(void *, size_t count,
                                             MMgc::FixedMallocOpts opts) {
  return (void *)MMgc::NewTaggedArray(
      count, sizeof(void *), opts, true,
      MMgc::kMMFXNewPrimitivePointerArrayPartition);
}
...
// If a class has private destructor adding this macro will allow the delete
// mechanisms to call that.
#define MMGC_DECLARE_GLOBAL_DELETE_FNCS_AS_FRIEND(TYPE)                        \
...
// If a class is a base class for multiple inheritance or virtual inheritance,
// it may need to define these macros (MMGC_DECLARE_OPERATOR_DELETES_FOR_CLASS)
// inside the class definition to create a class specific delete, and
// MMGC_DECLARE_SPECIALIZED_DESTRUCTORCALL_TEMPLATES outside the class
// definition to have specialized function template for the destructor. This is
// done in cases where the pointer to be destructed is actually the pointer of a
// non-first base class.
#define MMGC_DECLARE_SPECIALIZED_DESTRUCTORCALL_TEMPLATES(TYPE)                \
...
#define MMGC_DECLARE_SPECIALIZED_DESTRUCTORCALL_TEMPLATE(TYPE)                 \
  template <> REALLY_INLINE void MMgcDestructTaggedScalarChecked(TYPE *mem) {  \
    if (mem)                                                                   \
      delete mem;                                                              \
  }

#define MMGC_DECLARE_SPECIALIZED_DESTRUCTORARRAYCALL_TEMPLATE(TYPE)            \
  template <> REALLY_INLINE void MMgcDestructTaggedArrayChecked(TYPE *mem) {   \
    if (mem)                                                                   \
      delete[] mem;                                                            \
  }

// These dont need to call destructors as the operator delete will do that
...
#define MMGC_DECLARE_VECTOR_DELETE_FOR_CLASS                                   \
  void operator delete[](void *p) {                                            \
    MMgc::DeleteTaggedArrayWithHeaderChecked(p, false,                         \
                                             MMgc::kMMFXNewArrayPartition);    \
  }

// If a class derives from multiple baseclasses that define delete operator,
// this can be used to select with one to use.
#define MMGC_USING_DELETE_OPERATOR(TYPE) using TYPE::operator delete
...
// These help to track where global operator new/delete/new[]/delete[] are used
// in compile time in MSVC. If you compile the project with /EHsc /Ob1 /W4 flags
// and compile. This will generate C4714 warnings (forceinline failed) for
// places where these operators are used. So far the try catch seems to be the
// way to do this, but for loops are supposed to generate that as well. May not
// be turned on on builds that are to be executed.
REALLY_INLINE void *operator new(size_t size) MMGC_NEW_THROWS_CLAUSE {
  (void)size;
  GCAssert(false);
