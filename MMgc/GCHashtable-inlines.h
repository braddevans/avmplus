...
GCHashtableBase<VAL, KEYHANDLER, ALLOCHANDLER>::GCHashtableBase(
    uint32_t capacity)
    : table(NULL), tableSize(capacity), numValues(0), numDeleted(0)
#ifdef GCDEBUG
      ,
      numIterators(0)
#endif
...
void GCHashtableBase<VAL, KEYHANDLER, ALLOCHANDLER>::clear() {
  ...
  numDeleted = 0;
#ifdef MMGC_GCHASHTABLE_PROFILER
  // Do *not* clear probes and accesses, those are for the lifetime of the
  // object (for better or worse)
#endif
}

template <typename VAL, class KEYHANDLER, class ALLOCHANDLER>
uint32_t GCHashtableBase<VAL, KEYHANDLER, ALLOCHANDLER>::find(
    const void *key, Entry *table, uint32_t tableSize) {
#ifdef MMGC_GCHASHTABLE_PROFILER
  accesses++;
#endif
...
template <typename VAL, class KEYHANDLER, class ALLOCHANDLER>
void GCHashtableBase<VAL, KEYHANDLER, ALLOCHANDLER>::put(const void *key,
                                                         VAL value) {
#ifdef MMGC_GCHASHTABLE_PROFILER
  accesses++;
#endif
  ...
  // we recycle it rather than adding to the end of the probe chain. this allows
  // us to recycle deleted slots MUCH more quickly (without having to wait for a
  // full rehash) and can dramatically reduce the average probe depth if there
  // are a lot of insertions and removals.
  const uint32_t NO_DELINDEX = 0xffffffff;
  uint32_t delindex = NO_DELINDEX;
  // Quadratic probe: Hashtable size (S) is power of two
  //                  Probe function used: p(K,i) = (i*i + i)/2  [i =
  //                  0,1,2....S,....] ith value in the probe sequence: [h(K) +
  //                  p(K,i)] % S
  uint32_t n = 0;
  uint32_t const bitMask = (tableSize - 1);
  uint32_t i = KEYHANDLER::hash(key) & bitMask;
  ...
  while ((k = table[i].key) != NULL && !KEYHANDLER::equal(k, key)) {
    ...
    if (k == DELETED && delindex == NO_DELINDEX)
      delindex = i;
    i = (i + (n += 1)) & bitMask; // quadratic probe
#ifdef MMGC_GCHASHTABLE_PROFILER
    probes++;
...
template <typename VAL, class KEYHANDLER, class ALLOCHANDLER>
VAL GCHashtableBase<VAL, KEYHANDLER, ALLOCHANDLER>::remove(const void *key,
                                                           bool allowRehash) {
  ...
  if (table[i].key == key) {
    ...
    table[i].value = 0;
    numDeleted++;
    // this helps a bit on pathologic memory profiler use case, needs more
    // investigation 20% deleted == rehash
...
void GCHashtableBase<VAL, KEYHANDLER, ALLOCHANDLER>::grow(bool isRemoval) {
  if (isRemoval) {
    ...
    // avoid it, and 'grow' is called when the collector clears its weak
    // references.
  ...
  Entry *newTable;
  newTable =
      (Entry *)ALLOCHANDLER::alloc(newTableSize * sizeof(Entry), isRemoval);
  if (!newTable)
    return;
...
template <typename VAL, class KEYHANDLER, class ALLOCHANDLER>
int32_t
GCHashtableBase<VAL, KEYHANDLER, ALLOCHANDLER>::nextIndex(int32_t index) {
...
template <typename VAL, class KEYHANDLER, class ALLOCHANDLER>
uint64_t GCHashtableBase<VAL, KEYHANDLER, ALLOCHANDLER>::bytesUsed() const {
  return ((uint64_t)tableSize) * sizeof(Entry);
}
} // namespace MMgc
