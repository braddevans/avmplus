...
REALLY_INLINE GCHeap *GC::GetGCHeap() const { return heap; }
...
REALLY_INLINE uint64_t GC::bytesMarked() { return policy.bytesMarked(); }
...
REALLY_INLINE GCAutoEnter *GC::GetAutoEnter() { return stackEnter; }
...
template <class T> REALLY_INLINE void GC::TraceLocation(T *const *loc) {
...
REALLY_INLINE void GC::TraceLocation(MMgc::WriteBarrier<T> const *loc) {
  TracePointer((void *)loc->value()
                   HEAP_GRAPH_ARG((uintptr_t *)loc->location()));
}
...
REALLY_INLINE size_t GC::GetNumBlocks() { return policy.blocksOwnedByGC(); }
