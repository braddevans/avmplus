...
bool GCTraceableBase::gcTrace(GC *gc, size_t cursor) {
  // The most-base tracer must do nothing, because it's the tracer that gets
  // called if the object becomes visible very early, or survives after
  // destruction because it was referenced from the mark stack (see
  // GC::AbortFree).
  (void)gc;
  (void)cursor;
  return false;
