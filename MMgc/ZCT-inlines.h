...
REALLY_INLINE RCObject **ZCT::PointerTo(uint32_t idx) {
  GCAssert(
      (slowState && idx < slowTopIndex) ||
      (!slowState && idx < topIndex)); // Fascistic compiler settings compel us
                                       // to add the inner parens here
  return blocktable[BlockNumber(idx)] + EntryNumber(idx);
}

REALLY_INLINE RCObject *ZCT::Get(uint32_t idx) { return *PointerTo(idx); }
