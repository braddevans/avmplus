...
#ifdef GCDEBUG
// we turn on exceptions in GCDEBUG builds
#pragma warning(                                                               \
    disable : 4291) // no matching operator delete found; memory will not be
...
public:
  RCObjectAddRefInDtor(RCObject **_stackPinners, int _length)
      : rcs(_stackPinners), length(_length) {}
