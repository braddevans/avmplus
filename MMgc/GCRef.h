...
//  Define a member that's unprotected by reference counting or write barriers.
//  Generally speaking this is a dangerous practice that should be used with
//  extreme caution.  Please use "GCMember" to define garbage collected members
//  of a class.
#define GCREF_UNPROTECTEDMEMBER(type) GCRef<type>

//  Unions do not allow objects that have complex constructors, so unions should
//  always be considered "unprotected members". Similar to the warning above,
//  avoid using this macro wherever possible by NOT using unions that contain GC
//  values as members of a class.
#define GCREF_UNPROTECTEDMEMBER_UNION(type) GCRef_Union<type>

//  These Macros should be used when casting a GCRef or GCMember to or from
//  "void". This will assist future efforts to remove implicit casting between
//  GCRef/GCMember and raw pointers.
...
namespace avmplus {
template <class T2> class GCList;
}

namespace MMgc {
//  The MMgc::GCAPI namespace will be globally open to improve readability of
//  these commonly used utilities
namespace GCAPI {
...
protected:
  //  Allow GCRef subclasses to have access to the Raw GC pointer of any other
  //  GCRef
  template <class T2>
...
public:
  ...
  //  Allow assignment to T*(or T<derived>*) for implicit conversion from raw
  //  pointers
  REALLY_INLINE GCRef &operator=(T *tNew);

  //  Construct from T* (or T<derived>*) for implicit conversion from raw
  //  pointers
  REALLY_INLINE GCRef(T *valuePtr) : t(valuePtr) {}

  //  Copy the value of one GCRef from another GCRef
  template <class T1, class T2>
  static void memcpy(GCRef<T1> &dst, const GCRef<T2> &src, size_t size) {
    ::memcpy(dst.t, src.t, size);
  }
  ...
  template <class T2> REALLY_INLINE GCRef &operator=(const GCRef<T2> &other);

  //  Allow Any GCRef<derived> to be cast to a GCRef<base>
  template <class T2> REALLY_INLINE operator GCRef<T2>() const {
    GCRef<T2> temp;
    temp.t = t;
    return temp;
  }

  //  staticCast<T2> implements standard C++ "static_cast" for casting one
  //  GCRef<T> to another GCRef<T2> of defined base or derived type
  template <class T2> REALLY_INLINE GCRef<T2> staticCast() const {
    GCRef<T2> temp;
    temp.t = static_cast<T2 *>(t);
    return temp;
  }

  //  reinterpretCast<T2> implements standard C++ "reinterpret_cast" for casting
  //  one GCRef<T> to another GCRef<T2> arbitrarily
  template <class T2> REALLY_INLINE GCRef<T2> reinterpretCast() const {
    GCRef<T2> temp;
    temp.t = reinterpret_cast<T2 *>(t);
    return temp;
  }

  REALLY_INLINE T &operator*() { return *t; }
...
//  The GCRef_Union class should only be used in unions where members may not
//  have constructors defined.  This class is designed only to bridge union
//  members as closely to possible to GCRef's.  Note: Unions that define
//  GCRef_Union members ARE NOT protected by WriteBarriers or Reference
//  counting.  Use of this class is strongly discouraged.
template <class T> class GCRef_Union {
public:
