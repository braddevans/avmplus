...
REALLY_INLINE void GC::WriteField(const void *address, const void *value) {
  ...
  GC *gc = GetActiveGC();
  GCAssertMsg(gc != NULL,
              "Illegal for a GC write to occur when the GC isn't entered");
  GCAssert(gc->IsPointerIntoGCObject(address));
  GCAssert(Pointer(value) == NULL ||
           gc->IsPointerToGCObject(GetRealPointer(Pointer(value))));
...
/*static private*/
REALLY_INLINE void GC::InlineWriteBarrier(const void *address,
                                          const void *value) {
  if (Pointer(value) != NULL)
...
/*static private*/
REALLY_INLINE void GC::InlineWriteBarrierRC(const void *address,
                                            const void *value) {
  if (Pointer(value) != NULL)
    GetGC(value)->InlineWriteBarrierGuardedTrap(address);
...
/*static private*/
REALLY_INLINE void GC::InlineWriteBarrierRC_ctor(const void *address,
                                                 const void *value) {
...
REALLY_INLINE void GC::InlineWriteBarrierTrap(const void *container) {
  ...
  // something into it.
  //
  // Testing shows that it's /sometimes/ useful to check the right hand side of
  // the assignment for NULL, but this depends on the program and for the time
  // being the right hand side isn't available here and isn't checked.
...
REALLY_INLINE void GC::privateInlineWriteBarrier(const void *container,
                                                 const void *address,
                                                 const void *value) {
...
REALLY_INLINE void GC::privateInlineWriteBarrierRC(const void *container,
                                                   const void *address,
                                                   const void *value) {
  GCAssert(container != NULL);
  GCAssert(IsPointerToGCObject(GetRealPointer(container)));
...
REALLY_INLINE void GC::privateInlineWriteBarrier(const void *address,
                                                 const void *value) {
  InlineWriteBarrierGuardedTrap(address);
  WriteFieldNonRC(address, value);
}

REALLY_INLINE void GC::privateInlineWriteBarrierRC(const void *address,
                                                   const void *value) {
  InlineWriteBarrierGuardedTrap(address);
  WriteFieldRC(address, value);
}

REALLY_INLINE void GC::ConservativeWriteBarrierNoSubstitute(const void *address,
                                                            const void *value) {
...
template <class T> REALLY_INLINE WriteBarrier<T>::WriteBarrier() : t(0) {
#ifdef GCDEBUG
  GC::TracePointerCheck(&t);
#endif
}

template <class T> REALLY_INLINE WriteBarrier<T>::WriteBarrier(T _t) {
#ifdef GCDEBUG
  GC::TracePointerCheck(&t);
#endif
  set(_t);
...
// Always pay for a single real function call; then inline & optimize massively
// in WriteBarrier()
template <class T> REALLY_INLINE T WriteBarrier<T>::set(const T tNew) {
...
// Always pay for a single real function call; then inline & optimize massively
// in WriteBarrierRC()
template <class T> REALLY_INLINE T WriteBarrierRC<T>::set(const T tNew) {
  GC::WriteBarrierRC(&t, (const void *)tNew); // updates 't'
  return tNew;
}

template <class T> REALLY_INLINE WriteBarrierRC<T>::WriteBarrierRC() : t(0) {
#ifdef GCDEBUG
  GC::TracePointerCheck(&t);
#endif
}
...
REALLY_INLINE WriteBarrierRC<T>::WriteBarrierRC(
    const T _t) // : t(0) -- not necessary, WriteBarrierRC_ctor handles it
{
  GC::WriteBarrierRC_ctor(&t, (const void *)_t);
}

template <class T> REALLY_INLINE WriteBarrierRC<T>::~WriteBarrierRC() {
  GC::WriteBarrierRC_dtor(&t);
}

template <class T>
REALLY_INLINE void WriteBarrierRC<T>::set(MMgc::GC *gc, void *container,
                                          T newValue) {
