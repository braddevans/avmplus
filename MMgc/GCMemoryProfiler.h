...
const int kMaxStackTrace =
    16; // RtlCaptureStackBackTrace stops working when this is 32
...
public:
  StackTrace(uintptr_t *trace)
      : size(0), totalSize(0), sweepSize(0), firstNonMMgcFrame(-1),
        package(NULL), category(NULL), name(NULL), count(0), totalCount(0),
        sweepCount(0), master(NULL), skip(0), managed(false),
...
#define MMGC_MEM_TAG(_x)                                                       \
  if (MMgc::GCHeap::GetGCHeap()->HooksEnabled())                               \
  MMgc::SetMemTag(_x)
...
typedef GCHashtableBase<const void *, GCStackTraceHashtableKeyHandler,
                        GCHashtableAllocHandler_VMPI>
    GCStackTraceHashtable_VMPI;

template <class StackTraceHandler>
class ObjectPopulationProfiler : public GCAllocObject {
public:
  enum DumpMode { BY_VOLUME, BY_COUNT };

  // If 'roots' is true then root info is collected and printed, otherwise
  // accountForRoot() should not be called.  Ditto for 'stacks' and
  // accountForStack().
  ObjectPopulationProfiler(GC *gc, const char *profileName, bool roots = true,
                           bool stacks = true);
  ...
  typedef GCHashtableBase<PopulationNode *, StackTraceHandler,
                          GCHashtableAllocHandler_VMPI>
      NodeTable;
...
protected:
  ...
  //
  // For the ObjectPopulationProfiler it's the number of bytes and objects
  // scanned.
  virtual void dumpObjectInfo(unsigned long long numbytes, unsigned numobjects);

  // Query whether this address at the bottom of the trace should be skipped
  // during trace printing (it may follow other skipped addresses).  This is
  // useful for filtering out info about MMgc, for example.
  //
  // Skip MMgc addresses
...
class WeakRefAllocationSiteProfiler
    : public ObjectPopulationProfiler<AllocationSiteHandler> {
public:
...
public:
  ...
  inline static int GetSignificance(StackTrace *) { return kMaxStackTrace; }
};
...
public:
  ...
  enum SortMode { BY_VOLUME, BY_COUNT };
...
private:
  ...
  // Note: it's important to use the VMPI variant of GCHashtable for all of
  // these.
...
// more about that, bugzilla 697672 for a discussion, and comments on
// GCAlloc::Alloc for an interesting invariant that depends on
// USER_POINTER_WORDS.
#ifdef VMCFG_FLOAT
...
/**
 * decorate memory with debug information, return pointer to memory to return to
 * caller
 */
void DebugDecorate(const void *item, size_t size);

/**
 * Given a pointer to user memory do debug checks and return pointer to real
 * memory
 */
 ...
 */
inline void *GetRealPointer(const void *item) {
  return (void *)((uintptr_t)item - USER_POINTER_WORDS * sizeof(int32_t));
}

/**
 * Given a real pointer return pointer to user memory
 */
inline void *GetUserPointer(const void *item) {
  return (void *)((uintptr_t)item + USER_POINTER_WORDS * sizeof(int32_t));
}
...
#if USER_POINTER_WORDS != 0 && USER_POINTER_WORDS != 2 &&                      \
    USER_POINTER_WORDS != 4
