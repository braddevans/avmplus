...
//
// where 'fn' takes a single void* and will be passed 'arg'.  While 'fn' is
// active the stack and registers active at the time CreateRootFromCurrentStack
// was called will be a GC root.

// Enable our own alloca() replacement that always allocates in the heap, this
// is good on systems with limited memory or limited stack
#define AVMPLUS_PARAM_ALLOCA_CUTOFF                                            \
  4000 // Don't make real alloca() blow the stack; this limit is heuristic
#define AVMPLUS_PARAM_ALLOCA_DEFSIZE                                           \
  1000 // Default number of bytes in a stack segment for heap-based alloca()
#ifdef AVMPLUS_HEAP_ALLOCA
#define avmStackAlloc(core, autoptr, nbytes)                                   \
  core->gc->allocaPush(nbytes, autoptr)
#define avmStackAllocGC(gc, autoptr, nbytes) gc->allocaPush(nbytes, autoptr)
#else
#define avmStackAlloc(core, autoptr, nbytes)                                   \
  (nbytes > AVMPLUS_PARAM_ALLOCA_CUTOFF                                        \
       ? core->gc->allocaPush(nbytes, autoptr)                                 \
       : alloca(nbytes))
#define avmStackAllocGC(gc, autoptr, nbytes)                                   \
  (nbytes > AVMPLUS_PARAM_ALLOCA_CUTOFF ? gc->allocaPush(nbytes, autoptr)      \
                                        : alloca(nbytes))
#endif
#define avmStackAllocArray(core, autoptr, nelem, elemsize)                     \
  avmStackAlloc(core, autoptr,                                                 \
                MMgc::GCHeap::CheckForCallocSizeOverflow(nelem, elemsize))
#define avmStackAllocArrayGC(gc, autoptr, nelem, elemsize)                     \
  avmStackAllocGC(gc, autoptr,                                                 \
                  MMgc::GCHeap::CheckForCallocSizeOverflow(nelem, elemsize))

// For backwards compatibility with the Flash Player until we can rename its
// uses of the VMPI_ macros
...
// class.
#define GC_AS3_EXACT(cls, basecls)                                             \
  cls:                                                                         \
public                                                                         \
  basecls
#define GC_AS3_EXACT_IFDEF(cls, basecls, cond)                                 \
  cls:                                                                         \
public                                                                         \
  basecls
#define GC_AS3_EXACT_IFNDEF(cls, basecls, cond)                                \
  cls:                                                                         \
public                                                                         \
  basecls
#define GC_AS3_EXACT_IF(cls, basecls, cond)                                    \
  cls:                                                                         \
public                                                                         \
  basecls
#define GC_AS3_EXACT_WITH_HOOK(cls, basecls)                                   \
  cls:                                                                         \
public                                                                         \
  basecls
#define GC_AS3_EXACT_WITH_HOOK_IFDEF(cls, basecls, cond)                       \
  cls:                                                                         \
public                                                                         \
  basecls
#define GC_AS3_EXACT_WITH_HOOK_IFNDEF(cls, basecls, cond)                      \
  cls:                                                                         \
public                                                                         \
  basecls
#define GC_AS3_EXACT_WITH_HOOK_IF(cls, basecls, cond)                          \
  cls:                                                                         \
public                                                                         \
  basecls
...
#define GC_CPP_EXACT(cls, basecls)                                             \
  cls:                                                                         \
public                                                                         \
  basecls
#define GC_CPP_EXACT_IFDEF(cls, basecls, cond)                                 \
  cls:                                                                         \
public                                                                         \
  basecls
#define GC_CPP_EXACT_IFNDEF(cls, basecls, cond)                                \
  cls:                                                                         \
public                                                                         \
  basecls
#define GC_CPP_EXACT_IF(cls, basecls, cond)                                    \
  cls:                                                                         \
public                                                                         \
  basecls
#define GC_CPP_EXACT_WITH_HOOK(cls, basecls)                                   \
  cls:                                                                         \
public                                                                         \
  basecls
#define GC_CPP_EXACT_WITH_HOOK_IFDEF(cls, basecls, cond)                       \
  cls:                                                                         \
public                                                                         \
  basecls
#define GC_CPP_EXACT_WITH_HOOK_IFNDEF(cls, basecls, cond)                      \
  cls:                                                                         \
public                                                                         \
  basecls
#define GC_CPP_EXACT_WITH_HOOK_IF(cls, basecls, cond)                          \
  cls:                                                                         \
public                                                                         \
  basecls
...
// Substructure fields: the field must provide a method 'void gcTrace(MMgc::GC*
// gc)' that is typically inline.  All List<> fields are substructure fields,
// and the List<> templates provide the tracing functions.
...
#define GC_POINTERS_SMALL(field_and_size, count)                               \
  field_and_size // note, this is a hint

#define GC_ATOMS(field_and_size, count) field_and_size
#define GC_ATOMS_SMALL(field_and_size, count)                                  \
  field_and_size // note, this is a hint
...
private:                                                                       \
  static const uint32_t gcTracePointerOffsets[];                               \
                                                                               \
public:                                                                        \
  /*sometimes virtual*/ MMgc::GCTracerCheckResult gcTraceOffsetIsTraced(       \
      uint32_t) const;                                                         \
...
public:
  ...
  /**
   * The return value and both arguments are technically unnecessary, but by
   * using this signature the GCRoot tracers will be trivially compatible with
   * the tracer generator system; roots can therefore be annotated in the usual
   * manner and tracers for roots can be generated.
   */
  virtual bool gcTrace(GC *gc, size_t cursor = 0);

  /**
   * Special-purpose constructor.  Provided in order to allow non-FixedMalloc
   * memory to be root memory (used by AIR/APE).
   *
   * If you're about to use this constructor please consider carefully whether
   * you should be using the StackMemory subclass instead.
   */
  GCRoot(GC *gc, const void *object, size_t size);

  virtual ~GCRoot();

  // override new and delete so we can know the objects extents (via
  // FixedMalloc::Size())
  ...
  bool IsExactlyTraced() const {
    return (size & kIsExactlyTraced) ? true : false;
  }
  ...
  //  GCRoot's GCMember definition, no WriteBarrier but handle ref count
  //  updates.
  template <class T> class GCMember : public GCRef<T> {
  private:
    //  'set' method handles logic whenever the GC pointer value changes
    void set(T *tNew);

    // The AvmCore code uses an invalid pointer (AVMPLUS_STRING_DELETED) for the
    // intern table, whose value is '1'.
    bool valid();
  ...
  public:
    ...
    template <class T2> explicit GCMember(const GCRef<T2> &other);

    explicit GCMember(T *valuePtr);

    //  Make sure to decrement the refcount on RCObjects when this GCMember is
    //  destroyed
    ~GCMember();
...
private:
  ...
  GCRoot *next;
  GCRoot *prev;
  const void *object; // High w-2 bits: the pointer.  Bit 1: available.  Bit 0:
                      // available.
  size_t size; // High w-2 bits: the size in bytes.  Bit 1: isExactlyTraced
               // flag.   Bit 0: isStackMemory flag.
  uintptr_t markStackSentinel; // If not 0 this is the index on the mark stack
                               // that holds the RootProtector for this root.

  void GetConservativeWorkItem(const void *&_object, uint32_t &_size,
...
public:
  ...
  StackMemory(GC *gc, const void *object, size_t size,
              bool isExactlyTraced = false);

  /**
   * Return the value of the stack top in the range [0,size].  The GC will scan
   * the memory in the range [object,object+top).  The default implementation
   * just returns "size".
   */
  virtual size_t Top();

  /** Reinitialize with this memory instead of the old memory.  To clear use
   * Set(NULL,0). */
...
template <class T> class GCHiddenPointer {
public:
...
/**
 * GCObjectLock is abstract and instances of it are returned from
 * GC::LockObject.
 */
class GCObjectLock;
...
class GC : public OOMCallback {
  ...
  friend class avmplus::ST_mmgc_basics::ST_mmgc_basics;
#endif
  friend class avmplus::Traits; // We may be able to throttle back on this by
                                // making TracePointer visible, but OK for now
public:
  /**
  ...
  /**
   * ForceSweepAtShutdown allows clients to force all finalization to occur
   * first and then delete the GC later.  ~GC will still call this just in case.
   * See bugzilla 636376.
   */
   ...
   * incremental collection underway). See comments above the Collect(double)
   * method.
   */
   ...
   *
   * The parameter allocationBudgetFractionUsed will be pinned to the range
   * [0.25,1].
   *
   ...
   * this call is used to help schedule GC pauses but should otherwise not
   * impact GC policy more than absolutely necessary.
   */
  ...
  enum AllocFlags {
    ...
    kRCObject = 8,
    kInternalExact =
        16, // INTERNAL USE.  This must match kVirtualGCTrace in GCAlloc.h
    kCanFail = 32
  };
  ...
  /**
   * Specialized implementations of Alloc().  Flags are omitted, each function
   * is annotated with the flags they assume.   Additionally, 'size' is usually
   * known statically in the calling context (typically a 'new' operator).
   * Finally, these are always inlined. The result is that most computation
   * boils away and we're left with just a call to the underlying primitive
   * operator.
   */
  void *AllocPtrZero(size_t size,
                     int partition); // Flags: GC::kContainsPointers|GC::kZero
  void *AllocPtrZeroExact(
      size_t size,
      int partition); // Flags:
                      // GC::kContainsPointers|GC::kZero|GC::kInternalExact
  void *AllocPtrZeroFinalized(
      size_t size,
      int partition); // Flags: GC::kContainsPointers|GC::kZero|GC::kFinalize
  void *AllocPtrZeroFinalizedExact(
      size_t size,
      int partition); // Flags:
                      // GC::kContainsPointers|GC::kZero|GC::kFinalize|GC::kInternalExact
  void *AllocRCObject(
      size_t size,
      int partition); // Flags:
                      // GC::kContainsPointers|GC::kZero|GC::kRCObject|GC::kFinalize
  void *AllocRCObjectExact(
      size_t size,
      int partition); // Flags:
                      // GC::kContainsPointers|GC::kZero|GC::kRCObject|GC::kFinalize|GC::kInternalExact

  /**
   * Like Alloc but optimized for the case of allocating one 8-byte
   * non-pointer-containing non-finalizable non-rc non-zeroed object (a box for
   * an IEEE double).
   */
  void *AllocDouble();
...
#define AllocFloat4 AllocBibopType<avmplus::AtomConstants::kBibopFloat4Type>
   ...
   */
  void *
  AllocExtraPtrZero(size_t size, size_t extra,
                    int partition); // Flags: GC::kContainsPointers|GC::kZero
  void *AllocExtraPtrZeroExact(
      size_t size, size_t extra,
      int partition); // Flags:
                      // GC::kContainsPointers|GC::kZero|GC::kInternalExact
  void *AllocExtraPtrZeroFinalized(
      size_t size, size_t extra,
      int partition); // Flags: GC::kContainsPointers|GC::kZero|GC::kFinalize
  void *AllocExtraPtrZeroFinalizedExact(
      size_t size, size_t extra,
      int partition); // Flags:
                      // GC::kContainsPointers|GC::kZero|GC::kFinalize|GC::kInternalExact
  void *AllocExtraRCObject(
      size_t size, size_t extra,
      int partition); // Flags:
                      // GC::kContainsPointers|GC::kZero|GC::kRCObject|GC::kFinalize
  void *AllocExtraRCObjectExact(
      size_t size, size_t extra,
      int partition); // Flags:
                      // GC::kContainsPointers|GC::kZero|GC::kRCObject|GC::kFinalize|GC::kInternalExact
  ...
  /**
   * Signal that client code has performed an allocation from memory known not
   * to be controlled by this GC, and wants this memory accounted for because
   * its lifetime is tied to objects that this GC controls.  (A typical case is
   * when fixed or system memory is tied to a host object, eg BitmapData in
   * Flash Player.)
   *
   * The GC may use this information to drive garbage collection, and may
   * perform GC activity during this call.
   *
   * @param type is the type of the dependent memory
   */
  void SignalDependentAllocation(size_t nbytes,
                                 DependentMemoryType type = typeUnknown);

  /**
   * Signal that client code has performed a deallocation of memory known not to
   * be controlled by this GC, and wants this memory accounted for.
   *
   * The GC may use this information to drive garbage collection, and may
   * perform GC activity during this call.
   *
   * It is crucial that SignalDependentDeallocation is called only to account
   * for objects previously accounted for by SignalDependentAllocation, and that
   * it is called for all objects accounted for by SignalDependentAllocation as
   * those objects are being destroyed.  The GC verifies neither of those
   * conditions.
   *
   * @param type is the type of the dependent memory
   *
  ...
  /**
   * Move a subarray of pointers to managed objects from within one managed
   * object to within another managed object, with efficient handling of write
   * barriers.  The source and destination objects can be the same object.
   *
   * @param dstObject is a pointer to the base of the destination object (as
   * returned from GC::Alloc, say)
   * @param dstArray is the pointer to the base of the destination slot array
   * within dstObject
   * @param dstOffset is the offset within dstArray, in number of pointers, of
   * the first destination slot
   * @param srcArray is the pointer to the base of the source slot array within
   * some unspecified source object
   * @param srcOffset is the offset within srcArray, in number of pointers, of
   * the first source slot
   * @param numPointers is the number of pointers to move.
   */
  void movePointers(void *dstObject, void **dstArray, uint32_t dstOffset,
   ...
   *
   * NOTE that the units of offsets are bytes, not pointers, allowing for a
   * pointer array that is offset from the start of an object.
   *
   * FIXME: Bugzilla 615193 This method is misnamed, "Block" has a specific
   * meaning within MMgc and "managed object" ain't it.
   *
   * @param array is some base address inside the managed object
   * @param dstOffsetInBytes is the offset in bytes from array at which to start
   * writing pointers
   * @param srcOffsetInBytes is the offset in bytes from array at which to start
   * reading poitners
   * @param numPointers is the number of pointers to move
   * @param zeroEmptied is true if the space that is emptied (the top area if
   * moving down, the bottom area if moving up) should be zeroed.  Only pass
   * false for this parameter if you know that that will not create reference
   * counting or false retention bugs.
   */
  void movePointersWithinBlock(void **array, uint32_t dstOffsetInBytes,
                               uint32_t srcOffsetInBytes, size_t numPointers,
                               bool zeroEmptied = true);

  /**
   * Given a a GC memory block, reverse the order of numPointers worth of
   * pointers (or pointer-sized data), starting at offsetInBytes from mem.
   *
   * FIXME: Bugzilla 615193: This method is misnamed, "Block" has a specific
   * meaning within MMgc and "managed object" ain't it.
   */
  void reversePointersWithinBlock(void *mem, size_t offsetInBytes,
                                  size_t numPointers);
...
#endif
  /**
   * Signal that we've allocated some memory and that collection can be
   * triggered if necessary.
   */
  void SignalAllocWork(size_t size);

  /**
   * Signal that we've freed some memory.  This should not have any side
   * effects, ie, it should not trigger collection.
   */
...
public:
  ...
  /**
   * Save the processor context on the stack, make a GC root from the stack, and
   * call 'fn' with 'arg' as the only argument.  The GC root is active during
   * the invocation of 'fn'.
   *
   ...
   * this item must be a small item (<1968) or the first page of a large
   * allocation, it won't work on secondary pages of a large allocation.
   */
  ...
  /**
   * @return the queued bit for the given object: zero if not set, nonzero if
   * set.
   */
...
public:
   ...
   */
  void *AllocBlock(int size, int partitition, PageMap::PageType pageType,
                   bool zero = true, bool canFail = false);

  void FreeBlock(void *ptr, uint32_t size, int partition);

  // Host API: this should go away but Player code is currently using it.  VM
  ...
  /**
   * Perform some garbage collection work: the allocation budget has been
   * exhausted.
   */
  void CollectionWork();
  ...
  /**
   * Is the write barrier active?  If true, then pointer stores *must* be
   * filtered through the write barrier (see the "write barrier" section
   * directly below for the relevant APIs).
   *
   * NOTE: In the past this has been equivalent to IncrementalMarking(), or even
   * to reading the "marking" member, but in general BarrierActive() is
   * different from those two methods (generational collectors' barriers are
   * always active, for example).
   */
  bool BarrierActive();

  // The following APIs are really private and are shared only with specific
  // code that's not in the GC class.  Alas C++ does not provide a good solution
  // for expressing that, so they're public.
  //
...
public:
  // *** PRIVATE TO MMGC ***
  //
  // To be called from delete operators in GCObject / GCFinalizedObject / etc.
  // Does not check that the object is not finalizable.  Separate from
  // FreeFromGCNotNull to allow profiling / diagnosis of explicit deletion.

  void FreeFromDelete(const void *ptr);

private:
  // To be called from the GC/ZCT.  Does not check that the object is not
  // finalizable.
  ...
  static void FASTCALL WriteBarrierRC_ctor(const void *address,
                                           const void *value);
...
private:
  /**
   * The marker is running and the container object has just been transitioned
   * from 'marked' to 'queued'.
   */
...
private:
   ...
   */
  /*REALLY_INLINE*/ void privateInlineWriteBarrier(const void *container,
                                                   const void *address,
                                                   const void *value);

  /**
   * Like the preceding but computes 'container' only if 'marking' is true
   * (important optimization).
   */
  /*REALLY_INLINE*/ void privateInlineWriteBarrier(const void *address,
                                                   const void *value);

  /**
   * Implementation of privateWriteBarrierRC; too large to be inlined
   * everywhere.
   */
  /*REALLY_INLINE*/ void privateInlineWriteBarrierRC(const void *container,
                                                     const void *address,
                                                     const void *value);
   ...
   * (important optimization).
   */
  /*REALLY_INLINE*/ void privateInlineWriteBarrierRC(const void *address,
                                                     const void *value);

  /**
   * Out-of-line implementation of ConservativeWriteBarrierNoSubstitute, called
   * only when 'marking' is true.
   */
  void privateConservativeWriteBarrierNoSubstitute(const void *address);

public:
  /**
   * General, conditional write barrier trap.  Tests that incremental marking is
   * in fact ongoing and that the container is black (marked, not queued); if
   * so, it makes the container gray by scheduling it for rescanning at some
   * appropriate time.
   *
   * Container must be a non-NULL untagged pointer to the beginning of an object
   * on a page owned by this GC.
   */
  void WriteBarrierTrap(const void *container);

  /**
   * Inline implementation of WriteBarrierTrap; probably too large to be inlined
   * everywhere so use sparingly.  AvmCore::atomWriteBarrier benefits
   * substantially from inlining it.
   *
  ...
  /**
   * Standard write barrier write for non-RC values.  If marking is ongoing, and
   * the 'container' is black (ie marked and not queued) and the 'value' is
   * white (ie unmarked and not queued) then make sure to queue value for
   * marking.
   *
   * Finally stores value into *address.
   *
   * 'container' may be NULL, in which case this is just a store and 'address'
   * can be arbitrary.
   *
   * If 'container' is not NULL then it is constrained as for WriteBarrierTrap,
   * and 'address' must point into the object referenced by 'container'.
   *
   * Value can be anything except a reference to an RCObject.
   *
   * This is called by the WB macro in WriteBarrier.h - not an API to be used
   * otherwise.
   */
  void privateWriteBarrier(const void *container, const void *address,
                           const void *value);

  /**
   * Standard write barrier write for RC values.  If marking is ongoing, and the
   * 'container' is black (ie marked and not queued) and the 'value' is white
   * (ie unmarked and not queued) then make sure to queue value for marking.
   *
   * Finally stores value into *address, adjusting the reference counts of both
   * the old value and the new value.
   *
   * 'container' is constrained as for WriteBarrierTrap.
   *
   * 'address' must point into the object referenced by 'container'.  The value
   * there, if not NULL, must be a (possibly tagged) pointer to an RC object.
   *
   * 'value', if not NULL, must point to an RC object owned by this GC.
   *
   * This is called by the WBRC macro in WriteBarrier.h - not an API to be used
   * otherwise.
   */
  void privateWriteBarrierRC(const void *container, const void *address,
                             const void *value);

  /**
   * Host API: if 'address' points to a GC page (it can point into an object,
   * not just to the start of one), and 'value' points to a GC page as well,
   * record that 'value' has been stored into the object containing 'address'.
   *
   * The function does /not/ perform the store (this is what NoSubstitute
   * means).
   *
   * The function works for both RCObjects and GCObjects.
   */
  void ConservativeWriteBarrierNoSubstitute(const void *address,
                                            const void *value);

  /**
   * Host API: 'container' must be a non-NULL untagged pointer to the beginning
   * of an object on a page owned by this GC.  Record that 'value' has been
   * stored into 'container'.
   *
   * The function does /not/ perform the store (this is what NoSubstitute
   * means).
   *
...
public:
  /**
   * @return true if the gc item contains pointers.  Item must be valid GC
   * pointer.
   */
  ...
  // Used on the critical path of the write barrier path.  gcItem must point
  // into managed memory, and NULL is never returned.  Don't use this for
  // GCAssert and similar purposes, but use it when speed is key.
  void *FindBeginningFast(const void *gcItem);

  /**
   * @return true if the useritem is an RCObject.  Item must be valid GC
   * pointer.
   */
  ...
  // FIXME: these three static methods could be lifted out of the GC class to
  // the global level, they're simple utilities.
  ...
  // 'obj' is a possibly-null pointer to an object that may or may not be marked
  // or queued and large or small and may or may not contain pointers.  The
  // object will be marked (if not pointer-containing) or pushed onto the mark
  // queue (if pointer-containing) if not already marked or queued.
  template <class T> void TraceLocation(T *const *loc);

  template <class T> void TraceLocation(MMgc::WriteBarrier<T> const *loc);

  template <class T> void TraceLocation(MMgc::WriteBarrierRC<T> const *loc);

  template <class T> void TraceLocation(MMgc::GCMemberBase<T> const *loc);

  void TraceAtom(AtomWBCore *loc);

  // ditto, but the pointer at *loc may have tags or other information in the
  // low three bits that we will discard.
  void TraceLocation(uintptr_t *loc);

  // ditto, but the pointer at *loc will have tags in the low three bits that we
  // will use to direct interpretation of the value.  (This should be called
  // TraceLocation but as long as Atom is an uintptr_t C++ gets in the way.)
  void TraceAtom(avmplus::Atom *loc);

  // Let the contents at *loc be 'val'.
  // 'val' is some value that may be a pointer.  It may or may not be NULL.  It
  // may or may not carry a tag.  It may or may not point to GC'd storage.  If
  // it points to GC'd storage then it may or may not point to an object (as
  // opposed to into one, or into free space). If it is a pointer to an object
  // then that object may or may not already be marked or queued and large or
  // small.  If the object is neither marked nor queued it is pushed onto the
  // mark queue (if it contains pointers) or marked (if it does not contain
  // pointers).
  ...
  // Trace an array of pointers as if by TraceLocation(void**).  Does not bounds
  // check.
  template <class T> void TraceLocations(T **p, size_t numobjects);

  // Trace an array of possibly-tagged pointer values as if by
  // TraceLocation(uintptr_t*).  Does not bounds check.
  void TraceLocations(uintptr_t *p, size_t numobjects);

  // Trace an array of tagged values as if by TraceLocation(Atom*).  Does not
  // bounds check. (This should be called TraceLocation but as long as Atom is
  // an uintptr_t C++ gets in the way.)
...
private:
  ...
  // small.  If the object is neither marked nor queued it is pushed onto the
  // mark queue (if it contains pointers) or marked (if it does not contain
  // pointers).  If 'handleInteriorPtrs' is true then the pointer is recognized
  // as a pointer even if it points into an object. 'loc' is the location within
  // the containing object from which 'val' was read.
  void TraceConservativePointer(
      uintptr_t val, bool handleInteriorPtrs HEAP_GRAPH_ARG(uintptr_t *loc));

  // 'obj' is a possibly-null pointer to an object that may or may not be marked
  // or queued and large or small and may or may not contain pointers.  'loc' is
  // the location from which 'obj' was read.  The object will be marked (if not
  // pointer-containing) or pushed onto the mark queue (if pointer-containing)
  // if not already marked or queued.
  void TracePointer(
...
private:
  // heapAlloc is like heap->Alloc except that it also calls
  // policy.signalBlockAllocation if the allocation succeeded.
  void *heapAlloc(size_t size, int partition, int flags = GCHeap::flags_Alloc);

  // heapFree is like heap->Free except that it also calls
  // policy.signalBlockDeallocation.
  ...
  // preserved stack base for previous thread before a threadEnter
  uintptr_t m_stackBaseSaved;

  // Will be called from GCHeap::AbortCleanup before a longjmp that jumps past
  // the GC (effectively aborting an operation, be it allocation or GC).  This
  // function must be aware of the internal state of the GC and must take care
  // to make that state consistent.
  ...
  // reference in the table points to a marked object and is itself unmarked
  // then mark it.
  ...
  /**
   * Nonzero if IncrementalMark is currently active.  This is more specific than
   * 'marking'; Collect() uses this to protect itself from recursive calls
   * during OOM handling. (Those calls should not happen because the only
   * allocation to happen during marking is the mark stack and it should be
   * careful not to invoke OOM handling, but debugging code and other ad-hoc
   * code may interfere.  This state variable is cheap insurance.)
   *
   * The flag is a counter because some functions call both Mark and MarkItem_*,
   * and then Mark also calls MarkItem_*, so the flag may needs to be set and
   * reset on several levels. The counter takes care of this naturally.
   */
  ...
  void FinishIncrementalMark(bool scanNativeStack,
                             bool okToShrinkHeapTarget = true);
  ...
  // Set at initialization to the deepest recursion level MarkItem_* is allowed
  // to reach.  Managed entirely within MarkItem.
  ...
  // These two members help optimize GC::Alloc: by keeping a pointer in the GC
  // instance for the kSizeClassIndex table we avoid code generation SNAFUs when
  // compiling with -fPIC, which is the default on Mac at least.  (GCC generates
  // a call-to-next-instruction-and-pop to obtain the PC address, from which it
  // can compute the table address.  Keeping the member here effectively hoists
  // that computation out of the allocator.)  And by keeping a lookup table of
  // allocators indexed by the flag bits of interest we avoid a decision tree
  // inside GC::Alloc.
  ...
  GCAlloc *containsPointersNonfinalizedAllocs
      [kNumSizeClasses]
      [kNumGCPartitions]; // Non-RC non-finalized objects containing pointers
  GCAlloc *containsPointersFinalizedAllocs
      [kNumSizeClasses]
      [kNumGCPartitions]; // Non-RC finalized objects containing pointers
  GCAlloc *containsPointersRCAllocs[kNumSizeClasses]
                                   [kNumGCPartitions]; // RC finalized objects
                                                       // containing pointers
  GCAlloc *noPointersNonfinalizedAllocs
      [kNumSizeClasses][kNumGCPartitions]; // Non-RC non-finalized objects not
                                           // containing pointers
  GCAlloc
      *noPointersFinalizedAllocs[kNumSizeClasses]
                                [kNumGCPartitions]; // Non-RC finalized objects
                                                    // not containing pointers
...
private:
  ...
  void MarkItem_ConservativeOrNonGCObject(const void *object, uint32_t size,
                                          GCMarkStack::TypeTag type,
                                          const void *baseptr,
                                          bool interiorPtrs);
  void SplitExactGCObject(const void *object);
  void SplitItem_ConservativeOrNonGCObject(const void *object, uint32_t &size,
                                           GCMarkStack::TypeTag type,
                                           const void *baseptr);
...
public:
  // PreventImmediateReaping is used by Flash Player for older content: it means
  // to flag the object so that it won't be reaped until it goes through a 1 ->
  // 0 reference count transition.  Also see the 'dontAddToZCTDuringCollection'
...
public:
  /**
   * LockObject protects the object against being garbage collected, despite no
   * references to the object being found in a root, the stack, or any other
   * heap object.  The object must not be NULL.  Locks on an object accumulate:
   * if you lock an object n times you must unlock it n times - call
   * UnlockObject on all the returned locks - to make it collectable again. Each
   * lock returned is distinct and becomes an invalid object reference the
   * moment it is unlocked.
   *
   ...
   *
   * NOTE! It is an ERROR to call this if the lock has been unlocked!  YOUR
   * PROGRAM WILL CRASH.
   */
...
private:
  ...
  /**
   * Conservatively find pointers in the entire heap referencing unmarked
   * objects, triggering assertions when one is found.  This is a sanity check
   * on the GC marking algorithm.
   */
  void FindUnmarkedPointers();

  /**
   * Conservatively find pointers in an address range referencing unmarked
   * objects, triggering assertions when one is found.
   */
...
public:
  ...
  virtual void memoryStatusChange(MemoryStatus oldStatus,
   ...
   * mechanism frees the memory.  If the function leaves by throwing an
   * exception (or if one of its callees throws an exception) then the exception
   ...
   * handler was erected.
   *
   * The auto_ptr mechanism, based on the class MMgc::GC::AllocaAutoPtr, cannot
   * be circumvented, as allocaPush() takes a reference to such an object as an
   * argument.
   *
   * Typical usage:
   *
   *    MMgc::GC::AllocaAutoPtr _ptr;                      // by convention
   * prefixed by "_" int* ptr = (int*)core->allocaPush(_ptr, nbytes);  // by
   * convention same name, no "_"
   *
   * In practice the avmStackAlloc() macro should be used so that
   * real alloca() can be used on platforms where that makes sense.
   *
   * Benchmarks suggest that the performance differences from using this
   * mechanism instead of real alloca() are slight to nonexistent, and that the
   * heap allocation sometimes provides a performance improvement.
   */
public:
...
public:
  ...
  static MMgc::GCTracerCheckResult
  CheckOffsetIsInList(uint32_t offset, const uint32_t offsets[], size_t len);
...
public:
  enum EnterType { kNormal = 0, kTryEnter = 1 };
