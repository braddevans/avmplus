...
REALLY_INLINE size_t FixedAlloc::GetNumBlocks() const { return m_numBlocks; }
...
REALLY_INLINE void *FixedAllocSafe::Alloc(size_t size, FixedMallocOpts flags) {
  void *item;
  {
    ...
    GCAssertMsg(item != NULL || (flags & kCanFail),
...
REALLY_INLINE void *FixedAlloc::InlineAllocSansHook(size_t size,
  ...
  (void)size;
  GCAssertMsg(m_heap->IsStackEntered() || (opts & kCanFail) != 0,
    ...
    GCAssert(
        ...
        (b->firstFree >= b->items &&
         ...
         (uintptr_t)b->firstFree <
             ((uintptr_t)b & GCHeap::kBlockMask) + GCHeap::kBlockSize));
...
/*static*/
REALLY_INLINE void *FastAllocator::operator new(size_t size,
                                                FixedAlloc *alloc) {
