 ...
 * be swept are marked as such using a single, common flag, so the Free code
 * only has one test on the fast path; only the slow path needs to test whether
 ...
 * become very long.  There's one quick list per allocator; for some very
 * popular object types that are freed explicitly there's a chance that the
 * quick lists could become long.  We already have bad incrementality at most
 * points where the quick lists must be coalesced, however, so it's doubtful
 * that coalescing will make much of a difference.
 *
 ...
 * free space at the end of a block, but now there's no such memory.  In
 * practice this is not expected to be significant, as there should be very
 * little such free space globally in the system except in minuscule heaps.
 *
 ...
 * bytes that it will allow on quick lists for all its allocators.  Each
 * allocator has a budget - initially one block's worth of data - and will ask
 * to increase it when its quick list grows longer; that request will cause the
 * GC to flush some quick lists if the requested increase is not available. Thus
 * the total amount of fragmentation in the system resulting from the quick
 * lists is always bounded by a constant (which is better than the amount of
 * fragmentation resulting from the ordinary free lists - it is not bounded).
 * The cost of this accounting is a per-allocator counter that is
 * incremented/decremented at each allocation/deallocation and compared with
 * zero.  Regrettably this accounting is on the hot path and I've not yet found
 * a way to coalesce it with the ordinary alloc/free accounting.
 *
 *
 * Notes.
 *
 * An optimization that was tried but which is not yet in this code is
 * propagating the value of the object index from Free/CreateChunk/Sweep to
 * Alloc: Free must compute it, and both CreateChunk and Sweep can compute the
 * index much more cheaply than Alloc can, so it makes sense to do so.  Two
 * facts stand in the way.  One, on 64-bit system the object may only be one
 * word long.  Two, there's a system-wide invariant that free RC objects have a
 * 'composite' field (the second word) whose value is zero (or some poison
 * value, in GCDEBUG builds), and reference counting operations test the value
 * of that field to ignore operations on objects that are in the process of
 * being deleted.  This protocol makes finalization work in the current system.
 * See comments about this invariant in GCObject.h.
 */
...
GCAlloc::GCAlloc(GC *_gc, int _itemSize, bool _containsPointers, bool _isRC,
                 bool _isFinalized, int _sizeClassIndex, int _sliceIndex,
                 int _partitionIndex, uint8_t _bibopTag)
    : m_firstBlock(NULL), m_lastBlock(NULL), m_firstFree(NULL),
      m_needsSweeping(NULL), m_qList(NULL), m_qBudget(0), m_qBudgetObtained(0),
      m_itemSize((_itemSize + 7) &
                 ~7), // Round itemSize to the nearest boundary of 8
...
#else
      m_numBitmapBytes(((m_itemsPerBlock * sizeof(gcbits_t)) + 3) &
                       ~3), // round up to 4 bytes so we can go through the bits
                            // several items at a time
#endif
      m_sizeClassIndex(_sizeClassIndex), m_sliceIndex(_sliceIndex),
      m_partitionIndex(_partitionIndex),
#ifdef MMGC_MEMORY_PROFILER
      m_totalAskSize(0),
#endif
      m_bitsInPage(_containsPointers &&
                   kBlockSize -
                           int(m_itemsPerBlock *m_itemSize + sizeof(GCBlock)) >=
                       m_numBitmapBytes),
      m_bibopTag(_bibopTag), multiple(ComputeMultiply((uint16_t)m_itemSize)),
      shift(ComputeShift((uint16_t)m_itemSize)),
      containsPointers(_containsPointers), containsRCObjects(_isRC),
      containsFinalizedObjects(_isFinalized), m_finalized(false), m_gc(_gc) {
#ifdef GCDEBUG
...
GCAlloc::~GCAlloc() {
  ...
  while (m_firstBlock) {
    ...
    // check where any item within this block wasn't written to after being
    // poisoned
...
GCAlloc::GCBlock *GCAlloc::CreateChunk(int flags) {
  ...
  gcbits_t *bits = m_bitsInPage ? NULL
                                : (gcbits_t *)m_gc->AllocBits(m_numBitmapBytes,
                                                              m_sizeClassIndex,
                                                              m_sliceIndex);
  ...
  GCBlock *b =
      (GCBlock *)m_gc->AllocBlock(1, m_partitionIndex, PageMap::kGCAllocPage,
                                  /*zero*/ true, (flags & GC::kCanFail) != 0);
  ...
  if (b) {
    ...
    //
    // We must make the object look free, which means poisoning it properly and
    // setting the mark bits correctly.
...
#ifdef MMGC_HOOKS
#ifdef MMGC_MEMORY_INFO // DebugSize is 0 if MEMORY_INFO is off, so we get an
                        // "obviously true" warning from GCC.
      GCAssert(m_itemSize >= DebugSize());
#endif
      if (heap->HooksEnabled())
        heap->PseudoFreeHook(GetUserPointer(p), m_itemSize - DebugSize(),
...
#endif
{
  ...
  // Allocation must be signalled before we allocate because no GC work must be
  // allowed to come between an allocation and an initialization - if it does,
  // we may crash, as GCFinalizedObject subclasses may not have a valid vtable,
  // but the GC depends on them having it.  In principle we could signal
  // allocation late but only set the object flags after signaling, but we might
  // still cause trouble for the profiler, which also depends on
...
#endif
{
  ...
  b->bits[GetBitsIndex(b, item)] =
      (flags & (GC::kFinalize | GC::kInternalExact));
#elif defined VMCFG_SELECTABLE_EXACT_TRACING
  b->bits[GetBitsIndex(b, item)] =
      (flags &
       (GC::kFinalize |
        m_gc->runtimeSelectableExactnessFlag)); // 0 or GC::kInternalExact
#else
...
#endif
{
  ...
  if (!m_gc->collecting && !m_gc->greedy) {
    // Fast path: fill the quick list from b, then tail-call
    // AllocFromQuickList()
    FillQuickList(b);
...
void GCAlloc::Free(const void *item) // item is realPtr
{
  ...
  // both skip items on the free list.  For sweeping we could set it in the
  // already expensive must-be-swept logic.  For finalization we could  move the
  // setting into CoalesceQuickList, where it would be set only for objects
  // flushed to the regular free lists.  It would be a savings overall if the
  // quick list is busy, as we expect it to be.
  //
  ...
  // would mean that it may end up on both a free list and on the mark stack,
  // and that would be bad; it could also mean that objects reachable from the
  // dead object would be marked in turn and would be retained for a GC cycle.

  b->bits[bitsindex] |=
      kFreelist; // Don't clear the weak ref bit, FreeSlow may inspect it
  m_totalAllocatedBytes -= m_itemSize;
...
void GCAlloc::CoalesceQuickList() {
  ...
  if (m_qBudgetObtained > m_itemsPerBlock) {
    m_gc->RelinquishQuickListBudget((m_qBudgetObtained - m_itemsPerBlock) *
                                    m_itemSize);
    m_qBudgetObtained = m_itemsPerBlock;
  }
...
void GCAlloc::FinalizationPass() {
  ...
  for (GCBlock *b = m_firstBlock; b != NULL; b = next) {
    ...
    for (char *item = b->items, *limit = b->items + m_itemSize * b->GetCount();
         item < limit; item += m_itemSize) {
      ...
      GCAssertMsg(mq != kQueued,
                  "No queued objects should exist when finalizing");

      // GC::Finalize calls GC::MarkOrClearWeakRefs before calling
      // GCAlloc::Finalize, ergo there should be no unmarked objects with weak
      // refs.
      ...
      if (m_gc->heap->HooksEnabled()) {
        ...
        if (m_gc->heap->GetProfiler())
          m_totalAskSize -=
              m_gc->heap->GetProfiler()->GetAskSize(GetUserPointer(item));
#endif

        m_gc->heap->FinalizeHook(GetUserPointer(item),
                                 m_itemSize - DebugSize());
      }
#endif

      if (marks & kFinalizable) {
        GCFinalizedObject *obj =
            (GCFinalizedObject *)(void *)GetUserPointer(item);
        marks &= ~kFinalizable; // Clear bits first so we won't get second
                                // finalization if finalizer longjmps out

        /* See https://bugzilla.mozilla.org/show_bug.cgi?id=573737 for the case
         * where the object might remain in uninitialized state and thus crash
         * occurs while calling the dtor below.
         */
        if (*(intptr_t *)obj != 0)
      ...
      // GC::GetWeakRef will not allow a weak reference to be created to an
      // object that is ready for destruction.
    ...
    // 2) no freed items, partially empty or full, return to free if partially
    // empty 3) some freed item add to the to be swept list
    if (numMarkedItems == 0) {
...
// For each block that is not already on the sweep list, add it to the sweep
// list for lazy sweeping. We don't know if the block will have any free
// objects, or if it will have all free objects, so the sweeper must take that
// into account.
...
void GCAlloc::SweepGuts(GCBlock *b) {
  ...
  for (char *item = b->items, *limit = b->items + m_itemSize * b->GetCount();
    ...
    if (mq == kMark ||
        mq ==
            kQueued) // Sweeping is lazy; don't sweep objects on the mark stack
    {
...
#ifdef MMGC_HOOKS
    if (m_gc->heap->HooksEnabled())
      m_gc->heap->FreeHook(GetUserPointer(item), b->size - DebugSize(),
                           uint8_t(GCHeap::GCSweptPoison));
#endif
...
// Incrementality: Note that there is the possibility for an unbounded pause
// here, though it can be no worse than before the lazy sweeping.  And we can
// fix it, as shown below.
//
// AllocSlow calls Sweep, which will sweep the block but (a) may return the
// block to GCHeap rather than making it available to AllocSlow, if it is
// entirely empty, or (b) may find that the block is entirely full and keep it
// off the free list. In either case the result may be that no memory is made
// available.
//
// AllocSlow handles the situation gracefully by trying another block, but
// there's no bound on how many blocks it may have to try.  The alternative
// would be to bound the number of attempts, so that a fully free block is not
// returned to GCHeap after some have been freed, or a fresh block is allocated
// from GCHeap after some full blocks have unsuccessfully been swept.  In either
// case that can be implemented in AllocSlow, by AllocSlow just going to GCHeap
// after failing to obtain memory from the free list after some attempts.
...
void GCAlloc::CheckMarks() {
  ...
  while (b) {
    GCBlock *next = Next(b);
    GCAssertMsg(!b->needsSweeping(),
                "All needsSweeping should have been swept at this point.");

    for (char *item = b->items, *limit = b->items + m_itemSize * b->GetCount();
         item < limit; item += m_itemSize) {
      gcbits_t m = GC::GetGCBits(item) & kFreelist;
      GCAssertMsg(m == 0 || m == kFreelist,
                  "All items should be free or clear, nothing should be marked "
                  "or queued.");
    }
...
/*static*/
int GCAlloc::ConservativeGetMark(const void *item,
...
void GCAlloc::CheckFreelist() {
  ...
  while (b) {
    ...
    while (freelist) {
      // b->firstFree should be either 0 end of free list or a pointer into b,
      // otherwise, someone wrote to freed memory and hosed our freelist
      GCAssert(freelist == 0 ||
               ((uintptr_t)freelist >= (uintptr_t)b->items &&
                (uintptr_t)freelist < (uintptr_t)b + GCHeap::kBlockSize));
...
REALLY_INLINE void GCAlloc::GCBlock::FreeSweptItem(const void *item,
                                                   int bitsindex) {
...
/* static */
void GCAlloc::VerifyFreeBlockIntegrity(void *item, uint32_t size,
                                       uint32_t limit) {
  ...
  while (item) {
    ...
    for (int i = startIndex; i < n; i++) {
      ...
      if (data != uint32_t(GCHeap::GCSweptPoison) &&
          data != uint32_t(GCHeap::GCFreedPoison)) {
...
bool GCAlloc::IsOnEitherList(GCBlock *b) {
  return b->nextFree != NULL || b->prevFree != NULL || b == m_firstFree ||
         b == m_needsSweeping;
}
...
bool GCAlloc::IsPointerIntoGCObject(const void *item) {
  ...
  bool retval = false;
  if (item >= block->items)
    retval = (GC::GetGCBits(block->items +
                            GetObjectIndex(block, item) *
                                ((GCAlloc *)block->alloc)->m_itemSize) &
              kFreelist) != kFreelist;
  return retval;
}
