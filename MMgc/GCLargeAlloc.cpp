...
namespace MMgc {
GCLargeAlloc::GCLargeAlloc(GC *gc, int partitionIndex)
    : m_totalAllocatedBytes(0), m_gc(gc), m_partitionIndex(partitionIndex) {
  m_blocks = NULL;
...
#endif
{
...
#endif
  GCHeap::CheckForAllocSizeOverflow(requestSize,
                                    sizeof(LargeBlock) + GCHeap::kBlockSize);

  int blocks =
      (int)((requestSize + sizeof(LargeBlock) + GCHeap::kBlockSize - 1) /
            GCHeap::kBlockSize);
  ...
  LargeBlock *block = (LargeBlock *)m_gc->AllocBlock(
      blocks, m_partitionIndex, PageMap::kGCLargeAllocPageFirst,
  ...
  if (block) {
    ...
    GCAssert((unsigned long)GC::kInternalExact ==
             (unsigned long)kVirtualGCTrace);
...
void GCLargeAlloc::Finalize() {
  ...
  while (*prev) {
    ...
    if ((b->flags[0] & kMark) == 0) {
      ...
      GCAssertMsg((b->flags[0] & kHasWeakRef) == 0,
                  "No unmarked object should have a weak ref at this point");

      // Large blocks may be allocated by finalizers for large blocks, creating
      // contention for the block list.  Yet the block list must be live, since
      // eg GetUsageInfo may be called by the finalizers (or their callees).
      //
      ...
      *prev = Next(b);
      ...
      if (m_gc->heap->HooksEnabled()) {
        ...
        if (GCHeap::GetGCHeap()->GetProfiler())
          m_totalAskSize -= GCHeap::GetGCHeap()->GetProfiler()->GetAskSize(
              GetUserPointer(item));
#endif
...
GCLargeAlloc::~GCLargeAlloc() { GCAssert(!m_blocks); }

#ifdef GCDEBUG
/* static */
bool GCLargeAlloc::ConservativeGetMark(const void *item,
