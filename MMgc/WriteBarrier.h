...
#define WB(gc, container, addr, value)                                         \
  gc->privateWriteBarrier(container, addr, (const void *)(value))

// fast manual RC write barrier
#define WBRC(gc, container, addr, value)                                       \
  gc->privateWriteBarrierRC(container, addr, (const void *)(value))
...
namespace MMgc {
/**
 * WB is a smart pointer write barrier meant to be used on any field of a GC
 * object that may point to another GC object.  A write barrier may only be
 * avoided if if the field is const and no allocations occur between the
 * construction of the object holding the field and the assignment.
 */
...
public:
  ...
  REALLY_INLINE ~WriteBarrier() { t = 0; }

  REALLY_INLINE T operator=(const WriteBarrier<T> &wb) { return set(wb.t); }

  REALLY_INLINE T operator=(T tNew) { return set(tNew); }
...
/**
 * WriteBarrierRC is a write barrier for naked (not pointer swizzled) RC
 * objects. the only thing going in and out of the slot is NULL or a valid
 * RCObject
 */
...
public:
  ...
  REALLY_INLINE T operator=(const WriteBarrierRC<T> &wb) { return set(wb.t); }
  ...
  // Clear() clears the smart pointer without decrementing the reference count
  // of any object stored in the smart pointer.  It is essentially useful in
  // situations where the caller has already deleted the object and needs to
  // ensure that the destruction of the smart pointer does not access the
  // deleted storage.
