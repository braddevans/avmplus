...
public:
  ...
  /**
   * If useVirtualMemory is true then we use virtual memory
   * (reserve/commit/decommit/unreserve) for GCHeap memory.  Otherwise we go
   * through aligned allocation/deallocation platform interfaces.
   */
  bool useVirtualMemory;

  /**
   * If trimVirtualMemory is true then we try to reduce the use of virtual
   * address space by deallocating regions that have no used blocks in them.
   */
  bool trimVirtualMemory;

  /**
   * If mergeContiguousRegions is true then we merge adjoining regions into a
   * single region.  This is currently (now == 2010-03-19) unused.
   */
  bool mergeContiguousRegions;

  /**
   * If sloppyCommit is true then VMPI_commitMemory() and VMPI_decommitMemory()
   * can operate on ranges of memory containing both committed and decommitted
   * blocks.
   */
  bool sloppyCommit;
  ...
  /**
   * In GCDEBUG builds, if checkFixedMemory is true then a test is made to check
   * whether the fixed object about to be freed is owned by FixedMalloc.  That
   * test is somewhat expensive, so it can be toggled by means of this flag.
   *
   * This flag will probably disappear in the future when expected changes to
   * GCHeap makes the test much cheaper.
   *
   * The flag defaults to 'true' and can be changed to 'false' after startup,
   * but then sticks at false.  (Any large objects that have been allocated with
   * it set to 'false' will not be properly registered, and when trying to free
  ...
  bool gcstats;
  bool autoGCStats;
  int32_t gcbehavior; // Print gross history and policy decisions
                      // (MMGC_POLICY_PROFILING): 0=off, 1=at end, 2=after every
                      // gc and at end
  bool eagerSweeping; // Enable full-heap sweeping at the end of Sweep()
#ifdef MMGC_HEAP_GRAPH
  bool dumpFalsePositives;
#endif
  double gcLoad[kNumLoadFactors]; // GC load factors: policy aims for a heap
                                  // size that is gcLoad*H where H is the live
                                  // size following GC
  double gcLoadCutoff[kNumLoadFactors]; // Heap sizes (MB) following GC below
                                        // which the corresponding load factor
                                        // applies, last entry is +infinity
  double gcLoadCeiling; // Max multiple of gcLoad policy should use after
                        // adjusting L for various factors (0=unlimited)
  double gcEfficiency;  // Max fraction of time to spend in the collector while
                        // the incremental collector is active
...
public:
  ...
  /**
   * Tell every other GC that 'gc' is starting a collection (ie there may be
...
public:
  ...
  const static uintptr_t kBlockMask =
      ~(uintptr_t(kBlockSize) - 1); // Clear lower 12 bits
  const static uintptr_t kOffsetMask =
      (uintptr_t(kBlockSize) - 1); // Clear upper 20 or 52 bits
  ...
  const static uint32_t kNumFreeLists =
      (kHugeThreshold - kUniqueThreshold) / kFreeListCompression +
      kUniqueThreshold;
  ...
  const static uintptr_t GCEndOfObjectPoison =
      0xdeadbeef; // First word past legitimate payload in GC object
  const static uintptr_t GCFreedPoison =
      0xcacacaca; // Object fill when GC object has been freed
  const static uintptr_t GCSweptPoison =
      0xbabababa; // Object fill when GC object has been swept
  const static uintptr_t FXFreshPoison =
      0xfafafafa; // Object fill in fresh FixedAlloc object (before any zeroing)
  const static uintptr_t FXFreedPoison =
      0xedededed; // Object fill when FixedAlloc object has been freed
  const static uintptr_t MMFreedPoison =
      0xfbfbfbfb; // Object fill when heap block has been freed by GCHeap

  // Tag values used for debugging in GCGlobalNew
  const static uint32_t MMScalarTag = 0xafafafafU; // Tag for a non-array object
  const static uint32_t MMNormalArrayTag =
      0xbfbf0001U; // Tag for an array object with constructed/destructed
                   // elements
  const static uint32_t MMPrimitiveArrayTag =
      0xbfbf0002U; // Tag for an array object with primitive elements, must be
                   // one greater than the normal tag
   ...
   *
   * Referenced directly from JIT-compiled code.  It must be a static variable,
   * and cannot go in the GCHeap singleton.
   */
  ...
  /**
   * Destroy the GCHeap singleton, returns the number of bytes still allocated
   * in FixedMalloc (ie leaked)
   */
  ...
  /**
   * Signal a too-large allocation request.  This /will/ cause an immediate
   * shutdown of the entire system.  (The alternative is to return a NULL
   * pointer, which has the same effect but with considerably less grace.)
   * Clients that allocate arbitrarily large objects based on application or
   * end-user data may wish to be concerned about checking whether the object
   * might be too large.
   */
#if defined(__GNUC__)
  ...
  /**
   * Signal an inconsistent heap state (an unrecoverable program error).  This
   * /will/ cause an immediate shutdown of the entire system.  (The alternative
   * is to return a NULL pointer, which has the same effect but with
   * considerably less grace.)
   */
#if defined(__GNUC__)
  ...
  /**
   * Signal that the caller is about to longjmp to or past an MMGC_ENTER, and
   * that actions should be taken to leave the heap in a consistent state.
   *
   * MMgc code does not use this, it's intended for use by external agents.  It
   * can be called without ill effect even if there is no active GC or GCHeap.
   *
   * longjmp'ing to or past an MMGC_ENTER is not supported in all contexts: host
   * code should never do it on call-backs on allocation, deallocation, and
   * finalization hooks.
   */
   ...
   * to be controlled by MMgc, and wants this memory accounted for.  (A typical
   * case is when system memory is allocated for rendering buffers.)
   *
   * Let 'total memory volume' be the sum of the memory volume controlled by
   * MMgc, plus the volume added by SignalExternalAllocation, minus the volume
   * subtracted by SignalExternalDeallocation.  MMgc performs its out-of-memory
   * avoidance actions (actions for crossing the soft and hard limits) based on
   * total memory volume.
   */
   ...
   * be controlled by MMgc, and wants this memory accounted for.
   *
  ...
  /**
   * Signal that client code wants the MMgc to release memory back to the
   * system. This will trigger synchronous and asynchronous memory cleanup.
   *
   ...
   * will be accounted for there, and we're just tracking that it is code
   * memory; false if an external native API will be used.
   */
  ...
  class Region {
  public:
    Region(GCHeap::Partition *partition, char *baseAddr, char *rTop, char *cTop,
           size_t blockId);
    Region *prev;
    char *baseAddr;
    char *reserveTop;
   ...
   *
   * Allocations and deallocations always take place within a specific
   * partition.
   */
  class Partition {
    friend class GCHeap;
  ...
  public:
     ...
     *             This must not be zero and it should be greater than 1 only
     * when absolutely necessary.  (The VMPI layer may require greater alignment
    ...
    /**
     * Allocates a block from the heap, but is guaranteed never to run OOM
     * handling. You can call this without kCanFail but a failed allocation will
     * result in an immediate abort.
     */
    ...
    // Return the size (in blocks) of an item, or (size_t)-1 if the item is not
    // valid.
    size_t SafeSize(const void *item);

    size_t GetPartitionSize() const;

    // These members really should be private, as they are not for the use of
    // MMgc clients. They are referenced from outside class GCHeap, however, by
    // classes GC and FixedMalloc. MMgc classes rather freely friend each other,
    // which should allow presenting a more controlled interface to the world
    // outside MMgc.  Unfortunately, it is not possible to friend a member class
    // of an incomplete class, which is what happens from FixedMalloc. GCHeap
    // embeds an instance of FixedMalloc, thus the complete definition of
    // FixedMalloc must precede that of GCHeap.  This circularity could be
    // eliminated with a bit of refactoring and an extra level of indirection.
    // (insert David Wheeler quote here)
  ...
  private:
    ...
    // running total of number of blocks allocated and not freed (this partition
    // only)
    size_t numAlloc;

    Partition()
        : index(0), freeRegion(NULL), nextRegion(NULL), lastRegion(NULL),
          blocks(NULL), blocksLen(0), numDecommitted(0), numRegionBlocks(0),
          largeAllocation(0), numAlloc(0) {}
    ...
    // abandon a block of memory that may maps completely to the committed
    // portion of region
    void RemoveBlock(HeapBlock *block, bool release = true);

    // Add a block to the free list, coalescing committed blocks with adjacent
    // committed blocks.  The coalesced block is marked dirty if makeDirty is
    // true or if any of the blocks that were coalesced were dirty.
    ...
    HeapBlock *CreateCommittedBlock(HeapBlock *block, size_t size,
                                    size_t alignment);
    void PruneDecommittedBlock(HeapBlock *block, size_t available,
                               size_t request);
  ...
  /**
   * @return a pointer to a the partition of the heap indexed by parameter
   * partition.
   */
  ...
  /**
   * @return the amount of code memory currently allocated by GCHeap.  GCHeap
   * does not have a notion of "free" and "used" code memory.
   *
   * @see GetTotalCodeSize, SignalCodeMemoryAllocation and
   * SignalCodeMemoryDeallocated
   */
  ...
  /**
   * gives memory back to the OS when there hasn't been any memory activity in a
   * while and we have lots of free memory
   */
  void Decommit();
...
#ifdef MMGC_HOOKS
  /* Hooks are normally disabled in RELEASE builds, as there is a slight cost
     added to some hot paths.  */
   ...
   * Currently, in MMgc all object allocations are capped to 2^32-1
   * (=kMaxObjectSize) which is the largest object size on 32-bit systems.  To
   * detect overflow the standard way is to add the values and look for
   * wraparound by checking if the result is less than either of the operands.
   * However, on 64-bit systems sizeof(size_t) == 8 bytes and the wraparound
   * check would not work.  So this method also checks if the result exceeds
   * 2^32-1 to conform our allocation size cap. Requirement: All allocation
   * routines and methods that add extra payload (such as headers) to allocation
   * requests should call this method for overflow detection.
   ...
   * Currently, in MMgc all object allocations are capped to 2^32-1
   * (=kMaxObjectSize) which is the largest object size on 32-bit systems. To
   * detect overflow, we first check if either of N or M exceeds kMaxObjectSize.
   * This check is a guard against overflow on 64-bit systems where
   * sizeof(size_t) is greater than 4 bytes.  If this check succeeds then we
   * perform a 64-bit based product i.e. (N * M) and check if the result exceeds
   * kMaxObjectSize. Requirement: All allocation routines and methods that are
   * servicing allocation requests based on a product of object size and a
   * number of objects should call this method prior to allocation
   * @param count number of objects being allocated
   * @param elsize requested size pertaining to a single object being allocated
   * @return: None.  This method is designed to call Abort() which in turn
   * terminates the program if an overflow is detected.
   */
   ...
   * sizeof(size_t) is greater than 4 bytes.  If this check succeeds then we
   * perform a 64-bit based product i.e. (N * M) and check if the result exceeds
   * kMaxObjectSize. Requirement: Code that is allocating arrays using operator
   * new in the form "new type[count]" should call this method to validate that
   * the requested size will not cause overflow
   * @param count number of objects being allocated
   * @param elsize requested size pertaining to a single object being allocated
   * @param canFail specifies if the method should return 0 (true) or call Abort
   * (false)
   * @return: This method will return the count parameter that was passed in if
   * there was no overflow. If an overflow is detected, the method will return 0
   * if the canFail parameter is true, or will call Abort() otherwise
   */
  static size_t CheckForNewSizeOverflow(size_t count, size_t elsize,
                                        bool canFail);

  /**
   * AddToAddressWithOverflowCheck
   * Given a base address, verify that the sum of that address and a specified
   * offset remains a representable address.  If the test fails, we proceed as
   * if an attempt had been made to allocate an object larger than
   * kMaxObjectSize. While this may not be strictly correct, the only recourse
   * the user will have to avoid failure of this check is to reduce the size of
   * an allocation. See GC::allocaPush().
   */
  static uintptr_t AddToAddressWithOverflowCheck(uintptr_t base, size_t offset);
...
private:
  ...
  /** Resets the GCHeap statics. This is called from both GCHeap::GCHeap() and
     GCHeap::DestroyInstance() It sould be called at the start and end of
     GCHeap's lifetime. It avoids any mismatch in the static variable initial
     values
   */
  ...
  size_t totalNumAlloc; // running total of number of blocks allocated and not
                        // freed (entire heap)
  ...
  bool m_oomHandling; // temporarily false when allocating or deallocating with
                      // kNoOOMHandling
  bool m_notificationBeingSent;
  vmpi_spin_lock_t gclog_spinlock; // a lock used by GC::gclog for exclusive
                                   // access to GCHeap::DumpMemoryInfo

#ifdef MMGC_MEMORY_PROFILER
  static MemoryProfiler *profiler;
  bool hasSpy; // flag indicating whether profiler spy is active or not.  If
               // active, AllocHook will call VMPI_spyCallback
#endif
