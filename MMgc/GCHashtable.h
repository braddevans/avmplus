 ...
 */
template <typename V> struct HashTableEntry {
  V value;
  const void *key;
};
 ...
 */
template <typename VAL = const void *, class KEYHANDLER = GCHashtableKeyHandler,
          class ALLOCHANDLER = GCHashtableAllocHandler_new>
...
public:
  ...
  REALLY_INLINE VAL get(const void *key) {
    return table[find(key, table, tableSize)].value;
  }
  ...
  REALLY_INLINE void add(intptr_t key, VAL value) {
    put((const void *)key, value);
  }
  ...
  public:
    ...
    const void *nextKey() {
      do {
        index += 1;
      } while (index < (int32_t)ht->tableSize &&
               ht->table[index].key <= GCHashtableBase::DELETED);
...
/*static*/ const void *const
    GCHashtableBase<VAL, KEYHANDLER, ALLOCHANDLER>::DELETED = (const void *)1;

template <typename VAL, class KEYHANDLER, class ALLOCHANDLER>
/*static*/ typename GCHashtableBase<VAL, KEYHANDLER, ALLOCHANDLER>::Entry
    GCHashtableBase<VAL, KEYHANDLER, ALLOCHANDLER>::EMPTY[4] = {
        {0, 0}, {0, 0}, {0, 0}, {0, 0}};
...
/*
    ...
    On some platforms FixedMalloc is known to be faster than the system (VMPI)
   allocator, but really its about OOM: when we run out of memory we can
   shutdown in a leak proof manner by zapping the GCHeap; if we use system
   memory we can't do that (short of using a malloc zone). Also, it's about
   getting the right profiler data: we want the WeakRef hashtable to show up in
   the memory profile results.
*/
...
public:
  REALLY_INLINE static uint32_t hash(const void *k) {
    return uint32_t(
        (uintptr_t(k) >> 3) ^
        (uintptr_t(k)
         << 29)); // move the low 3 bits higher up since they're often 0
  }
...
typedef GCHashtableBase<const void *, GCHashtableKeyHandler,
                        GCHashtableAllocHandler_new>
    GCHashtable;
typedef GCHashtableBase<const void *, GCHashtableKeyHandler,
...
public:
  REALLY_INLINE static uint32_t hash(const void *k) {
    ...
    while (*s++) {
      hash =
          (hash >> 28) ^ (hash << 4) ^ ((uintptr_t)*s << ((uintptr_t)s & 0x3));
    }
    return hash;
  }
...
typedef GCHashtableBase<const void *, GCStringHashtableKeyHandler,
                        GCHashtableAllocHandler_new>
    GCStringHashtable;
typedef GCHashtableBase<const void *, GCStringHashtableKeyHandler,
                        GCHashtableAllocHandler_VMPI>
