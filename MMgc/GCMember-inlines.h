...
#endif
{
  ...
  char stackTop;
  return ((uintptr_t)address > (uintptr_t)&stackTop &&
          (uintptr_t)address < stackBase);
}
#endif

template <class T> REALLY_INLINE void GCMemberBase<T>::set(const T *tNew) {
  T::WriteBarrier((void **)&(this->t), (void *)tNew);
}

template <class T> REALLY_INLINE T *GCMemberBase<T>::value() const {
  return this->t;
}

template <class T> REALLY_INLINE T *const *GCMemberBase<T>::location() const {
  return &(this->t);
}

template <class T> REALLY_INLINE GCMemberBase<T>::GCMemberBase() : GCRef<T>() {
#ifdef GCDEBUG
...
template <class T> REALLY_INLINE GCMemberBase<T>::~GCMemberBase() {
  T::WriteBarrier_dtor((void **)&(this->t));
}

template <class T>
REALLY_INLINE GCMemberBase<T> &
GCMemberBase<T>::operator=(const GCMemberBase &other) {
  set(GCRef<T>::ProtectedGetOtherRawPtr(other));
  return *this;
}
...
template <class T> REALLY_INLINE void GCMemberBase<T>::operator=(T *tNew) {
  set(tNew);
}
...
template <class T> REALLY_INLINE void GCRoot::GCMember<T>::set(T *tNew) {
  T *tOld = NULL;
  if (valid()) {
    tOld = this->t;
  }
...
template <class T> REALLY_INLINE bool GCRoot::GCMember<T>::valid() {
  return (uintptr_t)(this->t) > 1;
}

template <class T> REALLY_INLINE GCRoot::GCMember<T>::GCMember() : GCRef<T>() {
  GCAssert(!IsAddressOnStack(this));
}
...
template <class T>
REALLY_INLINE GCRoot::GCMember<T>::GCMember(T *valuePtr) : GCRef<T>() {
  GCAssert(!IsAddressOnStack(this));
  set(valuePtr);
}

template <class T> REALLY_INLINE GCRoot::GCMember<T>::~GCMember() { set(NULL); }
...
template <class T> REALLY_INLINE void GCRoot::GCMember<T>::operator=(T *tNew) {
  set(tNew);
}

template <class T>
REALLY_INLINE GCRoot::GCMember<T> &
GCRoot::GCMember<T>::operator=(const GCMember &other) {
  set(GCRef<T>::ProtectedGetOtherRawPtr(other));
  return *this;
}
...
template <class T2>
REALLY_INLINE void
GCInlineObject::GCMember<T>::operator=(const GCRef<T2> &other) {
...
template <class T>
REALLY_INLINE GCInlineObject::GCMember<T>::GCMember() : GCMemberBase<T>() {}
...
template <class T> REALLY_INLINE GCObject::GCMember<T>::GCMember() {}
...
template <class T> REALLY_INLINE GCObject::GCMember<T>::GCMember(T *valuePtr) {
  GCMemberBase<T>::operator=(valuePtr);
}
...
template <class T2>
REALLY_INLINE void
GCTraceableBase::GCMember<T>::operator=(const GCRef<T2> &other) {
  GCMemberBase<T>::operator=(other);
}
...
template <class T>
REALLY_INLINE GCTraceableBase::GCMember<T>::GCMember() : GCMemberBase<T>() {}
