...
public:
   ...
   * the generated function is:
     ...
     if((result = super->gcTraceOffsetIsTraced(off)) != MMgc::kOffsetNotFound)
         return result;
     if((someStruct.gcTraceOffsetIsTraced(off, offsetof(MyClass, someStruct)))
   != MMgc::kOffsetNotFound) return result; return
   GC::CheckOffsetIsInList(off,myOffsets);
  ...
  void gcTrace(GC *, size_t cursor = 0) {
    (void)cursor;
    GCAssertMsg(false, "This class needs a gcTrace, did you forget to annotate "
                       "it or write a custom gcTrace?");
  }

  //  GCObject's local definition of GCMember
  template <class T> class GCMember : public GCMemberBase<T> {
  public:
    template <class T2> void operator=(const GCRef<T2> &other);

    void operator=(T *tNew);

    //  Since we're locking up the copy constructor, we need to explicitly
...
public:
  ...
  // 'throw()' annotation to avoid GCC warning: 'operator new' must not return
  // NULL unless it is declared 'throw()' (or -fcheck-new is in effect)
  ...
  //  Create a GCRef from "this" with parameter for auto template
  //  specialization.
  template <class T> REALLY_INLINE GCRef<T> GetThisRef(const T *ths) const {
...
public:
  ...
  GCWeakRef *GetWeakRef() const { return GC::GetWeakRef(this); }
...
public:
  ...
  T *AsArray() {
    ...
    for (char *dummy = (char *)&apiEnforcement,
              *end = (char *)(&apiEnforcement + 1);
         dummy < end; dummy++) {
      GCAssertMsg(*dummy == GCHeap::LeafApiEnforcementPoison,
                  "LeafVector underwrite, make sure AsArray is being used");
    }
#endif
    return elements;
  }
...
private:
  // 'throw()' annotation to avoid GCC warning: 'operator new' must not return
  // NULL unless it is declared 'throw()' (or -fcheck-new is in effect)
  static void *operator new(size_t size, GC *gc, size_t count)
      GNUC_ONLY(throw()) {
...
public:
  ...
  // The gcTrace method will be called to exactly mark the object.  It must
  // trace all locations in the object that may contain pointers to managed
  // objects.
  //
  ...
  // otherwise 'false'.  If 'true' is returned then the object will be
  // re-visited with cursor incremented.
  //
 ...
 *
 *   - No exactly traced field must ever contain a bogus value.  That's
 * generally not hard to ensure but there are three cases to watch out for:
 *
...
REALLY_INLINE void *GCObject::operator new(size_t size, GC *gc, size_t extra)
    GNUC_ONLY(throw()) {
  return gc->AllocExtraPtrZero(size, extra, kGCObjectNewPartition);
}

REALLY_INLINE void *GCObject::operator new(size_t size, GC *gc)
    GNUC_ONLY(throw()) {
...
REALLY_INLINE void *GCTraceableObject::operator new(size_t size, GC *gc,
                                                    size_t extra)
    GNUC_ONLY(throw()) {
...
REALLY_INLINE void *GCTraceableObject::operator new(size_t size, GC *gc,
  ...
  return gc->AllocExtraPtrZeroExact(size, extra,
                                    kGCTraceableObjectNewPartition);
}
...
REALLY_INLINE void *GCFinalizedObject::operator new(size_t size, GC *gc,
  ...
  return gc->AllocExtraPtrZeroFinalized(size, extra,
                                        kGCFinalizedObjectNewPartition);
}
 ...
 *
 * This object always has a finalizer (the C++ destructor).  The C++ destructor
 * /must/ leave the object with all-fields-zero.
 *
 * Reference counting is deferred: when an object's reference count drops to
 * zero it is inserted into the zero-count table (the ZCT), see ZCT.h etc.  If
 * the reference count grows above zero the object is removed from the ZCT
 * again.  Every new object is added to the ZCT initially - reference counts
 * start at zero.
 *
 * Occasionally the ZCT is reaped: objects in the ZCT that are not referenced
 * from stack memory or special ZCT roots, and that are not explicitly pinned by
 * client code, are deleted by calling their finalizers and reclaiming their
 * memory.   A finalizer may make the reference counts of more referenced
 * objects drop to zero, whereupon they too are entered into the ZCT (and may be
 * deleted by the ongoing reap, or a later reap).
 *
 * (Under complicated scenarios it is possible for an object allocated during
 * reaping to be erroneously deleted, see
 * https://bugzilla.mozilla.org/show_bug.cgi?id=506644, so client code may want
 * to take note of that.)
 *
 *
 * Invariants on 'composite'.
 *
 * The field 'compsite' holds reference counts, the ZCT index, and flag bits. We
 * depend on object layout being such that this field is the second word of the
 * object.  This field is used for a heuristic defense-in-depth mechanism:
 *
 *  - In release builds its value is zero iff the object's destructor has been
 * run and the object has not been reallocated (ie the object may or may not yet
 * have been placed on a free list).
 *  - In debug builds its value is zero iff the object's destructor has been run
 * and the object has not yet been put on the free list or been reallocated.
 * Once it's been put on the free list the composite word is a cookie:
 * GCHeap::GCFreedPoison or GCHeap::GCSweptPoison.
 *
 * The RC operations check whether the object is free - and if it is, then the
 * operations have no effect.  This is useful because of the current
 * finalization semantics: finalization and destruction are intertwined, so that
 * the C++ destructor performs both jobs.  Thus an object can't be finalized
 * without being destroyed.  Finalizers run in arbitrary order; thus a finalizer
 * for an object that's part of an object network will not know whether it is
 * safe to operate on other objects in that network.  The extra guards on RC
 * operations makes this safer.  It only works because all objects that are
 * finalizable are finalized together, before any of them are swept or
 * reallocated. It is also an expense that everyone pays for.  The only fix is a
 * change to how finalization is handled.
 */
...
public:
  ...
  REALLY_INLINE ~RCObject() {
    ...
    // GCAssert((GC::GetGC(this)->GetGCBits(this) & (kMark|kQueued)) !=
    // kQueued);
    composite = 0;
  }
   ...
   */
  REALLY_INLINE bool IsPinned() { return (composite & STACK_PIN) != 0; }

#ifdef GCDEBUG
  REALLY_INLINE bool IsGCPoisoned() {
    return composite == uint32_t(GCHeap::GCFreedPoison) ||
           composite == uint32_t(GCHeap::GCSweptPoison);
  }
#endif
   ...
   */
  REALLY_INLINE uint32_t RefCount() const { return (composite & RCBITS) - 1; }

  /**
   * @return non-zero if the object is sticky (RC operations have no effect
   * because the RC field is invalid, either as a consequence of an RC overflow
   * or because the sticky bit was set explicitly).
   */
  REALLY_INLINE uint32_t Sticky() const { return composite & STICKYFLAG; }
...
private:
  ...
  uint32_t InZCT() const { return composite & ZCTFLAG; }

  // Clear the ZCT flag and the ZCT index
  void ClearZCTFlag() { composite &= ~(ZCTFLAG | ZCT_INDEX); }

  // @return the ZCT index.  This is only valid if InZCT returns non-zero.
  uint32_t getZCTIndex() const { return (composite & ZCT_INDEX) >> 8; }
  ...
  void setZCTIndexAndUnpin(uint32_t index) {
    ...
    composite =
        (composite & ~(ZCT_INDEX | STACK_PIN)) | ((index << 8) | ZCTFLAG);
  }

  // Set the ZCT index and the ZCT flag.  If reaping==0 then clear the pinned
  // flag, otherwise preserve the pinned flag.
  void setZCTIndexAndMaybeUnpin(uint32_t index, uint32_t reaping) {
    GCAssert(reaping == 0 || reaping == 1);
    GCAssert(index <= (ZCT_INDEX >> 8));
    composite =
        (composite & ~(ZCT_INDEX | ((~reaping & 1) << STACK_PIN_SHIFT))) |
        ((index << 8) | ZCTFLAG);
  }
  ...
  static const uint32_t STICKYFLAG =
      0x40000000; // The object is sticky (RC overflow)
...
template <class T, bool checkMissingWB = true> class RCPtr {
public:
  RCPtr() {
    if (checkMissingWB)
